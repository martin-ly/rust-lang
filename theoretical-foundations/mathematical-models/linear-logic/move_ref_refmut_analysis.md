# Rust内部可变借用、外部可变借用与所有权转移的机制分析

## 目录

- [Rust内部可变借用、外部可变借用与所有权转移的机制分析](#rust内部可变借用外部可变借用与所有权转移的机制分析)
  - [目录](#目录)
  - [1. 引言](#1-引言)
  - [2. 基础概念](#2-基础概念)
    - [2.1 所有权系统](#21-所有权系统)
...
内部可变借用和外部可变借用是Rust 中处理可变性的两种机制，
...
- 在任意给定时间，要么只能有一个可变借用，要么只能有任意数量的不可变借用
- 借用必须始终有效
...
内部可变性允许在拥有不可变借用的情况下修改数据，通过将借用规则检查从编译时推迟到运行时：
...
fn main() {
    ...
    let r1 = &data;  // 对RefCell的不可变借用
    let r2 = &data;  // 可以同时存在多个不可变借用
    
    r1.borrow_mut().add_assign(1);  // 通过不可变借用修改内部值
...
fn main() {
    ...
    child1.add_child(Rc::clone(&root)); // 创建循环借用
...
fn main() {
    ...
    *state_clone2.counter.borrow_mut() += 10;
    ...
    // 查看借用计数
    println!("借用计数: {}", Rc::strong_count(&state)); // 3
}
```
...
- `Cell<T>`：通过复制/移动整个值实现安全修改，没有直接借用内部值。
...
1. 每个值在任何时刻只有一个所有者
2. 所有权转移后，原变量变为无效状态
3. 借用检查器确保借用不会比它们借用的数据存活得更久

这可以用帕斯卡三角原理（Pascal's triangle principle）来说明：
对于任意借用r，存在一个唯一路径从"根所有者"到r。
...
Rust的内部可变借用、外部可变借用和所有权转移机制提供了一个强大而灵活的系统，
使得内存安全和并发安全成为可能。
