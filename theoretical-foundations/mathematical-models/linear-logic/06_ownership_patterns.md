...
- [Rust 所有权设计模式与资源管理策略 {#所有权设计模式}](#rust-所有权设计模式与资源管理策略-所有权设计模式)
  ...
  - [独占所有权模式](#独占所有权模式)
  - [共享所有权与借用计数](#共享所有权与借用计数)
  - [所有权转移与资源转交](#所有权转移与资源转交)
  - [弱借用与循环依赖防护](#弱借用与循环依赖防护)
  - [工程实现与惯用法](#工程实现与惯用法)
  - [形式化分析与定理](#形式化分析与定理)
  - [交叉借用](#交叉借用)
...
- **所有权模式**：根据资源管理需求，选择独占、共享、转移、弱借用等策略。
...
- **`Rc<T>/Arc<T>`**：多所有者共享资源，借用计数自动管理生命周期。
...
- **`Weak<T>`**：弱借用不增加借用计数，防止Rc/Arc循环依赖。
...
- **定理 6.1 (所有权唯一性/共享性)**
  ...
  ```text
  Box<T>：唯一所有者；Rc/Arc<T>：多所有者+借用计数
  ```
