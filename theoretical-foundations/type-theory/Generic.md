...
有时你可能需要对泛型参数施加特定的约束，例如要求它实现某个特质（Trait）。
这可以通过 `where` 子句来实现：
...
在这个例子中，`T` 必须实现 `std::fmt::Display` 特质，以便可以使用 `to_string()` 方法。

## 高级泛型

Rust 还支持更高级的泛型特质，如关联类型（Associated Types），它们是特质的一部分，允许特质定义一个与特质关联的类型：
...
在涉及借用的泛型类型时，可能需要指定生命周期参数，以确保借用的安全性：
...
在这个例子中，`'a` 是一个生命周期参数，它指定了 `x` 和 `y` 借用的生命周期。
...
5. **生命周期参数**：类似于类型参数，生命周期参数用于定义借用的生命周期，确保代码的内存安全。
