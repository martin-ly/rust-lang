...
- 当一个类型实现了 `Deref` trait 时，Rust 允许自动解借用以满足函数调用、方法调用或类型匹配的需要。例如：
...
- **上转型（Upcasting）**  
  如果某个类型实现了一个 trait，当你需要一个 trait 对象时，可以将具体类型的借用自动转换为 trait 对象借用。
  ...
  **注意**：Rust 不支持自动的下转型（将 trait 对象转换为具体类型的借用），如果需要确定类型，则必须借助 `Any` 等机制进行显式判断和转换。

### **(3) 生命周期转换**

- Rust 中的借用生命周期是**协变**的，也就是说较短生命周期的借用可以被“提升”为较长生命周期的借用（前提是不会引起内存安全问题）。这种转换通常是隐式发生的，无需开发者干预。
...
- **类型转换**  
  包括显式转换（`as` 关键字），以及通过实现 `From`/`Into` 和 `TryFrom`/`TryInto` trait 实现的转换，同时还包括自动进行的解借用转换（Deref Coercion）。

- **类型上下转换**  
  主要是指在特定场景下 Rust 自动进行的转换：
  - **尺寸转换**：例如 `[T; N]` 到 `[T]`、具体类型到 trait 对象；
  - **Trait 对象转换**：即上转型（upcasting），从具体类型借用自动转换为 trait 对象借用；
  - **生命周期转换**：协变借用在生命周期上的自动转换。
