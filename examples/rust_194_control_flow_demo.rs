...
fn validate_registration(data: &UserRegistration) -> ControlFlow<ValidationError, ()> {
    ...
    if !data.email.contains('@') || !data.email.contains('.') {
        return ControlFlow::Break(ValidationError::InvalidEmail("邮箱格式不正确".to_string()));
    }

    // 验证密码强度
    if data.password.len() < 8 {
        return ControlFlow::Break(ValidationError::WeakPassword("密码至少8个字符".to_string()));
    }
...
fn process_data_pipeline(
    ...
    for item in items {
        ...
        if item.value.is_nan() || item.value.is_infinite() {
            return ControlFlow::Break(ProcessingError::InvalidValue(item.id, item.value));
        }
        ...
        if item.value > threshold {
            return ControlFlow::Break(ProcessingError::ThresholdExceeded(item.id, item.value));
        }
...
fn transition_state(current: State, event: &str) -> ControlFlow<StateError, State> {
    match (current.clone(), event) {
        ...
        (State::Processing(_), "retry") => ControlFlow::Break(StateError::MaxRetriesExceeded),
        (State::Processing(_), "fail") => ControlFlow::Continue(State::Failed("Error".to_string())),
        _ => ControlFlow::Break(StateError::InvalidTransition(
            format!("{:?}", current),
            event.to_string(),
        )),
    }
    ...
    for (key, value) in env_vars {
        match key.as_str() {
            ...
            "MAX_CONNECTIONS" => match value.parse::<u32>() {
                ...
                Err(_) => {
                    return ControlFlow::Break(ConfigError::InvalidValue(
                        "MAX_CONNECTIONS",
                        value.clone(),
                    ))
                }
