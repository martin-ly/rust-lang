...
- **“`unsafe` 是性能优化的终点”** —— 绝大多数情况下不成立。安全 Rust 与 unsafe 的性能差距通常在噪声范围内（编译器对安全代码的优化相当激进）；unsafe 的真实适用域是“借用检查无法表达的内存复用模式”，且每处 unsafe 都引入 UB 风险与 Miri 验证义务。先用 `#[inline]` 提示、布局调整、算法替换三板斧，unsafe 是最后手段。
- **边界极限**: 当热点在系统调用/IO/锁等待时，语言级优化的边际收益趋零——此时该优化的是架构（批处理、异步化、缓存）。
...
- **`mem::transmute` 大小不匹配**：源/目标类型大小不一致在编译期拒绝，但大小一致不代表语义合法（引用→整数、填充字节）。
