...
> **前置概念**: [Zero Cost Abstractions](../../01_foundation/00_start/02_zero_cost_abstractions.md) · [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md)
> **后置概念**: [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) · [Async](../../03_advanced/01_async/01_async.md)
...
性能优化的技术细节围绕“测量 → 定位 → 改造”的闭环，三个工具/主题各占一环：

- **Criterion（统计性基准测试）**: 替代 `cargo bench` 的标准选择——自动 warmup、异常值剔除、置信区间报告与跨次运行的回归对比；基准必须测量**稳定状态下的吞吐**，单次计时（`Instant::now` 包一段代码）受调度噪声支配，结论不可信。
- **Flamegraph（可视化性能分析）**: 把采样剖析（`perf`/samply）的调用栈按耗时宽度渲染，宽层即热点；读火焰图的纪律是“只看宽度不看高度”——高度是调用深度，宽度才是时间占比。
- **缓存友好性与内存布局**: 多数“计算瓶颈”实为内存瓶颈——顺序访问 vs 随机访问可差 10 倍以上；SoA 布局、预取友好的遍历顺序、`#[repr(C)]` 控制字段排列是三个最常用的改造手段。

判定依据：任何优化提交必须附 Criterion 前后对比与火焰图热点迁移证据，缺一不可。
...
- **“Rust 自动保证高性能，无需 profiling”** —— 不成立。Rust 保证的是零成本抽象的**上限**——误用（不必要的 `clone`、字符串拼接循环、锁粗化、分配器抖动）同样写出慢代码；零成本意味着“抽象本身免费”，不意味着“任意组合都快”。性能来自测量驱动的设计，不来自语言选择。
- **“`unsafe` 是性能优化的终点”** —— 绝大多数情况下不成立。安全 Rust 与 unsafe 的性能差距通常在噪声范围内（编译器对安全代码的优化相当激进）；unsafe 的真实适用域是“借用（Borrowing）检查无法表达的内存复用模式”，且每处 unsafe 都引入 UB 风险与 Miri 验证义务。先用 `#[inline]` 提示、布局调整、算法替换三板斧，unsafe 是最后手段。
- **边界极限**: 当热点在系统调用/IO/锁等待时，语言级优化的边际收益趋零——此时该优化的是架构（批处理、异步（Async）化、缓存）。
...
性能优化章节的所有技巧都以“正确性不被破坏”为前提，而 `unsafe` 优化恰恰是最容易 silently 破坏正确性的地方。本章覆盖八个代表性边界：

- **`unsafe` 优化的正确性假设**：`Vec::as_mut_ptr()` 取得的裸指针只在不再分配期间有效，任何 `push`/`reserve` 都可能使其悬垂——这类 UB 在 Miri 下才显形，普通测试可能永远“碰巧通过”。
- **`MaybeUninit` 未初始化内存**：绕过初始化写出高性能缓冲区时，读取未写入区域是 UB，即使类型是 `u8`。
- **`mem::transmute` 大小不匹配**：源/目标类型大小不一致在编译期拒绝，但大小一致不代表语义合法（引用（Reference）→整数、填充字节）。
- **内联汇编操作数约束**：`asm!` 的寄存器分配与 clobber 声明错误会破坏调用约定。
- 另四项覆盖缓存行伪共享、分支预测误导、`unchecked` 索引与 SIMD 对齐假设。

每节给出危险代码、UB 机理与安全替代写法。
...
性能分析的三个基础概念构成所有优化工作的共同词汇：

- **性能指标**: 吞吐（ops/s）、延迟分布（p50/p99/p999，平均数几乎无用）、资源效率（CPU 利用率/内存带宽占比）；优化目标必须先翻译成指标承诺（如“p99 从 50ms 降到 20ms”），否则无法验收。
- **基准测试**: 微基准（Criterion，函数级）与宏基准（端到端压测）回答不同问题——前者指导局部改造，后者验证系统级收益；两者结论冲突时以宏基准为准（阿姆达尔定律：局部优化受全局占比约束）。
- **性能分析工具链**: 采样剖析（`perf`/samply）找热点、计数器（PMC：IPC、cache miss）找原因、追踪（`tracing`）找时序异常；三层工具对应三个深度的问题。

判定依据：建立“指标 → 基准 → 剖析”的固定流程比任何单项技术都重要——优化工作的主要浪费来自跳过流程直接改代码。
...
| I/O | 异步（Async） I/O | 使用 `tokio::process` 避免阻塞线程 |
...
```mermaid
mindmap
  root((性能优化Rust 代码的测量与调优))
    一、核心概念
      1.1 测量优先原则
      1.2 编译器优化层级
      1.3 零成本抽象的验证
    二、技术细节
      2.1 Criterion统计性基准测试
      2.2 Flamegraph可视化性能分析
      2.3 缓存友好性与内存布局
    三、优化策略矩阵
    四、反命题与边界分析
      4.1 反命题树
      4.2 边界极限
    从 crates\c07_process\docs\
```
