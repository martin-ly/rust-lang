...
本节从边界测试：`unsafe` 性能优化的正确性假设（运行时 UB）、边界测试：`MaybeUninit` 的未初始化内存（运行时 UB）、边界测试：`mem::transmute` 的大小不匹配（编译错误）、边界测试：内联汇编的操作数类型约束（编译错误）等8个方面切入，剖析「边界测试：性能优化的编译错误」的核心内容。
