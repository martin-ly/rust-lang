...
「基准测试：Rust 代码性能测量与回归检测」部分按为什么需要基准测试、Criterion 基础用法、回归检测与 CI 集成、高级测量等7个方面的顺序逐层展开。
...
本节从简单基准 与 参数化基准 两个层面剖析「Criterion 基础用法」。
...
理解「回归检测与 CI 集成」需要把握基线对比、自动化阈值检测与CI 最佳实践，本节依次展开。
...
本节从自定义测量时间 与 吞吐量测量 两个层面剖析「高级测量」。
