...
> **前置概念**: [Toolchain](../00_toolchain/01_toolchain.md) · [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) · [FFI](../../03_advanced/04_ffi/01_rust_ffi.md)
...
Rust 测试技术栈按发现问题的类型分层：内置 `#[test]` 框架（单元/集成/文档测试三形态，零依赖起步）发现逻辑错误；`proptest` 属性测试（声明不变量、自动生成与收缩反例）发现边界输入错误；`cargo-fuzz`/`AFL` 模糊测试（覆盖引导的输入生成）发现解析器类崩溃；Miri（MIR 解释器）发现 unsafe 代码的未定义行为。分层策略：内置测试全覆盖是底线，解析/反序列化代码加 fuzz，含 unsafe 的模块（Module）加 Miri，核心算法加 proptest。
...
测试策略的常见误区是“覆盖率即质量”。反命题树反驳：100% 行覆盖可以零断言（执行过 ≠ 验证过），高覆盖与高缺陷检出率相关但不因果；测试代码本身有维护成本，过度 mock 的测试随重构批量失效，净价值为负。边界极限给出务实约束：CI 时间预算决定 fuzz/proptest 的运行时长上限（通常夜间长任务 + PR 短冒烟），Miri 的 10-100 倍减速使其只适合小模块。策略应按缺陷成本分配测试强度，而非均匀铺开。
...
测试策略边界测试固化三类高频错误：`#[should_panic]` 不带 `expected` 参数会把任何 panic（包括测试自身 bug）误判为通过；异步测试忘记 `#[tokio::test]` 而在同步 `#[test]` 中调用 `.await` 直接编译失败；`#[ignore]` 测试长期无人运行变成僵尸用例（应有 CI 任务定期 `--ignored` 全跑）。三类用例分别对应断言有效性、运行时适配与用例生命周期（Lifetimes）管理。试：属性宏测试的顺序依赖（运行时测试失败）、边界测试：`mockall` 的泛型 mock 限制（编译错误）等7个方面，本节依次展开。
...
> migrated from `crates/c12_wasm/docs/tier_05_wasm_engineering/06_testing_strategies.md`
...
> **认知功能**: 本 mindmap 从本页「Rust 测试策略 从单元测试到属性验证」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
