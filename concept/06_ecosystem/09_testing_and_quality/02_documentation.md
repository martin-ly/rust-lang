...
- **文档注释语法**: `///` 为紧随其后的项写文档（outer），`//!` 为包含它的模块（Module）/crate 写文档（inner）；支持完整 Markdown，标题层级从 `##` 起（`#` 保留给项名）。约定俗成的章节：`# Examples`（示例）、`# Errors`（返回 `Result` 的错误条件）、`# Panics`、`# Safety`（unsafe fn 的契约，缺它 `clippy::missing_safety_doc` 会警告）。
- **Intra-doc Links**: ``[`Type::method`]`` 直接链接到项的文档页，链接目标由 rustdoc 解析——写错路径在 `cargo doc` 时报 warning（配 `-D warnings` 可升级为门禁），这是文档与代码同步腐烂的最有效防线。
- **mdBook 与知识体系站点**: API 文档（rustdoc）讲“是什么”，mdBook 讲“为什么与怎么用”；两者经 intra-doc link 的反向引用（Reference）（书内链 docs.rs）形成闭环。
...
- **“文档应该等 API 稳定后再写”** —— 不成立。文档是 API 设计的检验工具——写不出清晰示例的 API 通常是设计有问题（参数太多、所有权（Ownership）语义混乱）；推迟文档等于推迟设计反馈。Rust 的 doctest 机制让“文档即测试”，示例代码随 API 变化而编译失败，不存在“先写后腐烂”的经典借口。
...
```mermaid
mindmap
  root((文档生态rustdoc、文档测试与 API 文档规范))
    一、核心概念
      1.1 rustdoc编译器集成的文档生成器
      1.2 文档测试Doc Tests
      1.3 文档作为类型系统的一部分
    二、技术细节
      2.1 文档注释语法
