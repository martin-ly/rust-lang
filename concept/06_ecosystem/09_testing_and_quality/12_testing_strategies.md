...
> **来源**: [Rust Book — Testing](https://doc.rust-lang.org/book/ch11-00-testing.html) · · [Brown University — Interactive Rust Book](https://rust-book.cs.brown.edu/) · [Jung et al. — RustBelt: Securing the Foundations of Rust](https://plv.mpi-sws.org/rustbelt/popl18/) · [Itanium C++ ABI](https://itanium-cxx-abi.github.io/cxx-abi/abi.html)
...
- [Rust 测试策略：从单元测试到属性验证](#rust-测试策略从单元测试到属性验证)
  ...
  - [补充视角：WASM 应用测试策略](#补充视角wasm-应用测试策略)
    - [测试金字塔的 WASM 映射](#测试金字塔的-wasm-映射)
    - [核心原则](#核心原则)
    - [WASM 特有测试层](#wasm-特有测试层)
    - [最佳实践](#最佳实践)
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
    - [反命题与边界](#反命题与边界)
...
> **使用建议**: 利用 Rust 的编译期保证减少运行时（Runtime）测试负担；对 unsafe 代码使用 Miri；对关键算法使用 Kani。
...
本节从内置测试框架、属性测试与模糊测试与Miri：未定义行为检测切入，剖析「技术细节」的核心内容。
...
- [Formal Methods](../../07_future/04_research_and_experimental/02_formal_methods.md) — 形式化方法工业化

---

> **权威来源**: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html), [The Rust Programming Language](https://doc.rust-lang.org/book/ch11-00-testing.html), [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)
...
理解「边界测试：测试策略的编译错误」需要把握边界测试：`#[should_panic]` 的误用（测试失败）、边界测试：异步测试的运行时要求（编译错误）、边界测试：属性宏测试的顺序依赖（运行时测试失败）、边界测试：`mockall` 的泛型 mock 限制（编译错误）等7个方面，本节依次展开。
...
本节从测验 1：`cargo test` 默认并行运行测试。如果测试间有共享…、测验 2：属性测试（Property-Based Testing，如…、测验 3：模糊测试（Fuzzing，如 `cargo-fuzz`）适合…、测验 4：集成测试应该验证什么？与单元测试的分工原则是什么？（理解层）等5个方面切入，剖析「嵌入式测验（Embedded Quiz）」的核心内容。

### 测验 1：`cargo test` 默认并行运行测试。如果测试间有共享状态冲突，该怎么办？（理解层）

**题目**: `cargo test` 默认并行运行测试。如果测试间有共享状态冲突，该怎么办？

<details>
<summary>✅ 答案与解析</summary>

使用 `cargo test -- --test-threads=1` 串行执行，或重构测试避免共享状态（每个测试独立初始化/清理），或使用临时文件/目录隔离。
</details>

---

### 测验 2：属性测试（Property-Based Testing，如 `proptest`）与单元测试的根本区别是什么？（理解层）

**题目**: 属性测试（Property-Based Testing，如 `proptest`）与单元测试的根本区别是什么？

<details>
<summary>✅ 答案与解析</summary>

单元测试用具体输入验证具体输出；属性测试自动生成大量随机输入，验证通用属性（如 `reverse(reverse(x)) == x`），能发现人工未想到的边界情况。
</details>

---

### 测验 3：模糊测试（Fuzzing，如 `cargo-fuzz`）适合发现什么类型的 bug？（理解层）

**题目**: 模糊测试（Fuzzing，如 `cargo-fuzz`）适合发现什么类型的 bug？

<details>
<summary>✅ 答案与解析</summary>

适合发现解析器、序列化/反序列化、协议实现中的崩溃和 panic。通过生成畸形输入触发代码中的异常路径，发现安全漏洞和健壮性问题。
</details>

---

### 测验 4：集成测试应该验证什么？与单元测试的分工原则是什么？（理解层）

**题目**: 集成测试应该验证什么？与单元测试的分工原则是什么？

<details>
<summary>✅ 答案与解析</summary>

集成测试验证模块（Module）/服务之间的交互和公共 API 契约。单元测试验证单个函数/模块的内部逻辑。分工：单元测试快速、细粒度；集成测试验证真实组合行为。
</details>

---

### 测验 5：`miri` 在测试中的主要用途是什么？（理解层）

**题目**: `miri` 在测试中的主要用途是什么？

<details>
<summary>✅ 答案与解析</summary>

Miri 是 Rust 的未定义行为（UB）检测器，解释执行测试用例并检查非法内存访问、数据竞争、对齐违规等。特别适用于验证 `unsafe` 代码的正确性。
</details>

## 补充视角：WASM 应用测试策略

> migrated from `crates/c12_wasm/docs/wasm_engineering/testing_strategies.md`
...
- **自动化**：尽可能自动化测试流程。
- **快速反馈**：单元测试应秒级完成。
- **真实环境**：测试接近生产环境（浏览器 / Node.js / WASI 运行时）。
- **全面覆盖**：覆盖边界和异常情况。
...
| 层级 | 工具/技术 | 关注点 |
|:---|:---|:---|
| 单元测试 | `wasm-bindgen-test` | Rust 逻辑在 wasm32 目标下正确性 |
| 集成测试 | Node.js / Deno / 浏览器 | JS 与 Rust 互操作、跨运行时行为 |
| 端到端测试 | Playwright | 真实浏览器场景与 UI 一致性（Coherence） |
| 性能测试 | Criterion / 浏览器性能 API | 内存占用、启动时间、计算吞吐 |
| 模糊测试 | `cargo-fuzz` | 不安全边界与 FFI 输入 |
| 回归测试 | 快照测试 / 金标准测试 | 防止输出漂移 |
| 属性测试 | `proptest` / `quickcheck` | 不变量形式化验证 |

### 最佳实践

- 使用 `wasm-bindgen-test_configure!(run_in_browser)` 在浏览器环境运行 DOM 相关测试。
- 对 JS 互操作边界使用 Node.js 集成测试覆盖类型转换与异常传播。
- 在 CI 中同时测试 `wasm32-unknown-unknown` 与 `wasm32-wasip1`/`wasm32-wasip2` 目标。
