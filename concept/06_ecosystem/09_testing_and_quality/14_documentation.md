...
> **后置概念**: [Cargo Toolchain](../00_toolchain/01_toolchain.md) · [WebAssembly](../11_domain_applications/11_webassembly.md)
...
「技术细节」部分按文档注释语法、Intra-doc Links与mdBook 与知识体系站点的顺序逐层展开。
...
「边界测试：文档工具的编译错误」部分按边界测试：`doctest` 中的隐式 `main`（编译错误）、边界测试：`rustdoc` 的链接解析失败（编译错误）、边界测试：`doctest` 的 `compile_fail` 与 `…与补充定理链的顺序逐层展开。
