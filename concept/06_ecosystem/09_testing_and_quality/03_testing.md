...
「技术细节」部分按属性测试（Property-Based Testing）、Mock 与依赖注入与模糊测试（Fuzzing）的顺序逐层展开。
...
「边界测试：测试的编译错误」涉及边界测试：`cargo test` 的编译失败与测试失败的区分、边界测试：mock 对象的 trait 约束（编译错误）、边界测试：`cargo test` 的测试名称冲突（编译错误）、边界测试：doc test 中的 `no_run` 与 `compil…等5个方面，本节逐一说明其要点。
