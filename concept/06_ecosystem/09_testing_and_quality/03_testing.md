...
> **后置概念**:
> [Miri](../../03_advanced/02_unsafe/01_unsafe.md) ·
...
- [测试生态：单元测试、集成测试与验证策略](#测试生态单元测试集成测试与验证策略)
  ...
  - [补充视角：进程管理场景测试要点](#补充视角进程管理场景测试要点)
    - [进程测试特殊性](#进程测试特殊性)
    - [推荐策略](#推荐策略)
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  ...
  - [⚠️ 反例与陷阱：测试中的浮点精确相等（运行时陷阱）](#️-反例与陷阱测试中的浮点精确相等运行时陷阱)
...
三种高级测试技术解决不同盲区：属性测试（proptest 声明“对任意输入，性质 P 成立”，自动生成并收缩最小反例，适合算法与解析 round-trip）；Mock 与依赖注入（mockall 生成 trait 的 mock 实现，前提是用 trait 抽象外部依赖——这也是“为可测性设计接口”的主要推力）；模糊测试（cargo-fuzz 用 libFuzzer 覆盖引导地探索输入空间，专攻 panic/溢出/UB）。三者与单元测试互补而非替代：单元测试钉死已知行为，这三者探索未知输入。
...
测试领域的反命题集中在“更多测试工具总是更好”。反命题树反驳：引入 proptest/fuzz 有学习成本与 CI 时长成本，对 CRUD 类代码收益趋近于零；mock 滥用会让测试验证“mock 的行为”而非真实行为（测试通过但生产崩溃）。边界极限给出适用判据：代码含复杂状态机/解析逻辑才值得属性测试，暴露于不可信输入才值得 fuzz，依赖外部服务且状态复杂才值得 mock。工具选择应被缺陷成本证明，而非被潮流驱动。
...
- [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) — unsafe 代码测试
- [Documentation](02_documentation.md) — 文档测试
...
测试工程的边界测试关注测试设施自身的失效：`cargo test` 输出中编译失败与测试失败混排（`--no-run` 分离编译阶段，CI 中应分开报告）；mockall 生成的 mock 要求被 mock trait 的所有类型满足 `Send + 'static` 等约束，约束不足时编译报错信息晦涩；测试间共享全局状态（环境变量、临时目录）导致并行执行 flaky（`serial_test` 或进程隔离是缓解手段）。三类问题分别对应报告、约束与隔离。试：`cargo test` 的测试名称冲突（编译错误）、边界测试：doc test 中的 `no_run` 与 `compil…等5个方面，本节逐一说明其要点。
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：`cargo test` 运行单元测试、集成测试和文档测试的…、测验 2：`tests/` 目录中的集成测试与 `src/` 中 `#…、测验 3：`#[ignore]`属性的作用是什么？如何只运行被忽略的…、测验 4：`cargo test -- --nocapture` 与默…等5个方面，本节逐一说明其要点。
...
- **非确定性**：子进程退出时间、输出顺序可能受系统负载影响。
- **平台依赖**：信号、进程组、资源限制在不同 OS 上行为不同。
- **资源泄漏**：未 `wait` 的子进程可能变成僵尸进程。

### 推荐策略

| 场景 | 方法 |
| :--- | :--- |
| 验证退出码 | `assert!(output.status.success())` |
| 超时控制 | `tokio::time::timeout` 或 `std::thread::sleep` + `try_wait` |
| 并发进程 | 使用信号量限制最大并发数 |
| 跨平台 CI | 在 Linux/macOS/Windows 上分别运行 |
| 性能基准 | Criterion.rs 对比不同 IPC 机制吞吐量 |
...
> **认知功能**: 本 mindmap 从本页「测试生态 单元测试 集成测试与验证策略」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。

---

## ⚠️ 反例与陷阱：测试中的浮点精确相等（运行时陷阱）

**反例**（运行时陷阱，代码可通过编译；实测 运行时 panic：assertion failed）：
...
测试生态中最常见的脆性断言：浮点求和误差使 `assert_eq!` 随机失败；集成测试里反复重跑「有时绿有时红」多源于此。
