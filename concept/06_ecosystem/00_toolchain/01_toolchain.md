...
> **[The Cargo Book](https://doc.rust-lang.org/cargo/)** Cargo is Rust's build system and package manager. Rustaceans use Cargo to manage their Rust projects because it handles a lot of tasks for you, such as building your code, downloading the libraries your code depends on, and building those libraries.
...
> **来源: [The Cargo Book](https://doc.rust-lang.org/cargo/)** Cargo 使用 TOML 格式的 manifest 文件，支持严格的 SemVer 约束、工作区、特性系统和编译期脚本。 ✅
...
**[The Cargo Book](https://doc.rust-lang.org/cargo/)** A workspace is a collection of one or more packages that share the same `Cargo.lock` and output directory. Workspaces help manage multiple related packages developed in tandem.

| **特性** | **说明** | **来源** |
|:---|:---|:---|
| 根 `Cargo.toml` | `[workspace]` 定义成员与共享依赖 | [The Cargo Book](https://doc.rust-lang.org/cargo/) |
| 成员路径 | `members = ["crate-a", "crate-b"]` | [The Cargo Book](https://doc.rust-lang.org/cargo/) |
| 共享 metadata | `workspace.package` / `workspace.dependencies` 统一版本 | [The Cargo Book](https://doc.rust-lang.org/cargo/) |
| 选择性构建 | `cargo build -p crate-a` 单独构建成员 | [The Cargo Book](https://doc.rust-lang.org/cargo/) |
| 跨 crate 测试 | `cargo test --workspace` 运行全部测试 | [The Cargo Book](https://doc.rust-lang.org/cargo/) |
...
**[The Cargo Book](https://doc.rust-lang.org/cargo/)** The resolver is the algorithm that selects which versions of dependencies to use. `resolver = "2"` (default since Rust 2021) changes how features are unified across the dependency graph.
...
**[The Cargo Book](https://doc.rust-lang.org/cargo/)** Publishing a workspace can be done crate by crate, but external tools like `cargo-workspaces` automate topological ordering and version bumping.
...
**[The Cargo Book](https://doc.rust-lang.org/cargo/)** Features are a mechanism for conditional compilation that allow a package to declare optional dependencies and togglable functionality.
...
**[The Cargo Book](https://doc.rust-lang.org/cargo/)** When the same crate appears multiple times in the dependency graph, Cargo uses feature unification to ensure only one copy of the crate is compiled, with the union of all enabled features.
...
> **来源**: [The Cargo Book](https://doc.rust-lang.org/cargo/) · [Rust API Guidelines](https://rust-lang.github.io/api-guidelines/) · 可信度: ✅

#### 3.2.3 `weak-dep-features` 与 `namespaced-features`（Cargo 1.60+）

**[The Cargo Book](https://doc.rust-lang.org/cargo/)** Cargo 1.60 引入 `namespaced-features`，允许在 `[features]` 中显式引用（Reference）可选依赖而不将其自动暴露为公共 feature 名。
...
```mermaid
graph TD
    A[Toolchain] --> B[Cargo](https://doc.rust-lang.org/cargo/)
    A --> C[Compiler]
    A --> D[Quality Tools]
    A --> E[Verification]
...
| **论断** | **来源** | **可信度** |
|:---|:---|:---|
| Cargo 是 Rust 官方构建系统 | [The Cargo Book](https://doc.rust-lang.org/cargo/) | ✅ |
| SemVer 用于依赖管理 | [The Cargo Book](https://doc.rust-lang.org/cargo/) · [SemVer 2.0.0] | ✅ |
...
> **来源: rustc Dev Guide; LLVM Documentation; [The Cargo Book](https://doc.rust-lang.org/cargo/); crates.io Docs; SemVer Spec** 工具链分析基于官方文档和社区最佳实践。✅
...
> **修正**: Cargo 的特性（features）是条件编译的核心机制。依赖冲突发生在：crate A 依赖 `serde`（无 `derive`），crate B 依赖 `serde`（有 `derive`），最终特性取并集（`derive` 启用）。但若 crate A 显式禁用 `derive`（`default-features = false`），冲突解决更复杂。Rust 的特性系统是无损的——启用特性不会破坏代码，只增加 API。这与 C++ 的宏（Macro）或 CMake 的选项不同——Cargo 的特性是声明式的、可组合的。[来源: [Cargo Documentation](https://doc.rust-lang.org/cargo/)]
