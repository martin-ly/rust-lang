...
> **前置概念**: [Toolchain](01_toolchain.md) · [Cross Compilation](../05_systems_and_embedded/02_cross_compilation.md)
> **后置概念**: [Cloud Native](../04_web_and_networking/02_cloud_native.md) · [Security Practices](../07_security_and_cryptography/01_security_practices.md)
...
Rust 项目的 DevOps 决策集中在两条主线：发布自动化（cargo-release 管版本 bump、 cargo-dist 管多平台二进制分发、release-plz 管 workspace 批量发布）按项目规模与产物形态选择；安全策略（cargo-audit 查已知漏洞、cargo-deny 管许可证与依赖图、cargo vet 管供应链审计）按合规要求叠加。矩阵按“配置复杂度 × 适用规模”排序，小项目从左下角起步即可。
...
Rust 部署的经典误解是“静态链接让部署总是简单”。反命题树拆解其前提：musl 静态链接虽消除 glibc 版本依赖，却带来 DNS 解析（无 NSS 插件）、locale 缺失与二进制体积膨胀等新问题；动态链接 glibc 则在老发行版上翻车。边界极限进一步给出量化约束：交叉编译矩阵的维护成本随目标数平方增长，CI 缓存命中率才是大型 workspace 构建时间的真正瓶颈。
...
- [Cross Compilation](../05_systems_and_embedded/02_cross_compilation.md) — 交叉编译技术
- [Cloud Native](../04_web_and_networking/02_cloud_native.md) — 云原生部署
- [Security Practices](../07_security_and_cryptography/01_security_practices.md) — 安全开发实践
- [Testing Strategies](../09_testing_and_quality/01_testing_strategies.md) — 测试策略
...
DevOps 边界测试验证构建链路与测试隔离的失效点：musl 目标上链接依赖 glibc 的 crate 在链接期报错（多阶段 Docker 构建的高发坑）、并行测试共享 `static mut` 触发数据竞争（测试隔离破坏的典型）、CI 缓存键错误导致增量编译失效。三类用例分别对应目标平台、测试并发与缓存正确性，是 Rust CI 流水线的三个最常见故障源。边界测试：Docker 多阶段构建的缓存失效（编译时间膨胀）、边界测试：缓存键未包含 Cargo.lock 导致的不一致构建（CI…等6个方面。
...
本节从测验 1：为什么 Rust 项目的 CI 构建时间通常比 Go/Nod…、测验 2：`sccache` 在 Rust CI 中的作用是什么？（理…、测验 3：Rust 的 `cross` 工具如何简化 CI 中的交叉编…、测验 4：在 Rust 项目的 CI 中，为什么建议同时运行 `cli…等5个方面切入，剖析「嵌入式测验（Embedded Quiz）」的核心内容。
