...
「DevOps 决策矩阵」部分包含发布自动化决策 与 安全策略矩阵 两条主线，本节依次说明。
...
本节将「边界测试：DevOps 与 CI/CD 的编译错误」分解为若干主题：边界测试：Docker 多阶段构建的 musl 目标链接错误（编译错误）、边界测试：测试隔离的 `static mut` 数据竞争（编译错误）、边界测试：Docker 多阶段构建的缓存失效（编译时间膨胀）、边界测试：缓存键未包含 Cargo.lock 导致的不一致构建（CI…等6个方面。
