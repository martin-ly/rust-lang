...
判定依据：`-Zdump-mir`（nightly）是诊断借用检查与优化行为的首选工具；MIR 阶段的错误（如 E0502）引用的是 MIR 位置而非源码表达式，需按提示行列号回溯。
...
| 代 | 模型 | 核心思想 | 状态 |
|---|---|---|---|
| 词法生命周期 | 作用域即生命周期 | 借用活到词法块尾 | 已废弃 |
| **NLL**（2018 起） | 控制流敏感的活性分析 | 借用活到「最后一次使用点」 | 现行默认 |
| **Polonius** | 基于 Datalog 的起源（origin）关系 | 把借用检查表达为逻辑查询，支持 NLL 拒绝的合法程序 | 开发中（nightly `-Zpolonius`） |

别名模型独立于借用检查，管 unsafe 代码：**Tree Borrows**（2023 起，Miri 默认）把内存组织为树，父指针失效后子指针仍可用的规则比 Stacked Borrows 更贴合真实代码模式。

判定依据：NLL 报错但逻辑正确的代码，先试 `-Zpolonius` 验证是否属已知局限（常见模式：两分支条件借用）。
...
编译器内部的常见误解是“了解编译器内部才能用好 Rust”。反命题树反驳：绝大多数开发者只需理解编译器暴露的契约（错误码、lint、属性），内部实现（查询系统、MIR  lowering）是贡献者与工具作者的领域。边界极限指出例外场景：自定义 lint（dylint）、MIR 级分析、编译性能诊断（`-Ztime-passes`）确实需要内部知识——按需深入而非系统学习。
...
编译器边界测试覆盖编译过程自身的三类失效：递归宏展开超过 `recursion_limit`（默认 128）导致栈溢出，需提升上限或重构宏；泛型单态化使每个类型实例生成独立代码，过度泛型化导致二进制膨胀与编译时间爆炸（可用 trait 对象或 `#[inline(never)]` 收敛）；unsafe 代码在 Miri 解释执行时暴露 stacked borrows 等 UB。三类用例分别标定宏、泛型与 unsafe 的工程边界。
...
- [类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) — 类型论基础
- [泛型（Generics）](../../02_intermediate/01_generics/01_generics.md) — 单态化（Monomorphization）、Trait bound
...
> **认知功能**: 本 mindmap 从本页「Rust 编译器内部原理」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
