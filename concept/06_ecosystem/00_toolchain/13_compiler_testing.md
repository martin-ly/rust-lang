> **内容分级**: [综述级]
> **本节关键术语**: Compiletest · UI Test · Package Test · Tidy · Crater · rustc-perf · Miri · `distcheck` — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# rustc 编译器测试体系
...
> **后置概念**: [Compiler Infrastructure](05_compiler_infrastructure.md)
...
**反例：UI 测试中滥用 `--bless` 固化真回归。**

rustc 的 UI 测试以 `.stderr` 快照为基线；诊断改进后批量 `x test ui --bless` 会把**所有**输出变化——包括真正的诊断退化——一并写入基线，测试继续绿灯而质量实际下降。

**修正对照**：

1. bless 之前先 `git diff` 逐文件审阅 `.stderr` 变化，确认每处都是预期改进；
2. PR 模板要求 bless 的 diff 必须单独成 commit 并在描述中说明原因；
3. 对关键错误码（E0xxx）的 UI 测试配 `//~ ERROR` 注解锚定错误码，防止 bless 漂移错误类别。
