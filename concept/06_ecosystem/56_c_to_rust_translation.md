...
- [C-to-Rust Translation Ecosystem（C 到 Rust 翻译生态）](#c-to-rust-translation-ecosystemc-到-rust-翻译生态)
  ...
  - [三、学术研究前沿](#三学术研究前沿)
    ...
    - [3.5 Hayroll：C 宏（Macro）与条件编译的模块（Module）化翻译（PLDI 2026）](LINK_PLACEHOLDER)
  - [四、现有工具链](#四现有工具链)
...
- **Skeleton-first**: 先建立可编译的项目级骨架（声明、模块结构、跨模块引用（Reference）），再生成函数体
...
Cpp2Rust 不尝试在编译期证明 C++ 的别名模式符合 Rust 的借用规则（这是不可判定的），而是将所有权和可变性检查**推迟到运行时（Runtime）**：

| C++ 特性 | Cpp2Rust 翻译策略 | 运行时（Runtime）开销 |
|:---|:---|:---|
| 原始指针（Raw Pointer） `T*` | 自定义 `Ptr<T>` 类型（引用（Reference）计数 + 借用检查） | 引用计数增减 |
...
**[University of Washington]** Hayroll 是一个**模块化包装器**，使 C→Rust 翻译工具能够**感知预处理器**（preprocessor-aware），而无需修改底层翻译器。这是解决 C-to-Rust 翻译中最顽固障碍之一——**C 预处理器宏和条件编译**——的首个系统性方案。

**问题背景**：现有 C→Rust 翻译器（如 C2Rust）在翻译前先运行 C 预处理器 `cpp`，这导致：

1. **丢失可配置性**：`#ifdef` 条件编译被展开为单一配置，丢失多平台/多配置支持；
2. **丢失抽象**：程序员定义的宏（常量、函数式宏）被内联展开，变成魔法数字和重复代码；
...
> **关键洞察**: C2Rust 生态的核心教训——"翻译容易，安全化难"。从 unsafe Rust 到 safe Rust 的转换需要**全局所有权推断**，而这不是局部重写能解决的。&inator 和 Scylla 的成功在于绕过这个问题：&inator 只翻译接口（类型系统（Type System）层面），Scylla 限制输入子集（语言层面）。[来源: [C2Rust](https://c2rust.com/)] · [来源: [Crown Paper](https://arxiv.org/abs/2305.02287)]
...
> **关键洞察**: 即使是最先进的 LLM（OpenAI o3）在**严格的 safe Rust 约束**下，迭代修复后也只能达到 48% 的成功率。这揭示了 C-to-Rust 翻译的核心瓶颈不在语法转换，而在**所有权语义推断**——将 C 的自由指针模型映射到 Rust 的受限借用模型需要超越模式匹配（Pattern Matching）的深层推理。CRUST-Bench 的严格协议（零 unsafe）使其成为衡量"真正的安全翻译"而非"语法转写"的黄金标准。
...
| **动态别名 → 借用（Borrowing）** | 未解决 | 可能需要运行时检查（如 RefCell） |
