...
- [类型系统（Type System）](../01_foundation/04_type_system.md) — 类型论基础
- [泛型（Generics）](../02_intermediate/02_generics.md) — 单态化（Monomorphization）、Trait bound
...
为每个泛型具体类型生成独立代码。好处是运行时（Runtime）零开销。代价是编译时间更长、二进制体积更大（代码膨胀）。`dyn Trait` 是避免单态化（Monomorphization）的替代方案。
</details>
