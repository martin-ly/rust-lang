...
洋葱架构（Onion Architecture）是六边形架构的同源变体，核心规则只有一条：**依赖只能指向内层**。从外到内依次是基础设施（Infrastructure）→ 应用服务（Application）→ 领域服务（Domain Services）→ 领域模型（Domain Model），内层对外层零感知。
...
Serverless/FaaS 把部署单元缩小到函数，平台负责扩缩容与毫秒级计费粒度。其架构特征对运行时提出苛刻要求：冷启动时间、内存占用与每请求成本直接相关。

- **Rust 的契合点**: 无 GC 停顿、二进制小（静态链接 + strip 后常 < 20MB）、冷启动通常在 50–200ms 量级，优于 JVM 系约一个数量级；AWS Lambda 提供官方 `lambda_runtime` crate。
- **实践形态**: 单函数单 crate，依赖最小化；连接池、配置等初始化放在懒静态（`LazyLock`）中跨调用复用。
- **冷启动优化**: 首选自定义运行时（如 `provided.al2023`）避免解释器层；其次用平台快照能力；最后是 provisioned concurrency 保底。

判定依据：请求间隔长（冷启动占比高）+ 计算密集时 Rust 收益最大；热路径上语言差异缩小，按团队技术栈选择。
...
架构模式的选择本质是权衡**变更成本**与**复杂度预算**。两个矩阵从不同角度回答“用哪个”：

- **决策矩阵（按团队与变更频率）**: 小团队 + 需求快速变化 → 模块化单体；多团队 + 独立部署诉求 → 微服务；领域复杂、规则稳定 → 洋葱/六边形分层。
- **适用场景矩阵（按技术约束）**: 延迟敏感、单进程可承载 → 单体；需要异构技术栈或独立扩缩 → 微服务；突发流量、按量付费 → Serverless。

| 信号 | 倾向 |
|---|---|
| 接口一天数变、调用方少 | 模块化单体 |
| 团队 > 3 个且发布互相阻塞 | 微服务 |
| 流量峰谷差 > 10 倍 | Serverless |

判定依据：先用矩阵排除明显不适配项，再用“回退成本”做最终决定——单体拆微服务难，微服务合并更难。
...
架构模式的价值在于边界清晰，但其适用性本身也有边界。本节用反命题树列举“某架构总是更优”类绝对化论断的反例（如简单 CRUD 不需要六边形架构的端口/适配器开销），再用边界极限说明各模式在团队规模、变更频率、性能约束下的失效点。阅读方式：先确认自己的系统是否落在反命题的反驳条件中，若落在其中则可直接降级到更简单的架构，不必为“架构正确性”支付复杂度税。
...
> **来源**: [Fowler — YAGNI](https://martinfowler.com/bliki/Yagni.html)
...
> **来源**: [Fowler — YAGNI](https://martinfowler.com/bliki/Yagni.html) · [AWS Lambda Limits](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) · [Martin — Clean Architecture](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html)
...
本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：Rust 中常用的分层架构（Layered Architec…、测验 2：六边形架构（Hexagonal Architecture /…、测验 3：Rust 的强类型系统对洋葱架构（Onion Archite…、测验 4：什么是"依赖倒置原则"（DIP）？Rust 的 trait…等5个方面。
...
架构不是一次性选择，而是随负载与团队规模演进的路径。本节先给出单体分层 → 模块化单体 → 微服务的典型演进阶梯及各阶段的触发信号（如部署耦合、团队冲突、独立扩缩容需求），再横向对比分层、六边形、洋葱、整洁架构在依赖方向、可测试性与迁移成本上的差异。判据核心是：演进应有明确痛点驱动，提前分布式化是最常见的过度工程。

### 架构演进路径

演进路径的关键洞察是“模块化单体是大多数系统的最优中间态”：它用 crate 边界获得微服务的大部分隔离收益（独立编译、依赖约束），却无需付出网络调用、分布式事务与运维复杂度。从单体到分布式的每一步都应回答三个决策因子：变更是否独立、故障是否需要隔离、团队是否已按边界拆分。三个答案全为“是”才值得拆分。
