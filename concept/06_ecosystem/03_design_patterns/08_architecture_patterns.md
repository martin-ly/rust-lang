...
本节将「边界测试」分解为若干主题：边界测试：适配器绕过端口直接依赖核心（编译错误）、边界测试：跨层依赖导致循环依赖（编译错误）与边界测试：Serverless 超时导致状态不一致（运行时错误）。
...
本节从架构演进路径 与 架构模式对比 两个层面剖析「架构演进与对比（crates 迁移）」。

### 架构演进路径

本节从单体 → 分布式演进 与 架构决策因子 两个层面剖析「架构演进路径」。
