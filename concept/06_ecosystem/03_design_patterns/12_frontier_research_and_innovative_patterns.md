...
2. **Effect 多态**：函数可对效果参数化（`fn log<E: Logging>(...)`），同一实现可跑在真实 I/O 与内存 mock 上——测试与生产的统一抽象，Rust 当前用泛型（Generics） trait 模拟（依赖注入），缺的是**效果推断**（自动传播，免标注）。
3. **Effect 约束与推断**：Koka/OCaml 5 的研究线表明效果推断与 Hindley-Milner 类型推断（Type Inference）兼容；Rust 的 `~const` 限定是效果标记的最小工程形态（constness 作为效果）。
...
- **Rust 实现现状**：`rumpsteak`（研究原型）用类型级编程编码会话类型，`session-types` 类 crate 限于两方；n 方场景的可行工程形态是「中心化编排 + 类型化消息」（编排器持有协议状态机 `enum`，各方消息用 `serde` 枚举（Enum））。
...
1. **能力令牌**：`struct FileCap { fd: RawFd }`——构造该类型的唯一途径是经过权限检查的 `open` API；后续 `read(&FileCap)` 无需再查权限表。Rust 的所有权（Ownership）语义天然契合：能力可移动（移交权限）、可借用（Borrowing）（临时授权）、不可复制（`!Clone` 即不可扩散权限）。
...
判定依据：权限逻辑分散在 `if user.can(x)` 检查中 → 重构为能力类型，让类型系统（Type System）强制「先取能力再操作」。
