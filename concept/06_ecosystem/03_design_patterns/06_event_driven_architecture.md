...
事件处理器的核心设计问题是投递语义：恰好一次（exactly-once）在分布式系统中不可真正实现，工程现实是至少一次（at-least-once）投递 + 消费端幂等。幂等实现的三种机制：幂等键去重（事件带唯一 ID，处理前查重，需存储支持）；状态机校验（只接受合法状态迁移，重复事件因状态已迁移而拒绝）；事务性 outbox（事件与业务数据同事务写入，保证不丢不重发）。选择判据：去重窗口大小与存储成本的权衡。
...
事件驱动边界测试验证总线架构的三类失效：事件反序列化安全（未知事件类型/版本的处理策略——严格失败 vs 跳过，schema 演化时的兼容边界）；处理器 `Send` 约束（捕获非线程安全状态的 handler 无法注册到多线程调度器，编译期拦截）；类型擦除总线的向下转型（`Any` + `downcast` 在事件类型不匹配时 panic，应改用枚举（Enum）或分类型 channel）。三类分别对应 schema、并发与类型安全。ic）、边界测试：事件处理顺序与生命周期管理（运行时悬垂引用）等6个方面。
...
> **认知功能**: 本 mindmap 从本页「事件驱动架构 Event-Driven」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
