...
事件驱动边界测试验证总线架构的三类失效：事件反序列化安全（未知事件类型/版本的处理策略——严格失败 vs 跳过，schema 演化时的兼容边界）；处理器 `Send` 约束（捕获非线程安全状态的 handler 无法注册到多线程调度器，编译期拦截）；类型擦除总线的向下转型（`Any` + `downcast` 在事件类型不匹配时 panic，应改用枚举或分类型 channel）。三类分别对应 schema、并发与类型安全。ic）、边界测试：事件处理顺序与生命周期管理（运行时悬垂引用）等6个方面。
