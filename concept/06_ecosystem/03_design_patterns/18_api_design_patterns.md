...
> [错误处理（Error Handling）](../../01_foundation/08_error_handling/01_error_handling_basics.md) ·
> [架构设计模式](08_architecture_patterns.md)
> **后置延伸**: [微服务架构模式](05_microservice_patterns.md) ·
...
判定依据：handler 签名里出现 `String`/`serde_json::Value` 作为请求体类型，是契约未类型化的信号，应定义具名结构体（Struct）。
...
**Schema 与类型系统**：`async-graphql` 用过程宏（Procedural Macro）从 Rust 类型生成 schema——`#[derive(SimpleObject)]` 的结构体即 GraphQL 类型，Rust 的 `Option<T>` 精确映射为 GraphQL 的可空标记，这比多数语言（需要运行时反射）的映射更严格。
...
- [微服务架构模式](05_microservice_patterns.md) — 服务发现、熔断、API Gateway
- [事件驱动架构](06_event_driven_architecture.md) — 发布-订阅、消息队列
- [分布式系统](../04_web_and_networking/01_distributed_systems.md) — gRPC、Raft、Actor
- [架构设计模式](08_architecture_patterns.md) — 分层/六边形/洋葱/整洁架构
- [Reactive Programming](../04_web_and_networking/09_reactive_programming.md) — Stream、背压、实时数据
- [安全实践](../07_security_and_cryptography/01_security_practices.md) — 防御性编程、密码学
- [Async/Await](../../03_advanced/01_async/01_async.md) — 异步编程基础
- [Trait](../../02_intermediate/00_traits/01_traits.md) — 接口设计、类型抽象
- [错误处理（Error Handling）](../../01_foundation/08_error_handling/01_error_handling_basics.md) — Result、? 运算符
...
- 测验 2：**`IntoIterator` 作为参数类型**——接受任何可迭代类型（`Vec`、`&[T]`、`HashSet`），是零成本泛型（Generics）化的典型手法。
...
本节以 getter 返回临时值引用（Reference）为反例，展示 `as_*` 风格 API 的生命周期（Lifetimes）约束。

### 反例：`as_*` 方法返回临时对象的引用（rustc 1.97.0 实测）

```rust,compile_fail,E0515
struct Name(String);
impl Name {
    fn as_str(&self) -> &str {
        &self.0.to_uppercase() // ❌ 临时 String 在表达式末析构
    }
}
```

**错误**：`E0515 cannot return reference to temporary value`——`as_*` 约定零成本借用视图，不能凭空制造新数据。

### ✅ 修正：返回拥有值（改名 `to_*`）或借用已有字段

```rust
struct Name(String);
impl Name {
    fn as_str(&self) -> &str { &self.0 }            // 借用既有字段
