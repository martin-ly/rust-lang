...
- **“补偿事务能保证分布式一致性”** —— 不成立。Saga 的补偿是**业务语义上的撤销**，不是 ACID 回滚——已发送的邮件收不回，已扣的款只能退回（且可能失败）；设计补偿动作时必须接受中间状态对外可见，必要时用“先冻结后确认”的两段式业务设计规避。
...
1. **状态机转换遗漏**：`match` 不穷尽导致某些状态永远无法到达（死代码），或遗漏重试/终止转换。Rust 的穷尽性检查能在编译期捕获前者，后者需要枚举所有 `(from, to)` 对做静态审查。
