...
CQRS+ES 的属性矩阵回答“这套模式组合到底换来了什么、付出了什么”，四个维度是评估的核心：

| 维度 | CQRS+ES | 传统 CRUD |
|---|---|---|
| 状态来源 | 事件流重放/投影 | 当前行数据 |
| 一致性模型 | 最终一致（读模型滞后） | 强一致（单库事务） |
| 审计能力 | 天然完整（事件即日志） | 需额外审计表 |
| 复杂度 | 高（事件 schema、投影、快照、回放运维） | 低 |

- **命令侧与查询侧的分离收益**: 写模型按聚合一致性优化（事件追加），读模型按查询模式优化（宽表/搜索索引），两侧可独立扩缩与换存储引擎。
- **代价的集中点**: 读模型滞后窗口内的“写后读不到”需要 UX 层补偿（乐观更新/读己之写路由）。

判定依据：矩阵中“审计能力 + 写读负载不对称”两项同时成立时，CQRS+ES 的复杂度才有正回报。
...
CQRS（Command Query Responsibility Segregation）的核心主张：**写模型与读模型分离**，各自按负载特征优化。

- **命令端（写）**：面向一致性与不变量校验，通常领域模型 + 聚合根（DDD），写入路径短而严；每次命令产生事件。
- **查询端（读）**：面向查询形态特化——为每个高频查询维护物化视图（宽表、倒排索引、缓存），写入时异步（Async）投影。
- **物理分离的层级**：最弱形态是同一库内不同表/视图；中间形态是读写不同库（如写 Postgres、读 Elasticsearch）；最强形态是读写不同服务，以事件总线连接。

代价是**最终一致性**：读模型滞后于写模型，UI 需处理「刚提交查不到」的问题（读己之写一致性可通过返回命令结果中的版本号并在查询时等待投影追上来缓解）。

判定依据：读写比 >10:1 且读模式与写模型差异大 → CQRS 有价值；CRUD 简单系统引入 CQRS 是过度设计。
...
事件溯源（Event Sourcing）把状态变化记录为**不可变的追加事件序列**，当前状态 = 事件重放（fold）。三个工程要点：

1. **不可变性与追加模型**：事件一旦写入不得修改——修正错误用补偿事件（compensating event），类比会计的红字冲销；这赋予系统完整审计轨迹与时间旅行调试能力。
2. **快照策略**：无快照时加载聚合需重放全部事件（边界测试中的「查询退化」）；每 N 个事件（如 100）存一次快照，加载时从最近快照重放增量；快照是性能优化而非真相来源，可随时丢弃重建。
3. **事件升级（Upcasting）**：事件 schema 演化时，旧事件读取路径上挂 upcaster 链（v1→v2→v3 逐级转换），存储层永远只追加不改写；Rust 中用 `#[serde(tag = "version")]` + 手动转换函数实现。

判定依据：事件 schema 设计时字段只增不改语义；任何「就地修改历史事件」的需求都是反模式信号。
...
CQRS+ES 协同的三个核心模式解决分布式一致性问题：

1. **Saga / Process Manager**：跨聚合的长事务分解为「本地事务 + 补偿动作」序列；Saga 状态机监听事件推进，失败时逆序补偿。Rust 中天然映射为 `enum SagaState` + `match` 转换，非法状态迁移编译期拒绝。
2. **Outbox 模式**：「写库 + 发事件」的双写不一致（边界测试中的数据丢失）的标准解法——事件与业务数据在**同一数据库事务**写入 outbox 表，后台 relay 轮询/CDC 转发到消息总线，保证 at-least-once；消费端需幂等。
3. **最终一致性的读模型**：投影（projection）消费者按事件顺序物化读模型，需处理乱序与重复——用事件全局序号做幂等去重（`processed_events` 表或版本号检查）。

判定依据：无 Outbox 的「先发消息后写库」或反之都是 bug；Saga 补偿必须自身幂等且可重试。
...
Rust 实现 CQRS+ES 的四个模块（Module）按数据流顺序组织，类型系统在每个模块都有独特落点：

- **事件定义与序列化**: `enum Event { OrderPlaced { .. }, PaymentCaptured { .. } }`——和类型 + serde 派生，事件 schema 演进用 `#[serde(tag)]` 版本标签与新变体追加（绝不修改既有变体语义）。
- **命令处理器**: 纯函数形态 `(State, Command) -> Result<Vec<Event>, Error>`——决策与副作用分离，使业务规则可脱离存储单测（给定状态 + 命令，断言产出事件序列）。
- **事件存储与投影**: 追加写存储（EventStoreDB/Postgres 分区表），投影是事件流的折叠（fold）——`stream.fold(ReadModel::default(), apply)`，失败即重建的特性要求 apply 必须幂等。
- **完整骨架**: 聚合根持有状态 + 未提交事件，`Save` 时原子追加；快照每 N 事件生成一次以控制重放成本。

判定依据：命令处理器的纯函数化是整个骨架可测性的支点，I/O 侵入该层则全部优势瓦解。
...
CQRS+ES 的两个高频误判是项目失败的主因：

- **“事件溯源 = 免费的审计日志，应该默认采用”** —— 不成立。审计能力用一张 append-only 审计表即可获得 80%；ES 的额外成本（schema 演进纪律、投影重建运维、重放性能、团队学习曲线）只有在“状态历史本身是业务资产”（金融账本、合规留痕、时序分析）时才回本。多数 CRUD 应用上 ES 是净负收益。
- **“读写分离后读模型总是滞后的，所以业务都能接受最终一致”** —— 不成立。“写后立读”是大量业务场景的硬需求（下单后看订单状态）；解法不是放弃 CQRS，而是针对性的读己之写（read-your-writes）策略：写后短时间窗口内把该用户请求路由到包含其事件的投影或临时从写模型读取。
- **边界极限**: 事件量 > 千万级/聚合时，无快照重放不可接受——快照策略与事件归档（冷存储分层）成为运维必需项。
...
CQRS/事件溯源的边界测试针对其三个已知软肋：无快照时聚合重建需重放全部事件（事件量增长使加载线性退化，快照策略是生产必需）；命令侧与查询侧双写不一致（没有事务边界时事件丢失导致读模型永久落后，outbox 模式是标准解）；事件 schema 演化破坏历史事件反序列化（加字段需默认值、改语义需版本化 upcast）。三类用例是评估事件溯源方案成熟度的最小检查集。
...
- [分布式系统](../04_web_and_networking/01_distributed_systems.md) — gRPC、Raft、Actor、消息队列
...
- [公理语义](../../04_formal/03_operational_semantics/05_axiomatic_semantics.md) — Hoare 逻辑、wp 计算
...
本节以重放期间追加事件为反例，展示事件溯源中「读日志」与「写日志」的借用（Borrowing）冲突。
