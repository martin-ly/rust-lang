...
模式组合的四种代数原语，借用函数式编程的组合子思想：

| 原语 | 记号 | 语义 | Rust 对应 |
|---|---|---|---|
| 并行组合 | A ⊗ B | 两模式独立共存，无交互 | 模块内并存 |
| 串行复合 | A ∘ B | B 的输入依赖 A 的输出 | `Iterator` 组合子链 |
| 选择和 | A ⊕ B | 按条件二选一 | `enum` + `match` |
| 迭代闭包 | A* | 重复应用直至收敛 | 循环/递归 |
...
判定依据：组合前先画出状态所有权图，确认每个 `&mut` 路径独占。
...
3. **Trait Object + Strategy**：`Box<dyn Strategy>` 提供运行期可替换，与泛型策略（零成本静态分派）形成「同一模式两种成本模型」的自由选择。
...
模式组合的失败案例比成功案例更有信息量。Singleton 与依赖注入的组合是经典冲突：全局单例绕过构造注入，使测试无法替换依赖（反例展示用 `OnceLock` 全局态破坏 DI 的整个过程）。Observer + Singleton 则撞上所有权：观察者注册需要共享可变状态，全局单例的 `'static` 生命周期与借用检查直接冲突，编译期即暴露组合不合法。这类反例的价值在于把“模式手感”转化为可编译验证的约束。式组合的状态空间爆炸，本节依次展开。
...
组合安全性定理试图回答“两个各自安全的模式组合后是否仍安全”。核心定理形式为：若模式 A 的安全不变量由类型 T_A 编码、模式 B 由 T_B 编码，且 T_A 与 T_B 的组合不违反孤儿规则与借用规则，则组合模式继承两者的不变量。失效路径主要有两条：共享可变状态突破 `&mut` 排他性（需 `RefCell`/锁降级为运行时检查）、生命周期交集无法命名（需重构为 arena/索引模式）。定理系统给出组合前的检查清单。
