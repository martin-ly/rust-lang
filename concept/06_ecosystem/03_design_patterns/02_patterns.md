...
| **Builder** | 创建型 | 复杂对象构造 | 消费型 Builder | 所有权（Ownership）链确保必填字段 |
...
| **Strategy** | 行为型 | 运行时（Runtime）算法切换 | `dyn Trait` / 泛型（Generics）参数 | 静态/动态分发选择 |
...
| **Builder** | 复杂对象构造易错 | 分步初始化 ⟹ 消费型链式API | 所有权（Ownership）转移+方法链 | 边界：字段<3时过度工程 |
| **Newtype** | 类型语义混淆 | 零成本区分 ⟹ 编译期标签 | `struct Wrapper(T)` | 边界：需重复实现大量标准Trait（Trait） |
| **Strategy** | 算法硬编码难扩展 | 行为参数化 ⟹ 静态/动态分发 | `dyn Trait`/泛型（Generics）单态化（Monomorphization） | 边界：仅一种实现时的无用抽象 |
...
**定义**：在运行时（Runtime）动态加载并注册扩展模块（Module），无需修改核心代码。
...
> **来源: [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html)** `Rc::downgrade` 产生 `Weak<T>`，允许引用（Reference）而不拥有所有权，是打破循环引用的标准手段。✅

---

#### Rust 实现方式二：异步广播（`tokio::sync::broadcast`）

在异步（Async）场景中，`tokio::sync::broadcast` 提供了多生产者多消费者的广播通道，天然适合 Observer 模式：
...
| **生命周期（Lifetimes）** | 手动管理订阅/取消订阅 | 系统自动跟踪实体生命周期 |
| **性能** | 动态分发 + 锁开销 | 无锁环形缓冲区（`Events<T>`） |
| **批量处理** | 逐个回调调用 | 每帧批量读取，支持事件排序 |
| **适用场景** | 小规模、紧耦合模块（Module） | 游戏引擎、大规模 ECS 架构 |
...
| **C 端风险** | **Rust 封装策略** | **类型系统（Type System）保证** |
|:---|:---|:---|
...
| **编译时间** | 泛型单态化（Monomorphization）导致代码膨胀 | debug 编译时间增长 >50% |
...
| 生命周期标注 >3 个嵌套 | 自引用或过度借用（Borrowing） | 考虑 `Pin<&mut Self>` 或所有权重新分配 |
...
|:---|:---|:---|:---|
| GoF 模式依赖继承/运行时多态 | 范式转换（如 Template Method → Trait 默认方法） | §六 命题二 C2_1 | 强行套用原始结构 ⟹ `dyn Trait` 滥用（C2_2） |
| 约束可编码进类型系统 | Typestate / Phantom 类型优先 | §六 命题三 C3_1 | 退回运行时状态检查 ⟹ 丧失编译期保证 |
| 模式选择依据 | 类型驱动 → 性能驱动 → 经验驱动 | §六 命题三修正 T3 | 纯凭经验 ⟹ 架构决策不可复核 |
| 需要运行时异构插件 | `dyn Trait` 对象 | §六 命题三 C3_2 | 仅凭经验选 `dyn` ⟹ 丧失内联优化 |
| 共享所有权图结构 | `Rc`/`Arc` + `Weak` 打破循环 | §八 学术来源（Weak 打破循环引用） | 全强引用 ⟹ 循环泄漏 |
| 完全拒绝设计模式 | 修正：Rust 减少但非消除模式需求 | §六 命题一修正 T1 | 重复发明已有解决方案 ⟹ 反模式 |
...
| 工具链 | [`./01_toolchain.md`](../00_toolchain/01_toolchain.md) | 工程支撑 |
| 设计模式对比 | [`../05_comparative/03_paradigm_matrix.md`](../../05_comparative/00_paradigms/03_paradigm_matrix.md) | 范式定位 |
...
- A. `fn name(&self, ...) -> &Self`（借用（Borrowing）并返回引用）
...
以下哪种情况更适合传统枚举（Enum）状态机而非 Typestate？
...
实践中两者常结合：Typestate 编码核心不变量，枚举（Enum）处理运行时变体。
</details>
...
| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| Design Patterns（设计模式） 基础原理 ⟹ 正确选型 | 理解核心概念与适用边界 | 能在实际项目中做出合理决策 | 高 |
| Design Patterns（设计模式） 选型实践 ⟹ 常见陷阱 | 忽视版本兼容性与生态成熟度 | 技术债务或迁移成本 | 中 |
| Design Patterns（设计模式） 陷阱规避 ⟹ 深度掌握 | 持续跟踪社区演进与最佳实践 | 能进行架构设计与技术预研 | 高 |

> **过渡**: 掌握 Design Patterns（设计模式） 的基础概念后，建议通过实际案例与源码阅读加深理解，建立从理论到实践的桥梁。
> **过渡**: 在工程实践中应用 Design Patterns（设计模式） 时，务必评估生态成熟度、社区支持与长期维护风险，避免过度依赖实验性技术。
> **过渡**: Design Patterns（设计模式） 反映了 Rust 生态系统的演进趋势与语言设计哲学，理解这些趋势有助于预判未来发展方向并做出前瞻性技术决策。
