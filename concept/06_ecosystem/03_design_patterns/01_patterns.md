...
**与 L1-L4 的关联**：过早抽象常表现为对 [L2 Trait](../../02_intermediate/00_traits/01_traits.md) 的过早承诺——Trait 一旦作为公共 API 发布，其变更即构成破坏性修改；同时，enum 优先于 dyn Trait 的策略正是 [L1 类型系统](../../01_foundation/02_type_system/01_type_system.md) 中"代数数据类型 + 穷尽性检查"优势的体现。
...
设计模式边界测试验证 Rust 实现对经典模式的额外约束：Builder 未消费 `self`（借用版 builder 与消费版的所有权差异，链式调用中途失效的编译错误）；多线程 Singleton 用 `static mut` 被 `Sync` 约束拒绝（正确解是 `OnceLock`/`LazyLock`）；Newtype 为外部类型实现外部 trait 触发孤儿规则（编译器强制“本地类型或本地 trait 至少其一”）。三类用例展示 Rust 模式实现的安全边界在哪里。（编译错误）、边界测试：访问者模式与 Rust 的枚举匹配（设计权衡）等5个方面。
...
本组测验覆盖 Rust 四大基础模式：RAII（Drop 与资源生命周期的绑定语义）、Typestate（状态编码进类型的编译期转换检查）、Builder 与所有权（消费 self vs 可变引用的两种 builder 风格取舍）、Newtype（零成本包装与孤儿规则绕行）。测验按理解层→应用层递进，建议全部作答后再对解析；应用层错题通常指向所有权语义的理解缺口。e 模式（分析层）等5个方面。
...
共同判定依据：Rust 的实现策略是“先问语言是否已内建”——迭代器、RAII、类型状态机已进入语言/标准库层面，剩余模式才需要显式结构设计。
