...
> **来源: [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html)** `Rc::downgrade` 产生 `Weak<T>`，允许引用（Reference）而不拥有所有权（Ownership），是打破循环引用的标准手段。✅
...
|:---|:---|:---|:---|
| Subject ↔ Observer 双向引用（Reference） | GC 自动回收 | `Rc<RefCell<T>>` + `Weak<T>` | 引用计数，弱引用不阻止释放 |
...
`Weak<T>` 在此起到关键作用：它允许 Observer 引用 Subject（或反之）而不增加强引用计数，从而保证当所有强引用消失时，资源可以被确定性释放。[`L1 借用（Borrowing）`](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) · [`L2 内存管理`](../../02_intermediate/02_memory_management/01_memory_management.md)
...
- 为单一函数引入复杂的 Trait 层次和泛型（Generics）参数，而仅有一个实现。
...
**与 L1-L4 的关联**：过度工程的泛型滥用直接违背 [L2 泛型](../../02_intermediate/01_generics/01_generics.md) 中"约束即文档"的原则——无约束的泛型参数丧失类型信息；过度使用 `dyn Trait` 则绕过了 [L1 所有权](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) 的编译期精确分析，引入运行时间接调用。
...
**与 L1-L4 的关联**：过早抽象常表现为对 [L2 Trait](../../02_intermediate/00_traits/01_traits.md) 的过早承诺——Trait 一旦作为公共 API 发布，其变更即构成破坏性修改；同时，enum 优先于 dyn Trait 的策略正是 [L1 类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) 中"代数数据类型 + 穷尽性检查"优势的体现。
...
**与 L1-L4 的关联**：Stringly typed 直接违背 [L1 类型系统](../../01_foundation/02_type_system/01_type_system.md) 的核心原则——"使非法状态不可表示"（Making Illegal States Unrepresentable）。enum 的穷尽性检查是 Rust 编译期保证的关键机制，参见 [L1 类型系统](../../01_foundation/02_type_system/01_type_system.md) §代数数据类型。
...
- `AppState` 被所有 handler 共享，任何字段变更都需重新编译所有依赖模块（Module）。
...
**与 L1-L4 的关联**：上帝对象的 `Mutex<GlobalState>` 模式违背了 L1 所有权 的核心理念——"单一所有者决定生命周期（Lifetimes）"。通过 Actor / Channel 模型将状态拆分，正是 L3 并发 中"共享状态转化为消息传递"原则的实践。参见 [L1 借用（Borrowing）](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) §内部可变性、L3 并发 §Actor 模型。
...
| Typestate | L1 所有权 + L2 泛型 | `01_foundation/`, `02_intermediate/01_generics/01_generics.md` | 编译期状态机验证 |
| Newtype | L1 类型系统 | `01_foundation/02_type_system/01_type_system.md` | 零大小类型包装 |
| Builder | L2 Trait + 方法链 | `../02_intermediate/00_traits/01_traits.md` | fluent API 构造 |
| Zero-cost Abstraction | L2 单态化（Monomorphization） | `02_intermediate/01_generics/01_generics.md` | 参数多态的编译期消除 |
| Interior Mutability | L1 借用（Borrowing） + L2 内存 | `01_foundation/01_ownership_borrow_lifetime/02_borrowing.md`, `02_intermediate/02_memory_management/01_memory_management.md` | 运行时权限检查替代编译期 |
...
| State Machine | L1 enum + match | `01_foundation/02_type_system/01_type_system.md` | 代数数据类型穷尽性 |
...
| 内存管理 | [`../02_intermediate/02_memory_management/03_memory_management.md`](../../02_intermediate/02_memory_management/01_memory_management.md) | 智能指针（Smart Pointer）模式 |
| 错误处理（Error Handling） | [`../02_intermediate/03_error_handling/04_error_handling.md`](../../02_intermediate/03_error_handling/01_error_handling.md) | Result 模式 |
| 工具链 | [`./01_toolchain.md`](../00_toolchain/01_toolchain.md) | 工程支撑 |
| 设计模式对比 | [`../05_comparative/03_paradigm_matrix.md`](../../05_comparative/00_paradigms/01_paradigm_matrix.md) | 范式定位 |
| 并发 / 异步（Async） | [`../03_advanced/00_concurrency/01_concurrency.md`](../../03_advanced/00_concurrency/01_concurrency.md) · [`../03_advanced/01_async/02_async.md`](../../03_advanced/01_async/01_async.md) | Observer 异步实现基础 |
...
设计模式边界测试验证 Rust 实现对经典模式的额外约束：Builder 未消费 `self`（借用（Borrowing）版 builder 与消费版的所有权差异，链式调用中途失效的编译错误）；多线程 Singleton 用 `static mut` 被 `Sync` 约束拒绝（正确解是 `OnceLock`/`LazyLock`）；Newtype 为外部类型实现外部 trait 触发孤儿规则（Orphan Rule）（编译器强制“本地类型或本地 trait 至少其一”）。三类用例展示 Rust 模式实现的安全边界在哪里。（编译错误）、边界测试：访问者模式与 Rust 的枚举（Enum）匹配（设计权衡）等5个方面。
...
> **相关文件**: [问题图谱](../../00_meta/04_navigation/10_problem_graph.md) · [能力图谱](../../00_meta/00_framework/competency_graph.md#三设计能力) · [Trait](../../02_intermediate/00_traits/01_traits.md)
...
本组测验覆盖 Rust 四大基础模式：RAII（Drop 与资源生命周期的绑定语义）、Typestate（状态编码进类型的编译期转换检查）、Builder 与所有权（消费 self vs 可变引用（Mutable Reference）的两种 builder 风格取舍）、Newtype（零成本包装与孤儿规则绕行）。测验按理解层→应用层递进，建议全部作答后再对解析；应用层错题通常指向所有权语义的理解缺口。e 模式（分析层）等5个方面。
...
行为型与结构型的补充模式从 crates 实践迁移而来，七个模式可按“控制流整形”与“对象关系整形”两组理解：

- **控制流整形**: 责任链（请求沿处理器链传递直到被消费）、模板方法（骨架固定、步骤可换——Rust 中用 trait 默认方法表达）、解释器（小语言求值）、迭代器（Rust 已内建为 `Iterator` trait，是标准库最成功的模式实例）。
- **对象关系整形**: 备忘录（状态快照）、中介者（把多对多交互收敛到中心协调者——Rust 中常用 channel + 消息循环实现）、访问者（为类型族添加操作，Rust 中多以 `match` 或 trait 下行转型替代）。

共同判定依据：Rust 的实现策略是“先问语言是否已内建”——迭代器（Iterator）、RAII、类型状态机已进入语言/标准库层面，剩余模式才需要显式结构设计。
...
备忘录模式（Memento）在不破坏封装的前提下捕获对象内部状态，支持撤销/重做。三要素是发起人（Originator）、备忘录（Memento）与管理者（Caretaker），其中备忘录对管理者不透明是封装性的关键。

- **Rust 实现**: 天然契合值语义——若状态类型实现 `Clone`（或快照本身不可变），备忘录就是状态的深拷贝，历史栈用 `Vec<Snapshot>` 维护；需要“窄接口”隔离时，可定义私有 `Snapshot` 类型，仅 Originator 能构造/恢复。
- **适用场景**: 编辑器撤销栈、事务补偿、游戏存档点；增量快照（只存变更 diff）是状态大时的标准优化，代价是恢复路径变长。

判定依据：状态小且变更频繁 → 全量快照 + `Clone`；状态大 → 命令模式记录操作而非状态；需要跨会话持久化 → 快照必须可序列化（`serde`）。
...
解释器模式为特定“语言”定义文法表示与解释器。Rust 实现简单表达式求值器时，`enum Expr { Num(i64), Add(Box<Expr>, Box<Expr>), Mul(..) }` 把文法直接编码为递归和类型，解释函数是 `match` 驱动的折叠（fold）——这比面向对象的类层次 + 双重分派简洁一个量级。

- **模式定义**: 适用于文法简单、变化不频繁的场景；文法复杂时（完整编程语言）应使用解析器生成器或解析组合子（`nom`/`chumsky`），而非手写解释器类层次。
- **适用场景**: 规则引擎、查询 DSL、配置表达式；性能敏感的热点解释可进一步编译为闭包链或字节码（tree-walking → 编译执行是经典演进路径）。

判定依据：`match` 分支超过十余个或文法每周变化，是“解释器模式超载”信号，应转向解析器基础设施。
