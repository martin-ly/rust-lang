...
L0 是惯用法谱系的「零层」：单个表达式/语句层面的地道写法，不引入任何抽象，只关乎「同样的意思怎么写最 Rust」。四个代表项（`?` 传播、`if let` 单模式匹配、复合赋值与范围循环、字面量与后缀约定）的共同判据：**用类型系统已有的糖，而不是手工展开**——`?` 替代 `match` 传播、`if let` 替代单臂 `match`、`for x in 0..n` 替代索引循环。L0 惯用法的特点是「无争议」：它们不改变程序的抽象结构，只减少样板；clippy 的 `redundant_*`/`needless_*` lint 族大多作用于这一层。学习建议：L0 是「能编译」到「能读懂」的最短路径，任何新代码都应先过一遍 clippy 的 L0 级 lint。
...
2. **Typestate**：泛型参数编码状态，非法状态转换无对应方法——编译期协议检查。
...
接口级惯用法决定 crate 的 API 是否“像 Rust”。四条核心规则来自 Rust API Guidelines：实现 `From<T>` 自动获得 `Into<U>`（转换语义单向实现、双向可用）；`AsRef`/`Borrow` 让函数接受更宽泛的借用类型；`Deref` 只用于智能指针语义而非继承模拟；迭代器适配器优先于显式循环暴露内部结构。共同判据：接口应让调用方写更少的类型标注、犯更少的转换错误。
...
- **守卫（guard）模式**：`MutexGuard`/`RefMut` 的「返回值即权限，drop 即归还」——资源权限与值的生命周期绑定，是 L3 最优雅的形态；自研资源（连接池租约、临时文件）应复刻此模式。
...
1. **`?` 传播**：函数返回 `Result`，错误沿调用链自动上抛，替代 `match` 金字塔；配合 `thiserror` 定义错误枚举保持上下文。
...
1. **通道优于共享**：`mpsc`/`broadcast` 传递消息所有权，竞争在类型层消失；「通信共享内存」优于「共享内存通信」。
2. **`Arc<Mutex<T>>` 的正确粒度**：锁内不做 `.await`（Tokio 中换 `tokio::sync::Mutex` 或重设计为消息传递），锁持有时间以微秒计。
3. **结构化并发**：`JoinSet`/`select!` 保证子任务生命周期不逃逸父作用域；泄漏的 `tokio::spawn` 后台任务是异步服务的内存与逻辑黑洞。
...
L6 架构级惯用法是「跨模块/跨 crate 的结构决策」，四个代表项：
...
本节以部分 move 后整体使用结构体为反例，展示 Rust 习惯用法中必须遵守的所有权粒度规则。
