...
系统可组合性的四种经典模式对应不同的数据流动假设：管道-过滤器（Unix 管道式，线性流、每级独立变换）、数据流管道（有向图拓扑，支持分叉合并，如 beam 式批处理）、事件驱动组合（发布订阅解耦生产者与消费者，时序不确定）、分层组合（每层只对下一层编程）。选择判据：数据流向是否线性、是否需要背压、组件是否需要独立演化。Rust 的类型系统让管道模式受益最大——`Iterator` 适配器链就是零成本的管道-过滤器。ven Composition)与层组合 (Layer Composition)的顺序逐层展开。
...
可组合性的四个反模式都源于“为组合而组合”：嵌套泛型过深导致类型爆炸（错误信息不可读、编译时间陡增）；热路径上为灵活性引入 `dyn Trait`，间接调用阻断内联使性能退化一个量级；每层抽象都付编译时间税，组合深度与 CI 时长正相关；channel 组合若无背压与关闭传播协议，环形依赖即死锁。判据：组合层级超过三层、或错误信息出现超过一屏的类型名时，应考虑用具体类型压平抽象。
...
组合边界测试覆盖三类失效：trait 对象的对象安全约束（泛型方法/Self 返回使其不可对象化，组合时编译报错）、动态加载插件跨 ABI 边界传递非 `#[repr(C)]` 类型（运行时 UB，Rust 无稳定 ABI）、trait 约束过宽导致组合时的连贯性冲突（孤儿规则触发）。三类分别对应类型系统、FFI 边界与 trait 解析，是搭建插件化/组合式架构前必须验证的约束。it 组合子的菱形继承问题（编译错误）、边界测试：组合优于继承时的 boilerplate 问题（逻辑错误）等5个方面，本节逐一说明其要点。
