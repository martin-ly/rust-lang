...
> **后置概念**: [Engineering and Production Patterns](13_engineering_and_production_patterns.md) · [Architecture Patterns](08_architecture_patterns.md)
...
模式选择应从场景约束出发而非从模式目录出发。三个典型场景的驱动因素各不相同：Web 服务器受并发模型驱动（async + 中间件塔式组合）；插件系统受 ABI 与隔离驱动（trait 对象 + 动态加载边界）；游戏引擎受数据局部性驱动（ECS + 连续内存布局）。每个场景给出“约束 → 候选模式 → 排除理由”的完整链条，演示如何先列出硬约束再筛模式，而不是先爱上某个模式再找理由。
...
真实系统几乎不用单一模式，组合规则比单模式更重要。常见组合有固定搭配：Builder + Typestate（构造期状态合法性编译期保证）、Strategy + Factory（按配置选算法族）、Observer + Command（事件驱动解耦）。HTTP 客户端示例展示完整组合链：Builder 构造配置、Connection Pool 管理资源、Retry 策略包装 transport、中间件塔处理横切逻辑——每层模式解决一个独立问题，接口契约保持单向依赖。
