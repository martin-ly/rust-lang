...
> **前置概念**: [Architecture Patterns](08_architecture_patterns.md)
...
- [Rust 系统设计原则与国际权威对齐](#rust-系统设计原则与国际权威对齐)
  ...
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：全局 `RefCell` 状态的嵌套可变借用（rustc 1.97.0 实测）](#反例全局-refcell-状态的嵌套可变借用rustc-1970-实测)
    - [✅ 修正：缩短借用作用域](#-修正缩短借用作用域)
...
七项核心设计原则与 Rust 语言机制的锚定关系：

| 原则 | Rust 机制 | 编译期/运行期 |
|---|---|---|
| 内存安全 | 所有权 + 借用检查（capability 由 `&`/`&mut` 表达） | 编译期 |
| 并发安全 | `Send`/`Sync` 标记 trait（会话类型的弱化编码） | 编译期 |
| 零成本抽象 | 泛型单态化 + 内联（Stroustrup 原则） | 编译期 |
| 组件组合 | trait + 关联类型（态射复合的工程近似） | 编译期 |
| 错误显式化 | `Result` + `?`（错误即值） | 类型层 |
| 资源确定性 | RAII + `Drop`（作用域绑定） | 运行期确定点 |
| 最小惊讶 | 无隐式转换、无空值（`Option` 显式） | 类型层 |

七原则的共同点：**把运行期检查上移为编译期证明**。系统设计的判读顺序应是：先用类型表达约束，其次借用检查，最后才接受运行期断言。

判定依据：设计评审时逐项问「该原则若违反，编译器能否捕获」——答案为「能」却仍靠测试兜底的，是设计债。
...
1. **设计原则依赖图**：七原则非并列——「内存安全」是地基（借用检查），「并发安全」与「资源确定性」建于其上（`Send`/`Sync` 与 `Drop` 依赖所有权），「零成本抽象」「错误显式化」是上层建筑。推翻地基（滥用 `unsafe`）会使上层所有保证失效——这解释了为何 `unsafe` 审计是系统设计的最高优先级。
2. **系统架构决策树**：第一问「该约束能否用类型表达？」→ 能则 newtype/typestate/trait；不能则第二问「能否用借用检查缩小可变范围？」→ 能则重构所有权；再不能才到「运行期机制」（`RefCell`/锁/通道）。决策树的每一层下沉都对应一类潜在运行期错误的引入。

判定依据：设计文档中应显式标注每个关键约束落在决策树的哪一层，逐层下移需写明理由。
...
系统设计原则的定理矩阵把经验法则形式化为可核查的蕴涵式：零成本抽象定理声明 release 模式下高层抽象与手写底层代码性能等价（失效于未内联/动态分发场景）；Capability 安全映射声明 safe Rust 的所有权规则蕴含无 UAF/双重释放（失效于 unsafe 块）。每条定理标注 L4 公理依赖与失效条件，使“原则何时不适用”与原则本身同样明确。
...
- [L3 异步](../../03_advanced/01_async/01_async.md) —— async/await 与 Tower 组合
- [L4 RustBelt](../../04_formal/02_separation_logic/01_rustbelt.md) —— Iris 逻辑与安全证明
- [L5 Rust vs Go](../../05_comparative/01_systems_languages/03_rust_vs_go.md) —— 并发模型哲学对比
- [L6 惯用法谱系](02_idioms_spectrum.md) —— 架构级惯用法
- [L6 设计模式](01_patterns.md) —— Error Kernel / 洋葱中间件
...
> **相关文件**: [问题图谱](../../00_meta/04_navigation/10_problem_graph.md) · [能力图谱](../../00_meta/00_framework/competency_graph.md) · [安全边界](../../05_comparative/03_domain_comparisons/01_safety_boundaries.md)

## 十、边界测试：系统设计原则的编译错误

系统设计边界测试验证原则的编译期可执行性：`Rc` 跨线程移动违反 `Send`（编译期拦截共享可变性）、trait object 缺少 `Send + Sync` bound 时的对象安全约束、依赖注入中动态分发的性能边界（trait object vs 泛型单态化）。三类用例分别对应并发安全、对象安全与性能权衡——它们是“原则落地为代码”时最先撞到的三面墙。权衡（运行时开销）、边界测试：过度工程化的类型状态机（编译复杂度爆炸）等6个方面，本节依次展开。
...
本节以全局 `RefCell` 状态的嵌套借用为反例，展示运行时借用检查对全局可变状态设计的约束。
...
> **认知功能**: 本 mindmap 从本页「Rust 系统设计原则与国际权威对齐」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
