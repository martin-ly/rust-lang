...
三条学习路径按目标深度设计：快速实战（2 小时）只做同步 echo + reqwest 客户端，建立 socket 抽象手感；系统学习（1 周）补 tokio 异步模型、axum 框架与错误处理分层，达到可写生产原型水平；深度研究（1 个月）进入协议实现、零拷贝与性能调优，对标基础设施开发。每档都有明确出口标准（能独立交付什么），避免“学了很多但写不出服务”的常见陷阱。
...
五个基础示例构成网络编程的最小完备集：同步 TCP echo（`std::net` 阻塞模型，理解 accept/read/write 生命周期）、异步 TCP echo（tokio 改写，对比事件驱动与线程模型）、UDP 收发（无连接语义、丢包由应用处理）、HTTP 客户端（reqwest 的高层抽象）、错误处理（`io::Error` 的分类与重试判据）。建议亲手敲一遍并互相改写——同步改异步、TCP 改 UDP——改写过程暴露的差异就是网络编程的核心知识点。
