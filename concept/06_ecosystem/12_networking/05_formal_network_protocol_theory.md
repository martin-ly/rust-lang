...
> **权威来源**: [concept/06_ecosystem/12_networking/05_formal_network_protocol_theory.md](05_formal_network_protocol_theory.md)
...
「协议形式化定义」部分按协议三元组、TCP 状态机示例与协议不变式的顺序逐层展开。
...
「状态机模型与验证」部分按扩展有限状态机（EFSM）、状态空间探索与Kani 形式化验证的顺序逐层展开。
...
本节从时间 Petri 网 与 π 演算 两个层面剖析「网络演算」。
...
本节从 TCP Reno 拥塞控制 与  IP 分片与重组 两个层面剖析「形式化 TCP/IP 片段」。
...
「协议组合与 Rust 类型系统验证」部分包含协议栈组合 与 幽灵类型强制协议状态 两条主线，本节依次说明。
