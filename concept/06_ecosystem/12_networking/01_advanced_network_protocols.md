...
本文系统梳理 Rust 生态中的高级网络协议，按通信模型组织为十五个主题：协议分类、协议栈架构、gRPC、MQTT、QUIC、AMQP、GraphQL、SSE、协议选型决策树、性能对比、安全考量及生产实践。核心框架是三条判据：

1. **通信模型匹配**：请求-响应（gRPC/GraphQL）、发布-订阅（MQTT/AMQP）、单向推送（SSE）、双向流（gRPC streaming/QUIC）——模型不匹配会导致用轮询模拟推送之类的反模式。
2. **网络环境约束**：IoT 弱网选 MQTT（QoS 分级、极小报文头），NAT/移动网络选 QUIC（连接迁移、0-RTT），企业集成选 AMQP（可靠投递与路由）。
3. **Rust 生态成熟度**：`tonic`、`rumqttc`、`quinn`、`lapin` 均为生产可用，但各自的功能覆盖（如 MQTT5 特性、QUIC 的 HTTP/3 栈）需按版本核对。

阅读路径：先用第 1–2 节建立全景，再按选型决策树定位协议，最后读对应章节。
