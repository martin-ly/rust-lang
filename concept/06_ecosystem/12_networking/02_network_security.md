...
DDoS 防护在网络层之外还需要应用层的自适应限流。本节给出两类互补机制的 Rust 实现模式：

- **连接速率限制**：按源 IP 维护并发连接数与滑动时间窗内的连接速率，超过阈值即拒绝或延迟 accept；实现要点是 `Arc<RwLock<HashMap<IpAddr, _>>>` 的锁粒度（高并发下应换 `DashMap` 分片）与过期条目的定期清理（否则限速表本身成为内存 DoS 面）。
- **慢速攻击防护**：Slowloris/Slow HTTP body 类攻击以极低速率发送请求占用连接；防御是给每个连接设置**首字节超时**与**整体读超时**（tokio 的 `timeout` 包裹 accept 循环内的读取），并限制单 IP 的半开连接数。

工程边界：应用层防护只能应对中小规模攻击，大规模流量型 DDoS 仍需 CDN/云清洗；本节机制的价值是过滤低速隐蔽攻击与保护后端连接池。
...
>
> **过渡**: 从密码学原语过渡到 Rust 内存安全（Memory Safety），可以建立“更少内存漏洞类”的安全信心。
>
