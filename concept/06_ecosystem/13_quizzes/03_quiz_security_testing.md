...
>
> **前置概念**:
> **后置概念**: [Security Practices](../07_security_and_cryptography/01_security_practices.md) · [Testing Strategies](../09_testing_and_quality/01_testing_strategies.md)
...
本节考查密码学基础：Q1 Kerckhoffs 原则，Q2 AEAD 原语识别，Q3 unsafe 边界管理的含义。
...
本节考查供应链审计分工：Q5 状态文件组成，Q6 辨析 `safe-to-run` 标准的保证范围，Q7 cargo vet 与 cargo audit 的职责划分。
...
**解析**：按 §2.2 内置审计标准（criteria）：`safe-to-deploy` 才是"代码不会引入严重安全漏洞（内存安全（Memory Safety） UB、恶意行为、不安全的 crypto 误用等），可进入生产"；`safe-to-run` 仅表示"代码可以执行（构建/测试工具、开发依赖），不做部署级安全保证"。二者级别不可混淆。
...
本节考查测试策略：Q9 列举编译期之外的动态验证手段，Q10 辨析测试金字塔在 Rust 中的映射。
...
**解析**：[测试策略](../09_testing_and_quality/01_testing_strategies.md) §1.3 提出"编译期即测试"（类型系统（Type System）排除整类错误），§二「技术细节」给出动态手段全景：内置测试框架（§2.1）、属性测试与模糊测试（§2.2，proptest 对随机输入断言属性）、Miri（§2.3，未定义行为检测）。四者构成从类型保证到运行时（Runtime）验证的完整谱系，对应 §三的分层测试策略。
...
本节（W3-b 扩展）考查属性测试与基准：Q11 proptest 相对示例测试的增量价值，Q12 性质表述的准确性。
