...
Rust 在区块链领域的采用不是偶然，两个技术优势直接对应链上代码的两类致命风险：

- **内存安全（Memory Safety） ⟹ 合约漏洞类别消除**: 重入攻击之外的溢出、越界、use-after-free 是 EVM/Solidity 合约的经典损失来源（整数溢出曾导致多起代币事件）；Rust 的借用（Borrowing）检查 + 默认溢出检查（debug panic / release 可配置）把这些类别在编译期或运行早期暴露，而非审计期事后发现。
- **无 GC 的确定性执行**: 链上执行要求所有节点对同一输入得到**逐字节一致**的结果——GC 停顿、分配器策略差异都会破坏确定性；Rust 的所有权（Ownership）模型让内存行为完全可预测，且 `no_std` 支持使运行时（Runtime）可裁剪到 WASM 沙箱友好的最小集。

判定依据：这两个优势成立的前提是合约平台本身选择了 Rust（Solana/Polkadot/Near）；EVM 链上的 Rust 合约（Stylus 等）是后起路径，生态成熟度需单独评估。
...
三条主流 Rust 链代表了三种架构哲学，差异集中在**执行模型**与**扩展路径**：

- **Solana（Sealevel）**: 并行合约执行引擎——交易预先声明读写的账户集合，无交集的交易并行执行，吞吐依赖账户级冲突率；合约（program）是无状态的纯函数，状态全部外置到账户。
- **Polkadot（Substrate）**: 异构分片框架——中继链提供共享安全，平行链用 FRAME 的 pallet（可组合模块（Module））自定义状态机；Rust + WASM 运行时可实现无分叉升级（runtime 即链上 WASM blob）。
- **Near Protocol**: 分片执行 + 用户友好账户模型（可读账户名、合约即账户）；合约编译为 WASM，异步（Async）跨分片调用是其编程模型的核心约束。

判定依据：高频 DeFi 看 Solana 的并行模型；需要自定义链逻辑看 Substrate；应用链快速启动看 Near。
...
智能合约安全的对比必须精确到漏洞类别，笼统的“Rust 更安全”没有工程价值：

- **漏洞类别消除矩阵**: 整数溢出/下溢（Rust 默认检查 ⟹ 消除）、越界访问（借用检查 ⟹ 消除）、未初始化状态（类型系统（Type System）强制初始化 ⟹ 消除）；但**重入攻击**（合约回调中状态未更新）是逻辑层漏洞，Rust 不自动消除——对策是 checks-effects-interactions 模式与账户租约锁，仍需人工纪律。逻辑错误（错误的清算公式、预言机价格操纵）两种语言同样暴露。
- **Kani 在合约验证中的应用**: 对关键不变量（“总余额守恒”“只有 owner 可提币”）写 `#[kani::proof]` 做有界模型检查，覆盖单元测试难以穷举的输入空间；适合验证核心金额逻辑，不适合全合约端到端验证（状态空间爆炸）。

判定依据：Rust 消除的是内存/数值类漏洞，业务逻辑漏洞的防线是形式化规约 + 审计，两条防线缺一不可。
...
| 类型系统 | [`../01_foundation/02_type_system/04_type_system.md`](../../01_foundation/02_type_system/01_type_system.md) | 漏洞类别消除机制 |
| Unsafe | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) | VM 运行时底层边界 |
| 线性逻辑 | [`../04_formal/01_linear_logic.md`](../../04_formal/01_ownership_logic/01_linear_logic.md) | 代币作为线性资源的形式化 |
| 形式化验证工具链 | [`../04_formal/05_verification_toolchain.md`](../../04_formal/04_model_checking/01_verification_toolchain.md) | Kani / Verus 的验证理论 |
| 核心库谱系 | [`./03_core_crates.md`](../02_core_crates/01_core_crates.md) | `solana-program`、`ink` 等 crate 定位 |
| 应用领域 | [`./04_application_domains.md`](../06_data_and_distributed/01_application_domains.md) | 区块链作为 L6 应用域 |
...
区块链代码的错误代价是直接的资产损失，因此边界标准比普通金融系统更严。本章覆盖六个代表性边界：

- **常量时间执行**：标准 `==` 提前返回泄露时序侧信道，签名/密钥比较必须用 `subtle` 的常量时间原语——这是运行时风险，编译器无法代劳。
- **智能合约状态一致性（Coherence）**：SVM/EVM 合约中跨指令（cross-program invocation）的状态读写顺序错误可被重入利用；Rust 的类型状态模式可把“账户已验证”编码进类型，编译期杜绝顺序错误。
- **签名类型安全**：`ed25519-zebra`/`k256` 用新类型区分公钥/签名/消息，杜绝参数传反——类型系统是最便宜的形式化验证。
- **`no_std` 与堆分配**：链上环境（Solana/Substrate wasm）禁用或限制堆分配，哈希计算必须用定长数组与栈缓冲。
- 另两项覆盖整数溢出（链上必须 `checked_*`）与随机数来源（链上禁用 OS 熵）。
...
本节从测验 1：为什么 Rust 是区块链开发的热门语言？（理解层）、测验 2：Solidity 与 Rust 在智能合约开发中的主要区别是…、测验 3：什么是"重入攻击"（Reentrancy Attack）？R…、测验 4：Rust 的 `no_std` 在区块链中有什么用途？（理解…等5个方面切入，剖析「嵌入式测验（Embedded Quiz）」的核心内容。
