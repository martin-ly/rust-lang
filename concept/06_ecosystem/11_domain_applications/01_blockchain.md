...
本节围绕「Rust 在区块链领域的独特优势」展开，覆盖内存安全 ⟹ 合约无重入/溢出漏洞 与 无 GC 的确定性执行 两个方面。
...
「Rust 链架构对比」涉及 Solana (Sealevel)：并行合约执行引擎、Polkadot (Substrate)：异构分片与 FRAME与Near Protocol：用户友好与分片执行，本节逐一说明其要点。
...
本节围绕「智能合约安全形式化：与 EVM/Solidity 对比」展开，覆盖漏洞类别消除矩阵 与 形式化验证工具链：Kani 在合约验证中的应用 两个方面。
