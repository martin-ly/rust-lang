...
- **内存安全 ⟹ 合约漏洞类别消除**: 重入攻击之外的溢出、越界、use-after-free 是 EVM/Solidity 合约的经典损失来源（整数溢出曾导致多起代币事件）；Rust 的借用检查 + 默认溢出检查（debug panic / release 可配置）把这些类别在编译期或运行早期暴露，而非审计期事后发现。
- **无 GC 的确定性执行**: 链上执行要求所有节点对同一输入得到**逐字节一致**的结果——GC 停顿、分配器策略差异都会破坏确定性；Rust 的所有权模型让内存行为完全可预测，且 `no_std` 支持使运行时可裁剪到 WASM 沙箱友好的最小集。
...
- **Polkadot（Substrate）**: 异构分片框架——中继链提供共享安全，平行链用 FRAME 的 pallet（可组合模块）自定义状态机；Rust + WASM 运行时可实现无分叉升级（runtime 即链上 WASM blob）。
- **Near Protocol**: 分片执行 + 用户友好账户模型（可读账户名、合约即账户）；合约编译为 WASM，异步跨分片调用是其编程模型的核心约束。
...
- **漏洞类别消除矩阵**: 整数溢出/下溢（Rust 默认检查 ⟹ 消除）、越界访问（借用检查 ⟹ 消除）、未初始化状态（类型系统强制初始化 ⟹ 消除）；但**重入攻击**（合约回调中状态未更新）是逻辑层漏洞，Rust 不自动消除——对策是 checks-effects-interactions 模式与账户租约锁，仍需人工纪律。逻辑错误（错误的清算公式、预言机价格操纵）两种语言同样暴露。
...
- **智能合约状态一致性**：SVM/EVM 合约中跨指令（cross-program invocation）的状态读写顺序错误可被重入利用；Rust 的类型状态模式可把“账户已验证”编码进类型，编译期杜绝顺序错误。
