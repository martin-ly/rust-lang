...
> **前置依赖**: [类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) · [泛型（Generics）](../../02_intermediate/01_generics/01_generics.md) · [Trait](../../02_intermediate/00_traits/01_traits.md) · [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md)
> **后置延伸**: [嵌入式系统](../05_systems_and_embedded/03_embedded_systems.md) · [性能优化](../10_performance/01_performance_optimization.md) · [并发编程](../../03_advanced/00_concurrency/01_concurrency.md)
>
...
传统机器学习（非深度学习）在 Rust 中有两个定位不同的实现，覆盖从原型到嵌入式两端：

- **linfa**: 对标 scikit-learn 的模块（Module）化算法库，采用 `Fit`/`Predict` trait 统一接口，算法（线性模型、聚类、SVM）以独立 crate 发布；优势是与 `ndarray` 生态无缝衔接，类型参数化的设计让数据布局错误在编译期暴露。
- **smartcore**: 无标准库（`no_std` 兼容）的 ML 库，目标是无操作系统的嵌入式设备上做推理（如传感器端异常检测）；算法覆盖面较窄但二进制体积极小。
...
判定依据：传统 ML 在 Rust 中的价值是“部署可控性”而非训练生态——特征工程复杂、需快速迭代时 Python 仍是训练主场。
...
Rust 数据科学生态围绕**列式内存格式**构建，三个组件分别占据存储格式、内存表示与计算引擎三层：

- **polars**: 高性能 DataFrame 库，惰性执行引擎（lazy API）自动做谓词下推与投影裁剪；多线程默认开启，单机性能常超 pandas 一个数量级，且无需 JVM 依赖。
- **Apache Arrow**: 列式内存格式的跨语言标准——polars、DataFusion 及 Python/Spark 间的零拷贝数据交换都建立在 Arrow 的内存布局上。
- **DataFusion**: 可嵌入的 SQL 查询引擎，直接查询 Arrow/Parquet，是构建自定义数据平台（如 InfluxDB IOx）的底座。

判定依据：探索性分析用 polars（有成熟 Python 绑定）；需要 SQL 接口或构建数据产品时引入 DataFusion；Arrow 通常作为依赖隐式存在，只有跨语言交换时才需直接关心。
...
模型部署阶段的核心矛盾是**精度 vs 资源**，两条主线分别针对模型本身与运行环境：

- **量化与压缩**: 把 FP32 权重降到 INT8/INT4，模型体积缩小 4–8 倍、推理吞吐提升 2–4 倍，代价是 1–3% 的精度损失；Rust 推理引擎（`candle`、`llama.cpp` 的 Rust 绑定）普遍支持 GGUF 等量化格式，量化通常在导出时一次性完成。
- **边缘部署**: 资源受限设备上的部署约束按严格度排序：内存上限 → 功耗预算 → 实时性；`no_std` 推理（如 `tract` 的嵌入式配置）可在 MB 级内存中运行小型模型。

判定依据：先建立精度基准集——量化后精度掉过业务阈值的层应保留高精度（混合量化）；边缘部署先测量再裁剪，盲目压缩常导致二次返工。
...
1. 内存安全（Memory Safety） + 无 GC——推理服务的 P99 延迟无 GC 停顿毛刺，这对实时推荐、高频交易类场景是硬指标；
2. 部署形态——单静态二进制 + 交叉编译，WASM/嵌入式目标可达；
3. 类型化张量形状（candle/burn 的 `Tensor<D>` 维度泛型（Generics））把部分形状错误前移到编译期。
...
Rust ML 生态讨论中有三个需要校准的流行命题，本节分别给出反命题树与根结论：

1. **“Rust ML 将完全替代 Python”**：Python 二十年生态积累 + 动态类型的快速迭代优势使其继续主导研究/原型；Rust 的确定性优势在**部署与推理**阶段。现实趋势是混合架构：Python 训练 → ONNX → Rust 推理服务。
2. **“纯 Rust 框架总是优于绑定框架”**：candle/burn 胜在内存安全与无二进制依赖，但 tch-rs 直接获得 PyTorch 全功能、ort 获得 ONNX Runtime 的图优化与成熟 GPU 支持；训练/复杂模型选绑定，边缘/推理选纯 Rust。
3. **“polars 在所有场景优于 pandas”**：>100MB 数据与分析查询 polars 优势明确；但 EDA、小数据与 pandas 特有功能（MultiIndex、时间序列重采样）场景 pandas 仍更便利。

选型原则：按工作流阶段（研究/训练/推理/分析）分别选工具，而非追求单一技术栈。
...
ML 系统的边界失效有三个独有特征：张量内存巨大、数据并行普遍、维度错误只在特定输入形状下触发。本章覆盖三类边界：

1. **未初始化张量内存（安全漏洞）**：GPU/CPU 缓冲区若未清零，可能向模型输出泄露先前租户的密钥或数据——多租户推理服务的真实事故模式；candle 的 `Tensor::zeros` 安全，但自定义分配或 FFI 张量需显式清零。
2. **DataFrame 的并发约束**：polars 的 `DataFrame` 不是 `Send + Sync`，直接跨线程共享被编译器拒绝——并行特征工程的正确姿势是按分区切分后各自处理再合并。
3. **模型输入维度不匹配**：reshape/广播错误在编译期不可见（形状是运行时值），只在特定 batch size 下 panic；防御手段是入口处的形状断言与 `ndarray` 的类型化维度。
...
- [嵌入式系统](../05_systems_and_embedded/03_embedded_systems.md) — `#![no_std]`、资源受限环境
- [并发编程](../../03_advanced/00_concurrency/01_concurrency.md) — Send/Sync、多线程并行
- [类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) — 泛型（Generics）、Trait、类型安全
- [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) — FFI 绑定、C 库交互
- [网络协议](../04_web_and_networking/07_network_protocols.md) — gRPC、HTTP/2、序列化
- [云原生](../04_web_and_networking/02_cloud_native.md) — 容器化部署、微服务
- [WebAssembly](03_webassembly.md) — WASM 目标、浏览器内推理
...
`candle` 纯 Rust 实现，无 Python 依赖，支持 WASM 部署。API 更简洁、类型安全。`PyTorch` 生态更成熟、研究支持更好，但依赖 Python 运行时（Runtime）。
</details>
...
推理需要低延迟、高吞吐、确定性的内存使用。Rust 的无 GC 特性避免了 Python 推理服务中的 GC 停顿，同时内存安全（Memory Safety）防止了 C++ 引擎的崩溃风险。
</details>
...
mindmap
  root((Machine Learning))
    ...
    二、概念属性矩阵
    三、深度学习框架
      3.1 candle纯 Rust 推理引擎
      3.2 burn可移植深度学习框架
      3.3 tch-rsPyTorch C++ API
    四、传统机器学习
      4.1 linfascikit-learn
      4.2 smartcore无标准库 ML
    六、模型部署与推理优化
      6.1 量化与压缩
      6.2 边缘部署
```
