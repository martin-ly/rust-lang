...
> **前置概念**: [Toolchain](../00_toolchain/01_toolchain.md) · [Cargo](../00_toolchain/01_toolchain.md) · [Security Practices](../07_security_and_cryptography/19_security_practices.md)
> **后置概念**: [Cross Compilation](../05_systems_and_embedded/17_cross_compilation.md) · [Distributed Systems](../04_web_and_networking/18_distributed_systems.md)
>
...
「技术细节」涉及许可证合规工具链、双重许可策略与商业使用考量，本节逐一说明其要点。
...
- [Cross Compilation](../05_systems_and_embedded/17_cross_compilation.md) — 交叉编译
...
「边界测试：许可证与合规的编译错误」涉及边界测试：`cargo deny` 的许可证冲突（编译错误/构建失败）、边界测试：`#[forbid(unsafe_code)]` 与依赖的…、边界测试：Copyleft 许可证的静态链接传染（法律合规风险）、边界测试：GPL 传染与动态链接的边界（法律风险）等6个方面，本节逐一说明其要点。
