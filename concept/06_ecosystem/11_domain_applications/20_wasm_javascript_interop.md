...
Rust 1.92.0 放宽了联合体（union）字段访问规则：在持有有效原始引用时，安全代码可以读取 union 字段而无需 `unsafe` 块。这对 WASM/FFI 互操作有直接价值：

- **场景**：C ABI 的 tagged union（如 WASI 的 `event`、JS 值的 tagged pointer 表示）在 Rust 侧用 union 建模时，访问已知活跃字段的样板 `unsafe` 可消除。
- **安全前提**：调用方仍需保证读取的是当前活跃变体（通常由 tag 字段判定），编译器不替你做判别——放宽的只是引用层面的限制，类型双关（type punning）错误仍可能发生。
- **工程收益**：FFI 边界代码的 `unsafe` 块减少，使安全审查能聚焦真正的风险点（指针有效性、生命周期）。

下面给出基于 c12_wasm crate 的 `WasmFFIUnion` 的可编译示例。
