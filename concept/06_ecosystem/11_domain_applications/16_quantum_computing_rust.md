...
判定依据：通用电路 <30 qubit → 态向量；结构规则电路 → 张量网络；Rust 的零成本抽象使两种路线都能贴近 C 性能。
...
- **“Rust 的类型系统能直接编码量子不可克隆定理”** —— 需要精确理解。所有权语义确实与不可克隆定理同构（量子态不可 `Clone`），但经典模拟器中的“量子态”只是经典数据，类型系统编码的是模拟的抽象纪律而非物理定律本身；这一对应是教学价值大于工程约束。
