...
| 概念 | 数学对象 | 经典对应 |
|---|---|---|
| 量子比特（qubit） | 二维复希尔伯特空间单位向量 `α\|0⟩ + β\|1⟩`，`\|α\|²+\|β\|²=1` | 经典比特（仅 \|0⟩/\|1⟩） |
| 叠加 | 状态的线性组合 | 无（概率混合 ≠ 相干叠加） |
| 测量 | 向基态投影，概率 `\|α\|²`/`\|β\|²`，不可逆 | 无损读取 |
| 纠缠 | 多比特态不可分解为单比特张量积（如贝尔态 `(\|00⟩+\|11⟩)/√2`） | 无 |
| 量子门 | 酉矩阵（可逆、保范数） | 逻辑门（多数不可逆） |

两个直接推论：**不可克隆定理**（酉演化无法复制未知量子态）——量子程序无「变量复制」概念；**测量即破坏**——调试量子算法不能中途窥视状态，只能大量重复采样统计分布。

判定依据：量子模拟器代码中任何「复制量子态」的 API 都是非物理的简化。
...
Rust 量子计算生态处于早期但层次已清晰，五个主题覆盖从电路表示到模拟执行的完整链路：

- **roqoqo / qoqo**: 量子电路的表示与操作工具包（Rust 核心 + Python 绑定），定位是“量子程序的 IR 层”，支持与多个硬件后端的编译对接。
- **rustqip**: 图构建式量子模拟器，把电路表示为有向图，模拟过程利用 Rust 的并行能力（`rayon`）加速状态向量演化。
- **q1tsim / qasmsim / rusq 等模拟器**: 覆盖不同抽象层——OpenQASM 解释执行、状态向量模拟、教学向简化模型。
- **与 Python 生态的交互**: PyO3 是标准桥——Rust 负责性能密集内核（状态向量更新、测量采样），Python 负责实验编排与可视化（Qiskit/Cirq 的混合工作流）。

判定依据：研究原型用 Python 生态；模拟性能成为瓶颈（>20 量子比特状态向量）时 Rust 内核的收益才开始兑现。
...
经典硬件模拟量子系统的本质是**指数爆炸**：n 个量子比特的态向量有 2ⁿ 个复数振幅，30 qubit 需 2³⁰×16B ≈ 17GB 内存——这是「笔记本模拟 30 比特」边界测试的定量依据。

三种模拟路线：

1. **态向量模拟**：直接演化 2ⁿ 向量，精确但受内存限制（上限 ~34 qubit）。Rust 优势在 SIMD（一次处理多振幅）+ `rayon` 并行门应用，可比朴素 Python 实现快 1–2 个数量级。
2. **张量网络收缩**：把电路表示为张量网络，收缩顺序决定成本；对低纠缠电路可模拟 100+ qubit，代价是实现复杂度。
3. **稳定子（Clifford）模拟**：仅对 Clifford 电路多项式时间有效，用于纠错码验证。

判定依据：通用电路 <30 qubit → 态向量；结构规则电路 → 张量网络；Rust 的零成本抽象（Zero-Cost Abstraction）使两种路线都能贴近 C 性能。
...
后量子密码（PQC）迁移的紧迫性来自「先存后破（harvest now, decrypt later）」攻击：今天截获的 TLS 流量可在量子计算机成熟后被解密，故**密钥交换**需先迁移（签名可稍缓，因签名验证在握手当下完成）。

NIST PQC 标准（2024 定稿）与 Rust 落地：

| 标准 | 用途 | Rust 生态 |
|---|---|---|
| ML-KEM（CRYSTALS-Kyber） | 密钥封装 | `pqcrypto-mlkem`、aws-lc-rs |
| ML-DSA（CRYSTALS-Dilithium） | 数字签名 | `pqcrypto-mldsa` |
| SLH-DSA（SPHINCS+） | 保守签名方案 | `pqcrypto-sphincsplus` |

rustls 已支持 X25519+ML-KEM 混合密钥交换（hybrid KEM），混合模式的工程意义：即使 PQC 算法被攻破，安全性仍不低于经典方案。

判定依据：新部署的 TLS 服务端应启用混合 KEM；签名迁移待 CA 生态就绪。
...
量子-经典混合工作流是 NISQ（含噪声中等规模量子）时代的主流范式，两个层面覆盖算法骨架与优化内核：

- **变分量子算法（VQE / QAOA）**: 经典优化器 + 量子协处理器的迭代闭环——量子侧用参数化电路（ansatz）制备态并测量期望值，经典侧（梯度下降/无梯度优化）更新参数；VQE 面向基态能量求解（化学模拟），QAOA 面向组合优化。Rust 的角色是经典侧优化器与测量数据处理的高性能实现。
- **参数移位与梯度计算**: 参数移位法则（parameter-shift rule）给出量子电路对旋转门参数的解析梯度——两次移位测量即可精确求导，无需有限差分；这与经典自动微分形成有趣的“硬件原生 AD”对照，Rust 实现可借助并行测量批量求梯度。

判定依据：混合工作流的瓶颈通常在经典-量子往返延迟（云端 QPU 排队），本地模拟器阶段 Rust 的并行采样才是主要优化点。
...
- **“量子计算将很快让经典密码与经典编程模型过时”** —— 不成立。容错量子计算（fault-tolerant QC）需要的物理量子比特数（百万级，含纠错开销）超出当前硬件两个数量级以上；对 Rust 生态的实际影响路径是**后量子密码迁移**（`rustls`/ring 的 PQC 算法集成），而非编程范式更替。
- **“Rust 的类型系统（Type System）能直接编码量子不可克隆定理”** —— 需要精确理解。所有权（Ownership）语义确实与不可克隆定理同构（量子态不可 `Clone`），但经典模拟器中的“量子态”只是经典数据，类型系统编码的是模拟的抽象纪律而非物理定律本身；这一对应是教学价值大于工程约束。
- **边界极限**: 状态向量模拟在 ~30 量子比特时耗尽单节点内存（2³⁰ 复数 ≈ 16GB），更大规模需张量网络或分布式模拟——这是算法选择问题，不是实现语言能突破的。
...
量子计算模拟的边界与普通 HPC 不同：物理定律本身就是约束。本章覆盖三个代表性边界：

1. **克隆量子态（不可克隆定理违反）**：量子信息论的基本定理禁止任意复制未知量子态；Rust 代码中可借类型系统表达——不实现 `Clone` 的量子态类型让“复制”成为编译错误，把物理约束编码进类型。
2. **模拟 30 量子比特（内存爆炸）**：n 比特态向量需 2ⁿ 个复数，30 比特 ≈ 16GB（`Complex64`），34 比特即超出单节点内存——这决定了笔记本模拟的硬上限约 28–30 比特，超过必须走张量网络/分布式模拟或真机。
3. **非厄米算符演化（类型/逻辑错误）**：合法演化必须幺正（保范数），用非厄米矩阵“演化”会使概率不守恒、结果无物理意义；模拟库应在校验层拒绝非幺正算符。

每节给出错误代码、物理判据与类型化防御写法。
...
- [安全与密码学](../07_security_and_cryptography/02_security_cryptography.md) — 对称/非对称加密、TLS、侧信道防护
- [性能优化](../10_performance/01_performance_optimization.md) — SIMD、缓存优化、内存布局、零拷贝
- [并发编程](../../03_advanced/00_concurrency/01_concurrency.md) — Send/Sync、多线程并行、内存模型
- [类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) — 泛型（Generics）、Trait、类型状态模式
- [泛型（Generics）](../../02_intermediate/01_generics/01_generics.md) — 零成本抽象（Zero-Cost Abstraction）、类型参数化
- [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) — FFI 绑定、C 库交互（QuEST、PQClean）
- [机器学习生态](13_machine_learning_ecosystem.md) — 优化算法、自动微分、张量运算
- [分布式系统](../04_web_and_networking/01_distributed_systems.md) — 分布式模拟、MPI、集群计算
- [形式化验证](../../04_formal/04_model_checking/01_verification_toolchain.md) — 密码学实现的形式化证明、常量时间验证
- [WebAssembly](03_webassembly.md) — 浏览器内量子模拟、跨平台部署
...
mindmap
  root((Rust 量子计算生态))
    ...
    四、经典硬件上的量子模拟
      4.1 态向量模拟
      4.2 张量网络收缩
      4.3 Rust 性能优势SIMD 与并行化
    五、量子安全密码学
      5.2 CRYSTALS-Kyber /
      5.3 pqclean-rust、pqcrypto
    六、量子-经典混合工作流
      6.1 变分量子算法VQE / QAOA
      6.2 参数移位与梯度计算
```
