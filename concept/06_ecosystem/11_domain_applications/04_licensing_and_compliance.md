...
合规边界测试把法务与安全策略转化为 CI 检查：`cargo deny check licenses` 在引入 GPL 依赖时使构建失败（配置白名单是团队许可证策略的可执行形式）；`#![forbid(unsafe_code)]` 在误加 unsafe 时编译失败，配合允许 unsafe 的模块白名单形成审计边界；`cargo audit` 对含 RUSTSEC 公告的依赖版本直接报错。三类检查的共同理念：合规规则若不能自动执行，就会在 deadline 压力下被绕过。de)]` 与依赖的…、边界测试：Copyleft 许可证的静态链接传染（法律合规风险）、边界测试：GPL 传染与动态链接的边界（法律风险）等6个方面，本节逐一说明其要点。
