...
Rust 项目的许可证合规有可自动化的工具链：`cargo deny` 在 CI 中按白名单拦截不兼容许可证（GPL 系传染到闭源产品的典型场景）、`cargo about` 生成依赖许可证聚合文件（二进制分发时随附 NOTICE 的法律要求）。双重许可（MIT OR Apache-2.0）是 Rust 生态惯例：MIT 降低采用门槛，Apache-2.0 提供专利授权条款——新 crate 默认应跟随此惯例。商业使用的核心考量是专利条款与商标，而非代码使用费。
...
“MIT/Apache 许可的依赖可以随便用”忽略了三个边界：许可证兼容是传递性的——深层依赖中的 GPL 组件会传染整个二进制；`include_str!`/代码复制把许可证义务从依赖关系变成衍生作品问题（需保留版权声明）；许可证与出口管制、专利授权是两个维度（Apache-2.0 有专利条款而 MIT 没有）。边界极限：cargo deny 只能识别标准 SPDX 标识，手写或非标准许可文本仍需人工审查，工具降低但不能消除法务成本。
...
- [Security Practices](../07_security_and_cryptography/01_security_practices.md) — 安全实践
- [Cross Compilation](../05_systems_and_embedded/02_cross_compilation.md) — 交叉编译
...
合规边界测试把法务与安全策略转化为 CI 检查：`cargo deny check licenses` 在引入 GPL 依赖时使构建失败（配置白名单是团队许可证策略的可执行形式）；`#![forbid(unsafe_code)]` 在误加 unsafe 时编译失败，配合允许 unsafe 的模块（Module）白名单形成审计边界；`cargo audit` 对含 RUSTSEC 公告的依赖版本直接报错。三类检查的共同理念：合规规则若不能自动执行，就会在 deadline 压力下被绕过。de)]` 与依赖的…、边界测试：Copyleft 许可证的静态链接传染（法律合规风险）、边界测试：GPL 传染与动态链接的边界（法律风险）等6个方面，本节逐一说明其要点。
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：Rust 生态中最常见的许可证是什么？为什么 MIT/Apa…、测验 2：`cargo-deny` 工具在许可证合规中起什么作用？（理…、测验 3：GPL 和 MIT 许可证在衍生作品分发上有什么根本区别？（…、测验 4：什么是"许可证冲突"？在 Rust workspace 中如…等5个方面，本节逐一说明其要点。
