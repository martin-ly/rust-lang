...
形式化算法理论用数学方法严格定义和分析算法，为 Rust 程序的**正确性、复杂度、可计算性**提供理论基础。Rust 的强类型系统与所有权模型将大量运行时不变量上移至编译期，从而缩小了需要外部验证的“信任基”。
...
Hoare 逻辑使用 `{P} S {Q}` 描述程序片段：若前置条件 $P$ 成立，执行 $S$ 后后置条件 $Q$ 成立。Rust 的类型系统可编码部分前置条件（如 `NonNull<T>` 排除空指针），而运行时断言负责剩余部分。
