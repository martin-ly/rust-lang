...
> **前置概念**: [Toolchain](../00_toolchain/01_toolchain.md) · [FFI](../../03_advanced/04_ffi/01_rust_ffi.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md)
> **后置概念**: [WASI](../05_systems_and_embedded/01_wasi.md)
...
**推荐迁移方案**：使用 `#[link(wasm_import_module = "env")]` 显式声明导入模块（Module）。该写法在 Rust 1.96 之前和之后行为一致：
...
- [Application Domains](../06_data_and_distributed/01_application_domains.md) — 应用领域分析
...
| 目标 | 系统接口 | 适用场景 | 推荐度 |
| :--- | :--- | :--- | :--- |
| `wasm32-unknown-unknown` | 无 | 浏览器/Node.js 纯计算 | ⭐⭐⭐⭐⭐ |
| `wasm32-wasip1` | WASI Preview 1 | 命令行工具/服务器 | ⭐⭐⭐⭐⭐ |
| `wasm32-wasip2` | WASI Preview 2 | 组件化服务端应用 | ⭐⭐⭐⭐ |
| `wasm32-unknown-emscripten` | Emscripten | 移植现有 C/C++ 代码 | ⭐⭐ |

### 内存与 JS 互操作

- **MaybeUninit**：适合未初始化内存管理与对象池，内存安全（Memory Safety）最高。
- **Vec<T> / Box<[T]>**：标准库动态数组，适用于大多数场景。
- **wasm-bindgen**：自动生成 Rust ↔ JS 绑定，类型安全且易用，推荐度最高。
- **js-sys / web-sys**：分别提供 JavaScript 内置对象与 Web API 的完整绑定。
- **手动 FFI**：性能最高但复杂度与风险最大，仅在极端场景使用。
