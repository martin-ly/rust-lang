...
> 来源: [TRPL] Rust 的零成本抽象（Zero-Cost Abstraction）原则意味着：使用高阶函数、迭代器（Iterator）、泛型（Generics）不会引入运行时（Runtime）开销。`slice::sort_unstable()` 在随机数据上通常比 C++ `std::sort` 更快。
...
| [`concept/04_formal/05_verification_toolchain.md`](../../04_formal/04_model_checking/05_verification_toolchain.md) | Kani / Verus / Creusot 形式验证工具链选型指南 |
...
| [`concept/06_ecosystem/15_performance_optimization.md`](../10_performance/15_performance_optimization.md) | Criterion / flamegraph 性能分析方法论 |
