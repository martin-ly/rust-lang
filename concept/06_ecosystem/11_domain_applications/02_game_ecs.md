...
> **核心洞察**: ECS 的"数据与行为分离"哲学与 Rust 的"数据与所有权（Ownership）分离"是**同构的**。Component 是被拥有的数据，System 是消耗/借用（Borrowing）数据的函数，Entity 是数据的逻辑分组标识。
...
> **思维表征说明**: 此 `classDiagram` 从**类型系统（Type System）**视角展示 ECS 架构——`World` 是容器根，`Entity` 是标识符，`Component` 是数据 trait，`System` 是行为 trait，`Query` 是借用（Borrowing）检查的代理，`Archetype` 是存储优化结构。`-->` 表示组合关系，`..>` 表示依赖关系，`<|--` 表示继承。这种表征帮助程序员理解「ECS 不是 OOP 的替代品，而是数据导向的重新组织」。 [Bevy ECS Docs](https://bevy.org/learn/); [Data-Oriented Design Book](https://dataorienteddesign.com/dodbook/)
...
wgpu 是基于 WebGPU 标准的 Rust GPU 抽象层，其 API 设计深度嵌入 Rust 所有权（Ownership）模型：
...
| wgpu 资源 | Rust 所有权（Ownership）表达 | GPU 安全语义 |
|:---|:---|:---|
...
> **与 Unsafe 的关系**: 固定容量 ECS 不可避免地触及 `MaybeUninit` 和原始指针（Raw Pointer），这直接关联到 [L3 Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) 中的核心原则——`unsafe` 块应被最小化并封装在不可变接口之后。固定 ECS 的 `unsafe` 通常集中在 `query_mut()` 的迭代器（Iterator）实现中，外层 System 完全处于 safe Rust。
...
> **ECS 存储映射的替代**: 在桌面 Bevy 中，`World` 使用 `HashMap<TypeId, Storage>` 动态管理组件存储。在 `no_std` + `alloc` 下，`hashbrown::HashMap` 可直接替代。但在无 `alloc` 场景，组件类型数量必须在编译期确定，使用泛型（Generics）结构体（Struct）或 `enum` 包裹所有可能类型，而非运行时（Runtime） `TypeId`。
...
| `RenderPass` | `encoder.begin_render_pass()` | 绘制命令 | `drop`（隐式）| 生命周期（Lifetimes）绑定到 `&mut encoder`，不能逃逸 |
| `CommandBuffer` | `encoder.finish()` | 无（已编码命令）| `queue.submit([cb])` | `submit` 接受 `CommandBuffer`，之后不可再用 |
| `TextureView` | `texture.create_view()` | 作为 attachment 绑定 | 无（可多次绑定不同 pass）| 借用自 `Texture`，生命周期（Lifetimes）不超过纹理 |
...
> **核心洞察**: wgpu 的 API 通过 Rust 所有权系统实现了**GPU 资源的编译期线性检查**——CommandEncoder 的一次性消费、RenderPass 的借用生命周期（Lifetimes）、TextureView 的不超过纹理生命期，这些都是线性类型理论在 GPU 编程中的工程实现。
...
| **关系完整性** | 手动维护（易出孤儿引用（Reference））| 自动双向同步 | `RelationshipTarget` 内部使用 `EntityHashMap`（类似 `HashMap<Entity, T>`）|
| **循环引用（Reference）** | 不可能（无双向链接原生支持）| 可能发生（A → B → A）| 运行时（Runtime）检查或 `acyclic` 约束（未来方向）|
| **关系数据所有权** | N/A | 关系数据属于"边"而非端点 | `ChildOf` 作为 Component 被源 Entity 所有 |

> **核心洞察**: Bevy 的 Relations 将**图论中的边（edge）**引入 ECS，但保留了 Rust 的所有权语义——关系数据（`ChildOf`）作为 Component 被**源 Entity 拥有**，而目标 Entity 通过 `RelationshipTarget` 进行**弱引用（Reference）式索引**。这与 Rust 的 `Rc<RefCell<T>>` 模式类似，但由 ECS 的 archetype 存储保证缓存友好性。
...
| Unsafe | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) | SIMD / GPU 底层边界 |
| 线性逻辑 | [`../04_formal/01_linear_logic.md`](../../04_formal/01_ownership_logic/01_linear_logic.md) | 消耗性资源的形式化对应 |
| 核心库谱系 | [`./03_core_crates.md`](../02_core_crates/01_core_crates.md) | `bevy`、`wgpu`、`rapier` 等 crate |
| 应用领域 | [`./04_application_domains.md`](../06_data_and_distributed/01_application_domains.md) | 游戏作为 L6 应用域 |

> **[Bevy Book](https://bevy.org/learn/); [Bevy ECS Docs](https://bevy.org/learn/); [Fyrox Docs](https://fyrox-book.github.io/); [wgpu Documentation](https://docs.rs/wgpu/latest/wgpu/); [Data-Oriented Design Book](https://dataorienteddesign.com/dodbook/)** 游戏开发分析基于官方引擎文档和 DOD 研究。✅
...
> **修正**: Bevy 的 ECS **system 参数冲突**：1) `Query<&mut T>` 与 `Query<&T>` 不能共存（同一组件的可变和不可变查询）；2) `ResMut<R>` 与 `Res<R>` 不能共存（同一资源的可变和不可变引用（Immutable Reference））；3) `Query<&mut T>` 与 `Commands` 在特定情况下冲突（`Commands` 可能删除实体，影响 Query）。解决：1) `ParamSet` — 显式声明互斥参数集；2) 分多个 system — 通过事件或 `Commands` 通信；3) `Without<T>` 过滤 — 排除特定组件。Bevy 的编译期检查利用 Rust 的类型系统（Type System）防止 ECS 冲突，是 ECS + Rust 的独特优势。这与 Unity 的 ECS（运行时检查冲突，可能抛出异常）或 flecs（C ECS，类似编译期检查但不完全）不同——Bevy 的编译期保证消除了大量运行时错误。[来源: [Bevy ECS](https://bevy.org/learn/)] · [来源: [Bevy Query](https://docs.rs/bevy_ecs/)]
...
在系统执行期间不能立即修改 ECS 世界（会违反迭代器（Iterator）借用规则）。`Commands` 将创建/删除/修改操作排队，在当前系统阶段结束后批量执行。
</details>
...
本节以 ECS 组件存储的双重可变别名为反例，展示借用规则对「系统并行写组件」的静态约束。
