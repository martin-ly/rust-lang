...
「权威定义（Definition）」部分包含游戏引擎的定义与边界 与 游戏引擎核心子系统 两条主线，本节依次说明。
...
本节围绕「引擎架构模式」展开，依次讨论主循环与更新阶段、子系统管理与Rust 中的引擎骨架。
...
理解「渲染管线」需要把握现代图形 API 演进、wgpu：安全的跨平台 GPU 抽象与渲染管线阶段，本节依次展开。
...
本节从 Rapier：纯 Rust 物理引擎 与 物理与渲染的同步 两个层面剖析「物理引擎集成」。
...
本节围绕「音频系统」展开，覆盖音频管线 与  Rust 音频生态 两个方面。
...
「资源管理」部分包含异步资源加载 与 热重载与版本控制 两条主线，本节依次说明。
...
- [网络协议](../04_web_and_networking/38_network_protocols.md) — QUIC、UDP、序列化
- [嵌入式系统](../05_systems_and_embedded/22_embedded_systems.md) — `#![no_std]`、资源受限
