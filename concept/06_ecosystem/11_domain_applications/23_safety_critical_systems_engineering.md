...
- [安全关键系统工程](#安全关键系统工程)
  ...
  - [八、权威来源索引](#八权威来源索引)
  - [反例与边界](#反例与边界)
    - [反例：Safe Rust 自动满足安全关键认证](#反例safe-rust-自动满足安全关键认证)
    - [反例：形式方法保证零缺陷](#反例形式方法保证零缺陷)
    - [反例：仅采用 MISRA 式子集即可达标](#反例仅采用-misra-式子集即可达标)
...
本节通过反例说明安全关键领域中三种常见但危险的简化观点。

### 反例：Safe Rust 自动满足安全关键认证

| 说法 | 为什么错 |
|:---|:---|
| “Rust 没有内存安全 bug，所以可以直接用于 ASIL D。” | 功能安全标准关注的是过程、证据与风险降低，语言特性只是输入之一。 |
| “通过 `cargo test` 就够了。” | 高安全完整性等级要求 MC/DC 覆盖、需求追溯、工具链鉴定等。 |

满足 ASIL D / DAL A 还需要：完整的需求追溯矩阵、充分的测试覆盖率、经鉴定的工具链、安全分析（FMEA、FTA）以及形式方法或等效强度的验证证据。

### 反例：形式方法保证零缺陷

| 误解 | 现实 |
|:---|:---|
| “用 Kani/Verus 证明过就没有 bug。” | 证明只针对所撰写的规约；规约本身可能遗漏需求或错误地刻画环境。 |
| “形式工具覆盖了整个系统。” | Kani 受状态空间限制，Prusti 对复杂生命周期/unsafe 支持有限，Verus 需要大量人工规约与证明提示。 |

形式方法能证明某类错误不存在，但不能证明“所有可能的错误都不存在”。

### 反例：仅采用 MISRA 式子集即可达标

语言子集（如禁用 `unsafe`、限制 `panic`）只能降低某些缺陷类别的风险：

- 它不能替代需求工程与追溯；
- 不能替代架构安全分析（FMEA、FTA）；
- 不能替代充分的测试与工具链鉴定。

子集是必要但不充分的条件；标准合规是一个覆盖“人、流程、工具、证据”的完整安全案例（safety case）。

```rust,compile_fail
#![deny(unsafe_code)]

fn main() {
    // 错误：在强制拒绝 unsafe 的 crate 中无法使用 unsafe 块
    unsafe {
        let _p = std::ptr::null::<i32>();
    }
}
```
