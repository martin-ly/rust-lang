...
> **Summary**: Systematic coverage of Cargo changes stabilized in Rust 1.97: `build.warnings` configuration, `resolver.lockfile-path`, the `-m` short flag for `--manifest-path`, `cargo clean` target-directory validation, and removal of the `curl` dependency from `crates-io`. All examples verified against local Cargo 1.97.1.
...
- [Cargo 1.97 新特性与工具链变更](#cargo-197-新特性与工具链变更)
  ...
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：常量求值溢出（rustc 1.97.1，--edition 2024 实测）](#反例常量求值溢出rustc-1971--edition-2024-实测)
...
实测（cargo 1.97.1，含 `unused_variables` 警告的 lib crate）：
...
对一个手工创建、只含 `file.txt` 的目录执行清理（cargo 1.97.1 实测）：
...
### 反例：常量求值溢出（rustc 1.97.1，--edition 2024 实测）
