...
> **内容分级**: [参考级]
> **本节关键术语**: cargo build · cargo test · cargo check · cargo run · cargo add · cargo publish — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
> **前置概念**: [Cargo Getting Started](15_cargo_getting_started.md) · [Cargo Workflow](16_cargo_workflow.md) · [Cargo Configuration](18_cargo_configuration.md)
> **后置概念**: [Cargo Manifest Targets](20_cargo_manifest_targets.md) · [Cargo Registry Internals](21_cargo_registry_internals.md) · [DevOps and CI/CD](../00_toolchain/03_devops_and_ci_cd.md)
...
| 检查安全漏洞 | `cargo audit` |
| 单文件脚本执行 | `cargo run --manifest-path script.rs`（见 [Cargo Script](01_cargo_script.md)） |
...
详见 [Cargo 1.96 新特性与工具链变更](04_cargo_196_features.md)。

## 十、与插件的集成

第三方工具通常以 `cargo-<name>` 可执行文件形式出现，例如 `cargo-expand`、`cargo-watch`、`cargo-audit`。详情见 [Cargo 子命令与插件生态](12_cargo_subcommands_and_plugins.md)。
...
在命令封装函数里返回局部临时值的引用（Reference），是悬垂引用陷阱的典型形态。
