...
依赖管理的三条实践主线：添加依赖优先 `cargo add`（自动解析最新兼容版本并写入清单，优于手抄版本号）；版本语义遵循 SemVer 期望——`"1.2"` 等价 `^1.2`（兼容升级），0.x 版本的次版本号视为破坏性（`0.3` ≠ `0.4` 兼容）；依赖分类按用途入段：运行时进 `[dependencies]`、测试进 `[dev-dependencies]`、构建期进 `[build-dependencies]`，错放会导致产物体积膨胀或 feature 泄漏。
