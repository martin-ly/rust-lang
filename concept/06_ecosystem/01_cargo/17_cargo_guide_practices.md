...
> **前置概念**: [Cargo Workflow](16_cargo_workflow.md) · [Cargo Dependency Resolution](06_cargo_dependency_resolution.md) · [Testing Strategies](../09_testing_and_quality/01_testing_strategies.md)
> **后置概念**: [Cargo Configuration](18_cargo_configuration.md) · [DevOps and CI/CD](../00_toolchain/03_devops_and_ci_cd.md) · [Performance Optimization](../10_performance/01_performance_optimization.md)
...
依赖管理的三条实践主线：添加依赖优先 `cargo add`（自动解析最新兼容版本并写入清单，优于手抄版本号）；版本语义遵循 SemVer 期望——`"1.2"` 等价 `^1.2`（兼容升级），0.x 版本的次版本号视为破坏性（`0.3` ≠ `0.4` 兼容）；依赖分类按用途入段：运行时（Runtime）进 `[dependencies]`、测试进 `[dev-dependencies]`、构建期进 `[build-dependencies]`，错放会导致产物体积膨胀或 feature 泄漏。
...
| 减少依赖 | 审计并移除不必要依赖 |
| Profile 调优 | 见 [Cargo Profiles and Lints](11_cargo_profiles_and_lints.md) |
...
详细解释见 [Cargo `public = true` 与 Resolver v3](02_public_private_deps.md)，可运行示例见 [Resolver v3 与 `public = true` 的 feature unification 演示](03_resolver_v3_public_feature_unification.md) 与 [`crates/c17_resolver_v3_public_demo`](../../../crates/c17_resolver_v3_public_demo/)。
...
详见 [Cargo Security CVEs](13_cargo_security_cves.md)。
...
实践指南中遍历/转换集合时，试图从借用（Borrowing）的容器里 move 出元素会被拒。
