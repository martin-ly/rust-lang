> **内容分级**: [综述级]
> [综述级]
> **本节关键术语**: Dependency Resolution · SemVer · Lockfile · Features Unification · Resolver Version · `rust-version` · `cargo tree` · Yanked — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# Cargo 依赖解析
...
> **后置概念**: [Cargo Build Scripts](05_cargo_build_scripts.md) · [Cargo Registries and Publishing](08_cargo_registries_and_publishing.md)
...
> **关于 `minimum` 策略**：当前稳定版 Cargo（1.84+）未提供与 RFC 草案中 `minimum` 对应的独立环境变量。如需测试最低依赖版本，可使用每日构建版的 `cargo update -Zminimal-versions`，或手动把依赖版本下界设高后执行 `cargo update --precise`。
...
> **权威来源**: [Cargo Book](https://doc.rust-lang.org/cargo/index.html), [SemVer](https://semver.org/lang/zh-CN/), [Rust Reference](https://doc.rust-lang.org/reference/introduction.html)
> **权威来源对齐变更日志**: 2026-06-23 更新，对齐 Rust 1.84.0+ / RFC 3537 / Cargo resolver v3 & MSRV-aware resolver；2026-06-26 新增第九章可运行实践 `examples/resolver_v3_practice/` [P2 Deep Content Sprint](../../00_meta/02_sources/05_international_authority_index.md)
...
本节以 feature 关闭时引用门控项为反例，展示 feature 统一后的典型构建失败与 cfg 修正。

### 反例：feature 关闭时引用 feature-gated 项（rustc 1.97.0 实测）

依赖解析/feature 统一后常见故障：调用方假定 feature 已启用：

```rust,compile_fail,E0425
pub struct Parser;

#[cfg(feature = "json")]
pub fn parse_json(p: &Parser) { let _ = p; }

fn main() {
    let p = Parser;
    parse_json(&p); // ❌ 未启用 feature = "json" 时该项不存在
}
```

**错误**：`E0425 cannot find function parse_json in this scope`（`cargo build` 不加 `--features json` 时）。
