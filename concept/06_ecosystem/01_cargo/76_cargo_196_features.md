...
理解「`pubtime`：crate 版本发布时间字段」需要把握背景、数据来源、典型应用与当前限制，本节依次展开。
...
本节从变更内容、使用示例、为什么重要与与 Build Dir Layout v2 的关系切入，剖析「`CARGO_BIN_EXE_<crate>` 运行时可用」的核心内容。
...
理解「TOML v1.1 支持」需要把握变更内容、主要语法改进、Cargo.toml 示例与发布兼容性注意，本节依次展开。
...
「嵌套子命令 manpage」部分包含变更内容 与 收益 两条主线，本节依次说明。
...
本节从推荐迁移路径 与 常见陷阱 两个层面剖析「迁移建议与陷阱」。
