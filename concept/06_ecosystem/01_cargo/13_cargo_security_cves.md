> **内容分级**: [综述级]
>
> **本节关键术语**: Cargo CVE · 第三方 Registry · URL 规范化 · Tarball Symlink · Source Replacement · `cargo audit` — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
> **前置概念**: [Toolchain](../00_toolchain/01_toolchain.md) · [Public/Private Dependencies](02_public_private_deps.md) · [Security Practices](../07_security_and_cryptography/01_security_practices.md) · [Rust vs C++](../../05_comparative/01_systems_languages/01_rust_vs_cpp.md)
> **后置概念**: [Cross Compilation](../05_systems_and_embedded/02_cross_compilation.md) · [DevOps and CI/CD](../00_toolchain/03_devops_and_ci_cd.md)
...
本节梳理 Cargo 近期的两个供应链安全 CVE 及修复。CVE-2026-5222：registry URL 规范化缺陷使凭证可能发送到非预期主机（token 泄露风险），影响使用第三方 registry 且配置凭证的用户；CVE-2026-5223：`cargo install`/发布 tarball 解包时未过滤符号链接，恶意包可越界写入解压目录外。两者均不影响 crates.io 默认配置用户，修复随 Rust 1.96.0 发布——启示是升级工具链本身也是供应链安全的一部分。复行为。
...
“只用 crates.io 官方源就免疫供应链攻击”是不完整的安全观。反命题树指出：typosquatting（相似名称抢注）、账号接管发布恶意版本、合法维护者变更都发生在官方源内；lockfile 锁定只防版本漂移不防首次引入。边界极限给出可执行的防线组合：`cargo vet` 维护审计基线、`cargo audit` 追踪已知漏洞、私有 registry 或 vendor 固化关键依赖——没有任何单一措施充分，纵深防御才是正确答案。
...
- [Public/Private Dependencies](02_public_private_deps.md) — 依赖可见性与 registry 选择
- [Security Practices](../07_security_and_cryptography/01_security_practices.md) — Rust 安全开发生命周期（Lifetimes）
- [DevOps and CI/CD](../00_toolchain/03_devops_and_ci_cd.md) — CI 安全扫描实践
- [Cross Compilation](../05_systems_and_embedded/02_cross_compilation.md) — 在隔离环境中构建与审计
...
> - [MVP 学习路径](../../00_meta/04_navigation/08_learning_mvp_path.md)
>
...
> **过渡**: 理解 CVE 修复后，应结合 [DevOps and CI/CD](../00_toolchain/03_devops_and_ci_cd.md) 把 `cargo audit`、`cargo vendor`、registry 白名单纳入流水线。
