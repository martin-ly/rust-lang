> **内容分级**: [综述级]
>
> **本节关键术语**: Cargo CVE · 第三方 Registry · URL 规范化 · Tarball Symlink · Source Replacement · `cargo audit` — [完整对照表](../../00_meta/01_terminology/terminology_glossary.md)
>
...
> **前置概念**: [Toolchain](../00_toolchain/01_toolchain.md) · [Public/Private Dependencies](10_public_private_deps.md) · [Security Practices](../07_security_and_cryptography/19_security_practices.md) · [Rust vs C++](../../05_comparative/01_systems_languages/01_rust_vs_cpp.md)
> **后置概念**: [Cross Compilation](../05_systems_and_embedded/17_cross_compilation.md) · [DevOps and CI/CD](../00_toolchain/28_devops_and_ci_cd.md)
...
本节围绕「核心概念」展开，依次讨论影响范围：仅第三方 registry、CVE-2026-5222：URL 规范化导致的凭证泄露、CVE-2026-5223：tarball 符号链接越界提取与Rust 1.96.0 的修复行为。
...
- [Security Practices](../07_security_and_cryptography/19_security_practices.md) — Rust 安全开发生命周期（Lifetimes）
- [DevOps and CI/CD](../00_toolchain/28_devops_and_ci_cd.md) — CI 安全扫描实践
- [Cross Compilation](../05_systems_and_embedded/17_cross_compilation.md) — 在隔离环境中构建与审计
...
>
> ✅ 正确做法：升级到 Rust 1.97.0+，让 Cargo 只对 git 协议 registry 做 `.git` 规范化；对 sparse registry 应把它们视为不同端点。

---

## 嵌入式测验（Embedded Quiz）

本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：这两个 CVE 是否影响只使用 crates.io 的开发者…、测验 2：CVE-2026-5222 的核心问题是什么？（理解层）、测验 3：CVE-2026-5223 能造成什么后果？（理解层）、测验 4：Rust 1.96.0 对 CVE-2026-5223 的修…等5个方面。
...
不会。官方公告明确指出 crates.io 用户不受这两个 CVE 影响：crates.io 禁止符号链接（免疫 CVE-2026-5223），其架构也不满足 CVE-2026-5222 的攻击前提。但升级 Rust 1.97.0+ 仍是推荐做法。
</details>
...
| sparse registry URL 被不规范归一化 ⟹ CVE-2026-5222 风险 | `.git` 后缀被错误剥离 | 升级到 1.97.0+，并隔离不同 registry 凭证 | 高 |
...
> **过渡**: 理解 CVE 修复后，应结合 [DevOps and CI/CD](../00_toolchain/28_devops_and_ci_cd.md) 把 `cargo audit`、`cargo vendor`、registry 白名单纳入流水线。
