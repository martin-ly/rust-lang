...
- [核心 Crate 选型](../02_core_crates/01_core_crates.md) — 脚本中常用依赖的选择策略
...
Cargo Script（`cargo script`，1.89+ 稳定的单文件脚本）边界测试覆盖四类失效：frontmatter 中依赖声明解析失败（TOML 语法或特性名错误）；单文件不能有外部 `mod` 文件（模块系统限制在单文件内）；脚本缓存的依赖锁定导致版本漂移（缓存清单与最新版不一致）；多脚本共享缓存时的版本冲突。这些用例标定了 cargo script 的适用边界——原型与工具脚本适用，复杂项目仍需完整 crate。/编译…等6个方面，本节逐一说明其要点。
...
> **认知功能**: 本 mindmap 从本页「Cargo Script 脚本化 Rust」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
