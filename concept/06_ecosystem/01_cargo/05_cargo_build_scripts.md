> **内容分级**: [综述级]
> [综述级]
> **本节关键术语**: Build Script · `build.rs` · `OUT_DIR` · `rerun-if-changed` · `links` · Native Dependency · `build-dependencies` — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
> **后置概念**: [Cargo Registries and Publishing](08_cargo_registries_and_publishing.md) · [DevOps and CI/CD](../00_toolchain/03_devops_and_ci_cd.md)
...
build.rs 的纪律可归纳为三条：生成代码必须写入 `OUT_DIR`（污染源目录会破坏可重复构建与 crates.io 发布）；平台探测用 `CARGO_CFG_*` 环境变量而非宿主信息（交叉编译时两者不同）；变更追踪靠 `cargo:rerun-if-changed` 显式声明（默认 rerun-if-changed 整个目录会导致过度重建）。违反任一条都会导致 CI 缓存失效或交叉编译翻车。
...
build.rs 的能力边界常被高估：它不适合需要修改源码目录的任务（应改用外部脚本或 proc macro），不应访问网络下载（破坏离线构建与可重复性），也无法保证执行顺序（多 crate 的 build.rs 并行运行）。反命题树按这两个问题分流；边界极限补充量化约束——build.rs 在每次 `cargo check` 也会运行，重逻辑必须配 rerun-if 声明，否则增量编译收益被吃掉。
...
本节从测验 1：`build.rs` 运行在目标平台还是构建主机？、测验 2：如何让 Cargo 在 `src/config.json`…、测验 3：`links = "mylib"` 的作用是什么？与测验 4：生成的代码应该放在哪个目录？切入，剖析「嵌入式测验」的核心内容。
...
> **权威来源**: [The Cargo Book](https://doc.rust-lang.org/cargo/index.html), [The Rust Reference](https://doc.rust-lang.org/reference/introduction.html), [Rust Standard Library](https://doc.rust-lang.org/std/index.html)
