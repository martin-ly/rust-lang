...
> **前置依赖**: [类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) · [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) · 形式化验证 · [生命周期（Lifetimes）](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md)
> **后置延伸**: [编译器内部原理](../00_toolchain/04_compiler_internals.md) · [安全与密码学](../07_security_and_cryptography/02_security_cryptography.md) · [嵌入式系统](../05_systems_and_embedded/03_embedded_systems.md)
>
...
- [形式化验证工具生态](#形式化验证工具生态)
  ...
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：const 上下文中的断言失败（rustc 1.97.0 实测）](#反例const-上下文中的断言失败rustc-1970-实测)
    - [✅ 修正：运行时校验](#-修正运行时校验)
...
> - v1.1 (2026-05-26): 补充 Generic Refinement Types (POPL 2025) — Flux 泛型（Generics）精化类型扩展 [Web Authority AlignmentSprint](../../00_meta/02_sources/05_international_authority_index.md)
...
形式化验证（Formal Verification）用数学方法证明「程序满足规约」，本页按「验证什么 × 自动到什么程度」组织工具图谱：

- **形式化验证的层次模型**：**L0 动态检测**（运行/解释执行中捕获违规——Miri、sanitizers，证伪不证全）；**L1 模型检验**（有界/状态空间枚举（Enum）证明性质——Kani，自动但受状态爆炸约束）；**L2 演绎验证**（把程序语义翻译为逻辑公式，由 SMT 求解器证明——Prusti/Creusot/Verus，需人工写规约与不变量）；**L3 交互式证明**（证明助手内人工构造证明——RefinedRust/Iris/hax 到 Rocq/F*，表达力最强、成本最高）。层次向右，自动化程度下降、可证性质上限上升。
- **Rust 形式化验证的独特挑战**：① **unsafe 与安全边界**——验证必须建模「安全抽象内部的 unsafe 块」的合法性（这是 RustBelt 的核心问题）；② **弱内存序**——并发 unsafe 代码需要内存模型感知的逻辑（Iris 的并发分离逻辑）；③ **trait 与单态化（Monomorphization）**——验证目标是 MIR 层（单态化后），规约却写在源码层，两层的映射保真性是工具正确性前提；④ **位精确语义**——溢出、布局、`usize` 宽度依赖目标平台，验证需参数化目标。

判定一个验证需求落在哪层：性质是「这条路径会崩吗」→ L0/L1；是「所有输入满足后置条件吗」→ L2；是「这个并发协议无数据竞争吗」→ L3。
...
模型检验层的两个主力工具，分别覆盖「有界证明」与「动态解释」：

- **Kani：基于 CBMC 的 Rust 验证器**：把 Rust 编译到 MIR 后翻译为 CBMC（C Bounded Model Checker）的 Goto 程序，对「`#[kani::proof]` harness 函数」做**有界模型检验**——展开循环/递归到指定深度，把「`kani::any::<T>()` 的非确定性输入 × 全部路径」编码为 SAT/SMT 公式求解。能证明：无 panic（越界/溢出/unwrap）、用户断言（`kani::assert`）、无 UB（有限范围）。局限：循环深度上界（`#[kani::unwind(N)]`）、堆/闭包（Closures）/复杂 trait 的支持在演进中、验证时间随路径数指数增长。工程定位：「关键算法/unsafe 模块（Module）的单元级证明」——比测试强在「穷尽有界输入」，比演绎验证弱在「无界性质不可证」。
- **MIRI：运行时 UB 检测器**：MIR 解释器，逐条执行时维护「借用（Borrowing）栈/树（Stacked/Tree Borrows）、初始化位图、分配表、弱内存序交错」——**可捕获**：越界/未对齐/悬垂访问、未初始化读、别名规则违规、（多次播种下的）数据竞争。**不能捕获**：未执行路径（覆盖率盲区）、真实外部世界（FFI 副作用）、所有线程交错（非穷尽——配 `loom` 互补）。工程定位：unsafe 代码的「最低验收线」——`cargo miri test` 全绿应成为任何含 unsafe crate 的 CI 标配。

两工具的分工一句话：Kani 问「所有有界输入下性质成立吗」（证明），Miri 问「这次执行有 UB 吗」（检测）——前者给信心上界，后者给缺陷下界。
...
演绎验证层把「函数应满足什么」写为规约（pre/postcondition + 循环不变量），由 SMT 求解器自动证明，三个主流工具：

- **Prusti：Viper 分离逻辑验证器**：注解语法（`#[requires]`/`#[ensures]`/`#[invariant]`），翻译 Rust（含 borrow 信息——Prusti 的独特贡献是把「借用」编码为 Viper 的权限谓词，&mut 的独占性自动成为证明前提）到 Viper 中间验证语言。优势：借用推理自动化程度高；限制：不支持 trait 动态分派、部分 unsafe 模式、异步（Async）。
- **Creusot：Why3/WhyML 验证器**：`pearlite!` 规约语言（纯函数子集），翻译到 Why3——可调用 Z3/CVC/Alt-Ergo 多求解器后端，失败时可升级到 Why3 的交互式证明。特色：`Snapshot`（`^x` 终态引用（Reference））与「逻辑函数」机制使规约表达力强；定位偏「算法正确性证明」（排序、数据结构）。
- **Verus：SMT-LIB 验证器**：「验证感知的 Rust 子集」——`spec fn`/`proof fn`/`exec fn` 三类函数区分「规约/证明/执行」代码，线性类型+权限模型内建，证明直接编译为 SMT-LIB。特色：对「系统代码」（并发、unsafe 容器）支持最好（Verus 已验证过页表、并发队列等），学习曲线最陡（需理解其权限/模式系统）。

选型判定：普通 safe Rust 业务算法 → Prusti/Creusot（注解轻）；unsafe/并发系统代码 → Verus；需要「证明失败时交互式推进」→ Creusot（Why3 后端）。三者的共同工作流：写规约 → 自动求解 → 失败则「加中间断言/不变量」引导求解器——「引导」是演绎验证的主要人工活动。
...
类型系统扩展类工具把验证能力推进到 Rust 类型系统之外，两个代表走相反的技术路线：

- **Flux（精化类型）**: 在 Rust 类型上附加逻辑谓词（如 `i32{v: v > 0}` 表示正整数），让“索引在界内”“除数非零”成为类型检查的静态结论；注解以 `#[flux::spec]` 写在现有代码旁，验证失败给出类似借用检查的错误信息——学习曲线在验证工具中最平缓。
- **Aeneas（向函数式语言转换）**: 把 Rust 的 LLBC IR 翻译为 Lean/F* 等证明助手中的纯函数模型（借用与可变性被建模为状态变换），之后在证明助手中用交互式证明验证任意性质；能力强但要求用户掌握目标证明助手，适合安全攸关的小内核（如加密原语）。

判定依据：需要“轻量注解 + 自动化”选 Flux；需要“任意性质的机器检查证明”且团队有证明助手经验选 Aeneas。
...
工具选型应按“验证目标 × 可投入成本”两维决策，能力矩阵与决策树是同一信息的两种呈现：

| 工具 | 技术 | 覆盖性质 | 自动化程度 |
|---|---|---|---|
| Miri | 解释执行 | UB 检测（运行到的路径） | 全自动 |
| Kani | 有界模型检查 | 断言/不变量（有界输入） | 全自动 |
| Prusti | 演绎验证（Viper） | 函数契约（前后条件） | 自动（需注解） |
| Creusot | 翻译到 Why3 | 完整函数正确性 | 半自动 |
| Verus | SMT + 线性类型 | 系统级验证（含并发） | 半自动 |

决策树要点：unsafe 代码先 Miri；关键断言有界验证用 Kani；需要函数级契约文档 + 自动验证用 Prusti；操作系统/数据库级完整验证用 Verus/Creusot。判定依据：工具能力上限（能证什么）与团队能力上限（会用什么）取交集，从自动化程度最高的工具起步。
...
形式化验证前沿的四个方向代表不同的验证哲学与成熟度：

- **RefinedRust（Iris 分离逻辑）**: 把 Rust 程序翻译到 Rocq/Iris 框架，用分离逻辑推理堆与并发——目标是 RustBelt 风格的“类型系统健全性 + 程序验证”统一，学术前沿定位。
- **RustBelt 验证框架**: 证明了 Rust 核心库（含 unsafe 实现）在 λ_Rust 模型中的健全性，是“Rust 为何安全”的理论基石；其方法论（逻辑关系 + 分离逻辑）影响后续所有工具。
- **hax**: 把 Rust 子集翻译到 F* 与 Rocq，已被用于验证生产级密码学实现（如 libcrux）——工业可用度最高的前沿工具链。
- **Kani verify-std 计划**: 对标准库的 unsafe 代码系统性地写验证 harness，目标是把“std 可信”从社会共识升级为机器检查——参与门槛低（写 harness 类似写测试），是社区贡献的友好入口。

判定依据：跟踪学术突破看 RefinedRust/RustBelt；近期工程应用看 hax 与 verify-std。
...
形式化验证在 Rust 社区有三个需要校准的流行命题，本节分别给出反命题树与根结论：

1. **“形式化验证可以消除所有 bug”**：验证器只检查“程序是否符合规范”，规范本身可能写错；Kani 有界、MIRI 只覆盖执行到的路径，且验证器自身（CBMC/Viper）属于可信计算基。它显著减少特定类别 bug，但不能替代测试与评审。
2. **“Unsafe Rust 无法被验证”**：恰恰相反——RustBelt 用分离逻辑验证了标准库的 `Box`/`Vec`/`Rc`，Kani 可直接符号执行 unsafe 代码，Verus 用 ghost 状态追踪资源；代价是需要更多手工规范，尤其是内存模型相关部分。
3. **“验证工具已成熟到可日常生产使用”**：目前只有 Kani 和 MIRI 达到日常可用；演绎验证工具（Prusti/Creusot/Verus/Flux）适合安全关键模块的定向验证，全项目使用会遭遇 10–100 倍编译开销与 API 不稳定。

选型含义：按模块安全等级分层投入验证强度，而非追求全覆盖。
...
形式化验证的边界测试揭示「工具的盲点形态」——知道工具不能做什么与知道能做什么同等重要：

- **Kani 数组越界未被 harness 覆盖**（验证盲区）：`#[kani::proof]` 只验证「从该 harness 可达且展开深度内」的路径——未被任何 proof 函数调用的代码、循环超过 unwind 深度后的迭代、以及「`kani::any()` 未覆盖的输入类型」都在证明范围之外。盲区判据：proof 覆盖率报告（`kani --coverage`）中「未覆盖行」与「unwind 截断警告」——证明结论只对「被覆盖且未截断」的部分成立。
- **MIRI 无法检测未执行的 unsafe 路径**（运行时盲区）：Miri 是解释执行——`if cfg!(debug_assertions)` 之外的条件 unsafe 块、需要特定输入才进入的分支、以及「需要 FFI 真实行为触发」的路径都不会被检查。盲区判据：miri 运行日志的「未执行 unsafe 块」统计（需测试套件本身有覆盖率——Miri 不替代测试设计）。
- **Prusti 前置条件过强导致合法调用被拒绝**（假阴性）：演绎验证是「保守证明」——`#[requires(x > 0)]` 写成 `x > 1` 会使「x == 1 的合法调用」验证失败（假阳性拒报的反镜像）；反之规约过弱则「证不出真性质」。盲区判据：「验证通过但调用点报错」 vs 「调用点合法但证不出」——规约与实现的「契约张力」需要像调试代码一样调试规约。

三类盲区的统一教训：**任何验证结论都带「适用范围谓词」**——Kani 的谓词是「有界 + 覆盖」，Miri 的是「执行到」，演绎的是「规约如实」。生产采信验证结论时，适用范围谓词必须随结论一起记录。
...
- [安全与密码学](../07_security_and_cryptography/02_security_cryptography.md) — 侧信道防护、常量时间操作
- [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) — Miri、UB、别名模型
- [形式化验证](../../04_formal/04_model_checking/01_verification_toolchain.md) — 定理证明器、SMT、分离逻辑
- [类型系统](../../01_foundation/02_type_system/01_type_system.md) — 类型论、泛型（Generics）、Trait
...
- [嵌入式系统](../05_systems_and_embedded/03_embedded_systems.md) — `#![no_std]`、资源受限验证
...
本节以 const 求值 panic 为反例，展示形式化工具之外编译器自身的证明能力边界。

### 反例：const 上下文中的断言失败（rustc 1.97.0 实测）

```rust,compile_fail,E0080
const fn check(x: u32) -> u32 {
    assert!(x > 0);
    x
}
const BAD: u32 = check(0); // ❌ 编译期求值即 panic
fn main() {
    println!("{}", BAD);
}
```

**错误**：`E0080 evaluation panicked: assertion failed: x > 0`——const 求值是编译器内置的最强静态检查点。
