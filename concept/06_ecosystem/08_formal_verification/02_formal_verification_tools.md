...
- **形式化验证的层次模型**：**L0 动态检测**（运行/解释执行中捕获违规——Miri、sanitizers，证伪不证全）；**L1 模型检验**（有界/状态空间枚举证明性质——Kani，自动但受状态爆炸约束）；**L2 演绎验证**（把程序语义翻译为逻辑公式，由 SMT 求解器证明——Prusti/Creusot/Verus，需人工写规约与不变量）；**L3 交互式证明**（证明助手内人工构造证明——RefinedRust/Iris/hax 到 Rocq/F*，表达力最强、成本最高）。层次向右，自动化程度下降、可证性质上限上升。
- **Rust 形式化验证的独特挑战**：① **unsafe 与安全边界**——验证必须建模「安全抽象内部的 unsafe 块」的合法性（这是 RustBelt 的核心问题）；② **弱内存序**——并发 unsafe 代码需要内存模型感知的逻辑（Iris 的并发分离逻辑）；③ **trait 与单态化**——验证目标是 MIR 层（单态化后），规约却写在源码层，两层的映射保真性是工具正确性前提；④ **位精确语义**——溢出、布局、`usize` 宽度依赖目标平台，验证需参数化目标。
...
- **Kani：基于 CBMC 的 Rust 验证器**：把 Rust 编译到 MIR 后翻译为 CBMC（C Bounded Model Checker）的 Goto 程序，对「`#[kani::proof]` harness 函数」做**有界模型检验**——展开循环/递归到指定深度，把「`kani::any::<T>()` 的非确定性输入 × 全部路径」编码为 SAT/SMT 公式求解。能证明：无 panic（越界/溢出/unwrap）、用户断言（`kani::assert`）、无 UB（有限范围）。局限：循环深度上界（`#[kani::unwind(N)]`）、堆/闭包/复杂 trait 的支持在演进中、验证时间随路径数指数增长。工程定位：「关键算法/unsafe 模块的单元级证明」——比测试强在「穷尽有界输入」，比演绎验证弱在「无界性质不可证」。
- **MIRI：运行时 UB 检测器**：MIR 解释器，逐条执行时维护「借用栈/树（Stacked/Tree Borrows）、初始化位图、分配表、弱内存序交错」——**可捕获**：越界/未对齐/悬垂访问、未初始化读、别名规则违规、（多次播种下的）数据竞争。**不能捕获**：未执行路径（覆盖率盲区）、真实外部世界（FFI 副作用）、所有线程交错（非穷尽——配 `loom` 互补）。工程定位：unsafe 代码的「最低验收线」——`cargo miri test` 全绿应成为任何含 unsafe crate 的 CI 标配。
...
- **Prusti：Viper 分离逻辑验证器**：注解语法（`#[requires]`/`#[ensures]`/`#[invariant]`），翻译 Rust（含 borrow 信息——Prusti 的独特贡献是把「借用」编码为 Viper 的权限谓词，&mut 的独占性自动成为证明前提）到 Viper 中间验证语言。优势：借用推理自动化程度高；限制：不支持 trait 动态分派、部分 unsafe 模式、异步。
- **Creusot：Why3/WhyML 验证器**：`pearlite!` 规约语言（纯函数子集），翻译到 Why3——可调用 Z3/CVC/Alt-Ergo 多求解器后端，失败时可升级到 Why3 的交互式证明。特色：`Snapshot`（`^x` 终态引用）与「逻辑函数」机制使规约表达力强；定位偏「算法正确性证明」（排序、数据结构）。
