...
> **后置概念**: [Application Domains](../06_data_and_distributed/01_application_domains.md) · [Toolchain](../00_toolchain/01_toolchain.md)
...
> **认知功能**: 此图是 Rust 生态的**形式化成熟度分层塔**。读者可按项目可靠性需求「对号入座」——需要基本内存安全（Memory Safety）选 L0（ rustc 自动保证），需要类型契约验证选 L1（Serde/SQLx），需要架构组合正确性选 L2（Tokio/Tower），需要可观测性选 L3（Tracing），需要功能正确性证明选 L4（Kani/Verus）。关键认知：形式化不是「全有或全无」的二元选择，而是**可逐层递增的投资**——从 L0 到 L4，每上一层都增加验证深度和开发成本，读者应根据项目安全关键性选择适当的层级组合。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **MSRV 策略**：Rust 生态的 MSRV 演进速度约为**每 6-9 个月提升一个 minor 版本**。团队应在 `Cargo.toml` 中显式声明 `rust-version = "1.70"`，并利用 `cargo check --minimum-version` 验证兼容性。 [来源: [Cargo Book](https://doc.rust-lang.org/cargo/index.html)]
...
- [L6: Core Crates（核心库谱系）](../02_core_crates/01_core_crates.md) —— 传统功能域分类视角
- [L6: Application Domains](../06_data_and_distributed/01_application_domains.md) —— 工程落地场景
...
- [L6: Toolchain](../00_toolchain/01_toolchain.md) —— Cargo、审计与供应链安全
- [L3: Async](../../03_advanced/01_async/01_async.md) —— Tokio/Tower 的 async 根基
- [L3: Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) —— Firecracker/Wasmtime 的 unsafe 边界
- [L4: Type Theory](../../04_formal/00_type_theory/01_type_theory.md) —— 范畴论与类型论根基
- [L7: Formal Methods](../../07_future/04_research_and_experimental/02_formal_methods.md) —— Kani/Verus/Creusot 的工业化路径
...
形式化工具的边界测试揭示验证本身的失效模式：Prusti 前置条件过弱导致验证失败（规约不充分而非代码错误，区分这两者是使用门槛）；Kani 基于有界模型检查，循环展开超过 `unwind` 上界时结论只对有限深度成立（需 unwinding assertion 确认）；工具对 trait 动态分发、async 状态机的支持缺口使部分代码不可验证。三类用例提醒：验证工具的输出是“在假设与界限内证明”，读懂界限与读懂证明同样重要。式化工具链的生态系统碎片化（编译错误）、边界测试：依赖树中的形式化安全与 unsafe 代码传播（编译错误）等6个方面，本节依次展开。
...
### 10.3 边界测试：形式化工具链的每日构建版依赖与稳定化鸿沟（编译错误）

```rust,compile_fail
// 需启用实验特性门 proc_macro_hygiene（每日构建版工具链）

// ❌ 编译错误: 若使用 stable rustc，每日构建版特性门不可用
...
本节以 unsized trait object 局部变量为反例，展示类型系统对 `dyn` 间接层的强制要求。

### 反例：把 `dyn Trait` 当作 sized 局部变量（rustc 1.97.0 实测）

```rust,compile_fail,E0277
trait Shape { fn area(&self) -> f64; }

fn main() {
    let s: dyn Shape; // ❌ dyn Trait 是 unsized 类型
    let _ = s;
}
```

**错误**：`E0277 the size for values of type dyn Shape cannot be known at compilation time`。

### ✅ 修正：通过指针间接持有 trait object
