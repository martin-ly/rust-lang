...
> **前置概念**: [Rustc Query System](../04_formal/19_rustc_query_system.md) · [Name Resolution and HIR](../04_formal/25_name_resolution_and_hir.md) · [Trait Solver in rustc](../04_formal/26_trait_solver_in_rustc.md)
...
LLVM 是一套模块化的编译器工具链，核心是**可插拔的后端（backend）**。`rustc` 默认使用 LLVM 后端：
...
1. 对 MIR 做**单态化**（monomorphization）：把泛型函数实例化为具体类型版本；
...
Codegen unit 是可以**并行编译**的最小 LLVM module 单元。它们在单态化收集阶段就已经划分好：
...
> **关键洞察**: Codegen unit 是编译时间与运行时性能之间的调优杠杆。
>
