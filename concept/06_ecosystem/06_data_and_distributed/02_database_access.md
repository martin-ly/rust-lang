...
「查询模式」部分按原始 SQL、查询构建器与迁移管理的顺序逐层展开。
...
理解「边界测试：数据库访问的编译错误」需要把握边界测试：SQLx 的编译期查询验证（编译错误）、边界测试：连接池的生命周期管理（编译错误）、边界测试：连接池的 `deadlock` 与异步等待（运行时死锁）、边界测试：连接池耗尽与异步等待超时（运行时超时/崩溃）等6个方面，本节依次展开。
