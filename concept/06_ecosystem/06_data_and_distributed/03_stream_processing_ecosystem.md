...
- **代码示例形态**: `worker.dataflow::<u64, _, _>(|scope| { source.map(...).filter(...).inspect(...) })`——数据流图以闭包（Closures）内链式算子声明，`timely::execute` 驱动多 worker 并行；所有权（Ownership）模型使数据在 worker 间的传递零拷贝且线程安全由类型保证。
...
- **与 Flink 的对比**: Flink 是通用流处理引擎（任意算子、事件时间、精确一次状态），Materialize 是“增量物化视图数据库”——前者灵活但需 Java/运维集群，后者以 SQL 表达力换取零运维的流式一致性（Coherence）。判定依据：用 SQL 能表达的持续查询选 Materialize；需要自定义算子/复杂事件处理选 Flink（或 timely 自研）。
...
流处理系统的失效模式与批处理根本不同：数据无界、到达无序、背压传导。本章覆盖五个代表性边界：

- **Kafka 消费者的 `Deserialize` 约束**：`rdkafka`/`lapin` 要求消息类型实现 `DeserializeOwned`，编译期保证消费者不会“假装”能解析某种格式——类型即契约跨越了进程边界。
- **背压与无界缓冲**：`mpsc::unbounded_channel` 在生产快于消费时内存无界增长直至 OOM；流处理必须用有界通道 + `send().await` 让背压向上游传导。
- **窗口与 watermark**：事件时间窗口若 watermark 配置过紧，延迟数据被静默丢弃，造成结果偏低且无报错——这是最危险的“逻辑正确性”边界。
- 另两项覆盖恰好一次（exactly-once）语义的偏移量提交时机与状态后端恢复。
