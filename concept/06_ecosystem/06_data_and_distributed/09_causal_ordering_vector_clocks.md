...
> **前置概念**: [L4 线性化与一致性（Coherence）谱系](../../04_formal/07_concurrency_semantics/02_linearizability_and_consistency.md) · [L5 五模型定义矩阵](../../05_comparative/00_paradigms/04_five_models_definition_matrix.md) · [L3 并发编程（共享内存 happens-before）](../../03_advanced/00_concurrency/01_concurrency.md)
...
- [因果序与向量时钟：Lamport 偏序的算法化](#因果序与向量时钟lamport-偏序的算法化)
  ...
  - [六、反例与边界](#六反例与边界)
    ...
    - [边界：物理时间的回归——HLC（混合逻辑时钟）](#边界物理时间的回归hlc混合逻辑时钟)
...
本节先给出正确的并发检测（3.1），再用两个反例展示 Lamport 标量与丢失分量的向量时钟为何失效（3.2–3.3）。
...
|:---|:---|:---|
| 事件 | 内存读/写/原子操作（Atomic Operations） | 发送/接收/本地步 |
| 建立方式 | `Release`/`Acquire` 同步、线程 spawn/join | 消息传递、进程内程序序 |
| 违反的代价 | **数据竞争 = 未定义行为** | 因果倒置 = 语义错误（非 UB） |
| 检测工具 | miri / ThreadSanitizer | 向量时钟运行时（Runtime）比较 |
...
本节用反例标定向量时钟的误用（当作全局时钟、N 动态变化时复用定长向量），并讨论 O(N) 空间边界与 HLC 的折中。
...
### 边界：物理时间的回归——HLC（混合逻辑时钟）

工程系统往往需要「近似物理时间 + 因果序」两全（租约、TTL、快照读）。HLC（Hybrid Logical Clock，Kulkarni 等 2014）把时间戳设计为 `(物理分量, 逻辑分量)` 二元组：

```text
HLC 更新规则（节点 j 处理事件，收到时间戳 (l, c) 的消息时）：
  lⱼ := max(lⱼ, l, 本地物理时钟)          -- 物理分量取三者最大
  cⱼ := 若 lⱼ 与旧值相同则 cⱼ + 1 否则 0    -- 逻辑分量只在「钟没走」时递增

性质：HLC 保持 Lamport 时钟条件（a → b ⟹ HLC(a) < HLC(b)，字典序），
     且物理分量与真实时钟的误差有界（受 NTP 同步质量约束）⟹
     可同时充当因果判定依据与近似物理时间（CockroachDB 的核心时钟机制）。
```

代价与边界：HLC 与 Lamport 标量一样是**单向**的（戳小推不出因果），精确并发检测仍需向量时钟；物理分量仍受时钟漂移影响 ⟹ HLC 给的是「有界误差内的因果+时间」折中，不是 Google TrueTime 式的确定性承诺。
...
- Lamport, L. *Time, Clocks, and the Ordering of Events in a Distributed System*. CACM 21(7), 1978, 558–565. [作者主页 PDF](https://lamport.azurewebsites.net/pubs/time-clocks.pdf) · [DOI](https://doi.org/10.1145/359545.359563) · [ACM DL](https://dl.acm.org/doi/10.1145/359545.359563)（ACM 反爬，浏览器可访问）
...
- [vclock — Rust 向量时钟库（docs.rs）](https://docs.rs/vclock/latest/vclock/)
