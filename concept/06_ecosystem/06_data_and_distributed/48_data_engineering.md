...
- [Data Engineering（数据工程）](#data-engineering数据工程)
  ...
  - [九、边界测试](#九边界测试)
    ...
    - [9.3 边界测试：ETL 管道中类型推断（Type Inference）失败导致运行时 panic（类型错误）](#93-边界测试etl-管道中类型推断失败导致运行时-panic类型错误)
  - [相关概念文件](#相关概念文件)
    - [补充定理链](#补充定理链)
...
- [云原生](../04_web_and_networking/24_cloud_native.md) — 容器化、对象存储、微服务部署
- [性能优化](../10_performance/15_performance_optimization.md) — SIMD、缓存优化、内存布局
- [API Design Patterns](../03_design_patterns/42_api_design_patterns.md) — REST/gRPC 数据服务
- [并发编程](../../03_advanced/00_concurrency/01_concurrency.md) — Send/Sync、异步（Async）并行
- [数据库系统](37_database_systems.md) — 存储引擎、事务、索引
- [网络协议](../04_web_and_networking/38_network_protocols.md) — HTTP/2、gRPC、对象存储协议
