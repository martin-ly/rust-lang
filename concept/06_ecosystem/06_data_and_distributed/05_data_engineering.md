...
数据工程管道的失效多在“数据契约”与“资源边界”两处，且常在 TB 级数据上才暴露。本章聚焦三类边界：

1. **Parquet schema 演化**：新增列必须为 nullable，否则旧 reader 直接拒绝整个文件；类型变更不受支持，只能重写。Delta Lake 的事务日志把 schema 变更变为可审计操作，是生产湖仓的标配。
2. **对象存储流式下载**：S3 等对象存储的单文件可达 100GB+，`result.bytes().await` 一次性载入必然 OOM；必须用 `get_stream()` 分块流式处理，让内存占用与并发数而非文件大小成正比。
3. **ETL 类型推断失败**：Arrow/Parquet 读取时 schema 推断与实际数据不符（如整数列混入空值被推断为 float），下游 `unwrap()` 转 panic——生产管道应显式声明 schema 并记录脏数据。

每节给出错误代码、失效规模估算与修正模式。
