...
> **来源**:
> [embedded-graphics](https://docs.rs/embedded-graphics/) · [lvgl-rs](https://docs.rs/lvgl/) ·
...
> [LVGL Porting Guide](https://docs.lvgl.io/8.3/porting/)
...
> [LVGL Overview](https://docs.lvgl.io/8.3/overview/)
...
嵌入式 GUI 领域流传三个需要检验的全称命题，本节分别给出反命题树与边界条件：

1. **“嵌入式 GUI 必须依赖操作系统”**：仅在需要复杂多窗口管理、文件系统与网络栈时 OS 才有明显帮助；资源充足（>512KB Flash）的裸机/RTOS 场景，Rust `no_std` + embedded-graphics 完全胜任。仅在 <64KB Flash 且 UI 复杂时需妥协（C + LVGL）。
2. **“Rust 太慢，无法 60fps”**：帧率瓶颈几乎总在显示接口带宽与 DMA 配置，而非语言；零成本抽象（Zero-Cost Abstraction）保证 Rust 代码与手写 C 同级，关键是用 DMA 搬运帧缓冲、避免每像素函数调用与堆分配。
3. **“所有嵌入式 UI 都长得一样”**：embedded-graphics 提供像素级控制，外观完全自定义；相似感来自高级框架的默认主题，Slint/LVGL 的主题系统可深度定制。

判定依据：先按 Flash/RAM 资源框定可行方案集，再按 UI 复杂度选择抽象层级。
...
> [LVGL Overview — Memory](https://docs.lvgl.io/8.3/overview/)
...
>
> **权威来源对齐变更日志**: 2026-05-26 创建 [Authority Source Sprint Batch 12](../../00_meta/02_sources/05_international_authority_index.md)
...
- [Embedded Systems](03_embedded_systems.md) — 嵌入式系统基础
- [Performance Optimization](../10_performance/01_performance_optimization.md) — 性能优化
- [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) — unsafe Rust 模式
- [Async/Await](../../03_advanced/01_async/01_async.md) — 异步（Async）编程
