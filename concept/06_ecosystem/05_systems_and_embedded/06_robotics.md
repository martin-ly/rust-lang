...
- **“实时性要求意味着不能用高级抽象”** —— 不成立。Rust 的零成本抽象使高层 API（迭代器、泛型状态机）编译后与控制流手写代码等价；真正要规避的是堆分配、锁与动态调度，这与抽象层级无关。
...
- 测验 1：**为什么是 Rust**——实时性（无 GC 停顿）、内存安全（避免控制回路崩溃）与并发安全（传感器/规划/控制并行）三者的交集，DARPA 与 NASA 的项目资助印证了这一趋势。
