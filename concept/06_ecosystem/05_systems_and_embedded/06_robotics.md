...
Rust 的切入点：内存安全（Memory Safety）消除控制软件中最高危的崩溃模式（空指针/越界），所有权（Ownership）模型使实时分析（无隐藏分配）可审计。
...
1. **`rclrs`**：官方维护的 Rust 客户端库，绑定 `rcl` C API。设计特色是把 ROS2 的执行器（executor）模型映射为 Rust 的轮询式回调——消息回调拿 `&mut` 状态，借用（Borrowing）检查保证单回调内无别名冲突，但多线程执行器下的共享状态仍需 `Arc<Mutex>`。
...
> **[CycloneDDS](https://cyclonedds.io/)** 和 **[Fast DDS](https://github.com/eProsima/Fast-DDS)** 是 ROS2 的两个主要 DDS 实现。Rust 生态通过 FFI 绑定或直接协议实现与之交互。[来源: [CycloneDDS](https://cyclonedds.io/)]
...
实时机器人系统的核心是**确定性时延**——控制回路（通常 100Hz–1kHz）的每次迭代必须在截止期限内完成，三个主题从软件栈到内核层层递进：

- **ROS2 执行器模型**: 单线程执行器顺序处理回调，天然避免数据竞争但一个慢回调会阻塞全部；多线程执行器需组（callback group）划分保证互斥/可重入语义。
- **优先级继承与 PREEMPT_RT**: 优先级反转是实时系统经典陷阱——低优先级任务持锁阻塞高优先级任务；内核级方案是 PREEMPT_RT 补丁 + 互斥锁的优先级继承协议，应用层则需避免在控制回路内分配内存与打日志。
- **no_std + RTOS 集成**: 微控制器侧用 `no_std` Rust + RTOS（如 Embassy、RTIC）获得无堆、中断驱动的确定性，与上层 ROS2 节点通过 micro-ROS/DDS-XRCE 桥接。

判定依据：硬实时回路放 MCU/RTOS 侧，ROS2 侧只做软实时调度与感知融合。
...
传感器融合与 SLAM 是机器人软件中计算最密集的部分，Rust 生态按“数值基础 → 框架 → 应用”三层组织：

- **点云处理与线性代数**: `nalgebra` 提供编译期维度检查的几何类型（`Isometry3`、`UnitQuaternion`），姿态表示错误在编译期暴露；点云处理常用 `kiss-icp` 类实现或 PCL 的 Rust 绑定，KD-Tree 加速靠 `kiddo`。
- **OpenRR 框架**: Rust 原生的机器人中间件，提供运动规划（`openrr-planner`）、IK 求解与执行器抽象，定位是 ROS 之外的轻量替代。
- **导航栈**: 定位（AMCL 类粒子滤波）+ 路径规划（A*/RRT）+ 局部避障的三层结构，Rust 实现尚处早期，生产多采用 ROS2 Nav2 + Rust 自定义节点的混合方案。

判定依据：算法内核（ICP、因子图优化）适合 Rust 重写以获得内存安全；整机系统集成短期仍以 ROS2 为主干。
...
机器人领域对 Rust 的两个常见误判需要澄清：

- **“Rust 没有成熟的机器人生态，不适合生产”** —— 部分成立。生态广度确实不及 C++/Python，但关键路径（DDS 绑定、实时内核侧、算法内核）已可用；采用策略应是“Rust 重写安全攸关内核 + 保留 ROS2 集成层”，而非全栈替换。
- **“实时性要求意味着不能用高级抽象”** —— 不成立。Rust 的零成本抽象（Zero-Cost Abstraction）使高层 API（迭代器（Iterator）、泛型（Generics）状态机）编译后与控制流手写代码等价；真正要规避的是堆分配、锁与动态调度，这与抽象层级无关。
- **边界极限**: 硬实时截止期限 < 10μs 的场景（如电机 FOC 内环）需要裸机/RTOS，任何含操作系统调度的方案（含 Linux PREEMPT_RT）都不在该适用域。
...
机器人系统的失效代价是物理性的（碰撞、失控），因此边界测试必须覆盖从通信中间件到实时调度的完整链路。本章聚焦三类边界：

1. **DDS 消息无模式校验**：发布/订阅两端类型不匹配时，朴素反序列化直接把字节 reinterpret 为另一类型——这是 UB；ROS2 IDL 的类型指纹与 Rust 泛型锁定（`Subscription<T>`）把这类错误提前到编译期/握手期。
2. **回调阻塞执行器**：ROS2 单线程执行器中一个回调做阻塞 I/O 会饿死所有定时器与控制循环，直接违反实时截止期限；修正方向是多线程执行器或回调组隔离。
3. **多线程节点共享可变状态**：ROS1 风格的裸共享指针在 Rust 中被 `Send`/`Sync` 约束拒绝，必须用 `Arc<Mutex<T>>` 或消息传递重构。

每节给出危险代码、失效机理与 Rust 惯用修正。
...
> [来源: [Fast DDS](https://github.com/eProsima/Fast-DDS)]
...
以下测验检验本章五个机器人学 × Rust 的理解点：

- 测验 1：**为什么是 Rust**——实时性（无 GC 停顿）、内存安全（避免控制回路崩溃）与并发安全（Concurrency Safety）（传感器/规划/控制并行）三者的交集，DARPA 与 NASA 的项目资助印证了这一趋势。
- 测验 2：**ROS2 的 Rust 支持**——`rclrs` 提供节点、发布/订阅、服务等核心绑定，且能与 C++/Python 节点在同一 DDS 总线上互操作。
- 测验 3：**实时系统中的语言对比**——Python/Java 的 GC 与 JIT 引入不可界定的延迟，硬实时场景直接出局。
- 测验 4–5：**数学与仿真生态**——`nalgebra`（线性代数）与 rapier（物理引擎）的成熟度与适用边界。
