...
> **前置概念**: [Toolchain](../00_toolchain/01_toolchain.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md)
> **后置概念**: [WASI](01_wasi.md) · [WebAssembly](../11_domain_applications/03_webassembly.md)
...
交叉编译的技术细节分三层：工具链层（`rustup target add` 装 std 预编译包 + 系统 linker 如 `aarch64-linux-gnu-gcc`，或 `cross`/`cargo-zigbuild` 用容器/zig 屏蔽宿主差异）、自定义 target 层（JSON target spec 描述裸机/特殊平台，配合 `-Z build-std` 从源码构建 core）、嵌入式目标层（thumbv7em 等无 OS 目标，链接脚本与内存布局由 `cortex-m-rt` 管理）。三层的共同痛点是 C 依赖——FFI crate 的链接器配置是交叉编译失败的首要原因。
...
交叉编译的常见误解是“`--target` 一加就能出二进制”。反命题树拆解其前提：纯 Rust crate 确实如此，但任何 C 依赖（openssl-sys、ring 的部分平台）都需要目标平台的 C 工具链与 sysroot；动态链接 glibc 的产物只能在 glibc 版本不低于构建机的系统上运行（应换 musl 或旧版构建镜像）。边界极限给出量化约束：自定义 target 无 std 预编译时必须 nightly build-std，编译时间随目标数线性增长。
...
> [来源: [Rust Embedded FAQs](https://docs.rust-embedded.org/book/index.html)]
...
- [Toolchain](../00_toolchain/01_toolchain.md) — Cargo 与工具链
- [WASI](01_wasi.md) — WASI 与 Wasm
- [WebAssembly](../11_domain_applications/03_webassembly.md) — WebAssembly 生态
...
交叉编译边界测试固化三类高发错误：目标平台特性缺失（如 32 位目标上 `usize` 溢出假设、无原子 64 位操作的平台上用 `AtomicU64` 编译失败）；`std`/`no_std` API 差异（`std::time::Instant` 在裸机不存在，代码需 `#[cfg]` 分流）；链接器脚本与符号可见性错误（嵌入式目标的 section 布局问题在链接期暴露）。三类用例分别对应平台能力、API 可用性与链接约束。交叉编译的链接器缺失（编译错误）、边界测试：`cfg` 条件与目标平台的不一致（编译错误/逻辑错误）等5个方面，本节逐一说明其要点。
...
> **修正**: `cfg` 条件编译根据目标平台选择代码，但若所有条件都不满足，函数不存在，调用点编译错误。安全模式：添加默认实现或 `_` 通配：`#[cfg(not(any(target_os = "linux", target_os = "macos")))] fn platform_specific() -> &'static str { "other" }`。或使用 `cfg!` 宏（Macro）在运行时（Runtime）检查：`if cfg!(target_os = "linux") { ... } else { ... }`。交叉编译时的常见陷阱：1) 开发在 Linux 上，忘记 Windows 路径；2) `target_family = "unix"` 和 `target_family = "windows"` 的互斥性；3) `target_arch` 的嵌套（`target_arch = "x86_64"` 下还有 `target_feature = "sse2"`）。这与 C 的 `#ifdef _WIN32`（预处理器，相同模式）或 Go 的 `//go:build`（构建约束，类似 `cfg`）类似——跨平台代码的 `cfg` 覆盖是质量保证的关键。来源: [Rust Reference — Conditional Compilation](https://doc.rust-lang.org/reference/introduction.html) · 来源: [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html)

## 嵌入式测验（Embedded Quiz）

「嵌入式测验（Embedded Quiz）」涉及测验 1：Rust 的 target triple 格式是什么？请举例…、测验 2：`rustup target add aarch64-unk…、测验 3：`cargo build --target <triple>…、测验 4：为什么在 Windows 上交叉编译到 Linux 比在 L…等5个方面，本节逐一说明其要点。
...
- **P2 生态/社区**: [docs.rs/bytemuck — 生态权威 API 文档](https://docs.rs/bytemuck) · [docs.rs/zerocopy — 生态权威 API 文档](https://docs.rs/zerocopy)

## 🧭 思维导图（Mindmap）

```mermaid
mindmap
  root((交叉编译多目标平台支持与条件编译))
    一、核心概念
      1.1 Target Triple 的语义
      1.2 条件编译与 cfg
      1.3 std 与 no_std
