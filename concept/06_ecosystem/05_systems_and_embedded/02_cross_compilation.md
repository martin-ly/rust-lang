...
「技术细节」涉及交叉编译工具链、自定义 Target与嵌入式目标，本节逐一说明其要点。
...
「边界测试：交叉编译的编译错误」涉及边界测试：目标平台特性缺失（编译错误）、边界测试：`std` 与 `no_std` 的 API 差异（编译错误）、边界测试：交叉编译的链接器缺失（编译错误）、边界测试：`cfg` 条件与目标平台的不一致（编译错误/逻辑错误）等5个方面，本节逐一说明其要点。
