...
- v1.1 (2026-07-09): 重新确认 Rust 工具链状态——`wasm32-wasip2` 自 Rust 1.82（2024-10-17）起为 Tier 2 目标；旧目标名 `wasm32-wasi` 已在 Rust 1.84（2025-01-09）移除；当前 stable Rust 1.97.0 同时提供 `wasm32-wasip1` 与 `wasm32-wasip2`；WASI 0.2 已稳定，WASI 0.3 仍在开发。
- v1.0 (2026-05-13):
...
> **[WASI Preview 2 Docs](https://wasi.dev/); [Capability-Based Security Research](https://www.cl.cam.ac.uk/research/security/ctsrd/)** WASI 采用**能力安全（Capability Security）**模型替代传统的 POSIX 系统调用。程序不通过全局文件描述符访问资源，而是通过显式传递的**能力句柄（capability handle）**。
...
> **关键洞察**: WASI 的能力安全模型与 Rust 的所有权模型存在深层同构——二者都通过**资源唯一标识 + 显式转移**来消除隐式全局访问。这是 Rust 成为 Wasm 生态首选语言的深层原因。[WASI Docs](https://wasi.dev/); [Rust Ownership Model](https://doc.rust-lang.org/book/ch04-01-what-is-ownership.html) ✅
...
| Unsafe / FFI | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) | Wasm 与宿主边界 |
| 泛型（Generics）与 Trait | [`../02_intermediate/00_traits/01_traits.md`](../../02_intermediate/00_traits/01_traits.md) | WIT 接口的 Rust 映射 |
| 形式化生态塔 | [`./05_formal_ecosystem_tower.md`](../08_formal_verification/01_formal_ecosystem_tower.md) | Wasm 在 L0-L4 中的位置 |
| 应用领域 | [`./04_application_domains.md`](../06_data_and_distributed/01_application_domains.md) | Wasm 的工程落地场景 |
...
> **[Capability-Based Security Research](https://www.cl.cam.ac.uk/research/security/ctsrd/); [Dennis & Van Horn, CACM 1966](https://doi.org/10.1145/365230.365252); [Rust Ownership Model](https://doc.rust-lang.org/book/ch04-01-what-is-ownership.html)** 能力安全模型基于操作系统安全研究的经典文献。✅
---
