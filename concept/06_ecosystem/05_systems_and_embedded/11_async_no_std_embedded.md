...
> **前置概念**:
> [Async/Await](../../03_advanced/01_async/01_async.md) · [Pin 与 Unpin](../../03_advanced/01_async/08_pin_unpin.md) ·
> [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) · [交叉编译](02_cross_compilation.md) ·
...
在裸机环境中，外设事件是驱动 async 任务的主要来源；本节说明如何把**中断服务例程（ISR）**与 `Waker` 机制对接，使 executor 能安全地重新 poll 等待硬件事件的 Future，同时避免在中断上下文中执行复杂逻辑或错误地 await。
...
`no_std` 环境通常没有默认堆分配器，因此 Future 的生命周期与固定方式必须与标准 async 区别对待；本节先说明如何通过 `static` 或栈上 `Pin` 固定自引用 Future，再给出避免隐式堆分配、确保中断安全释放的实用模式。
