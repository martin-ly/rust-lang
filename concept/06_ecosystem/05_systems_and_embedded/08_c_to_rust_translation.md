...
C2Rust 是目前唯一成熟的 C→Rust 机械翻译器：基于 Clang AST 把 C 逐语句翻译为语义等价的 **unsafe** Rust（保持 bug-for-bug 兼容），产出可直接编译但充满裸指针与 transmute。因此后处理生态才是关键：`c2rust-refactor` 做批量重构（如把 malloc/free 改写为 Box）、第三方分析工具按模块边界收敛 unsafe。工程实践是“翻译 + 收敛”两阶段：先全量翻译保证行为不变，再按测试覆盖率驱动的顺序逐模块 safe 化。
...
评估 C→Rust 翻译质量需要专门基准：CRUST-Bench 提供带测试套件的 C 项目集，统一度量三项指标——编译通过率、测试通过率（功能保持）、safe 代码比例（安全收益）。其结论对工程选型有直接参考：纯 LLM 翻译在小程序上 safe 比例高但规模放大后功能保持率骤降；机械翻译功能保持最好但 safe 比例接近零；混合流水线在两者间取得最佳平衡，是当前唯一可用于生产的路径。
