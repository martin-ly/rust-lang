...
- **实时约束**: 硬实时（错过截止期限 = 系统失败，如气囊控制）vs 软实时（降级可容忍，如传感器融合）；Rust 的所有权模型消除整类并发错误，但**不自动提供时序保证**——确定性来自无堆分配、无 GC 与受控的中断设计。
- **RTIC 框架**: 基于优先级的抢占式任务框架，`#[app]` 宏在编译期做资源冲突分析与优先级天花板（priority ceiling）检查——数据竞争与死锁在编译期被排除，是其最强卖点。
...
- **“`no_std` 意味着没有标准库功能可用”** —— 不成立。`core`（类型、迭代器、原子操作）全部可用，`alloc`（`Vec`/`String`/`Box`）在配置全局分配器后也可用；真正失去的是 `std` 的 OS 依赖部分（线程、文件、网络、系统时钟）——多数“不能用”的抱怨实为未引入 `alloc` 或替代 crate（如 `heapless` 的静态容量容器）。
