...
> **前置概念**: [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) ·
...
> **后置概念**: [Cross Compilation](02_cross_compilation.md) ·
> [RTOS](../06_data_and_distributed/01_application_domains.md)
...
嵌入式 Rust 的硬件访问分三层抽象，每层解决不同的问题且不可互相替代：

- **PAC（Peripheral Access Crate）**: 由厂商 SVD 文件经 `svd2rust` 自动生成，提供寄存器的类型安全访问（`periph.UART.baud.write(|w| ...)`）；保证“不会读错寄存器位”，但不保证“配置序列正确”——时钟未使能就写外设仍是逻辑错误。
- **HAL（Hardware Abstraction Layer）**: 在 PAC 之上实现 `embedded-hal` trait，把“使能时钟 → 配置引脚复用 → 设置波特率”的初始化序列封装为构造函数；`OutputPin`/`SpiDevice` 等 trait 让驱动与具体 MCU 解耦。
- **BSP（Board Support Package）**: 面向具体开发板（如 nRF52840-DK），把引脚别名、板载 LED/按键/传感器封装为即用对象——BSP 是“开箱即用”层，量产产品设计通常绕过 BSP 直接用 HAL。

判定依据：写可移植驱动依赖 HAL trait；写板级应用从 BSP 起步；PAC 只在 HAL 缺失的外设上直接使用。
...
嵌入式实时系统的五个主题按“约束 → 框架 → 运行时 → 抽象”递进：

- **实时约束**: 硬实时（错过截止期限 = 系统失败，如气囊控制）vs 软实时（降级可容忍，如传感器融合）；Rust 的所有权（Ownership）模型消除整类并发错误，但**不自动提供时序保证**——确定性来自无堆分配、无 GC 与受控的中断设计。
- **RTIC 框架**: 基于优先级的抢占式任务框架，`#[app]` 宏（Macro）在编译期做资源冲突分析与优先级天花板（priority ceiling）检查——数据竞争与死锁在编译期被排除，是其最强卖点。
- **Embassy**: 中断驱动的 async 执行器，任务即状态机，I/O 等待零线程开销；适合协议栈密集的联网设备。
- **embedded-hal-async**: 异步硬件抽象 trait，让驱动在同步/异步生态间复用。
- **选型补遗**: 裸 `cortex-m-rt` + 手写中断适合极简固件。

判定依据：硬实时 + 资源共享复杂 → RTIC；I/O 并发多 + 协议栈 → Embassy；教学/最小系统 → 裸运行时。
...
- **“Rust 嵌入式已经全面成熟，可以无脑替代 C”** —— 部分成立。主流 MCU 家族（STM32/nRF/RP2040/ESP32）的 HAL 已生产可用，但小众芯片的 PAC/HAL 可能缺失或质量参差；评估的首要动作是查目标芯片的 `awesome-embedded-rust` 支持状态与 HAL 仓库活跃度，而非假设覆盖。
- **“`no_std` 意味着没有标准库功能可用”** —— 不成立。`core`（类型、迭代器（Iterator）、原子操作（Atomic Operations））全部可用，`alloc`（`Vec`/`String`/`Box`）在配置全局分配器后也可用；真正失去的是 `std` 的 OS 依赖部分（线程、文件、网络、系统时钟）——多数“不能用”的抱怨实为未引入 `alloc` 或替代 crate（如 `heapless` 的静态容量容器）。
- **边界极限**: 固件尺寸 < 32KB Flash 的超低端 MCU 上，Rust 的运行时开销（panic 处理、格式化）需要刻意裁剪（`panic-halt`、`defmt`），否则二进制放不下。
...
- [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) — unsafe Rust
- [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) — 内存管理
- [Cross Compilation](02_cross_compilation.md) — 交叉编译
...
嵌入式 Rust 的错误面与桌面环境根本不同：没有操作系统兜底，`no_std` 把大量“运行时失败”变成了编译期拒绝。本章覆盖六个代表性边界：

- **`no_std` 中的 `println!`**：标准输出依赖 OS，`no_std` 下直接编译失败；替代方案是 semihosting、RTT 或 UART HAL。
- **中断处理器中的 `static mut`**：Rust 2024 edition 起 `static_mut_refs` 变为硬错误，共享状态必须用 `Mutex<RefCell<T>>`（cortex-m）或 `AtomicXxx`。
- **临界区与 `unsafe` 误用**：关中断临界区内调用可能睡眠/分配的操作会引发数据竞争或死锁。
- **`panic` 处理与固件大小**：`panic-halt`/`panic-reset` 的选择直接影响二进制体积与可调试性。
- 另两项覆盖 DMA 缓冲区对齐与外设时钟未使能等硬件边界。

每节给出编译错误原文、根因分析与嵌入式惯用修正。
...
> **认知功能**: 本 mindmap 从本页「Rust 嵌入式系统开发」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
