...
「硬件抽象层」部分按 PAC — 外设访问 crate、HAL — 硬件抽象层与BSP — 板级支持包的顺序逐层展开。
...
本节围绕「实时系统」展开，依次讨论实时约束、RTIC 框架、Embassy：嵌入式异步运行时、embedded-hal-async：异步硬件抽象等5个方面。
...
「边界测试：嵌入式系统的编译错误」部分按边界测试：`no_std` 中的 `println!`（编译错误）、边界测试：中断处理器的 `static mut`（编译错误）、边界测试：临界区的中断禁用与 `unsafe` 的误用（运行时数据竞争）、边界测试：`no_std` 中的 `panic` 处理与固件大小（编译…等6个方面的顺序逐层展开。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：`no_std` 在 Rust 嵌入式开发中意味着什么？（理…、测验 2：Rust 的 `embedded-hal`（硬件抽象层）项目…、测验 3：为什么在嵌入式系统中，`static mut` 被认为是不安…、测验 4：`defmt`（deferred formatting）相比…等5个方面。
