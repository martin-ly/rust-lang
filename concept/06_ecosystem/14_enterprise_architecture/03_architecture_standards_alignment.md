...
| 误解 | 反例场景 | 为什么错误 |
|---|---|---|
| "ISO 42010 合规等于架构优良" | 项目文档满足所有 viewpoint/view 要求，但核心 crate 存在循环依赖且 unsafe 边界未审计 | 标准描述完整性不等于实现质量；仍需代码级验证和架构测试 |
| "标准可以直接套用，无需裁剪" | 5 人内部工具团队强制应用 12207 全部过程组，评审会议占用 30% 工期 | 标准是参考坐标系，必须按组织规模、风险等级和领域上下文裁剪 |
| "通过认证就无需持续验证" | 系统通过 25010 安全审查后，新增 FFI 代码未再审计，引入 UAF 漏洞 | 标准对齐是快照；工程变更需要持续质量门和回归验证 |
