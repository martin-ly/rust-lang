...
- [架构治理与架构决策记录（Architecture Governance and ADRs）](#架构治理与架构决策记录architecture-governance-and-adrs)
  ...
  - [六、反命题与边界](#六反命题与边界)
    ...
    - [反例：ADR 与治理的常见误用](#反例adr-与治理的常见误用)
...
| 误解 | 反例场景 | 为什么错误 |
|---|---|---|
| "ADR 可以替代代码审查" | 团队认为 ADR 已记录决策理由，便将 unsafe 优化直接合入主干而跳过 review | ADR 记录"为什么做"，不替代"做得对不对"的实现验证 |
| "每个决策都需要 ADR" | 为变量命名风格、函数签名缩进等局部问题创建 ADR，维护噪音淹没关键决策 | ADR 应只用于影响深远、跨 crate/跨团队、长期有效的架构决策 |
| "治理必然拖慢交付" | 团队绕过 MSRV 和依赖白名单快速升级依赖，三个月后下游 crate 出现兼容性危机 | 治理通过前置约束减少返工；绕过治理的"快"通常以技术债务偿还 |
