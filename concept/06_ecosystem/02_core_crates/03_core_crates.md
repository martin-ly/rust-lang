...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Traits](../../02_intermediate/00_traits/01_traits.md) · [Generics](../../02_intermediate/01_generics/02_generics.md) · [Async](../../03_advanced/01_async/02_async.md) · [Unsafe](../../03_advanced/02_unsafe/03_unsafe.md) [来源: [Rust FFI Guidelines](https://doc.rust-lang.org/nomicon/ffi.html)]
> **后置概念**: [Application Domains](../06_data_and_distributed/04_application_domains.md)
...
- 自动生成实现
- 基于 trait： /  /  模式
- 支持 JSON（）、YAML、TOML、MessagePack 等数十种格式
- 零运行时（Runtime）反射，完全编译期生成代码

与 Go 的  反射驱动不同，serde 是**编译期**的，性能更高。
</details>

---

### 测验 2：tokio 运行时选择（应用层）

以下代码应该使用 tokio 的哪种运行时（Runtime）？
...
- ****（默认）：多线程工作窃取调度器，适合 I/O 密集 + 并发任务
- ****：所有任务在单个 OS 线程上运行，适合资源受限环境（如 wasm）

CPU 密集型任务不应直接运行在 async 任务中，应使用  避免阻塞事件循环。
</details>

---

### 测验 3：thiserror vs anyhow（应用层）

库代码（library）和应用代码（application）分别应优先使用哪个错误库？
...
| 库 | 用途 | 特点 |
|:---|:---|:---|
|  | 库代码 | 帮助定义结构化错误类型（enum），支持错误源链 |
|  | 应用代码 | 提供 ，简化错误传播 |

库需要暴露具体错误类型给调用者；应用通常只关心"出错了吗"和"错误消息是什么"， 的  自动转换更方便。
</details>

---

### 测验 4：bytes crate 的设计目标（分析层）

 最适合哪种场景？

- A. 存储大量小字符串
- B. 网络协议解析中共享数据切片（Slice）
- C. 替代  作为通用缓冲区
...
- 引用（Reference）计数共享底层缓冲区
- 零拷贝创建子视图
- 线程安全（）

最适合：数据读取一次后，多个组件分别处理不同切片（Slice）（如 HTTP 解析、TCP 帧拆分）。若只需要临时缓冲区， 更简单高效。
</details>

---

### 测验 5：核心 crate 选型原则（评价层）
...
Rust的强大来自于生态，核心crate（Crate）是基石
...
零成本抽象（Zero-Cost Abstraction）/组合优于继承/类型驱动API

### 第 5 步：async生态的核心组件？
>

tokio/Tokio/futures/smol的比较和选择
...
| **CLI 解析** | clap | 2 亿+ | L2 Trait (derive) + L1 类型系统（Type System） | ❌ 纯 safe | ⭐⭐⭐⭐⭐ |
...
| **测试/基准** | criterion, proptest | 3000 万+ | L2 泛型（Generics） + L1 类型系统（Type System） | ❌ 纯 safe | ⭐⭐⭐⭐ |
| **集合/数据结构** | hashbrown, indexmap | 1 亿+ | L2 泛型（Generics） + L1 所有权（Ownership） | ⚠️ 少量 unsafe | ⭐⭐⭐⭐⭐ |
...
| 异步（Async） HTTP 服务端 | axum | actix-web, poem | 手写 hyper | axum = tokio 官方生态 |
| 异步（Async） HTTP 客户端 | reqwest | hyper | 手写 TCP | reqwest 封装了最佳实践 |
...
| TLS/HTTPS | rustls | ring + 手动 | openssl-sys | rustls 纯 Rust，内存安全（Memory Safety） |
| 数据并行 | rayon | crossbeam | 手写线程池 | rayon 迭代器（Iterator）抽象 |
...
| **Crate** | **格式** | **特点** | **L2 概念根基** |
|:---|:---|:---|:---|
| **serde** | 框架核心 | derive 宏（Macro）驱动、零成本、生态标准 | Trait (Serialize/Deserialize) |
...
| **Tokio** | M:N 协作调度 | 标准库 API 风格、await 一切 | 同上 |
...
| **poem** | 模块（Module）化 | OpenAPI 原生支持、GRAPHQL 集成 | 企业 API |
...
| **rustls** | TLS | 纯 Rust TLS、内存安全（Memory Safety）、替代 OpenSSL | L3 unsafe(极少) |
...
| **prometheus** | 指标 | 原生 Rust、Histogram/Counter/Gauge | 并发安全（Concurrency Safety） |
...
| **Crate** | **模型** | **特点** | **L3 概念根基** |
|:---|:---|:---|:---|
| **rayon** | 数据并行 | `par_iter()`、work-stealing、自动负载均衡 | Send/Sync + 所有权（Ownership） |
...
> **关键洞察**: dashmap 通过**分片锁**（sharding）将全局锁竞争转化为局部竞争。当 key 的哈希分布均匀时，不同 key 的读写几乎无竞争。这是标准库 `HashMap` 无法实现的，因为 `std` 不提供并发安全（Concurrency Safety）保证。
...
| 错误处理（Error Handling） | `Box<dyn Error>` | `anyhow`/`thiserror`（人体工学） |
...
| 二进制体积 | `cargo bloat` | 单态化（Monomorphization）膨胀可控 |
...
| tokio 是异步（Async）运行时标准 | [tokio.rs](https://tokio.rs/) · [crates.io](https://crates.io/) | ✅ |
...
| 宏系统 | [`../03_advanced/03_proc_macros/04_macros.md`](../../03_advanced/03_proc_macros/04_macros.md) | serde/clap derive |
| 工具链 | [`./01_toolchain.md`](../00_toolchain/01_toolchain.md) | Cargo/crates.io 支撑 |
| 设计模式 | [`./02_patterns.md`](../03_design_patterns/02_patterns.md) | Builder/Typestate 模式 |
| 应用领域 | [`./04_application_domains.md`](../06_data_and_distributed/04_application_domains.md) | crate 的工程落地 |
| 安全边界 | [`../05_comparative/04_safety_boundaries.md`](../../05_comparative/03_domain_comparisons/04_safety_boundaries.md) | unsafe crate 审计 |
...
> **关键洞察**: serde 的 `derive(Serialize, Deserialize)` 在编译期生成与类型结构**同构**的编解码逻辑，保证运行时不会出现字段缺失或类型不匹配。tokio 的 `spawn` 将 `Future` 的所有权转移给运行时，利用 Rust 的所有权系统保证任务生命周期（Lifetimes）的安全。
...
- [x] **中**: 补充嵌入式 crate 生态（embedded-hal / embassy / probe-rs） —— 已完成 §9.5
 [x] **低**: 补充游戏开发 crate 生态深度案例 —— 已完成 §9.6- [x] **低**: 补充 ML 推理 crate 生态（candle / burn / tract） —— 已完成 §9.7
...
| **thiserror/anyhow 简化错误** | 类型安全/人体工学 ⟹ | 减少样板代码 | 隐式转换注意 | 错误处理（Error Handling）策略 |
...
| **C3** | `rayon` 数据并行 | 迭代器（Iterator）无副作用 | 并行执行结果 = 串行执行结果 | 全局可变状态；非原子计数 | 非确定性结果 |
...
async fn tokio_task() {
    ...
    // ❌ 编译错误: 不能在 Tokio runtime 中直接使用 tokio 的 channel
