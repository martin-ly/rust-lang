...
> **前置依赖**: [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) · [Trait](../../02_intermediate/00_traits/01_traits.md) · [类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) · 安全实践
> **后置延伸**: [区块链与智能合约安全](../11_domain_applications/01_blockchain.md) · [网络协议](../04_web_and_networking/07_network_protocols.md) · [TLS/QUIC](../04_web_and_networking/07_network_protocols.md)
>
...
- **“用了安全 crate 就等于应用安全”** —— 不成立。库只保证原语正确，协议层的误用（nonce 复用、密钥硬编码、跳过证书验证、错误的消息认证顺序）在库的能力边界之外；AES-GCM 的 nonce 复用会灾难性地泄露认证密钥，而编译器无从知晓你的 nonce 管理策略。安全是系统属性，不是依赖属性。
- **“密码学代码应该用 unsafe 榨取性能”** —— 不成立且危险。现代密码库的性能来自恒定时间汇编内核（已封装在 ring/dalek 内），应用层 unsafe 不仅无性能收益，还引入侧信道与内存安全双重风险；密码学代码的正确性价值远高于微秒级性能。
- **边界极限**: 需要 FIPS 140-3 认证的合规场景，开源 Rust 库的认证状态需逐项核实（aws-lc-rs 是已认证路径之一）——技术优秀不等于合规可用。
...
三个边界测试覆盖密码学误用最致命的三种形态，按隐蔽性递增：

- **非常量时间比较**: 用 `==` 比较 MAC/令牌时，提前退出使执行时间泄露匹配前缀长度，攻击者可逐字节恢复密钥材料；必须使用 `subtle::ConstantTimeEq` 或库提供的 `verify` 函数——这类漏洞编译器与测试都无法发现，只能靠代码审查纪律。
- **Nonce 复用（AES-GCM）**: 同一密钥下 nonce 重复使两条密文的异或暴露明文异或，并泄露 GHASH 认证密钥；对策是计数器 nonce（带持久化防回滚）或随机 nonce + 消息上限（2³² 条），或改用 nonce-misuse 抵抗的 AES-GCM-SIV。
- **低迭代次数 KDF**: PBKDF2 用 1000 次迭代或直接用 SHA-256 存密码，使 GPU 暴力破解成本趋零；2026 年的基线是 Argon2id（内存硬度）或 PBKDF2 ≥ 600k 迭代。

判定依据：三类错误的共性是“代码看起来能跑、测试全绿、安全已破”——密码学代码评审必须对照此类清单逐项核查。
...
- [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) — 密码学实现中的 unsafe 边界
- [类型系统（Type System）](../../01_foundation/02_type_system/01_type_system.md) — 类型安全与密码学抽象
- [内存管理](../../02_intermediate/02_memory_management/01_memory_management.md) — 密钥材料的内存管理
- [并发编程](../../03_advanced/00_concurrency/01_concurrency.md) — 密码学操作的线程安全
- [形式化验证](../../04_formal/04_model_checking/01_verification_toolchain.md) — 密码学实现的形式化证明
...
以下测验检验本章五个密码学工程判断点，均为安全代码评审中的硬性要求：

- 测验 1：**`ring` 的定位**——基于 BoringSSL 的经审计密码学库，提供 AEAD、签名、HKDF 等原语；铁律是“不要自己实现密码学”。
- 测验 2：**为什么禁用 `unsafe`**——密码学代码中任何内存错误都可能泄露密钥（包括写入 swap、未清零的缓冲区），应完全依赖经审计库。
- 测验 3：**常量时间比较**——标准 `==` 提前返回会泄露匹配前缀长度，时序攻击可逐字节恢复密钥；`subtle` 提供常量时间原语。
- 测验 4–5：**rustls vs OpenSSL 与密钥管理**——内存安全 TLS 栈消除整类 CVE，以及密钥轮换/HSM 的工程实践。
