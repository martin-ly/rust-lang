...
> **前置概念**: [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Error Handling](../../02_intermediate/03_error_handling/02_error_handling_deep_dive.md)
> **后置概念**: [Blockchain](../11_domain_applications/01_blockchain.md) · [Formal Methods](../../04_formal/02_separation_logic/01_rustbelt.md)
>
> **来源**: [Rust Secure Code WG](https://github.com/rust-secure-code/wg) · [Cargo — Specifying Dependencies](https://doc.rust-lang.org/cargo/reference/specifying-dependencies.html) · [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html) · [Brown University — Interactive Rust Book](https://rust-book.cs.brown.edu/) · [Jung et al. — RustBelt: Securing the Foundations of Rust](https://plv.mpi-sws.org/rustbelt/popl18/) · [Itanium C++ ABI](https://itanium-cxx-abi.github.io/cxx-abi/abi.html)
---
...
- [安全实践：Rust 代码的防御性编程](#安全实践rust-代码的防御性编程)
  - [📑 目录](#-目录)
  - [一、核心概念](#一核心概念)
  ...
  - [二、技术细节](#二技术细节)
    ...
    - [2.3 审计工具链](#23-审计工具链)
    - [2.4 输入验证系统化](#24-输入验证系统化)
    - [2.5 密码学 misuse：不要重复造轮子](#25-密码学-misuse不要重复造轮子)
    - [2.6 Secret 处理：内存中的敏感数据](#26-secret-处理内存中的敏感数据)
    - [2.7 沙箱与边界安全](#27-沙箱与边界安全)
    - [2.8 供应链安全工具链深化](#28-供应链安全工具链深化)
  - [三、安全模式矩阵](#三安全模式矩阵)
...
“Rust 内存安全所以 Rust 程序安全”是最危险的过度推断。反命题树逐层反驳：内存安全不覆盖逻辑漏洞（鉴权绕过、IDOR 与语言无关）、不覆盖 unsafe 块内部（FFI 边界是漏洞富集区）、不覆盖供应链（依赖投毒与 crate 本身无关）。边界极限给出量化视角：`cargo geiger` 的 unsafe 密度、依赖树深度、RUSTSEC 未修复公告数是可度量的风险代理指标，安全姿态应建立在这些指标的趋势上而非语言神话上。
...
**修复**：Rust 1.97.0 仅对 **git 协议** 的 registry URL 剥离 `.git` 后缀；sparse 索引不再进行此规范化。
...
- [Formal Methods](../../04_formal/02_separation_logic/01_rustbelt.md) — 形式化验证
...
安全边界测试覆盖三类高危失效：常量时间比较被编译器优化破坏（普通 `==` 比较密钥有计时侧信道，必须 `subtle::ConstantTimeEq`）；unsafe 块跨越预期审计边界（封装泄漏使 safe 调用者触发 UB，`#![forbid(unsafe_code)]` 与审计范围的冲突）；密钥材料未零化残留内存（`zeroize` 的 `ZeroizeOnDrop` 派生 vs 手动 drop 的差异，以及编译器可能优化掉“无用”覆写的风险）。e` 与编译器优化的冲突（逻辑错误）、边界测试：依赖供应链的 typo-squatting（运行时安全风险）等7个方面，本节逐一说明其要点。
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：`cargo audit` 工具的主要功能是什么？（理解层）、测验 2：为什么在 Rust 中处理密码时，建议使用 `secrecy…、测验 3：Rust 的所有权系统如何帮助防御缓冲区溢出攻击？（理解层）、测验 4：`#[forbid(unsafe_code)]` 属性有什么…等5个方面，本节依次展开。
...
> **认知功能**: 本 mindmap 从本页「安全 实践 Rust 代码的防御性编程」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
