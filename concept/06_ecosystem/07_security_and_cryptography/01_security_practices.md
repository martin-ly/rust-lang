...
本节将「技术细节」分解为若干主题：输入验证与清洗、加密与安全原语与审计工具链。
...
「边界测试：安全实践的编译错误」涉及边界测试：密码学常量时间操作（运行时风险）、边界测试：`unsafe` 代码的审计边界（编译错误）、边界测试：`zeroize` 与编译器优化的冲突（逻辑错误）、边界测试：依赖供应链的 typo-squatting（运行时安全风险）等7个方面，本节逐一说明其要点。
