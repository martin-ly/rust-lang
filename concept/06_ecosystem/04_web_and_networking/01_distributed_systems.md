...
> **后置概念**: [WebAssembly](../11_domain_applications/03_webassembly.md) ·
> [Observability](../00_toolchain/02_logging_observability.md)
>
...
- [分布式 系统：Rust 在微服务 与集群中的工程实践](#分布式-系统rust-在微服务-与集群中的工程实践)
  ...
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  - [十一、分布式网络系统技术选型](#十一分布式网络系统技术选型)
    - [11.1 基础理论](#111-基础理论)
    - [11.2 共识与容错](#112-共识与容错)
    - [11.3 分布式协调](#113-分布式协调)
...
Rust 在分布式系统中的定位由三个相互支撑的核心概念界定：

- **语言定位**: 分布式系统的故障面分两类——实现缺陷（内存破坏、数据竞争、空指针）与设计缺陷（一致性、分区处理、时钟假设）；Rust 在编译期消除第一类的大部分，让工程精力集中于第二类。对比 Java/Go：无 GC 停顿使尾延迟可预测（p999 在协调服务中至关重要），内存效率降低集群规模。
- **异步运行时作为分布式基础**: tokio 的任务模型把“万级连接 + 跨节点 RPC”映射为协作调度的状态机，`Send` 约束在编译期保证跨任务共享数据的安全——分布式代码的并发正确性从“评审纪律”部分转为“类型证明”。
- **服务发现与负载均衡**: 控制面（etcd/Consul）维护成员与配置，数据面（客户端负载均衡 + 健康检查）消费其变更；Rust 侧的典型栈是 `tonic` 的 channel + `tower::balance` 组合。

判定依据：Rust 写分布式系统的收益与服务的“安全攸关 × 性能敏感”程度正相关。
...
三个技术主题对应分布式系统的三类基础能力——通信、一致、并发模型：

- **gRPC 与 Protocol Buffers**: 服务间通信的强类型契约——`.proto` 定义即跨语言的事实源，`tonic` 生成的 stub 让 RPC 调用是类型检查的本地调用形态；流式 RPC 覆盖订阅与大结果集场景。
- **分布式共识与 Raft**: 一致性写入的算法基础——leader 选举 + 日志复制 + 多数派确认；Rust 生态的 `raft-rs`/`openraft` 提供状态机库，存储与网络由应用注入（详见 `06_data_and_distributed/06_distributed_consensus.md`）。
- **Actor 模型与消息传递**: 把共享状态并发转为消息传递并发——actor 各自持有状态、经邮箱串行处理消息，与 Rust“独占所有权（Ownership） + channel”的设计天然同构；`actix`（经典）与 `ractor`/`kameo`（tokio 原生）是代表实现。

判定依据：三者的组合顺序通常是：先定通信契约（proto），再定状态分布（哪些状态需共识），最后选进程内并发模型。
...
- **“Rust 的内存安全（Memory Safety）对分布式系统意义不大，瓶颈在网络上”** —— 部分成立。稳态吞吐确实常由网络/磁盘界定，但**故障面**不同：Java/Go 的 GC 停顿在协调服务（选主、分布式锁）中可触发假死误判与级联故障，Rust 的确定性延迟消除这一整类事故；此外内存效率直接换算为集群成本（同等负载更少的节点）。
- **“微服务必须用 gRPC/消息队列等重型基础设施”** —— 不成立。服务间通信方式应由耦合度决定：同步查询用 REST 足够（工具链成熟、可缓存），强类型内部调用才需要 gRPC，解耦的异步事件才需要消息队列；一上来全量引入三件套，复杂度成本超过分布式本身的收益。
- **边界极限**: 延迟预算 < 1ms 的服务间调用（如同机架高频交易）需要共享内存或 RDMA，gRPC/HTTP 栈本身已超出预算。
...
- [Stream Processing Semantics](../../03_advanced/06_low_level_patterns/05_stream_processing_semantics.md) — 流处理核心语义
- [Stream Processing Ecosystem](../06_data_and_distributed/03_stream_processing_ecosystem.md) — 流处理生态
...
分布式系统的 Rust 代码失效点集中在进程边界两侧：类型如何跨越网络、数据如何在任务间移动。本章的边界测试覆盖七个代表性场景，可归为四组：

- **序列化契约**：消息 schema 演进（新增字段无 `#[serde(default)]` 导致旧消息反序列化失败）、消息大小无上限导致内存被打爆——前者是兼容性错误，后者是可被利用的 DoS 面。
- **并发约束**：分布式事务状态跨 `.await` 持有时必须满足 `Send`，否则编译器拒绝将其 spawn 到多线程运行时——这是 Rust 把分布式系统的线程安全提前到编译期的典型体现。
- **时间假设**：共识协议依赖时钟偏差有界，单调时钟与墙钟混用会产生逻辑错误（如租约提前过期）。
- **重试与幂等**：无幂等键的重试在部分失败时重复执行副作用。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：Rust 在分布式系统中相比 Java/Go 的主要优势是什…、测验 2：`tonic` 在 Rust gRPC 生态中扮演什么角色？…、测验 3：在分布式系统中，为什么 Rust 的 Serde + 强类型…、测验 4：`etcd` / `consul` 的服务发现与 Rust…等5个方面。
...
> 内容来源：`crates/c10_networks/docs/tier_04_advanced/05_distributed_network_systems.md`，已按 AGENTS.md §6.4 迁移至此。

分布式系统的核心概念与 Rust 生态选型可归纳为：

### 11.1 基础理论

- **CAP 定理**：一致性（Consistency）、可用性（Availability）、分区容错性（Partition Tolerance）不可同时满足。
- **一致性模型**：强一致性、因果一致性、最终一致性。
- **逻辑时钟**：Vector Clock、Lamport Timestamp 用于事件排序与因果关系判定。
...
| :--- | :--- | :--- | :--- |
| **Paxos** | 强一致 | 非拜占庭 | 学习实现 |
| **Raft** | 强一致 | 非拜占庭 | `raft-rs`、`openraft` |
| **PBFT** | 强一致 | 拜占庭容错 | 研究/联盟链场景 |
| **CRDT** | 最终一致 | 冲突自动合并 | `automerge`、自定义实现 |

### 11.3 分布式协调

- **分布式锁**：基于 Redis（RedLock）或 etcd（lease + compare-and-swap）。
- **分布式事务**：两阶段提交（2PC）适合短事务；Saga 模式适合长事务与微服务。
- **服务发现**：Consul、etcd、Zookeeper，Rust 客户端通过 HTTP/gRPC 集成。
- **负载均衡**：轮询、加权轮询、一致性哈希。
- **服务网格**：Istio、Linkerd 提供 mTLS、流量管理与可观测性。
- **熔断器模式**：快速失败并触发降级，避免雪崩。

```rust,ignore
// Raft 状态机接口示例（伪代码）
pub trait RaftStateMachine {
    fn apply(&mut self, entry: LogEntry) -> Result<ApplyResult, Error>;
