...
- **Actor 模型与消息传递**: 把共享状态并发转为消息传递并发——actor 各自持有状态、经邮箱串行处理消息，与 Rust“独占所有权 + channel”的设计天然同构；`actix`（经典）与 `ractor`/`kameo`（tokio 原生）是代表实现。
...
- **“Rust 的内存安全对分布式系统意义不大，瓶颈在网络上”** —— 部分成立。稳态吞吐确实常由网络/磁盘界定，但**故障面**不同：Java/Go 的 GC 停顿在协调服务（选主、分布式锁）中可触发假死误判与级联故障，Rust 的确定性延迟消除这一整类事故；此外内存效率直接换算为集群成本（同等负载更少的节点）。
