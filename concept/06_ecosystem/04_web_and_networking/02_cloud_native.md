...
云原生 Rust 服务的失效点横跨编译期与运行时，且多与部署环境（容器、Kubernetes）交互产生。本章选取六个高频边界：

- **异步运行时混用**：tokio 与 async-std 的 reactor 互不兼容，跨运行时执行 I/O future 会 panic；编译期不报错，是最隐蔽的运行时陷阱。
- **配置反序列化生命周期**：从环境变量/配置文件反序列化时借用临时缓冲区，触发生命周期编译错误——根因是配置应拥有数据（`String`）而非借用。
- **优雅关闭**：Kubernetes 先发 `SIGTERM` 再等待 `terminationGracePeriodSeconds`，未监听信号的 Rust 服务会被强杀，丢失在途请求。
- **探针配置**：readiness/liveness 探针阈值过严会触发级联重启（重启 → 未就绪 → 流量打到更少实例 → 更过载）。
- 另两项覆盖镜像体积与 DNS 缓存等部署期边界。
