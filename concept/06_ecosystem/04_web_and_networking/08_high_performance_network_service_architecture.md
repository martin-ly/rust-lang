...
理论架构需要生产案例验证。Pingora 是 Cloudflare 开源的代理框架（Rust 实现，承载其 CDN 边缘流量），其设计集中体现了本章前文的多项原则：多线程 + per-core 连接亲和替代传统 event-loop 共享状态、连接池复用降低上游握手开销、可插拔的 request filter 阶段化流水线。注意该 crate 当前存在安全公告（见下文提示），本节仅作架构分析样本，不作为依赖推荐。
...
网络服务的性能结论必须可复现：单机数字受 CPU 频率、网卡中断亲和、内核参数影响巨大，脱离环境的 QPS 对比没有意义。下面的脚本组合三类测量：wrk 测吞吐与延迟分布、系统计数器（`vmstat`/`ss`）定位瓶颈层、连接跟踪观察 TIME_WAIT 等状态累积。基准纪律：客户端与服务端分离机器、固定并发梯度扫描、每次改一个变量。
...
本节把前文收敛为可执行的工程清单：决策树回答“我该选哪种架构”（按 QPS、延迟、团队复杂度分层），优化检查清单按收益/成本排序（零拷贝、io_uring、per-core 分区、NUMA 亲和），系统调优参数给出 Linux 侧的最小改动集（`somaxconn`、TCP keepalive、文件描述符上限）。使用顺序：先决策树定架构，再按清单逐项验证，最后调参——调参永远不应该是第一步。
...
总结部分把全章技术点压缩为两张视图：对比表按“性能提升 × 实现复杂度 × 适用场景”三维排序，揭示高收益低成本的优先项（零拷贝、多队列网卡）与高收益高成本的慎选项（Lock-Free 结构）；推荐组合则给出三档配置——通用 Web 服务（tokio + 零拷贝）、高并发代理（per-core + io_uring）、极致场景（NUMA + 内核旁路）。技术选型按档起步，用基准数据决定是否升级。
