...
WebSocket 的知识结构可按四个维度组织，帮助建立完整的概念地图：

- **概念定义**：RFC 6455 定义的全双工协议，在单个 TCP 连接上通过 HTTP Upgrade 握手建立，握手后脱离 HTTP 语义，帧（frame）开销仅 2–14 字节。
- **属性特征**：长连接、双向主动推送、低延迟——与 HTTP 轮询/SSE 的根本区别是服务端无需等待请求即可发送。
- **关系连接**：继承 TCP 的可靠有序字节流，与 HTTP 共享 80/443 端口以穿透代理；上层可承载 STOMP、Socket.IO 等子协议。
- **思维导图**：将上述维度可视化，标注各概念间的 is-a / extends / uses 关系。
...
本章从协议本质到 Rust 实现完整覆盖了 WebSocket 实时通信，核心结论可归纳为三条：

1. **选型依据**：需要服务端主动推送且双向频繁通信时选 WebSocket；只需单向推送用 SSE 更简单；低频更新用轮询即可——不要为了“实时”而实时。
2. **Rust 生态**：`tokio-tungstenite` 提供 async 原语，axum/actix 均内置 WebSocket 升级支持；生产实现必须处理 Ping/Pong 心跳、背压（发送队列有界化）与优雅关闭。
3. **运维要点**：连接数是有状态资源，横向扩容需要 sticky session 或外部消息总线（Redis/NATS）扇出。

下面的要点表与最佳实践清单可作为上线前的检查项。
