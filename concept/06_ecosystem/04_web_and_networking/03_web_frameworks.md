...
> **前置概念**: [Async](../../03_advanced/01_async/01_async.md) · [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) · [Traits](../../02_intermediate/00_traits/01_traits.md)
...
中间件是 Web 框架可扩展性的决定因素，两个框架族采用根本不同的模型：

- **模型分类**: ① **Tower 模型（axum）**——中间件是 `Service` trait 的嵌套包装（onion 式），每层是一个实现 `Service` 的类型，组合在编译期定型，零运行时开销但类型签名复杂；② **Actor/对象模型（actix-web）**——中间件是 trait object 链，注册简单、运行时可动态组装，代价是虚调用与堆分配；③ **请求守卫（rocket）**——以请求级守卫 + 公平器（fairing）拦截生命周期事件，声明式最强但灵活性受框架约束。
- **对比矩阵维度**: 组合方式（类型级/对象级/声明式）、错误传播（类型化/统一错误）、异步支持（原生 async/await 或自定义运行时）、生态复用（Tower 中间件跨框架通用：axum、tonic、hyper 共享同一中间件生态）。

判定依据：需要与 tonic/hyper 共享中间件（限流、追踪）选 Tower 系；需要运行时动态装配选 actix-web。
...
框架选型讨论中有两个需要破除的全称命题，本节分别给出反命题树与判定条件：

1. **“Axum 总是最佳选择”**：Axum 的优势（Tower 生态、类型安全 extractor）同时也是边界——`Service`/`Layer` 抽象的学习曲线、缺少声明式路由宏、OpenAPI 需依赖 utoipa 等外部 crate。当团队不熟悉 Tower 或需要快速声明式开发时，Poem/Rocket 可能更合适。
2. **“Web 框架性能决定一切”**：TechEmpower 基准中框架间差距多在 10–30%，而数据库访问、序列化与业务逻辑通常占 90%+ 的请求耗时。性能只在框架成为瓶颈的极少数场景（高吞吐网关）是首要判据。

判定依据：先用团队能力与交付节奏筛掉不合适选项，性能只作为同档候选间的 tie-breaker。
...
|:---|:---|:---|:---|
| 新项目 + Tower 生态兼容（如 Tonic gRPC） | Axum | §6.1 决策树 Q1/Q2 | 团队不熟 Tower/`Service` ⟹ 学习曲线陡（§7.1 F1） |
| 声明式路由宏 + 快速原型 | Rocket | §6.1 决策树 Q3 / §6.2 | 生态隔离；复杂宏展开拖慢 debug 编译（§7.2 反例 2） |
| 最成熟社区、企业长期维护 | Actix-web | §6.1 决策树 Q4 / §6.2 | Actor 模型带来额外心智负担 |
| OpenAPI/文档驱动开发 | Poem（poem-openapi） | §6.1 决策树 Q5 | Axum 需 utoipa 等外部集成，非原生（§7.1 F3） |
| AWS Lambda/Serverless | Poem / Axum | §6.2 场景化推荐矩阵 | 冷启动敏感场景需实测 |
| 瓶颈在数据库查询 | 框架性能差异无关紧要 | §7.2 反例 1 | 优化重点应在查询、索引、连接池 |
...
- [云原生生态](02_cloud_native.md) — 容器化与微服务部署
- [设计模式](../03_design_patterns/01_patterns.md) — Rust 设计模式
...
axum 的类型安全把大量 Web 错误提前到编译期，但也带来框架特有的编译错误与运行时陷阱。本章覆盖五个代表性边界：

- **处理函数签名约束**：handler 必须实现 `Handler` trait（参数为 extractor、返回 `IntoResponse`），签名错误会触发冗长的 trait bound 报错——学会读这类错误是 axum 开发的必修课。
- **共享状态的 `Clone` 约束**：`State<T>` 要求 `T: Clone`，通常用 `Arc` 包裹；直接传非 `Clone` 的服务句柄是新手最常见的编译错误。
- **body 大小与内存 DoS**：默认 2MB 的 `DefaultBodyLimit` 既是保护也是陷阱——文件上传接口需显式放宽，同时承担内存风险。
- **extractor 顺序与请求体消耗**：`Bytes`/`String` extractor 消耗 body，必须放在参数列表最后，顺序错误在编译期或运行期才暴露。
- 第五项覆盖路由通配符冲突。
...
> **修正**: Axum 的 **extractor** 从请求中提取数据：`Path`、`Query`、`Json`、`Form` 等。
> 请求体（body）是**单次消耗**的流：一个 extractor 读取后，后续 extractor 无法再次读取。
> 规则：
>
> 1) 最多一个 body extractor 每个 handler；
> 2) `Json` 和 `Form` 互斥；
> 3) 若需多次使用 body，先提取为 `Bytes`，再克隆解析。
> 这与 Actix-web（类似 body 限制，但 `web::Json` 和 `web::Form` 同样互斥）或 Rocket（类似，使用 `Data` guard）不同——Axum 的编译期类型安全不保护 body 消耗次数（流性质决定），需运行时检查。axum 0.8+ 的改进：更清晰的错误消息（"body has already been consumed"）。
