...
- [Rust vs C++：形式系统模型 vs 机制工程模型 —— 核心论点索引](#rust-vs-c形式系统模型-vs-机制工程模型--核心论点索引)
  ...
  - [分支B：数学统一层（您骨子里的追问）](#分支b数学统一层您骨子里的追问)
    - [B1：统一框架：从 Rust 所有权（Ownership）到分布式一致性](LINK_PLACEHOLDER)
...
| 形式化边界 | Pin、FFI、循环引用（Reference） | 能力边界 | [`03_advanced/03_unsafe.md`](LINK_PLACEHOLDER) / [`02_intermediate/03_memory_management.md`](LINK_PLACEHOLDER) |
...
| **借用检查** | **区域类型系统（Type System）**（Region Types）的编译期证明 | 引用（`&`）是地址别名，无编译期约束 | 形式证明 vs 语法糖 |
| **生命周期（Lifetimes）** | 形式化的时间逻辑：引用不能比数据活得更久 | 程序员责任：悬垂指针是合法代码 | 逻辑必然性 vs 纪律约束 |
| **并发安全（Concurrency Safety）** | 类型系统禁止数据竞争（Send/Sync trait 作为逻辑标记） | 程序员使用 mutex/atomic 手动同步 | 逻辑不可能性 vs 机制协调 |
...
|:---|:---|:---|
| **词法/语法** | 常规解析 | 常规解析 |
| **语义分析** | **借用检查器**执行形式证明：生命周期、所有权、Send/Sync | **类型检查**确认名称合法，不验证内存安全（Memory Safety） |
...
- Rust 编译器的**借用检查**是矛盾律的工业实现：同一资源不能同时被可变和不可变引用（Mutable Reference）（¬(A ∧ ¬A)）
