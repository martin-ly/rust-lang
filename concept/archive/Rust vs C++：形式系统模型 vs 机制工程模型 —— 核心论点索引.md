...
| **所有权语义** | 逻辑公理（Affine Logic） | `unique_ptr` 运行时惯例 | 编译期定理 vs 运行时机制 |
| **借用检查** | 区域类型系统编译期证明 | 引用是地址别名 | 形式证明 vs 语法糖 |
| **生命周期** | 形式化时间逻辑 | 程序员责任 | 逻辑必然性 vs 纪律约束 |
| **并发安全** | `Send/Sync` 类型级证明 | 无编译期保证 | 逻辑不可能性 vs 机制协调 |
| **错误处理** | `Result<T,E>` ADT 穷尽匹配 | 异常 + 返回值 | 类型代数 vs 运行时跳转 |
| **泛型系统** | Trait 逻辑约束 | Template 代码生成 | 逻辑蕴含 vs 文本替换 |
| **unsafe 含义** | 显式公理突破标记 | 默认无边界 | 安全是公理 vs 控制是默认 |
| **零成本抽象** | 编译期证明消除 | 模板实例化消除 | 证明优化 vs 代码生成 |
...
| 形式化边界 | Pin、FFI、循环引用 | 能力边界 | [`03_advanced/03_unsafe.md`](03_advanced/03_unsafe.md) / [`02_intermediate/03_memory_management.md`](02_intermediate/03_memory_management.md) |
...
- **L1-L3**: 所有权、借用、生命周期、Trait、Unsafe 的具体概念定义
...
> **对应规范文件**: [`05_comparative/01_rust_vs_cpp.md`](05_comparative/01_rust_vs_cpp.md) —— 包含 Wikipedia 权威定义、认知路径、定理一致性矩阵、反命题决策树及完整跨文件链接。
...
> **认知功能**: 该思维导图将 Rust 与 C++ 的本体论差异拆解为可对比的层级结构，帮助读者从"设计哲学"高度理解两种语言的工程选择。建议在学习具体语法前先看此图建立心智模型，避免用 C++ 的机制思维套用 Rust 的形式规则。关键洞察：两种语言的零成本抽象实现路径完全不同——Rust 是编译期证明消除，C++ 是模板实例化消除。[来源: 💡 原创分析]
...
> **认知功能**: 决策树将"Rust vs C++"的抽象对比转化为可操作的选型判断，帮助团队根据核心诉求（数学证明 vs 字节控制）快速定位技术路径。建议在技术评审或架构选型时作为 Checklist 使用，减少主观偏好干扰。关键洞察：两者并非互斥，混合策略（Rust 写新模块 + C++ 写核心引擎）在 2026 年已成为工业常态。[来源: 💡 原创分析]
...
- Rust 编译器的**借用检查**是矛盾律的工业实现：同一资源不能同时被可变和不可变引用（¬(A ∧ ¬A)）
...
- **PG18+ ACID**：最严格的事务约束，支撑最复杂的查询组合（关系代数闭包）
...
|:---|:---|:---|:---|
| 数据库 ↔ 服务 | PG18+ 关系代数 | Rust/Go 类型系统 | ORM 映射的语义泄漏（N+1、阻抗失配） |
| 同步 ↔ 异步 | OpenAPI 请求-响应 | AsyncAPI 发布-订阅 | 同一实体在两种时间模型中的一致性 |
