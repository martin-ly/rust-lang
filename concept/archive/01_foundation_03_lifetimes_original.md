> **Summary**:
>
> Foundation 03 Lifetimes Original. Core Rust concept.
>
# Lifetimes（生命周期）
>
...
- [Lifetimes（生命周期）](#lifetimes生命周期)
  ...
  - [二、概念属性矩阵（Attribute Matrix）](#二概念属性矩阵attribute-matrix)
    - [2.1 生命周期（Lifetimes）标注矩阵](#21-生命周期标注矩阵)
    - [2.2 生命周期（Lifetimes）关系矩阵](#22-生命周期关系矩阵)
  ...
  - [四、定理推理链（Theorem Chain）](#四定理推理链theorem-chain)
    ...
    - [4.3 定理：函数签名中的生命周期省略（Lifetime Elision）规则 ⟹ Elision 的完备性](#43-定理函数签名中的生命周期省略规则--elision-的完备性)
    ...
    - [4.8 推论：GATs + where Self: 'a ⟹ 自引用（Reference）集合的表达能力](#48-推论gats--where-self-a--自引用集合的表达能力)
  ...
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    - [测验 1：《Lifetimes（生命周期）》是一份归档文件。归档文件在知识体系中有什么作用？（理解层）](#测验-1lifetimes生命周期是一份归档文件归档文件在知识体系中有什么作用理解层)
...
> **过渡**: 权威定义从学术和官方来源确立了生命周期的语义——引用（Reference）有效期的编译期保证。而概念属性矩阵则将这些语义转化为可操作的规则对比——`'a` 标注的不同形式、生命周期关系的推导规则、以及它们与所有权（Ownership）、借用（Borrowing）系统的交互约束。
...
> **[来源: RFC 2094]** NLL 将生命周期从词法作用域扩展到基于数据流的实际使用期，减少不必要的借用（Borrowing）冲突。✅
...
>
> 很多程序员在 C/C++ 中习以为常地返回局部指针，却在 Rust 中遭遇编译拒绝。这种困惑源于将"引用（Reference）"理解为无成本的轻量指针，而忽略了引用背后的**时效契约**——它必须指向在特定时段内保证存活的数据。
...
>
> 边界测试回答：'static 可以通过泄漏安全地构造吗？HRTB 的闭包（Closures）能接受多短的引用？NLL 在循环引用交叉时是否仍然精确？通过刻意构造极限代码，验证定理在极端条件下的行为，完成从"学习规则"到"驾驭规则"的跃迁。
>
...
**关键洞察**: Rust 是唯一将生命周期作为**显式、强制、核心类型系统特征**的工业级主流语言。C++ 依赖运行时（Runtime） RAII 和程序员自律；Haskell LinearTypes 提供了类似的编译期保证但尚未成为主流实践；Go 完全依赖 GC。
...
Elision 不是语法便捷性的简单堆砌，而是一组基于 Hindley-Milner 风格模式匹配（Pattern Matching）的完备推导规则。以下给出三条规则在函数签名层面的形式化定义，并证明其 soundness。
...
**跨层映射**: 本章节 APIT/RPIT 语义 ↔ [`./04_type_system.md`](../01_foundation/02_type_system/04_type_system.md) §11 "类型系统前沿" · [`../02_intermediate/01_generics/02_generics.md`](../02_intermediate/01_generics/02_generics.md) §4.1 "泛型参数推断"
...
> **一致性（Coherence）检查**: union 是 Rust 类型系统的"逃生舱"——它在 `unsafe` 边界内提供了 C 兼容的零成本类型双关，但将类型安全的全部责任转移给程序员。Safe Rust 的默认选择应是 `enum`。✅
...
### 测验 1：《Lifetimes（生命周期）》是一份归档文件。归档文件在知识体系中有什么作用？（理解层）

**题目**: 《Lifetimes（生命周期）》是一份归档文件。归档文件在知识体系中有什么作用？
