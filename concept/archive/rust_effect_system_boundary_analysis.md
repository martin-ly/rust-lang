...
- [Rust 效应系统（Effect System）与其语言边界的分析论证](#rust-效应系统effect-system与其语言边界的分析论证)
  ...
  - [三、Rust 的内存安全（Memory Safety）效应：所有权与借用（Borrowing）](#三rust-的内存安全效应所有权与借用)
...
| 语言/系统 | 效应模型 | 所有权（Ownership）/能力 | 运行时成本 | 用户可扩展性 | 与 Rust 的边界关系 |
|-----------|----------|-------------|------------|--------------|-------------------|
...
这形成了 Rust 的**安全边界定理**：safe Rust 的程序行为仅依赖于 safe API 的契约，unsafe 实现的内部细节被模块化隔离。
