...
- [Rust 效应系统（Effect System）与其语言边界的全面深化广化分析论证](#rust-效应系统effect-system与其语言边界的全面深化广化分析论证)
  ...
  - [第一章：概念定义总表（Alphabetical Encyclopedia）](#第一章概念定义总表alphabetical-encyclopedia)
    ...
    - [1.3 Borrow Checker（借用（Borrowing）检查器）](LINK_PLACEHOLDER)
  ...
  - [第五章：跨语言代码示例对比矩阵](#第五章跨语言代码示例对比矩阵)
    ...
    - [5.2 异步（Async）编程对比](LINK_PLACEHOLDER)
  ...
  - [第八章：工业实践案例深化](#第八章工业实践案例深化)
    ...
    - [8.3 Rust for Linux：内核模块（Module）的安全标注](LINK_PLACEHOLDER)
  ...
  - [第十章：结论与三重边界的最终形式化](#第十章结论与三重边界的最终形式化)
    - [10.1 边界一：Carried vs Uncarried 类型系统（Type System）断层](LINK_PLACEHOLDER)
    - [10.2 边界二：零成本抽象（Zero-Cost Abstraction）与通用效应处理器的不相容性](LINK_PLACEHOLDER)
...
在编译期验证所有借用（Borrowing）（引用）遵守所有权纪律：任意时刻，对任何数据，要么存在一个可变引用（Mutable Reference），要么存在任意数量的不可变引用（Immutable Reference），二者不可兼得。
...
| Rust 支持 | 无 | 零成本抽象（Zero-Cost Abstraction）禁止运行时捕获 |
...
**【定义】**:
Rust 中引用有效的作用域标注。生命周期（Lifetimes） `'a` 是一个**区域（region）**——程序执行中的一段代码范围。借用检查器通过约束求解验证所有引用在其生命周期内始终有效。
...
| 1973 | 类型理论 | ML 类型推断（Milner） | Hindley-Milner 类型系统（Type System） |
