...
- [Rust vs C++：形式系统模型 vs 机制工程模型 —— 核心论点索引](#rust-vs-c形式系统模型-vs-机制工程模型--核心论点索引)
  ...
  - [四、System-Level 突破：分布式形式化从设计时到运行时](#四system-level-突破分布式形式化从设计时到运行时)
    - [4.1 TLA+：设计时的 exhaustive 证明](#41-tla设计时的-exhaustive-证明)
    - [4.2 P 语言：事件驱动系统的形式化 + Rust 运行时对齐](#42-p-语言事件驱动系统的形式化--rust-运行时对齐)
    - [4.3 运行时一致性检查的演进谱系](#43-运行时一致性检查的演进谱系)
  ...
  - [分支B：数学统一层（您骨子里的追问）](#分支b数学统一层您骨子里的追问)
    - [B1：统一框架：从 Rust 所有权到分布式一致性](#b1统一框架从-rust-所有权到分布式一致性)
