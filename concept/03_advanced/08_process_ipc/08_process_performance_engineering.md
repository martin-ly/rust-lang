...
进程级性能优化的三个方向，按收益/成本比排列：

1. **减少进程创建开销（4.1）**：fork+exec 在 Linux 上约 100μs 量级、Windows 的 `CreateProcess` 更慢（毫秒级）——高频创建场景用进程池（预 spawn + 复用）、`posix_spawn`（避免 fork 拷贝页表，musl/glibc 均支持）、或干脆重构为线程/任务（进程是隔离手段不是并发原语）；
2. **I/O 优化（4.2）**：管道缓冲区（Linux 默认 64KB）决定吞吐——大批量数据用 `BufReader`/`BufWriter` 包装子进程标准流，避免同步往返（写一段等一段）；双向通信警惕「双方都等对方写」的死锁；
3. **零拷贝 IPC（4.3）**：共享内存（`shm` + 同步原语）与 `sendmsg` 的 fd 传递（Unix）是大规模数据交换的终极方案——Rust 的 `nix`/`passfd` crate 提供类型安全封装，但同步协议（谁写谁读何时读）仍需自行设计。

优化纪律：先 profile 进程边界处的等待时间（`strace -c`/`procmon`），进程内优化通常收益远高于进程间优化。
