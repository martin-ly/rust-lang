...
| [02_async.md](01_async/01_async.md) | 异步（Async）编程 | `Future`、`async/await`、`Pin`、AFIT/RPITIT、运行时（Runtime） | ✅ v1.0 | Generics + Trait + Pin (L2) | 形式化 (Pin 语义), 生态 (Tokio) |
...
| 异步（Async） | 能用 `tokio::spawn` + `async/await` 实现并发 HTTP 请求，理解 `Future` 轮询 | 回到 [L3 异步](01_async/01_async.md) |
| Unsafe | 能写出 `unsafe` 块并说明为什么这是安全的（Safety Contract） | 回到 [L3 Unsafe](02_unsafe/01_unsafe.md) |
| 生命周期（Lifetimes） | 能标注含多个引用（Reference）的函数签名，理解 HRTB | 回到 [L1 生命周期高级主题](../01_foundation/01_ownership_borrow_lifetime/04_lifetimes_advanced.md) |
| Trait 与泛型（Generics） | 能解释 `Send`/`Sync` 为何是 auto trait，并为泛型 Future 写出正确约束 | 回到 [L2 Traits](../02_intermediate/00_traits/01_traits.md) · [L2 泛型](../02_intermediate/01_generics/01_generics.md) |
| 内存管理 | 能说明 `Arc`/`Mutex` 与 `Pin` 的所有权（Ownership）语义 | 回到 [L2 内存管理](../02_intermediate/02_memory_management/01_memory_management.md) |
...
- ⏸️ **留在 L3 巩固**：如果你仍有上述检查项未达标，建议先完成 [MVP 学习路径](../00_meta/04_navigation/08_learning_mvp_path.md) 的 Week 2
