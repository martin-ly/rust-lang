...
>
> **来源**: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html) · [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html) · [Async Book](https://rust-lang.github.io/async-book/index.html)
> **前置概念**: N/A
> **后置概念**: N/A
...
> **认知功能**: 全局概念导航图——在学习具体子主题前建立"并发-异步（Async）-Unsafe-宏（Macro）"四域心智框架。
> 使用建议：将本图作为目录锚点，定位感兴趣的概念后再深入对应文件阅读。
> 关键洞察：L3 并非新增孤立语法，而是 L1 所有权（Ownership）与 L2 Trait 在复杂场景中的组合应用与边界突破。[💡 原创分析](../00_meta/00_framework/methodology.md)
...
> 关键洞察：Unsafe 处于中心辐射位——它是突破 safe 边界的唯一通道，也是所有 L3 概念通向形式化验证的必经枢纽。[💡 原创分析](../00_meta/00_framework/methodology.md)
...
| 2 | **Unsafe** | **所有 safe 概念** | `-.->` 边界/突破 | `unsafe` 是 Rust 安全保证的**边界**。在 unsafe 块内，L1 的所有权（Ownership）规则、L2 的 Trait 规则均可能被手动突破。 |
| 3 | **L2 Send/Sync** | **Concurrency** | `==>` 充分条件 | `Send + Sync` 是编译期对并发安全（Concurrency Safety）的**充分条件**（非必要：可通过 unsafe 手动实现）。 |
| 4 | **L2 Pin** | **Async** | `-.->` 前置 | 自引用（Reference）结构（如 async 状态机）需要 `Pin` 保证内存位置不动，否则引用失效。 |
...
|:---|:---|:---|:---|:---|:---|
| [01_concurrency.md](00_concurrency/01_concurrency.md) | 并发模型 | `Send`/`Sync`、fearless concurrency、同步原语、原子操作（Atomic Operations） | ✅ v1.0 | Ownership + Borrowing + Trait (Auto) | RustBelt (并发验证), AI (并发代码生成) |
| [02_async.md](01_async/02_async.md) | 异步（Async）编程 | `Future`、`async/await`、`Pin`、AFIT/RPITIT、运行时（Runtime） | ✅ v1.0 | Generics + Trait + Pin (L2) | 形式化 (Pin 语义), 生态 (Tokio) |
| [03_unsafe.md](02_unsafe/03_unsafe.md) | Unsafe Rust | 裸指针、FFI、UB 边界、Safety 契约、Miri | ✅ v1.0 | 所有 L1-L2 概念 | RustBelt (unsafe 验证), C++ 对比 |
| [04_macros.md](03_proc_macros/04_macros.md) | 宏系统 | `macro_rules!`、过程宏（Procedural Macro）、DSL、卫生宏 | ✅ v1.0 | Type System + Trait | 生态 (代码生成), AI (模板生成) |
| [05_rust_ffi.md](04_ffi/05_rust_ffi.md) | FFI 跨语言 | extern 块、ABI 兼容、类型映射、bindgen、回调封装 | ✅ v1.0 | Type System + Unsafe | 生态 (跨语言), C++ 对比 |
| 06_pin_unpin.md | Pin 与 Unpin | 自引用（Reference）类型、PhantomPinned、async 状态机、Pin API 契约 | ✅ v1.0 | Ownership + Generics | Async, Gen Blocks |
| [07_proc_macro.md](03_proc_macros/07_proc_macro.md) | 过程宏（Procedural Macro） | Derive/Attribute/Function-like、TokenStream、syn/quote | ✅ v1.0 | Trait + Type System | 生态 (代码生成), DSL |
| [08_nll_and_polonius.md](02_unsafe/08_nll_and_polonius.md) | NLL 与 Polonius | 非词法生命周期、数据流分析、Origin 模型、借用检查演进 | ✅ v1.0 | Borrowing, Lifetimes | RustBelt, Pin |
| [19_parallel_distributed_pattern_spectrum.md](00_concurrency/19_parallel_distributed_pattern_spectrum.md) | 并行-分布式谱系 | 线程池→工作窃取→Actor→CSP→数据流→共识的连续体 | ✅ v1.0 | Concurrency, Async | Distributed Systems |
| [20_stream_processing_semantics.md](06_low_level_patterns/20_stream_processing_semantics.md) | 流处理语义 | Dataflow Model、Watermark、Exactly-Once、Differential Dataflow | ✅ v1.0 | Concurrency, Async | Stream Processing Ecosystem |

---

### 补充文件索引

- [Async/Await 高级主题](01_async/25_async_advanced.md)
- [异步（Async）模式：从 Future 到生产级并发](01_async/26_async_patterns.md)
- [Async Closures（异步闭包）](01_async/24_async_closures.md)
- [FFI 高级主题：跨语言边界的安全与性能](04_ffi/09_ffi_advanced.md)
- [并发 模式：从消息 传递到锁自由的数据结构](00_concurrency/10_concurrency_patterns.md)
- 原子操作（Atomic Operations）与内存序：无锁并发的精确控制
- [Unsafe Rust 模式：安全抽象的核心技术](02_unsafe/12_unsafe_rust_patterns.md)
- 内联汇编（Inline Assembly）：`asm!` 宏（Macro）与跨平台 SIMD
- [自定义分配器与内存布局优化](06_low_level_patterns/14_custom_allocators.md)
- [零拷贝解析与序列化优化](06_low_level_patterns/15_zero_copy_parsing.md)
- [无锁编程与内存模型](00_concurrency/16_lock_free.md)
- [类型擦除与动态分发](06_low_level_patterns/17_type_erasure.md)
- [测验：并发与异步（Async）（嵌入式互动试点）](00_concurrency/21_quiz_concurrency_async.md)
- [测验：Unsafe Rust（嵌入式互动试点）](02_unsafe/22_quiz_unsafe.md)
- [测验：宏（Macro）系统（嵌入式互动试点）](03_proc_macros/23_quiz_macros.md)
...
| 检查项 | 自检标准 | 若未达标 |
|:---|:---|:---|
| 并发 | 能独立编写含 `Mutex`/`Arc`/`Channel` 的多线程程序，并解释死锁原因 | 回到 [L3 并发](00_concurrency/01_concurrency.md) |
| 异步（Async） | 能用 `tokio::spawn` + `async/await` 实现并发 HTTP 请求，理解 `Future` 轮询 | 回到 [L3 异步](01_async/02_async.md) |
| Unsafe | 能写出 `unsafe` 块并说明为什么这是安全的（Safety Contract） | 回到 [L3 Unsafe](02_unsafe/03_unsafe.md) |
| 生命周期（Lifetimes） | 能标注含多个引用（Reference）的函数签名，理解 HRTB | 回到 [L1 生命周期高级主题](../01_foundation/01_ownership_borrow_lifetime/30_lifetimes_advanced.md) |
...
- ⏸️ **留在 L3 巩固**：如果你仍有上述检查项未达标，建议先完成 [MVP 学习路径](../00_meta/04_navigation/learning_mvp_path.md) 的 Week 2
...
- **L4 形式化**: RustBelt（并发安全（Concurrency Safety）验证）、Pin 不动性形式化、unsafe 语义
- **L5 对比**: Rust vs C++（unsafe vs 无约束）、Rust vs Go（async vs goroutine）
- **L6 生态**: Tokio/Async-std 运行时（Runtime）、unsafe 代码审查规范
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：《L3 高级概念层（Advanced）》在本知识体系中扮演什…、测验 2：使用本索引文件时，最有效的学习策略是什么？（理解层）与测验 3：索引文档能否替代具体概念文件的学习？（理解层），本节依次展开。
