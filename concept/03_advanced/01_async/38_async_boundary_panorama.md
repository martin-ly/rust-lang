...
> **分工声明**: async 概念推导（状态机变换、Future trait、语法糖展开）留在 [Async/Await](02_async.md)；取消安全的系统化形式化留在 [Async 取消安全](37_async_cancellation_safety.md)；Pin 机制推导留在 [Pin 与 Unpin](06_pin_unpin.md)。本页只做**边界视角的全景汇总**，不重复概念推导（AGENTS.md §2 Canonical 规则）。
...
> **全局对应**: 本页是 [安全边界全景](../../05_comparative/03_domain_comparisons/04_safety_boundaries.md) 在 async 域的纵深展开；unsafe 域的对应页为 [Unsafe 边界全景](../02_unsafe/32_unsafe_boundary_panorama.md)。
>
...
> **前置概念**: [Async/Await](02_async.md) · [Pin 与 Unpin](06_pin_unpin.md) · [Future 与 Executor 机制](39_future_and_executor_mechanisms.md)
> **后置概念**: [Async 取消安全](37_async_cancellation_safety.md) · [Async Drop（预览）](../../07_future/03_preview_features/18_async_drop_preview.md) · [Unsafe 边界全景](../02_unsafe/32_unsafe_boundary_panorama.md)
...
> 系统化形式化（drop 即取消、操作不变量分类、Tokio API 安全性判定）见权威页 [Async 取消安全](37_async_cancellation_safety.md)；本节只做边界全景定位。
...
> **修复策略**: 用 `Framed`/缓冲读取保证消息原子性；把状态推进到 await 之后；或引入显式 checkpoint（见 [37](37_async_cancellation_safety.md) 的修正模式目录）。
...
> **修复策略**: 首选 `pin!` 宏或 `Box::pin`；手写自引用结构用 `pin-project`/`pin-project-lite`；详见 [Pin 与 Unpin](06_pin_unpin.md) 与 [39](39_future_and_executor_mechanisms.md)。
...
> **修复策略**: `Rc`→`Arc`；缩短非 Send 值作用域使其不跨 await；持锁不跨 await（见 [Async 模式](26_async_patterns.md)）；确实单线程的场景用 `LocalSet`。
...
> **修复策略**: 阻塞工作移入 `spawn_blocking`；用 `Handle::enter`/`Runtime::block_on` 建立上下文；长循环插 `tokio::task::yield_now().await`；详见 [Future 与 Executor 机制](39_future_and_executor_mechanisms.md)。
...
> **修复策略**: 热路径用泛型 + 静态分发；插件/扩展点用 `async_trait` 或手写 boxed future；关注 `dynosaur`/RTN（Return Type Notation，预览）演进，见 [Async 高级主题](25_async_advanced.md)。
...
| 文件 | 职责 | 本页引用方式 |
|:---|:---|:---|
| [02_async.md](02_async.md) | async/await 概念推导、语法糖展开、Future trait | 概念定义的来源，本页不重复 |
| [37_async_cancellation_safety.md](37_async_cancellation_safety.md) | 取消安全的形式化、Tokio API 安全性判定目录 | §五 的全部深入内容指向该页 |
| [06_pin_unpin.md](06_pin_unpin.md) | Pin/Unpin 机制推导、投影规则 | §六 的机制细节指向该页 |
| [39_future_and_executor_mechanisms.md](39_future_and_executor_mechanisms.md) | executor/waker 机制 | §八 的机制细节指向该页 |
| [25_async_advanced.md](25_async_advanced.md) / [26_async_patterns.md](26_async_patterns.md) | 高级主题与模式 | 修复策略的模式目录 |
| [04_safety_boundaries.md](../../05_comparative/03_domain_comparisons/04_safety_boundaries.md) | 全局安全边界全景 | 本页是其在 async 域的纵深 |
| [32_unsafe_boundary_panorama.md](../02_unsafe/32_unsafe_boundary_panorama.md) | unsafe 域边界全景 | Pin/FFI 边界交汇（自引用结构的手写实现） |

> **双向链接**: [04_safety_boundaries.md](../../05_comparative/03_domain_comparisons/04_safety_boundaries.md) 与 [02_async.md](02_async.md) 均已回链本页。

---

## 十三、待补充与演进方向

- **Async Drop（预览）**: drop 语义与 await 的交汇将新增"析构边界"，跟踪 [18_async_drop_preview.md](../../07_future/03_preview_features/18_async_drop_preview.md)。
