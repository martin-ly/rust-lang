...
3. **取消点可枚举**（1.3）：状态机只在 `.await` 处可被取消——两个 await 之间的代码是原子的（对取消而言）。这给出取消安全审计的可操作方法：列出所有 await 点，逐点问「在此处 drop 会留下什么状态」。

后续小节给出状态机枚举实例，把「取消点枚举」从原则落到代码。
...
取消安全（cancellation safety）的形式化需要一个操作级定义（2.1）：**操作 O 是取消安全的，当且仅当在任意 await 点取消 O 后，系统状态等价于「O 从未开始」或「O 已完成」之一**——不存在可观察的「半截」状态。
...
本节以五个真实陷阱演示取消安全审计的操作方法，每个给出「触发时序 → 失败状态 → 修正模式」：

- **`BufReader::read_line` 在 select 循环中丢数据**（3.1）：await 时已读入缓冲区的字节在取消时随 Future 丢弃——修正是把 `BufReader` 与缓冲区提升到循环外，只把「读操作」放进 select；
- **持锁跨 await 被取消**（3.2）：`Mutex` guard 在 await 点被取消时临界区半截——修正是 `tokio::sync::Mutex`（取消时 guard drop 正常解锁）+ 确保状态修改原子化；
- **事务半截状态**（3.3）：多步写入在 await 间取消——修正是「prepare → select → commit」模式，副作用只在确认后进行；
- **`oneshot` 接收端落选**（3.4）：发送端的值随 channel 关闭而丢失——发送方需处理 `SendError` 退回的值；
- **select 中反复构造 Future**（3.5）：每轮循环新建 Future 导致状态重置与分配开销——修正是 `pin!` 到循环外复用。

共同模式：**把可变状态移出可取消区域，await 只做不可变等待**。
...
```mermaid
mindmap
  root((Async 取消安全Cancellation))
    一、取消的语义drop future 意味着什么
      1.1 取消不是停止执行，而是不再 poll
      1.2 取消的三个触发源
      1.3 状态机视角取消点是可枚举的
    二、取消安全的形式化定义
      2.1 操作级定义
      2.2 tokioselect! 文档约定的精确含义
      2.3 select! 分支落选的语义细节
    六、取消与 drop 顺序、async_drop
      6.1 Drop 顺序在取消路径上的保证
      6.2 同步 Drop 的天花板
    九、来源
```
