...
本节剖析 work-stealing 执行器的 LIFO 槽：3.1 解释其缓存局部性收益的机制，3.2 给出运行时内置的公平性阀门。
...
调度公平性的类型级前提：多线程 worker 可窃取任何任务 ⟹ 任务必须 `Send`。类型系统把这条调度不变量提前到编译期：
...
若确实需要非 Send 状态：用 `LocalSet` + `spawn_local`（钉在当前线程，不参与窃取——公平性语义随之改变：该任务只能被本线程调度，饥饿风险上升），或把非 Send 状态包进 `Mutex`/`spawn_blocking` 闭包。
...
- [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) — 任务状态机的堆分配与迁移代价（L2 向下引用）
