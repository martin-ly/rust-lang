...
本节把「Async Closures（异步闭包）」组织成一条从直观到形式化的认知路径：核心推理链 与 反命题与边界。起点是该主题最常见的使用场景，终点是它与类型系统/所有权模型的形式化衔接；沿途每一步都标注「为什么必须如此」的推理依据。按路径完整走一遍，应能独立复现本页的全部结论。
...
本节练习围绕练习 1：改写 与 练习 2：判断正误设计，难度从改写（apply）递进到判断正误（analyze）。每题给出明确的判定标准：改写题要求代码通过 rustc 1.97 编译并满足题述约束，判断题要求引用具体语言规则说明理由，不接受「凭感觉」。建议先预测结果再动手，最后用编译器或运行结果验证自己的判断。
