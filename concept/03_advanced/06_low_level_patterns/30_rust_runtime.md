...
> **前置依赖**: [Memory Management](../../02_intermediate/02_memory_management/03_memory_management.md) · [Custom Allocators](14_custom_allocators.md) · [Linkage](../04_ffi/27_linkage.md)
> **后置概念**: [Embedded Systems](../../06_ecosystem/05_systems_and_embedded/22_embedded_systems.md) · [Unsafe Rust](../02_unsafe/03_unsafe.md) · [Panic](../../02_intermediate/03_error_handling/31_panic.md)
...
1. **问题识别**: 为什么 Rust 运行时（Runtime）值得关注？运行时属性决定内存分配、Windows 子系统和 panic 行为，影响系统级和嵌入式编程。
...
Rust 运行时（Runtime）指程序运行期间由语言定义或依赖的基础设施行为。与其他一些语言不同，Rust 的运行时非常轻量，大部分核心行为（如内存分配）都可以通过属性进行定制或替换。(Source: [Rust Reference — The Rust Runtime](https://doc.rust-lang.org/reference/runtime.html))
...
Rust 支持两种 panic 运行时策略，通过 `Cargo.toml` 的 `[profile]` 段配置：(Source: [Cargo Reference — Profiles](https://doc.rust-lang.org/cargo/reference/profiles.html#panic))
...
> **注意**: 即使使用 `panic = "abort"`，`unsafe` 代码仍需保持内存安全（Memory Safety）不变式。
...
Windows 平台下，`#[windows_subsystem]` 控制可执行文件的子系统：(Source: [Rust Reference — The Rust Runtime](https://doc.rust-lang.org/reference/runtime.html#the-windows_subsystem-attribute))
...
`#[global_allocator]` 用于选择程序的**全局内存分配器**。(Source: [Rust Reference — The Rust Runtime](https://doc.rust-lang.org/reference/runtime.html#global-allocator), [std::alloc::GlobalAlloc](https://doc.rust-lang.org/std/alloc/trait.GlobalAlloc.html))
...
详见 [Panic](../../02_intermediate/03_error_handling/31_panic.md)。
...
| 概念 | 关系 |
|:---|:---|
| [Custom Allocators](14_custom_allocators.md) | `#[global_allocator]` 是自定义分配器的入口 |
...
| [Unsafe Rust](../02_unsafe/03_unsafe.md) | `GlobalAlloc` 实现需要 `unsafe` |
| [Panic](../../02_intermediate/03_error_handling/31_panic.md) | panic 策略是运行时核心部分 |
| [Memory Model](../02_unsafe/29_memory_model.md) | 分配器行为受内存模型约束 |
