...
> **前置依赖**: [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) · [Custom Allocators](01_custom_allocators.md) · [Linkage](../04_ffi/03_linkage.md)
...
2. **概念建立**: 掌握 `#[global_allocator]`、`#[windows_subsystem]`、panic 运行时（Runtime）和启动流程的核心概念。
3. **机制推理**: 通过 ⟹ 定理链将运行时（Runtime）属性、分配行为和二进制输出串联起来。
...
详见 [Panic](../../02_intermediate/03_error_handling/03_panic.md)。
...
更多细节参见 [Behavior Considered Undefined](../../04_formal/01_ownership_logic/06_behavior_considered_undefined.md)。
...
| 概念 | 关系 |
|:---|:---|
| [Custom Allocators](01_custom_allocators.md) | `#[global_allocator]` 是自定义分配器的入口 |
| [Linkage](../04_ffi/03_linkage.md) | 运行时属性影响链接器行为 |
| [Application Binary Interface](../../04_formal/05_rustc_internals/05_application_binary_interface.md) | ABI 属性与运行时属性共同决定二进制输出 |
| [Embedded Systems](../../06_ecosystem/05_systems_and_embedded/03_embedded_systems.md) | 嵌入式场景经常需要替换全局分配器 |
| [Unsafe Rust](../02_unsafe/01_unsafe.md) | `GlobalAlloc` 实现需要 `unsafe` |
| [Panic](../../02_intermediate/03_error_handling/03_panic.md) | panic 策略是运行时核心部分 |
| [Memory Model](../02_unsafe/06_memory_model.md) | 分配器行为受内存模型约束 |
