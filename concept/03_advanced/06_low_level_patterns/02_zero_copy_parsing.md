...
- **反命题 1：「零拷贝 = 用 `transmute` 把字节变结构体」** —— 危险且多数错误。`transmute` 路线需要同时满足：对齐（输入缓冲区地址对齐到类型要求）、尺寸（输入长度精确匹配）、有效位模式（所有字节组合对该类型合法，如 `bool` 只有 0/1 合法）、字节序（多字节类型的端序转换）。四条件缺一即 UB——正确工具是 `bytemuck`（`Pod`/`Zeroable` 用类型约束编码前三条）或 `zerocopy`；
