...
- **Chandy-Lamport 分布式快照**：1985 年的经典算法——协调者注入 marker，各节点「收到 marker 时记录本地状态 + 此后该通道的消息入快照」，全局快照 = 所有本地状态 + 在途消息的一致性（Coherence）切割（consistent cut：不存在「消息被接收记录但发送未记录」的因果倒挂）。Flink 的 checkpoint 即其工程化（barrier 注入 + 状态后端持久化 + 失败时从最近快照重放）。
...
- **共享状态管理器的并发访问**：多任务直接共享状态句柄绕过框架——Rust 中以编译错误形式暴露（`!Sync`），对应 Java 生态的运行时（Runtime）竞态；
...
本节用 Rust 特有的编译错误展示「流处理规则如何被类型系统（Type System）编码」：

- **Tokio Stream 与所有权冲突**：`stream::unfold` 的状态闭包（Closures）按值捕获，试图共享可变状态触发 E0506/E0524——正确模式是 `Arc<Mutex<_>>` 或把状态内聚到 unfold 的 state 参数；
- **背压传播中的类型不匹配**：`Stream`/`Sink` 组合时 item 类型链必须贯通——`map` 后的类型与下游 `sink` 的输入类型不符触发 E0271，错误信息中的 trait 求解链即数据流类型检查器；
- **`fuse` 与重复 poll**：`Stream::next` 返回 `None` 后继续 poll 是未定义语义——`FusedStream` 用类型标记承诺耗尽后幂等，`select!` 宏（Macro）内部依赖此承诺。
...
- **依赖**：概念上依赖 [异步（Async）模式](../01_async/03_async_patterns.md) 的流抽象。
...
```mermaid
mindmap
  root((流处理语义从 Dataflow Model 到))
    一、核心命题流处理的本质
    二、时间域事件时间 vs 处理时间 vs 摄取时间
      2.1 三种时间语义
      2.2 事件时间的不可替代性
    三、窗口语义在事件时间中划界
      3.1 窗口类型形式化
      3.2 窗口与乱序数据
    四、Watermark事件时间进度的推断机制
      4.1 Watermark 的形式化定义
      4.2 Watermark 的两种失败模式
    五、Trigger结果物化的时机控制
      5.1 Trigger 类型
      5.2 累积模式Accumulation
```
