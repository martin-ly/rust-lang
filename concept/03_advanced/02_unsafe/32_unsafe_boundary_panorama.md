...
> **定位**: 从 [Unsafe Rust](03_unsafe.md) 中抽离**边界视角**：不做 unsafe 概念推导，只回答五类边界问题——什么算 UB、别名模型管到哪、Miri 能查到什么、安全抽象的契约划在哪、FFI 布局契约何时成立。每一节给出**边界陈述 → 反例 → 判定条件**三段式。
> **分工声明**: unsafe 的概念推导（五类 unsafe 操作、unsafe fn/trait、SAFETY 注释规范、完整示例）留在 [Unsafe Rust](03_unsafe.md)；unsafe 工程模式留在 [Unsafe Rust 模式](12_unsafe_rust_patterns.md)；Miri/Kani 工具链推导留在 [Miri](../../04_formal/04_model_checking/31_miri.md) 与 [Kani](../../04_formal/04_model_checking/32_kani.md)。本页只做**边界全景汇总**，不重复概念推导（AGENTS.md §2 Canonical 规则）。
...
> **修复策略**: 无效值用 `MaybeUninit` + 逐字段初始化；悬垂用所有权重构替代裸指针；详见 [03_unsafe.md](03_unsafe.md) §八与 [Unsafe Rust 模式](12_unsafe_rust_patterns.md)。
...
> **修复策略**: 重借用用"从最新引用再派生"；裸指针运算限制在单分配内（`wrapping_*` 不换分配）；用 `NonNull`/`addr_of!` 减少中间引用；验证走 [Miri](../../04_formal/04_model_checking/31_miri.md)（默认 Tree Borrows）。
...
> **修复策略**: 用 `cargo miri test` + `cargo kani` 组合（动态覆盖 + 有界全输入）；FFI 侧用 cbindgen/ABI 测试补位；工具细节见 [31_miri.md](../../04_formal/04_model_checking/31_miri.md)。
...
> **修复策略**: 不变量封装进私有字段 + 构造器校验；`get_unchecked` 类 API 一律 `unsafe fn`；用 [Safety Tags/模式](12_unsafe_rust_patterns.md) 规范化文档；`unsafe impl Send/Sync` 必须逐条论证（见 [并发](../../03_advanced/00_concurrency/01_concurrency.md) 的 auto trait 章）。
...
> **修复策略**: 边界类型一律 `#[repr(C)]`/`transparent`；用 `cbindgen`/`bindgen` 从单一事实源生成两侧声明；枚举用整数 + 转换层；回调包 `catch_unwind`；详见 [03_unsafe.md](03_unsafe.md) §九（Unsafe/FFI 2024：权限分离与显式契约）与 [Rust FFI](../04_ffi/05_rust_ffi.md) · [FFI Advanced](../04_ffi/09_ffi_advanced.md)。
...
## 十一、与 03_unsafe.md 的分工与交叉引用

| 文件 | 职责 | 本页引用方式 |
|:---|:---|:---|
| [03_unsafe.md](03_unsafe.md) | unsafe 概念推导：五类 unsafe 操作、unsafe fn/trait、SAFETY 注释规范、完整示例与形式化根基 | 概念定义与示例的来源，本页不重复推导 |
| [12_unsafe_rust_patterns.md](12_unsafe_rust_patterns.md) | 安全抽象的工程模式（Safety Tags 等） | §七 修复策略的模式目录 |
| [29_memory_model.md](29_memory_model.md) | 内存模型（Ordering、happens-before） | 数据竞争边界的机制细节 |
| [08_nll_and_polonius.md](08_nll_and_polonius.md) | 借用检查器演进 | §五 别名模型的 safe 侧对应 |
