...
> **双维定位**: S×Ana — 规范分析
> **前置依赖**: [Unsafe Rust](03_unsafe.md) · [Behavior Considered Undefined](../../04_formal/01_ownership_logic/37_behavior_considered_undefined.md) · [Variables](../06_low_level_patterns/33_variables.md)
> **后置概念**: [Inline Assembly](../05_inline_assembly/13_inline_assembly.md) · [FFI Advanced](../04_ffi/09_ffi_advanced.md) · [Custom Allocators](../06_low_level_patterns/14_custom_allocators.md)
...
5. **迁移应用**: 将 Unsafe 参考与内存模型、运行时、FFI 和内联汇编链接，形成跨层知识网络。
...
> **反命题 3**: "其他语言对 unsafe 的处理方式可以直接迁移到 Rust" ⟹ 不成立。Rust 的 `unsafe` 块不禁用借用检查器，只是放宽特定操作限制。
...
`unsafe` 代码必须避免 UB。完整 UB 清单见 [Behavior Considered Undefined](../../04_formal/01_ownership_logic/37_behavior_considered_undefined.md)。
...
详见 [Memory Model](29_memory_model.md)。
