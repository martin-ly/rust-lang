...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Borrowing](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) · [Memory Management](../../02_intermediate/02_memory_management/03_memory_management.md) · [Concurrency](../00_concurrency/01_concurrency.md)
...
>
> **内容去重提示**: 本文是 Unsafe Rust 的 `concept/` 权威来源。[`knowledge/03_advanced/unsafe/04_unsafe_rust.md`](../../../knowledge/03_advanced/unsafe/04_unsafe_rust.md) 与 [`concept/03_advanced/02_unsafe/12_unsafe_rust_patterns.md`](12_unsafe_rust_patterns.md) 已重定向至本文件，相关内容已在本文件中统一维护。
...
> **[Rustonomicon](https://doc.rust-lang.org/nomicon/)** A block of code prefixed with `unsafe` does not permit the writing of arbitrary code. The `unsafe` keyword has two meanings: it declares the existence of a contract the compiler doesn't know about, and it declares that you have verified that contract.
...
> **来源: [Rust Reference: Unsafe Rust; Rustonomicon](https://doc.rust-lang.org/reference/)** Unsafe 操作分为 7 类，每类有明确的安全契约。
...
> **来源: [Rust Reference: Behavior considered undefined; The Rust Unsafe Code Guidelines (UCG) Book; Ralf Jung Blog](https://doc.rust-lang.org/reference/)** Rust 的 UB 清单不是封闭的——随着编译器优化假设的演进，新的 UB 类别可能被加入。以下分类基于 Rust Reference 和 UCG 的最权威定义。
...
> **来源: [Rust Reference: Invalid values](https://doc.rust-lang.org/reference/)** 类型系统（Type System）类 UB 涉及编译器对值表示的优化假设。
...
> **来源: [Rust Reference: Data races](https://doc.rust-lang.org/reference/)** 并发类 UB 在 Miri 中无法检测，需用 Loom 或 ThreadSanitizer。
...
> **来源**: [Rust Reference: Behavior considered undefined — 完整清单](https://doc.rust-lang.org/reference/) · [UCG Book: What is undefined behavior?] · [Ralf Jung Blog: The scope of unsafe]
...
> **[Rustonomicon](https://doc.rust-lang.org/nomicon/)** 安全抽象定理：unsafe 实现 + safe 接口 + 人工证明 = 用户无需了解内部 unsafe 即可安全使用。✅ 已验证
...
> **[Rustonomicon](https://doc.rust-lang.org/nomicon/)** 类比：unsafe 像手术刀——精确、强大，但需要专业训练和明确的安全协议。✅ 已验证
...
| **[Rustonomicon](https://doc.rust-lang.org/nomicon/)** | Unsafe 编程规范、安全抽象设计 | 实践指南 |
...
> **跨层映射**: `L3::别名模型` ↔ [`L1::借用规则`](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) 静态检查 · [`L4::RustBelt`](../../04_formal/02_separation_logic/04_rustbelt.md) 形式化证明
...
> **认知功能**: 概念校准工具，通过条件分支逐步拆解"unsafe 关闭所有检查"的常见误解。建议初学者在首次编写 unsafe 代码前阅读此图，建立"局部关闭而非全局关闭"的精确认知。关键洞察：`unsafe` 仅关闭编译器无法自动验证的特定检查（裸指针解引用、FFI 契约、trait 语义），类型系统、生命周期（Lifetimes）检查（对引用而言）和 trait bound 推导仍在正常运行。[来源: 💡 原创分析]
...
> **认知功能**: FFI 风险评估的层次化检查框架，将"外部代码是否可信"的模糊问题分解为 ABI、布局、生命周期（Lifetimes）、协议四个可验证的子问题。建议在每次添加 `extern` 函数声明或修改 `#[repr(C)]` 结构体（Struct）后，按此四层顺序自检。关键洞察：FFI 是 Rust 类型证明系统的完整公理缺口——编译器对 C 代码的假设为零，任何外部代码的"安全"都必须由程序员在四个维度上逐条人工保证。[来源: 💡 原创分析]
...
> **[Rustonomicon: Safe Abstractions](https://doc.rust-lang.org/nomicon/)** 安全抽象定理：unsafe 实现 + safe 接口 + 人工证明 = 用户无需了解内部 unsafe 即可安全使用。Rust 标准库的核心类型（Vec, String, HashMap 等）均基于此模式构建。✅ 已验证
...
> **来源**: [LLVM LangRef: Function Attributes — noalias, dereferenceable, noundef] · [Rustonomicon: What is undefined behavior?](https://doc.rust-lang.org/nomicon/) · [Ralf Jung Blog: Why undefined behavior is scary]
...
> **新增跨层映射**: `L3::别名模型` ↔ [`L1::所有权`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) 运行时动态验证 · [`L4::RustBelt`](../../04_formal/02_separation_logic/04_rustbelt.md) 操作语义实例与逻辑关系
...
> **[Rust Reference: NonNull](https://doc.rust-lang.org/reference/)** `std::ptr::NonNull<T>` evolved from unstable `Unique<T>` and `Shared<T>` to provide a covariant raw pointer with a non-null invariant. ✅ 已验证
...
| unsafe 是程序员承担证明责任 | [Rustonomicon](https://doc.rust-lang.org/nomicon/) | ✅ |
...
| 未定义行为（UB）定义 | [Wikipedia: Undefined behavior](https://en.wikipedia.org/wiki/Undefined_behavior) · [Rust Reference](https://doc.rust-lang.org/reference/) | ✅ |
| 内存安全（Memory Safety） | [Wikipedia: Memory safety](https://en.wikipedia.org/wiki/Memory_safety) · [Rustonomicon](https://doc.rust-lang.org/nomicon/) | ✅ |
...
| unsafe 操作分类 | [Rust Reference: Unsafe Rust](https://doc.rust-lang.org/reference/) · [TRPL Ch19.1](https://doc.rust-lang.org/book/ch19-01-unsafe-rust.html) | ✅ |
| Validity Invariant | [Rustonomicon: What is unsafe?](https://doc.rust-lang.org/nomicon/what-unsafe-does.html) · [UCG Book] | ✅ |
| 裸指针语义 | [Rust Reference: Pointer types](https://doc.rust-lang.org/reference/) · [std::ptr docs] | ✅ |
| FFI 边界安全 | [Rust Reference: External blocks](https://doc.rust-lang.org/reference/) · [Rustonomicon: FFI](https://doc.rust-lang.org/nomicon/ffi.html) | ✅ |
| 内存布局控制 | [Rust Reference: Type Layout](https://doc.rust-lang.org/reference/type-layout.html) · [Rustonomicon: Data Layout](https://doc.rust-lang.org/nomicon/) | ✅ |
...
| FFI 与外部函数接口 | [Wikipedia: Foreign function interface](https://en.wikipedia.org/wiki/Foreign_function_interface) · [Rust Reference: FFI](https://doc.rust-lang.org/reference/) | ✅ |
...
> **来源: [Rustonomicon: Ownership and Move Semantics](https://doc.rust-lang.org/nomicon/)** Move 语义在底层就是 bitwise copy + 使原位置失效。`ptr::read` 只做前半部分，后半部分由程序员负责。✅ 已验证
...
> **来源: [Rustonomicon: RAII](https://doc.rust-lang.org/nomicon/)** 混淆 `*ptr = val` 与 `ptr::write` 是 unsafe 代码中内存泄漏和 double-free 的首要原因。💡 原创分析
...
> **来源: [Rustonomicon: Working With Memory](https://doc.rust-lang.org/nomicon/)** Using `ptr::read` without ensuring the source is no longer considered initialized can lead to double-drops, which is undefined behavior. ✅ 已验证
...
> **跨层映射**: `L3::裸指针语义` ↔ [`L1::所有权`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) move 与 copy 语义 · [`L2::内存管理`](../../02_intermediate/02_memory_management/03_memory_management.md) drop 触发规则 · [`L4::形式化`](../../04_formal/01_ownership_logic/03_ownership_formal.md) Validity Invariant 与初始化要求
...
`NonNull<T>` 是 Rust 标准库中所有"拥有指向堆内存指针"类型的内部基石。它不携带所有权语义，但提供**非空保证**和**协变性**，使智能指针（Smart Pointer）的实现既高效又类型安全。
...
> ✅ 已验证
> **来源: [Rustonomicon: Variance](https://doc.rust-lang.org/nomicon/)**
...
> 验证框架见 [`../04_formal/04_rustbelt.md`](../../04_formal/02_separation_logic/04_rustbelt.md)，
> 别名模型的精确规则见 [`../04_formal/03_ownership_formal.md`](../../04_formal/01_ownership_logic/03_ownership_formal.md) §5（Tree Borrows / Stacked Borrows 对比）。
...
Rust 中 `unsafe` 块的 **5 种能力** 来源: [Rust Reference](https://doc.rust-lang.org/reference/)：
...
> 该文档侧重工程实践与 Rustonomicon 逐章对标。因其内容已被 `concept/03_advanced/02_unsafe/03_unsafe.md`
