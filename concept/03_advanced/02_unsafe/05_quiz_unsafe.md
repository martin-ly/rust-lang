...
**解析**：unsafe 的超能力**只有**这五类（Rust Reference 明确列举）。B、C 是最危险的误解：`unsafe` 块内借用检查、类型检查、trait 约束**全部照常生效**，编译器并未"闭眼"。D 把"编译器不阻止"误读为"安全"——unsafe 的语义是"程序员担保安全"，不是"操作本身安全"。
...
- C. 从裸指针创建 `&T`/`&mut T` 时，程序员必须保证引用的全部规则（有效、对齐、别名约束）都满足
...
**解析**：裸指针是"去掉了所有编译器护栏的引用"：无有效性、无生命周期、无别名规则（A），解引用必须 unsafe（D）。B 错：裸指针**没有任何**所有权（Ownership）语义，drop 检查器完全忽略它，内存管理全归程序员。C 是正确用法的核心：创建引用 = 向编译器做出全套安全承诺，且该承诺覆盖引用的整个生命周期。
