...
>
> **前置概念**: [Error Handling](../../02_intermediate/03_error_handling/01_error_handling.md)
> [Unsafe Rust](01_unsafe.md) ·
...
**知识点**：`unsafe` 是"信任程序员"的契约，不是"关闭检查器"。每个 `unsafe` 块都应附带安全不变量的注释。[→ Unsafe 详解](01_unsafe.md)
...
**知识点**：`unsafe fn` 将安全责任**转移给调用者**。调用方必须在 `unsafe` 块中显式承担这一责任。[→ Unsafe 详解](01_unsafe.md)
...
本节围绕「原始指针（Raw Pointer）与内存安全（Memory Safety）」展开，覆盖 Q3. 以下代码能否编译？存在什么风险？ 与  Q4. 以下代码能否编译？Miri 会如何报告？ 两个方面。
...
**知识点**：原始指针（Raw Pointer）的主要用途是与 C 代码交互（FFI）和构建安全抽象（如 `Vec`、`Box` 的内部实现）。[→ Unsafe 详解](01_unsafe.md)
...
**知识点**：永远不要为包含非线程安全内部可变性的类型 `unsafe impl Sync`。[→ 并发模型详解](../00_concurrency/01_concurrency.md)
...
**知识点**：FFI 是 Rust 与外部世界交互的边界，也是 UB 的主要来源。精确的签名声明和边界测试是必须的。[→ FFI 详解](../04_ffi/01_rust_ffi.md)
...
**知识点**：`MaybeUninit` 是 Rust 1.36+ 替代 `std::mem::uninitialized` 的安全工具。后者已废弃，因为可能实例化无效值。[→ Unsafe 详解](01_unsafe.md)
...
**知识点**：`ptr::read` / `ptr::write` 是 Rust 标准库内部实现的核心原语，也是手动内存管理的起点。[→ Unsafe 模式详解](04_unsafe_rust_patterns.md)
...
**知识点**：Rust 2024 Edition 的 `unsafe_op_in_unsafe_fn` 是 Unsafe Rust 可用性的重要改进，已在项目中通过 `.cargo/config.toml` 的 `edition = "2024"` 启用。[→ Unsafe 详解](01_unsafe.md)
...
**知识点**：原始指针（Raw Pointer）的类型转换不等于合法访问。Rust 的别名模型（Stacked Borrows / Tree Borrows）对指针的类型和权限有严格要求。[→ Unsafe 详解](01_unsafe.md)
...
> 本节按四级题型规范补充单选、多选与判断题，知识点与 [Unsafe Rust](01_unsafe.md) 权威页一致；
> 干扰项针对常见误解设计。

### Q11. 🟢【单选】`unsafe` 块/函数中允许执行、而安全 Rust 禁止的操作包括哪些？

- A. 解引用（Reference）裸指针；调用 `unsafe fn`；访问或修改可变静态变量；实现 `unsafe trait`；访问 `union` 字段
- B. 绕过借用（Borrowing）检查器的所有规则
- C. 关闭类型检查
- D. 任意内存访问都是安全的
...
**解析**：unsafe 的超能力**只有**这五类（Rust Reference 明确列举）。B、C 是最危险的误解：`unsafe` 块内借用（Borrowing）检查、类型检查、trait 约束**全部照常生效**，编译器并未"闭眼"。D 把"编译器不阻止"误读为"安全"——unsafe 的语义是"程序员担保安全"，不是"操作本身安全"。

</details>

---

### Q12. 🟡【单选】关于 `unsafe` 的正确心智模型是？

- A. unsafe 代码不需要任何验证，跑通即可
- B. unsafe 关闭编译器的**部分**检查，程序员承担维护安全不变量的责任；正确做法是把 unsafe 封装在健全（sound）的安全抽象内部
- C. unsafe 代码必然包含未定义行为
- D. unsafe 只影响块内代码的性能
...
**解析**：标准库本身就是范例：`Vec`、`String` 内部全是 unsafe，但对外暴露的每个安全 API 都维护了不变量（长度 ≤ 容量、UTF-8 有效性等）。C 错：unsafe 是"有能力制造 UB"，不是"必然 UB"——大量 unsafe 代码是完全正确的。A 错：unsafe 代码恰恰需要**更强**的验证（Miri、review、形式化），而非更少。

</details>

---

### Q13. 🔴【多选】关于裸指针（`*const T` / `*mut T`），下列说法正确的有？（选出所有正确项）

- A. 可以为空、悬垂或未对齐——编译器不做任何有效性保证
- B. 自带生命周期（Lifetimes）管理，离开作用域自动释放指向的内存
- C. 从裸指针创建 `&T`/`&mut T` 时，程序员必须保证引用（Reference）的全部规则（有效、对齐、别名约束）都满足
- D. 解引用裸指针必须放在 `unsafe` 块中
...
**解析**：裸指针是"去掉了所有编译器护栏的引用"：无有效性、无生命周期（Lifetimes）、无别名规则（A），解引用必须 unsafe（D）。B 错：裸指针**没有任何**所有权（Ownership）语义，drop 检查器完全忽略它，内存管理全归程序员。C 是正确用法的核心：创建引用 = 向编译器做出全套安全承诺，且该承诺覆盖引用的整个生命周期。

</details>

---

### Q14. 🟢【判断】Miri 是一个解释执行 Rust MIR 的解释器，能检测越界访问、use-after-free、Stacked/Tree Borrows 违规等未定义行为；但"Miri 通过"不能证明代码没有 UB。（对 / 错）
...
**解析**：Miri 是**动态**分析工具：它只检查**实际执行到的路径与输入**。未覆盖的分支、未触发的对齐/别名场景仍可能藏 UB——与测试"只能证明有 bug 不能证明无 bug"同理。因此 unsafe 代码的保障组合是：Miri（动态）+ 最小化 unsafe 面 + 健全性论证（静态推理），而非单靠 Miri 绿灯。

</details>

---

### Q15. 🔴【判断】`unsafe` 块内的代码若触发 UB，其破坏范围仅限于该 unsafe 块内部，程序其余部分仍是安全的。（对 / 错）
...
**解析**：UB 的影响是**全程序、全时间**的：编译器与 LLVM 的优化基于"程序无 UB"的假设，一处 UB 可以让相距甚远的代码产生任意行为（时间旅行式破坏）。所以 unsafe 的责任边界不是"unsafe 块"而是"**安全抽象边界**"：封装者必须保证无论外部如何调用安全 API，内部 unsafe 的不变量都不被破坏——这正是"soundness（健全性）"的定义。
...
| 得分 | 评价 | 建议 |
|:---:|:---|:---|
| 10/10 | 🏆 Unsafe 边界已内化 | 进阶至 [Unsafe Patterns](04_unsafe_rust_patterns.md) 或尝试为 crates/ 编写 unsafe 抽象 |
| 7–9/10 | ✅ 核心概念掌握 | 强化 [Unsafe 练习](../../exercises/src/unsafe_rust)，用 Miri 验证所有代码 |
| 4–6/10 | 🔄 需巩固基础 | 重读 [Unsafe Rust](01_unsafe.md) · [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html) |
