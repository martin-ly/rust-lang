...
- A. 解引用裸指针；调用 `unsafe fn`；访问或修改可变静态变量；实现 `unsafe trait`；访问 `union` 字段
- B. 绕过借用检查器的所有规则
- C. 关闭类型检查
- D. 任意内存访问都是安全的
...
**解析**：裸指针是"去掉了所有编译器护栏的引用"：无有效性、无生命周期、无别名规则（A），解引用必须 unsafe（D）。B 错：裸指针**没有任何**所有权语义，drop 检查器完全忽略它，内存管理全归程序员。C 是正确用法的核心：创建引用 = 向编译器做出全套安全承诺，且该承诺覆盖引用的整个生命周期。
