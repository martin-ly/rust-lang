...
- **边界陈述**：Rust 的 UB 清单（继承自 LLVM 并叠加自身规则）包括：数据竞争、悬垂/未对齐/越界指针解引用、无效值（`bool` 非 0/1、枚举判别值越界、空引用）、违反别名规则（`&mut` 非独占）、调用 ABI 不匹配、以及「在无效状态上执行安全代码」。关键性质：UB 可以「时间倒流」——编译器基于「无 UB」假设的优化，可使 UB 发生**之前**的代码行为异常。
...
- **反例（8.2）**：把 `#[repr(Rust)]` 结构体指针传给 C（重排导致字段错位）、`enum` 直接跨边界（Rust enum 布局无承诺，需 `#[repr(C)]` 或 `#[repr(u8)]` + 手动 tag）、`bool` 与 C `_Bool` 的宽度/有效值差异（Rust bool 只许 0/1，C 侧传入其他值即 UB）；
