...
- **核心层（永远 UB）**: 悬垂/未对齐/空指针解引用、数据竞争、无效值（`bool` 取 2、`!` 类型实例化、枚举（Enum）越界判别值）、别名规则违反、跨 FFI 边界 unwind。
...
- **边界陈述**：Stacked Borrows（SB）与 Tree Borrows（TB）是 Rust 别名规则的两种操作化模型。核心思想：每次从引用创建裸指针、或从裸指针重建引用，都在「借用（Borrowing）栈/树」上记录权限来源；访问内存时校验「该访问是否被栈顶/树中存活节点的权限覆盖」。SB 用严格的栈（`&mut` 压栈后其下条目不可再用），TB 用树放宽了「只读兄弟共存」的场景——TB 是 SB 的继任者，接受更多合理的 unsafe 模式（如「裸指针与共享引用交错读」）。
...
- **边界陈述**：安全抽象要求区分两层契约——**validity**（值级不变量：`bool` 必须 0/1、引用非空对齐，任何时刻任何代码都不得违反，违反即 UB）与 **safety**（API 级契约：`str` 必须 UTF-8、`Vec` 的 `len ≤ capacity`，由模块（Module）内部 unsafe 代码维护，safe 接口保证外部无法破坏）。安全抽象成立 ⟺ 「模块内 unsafe 代码维护 safety 不变量」且「safety 不变量蕴含所有对外暴露操作的 validity」——即不变量的「可信度边界」恰好等于模块边界。
...
- **判定条件（8.3）**：类型跨 FFI 合法 ⟺ ① `#[repr(C)]`/`#[repr(transparent)]` 或 POD 标量；② 字段递归满足同样条件；③ 无所有权（Ownership）语义（`Box`/`Vec` 需转裸指针 + 容量传递）；④ `cbindgen` 生成的头文件与 Rust 定义同步（CI 校验）。

审计要点：FFI 边界的每个类型都应能在「C 头文件 ↔ Rust 定义」间双向核对——`cbindgen`/`bindgen` 自动化是消除漂移的唯一可靠手段。

### 8.1 边界陈述

FFI 边界没有共享类型系统（Type System），契约靠**两侧人工对齐**：
