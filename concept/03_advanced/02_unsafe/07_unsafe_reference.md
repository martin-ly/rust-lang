...
| 五种能力 | 解引用（Reference）裸指针、调用 unsafe 函数、访问 `static mut`、实现 unsafe trait、访问 union 字段 | 本文 §二 |
...
- **依赖**：依赖 [所有权（Ownership）](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) 的安全保证基线。
