...
- **反命题 1：「NLL 之后借用检查不再有假拒绝」** —— 错误。NLL 改善了「作用域尾部」的保守性，但**路径敏感性**仍是边界：`if let Some(v) = map.get(k) { return v } map.insert(k, new());` 被拒（`get` 的借用在「未返回的分支」上实际已结束，NLL 不区分路径）——这就是 Polonius 的标志性改进案例；
- **反命题 2：「Polonius 上线后现有代码行为会改变」** —— 不会。Polonius 只**放宽**（接受更多程序），不收紧——所有当前合法的代码保持合法。这是借用检查演进的兼容性红线：借用规则收紧属破坏性变更，必须经 edition 机制。

边界极限小节量化：NLL/Polonius 接受程序集的差异示例、two-phase borrow 的特殊规则（`vec.push(vec.len())` 的合法性）、以及区域变量在错误信息中的呈现。
...
> **修正**: NLL 下，可变借用（Mutable Borrow） `r` 的生命周期（Lifetimes）精确到其最后一次使用。`drop(r)` 显式结束借用后，`data` 才能被移动/释放。
...
本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：词法生命周期的问题（理解层）、测验 2：NLL 的改进范围（应用层）、测验 3：Polonius 与 NLL 的关系（理解层）、测验 4：NLL 与性能（分析层）等5个方面。
...
- ❌ 仍然禁止同时存在活跃的可变借用和不可变借用（Immutable Borrow）
- ❌ 仍然禁止返回局部变量引用（Reference）
- ✅ 允许引用（Reference）在其最后一次使用后"提前死亡"，从而释放原始变量的借用
...
- A. 有，NLL 需要在运行时（Runtime）跟踪引用（Reference）
- B. 没有，NLL 完全是编译期分析
- C. 仅在 Debug 模式有开销
...
运行时（Runtime）没有任何引用（Reference）计数、借用检查或生命周期跟踪。Rust 的安全保证完全在编译期建立，这是其零成本抽象（Zero-Cost Abstraction）的核心。
</details>
...
| NLL 与 Polonius：借用检查器的演进 正确用法 ⟹ 常见陷阱 | 忽略边界条件 | 编译错误或运行时（Runtime） bug | 高 |
...
本节以 Polonius 经典 problem case #3 为反例，展示 NLL 区域流不敏感导致的误拒。

### 反例：条件返回中的借用被 NLL 误拒（rustc 1.97.0 实测）

```rust,compile_fail,E0502
fn f(v: &mut Vec<i32>, b: bool) -> &i32 {
    if let Some(x) = v.get(0) {
        if b {
            return x; // x 的借用只在这条路径存活
        }
    }
    v.push(1); // ❌ NLL 认为借用可能仍存活
    &v[0]
}
```

**错误**：`E0502 cannot borrow v as mutable because it is also borrowed as immutable`——这正是 Polonius 流敏感分析要解决的问题案例。
