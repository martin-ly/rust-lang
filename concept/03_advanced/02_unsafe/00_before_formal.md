...
- **通过面试**：只需掌握「unsafe 五能力」（解引用裸指针、调 unsafe fn、访问/修改可变 static、实现 unsafe trait、访问 union 字段）+ `Send`/`Sync` 含义——跳过全部形式化章节，直接读 unsafe 边界全景页；
- **写更好的代码**：先读内存模型与别名校验（Stacked/Tree Borrows 直觉），再读 Miri 使用——目标是「能论证自己 unsafe 代码的健全性」；
- **理解编译器报错**：读 NLL/Polonius 与类型推断章节——借用错误的信息结构直接对应检查器的内部算法；
