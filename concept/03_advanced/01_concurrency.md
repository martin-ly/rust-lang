...
- [Concurrency（并发模型）](#concurrency并发模型)
  ...
  - [一、权威定义（Definition） \[来源: Rust 并发基于所有权（Ownership）系统——每个值有唯一所有者（单一所有权，资源唯一性），所有权通过 move/转移在线程间传递（赋值、传参），owner 离开作用域时自动 drop/释放\]](#一权威定义definition-来源-rust-并发基于所有权系统每个值有唯一所有者单一所有权资源唯一性所有权通过-move转移在线程间传递赋值传参owner-离开作用域时自动-drop释放)
...
在单线程程序中，借用（Borrowing）检查器（Borrow Checker）已经保证了**Alias XOR Mutation**：任意时刻，对同一块内存要么有多个不可变引用（Immutable Reference），要么只有一个可变引用（Mutable Reference）。编译器在 `01_foundation/01_ownership.md §3.1` 中通过所有权规则消除了 use-after-free 和数据竞争的所有可能。
...
> **对应标注**：T1 中"编译期排除数据竞争"为 [`01_foundation/01_ownership.md`](../01_foundation/01_ownership.md) §3.1 "借用（Borrowing）检查器的安全性定理" 的并发延伸。
