...
> [Reference — Unsafe Rust](https://doc.rust-lang.org/reference/unsafe-blocks.html)
...
- [Unsafe Rust 模式：安全抽象的核心技术](#unsafe-rust-模式安全抽象的核心技术)
  ...
  - [一、核心概念](#一核心概念)
    ...
    - [1.2 原始指针（Raw Pointer）操作](#12-原始指针操作)
  ...
  - [十、边界测试：Unsafe Rust 模式的编译错误](#十边界测试unsafe-rust-模式的编译错误)
    - [10.1 边界测试：自定义 `Drop` 中的 `mem::forget` 循环（运行时（Runtime） UB）](#101-边界测试自定义-drop-中的-memforget-循环运行时-ub)
    - [10.2 边界测试：`unsafe impl` 的 trait 契约违反（编译错误 / 运行时 UB）](#102-边界测试unsafe-impl-的-trait-契约违反编译错误--运行时-ub)
    - [10.3 边界测试：自引用（Reference）结构的 `Pin` 误用（编译错误/运行时 UB）](#103-边界测试自引用结构的-pin-误用编译错误运行时-ub)
...
- A. 原始指针不能为 null
- B. 原始指针不受借用（Borrowing）检查器管理，可存在多个 `*mut` 指向同一数据
- C. 原始指针自动解引用
...
> **过渡**: 掌握 Unsafe Rust 模式：安全抽象的核心技术 的基础语法后，下一步需要理解其在类型系统（Type System）中的位置与与其他概念的交互关系。
