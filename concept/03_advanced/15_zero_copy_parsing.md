...
- [零拷贝解析与序列化优化](#零拷贝解析与序列化优化)
  ...
  - [一、核心概念](#一核心概念)
    - [1.1 零拷贝原理](#11-零拷贝原理)
    - [1.2 生命周期（Lifetimes）约束](#12-生命周期约束)
  - [二、关键技术](#二关键技术)
  ...
  - [十、边界测试：零拷贝解析的编译错误](#十边界测试零拷贝解析的编译错误)
    - [10.1 边界测试：`mem::transmute` 的字节对齐假设（运行时（Runtime） UB）](#101-边界测试memtransmute-的字节对齐假设运行时-ub)
    ...
    - [10.4 边界测试：零拷贝解析的生命周期依赖与所有权（Ownership）转移（编译错误）](#104-边界测试零拷贝解析的生命周期依赖与所有权转移编译错误)
...
- B. 使用引用（Reference）计数共享同一底层缓冲区，切片只是增加计数并记录偏移
- C. 使用 `Vec<u8>` 的内部分段

<details>
<summary>✅ 答案</summary>

**B. 使用引用计数共享同一底层缓冲区，切片只是增加计数并记录偏移**。

`Bytes` 是 tokio 生态中的零拷贝原语：

- 内部使用 `Arc<[u8]>` 或类似结构引用计数
