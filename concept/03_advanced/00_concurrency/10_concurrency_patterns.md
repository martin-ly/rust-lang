...
「技术细节」涉及通道模式、无锁数据结构与内存顺序，本节逐一说明其要点。
...
理解「嵌入式测验」需要把握测验 1：并发模式识别（记忆层）、测验 2：Arc 引用计数（理解层）、测验 3：工作窃取模式（应用层）与测验 4：死锁预防（分析层），本节依次展开。
...
「消息传递核心知识图谱」部分包含消息传递概念关系图 与 通道数据流图 两条主线，本节依次说明。
...
「通道类型多维对比」部分按标准库 vs 第三方库对比、通道性能特征对比与接收方法对比的顺序逐层展开。
...
「快速参考」部分包含常用通道API 与 错误类型速查 两条主线，本节依次说明。
...
本节围绕「Actor模型实现」展开，覆盖基础Actor框架 与 高级Actor特性 两个方面。
...
本节从监督策略 与 路由Actor 两个层面剖析「高级Actor特性」。
...
本节从类型安全通道 与 背压控制通道 两个层面剖析「高级通道通信」。
...
「发布订阅模式」部分包含类型安全的事件总线 与 异步事件流 两条主线，本节依次说明。
