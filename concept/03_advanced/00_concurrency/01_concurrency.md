...
> **Send/Sync 契约权威页**: Send/Sync 的形式化契约、auto 推导规则与判定矩阵已收敛至 [Send 与 Sync：Auto Trait 的并发安全（Concurrency Safety）契约](02_send_sync_auto_traits.md)，本页保留并发模型全景与场景章节。
>
> **层次定位**: L3 高级概念 / 并发子域
...
- [Concurrency（并发模型）](#concurrency并发模型)
  ...
  - [迁移内容（来自 `crates/c05_threads/docs/04_thread_synchronization.md`）](#迁移内容来自-cratesc05_threadsdocs04_thread_synchronizationmd)
  ...
  - [迁移内容（来自 `crates/c05_threads/docs/06_synchronization_primitives.md`）](#迁移内容来自-cratesc05_threadsdocs06_synchronization_primitivesmd)
...
在单线程程序中，借用（Borrowing）检查器（Borrow Checker）已经保证了**Alias XOR Mutation**：任意时刻，对同一块内存要么有多个不可变引用（Immutable Reference），要么只有一个可变引用（Mutable Reference）。编译器在 `01_foundation/01_ownership_borrow_lifetime/01_ownership.md §3.1` 中通过所有权规则消除了 use-after-free 和数据竞争的所有可能。
...
多线程引入了**交错执行（interleaving）**：线程 A 的 `load` 可能发生在线程 B 的 `store` 之前或之后，产生不可预测的结果。`01_foundation/01_ownership_borrow_lifetime/01_ownership.md §2.2` 中的所有权转移规则仍然成立，但"转移给谁"变成了"跨线程转移"。

> **对应标注**：此处为 [`01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) §2.2 "所有权转移规则" 的并发延伸。
...
这些属于**活性（liveness）**或**逻辑错误**，不在类型系统（Type System）的安全保证范围内。
...
> **对应标注**：此处为 [`01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) §5.1 "所有权规则的验证路径" 的并发扩展。
...
## 一、权威定义（Definition） [Rust 并发基于所有权系统——每个值有唯一所有者（单一所有权，资源唯一性），所有权通过 move/转移在线程间传递（赋值、传参），owner 离开作用域时自动 drop/释放](../../00_meta/00_framework/methodology.md)

Rust 并发的权威定义可归纳为一句话：**并发安全（Concurrency Safety）不是附加机制，而是所有权系统的推论**。三个视角展开这一定义：

- **Wikipedia 权威定义**：并发（concurrency）是「多个计算在同一时间段内独立推进」的程序性质，区别于并行（parallelism，同一时刻物理同时执行）。Rust 的独特贡献是把「并发程序无数据竞争」从运行时纪律提升为编译期定理——「fearless concurrency」的正式含义。
- **TRPL 官方定义**：Rust 的并发基于「消息传递（channel）与共享状态（Mutex/Arc）两条正交路径」，而两条路径的安全性都由 `Send`/`Sync` 两个 auto trait 在类型层把关：值能否跨线程移动（`Send`）、能否跨线程共享引用（Reference）（`Sync`）是编译期可判定的。
- **形式化定义**：数据竞争自由的保证来自「别名 XOR 可变性」规则在线程维度的推广——`&mut T: Send` 当且仅当 `T: Send`（独占权可转移），`&T: Send` 当且仅当 `T: Sync`（共享只读可转移）。RustBelt 在 Iris 中证明了该类型规则蕴含数据竞争自由。

判定一个并发设计的合法性，最终都归约为两个 auto trait 的判定：跨线程边界的每个类型，编译器自动推导其 `Send`/`Sync`——推导失败即设计被拒（E0277），无需运行时事故来发现。
...
并发的属性矩阵沿「类型标记 × 同步原语 × 语言模型」三个维度组织：

1. **Send/Sync 判定矩阵**：`Send` = 所有权可跨线程转移（几乎所有类型默认满足，例外：`Rc`（非原子计数）、裸指针、`*mut`/`Cell`/`RefCell` 的某些组合）；`Sync` = `&T` 可跨线程共享（等价于 `&T: Send`，例外：含内部可变性且无线程同步的类型如 `Cell`/`RefCell`）。判定规则是结构化的：复合类型 `Send`/`Sync` ⟺ 全部字段 `Send`/`Sync`。
2. **同步原语对比矩阵**：`Mutex<T>`（互斥，独占访问）、`RwLock<T>`（读写分离，读并发）、`Atomic*`（无锁单字操作，内存序可选）、`mpsc` 通道（所有权经消息转移）、`Condvar`（条件等待）。选择维度：保护的数据量（单字 → atomic；结构 → 锁）、访问模式（读多写少 → RwLock）、是否需要等待条件 → Condvar。
3. **并发模型对比（跨语言）**：C++（线程 + 手动同步，数据竞争是 UB 但编译器不检查）、Go（goroutine + channel 一等公民，数据竞争运行时检测器可选）、Java（监视器 + `volatile`，数据竞争有 JMM 定义的弱语义）、Rust（编译期拒绝数据竞争）。四者的本质差异是「数据竞争在哪个阶段被发现」：编译期（Rust）/ 运行检测（Go race detector）/ 永不（C++ UB）。

矩阵用法：并发 bug 定位时先在维度 3 确认该错误在 Rust 中是否本应被编译期拦截（是 → 必有 `unsafe` 或逻辑错误），再在维度 1/2 查具体类型的约束与原语选择。

### 2.1 Send/Sync 判定矩阵

> **对应标注**：此处为 [`01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) §2.1 "所有权与借用（Borrowing）规则" 的并发类型级对应。
...
| `crossbeam::scope` | 借用（Borrowing）检查 | 阻塞 join |  scoped 线程 | 无 'static 要求 |
...
> **[Go Spec: Concurrency](https://go.dev/ref/spec#Go_statements)** Go 通过 goroutine + channel 实现 CSP 模型，但编译器不保证数据竞争自由。 ✅
> **[C++ Reference: Thread support library](https://en.cppreference.com/w/cpp/thread)** C++11 引入 `std::thread` 和内存模型，但无编译期数据竞争检查。 ✅
> **[Haskell GHC User Guide: Parallel Haskell](https://downloads.haskell.org/ghc/latest/docs/users_guide/exts/parallel.html)** Haskell 通过 `Par` monad 和 STM (`TVar`) 实现并发，纯函数隔离消除数据竞争，STM 有形式化语义基础。 ✅
...
>
> **[RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/)** Rust 的类型系统（Type System）通过 Send/Sync 与所有权规则，可在逻辑上证明 Safe Rust 程序无数据竞争。该定理是 Rust 并发安全（Concurrency Safety）的核心形式化保证。 ✅ 已验证
...
> **[TRPL Ch16.0](https://doc.rust-lang.org/book/ch16-00-concurrency.html)** Fearless concurrency 强调：所有权和类型系统（Type System）是消除并发 bug 的盟友，程序员无需手动推理所有交错执行路径。✅ 已验证
...
前提:
  1. Alias-XOR-Mutation: 任意时刻，对任意内存位置，要么存在多个不可变引用（Immutable Reference），要么存在一个可变引用（Mutable Reference）
...
> synchronizes-with 边是跨线程可见性的最小充分条件——没有这条边，就没有 happens-before。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> [Mermaid sequenceDiagram 文档](https://mermaid.js.org/syntax/sequenceDiagram.html); [Boehm & Adve PLDI 2008](https://doi.org/10.1145/1375581.1375591)
...
§3.2 给出了 Send/Sync 的代数结构（格与组合规则），但未解释它们与 §3.1b 的 C11 内存模型如何衔接。本节建立类型系统（Type System）标记与底层内存模型之间的**精化关系（refinement）**。
...
> **认知功能**: 概念全景地图，建立并发知识的空间结构。初学时用作导航目录，复习时快速定位薄弱概念。该图揭示 Rust 并发安全（Concurrency Safety）的三元支柱——类型系统（Send/Sync）、同步原语（Mutex/Atomic）与消息传递（Channel）——缺少任一维度都无法覆盖全部并发场景。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
本节从共享状态 vs 消息传递？" 决策树 与  Send/Sync 手动实现边界 两个层面剖析「决策/边界判定树（Decision / Boundary…」。
...
> **认知功能**: 工程选型决策树，根据场景特征逐层分叉推荐并发模型。面对具体问题时从根节点逐层回答二元判断，最终落点的叶子节点即为候选方案。关键洞察：共享状态与消息传递并非互斥，性能需求与通信拓扑复杂度是决策的核心分叉条件。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **[Rust Reference: Deadlocks](https://doc.rust-lang.org/reference/introduction.html)** Rust 不保证防止死锁；死锁是活性（liveness）性质，而非安全性（safety）性质，超出当前类型系统（Type System）的保证范围。✅ 已验证
...
本节给出的示例与反例沿「所有权转移 → 共享状态 → 消息传递」三条主线组织，它们是 Rust 并发模型的三种基本用法，也对应三种不同的安全保证来源。

- **spawn + move 闭包**：`move` 把捕获变量的所有权转移给新线程，编译器借此静态排除「旧线程继续访问已转移数据」这一最常见的数据竞争来源；`JoinHandle::join` 则提供线程终止与返回值回收的同步点。
- **Mutex 共享状态**：`Mutex<T>` 把「数据」与「锁」绑定为同一类型，访问数据必须先获得守卫（guard），守卫的析构即解锁——锁的获取/释放在类型层面不可分离，忘记解锁和重复解锁都无法通过编译。
- **Channel 消息传递**：`mpsc` 通道以「发送即转移所有权」的语义把共享内存问题转化为消息传递问题，接收端拿到的是独占所有权，天然不存在并发读写。
- **反例（E0277）**：跨线程共享 `Rc<T>` 会被 `Send` 约束在编译期拒绝——这不是运行时检查，而是 trait bound 不满足导致的类型错误，正是 Rust「无畏并发」的静态根基。

阅读时建议对照每个反例的错误码：E0277（trait 约束不满足）与运行时死锁属于两个不同层面的失败，前者免费，后者需要本节后续的死锁分析工具应对。
...
> **对应标注**：`move` 闭包（Closures）的所有权转移行为与 [`01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) §2.2 "所有权转移规则" 完全一致，只是接收方变为新线程。
...
反命题分析的目的，是标定类型系统（Type System）并发保证的**边界**：`Send`/`Sync` 与所有权规则保证的是「Safe Rust 中无数据竞争」，而不是「并发程序总是正确」。下面五个反命题逐一划定这条边界。

| 反命题 | 反例的本质 | 边界结论 |
| :--- | :--- | :--- |
| 并发总是安全的 | `unsafe` 或错误的手写 `unsafe impl Send/Sync` 可绕过检查 | 保证以 Safe 子集为前提 |
| Mutex 保证线程安全 | 互斥只防数据竞争，不防死锁与逻辑竞争（check-then-act） | 安全 ≠ 活性（liveness） |
| Arc 替代所有共享 | `Arc<T>` 只共享只读访问；共享可变仍需 `Mutex`/`RwLock`/原子类型 | 共享所有权 ≠ 共享可变性 |
| Atomic 总是线程安全 | 单字操作原子，但跨变量的不变量仍需额外排序或锁 | 原子性 ≠ 一致性 |
| 无数据竞争即无 bug | 内存序（memory ordering）选错会导致合法但非预期的可见性 | 正确性依赖 Ordering 选择 |

判定依据：凡涉及跨线程共享可变状态的论证，必须同时回答三个问题——谁拥有数据（所有权）、谁能修改它（可变性）、修改何时对他人可见（内存序）。三个问题任一缺失，安全论证就不完整。
...
> **认知功能**: 谬误识别流程图，逐层拆解"并发安全（Concurrency Safety）"的隐含前提与失效条件。当有人断言"Rust 并发绝对安全"时，用此图展示安全边界：编译期仅保证无数据竞争，运行时死锁、活锁、poison 和 unsafe 误用仍需人工防范。[💡 原创分析](../../00_meta/00_framework/methodology.md)
> [来源: [Rust std::sync::atomic](https://doc.rust-lang.org/std/sync/atomic/index.html)]

**分析**: 并发安全（Concurrency Safety）是多层保证的——编译期排除数据竞争，但运行时仍需避免死锁、活锁、poison 和 unsafe 误用。
...
> **认知功能**: 边界澄清图，区分 Mutex 提供的"互斥"（safety）与其他安全维度（liveness、性能、async 兼容性）。选型时检查三个维度：是否重入、是否可能 poison、是否跨越 await。核心洞察：线程安全 ≠ 程序正确，Mutex 不保证无死锁。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **对应标注**：Mutex 的不可重入性与 [`01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) §7.2 "常见陷阱：双重释放" 同属"同一实体多次获取导致错误"的模式。
...
> **认知功能**: 选型纠偏图，揭示 Arc 的能力边界与典型误用场景。需要共享可变时配合 Mutex/RwLock，存在循环引用（Reference）风险时考虑 Weak，单次共享且生命周期（Lifetimes）确定时可用 scoped thread 替代。核心洞察：Arc 解决"多所有者"问题，不解决"可变共享"和"循环引用"。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**: 内存序警示图，揭示 Atomic 原子性与可见性的本质区别。使用 Atomic 时首先确认 Ordering 选择：Relaxed 不做同步用，Acquire/Release 需配对覆盖所有观察者。核心洞察：原子性 ≠ 顺序保证，错误的 Ordering 是并发 bug 中最隐蔽的来源。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**: 编译器重排边界图，澄清 Mutex 的 Acquire/Release 语义覆盖范围。lock/unlock 保证临界区之间的 happens-before，但不限制临界区内部的编译器优化。与硬件/FFI 交互时若需禁止编译器重排，显式使用 compiler_fence。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
背压（backpressure）是「生产速度超过消费速度时，系统如何反压生产者」的流控问题——它不属于并发正确性而属于并发系统的稳定性：

- **背压的本质**：任何「生产者-消费者」结构都有速率匹配问题。无背压时，中间缓冲（队列/channel）无界增长 → 内存耗尽 → OOM，故障从「慢」升级为「崩」。背压把「慢」的信号反向传播给生产者，使全系统速率收敛到最慢环节。
- **背压与并发原语的映射**：`mpsc::sync_channel(n)` 的有界缓冲在满时阻塞 `send`（同步背压）；`mpsc::channel()` 无界（无背压，显式的工程取舍）；`tokio::mpsc::channel(n)` 的 `send().await` 在满时挂起（异步（Async）背压）；`Arc<Semaphore>` 限制在途任务数（总量背压）。判定一个通道是否提供背压：查缓冲是否有界 + 发送端满时行为（阻塞/挂起/丢弃/报错）。
- **无背压的风险**：日志/指标上报、事件总线、消息队列消费端是典型事故点——生产端突发 10 倍流量时，无界队列把「消费端慢 10 倍」转化为「内存涨 10 倍」。缓解按场景选：可丢数据用丢弃策略（`try_send` + 计数），不可丢用阻塞/挂起 + 上游告警，关键路径加「队列深度」监控指标。

判定一个并发设计的背压完备性：沿数据流逐跳检查每个缓冲的容量策略——存在无界缓冲且无丢弃/告警策略的跳，即为背压缺口。
...
并发边界测试按「编译期 / 运行时 / 逻辑层」三档分类，对应安全保证的三个层次：

- **`Send` 不满足时跨线程移动**（编译错误 E0277）：`thread::spawn(move || { rc.clone() })` 中 `Rc` 捕获进闭包使闭包 `!Send`——错误在 spawn 调用点报出，信息指明 `Rc<T>: !Send`。这是「fearless concurrency」的直接体现：数据竞争在编译期不存在讨论空间。
- **死锁——嵌套锁顺序不一致**（运行时/逻辑错误）：两线程按相反顺序获取 `MutexA`/`MutexB` 形成循环等待——类型系统无法表达「锁顺序」协议，属逻辑层错误。防御手段：全局锁顺序约定、一次性获取（同时 lock 两锁的封装）、或改用无锁结构/消息传递。检测：调试构建可用 `parking_lot` 的 deadlock_detection feature。
- **`Mutex` 的毒化与恢复**：持锁线程 panic → 锁被毒化 → 后续 `lock()` 返回 `Err(PoisonError)`。`unwrap()` 传播失败是保守默认；确认不变量未破坏时可 `unwrap_or_else(|e| e.into_inner())` 显式恢复——该选择必须伴随「为什么毒化可忽略」的注释。
- **`mpsc` 的多生产者单消费者限制**：`Receiver` 不实现 `Clone`（单消费者是类型级约束），`Sender` 可 `clone`（多生产者）。需要多消费者时换 `crossbeam-channel`（MPMC）或 `tokio::broadcast`——类型签名即并发拓扑的声明。

判定并发错误归属：E0277 → 类型层（改设计）；panic/deadlock → 同步协议层（改锁策略）；静默错序 → 内存序/逻辑层（用 `SeqCst` 回归正确再优化）。
...
> **修正**: `Rc<T>` 使用非原子引用计数，不能跨线程。`Arc<T>` 使用原子操作（Atomic Operations），是 `Send + Sync`（当 `T: Send + Sync`）。即使 `T` 是线程安全的（如 `Mutex<i32>`），包装器 `Rc` 也会破坏 `Send` 保证。[来源: [Rust Standard Library](https://doc.rust-lang.org/std/index.html)]
...
以下四道嵌入式测验按 Bloom 层级递进：记忆层核对 `Send`/`Sync` 的精确定义（哪个方向是移动、哪个方向是共享引用）；理解层区分 `Rc` 与 `Arc` 的适用域（单线程引用计数 vs 原子引用计数及其代价）；应用层考察用 `Mutex` 保护共享可变状态的正确写法；分析层给出一段加锁代码要求识别死锁条件。作答时若分析层出错，通常意味着对「锁守卫生命周期（Lifetimes） = 临界区」这一核心机制理解不牢，应回看 8.2 节的锁顺序策略。
...
`Rc<T>` 使用**非原子**引用计数，因此它既不是 `Send` 也不是 `Sync`。`thread::spawn` 要求闭包（Closures）捕获的值实现 `Send`，以便将所有权 move 到新线程。
...
对比矩阵中的延迟数字是无竞争（uncontended）条件下 x86-64 平台的经验量级，用于相对比较而非绝对承诺；真实负载下竞争程度才是主导因素。读表时把握三条经验法则：

1. **获取延迟**只在无竞争时有意义；高竞争下排队等待与缓存行乒乓（cache line ping-pong）会使所有原语退化一个数量级以上，此时缩小临界区比更换原语更有效。
2. **内存占用**影响可组合性：`Mutex`（40B）可以内联进结构体（Struct）每个元素做细粒度锁，`RwLock`（56B）与条件变量的组合在大数组场景下内存开销不可忽视。
3. **死锁风险**列是使用约束而非性能指标：`Mutex`/`RwLock` 的多锁场景必须约定全局锁顺序，通道虽不产生死锁但可能产生活锁与积压。

决策顺序：先问「是否真的需要共享可变状态」（优先考虑消息传递或数据并行），再问「读写比例」（`RwLock` 的收益约在读写比 4:1 以上才兑现），最后才比较纳秒级延迟。
...
并发最佳实践可归结为两条互补原则：**让锁的保护范围尽可能小**（锁的粒度），以及**让锁的获取顺序尽可能可预测**（避免死锁）。粒度决定吞吐——临界区每多一条无关语句，竞争线程就多一分排队；顺序决定活性——没有全局顺序约定，两个线程各持一锁互相等待只是时间问题。以下两小节分别给出工程化做法，所有建议都以「守卫（guard）的生命周期（Lifetimes）即临界区」这一 RAII 语义为前提。

### 8.1 锁的粒度

锁的粒度指单个锁保护的数据量与临界区长度。粗粒度锁（一把大锁保护整个结构）实现简单、死锁面小，但竞争激烈时退化为串行执行；细粒度锁（按元素/按分片加锁）提升并行度，代价是锁数量增多、内存开销增大、死锁面变大。工程上的折中策略：

- 默认从粗粒度开始，用性能分析（profiling）确认锁竞争确为瓶颈后再细化；
- 细化时优先选择**数据分片**（sharding，如 `DashMap` 的段锁）而非手工多级锁，分片把「锁顺序」问题转化为「键到分片的哈希」问题；
- 临界区内禁止 `.await`、I/O 与回调用户代码——这些操作耗时不可控且可能重入加锁路径。
...
死锁的四个必要条件（互斥、占有并等待、不可抢占、循环等待）中，工程上最容易打破的是**循环等待**：给所有锁定义一个全局偏序（如按锁对象地址、按层级编号），任何线程都按序获取，循环等待在结构上就不可能发生。配套策略包括：

- **锁顺序策略**：跨模块（Module）的锁在文档中登记顺序编号，code review 时核对所有多锁获取点；
- **超时与试探**：`try_lock` + 退避用于无法统一顺序的边界场景，避免无限等待；
- **缩小持锁范围**：能用 `DashMap`、消息传递或不可变快照（clone-on-read）替代的共享状态，优先替代——没有多锁就没有顺序问题；
- **检测**：测试环境启用死锁检测（如 `parking_lot` 的 deadlock detection feature），把死锁从生产事故前移到 CI 失败。
