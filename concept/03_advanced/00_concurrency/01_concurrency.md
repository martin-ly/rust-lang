...
本节将「权威定义（Definition） Rust 并发基于所有权…」分解为若干主题： Wikipedia 权威定义、TRPL 官方定义与形式化定义。
...
「概念属性矩阵（Attribute Matrix）」涉及 Send/Sync 判定矩阵、同步原语对比矩阵与并发模型对比（跨语言），本节逐一说明其要点。
...
本节围绕「背压（Backpressure）：从并发到流控」展开，依次讨论背压的本质、背压与并发原语的映射与无背压的风险。
...
本节将「边界测试：并发规则的编译错误」分解为若干主题：边界测试：`Send` 不满足时跨线程移动（编译错误）、边界测试：死锁——嵌套锁顺序不一致（运行时错误 / 逻辑错误）、边界测试：`Mutex` 的毒化（poisoning）与错误恢复（运行…与边界测试：`std::sync::mpsc` 的多生产者单消费者限制（…。
