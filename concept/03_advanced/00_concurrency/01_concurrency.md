...
Rust 并发的权威定义可归纳为一句话：**并发安全不是附加机制，而是所有权系统的推论**。三个视角展开这一定义：
...
反命题分析的目的，是标定类型系统并发保证的**边界**：`Send`/`Sync` 与所有权规则保证的是「Safe Rust 中无数据竞争」，而不是「并发程序总是正确」。下面五个反命题逐一划定这条边界。
...
> **认知功能**: 选型纠偏图，揭示 Arc 的能力边界与典型误用场景。需要共享可变时配合 Mutex/RwLock，存在循环引用风险时考虑 Weak，单次共享且生命周期（Lifetimes）确定时可用 scoped thread 替代。核心洞察：Arc 解决"多所有者"问题，不解决"可变共享"和"循环引用"。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
并发最佳实践可归结为两条互补原则：**让锁的保护范围尽可能小**（锁的粒度），以及**让锁的获取顺序尽可能可预测**（避免死锁）。粒度决定吞吐——临界区每多一条无关语句，竞争线程就多一分排队；顺序决定活性——没有全局顺序约定，两个线程各持一锁互相等待只是时间问题。以下两小节分别给出工程化做法，所有建议都以「守卫（guard）的生命周期即临界区」这一 RAII 语义为前提。
