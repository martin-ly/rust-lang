...
- **TRPL 官方定义**：Rust 的并发基于「消息传递（channel）与共享状态（Mutex/Arc）两条正交路径」，而两条路径的安全性都由 `Send`/`Sync` 两个 auto trait 在类型层把关：值能否跨线程移动（`Send`）、能否跨线程共享引用（`Sync`）是编译期可判定的。
...
- **背压与并发原语的映射**：`mpsc::sync_channel(n)` 的有界缓冲在满时阻塞 `send`（同步背压）；`mpsc::channel()` 无界（无背压，显式的工程取舍）；`tokio::mpsc::channel(n)` 的 `send().await` 在满时挂起（异步背压）；`Arc<Semaphore>` 限制在途任务数（总量背压）。判定一个通道是否提供背压：查缓冲是否有界 + 发送端满时行为（阻塞/挂起/丢弃/报错）。
...
以下四道嵌入式测验按 Bloom 层级递进：记忆层核对 `Send`/`Sync` 的精确定义（哪个方向是移动、哪个方向是共享引用）；理解层区分 `Rc` 与 `Arc` 的适用域（单线程引用计数 vs 原子引用计数及其代价）；应用层考察用 `Mutex` 保护共享可变状态的正确写法；分析层给出一段加锁代码要求识别死锁条件。作答时若分析层出错，通常意味着对「锁守卫生命周期 = 临界区」这一核心机制理解不牢，应回看 8.2 节的锁顺序策略。
...
2. **内存占用**影响可组合性：`Mutex`（40B）可以内联进结构体每个元素做细粒度锁，`RwLock`（56B）与条件变量的组合在大数组场景下内存开销不可忽视。
...
- **锁顺序策略**：跨模块的锁在文档中登记顺序编号，code review 时核对所有多锁获取点；
