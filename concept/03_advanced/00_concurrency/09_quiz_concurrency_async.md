...
本节从 Q4. 以下代码的输出是什么？解释 `.await` 的作用、Q5. 以下代码能否编译？解释 `Pin` 的作用与Q6. 以下代码存在什么问题？如何修复？切入，剖析「异步（Async）编程」的核心内容。
...
- A. 类型的引用（Reference）可以被多个线程同时持有
- B. 类型的**所有权（Ownership）**可以安全地从一个线程转移到另一个线程
- C. 类型实现了 `Clone`
- D. 类型是 `'static` 的
...
- A. 前者用于单线程，后者用于多线程
- B. 前者线程安全（原子引用计数 + 阻塞锁），后者仅限单线程（非原子计数 + 运行时（Runtime）借用（Borrowing）检查）
- C. 两者性能完全相同
- D. 后者更安全，应总是优先使用
...
**解析**：两对组合是同一模式（共享所有权 + 内部可变性）在"单线程 / 多线程"两个世界的对应物。`Rc`/`RefCell` 零原子操作（Atomic Operations）、无锁，更快但 `!Send`；`Arc`/`Mutex` 付出原子指令与锁开销换取跨线程能力。选型规则：不跨线程就用 `Rc<RefCell<T>>`（D 错——跨线程用它是编译错误而非"更不安全"）。
...
**解析**：数据竞争 = 并发 + 共享 + 至少一方写 + 无同步。Rust 的"别名-可变分离"在单线程内已排除"同时读写"，跨线程时 `Send`/`Sync` 边界保证只有线程安全的共享方式能编译通过。注意精确表述：编译期保证的是**无数据竞争（UB 层面）**，不排除逻辑层面的竞态条件（race condition）或死锁——后者属于正确性问题，不是内存安全（Memory Safety）问题。
