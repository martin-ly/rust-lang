...
> **前置概念**: [Unsafe](../02_unsafe/01_unsafe.md) · [FFI Basics](01_rust_ffi.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md)
> **后置概念**: [Cross Compilation](../../06_ecosystem/05_systems_and_embedded/02_cross_compilation.md) · [WASI](../../06_ecosystem/05_systems_and_embedded/01_wasi.md)
...
> **边界要点**: FFI 的边界主要与**C++ 互操作**、**回调生命周期（Lifetimes）**、**异常模型**、**调试**和**构建系统**相关。
...
- [FFI Basics](01_rust_ffi.md) — FFI 基础
...
> **权威来源**: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html), [The Rust Programming Language](https://doc.rust-lang.org/book/ch20-01-unsafe-rust.html)
>
...
- **可变静态变量的线程安全**（编译错误）：FFI 全局状态常映射为 `static mut`——2024 edition 起创建其引用（Reference）为 deny（`static_mut_refs`），安全替代是 `static X: Mutex<T>` 或 `Atomic*`。此错误标志「C 式全局可变状态」未适配 Rust 别名规则。
- **`Box::into_raw` 后重复释放**（运行时 UB）：`into_raw` 移交所有权（Ownership）后原 `Box` 仍 drop、或两次 `from_raw`——双重释放。配对纪律：`into_raw` 与 `from_raw` 必须 1:1 且所有权流向文档化（谁负责最终 `from_raw` + drop）。
...
- **回调生命周期与 `Box::into_raw` 泄漏**：注册到 C 的回调（`into_raw` 移交）若无对应「注销时 `from_raw`」路径则永久泄漏——泄漏不是 UB 但违反 RAII 承诺。审计方法：grep 所有 `into_raw`，逐一标注回收点；无回收点的必须改为借用（Borrowing） + 文档化「C 不保留指针」的前提。

判定框架沿用 unsafe 五步清单（指针有效/初始化/别名/生命周期/panic 安全），FFI 特化项是「跨边界的所有权（Ownership）流向图」——每个指针的分配方、持有期、释放方三方信息必须可文档化，缺一项即设计缺口。
...
> **修正**: `static mut` 在 Rust 中几乎永远不应使用。它绕过所有权（Ownership）和借用（Borrowing）检查，允许数据竞争。FFI 回调若需维护全局状态，应使用 `Mutex<T>`、`RwLock<T>` 或原子类型。`static mut` 的访问需要 `unsafe`，且即使单线程 FFI 调用也可能因信号处理或重入导致未定义行为。[来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)]
...
> 这与 C 的 `malloc`/`free` 管理相同——Rust 的 unsafe 边界将责任完全转移给程序员。[来源: [Rust Standard Library](https://doc.rust-lang.org/std/index.html)]
...
> [来源: [The Rust FFI Omnibus](https://jakegoulding.com/rust-ffi-omnibus/)] · [来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)]
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：`extern "C"` 块中声明的函数在调用时为什么通常需…、测验 2：`#[repr(C)]` 对 struct 的内存布局有什么…、测验 3：`*const T` 与 `&T` 在 Rust 中的主要区…、测验 4：向 C 函数传递 Rust `String` 存在什么风险？…等5个方面，本节逐一说明其要点。
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| 安全契约 | FFI 边界两侧须共同维持内存、线程与错误契约 | 本文 §1.1 |
| 布局控制 | `#[repr(C)]` 保证 C 兼容的字段布局与对齐 | 本文 §1.2 |
| 回调机制 | 闭包 → 函数指针须经 `extern "C" fn` 与裸指针上下文传递 | 本文 §1.3 |
| Panic 安全 | panic 不得越过 FFI 边界（`catch_unwind` 或 `extern "C-unwind"`） | 本文 §2.3 |
| 线程边界 | `static mut`、跨线程回调须显式同步 | 本文 §2.2 |

## 🔗 概念关系

- **上位（is-a）**：[Rust FFI](01_rust_ffi.md) 的高级专题（复杂类型 / 线程 / panic 安全）。
- **下位（实例）**：复杂类型映射、回调模式、panic 安全模式（本文 §三 模式矩阵）。
- **组合**：与 [Unsafe 边界全景](../02_unsafe/02_unsafe_boundary_panorama.md)（边界五 FFI 布局契约）、[内存模型](../02_unsafe/06_memory_model.md) 组合。
- **依赖**：依赖 [Unsafe](../02_unsafe/01_unsafe.md) 的契约推理。
