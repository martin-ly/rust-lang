...
- **系统级动态产物**：`dylib`（Rust 动态库——Rust 版本/编译器必须严格匹配，几乎只用于 Rust 插件系统）、`cdylib`（C 动态库 `.so`/`.dll`/`.dylib`——FFI 导出标准形态，只暴露 `extern` 符号，Rust 运行时（Runtime）可被裁剪）。
...
**何时仍需 `no_mangle`**：当符号要被 C/汇编/链接器脚本/`dlsym` 按**固定字面名**引用（Reference）时，仍必须用 `#[unsafe(no_mangle)]`（通常配 `extern "C"`）或 `#[unsafe(export_name = "...")]`；否则 v0 混淆名无法被外部按名定位。
...
3. **macho 目标 `#[link_section]` 非法说明符报错**：macOS 目标的段/节说明符采用 Mach-O `segname,sectname` 形式，其中 `segname` 与 `sectname` 各最多 **16 字节**（含终止 NUL 的字段宽度，对应 Mach-O `section_64` 的 `char sectname[16]` / `char segname[16]`）；非法说明符现在报错（发布说明："Error on invalid macho `link_section` specifier"）。

> **已核对**：mach-o `section_64` 的 `sectname[16]` 与 `segname[16]` 字段宽度决定 `segname,sectname` 形式的两部分各最多 16 字节（含终止 NUL）。非法字符集/段名规则请进一步复核 Rust Reference 与目标 Mach-O ABI（来源：版本页 §7；[OS X ABI Mach-O File Format Reference](https://github.com/aidansteele/osx-abi-macho-file-format-reference)）。
