...
- [FFI 深度解析：C ABI、调用约定与链接](#ffi-深度解析c-abi调用约定与链接)
  ...
  - [九、常见陷阱](#九常见陷阱)
  - [十、边界测试 / 反例](#十边界测试--反例)
    - [10.1 反例：未加 `#[repr(C)]` 的结构体传给 C](#101-反例未加-reprc-的结构体传给-c)
    - [10.2 反例：`CString` 生命周期导致的悬垂指针](#102-反例cstring-生命周期导致的悬垂指针)
    - [10.3 反例：C 变长参数类型错配（运行时 UB）](#103-反例c-变长参数类型错配运行时-ub)
    - [10.4 反例：用 C `free` 释放 Rust `Box`](#104-反例用-c-free-释放-rust-box)
  - [十一、嵌入式测验](#十一嵌入式测验)
    - [测验 1：ABI 字符串的选择](#测验-1abi-字符串的选择)
    - [测验 2：`#[repr(C)]` 的作用](#测验-2reprc-的作用)
    - [测验 3：panic 跨越 FFI 边界](#测验-3panic-跨越-ffi-边界)
    - [测验 4：符号可见性](#测验-4符号可见性)
  - [十二、国际权威参考](#十二国际权威参考)
