...
- A. 宏在编译期按模式匹配展开成代码后再参与编译，函数在运行期调用
- B. 宏一定比函数执行得快
- C. 函数不能接受可变数量的参数
- D. 宏自动进行类型检查，函数不做
...
**解析**：宏是**代码生成器**：`vec![1, 2, 3]` 在编译早期展开为构造 `Vec` 的语句序列，之后与普通代码无差别。B 错：展开后的代码与手写代码性能相同（有时宏生成更多代码反而更慢）。C 错：`println!` 的"可变参数"正是用宏实现的，但函数也可以泛型/可变长切片模拟；D 恰好相反：宏展开**前**无类型信息，类型检查作用于展开结果。
...
- B. 属性宏（attribute macro）可作用于函数、结构体等项，读取并改写它们
