...
- **形式化定义**：宏展开是编译管道中「解析后、类型检查前」的语法变换 `T : TokenTree → TokenTree`，满足卫生性（hygiene）——展开代码的标识符绑定按「定义点/调用点/混合点」三种 span 解析，使宏不意外捕获也不意外暴露名字。卫生宏的形式化可追溯到 Kohlbecker et al. (1986) 的 hygiene 条件。
...
1. **宏类型对比矩阵**：声明宏（`macro_rules!`：模式 → 转录，卫生，能力限于「语法结构变换」，调试靠 `cargo expand`）；函数式过程宏（`my_macro!(tokens)`：任意 token 变换，最自由）；derive 宏（`#[derive(X)]`：只能附加代码、不能修改原项）；属性宏（`#[my_attr]`：可改写/替换被标注的项，权力最大）。权力递增序：声明宏 < 函数式 < derive < 属性宏；调试难度同序递增。
...
- **配套反例**：展示「片段限制违反」（`expr` 后接非法 token 的展开错误）、「卫生性边界」（宏内引用调用点未导入的路径，2018 edition 下需 `::std::` 全路径）、「`$crate` 元变量」（宏内引用定义 crate 自身路径的唯一正确方式——重导出场景下裸 crate 名会解析失败）。
...
- **命题「卫生宏完全防止命名冲突」** —— 部分成立。`macro_rules!` 的卫生性只覆盖**局部变量与 let 绑定**：宏内 `let x = ...` 不会污染调用点的 `x`；但类型名、函数名、trait 方法、生命周期与标签**不卫生**——宏内定义 `struct Foo;` 会在调用点作用域产生真实项，多次调用即冲突。
...
| 可变参数数量 | `macro_rules!` | §5.1 决策树 Q1 | 函数与泛型无法表达变参 |
