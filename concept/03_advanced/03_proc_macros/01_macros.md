...
宏（Macro）是 Rust 的编译期代码生成机制，三个权威定义视角：

- **Wikipedia 权威定义**：宏（Macro）是「把输入文本/语法结构按规则变换为输出代码」的元编程设施，谱系从 C 预处理器的文本替换，到 Lisp 的同像性（homoiconic）宏，到 Rust 的「token 树 + 卫生性」宏——Rust 位于「结构化但非完全同像」的位置。
- **TRPL 官方定义**：Rust 有两类宏——声明宏（Declarative Macro）（`macro_rules!`，模式匹配（Pattern Matching）驱动的「宏即规则」）与过程宏（Procedural Macro）（`proc_macro`，Rust 函数操作 `TokenStream`）。宏与函数的本质差异：宏在编译期展开为代码（「写代码的代码」），函数在运行期执行；宏可接受可变参数与语法片段，函数不能。
- **形式化定义**：宏（Macro）展开是编译管道中「解析后、类型检查前」的语法变换 `T : TokenTree → TokenTree`，满足卫生性（hygiene）——展开代码的标识符绑定按「定义点/调用点/混合点」三种 span 解析，使宏不意外捕获也不意外暴露名字。卫生宏的形式化可追溯到 Kohlbecker et al. (1986) 的 hygiene 条件。

判定一个元编程需求用哪类宏：输入是「Rust 语法片段的重复模式」→ `macro_rules!`；需要「分析类型结构/生成复杂代码」→ 过程宏（derive/attribute/function-like 三选一）。
...
> **[Wikipedia: Metaprogramming](https://en.wikipedia.org/wiki/Metaprogramming)** Metaprogramming is a programming technique in which computer programs have the ability to treat other programs as their data. It means that a program can be designed to read, generate, analyze or transform other programs, and even modify itself while running.
> **[Wikipedia: Hygienic macro](https://en.wikipedia.org/wiki/Hygienic_macro)** Hygienic macros are macros whose expansion is guaranteed not to cause the accidental capture of identifiers. A hygienic macro system preserves lexical scoping and ensures that binding structure is respected during macro expansion.
...
宏的属性矩阵沿「类型 × 跨语言 × 管道位置」三个维度组织：

1. **宏类型对比矩阵**：声明宏（Declarative Macro）（`macro_rules!`：模式 → 转录，卫生，能力限于「语法结构变换」，调试靠 `cargo expand`）；函数式过程宏（Procedural Macro）（`my_macro!(tokens)`：任意 token 变换，最自由）；derive 宏（`#[derive(X)]`：只能附加代码、不能修改原项）；属性宏（`#[my_attr]`：可改写/替换被标注的项，权力最大）。权力递增序：声明宏 < 函数式 < derive < 属性宏；调试难度同序递增。
2. **Rust 宏 vs 其他语言元编程**：C 预处理器（文本替换，无卫生性，无类型感知）；C++ 模板（类型级计算强，但错误在实例化点、无卫生概念）；Lisp 宏（同像性最优雅，语法代价是 S 表达式）；Zig comptime（统一「编译期函数」模型，无独立宏语言）。Rust 的取舍：宏是「二等公民但有明确边界」——不追求 comptime 的统一，换取「展开结果总是合法 token 树」的可推理性与卫生性。
3. **宏展开在编译管道中的位置**：`解析(tokenize/parse) → 宏展开（声明宏 + 过程宏，可递归嵌套，深度受 recursion_limit 约束）→ 名称解析 → 类型检查 → 单态化（Monomorphization）`。位置决定了两个关键推论：宏看不到类型信息（类型检查在展开后）；`cfg` 裁剪发生在展开早期（`#[cfg]` 内的代码可不合法地引用（Reference）不存在的模块（Module））。

矩阵用法：遇到宏相关编译错误，先在维度 3 定位阶段（展开期 vs 展开后），再在维度 1 确认该宏类型的能力边界——多数「宏做不到」的需求是选错了宏类型。
...
1. **宏展开在语义分析之前** ⟹ 宏无法访问类型信息。过程宏（Procedural Macro）接收的是 `TokenStream`，不是带有类型标注的 AST。
...
| 特性 | C 预处理器 | Rust 声明宏（Declarative Macro） | Rust 过程宏（Procedural Macro） |
|:---|:---|:---|:---|
...
> **[Rust Reference: Macro matchers](https://doc.rust-lang.org/reference/introduction.html)** 展开过程 = 模式变量替换 + 重复展开；vec![1, 2, 3] 匹配 $($x:expr),* 并展开为对应的数组初始化代码。 ✅ 已验证
> **[TRPL Ch19.5](https://doc.rust-lang.org/book/ch19-05-advanced-functions-and-closures.html)** `macro_rules!` performs compile-time pattern matching on token trees, with fragment specifiers matching syntactic categories (`expr`, `ty`, `ident`) rather than semantic types. ✅ 已验证
...
> 核心洞察：卫生性（Hygiene）是贯穿所有宏类型的核心安全保证，而非仅属于声明宏（Declarative Macro）的局部特性。[💡 原创分析](../../00_meta/00_framework/methodology.md)
> [来源: [Rust Reference: Macros](https://doc.rust-lang.org/reference/macros.html)]
...
> [来源: [Rust Reference — Macros](https://doc.rust-lang.org/reference/macros.html)]
> 建议在犹豫"这里该写宏还是泛型（Generics）"时沿树遍历，避免过度使用宏。
> 核心洞察：宏是最后手段——仅当可变参数、DSL 或语法树操作需求超出函数/泛型（Generics）/const fn 能力边界时才应选择。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> 核心洞察：宏与函数在 λ-演算中属于不同计算阶段（语法变换 vs 语义求值），不可互换。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> 核心洞察：Rust 宏系统被刻意设计为非图灵完备，递归深度、I/O、类型信息三重受限。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> [来源: [Rust Reference — Macros](https://doc.rust-lang.org/reference/macros.html)]
> 当需要跨模块（Module）代码分析、类型查询或自定义优化时，应转向 lint 工具或编译器插件，而非过程宏。
> 核心洞察：过程宏是编译管道特定阶段的局部变换器，不具备全局符号表访问或后端 IR 修改能力。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> 核心洞察：过程宏的修改范围严格受限于其被显式调用的 item，无法像文本替换一样"查找替换"任意代码。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **[Rust Reference: Hygiene](https://doc.rust-lang.org/reference/macros-by-example.html#hygiene)** 定理：Rust 宏系统是卫生的——宏内部定义的变量不会与外部变量冲突，宏不会意外捕获外部变量。这是 macro_rules! 和过程宏的核心保证。✅ 已验证
> **[TRPL Ch19.5](https://doc.rust-lang.org/book/ch19-05-advanced-functions-and-closures.html)** 对比：C 预处理器 #define 不卫生，如 SQUARE(a+b) 展开为 a+b*a+b 导致运算优先级错误；Rust 的卫生宏避免此类问题。✅ 已验证
...
| **T4** | 编译期展开 ⟹ 零运行时（Runtime）开销 | 性能保证 | T1 + T2 | 生成代码含大量冗余表达式 | `macro_rules!` 内联展开 vs 函数调用 |
...
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/04_inter_layer_map.md) §4.3 "async 正确性"
...
本节示例按「声明宏建直觉、过程宏建工程能力」的路径组织，所有展开行为以 rustc 1.97 + `cargo expand` 可复现：

- **正确示例：`macro_rules!` 声明宏**：展示「片段分类符 × 重复 × 递归」三要素——`vec!` 式列表宏（`$( $x:expr ),*` 重复）、`hashmap!` 式键值对宏（`$( $k:expr => $v:expr ),*` 带分隔符）、以及「内部规则」（`@impl` 前缀约定）拆分复杂宏为可读子规则。阅读重点是「匹配臂按序尝试」与「片段匹配后的后续 token 限制」（如 `expr` 后只能跟 `=>`、`;`、`,` 等少数 token——这是最常见的宏编写错误）。
- **正确示例：Derive 过程宏框架**：展示 `proc-macro = true` 的 crate 骨架——`#[proc_macro_derive(Name, attributes(helper))]` 注册、`syn::parse_macro_input!` 解析、`quote!` 生成。derive 宏只能「附加 impl」，不能改原结构——这一限制是它与属性宏的分界。
- **配套反例**：展示「片段限制违反」（`expr` 后接非法 token 的展开错误）、「卫生性边界」（宏内引用（Reference）调用点未导入的路径，2018 edition 下需 `::std::` 全路径）、「`$crate` 元变量」（宏内引用定义 crate 自身路径的唯一正确方式——重导出场景下裸 crate 名会解析失败）。

学习路径：先用 `macro_rules!` 复刻一个标准库宏（如 `dbg!`），再用 derive 宏复刻一个简化版 `Debug`——两个练习覆盖宏系统 80% 的工程场景。
...
pub fn my_debug_derive(input: TokenStream) -> TokenStream {
    // 解析 input（结构体（Struct）/枚举（Enum）定义）
...
宏系统的边界由「展开发生在类型检查之前」这一事实决定，本节的四个极限测试各压一条边界：

1. **错误信息定位**：宏展开后的代码报错指向展开结果而非调用点——`macro_rules!` 可用 `$crate` 与 `#[track_caller]` 改善，过程宏用 `syn::Error::new_spanned` 把错误绑回输入 token；
2. **递归深度**：`macro_rules!` 递归受 `recursion_limit`（默认 128）约束，超深递归需提升属性或改用增量构建模式（push-down accumulation）；
3. **TokenStream 解析失败**：过程宏输入是 token 流而非 AST，非法语法要等 `syn::parse` 才报错——友好诊断要求宏作者主动校验而非 `unwrap`；
4. **卫生性边界**：`macro_rules!` 对标识符卫生但对生命周期（Lifetimes）/标签/项名（如 `'label`、`loop` 标签）不卫生，跨宏传递标签需显式设计。

这四个测试覆盖了宏开发中 90% 的「宏写对了但用起来出错」场景。
...
- **命题「宏生成代码总是安全的」** —— 错误。宏展开后的代码享有与手写代码完全相同的权限：声明宏可以生成 `unsafe` 块，过程宏可以生成任意 token（包括调用 `std::mem::transmute`）。宏的安全性取决于**宏作者是否在生成代码中维持了不变量**，审查宏 crate 时应把展开结果（`cargo expand`）纳入审计范围。
- **命题「卫生宏完全防止命名冲突」** —— 部分成立。`macro_rules!` 的卫生性只覆盖**局部变量与 let 绑定**：宏内 `let x = ...` 不会污染调用点的 `x`；但类型名、函数名、trait 方法、生命周期（Lifetimes）与标签**不卫生**——宏内定义 `struct Foo;` 会在调用点作用域产生真实项，多次调用即冲突。

判定准则：评估一个宏的调用安全性时，问三个问题——展开后引入哪些名字？是否生成 `unsafe`？失败时错误信息指向哪里？
...
> 核心洞察：卫生性与编译器二次检查共同构成宏安全的双重防线，但二者均不能阻止宏故意生成 unsafe 代码。[💡 原创分析](../../00_meta/00_framework/methodology.md)
> [来源: [Rust Reference: Built-in Macros](https://doc.rust-lang.org/reference/macros.html#built-in-macros)]
> **[Rust Reference: Hygiene](https://doc.rust-lang.org/reference/macros-by-example.html#hygiene)** macro_rules! 的局部变量和 macro_export 跨 crate 是完全卫生的；但标签（labels）和过程宏生成的标识符可能存在边界情况。✅ 已验证
...
> **[TRPL Ch19.5](https://doc.rust-lang.org/book/ch19-05-advanced-functions-and-closures.html)** 认知类比：macro_rules! 像"文本模板引擎"（但操作 Token 而非纯文本），过程宏像"编译器插件"。✅ 已验证
> **[Rust Reference: Macros](https://doc.rust-lang.org/reference/macros.html)** 反直觉点：宏展开在类型检查之前，宏无法"知道"类型信息；过程宏只能解析 Token 语法，无法访问类型表。✅ 已验证
...
> **层次一致性（Coherence）标注**：本节内容属于 L3 宏系统的工程实践延伸，涵盖过程宏生态 crate 的使用、声明宏（Declarative Macro）高级语法、编译期计算替代趋势，以及宏系统的演进方向。需在理解 §三形式化根基与 §七示例反例后阅读。
...
> **[The Little Book of Rust Macros](https://veykril.github.io/tlborm/)** 手写 TokenStream 拼接极易出错；`syn` 和 `quote` 通过类型化 AST 操作大幅降低过程宏的开发难度。✅ 已验证
...
> **[Rust Reference: Repetition](https://doc.rust-lang.org/reference/introduction.html)** `macro_rules!` 的重复模式使用 `$($name:kind)` 后跟分隔符和重复计数器。分隔符可以是任意 token（逗号、分号、竖线等），重复计数器为 `*`（零或多）、`+`（一或多）、`?`（零或一，Rust 1.32+）。✅ 已验证
> **[The Little Book of Rust Macros](https://veykril.github.io/tlborm/)** 多个重复器的匹配需要遵循特定规则：重复模式的长度必须一致，除非其中一个为零宽度。✅ 已验证
...
> **[Rust Reference: Macros by Example](https://doc.rust-lang.org/reference/macros-by-example.html)** `?` 重复计数器在宏参数匹配时非常有用，表示某个模式整体可选。但注意 `?` 不能嵌套使用。✅ 已验证
...
| 宏 | 输入 | 输出 | 典型场景 |
|:---|:---|:---|:---|
| `concat!(...)` | 字面量/数字 | `&'static str` | 拼接模块（Module）路径、版本号 |
...
> **[Rust Reference](https://doc.rust-lang.org/reference/introduction.html)** `cfg!` 与 `#[cfg]` 的本质区别：`cfg!` 是运行期 bool 值（代码始终编译），`#[cfg]` 是编译期条件（代码可能不被编译）。✅ 已验证
...
|:---|:---|:---|:---|:---|
| quote 包装 | 在函数体前后注入代码 | 低 | 高（不改动内部结构） | 低（闭包（Closures）可捕获） |
...
若函数可能通过 `return` 或 `?` 提前退出，简单地在 block 末尾插入代码会失效。此时应采用**闭包（Closures）包装**策略，将原函数体捕获为闭包执行：
...
> **跨层映射**: 本文件属性宏示例 ↔ [`01_foundation/02_type_system/01_type_system.md`](../../01_foundation/02_type_system/01_type_system.md) § 泛型（Generics）与 trait bound（签名保留中的泛型参数）
> **跨层映射**: 本文件 `Fold` trait ↔ [`04_formal/02_type_theory.md`](../../04_formal/00_type_theory/01_type_theory.md) § 语法树归纳定义（AST 递归结构的归纳遍历）
...
> **[Rust Reference](https://doc.rust-lang.org/reference/introduction.html)** 声明宏 2.0 的完全稳定尚无明确时间表。当前稳定 Rust 中，`macro_rules!` + `pub use` 是模拟 `macro` 可见性控制的最佳方案。✅ 已验证
...
<!-- 层级一致性（Coherence）: L3 知识溯源 — 每个论断的可信度评级与来源锚定 -->
...
| 宏非图灵完备 | [Rust Reference: recursion limit](https://doc.rust-lang.org/reference/introduction.html) · [TLBORM] | ✅ |
...
## 判定表：宏 vs 函数/泛型/const fn 判定

| 场景/条件 | 判定结论 | 依据（定理/规则） | 反例或失效条件 |
|:---|:---|:---|:---|
| 普通计算逻辑 | 函数 / 泛型（Generics） | §5.1 决策树 A3 | 用宏 ⟹ 错误延迟到展开后、信息晦涩（§5.2 F3） |
| 编译期常量计算 | `const fn` 优先 | §5.1 决策树 Q3/Q4 | `const fn` 表达力不足 ⟹ 宏或 `build.rs` |
| 可变参数数量 | `macro_rules!` | §5.1 决策树 Q1 | 函数与泛型（Generics）无法表达变参 |
| DSL / 自定义语法 | 过程宏 | §5.1 决策树 Q5 | 简单文本替换场景 ⟹ 声明宏足够 |
| 根据类型生成代码 | 过程宏（derive） | §5.2 反例：宏不感知类型 | `macro_rules!` 无法按类型分支 |
...
> **权威来源**: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html), [The Rust Programming Language](https://doc.rust-lang.org/book/ch20-05-macros.html), [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)
...
> **相关问题树**: [性能优化问题树](../../00_meta/04_navigation/10_problem_graph.md#六性能优化问题树)
...
1. 阅读 `crates/c11_macro_system_proc/` 中与"声明宏与过程宏"相关的源码和示例
2. 运行 `cargo test -p c11_macro_system_proc` 验证理解
3. 完成 `exercises/src/macros/` 中的练习任务
4. 完成 [L3 宏系统测验](10_quiz_macros.md) 验证理解程度
...
本节 4 道测验覆盖宏系统的核心判别能力，按认知层级排列：

- 测验 1（记忆层）：声明宏与过程宏在编译管线中的位置差异（前者在解析后展开、后者操作 token 流并由独立 crate 编译执行）；
- 测验 2（理解层）：`macro_rules!` 模式匹配臂的匹配顺序与 `tt`/`expr`/`ty` 片段分类符的能力差（如 `expr` 后只能跟 `=>`/`,`/`;`）；
- 测验 3（应用层）：派生宏实战——从 `TokenStream` 解析到生成 `impl` 块的完整链路；
- 测验 4（分析层）：卫生性案例判别——给定展开结果判断哪个标识符会冲突、哪个不会。

作答建议：测验 3 配合 `cargo expand` 实操；其余可先默写规则再核对答案。
...
**题目**: 你想为一个结构体（Struct）自动实现 `Builder` 模式（每个字段都有 setter 方法）。应该使用哪种宏？
...
| 选项 | 可行性 | 评价 |
|:---|:---|:---|
| A | ❌ | 声明宏无法访问结构体（Struct）字段的 Token 流，只能做文本替换 |
