...
- **Wikipedia 权威定义**：宏是「把输入文本/语法结构按规则变换为输出代码」的元编程设施，谱系从 C 预处理器的文本替换，到 Lisp 的同像性（homoiconic）宏，到 Rust 的「token 树 + 卫生性」宏——Rust 位于「结构化但非完全同像」的位置。
- **TRPL 官方定义**：Rust 有两类宏——声明宏（`macro_rules!`，模式匹配驱动的「宏即规则」）与过程宏（`proc_macro`，Rust 函数操作 `TokenStream`）。宏与函数的本质差异：宏在编译期展开为代码（「写代码的代码」），函数在运行期执行；宏可接受可变参数与语法片段，函数不能。
...
3. **宏展开在编译管道中的位置**：`解析(tokenize/parse) → 宏展开（声明宏 + 过程宏，可递归嵌套，深度受 recursion_limit 约束）→ 名称解析 → 类型检查 → 单态化`。位置决定了两个关键推论：宏看不到类型信息（类型检查在展开后）；`cfg` 裁剪发生在展开早期（`#[cfg]` 内的代码可不合法地引用不存在的模块）。
...
4. **卫生性边界**：`macro_rules!` 对标识符卫生但对生命周期/标签/项名（如 `'label`、`loop` 标签）不卫生，跨宏传递标签需显式设计。
...
|:---|:---|:---|:---|
| 普通计算逻辑 | 函数 / 泛型 | §5.1 决策树 A3 | 用宏 ⟹ 错误延迟到展开后、信息晦涩（§5.2 F3） |
