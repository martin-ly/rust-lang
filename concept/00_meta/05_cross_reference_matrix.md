...
> **Summary**: ```rust,compile_fail // 概念: 交叉引用的反向验证 // 若文件 A 引用文件 B 的概念，但文件 B 不存在或重命名，读者会看到死链 // 模拟: 假设引用了一个不存在的文件 // [不存在的概念](./README.md) fn main() { // 这不是 Rust 编译错误，而是文档维护问题 // 但 Rust 的compiler错误信息模式类似： // "cannot find value `x` in this scope" — 类似于 "cannot find file" let_y = x; // ❌ 编译错误: cannot find value `x` }
...
| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 05 Cross Reference Matrix 结构化定义 ⟹ 学习者认知锚点可建立 | 本文件定义了元层结构 | 支持上层概念定位 | 高 |
...
### 测验 1：本文档《Cross Reference Matrix（交叉引用矩阵）》在 Rust 知识体系中属于哪一层级的元数据？（理解层）

**题目**: 本文档《Cross Reference Matrix（交叉引用矩阵）》在 Rust 知识体系中属于哪一层级的元数据？
...
### 测验 2：《Cross Reference Matrix（交叉引用矩阵）》的主要用途是什么？（理解层）

**题目**: 《Cross Reference Matrix（交叉引用矩阵）》的主要用途是什么？
