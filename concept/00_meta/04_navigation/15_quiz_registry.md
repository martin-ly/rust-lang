...
> **Summary**: Unified index of the 21 standalone quizzes, per-page embedded quizzes, exercises test suites, Brown inventories, and the rustlings mapping, with question counts, type composition, difficulty distribution, and backlink status.
...
**合计**：309 题（🟢 68 / 🟡 164 / 🔴 77），难度标注率 100%，题型多样性每 quiz ≥3 种（19 个为 4 种）。
**W3-b 缺口已闭合**：L0/L7 独立 quiz 已建并扩至 15 题；L6 生态子领域 quiz 已补 4 个（网络/数据库/安全测试/领域应用）；
concept→quiz 回链 21/21 完成（`scripts/add_quiz_backlinks.py` 幂等批量补链，`scripts/refresh_quiz_registry.py` 从文件实测重算统计）。
...
③ 难度标注率（目标 100%）；④ quiz→concept 与 concept→quiz 双向链接率（W3-b 已完成 21/21 回链）。

> 本页统计由 `scripts/check_quiz_system.py` 机器复核；最后核对：2026-07-13（`--strict` 0 失败 / 0 警告；回链 21/21；309/309 题难度标注 100%）。
