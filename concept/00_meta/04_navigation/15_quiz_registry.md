...
**Summary**: Unified index of the 20 standalone quizzes, per-page embedded quizzes, exercises test suites, Brown inventories, and the rustlings mapping, with question counts, type composition, difficulty distribution, and backlink status.
...
> **回链状态**: ✅ 对应 concept 权威页已回链 quiz（20/20，W3-b 批量补链完成）

---

## 一、独立 quiz（20 个 / 269 题）
...
|:---|:---|:---:|:---|:---:|:---:|
| [01_quiz_meta_framework](../05_quizzes/01_quiz_meta_framework.md) | 元层框架与知识体系架构 | 10 | 代码阅读/单选/多选/判断 | 2/5/3 | ✅ |
...
| Quiz | 主题 | 题数 | 题型构成 | 难度分布（🟢/🟡/🔴） | 回链 |
|:---|:---|:---:|:---|:---:|:---:|
| [01_quiz_networking_async_ecosystem](../../06_ecosystem/13_quizzes/01_quiz_networking_async_ecosystem.md) | 网络与异步生态 | 10 | 代码阅读/单选/多选/判断 | 2/5/3 | ✅ |
| [02_quiz_database_storage](../../06_ecosystem/13_quizzes/02_quiz_database_storage.md) | 数据库与存储生态 | 10 | 代码阅读/单选/多选/判断 | 2/5/3 | ✅ |
| [03_quiz_security_testing](../../06_ecosystem/13_quizzes/03_quiz_security_testing.md) | 安全与测试生态 | 10 | 代码阅读/单选/多选/判断 | 2/5/3 | ✅ |

### L7 前沿层（`concept/07_future/05_quizzes/`，W3-b 新增）

| Quiz | 主题 | 题数 | 题型构成 | 难度分布（🟢/🟡/🔴） | 回链 |
|:---|:---|:---:|:---|:---:|:---:|
| [01_quiz_version_and_preview](../../07_future/05_quizzes/01_quiz_version_and_preview.md) | 版本演进、Edition 机制与前沿特性 | 10 | 代码阅读/单选/多选/判断 | 2/5/3 | ✅ |

**合计**：269 题（🟢 60 / 🟡 147 / 🔴 62），难度标注率 100%，题型多样性每 quiz ≥3 种（18 个为 4 种）。
**W3-b 缺口已闭合**：L0/L7 独立 quiz 已建；L6 生态子领域（网络/数据库/安全测试）quiz 已补 3 个；
concept→quiz 回链 20/20 完成（`scripts/utils/add_quiz_backlinks.py` 幂等批量补链，`scripts/update_quiz_registry.py` 重算统计）。
...
|:---|---:|---:|
| 00_meta | 38 | 112 |
| 01_foundation | 38 | 156 |
| 02_intermediate | 29 | 129 |
| 03_advanced | 28 | 113 |
| 04_formal | 27 | 122 |
...
③ 难度标注率（目标 100%）；④ quiz→concept 与 concept→quiz 双向链接率（W3-b 已完成 20/20 回链）。

> 本页统计由 `scripts/check_quiz_system.py` 机器复核；最后核对：2026-07-12（`--strict` 0 失败 / 0 警告；回链 20/20）。
