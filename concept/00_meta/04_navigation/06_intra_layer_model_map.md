...
> **符号约定**: 同于 `05_inter_layer_topology.md`
...
本节给出 L1 层的概念关系地图，核心结构是「一个根基 + 三个支柱」：

- **根基：所有权**——值与所有者的绑定关系，是其他三柱的共同前提；
- **支柱一：借用**——所有权的临时授予机制（`&T`/`&mut T` 的排他规则）；
- **支柱二：生命周期**——借用有效性的验证机制（存活区间的静态证明）；
- **支柱三：类型系统**——值的分类与操作约束（Sized/DST、泛型、trait 的入口）。

地图的读法：箭头方向是「依赖」——借用依赖所有权（无所有者无可借用）、生命周期服务借用、类型系统承载全部。学习顺序沿依赖方向：所有权 → 借用 → 生命周期 → 类型深化，反向跳跃是入门失败的首因。
...
L2 进阶概念层的四元组 **Trait · 泛型 · 内存管理 · 错误处理** 构成 Rust "抽象能力"的核心，四元关系的特点是泛型与 trait 互为底座：

- **四元关系拓扑**：泛型提供参数化，trait 提供约束——`fn f<T: Display>` 是二者最简组合；内存管理（`Box`/`Rc`/`Arc`）通过泛型实现（`Box<T>`），其语义由 trait 表达（`Drop`、`Deref`）；错误处理的 `Result<T, E>` 是泛型枚举，`?` 依赖 `From` trait 转换。
- **四元关系矩阵**：6 对关系中最强的是（泛型， trait）——monomorphization 使每对组合编译为专用代码；最微妙的是（内存管理，错误处理）——`Drop` 与 panic 展开的交互（panic 时析构仍运行，二次 panic 即 abort）。学习路径上四者不可孤立：先泛型语法 → trait 约束 → 智能指针作为泛型实例 → Result 作为 trait+泛型的综合应用。
...
L3 高级概念层的四元组 **并发 · 异步 · Unsafe · 宏** 是"信任与抽象"主题下的一体四面：

- **四元关系拓扑**：Unsafe 是中心节点——并发的 `Mutex`/channel、异步的 `Pin`、宏生成的代码，其安全性最终都归约为对少量 `unsafe` 块的审查。并发与异步通过 `Send`/`Sync` 耦合（Future 的 `Send` 性决定能否跨线程）；宏与三者的关系是"生成"——过程宏可生成并发/异步代码，但宏展开结果必须重新通过借用检查。
- **四元关系矩阵**：6 对关系中（并发，异步）最常被混淆——并发是"同时处理多任务"的结构，异步是"非阻塞等待"的机制，tokio 把二者组合但概念上独立（单线程 runtime 有异步无并行）。（Unsafe，宏）的交互点是宏卫生：宏无法意外创建未检查的不安全别名，`unsafe` 必须显式书写。
...
本节聚焦「L4 形式化层：线性逻辑 · 类型论 · 所有权形式化 · RustBelt · 验证工具链」，覆盖五元关系拓扑 与 五元关系矩阵。论述顺序由定义到边界：先明确「L4 形式化层：线性逻辑 · 类型论 · 所有权形式化 · RustBelt · 验证工具链」在「Rust 知识体系层次内模型间映射图」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「L4 形式化层：线性逻辑 · 类型论 · 所有权形式化 · RustBelt · 验证工具链」的判定标准，并指出它在全页论证链中的位置。
...
本节给出跨层概念映射的形式化依据——「L1 的借用」与「L4 的分离逻辑谓词」之间为何是同一事物的两个抽象层：

- **映射类型**：精化（refinement，L4 → L1 的抽象化）与实例化（L1 → L4 的具体化）——`&mut T` 精化自 `x ↦ v`（points-to 谓词），`x ↦ v` 实例化为「独占引用 + drop 义务」；
- **保持性质**：有效映射必须保持「可判定性」——L4 中可判定的性质映射到 L1 仍应可判定（借用检查的存在性即此性质的实例）；
- **映射失效点**：`unsafe` 区域是映射的断层——L1 的借用规则在 L4 对应「程序员手写的分离逻辑证明」，形式化工具（Creusot/Prusti）在断层处工作。

本节是知识库「L1-L7 纵向一致」主张的理论底座——概念跨层移动时语义不丢失的根据。
...
- [跨层依赖拓扑](05_inter_layer_topology.md) —— L0-L7 纵向关系
...
- [L4 所有权形式化](../../04_formal/01_ownership_logic/02_ownership_formal.md)
...
> **思维表征说明**: 此决策树是**层内模型选择的认知导航**——与跨层的 `05_inter_layer_topology.md` 不同，它回答「在同一层级（如 L1）内，面对具体问题时应该选择哪个模型」。
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：本文档《Rust 知识体系层次内模型间映射图》在 Rust…、测验 2：《Rust 知识体系层次内模型间映射图》的主要用途是什么？（…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层）的顺序逐层展开。
