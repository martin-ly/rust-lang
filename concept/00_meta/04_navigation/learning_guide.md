...
> **关联文件**: [能力图谱 `competency_graph.md`](../00_framework/competency_graph.md) · [双维认知矩阵 `cognitive_dimension_matrix.md`](../00_framework/cognitive_dimension_matrix.md)
---
...
- [Rust 知识体系学习指南（Learning Guide）](#rust-知识体系学习指南learning-guide)
  ...
  - [二、按背景的详细起点指南](#二按背景的详细起点指南)
    ...
    - [2.2 C/C++ 开发者路径 概念迁移理论 — 已有 RAII/指针/内存管理知识可作为正向迁移; ISO C++20 § 作为对比基准](#22-cc-开发者路径-概念迁移理论--已有-raii指针内存管理知识可作为正向迁移-iso-c20--作为对比基准)
    - [2.3 Java/Go 开发者路径 Java JLS § — GC 背景下的所有权概念缺失; Go Language Specification — CSP 并发模型与 Rust 所有权并发的对比](#23-javago-开发者路径-java-jls---gc-背景下的所有权概念缺失-go-language-specification--csp-并发模型与-rust-所有权并发的对比)
    - [2.4 Haskell/ML 开发者路径 Hindley-Milner 类型推断的已有知识可作为类型系统学习的前置; Wadler — *Theorems for Free!* / 1989; GHC User's Guide](#24-haskellml-开发者路径-hindley-milner-类型推断的已有知识可作为类型系统学习的前置-wadler--theorems-for-free--1989-ghc-users-guide)
...
> **[Bloom Taxonomy 2001; 认知科学前置依赖原则](https://cft.vanderbilt.edu/guides-sub-pages/blooms-taxonomy/)** 学习路径基于认知层级和前置依赖设计。
...
> [Make It Stick — Brown, Roediger & McDaniel / 2014](https://www.hup.harvard.edu/books/9780674729018); 核心论点: 间隔重复（spaced repetition）和检索练习（retrieval practice）比被动重读更有效; > Willingham — *Why Don't Students Like School?* / 2009。
...
### 2.2 C/C++ 开发者路径 [概念迁移理论 — 已有 RAII/指针/内存管理知识可作为正向迁移; ISO C++20 § 作为对比基准](../00_framework/methodology.md)
...
### 2.3 Java/Go 开发者路径 [Java JLS § — GC 背景下的所有权概念缺失; Go Language Specification — CSP 并发模型与 Rust 所有权并发的对比](https://docs.oracle.com/javase/specs/jls/se23/html/jls-0.html)
...
### 2.4 Haskell/ML 开发者路径 [Hindley-Milner 类型推断的已有知识可作为类型系统学习的前置](https://en.wikipedia.org/wiki/Hindley%E2%80%93Milner_type_system); Wadler — *Theorems for Free!* / 1989; [GHC User's Guide](https://downloads.haskell.org/ghc/latest/docs/users_guide/)
...
> **[concept/ 目录结构](../02_sources/international_authority_index.md); [00_meta/inter_layer_map.md](../02_sources/international_authority_index.md)** 按 L0-L7 层级组织的 27+ 概念文件。
...
> **[Make It Stick (Brown et al. 2014); 间隔重复研究](https://www.hup.harvard.edu/books/9780674729018)** 三遍阅读法和间隔重复基于认知科学证据。
...
> **深入阅读**: 本知识体系的元理论分析见 [`semantic_space.md`](../00_framework/semantic_space.md)；全局概念索引见 [`concept_index.md`](concept_index.md)；层间依赖映射见 [`inter_layer_map.md`](inter_layer_map.md)。

---

> **[rustc 错误码索引](https://doc.rust-lang.org/error_codes/error-index.html); [Rust Compiler Error Index](https://doc.rust-lang.org/error_codes/error-index.html)** 错误码映射基于 rustc 官方文档和概念知识体系的一致性标注。
...
| 错误码 | 错误信息关键词 | 根本原因 | 推荐阅读 | 对应章节 |
|:---|:---|:---|:---|:---|
| **E0277** | `Rc<Cell<T>>` cannot be sent between threads safely | 类型未实现 Send | `03_advanced/00_concurrency/01_concurrency.md` | §2.1 Send/Sync 判定矩阵 |
| **E0373** | closure may outlive the current function | 闭包捕获的生命周期不够长 | `03_advanced/00_concurrency/01_concurrency.md` | §7.4 反例：跨线程共享 Rc |
| **E0716** | temporary value dropped while borrowed | async 块中借用跨越 await | `03_advanced/01_async/02_async.md` | §8.6 跨越 await 的 Send 约束 |

#### Unsafe 与 FFI 类错误（L3 高级概念）

| 错误码 | 错误信息关键词 | 根本原因 | 推荐阅读 | 对应章节 |
|:---|:---|:---|:---|:---|
| **E0133** | use of `unsafe` block | （信息性提示，非错误） | `03_advanced/02_unsafe/03_unsafe.md` | §1.4 形式化定义 |
| **E0569** | an `unsafe` trait cannot be implemented safely | 试图 safe impl unsafe trait | `03_advanced/02_unsafe/03_unsafe.md` | §2.1 Unsafe 操作分类矩阵 |
...
> **[Niko Matsakis — Teaching Rust Through Errors (RustConf 2023); Rust Compiler Team — Diagnostic UX Guidelines](https://smallcultfollowing.com/babysteps/)** 本章节基于 Rust 编译器团队的教学研究和认知科学原理，提出"错误驱动的概念学习"方法论。
