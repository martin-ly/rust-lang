...
> **后置概念**: [Concept Audit Guide](../03_audit/concept_audit_guide.md)
...
本节从 SUMMARY.md 占位文件 ✅ 已解决、内容文件中的 LINK_PLACEHOLDER、自动双语标注残留与重复文件已清理切入，剖析「已知遗留问题」的核心内容。

### 6.1 SUMMARY.md 占位文件 ✅ 已解决

`concept/SUMMARY.md` 中的 22 个 `LINK_PLACEHOLDER` 条目已替换为 `00_meta/placeholders/placeholder-N.md`。

- **解决方案**: 创建了 22 个占位文件，保留导航结构，同时使 `mdbook build` 成功通过。
- **状态**: 占位文件保留待创建主题的标题和说明，未来可逐步替换为真实内容。
...
1. **处理 SUMMARY.md 占位文件**：决定是为每个 LINK_PLACEHOLDER 创建文件，还是重构为纯文本列表。
...
1. 运行 `kb_auditor.py` 获取最新缺口统计。
2. 对比 `reports/kb_quality_dashboard.md` 中的指标变化。
3. 修正已完成项的状态，新增发现的问题。
4. 提交 PR 时引用本文件的相关章节。
...
| **L3** | 基础知识缺口已从"分散缺失"转变为"有文件覆盖、有索引导航、有状态追踪"的受控状态，但仍需解决 SUMMARY.md 占位文件和模板去同质化等工程问题。 |
