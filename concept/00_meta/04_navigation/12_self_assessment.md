...
> **使用方式**：先做题，再对答案。建议配合 [`07_learning_guide.md`](07_learning_guide.md) 使用。
...
1. **所有权 vs GC**：Rust 静态所有权与 Java/Go 追踪式 GC 的代价模型差异（暂停时间 vs 编译期约束）；
2. **trait vs 接口/类**：Rust trait 与 Go interface（结构式）/Java interface（名义式）/C++ 虚函数的调度与约束时机差异；
3. **错误处理范式**：`Result` 与异常（C++/Java）、多返回值（Go）的可组合性对比；
4. **并发模型**：共享状态 + 类型级安全（Rust）vs CSP（Go）vs Actor（Erlang）的失效模式谱系；
5. **零成本抽象**：Rust 泛型单态化与 Java 泛型擦除的二进制产物差异。

每题为「场景 → 选择正确对比」形式——目标是纠正「Rust 就是 X 语言的内存安全版」这类扁平类比。
...
1. **Cargo 工作流**：workspace/依赖解析/feature 统一的工程后果（与 npm/pip 的对照）；
2. **版本与 SemVer**：`Cargo.lock` 的角色、破坏性变更的语义化判定、MSRV 策略的取舍；
3. **unsafe 的工程纪律**：unsafe 边界封装、Miri 验证、安全注释的审查实践；
4. **构建与分发**：交叉编译、静态链接、`build.rs` 的适用边界；
5. **质量工具链**：clippy/rustfmt/cargo-audit/cargo-vet 各解决什么问题、CI 中的最小质量门组合。

每题为「工程场景 → 正确工具/决策」形式——目标是检验「知道生态中存在什么工具及其适用边界」，而非记忆命令行参数。
...
1. **move 与 Copy 的分界**：哪些类型隐式复制、哪些移动，「move 后使用」错误的修复策略谱系；
2. **借用排他性的精确表述**：「同时刻只能一个 `&mut` 或任意多个 `&`」的边界（NLL 下的非词法放宽实例）；
3. **生命周期省略规则**：三条规则的适用判定与「何时必须显式标注」；
4. **`'static` 的真实含义**：程序整个存活期 vs 「泄漏得来的永生」，`&'static str` 的特殊性；
5. **Sized 与 DST**：`str`/`[T]`/`dyn Trait` 为何只能间接使用（`Box`/`&`）；
6. **泛型单态化**：编译期代码生成的后果（二进制膨胀 vs 零成本）；
7. **类型推断的局部性**：`let` 推断的约束来源与 E0282 的修复；
8. **所有权与设计**：「谁该拥有数据」的结构设计判据（调用者拥有 ⟷ 被调者拥有 ⟷ 共享拥有）。

每题配错误码锚点——错题回链对应概念页而非仅看答案。
...
本组扩展测验检验形式化层的理解深度，覆盖八个判别点：

1. **线性逻辑基础**：contraction/weakening 缺失与 Rust move/drop 的对应；仿射 vs 线性的精确区分（Rust 是仿射）；
2. **λ 演算与类型化**：Church 编码的表达力演示、Y 组合子为何不可简单类型化；
3. **HM 类型推断**：合一算法的基本步骤、let 多态与 Rust 局部推断的差异；
4. **子类型与变型**：协变/逆变/不变的判定口诀，`&mut` 不变性的反例论证；
5. **分离逻辑**：`∗` 与 `∧` 的语义差异、frame 规则的局部推理意义；
6. **RustBelt 架构**：λRust/Iris/逻辑关系三组件的分工，`own`/`shr` 谓词的含义；
7. **借用检查可判定性**：NLL 的约束求解为何收敛（有限格 + 单调性）；
8. **形式化的工程边界**：λRust 未覆盖什么（完整 trait/弱内存序/真实 ABI）——「形式化范围意识」是本组的核心素养。

每题附论文锚点（TAPL、RustBelt POPL 2018、Girard 1987）。
...
> **权威来源对齐变更日志**: 2026-05-22 批量补充来源标注（TRPL、Rust Reference、Wikipedia、学术论文等） [Authority Source Sprint Batch 8](../02_sources/05_international_authority_index.md)
...
> - **<27 题正确**：建议从 [`07_learning_guide.md`](07_learning_guide.md) 基础路径重新开始
> **深入阅读**：
