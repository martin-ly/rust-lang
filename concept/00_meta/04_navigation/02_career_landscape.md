...
本节聚焦「核心数据」，覆盖美国薪资范围（2026）、全球薪资对比与岗位分布与增长率。论述顺序由定义到边界：先明确「核心数据」在「Rust 职业市场全景：2026 年数据与分析」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「核心数据」的判定标准，并指出它在全页论证链中的位置。
...
本节展开「Rust 职业市场全景：2026 年数据与分析」的技术细节：技能需求矩阵、行业分布与远程工作趋势。重点是类型签名、所有权语义与编译期约束如何相互作用，而不是 API 罗列；每个小节给出可编译的最小示例，并标注对应反例的失败规则。读完后应能解释：为什么这种写法能通过编译，而那种写法会被借用检查器或类型系统拒绝。
...
- [Application Domains](../../06_ecosystem/06_data_and_distributed/01_application_domains.md) — Rust 应用领域分析
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：本文档《Rust 职业市场全景：2026 年数据与分析》在…、测验 2：《Rust 职业市场全景：2026 年数据与分析》的主要用途…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层）的顺序逐层展开。
