...
- [跨层知识图谱（Inter-Layer Dependency Map）](#跨层知识图谱inter-layer-dependency-map)
  ...
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    - [测验 1：本文档《跨层知识图谱（Inter-Layer Dependency Map）》在 Rust 知识体系中属于哪一层级的元数据？（理解层）](#测验-1本文档跨层知识图谱inter-layer-dependency-map在-rust-知识体系中属于哪一层级的元数据理解层)
...
本节聚焦「关键跨层推理链（定理一致性）」，覆盖链 1: 内存安全完备性、链 2: 类型系统一致性与链 3: 异步正确性。论述顺序由定义到边界：先明确「关键跨层推理链（定理一致性）」在「跨层知识图谱（Inter-Layer Dependency Map）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「关键跨层推理链（定理一致性）」的判定标准，并指出它在全页论证链中的位置。
...
子定理 3 (L1): 生命周期约束 ⟹ 无悬垂指针
    ...
    来源: ✅ [TRPL Ch10.3](https://doc.rust-lang.org/book/ch10-03-lifetime-syntax.html) · ✅ [Rust Reference: Lifetime Elision](https://doc.rust-lang.org/reference/lifetime-elision.html)
...
本节聚焦「层间一致性检查点」，覆盖定义一致性检查 与 定理一致性检查。论述顺序由定义到边界：先明确「层间一致性检查点」在「跨层知识图谱（Inter-Layer Dependency Map）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「层间一致性检查点」的判定标准，并指出它在全页论证链中的位置。
...
「反事实与边界条件」部分包含什么情况下形式化保证失效？ 与 跨层断裂点 两条主线，本节依次说明。
...
- [x] **高**: 补充 Pin 的形式化语义来源（location stability 的精确对应论文） —— ✅ 已完成，来源：[RFC 2349](https://rust-lang.github.io/rfcs//2349-pin.html)、PLDI 2024 RefinedRust
...
- [L1 所有权与借用](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) — 形式化映射起点
- [L4 所有权形式化](../../04_formal/01_ownership_logic/02_ownership_formal.md) — 线性逻辑与分离逻辑
- [知识体系方法论](../00_framework/methodology.md) — 语义链接类型定义规范
...
> **认知路径**: 本文件作为 Rust 分层知识体系的 **跨层知识图谱（Inter-Layer Dependency Map）** 元层导航节点，连接概念定义、学习路径与质量评估框架。
...
> **反命题**: "元层文档可以替代具体概念学习" —— 错误。跨层知识图谱（Inter-Layer Dependency Map） 提供的是导航与评估框架，不能替代对核心概念（L1-L5）的深入理解与实践。
> **内容分级**: [综述级]

## 嵌入式测验（Embedded Quiz）

本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：本文档《跨层知识图谱（Inter-Layer Depende…、测验 2：《跨层知识图谱（Inter-Layer Dependency…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层）。

### 测验 1：本文档《跨层知识图谱（Inter-Layer Dependency Map）》在 Rust 知识体系中属于哪一层级的元数据？（理解层）

**题目**: 本文档《跨层知识图谱（Inter-Layer Dependency Map）》在 Rust 知识体系中属于哪一层级的元数据？
...
### 测验 2：《跨层知识图谱（Inter-Layer Dependency Map）》的主要用途是什么？（理解层）

**题目**: 《跨层知识图谱（Inter-Layer Dependency Map）》的主要用途是什么？
