...
> **Summary**: Learning Mvp Path — The minimum viable learning path: from zero to writing a concurrent Rust CLI in about forty hours.
...
本节聚焦「Week 1: 基础能力构建」，覆盖Day 1-2: Hello World + 基础语法 [4h]、Day 3-4: 所有权与借用 [6h]、Day 5-6: 类型系统与错误处理 [6h]与Day 7: 第一个 CLI 工具（无并发） [4h]。论述顺序由定义到边界：先明确「Week 1: 基础能力构建」在「MVP 学习路径：从零到多线程 CLI（40 小时）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「Week 1: 基础能力构建」的判定标准，并指出它在全页论证链中的位置。
...
| 阅读 | [concept/01_foundation/03_values_and_references/03_variable_model.md](../../01_foundation/03_values_and_references/03_variable_model.md) | 0.5h | **必修** | 区分 `let` / `let mut` / `const` |
| 练习 | [exercises/rustlings_style/variables.rs](../exercises/rustlings_style/variables.rs) | 0.5h | **必修** | 全部编译通过 |
| 阅读 | [concept/01_foundation/03_control_flow.md](../../01_foundation/04_control_flow/01_control_flow.md) | 1h | **必修** | 掌握 `if` / `match` / `for` / `while` |
...
> 📝 **自测**: [所有权清单自测（Brown University Ownership Inventory）](../../01_foundation/01_ownership_borrow_lifetime/06_ownership_inventories_brown_book.md) — Inventory #1 本地映射与样题
...
| 任务 | 内容 | 时长 | 类型 | 验证标准 |
|:---|:---|:---:|:---:|:---|
| 阅读 | [concept/01_foundation/02_type_system/01_type_system.md](../../01_foundation/02_type_system/01_type_system.md) | 1.5h | **必修** | 区分 `struct` / `enum` / `tuple` |
...
| 阅读 | [concept/01_foundation/05_error_handling.md](../../01_foundation/08_error_handling/01_error_handling_basics.md) | 1h | **必修** | 掌握 `Result` / `Option` / `?` 运算符 |
| 练习 | 重构猜数字游戏：添加输入验证、优雅错误处理 | 1.5h | **必修** | 输入非数字时不 panic |
| 阅读 | [concept/02_intermediate/01_generics/01_generics.md](../../02_intermediate/01_generics/01_generics.md) | 0.5h | **必修** | 理解泛型函数的基本写法 |
...
本节聚焦「Week 2: 并发与工程化」，覆盖Day 8-9: 集合与迭代器 [4h]、Day 10-11: 多线程与并发 [6h]、Day 12-13: 异步基础 [6h]与Day 14: 综合项目：多线程/异步 CLI [4h]。论述顺序由定义到边界：先明确「Week 2: 并发与工程化」在「MVP 学习路径：从零到多线程 CLI（40 小时）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「Week 2: 并发与工程化」的判定标准，并指出它在全页论证链中的位置。
...
| 任务 | 内容 | 时长 | 类型 | 验证标准 |
|:---|:---|:---:|:---:|:---|
| 阅读 | [concept/02_intermediate/04_collections.md](../../01_foundation/05_collections/01_collections.md) | 1h | **必修** | 知道何时用 `Vec` / `HashMap` / `BTreeMap` |
| 练习 | 用迭代器重构 `rsgrep`（消除显式 `for` 循环） | 1h | **必修** | 代码行数减少 30%+ |
| 阅读 | [concept/02_intermediate/05_iterators.md](../../02_intermediate/07_iterators_and_closures/01_iterator_patterns.md) | 1h | **必修** | 掌握 `map` / `filter` / `fold` / `collect` |
...
> 📝 **自测**: 生命周期相关 Inventory 见 [所有权清单自测](../../01_foundation/01_ownership_borrow_lifetime/06_ownership_inventories_brown_book.md) Inventory #3
...
| 任务 | 内容 | 时长 | 类型 | 验证标准 |
|:---|:---|:---:|:---:|:---|
| 阅读 | [concept/03_advanced/01_async/01_async.md](../../03_advanced/01_async/01_async.md)（前半部分） | 2h | **选修** | 理解 `async fn` 返回 `Future` |
| 环境 | 添加 `tokio = { version = "1", features = ["full"] }` | 0.5h | **选修** | `cargo build` 成功 |
| 练习 | 用 `tokio::spawn` 并行执行多个 HTTP 请求 | 1.5h | **选修** | 并发请求总时间 < 串行时间 |
| 阅读 | [concept/03_advanced/01_async/01_async.md](../../03_advanced/01_async/01_async.md)（基础部分） | 1.5h | **选修** | 理解 `await` 的挂起语义 |
...
| 方向 | 下一步文件 | 预估时间 |
|:---|:---|:---:|
| 系统编程 | [concept/03_advanced/02_unsafe/01_unsafe.md](../../03_advanced/02_unsafe/01_unsafe.md) | +20h |
| Web 后端 | [concept/06_ecosystem/03_web_frameworks.md](../../06_ecosystem/06_data_and_distributed/01_application_domains.md) | +20h |
| 嵌入式 | [concept/06_ecosystem/05_embedded.md](../../06_ecosystem/05_systems_and_embedded/03_embedded_systems.md) | +30h |
| 形式化验证 | [concept/04_formal/03_ownership_formal.md](../../04_formal/01_ownership_logic/02_ownership_formal.md) | +40h |
| 性能优化 | [concept/03_advanced/04_optimization.md](../../06_ecosystem/10_performance/01_performance_optimization.md) | +15h |
...
本节从测验 1：本文档《MVP 学习路径：从零到多线程 CLI（40 小时）…、测验 2：《MVP 学习路径：从零到多线程 CLI（40 小时）》的主…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层）切入，剖析「嵌入式测验（Embedded Quiz）」的核心内容。
