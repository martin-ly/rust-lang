...
|:---|:---|:---|
| 环境模型 vs 存储模型 | 所有权 = 存储模型 + 线性约束 | [Variable Model](../01_foundation/03_values_and_references/20_variable_model.md) |
| 值语义 vs 引用语义 | `Copy` / `Move` / `Clone` | [Variable Model](../01_foundation/03_values_and_references/20_variable_model.md) |
| L-value / R-value | place expression / value expression | [Variable Model](../01_foundation/03_values_and_references/20_variable_model.md) |
| 赋值语义 | 绑定 vs 存储更新 | [Variable Model](../01_foundation/03_values_and_references/20_variable_model.md) |
...
| 通用 PL 概念 | Rust 具体机制 | 文件 |
|:---|:---|:---|
| 引用透明 | 纯函数、不可变借用 | [Effects and Purity](../01_foundation/00_start/21_effects_and_purity.md) |
| 副作用分类 | State / IO / Exception / Nondeterminism / Time | [Effects and Purity](../01_foundation/00_start/21_effects_and_purity.md) |
| Mutation as Effect | `&mut T` 作为 write effect | [Effects and Purity](../01_foundation/00_start/21_effects_and_purity.md) |
| 结构化程序定理 | sequence / selection / iteration | [Control Flow](../01_foundation/04_control_flow/07_control_flow.md) |
| Continuation | `?` / `break 'label` / async / CPS | [Control Flow](../01_foundation/04_control_flow/07_control_flow.md) |
| 控制流图（CFG） | 基本块、支配树、编译器优化 | [Control Flow](../01_foundation/04_control_flow/07_control_flow.md) |

### 2.4 数据抽象

| 通用 PL 概念 | Rust 具体机制 | 文件 |
|:---|:---|:---|
| C struct / 内存布局 | `struct` / `repr(C)` | [Data Abstraction Spectrum](../01_foundation/02_type_system/22_data_abstraction_spectrum.md) |
| C++ class / 封装 | `struct` + `impl` + 模块可见性 | [Data Abstraction Spectrum](../01_foundation/02_type_system/22_data_abstraction_spectrum.md) |
| ADT / 代数数据类型 | `enum` + 模式匹配 | [Data Abstraction Spectrum](../01_foundation/02_type_system/22_data_abstraction_spectrum.md) |
| 子类型多态 | Trait + `dyn Trait` | [Data Abstraction Spectrum](../01_foundation/02_type_system/22_data_abstraction_spectrum.md) |
...
|:---|:---|:---|
| 为什么 move 后不能访问原变量？ | 存储模型 + 线性类型 | [Variable Model](../01_foundation/03_values_and_references/20_variable_model.md) |
| 为什么 `&mut` 只能有一个？ | 副作用模型 + alias discipline | [Effects and Purity](../01_foundation/00_start/21_effects_and_purity.md) |
| 为什么 `?` 能传播错误？ | Continuation / 控制流 | [Control Flow](../01_foundation/04_control_flow/07_control_flow.md) |
| 为什么 `Iterator` 是惰性的？ | 求值策略 | [Evaluation Strategies](../04_formal/18_evaluation_strategies.md) |
| 为什么 `enum` 比 class 更适合某些场景？ | 数据抽象谱系 | [Data Abstraction Spectrum](../01_foundation/02_type_system/22_data_abstraction_spectrum.md) |
...
- 变量模型 ✅ [Variable Model](../01_foundation/03_values_and_references/20_variable_model.md)
- 求值策略 ✅ [Evaluation Strategies](../04_formal/18_evaluation_strategies.md)
- 副作用与纯度 ✅ [Effects and Purity](../01_foundation/00_start/21_effects_and_purity.md)
- 控制流深化 ✅ [Control Flow](../01_foundation/04_control_flow/07_control_flow.md)
- 数据抽象谱系 ✅ [Data Abstraction Spectrum](../01_foundation/02_type_system/22_data_abstraction_spectrum.md)
