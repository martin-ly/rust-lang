...
> **认知功能**: 本 mindmap 提供八棵判定树的**鸟瞰视图**。每棵树的根是**概念定义**，叶节点是**判定结果**（合法 / 失效）。与 `theorem_inference_forest.md` 的关键区别：后者从 L4 **公理**出发（"仿射逻辑 ⟹ 所有权唯一性"），本文件从 L1 **概念定义**出发（"给定代码片段，逐步判定其所有权是否合法"）。前者是"理论保证从哪来"，后者是"工程实践怎么判"。[来源: 💡 原创分析]
