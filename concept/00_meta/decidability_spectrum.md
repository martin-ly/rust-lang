...
> **定理 T-DS-001（Rust 局部类型推断可判定性）**: 在函数签名显式注解、无 HKT、无递归类型直接展开的约束下，Rust 函数体内的局部类型推断问题是**可判定的**，且编译器总能在有限步内给出 Yes（存在合法类型）或 No（类型不匹配）。 [来源: Pierce, *Types and Programming Languages*, §22; Rust Reference §6]
