...
| **02_async** | F×Mem: `Future` / `Poll` / `Waker` API F×App: `async fn` / `.await` 语法 | C×Und: async = CPS 变换 + 状态机 C×Ana: 分析 `Pin` 在自引用中的必要性 | P×App: 实现自定义 Future P×Eva: 评估 tokio vs async-std 生态 | M×Ana: 诊断自身对 async 执行模型的误解 |
