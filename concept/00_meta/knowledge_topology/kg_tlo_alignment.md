...
BFO 与 DOLCE 是知识图谱对齐的两大顶层本体（TLO），核心类别速览：

- **BFO 顶层二分法**：Continuant（持续体——在时间中持续存在，如对象、质量、角色）vs Occurrent（发生体——在时间里展开，如过程、事件）。Rust 概念映射：类型、trait、所有权规则是 continuant；编译过程、借用检查的执行是 occurrent。BFO 的细分（独立持续体/依赖持续体）可区分"类型本身"（独立）与"某变量的类型状态"（依赖）。
- **DOLCE 顶层二分法**：Endurant（持久体）vs Perdurant（持续事件），与 BFO 类似但 DOLCE 额外强调**描述与情境**（Descriptions & Situations）维度——同一概念在不同描述框架下可呈现不同类别。对知识图谱的意义：`ex:Borrowing` 在 DOLCE 下既可建模为 endurant（规则）也可建模为 perdurant（检查事件），需在映射时显式选择并记录。
...
`kg_data_v3.json` 中关键实体到顶层本体的映射示例与判定依据：

- **`ex:Ownership`（所有权）** → BFO: 依赖持续体中的"关系性质量"；DOLCE: Quality。判定依据：所有权不能脱离对象独立存在，且是对象间（拥有者与资源）的关系。
- **`ex:Borrowing`（借用）** → BFO: 过程边界/关系；DOLCE: Perdurant（借用检查事件）+ Relation（借用关系）双建模。判定依据：作为静态规则时是关系，作为编译期事件时是过程。
- **`ex:Lifetime`（生命周期）** → BFO: 时间区域（Temporal Region）；DOLCE: Temporal Interval。判定依据：生命周期本质是代码区域到时间区间的映射，无独立实体性。

映射冲突处理：同一实体在两个 TLO 下类别不一致时，以 DOLCE 的描述维度为准并在 KG 中标注 `alignment_note`，供 SHACL 检查豁免。ifetimes`（生命周期）、`ex:Trait`（ trait ）等5个方面的顺序逐层展开。
...
本节聚焦「边界案例与讨论」，覆盖`Theory` 是 Abstract 还是 Information…、`Rule` 是 Information Content Entity…、`Property` 是 Dependent Continuant 还…与关系（Relation）本身的 TLO 地位。论述顺序由定义到边界：先明确「边界案例与讨论」在「Rust 知识体系顶层本体（TLO）对齐」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「边界案例与讨论」的判定标准，并指出它在全页论证链中的位置。
