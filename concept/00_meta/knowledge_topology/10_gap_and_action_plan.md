...
本节聚焦「优先修复任务」，覆盖P0：补全权威来源（L1–L4 核心概念）、P1：增强知识表征与P2：对齐国际标准。论述顺序由定义到边界：先明确「优先修复任务」在「缺口与行动计划（Gap and Action Plan）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「优先修复任务」的判定标准，并指出它在全页论证链中的位置。
