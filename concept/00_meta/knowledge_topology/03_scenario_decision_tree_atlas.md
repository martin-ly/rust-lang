...
本节聚焦「按场景索引」，覆盖内存管理场景、并发与异步场景、错误处理场景、FFI 与跨语言场景等方面。论述顺序由定义到边界：先明确「按场景索引」在「场景决策树图谱（Scenario Decision Tree Atlas）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「按场景索引」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「典型决策树示例」，覆盖内存管理决策树 与 并发模型决策树。论述顺序由定义到边界：先明确「典型决策树示例」在「场景决策树图谱（Scenario Decision Tree Atlas）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「典型决策树示例」的判定标准，并指出它在全页论证链中的位置。
...
| [Rust 编译期可判定性谱系全景](../../00_meta/00_framework/decidability_spectrum.md) | 决策/场景节 ×1 · mermaid 判定图 ×3 | 判定路径决策树 |
...
| [方法论：思维表征与知识结构规范](../../00_meta/00_framework/methodology.md) | 决策/场景节 ×5 · mermaid 判定图 ×1 | 决策/边界判定树（Decision / Boundary Tree） · 决策树图（Decision Tree） 决策树方法论参照 Quinlan, J… |
| [Rust 范式转换模式矩阵](../../00_meta/00_framework/paradigm_transition_matrix.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 转换决策树 |
| [模式语义空间索引：设计模式在概念体系中的坐标](../../00_meta/00_framework/pattern_semantic_space_index.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 模式选择决策树 |
| [Rust 表征空间](../../00_meta/00_framework/semantic_space.md) | 决策/场景节 ×1 · mermaid 判定图 ×4 | 组合选择决策树 |
| [Concept 文件双语模板 v2](../../00_meta/01_terminology/02_bilingual_template_v2.md) | 决策/场景节 ×3 | 判断推理与决策树 · 反例 / 边界 / 反推 / 决策树规范 |
| [Concept 文件双语模板](../../00_meta/01_terminology/03_bilingual_template.md) | 决策/场景节 ×1 | 决策树/矩阵（如适用） |
...
| [Rust 关键字](../../01_foundation/00_start/06_keywords.md) | 决策/场景节 ×1 | 典型使用场景 |
| [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) | 决策/场景节 ×4 · mermaid 判定图 ×4 | 决策/边界判定树（Decision / Boundary Tree） · "我的类型应该是 Copy 吗？" 决策树 |
| [Borrowing](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) | 决策/场景节 ×3 · mermaid 判定图 ×3 | 决策/边界判定树（Decision / Boundary Tree） · "我该用 `&T` 还是 `&mut T`？" 决策树 |
| [Lifetimes](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md) | 决策/场景节 ×2 · mermaid 判定图 ×3 | 判定表：生命周期标注与失效场景判定 · 测验 4：HRTB 场景（专家级） |
| [Lifetimes 高级主题](../../01_foundation/01_ownership_borrow_lifetime/04_lifetimes_advanced.md) | 决策/场景节 ×2 | Step 2: 具体场景（Concrete Scenario） · 测验 2：HRTB 的适用场景（应用层） |
| [Type System Basics](../../01_foundation/02_type_system/01_type_system.md) | 决策/场景节 ×2 · mermaid 判定图 ×9 | Step 2: 具体场景（Concrete Scenario） · 选择决策树 |
...
| [高级集合类型：BTreeMap、VecDeque、BinaryHeap 与自定义 Hasher 深度分析](../../01_foundation/05_collections/02_collections_advanced.md) | 决策/场景节 ×2 · mermaid 判定图 ×1 | 选型决策矩阵 · 集合选型决策树 |
| [字符串与文本：Rust 的 Unicode 处理与格式化系统](../../01_foundation/06_strings_and_text/01_strings_and_text.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 选型决策矩阵 |
| [字符串与编码：Rust 的文本处理类型系统](../../01_foundation/06_strings_and_text/02_strings_and_encoding.md) | 决策/场景节 ×2 · mermaid 判定图 ×2 | 选型决策矩阵 · 字符串类型选型 |
...
| [Rust 错误处理基础](../../01_foundation/08_error_handling/01_error_handling_basics.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 测验 5：panic! 的适用场景（评价层） |
| [错误处理控制流](../../01_foundation/08_error_handling/02_error_handling_control_flow.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 决策树 |
...
| [分发机制 (Dispatch Mechanisms)](../../02_intermediate/00_traits/02_dispatch_mechanisms.md) | 决策/场景节 ×4 | 决策树 · 使用场景 |
| [Serde 序列化模式：Rust 的类型驱动数据转换](../../02_intermediate/00_traits/03_serde_patterns.md) | 决策/场景节 ×1 · mermaid 判定图 ×2 | 测验 3：`#[serde(untagged)]` 的序列化/反序列化行为有什… |
| [高级 Trait 主题：从关联类型到特化](../../02_intermediate/00_traits/04_advanced_traits.md) | mermaid 判定图 ×1 | mermaid 判定节点图 |
| [泛型关联类型](../../02_intermediate/00_traits/07_generic_associated_types.md) | 决策/场景节 ×2 · mermaid 判定图 ×1 | GAT 替代 HRTB 的判定场景 · 选型判定表：GAT vs 关联类型 vs HRTB vs `impl Trai… |
| [Generics](../../02_intermediate/01_generics/01_generics.md) | 决策/场景节 ×1 · mermaid 判定图 ×5 | 决策树图 |
| [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) | mermaid 判定图 ×4 | mermaid 判定节点图 |
| [内部可变性：编译期规则的运行时逃逸](../../02_intermediate/02_memory_management/02_interior_mutability.md) | 决策/场景节 ×4 · mermaid 判定图 ×1 | 判定表：内部可变性选型判定 · 测验 3：内部可变性的适用场景（应用层） |
| [Cow：写时克隆与零拷贝抽象](../../02_intermediate/02_memory_management/03_cow_and_borrowed.md) | 决策/场景节 ×1 · mermaid 判定图 ×2 | 测验 4：`Cow` 适合解决什么场景的问题？（理解层） |
| [智能指针：堆内存管理与共享语义](../../02_intermediate/02_memory_management/04_smart_pointers.md) | 决策/场景节 ×4 · mermaid 判定图 ×1 | 选型决策矩阵 · 判定表：智能指针选型判定 |
| [Error Handling](../../02_intermediate/03_error_handling/01_error_handling.md) | mermaid 判定图 ×7 | mermaid 判定节点图 |
| [错误处理深入：从 Result 到自定义错误生态](../../02_intermediate/03_error_handling/02_error_handling_deep_dive.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 测验 2：`thiserror` 和 `anyhow` 在错误处理中各自适合什… |
...
| [Newtype 与包装器模式：类型安全的零成本抽象](../../02_intermediate/04_types_and_conversions/03_newtype_and_wrapper.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 测验 5：Newtype 模式与类型别名（type alias）相比，在什么场… |
| [高级类型系统：从关联类型到类型级编程](../../02_intermediate/04_types_and_conversions/04_type_system_advanced.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 决策树图 |
...
| [Async Closures](../../03_advanced/01_async/07_async_closures.md) | 决策/场景节 ×1 | 使用场景：高阶异步函数 |
...
| [Async Trait 对象安全：dyn 兼容解决方案谱系与选型矩阵](../../03_advanced/01_async/13_async_trait_object_safety.md) | 决策/场景节 ×2 · mermaid 判定图 ×2 | 选型矩阵（场景 × 方案 × 开销 × MSRV） · 判定树 |
| [是否需要进入 L4 形式化层？](../../03_advanced/02_unsafe/00_before_formal.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 快速决策树 |
| [Unsafe Rust 安全编程](../../03_advanced/02_unsafe/01_unsafe.md) | 决策/场景节 ×7 · mermaid 判定图 ×7 | 决策/边界判定树（Decision / Boundary Tree） · "我需要用 unsafe 吗？" 决策树 |
...
| [Rust 运行时](../../03_advanced/06_low_level_patterns/07_rust_runtime.md) | 决策/场景节 ×2 | 反命题决策树 · 使用场景 |
...
| [Rust 进程间通信机制](../../03_advanced/08_process_ipc/05_ipc_mechanisms.md) | 决策/场景节 ×4 · mermaid 判定图 ×1 | 9. 选型建议 · 补充视角：IPC 机制工程选型速查 |
...
| 概念页 | 表征信号 | 主题提示 |
|:---|:---|:---|
| [Type Theory](../../04_formal/00_type_theory/01_type_theory.md) | 决策/场景节 ×2 · mermaid 判定图 ×4 | 反命题决策树（Counter-proposition Decision Tre… · 决策树图 |
| [子类型与变型：Rust 类型系统中的协变、逆变与不变](../../04_formal/00_type_theory/02_subtype_variance.md) | mermaid 判定图 ×1 | mermaid 判定节点图 |
| [类型推断：Hindley-Milner 算法与 Rust 的工业实现](../../04_formal/00_type_theory/03_type_inference.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | Turbofish 使用场景 |
...
| [Type Semantics](../../04_formal/00_type_theory/06_type_semantics.md) | 决策/场景节 ×1 | 反命题决策树 |
...
| [类型系统参考](../../04_formal/00_type_theory/09_type_system_reference.md) | 决策/场景节 ×2 | 反命题决策树 · 决策树图 |
...
| [Ownership Formalization](../../04_formal/01_ownership_logic/02_ownership_formal.md) | 决策/场景节 ×4 · mermaid 判定图 ×4 | 反命题决策树 · 决策树 1: "形式化所有权证明所有程序安全" |
| [线性逻辑在 Rust 中的工程应用](../../04_formal/01_ownership_logic/03_linear_logic_applications.md) | mermaid 判定图 ×1 | mermaid 判定节点图 |
| [RustBelt & Verification Toolchain](../../04_formal/02_separation_logic/01_rustbelt.md) | 决策/场景节 ×2 · mermaid 判定图 ×3 | 反命题决策树（Antithesis Decision Trees） · 工业选型决策路径 |
...
| [操作语义：程序行为的形式化定义](../../04_formal/03_operational_semantics/03_operational_semantics.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 形式化方法的技术对比与选型指南 |
| [Axiomatic Semantics](../../04_formal/03_operational_semantics/05_axiomatic_semantics.md) | 决策/场景节 ×1 | 反命题决策树 |
| [Verification Toolchain Selection Guide](../../04_formal/04_model_checking/01_verification_toolchain.md) | 决策/场景节 ×9 · mermaid 判定图 ×2 | TL;DR —— 30 秒选型 · 工具链全景矩阵（选型版） |
...
| [Application Binary Interface](../../04_formal/05_rustc_internals/05_application_binary_interface.md) | 决策/场景节 ×2 | 使用场景 · 典型场景 |
...
| [五模型定义矩阵：同步 · 并发 · 并行 · 异步 · 分布式](../../05_comparative/00_paradigms/04_five_models_definition_matrix.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 选型决策树 |
...
| [Rust vs Go：线性所有权 vs CSP 过程逻辑](../../05_comparative/01_systems_languages/03_rust_vs_go.md) | 决策/场景节 ×5 · mermaid 判定图 ×4 | 第 5 步：生态系统成熟度和适用场景？ · 决策树：何时选 Rust vs Go |
...
| [Rust vs Java：系统编程与托管运行时的范式对比](../../05_comparative/02_managed_languages/01_rust_vs_java.md) | 决策/场景节 ×2 · mermaid 判定图 ×1 | 场景适用矩阵 · 测验 5：在需要与大量现有 Java 生态集成的场景下，Rust 通常如何与… |
...
| [Rust 编译器基础设施深度解析](../../06_ecosystem/00_toolchain/05_compiler_infrastructure.md) | 决策/场景节 ×4 | 反命题与选型建议 · 编译后端选型决策树 |
| [Rustdoc 1.96–1.97 变更](../../06_ecosystem/00_toolchain/07_rustdoc_196_changes.md) | 决策/场景节 ×1 | 典型应用场景 |
| [将 Rust 集成到现有平台](../../06_ecosystem/00_toolchain/08_platform_rust_integration.md) | 决策/场景节 ×1 | 选型决策树 |
| [Rust 编译器的 LLVM 后端与代码生成](../../06_ecosystem/00_toolchain/09_llvm_backend_and_codegen.md) | 决策/场景节 ×1 | 测验 4：Cranelift 后端适合什么场景？ |
| [rustc Driver、Interface 与 Stable MIR](../../06_ecosystem/00_toolchain/10_rustc_driver_and_stable_mir.md) | 决策/场景节 ×1 | 典型应用场景 |
| [Rust 常用开发工具](../../06_ecosystem/00_toolchain/14_development_tools.md) | 决策/场景节 ×1 | 典型场景 |
| [Cargo Script 脚本化 Rust](../../06_ecosystem/01_cargo/01_cargo_script.md) | 决策/场景节 ×1 · mermaid 判定图 ×2 | 测验 3：Cargo Script 适合替代哪些传统脚本语言（如 Python… |
...
| [Cargo Manifest 参考速查](../../06_ecosystem/01_cargo/10_cargo_manifest_reference.md) | 决策/场景节 ×1 | 测验 2：`[dev-dependencies]` 在什么场景下使用？ |
| [Cargo 子命令与插件生态](../../06_ecosystem/01_cargo/12_cargo_subcommands_and_plugins.md) | 决策/场景节 ×1 | 测验 3：`--message-format=json` 适合哪些工具集成场景？ |
...
| [Cargo build-std](../../06_ecosystem/01_cargo/22_build_std.md) | 决策/场景节 ×2 · mermaid 判定图 ×1 | 嵌入式场景 · 何时选择 build-std（决策树） |
| [Core Crates](../../06_ecosystem/02_core_crates/01_core_crates.md) | 决策/场景节 ×6 · mermaid 判定图 ×5 | 测验 5：核心 crate 选型原则（评价层） · 选型决策快速矩阵 |
| [Design Patterns](../../06_ecosystem/03_design_patterns/01_patterns.md) | 决策/场景节 ×4 · mermaid 判定图 ×4 | 反命题决策树 {L6} · 判定表：设计模式迁移与选型判定 |
| [Rust 惯用法谱系全景](../../06_ecosystem/03_design_patterns/02_idioms_spectrum.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 惯用法选择决策树 |
| [Rust 系统设计原则与国际权威对齐](../../06_ecosystem/03_design_patterns/03_system_design_principles.md) | 决策/场景节 ×4 · mermaid 判定图 ×1 | 系统设计决策矩阵 · 场景驱动的设计选择 |
| [系统可组合性 (System Composability)](../../06_ecosystem/03_design_patterns/04_system_composability.md) | 决策/场景节 ×1 | 组合模式选型决策矩阵 |
| [CQRS & Event Sourcing](../../06_ecosystem/03_design_patterns/07_cqrs_event_sourcing.md) | 决策/场景节 ×1 | 测验 5：CQRS + Event Sourcing 相比传统架构增加了什么复… |
| [Architecture Patterns](../../06_ecosystem/03_design_patterns/08_architecture_patterns.md) | 决策/场景节 ×2 | 架构模式决策矩阵 · 架构模式适用场景 |
| [模式实现对比 (Pattern Implementation Comparison)](../../06_ecosystem/03_design_patterns/09_pattern_implementation_comparison.md) | 决策/场景节 ×1 | 选择决策树 |
...
| [Glommio 与 Thread-per-Core 异步运行时](../../06_ecosystem/04_web_and_networking/05_glommio_and_thread_per_core.md) | 决策/场景节 ×2 | 适用场景与不推荐场景 · 选择决策树 |
| [网络协议：QUIC/HTTP-3 与 Rust 实现](../../06_ecosystem/04_web_and_networking/07_network_protocols.md) | 决策/场景节 ×2 | Rust 网络协议栈的选型 · 测验 3：Rust 中常用的序列化协议有哪些？各自适合什么场景？（理解层） |
| [高性能网络服务架构 (High-Performance Network Service Architecture)](../../06_ecosystem/04_web_and_networking/08_high_performance_network_service_architecture.md) | 决策/场景节 ×1 | 架构选择决策树 |
...
| [Embedded-HAL 1.0 迁移与 Embassy 生产状态](../../06_ecosystem/05_systems_and_embedded/09_embedded_hal_1_0_migration.md) | 决策/场景节 ×1 | Embassy vs RTIC 选型 |
| [Application Domains](../../06_ecosystem/06_data_and_distributed/01_application_domains.md) | 决策/场景节 ×4 · mermaid 判定图 ×5 | 第 5 步：Rust不适合哪些场景？ · 领域选型决策矩阵 |
| [Rust 数据库访问生态](../../06_ecosystem/06_data_and_distributed/02_database_access.md) | mermaid 判定图 ×1 | mermaid 判定节点图 |
| [流处理生态：Rust 实现与工业系统全景](../../06_ecosystem/06_data_and_distributed/03_stream_processing_ecosystem.md) | 决策/场景节 ×1 | 选型决策树 |
| [数据库系统：Rust 在存储引擎中的语义](../../06_ecosystem/06_data_and_distributed/04_database_systems.md) | 决策/场景节 ×1 | 跨系统选型决策矩阵 |
| [Distributed Consensus](../../06_ecosystem/06_data_and_distributed/06_distributed_consensus.md) | 决策/场景节 ×1 | 选型决策树 |
| [CRDT 谱系：状态基、操作基与合并格形式化](../../06_ecosystem/06_data_and_distributed/08_crdt_type_zoo.md) | 决策/场景节 ×1 | 选型速查 |
| [安全 实践：Rust 代码的防御性编程](../../06_ecosystem/07_security_and_cryptography/01_security_practices.md) | mermaid 判定图 ×1 | mermaid 判定节点图 |
| [Formal Verification Tools](../../06_ecosystem/08_formal_verification/02_formal_verification_tools.md) | 决策/场景节 ×2 | 验证工具对比与选型 · 选型决策树 |
...
| [Rust 游戏开发生态](../../06_ecosystem/11_domain_applications/05_game_development.md) | 决策/场景节 ×1 · mermaid 判定图 ×1 | 测验 3：Rust 的所有权系统对游戏开发中的"场景图"（Scene Grap… |
| [算法与竞赛编程 (Algorithms & Competitive Programming)](../../06_ecosystem/11_domain_applications/07_algorithms_competitive_programming.md) | 决策/场景节 ×2 | 测验 3：Rust 的 `BinaryHeap` 在竞赛中适合什么场景？（理解… · 算法选择决策树 |
...
| [Rust 1.97 兼容性迁移判定树](../../07_future/00_version_tracking/migration_197_decision_tree.md) | 决策/场景节 ×9 · mermaid 判定图 ×8 | 2. 总路由判定树（先定位变化，再进入对应小节） · 判定树（空 export_name） |
| [Rust 1.91 稳定特性](../../07_future/00_version_tracking/rust_1_91_stabilized.md) | 决策/场景节 ×2 | 实际应用场景 · 内存分配优化对异步场景的影响 |
...
| [Rust for WebAssembly：从 wasm-bindgen 到前端框架的深度技术栈](../../07_future/04_research_and_experimental/06_rust_for_webassembly.md) | 决策/场景节 ×2 | 框架选型矩阵 · 反命题决策树 |
| [eBPF / Aya / Rex 的 Rust 映射](../../07_future/04_research_and_experimental/07_ebpf_rust.md) | 决策/场景节 ×1 | 测验 5：Rust eBPF 目前的主要应用场景有哪些？（理解层） |
