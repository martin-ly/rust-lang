...
L0 元信息层收纳知识库自身的「关于知识的知识」——方法论、术语表、拓扑图、审计机制。本节的层内引用关系表回答两个问题：

- **导航入口如何分工**：`00_framework/`（方法论与形式化框架）与 `04_navigation/`（学习路径与图谱）的分工是「规范 vs 工具」——先建规范再生成导航；
- **术语一致性的锚点**：术语表是全库术语的唯一权威，其他文件引用术语时应对齐其定义（`check_glossary_alignment.py` 机器执行）。

层内引用关系表逐文件列出「被谁引用 / 引用谁」，用于评估元层文件的影响面——修改高被引文件（如方法论）前应检查下游一致性。
...
L1 基础概念层是知识库流量最大的层级——所有权、借用、类型、控制流等入门必修概念的唯一权威页所在。本节层内引用关系表揭示 L1 的内部结构：

- **核心三角**：ownership/borrowing/lifetimes 三页互引密度最高（知识图谱中的稠密子图）——修改任一页都应同步检查另两页的概念一致性；
- **入口页与专题页**：各主题目录的 `01_*` 起始页承担「概念定义 + 导航」职责，后续编号页是专题深化——引用方向应从专题页指向起始页（下位指向上位）；
- **引用缺失信号**：入度为零的 L1 页面（无其他 L1 页引用）通常是导航断裂，应在相关页面补交叉链接。

本表由拓扑检查器（`check_topology_quality.py`）持续校验，引用关系的退化会被质量门检出。
