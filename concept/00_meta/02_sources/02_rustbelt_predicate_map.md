...
本节建立 RustBelt 核心谓词 `own(τ)`（「拥有类型 τ 的值」）与 Rust 概念的精确对应：

- **谓词语义**：`own(τ)` 在分离逻辑中表示「当前线程独占拥有一个 τ 值」——独占性对应 Rust 的所有权唯一性，谓词的**不可复制性**（线性逻辑资源）对应 move 语义（不能复制所有权）；
- **映射表**：`let x: T` ⟷ `own(T)` 的引入；`let y = x`（move）⟷ 谓词从 x 转移到 y；`drop(x)` ⟷ 谓词的消费（配合 τ 的 drop 不变量）；`Box<T>` ⟷ `own(T)` 的堆位置化（`own` 作用于位置 ℓ）；
- **推理示例**：「move 后使用」在 λRust 中对应「消费后再次使用线性资源」——逻辑直接推导矛盾，这解释了 E0382 的逻辑本质而非仅规则记忆。

阅读前提：分离逻辑的 `∗`（分离合取）与 `∗-`（魔杖）基本直觉——第一节的预备材料。
...
本节建立共享借用谓词 `shr(κ, ℓ)`（「位置 ℓ 在生命周期 κ 内被共享」）与 Rust `&T` 的对应：

- **谓词语义**：`shr` 是**可复制的**（共享引用可任意复制别名）但**受生命周期约束**——κ 结束后谓词失效，对应「借用不得比被借值活得久」；
- **生命周期令牌 `[α]₁`**：分数权限模型——完整令牌对应「可终止生命周期」（`&mut` 的独占本质），分数令牌对应「共享中的只读访问」；`&mut` 转 `&`（重借）对应「完整令牌拆分为分数」；
- **与 Rust 错误的对应**：E0502（可变与共享冲突）⟷ 「同时持有完整令牌与分数令牌」的逻辑矛盾；E0597（活得不够久）⟷ 「κ 结束后仍使用 shr 谓词」。

映射的价值：RustBelt 证明「借用规则在 λRust 模型下健全」——本节让读者能在具体编译错误与形式化保证之间双向翻译。
...
本节聚焦「生命周期令牌」，覆盖数学定义、到 Rust 的映射与生命周期与借用的组合。论述顺序由定义到边界：先明确「生命周期令牌」在「RustBelt 谓词映射图（RustBelt Predicate Map）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「生命周期令牌」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「谓词组合与 Rust 代码对应」，覆盖标准库组件的谓词规范 与 谓词组合可视化。论述顺序由定义到边界：先明确「谓词组合与 Rust 代码对应」在「RustBelt 谓词映射图（RustBelt Predicate Map）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「谓词组合与 Rust 代码对应」的判定标准，并指出它在全页论证链中的位置。
