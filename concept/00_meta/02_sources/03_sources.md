...
本节聚焦「使用规范」，覆盖引用规范、来源优先级与 Wikipedia 定位与版本对齐要求。论述顺序由定义到边界：先明确「使用规范」在「权威来源清单与知识来源关系分析」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「使用规范」的判定标准，并指出它在全页论证链中的位置。
...
- [概念一致性检查清单](../03_audit/03_audit_checklist.md) — 来源引用质量审计
