...
ASP（Aspect-Severity-Priority）标记体系为知识库内容提供三维元数据，每维的理论根基：

- **Aspect（方面）**：内容所属的认知侧面——定义/机制/对比/实践/形式化——源自布鲁姆知识维度的细化，回答「这段内容教什么类型的知识」；
- **Severity（严重度）**：错误理解此内容的代价分级——致命（安全/健全性）、重要（正确性）、一般（效率/风格）——源自缺陷管理的严重度分级，决定「审查精力投放」；
- **Priority（优先级）**：学习顺序中的位置——必修/进阶/选读——源自课程设计的螺旋式原则，决定「读者何时遇到它」。

三维正交：一段「致命」内容可以是「选读」（如形式化验证），一段「必修」内容可以「一般」严重度（如命名惯例）——正交性是标记体系不冗余的保证。
...
> **Rust 示例**: 设计 unsafe 代码的安全论证策略、评估 `tokio` vs `async-std [已停止维护]` 的选型、制定团队代码审查中 unsafe 使用的政策 —— 这些都属于 P 类技能。
...
- **语法**：概念页元数据区标注 `**ASP**: [Aspect]/[Severity]/[Priority]`（如 `机制/致命/必修`），章节级可用 HTML 注释细化；
- **分配规则**：Aspect 取唯一主值（多侧面的内容按「读者带走什么」判定主侧面）；Severity 按「最坏误解后果」评定（`unsafe` 相关内容默认 ≥ 重要）；Priority 与 Bloom 层级联动（L1-L2 默认必修，L5+ 默认选读，可覆盖）；
- **一致性校验**：审计脚本检查标记存在性与取值合法性——缺失标记在审计报告中按层统计。

标记的用途：生成「按严重度排序的审查清单」「按优先级生成的学习路径」「按方面统计的覆盖报告」——标记是知识库元数据自动化的输入。
...
本节聚焦「Rust 知识体系全量标记」，覆盖L1 基础概念层、L2 进阶概念层、L3 高级概念层、L4 形式化层等方面。论述顺序由定义到边界：先明确「Rust 知识体系全量标记」在「Rust 知识体系 A/S/P 三维认知标记规范」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「Rust 知识体系全量标记」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「标记应用示例」，覆盖示例 1：借用检查错误诊断、示例 2：并发设计决策与示例 3：unsafe 安全论证。论述顺序由定义到边界：先明确「标记应用示例」在「Rust 知识体系 A/S/P 三维认知标记规范」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「标记应用示例」的判定标准，并指出它在全页论证链中的位置。
...
- v1.0 (2026-05-23): 初始版本 — A/S/P 三维标记定义 + 全量 58 文件标记 + 自动化边界分析 + 概念索引整合规范 [权威来源对齐 Wave 1](../02_sources/05_international_authority_index.md)
