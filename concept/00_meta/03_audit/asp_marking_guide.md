...
> 在 AI 辅助编程时代，这一区分具有战略意义：A 类技能可被 AI 高度替代，学习者应将认知资源集中在 S 和 P 类技能上。[💡 原创分析](../00_framework/methodology.md)
...
理解「标记定义与理论根基」需要把握 A — Application（应用）、S — Structure（结构）、P — Procedure（程序）与三标记与 Bloom/Krathwohl 的映射，本节依次展开。
...
> **认知功能**: 本图展示 A/S/P 作为**双维矩阵的压缩投影**。注意这不是严格的一一对应：A 标记主要覆盖"事实性知识 + 低阶认知"，S 标记覆盖"概念性知识 + 中阶认知"，P 标记覆盖"程序性/元认知知识 + 高阶认知"。这种聚类映射使双维矩阵的 24 个单元格可操作化。[💡 原创分析](../00_framework/methodology.md)

---

## 二、标记规范

本节围绕「标记规范」展开，依次讨论文件头部标注格式、多标记组合规则与与现有 Bloom 标注的整合。
...
本节围绕「Rust 知识体系全量标记」展开，依次讨论 L1 基础概念层、L2 进阶概念层、L3 高级概念层、L4 形式化层等5个方面。
...
理解「标记应用示例」需要把握示例 1：借用检查错误诊断、示例 2：并发设计决策与示例 3：unsafe 安全论证，本节依次展开。
...
> **教学启示**: 遇到借用错误时，不要停留在 A 层（读错误消息），而要推动到 S 层（理解规则根源）和 P 层（设计修复策略）。[💡 原创分析](../00_framework/methodology.md)
...
- v1.0 (2026-05-23): 初始版本 — A/S/P 三维标记定义 + 全量 58 文件标记 + 自动化边界分析 + 概念索引整合规范 [权威来源对齐 Wave 1](../02_sources/international_authority_index.md)

---

> **相关文件**: [双维认知矩阵](../00_framework/cognitive_dimension_matrix.md) · [概念判定森林](../00_framework/concept_definition_decision_forest.md) · [能力图谱](../00_framework/competency_graph.md) · [方法论](../00_framework/methodology.md)
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：本文档《Rust 知识体系 A/S/P 三维认知标记规范》在…、测验 2：《Rust 知识体系 A/S/P 三维认知标记规范》的主要用…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层），本节逐一说明其要点。
