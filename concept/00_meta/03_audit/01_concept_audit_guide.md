...
> **Summary**:
> The operating manual for auditing `concept/` files: a five-step workflow (metadata → links → cross-layer references → content duplication → archive status),
> the audit dimensions each file is scored on, and the pass/fix criteria that keep the knowledge base canonically consistent.
...
- [交叉引用矩阵](../04_navigation/01_cross_reference_matrix.md) — 概念关联映射
- [学习指南](../04_navigation/07_learning_guide.md) — 分层学习路径
