...
「自评工具」部分包含六维自评量表 与 快速自评问卷 两条主线，本节依次说明。
...
本节聚焦「薄弱点诊断与训练路径」，核心内容为薄弱点 → 训练路径映射。
...
「背景定制能力路径」部分按路径 A：C++ 背景、路径 B：Haskell 背景与路径 C：完全新手的顺序逐层展开。
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：本文档《Rust 知识体系能力图谱（Competency G…、测验 2：《Rust 知识体系能力图谱（Competency Grap…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层）的顺序逐层展开。
