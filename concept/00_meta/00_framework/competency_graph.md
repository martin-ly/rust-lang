...
1. **能力维度清单**：按图节点的能力项逐条自评——评分锚点四级（能复述定义 / 能正确使用 / 能诊断错误 / 能设计权衡），避免「熟悉度」这类无锚点自评；
2. **证据锚定**：每个自评等级要求可举证的实例（「能诊断错误」= 举出最近实际诊断过的一个该领域编译/运行错误）——无证据的等级自动降一档；
3. **盲点扫描**：对照能力图的「必修节点」检查空自评项——「不知道这个节点存在」本身就是诊断信号（未知未知，unknown unknowns）。

使用建议：自评每季度一次，两次之间的等级变化是学习路径有效性的直接反馈——下降（技能生锈）与停滞同样是信号。
...
1. **薄弱点定位**：自评中低于「能正确使用」的节点按依赖图排序——前置节点的薄弱优先修复（在借用检查薄弱时补生命周期是顺序错误）；
2. **训练路径生成**：每个薄弱节点映射到「概念页 + 配套测验 + 实战任务」三件套——概念页修复理解，测验验证，实战任务（带 starter 代码的练习）固化；
3. **验收与回归**：训练后重测对应节点——等级提升且能给出新证据锚点才算闭环；两周后抽查回归（防「学完即忘」）。

路径原则：一次只修 1-2 个薄弱点——能力图的密度容易诱发「全补」冲动，而并行修补多个前置链是放弃的首要原因。
...
本节聚焦「背景定制能力路径」，覆盖路径 A：C++ 背景、路径 B：Haskell 背景与路径 C：完全新手。论述顺序由定义到边界：先明确「背景定制能力路径」在「Rust 知识体系能力图谱（Competency Graph）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「背景定制能力路径」的判定标准，并指出它在全页论证链中的位置。
