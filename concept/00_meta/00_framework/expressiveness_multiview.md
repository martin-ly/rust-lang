...
「权威来源与梳理方法论」部分包含理论视角与形式化根基 与  Felleisen 表达力度量框架 两条主线，本节依次说明。
...
理解「类型语义视角」需要把握 Curry-Howard 对应在 Rust 中的实例、类型即命题：Result 与排中律与刻意缺失的表达力：HKT 与 1st-class 模块，本节依次展开。
...
理解「控制语义视角」需要把握 Continuation-Passing Style 与 async/…、状态机变换的三重等价性与生成器（Generator）与协程，本节依次展开。
...
「并发语义视角」部分包含π 演算（Pi Calculus）与 Channel 移动 与 会话类型（Session Types）的 Rust 编码 两条主线，本节依次说明。
...
本节从参数性（Parametricity）与 "Theorems for F… 与 模块系统表达力边界 两个层面剖析「抽象语义视角」。
...
本节围绕「安全语义视角」展开，覆盖信息流控制（IFC）的可编码性 与 类型安全作为格（Lattice）上的推导 两个方面。
