...
> 这是纯 π 演算（无线性类型）需要运行时检查或设计约定才能避免的错误。 [Kobayashi, *A New Type System for Fault-Tolerant π-Calculus*, 2003](https://web.archive.org/web/*/https://www-kb.is.s.u-tokyo.ac.jp/~koba/papers.html)
...
> **表达力边界**: Rust 的类型系统足够表达**静态 IFC 策略**（通过类型状态和 PhantomData），但无法表达**动态 IFC**（需要运行时标签系统）。这通常通过外部工具（如 Miri 的特定插件或独立分析器）补充。 [Zdancewic, *Programming Languages for Information Security*, 2004](https://www.cs.cornell.edu/~andru/papers/thesis.pdf)
