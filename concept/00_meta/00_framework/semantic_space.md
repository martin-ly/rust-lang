...
本节围绕「语义封闭性（Semantic Closure）」展开，依次讨论封闭世界假设、Unsafe：逃逸舱口而非破坏者、类型系统的图灵完备性与不是 Total 的，但是定义时错误的。
...
本节将「能表达 vs 不能表达的边界（Expressibility…」分解为若干主题：三维分类框架、能且高效表达（Sweet Spot）、能但低效/痛苦表达、不能表达 / 故意排除等7个方面。
...
本节从等价表达谱系、等价性判定：继承 → Trait + 组合、等价性判定：异常 → Result、等价性判定：虚函数 → enum vs dyn Trait等6个方面切入，剖析「等价表达的语义保持（Equivalent Expressi…」的核心内容。
...
理解「机制组合的语义空间（Combinatorial Seman…」需要把握基础算子的代数表示、组合爆炸与约束与组合选择决策树，本节依次展开。
