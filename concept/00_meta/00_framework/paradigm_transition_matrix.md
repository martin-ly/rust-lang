...
本节聚焦「C++ → Rust 转换矩阵」，覆盖核心转换模式 与 转换决策树。论述顺序由定义到边界：先明确「C++ → Rust 转换矩阵」在「Rust 范式转换模式矩阵（Paradigm Transition Matrix）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「C++ → Rust 转换矩阵」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「学习难点与认知陷阱」，覆盖通用认知陷阱 与 背景特定难点。论述顺序由定义到边界：先明确「学习难点与认知陷阱」在「Rust 范式转换模式矩阵（Paradigm Transition Matrix）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「学习难点与认知陷阱」的判定标准，并指出它在全页论证链中的位置。
...
> [范式矩阵](../../05_comparative/00_paradigms/01_paradigm_matrix.md) ·
> [学习指南](../04_navigation/07_learning_guide.md)
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：本文档《Rust 范式转换模式矩阵（Paradigm Tra…、测验 2：《Rust 范式转换模式矩阵（Paradigm Transi…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层），本节逐一说明其要点。
