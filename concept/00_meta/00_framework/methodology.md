...
- **TODO 标记规范**：`TODO` 表示内容缺口（待补充的论证/示例），`TBD` 表示待定决策（方案未选定），`FIXME` 表示已知错误。每条标记必须带归属（谁提出）与上下文（为何未决），禁止裸 `TODO`。CI 的内容完整性审计（`audit_content_completeness.py`）按 body/heading/meta/toc/version_log 五类统计标记，趋势上升需在 PR 中说明。
- **变更日志**：文件头部 `**变更日志**` 列表按 `vX.Y (YYYY-MM-DD): 摘要` 格式追加，大版本（结构重组）进位 X，小版本（内容增补）进位 Y。变更日志是版本追踪门（rust_1_9x 系列）与审计报告定位"何时引入"的唯一事实源，禁止回填伪造日期。
