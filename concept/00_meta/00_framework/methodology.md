...
- [方法论：思维表征与知识结构规范](#方法论思维表征与知识结构规范)
  ...
  - [四、来源引用规范](#四来源引用规范)
    ...
    - [4.3 项目创新内容标记（新增 v1.2）](#43-项目创新内容标记新增-v12)
  - [五、代码示例规范](#五代码示例规范)
    - [5.1 代码块格式](#51-代码块格式)
    - [5.2 示例要求](#52-示例要求)
  - [六、持续演进标记](#六持续演进标记)
...
### 4.3 项目创新内容标记（新增 v1.2）

`concept/` 中大量内容为**项目独创的教学脚手架、对比框架、学习路径或生态深潜**，这些主题在国际权威来源中没有直接对应章节。为避免读者将其误作官方规范，也便于未来对齐国际来源时快速识别，统一使用 `project-specific` 标记：

```markdown
> **内容定位**: **project-specific** — 本文件/本节为项目教学创新或中文语境下的学习路径设计，
> 非 Rust 官方规范。权威语义解释请参见 [concept/xxx/xxx.md](...)。
```
...
| 场景 | 示例 | 标记位置 |
|---|---|---|
| L0 元认知框架 | Bloom Taxonomy、双维认知矩阵、能力图谱 | 文件顶部 `**定位**` 段 |
| 跨语言对比矩阵 | Rust vs C++/Go/Java 等 L5 对比页 | 文件顶部或相关节开头 |
| 版本跟踪与路线图 | `rust_1_XX_stabilized.md`、preview 页 | 文件顶部（同时标注官方来源） |
| 生态深潜 | Tokio 运行时内部、Cargo 工作流 | 相关深度分析节前 |
| 项目特有用法 | 本知识体系的 A/S/P 三维标记 | 元数据规范页 |

**纪律**：

- 若某主题已被国际权威来源系统覆盖，则**不得**使用 `project-specific` 标记；应直接引用权威来源。
- 若项目创新内容未来被官方规范吸收或替代，应移除 `project-specific` 标记并改为 canonical 对齐。
- `scripts/check_authority_coverage.py` 等工具在统计覆盖率时，可识别 `project-specific` 并排除，避免虚高。
