...
> [The Rustonomicon — What Unsafe Rust Can Do](https://doc.rust-lang.org/stable/nomicon/index.html) ·
> [MIRI 未定义行为检测]
> **符号约定**: ⬡ 顶事件 / ◇ 中间事件 / ○ 基本事件 / ∧ 与门 / ∨ 或门
> **定理链**: N/A — 描述性/综述性/导航性文档，不涉及形式化定理链
>
> **来源**: [TRPL](https://doc.rust-lang.org/stable/book/title-page.html) · [Rust Reference](https://doc.rust-lang.org/stable/reference/introduction.html)
---

> **来源**: [IEC 61025 — *Fault Tree Analysis*]
> **来源**: [NASA — *Fault Tree Handbook with Aerospace Applications*]
> **来源**: [RustBelt](https://plv.mpi-sws.org/rustbelt/)
> **来源**: [The Rustonomicon — *What Unsafe Rust Can Do*](https://doc.rust-lang.org/stable/nomicon/index.html)
...
故障树分析（Fault Tree Analysis, FTA）是自顶向下的可靠性分析方法：从「顶事件」（系统级失效）出发，用逻辑门（AND/OR）逐层分解到「基本事件」（组件级失效），用于识别失效路径与薄弱环节。本节规定知识库中 FTA 的统一格式：

- **符号系统（1.1）**：AND 门（全部输入发生才触发）、OR 门（任一输入即触发）、基本事件（圆圈，含概率估计）、中间事件（矩形，含子树引用）、转移符号（子树复用）；
- **分析流程（1.2）**：定义顶事件（具体、可观测）→ 逐级分解「什么会直接导致它」→ 到达基本事件（无法再分或概率已知）→ 定量分析（最小割集、顶事件概率）→ 补偿措施映射（每个基本事件对应的 Rust 防护机制）。

本知识库的 FTA 特色：基本事件的概率列附带「Rust 类型系统是否消除此事件」的判定——被消除的事件标记为 ✅，残余事件构成 `unsafe` 审计清单。
...
本节给出内存安全领域的故障树实例，顶事件为「程序发生内存安全违规（UAF/越界/双重释放）」：

- **顶事件分解（2.1）**：OR 门分解为「use-after-free」「越界访问」「双重释放」「未初始化读取」四类中间事件——每类继续分解到基本事件（如「悬垂引用产生」分解为「返回局部引用」「所有者提前 drop」「`unsafe` 裸指针误用」）；
- **补偿映射（2.3）**：每个基本事件标注 Rust 的防护——借用检查（编译期消除大部分）、`Drop` 语义（析构唯一性）、`unsafe` 块边界（残余风险的审计入口）、Miri（运行期检测残余 UB）。

使用方法：审计 `unsafe` 代码时沿树自底向上——「我的代码可能触发哪个基本事件？它的补偿机制是否仍有效？」树的叶节点即 `unsafe` 代码审查清单。
...
> 来源: [Rust Reference §13 — Memory Model; RustBelt Soundness Theorem 的反面](https://doc.rust-lang.org/stable/reference/introduction.html)
...
本节给出并发领域的故障树实例，顶事件为「并发系统失效」：

- **顶事件分解（3.1）**：OR 门分解为「数据竞争」「死锁」「活锁/饥饿」「时序错误（TOCTOU）」四类中间事件——数据竞争子树在 Rust safe 代码中应全部标记为「类型系统消除」（叶子全部 ✅），残余分支集中在 `unsafe`、FFI 与跨进程共享内存；
- **死锁子树（3.2）**：分解为「锁顺序环」「递归加锁」「持锁等待外部事件」——Rust 不提供死锁免疫（活性问题），补偿是 `Mutex` 的 poisoning（panic 传播）+ 锁顺序规范 + `try_lock` 超时降级；
- **定量分析（3.3）**：基本事件概率列给出各失效模式的经验频率（死锁占并发缺陷的约 30%，文献估计）与检测成本（死锁可运行期检测，数据竞争需 Miri/TSan）。

使用：并发设计评审时沿树检查「每个基本事件在我们的设计中被什么机制覆盖」——未覆盖的叶节点即风险登记项。
...
本节聚焦「类型系统失效树」，覆盖顶事件、故障树 Mermaid与基本事件概率与补偿。论述顺序由定义到边界：先明确「类型系统失效树」在「Rust 知识体系失效分析树集（Fault Tree Analysis Collection）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「类型系统失效树」的判定标准，并指出它在全页论证链中的位置。
...
> 来源: [Rust Reference §10 — Type System; RustBelt Type Safety 的反面](https://doc.rust-lang.org/stable/reference/introduction.html)
...
本节聚焦「异步安全失效树」，覆盖顶事件、故障树 Mermaid与基本事件概率与补偿。论述顺序由定义到边界：先明确「异步安全失效树」在「Rust 知识体系失效分析树集（Fault Tree Analysis Collection）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「异步安全失效树」的判定标准，并指出它在全页论证链中的位置。
...
本节给出 `unsafe` 代码的故障树实例，顶事件为「`unsafe` 代码引发未定义行为」：

- **顶事件分解（6.1）**：OR 门分解为「别名规则违反」（Stacked/Tree Borrows）、「有效性违反」（无效位模式、未初始化读取）、「布局契约违反」（FFI/repr 错误）、「释放责任违反」（double-free/泄漏外的 drop 错误）四大类；
- **基本事件与补偿（6.3）**：每个基本事件对应检测手段——Miri（别名/有效性/未初始化）、`#[repr]` 审查 + cbindgen 一致性（布局）、`Box::from_raw` 配对审计（释放责任）；概率列给出各模式在真实 CVE/安全公告中的占比估计；
- **审计映射**：树可直接转为 `unsafe` 审查清单——「本代码块可能触发哪些基本事件？每个的补偿是否在位？」

实践建议：把本树打印作为 `unsafe` PR 的审查附件——审查者逐叶签字，未覆盖项必须给出论证。
...
> 来源: [Rust Reference §16 — Unsafety; The Rustonomicon — List of UB](https://doc.rust-lang.org/stable/reference/introduction.html)
...
| **一级** | [Rust Reference — Memory Model / Concurrency / Unsafe](https://doc.rust-lang.org/stable/reference/introduction.html) | Rust 语义定义和 UB 分类 |
...
- v1.0 (2026-05-23): 初始版本 — 五棵标准 FTA（内存/并发/类型/异步/Unsafe）+ IEC 61025 符号系统 + 最小割集分析 + 跨树关联 [权威来源对齐 Wave 3](https://doc.rust-lang.org/stable/)
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：本文档《Rust 知识体系失效分析树集（Fault Tree…、测验 2：《Rust 知识体系失效分析树集（Fault Tree An…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层）。
