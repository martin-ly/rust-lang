...
> **关键洞察**: A5（Iris）同时支撑所有权树、借用树和并发树——这意味着 Rust 的内存安全与并发安全并非两套独立机制，而是同一套分离逻辑资源语义的不同投影。[💡 原创分析](methodology.md)
...
本节把所有权领域的定理组织为推理树：根定理「每个值恰一所有者」向下派生三条主链——

- **释放链（2.1）**：所有者唯一 ⟹ drop 点唯一确定 ⟹ 无 double-free / 无泄漏（除显式 `forget`/循环引用）；
- **移动链（3.1）**：move 使源失效 ⟹ 移动后使用被静态拒绝 ⟹ 无 use-after-move；
- **借用前提链（4.1）**：借用必须指向有效值 ⟹ 借用存活 ⊆ 所有者存活 ⟹ 借用检查的存在性证明。

每条推理链附「失效条件与反例」：`unsafe` 断点（责任转移）、`mem::forget`（显式放弃）、内部可变性（检查时机后移）。定理树的价值：调试所有权问题时沿树定位「哪一环被违反」，比通读规则更快。
...
本节组织借用规则的定理推理树，根定理是别名纪律：**任意时刻，对同一数据的访问要么是「多个只读」要么是「一个可写」**（XOR 关系）。派生链：

- **数据竞争排除链（3.1）**：独占可变 XOR 共享只读 ⟹ 不存在「读写并发」 ⟹ 数据竞争在类型层面不可表达（配合 `Send`/`Sync` 扩展到并发域）；
- **悬垂排除链（4.1）**：借用存活 ⊆ 值存活 ⟹ 引用永不悬垂 ⟹ 无 use-after-free（safe 子集内）；
- **重借链（5.1）**：`&mut` 可降级重借为 `&` ⟹ 权限收缩方向不可逆 ⟹ 「拿到 `&mut` 后可临时只读」合法，「从 `&` 升级可写」不可能。

失效条件：`UnsafeCell` 是树内唯一的合法例外（内部可变性协议），`unsafe` 是树外区域。每链附反例与对应编译错误码。
...
推论 C-010: 无数据竞争（Safe Rust） 来源: [RustBelt](https://plv.mpi-sws.org/rustbelt/)
    ...
    ↓
推论 C-011: 迭代器失效排除 来源: [Rust Reference §11](https://doc.rust-lang.org/reference/introduction.html)
...
本节组织生命周期领域的定理推理树，根定理是**引用的有效性由静态证明保证**：每个引用的存活区间都被编译期验证包含于被引用值的存活区间。派生链：

- **省略规则链（4.1）**：输入引用唯一/self 优先 ⟹ 多数签名无需标注 ⟹ 省略是「惯例的形式化」而非推断魔法；
- **标注语义链（5.1）**：`'a: 'b` 是子类型关系声明 ⟹ 标注只收缩不扩展程序行为 ⟹ 「加标注延长生命」的直觉是错误的（标注向编译器提供关系，编译器验证）；
- **高阶链（6.1）**：`for<'a>` 量化所有生命周期 ⟹ HRTB 表达「对任意借用都成立」 ⟹ 闭包/trait 方法接受任意生命周期参数的能力来源。

失效条件：`unsafe` 裸指针无生命周期（证明责任转移）、`'static` 的误用（泄漏换取的「永生」不是解决方案）。每链附反例与对应错误码。
...
类型系统定理树以"类型保持（preservation）+ 进展（progress）= 类型安全"为根定理，向下派生：

- **推理链**：良类型程序不会"卡住"（progress：非值表达式必可继续求值）且求值不改变类型（preservation）⇒ 良类型程序不会出现类型错误 ⇒ Rust 的内存安全定理（在 safe 子集上，悬垂引用不可构造）。链上每条边对应一个引理：替换引理支撑 preservation，规范形式引理支撑 progress。
- **失效条件与反例**：定理在三个边界失效——①`unsafe` 块内（类型保持依赖程序员维护的不变式）；②`mem::transmute` 等显式类型颠覆；③编译器 bug（soundness hole，如历史上的 CVE-2022-21658 类问题）。反例的意义：安全定理的适用范围是"safe Rust + 正确编译器"，工程上 Ferrocene 认证覆盖后者，unsafe 代码审查覆盖前者。
...
定理 T-030: 局部类型推断可判定性 [来源: 04_type_system.md T-030, Pierce TAPL §22]
    ...
    ↓
定理 T-031: Trait 约束求解受限可判定性 [02_generics.md T-032, Rust Reference — Orphan rules](https://doc.rust-lang.org/reference/items/implementations.html#orphan-rules)
...
本节组织并发安全的定理推理树，根定理是 **Rust 的 safe 代码无数据竞争**（数据竞争自由的类型化保证）。派生链：

- **标记 trait 链（6.1）**：`Send`（可跨线程转移）+ `Sync`（可跨线程共享引用）是 auto trait ⟹ 编译器按结构自动推导 ⟹ 「什么能跨线程」无需人工审计（安全子集内）；
- **互斥链（7.1）**：`Mutex<T>` 把「访问」与「锁持有」绑定（guard 类型）⟹ 解锁即失去访问权 ⟹ 「忘记加锁」不可表达；
- **消息传递链**：通道按值移动消息 ⟹ 发送方失去所有权 ⟹ 无共享即无竞争。

失效条件与边界：死锁/活锁**不在**保证范围（活性 vs 安全性）、`unsafe impl Send/Sync` 的错误标注（健全性责任转移）、`static mut`（保证的例外）。树的使用：并发 bug 分类时先判定「属于安全失效还是活性失效」——前者在 Rust 中只剩 `unsafe` 来源。
...
异步定理树围绕"Future 的可取消性与内存安全共存"这一核心定理展开：

- **推理链**：`Future` 是惰性的状态机 ⇒ `drop(future)` 即取消，无需额外协议 ⇒ 取消安全等价于"Future 在任一 await 点被 drop 后不变式仍成立" ⇒ `Pin` 保证自引用 Future 的地址稳定 ⇒ async/await 的类型安全归约为 generator 变换的 correctness + Pin 的稳定性定理。链的关键引理：编译器生成的状态机与手写的等价（语义保持变换）。
- **失效条件与反例**：①取消不安全（cancel-unsafe）的 Future——如跨 await 持有借用的缓冲区被 drop 后重用；②`Pin` 被 `unsafe` 代码违反（如错误的 `Pin::new_unchecked`）；③`select!` 宏的公平性与取消交互——轮询顺序导致的饥饿不被类型系统捕获。这三类反例分别对应库设计、unsafe 审查与运行时语义三个治理层。
...
定理 T-051: Pin 内存稳定性 来源: [RFC 2349, Pin API docs](https://rust-lang.github.io/rfcs/2349-pin.html)
    ...
    ↓
推论 C-050: 自引用结构安全 来源: [Rustonomicon §4.8](https://doc.rust-lang.org/nomicon/index.html)
    「自引用字段的指针在 Pin 下保持有效」
```
...
本节聚焦「跨树关联与一致性」，覆盖核心定理依赖图 与 一致性检查清单。论述顺序由定义到边界：先明确「跨树关联与一致性」在「Rust 知识体系定理推理森林」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「跨树关联与一致性」的判定标准，并指出它在全页论证链中的位置。
...
> **关键洞察**: T-040（Send/Sync）是跨树连接的「超级节点」——它同时合并了所有权树（T-001）和借用树（T-010）的结论，是 Rust 「fearless concurrency」的工程化交汇点。[💡 原创分析](methodology.md)
...
- [L3 并发](../../03_advanced/00_concurrency/01_concurrency.md) —— T-040 / T-041 / T-042
...
本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：本文档《Rust 知识体系定理推理森林》在 Rust 知识体…、测验 2：《Rust 知识体系定理推理森林》的主要用途是什么？（理解层）与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层）。
