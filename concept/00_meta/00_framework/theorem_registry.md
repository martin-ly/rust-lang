...
本节聚焦「已注册定理清单」，覆盖所有权域（T-001 – T-009）、借用域（T-010 – T-019）、生命周期域（T-020 – T-029）、类型系统域（T-030 – T-039）等方面。论述顺序由定义到边界：先明确「已注册定理清单」在「定理链全局注册表（Theorem Registry）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「已注册定理清单」的判定标准，并指出它在全页论证链中的位置。
...
- **P0 官方**: [Rust Reference — Type System / Traits / Lifetimes](https://doc.rust-lang.org/reference/types.html) · [The Rustonomicon](https://doc.rust-lang.org/nomicon/) · [Rust RFCs](https://rust-lang.github.io/rfcs/)
