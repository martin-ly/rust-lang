...
可判定性谱系（decidability spectrum）把 Rust 的各项静态检查按「判定问题的复杂性类」分层排列——从多项式时间可判定到理论不可判定。本节建立梳理方法论：

- **判定问题的形式化**：每个检查器对应一个判定问题（输入：程序；输出：是/否）——借用检查 = 「该程序是否存在借用冲突」；
- **谱系分层**：P 层（类型检查、借用检查）→ NP/EXPTIME 层（trait 求解在最坏情况）→ 半可判定层（宏展开后的任意性质）→ 不可判定层（任意程序性质，Rice 定理）；
- **来源锚定**：每层附原始文献——Rice 1953、TAPL（Pierce 2002）的可判定性章节、trait 求解的 coinductive 证明（Chalk/Rust 团队）。

梳理原则：一个检查「能稳定存在」的根据是它落在可判定层——理解谱系即理解「为什么 Rust 的类型系统要刻意拒绝某些表达」。
...
本节给出全文的核心框架：把 Rust 的各项静态检查按**可判定性**排成谱系——从「完全可判定」（词法、语法）到「可判定但有复杂度代价」（类型推断、借用检查）再到「刻意保持可判定的边界设计」（无 HKT、无高阶类型递归），最后到「本质上不可判定、只能近似」（活跃性、任意程序性质）。

阅读主线：

- **谱系全景图（2.1）**：五级分层（L1 语法层 → L5 并发安全层）与各层的判定状态色标；
- **判定边界色标矩阵（2.2）**：每项检查标注「可判定/半可判定/不可判定」+ 复杂度类——绿色区域是 Rust 的设计成就区（把重要性质放进可判定区），黄色是工程近似区。

使用方法：遇到「为什么编译器不检查 X」的问题，先在矩阵中定位 X 的层级——若在不可判定区，答案是「理论上不可能」而非「实现未完成」。
...
类型系统层是可判定性谱系中**张力最大的一层**：Rust 既要保留足够表达力（泛型、Trait、生命周期），又必须保证编译一定终止。三个子问题给出三种不同的判定答案：

| 子问题 | 判定结论 | 关键机制 |
|:---|:---|:---|
| 局部类型推断 | 可判定（受限 HM 子集） | 约束生成 + 统一化，禁止跨函数推断 |
| Trait 约束求解 | 有条件可判定（设终止性限制） | 贪心选择 + 递归深度上限 |
| 类型等价与一致性 | 可判定 | 结构化等价 + 正则化（normalization）边界 |

Rust 的设计哲学在此处体现为**以表达力换取判定性**：放弃 HM 的全局推断与 let-多态，要求函数签名显式标注，从而把类型推断限制在函数体内这一有限问题上。Trait 求解则通过 `recursion_limit` 把理论上不可判定的逻辑编程问题截断为可判定的有界搜索。
...
本节定位借用检查在可判定性谱系中的位置，回答「借用检查为什么是可靠的工程工具而非启发式」：

1. **判定问题**：「程序 P 中是否存在悬空引用或别名违规」——NLL 将其归约为约束求解（outlives 约束 + liveness 区间），约束系统是**可判定的**（有限区域集合上的单调方程组，不动点收敛）；
2. **复杂性**：实践中近线性（借用检查不是编译瓶颈），最坏情况与 CFG 规模多项式相关——与类型推断的指数最坏情况形成对照；
3. **不判定的边界**：跨过程分析被刻意排除（函数签名即接口约定）——「模块化可判定」是设计选择；`unsafe` 区域的可判定性由程序员承担；
4. **Polonius 的位置**：Datalog 化的借用检查仍是可判定的（Datalog 有限模型性质），代价是计算资源——谱系内移动而非跨界。

附与 C++ 对照：C++ 的别名分析不可判定是「无所有权模型」的直接后果。
...
> **边界**: Polonius 的挑战不在于判定性，而在于**性能可扩展性**。大型函数产生的事实集可能很大，因此引入了分级分析（graded analysis）和近似。 Polonius Update, Rust Blog 2023（该日期无对应官方博文）
...
> **定理 T-DS-004（生命周期约束可满足性）**: 对有限生命周期变量集和有限约束集，生命周期约束可满足性问题是**可判定的**，且可在多项式时间内求解（传递闭包 = O(V³)）。 [Tofte-Talpin, *Region-Based Memory Management*](https://doi.org/10.1145/176454.176456)
...
并发安全层呈现了谱系中**最鲜明的判定性二分**：同样是并发缺陷，数据竞争可以在编译期完全排除，死锁却连运行时都不可判定。原因在于二者的可归约问题不同：

- **数据竞争**可归约为"两个访问是否可能并发且至少一个为写"——这是关于类型与所有权的静态性质，`Send`/`Sync` 把它编码进类型系统，编译期即可判定。
- **死锁**涉及锁的获取顺序、运行时调度与条件变量的交互，本质是可达性问题，与停机问题同属不可判定类；Rust 不提供也不试图提供静态保证。

这一对比说明一个重要方法论原则：Rust 的编译期保证只覆盖**可归约为类型/所有权性质**的缺陷类。判断某类 bug 能否被 Rust 静态排除，等价于判断它能否编码为类型不变式。
...
> **定理 T-DS-007（死锁检测的不可判定性）**: 判定任意程序是否会在某个执行路径上发生死锁是**不可判定的**。证明思路：将停机问题归约到死锁检测——构造一个程序，其死锁条件等价于某图灵机的停机条件。 [Rice 1953, *Classes of Recursively Enumerable Sets*](https://doi.org/10.2307/2268381)
...
本节界定 Rust 静态分析的理论天花板，两个支柱定理：

- **Rice 定理的映射（10.1）**：程序的任何非平凡语义性质（「这个函数会 panic 吗」「这个通道会死锁吗」）都不可判定——Rust 编译器的所有检查都是**可判定的保守近似**：拒绝一些实际正确的程序（假阳性）以保证永不接受错误程序（soundness）。`unsafe` 的存在意味着「内存安全」整体也是 Rice 定理管辖——编译器只保证「safe 子集」内的安全；
- **停机问题与 CTFE 的边界（10.2）**：常量求值（CTFE）理论上面临停机问题——`const fn` 的无限循环无法在编译前判定。Rust 的工程解法是**步数限制**（`long_running_const_eval` lint + 求值预算）：把不可判定的「是否停机」转换为可判定的「是否在预算内停机」。

推论：任何声称「完全静态验证程序性质」的工具都在做近似——理解近似的方向（保守/激进）比理解算法本身更重要。
...
思维表征体系把可判定性谱系从"线性列表"重构为两种可操作的工具：

- **判定路径决策树**：给定一个"编译器能否静态判定性质 P"的问题，沿树下行——P 是否可归约为类型/所有权性质？是 → 可判定（类型系统直接覆盖）；否 → P 是否可归约为有限状态可达性？是 → 有界可判定（模型检测，如 Kani）；否 → 不可判定（Rice 定理），只能运行时检测或接受。该树可直接用于评估"某类 bug 能否被 Rust 静态排除"类面试题。
- **谱系演化时间线**：从 Rice 定理（1953，理论基础）→ Hindley-Milner（1978，可判定推断）→ Cyclone（2002，区域类型系统）→ RustBelt（2018，机器验证的核心演算）→ Polonius（进行中，更精确的借用分析）。时间线显示：每一代进展都是把"原本不可判定的性质"通过**限制表达力**变成可判定。
...
可判定性谱系的定理推理链以 Rice 定理为根，向下派生出谱系中每个"不可判定"节点的证明，向上支撑每个"可判定"节点的构造：

- **定理一致性矩阵**：行是谱系节点（词法判定、类型推断、借用检查、CTFE 终止、数据竞争排除、死锁检测），列是三个一致性检查（与 Rice 定理不矛盾 / 与已发布的形式化模型一致 / 与 rustc 实际行为一致）。例如"借用检查可判定"通过三检：它不是语义性质（不受 Rice 约束）、与 RustBelt 的形式化一致、与 rustc 的编译终止性一致。
- **推理链图谱**：核心链为 Rice 定理 ⇒ 运行时性质不可判定 ⇒ Rust 只静态保证可归约为类型性质的安全子集 ⇒ 借用检查必须设计为句法/数据流性质 ⇒ NLL 的约束求解算法。链上每条边标注支撑文献（Rice 1953、NLL RFC 2094、RustBelt POPL 2018）。
...
本节给出可判定性谱系在知识库 L0-L7 各层的概念映射，供按层深入：

- **L1 基础**：借用检查的可判定性直觉（「编译器总能给出答案」）⟶ 04_borrow_checking_decidability；
- **L2 进阶**：类型推断的约束求解（HM 可判定性）⟶ type_inference；trait 求解的终止性条件；
- **L3 高级**：宏展开的可判定性边界（过程宏 = 任意计算 ⟹ 展开后分析不可判定）；const 求值的终止性保证；
- **L4 形式化**：Rice 定理证明、停机问题归约、HM/子类型可判定性的形式证明 ⟶ type_theory / borrow_checking_decidability；
- **L5 对比**：C++ 模板实例化的不可判定性（图灵完备模板）vs Rust 泛型的受控表达；Java 泛型擦除对可判定性的简化；
- **L6 生态**：Miri/Kani 的「欠拒绝」定位（执行路径有限）；
- **L7 前沿**：Polonius（更精确的可判定近似）、generic_const_exprs（类型级求值的终止性问题）。

每个链接标注「该层视角回答什么问题」——避免跨层重复阅读。
...
本节聚焦「可判定性论文精确引用与前沿映射」，覆盖POPL / PLDI 可判定性相关论文精确引用、Rust 特性可判定性状态 1.95+、开放问题（Open Problems）与边界测试：类型推断的不可判定性与人工标注（编译错误）。论述顺序由定义到边界：先明确「可判定性论文精确引用与前沿映射」在「Rust 编译期可判定性谱系全景（Decidability Spectrum）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「可判定性论文精确引用与前沿映射」的判定标准，并指出它在全页论证链中的位置。
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：本文档《Rust 编译期可判定性谱系全景（Decidabil…、测验 2：《Rust 编译期可判定性谱系全景（Decidability…与测验 3：元数据层文档能否替代 L1-L7 的核心概念学习？（理解层），本节逐一说明其要点。
