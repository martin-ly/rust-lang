...
| **Lifetimes** | TRPL Ch10 · Reference § Lifetime Elision · RFC 2094 (NLL) | Villani, Hostert, Dreyer & Jung *"Tree Borrows: A New Aliasing Model for Rust"* (PLDI 2025, Distinguished Paper Award) | Jon Gjengset *"Crust of Rust: Lifetime Annotations"* | 无直接对应（RAII + 智能指针处理） | 显式 region / let-binding scope | 无（GC 决定） |
| **Type System** | Reference § Types · RFC 1210 (impl trait) | Pierce *"Types and Programming Languages"* (TAPL, MIT Press) · Hindley-Milner 类型推断 | Without Boats *"Implied bounds and perfect derive"* | Templates · Concepts (C++20) | HM 类型推断 · Type Classes (Wadler & Blott, 1989) | 结构化类型 · 接口（隐式实现） | [来源: TAPL 是类型系统的标准教材; HM 类型推断是 Rust 类型系统的基础]
...
| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 权威来源映射表（Authority Source Map） 结构化组织 ⟹ 高效检索 | 理解分类维度与索引关系 | 能快速定位目标概念 | 高 |
| 权威来源映射表（Authority Source Map） 质量评估 ⟹ 持续改进 | 建立量化指标与审计流程 | 识别知识缺口并优先修复 | 高 |
