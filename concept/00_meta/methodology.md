...
### 关系质量（v1.1 新增）
- [ ] **定理一致性矩阵**: 包含完整的"前提-结论-L4公理-失效条件-错误码"矩阵
- [ ] **反命题树**: 包含 Mermaid 反命题决策图 + 边界极限测试代码
- [ ] **认知路径**: 包含 6 步递进 + 认知脚手架
- [ ] **跨层映射**: 文件末尾链接到 `inter_layer_map.md` 的对应章节
- [ ] **交叉概念一致性**: 非主定义文件使用链接引用，不重复定义
- [ ] **跨引用密度**: 文件内包含 ≥3 个跨文件链接
- [ ] **层间出口**: 文件末尾标注掌握后可进入的层级
