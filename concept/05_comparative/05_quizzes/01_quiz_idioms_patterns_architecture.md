...
- A. 能，输出 `1`
- B. 不能，`Meters` 与 `Kilometers` 不能直接这样用
- C. 能，输出 `1000`
- D. 不能，因为 `u32` 不能整除
...
Newtype 通过元组结构体包装底层类型；访问内部字段 `m.0` 得到原始 `u32`，因此表达式合法，输出 `1`。

</details>

---

### Q3. 🟡 Typestate 模式的核心收益是什么？
...
### Q5. 🟡 线段树（Segment Tree）的单点更新和区间查询时间复杂度分别是？
...
### Q6. 🟢 并查集（Union-Find）使用路径压缩后的均摊查找复杂度接近？
...
### Q11. 🟡 Hexagonal Architecture（端口与适配器）强调？
