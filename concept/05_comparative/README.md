...
对比分析遵循四条原则，保证结论可复核而非印象式评价：

1. **同任务基线**：对比双方实现同一可运行任务（如并发计数器、JSON 解析），而非对比语言宣传材料；
2. **版本固定**：标注 rustc 与对方编译器的精确版本，结论仅在该版本组合下成立；
3. **维度正交**：将"安全性""性能""人机工学""生态成熟度"拆为独立维度，禁止用单一维度代表整体；
4. **证据可追溯**：性能结论附基准代码，语义结论附编译器报错或标准文档链接。

下文的"对比维度矩阵"将这些原则固化为评分表，供 `05_comparative/` 下所有跨语言页面复用；各页结论的权重应回链到矩阵中的维度定义。
...
- **L4 形式化**: 对比结论依赖的理论基础 → [类型论](../04_formal/00_type_theory/01_type_theory.md) · [所有权（Ownership）形式化](../04_formal/01_ownership_logic/02_ownership_formal.md)
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：《L5 对比分析层（Comparative Analysis…、测验 2：使用本索引文件时，最有效的学习策略是什么？（理解层）与测验 3：索引文档能否替代具体概念文件的学习？（理解层），本节依次展开。
