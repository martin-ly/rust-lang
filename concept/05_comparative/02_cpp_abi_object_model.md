...
> **Rust 与 C++ 在源代码层面的差异（所有权、生命周期、Trait）已被广泛讨论，但二进制层面的差异
...
> **[来源: System V AMD64 ABI] · [Wikipedia: Application binary interface]** ✅

ABI 定义了程序在二进制层面的交互规则：

- **调用约定**: 参数如何传递（寄存器/栈）、返回值如何处理
- **类型布局**: 结构体/枚举在内存中的表示（大小、对齐、padding）
...
> **[来源: Rust Foundation Interop Initiative] · [Itanium C++ ABI]** ✅
...
> **[来源: Rust Reference — §7.4 ABI] · [Rust RFC — ABI Stability]** ✅
...
> **[来源: System V AMD64 ABI] · [Rust Reference — §7.4]** ✅
...
> **[来源: Itanium C++ ABI — §2.4 Data Layout] · [C++ Standard — §11.4 Class layout]** ✅
...
> **[来源: Rust Reference — §4.1.13 Trait objects] · [Rustonomicon — Data Layout]** ✅
...
| 对象切片 | 可能（值拷贝时） | **不可能**（`dyn Trait` 只能通过引用/指针使用） |
...
> **关键洞察**: Rust 通过禁止 `dyn Trait` 的值类型（必须配合 `&`、`Box` 或 `Rc` 使用），**在类型层面消除了对象切片问题**。这是 Rust 设计优于 C++ 的核心工程决策之一。[来源: 💡 原创分析]

### 3.3 结构体内存布局对比

#### 3.3.1 Padding 与对齐

> **[来源: C++ Standard — §11.4.1] · [Rust Reference — §4.2.1 Type layout]** ✅
...
> **[来源: Rust Reference — §7.3 Linkage] · [rustc-dev-guide — Symbol Mangling]** ✅
...
- C++ 修饰名依赖参数类型（支持重载）
- Rust 修饰名依赖 crate hash（支持泛型单态化后的唯一标识）
...
| 类型安全 | 运行时 `std::get` 可能 `bad_variant_access` | 编译期 match 穷尽性保证 |
...
> **[来源: Itanium C++ ABI — §2.4 Exception Handling] · [C++ Standard — §15 Exception handling]** ✅
...
> **[来源: Rust Reference — §7.9 Panics] · [RFC 2945 — Panics in Rust]** ✅
...
| 类型匹配 | 运行时类型匹配（RTTI） | 无类型匹配（panic 对象统一为 `&dyn Any`） |
...
| 结构体布局 | 声明顺序 | 声明顺序 | `#[repr(C)]` = C 顺序 | 编译器可重排 |
| 虚函数 / 动态分发 | 无 | vptr 内嵌对象 | 胖指针 | 胖指针 |
| 对象切片 | 无 | 可能 | **不可能** | **不可能** |
| 异常跨边界 | 无 | 困难 | `panic=abort` / `catch_unwind` | — |
| 名称修饰 | 无 | 复杂（`_Z...`） | 无 | `_R...` |
| 枚举布局 | 整数 | `std::variant`（大） | `#[repr(C)]` = 整数 | 紧凑 + niche opt |
