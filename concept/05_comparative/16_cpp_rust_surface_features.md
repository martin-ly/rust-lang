...
> **双维定位**: C×Ana
> **前置概念**: [Rust vs C++](./01_rust_vs_cpp.md) · [Variable Model](../01_foundation/20_variable_model.md) · [RTTI](../02_intermediate/25_rtti_and_dynamic_typing.md)
> **后置概念**: [C++ ABI Object Model](./02_cpp_abi_object_model.md) · [Traits](../02_intermediate/01_traits.md)
> **主要来源**: [The C++ Programming Language, Stroustrup] · [TRPL Ch 5 — Using Structs] · [Rust Reference — Operators]
---
...
> Rust 则把这些能力拆散到类型系统、trait、模块可见性和标准库中，用显式约束替代隐式规则。**
...
> **详细讨论见 [类型系统基础：运算符重载](../01_foundation/04_type_system.md)。
...
| 可重载运算符 | 大量（含 `new`、`delete`、`->`） | 有限集合（算术/位运算/索引/解引用等） |
...
C++ 的 OO 特性是**语法内建**的：构造函数、运算符、`friend` 都是编译器直接识别的特殊语法。Rust 把这些能力**去语法化**，全部表达为类型系统或库机制：
...
> **关键洞察**：Rust 的设计哲学是"没有特权语法"——同样的类型系统规则同时约束普通代码和"面向对象"代码，从而减少意外行为和隐式依赖。
