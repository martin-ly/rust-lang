...
- **内存安全（memory safety）**: 程序不会访问其不应访问的内存——无越界、无悬垂指针、无未初始化读取、无重复释放；这是 Rust 借用检查器在编译期**强制**的属性（safe Rust 子集）。
- **类型安全（type safety）**: 值不会被当作不属于它的类型解释——`mem::transmute` 之外，Rust 的类型系统保证表示与解释一致；内存安全是类型安全的推论（类型混淆是内存错误的主要途径）。
...
- **内存安全边界**: 越界条件 = 原始指针解引用、`unsafe` 索引（`get_unchecked`）、未初始化内存读取（`MaybeUninit` 误用）、`transmute` 大小/布局不匹配；边界两侧的责任划分：safe 侧保证输入合法，unsafe 侧保证不变量维持。
- **并发安全边界**: 越界条件 = 跨线程共享非 `Sync` 类型、无同步地写共享数据、锁顺序不一致导致死锁（死锁不属数据竞争，是活性问题）；`Send`/`Sync` 的 unsafe impl 是边界的信任声明点。
...
- **按失效层级分类**: ① 编译期被拦截（绝大多数——借用错误、Send/Sync 违反、生命周期不足）；② 运行期受控失败（panic——索引越界、整数溢出 debug 模式、unwrap 失败；进程终止但无 UB）；③ 未定义行为（仅在 unsafe 代码或编译器 bug 下可达——悬垂访问、数据竞争、无效值构造）；安全工程的全部目标是让缺陷停留在 ①② 层。
...
| **safe Rust 保证内存安全（Memory Safety）** | 所有权+借用+生命周期（Lifetimes） ⟹ | 无UAF/无数据竞争 | 逻辑错误未覆盖 | 默认编程模式 |
...
| FFI 边界 | 所有权跨边界转移、空指针、布局一致性 | `#[repr(C)]` + sanitizer |
