...
- v1.1 (2026-05-12): 补充 Wikipedia 权威定义、课程引用（Reference）、学术论文、跨文件链接
- v1.2 (2026-05-13): 修复章节编号错误；新增"跨系统信任边界对比"（从 PostgreSQL 零信任分析中提取 Rust 同构映射）；补充 PG MVCC ↔ Rust 所有权（Ownership）、借用（Borrowing）检查器 ↔ Repeatable Read、多系统信任矩阵

---

> **后置概念**: [Ecosystem](../../06_ecosystem/00_toolchain/01_toolchain.md)
> **前置概念**: [Rust vs C++](../01_systems_languages/01_rust_vs_cpp.md)

## 一、权威定义

安全边界的权威定义需要区分三个常被混用的层次，Wikipedia 的定义提供了中性起点：

- **内存安全（memory safety）**: 程序不会访问其不应访问的内存——无越界、无悬垂指针、无未初始化读取、无重复释放；这是 Rust 借用（Borrowing）检查器在编译期**强制**的属性（safe Rust 子集）。
- **类型安全（type safety）**: 值不会被当作不属于它的类型解释——`mem::transmute` 之外，Rust 的类型系统（Type System）保证表示与解释一致；内存安全（Memory Safety）是类型安全的推论（类型混淆是内存错误的主要途径）。
- **并发安全（data-race freedom）**: 多线程访问共享数据时无数据竞争——`Send`/`Sync` trait 系统把这一性质同样编码为编译期约束。

关键界定：Rust 的“安全”是**可机器验证的子集属性**，unsafe 块是显式的信任边界——边界外程序员向编译器承诺不变量，边界内编译器向程序员保证属性。
...
> **[Wikipedia: Type safety](https://en.wikipedia.org/wiki/Type_safety)** Type safety is the extent to which a programming language discourages or prevents type errors.
> **来源**: <https://en.wikipedia.org/wiki/Type_safety>
> **[Wikipedia: Race condition](https://en.wikipedia.org/wiki/Race_condition)** A race condition is the condition of a system where the system's substantive behavior is dependent on the sequence or timing of other uncontrollable events.
> **来源**: <https://en.wikipedia.org/wiki/Race_condition>
> **[Wikipedia: Deadlock](https://en.wikipedia.org/wiki/Deadlock)** A deadlock is a state in which each member of a group is waiting for another member, including itself, to take action.
...
抽象封装unsafe（Unsafe）/最小化unsafe面积/文档化不变量

### 第 6 步：安全边界的未来方向？
>

safe transmute/更好的FFI（FFI）/编译器辅助验证
...
> **认知功能**: 建立 Rust 安全保证的三层分类框架（Safe/边界/Unsafe），展示编译期保证与突破方式之间的完整映射关系。在安全架构评审时对照此图检查每个模块（Module）所处的层级。**关键洞察**: Safe Rust 的六大保证并非全部被 Unsafe 打破——部分可通过 Safe 边界条件合法削弱。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
三类安全边界各有一张条件清单，明确“什么操作越界”是理解边界的第一步：

- **内存安全边界**: 越界条件 = 原始指针（Raw Pointer）解引用（Reference）、`unsafe` 索引（`get_unchecked`）、未初始化内存读取（`MaybeUninit` 误用）、`transmute` 大小/布局不匹配；边界两侧的责任划分：safe 侧保证输入合法，unsafe 侧保证不变量维持。
- **并发安全（Concurrency Safety）边界**: 越界条件 = 跨线程共享非 `Sync` 类型、无同步地写共享数据、锁顺序不一致导致死锁（死锁不属数据竞争，是活性问题）；`Send`/`Sync` 的 unsafe impl 是边界的信任声明点。
- **类型安全边界**: 越界条件 = `transmute`、union 字段误读、`Any` 下行转型失败（受控 panic，非 UB）、FFI 边界的类型表示假设（`#[repr(C)]` 之外无布局保证）。

判定依据：安全审计的起点是列出 crate 的所有 unsafe 点，逐点核对它维持的是哪类边界的哪个不变量。
...
| **无悬垂指针** | 生命周期约束 | 自引用（Reference）结构（无 Pin） | Safe 编译错误 | E0597 | L1 Lifetimes |
| | | `Box::leak` 获 'static | Safe | 合法但泄漏 | L2 Memory |
| | | `unsafe` 构造悬垂引用（Reference） | Unsafe | 悬垂指针（UB） | L3 Unsafe |
...
| | | `RefCell` 运行时（Runtime） panic | Safe | 运行时借用（Borrowing）冲突 | L2 Memory |
...
| **类型一致性（Coherence）** | 类型检查 | `dyn Any::downcast` | Safe | 可能返回 None | L1 Type System |
...
失效分类学把“安全属性如何被破坏”系统化，两个正交维度交叉出完整的失效图谱：

- **按失效层级分类**: ① 编译期被拦截（绝大多数——借用错误、Send/Sync 违反、生命周期（Lifetimes）不足）；② 运行期受控失败（panic——索引越界、整数溢出 debug 模式、unwrap 失败；进程终止但无 UB）；③ 未定义行为（仅在 unsafe 代码或编译器 bug 下可达——悬垂访问、数据竞争、无效值构造）；安全工程的全部目标是让缺陷停留在 ①② 层。
- **按后果严重性分类**: 信息泄露（未初始化读）< 拒绝服务（panic/死锁）< 内存破坏（越界写）< 控制流劫持（函数指针/返回地址覆写）；Rust 安全属性直接消除后两类的 safe-代码路径。

判定依据：代码评审按“该 unsafe 点最坏能造成哪一层哪一类失效”提问，回答不出具体不变量的 unsafe 块应拒绝合并。
...
| 严重性 | 场景 | 示例 | 可检测性 |
|:---|:---|:---|:---|
| **可恢复错误** | panic、死锁、超时 | `unwrap()`、`RefCell` panic | 运行时（Runtime）可见 |
...
| **无生命周期（Lifetimes）** | 悬垂指针风险 | 智能指针（C++）、引用（Reference）计数 | C++ `shared_ptr`、Swift ARC |
...
> **认知功能**: 提供代码安全性的完整判定流程图，展示从编译检查到 Miri 动态验证的安全决策链。遇到安全相关问题时按图索骥，判断代码所处阶段和可选验证手段。**关键洞察**: 安全不是二元的——即使进入 unsafe，完整契约 + Miri 验证仍可趋近安全。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **来源**: [Rustonomicon — FFI](https://doc.rust-lang.org/nomicon/ffi.html) · [cxx 文档] · [cbindgen 文档] · [diplomat 文档]
...
> **认知功能**: 将 FFI 安全假设拆解为可逐项验证的前提链，暴露每个环节的失效模式。编写 FFI 绑定前逐条核对 C 端的别名规则、线程安全性和内存布局。**关键洞察**: FFI 安全不是"Rust 端正确"就够的——C 端的行为假设必须通过外部审计确认。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> 4. **组织审计**: `cargo vet` 建立"允许列表"，只有被团队审查过的 crate 才能进入构建。 来源: cargo-audit docs / cargo-vet docs / [Mozilla Supply Chain](https://www.mozilla.org/en-US/security/)
...
> **深层意义**:
> TrapDoor 攻击揭示了一个新型威胁向量——**AI 辅助开发工具的供应链攻击**。
> 传统的 `cargo audit` 和 RustSec 数据库只能检测已知的 CVE，但 TrapDoor 利用的是"AI 上下文污染"，这在现有安全工具链中没有直接对应检测手段。
> 这要求 Rust 生态在三个层面加强防御：
>
> 1) 注册表层面（更严格的包内容扫描，包括 Unicode 异常检测）；
> 2) 工具层面（AI 编码助手的上下文隔离）；
> 3) 开发者层面（对 AI 生成代码的审计意识）。
> [来源: [Socket.dev — TrapDoor Supply Chain Attack](https://socket.dev/blog/trapdoor-crypto-stealer-npm-pypi-crates)] · 可信度: ✅
...
> **过渡**:
> 安全边界不仅是 Rust 内部的 `safe`/`unsafe` 分界线，更是不同系统在不同层次上建立"不信任假设"的工程选择。
> 将 Rust 放在 PostgreSQL、区块链、Git 的零信任谱系中对比，能更清晰地定位其安全模型的独特性和边界。
> **方法论对齐**:
> [Wikipedia: Zero trust security model](https://en.wikipedia.org/wiki/Zero_trust_security_model) ·
> [Wikipedia: Trust (social)](https://en.wikipedia.org/wiki/Trust_(social)) ·
> Felleisen 1989 表达力比较

---

### 10.1 与 PostgreSQL MVCC 的所有权同构

PostgreSQL 的 MVCC（Multi-Version Concurrency Control）与 Rust 的所有权（Ownership）系统，是**同一安全公理在不同时间尺度的实现**：

| 维度 | Rust 所有权（Ownership） | PG MVCC |
|:---|:---|:---|
| **时间尺度** | 编译期（代码生成前） | 运行期（事务执行时） |
| **版本策略** | 单版本（任一时刻只有一个所有者） | 多版本（旧版本与新版本共存） |
| **冲突解决** | 编译错误（借用（Borrowing）检查器拒绝） | 快照隔离（事务看到历史版本） |
| **安全保证** | 无数据竞争、无 UAF | 无脏读、无不可重复读（RR 级别） |
| **形式化根基** | 线性逻辑 / 分离逻辑 | 时态逻辑 / 可串行化理论 |

> **关键洞察**:
> Rust 在代码生成前消除冲突，PG 在执行时允许多重现实共存。
> 两者都是"通过限制可见性来保证安全性"——Rust 限制引用（Reference）可见性，PG 限制事务可见性。
> **来源**:
> [PostgreSQL 官方文档 - MVCC] · [CMU 15-721] ·
...
|:---|:---|:---|:---|:---|
| **硬件位翻转** | CRC32C + Full Page Write | 多节点哈希验证 | 内存安全（Memory Safety）类型（编译期对齐约束） | SHA-256 对象校验 |
| **OS 崩溃** | fsync + 进程隔离 | 节点自治 + P2P 恢复 | 运行时（Runtime）边界检查（panic = 线程终止） | 文件系统抽象（对象不可变） |
| **并发冲突** | MVCC + SSI（快照 + 危险结构检测） | 共识算法 + UTXO/Nonce | 所有权（Ownership） / 借用（Borrowing） / Send/Sync | 分支隔离（无共享状态） |
| **进程崩溃** | Postmaster 监督 + 共享内存清理 | 节点快速重启 + 多数共识 | panic = 线程终止（不污染其他线程） | 无进程概念 |
| **网络/分布式** | 主从信任（流复制假设主节点诚实） | 拜占庭容错（BFT） | N/A（单进程模型） | 离线工作 + 合并冲突检测 |
| **用户输入** | 约束 + 触发器 + ACL | 脚本验证 + Gas 限制 | 类型系统（Type System）（穷尽检查 + 模式匹配（Pattern Matching）） | 无（信任开发者） |
...
> [Ethereum 黄皮书] ·
> [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html) ·
> [Git 内部原理] · [Wikipedia: Zero trust security model](https://en.wikipedia.org/wiki/Zero_trust_security_model)

### 10.3 借用检查器的跨系统对应

Rust 的借用（Borrowing）检查器与 PostgreSQL 的隔离机制存在深层同构：
...
| 机制 | 不信任假设 | 验证方式 | 失效后果 |
|:---|:---|:---|:---|
| `&mut T` | 存在其他活跃引用（Reference） | 编译期借用（Borrowing）检查 | 编译错误 E0597 / E0502 |
| `Repeatable Read` | 其他事务修改数据 | 事务快照冻结 | 幻读（Phantom Read，仅 PG RR 级别允许） |
| `Serializable` | 事务间存在非串行化交错 | SSI 危险结构检测 | 事务回滚（主动放弃而非被动出错） |

> **形式化对应**:
> Rust 的 `&mut T` 是**分数权限（Fractional Permissions）**的极端情况（1.0 = 独占）；
> PG 的 `Repeatable Read` 是**快照隔离（Snapshot Isolation）**的保守实现。
> 两者都通过"限制并发可见性"来维护一致性（Coherence）。
> **来源**:
> [Boyd-Wickizer et al. OSDI 2008] ·
...
> **认知功能**:
> 将 Rust 的安全边界放在更广泛的零信任谱系中定位，明确其覆盖范围与盲区。
> 评估系统安全需求时，确认 Rust 的编译期保证是否足以覆盖目标威胁模型。
> **关键洞察**: Rust 的信任止于进程地址空间——硬件位翻转、OS 崩溃、分布式拜占庭故障均在其外。
> [💡 原创分析](../../00_meta/00_framework/methodology.md)

**Rust 的不信任哲学**：

- **不信任程序员**：编译器通过借用（Borrowing）检查器、类型系统（Type System）、穷尽检查，主动拒绝可能不安全的代码。
- **信任硬件/OS**：Rust 不验证磁盘位翻转、不处理 OS 崩溃、不信任边界止于进程地址空间。
- **不信任其他代码**：`unsafe` 块是显式逃逸门，但 `safe` API 封装者承担证明责任。

**与区块链的对比**：
区块链将不信任推向物理边界（全球冗余），Rust 将不信任推向编译边界（静态证明）。
两者都是"用确定性替代信任"，但一个用**分布式共识**，一个用**形式化类型系统（Type System）**。

> **来源**:
> [Wikipedia: Rust (programming language)](https://en.wikipedia.org/wiki/Rust_(programming_language)) ·
> [Wikipedia: Zero trust security model](https://en.wikipedia.org/wiki/Zero_trust_security_model) ·
> [Felleisen 1989 · On the Expressive Power of Programming Languages]
...
| 边界场景 | 详细分析位置 |
|:---|:---|
| 所有权（Ownership）泄漏边界 | [`../01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) §7.1 反命题与边界分析 |
| 借用（Borrowing）规则突破 | [`../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md`](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) §7.1 反命题与边界分析 |
| 生命周期（Lifetimes）陷阱 | `../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md` §7.1 反命题与边界分析 |
| 类型系统（Type System）绕过 | [`../01_foundation/02_type_system/04_type_system.md`](../../01_foundation/02_type_system/01_type_system.md) §7.1 反命题与边界分析 |
| Rc/RefCell 循环 | [`../02_intermediate/02_memory_management/03_memory_management.md`](../../02_intermediate/02_memory_management/01_memory_management.md) §7.1 反命题与边界分析 |
| 并发死锁 | [`../03_advanced/00_concurrency/01_concurrency.md`](../../03_advanced/00_concurrency/01_concurrency.md) §7.1 反命题与边界分析 |
| Pin 不动性突破 | [`../03_advanced/01_async/02_async.md`](../../03_advanced/01_async/01_async.md) §7.1 反命题与边界分析 |
| unsafe 契约失效 | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) §7.1 反命题与边界分析 |
| async 语义边界（await/取消/Pin/Send/executor/dyn） | [Async 边界全景](../../03_advanced/01_async/06_async_boundary_panorama.md) — 本全景在 async 域的纵深 |
| unsafe 语义边界（UB/别名模型/Miri 可检测性/契约/FFI） | [Unsafe 边界全景](../../03_advanced/02_unsafe/02_unsafe_boundary_panorama.md) — 本全景在 unsafe 域的纵深 |
| RustBelt 证明边界 | [`../04_formal/04_rustbelt.md`](../../04_formal/02_separation_logic/01_rustbelt.md) §7.1 反命题与边界分析 |
...
| **论断** | **来源** | **可信度** |
|:---|:---|:---|
| Safe Rust 无 UAF/double-free | [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html) · [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/) | ✅ |
| Rc 循环导致泄漏 | [TRPL Ch15](https://doc.rust-lang.org/book/ch15-00-smart-pointers.html) | ✅ |
| mem::forget 阻止 Drop | [std docs] | ✅ |
| UnsafeCell 允许共享可变 | [std docs] · [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html) | ✅ |
| Miri 检测 UB | [Miri 文档] | ✅ |
| RustBelt 不覆盖 unsafe | [Jung et al. POPL 2017] | ✅ |
| Stacked/Tree Borrows 争议 | [Rust 内部讨论] · [Jung et al. POPL 2019] | ⚠️ |
| 内存安全定义 | [Wikipedia: Memory safety](https://en.wikipedia.org/wiki/Memory_safety) | ✅ |
| 未定义行为定义 | [Wikipedia: Undefined behavior](https://en.wikipedia.org/wiki/Undefined_behavior) | ✅ |
| 类型安全定义 | [Wikipedia: Type safety](https://en.wikipedia.org/wiki/Type_safety) | ✅ |
| 数据竞争定义 | [Wikipedia: Race condition](https://en.wikipedia.org/wiki/Race_condition) | ✅ |
| 死锁定义 | [Wikipedia: Deadlock](https://en.wikipedia.org/wiki/Deadlock) | ✅ |
...
Rust 的安全边界在编译期和运行时（Runtime）有不同的表现形式：
...
| **E0505** | cannot move out of `x` because it is borrowed | 所有权（Ownership）与借用互斥 | move vs borrow | `let r = &x; let y = x;` |
...
| **E0597** | `x` does not live long enough | 引用生命周期（Lifetimes）不足 | 'static / scoped | `let s: &'static str = &String::new();` |
...
**运行时（Runtime）错误（panic / abort）**：

| 错误信息 | 触发条件 | 安全边界 | 缓解策略 |
|:---|:---|:---|:---|
| `panic: index out of bounds` | `v[i]` 且 `i >= v.len()` | 内存安全（Memory Safety）边界 | 使用 `get()`、断言检查 |
...
| `panic: already borrowed: BorrowMutError` | `RefCell` 运行时（Runtime）借用冲突 | 动态借用安全 | 重构为编译期借用 |
...
E0502 (借用冲突)
  └─→ 边界: 内存安全 · 借用唯一性
      ...
      └─→ 文件: ../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md §3
      └─→ 修复: 缩小借用范围 / 使用内部可变性

E0499 (多次可变借用)
  └─→ 边界: 内存安全 · 可变借用唯一性
      └─→ 概念: L1 所有权 → L2 &mut T 语义 → L4 独占权限
      └─→ 文件: ../01_foundation/01_ownership_borrow_lifetime/01_ownership.md §3.1
      └─→ 修复: 重构为单一 &mut / 使用 split_mut

E0373 (闭包捕获逃逸)
  └─→ 边界: 生命周期安全 · 'static 约束
      └─→ 概念: L1 生命周期 → L2 HRTB → L4 全称量词 ∀
      └─→ 文件: ../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md §6
...
|:---|:---|:---|:---|:---|
| **ABI 不匹配** | C `int` vs Rust `c_int`、结构体（Struct） padding 差异 | 编译警告、`bindgen` | 使用 `std::ffi::*` 类型、`#[repr(C)]` | Rustonomicon |
| **生命周期（Lifetimes）不匹配** | C 指针无生命周期，Rust 引用需 'static 或 scoped | Miri、代码审查 | 使用 `PhantomData<&'a T>` 标记生命周期 | Rustonomicon |
...
| **未初始化内存** | C 可能返回部分初始化的结构体（Struct） | Valgrind、Miri | 使用 `MaybeUninit<T>`、显式初始化 | Unsafe Guidelines |
| **回调生命周期（Lifetimes）** | C 保存 Rust 回调指针，超出 Rust 对象生命周期 | 代码审查 | 使用 `Arc` + `weak` 模式、注册/注销配对 | Rustonomicon |
...
> [RFC 2945: C-unwind ABI](https://github.com/rust-lang/rfcs/pull/2945) ·
> [Unsafe Code Guidelines: FFI] ·
...
- [x] **高**: 补充每个边界条件的具体编译错误码和运行时（Runtime）错误信息 —— 已完成 §11.1
...
| **safe Rust 保证内存安全（Memory Safety）** | 所有权（Ownership）+借用+生命周期（Lifetimes） ⟹ | 无UAF/无数据竞争 | 逻辑错误未覆盖 | 默认编程模式 |
...
> **认知功能**:
> 破除"unsafe = 无法无天"的常见误解，展示 unsafe 内部仍受的部分约束。
> 在团队内推广 unsafe 最佳实践时引用此图，强调最小化范围原则。
> **关键洞察**: unsafe 块内类型检查和借用检查仍部分生效，Miri 也能检测子集 UB——并非真空地带。
> [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**:
> 校准对 Safe Rust 安全边界的期望——内存安全（Memory Safety）不等于逻辑正确。
> 在架构讨论中使用此图，区分"编译器保证消除的 bug 类别"与"仍需测试覆盖的类别"。
> **关键洞察**: Safe Rust 消除了 UB 类 bug，但活性（死锁/终止）和功能性错误仍需额外验证。
> [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**: 澄清形式化证明在 unsafe 代码验证中的实际地位，避免过度工程或盲目信任。根据 unsafe 代码的复杂度和风险等级，在审计、Miri 和形式化证明之间选择验证手段。**关键洞察**: 形式化证明成本高且规格编写本身困难——工程审计 + Miri 子集检测是更普遍的实践路径。[💡 原创分析](../../00_meta/00_framework/methodology.md)
> **过渡: L5 → L3**
> 安全边界不是抽象概念——`unsafe` 块的每一次原始指针（Raw Pointer）解引用、每一个 `unsafe impl Send` 都是边界的具体体现。理解 "边界在哪里" 需要同时掌握 safe Rust 的编译期保证和 unsafe Rust 的运行时（Runtime）风险。
> unsafe 的具体逃逸门见 [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md)。
> **过渡: L5 → L4**
> 形式化验证能将 "我相信这是安全的" 转化为 "机器证明了这是安全的"，但形式化工具有自己的边界——它们无法验证未规格化的需求、无法处理无限状态空间、无法覆盖所有 unsafe 模式。安全边界的形式化分析是理解 "什么能被证明" 的关键。
> 验证工具的边界见 [`../04_formal/04_rustbelt.md`](../../04_formal/02_separation_logic/01_rustbelt.md)（RustBelt 能力边界）。
...
| **B5** | 边界：死锁 / 活性 | 所有上述层级成立 | 内存安全（Memory Safety）保证有效 | 任何 safe Rust 代码 | 逻辑正确但无法终止 |
...
> 依赖安全实践见 [`../06_ecosystem/01_toolchain.md`](../../06_ecosystem/00_toolchain/01_toolchain.md)（Cargo 安全审计）。
> **来源:
> [Rust Reference](https://doc.rust-lang.org/reference/introduction.html);
> [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html);
> [Rust RFCs](https://github.com/rust-lang/rfcs); Academic Papers** 本文件内容基于官方文档、学术研究和工业实践的综合分析。✅
> **来源:
> [Wikipedia](https://en.wikipedia.org/wiki/Main_Page);
> POPL/PLDI/ECOOP Papers;
> [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/)/Iris Project** 形式化概念参考了权威学术来源和类型论研究。✅
---
...
> **相关文件**: [失效分析树集](../../00_meta/00_framework/fault_tree_analysis_collection.md) · [边界扩展树](../../00_meta/00_framework/boundary_extension_tree.md) · [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md)

## 十、边界测试：安全边界的编译错误

安全边界测试按"穿越方向"分类：**safe→unsafe**（入口审查）、**unsafe→safe**（出口不变式恢复）、**FFI 边界**（跨语言契约）。每个用例标注失败类型：编译错误（借用检查器拒绝）、运行时 UB（Miri 可检测）、或逻辑错误（编译通过但违反契约）。判定依据如下表：

| 边界类型 | 关键检查点 | 验证工具 |
|:---|:---|:---|
| safe/unsafe 穿越 | `unsafe` 块内操作是否超出封装承诺 | rustc + 代码审计 |
| `unsafe impl` | 实现是否满足 trait 的安全契约 | 文档 + Miri |
| FFI 边界 | 所有权跨边界转移、空指针、布局一致性（Coherence） | `#[repr(C)]` + sanitizer |
| unsafe 粒度 | 块范围是否最小、是否泄漏到 safe API | clippy + 审计清单 |

判定原则：一个封装是否合格，取决于 safe API 的调用者**是否需要**了解 unsafe 内部的不变式；需要了解即封装失败。
...
> **修正**:
> Rust 的安全边界由 `unsafe` 关键字标记。
> Safe Rust 保证无 UB（无数据竞争、无 use-after-free、无悬垂引用）。
> Unsafe Rust 放弃这些保证，允许：解引用裸指针、调用 unsafe 函数、访问 union 字段、内联汇编（Inline Assembly）。
> `unsafe` 块不自动使周围代码 unsafe——它只是告诉编译器"程序员已验证此处的安全性"。
> 安全抽象的目标是将 unsafe 代码封装为 safe API，使调用者无需了解内部 unsafe 细节。
...
> **修正**:
> `unsafe impl Send` / `unsafe impl Sync` 告诉编译器"我保证此类型可安全跨线程"，但编译器**不验证**此保证。
> 错误标记会导致数据竞争、use-after-free 等严重 UB。
> 正确的做法是通过 `Mutex<T>`、`RwLock<T>`、原子类型等同步原语实现线程安全，而非手动标记。
> `unsafe impl` 仅在封装底层同步机制时使用（如标准库的 `Mutex` 本身需要 `unsafe impl Sync`）。
> [来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)]
...
> **修正**:
> FFI（Foreign Function Interface）边界是 Rust 安全保证的**终点**：Rust 编译器无法验证 C 代码的内存安全（Memory Safety）。
> `unsafe` 块标记"开发者保证此调用安全"，但错误的保证导致 UB。
> 安全封装模式：
>
> 1) 在 Rust 侧验证所有参数（长度、指针非空、对齐）；
> 2) 使用 `slice::from_raw_parts` 的边界检查版本；
> 3) 使用 `cxx` crate 生成类型安全的 C++ 绑定。这与 Java 的 JNI（同样无法验证 C 代码，但 JVM 有沙箱限制）或 Python 的 `ctypes`/`cffi`（同样依赖开发者正确性）相同——跨语言边界是类型系统（Type System）的天然弱点。
> Rust 的优势在于 `unsafe` 块的显式标记：所有 FFI 调用都在审计范围内，不像 C++ 的隐式 unsafe（每行代码都可能是 UB）。
> [来源: [The Rust FFI Omnibus](https://jakegoulding.com/rust-ffi-omnibus/)] ·
> [来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)]
...
> **修正**:
> `unsafe` 块的粒度是代码质量和安全审计的关键因素。
> 最佳实践：
>
> 1) **最小化 unsafe**：只将真正需要 unsafe 的操作放入 `unsafe` 块；
> 2) **封装 unsafe**：将 unsafe 操作封装在安全函数中，文档化不变式；
> 3) **审计边界**：每个 `unsafe` 块都应有人工审查记录。
> 上述代码中，`println!` 不需要 `unsafe`，放入 `unsafe` 块增加了审计负担。
> Rust 社区的工具（`cargo geiger` 统计 unsafe 比例、`cargo vet` 审计依赖）帮助管理 unsafe 边界。
> 这与 C/C++ 的"全代码 unsafe"（无边界标记）或 Java 的"无 unsafe"（JVM 保证安全，JNI 除外）不同——Rust 的 unsafe 是 opt-in 的显式契约，使安全审计可聚焦。
> [来源:
...
> **修正**:
> Rust 的**安全边界**（safety boundary）是 `unsafe` 块与 safe 代码的分界。
> safe 函数**不能**假设输入满足 unsafe 契约——必须验证或要求调用者通过类型系统（Type System）保证。`safe_wrapper` 的 design flaw：接收裸指针（`unsafe` 类型的值）但在 safe 函数签名中，未验证就解引用。
> 正确设计：
>
> 1) `fn safe_wrapper(ptr: &mut i32)` — safe 引用（Reference），编译器保证有效；
> 2) `unsafe fn raw_wrapper(ptr: *mut i32)` — unsafe 函数，调用者负责验证；
> 3) `fn safe_wrapper(ptr: NonNull<i32>)` — 类型系统保证非空。
> 安全边界的文档化：`SAFETY:` 注释说明 unsafe 块的先决条件。这与 C 的"所有函数都隐式 unsafe"（无边界概念）或 Java 的 JNI（边界在 JVM/native 接口，但 native 代码无编译期检查）不同——Rust 显式标记 unsafe 边界，文档化契约。
> [来源: [The Rustonomicon](https://doc.rust-lang.org/nomicon/safe-unsafe-meaning.html)] · [来源: [Rust Reference — Unsafe Blocks](https://doc.rust-lang.org/reference/unsafe-blocks.html)]
...
1) 原始指针（Raw Pointer）是否有效？2) 是否有数据竞争？3) 类型转换是否正确？4) 生命周期是否足够？5) 是否违反任何安全 invariant？
