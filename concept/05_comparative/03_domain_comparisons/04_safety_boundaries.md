...
> **认知功能**: 建立 Rust 安全保证的三层分类框架（Safe/边界/Unsafe），展示编译期保证与突破方式之间的完整映射关系。在安全架构评审时对照此图检查每个模块（Module）所处的层级。**关键洞察**: Safe Rust 的六大保证并非全部被 Unsafe 打破——部分可通过 Safe 边界条件合法削弱。[来源: 💡 原创分析]
...
> **认知功能**: 提供代码安全性的完整判定流程图，展示从编译检查到 Miri 动态验证的安全决策链。遇到安全相关问题时按图索骥，判断代码所处阶段和可选验证手段。**关键洞察**: 安全不是二元的——即使进入 unsafe，完整契约 + Miri 验证仍可趋近安全。[来源: 💡 原创分析]
...
> **认知功能**: 将 FFI 安全假设拆解为可逐项验证的前提链，暴露每个环节的失效模式。编写 FFI 绑定前逐条核对 C 端的别名规则、线程安全性和内存布局。**关键洞察**: FFI 安全不是"Rust 端正确"就够的——C 端的行为假设必须通过外部审计确认。[来源: 💡 原创分析]
...
> **认知功能**: 将 Rust 的安全边界放在更广泛的零信任谱系中定位，明确其覆盖范围与盲区。评估系统安全需求时，确认 Rust 的编译期保证是否足以覆盖目标威胁模型。**关键洞察**: Rust 的信任止于进程地址空间——硬件位翻转、OS 崩溃、分布式拜占庭故障均在其外。[来源: 💡 原创分析]
...
| UnsafeCell 允许共享可变 | [std docs] · [Rustonomicon](https://doc.rust-lang.org/nomicon/) | ✅ |
| Miri 检测 UB | [Miri 文档] | ✅ |
...
> **来源**: [Rust Reference: Memory Model](https://doc.rust-lang.org/reference/) · [LLVM Memory Model] · [Herlihy & Shavit: The Art of Multiprocessor Programming] · [Wikipedia: Memory ordering](https://en.wikipedia.org/wiki/Memory_ordering)
...
> **认知功能**: 破除"unsafe = 无法无天"的常见误解，展示 unsafe 内部仍受的部分约束。在团队内推广 unsafe 最佳实践时引用此图，强调最小化范围原则。**关键洞察**: unsafe 块内类型检查和借用检查仍部分生效，Miri 也能检测子集 UB——并非真空地带。[来源: 💡 原创分析]
...
> **认知功能**: 校准对 Safe Rust 安全边界的期望——内存安全（Memory Safety）不等于逻辑正确。在架构讨论中使用此图，区分"编译器保证消除的 bug 类别"与"仍需测试覆盖的类别"。**关键洞察**: Safe Rust 消除了 UB 类 bug，但活性（死锁/终止）和功能性错误仍需额外验证。[来源: 💡 原创分析]
...
> **修正**: Rust 的安全边界由 `unsafe` 关键字标记。Safe Rust 保证无 UB（无数据竞争、无 use-after-free、无悬垂引用）。Unsafe Rust 放弃这些保证，允许：解引用裸指针、调用 unsafe 函数、访问 union 字段、内联汇编（Inline Assembly）。`unsafe` 块不自动使周围代码 unsafe——它只是告诉编译器"程序员已验证此处的安全性"。安全抽象的目标是将 unsafe 代码封装为 safe API，使调用者无需了解内部 unsafe 细节。[来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/)]
