...
- **不信任程序员**：编译器通过借用检查器、类型系统（Type System）、穷尽检查，主动拒绝可能不安全的代码。
...
| 边界场景 | 详细分析位置 |
|:---|:---|
| 所有权泄漏边界 | [`../01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) §7.1 反命题与边界分析 |
...
| **safe Rust 保证内存安全（Memory Safety）** | 所有权+借用+生命周期 ⟹ | 无UAF/无数据竞争 | 逻辑错误未覆盖 | 默认编程模式 |
