...
>
> **前置概念**: [所有权（Ownership）形式化](../../04_formal/01_ownership_logic/02_ownership_formal.md)
> [Rust vs C++](../01_systems_languages/01_rust_vs_cpp.md) ·
> [Rust vs Go](../01_systems_languages/03_rust_vs_go.md)
...
**知识点**：Rust 的所有权（Ownership）系统在编译期消除了整类 C 语言内存错误。返回局部变量引用（Reference）是最常见的 C bug 之一，在 Rust 中完全不可能。→ Rust vs C++ 详解
...
**核心差异**：Go 的 `nil` 是**十亿美元错误**（C.A.R. Hoare）的现代延续。Rust 通过 `Option<T>` 将"可能缺失"显式编码到类型系统（Type System）中。[→ Rust vs Go 详解](../01_systems_languages/03_rust_vs_go.md)
...
1. **所有权（Ownership）追踪**：`tx` 的生命周期（Lifetimes）与任务绑定，若 `tx` 被 drop 而 `rx` 仍在等待，`recv()` 返回 `None`
...
**知识点**：Rust 的所有权（Ownership）系统为并发原语提供了编译期保证，而 Go 依赖运行时检测和程序员纪律。[→ Rust vs Go 详解](../01_systems_languages/03_rust_vs_go.md)
...
**知识点**：Rust 的零成本抽象（Zero-Cost Abstraction）意味着你可以使用高阶概念（迭代器（Iterator）、闭包（Closures）、泛型（Generics））而不牺牲性能——编译器生成的代码与手写底层代码一样高效。→ 零成本抽象详解
...
**知识点**：Rust 在安全性上接近 Go（自动内存管理、边界检查），在性能上接近 C++（零成本抽象（Zero-Cost Abstraction）、无 GC）。这是 Rust 独特定位的来源。[→ Rust vs C++ 详解](../01_systems_languages/01_rust_vs_cpp.md)
...
**知识点**：Rust 正在从"系统编程语言"向"所有层次编程语言"扩展，从嵌入式到 Web 都有成熟的生态。[→ Rust for Linux 详解](../../07_future/04_research_and_experimental/04_rust_for_linux.md)
...
|:---|:---|:---|
| 循环引用（Reference） | 导致内存泄漏 | 同样泄漏（需 `Weak` 解决） |
...
**知识点**：Rust 的智能指针（Smart Pointer）设计在功能上接近 C++，但通过类型系统（Type System）（`Send`/`Sync`）在编译期排除了整类并发错误。[→ 内存管理详解](../../02_intermediate/02_memory_management/01_memory_management.md)
...
**知识点**：Rust 的错误处理（Error Handling）结合了 Go 的显式和 C++ 的简洁，通过类型系统（Type System）和 `?` 运算符实现了"显式但不冗长"的理想平衡。[→ 错误处理详解](../../01_foundation/08_error_handling/01_error_handling_basics.md)
...
> [Rust vs C++](../01_systems_languages/01_rust_vs_cpp.md)、
> [Rust vs Go](../01_systems_languages/03_rust_vs_go.md) 权威页一致；
> 干扰项针对常见误解设计。
...
**解析**：A 是最常见误解：GC 只保证"对象不会被提前释放"，**不**阻止两个 goroutine 同时写同一变量（Go 中这是数据竞争，行为未定义）。Go 的 `-race` 检测器是动态的——只发现实际执行到的竞争；Rust 把检查前移到编译期，未被执行的代码路径同样受约束。

</details>

---

### Q12. 🟡【单选】Rust 泛型与 C++ 模板在约束机制上的关键差异是？

- A. Rust 泛型用 trait bound 在**定义处**检查约束，错误早且清晰；C++ 模板（C++20 前）是鸭式约束，实例化时才报错
- B. C++ 模板有约束机制而 Rust 泛型没有
- C. 两者都采用动态分发
- D. Rust 泛型有运行时类型检查开销
...
### Q13. 🔴【多选】关于"零成本抽象"，下列说法正确的有？（选出所有正确项）
...
**解析**：零成本的两条准则来自 C++ 并被 Rust 继承：无全局税（A）、无局部损（B），迭代器融合、单态化泛型、RAII 都是实例（D）。C 错："零成本"严格限定在**运行时**——编译时间、二进制体积、学习曲线都是真实成本；把"零运行时成本"偷换成"零一切成本"是宣传式误读。

</details>

---

### Q14. 🟢【判断】C 的手动内存管理（malloc/free）把内存安全责任完全交给程序员，use-after-free、double free 是常见漏洞来源；Rust 用所有权在编译期消除了这类错误。（对 / 错）
...
**解析**：经验数据支撑这一判断：微软/Chromium 均报告约 70% 的严重安全漏洞源于内存安全。Rust 的所有权规则使"每个值恰好释放一次、释放后不可访问"成为编译期定理而非纪律——这类 CWE 在安全 Rust 子集中结构上不可能出现（unsafe 除外，这也是 unsafe 需要额外审计的原因）。

</details>

---

### Q15. 🟡【判断】goroutine 泄漏（永久阻塞、无法退出的 goroutine）是 GC 语言特有的问题；Rust 的所有权保证任务/资源永远不会泄漏。（对 / 错）
...
| 4–6/10 | 🔄 需巩固基础 | 重读 [Rust vs C++](../01_systems_languages/01_rust_vs_cpp.md) · [Rust vs Go](../01_systems_languages/03_rust_vs_go.md) |
...
**题目**: 本文件是 测验：Rust vs 系统编程语言（L5 试点扩展） 的专项测验集。这类测验文件的主要作用是什么？
...
### 测验 2：🟢 在 测验：Rust vs 系统编程语言（L5 试点扩展） 的测验中，若遇到不确定答案的题目，最佳的学习策略是什么？（理解层）

**题目**: 在 测验：Rust vs 系统编程语言（L5 试点扩展） 的测验中，若遇到不确定答案的题目，最佳的学习策略是什么？
