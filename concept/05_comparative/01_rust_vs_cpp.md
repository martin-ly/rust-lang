...
- **编译器角色**：不是翻译工具，而是**证明检查器**（Proof Checker）。你的代码通过编译，意味着编译器已验证你的程序满足所有形式规则（无数据竞争、无悬垂指针、无 use-after-free）。[来源: [RustBelt Paper, POPL 2018](https://people.mpi-sws.org/~dreyer/papers/rustbelt/paper.pdf)]
