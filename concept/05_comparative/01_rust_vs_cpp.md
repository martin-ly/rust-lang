...
- **核心公理**：所有权（Ownership）不是"内存管理技巧"，而是**资源使用的逻辑公理**——每个值有且仅有一个所有者，转移即消耗，借用即临时授权。
...
| **并发安全** | 类型系统禁止数据竞争（Send/Sync trait 作为逻辑标记） | 程序员使用 mutex/atomic 手动同步 | 逻辑不可能性 vs 机制协调 |
...
| "unsafe Rust等价于C++" ⟹ | 两者都可引入UB | 但比例/文化/工具链差异巨大 | Rust `unsafe` 仅占 ~0.5%；C++ 默认全局unsafe | FFI绑定、底层内存操作 |
