...
| **并发安全（Concurrency Safety）** | 类型系统（Type System）禁止数据竞争（Send/Sync trait 作为逻辑标记：`Send` = 可安全跨线程转移所有权（Ownership），`Sync` = 可安全跨线程共享引用） | 程序员使用 mutex/atomic 手动同步 | 逻辑不可能性 vs 机制协调 |
...
| "零成本抽象（Zero-Cost Abstraction）无运行时开销" ⟹ | 单态化（monomorphization） | 生成等价于手写的机器码 | 编译时间显著增加；二进制体积膨胀（代码重复） | 泛型（Generics）算法、迭代器（Iterator）链 |
...
| :--- | :--- | :--- |
| **词法/语法** | 常规解析 | 常规解析 |
| **语义分析** | **借用检查器**执行形式证明：生命周期（Lifetimes）、所有权、Send/Sync | **类型检查**确认名称合法，不验证内存安全（Memory Safety） |
...
| **Rust 业务层** | 功能正确性 + 并发安全 | Verus/Creusot（关键模块）+ Kani（并发路径）+ Rust 编译器 | 业务不变式、无数据竞争、异步（Async）状态机转换 |
