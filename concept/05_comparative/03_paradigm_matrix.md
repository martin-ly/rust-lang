...
|:---|:---|:---|:---|:---|:---|:---|
| **内存安全** | ❌ 手动 | ⚠️ 智能指针（Smart Pointer） | ✅ 所有权 | ✅ GC | ✅ GC | ✅ GC/纯函数 |
| **并发安全（Concurrency Safety）** | ❌ 无 | ⚠️ 库支持 | ✅ 类型级 | ⚠️ 通道约定 | ⚠️ 同步原语 | ✅ 不可变性 |
| **零成本抽象** | ✅ | ✅ | ✅ | ❌ 接口有开销 | ❌ 泛型（Generics）擦除 | ❌ 惰性求值开销 |
| **形式化基础** | ❌ | ❌ | ✅ 线性逻辑 | ❌ | ❌ | ✅ 范畴论 |
...
> **修正**: Rust 没有 `class` 关键字，封装通过模块（Module）（`mod`）和可见性修饰符（`pub`）实现。结构体（Struct）字段默认私有，方法默认私有。这与 Java/C++ 的"类即封装边界"不同——Rust 的封装边界是模块，一个模块内的多个类型可互相访问私有字段。trait 只定义行为接口，不包含字段。这种设计鼓励"组合优于继承"，通过 trait 实现多态，通过模块实现封装。[来源: [The Rust Programming Language](LINK_PLACEHOLDER)]
...
命令式（变量、循环、赋值）、函数式（闭包、迭代器（Iterator）、模式匹配（Pattern Matching））、面向对象（封装、多态通过 trait，无继承）、并发（async/await、线程、消息传递）。
</details>
