...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) · [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) · [Operational Semantics](../../04_formal/03_operational_semantics/03_operational_semantics.md)
...
Rust 与 Go 都诞生于 2009–2015 年间的系统编程复兴浪潮，但两者的权威定义指向不同的设计目标：Go 官方定位为“让软件开发规模化的简单语言”，以 GC、goroutine 与快速编译换取极简心智模型；Rust 官方定位是“让每个人都能构建可靠且高效软件的语言”，以所有权（Ownership）与借用（Borrowing）检查在编译期消除内存与数据竞争错误。

本节定义部分采用三类权威来源交叉印证：

- **语言官方文档**：The Rust Book、Effective Go、Go FAQ，给出各自的设计哲学表述；
- **百科与课程**：Wikipedia 条目与 MIT/Stanford 系统课程大纲，用于校准术语与教学定位；
- **同行评审文献**：RustBelt（POPL 2018）等论文给出所有权模型的形式化表述，Go 侧则以 Google 内部工程报告与 GopherCon 资料为补充。

阅读时建议先对照 1.1 的设计哲学表建立坐标系，再用 1.2 的核心命题表逐条验证——凡是表中 Rust 占优的维度，都应能追溯到“编译期检查替代运行时（Runtime）机制”这一根本差异。
...
技术选型讨论中最常见的谬误，是把“Rust 在内存安全（Memory Safety）上严格优于 Go”外推为“Rust 在所有场景优于 Go”。本节采用反命题驱动的方法：先陈述社区中真实流传的绝对化命题，再用可复现的反例（延迟分布、迭代速度、团队规模约束）逐条检验其边界。

判定一条命题是否成立，需要同时回答三个问题：

1. **workload 特征**：请求是 CPU 密集还是 IO 密集？p99 延迟预算是否小于 GC 停顿量级（通常 100µs–数 ms）？
2. **团队与交付约束**：成员能否承受所有权模型的学习曲线？交付节奏是否以周为单位快速迭代？
3. **抽象成本敏感性**：接口调用的动态派发开销在热路径上是否可测量？

下面的反命题树与两组反例即按这三问组织；结论不是“谁更好”，而是给出每个命题失效的具体条件。
...
对比代码示例遵循“同一问题、两种实现”的原则：两侧实现同一个并发消息传递任务，刻意保持业务逻辑一致，使差异只来自语言机制本身。选取 channel 作为载体，是因为它最能暴露两种并发哲学的分歧：

- Rust 的 `std::sync::mpsc` channel 在 `send` 时**转移所有权**，发送后原变量失效，编译期保证同一份数据不会两边并发访问；
- Go 的 channel 传递的是**值拷贝或指针**，指针共享由 GC 兜底，数据竞争需靠 `go vet -race` 在运行期发现。

阅读 7.1 与 7.2 时重点观察三处：发送端变量的后续可用性、接收端是否需要额外同步、以及错误路径（channel 关闭）的处理方式。7.4 的混合架构建议则说明真实系统中两者常按服务边界共存，而非二选一。
...
- [对应测验](../03_domain_comparisons/02_quiz_rust_vs_systems.md) — Rust vs 系统编程语言（C/C++/Go 内存安全、并发、错误处理（Error Handling）对比）
| 概念 | 文件 | 关系 |
| :--- | :--- | :--- |
| 并发 | [`../03_advanced/00_concurrency/01_concurrency.md`](../../03_advanced/00_concurrency/01_concurrency.md) | Rust 并发模型 |
| 内存管理 | [`../02_intermediate/02_memory_management/03_memory_management.md`](../../02_intermediate/02_memory_management/01_memory_management.md) | Rust 所有权 vs Go GC |
| 错误处理（Error Handling） | [`../02_intermediate/03_error_handling/04_error_handling.md`](../../02_intermediate/03_error_handling/01_error_handling.md) | Result vs 多返回值 |
...
> **相关文件**: [Rust vs C++](01_rust_vs_cpp.md) · [范式矩阵](../00_paradigms/01_paradigm_matrix.md) · [问题图谱](../../00_meta/04_navigation/10_problem_graph.md#三并发系统设计问题树)

## 十、边界测试：Rust 与 Go 的编译错误对比

边界测试的目的不是展示“Go 会出错而 Rust 不会”，而是定位两类语言把错误暴露在哪个阶段：Go 将接口满足性、nil 解引用（Reference）等问题推迟到运行期（panic 或 race detector 报告），Rust 则在编译期以 trait bound 不满足、`Option` 未处理等形式拒绝程序通过。

每组测试按固定格式组织：

1. **Go 侧最小复现**：能通过编译、运行期才暴露缺陷的代码；
2. **Rust 侧等价写法**：触发编译错误的最小代码及完整错误输出（rustc 1.97，edition 2024）；
3. **机理分析**：错误信息对应的类型系统（Type System）规则，以及 Go 侧若要获得同等保证需要付出的工程代价（代码评审、lint、测试覆盖率）。

阅读时可对照第 10.1–10.3 的隐式接口与 nil 两组核心差异：它们分别对应“结构化约束的显式化”与“空值在类型系统中的可表达性”这两个根本分歧。
...
本节从测验 1：Go 和 Rust 在设计哲学上有什么根本差异？（理解层）、测验 2：Go 的 goroutine 与 Rust 的 `tokio…、测验 3：Go 的错误处理（`if err != nil`）与 Rus…、测验 4：为什么 Go 没有泛型（Generics）（直到 1.18），而 Rust 从设…等5个方面切入，剖析「嵌入式测验（Embedded Quiz）」的核心内容。
