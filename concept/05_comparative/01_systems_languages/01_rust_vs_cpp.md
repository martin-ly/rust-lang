...
| 循环引用的安全释放 | `std::shared_ptr` + 自定义删除器 | Rust 的 `Rc<RefCell<T>>` 在运行时（Runtime）检查，打破了纯静态证明 |
...
- **C1：AI + Rust 形式化的具体工作流**——AI 生成初稿 → `cargo check` 拒绝内存/并发错误（零成本过滤）→ 开发者审查逻辑正确性 → Kani/Prusti 对关键模块生成验证条件。Rust 在此工作流中的角色是"把 AI 的不可靠性约束在逻辑层"：AI 最常犯的内存错误（释放后使用、越界、数据竞争）被编译器完全拦截，人类审查只需聚焦业务逻辑。
...
2. **迭代器失效**：C++ `vector` push_back 后旧迭代器 UB，Rust 借用检查 E0502 拒绝「迭代中修改」——错误信息的教学质量对比；
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：Rust 和 C++ 在内存安全保证上最根本的区别是什么？（…、测验 2：C++ 的模板元编程与 Rust 的宏系统有什么主要区别？（…、测验 3：为什么 Rust 没有 C++ 的"多重继承"？（理解层）、测验 4：在系统编程中，什么时候应该选择 Rust 而不是 C++？（…等5个方面，本节依次展开。
...
本节以 C++ 中最隐蔽的悬垂引用写法为反例，展示 Rust 生命周期检查如何在编译期拦截同类错误。
