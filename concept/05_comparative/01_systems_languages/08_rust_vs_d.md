...
本节建立 Rust 与 D 的对比坐标：先定位两种系统语言的设计起点，再提出核心命题，最后用认知路径图展示后续章节的展开顺序。
...
本节通过综合对比矩阵，从内存管理、类型系统、安全子集、并发与元编程等维度快速刻画 Rust 与 D 的差异。
...
本节对比 Rust 的所有权驱动无 GC 模型与 D 的默认 GC 加可选控制策略，并聚焦借用检查与 DIP 1000 的边界。
...
本节对比 Rust 的 trait 驱动结构化多态与 D 的模板、接口、混入组合多态，揭示两种抽象机制的设计取向。
...
本节对比 Rust 的 safe/unsafe 二元边界与 D 的 @safe/@trusted/@system 三级模型，说明默认假设与逃逸路径的差异。
...
本节对比 Rust 的 Send/Sync 类型级并发安全与 D 的消息传递/共享不可变模型，分析数据竞争保证的来源。
...
本节对比 Rust 的声明宏/过程宏/const 泛型与 D 的 CTFE/模板/mixin，讨论卫生性、灵活性与编译期能力的取舍。
...
本节从包管理、工具链与典型场景出发，区分 Rust 的基础设施生态与 D 的快速系统原型/游戏工具链定位。
...
本节主动检验两种语言互换的误区，分别列出 Rust 不适合直接替代 D 的场景及其反例。
...
本节汇总 Rust 与 D 的权威外部来源、项目内部延伸阅读与对比阅读，为深入理解两种语言提供可追溯入口。
