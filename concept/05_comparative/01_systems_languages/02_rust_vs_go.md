...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) · [Memory Management](../../02_intermediate/02_memory_management/03_memory_management.md) · [Operational Semantics](../../04_formal/03_operational_semantics/17_operational_semantics.md)
> **后置概念**: [Paradigm Matrix](../00_paradigms/03_paradigm_matrix.md)
...
|:---|:---|:---|
| **机制** | 所有权 + RAII | 垃圾回收（GC） |
| **运行时（Runtime）开销** | 零（除 Drop） | GC 停顿（通常 <1ms） |
...
| **泛型（Generics）实例化** | 单态化（Monomorphization）（零运行时（Runtime）成本） | 接口擦除（box 化或反射） |
...
|:---|:---|:---|
| **实现机制** | 静态派发（单态化）+ 动态派发（trait object） | 运行时（Runtime）结构体（Struct） itab + 动态派发 |
| **调用开销** | 静态：零开销内联；动态：一次虚表查找 | 两次间接跳转（接口值 → itab → 方法） |
| **跨包实现** | 孤儿规则（Orphan Rule）限制（coherence） | 任意包可为任意类型实现接口 |
| **反射开销** | 编译期擦除，运行时（Runtime）需 TypeId | 运行时保留完整类型信息 |
...
> **认知功能**: 建立 Rust vs Go 的全景认知框架，将复杂对比简化为四个可记忆维度。在学习初期用作"导航图"，遇到具体问题时快速定位对比维度。关键洞察：两种语言并非简单的优劣之分，而是在"安全-效率"与"简单-性能"光谱上的不同权衡点。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**: 通过时序对比可视化 Rust 所有权转移与 Go 值拷贝的并发通信差异。对照阅读时关注"编译期验证 vs 运行时调度"的本质区别。关键洞察：Rust channel 传递所有权并由编译器验证 Send/Sync，Go channel 传递值拷贝且安全依赖程序员纪律。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**: 通过反例分析破除"Rust 适合所有后端"的绝对化认知，培养批判性思维。建议在每个分支节点先自行判断再查看结论，训练辩证分析能力。关键洞察：Go 在快速迭代场景的反例与 Rust 在低延迟场景的优势形成互补，而非对立。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **关键差异**: Rust 的 `?` 使错误传播成为"默认路径"，Go 需要显式 `if err != nil`。Go 1.18+ 的泛型（Generics）未改变错误处理（Error Handling）范式。 来源: [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html) / Effective Go
...
> **认知功能**: 揭示 Rust 与 Go 错误处理（Error Handling）各自的优势边界，避免非黑即白的判断。建议结合 §8.1 对比矩阵阅读，关注 Go 的堆栈追踪优势与 Rust 的强制安全边界。关键洞察：当错误需要丰富上下文或代码呈线性脚本风格时，Go 的显式处理反而更具工程价值。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**: 纠偏"Rust 性能全面碾压 Go"的刻板印象，建立场景化性能认知。建议在阅读 §10.1 基准数据后回看此图，区分"绝对性能"与"工程效率"的不同维度。关键洞察：IO 密集型和编译迭代速度是 Go 的两个关键反例阵地，CPU/内存敏感路径才是 Rust 的主场。[💡 原创分析](../../00_meta/00_framework/methodology.md)
...
| 范式矩阵 | [`./03_paradigm_matrix.md`](../00_paradigms/03_paradigm_matrix.md) | 定位参考 |
| 安全边界 | [`./04_safety_boundaries.md`](../03_domain_comparisons/04_safety_boundaries.md) | 安全保证对比 |
...
> **相关文件**: [Rust vs C++](01_rust_vs_cpp.md) · [范式矩阵](../00_paradigms/03_paradigm_matrix.md) · [问题图谱](../../00_meta/04_navigation/problem_graph.md#三并发系统设计问题树)
...
Go 追求简单性和快速开发，语言特性少，GC 简化内存管理。Rust 追求性能和安全性，通过复杂类型系统（Type System）在编译期消除错误，无 GC 但学习曲线陡。
</details>
...
**题目**: Go 的错误处理（Error Handling）（`if err != nil`）与 Rust 的 `Result<T, E>` 各有什么优劣？

<details>
<summary>✅ 答案与解析</summary>

Go 错误处理（Error Handling）简单显式，但容易遗漏检查，错误传播模板代码多。Rust `?` 运算符简化传播，类型系统（Type System）强制处理，但类型签名更复杂。
</details>
...
**题目**: 为什么 Go 没有泛型（Generics）（直到 1.18），而 Rust 从设计之初就有？

<details>
<summary>✅ 答案与解析</summary>

Go 设计者优先 simplicity，认为泛型（Generics）增加复杂性。Rust 作为系统语言，需要泛型实现零成本抽象（Zero-Cost Abstraction）和类型安全容器。Go 1.18 引入泛型后两者差距缩小。
</details>
