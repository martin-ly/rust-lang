...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md)
> **后置概念**: [iOS Development](../../06_ecosystem/06_data_and_distributed/01_application_domains.md) · [Cross Platform](../../06_ecosystem/05_systems_and_embedded/02_cross_compilation.md)
...
Rust 与 Swift 在工程实践上的可比性高于多数语言对：两者都是 LLVM 后端、都主打内存安全（Memory Safety）、都在各自生态（Apple 平台 / 系统与后端）占主导地位。差异集中在平台绑定深度、互操作成本与性能可预测性三个维度。

三个维度的核心问题：

| 维度 | 关键问题 | 结论预览 |
|---|---|---|
| 平台与生态 | 是否锁定 Apple 平台？ | Swift 在 Apple 生态外部署面窄；Rust 跨平台一等公民 |
| 互操作与 FFI | 与 C/ObjC 的互操作成本 | Swift↔ObjC 无缝但绑定 Apple ABI；Rust↔C 需显式 `extern` 但无平台前提 |
| 性能特征 | 内存管理开销是否可预测 | ARC 的引用计数峰值不可预测；所有权在编译期定界 |

阅读 2.2 时注意：Swift 的 ARC 与 Rust 的 `Rc`/`Arc` 看似同构，但 Swift 中**所有**类实例都走引用计数，Rust 中引用计数是显式选择的局部机制——这一范围差异决定了性能调优的工作方式完全不同。
...
“Swift 和 Rust 一样安全，选哪个只看平台”是移动端与系统编程讨论中最常见的过度简化。本节的反命题树检验这类命题，边界极限部分给出结论反转的条件。

重点检验的三条命题：

1. **“ARC 与所有权等价”**——ARC 无法静态防止循环引用（需手动 `weak`），所有权 + 借用检查在编译期排除；但 ARC 无需学习曲线，Swift 的 UI 代码因此更易写；
2. **“离开 Apple 平台就不该考虑 Swift”**——swift-server 工作组与 Foundation 的跨平台重写正在改变这一前提，但生态深度差距仍是 2026 年的现实约束；
3. **“Rust 在 Apple 平台上是二等公民”**——Xcode 集成确实不如 Swift 原生，但 `cargo` + FFI 的稳定 ABI 路径已被多个跨平台应用（如 Firefox iOS 的组件）验证。

判定依据统一为“目标平台 × 团队语言存量 × 性能可预测性要求”三元组，详见 4.1 的决策树。
...
> [Swift vs Rust Performance — Ben Morris Blog](https://www.ben-morris.com/)
...
| [Ben Morris Blog](https://www.ben-morris.com/) | ✅ 二级 | 对比分析 |
...
- [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) — 内存管理
- [Application Domains](../../06_ecosystem/06_data_and_distributed/01_application_domains.md) — 应用领域
...
Swift 与 Rust 的内存安全机制表面相似（都宣称无 GC、都防空指针），但边界测试能揭示两者的保证强度差异：Swift 的保证依赖 ARC + 运行时检查的组合，Rust 的保证来自编译期的所有权与借用规则。本节四组测试逐一量化这一差距。

测试矩阵：

| 测试 | Swift 侧行为 | Rust 侧行为 |
|---|---|---|
| ARC vs 所有权 | 强引用循环运行期泄漏（需 `weak` 打破） | 循环引用在 `Rc` 下同样泄漏，但借用检查排除数据竞争 |
| Optional 链 vs `?` | `a?.b?.c` 链式短路 | `Option` + `?` 在编译期强制处理 `None` |
| 循环引用差异 | 运行期内存泄漏，工具可检测 | `Rc<RefCell<T>>` 下可构造，但需显式选择该类型 |
| 内存管理对比 | 所有类隐式引用计数 | 仅显式 `Rc`/`Arc` 有计数开销 |

每组给出两侧最小代码与 rustc 1.97（edition 2024）错误输出；Swift 侧以 Swift 5.9+ 语义为准。
