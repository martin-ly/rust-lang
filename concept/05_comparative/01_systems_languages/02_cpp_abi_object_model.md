...
- **什么是 ABI**：源码级 API 的二进制投影——「函数参数放哪个寄存器/栈位」「结构体字段偏移多少」「符号名如何编码」「栈展开信息如何组织」。ABI 兼容 = 不同编译单元（甚至不同编译器/语言）的产物可直接链接调用；ABI 不兼容 = 链接成功但运行时数据错位（最难排查的 bug 类别）。
...
- **Rust 对象模型**：没有「对象」只有「值 + 类型」——结构体无隐藏 vptr（多态由 `dyn Trait` 的**指针侧**承担：胖指针 = 数据指针 + vtable 指针，对象本身保持纯净布局）。泛型单态化使「编译期已知类型」的代码无任何运行时类型信息。**无对象切片**：Rust 无实现继承，「派生值赋给基类变量」的语法不存在——`dyn Trait` 强制经引用/智能指针（unsized 类型不能按值存放），切片问题在语言层面不存在。
...
本节聚焦「枚举的内存布局对比」，覆盖C++ `enum` 与 `std::variant` 与  Rust `enum`。论述顺序由定义到边界：先明确「枚举的内存布局对比」在「Rust vs C++：ABI、对象模型与内存布局」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「枚举的内存布局对比」的判定标准，并指出它在全页论证链中的位置。
