...
> **后置概念**: [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) ·
...
ABI（应用二进制接口）是「编译产物之间如何对话」的契约集合——调用约定、类型布局、名称修饰、异常传播四件套。两种语言在此层面走了截然不同的路：

- **什么是 ABI**：源码级 API 的二进制投影——「函数参数放哪个寄存器/栈位」「结构体（Struct）字段偏移多少」「符号名如何编码」「栈展开信息如何组织」。ABI 兼容 = 不同编译单元（甚至不同编译器/语言）的产物可直接链接调用；ABI 不兼容 = 链接成功但运行时（Runtime）数据错位（最难排查的 bug 类别）。
- **C++ ABI 的困境**：Itanium C++ ABI（GCC/Clang）与 MSVC ABI 双轨制，且「编译器版本 × 标准库版本 × 异常/RTTI 开关」构成兼容矩阵——`std::string` 布局在 libstdc++ ABI 切换（C++11 的 SSO 变更）时强制全量重编译。C++ 没有稳定 ABI 的代价是「二进制分发库几乎必须源码级重编译」。
- **Rust ABI 的设计**：`#[repr(Rust)]` 布局刻意**不保证稳定**（编译器可重排字段优化）——稳定 ABI 的承诺只给 `#[repr(C)]` 与 `extern "C"`。这是有意的自由：换来 `Option<&T>` 空指针优化、字段重排减小体积等布局优化空间。推论：Rust-to-Rust 的 FFI（动态库插件）必须经过 C ABI 边界或 `abi_stable`/`stabby` 类 crates。
- **调用约定对比**：两者默认都平台 C 调用约定（System V AMD64 / Win64）；差异在「胖指针」（Rust `&dyn Trait`/`&[T]` 双字参数）与「返回值优化」（C++ NRVO 依赖移动构造可调用，Rust 移动是 memcpy 无此约束）。
...
对象模型决定「一个值在内存中如何躺」，两种语言的差异根源是「多态机制」：

- **C++ 对象模型**：**POD/简单类**——字段按声明顺序排列（布局固定可预测）；**含虚函数的类**——对象首部嵌入 vptr（虚表指针，通常 8 字节），单继承时派生类「追加」字段共享同一 vptr；**多重继承**——每个有虚函数的基类贡献一个 vptr，对象内含多个基类子对象的串联布局，`this` 指针调整（thunk）使「基类指针 ≠ 派生对象首地址」成为可能。对象切片（slicing）是此模型的固有陷阱：按值传递派生对象到基类参数，vptr 被替换、派生字段被截断。
- **Rust 对象模型**：没有「对象」只有「值 + 类型」——结构体无隐藏 vptr（多态由 `dyn Trait` 的**指针侧**承担：胖指针 = 数据指针 + vtable 指针，对象本身保持纯净布局）。泛型（Generics）单态化（Monomorphization）使「编译期已知类型」的代码无任何运行时类型信息。**无对象切片（Slice）**：Rust 无实现继承，「派生值赋给基类变量」的语法不存在——`dyn Trait` 强制经引用（Reference）/智能指针（Smart Pointer）（unsized 类型不能按值存放），切片问题在语言层面不存在。
- **结构体内存布局对比**：C++ `struct` 字段顺序固定（标准保证），padding 由对齐决定；Rust `#[repr(Rust)]` 允许编译器重排字段（通常按大小降序以减 padding）——同名字段的两个语言结构体，大小与偏移可能不同。跨语言布局一致的唯一保证是两侧都 `#[repr(C)]`/`extern "C" struct`。

判定一个布局问题：C++ 侧查「vptr 位置与继承结构」，Rust 侧查「`repr` 属性与胖指针」——两套排查清单不可混用。
...
- **trivial 语义由 trait 表达**：`Copy`（可按位复制）+ `Send`/`Sync`（线程安全）替代 C++ 的 trivial 分类——语义属性从 ABI 细节提升为类型系统（Type System）的一等谓词。
...
| 编码内容 | 命名空间 + 类型 + 模板参数 | crate 名 + 模块（Module）路径 + 泛型参数 + 哈希 |
...
本节聚焦「枚举（Enum）的内存布局对比」，覆盖C++ `enum` 与 `std::variant` 与  Rust `enum`。论述顺序由定义到边界：先明确「枚举的内存布局对比」在「Rust vs C++：ABI、对象模型与内存布局」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「枚举的内存布局对比」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「Move 语义与析构的 ABI 层面」，覆盖C++ 移动构造与 ABI 与  Rust 移动与 ABI。论述顺序由定义到边界：先明确「Move 语义与析构的 ABI 层面」在「Rust vs C++：ABI、对象模型与内存布局」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「Move 语义与析构的 ABI 层面」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「异常处理与栈展开」，覆盖C++ 异常 ABI 与  Rust Panic ABI。论述顺序由定义到边界：先明确「异常处理与栈展开」在「Rust vs C++：ABI、对象模型与内存布局」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「异常处理与栈展开」的判定标准，并指出它在全页论证链中的位置。
...
本节的反例与边界测试聚焦「布局假设跨语言泄漏」的三个经典事故形态：

- **反例：FFI 中因布局差异导致的数据损坏**：C++ 侧 `struct { char a; int b; }`（offset of b = 4，size = 8）与 Rust 侧 `#[repr(Rust)]` 同名结构体（可能重排为 b 在前、a 在后，size 仍为 8）——按字节映射读写字段即读到错位数据。防御：FFI 边界结构体两侧必须 `#[repr(C)]`/标准布局，并用 `static_assert`/`const assert_eq!(offset_of!(S, b), 4)` 把布局钉进编译期检查。
- **边界测试：`Option<&T>` 的 null pointer optimization**（NPO）：Rust 保证 `Option<&T>`、`Option<Box<T>>`、`Option<NonNull<T>>` 与裸指针同大小（`None` = 0）——这使「`Option<Box<T>>` 直接传给期望「可空指针」的 C 函数」在布局层面合法（语义仍需人工确认）。边界：NPO 只适用于「永不合法为零」的类型，`Option<i32>` 无此优化（8 字节）；依赖 NPO 的 FFI 代码必须在注释中写明该依赖（编译器不承诺永不改变，但 Reference 已标准化该优化）。
- **边界测试：`dyn Trait` 胖指针的 FFI 传递**：`&dyn Trait` 是双字（数据 + vtable），传给期望单字 `void*` 的 C 函数会「丢一半」——编译期不报错（`as *const _` 只取数据指针），运行时 C 侧拿到「只有数据指针」的不完整引用。正确模式：FFI 边界只传「单字指针 + 显式函数表」，或用 `#[repr(C)]` 的函数指针结构体手写 vtable。

三个测试的统一教训：**Rust 与 C++ 的 ABI 兼容性只存在于「双方显式约束到 C 子集」的交集**——交集之外的任何类型（胖指针、NPO 依赖、repr(Rust) 布局）都必须翻译。
...
本节把「Rust vs C++：ABI、对象模型与内存布局」的规则推到编译器与运行时的边界上逐一实测：边界测试：C++ 的多重继承 vs Rust 的 trait 组合（编…、边界测试：C++ 的隐式构造 vs Rust 的显式构造（编译错误）、边界测试：C++ 虚函数表与 Rust trait 对象的 ABI 差…、边界测试：C++ 的 RAII 与 Rust 的 Drop 顺序差异（…等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> **C++ 对比**:
>
> C++ 支持多重继承，`class Button : public Drawable, public Clickable` 可直接创建包含两个基类子对象的对象。
...
> **C++ 对比**:
>
> C++ 支持隐式构造函数（`Point(int x) : x(x), y(0) {}`），允许 `Point p = 5;` 这样的代码。
> 这可能导致意外转换和性能问题（临时对象）。
> Rust 禁止隐式构造——必须显式写出字段名或使用构造函数函数。
> 这消除了 C++ 中常见的"unexpected implicit conversion" bug，但增加了样板代码。
> Rust 的 `From`/`Into` trait 提供显式、可追踪的类型转换，编译器拒绝未实现的转换。
...
> **修正**:
>
> C++ 的虚函数表（vtable）和 Rust 的 trait 对象（`dyn Trait`）在概念上相似（动态分发），但**ABI 不兼容**。
> C++ 的 vtable 通常包含：1) type_info 指针（RTTI）；2) 虚析构函数；3) 虚函数指针。
> Rust 的 vtable 包含：1) drop 函数指针；2) 大小和对齐；3) trait 方法指针。
> 布局不同，不能直接互操作。跨语言虚函数调用必须通过 C ABI（函数指针）或 `cxx` crate 的桥接层。
> 这与 COM（Windows 的组件对象模型，定义了跨语言 vtable 标准）或 Swift 的 witness table（与 Rust 的 vtable 类似但不兼容）类似——语言间的动态分发需要显式适配层。
> Rust 的 `cbindgen` 生成 C 头文件时，不导出 trait 对象，只导出 `#[repr(C)]` struct 和函数指针。
...
> **修正**:
>
> Rust 和 C++ 的析构顺序相同：struct 的字段按**声明顺序的逆序**析构（最后声明的先析构），局部变量按声明顺序的逆序析构。
> 但**依赖管理**不同：C++ 允许在析构函数中访问其他字段（通过 `this` 指针），Rust 的 `Drop` 只能访问自身字段。
> 若 `inner2` 的 drop 逻辑需要 `inner1` 的数据，C++ 中可能侥幸成功（若 `inner1` 尚未析构），Rust 中必然失败（`inner1` 已析构，或 borrow checker 阻止访问）。
> 这是 Rust 更严格的安全保证：drop 必须是自包含的，不能依赖其他字段的生命周期（Lifetimes）。
> 这与 C++ 的"析构函数可做任何事"（包括访问已析构的成员，UB 但常见）不同——Rust 的 borrow checker 在 drop 边界上同样严格。
...
> **修正**:
>
> Rust trait object（`dyn Trait`）的内存布局：**fat pointer** = 数据指针 + vtable 指针。C++ 虚函数：**单指针** = 对象指针（对象开头含 vptr）。
> 两者不兼容：
>
> 1) Rust 的 vtable 是独立的（对象外），C++ 的 vtable 指针在对象内；
> 2) Rust 的 vtable 包含 drop、size、align 和方法指针，C++ 的 vtable 只含虚函数指针；
> 3) 多重继承时 C++ 有多个 vptr，Rust 无多重继承（单继承 + trait）。
>
> 跨语言互操作：
>
> 1) C 接口（`extern "C"`）+ 手动 vtable；
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：C++ 的 vtable 与 Rust 的 `dyn Tra…、测验 2：`#[repr(C)]` 对 Rust struct 的字段…、测验 3：Rust 的 ABI 为什么默认不稳定？这对动态链接有什么影…、测验 4：C++ 的 RAII 与 Rust 的所有权（Ownership）系统在资源释放时…等5个方面，本节依次展开。
...
Rust 编译器优化可能改变结构体（Struct）布局、枚举（Enum）表示和调用约定。不稳定 ABI 意味着不同编译器版本编译的动态库可能不兼容。跨版本动态链接需使用 C ABI（`#[repr(C)]` + `extern "C"`）。
</details>
...
| Rust vs C++：ABI、对象模型与内存布局 正确用法 ⟹ 常见陷阱 | 忽略边界条件 | 编译错误或运行时（Runtime） bug | 高 |
...
mindmap
  root((Rust vs C ABI 对象模型与内存布局))
    ...
    名称修饰 Name Mangling
      ...
      Rust 名称修饰
    枚举的内存布局对比
      C enum 与 std variant
      Rust enum
    Move 语义与析构的 ABI 层面
      C 移动构造与 ABI
```

> **认知功能**: 本 mindmap 从本页「Rust vs C ABI 对象模型与内存布局」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
