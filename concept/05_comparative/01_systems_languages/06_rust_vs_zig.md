...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Comptime](../../06_ecosystem/02_core_crates/01_core_crates.md)
> **后置概念**: [Cross Compilation](../../06_ecosystem/05_systems_and_embedded/02_cross_compilation.md) · [Embedded](../../06_ecosystem/06_data_and_distributed/01_application_domains.md)
...
Rust 与 Zig 争夺的是同一个生态位：C 的现代化替代。但两者的工程实践哲学相反——Zig 追求“比 C 更简单的语言 + 更好的构建系统”，Rust 追求“用类型系统（Type System）换取安全证明”。本节三个维度正是这一哲学分歧的落地点。

| 维度 | Zig | Rust | 工程含义 |
|---|---|---|---|
| 构建系统 | `build.zig` 内置、交叉编译开箱即用 | Cargo + rustc，交叉编译需配 target 工具链 | Zig 在嵌入式交叉编译场景上手更快 |
| 交叉编译 | 自带全部 target 的 libc（含 musl/glibc 版本选择） | 依赖 `rustup target add` + 外部链接器 | Zig 可零依赖为其他平台编译 C 代码 |
| C 互操作 | 直接 `#include` 等价的 `@cImport` | `bindgen` 生成绑定，显式 `unsafe extern` | Zig 的 C 互操作几乎无摩擦；Rust 以显式 unsafe 换取审计边界 |

判定选型的核心问题：项目是否需要编译期的内存安全（Memory Safety）证明？若答案为否（如一次性工具、已充分测试的底层库），Zig 的简洁性是真实优势；若答案为是（长期维护的网络服务、多团队协作），Rust 的类型系统投入才回本。
...
Zig 社区常提出的命题是“Rust 的安全保证可以用纪律与测试替代，因此 Zig 的简单性严格占优”；Rust 社区则反向主张“无借用（Borrowing）检查的语言不适合多人长期项目”。本节用反命题树同时检验这两种立场。

检验框架：

- **反命题树**：把“简单性占优”分解为子命题（学习成本、调试成本、缺陷率），每个子命题给出可测量的判定标准，如“同类缺陷在两侧的平均定位时间”；
- **边界极限**：构造两个极端场景——单人维护的 5000 行嵌入式固件 vs 十人团队维护的 50 万行网络服务，观察“简单性收益”与“类型安全收益”随项目规模的交叉点。

关键边界结论预览：Zig 的 `comptime` 与显式分配器在编译期能力上接近 Rust 的宏（Macro） + const generics，但 Zig 不检查分配后的使用安全性——use-after-free 仍需运行期工具（如 Zig 的 safe 构建模式）发现，这正是 4.2 边界极限实验的测量对象。
...
- [Cross Compilation](../../06_ecosystem/05_systems_and_embedded/02_cross_compilation.md) — 交叉编译
- [FFI](../../03_advanced/04_ffi/01_rust_ffi.md) — 外部函数接口
...
Zig 与 Rust 都把“编译期计算”与“显式内存控制”作为卖点，但两者的边界测试揭示了保证层级的本质差异：Zig 的 `comptime` 是元编程工具，Rust 的 `const fn`/const generics 是类型系统的一部分；Zig 的显式分配器提供控制，Rust 的所有权提供证明。

四组测试的分歧点：

1. **comptime vs const generics**：Zig 在编译期可执行任意代码生成类型，Rust 的 const generics 受限于可求值表达式——Zig 更灵活，但错误信息推迟到实例化点；
2. **显式内存管理 vs 所有权**：Zig 中忘记 `defer allocator.free()` 在 safe 模式下运行期才报，Rust 中对应代码要么编译期拒绝要么自动 Drop；
3. **`comptime` vs `const fn`**：两者的能力交集与各自独占能力清单；
4. **显式分配 vs 全局分配器**：Zig 要求每个分配点传 allocator，Rust 默认全局分配器 + 可选 `#[global_allocator]` 替换。

每组测试给出两侧代码、Zig 0.13+ 与 rustc 1.97（edition 2024）的诊断输出对比。
