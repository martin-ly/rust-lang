...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md)
...
Rust 与 Java 的技术对照是“显式控制 vs 托管运行时”的经典样本：两者都追求内存安全（Memory Safety），但 Java 通过 GC + 运行期检查实现，Rust 通过所有权 + 编译期证明实现。本节三组对照分别落在运行时、错误处理（Error Handling）与泛型三个层面，恰好对应 JVM 生态与 Rust 生态工程文化的全部主要分歧。

| 层面 | Java | Rust | 代价落点 |
|---|---|---|---|
| 运行时架构 | JVM：JIT + GC + 类加载 | 无运行时：AOT 到机器码 | 启动时间/内存基线 vs 编译时间 |
| 异常处理 | checked/unchecked exception | `Result` + `panic!` 二分 | 错误路径是否类型可见 |
| 泛型实现 | 类型擦除，运行期无泛型信息 | 单态化，每实例一份代码 | 二进制体积 vs 运行期反射能力 |

阅读 2.3 时特别注意擦除的连锁后果：`instanceof List<String>` 不可写、`T.class` 不存在、泛型数组创建受限——这些限制在 Rust 的单态化模型下全部不存在，但 Rust 因此无法在运行期做泛型反射，两者是同一权衡的两面。
...
“Java 慢、Rust 快，所以应全面迁移”与“JVM 的 JIT 能超越 AOT，Rust 没有性能优势”是两种都有真实数据支撑、却都被过度外推的命题。本节的反命题树把双方的论据拆开检验。

检验维度：

1. **延迟分布**：JIT 的峰值吞吐在长时间运行后确实可接近甚至局部超越 AOT（借助运行期 profile），但 p99/p999 受 GC 与去优化影响；Rust 的延迟分布更紧，适合尾延迟敏感场景；
2. **内存占用**：JVM 应用的内存基线（堆 + 元空间 + 线程栈）通常在数百 MB，Rust 等价服务可低一个数量级——容器密度高的场景这一差异直接折算为成本；
3. **生态惯性**：Java 的库生态（尤其企业集成、大数据）深度远超 Rust，迁移成本常高于性能收益。

边界极限实验（4.2）构造短生命周期（Lifetimes） CLI 与长生命周期高并发网关两个端点，分别验证两种命题失效的条件。
...
- [Rust vs Go](../01_systems_languages/03_rust_vs_go.md) — Rust 与 Go 的对比
...
Java 开发者初学 Rust 时撞上的编译错误高度集中在三类：泛型行为差异、null 的不可表达、以及资源管理从“GC 兜底”变为“作用域确定”。本节边界测试按这三类组织，每组给出 Java 侧“合法但有隐患”的代码与 Rust 侧的编译期拒绝。

六组测试的对应关系：

- **泛型擦除 vs 单态化**：Java 中 `List<String>` 与 `List<Integer>` 运行期同类型导致的缺陷模式，在 Rust 中因单态化根本不存在对应写法；
- **null vs `Option`**：Java 中返回 null 表示缺失的惯例，Rust 中必须声明 `Option<T>`，调用方不处理即编译失败（`E0xxx`）；
- **GC vs 所有权的资源管理**：`try-with-resources` 依赖开发者记得使用，Rust 的 Drop 在作用域结束处自动且确定地执行；
- 其余三组覆盖异常可见性、可变共享与接口默认方法的差异。

所有 Rust 侧错误输出基于 rustc 1.97（edition 2024）实测，Java 侧以 Java 21 LTS 语义为准。
...
```mermaid
mindmap
  root((Rust vs Java系统编程与托管运行时的范式对))
    一、核心概念
      1.1 内存管理所有权 vs GC
      1.2 类型系统静态显式 vs 静态推断
      1.3 并发模型所有权约束 vs JMM
    二、技术细节
      2.1 运行时架构对比
      2.2 异常处理哲学
      2.3 泛型实现单态化 vs 擦除
