...
Rust 与 Java 的技术对照是“显式控制 vs 托管运行时”的经典样本：两者都追求内存安全，但 Java 通过 GC + 运行期检查实现，Rust 通过所有权 + 编译期证明实现。本节三组对照分别落在运行时、错误处理与泛型三个层面，恰好对应 JVM 生态与 Rust 生态工程文化的全部主要分歧。
...
边界极限实验（4.2）构造短生命周期 CLI 与长生命周期高并发网关两个端点，分别验证两种命题失效的条件。
