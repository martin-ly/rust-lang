...
> **后置概念**: [Rust vs Go](../01_systems_languages/03_rust_vs_go.md) · [Rust vs Java](01_rust_vs_java.md)
...
Rust 与 Python 的技术差异可以用一条主线贯穿：**把多少正确性责任交给机器、在哪个阶段兑现**。Python 选择动态类型 + 运行期异常，把灵活性留给开发者、把错误发现推迟到执行路径覆盖；Rust 选择静态类型 + 编译期借用（Borrowing）检查，把证明责任交给编译器、把灵活性交由泛型（Generics）与 trait 显式表达。

本节三组对照分别落在语言设计的三个层面：

| 层面 | Python 机制 | Rust 机制 | 错误暴露阶段 |
|---|---|---|---|
| 错误处理 | Exception 沿调用栈传播 | `Result<T, E>` 作为返回值 | 运行期 vs 编译期强制处理 |
| 并发 | GIL 串行化字节码执行 | 所有权 + `Send`/`Sync` 编译期证明 | 运行期不确定性 vs 编译期拒绝 |
| 元编程 | 装饰器/元类运行期改写 | 宏在编译期展开为 AST | 运行期 vs 编译期 |

阅读时建议带着一个问题：同一处缺陷（如忘记处理错误分支），在两种语言中分别会在什么时候、以什么形式被发现。
...
“Rust 应该取代 Python”与“Python 的性能对真实业务不重要”是两个方向相反、同样经不起推敲的命题。本节的反命题树把这类绝对化说法拆解为可检验的子命题，边界极限部分则构造极端场景观察结论何时反转。

典型反命题及其失效条件：

1. **“Python 慢，所以计算密集任务必须换 Rust”**——当热点已在 NumPy/PyTorch 的 C 内核中时，Python 只是胶水层，改写收益趋近于零；
2. **“Rust 学习成本高，不适合数据团队”**——当模型服务需要长期维护且延迟预算严格时，编译期类型检查反而降低维护期的心智负担；
3. **“两者只能二选一”**——PyO3/maturin 让 Rust 以扩展模块形式嵌入 Python 生态，混合架构才是工业界主流。

判定依据统一为“热点代码占比 × 调用频率 × 错误代价”：只有三项同时偏高的场景，改写才是正收益。
...
- [Rust vs Go](../01_systems_languages/03_rust_vs_go.md) — Rust vs Go 对比
- [Rust vs Java](01_rust_vs_java.md) — Rust vs Java 对比
...
本节保留最初的一组核心边界测试：把一段依赖动态类型特性的 Python 代码直译为 Rust，观察编译器在何处拒绝。它是理解两种类型系统分野的最小完整案例。

测试逻辑三步走：

1. **Python 侧**：一个接受“任何有 `name` 属性对象”的函数，运行期传入错误类型才抛 `AttributeError`；
2. **Rust 侧**：同样的意图必须写成泛型 + trait bound 或 `dyn Trait`，缺失约束在编译期即报 `E0xxx` 错误；
3. **结论**：Python 的“运行期才失败”不是缺陷而是设计选择，其代价是需要测试覆盖率兜底；Rust 把同等保证前移到编译期，代价是必须预先命名契约（trait）。

后续扩展测试套件见下一个“十、边界测试”小节，本节作为该套件的第 0 组基准案例保留。
...
本套件在上一节基准案例之外，补充并发维度与“不可表达性”维度的边界测试，构成从类型系统到并发模型的完整对照。

四组测试各回答一个问题：

- **10.1 动态类型 vs 静态类型**：Python 中“先运行再说”的代码，在 Rust 中需要补多少类型标注才能编译？
- **10.2 GIL vs 所有权并发**：Python 多线程因 GIL 退化为交错的场景，Rust 如何用 `Send` 约束在编译期排除数据竞争？
- **10.3 `Arc<Mutex<T>>` 的运行时开销**：Rust 的共享状态并发并非零成本，锁竞争与引用（Reference）计数开销需要用基准测试量化，避免“Rust 并发一定快”的误解；
- **10.4 不可表达性**：依赖 `eval`、运行期注入属性等元编程技巧的 Python 代码，在 Rust 中不存在直译，必须改用宏或配置驱动重构。

每组均给出两侧最小代码与 rustc 1.97（edition 2024）的完整错误输出，便于读者本地复现。
...
- **P2 生态/社区**: [docs.rs/pyo3 — 生态权威 API 文档（Rust 与 Python 互操作绑定，跨语言对照实践）](https://docs.rs/pyo3)（2026-07-12 验证 HTTP 200）

## 🧭 思维导图（Mindmap）

```mermaid
mindmap
  root((Rust vs Python系统编程与动态脚本的对照))
    一、核心概念
      1.1 设计哲学对比
      1.2 类型系统静态 vs 动态
      1.3 内存模型所有权 vs GC
