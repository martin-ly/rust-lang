...
Rust 与 Python 的技术差异可以用一条主线贯穿：**把多少正确性责任交给机器、在哪个阶段兑现**。Python 选择动态类型 + 运行期异常，把灵活性留给开发者、把错误发现推迟到执行路径覆盖；Rust 选择静态类型 + 编译期借用检查，把证明责任交给编译器、把灵活性交由泛型与 trait 显式表达。
...
- **10.3 `Arc<Mutex<T>>` 的运行时开销**：Rust 的共享状态并发并非零成本，锁竞争与引用计数开销需要用基准测试量化，避免“Rust 并发一定快”的误解；
