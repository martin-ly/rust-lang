...
Rust 与 Scala 在类型系统能力上重叠最大（两者都有 ADT、泛型、类型推断），差异集中在三个机制的**显式程度**：
...
- **宏系统**: Scala 宏（scala-meta/inline）操作 AST 且与类型系统深度交互；Rust 声明宏是模式替换、过程宏操作 token 流，刻意**不做**类型驱动的代码生成（const generics 之外的类型计算被刻意限制）。
...
- **构建系统**: sbt（Scala）是增量编译 + 交互式 shell 的一体环境，但多模块构建配置的 DSL 复杂度高、构建服务器内存占用大；Cargo 是声明式 manifest + 约定优于配置，多 crate workspace 的依赖解析与锁文件（`Cargo.lock`）语义更清晰。两者都支持多模块，但 Cargo 的“workspace 即版本单元”心智模型更简单。
- **互操作性**: Scala 天然互操作整个 JVM 生态（直接调用 Java 库），代价是继承 JVM 的部署重量与 GC 语义；Rust 的互操作主场是 C ABI（`extern "C"` 双向）与 WASM——调用 Java/Python 需经 JNI/PyO3 桥接，但反过来“被其他语言调用”时 Rust 的零运行时依赖是部署优势。
...
- **“JVM 性能经过 JIT 后不输原生代码”** —— 部分成立。JIT 在稳态热点上可接近原生，但启动时间、内存 footprint（2–5 倍）、尾延迟（GC 停顿）三个维度结构性落后；短生命周期进程（CLI、Serverless 函数）与延迟敏感服务中差距放大。
