...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Generics](../../02_intermediate/01_generics/01_generics.md)
> **后置概念**: [JVM Ecosystem](../../06_ecosystem/02_core_crates/01_core_crates.md) · [Functional Programming](../00_paradigms/01_paradigm_matrix.md)
...
Rust 与 Scala 在类型系统（Type System）能力上重叠最大（两者都有 ADT、泛型（Generics）、类型推断（Type Inference）），差异集中在三个机制的**显式程度**：

- **类型推断**: Scala 是全局 Hindley-Milner 推断（可省略函数返回类型），Rust 是局部推断（函数签名必须完整）——Scala 的简洁代价是错误信息可能远离真实错误点，Rust 的冗长换来错误的局部化。
- **隐式（implicits）vs Trait**: Scala 的隐式参数/转换由编译器在作用域内自动搜寻填充，灵活但“代码中看不见依赖从何而来”；Rust 的 trait 约束必须显式写在签名上（`T: Trait`），依赖注入经参数传递——Rust 的设计哲学是“隐式行为必须显式声明”，Scala 信任程序员管理隐式作用域。
- **宏系统**: Scala 宏（scala-meta/inline）操作 AST 且与类型系统深度交互；Rust 声明宏（Declarative Macro）是模式替换、过程宏（Procedural Macro）操作 token 流，刻意**不做**类型驱动的代码生成（const generics 之外的类型计算被刻意限制）。

判定依据：从 Scala 迁移的最大认知调整是“把所有隐式改为显式参数”，代码量上升但可推理性同步上升。
...
- **构建系统**: sbt（Scala）是增量编译 + 交互式 shell 的一体环境，但多模块（Module）构建配置的 DSL 复杂度高、构建服务器内存占用大；Cargo 是声明式 manifest + 约定优于配置，多 crate workspace 的依赖解析与锁文件（`Cargo.lock`）语义更清晰。两者都支持多模块，但 Cargo 的“workspace 即版本单元”心智模型更简单。
- **互操作性**: Scala 天然互操作整个 JVM 生态（直接调用 Java 库），代价是继承 JVM 的部署重量与 GC 语义；Rust 的互操作主场是 C ABI（`extern "C"` 双向）与 WASM——调用 Java/Python 需经 JNI/PyO3 桥接，但反过来“被其他语言调用”时 Rust 的零运行时（Runtime）依赖是部署优势。

判定依据：重度依赖 JVM 库生态（如 Spark、Akka）的系统留在 Scala；需要嵌入、边缘部署或成为其他语言的高性能内核时选 Rust。
...
Rust 与 Scala 对比中的两个高频误判：

- **“Scala 的函数式能力严格强于 Rust”** —— 需要限定。类型类、高阶类型（HKT）、泛函编程库（cats/ZIO）确实是 Scala 独有优势；但 Rust 的仿射类型 + 所有权提供了 Scala 没有的资源安全保证（`IO` 里藏不住的文件句柄泄漏，Rust 编译期拒绝）。两者是“表达力 vs 约束力”的不同取舍，不是强弱关系。
- **“JVM 性能经过 JIT 后不输原生代码”** —— 部分成立。JIT 在稳态热点上可接近原生，但启动时间、内存 footprint（2–5 倍）、尾延迟（GC 停顿）三个维度结构性落后；短生命周期（Lifetimes）进程（CLI、Serverless 函数）与延迟敏感服务中差距放大。
- **边界极限**: 大数据生态（Spark/Flink）的绑定语言选择基本被平台锁定——这类场景“换语言”的选项本身不存在，对比的意义在于架构边界处的服务选型。
...
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 泛型（Generics）
- [Paradigm Matrix](../00_paradigms/01_paradigm_matrix.md) — 范式矩阵
...
边界测试的目标是把 Scala 与 Rust 的**隐式/显式哲学差异**转化为可机器复核的编译结果：Scala 依赖隐式解析（implicits）与 JVM 运行时代偿的写法，在 Rust 中应被 trait 求解器或借用（Borrowing）检查器在编译期拒绝。每个用例给出三段式证据：

1. **Scala 侧**：惯用代码 + 隐式解析发生的具体位置；
2. **Rust 侧**：等价尝试 + rustc 1.97（Edition 2024）的精确报错；
3. **判定依据**：差异来自 trait 一致性（Coherence）、null 建模还是并发运行时。

| 用例 | Scala 依赖的机制 | Rust 的拦截点 |
|:---|:---|:---|
| 隐式转换 vs 显式 trait | 编译器自动插入 `implicit` | 必须显式调用 `From`/`Into` |
| `null` vs `Option` | JVM 引用（Reference）可空 | 类型不匹配，`Option` 强制解包 |
