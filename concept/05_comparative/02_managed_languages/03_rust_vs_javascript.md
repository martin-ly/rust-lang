...
> **后置概念**: [WebAssembly](../../06_ecosystem/11_domain_applications/03_webassembly.md) · [Async](../../03_advanced/01_async/01_async.md)
...
Rust 与 JavaScript 的技术对照集中在异步、错误处理与互操作三个战场——这并非巧合：两者今天最常相遇的场景就是“Rust 编译为 WebAssembly，在 JS 宿主中运行”。理解各自机制的差异，是设计 WASM 边界处 API 的前提。

三组对照的核心分歧：

| 主题 | JavaScript | Rust | 关键差异 |
|---|---|---|---|
| 异步 | Promise 立即开始执行（hot） | `Future` 惰性、poll 驱动（cold） | 取消语义与执行时机不同 |
| 错误 | `throw` 任意值，类型不可知 | `Result<T, E>` 类型化且必须处理 | 错误是否为类型系统一部分 |
| 互操作 | WASM 线性内存共享 | `wasm-bindgen` 生成绑定 | 所有权跨越边界时需显式转移/克隆 |

阅读 2.1 时特别注意“hot vs cold”差异：JS 中创建一个 Promise 即启动了工作，Rust 中创建一个 `Future` 什么也不做，直到 executor 轮询——这决定了两侧取消与超时语义的实现方式完全不同。
...
前端与 Node 生态中流传着两个极端判断：“WASM + Rust 将取代 JavaScript”和“Rust 对 Web 开发毫无价值”。本节的反命题树同时检验两者，边界极限部分给出各命题失效的具体条件。

检验维度三条：

1. **计算密度**：页面逻辑以 DOM 事件为主时，WASM 的调用边界开销（字符串编解码、内存拷贝）会吃掉全部计算收益；只有图像/音视频/密码学等连续数值计算场景才反转；
2. **生态依赖度**：深度依赖 npm 生态（框架、构建链）的项目，引入 Rust 工具链的边际成本高于收益；
3. **团队结构**：全栈 JS 团队引入第二种语言意味着双倍的招聘与维护面。

边界极限实验（4.2）构造了两个极端：纯 DOM 交互页面与 4K 视频滤镜管线，分别验证“Rust 无益”与“Rust 必需”两个端点。结论是谱系而非开关：按模块边界选择语言，而不是按项目。
...
- [WebAssembly](../../06_ecosystem/11_domain_applications/03_webassembly.md) — WebAssembly 生态
...
- [Rust vs Python](02_rust_vs_python.md) — Rust vs Python 对比
...
JavaScript 的三类“便利”——隐式类型转换、闭包自由捕获、动态 `this` 绑定——恰好是 Rust 类型系统重点约束的三类行为。本节边界测试把每一类便利写成最小代码，展示它在 JS 侧静默运行、在 Rust 侧被编译器拒绝的对照。

阅读要点：

- **隐式转换**：`"1" + 2 === "12"` 在 JS 中合法，Rust 中字符串与整数相加直接报 `E0369`，必须显式 `to_string()` 或 `parse()`；
- **闭包捕获**：JS 闭包默认按引用（Reference）捕获外层变量且可随时修改，Rust 要求明确 `move`/`&`/`&mut`，违反借用（Borrowing）规则即编译失败；
- **`this` 绑定**：JS 中方法脱离对象调用会丢失 `this`，Rust 的方法调用语法在类型层面绑定接收者，不存在同类缺陷。

每组测试标注错误类别（编译错误/逻辑错误）与对应的 rustc 诊断码，便于与第六章的错误处理对比交叉阅读。
...
本组五道测验检验读者能否把两种语言的机制差异映射到同一组概念坐标上，全部位于理解层。作答前建议先合上前文，凭记忆回答，再逐题回查。

题目与正文的对照关系：

| 题号 | 考察点 | 回查章节 |
|---|---|---|
| 1 | 内存管理：GC vs 所有权 | 内存模型对比节 |
| 2 | Promise vs Future 的 hot/cold 语义 | 2.1 异步模型对比 |
| 3 | WASM 作为互操作层的位置 | 2.3 WASM 桥接 |
| 4 | 原型继承 vs trait 的代码复用 | 类型系统对比节 |
| 5 | 模块系统：require/import vs crate/mod | 工程实践节 |

通过标准：每题能用一段具体代码场景作答，且能指出对方语言中“不存在直译”的机制（如 JS 无借用检查、Rust 无原型链）。若只能给出“更快/更安全”式形容词回答，请回到对应小节重读边界条件。
