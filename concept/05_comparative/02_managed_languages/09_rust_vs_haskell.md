...
- [Rust vs Haskell：所有权/RAII 与惰性纯函数式语言的系统对比](#rust-vs-haskell所有权raii-与惰性纯函数式语言的系统对比)
  ...
  - [效应系统](#效应系统)
    ...
    - [6.4 形式化语义快照：调用点与求值序](#64-形式化语义快照调用点与求值序)
  - [形式化与验证](#形式化与验证)
  ...
  - [反命题/边界](#反命题边界)
    ...
    - [10.4 反例：惰性求值导致的空间泄漏](#104-反例惰性求值导致的空间泄漏)
  - [来源与延伸阅读](#来源与延伸阅读)
    - [国际权威参考](#国际权威参考)
    - [相关概念](#相关概念)
    - [权威来源索引](#权威来源索引)
...
本节对比 Rust 的编译期所有权/RAII 与 Haskell 的托管堆/分代 GC/惰性求值，并从延迟可预测性、空间泄漏与堆布局控制三个角度总结空间-时间权衡。
...
本节从 trait vs typeclass、System F/Hindley-Milner 形式化根基、单态化 vs 字典传递、高阶类型四个子主题，展开 Rust 与 Haskell 的类型系统差异。
...
本节对比 Rust 的所有权驱动 OS 线程/async 并发与 Haskell 的 green threads/STM/MVar，揭示两种线程安全策略的来源与限制。
...
本节对比 Rust 的词法/上下文显式效应（unsafe/async/const）与 Haskell 的单子/纯函数默认，澄清两种副作用追踪机制的语义区别。
...
从操作语义角度，Rust 的函数调用是**严格求值 + 立即效应**：

```text
E[ f(e) ]  ⟶  E[ f(v) ]  ⟶  E[ e' ]     (e ↓ v, 然后执行 f 体)
```

调用前 `e` 必须归约到值 `v`，且 `f` 体执行时所有副作用立即发生。`unsafe` 不改变这一求值模型，只是取消了对某些操作的静态禁止。

Haskell 的函数调用在纯代码中是**按需惰性求值**：

```text
E[ f(e) ]  ≈  E[ 将 e 包装为 thunk ]  ⟶  在真正需要值时求值
```

`IO a` 的动作在 `main` 被运行时系统解释时才产生副作用，纯代码中的 `e` 可能只是被传递而永不求值。这意味着：

- Rust 的**时间/空间行为更接近 C/C++**，便于预测底层性能；
- Haskell 的**等式推理更简洁**，但 thunk 累积、空间泄漏（space leak）和求值序错误需要额外心智模型。

**不可等价性**：Rust 的 `unsafe` 块无法通过纯函数式重写消除，因为它涉及底层内存布局和调用约定；Haskell 的 `IO` 也无法通过 Rust 的普通函数表达，因为后者不追踪副作用类型。

---

## 形式化与验证

本节对比 Rust 围绕内存安全/UB/并发协议的验证工具链与 Haskell 偏向精化类型/程序等价的验证生态，说明两者关注正确性层级的不对称性。
...
本节从 Rust 的仿射类型、Haskell 的经典类型与 Linear Haskell 三个视角，讨论资源控制与抽象自由之间的表达力光谱。
...
本节区分 Rust 在系统/嵌入式/网络基础设施的优势领域与 Haskell 在编译器/数据转换/金融建模/学术研究的优势领域，并通过场景矩阵给出互补性建议。
...
本节主动检验 Rust 与 Haskell 互换的误区，分别列出两者不适合的场景并澄清常见误解。
...
以下 Haskell 代码在语义上正确，但运行时会因为 thunk 累积导致空间泄漏：

```haskell
-- 求 [1..1000000] 的和；中间 thunk 链占用 O(n) 空间
sumToN :: Int -> Int
sumToN n = sum [1..n]
```

在 Rust 中，等价的严格求值代码不会累积 thunk，但大区间迭代仍需注意内存分配：

```rust
fn sum_to_n(n: i64) -> i64 {
    (1..=n).sum()   // 严格求值，无 thunk 累积
}
```

**教训**：Haskell 的惰性求值使等式推理更简洁，但引入了「何时求值」的额外语义维度；Rust 的严格求值牺牲了部分抽象简洁性，换取了更可预测的资源行为。两种语义不可互相模拟而不改变语言核心。

---

## 来源与延伸阅读

本节整理国际权威参考、项目内部相关概念链接与权威来源索引，为全文论断提供可追溯依据。
