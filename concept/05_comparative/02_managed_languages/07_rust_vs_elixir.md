...
> **前置概念**: [Rust vs Go](../01_systems_languages/03_rust_vs_go.md) · [Rust vs Haskell](../00_paradigms/01_paradigm_matrix.md) · [Async](../../03_advanced/01_async/01_async.md)
> **后置概念**: [Ecosystem](../../README.md) · [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md)
...
- [Rust vs Elixir 对比分析](#rust-vs-elixir-对比分析)
  ...
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：把非 `Send` 数据发送给另一个线程（rustc 1.97.0 实测）](#反例把非-send-数据发送给另一个线程rustc-1970-实测)
    - [✅ 修正：换成 `Arc`（线程安全引用计数）](#-修正换成-arc线程安全引用计数)
...
两种并发模型的本质差异在**隔离粒度与失败语义**：

| 维度 | BEAM（Elixir） | Rust（Tokio/std） |
|---|---|---|
| 并发单元 | 进程（process），独立堆，崩溃隔离 | 线程/任务，共享地址空间 |
| 调度 | 抢占式（reduction 计数），公平 | 协作式（async await 点） |
| 通信 | 仅消息传递（mailbox），无共享内存 | 消息（channel）+ 共享内存（Arc/Mutex） |
| 失败语义 | 「Let it crash」+ 监督树重启 | `Result` 显式传播 + panic 边界 |

BEAM 的抢占调度保证单进程死循环不拖垮系统，适合电信级软实时；Rust 的协作调度换得零拷贝与内存效率，适合计算密集场景。**两者组合**（Rust NIF/Port 嵌入 BEAM）是 Discord 等公司的实践：BEAM 管编排与容错，Rust 管热路径。

判定依据：需要百万级轻量隔离单元 → BEAM；需要单节点极限吞吐/无 GC 尾延迟 → Rust。
...
- **Elixir（动态 + 可选 spec）**：Dialyzer 的 success typing 是「不会误报」的近似分析，可发现部分错误但不保证无漏报；模式匹配失败是运行期 `MatchError`。
- **Rust（静态 + 代数类型）**：`match` 穷尽性编译期强制，遗漏分支即编译错误；`Option`/`Result` 使空值与错误成为类型的一部分而非运行期惊喜。

两者共享的模式匹配语法掩盖了深度差异：Elixir 的匹配是运行期绑定/断言，Rust 的匹配是编译期消去分析。迁移经验：Elixir 中靠测试覆盖的「不可能状态」在 Rust 中应建模为不可构造的类型（`enum` 变体即合法状态全集）。

判定依据：协议/状态机密集的逻辑迁移到 Rust 收益最大；胶水脚本类逻辑留在 Elixir 损失不大。
...
> **部署洞察**: **Rust 的零运行时（Runtime）适合静态部署，Elixir 的热更新适合长运行服务**。
...
**反命题**：「Rust 总是比 Elixir 快，所以应该重写一切」——错误。Web/通信类服务的瓶颈常在 I/O 与并发调度，BEAM 的抢占调度 + 热代码升级在此类负载上综合表现不输，而重写成本（团队、生态库、运维工具链）常占项目预算大半。

**适用场景矩阵**：

| 场景 | 推荐 |
|---|---|
| 聊天/推送/IoT 长连接网关 | Elixir（BEAM 原生场景） |
| 音视频编解码、风控引擎热路径 | Rust（计算密集） |
| 强一致性（Coherence）分布式数据库 | Rust（无 GC 停顿）或两者分层 |
| 快速迭代的全栈 Web 应用 | Elixir（Phoenix LiveView 生产力） |

判定依据：重写决策必须包含「机会成本」项——用 BEAM 生态缺失的库数量 × 自研成本量化。
...
- [Rust vs Go](../01_systems_languages/03_rust_vs_go.md) — Rust vs Go
- [Rust vs Haskell](../00_paradigms/01_paradigm_matrix.md) — Rust vs Haskell
- [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) — 并发
- [Async](../../03_advanced/01_async/01_async.md) — 异步（Async）
...
本节保留 Elixir 对比中最基础的一组边界测试：动态类型与静态模式匹配的分野。它是后续扩展套件的基准案例，单独成节以保持原始论证结构的完整性。

测试的核心场景：

1. **Elixir 侧**：函数子句（function clause）按模式匹配分派，传入未覆盖的形状在运行期抛 `FunctionClauseError`；
2. **Rust 侧**：同一意图用 `enum` + `match` 表达，编译器做穷尽性检查，遗漏变体即编译错误 `E0004`；
3. **机理差异**：Elixir 的分派发生在运行期且子句顺序敏感，Rust 的匹配在编译期完备性可证——前者灵活（可热更新增子句），后者安全（重构时编译器列出所有待更新匹配点）。

该案例同时说明了为什么 Elixir 适合热升级场景而 Rust 适合重构频繁的大型代码库：同一种机制差异，在两个工程场景中的评价相反。
...
本扩展套件在基准案例之外补充两组并发维度的边界测试，覆盖 Rust 与 Elixir 并发哲学的核心分歧：进程隔离 + 消息传递 vs 共享内存 + 所有权（Ownership）约束。

三组测试的对照：

| 测试 | Elixir | Rust |
|---|---|---|
| 动态类型 vs 静态模式 | 运行期 `FunctionClauseError` | 编译期穷尽性检查 |
| 进程邮箱 vs channel | 每进程内置有序邮箱，选择性接收 | `mpsc` channel 按发送顺序，选择需 `select!` 宏（Macro） |
| 进程隔离 vs 共享内存并发 | 进程间零共享，崩溃隔离 | 线程可共享内存，数据竞争编译期排除 |

阅读重点是第三组：Elixir 用隔离换取容错（一个进程崩溃不影响其他进程），Rust 用类型系统换取共享内存的安全——两者解决了“并发可靠性”这一问题的不同半边，第 10.5 节的混合架构（Elixir 做编排、Rust NIF 做计算）正是利用这一互补性。
...
本组五道测验检验读者对两种并发哲学互补性的理解，全部位于理解层。题目刻意避免语法细节，聚焦设计取舍。

题目与正文对照：

| 题号 | 考察点 | 关键概念 |
|---|---|---|
| 1 | Actor 进程隔离 vs 共享内存并发 | 崩溃域、内存模型 |
| 2 | Let it crash vs 编译期预防 | 错误处理哲学 |
| 3 | BEAM 百万进程的原理 | 抢占式调度、进程堆独立 GC |
...
- **P2 生态/社区**: [docs.rs/rustler — 生态权威 API 文档（Rust 编写 Elixir/Erlang NIF，跨语言对照实践）](https://docs.rs/rustler)（2026-07-12 验证 HTTP 200）

## ⚠️ 反例与陷阱

本节以跨线程发送 `Rc` 为反例，展示 Rust 与 Elixir 在消息传递安全模型上的根本差异。

### 反例：把非 `Send` 数据发送给另一个线程（rustc 1.97.0 实测）

Elixir 进程间消息靠复制；Rust 线程间移动所有权须满足 `Send`：
...
**错误**：`E0277 Rc<Vec<i32>> cannot be sent between threads safely`。
