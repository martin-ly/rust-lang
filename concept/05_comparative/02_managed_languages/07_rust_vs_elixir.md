...
本扩展套件在基准案例之外补充两组并发维度的边界测试，覆盖 Rust 与 Elixir 并发哲学的核心分歧：进程隔离 + 消息传递 vs 共享内存 + 所有权约束。
...
| 进程邮箱 vs channel | 每进程内置有序邮箱，选择性接收 | `mpsc` channel 按发送顺序，选择需 `select!` 宏 |
