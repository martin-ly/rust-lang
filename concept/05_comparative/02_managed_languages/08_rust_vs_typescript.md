...
1. **类型系统哲学**：TS 是**渐进结构类型**（gradual + structural）——类型可逐步添加，`any` 是逃生舱，类型在运行期完全擦除；Rust 是**强制名义类型**——类型是值的构造规则，编译期不变量运行期零成本。推论：TS 的类型错误最多延迟到运行期暴露（`undefined is not a function`），Rust 的同类错误不存在运行期形态。
2. **异步模型**：TS 的 Promise 是**热切**（创建即执行）、隐式调度；Rust 的 Future 是**惰性**（`await`/`poll` 才驱动）、显式运行时——同一 `fetch()` 在两侧的重试/取消语义完全不同。
3. **错误处理**：TS `throw` 不可见于签名（类型层无 `throws` 标注），Rust `Result` 是类型的一部分；`tsc` 无法强制捕获，`rustc` 强制处理。
...
- **工程选型决策树（4.2）**：从项目约束（团队构成、性能要求、部署目标）出发，给出可操作的选型路径，包括“前后端同构用 TS、性能关键模块用 Rust + WASM”的混合方案。
...
3. **运行期行为不可类型化**：内存使用、数据竞争、尾延迟完全在 TS 类型系统视野之外；Rust 的类型系统（所有权 + Send/Sync）覆盖这些维度。
