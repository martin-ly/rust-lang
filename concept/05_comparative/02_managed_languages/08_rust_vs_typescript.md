...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Async](../../03_advanced/01_async/01_async.md)
> **后置概念**: [WebAssembly](../../06_ecosystem/11_domain_applications/03_webassembly.md) · [Rust vs JavaScript](03_rust_vs_javascript.md)
...
TypeScript 与 Rust 技术细节的四维对比：

1. **类型系统（Type System）哲学**：TS 是**渐进结构类型**（gradual + structural）——类型可逐步添加，`any` 是逃生舱，类型在运行期完全擦除；Rust 是**强制名义类型**——类型是值的构造规则，编译期不变量运行期零成本。推论：TS 的类型错误最多延迟到运行期暴露（`undefined is not a function`），Rust 的同类错误不存在运行期形态。
2. **异步（Async）模型**：TS 的 Promise 是**热切**（创建即执行）、隐式调度；Rust 的 Future 是**惰性**（`await`/`poll` 才驱动）、显式运行时（Runtime）——同一 `fetch()` 在两侧的重试/取消语义完全不同。
3. **错误处理（Error Handling）**：TS `throw` 不可见于签名（类型层无 `throws` 标注），Rust `Result` 是类型的一部分；`tsc` 无法强制捕获，`rustc` 强制处理。
4. **WASM 互操作**：TS 是 WASM 的主要宿主语言（胶水层），Rust 是主要 guest 语言——二者在 WASM 生态是**互补而非竞争**。

判定依据：类型安全关键路径（金融、解析器）→ Rust；DOM 交互与快速迭代 → TS。
...
> **认知功能**: TypeScript 的错误处理（Error Handling）依赖**约定和 linter**，Rust 的错误处理是**类型系统（Type System）的一部分**——调用 `?` 的函数必须返回 Result，形成编译期传播链。
...
Rust 与 TypeScript 的对比有一个其他语言对不具备的张力：两者都宣称“类型安全”，但 TypeScript 的类型系统是**结构化的、可擦除的、设计为可绕过的**（`any`、类型断言、`// @ts-ignore`），Rust 的类型系统是**名义化的、编译期强制的、绕过需显式 unsafe**。本节两张图服务于理解这一张力。

两张图的阅读分工：

- **类型系统哲学对比图（4.1）**：沿“表达力”与“保证强度”两个坐标轴定位两种类型系统，并标注 TypeScript 中每个“逃生舱”对应的保证损失；
- **工程选型决策树（4.2）**：从项目约束（团队构成、性能要求、部署目标）出发，给出可操作的选型路径，包括“前后端同构用 TS、性能关键模块（Module）用 Rust + WASM”的混合方案。

阅读建议：先在 4.1 中找到自己当前项目的位置，再用 4.2 验证选型结论是否一致——若不一致，通常是某个约束（如招聘市场、生态依赖）被低估了。
...
反命题：「TypeScript 严格模式（strict）下类型安全足够，不需要 Rust」——三条边界反驳：

1. **擦除边界**：TS 类型在编译后消失，跨网络/FFI/序列化边界的值不受保护——`JSON.parse` 返回 `any`，`as` 断言是 unchecked cast；Rust 的 `serde` 反序列化是**验证式**的，schema 不符即错误。
2. **结构类型的意外兼容**：`{a: number}` 与 `{a: number, b: string}` 在 TS 中互相赋值合法（宽度子类型），配置对象字段漂移类 bug 由此产生；Rust 名义类型要求显式转换。
3. **运行期行为不可类型化**：内存使用、数据竞争、尾延迟完全在 TS 类型系统视野之外；Rust 的类型系统（所有权（Ownership） + Send/Sync）覆盖这些维度。

**边界极限**：TS 团队引入 Rust 的正确姿势是「TS 管 UI 与编排，Rust 经 WASM/NAPI 管计算内核」——参考 SWC、Rolldown、Biome 的工具链重写浪潮。

判定依据：性能分析显示 JS 引擎占 CPU >30% 的模块是 Rust 重写候选。
...
- [Rust vs JavaScript](03_rust_vs_javascript.md) — Rust 与 JavaScript 对比
- [Async/Await](../../03_advanced/01_async/01_async.md) — 异步编程深度分析
- [WebAssembly](../../06_ecosystem/11_domain_applications/03_webassembly.md) — WASM 生态与工具链
...
TypeScript 类型系统的三类“逃生舱”——`any`、可选属性、结构子类型——在 Rust 中要么不存在、要么需要显式机制表达。本节边界测试逐一展示：同一段 TS 代码通过类型检查却在运行期失败，对应的 Rust 写法在编译期即被拒绝。

五组测试的核心对照：

1. **`any` vs 显式类型**：TS 中 `any` 关闭一切检查且会“传染”上下游类型推导；Rust 中没有等价物，最接近的 `dyn Any` 仍需显式 downcast；
2. **可选属性 vs `Option`**：TS 的 `{ name?: string }` 允许读取后得到 `undefined` 而不报警（非 strict 模式），Rust 的 `Option<String>` 不解包无法使用；
3. **结构类型 vs 名义类型**：TS 中形状相同即兼容，Rust 中类型相等要求同一声明——API 边界处的“恰好形状相同”在 Rust 中必须显式转换；
4. 其余两组覆盖类型断言与 `unknown` 的差异。

每组给出 tsc（5.x，strict 模式）与 rustc 1.97（edition 2024）的诊断对照。
...
本组五道测验检验读者是否理解两种类型系统的“保证强度谱系”——从 TS 的“尽力而为”到 Rust 的“编译期强制”。全部位于理解层，不要求记忆语法细节。

作答指引：

1. 测验 1 的关键是“擦除”：TS 类型在运行期不存在，因此无法做运行期验证；回答需点明这一根因而非罗列现象；
2. 测验 2 要求给出渐进类型各适用一个真实场景（如存量 JS 迁移 vs 全新安全关键模块）；
3. 测验 3 注意 Discriminated Union 的判别字段是约定，Rust 的 enum 变体是类型本身；
4. 测验 4 的标准答案是“类型在编译后被擦除 + 外部数据不受类型约束”，需结合 API 边界验证（如 zod/io-ts）说明工程补救；
5. 测验 5 需给出当前 Rust 前端生态（Leptos/Yew/Dioxus）的成熟度评估与限制。

每题参考答案后均标注回查章节，建议先独立作答再对照。
...
```mermaid
mindmap
  root((Rust vs TypeScript静态类型系统的两))
    一、权威定义与核心概念
      1.1 TypeScript 权威定义
      1.2 类型系统哲学对比
      1.3 编译模型差异
