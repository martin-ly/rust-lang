...
> **后置概念**: [Cross Compilation](../../06_ecosystem/05_systems_and_embedded/02_cross_compilation.md) · [.NET Ecosystem](../../06_ecosystem/02_core_crates/01_core_crates.md)
...
判定依据：从 C# 迁移时，先映射异常层次到 `thiserror` 枚举（Enum），这是设计差异最大的环节。
...
2. **“LINQ 与迭代器等价”**：LINQ 的延迟执行与 Rust 迭代器的惰性链确实同构，但 LINQ 的 `IEnumerable` 可多次枚举（可能重复执行副作用），Rust 迭代器消费即转移所有权（Ownership）——这一差异在边界测试中可复现；
...
> C# 的 `async Main` 隐藏了 runtime 创建，Rust 要求显式选择 runtime（tokio、smol [历史: async-std [已停止维护]]）。
...
- 测验 3 考察 `unsafe` 在两种语言中的文化地位差异，回答需引用（Reference）使用频率数据而非印象；
