...
「定理推理链」部分的核心主题是定理一致性矩阵（执行模型同构性专集），本节展开说明。
...
本节从 L0-L7 纵向映射 与 相关概念 两个层面剖析「相关概念链接（L0-L7 映射）」。
...
本节将「边界测试：执行模型同构的编译错误」分解为若干主题：边界测试：栈展开与 `panic = abort` 的行为差异（运行时…、边界测试：`async` 与线程的执行模型混淆（编译错误）、边界测试：绿色线程与 OS 线程的 API 混用（编译错误）、边界测试：CPS 变换与 Rust 的 `?` 运算符（编译错误）等6个方面。
