...
- **Rust 相关映射**：`async`/await 的状态机编译 = CPS 变换的 defunctionalization 实例；迭代器融合 = 短路融合（shortcut fusion，GHC 的 foldr/build 规则的 Rust 对应）。
...
- **执行模型维度雷达图**：以六个维度（求值严格性、并发粒度、内存管理、错误传播、可变性默认、元编程时机）对每种语言打分（0–5）。Rust 的雷达图特征：可变性默认=0（默认不可变）、内存管理=5（完全静态）、元编程时机=5（完全编译期）。C++ 与 Rust 的雷达图在"错误传播"和"可变性默认"两轴差异最大，这定量解释了为什么 C++ 程序员迁移时错误处理与 `let mut` 习惯是主要摩擦点。
- **内存-计算-通信三维空间**：任何并发程序可投影到此空间——内存维（共享/私有）、计算维（顺序/并行）、通信维（同步/异步）。`Arc<Mutex<T>>` 位于（共享, 并行, 同步）象限，`mpsc::channel` 位于（私有, 并行, 异步）象限。Rust 的所有权规则等价于该空间中的一条约束：共享象限中的点若涉及可变访问，必须携带同步原语类型。
...
本节聚焦「定理推理链」，覆盖定理一致性矩阵（执行模型同构性专集）。论述顺序由定义到边界：先明确「定理推理链」在「Rust 执行模型同构性矩阵：同步 · 异步 · 并发 · 并行」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「定理推理链」的判定标准，并指出它在全页论证链中的位置。
