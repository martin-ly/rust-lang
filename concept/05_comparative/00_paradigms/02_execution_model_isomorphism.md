...
> **前置概念**: [Rust vs Java](../02_managed_languages/01_rust_vs_java.md)
...
执行模型同构（execution model isomorphism）研究「不同语言的执行机制是否本质相同」。本节建立方法论与权威锚点：

- **同构的判定标准**：两个执行模型同构当且仅当存在双向翻译，保持「步数上界」（复杂度类）与「可观察行为」（I/O 等价）——弱于语法相似、强于图灵等价；
- **核心参照**：Reynolds 关于「定义性解释器」（definitional interpreter）的工作——语言的执行模型由其元语言解释器刻画；Danvy 的「CPS 变换与抽象机」对应（CEK/SECD 机器即 CPS 程序的 defunctionalization）；
- **Rust 相关映射**：`async`/await 的状态机编译 = CPS 变换的 defunctionalization 实例；迭代器（Iterator）融合 = 短路融合（shortcut fusion，GHC 的 foldr/build 规则的 Rust 对应）。

后续各节按「线程模型 / 事件循环 / 协程 / Actor」四组同构对展开。
...
|:---|:---|:---|:---|:---|:---|:---|
| **OS 线程** | 抢占式（OS 内核） | 共享内存 + 缓存一致性（Coherence） | 共享内存 / channel | `std::thread` | `runtime.GOMAXPROCS` | CCS / 进程代数 |
...
| 切换开销 | 高（内核态） | 低（用户态） |
| 调度器 | OS 内核 | Go 运行时（Runtime）（协作+抢占） |
| 最大并发数 | ~数千 | ~数十万 |
| 阻塞影响 | 阻塞整个 OS 线程 | 运行时（Runtime）自动将 goroutine 移到其他线程 |
...
| 调度器 | 外部 executor（Tokio / smol [历史: async-std [已停止维护]]） | Go 运行时（Runtime）内置 |
...
> **形式化见**: [L4 进程代数与 Rust §二 CSP 形式骨架](../../04_formal/07_concurrency_semantics/01_process_calculi_for_rust.md)（本页保留 Rust↔Go 对比视角，形式推导以该页为准）
>
...
> [Hoare 1978](https://doi.org/10.1145/359576.359585); [Go Concurrency Patterns: Pipelines](https://go.dev/blog/pipelines); [rustvsgo.com](https://www.rustvsgo.com/)

---

## 七、Actor 模型

> **形式化见**: [L4 Actor 模型形式语义](../../04_formal/07_concurrency_semantics/03_actor_semantics.md)（本页保留 Actor vs CSP 对比视角，公理/邮箱/监督的形式语义以该页为准）
>
...
- **生命周期（Lifetimes）隔离**: Actor 状态封装在 Actor 内部，外部只能通过消息接口访问。
...
| 通信同步性 | 同步 rendezvous（或缓冲） | 异步（Async） mailbox |
...
| `SeqCst` | `memory_order_seq_cst` | 全局顺序一致性（Coherence） |
...
Go 的事件驱动是**隐式的**：`netpoller` 集成在运行时（Runtime），goroutine 的阻塞 I/O 自动被转换为事件驱动。

> **同构性评价**: Tokio 的 Reactor 与 Go 的 netpoller 在**epoll/kqueue 层面同构**——二者都基于操作系统的事件通知机制。但在**用户接口层面不同构**：Rust 要求显式 `async/await` + executor 选择，Go 将事件驱动透明化为阻塞语义。 [Tokio Runtime API 文档; Go Runtime 文档](https://docs.rs/tokio/latest/tokio/runtime/index.html)
...
> **边界条件**: 等价性在 `Pin<&mut Self>` 的内存稳定性约束下成立。若状态包含自引用（Reference）（如 `async` 块中引用了栈变量），则必须通过 `Pin` 保证状态机不会在内存中移动。破坏 `Pin` 契约（通过 `unsafe`）将导致等价性失效。 [来源: [RFC 2394](https://rust-lang.github.io/rfcs//2394-async_await.html); Danvy & Filinski, *Representing Control*, 1990]
...
思维表征体系把抽象的"执行模型"降维为可可视化、可比较的坐标：

- **执行模型维度雷达图**：以六个维度（求值严格性、并发粒度、内存管理、错误传播、可变性默认、元编程时机）对每种语言打分（0–5）。Rust 的雷达图特征：可变性默认=0（默认不可变）、内存管理=5（完全静态）、元编程时机=5（完全编译期）。C++ 与 Rust 的雷达图在"错误传播"和"可变性默认"两轴差异最大，这定量解释了为什么 C++ 程序员迁移时错误处理（Error Handling）与 `let mut` 习惯是主要摩擦点。
- **内存-计算-通信三维空间**：任何并发程序可投影到此空间——内存维（共享/私有）、计算维（顺序/并行）、通信维（同步/异步）。`Arc<Mutex<T>>` 位于（共享, 并行, 同步）象限，`mpsc::channel` 位于（私有, 并行, 异步）象限。Rust 的所有权（Ownership）规则等价于该空间中的一条约束：共享象限中的点若涉及可变访问，必须携带同步原语类型。
...
本节聚焦「定理推理链」，覆盖定理一致性（Coherence）矩阵（执行模型同构性专集）。论述顺序由定义到边界：先明确「定理推理链」在「Rust 执行模型同构性矩阵：同步 · 异步 · 并发 · 并行」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「定理推理链」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「相关概念链接（L0-L7 映射）」，覆盖L0-L7 纵向映射 与 相关概念。论述顺序由定义到边界：先明确「相关概念链接（L0-L7 映射）」在「Rust 执行模型同构性矩阵：同步 · 异步 · 并发 · 并行」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「相关概念链接（L0-L7 映射）」的判定标准，并指出它在全页论证链中的位置。
...
- [L3 异步编程](../../03_advanced/01_async/01_async.md) —— Future / Pin / async/await
- [L4 线性逻辑](../../04_formal/01_ownership_logic/01_linear_logic.md) —— 资源与通道的形式化根基
- [L5 Rust vs Go](../01_systems_languages/03_rust_vs_go.md) —— 语言级并发哲学对比
...
> **相关文件**: [范式矩阵](01_paradigm_matrix.md) · [异步](../../03_advanced/01_async/01_async.md) · [并发](../../03_advanced/00_concurrency/01_concurrency.md)

## 十、边界测试：执行模型同构的编译错误

本节把「Rust 执行模型同构性矩阵：同步 · 异步 · 并发 · 并行」的规则推到编译器与运行时的边界上逐一实测：边界测试：栈展开与 `panic = abort` 的行为差异（运行时…、边界测试：`async` 与线程的执行模型混淆（编译错误）、边界测试：绿色线程与 OS 线程的 API 混用（编译错误）、边界测试：CPS 变换与 Rust 的 `?` 运算符（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> Rust 的 async/await 是**零成本抽象（Zero-Cost Abstraction）**——Future 被编译为状态机，无运行时开销（除非使用运行时如 Tokio）。
...
> 真正的 CPS 优化：使用 trampoline（蹦床）模式——返回 "下一步" 闭包（Closures）而非调用它，外层循环执行。
...
1. **辨识档**：给出执行机制描述（如「任务在协作点让出，由运行时调度」），判定其同构族（协程/绿色线程/Actor）及在各语言中的实例；
2. **翻译档**：给出一个语言的习惯写法（如 Go 的 `select`），写出 Rust（tokio `select!`）与 C++（无直接对应时的模拟）的同构表达——检验双向翻译能力；
3. **边界档**：判定「同构声称」的真伪——如「async/await 与线程池等价」（伪：内存与调度语义不同）、「`mpsc` channel 与 Go channel 同构」（部分真：缓冲语义与 select 语义差异）。

每题附答案与对应小节回链——错题应重读同构判定的形式化标准节。
...
```mermaid
mindmap
  root((Rust 执行模型同构性矩阵 同步 异步 并发 并行))
    同步模型
      Rust 1 1 线程模型
      与 Go M N 调度的对比
    异步模型
    ...
    Actor 模型
      ...
      Actor vs CSP 的形式化区分
    内存共享模型
      Rust 与 C 11 内存模型的同构性
```

> **认知功能**: 本 mindmap 从本页「Rust 执行模型同构性矩阵 同步 异步 并发 并行」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
