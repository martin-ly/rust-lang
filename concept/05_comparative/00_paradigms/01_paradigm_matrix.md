...
- **Rust 的定位**：「带 ML 血统的系统语言」——命令式为表（可变状态、显式控制流），函数式为里（不可变默认、代数类型、闭包、模式匹配源自 ML/Haskell），OOP 提供子集（封装 + trait 多态，无继承）；
...
1. **「范式是正交的纯净分类」**：错——真实语言是范式混合体，矩阵单元格是程度而非布尔；Rust 的 `impl` 块既服务 OOP 封装又服务泛型分派，强行归类会丢失理解；
2. **「支持范式越多越好」**：多范式语言的认知成本非线性增长（C++ 的「四种方式做一件事」）——Rust 的取舍是「每范式只收编与零成本抽象兼容的子集」（无继承、无高阶类型）；
3. **「范式决定性能」**：范式影响的是「表达成本与可推理成本」，性能由编译模型决定——函数式风格的迭代器在 Rust 中编译为与手写循环等价的机器码（零成本抽象的反例检验）。
...
形式化谱系把语言按"语义被形式化的程度"排列：从仅有自然语言参考（Python 早期）到全机器验证语义（CompCert C、CakeML）。Rust 位于中间偏上——RustBelt 形式化了核心 λRust 演算，Ferrocene 提供了安全关键场景的语义规范，但完整语言（含宏、const 泛型）尚无机器验证语义。

下表的六语言扩展对比沿四个轴：语义形式化程度、类型系统强度（能否表达线性资源）、并发模型、运行时开销。阅读要点：Rust 是唯一在"无 GC + 静态内存安全 + 零成本抽象"三轴同时打满的语言，代价是编译期复杂度（借用检查的学习成本）。对比 C++：表达力相当但安全靠纪律；对比 Haskell：纯度换推理能力，Rust 用所有权换可变性下的安全。
...
2. **面向对象 vs trait 的封装差异**：C++ 中子类可访问父类 protected 成员形成隐式耦合；Rust 无继承，`pub(crate)` + trait 默认方法把可见性收敛到 crate 边界，试图跨模块访问私有字段是 E0616/E0624。
...
1. **类型系统同源**：代数数据类型、模式匹配、类型类（typeclass ⟷ trait）——Haskell 的类型类更纯（无对象安全限制、支持高阶），Rust 的 trait 更工程（对象安全子集、孤儿规则）；
