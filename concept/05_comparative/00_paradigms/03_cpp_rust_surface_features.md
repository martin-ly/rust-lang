...
构造与初始化是 C++ 与 Rust 表面语法差异最大、语义差异也最深的主题之一：C++ 的构造函数是特殊成员函数，支持重载、初始化列表与隐式转换；Rust 没有构造函数概念，以结构体（Struct）字面量 + 普通关联函数（惯例命名 `new`）取而代之。

本节对比的三个切入点：

| 切入点 | C++ | Rust | 关键差异 |
|---|---|---|---|
| 语法形式 | 构造函数 + 初始化列表 | 结构体字面量 / `new()` 关联函数 | Rust 的构造是显式表达式，无隐式调用点 |
| 转换语义 | 单参数构造函数默认是隐式转换 | `From`/`Into` trait 必须显式调用 | C++ 需 `explicit` 标注防御，Rust 默认安全 |
| 部分初始化 | 成员可在构造函数体内赋值 | 结构体必须整体构造（除 `MaybeUninit`） | Rust 消除了“半构造对象”状态 |

2.3 的对比表给出逐条语法对照；阅读时特别注意 C++ 初始化列表的“声明顺序初始化”陷阱在 Rust 中不存在对应物——因为 Rust 要求字段在字面量中全量给出。
...
C++ 的 `friend` 与 Rust 的模块（Module）可见性解决的是同一个问题——“谁可以访问非公开成员”——但授权方向相反：`friend` 由被访问者**点名授权**外部实体，Rust 的可见性由模块层级**结构性决定**。本节对比这一方向性差异的工程后果。

三个层面的对照：

1. **C++ `friend`**：类可以声明某个函数或类为友元，突破 `private` 边界；授权是双向不可传递的（A 是 B 的友元不意味着 B 能访问 A），但无法限制友元访问的范围（全部私有成员开放）；
2. **Rust 模块可见性**：`pub`、`pub(crate)`、`pub(super)`、`pub(in path)` 构成层级化的可见性梯度，私有项默认对**当前模块及其子模块**可见——“友元”角色由共同的父模块隐式承担；
3. **对比**：C++ 的友元是点对点白名单，Rust 的可见性是树状授权；前者灵活但难以审计（友元声明散落各处），后者结构性可推导（`pub` 修饰即完整声明可达范围）。

迁移提示：C++ 中用 `friend` 暴露给测试代码的私有成员，在 Rust 中的惯用法是 `#[cfg(test)]` 子模块——测试模块作为子模块天然可见父模块私有项。
...
**反例：C++ 迭代器（Iterator）失效的 Rust 对应** —— C++ 中遍历 `vector` 时 `push_back` 是未定义行为（迭代器失效），Rust 在编译期直接拒绝。
...
fn main() {
    let mut v = vec![1, 2, 3];
    for x in &v {
        v.push(*x); // C++: 迭代器失效 UB；Rust: 编译期拒绝
    }
}
```
...
**陷阱要点**：同一表面写法，C++ 靠程序员纪律规避 UB，Rust 把「迭代失效」转化为借用（Borrowing）冲突（E0502）在编译期消灭——这是两语言安全哲学的代表性分野。
