...
构造与初始化是 C++ 与 Rust 表面语法差异最大、语义差异也最深的主题之一：C++ 的构造函数是特殊成员函数，支持重载、初始化列表与隐式转换；Rust 没有构造函数概念，以结构体字面量 + 普通关联函数（惯例命名 `new`）取而代之。
...
C++ 的 `friend` 与 Rust 的模块可见性解决的是同一个问题——“谁可以访问非公开成员”——但授权方向相反：`friend` 由被访问者**点名授权**外部实体，Rust 的可见性由模块层级**结构性决定**。本节对比这一方向性差异的工程后果。
...
**反例：C++ 迭代器失效的 Rust 对应** —— C++ 中遍历 `vector` 时 `push_back` 是未定义行为（迭代器失效），Rust 在编译期直接拒绝。
...
**陷阱要点**：同一表面写法，C++ 靠程序员纪律规避 UB，Rust 把「迭代失效」转化为借用冲突（E0502）在编译期消灭——这是两语言安全哲学的代表性分野。
