...
> **前置概念**: [Rust vs C++](../01_systems_languages/01_rust_vs_cpp.md) · [Rust vs Go](../01_systems_languages/02_rust_vs_go.md) · [Type Theory](../../04_formal/00_type_theory/02_type_theory.md)
...
「反命题与边界分析」部分按反命题: "Rust 是系统编程的最优解"、反命题: "Rust 是纯函数式语言"、反命题: "多范式总是好的"与反命题: "类型系统限制表达力"的顺序逐层展开。
...
本节围绕「扩展内容：形式化谱系与更多语言对比」展开，覆盖编程语言形式化谱系 与 扩展对比矩阵（6 语言） 两个方面。
...
| Rust vs Go | [`./02_rust_vs_go.md`](../01_systems_languages/02_rust_vs_go.md) | 并发对比 |
| 线性逻辑 | [`../04_formal/01_linear_logic.md`](../../04_formal/01_ownership_logic/01_linear_logic.md) | 形式化根基 |
| 类型论 | [`../04_formal/02_type_theory.md`](../../04_formal/00_type_theory/02_type_theory.md) | 类型系统谱系 |
| RustBelt | [`../04_formal/04_rustbelt.md`](../../04_formal/02_separation_logic/04_rustbelt.md) | 验证能力 |
| 语言演进 | [`../07_future/03_evolution.md`](../../07_future/04_research_and_experimental/03_evolution.md) | 演进方向 |
| 安全边界 | [`./04_safety_boundaries.md`](../03_domain_comparisons/04_safety_boundaries.md) | 能力边界 |
...
> **相关文件**: [范式转换矩阵](../../00_meta/00_framework/paradigm_transition_matrix.md) · [执行模型同构](05_execution_model_isomorphism.md) · [类型论](../../04_formal/00_type_theory/02_type_theory.md)

## 十、边界测试：范式矩阵中的编译错误

「边界测试：范式矩阵中的编译错误」部分按边界测试：命令式与函数式的边界（编译错误）、边界测试：面向对象与 trait 的封装差异（编译错误）、边界测试：多范式代码的 trait bound 爆炸（编译错误）、边界测试：函数式与命令式的内存布局冲突（逻辑错误）等6个方面的顺序逐层展开。
...
本节从内存管理 与 空指针 两个层面剖析「Rust vs C/C++」。
...
「Rust vs Haskell」部分包含纯度 与 高阶类型 两条主线，本节依次说明。
