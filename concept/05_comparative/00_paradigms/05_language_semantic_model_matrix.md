...
本节从六个语义模型维度中抽取内存安全、类型系统、效应与并发、验证生态四个关键切片，用一维谱系图展示语言的聚集模式，并提炼出影响技术选型的核心洞察。
...
本节整理构建矩阵所依赖的学术来源、官方文档、项目内部相关页与知识来源关系表，为各维度的评级与论断提供可追溯的证据链。
...
> 本页对应 [`concept/00_meta/00_framework/semantic_space.md`](../../00_meta/00_framework/semantic_space.md) 的 **§6 Rust 表征空间与其他语言的对比**：
>
> - 本矩阵的六个维度（内存安全模型、类型系统表达力、效应系统、并发模型、验证生态、编译目标灵活性）是语义空间 §6 的细化展开。
> - Rust 在“类型级无数据竞争”“native+WASM+embedded+GPU”“关键字效应”等单元中的定位，直接对应语义空间 §3 “能且高效表达”的 sweet spot。
> - 语言之间的语义边界差异（如 GC vs 所有权、Monad vs 关键字效应）可通过语义空间的“等价表达谱系”进一步形式化。
