...
Scala 保留完整的 OOP（继承、多态、可变状态），函数式是可选风格。Rust 强制所有权和不可变引用（Mutable Reference），函数式是默认，OOP 通过 trait 实现且不支持继承。
</details>
...
Rust 要求函数签名通常显式标注类型（生命周期（Lifetimes）和泛型（Generics））。Scala 支持更激进的局部类型推断（如 Hindley-Milner 扩展）。Rust 的保守性是为了编译速度和明确的接口契约。
</details>
