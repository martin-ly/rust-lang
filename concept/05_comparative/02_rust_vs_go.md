...
> **主要来源**: [The Rust Programming Language](https://doc.rust-lang.org/book/) · [Effective Go] · [Wikipedia: Communicating sequential processes](https://en.wikipedia.org/wiki/Communicating_sequential_processes) · [Wikipedia: Go](https://en.wikipedia.org/wiki/Go)
...
> **[The Rust Programming Language](https://doc.rust-lang.org/book/)** Rust's central feature is ownership — a set of rules checked by the compiler that govern how a Rust program manages memory.
...
> **反例分析**: Go 的接口在每次调用时需要通过 `iface` 或 `eface` 结构进行 itab 查找。在热路径（如排序比较器、序列化接口）中，这种运行时间接开销不可忽视。Rust 的 trait 单态化在编译期将泛型函数特化为具体类型，调用点完全内联，实现零成本抽象。但若需要动态分发（`dyn Trait`），Rust 同样有一次虚表指针开销，只是总体可控。 来源: Go Data Structures: Interfaces (Russ Cox) / [The Rust Programming Language](https://doc.rust-lang.org/book/)
...
> - 编译期数据竞争保证：只有 Rust 的 Send/Sync 能在编译期消除数据竞争；Go 依赖 `go test -race` 等运行时检测 来源: [The Rust Programming Language](https://doc.rust-lang.org/book/) / Go Memory Model
...
> **结论**: 在每秒百万次调用的热路径（如序列化、日志、比较器）中，Go 的接口间接开销可能成为瓶颈。Rust 的零成本抽象在此类场景具有显著优势。但若调用频率不高，Go 接口带来的灵活性和可测试性更具工程价值。 来源: Go Data Structures: Interfaces (Russ Cox, 2009) / [The Rust Programming Language](https://doc.rust-lang.org/book/)
...
> **关键机制**: `move` 闭包将 `tx` 的所有权转移到新线程；`send(data)` 将 `String` 的所有权转移到 channel；`recv()` 将所有权转移到接收端。整个过程中编译器通过 `Send` trait 验证跨线程传递的合法性。 来源: [TRPL Ch16](https://doc.rust-lang.org/book/ch16-00-concurrency.html)
...
> **关键差异**: Rust 的 `?` 使错误传播成为"默认路径"，Go 需要显式 `if err != nil`。Go 1.18+ 的泛型未改变错误处理范式。 来源: [The Rust Programming Language](https://doc.rust-lang.org/book/) / Effective Go
...
> **关键差异**: Rust 的文档测试深度集成到文档生成（`rustdoc`），代码示例不编译通过即阻止发布。Go 的 Example 测试较弱，输出匹配而非断言。 来源: [TRPL Ch14](https://doc.rust-lang.org/book/)
...
| **论断** | **来源** | **可信度** |
|:---|:---|:---|
| Rust 所有权编译期保证安全 | [The Rust Programming Language](https://doc.rust-lang.org/book/) · [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/) | ✅ |
...
| Rust 无 GC 有确定性释放 | [The Rust Programming Language](https://doc.rust-lang.org/book/) | ✅ |
...
| Send/Sync 消除数据竞争 | [The Rust Programming Language](https://doc.rust-lang.org/book/) · [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/) | ✅ |
