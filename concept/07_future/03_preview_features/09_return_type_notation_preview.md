...
本节将「边界测试：Return Type Notation 预览的…」分解为若干主题：边界测试：RTN 在类型位置使用（编译错误）、边界测试：RTN 用于非 AFIT/RPITIT 方法（编译错误）、边界测试：RTN 与 const/type 泛型方法（编译错误）、边界测试：RTN 与缺少 `feature(return_type_n…等5个方面。
