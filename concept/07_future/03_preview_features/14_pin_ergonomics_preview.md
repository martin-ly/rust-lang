...
理解「解决方案 1：Reborrow Traits（已推进）」需要把握问题：`Pin<&mut T>` 不能自动 reborrow、`Reborrow` 和 `CoerceShared` Traits与预期效果，本节依次展开。
...
本节将「解决方案 2：Pinned Places（RFC 讨论中）」分解为若干主题：核心想法：`pin` 关键字、Pinned Fields与Pinned Drop。
...
本节从反命题树 与 关键边界 两个层面剖析「反命题与边界分析」。
