...
> **前置概念**: [Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md) · [Version Tracking](../00_version_tracking/05_rust_version_tracking.md)
...
「核心概念」部分按 Rust 编译器架构回顾、前端瓶颈：单线程限制与并行前端的设计目标的顺序逐层展开。
...
「技术方案」部分按查询系统的并行化、类型检查的并行化与与增量编译的协同的顺序逐层展开。
