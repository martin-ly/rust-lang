...
> **前置概念**: [Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md) · [Formal Methods](../04_research_and_experimental/02_formal_methods.md) · [MC/DC Coverage](02_mcdc_coverage_preview.md)
> **后置概念**: [Version Tracking](../00_version_tracking/01_rust_version_tracking.md)
> **定理链**: N/A — 描述性/综述性/导航性文档，不涉及形式化定理链
>
> **来源**: [Ferrocene Project](https://ferrocene.dev/) · [Ferrocene Specification](https://spec.ferrocene.dev/) · [ISO 26262](https://www.iso.org/standard/68383.html) · [Inside Rust Blog](https://blog.rust-lang.org/inside-rust/) · [Brown University — Interactive Rust Book](https://rust-book.cs.brown.edu/)
---
...
本节从安全关键软件的认证挑战、Ferrocene 的定位与认证范围与限制切入，剖析「核心概念」的核心内容。
...
本节从认证工具链的构成、与上游 Rust 的关系与证据包与审计追踪切入，剖析「技术细节」的核心内容。
...
本节将「边界测试：Ferrocene 预览的编译错误」分解为若干主题：边界测试：安全关键子集的 unsafe 禁止（编译错误）、边界测试：确定性执行与 `HashMap` 的禁用（编译错误）、边界测试：Ferrocene 子集与标准库的不完全覆盖（编译错误）、边界测试：Ferrocene 的确定性执行与浮点数（逻辑错误）等5个方面。
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：Ferrocene 是什么？它与标准 Rust 编译器有什么…、测验 2：为什么航空航天和汽车领域需要"认证"的编译器？（理解层）、测验 3：Ferrocene 的认证范围包括哪些组件？（理解层）、测验 4：Ferrocene 对 Rust 进入安全关键市场有什么意义…等5个方面，本节依次展开。
