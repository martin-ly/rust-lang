...
> **前置概念**: [Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md) · [Version Tracking](../00_version_tracking/01_rust_version_tracking.md)
...
- [并行 前端编译预研：Rust 编译器 的多核扩展](#并行-前端编译预研rust-编译器-的多核扩展)
  ...
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    ...
    - [测验 3：为什么借用检查器（Borrow Checker）的并行化特别困难？（理解层）](#测验-3为什么借用检查器borrow-checker的并行化特别困难理解层)
...
并行前端的背景需从编译器架构说起：rustc 的查询系统（query system）自 2018 年起以「需求驱动的增量编译」组织——类型检查、借用检查等被建模为带依赖图的查询（`tcx.type_of(def_id)`），单线程执行器按拓扑序求值。

**前端瓶颈**：单线程意味着 16 核机器上「类型检查 → MIR 生成」段只用 1 核；对大型 crate（如 `rustc` 自身、Bevy），该段占 debug 构建 40–60% 时间——而后端代码生成早已并行（codegen units）。

设计目标三条：

1. **保持查询系统语义**：并行化对 crate 作者透明，产出与单线程逐位一致（确定性是硬约束，见边界测试）；
2. **数据竞争自由**：rustc 内部大量使用 arena 分配与 `RefCell` 等价物，并行化要求重构为 `Send`-安全的并发结构（`rustc_data_structures` 的 `Lock`/`WorkerLocal`）；
3. **可回退**：`-Zthreads=N` 开关，默认单线程直到确定性验证完成。

判定依据：该特性受益面 = 单 crate 体量大且依赖浅的项目；多 crate workspace 本就有 crate 级并行，收益递减。
...
1. **查询系统的并行化**：查询引擎改为工作窃取（work-stealing）调度——每个查询是任务，依赖边即同步点；关键不变量是**无死锁调度**：查询间的循环依赖（如递归类型定义）需由专门的循环检测机制处理，并行版必须保持与串行版相同的循环错误诊断。
2. **类型检查的并行化**：函数体级并行——不同 `DefId` 的类型检查天然独立；单个大函数内部的推断仍串行（约束求解的顺序性），这决定了加速比上限（Amdahl 定律：大单体函数是并行化的漏网之鱼）。
3. **与增量编译的协同**：增量指纹（fingerprint）计算必须对执行顺序不敏感——并行化暴露了若干「结果依赖求值顺序」的既有 bug（哈希序泄漏），修复这些 bug 本身是项目的重要副产品。

判定依据：`-Zthreads` 实验构建与串行构建产物 diff（目标文件哈希）是验证确定性的标准做法。
...
> **注意**: 小型 crate（<10 模块（Module））的并行化收益可能为负，因为线程创建和同步开销超过并行收益。
...
反命题：「crate 级并行已足够，前端并行化 ROI 低」——边界分析：

1. **适用面确实受限**：小型项目（单 crate <10 万行）前端占比低，并行收益 <10%；收益集中在巨型单 crate（编译器自身、游戏引擎、单体式 web 后端）——这类项目恰恰是「编译时间最痛」的群体，ROI 集中在头部但头部痛感最强。
2. **复杂度的隐性成本**：查询引擎并行化使 rustc 内部并发原语成为新的 bug 源（确定性违规、罕见死锁），这些 bug 难以复现——社区需接受一段不稳定期。
3. **与 Cranelift 的关系**：两者加速不同编译段（前端 vs 后端），叠加收益近线性；单独看任一项都不够颠覆，组合才是 debug 体验质变路径。

判定依据：评估收益用「本 crate 前端耗时 / 总构建时间」比值（`-Ztime-passes` 可测）；>40% 才值得跟踪该特性。
...
> **认知功能**: 此决策树帮助项目评估是否应启用并行前端。核心判断标准是项目规模和模块（Module）数量。
...
| 类型检查并行化 | 🟡 开发中 | 2026 | 模块（Module）级并行类型检查 |
| 借用（Borrowing）检查并行化 | ⬜ | 2026-2027 | MIR borrow check 并行 |
...
并行前端的边界测试针对**确定性**而非正确性：同一输入在单线程与 `-Zthreads=N` 下必须产生一致的诊断集合与编译产物。用例按失败来源分类：

| 用例 | 风险来源 | 检测手段 |
|:---|:---|:---|
| 宏展开顺序 | 宏依赖全局状态（计数器、env） | 单/多线程结果 diff |
| 增量编译不一致 | query 系统缓存与并行的交互 | 二次 `cargo check` diff |
| 确定性输出 | 代码生成阶段的调度顺序 | 二进制/元数据哈希对比 |
| 宏顺序依赖 | `macro_rules!` 的定义可见性 | 并行编译报错 diff |

判定原则：并行前端只承诺加速、不承诺容忍非确定性宏；依赖展开顺序的宏属于既有 bug，并行只是放大器。
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：Rust 编译器前端目前为什么是单线程的？这带来了什么瓶颈？…、测验 2："并行前端"（Parallel Frontend）提案的核心…、测验 3：为什么借用检查器（Borrow Checker）的并行化特别…、测验 4：并行前端对日常 Rust 开发者的编译体验有什么预期改善？（…等5个方面的顺序逐层展开。
...
前端涉及名称解析、类型检查、借用（Borrowing）检查等阶段，这些阶段共享可变状态（如类型表、解析表），并行化困难。瓶颈是大 crate 的编译时间长，无法利用多核。
</details>
...
将类型检查、借用（Borrowing）检查等前端阶段并行化，减少大 crate 的编译时间。通过细粒度锁或无锁数据结构共享编译器状态。
</details>
...
借用检查需要全局分析数据流和生命周期（Lifetimes）依赖，这些依赖关系在函数间甚至模块（Module）间传播。并行化需要确保分析结果的一致性（Coherence）和完整性，避免错误地接受不安全代码。
</details>
