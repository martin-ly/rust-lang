...
> **前置概念**: [Async](../../03_advanced/01_async/02_async.md) · [Traits/Iterators](../../02_intermediate/00_traits/01_traits.md) · [Type System](../../01_foundation/02_type_system/04_type_system.md)
...
「核心概念」部分按从 async 到 gen 的泛化、Gen Blocks 语法与语义与与现有迭代器生态的关系的顺序逐层展开。
...
「技术细节」涉及生成器状态机、与 Stream 的协同与与异步生成器的对比，本节逐一说明其要点。
...
- [Async](../../03_advanced/01_async/02_async.md) — 异步编程与 Future
...
「边界测试：Gen Blocks 预览的编译错误」涉及边界测试：`gen` 块与 `Iterator` trait 的自动实…、边界测试：`gen` 块与借用生命周期的冲突（编译错误）、边界测试：gen 块与 `Pin` 的隐式需求（编译错误）、边界测试：gen 块与异常控制流（`return`、`break`）的…等5个方面，本节逐一说明其要点。
