...
本节从自动微分（AD）是什么 与  Rust 为什么需要语言级 AD 两个层面剖析「核心概念」。
...
理解「`std::autodiff` 设计预览」需要把握语法构想（基于 Project Goals 2026）、前向模式 vs 反向模式与与现有生态的对比，本节依次展开。
...
本节围绕「技术挑战与状态」展开，覆盖当前状态（2026-06） 与 核心挑战 两个方面。
...
本节围绕「边界与反命题」展开，覆盖反命题树 与 边界极限 两个方面。
