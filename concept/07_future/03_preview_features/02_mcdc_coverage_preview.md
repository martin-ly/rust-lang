...
本节将「核心概念」分解为若干主题：覆盖率等级的层次结构、MC/DC 的数学定义与Rust 编译器实现路径。
...
本节围绕「形式化语义」展开，覆盖独立影响的形式化 与 与编译器优化的冲突 两个方面。
...
本节围绕「边界测试：MCDC 覆盖率预览的编译错误」展开，依次讨论边界测试：MCDC 测试的条件分解（编译错误/逻辑错误）、边界测试：覆盖率检测的编译器标志冲突（编译错误）、边界测试：MCDC 与短路求值的复杂交互（逻辑错误）、边界测试：覆盖率工具的 LLVM IR 级别插桩（编译错误/性能下降）等5个方面。
