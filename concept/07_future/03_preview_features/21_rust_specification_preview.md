...
本节围绕「技术细节」展开，依次讨论规范的内容层次、与形式化验证的衔接与维护挑战。
...
「边界测试：Rust 规范预览的编译错误」部分按边界测试：规范未定义行为的边界（编译错误/运行时 UB）、边界测试：规范与编译器实现的差异（编译错误）、边界测试：规范与编译器实现的临时不一致（编译错误）、边界测试：规范草案与编译器实现的不一致（编译行为漂移）等6个方面的顺序逐层展开。
