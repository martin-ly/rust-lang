...
「技术细节」涉及语法与语义、不变量契约与与 Safety Tags 的协同，本节逐一说明其要点。
...
本节将「边界测试：Unsafe Fields 预览的编译错误」分解为若干主题：边界测试：unsafe 字段的显式访问要求（编译错误）、边界测试：unsafe 字段与 Drop 的交互（运行时 UB）、边界测试：unsafe 字段与 `#[repr(C)]` 的交互（编译…、边界测试：unsafe 字段与不变式的文档化（逻辑错误）等5个方面。
