...
本节从从固定向量到可伸缩向量、SVE 核心机制：谓词（Predicate）与SME：从向量到矩阵切入，剖析「核心概念」的核心内容。
...
本节将「Rust 支持现状（截至 2026-06-06）」分解为若干主题： Feature Gate、跟踪 Issue、RFC 状态、Project Goal 历史等5个方面。
...
理解「技术挑战」需要把握 Rust 类型系统适配、ABI 与调用约定与SME 的额外复杂度，本节依次展开。
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：SVE（Scalable Vector Extension）…、测验 2：SVE/SME 与 x86 的 AVX-512 有什么区别？…、测验 3：Rust 目前对 SVE/SME 的支持状态如何？（理解层）、测验 4：SVE/SME 对 Rust 的高性能计算（HPC）和 ML…等5个方面，本节逐一说明其要点。
