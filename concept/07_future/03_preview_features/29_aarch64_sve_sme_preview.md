...
> **前置概念**:
>
> [SIMD / 向量化](../../01_foundation/00_start/02_zero_cost_abstractions.md) ·
...
1. **从固定向量到可伸缩向量**：NEON 的 128 位固定宽度让位于 SVE 的 128–2048 位可伸缩宽度，代码必须写成“对任意向量长度正确”的形式——循环边界、掩码操作都不能假设宽度；
2. **谓词（Predicate）**：SVE 用谓词寄存器替代传统的分支处理尾循环，一条指令可同时对向量的部分通道生效——这是“宽度无关”编程的关键机制；
3. **SME：从向量到矩阵**：SME 在 SVE 之上增加矩阵外积累加（outer product accumulate），面向 AI 推理的 GEMM 类负载，其 ZA 存储阵列引入了新的寄存器状态与上下文切换成本。

理解顺序不可颠倒：可伸缩概念不消化，谓词机制会显得多余；谓词不理解，SME 的流式模式无从谈起。
...
截至 2026-06，Rust 对 SVE/SME 的支持处于“编译器基础设施就绪、语言层面待设计”的阶段。本节现状梳理回答一个实际问题：现在能不能在 Rust 中用上 SVE？

现状的五个维度：

- **Feature Gate**：`stdarch_aarch64_sve` 等 gate 控制 intrinsic 的可用性，全部 nightly-only；
- **跟踪 Issue**：`stdarch` 仓库的 SVE tracking issue 汇总了 intrinsic 实现进度（大部分 SVE2 指令已有 intrinsic）；
- **RFC 状态**：可伸缩向量类型（`#[repr(scalable)]` 或等价机制）的 RFC 尚在草案——这是 ABI 稳定化的前置条件；
- **Project Goal 历史**：SVE 支持曾是 2024 项目目标的子项，后因 ABI 设计争议降级为持续跟踪项；
- **已实现基础设施**：LLVM 后端对 SVE/SME 的代码生成已成熟，瓶颈完全在 Rust 语言层。

结论：intrinsic 路径（unsafe、nightly）已可用于实验，可移植 SIMD（`std::simd`）与稳定 ABI 路径仍需等待 RFC 落地。
...
> **后置概念**: [SIMD / 向量化](../../01_foundation/00_start/02_zero_cost_abstractions.md) · [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) · [Rust for Linux](../04_research_and_experimental/04_rust_for_linux.md)
