...
- v1.3 (2026-05-22): 网络权威内容对齐 Batch 9：补充 KVerus、AutoVerus、Vest、Rustlantis 等 2025–2026 新兴工具到定理一致性矩阵；添加与 concept/04_formal/05_verification_toolchain.md 交叉引用
...
RustBelt 属于第 4 层，证明了 Rust 类型系统本身的 soundness。
</details>
...
- A. 所有 Rust 代码（包括任意 unsafe）都是内存安全的
- B. Safe Rust 子集是内存安全的，且满足契约的 unsafe 代码不破坏该保证
- C. Rust 编译器没有 bug
...
一旦获得 `NonZeroI32`，编译器就"知道"它非零，因此 `divisor: NonZeroI32` 的除法无需运行时检查。
