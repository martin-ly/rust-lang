...
> **Summary**: 传统 SIMD（如 SSE/AVX/NEON）使用**固定宽度**向量寄存器（128-bit、256-bit、512-bit）。SVE 引入了**可伸缩向量长度（VL）**： | 特性 | NEON（固定） | SVE（可伸缩） | |:---|:---|:---| | 向量长度 | 固定 128-bit | 128-bit 到 2048-bit（以 128-bit 为增量） | | 运行时确定 | ❌ 编译期固定 | ✅ 硬件决定，运行时暴露 | | 代码可移植性 | 同一二进制无法跨硬件 | 同一代码可在不同 VL 硬件运行 | | 循环向量化 | 需手动处理剩余元素 | 通过 `WHIL
...
> **后置概念**: [SIMD / 向量化](../01_foundation/06_zero_cost_abstractions.md) · [Unsafe Rust](../03_advanced/03_unsafe.md) · [Rust for Linux](../06_ecosystem/19_rust_for_linux.md)
