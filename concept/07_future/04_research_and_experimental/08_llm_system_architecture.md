...
| 反例命题 | 为什么错 | 工程边界 |
|---|---|---|
| "更大的模型 = 更好的系统" | 70B 模型在延迟、成本、部署复杂度上可能劣于 7B + 高质量 RAG | 系统质量取决于契约、检索、失败处理与反馈闭环的整体设计，而非单点模型规模 |
| "RAG 可以完全消除幻觉" | 检索内容错误、embedding 领域不匹配或模型忽视上下文时仍会输出错误 | RAG 是**风险缓解**而非**风险消除**，需配合事实性评估与人工审核 |
| "LLM 可以替代所有验证" | 模型可能生成格式正确但逻辑错误的代码或数据 | 必须保留类型/schema/单元测试/人工验证层，把不确定性限制在 `Option<T>`/`Result` 内 |
| "Agent 越自主越好" | 无审计边界的 Agent 可能无限循环、成本失控或调用危险工具 | 自主性应与可解释性、可控性、步数/预算上限同步增长 |
| "向量数据库是 RAG 的唯一选择" | 小规模、结构化或关键词密集型知识用 Elasticsearch / SQLite FTS 更简单高效 | 按数据规模、查询类型和延迟要求选型，避免过度工程 |

> 判定依据：这些反例划定了 LLM 系统架构中"模型能力"与"系统可靠性"之间的边界。单点优化不能替代系统工程。
