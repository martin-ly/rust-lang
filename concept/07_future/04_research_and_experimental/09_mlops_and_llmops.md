...
| 反例命题 | 为什么错 | 工程边界 |
|---|---|---|
| "部署模型就是项目结束" | 生产后会出现数据漂移、性能退化、schema 变更和成本泄露 | 部署只是生命周期中点，监控与再训练闭环才是核心 |
| "监控可有可无" | 无监控则无法发现静默退化，问题往往在业务指标下跌后才暴露 | 应用层、模型层、数据层指标是反馈闭环的基础 |
| "Prompt 工程可以无限提升性能" | 提示优化的边际收益递减，且跨模型/跨版本一致性差 | 需与微调、RAG、评估基准和 prompt 版本化协同 |
| "模型版本越多越好" | 版本过多会增加评估、回滚、管理和合规成本 | 版本管理的目标是**可追溯**与**可控**，不是数量最大化 |
| "LLMOps 只是 MLOps 加 LLM" | LLMOps 还要管理 prompt、检索上下文、开放式输出评估和人类反馈循环 | LLMOps 是 MLOps 的扩展，但有独立的实践集合 |

> 判定依据：MLOps/LLMOps 的可靠性来自持续迭代与分层治理，而非单次部署或单一优化手段。
