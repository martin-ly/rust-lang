...
> **内容重叠提示**: 本文与 [`docs/06_research/08_rust_for_linux.md`](../../../docs/06_research/08_rust_for_linux.md) 内容高度重叠。`docs/` 版本已改为重定向页；`concept/` 版本为项目权威主轨。
> **代码状态**: [示例级 — 已补充代码]
>
...
> **前置概念**: [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) · [FFI](../../03_advanced/04_ffi/01_rust_ffi.md) · [Cross Compilation](../../06_ecosystem/05_systems_and_embedded/02_cross_compilation.md)
> **后置概念**: [Formal Methods](../../04_formal/02_separation_logic/01_rustbelt.md) · [Evolution](03_evolution.md)
...
- [Rust for Linux ：操作系统内核中的内存安全（Memory Safety）](04_rust_for_linux.md)
  - [📑 目录](#-目录)
  - [一、核心概念](#一核心概念)
...
### 3.3 历史里程碑总览（自 `docs/06_research/08_rust_for_linux.md` 合并）
...
### 3.4 生产级驱动案例（自 `docs/06_research/08_rust_for_linux.md` 合并）

本节围绕「生产级驱动案例（自 `docs/04_research/0…」展开，依次讨论 Android Binder IPC、Apple GPU (Asahi)与NVMe 基础设施。
...
- **[Safety Tags](../03_preview_features/03_safety_tags_preview.md)**：内核中存在大量 `unsafe` 边界，Safety Tags（RFC #3842）可将 `# Safety` 文档注释转化为机器可读契约，帮助内核维护者审查 `unsafe` 调用点。
- **[BorrowSanitizer](../../04_formal/02_separation_logic/04_borrow_sanitizer_in_formal.md)**：在 Rust/C 混合代码中检测别名模型违规，补充 Miri 无法覆盖的生产环境。
- **[Tree Borrows](../../04_formal/01_ownership_logic/05_tree_borrows_deep_dive.md)**：相比 Stacked Borrows 更适合内核中常见的复杂借用（Borrowing）模式，已被 Miri 支持。
- **[AutoVerus / Verus](../../04_formal/04_model_checking/07_autoverus.md)**：未来可用于验证内核抽象层（如 `kernel::sync`）的功能正确性。

> **趋势判断**：内核代码对工具链的可审查性要求极高，Safety Tags + BorrowSanitizer 的组合有望成为 Rust for Linux 进入核心子系统前的标准流程。

### 4.2 与 Ferrocene / 安全关键系统的关联（自 `docs/06_research/08_rust_for_linux.md` 合并）
...
反命题树：「Rust for Linux 证明 Rust 将取代内核 C」——需要三条边界修正：

1. **增量而非取代**：R4L 的官方定位是新驱动/子系统的**可选**语言；核心调度器、MM 等路径无重写计划——C 与 Rust 将长期共存，FFI 边界（`kernel` crate）是永久设施而非过渡脚手架。
2. **抽象覆盖率的硬边界**：`kernel` crate 的 safe 抽象只覆盖常用子系统；触及未覆盖子系统时，驱动作者必须自写 unsafe 封装——此时「Rust 安全性」退化为「作者封装的正确性」，审计要求不降反升。
3. **维护者生态边界**：内核社区 Rust 知识密度低，review 带宽是合入瓶颈（2024 年维护者辞职事件的本质）；技术上可行 ≠ 组织上可维护。

**边界极限**：R4L 成功的真正标志不是合入代码量，而是「首个 C 子系统主动请求 Rust 重写」——截至 2026 尚未出现。

判定依据：评估 R4L 进度看 Asahi GPU/Binder 等生产驱动的主线合入与缺陷率数据。
...
「边界测试：Rust for Linux 的编译错误」涉及边界测试：内核模块的 `no_std` 与标准库缺失（编译错误）、边界测试：内核锁的原子顺序与 `unsafe` 封装（编译错误）、边界测试：内核模块的 `no_std` 与 `alloc` 的谨慎使用…、边界测试：内核锁的 `spinlock` 与睡眠的互斥（运行时死锁）等6个方面，本节逐一说明其要点。
