...
以下三个案例的选择标准是**代码已公开且经历过真实硬件/生产负载**，而非实验室原型：
...
- **[Safety Tags](../02_preview_features/03_safety_tags_preview.md)**：内核中存在大量 `unsafe` 边界，Safety Tags（RFC #3842）可将 `# Safety` 文档注释转化为机器可读契约，帮助内核维护者审查 `unsafe` 调用点。
...
内核环境的边界测试多一类宿主约束：**没有标准库、没有 unwinding、分配可能失败**。用例因此按"标准库依赖剥离程度"组织：
