...
理解「边界测试：AI 集成的编译错误」需要把握边界测试：ML 模型输入维度不匹配（运行时错误）、边界测试：不安全模型加载与所有权（编译错误）、边界测试：AI 生成代码的 unsafe 误用与形式化保证缺失（运行时…、边界测试：AI 生成 unsafe 代码的 Miri 验证失败（运行时…等6个方面，本节依次展开。
