...
1. **Candle（框架层）**：纯 Rust 的极简 ML 框架，张量 API 借鉴 PyTorch 但用所有权管理设备内存，适合嵌入式推理与需要精细内存控制的场景；
...
Rust AI 工程的边界测试集中在一个核心矛盾：张量计算的维度信息在运行期才确定，而 Rust 的类型系统偏好编译期已知——形状错误因此从“编译期可防”退化为“运行期 panic”，这正是测试的重点覆盖区。
