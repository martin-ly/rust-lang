...
> **前置概念**: [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) · [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) · [WebAssembly](../../06_ecosystem/11_domain_applications/03_webassembly.md)
...
Rust 在 AI 工程栈中的定位是“性能关键的推理与基础设施层”，而非训练框架——这一分工由生态成熟度决定，短期内不会改变。本节技术细节按四层栈展开，从框架到硬件目标。

四层的关系：

1. **Candle（框架层）**：纯 Rust 的极简 ML 框架，张量 API 借鉴 PyTorch 但用所有权（Ownership）管理设备内存，适合嵌入式推理与需要精细内存控制的场景；
2. **ONNX Runtime 集成（互操作层）**：以 `ort` crate 绑定微软的 ONNX Runtime，让 Rust 服务直接加载 Python 侧训练的模型——这是当前最务实的生产路径；
3. **WebAssembly 推理（部署层）**：轻量模型经 WASM 部署到浏览器与边缘节点，受限于 SIMD/线程提案的支持度；
4. **NVIDIA GPU 目标基线提升（Rust 1.97）**：`nvptx64-nvidia-cuda` target 的改进使 Rust 编写 CUDA kernel 的路径进一步收敛，虽仍需 nightly 组件，但 PTX 生成的稳定性显著提高。

四层的共同约束：自动微分生态（`dfdx` 等）尚不成熟，训练侧仍以 Python 为主、Rust 承接推理侧。
...
“Rust 将取代 Python 成为 AI 主力语言”与“Rust 在 AI 领域没有机会”是同一光谱的两端，都与 2026 年的生态现实不符。本节反命题树把争论锚定在可验证事实上。

检验维度：

- **反命题树**：把“取代论”分解为训练框架、自动微分、生态惯性三个子命题——三者当前全部不成立（CUDA 绑定深度、autograd 成熟度、人才存量都在 Python 侧）；把“没有机会论”分解为推理延迟、内存效率、边缘部署三个子命题——三者全部成立（Candle/ONNX 路径在生产中已有可量化收益）；
- **边界极限**：构造两个端点场景——大模型训练集群（Rust 无优势）与微秒级延迟的嵌入式推理（Rust 优势最大化），中间地带的判定规则是“模型是否已冻结 + 部署目标是否有资源约束”。

结论：Rust in AI 的正确问题不是“能否取代”，而是“推理管线的哪些环节值得重写”。
...
- [WebAssembly](../../06_ecosystem/11_domain_applications/03_webassembly.md) — WebAssembly
...
>
> **权威来源对齐变更日志**: 2026-05-22 创建 [Authority Source Sprint Batch 10](../../00_meta/02_sources/05_international_authority_index.md)；2026-05-22 Batch 9 对齐完成
...
Rust AI 工程的边界测试集中在一个核心矛盾：张量计算的维度信息在运行期才确定，而 Rust 的类型系统（Type System）偏好编译期已知——形状错误因此从“编译期可防”退化为“运行期 panic”，这正是测试的重点覆盖区。

六组测试的分类：

| 测试 | 失败形态 | 防护手段 |
|---|---|---|
| Candle 张量形状不匹配 | 运行期 panic（`shape mismatch`） | 类型级形状（实验性）或输入校验层 |
| `unsafe` 与 SIMD intrinsic | 编译错误/UB | `target_feature` 边界与运行时检测 |
| 模型序列化版本兼容 | 运行期加载失败 | safetensors 格式 + 版本协商 |
| 张量生命周期与 GPU 内存 | 设备内存泄漏/重复释放 | 所有权映射到设备缓冲区的 RAII |
| 其余两组 | ONNX 算子覆盖缺口、WASM 推理的内存限制 | 算子回退策略、模型量化 |

每组测试给出 candle 0.x / ort 2.x 的最小复现与工程级防护模式，便于直接移植到生产代码。
...
```mermaid
mindmap
  root((Rust 在 AI 与机器学习中的新兴角色))
    一、核心概念
      1.1 为什么 AI 需要 Rust
      1.3 推理 vs 训练
    二、技术细节
      2.1 Candle纯 Rust ML 框架
      2.2 ONNX Runtime 集成
      2.3 WebAssembly 推理
    三、应用场景矩阵
    四、反命题与边界分析
