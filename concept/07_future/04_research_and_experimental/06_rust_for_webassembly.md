...
> **前置概念**: [Rust 路线图](../01_edition_roadmap/04_roadmap.md) · [WebAssembly 生态](../../06_ecosystem/11_domain_applications/03_webassembly.md) · [Web 框架](../../06_ecosystem/04_web_and_networking/03_web_frameworks.md)
> **后置概念**: [WASI 预览](../../06_ecosystem/05_systems_and_embedded/01_wasi.md) · [形式化方法](02_formal_methods.md)
...
- **wasm-bindgen 互操作语义**: 在模块边界生成胶水代码，把 Rust 的 `String`/`Vec`/结构体（Struct）编组为 JS 对象，核心是线性内存（linear memory）上的指针 + 长度协议。
...
Rust → Wasm 编译管线:
  ...
  └── 类型系统检查（Trait 求解、泛型单态化）
        [来源: [Rust 路线图](../01_edition_roadmap/04_roadmap.md)]
...
> **选型原则**: 优先 Yew 如果团队熟悉 React/VDOM 模型；优先 Leptos 如果追求极致性能或需要 SSR。两者均依赖 [Web 框架](../../06_ecosystem/04_web_and_networking/03_web_frameworks.md) 的异步（Async）运行时思维。[来源: [Web 框架](../../06_ecosystem/04_web_and_networking/03_web_frameworks.md)]
...
"Rust 适合所有 WASM 场景"的反命题分析：① 反命题决策树——按"是否需要 WASI 系统接口""是否受产物大小硬约束""是否需与 JS 高频互操作"三个判断节点，逐节点给出反例（如纯算法模块选 `wasm32-unknown-unknown` 反而更小，组件模型增加宿主要求）；② 边界极限测试——在模块体积（wasm-opt 后仍超限）、互操作开销（字符串/数组每次跨界拷贝）、调试信息（DWARF 在浏览器中的可用性）三处测量退化点。判定原则：WASM 方案选型先定宿主与接口世界，再谈 crate 生态；顺序颠倒会落入"生态有 crate 但宿主不支持"的反例。
...
- [Rust 路线图](../01_edition_roadmap/04_roadmap.md) — Rust 语言演进方向与 Wasm 目标支持策略
- [WebAssembly 生态](../../06_ecosystem/11_domain_applications/03_webassembly.md) — Wasm 通用生态与工具链概览
- [Web 框架](../../06_ecosystem/04_web_and_networking/03_web_frameworks.md) — Rust 服务端 Web 框架对比
- [WASI 预览](../../06_ecosystem/05_systems_and_embedded/01_wasi.md) — WebAssembly 系统接口标准
...
>
> **权威来源对齐变更日志**: 2026-05-22 初始创建，覆盖工具链、框架、性能、内存模型与互操作 [Authority Source Sprint Batch 11](../../00_meta/02_sources/05_international_authority_index.md)
...
WASM 的边界测试按**边界穿越点**分类：Rust↔WASM 运行时、WASM↔宿主（JS/系统）、组件↔组件，三类边界各自有不同的失败形态：

| 用例 | 边界 | 失败形态 |
|:---|:---|:---|
| WASI 文件系统权限 | WASM↔宿主 | 运行时错误（capability 缺失） |
| `wasm-bindgen` 类型不匹配 | Rust↔JS | 编译期宏（Macro）报错 / 运行时 TypeError |
| 模块大小与 `wee_alloc` | 产物约束 | 运行时 OOM / 加载超时 |
| 组件模型类型映射 | 组件↔组件 | wit 接口不匹配，链接期失败 |

判定原则：先定位失败发生在哪条边界，再选工具——Rust 侧类型错误用 rustc，宿主侧用浏览器/wasmtime 日志，组件侧用 `wasm-tools` 校验。
