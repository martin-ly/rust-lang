...
|---|---|---|
| 指令复杂度 | 每程序百万级指令上限（有界循环后） | 迭代器链需确认可被 LLVM 完全展开为界循环 |
| 栈与 Map 内存 | 512 字节栈、Map 值大小受限 | 大结构体必须放 Map 或 per-CPU 暂存区 |
| CO-RE 与 BTF 依赖 | 目标内核需暴露 BTF | 旧内核（<5.x）需携带 BTF 或放弃 CO-RE |
| 编译时延 | `bpfel-unknown-none` target + LTO | 构建慢于普通 target，需缓存策略 |

其中指令复杂度限制最常见：Rust 的泛型与迭代器生成的代码若不能被验证器证明有界，会在 `load` 阶段被拒——8.1 给出诊断与改写模式。
...
阅读重点是“抽象泄漏”类：Rust 的高级抽象（迭代器、闭包、泛型）在普通 target 上保证有界，但 eBPF 验证器的证明能力弱于 rustc 的类型系统——验证器拒绝不等于代码错误，改写模式见各测试的修复方案。
