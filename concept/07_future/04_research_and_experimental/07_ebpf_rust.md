...
「Rust 实现示例」部分按 Aya XDP 程序骨架（内核态）、Aya 用户态加载器与Map 交互：内核态 ↔ 用户态的顺序逐层展开。
...
本节将「边界与限制」分解为若干主题： eBPF 指令复杂度限制、Map 大小与内存约束、CO-RE 与 BTF 的依赖与Rust eBPF 的编译时延。
...
「边界测试：eBPF Rust 的编译错误」部分按边界测试：eBPF 程序的栈大小限制（编译错误/验证失败）、边界测试：eBPF 与用户空间的类型布局不匹配（编译错误）、边界测试：eBPF 的验证器拒绝与 Rust 的抽象泄漏（运行时加载失…、边界测试：eBPF 与用户空间的 map 类型共享（编译错误）等5个方面的顺序逐层展开。
...
> **反命题**: "eBPF / Aya / Rex 的 Rust 映射 是万能解决方案，适用于所有场景" —— 错误。任何技术选择都有权衡，需根据具体需求、团队能力与项目约束综合评估。
