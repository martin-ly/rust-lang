...
- **内存安全（Memory Safety）缺失**：C 语言的原生漏洞（use-after-free、空指针解引用（Reference））在 eBPF 中同样存在
...
| **sock_ops** | TCP 连接生命周期（Lifetimes）操作 | ✅ 支持 | ❌ 未支持 | `aya-ebpf::programs::SockOpsContext` |
...
| Relocation | 运行时（Runtime）的字段偏移调整 | Aya 加载器自动处理 |
...
Aya 的过程宏（Procedural Macro）将 Rust 函数转换为合法的 eBPF 程序入口：
...
Aya 的用户态库与 `tokio` 深度集成，支持异步（Async）事件处理：
...
| 内存安全（Memory Safety） | 追踪指针算术和访问范围 | 误报率高，数组访问常需展开 |
| 无 NULL 解引用（Reference） | 追踪指针的初始化状态 | 状态爆炸导致复杂程序被拒绝 |
...
Rust for Linux 允许编写 Rust 内核模块（Module），这些模块可以像 C 模块一样通过 `bpf()` 系统调用加载和管理 eBPF 程序。这意味着内核态基础设施可以用 Rust 编写，同时复用已有的 eBPF 生态。
...
本节示例以 Aya 框架为主线——它是 2026 年 Rust eBPF 生态的事实标准，提供从内核态程序编译到用户态加载的完整链路。三个示例构成最小可用闭环。

示例间的数据流关系：

```text
内核态 XDP 程序（6.1） ──MAP── 用户态加载器（6.2）
        │                        │
   过滤/统计报文            附加到网卡、读写 Map、输出指标
```

- **6.1 XDP 程序骨架**：`#[xdp]` 标注的内核态函数，演示 `no_std` 环境下的包解析与 `xdp_action` 返回；注意 eBPF 的 512 字节栈限制直接约束了可写的解析深度；
- **6.2 用户态加载器**：`Ebpf::load` + 程序附加（attach）到网络接口，演示 Map 的声明式获取；
- **Map 交互**：内核态写入 per-CPU 数组 Map，用户态以 `tokio` 任务周期读取聚合——这是 eBPF 观测程序的标准模式。

所有示例以 aya 0.13+ 与 stable Rust（内核态目标 `bpfel-unknown-none`）实测，构建命令见各小节。
...
eBPF 的强大来自验证器（verifier）的安全保证，而验证器的保守性正是 Rust eBPF 开发的主要摩擦来源。本节边界与限制回答“哪些 Rust 代码模式无法通过验证”。

四类限制的工程影响：

| 限制 | 数值/机制 | 对 Rust 代码的约束 |
|---|---|---|
| 指令复杂度 | 每程序百万级指令上限（有界循环后） | 迭代器（Iterator）链需确认可被 LLVM 完全展开为界循环 |
| 栈与 Map 内存 | 512 字节栈、Map 值大小受限 | 大结构体（Struct）必须放 Map 或 per-CPU 暂存区 |
| CO-RE 与 BTF 依赖 | 目标内核需暴露 BTF | 旧内核（<5.x）需携带 BTF 或放弃 CO-RE |
| 编译时延 | `bpfel-unknown-none` target + LTO | 构建慢于普通 target，需缓存策略 |

其中指令复杂度限制最常见：Rust 的泛型（Generics）与迭代器生成的代码若不能被验证器证明有界，会在 `load` 阶段被拒——8.1 给出诊断与改写模式。
...
- [Rust for Linux 内核编程](04_rust_for_linux.md) —— Rust 内核模块（Module）开发与 eBPF 的交叉领域
- 异步（Async）编程与 Tokio —— Aya 用户态异步事件处理的基础
- [形式验证工具链](../../04_formal/04_model_checking/01_verification_toolchain.md) —— Kani、Miri 等工具对 eBPF 程序的验证潜力
- [unsafe Rust 模式](../../03_advanced/02_unsafe/01_unsafe.md) —— eBPF 内核态编程中的 unsafe 边界管理
- [网络编程](../../03_advanced/06_low_level_patterns/04_network_programming.md) —— XDP/TC 网络处理的 Rust 生态
...
eBPF 程序的错误发生在三个不同阶段——编译期（rustc）、加载期（内核验证器）、运行期（用户态交互）——本节边界测试按阶段组织，帮助定位“错误属于哪一层”。

五组测试的阶段映射：

| 测试 | 阶段 | 典型报错 |
|---|---|---|
| 栈大小限制 | 加载期 | verifier 报 `combined stack size exceeds limit` |
| 类型布局不匹配 | 编译/加载期 | 用户态与内核态结构体字段偏移不一致 |
| 验证器拒绝与抽象泄漏 | 加载期 | Rust 迭代器/泛型生成的无界循环被拒 |
| Map 类型共享 | 编译期 | 两侧 Map 值类型声明漂移 |
| 第五组 | 运行期 | per-CPU Map 的聚合竞争 |

阅读重点是“抽象泄漏”类：Rust 的高级抽象（迭代器、闭包（Closures）、泛型）在普通 target 上保证有界，但 eBPF 验证器的证明能力弱于 rustc 的类型系统（Type System）——验证器拒绝不等于代码错误，改写模式见各测试的修复方案。
...
> Rust 的 `aya` 库在编译期通过泛型（Generics）参数检查类型：`AyaHashMap<_, u32, u64>` 要求键 4 字节、值 8 字节，与 eBPF 的 `HashMap<u32, u64>` 匹配。
...
> 这与 C 的 eBPF（手动计算大小，易错）或 Go 的 `cilium/ebpf`（反射检查类型）类似——Rust 的泛型（Generics）在编译期提供了部分保护，但跨语言边界的类型一致性（Coherence）仍需人工保证。
> [来源: [aya Documentation](https://aya-rs.dev/)] ·
...
本节从测验 1：eBPF 是什么？它为什么需要一门安全的语言来编写？（理解层）、测验 2：Rust 为什么适合编写 eBPF 程序？（理解层）、测验 3：`aya` 在 Rust eBPF 生态中提供什么功能？（理…、测验 4：eBPF 的验证器（Verifier）对 Rust 编译输出…等5个方面切入，剖析「嵌入式测验（Embedded Quiz）」的核心内容。
...
eBPF 是在 Linux 内核中运行的沙箱字节码，用于可观测性、网络和安全。由于运行在内核态，任何 bug 都可能导致系统崩溃，因此需要内存安全（Memory Safety）保证。
</details>
...
Rust 的内存安全（Memory Safety）消除了 eBPF 程序中的缓冲区溢出和 use-after-free 风险。 fearless 并发防止了内核中的数据竞争。性能接近 C。
</details>
...
网络过滤和负载均衡（替代 iptables）、可观测性（跟踪系统调用、性能分析）、安全策略执行（运行时（Runtime）威胁检测）、设备驱动。
</details>
