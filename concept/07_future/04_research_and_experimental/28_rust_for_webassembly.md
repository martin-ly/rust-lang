...
> **选型原则**: 优先 Yew 如果团队熟悉 React/VDOM 模型；优先 Leptos 如果追求极致性能或需要 SSR。两者均依赖 [Web 框架](../../06_ecosystem/04_web_and_networking/27_web_frameworks.md) 的异步（Async）运行时思维。[来源: [Web 框架](../../06_ecosystem/04_web_and_networking/27_web_frameworks.md)]
...
> **修正**:
> WASI（WebAssembly System Interface）是 Wasm 的模块（Module）化系统接口，设计原则是**能力安全**（capability-based security）。
