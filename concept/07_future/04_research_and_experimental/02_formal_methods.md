...
本组测验覆盖形式化方法入门的五个关键判断点，难度从理解层递进到分析层。与前文不同，测验 4–5 要求读者给出量化论证而非定性判断。
...
> **最佳实践**：对解析器和 FFI 边界使用 `cargo fuzz` 进行持续模糊测试；对发现的问题用 Kani 构造最小反例验证修复。 [cargo-fuzz docs; Rust Fuzz Book](https://rust-fuzz.github.io/book/)
...
形式化方法的工业价值最终要由生产案例回答。本节三个案例分别代表三种采纳路径：云服务商以 Kani 验证基础设施代码、系统厂商以 Verus 构建可验证系统组件、操作系统社区以形式化手段加固内核接口。

案例的阅读维度统一为四项，便于横向对比：

- **验证对象**：被验证代码的规模、unsafe 密度与业务关键度；
- **规格来源**：形式化规格从哪里来（既有文档形式化、设计阶段同步编写、还是从测试用例反向提炼）；
- **发现的缺陷类别**：验证过程中实际发现的 bug 类型分布——这是评估工具价值的硬数据；
- **持续集成方式**：验证是否进入 CI、每次运行的时间预算、以及回归时的处理流程。

三个案例共同说明一件事：工业界的形式化采纳不是“全程序验证”，而是“关键模块 + 可承受成本”的精确投放。
...
> **[Specifying Systems, Lamport 2002](https://lamport.azurewebsites.net/tla/book.html); [TLA+ Video Course](https://lamport.azurewebsites.net/video/videos.html)** TLA+ 的数学基础是 ZF 集合论 + 线性时序逻辑。`[Next]_vars` 的 stuttering 不变性（stuttering invariance）是 TLA+ 支持层次化规约和实现精化的关键——规约不限制系统"什么都不做"的次数，从而允许不同抽象层级的规约相互精化。
...
> **关键差异**：TLA+ 规约验证了**协议设计**层面的正确性；Rust 的 `Mutex` 实现还需要 Kani/Miri 验证 `unsafe` 内的原子操作（Atomic Operations）和内存顺序是否正确实现了这一状态机。 [Rust std::sync::Mutex docs; TLA+ Examples](https://doc.rust-lang.org/std/sync/struct.Mutex.html)
...
> **[AWS CACM 2015 — How Amazon Web Services Uses Formal Methods; Kani Docs; Verus Docs](https://www.amazon.science/publications/how-amazon-web-services-uses-formal-methods)** TLA+ 用于验证"设计是否正确"，Kani/Verus 用于验证"实现是否与设计一致"。在工业实践中，TLA+ 往往在项目早期使用（甚至先于代码），而 Kani/Verus 在编码阶段迭代使用。
...
> **[AWS Kani Blog 2023; Microsoft Verus Docs; Creusot Tutorial; Prusti GitHub](https://model-checking.github.io/kani/)** 工业工具分析基于各工具的官方文档和公开发表的论文。✅
> **[TLA+ Spec; P Language Docs; Spin Documentation](https://lamport.azurewebsites.net/tla/tla.html)** 协议验证工具参考了形式化方法领域的成熟工具文档。✅
---
...
形式化工具的“编译错误”分两层：代码本身的编译诊断，以及验证层面的规格违反（verification failure）。本节边界测试覆盖两层，并特别关注工具链锁定这一工程现实。

四组测试的分类：

| 测试 | 层面 | 典型失败 |
|---|---|---|
| Kani 的 `unsafe` 假设冲突 | 验证层 | `kani::assume` 与代码实际行为矛盾，CBMC 报反例 |
| Prusti 前置条件违反 | 验证层 | `#[requires]` 在调用点不满足，Viper 验证失败 |
| 规格与实现不一致 | 验证层 | 实现修改后规格未同步，原有证明失效 |
| 编译器版本锁定 | 工程层 | Kani/Prusti 绑定特定 nightly，工具链升级即构建失败 |

第四组是实践中最常见的痛点：形式化工具通常滞后 stable 数个版本，项目需在“使用新语言特性”与“保持可验证性”之间显式取舍——各测试给出版本对齐的实操建议。
