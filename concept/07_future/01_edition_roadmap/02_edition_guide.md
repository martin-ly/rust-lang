...
- **Edition 2024 主要特性**: `gen` 关键字预留、异步（Async）闭包（Closures）（`async ||`）、`let` 链（`if let ... && let ...`）、尾表达式临时值作用域收窄、`unsafe extern` 与 `unsafe` 属性的显式化——共同主题是**让隐式行为显式化**。
...
- **`if let` 临时作用域收窄**: `if let` 条件中创建的临时值在 `else` 分支前释放，修复了长期存在的意外延长生命周期（Lifetimes）坑——极少数依赖旧行为的代码会触发借用（Borrowing）检查错误。

判定依据：升级后编译错误九成来自后两项，按错误信息提示补标注即可，无运行时（Runtime）语义风险。
...
Edition 边界测试关注**同一 crate 跨 edition 编译**时的行为差：有些差异是编译期硬错误（关键字保留、语法移除），有些是静默语义变更（drop 时机、模式匹配（Pattern Matching）穷尽性）。每个用例标注三要素：

1. **触发 edition 组合**（如 2021→2024）；
2. **失败形态**：`cargo fix` 可自动修复 / 编译错误需人工 / 编译通过但语义改变（最危险）；
3. **检测手段**：`cargo fix --edition` 干跑、CI 双 edition 矩阵、或 rustc 的 future-incompat lint。

判定原则：能被 `cargo fix` 覆盖的差异属于低风险迁移；需要人工的只有语义变更与宏展开差异两类。
...
| `linker_messages` 默认 warn | **工具链 1.97**（全 edition） | **是**（默认 warn，属于 `warnings` group；`-D warnings` / `build.warnings` 可将其提升为错误） |
...
| `linker_messages` | **工具链 1.97** | warn | **是**（属于 `warnings` group；`-D warnings` / `build.warnings` 可提升为错误） | `#![allow(linker_messages)]` / `[lints.rust] linker_messages = "allow"` 静默 |
