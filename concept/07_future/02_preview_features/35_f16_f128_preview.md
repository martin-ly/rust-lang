...
1. **一等原始类型（Primitive Type）**：`f16`/`f128` 是语言级原始类型，不是库包装类型，享有字面量语法、模式匹配（Pattern Matching）与 `#[repr]` 布局保证。
2. **零抽象惩罚目标**：在硬件原生支持的平台上（如 ARM FP16、x86 AVX10.2、RISC-V Zfh）直接映射到机器指令；无原生支持时经编译器运行时（Runtime）库（compiler-builtins）软件模拟。
