...
Pin 人体工学的核心痛点之一：**`Pin<&mut T>` 不能自动 reborrow**。普通 `&mut T` 在传给函数时会隐式 reborrow（`&mut *x`），原引用（Reference）之后仍可用；`Pin<&mut T>` 无此机制，每次传递都是移动——调用一次 `poll` 类 API 后 `Pin` 引用即被消耗，迫使开发者手写 `x.as_mut()` 重借。

Reborrow Traits 方案引入两个标记 trait：

- **`Reborrow`**：表达「该类型可产生一个更短的同类型借用（Borrowing）视图」（`Pin<&mut T>` → `Pin<&mut T>` 缩短生命周期（Lifetimes））；
...
2. **Pinned Fields**：结构体（Struct）字段可标记为固定，投影（projection）规则决定「固定父结构 → 哪些子字段引用合法」——这是当前 `pin-project` 宏（Macro）手工实现的语言级化；
