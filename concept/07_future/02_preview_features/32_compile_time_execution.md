...
1. **`const fn`**：可在常量上下文与运行期双重使用的函数；限制集（截至 1.97 stable）：允许算术、控制流、切片/数组操作、`const` trait 调用（部分）；禁止堆分配（`String`/`Vec`）、`Rc`/`Arc`、大多数浮点运算的语义保证（可计算但非确定性保证）、`unsafe` 中仅允许受限操作。
...
反例：用 `const fn` 模拟「编译期反射」（如遍历结构体字段）——Rust 无此能力，该需求属于过程宏领域；强行用 const 技巧会导致不可维护的宏嵌套。
