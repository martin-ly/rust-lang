...
> **前置概念**: [Generics](../../02_intermediate/01_generics/01_generics.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Trait](../../02_intermediate/00_traits/01_traits.md)
> **后置概念**: [Macros](../../03_advanced/03_proc_macros/01_macros.md) · [Zero Cost Abstractions](../../01_foundation/00_start/02_zero_cost_abstractions.md)
...
1. **`const fn`**：可在常量上下文与运行期双重使用的函数；限制集（截至 1.97 stable）：允许算术、控制流、切片（Slice）/数组操作、`const` trait 调用（部分）；禁止堆分配（`String`/`Vec`）、`Rc`/`Arc`、大多数浮点运算的语义保证（可计算但非确定性保证）、`unsafe` 中仅允许受限操作。
2. **const 上下文**：`const X: T = ...`、数组长度、泛型常量参数——求值由 Miri 引擎（CTFE）执行，能检测出常量上下文中的 UB（越界、未初始化读取）。
3. **const 泛型**：`[T; N]` 与 `struct Foo<const N: usize>`，长度/容量参与类型检查，是「编译期计算影响类型」的唯一稳定通道；`generic_const_exprs`（复杂常量表达式作参数）仍 nightly。

判定依据：能在 `const fn` 写的校验逻辑（如表查找、位掩码计算）就下沉编译期，把运行期错误前移为编译错误。
...
编译期能力的稳定/不稳定边界（1.97 stable 视角）：

| 操作 | 状态 |
|---|---|
| 整数/布尔/字符运算、数组索引（带边界检查） | ✅ stable |
| `const fn` 中循环/递归 | ✅ stable |
| `const trait` 方法调用（如 `~const Drop`） | ⚠️ 部分 nightly |
| 堆分配（`const Box`/`const Vec`） | ❌ nightly（`const_heap`） |
| 浮点运算 | ✅ 可算，但 NaN 位模式等非确定性 |
| `generic_const_exprs`（`[T; N + 1]` 类） | ❌ nightly |

工程推论：跨 crate 的公共 API 不应依赖 nightly const 特性；库作者用 build.rs 代码生成作为「编译期堆计算」的稳定替代（把复杂表生成移到构建脚本）。

判定依据：`cargo +stable check` 通过即边界安全；需要 `feature(generic_const_exprs)` 的 API 设计应提供无该特性的降级路径。
...
1. **编译期计算**：查找表、CRC 多项式表、位掩码常量——`const fn` 生成 + `const` 项存储，零运行期初始化成本，且结果受 Miri 验证（无越界）。对比 build.rs 生成：`const fn` 方案无构建脚本、增量编译友好，优先选之。
2. **类型状态机（Typestate）**：const 泛型 + 泛型类型参数把协议状态编码进类型，如 `struct Buf<const N: usize, State>`；编译期保证「缓冲区长度的算术关系」与「状态转换合法性」同时成立。

反例：用 `const fn` 模拟「编译期反射」（如遍历结构体（Struct）字段）——Rust 无此能力，该需求属于过程宏（Macro）领域；强行用 const 技巧会导致不可维护的宏嵌套。

判定依据：计算输入全部是常量字面量 → `const fn`；需要类型信息遍历 → 过程宏/derive。
...
> **计算洞察**: **编译期计算将运行时（Runtime）开销转移到编译期**——牺牲编译时间换取运行时性能。
...
编译期执行的反命题分析从主张"把更多计算移到编译期总是净收益"出发：① 反命题树——推翻"编译期执行无副作用"（编译时间、确定性、`cfg` 分裂三大代价）、"const 泛型可替代所有类型级编程"（表达力受 const 求值规则硬约束）等子主张；② 边界极限——在 const 求值器的能力边界（无堆分配、无外部 I/O、无未稳定操作）上构造"刚好不能编译期执行"的最小用例。判定原则：把计算移到编译期的必要条件是**输入在编译期已知且求值规则封闭**；缺一条就应留在运行期。
...
> **认知功能**: **编译期计算只适用于静态已知的数据**——运行时（Runtime）输入必须用运行时计算。
...
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 泛型
- [Trait](../../02_intermediate/00_traits/01_traits.md) — Trait
- [Macros](../../03_advanced/03_proc_macros/01_macros.md) — 宏（Macro）
- [Zero Cost Abstractions](../../01_foundation/00_start/02_zero_cost_abstractions.md) — 零成本抽象（Zero-Cost Abstraction）
...
编译期执行的边界测试按"求值器拒绝点"分类：每个用例构造一个看似可编译期求值、实则触碰 const 规则边界的操作，验证编译器在正确位置拒绝。

| 用例 | 触碰的 const 规则 | 期望结果 |
|:---|:---|:---|
| `const fn` 受限操作 | 无堆分配/无外部可变状态 | E0xxx 编译错误 |
| 过程宏 TokenStream | 宏输入必须自包含 | 解析错误定位到宏展开 |
| 浮点限制 | const 浮点运算的确定性要求 | 受限操作报错 |
| `const Heap` 展望 | 编译期分配提案的能力边界 | 未稳定特性 gate 报错 |

判定原则：const 报错若指向"调用不允许的操作"，检查点在**操作**而非函数签名；若指向 trait bound，检查点在 `~const` 推断链。
...
本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：Rust 的常量求值（const evaluation）目前…、测验 2：`const fn` 与 C++ 的 `constexpr`…、测验 3：为什么 Rust 对 `const fn` 的能力限制比 C…、测验 4：编译期执行对嵌入式开发有什么特别价值？（理解层）等5个方面。
...
逐步支持更多标准库功能（`Vec`、`String` 在 const 上下文）、用户自定义类型的 const 构造函数、更强大的 const 泛型（Generics）计算。
</details>
