...
Safety Tags 针对的问题定义：**unsafe 契约的表达缺口**。当前 Rust 中 `unsafe fn` 的前置条件（如「ptr 必须有效且对齐」）只能写在文档注释里，编译器与工具均不可读——`clippy::missing_safety_doc` 只检查注释存在性，不检查内容。

设计目标三层：

1. **结构化**：`#[safety(requires: ...)]` / `#[safety(ensures: ...)]` 属性形式，谓词语法收敛到可解析子集；
2. **机器可读**：使 AI 代码生成、审计工具、形式化验证器（Kani/Verus）可消费契约，而不依赖自然语言；
3. **与 `#[safety]` 属性家族的关系**：安全标签是契约的**陈述层**，BorrowSanitizer 等是**动态检查层**——标签说明「应当满足什么」，运行时（Runtime）工具检测「是否被违反」。

判定依据：RFC #3842 仍处讨论期；当前可落地的等效实践是 `/// # Safety` 注释 + Miri/BSan 动态验证。
...
1. **AI 生成代码的安全标注**：LLM 生成 `unsafe` 块时最大风险是「沉默地假设了契约」；强制要求生成 `#[safety]` 标签把隐含假设显式化，使人类 review 与下游验证工具有抓手——这是该 RFC 在 2025–2026 重新获得关注的主要驱动力。
2. **Rust for Linux 内核契约**：内核抽象层（`kernel` crate）的每个 `unsafe impl Send/Sync` 与 FFI 包装都依赖大量非局部不变量；结构化标签使跨子系统审计可脚本化（grep 标签谓词而非自然语言）。
3. **FFI 边界的前置/后置条件**：`extern "C"` 边界的所有权（Ownership）移交（谁负责 free、线程亲和性）是最常见的 FFI bug 来源；标签化后可由 `bindgen`/审计工具生成边界检查清单。

判定依据：凡 `unsafe` 密度高的 crate（FFI 绑定、内核模块（Module）、no_std 驱动）是 Safety Tags 的首批受益对象。
...
Safety Tags 提案的核心争议是“文档化能否机器化”：批评者认为把 `// SAFETY:` 注释升级为结构化标签只是给不可验证的约定换了个格式，支持者认为结构化是机器检查的前提，哪怕标签内容本身仍需人工撰写。本节分别检验两种立场。

- **反命题树**：把“标签无用论”分解为三个可检验子命题——标签是否能减少审计遗漏（可通过“补全检查”度量）、是否增加 unsafe 编写的摩擦成本（可通过代码行数与评审轮次度量）、是否与现有 clippy/rustdoc 基础设施兼容（工具链改造成本）；
- **边界极限**：考察标签体系的退化情形——所有 unsafe 块都标同一标签时体系等价于无标签；以及理想情形——每个 unsafe 操作都有指向具体不变量的标签，审计可机械化到“标签引用（Reference）的不变量是否存在对应论证”。

当前共识预览：Safety Tags 的价值与 unsafe 代码密度正相关，标准库与 unsafe 密集 crate 是主要受益面；对偶发 unsafe 的业务代码，强制标签的摩擦可能超过收益。
...
Safety Tags 的边界测试围绕“标签语义与 unsafe 操作的对应关系”展开——标签体系的风险不是语法错误，而是标签与实际安全论证脱节，造成虚假的安全感。

六组测试的覆盖：

| 测试 | 风险点 | 检测手段 |
|---|---|---|
| 层级不匹配 | 标签声明的保证强于实际操作所需 | 标签-操作一致性（Coherence） lint |
| 标签传播与 unsafe 块边界 | 内层 unsafe 操作被外层标签“覆盖” | 作用域分析 |
| 层级蕴含与工具支持缺失 | 标签间的蕴含关系无工具验证 | 当前需人工评审 |
| 审计中的人为遗漏 | unsafe 块无标签 | 强制 lint（可配置 deny） |
| 其余两组 | 标签与 Safety Comments 的并存迁移、泛型（Generics） unsafe 的标签传导 | 迁移指南与边界案例 |

测试基于提案语法原型与 clippy 实验 lint 实测；所有标签语法在 stable 1.97 上尚不可用，本节同时给出当前等效实践（结构化 SAFETY 注释模板）。
