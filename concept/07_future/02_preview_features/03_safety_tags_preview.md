...
3. **与 `#[safety]` 属性家族的关系**：安全标签是契约的**陈述层**，BorrowSanitizer 等是**动态检查层**——标签说明「应当满足什么」，运行时工具检测「是否被违反」。
...
3. **FFI 边界的前置/后置条件**：`extern "C"` 边界的所有权移交（谁负责 free、线程亲和性）是最常见的 FFI bug 来源；标签化后可由 `bindgen`/审计工具生成边界检查清单。

判定依据：凡 `unsafe` 密度高的 crate（FFI 绑定、内核模块、no_std 驱动）是 Safety Tags 的首批受益对象。
...
- **边界极限**：考察标签体系的退化情形——所有 unsafe 块都标同一标签时体系等价于无标签；以及理想情形——每个 unsafe 操作都有指向具体不变量的标签，审计可机械化到“标签引用的不变量是否存在对应论证”。
