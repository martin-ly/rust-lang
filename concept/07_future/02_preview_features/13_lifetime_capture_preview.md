...
> **前置依赖**: [Lifetime](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md) · [Trait](../../02_intermediate/00_traits/01_traits.md) · [Advanced Lifetimes](../../01_foundation/01_ownership_borrow_lifetime/04_lifetimes_advanced.md)
> **后置延伸**: [RPITIT](15_rpitit_preview.md) · [Return Type Notation](09_return_type_notation_preview.md)
> **来源**:
>
...
`use<'lt>` 精确捕获语法针对 RPIT（返回位置 `impl Trait`）的**默认捕获规则变更**：

| 场景 | Rust 2021 及之前 | Rust 2024 起 |
|:---|:---|:---|
| 返回 `impl Trait` | 只捕获签名中出现的参数；生命周期（Lifetimes）需出现在约束中才捕获 | 默认捕获**所有**在作用域内的生命周期与类型参数 |
| 借用（Borrowing）导致的意外约束 | 常见 "borrowed value escapes" 错误 | 自动捕获生命周期，代码直接通过 |
| 需要收窄捕获 | 无法表达"不捕获" | `use<'a, T>` 显式列举捕获集 |

`use<..>` 表示捕获全部，`use<>` 表示不捕获任何外层参数。判定原则：Rust 2024 下若返回的 `impl Trait` 需要比默认更窄的约束以释放调用者灵活性，用 `use<...>` 收窄；否则依赖默认捕获即可。

### 2.1 旧行为（Rust 2021 及之前）

```rust,compile_fail,edition2021
fn make_iter<'a, 'b>(s: &'a str, _t: &'b str) -> impl Iterator<Item = char> {
    s.chars()   // E0700: 隐藏类型捕获了未出现在约束中的 'a（rustc ≥1.82 后各 edition 统一行为）
}

fn main() {
    let a = "hello".to_string();
    let b = "world".to_string();
    // rustc <1.82 的 2021 edition 中，返回类型还会静默过度捕获 'b，导致 drop(b) 报错；
    // 现行编译器直接在函数签名处拒绝未声明的捕获（E0700），见 2.2 的 use<'a> 修法。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1："生命周期捕获规则"的改进解决了什么问题？（理解层）、测验 2：`impl Trait + use<'a>` 语法中的 `u…、测验 3：这个特性对 API 设计有什么影响？（理解层）、测验 4：生命周期捕获规则与 Return Type Notation…等5个方面。
...
```mermaid
mindmap
  root((Lifetime Capture in impl))
    一、功能动机为什么需要精确生命周期捕获？
    二、语法说明uselt 与捕获规则
      2.1 旧行为Rust 2021 及之前
      2.2 新行为Rust 2024 / usea
      2.3 use.. 捕获类型与 lifetime
    三、与稳定 Rust 的对比
```
