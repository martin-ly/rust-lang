...
> **前置概念**: [Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md) · [Parallel Frontend](04_parallel_frontend_preview.md)
...
Cranelift 后端的反命题集中在成熟度：“Cranelift 已经能替代 LLVM”与“Cranelift 永远只是调试后端”两个极端都与现状不符。本节的反命题树以 2026 年的实际能力矩阵为锚。

检验维度：

- **反命题树**：把“可替代论”分解为编译速度、运行性能、平台覆盖、调试信息质量四个子命题——前两项 Cranelift 在 debug 场景已占优（编译快 20–30%），后两项仍有明确缺口（SIMD 覆盖、内联汇编支持、平台广度）；
- **边界极限**：考察 release 场景的差距——Cranelift 的优化管线深度不及 LLVM，计算密集型 release 二进制性能差距在 10–40% 区间，这决定了它当前的定位是“dev profile 后端”而非全量替代。

实践判定：启用 `cranelift` 后端的收益与项目的“编译-运行循环频率”成正比——TDD 工作流与大型 workspace 的增量编译场景收益最大，发布构建应保留 LLVM。
...
- [Parallel Frontend](04_parallel_frontend_preview.md) — 并行前端编译
...
Cranelift 的边界测试回答一个实操问题：把项目切换到 Cranelift 后端后，会在哪里出问题？八组测试按“发现阶段”排序——从编译期直接失败到运行期静默差异。

测试矩阵：

| 类别 | 测试 | 表现 |
|---|---|---|
| 编译期拒绝 | 平台特定内联汇编、SIMD 向量宽度超限 | 明确编译错误 |
| 运行期差异 | 调试构建语义差异、尾调用优化缺失 | 行为不同或栈溢出 |
| 性能退化 | 浮点优化、自动向量化缺失 | 基准回归 |
| 工具链集成 | 调试信息、sanitizer 支持 | 功能不可用 |

其中最需要警惕的是“运行期差异”类：LLVM 与 Cranelift 对未定义行为周边的实现选择不同，依赖特定行为的代码（虽不规范）切换后端后可能暴露潜在 bug——各测试给出对照实验方法。
