...
> **前置概念**: [Lifetimes](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md) · [Async](../../03_advanced/01_async/01_async.md) · [Traits](../../02_intermediate/00_traits/01_traits.md) · [Async Closures](../../03_advanced/01_async/07_async_closures.md)
> **后置概念**: [Version Tracking](../00_version_tracking/01_rust_version_tracking.md) · [Async Drop Preview](22_async_drop_preview.md)
...
与 `use<..>` 精确捕获的区别：`use<>` 管**生命周期（Lifetimes）捕获集**（哪个泛型参数的生命周期被返回类型借用（Borrowing）），RTN 管**返回类型的可命名性与约束**——两者正交，解决不同问题，可能共存于最终设计。
...
Return Type Notation（RTN，`T::method(..): Send` 式语法）自提出起就伴随争议：支持者认为它补上了“约束异步（Async）/trait 方法返回类型”的最后一块拼图，反对者认为它引入了一种新的类型位置语法，复杂度收益比存疑。本节的反命题树把双方论据形式化。

两条主线的分工：

- **反命题树**：从“RTN 是 RPITIT 的必要补全”这一核心命题出发，分解为可检验子命题——如“没有 RTN 就无法约束 `async fn` in trait 的返回 future 的 `Send` 性”，并给出 `-> impl Trait + use<..>` 等替代方案的反例；
- **边界极限**：构造 RTN 失效或退化的场景（非 AFIT 方法、const 上下文、类型位置误用），观察编译器诊断是否足以引导修正。

判定 RTN 是否值得在某项目中启用的依据：项目是否大量使用 RPITIT/AFIT 且需要跨 crate 约束返回类型的 auto trait——若是，RTN 消除的 workaround（手写 wrapper trait）收益明确；否则保持观望。
...
| 概念 | 文件 | 关系 |
|:---|:---|:---|
| Async Closures | [](../../03_advanced/01_async/07_async_closures.md) | RTN 未来可能扩展至 async closures |
| AFIT / RPITIT | [](../../03_advanced/01_async/01_async.md) | RTN 的主要应用场景 |
| Precise Capturing | [](../../02_intermediate/04_types_and_conversions/04_type_system_advanced.md) | 与 RTN 互补但不同 |
| Version Tracking | [](../00_version_tracking/01_rust_version_tracking.md) | 稳定化时间线 |
...
RTN 的边界测试围绕“语法看似可用但语义不支持”的场景组织——作为 nightly 特性，RTN 的主要风险不是写错，而是在不适用的位置套用，得到难以理解的诊断。

五组测试各覆盖一类误用：

1. **类型位置使用**：把 `T::method(..): Send` 写在需要完整类型的位置（如结构体（Struct）字段），编译器应拒绝——验证诊断信息是否指明“RTN 仅用于 bound 位置”；
2. **非 AFIT/RPITIT 方法**：对普通同步方法套用 RTN，观察拒绝理由；
3. **const/type 泛型方法**：RTN 与 const 泛型参数的组合尚无定义语义；
4. **缺少 feature gate**：stable 工具链上使用 RTN 的报错路径；
5. 第五组覆盖 RTN 与 `use<..>` 精确捕获的交互边界。

所有测试基于 nightly 2026-06 工具链与 `feature(return_type_notation)` 实测，stable 1.97 上应全部报 feature-gate 错误。
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：Return Type Notation（RTN）解决的是什…、测验 2：RTN 的语法 `T::method(..): Send`…、测验 3：RTN 与 `use<..>` 精确捕获有什么区别？（理解层）、测验 4：RTN 当前支持在哪些位置使用？（理解层）等5个方面，本节逐一说明其要点。
...
RTN 是一个典型的“小问题、深机制”特性：表面只是给 trait 方法返回类型加约束的语法，底层却牵动 RPITIT 的 desugaring、auto trait 泄露与生命周期捕获三条机制链。本节的认知路径按“先建立问题、再理解机制、最后检验边界”的顺序组织。

两条主线的阅读顺序：

1. **核心推理链**：从“为什么 `-> impl Trait` 在 trait 中无法直接加 `Send` 约束”这一具体问题出发，沿 RPITIT desugar → 返回类型匿名化 → auto trait 约束缺失的链条推导，最终理解 RTN 为何以“在 bound 位置引用（Reference）方法返回类型”的方式补位；
2. **反命题与边界**：在理解机制后回看第四节的反命题树，此时应能自行判断每条子命题的成立条件，并对照第十节的边界测试验证。

完成本路径的检验标准：能不查资料解释“RTN 为什么不设计为完整的类型位置语法”，并给出至少一个反例说明该设计选择的合理性。
