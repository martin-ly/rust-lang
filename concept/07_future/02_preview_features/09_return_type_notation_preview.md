...
Return Type Notation（RTN，`T::method(..): Send` 式语法）自提出起就伴随争议：支持者认为它补上了“约束异步/trait 方法返回类型”的最后一块拼图，反对者认为它引入了一种新的类型位置语法，复杂度收益比存疑。本节的反命题树把双方论据形式化。
...
1. **类型位置使用**：把 `T::method(..): Send` 写在需要完整类型的位置（如结构体字段），编译器应拒绝——验证诊断信息是否指明“RTN 仅用于 bound 位置”；
...
1. **核心推理链**：从“为什么 `-> impl Trait` 在 trait 中无法直接加 `Send` 约束”这一具体问题出发，沿 RPITIT desugar → 返回类型匿名化 → auto trait 约束缺失的链条推导，最终理解 RTN 为何以“在 bound 位置引用方法返回类型”的方式补位；
