...
> **前置概念**: [Formal Methods](../04_research_and_experimental/02_formal_methods.md) · [RustBelt](../../04_formal/02_separation_logic/01_rustbelt.md) · [Ferrocene](12_ferrocene_preview.md)
...
Rust Specification（Ferrocene Language Spec 的上游化）要解决的根本问题是：rustc 的行为目前只由“实现即规范”定义，安全关键行业（ISO 26262、IEC 61508）需要的是**独立、可评审、可版本化**的语义文档。本节技术细节按规范的内部结构展开。

三个层面的递进关系：

1. **规范的内容层次**：词法/语法层（可机器生成）、类型与 trait 层（借用（Borrowing） chalk 的形式化）、动态语义层（最困难，涉及求值顺序、内存模型与 unsafe 的未定义行为边界）；
2. **与形式化验证的衔接**：规范文本中的规则需与 RustBelt 的 λRust、Ferrocene 的验证工件对齐，使“符合规范”成为可机器检查的命题；
3. **维护挑战**：rustc 每周合并数十个 PR，规范滞后是结构性风险——当前策略是“稳定特性优先、nightly 特性不追”，并约定特性稳定化时必须附带规范章节。

阅读时建议对照 FLS 文档的实际章节结构，本节各小节给出对应链接。
...
> [来源: [Formal Methods Preview](../04_research_and_experimental/02_formal_methods.md)]
...
围绕 Rust 规范存在两个方向的怀疑：“规范永远追不上实现，写出来也是过时的”与“有了规范就能证明程序正确”。本节的反命题树同时回应两种误解。

检验要点：

- **反命题树**：把“规范无用论”分解为子命题（规范滞后率、覆盖度、行业采纳度），用 Ferrocene 在汽车行业认证中的实际使用数据回应；把“规范万能论”分解为（规范覆盖的语义范围、未定义行为的处理、与具体实现的偏差容忍度），说明规范是认证的必要条件而非充分条件；
- **边界极限**：考察规范刻意不覆盖的区域——nightly 特性、目标平台特定行为、以及尚未完成形式化的动态语义章节，这些区域的程序正确性仍需依赖测试与 Miri。

关键判定：评估一个项目是否需要“规范级”保证时，问题不是“规范是否完美”，而是“项目的认证/审计要求是否接受实现即规范”——安全关键行业通常不接受。
...
- [RustBelt](../../04_formal/02_separation_logic/01_rustbelt.md) — Rust 所有权（Ownership）的形式化模型
- [Ferrocene](12_ferrocene_preview.md) — Rust 安全关键认证工具链
...
规范类项目的“编译错误”有特殊含义：它不仅指代码不通过编译，还包括“规范文本与编译器行为不一致”导致的合规失败。本节边界测试覆盖这两类。

六组测试的分类：

| 类别 | 测试 | 失败形态 |
|---|---|---|
| 规范未定义行为 | 依赖 rustc 特定实现细节（如未指定的求值顺序） | 换编译器版本后行为漂移 |
| 规范与实现差异 | 规范允许而 rustc 拒绝（或反之）的代码 | 合规审计不通过 |
| 临时不一致 | 规范章节更新滞后于已稳定特性 | 版本窗口期内无法认证 |
| 草案漂移 | 引用（Reference）规范草案而非已批准章节 | 审计基线失效 |
| 其余两组 | unsafe 语义边界与内存模型条款 | Miri 通过但规范未覆盖的灰色地带 |

每组测试给出最小代码、涉及的规范章节编号与 rustc 1.97 的实际行为对照，便于读者自行验证“规范—实现”差距的现状。
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：为什么 Rust 需要一份官方语言规范？目前 Rust 的定…、测验 2：Rust 规范项目与 C/C++ 的 ISO 标准有什么区别…、测验 3：`ferrocene` 与 Rust 规范有什么关系？（理解…、测验 4：编写语言规范对 Rust 的"替代实现"（如 `gccrs`…等5个方面，本节依次展开。
