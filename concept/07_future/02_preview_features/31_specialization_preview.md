...
> **后置概念**:
>
> [Const Trait Impl](06_const_trait_impl_preview.md) ·
...
与 Coherence（一致性）的交互是核心难点：特化引入「实现之间的偏序关系」，与孤儿规则叠加后，「哪个实现被选中」必须在编译期唯一确定——关联类型的特化（`default type`）触及类型推断（Type Inference）根基，是稳定化的主要障碍（`min_specialization` 子集已 nightly 可用多年但未转正）。
...
特化（specialization）的反命题分析从核心主张"允许重叠 impl 且选择最具体者"出发，逐条构造反例：① 反命题树——推翻"特化是纯优化、不改变语义"（`default` 方法体差异可改变行为）、"特化与孤儿规则正交"（重叠检查与一致性检查交织）等子主张；② 边界极限——在关联类型、`const` 泛型、跨 crate impl 三处测量求解器的拒绝边界。判定原则：一个特化提案要被接受，必须在边界极限处保持"局部可判定"——只看签名不看实现体即可裁决哪个 impl 更具体；否则类型检查不可模块（Module）化。
...
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 泛型系统
- [Const Trait Impl](06_const_trait_impl_preview.md) — 常量 Trait 实现
- [Effects System](01_effects_system.md) — 效果系统
...
特化的边界测试全部围绕**重叠（overlap）与一致性（coherence）**两条规则：编译器必须同时保证"选择的 impl 确实最具体"与"不存在两个同等具体的候选"。用例分布：

| 用例 | 攻击的规则 | 期望报错 |
|:---|:---|:---|
| 重叠实现 | coherence：两 impl 无严格偏序 | E0119 冲突实现 |
| `default` 方法冲突 | 默认实现 vs 最终实现的解析 | 方法解析歧义 |
| 关联类型 | 特化后关联类型一致性 | 类型约束失败 |
| trait 一致性 | 跨 crate 特化的可见性 | coherence 拒绝 |

判定原则：`min_specialization` 之外的完整特化仍未稳定，用例中的错误信息以 nightly `#![feature(specialization)]` 实测为准。
...
> **修正**: 特化（specialization）允许为泛型（Generics）类型提供默认实现，并为特定类型提供更优实现。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：什么是"特化"（Specialization）？它与当前 R…、测验 2：为什么 Rust 的特化比 C++ 的模板特化更难设计？（理…、测验 3：`default impl` 语法在特化中有什么作用？（理解…、测验 4：特化对性能优化有什么帮助？（理解层）等5个方面。
