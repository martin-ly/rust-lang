...
特化（specialization）的反命题分析从核心主张"允许重叠 impl 且选择最具体者"出发，逐条构造反例：① 反命题树——推翻"特化是纯优化、不改变语义"（`default` 方法体差异可改变行为）、"特化与孤儿规则正交"（重叠检查与一致性检查交织）等子主张；② 边界极限——在关联类型、`const` 泛型、跨 crate impl 三处测量求解器的拒绝边界。判定原则：一个特化提案要被接受，必须在边界极限处保持"局部可判定"——只看签名不看实现体即可裁决哪个 impl 更具体；否则类型检查不可模块化。
