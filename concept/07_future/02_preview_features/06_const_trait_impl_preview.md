...
> [来源: [Rust Effects System 预研](01_effects_system.md)]
>
> **受众**: [专家]
> **内容分级**: [实验级]
> **Bloom 层级**: L3-L4
...
> **前置概念**: [Generics](../../02_intermediate/01_generics/01_generics.md) · [Traits](../../02_intermediate/00_traits/01_traits.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md)
...
const trait impl 针对「常量上下文中的 Trait 鸿沟」：`const fn` 中不能调用任何 trait 方法（即使实现本身是 const-safe），导致编译期代码被迫退化为宏（Macro）或单态化（Monomorphization）手写——例如编译期 `Default::default()`、`Add::add()` 均不可用。

方案概览：

1. **`const impl Trait for T`**：声明「该 impl 的所有方法满足 const 约束」，方法体受 `const fn` 同等限制（无堆分配、无运行期行为）；
2. **`~const` 限定**：`fn f<T: ~const Trait>()` 表达「调用方若在 const 上下文，则 T 的 impl 必须是 const impl」——`~`（约）标记是**效果系统的首个工程形态**：constness 作为可约束的效果进入类型系统（Type System）；
...
> [来源: [Effects System Preview](01_effects_system.md)]
...
> [来源: [Rust Version Tracking](../00_version_tracking/01_rust_version_tracking.md)]
...
| [Effects System Preview](01_effects_system.md) | ✅ 一级 | 效果系统关联概念 |
...
- [Const Generics（常量泛型）](../../02_intermediate/01_generics/02_const_generics.md) — 值级参数，与 const trait（效果级）互补
...
- [Effects System](01_effects_system.md) — 效果系统预研
...
| trait bound 兼容性 | 运行时（Runtime） impl 用于 const 上下文 | bound 求解处 |
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：`const trait` 与 `const fn` 有什么…、测验 2：为什么 `Vec::new()` 在 Rust 1.96 中…、测验 3：`~const Trait` 语法是什么意思？（理解层）、测验 4：`const trait` 对嵌入式开发有什么意义？（理解层）等5个方面的顺序逐层展开。
