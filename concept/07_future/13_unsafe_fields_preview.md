...
> **使用建议**: 公开结构体中所有原始指针（Raw Pointer）字段都应 unsafe；私有结构体中若通过 safe 方法完全封装，可酌情省略。
...
- **定理**: Unsafe Fields 预研：字段级安全边界的精确标注 定义 ⟹ 类型安全保证
- **定理**: Unsafe Fields 预研：字段级安全边界的精确标注 定义 ⟹ 类型安全保证
- **定理**: Unsafe Fields 预研：字段级安全边界的精确标注 定义 ⟹ 类型安全保证
...
> **反命题**: "Unsafe Fields 预研：字段级安全边界的精确标注 是万能解决方案，适用于所有场景" —— 错误。任何技术选择都有权衡，需根据具体需求、团队能力与项目约束综合评估。
