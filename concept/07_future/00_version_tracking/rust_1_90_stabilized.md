...
- [Rust 1.90 网络特性参考](#rust-190-网络特性参考)
  ...
  - [1. 异步（Async）特性增强](#1-异步特性增强)
    ...
    - [1.2 异步（Async）闭包（Closures）改进](#12-异步闭包改进)
    - [1.3 async fn in trait（Trait）生命周期（Lifetimes）推断](#13-async-fn-in-trait生命周期推断)
  ...
  - [4. impl Trait增强](#4-impl-trait增强)
    - [返回类型优化](#返回类型优化)
  - [5. 常量泛型（Generics）改进](#5-常量泛型改进)
    - [固定大小缓冲区](#固定大小缓冲区)
  - [6. 迭代器（Iterator）组合子增强](#6-迭代器组合子增强)
    - [网络数据处理](#网络数据处理)
  - [7. 错误处理（Error Handling）改进](#7-错误处理改进)
    - [网络错误类型](#网络错误类型)
