...
Rust 1.90 的迭代器组合子增强并非新增语法，而是标准库在既有组合子上补齐了若干缺失的适配器与特化实现，使“以迭代器表达数据处理流水线”的写法在网络数据场景中不再需要手写中间 `collect`。

本节以网络数据处理为唯一案例场景，原因有三：

1. **数据形态典型**：字节流 → 帧 → 消息的三级切分天然契合 `Iterator` 的惰性链式模型；
2. **性能敏感**：每次不必要的 `collect::<Vec<_>>()` 都意味着一次堆分配与拷贝，组合子增强的收益可直接测量；
3. **可对比**：同一解析逻辑可用命令式循环与组合子链两种写法实现，便于量化可读性与吞吐的差异。

阅读时请对照两种写法的中间分配次数，这是判断组合子是否真正“零成本”的基准。
...
1.90 版本周期内稳定的网络相关能力，主体来自 Edition 2024（随 1.85.0 稳定）的特性在 1.90 工具链上的完整落地。本节集中说明这些特性在网络编程中的具体应用方式，而非罗列版本日志。

涉及的能力分三类：

- **异步语法层**：`async fn` in trait（1.75 稳定）在 1.90 上与精确生命周期捕获配合，使网络服务 trait 的定义不再需要 `async_trait` 宏改写；
- **标准库层**：`let`-chains（1.88 稳定）简化连接状态机的多层模式匹配，`LazyLock`（1.80 稳定）替代 `lazy_static` 管理全局连接池配置；
- **工具链层**：1.90 的 rustc 对异步代码的调试信息生成有改进，`tokio-console` 等观测工具可显示更完整的任务状态。

判断某个特性是否已可用于生产，以“stable 版本号 ≤ 当前 MSRV（1.97.0）”为准，本节各小节均标注了稳定版本。
...
Rust 的性能叙事常被简化为“零成本抽象”，但 1.90 时代的准确表述是：**抽象的成本在编译期支付，运行期只保留必要的机器码**。本节的性能优化特性围绕这一原则展开，区分“编译器自动完成”与“需要开发者配合”两类。

判定一个抽象是否真正零成本，可用三个检查：
...
实战示例集把前文分散的特性组合为两个可运行的完整程序：HTTP 客户端与 WebSocket 服务器。选取这两个场景，是因为它们分别代表网络编程的两种基本交互模式——请求/响应与长连接双向流。

示例的阅读顺序建议：

1. **先跑通**：两个示例均以 Rust 1.97 stable（edition 2024）实测，依赖版本锁定在示例头部注释中；
2. **再对照**：每个示例末尾的“特性映射表”列出代码中每处用到的 1.85–1.90 稳定特性及其位置；
3. **最后改造**：练习部分要求把 HTTP 客户端的连接池从 `LazyLock` 全局改为显式注入，验证对依赖注入模式的理解。

示例刻意不使用 `unwrap()` 以外的错误处理简化，生产可用性相关的取舍（重试、超时、背压）在各示例的“工程化延伸”小节讨论。
