...
- `pin!` 宏（Macro）阻止隐式 deref coercions。
...
比较两个函数指针是否指向同一地址，避免闭包（Closures）/function item 比较时的意外行为：
...
无需解引用（Reference）即可获取 `Box<T>` 的原始指针（Raw Pointer）：
...
将 `Option<T>` 视为长度为 0 或 1 的切片（Slice）：
