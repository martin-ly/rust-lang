...
- [Rust 1.91 所有权（Ownership）、借用（Borrowing）、生命周期（Lifetimes）改进文档（历史版本）](#rust-191-所有权借用生命周期改进文档历史版本)
  - [📊 目录](#-目录)
  - [概述](#概述)
  - [改进的类型检查器（借用（Borrowing）检查器优化）](#改进的类型检查器借用检查器优化)
    ...
    - [性能对比](#性能对比)
    - [使用示例](#使用示例)
  - [增强的 const 上下文（对生命周期（Lifetimes）的影响）](#增强的-const-上下文对生命周期的影响)
    - [Rust 1.91 改进概述](#rust-191-改进概述-1)
    - [核心改进](#核心改进-1)
      - [1. const 上下文中的引用（Reference）](#1-const-上下文中的引用)
      - [2. const 上下文中的生命周期](#2-const-上下文中的生命周期)
    - [实际应用场景](#实际应用场景)
      - [配置系统](#配置系统)
      - [常量生命周期参数](#常量生命周期参数)
  - [优化的内存分配器（所有权（Ownership）和内存管理改进）](#优化的内存分配器所有权和内存管理改进)
...
- [Rust 1.91 类型系统（Type System）改进文档（历史版本）](#rust-191-类型系统改进文档历史版本)
  - [📊 目录](#-目录)
  - [概述](#概述)
...
- [Rust 1.91 稳定特性](#rust-191-稳定特性)
  ...
  - [函数性能优化](#函数性能优化)
    ...
    - [核心改进](#核心改进-7)
      - [1. 优化的迭代器（Iterator）链式调用](#1-优化的迭代器链式调用)
      - [2. 优化的递归函数](#2-优化的递归函数)
    - [性能对比](#性能对比-2)
  - [优化的条件语句和模式匹配（Pattern Matching）](#优化的条件语句和模式匹配)
    - [Rust 1.91 改进概述](#rust-191-改进概述-8)
    - [核心改进](#核心改进-8)
      - [1. 编译时条件计算](#1-编译时条件计算)
      - [2. 优化的模式匹配（Pattern Matching）](#2-优化的模式匹配)
  ...
  - [异步错误处理（Error Handling）改进](#异步错误处理改进)
