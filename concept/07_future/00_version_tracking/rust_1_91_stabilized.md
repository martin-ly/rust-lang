...
- [Rust 1.91 稳定特性](#rust-191-稳定特性)
  ...
  - [迁移内容（来自 `crates/c06_async/docs/rust_191_async_improvements.md`）](#迁移内容来自-cratesc06_asyncdocsrust_191_async_improvementsmd)
  - [概述](#概述-3)
  - [异步迭代器改进](#异步迭代器改进)
...
理解「改进的类型检查器（借用检查器优化）」需要把握 Rust 1.91 改进概述、核心改进、性能对比与使用示例，本节依次展开。
...
本节从借用检查器缓存机制 与 优化的借用检查算法 两个层面剖析「核心改进」。
...
「增强的 const 上下文（对生命周期的影响）」部分按 Rust 1.91 改进概述、核心改进与实际应用场景的顺序逐层展开。
...
本节从 const 上下文中的引用 与  const 上下文中的生命周期 两个层面剖析「核心改进」。
...
本节从配置系统 与 常量生命周期参数 两个层面剖析「实际应用场景」。
...
「优化的内存分配器（所有权和内存管理改进）」涉及 Rust 1.91 改进概述、核心改进与所有权管理改进，本节逐一说明其要点。
...
「改进的生命周期推断（编译时优化）」涉及 Rust 1.91 改进概述、核心改进与实际应用，本节逐一说明其要点。
...
本节从生命周期推断缓存 与 优化的推断算法 两个层面剖析「核心改进」。
...
本节从示例 1: 高性能借用检查、示例 2: const 上下文中的配置与示例 3: 小对象高频分配切入，剖析「实际应用示例」的核心内容。
...
「迁移指南」部分包含从 Rust 1.90 迁移到 Rust 1.91 与 兼容性说明 两条主线，本节依次说明。

### 从 Rust 1.90 迁移到 Rust 1.91

本节将「从 Rust 1.90 迁移到 Rust 1.91」分解为若干主题：更新 Rust 版本、利用新特性与性能优化建议。
...
本节将「改进的类型检查器（类型系统核心优化）」分解为若干主题： Rust 1.91 改进概述、核心改进、性能对比与使用示例。
...
本节从类型推断缓存机制 与 优化的类型检查算法 两个层面剖析「核心改进」。
...
本节将「增强的 const 上下文（类型推断改进）」分解为若干主题： Rust 1.91 改进概述1、核心改进1与实际应用场景。
...
本节围绕「核心改进1」展开，覆盖 const 上下文中的类型推断 与  const 上下文中的类型操作 两个方面。
...
本节从缓存架构、缓存策略与性能优势切入，剖析「类型推断缓存机制」的核心内容。
...
本节从 Rust 1.91 改进 与 性能对比1 两个层面剖析「泛型类型推断优化」。
...
本节围绕「const 上下文中的类型系统」展开，覆盖类型信息获取 与 类型推断 两个方面。
...
「实际应用示例」部分包含示例 1: 高性能类型推断 与 示例 2: const 上下文类型系统 两条主线，本节依次说明。
...
本节从 Rust 1.91 改进概述、核心改进与实际应用场景切入，剖析「const 上下文增强（在控制流中使用）」的核心内容。
...
本节从 const 上下文中的控制流 与  const 配置系统 两个层面剖析「核心改进」。
...
本节将「改进的 ControlFlow」分解为若干主题： Rust 1.91 改进概述、核心改进与实际应用。
...
本节从携带错误信息的 ControlFlow 与 早期退出循环 两个层面剖析「核心改进」。
...
本节从 Rust 1.91 改进概述、核心改进与性能对比切入，剖析「函数性能优化」的核心内容。
...
本节从优化的迭代器链式调用 与 优化的递归函数 两个层面剖析「核心改进」。
...
本节围绕「优化的条件语句和模式匹配」展开，覆盖 Rust 1.91 改进概述 与 核心改进 两个方面。
...
本节将「核心改进」分解为若干主题：编译时条件计算、优化的模式匹配与const 上下文中的模式匹配。
...
本节围绕「优化的循环结构」展开，覆盖 Rust 1.91 改进概述 与 核心改进 两个方面。
...
本节从优化的 for 循环 与  const 上下文中的循环 两个层面剖析「核心改进」。
...
本节从 Rust 1.91 改进概述 与 核心改进 两个层面剖析「函数调用优化」。
...
本节从函数调用缓存机制 与 优化的递归函数 两个层面剖析「核心改进」。
...
「闭包优化」部分包含 Rust 1.91 改进概述 与 核心改进 两条主线，本节依次说明。
...
本节从优化的闭包捕获 与 高阶函数优化 两个层面剖析「核心改进」。
...
本节从示例 1: 使用 const 配置系统、示例 2: 使用改进的 ControlFlow与示例 3: 优化的迭代器链切入，剖析「实际应用示例」的核心内容。
...
本节从更新 Rust 版本 与 利用新特性 两个层面剖析「从 Rust 1.90 迁移到 Rust 1.91」。
...
> 来源：`crates/c11_macro_system_proc/docs/rust_191_macro_improvements.md`
...
## 迁移内容（来自 `crates/c06_async/docs/rust_191_async_improvements.md`）

> <!-- migrated from crates/c06_async/docs/rust_191_async_improvements.md -->
>
> 以下内容根据 AGENTS.md §6.4 从 `crates/c06_async/docs/rust_191_async_improvements.md` 迁移至本权威页。
...
「异步迭代器改进」涉及 Rust 1.91 改进概述、核心改进与性能对比，本节逐一说明其要点。
...
本节从异步流处理性能提升 与 复杂异步管道性能提升 两个层面剖析「核心改进」。
...
「const 上下文增强在异步配置中的应用」部分按 Rust 1.91 改进概述1、核心改进1与实际应用场景的顺序逐层展开。
...
本节聚焦「核心改进1」，核心内容为 const 上下文中的异步配置。
...
本节从 Rust 1.91 改进概述2 与 核心改进2 两个层面剖析「JIT 编译器优化对异步代码的影响」。
...
本节围绕「核心改进2」展开，覆盖异步迭代器链式操作优化 与 异步批处理优化 两个方面。
...
本节围绕「内存分配优化对异步场景的影响」展开，覆盖 Rust 1.91 改进概述3 与 核心改进3 两个方面。
...
「核心改进3」部分包含异步小对象分配优化 与 异步 HashMap 操作优化 两条主线，本节依次说明。
...
理解「异步错误处理改进」需要把握 Rust 1.91 改进概述4、核心改进4与实际应用，本节依次展开。
...
本节从示例 1: 高性能异步流处理、示例 2: 异步任务管理器与示例 3: 异步缓存系统切入，剖析「实际应用示例」的核心内容。
