...
> **本节关键术语**: 属性 (Attribute) · 宏 (Macro) · 声明宏 (Declarative Macro) · 过程宏 (Procedural Macro) · derive — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# 属性与声明宏：编译期元编程基础
>
> **EN**: Macros
...
判定一个元编程需求该用属性还是宏（Macro）：改变「项的语义/编译方式」用属性（它是给编译器的注解）；「生成新代码」用宏。
...
宏与属性的技术细节决定「能匹配什么、能生成什么、在哪里生效」：

- **模式匹配与重复**：`macro_rules!` 的左侧模式由「字面 token + 元变量（`$name:fragment`）」构成，片段分类符（`expr`、`ident`、`ty`、`pat`、`stmt`、`block`、`meta`、`literal`、`tt` 等）决定能匹配哪种语法结构；`$( ... ),*` 形式的重复（`repetition`）支持 `*`/`+`/`?` 与可选分隔符，是生成列表类代码（如 `vec![]`）的机制。转录侧用同名 `$name` 与 `$(...)*` 展开，匹配与转录的重复结构必须对应。
- **递归宏**：宏可以在转录中调用自身，编译器设展开深度上限（默认 128，`#![recursion_limit]` 可调）。递归是声明宏（Declarative Macro）表达「循环」的唯一方式——处理变长输入（如实现 `impl Trait for (T1, T2, ..., Tn)` 的 tuple 链）的标准技巧。
- **常见属性详解**：`#[derive(...)]` 调用手写或过程宏（Procedural Macro） derive（有内建白名单：`Debug`、`Clone`、`PartialEq` 等）；`#[cfg(...)]` 条件编译在解析早期生效，被裁掉的代码甚至不需要语法合法之外的正确性；`#[inline]`/`#[cold]` 是优化提示而非保证。
...
本节测验聚焦「属性与宏的边界判别」，按认知层级递进：

- **理解层**：`#[derive(Debug)]` 与手写 `impl Debug` 的关系、`macro_rules!` 与函数调用的求值时机差异（展开期 vs 运行期）；
- **应用层**：为给定场景选择正确的宏形式——重复模式用声明宏、需要解析语法结构用过程宏、仅加标注用属性；
- **分析层**：`cfg` 条件编译与 `if cfg!()` 运行期分支的区别——前者裁掉的代码**不做类型检查**，这是跨平台代码最常见的编译错误来源。

作答建议：每题先给出「编译期会发生什么」的完整叙述再核对答案。
