...
- **递归宏**：宏可以在转录中调用自身，编译器设展开深度上限（默认 128，`#![recursion_limit]` 可调）。递归是声明宏表达「循环」的唯一方式——处理变长输入（如实现 `impl Trait for (T1, T2, ..., Tn)` 的 tuple 链）的标准技巧。
- **常见属性详解**：`#[derive(...)]` 调用手写或过程宏 derive（有内建白名单：`Debug`、`Clone`、`PartialEq` 等）；`#[cfg(...)]` 条件编译在解析早期生效，被裁掉的代码甚至不需要语法合法之外的正确性；`#[inline]`/`#[cold]` 是优化提示而非保证。
