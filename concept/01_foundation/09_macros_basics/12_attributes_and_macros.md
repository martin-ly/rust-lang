...
- [属性与声明宏（Declarative Macro）：编译期元编程基础](12_attributes_and_macros.md)
  ...
  - [相关概念](#相关概念)
  - [权威来源索引](#权威来源索引)
  - [十二、边界测试：属性与宏（Macro）的编译错误](#十二边界测试属性与宏的编译错误)
    - [12.1 边界测试：`derive` 宏（Macro）的 trait 冲突（编译错误）](#121-边界测试derive-宏的-trait-冲突编译错误)
    - [12.2 边界测试：宏（Macro）递归展开溢出（编译错误）](#122-边界测试宏递归展开溢出编译错误)
  ...
  - [认知路径](#认知路径)
    ...
    - [反命题与边界](#反命题与边界)
  - [嵌入式测验](#嵌入式测验)
    - [测验 1：属性分类（记忆层）](#测验-1属性分类记忆层)
    - [测验 2：宏（Macro）的卫生性（理解层）](#测验-2宏的卫生性理解层)
    - [测验 3：宏（Macro）模式匹配（Pattern Matching）（应用层）](#测验-3宏模式匹配应用层)
    - [测验 4：`derive` 与浮点数（分析层）](#测验-4derive-与浮点数分析层)

---

## 一、核心概念

本节将「核心概念」分解为若干主题：属性系统全景、声明宏基础与宏的卫生性。
...
> **宏（Macro）洞察**: `macro_rules!` 是 Rust **声明式元编程**的基础工具——它比 C 预处理器**更安全**（语法树级操作）且**更强大**（模式匹配）。
...
「技术细节」部分按模式匹配与重复、递归宏与常见属性详解的顺序逐层展开。
...
本节从边界测试：`derive` 宏的 trait 冲突（编译错误）、边界测试：宏递归展开溢出（编译错误）、边界测试：过程宏的 hygiene 与标识符捕获（编译错误）、边界测试：`cfg` 条件编译的互斥性（编译错误/逻辑错误）等5个方面切入，剖析「边界测试：属性与宏的编译错误」的核心内容。
...
| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 属性与声明宏（Declarative Macro）：编译期元编程基础 基础定义 ⟹ 正确用法 | 理解语法与语义 | 能写出符合惯用法的代码 | 高 |
| 属性与声明宏（Declarative Macro）：编译期元编程基础 正确用法 ⟹ 常见陷阱 | 忽略边界条件 | 编译错误或运行时（Runtime） bug | 高 |
| 属性与声明宏（Declarative Macro）：编译期元编程基础 常见陷阱 ⟹ 深度掌握 | 系统学习反模式 | 能进行代码审查与优化 | 高 |

> 元编程安全 ⟸ 编译期代码生成 ⟸ 宏卫生性
> 条件编译正确 ⟸ cfg 属性求值 ⟸ 平台抽象
> **过渡**: 掌握 属性与声明宏（Declarative Macro）：编译期元编程基础 的基础语法后，下一步需要理解其在类型系统（Type System）中的位置与与其他概念的交互关系。
> **过渡**: 在实践中应用 属性与声明宏（Declarative Macro）：编译期元编程基础 时，务必关注边界条件与异常处理，这是从"能编译"到"能生产"的关键跃迁。
> **过渡**: 属性与声明宏（Declarative Macro）：编译期元编程基础 的设计理念体现了 Rust 零成本抽象（Zero-Cost Abstraction）与安全保证的核心权衡，理解这一权衡有助于迁移到更高级的并发与形式化验证领域。

### 反命题与边界

> **反命题**: "属性与声明宏（Declarative Macro）：编译期元编程基础 在所有场景下都是最佳选择" —— 错误。需要根据具体上下文权衡性能、可读性与安全性，某些场景下显式替代方案可能更优。

---

## 嵌入式测验

本节将「嵌入式测验」分解为若干主题：测验 1：属性分类（记忆层）、测验 2：宏的卫生性（理解层）、测验 3：宏模式匹配（应用层）与测验 4：`derive` 与浮点数（分析层）。

### 测验 1：属性分类（记忆层）

**题目**: `#[cfg(target_os = "linux")]` 属于哪类属性？它的作用是什么？

- A. 诊断属性；在 Linux 上 suppress 警告
- B. 条件编译属性；仅在目标 OS 为 Linux 时包含该项
- C. 派生属性；为函数自动实现 Linux 特定的 trait
- D. 链接属性；指定链接到 Linux 系统库
...
`#[cfg(...)]` 是**条件编译属性**，根据编译目标选择性地包含/排除代码：

- `#[cfg(target_os = "linux")]` → 仅在 Linux 目标时编译该项
- `#[cfg(feature = "serde")]` → 仅在启用了 `serde` feature 时编译
- `#[cfg(all(unix, not(target_os = "macos")))]` → 组合条件

`cfg` 在编译期求值：条件为假时，代码**完全不被解析和类型检查**（不同于 C 的 `#ifdef`，后者仍需语法正确）。
...
Rust 宏具有**卫生性（Hygiene）**——宏内部定义的标识符和外部作用域是隔离的：

1. 宏内部的 `x` 在宏定义处解析，而不是调用处。
2. 宏定义处没有 `x`（`macro_rules!` 块内没有声明 `x`），因此编译错误。
3. 外部作用域的 `x` 不会被宏捕获。

**与 C 预处理器的对比**: C 的 `#define` 是纯文本替换，会意外捕获外部变量，导致命名冲突。Rust 的卫生性从根本上消除了这类错误。

**正确做法**: 显式传递需要的变量：`print_value!(x)`。
</details>
...
- `$($x:expr),+` → 一个或多个表达式，逗号分隔（`+` 表示至少一个）
- `$(,)?` → 可选的尾部逗号（`?` 表示零或一个）

匹配结果：

1. **① `my_macro!(1)`**: 一个表达式 ✅
2. **② `my_macro!(1, 2, 3)`**: 三个表达式，逗号分隔 ✅
3. **③ `my_macro!(1, 2, 3,)`**: 三个表达式 + 尾部逗号 ✅
4. **④ `my_macro!()`**: 空，不满足 `+`（至少一个）❌

若要让 ④ 也合法，需要增加一个分支：`() => { ... }` 或改用 `*`（零或多个）。
</details>

---

### 测验 4：`derive` 与浮点数（分析层）
...
`#[derive(Eq)]` 要求所有字段实现 `Eq`（等价关系，要求反身性：`a == a`）。`f64` 因 IEEE 754 NaN 语义（`NaN != NaN`）未实现 `Eq`，只实现了 `PartialEq`。

同理，`#[derive(Ord)]` 也会失败，因为 `f64` 未实现 `Ord`（全序关系）。

**修复方案**: 移除 `Eq`（和 `Ord`）：

```rust
#[derive(Debug, Clone, PartialEq)]  // ✅ 仅保留 PartialEq
struct Point {
    x: f64,
    y: f64,
}
