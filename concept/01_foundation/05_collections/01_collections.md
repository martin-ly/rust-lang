...
>
> **本节关键术语**: 集合 (Collection) · 向量 (Vec) · 哈希映射 (HashMap) · 哈希集合 (HashSet) · 二叉堆 (BinaryHeap) — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
...
> **后置概念**: [Smart Pointers](../../02_intermediate/02_memory_management/04_smart_pointers.md) ·
...
- **下层概念**: [Smart Pointers](../../02_intermediate/02_memory_management/04_smart_pointers.md)
...
- [Smart Pointers](../../02_intermediate/02_memory_management/04_smart_pointers.md) — 智能指针（Smart Pointer）
...
集合边界测试按「编译期拒绝」与「运行时 panic」两类组织，区分标准是错误能否在类型层面表达：

- **编译错误**：`HashMap` 键未实现 `Hash + Eq`（E0277，约束不满足）；迭代器消费后重复使用（E0382，`into_iter` 已 move 集合）；`Entry` 持有时再借用同一 map（E0502，可变借用冲突）。这些错误反映集合 API 的类型契约：键的能力要求、迭代的所有权语义、借用的排他性。
- **运行时 panic**：`Vec::drain` 范围越界、索引越界（`v[i]` 的 `Index` impl 调用 `panic!`）、`remove` 空 `BinaryHeap` 返回 `None` 后的 `unwrap`。这类错误无法用类型静态排除（索引值依赖运行时数据），标准库的选择是 panic 而非 UB——与 C++ `vector::operator[]` 的未定义行为形成对照。

判定某个集合操作属于哪类错误，只需查其签名：返回 `Option`/`Result` 或带 `#[track_caller]` panic 文档的是运行时检查；要求 trait bound 或产生借用冲突的则是编译期拒绝。
...
本节测验覆盖标准库集合的核心判别点，按认知层级递进：

- **理解层**：`Vec`/`VecDeque`/`LinkedList` 的缓存行为差异——前两者连续内存、后者每节点独立分配（指针追逐，实测几乎总是更慢）；
- **应用层**：`HashMap` vs `BTreeMap` 选型——哈希 O(1) 但无序、B 树 O(log n) 但有序且范围查询高效；`entry` API 避免双重查找；
- **分析层**：迭代失效问题——Rust 用借用检查在编译期拒绝「迭代中修改集合」，与 Java 的 `ConcurrentModificationException`（运行期）对比，理解「把并发修改错误提前到编译期」的设计价值。

作答建议：测验前先默写各集合的「时间复杂度 × 内存布局 × 迭代顺序」三列表，再逐题核对。
...
这是 Rust 借用（Borrowing）检查器防止迭代器失效的经典保护。
...
>
> - 本地映射与样题：[所有权清单自测：Brown University Ownership Inventory](../01_ownership_borrow_lifetime/06_ownership_inventories_brown_book.md)
