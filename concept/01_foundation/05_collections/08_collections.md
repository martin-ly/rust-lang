...
>
> **本节关键术语**: 集合 (Collection) · 向量 (Vec) · 哈希映射 (HashMap) · 哈希集合 (HashSet) · 二叉堆 (BinaryHeap) — [完整对照表](../../00_meta/01_terminology/terminology_glossary.md)
...
> **后置概念**: [Smart Pointers](../../02_intermediate/02_memory_management/12_smart_pointers.md) ·
...
- [集合类型：Rust 标准库的数据结构谱系](#集合类型rust-标准库的数据结构谱系)
  ...
  - [十四、边界测试：集合的编译错误](#十四边界测试集合的编译错误)
    ...
    - [14.2 边界测试：迭代器（Iterator）消费后重复使用（编译错误）](#142-边界测试迭代器消费后重复使用编译错误)
    - [10.3 边界测试：`Vec::drain` 的范围越界（运行时（Runtime） panic）](#103-边界测试vecdrain-的范围越界运行时-panic)
    ...
    - [10.4 边界测试：不可变借用（Immutable Borrow）与可变借用的冲突](#104-边界测试不可变借用与可变借用的冲突)
  - [实践](#实践)
  - [参考来源](#参考来源)
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  ...
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    ...
    - [测验 2：HashMap 所有权（Ownership）（应用层）](#测验-2hashmap-所有权应用层)
    - [测验 3：迭代器与借用（Borrowing）（分析层）](#测验-3迭代器与借用分析层)
...
- [Smart Pointers](../../02_intermediate/02_memory_management/12_smart_pointers.md) — 智能指针（Smart Pointer）
...
>
> - 本地映射与样题：[所有权清单自测：Brown University Ownership Inventory](../01_ownership_borrow_lifetime/28_ownership_inventories_brown_book.md)
