...
本节的 Mermaid 图把「高级集合类型：BTreeMap、VecDeque、BinaryHeap 与自定义 Hasher 深度分析」压缩为可视化结构：集合选型决策树 与 内存布局对比图。节点是概念或组件，边标注「依赖 / 细化 / 数据流」关系；看图顺序建议自顶向下先建立骨架，再沿边追问每条关系在正文哪一节有完整论证。图是索引而非替代品，细节与边界条件仍以正文为准。
...
- **反命题 1：「`HashMap` 总是比 `BTreeMap` 快」** —— 有边界。小键集（<50 项）时 B 树的缓存友好性常反超哈希计算成本；有序遍历、范围查询、前缀扫描场景 B 树是唯一选择；哈希的另一隐性成本是 DoS 抗性（`HashMap` 默认 SipHash 有防护，换 `FxHash` 提速即放弃防护）。判定准则：需要顺序 ⟹ B 树；纯点查且规模大 ⟹ 哈希。
- **反命题 2：「`Vec` 是所有场景的最佳默认」** —— 过度。头部插入频繁 ⟹ `VecDeque`（环形缓冲 O(1) 双端）；频繁中间插入删除且遍历少 ⟹ 才考虑 `LinkedList`（实测中缓存不友好多数抵消理论优势）；读远多于写且需共享 ⟹ `Arc<[T]>` 或 `im` 的持久化结构。

边界极限小节量化：各集合的摊还复杂度与常数因子、`Entry` API 避免的双重查找、以及自定义哈希的一致性要求（`Eq` ⟹ 同哈希）。
...
> **过渡**: 理解高级集合的内存布局后，可进一步根据访问模式、顺序需求和 `Ord`/`Hash` 约束选择合适容器。
> **过渡**: 将高级集合与迭代器适配器结合，可构建高效的数据处理管线，同时注意不同集合的消耗语义。
> **过渡**: 从标准集合实现出发，可进入自定义数据结构、allocator API 与 unsafe 内部布局的深入学习。
