...
映射与过滤是迭代器链中最常见的数据转换模式。`filter_map`、`flat_map` 和 `find_map` 把两个基本操作压缩成一次惰性遍历，既减少中间集合的分配，也避免了手写循环时常见的索引越界和所有权（Ownership）纠纷。掌握这些组合子，能用更短的代码表达“转换并筛选”或“一对多展开”的意图，同时让编译器保留零成本优化的机会。
...
聚合操作把序列折叠为单个值，而短路组合在失败或可空路径上提前终止遍历。`reduce`、`try_fold` 和 `try_for_each` 利用 Rust 的所有权与 `Result` 类型系统，把错误处理嵌入迭代链，避免在循环里手动维护累加器和返回标志。它们特别适合需要“要么成功求值，要么在第一个错误处停止”的场景，使控制流与数据流保持一致。
...
当序列需要按条件拆分、跳过前缀或按固定步长取样时，分组与切片组合提供了声明式的解决方案。`partition` 在一次遍历中完成分类，`skip_while` / `take_while` 利用惰性求值精确控制窗口边界，`step_by` 则以零额外内存实现下采样。这些适配器的共同特点是把状态机式的循环转换为类型安全、可组合的迭代器表达式。
...
多流组合把两个或多个迭代器对齐、串联或循环展开，形成更复杂的输入流。`zip` 按最短长度安全配对，`chain` 顺序拼接而不产生临时容器，`cycle` 则把有限模式扩展为无限流。由于 Rust 迭代器是惰性的，这些组合在构建流水线时不会提前物化数据，从而与所有权借用（Borrowing）机制协同，减少不必要的克隆。
...
状态化适配器在遍历过程中维护额外上下文，如索引、预览位置或调试标记。`enumerate`、`peekable`、`fuse`、`inspect` 和 `by_ref` 解决的是“需要知道遍历状态但不破坏链式调用”的问题，它们通过借用（Borrowing）原迭代器来避免所有权转移。合理使用这些适配器，可以让解析器、调试输出和分段消费代码保持声明式风格，同时精确控制哪些状态被推进、哪些被保留。
...
惯用法的力量也伴随着误用风险。本节收集若干常见陷阱：把 `filter_map` 当成 `filter` 使用、在链式求值中过早 `collect`、以及忽略 `zip` 的静默截断行为。每个反例都展示一种“能编译但非最佳”的写法，并给出更符合 Rust 零成本抽象和类型安全原则的修正，帮助读者在代码审查中快速识别这些问题。
...
边界测试把迭代器适配器推向长度不匹配、前缀语义和无限循环等极限场景。它们验证的不仅是 API 行为，更是 Rust 类型系统与运行时契约的交互：某些错误在编译期被阻止，另一些则通过运行时 panic 或显式长度检查暴露。通过边界用例，可以理解为什么“惰性求值”不等于“无限制求值”，以及如何在工程代码中为这些边界添加断言。
