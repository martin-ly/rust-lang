...
> **主要来源**: [TRPL 3rd Ed: Ch4.2](https://doc.rust-lang.org/book/ch04-02-references-and-borrowing.html) · [Brown University Interactive Book: Ch4.2](https://rust-book.cs.brown.edu/ch04-02-references-and-borrowing.html) · [Wikipedia: Reference (computer science)](https://en.wikipedia.org/wiki/Reference_(computer_science)) · [Rust Reference: References](https://doc.rust-lang.org/reference/types/pointer.html#reference-types)
...
- v1.1 (2026-06-19): 对齐 TRPL 3rd Ed 与 Brown University Interactive Book；引用 "Fixing Ownership Errors" 小节研究
...
> **来源: [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/)** 借用的形式化语义为"所有权的临时授权"，不改变资源的最终归属（所有权归还）。 ✅
...
> **来源: [Rust Reference: NLL](https://doc.rust-lang.org/reference/nll.html)** T4 — 非词法生命周期基于控制流图的精确存活期分析 ([RFC 2094](https://rust-lang.github.io/rfcs//2094-nll.html))。 ✅
> **来源: [RFC 2025](https://github.com/rust-lang/rfcs/pull/2025)** T6 — Two-Phase Borrows 允许方法调用中的临时不可变借用。 ✅
...
> **来源: [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/)** "分离逻辑: 分数权限" — 不可变借用将所有权分割为只读分数，可变借用要求完整独占权限。 ✅
> **来源: [Tofte & Talpin 1994](https://en.wikipedia.org/wiki/Region-based_memory_management)** "区域类型: 偏序约束" — 引用的生命周期不能超过被引用数据的区域。 ✅
> **来源: [Rust Reference: NLL](https://doc.rust-lang.org/reference/nll.html)** "NLL: 实际使用期有效" — 非词法生命周期基于数据流分析 ([RFC 2094](https://rust-lang.github.io/rfcs//2094-nll.html))。 ✅
...
1. **这段代码是否真的不安全？** 如果是，找到不安全的根因（生命周期、别名、权限）。
...
- 缩短 `largest` 生命周期：先比较长度并收集结果，再统一 `dst.extend`
...
| `RefCell<T>` 运行时借用检查 | [TRPL Ch15.5](https://doc.rust-lang.org/book/) | ✅ |
| `UnsafeCell<T>` 与内部可变性 | [TRPL Ch19.1](https://doc.rust-lang.org/book/ch19-01-unsafe-rust.html) · [Rust Reference: Interior Mutability](https://doc.rust-lang.org/reference/) | ✅ |
| `Pin<&mut T>` 与自引用结构 | [TRPL Ch20.3](https://doc.rust-lang.org/book/) · [Rust Reference: Pin](https://doc.rust-lang.org/std/pin/struct.Pin.html) | ✅ |
| NLL (Non-Lexical Lifetimes) | [Rust Reference: NLL](https://doc.rust-lang.org/reference/lifetime-elision.html) · [RFC 2094](https://rust-lang.github.io/rfcs/2094-2094-nll.html) | ✅ |
...
> **[Rust Reference: Deref](https://doc.rust-lang.org/reference/items/traits.html)** · **[Rust Reference: DerefMut](https://doc.rust-lang.org/reference/items/traits.html)** · **[TRPL Ch15.2](https://doc.rust-lang.org/book/ch15-02-deref.html)** `Deref` 和 `DerefMut` 是 Rust 的**强制解引用（deref coercion）**机制的核心。它们允许智能指针和包装类型**自动转换为内部类型的引用**，从而无缝使用内部类型的方法。✅
