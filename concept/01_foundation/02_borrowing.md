...
> **[来源: Rust Reference: References]** Rust 引用分 `&T`（共享只读）和 `&mut T`（独占可写），由编译器在类型检查和借用检查阶段强制执行。 ✅
...
| **Rust 版本特性演进** | [`../07_future/rust_version_tracking.md`](../07_future/rust_version_tracking.md) | `let_chains`、模式匹配扩展 |
...
> **[Rust Reference: std::borrow::Cow]** · **[TRPL: Ch15.4]** `Cow<T>`（Clone on Write）是 Rust 标准库中**借用与所有权的桥接类型**。它允许代码在**不需要修改时零成本借用**，在**需要修改时才克隆获得所有权**。这是"延迟付费（pay-as-you-go）"策略在类型系统中的体现。✅
...
>
> **来源**: [Rust Reference: Cow] · [TRPL: Ch15.4] · [Wikipedia: Copy-on-write]

---

### 8.2 补充：`Deref` / `DerefMut` 与自动借用的交互

> **[Rust Reference: Deref]** · **[Rust Reference: DerefMut]** · **[TRPL: Ch15.2]** `Deref` 和 `DerefMut` 是 Rust 的**强制解引用（deref coercion）**机制的核心。它们允许智能指针和包装类型**自动转换为内部类型的引用**，从而无缝使用内部类型的方法。✅
...
> **[Rust Reference: AsRef]** · **[Rust Reference: AsMut]** `AsRef<T>` 和 `AsMut<T>` 提供**廉价的引用转换**，但与 `Deref` 有本质区别：`Deref` 是"我是 T 的智能指针/包装器"，`AsRef` 是"我可以被看作 T"。✅
...
>
> **来源**: [Rust Reference: Deref] · [Rust Reference: AsRef] · [TRPL: Ch15.2] · [Rust API Guidelines: Deref / AsRef]
...
## 九、`let chains`：模式匹配的逻辑合取扩展（1.88 stable，RFC 2497）
