...
> 三条红色路径展示了三种突破方式，按「隐蔽性」排列：UnsafeCell（合法 API 但内部用 unsafe）、裸指针（显式 unsafe）、transmute（类型系统欺骗）。
...
>
> 此图修正了「&T 总是只读」的直觉误解。
> 关键认知：&T 的「只读」是编译期保证，但存在三类运行时例外——RefCell（内部可变性，运行时 panic 守卫）、Mutex（同步原语，运行时锁竞争）、UnsafeCell（unsafe 封装，信任程序员）。
> 这三类都不破坏内存安全（无 UB），但改变了「&T 意味着无人修改」的直觉。
