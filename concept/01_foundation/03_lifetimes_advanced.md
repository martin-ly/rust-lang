...
> **本节关键术语**: 生命周期省略 (Lifetime Elision) · HRTB (Higher-Ranked Trait Bounds) · 协变 (Covariance) · 逆变 (Contravariance) · 不变 (Invariance) — [完整对照表](../00_meta/terminology_glossary.md)
>
# Lifetimes 高级主题
>
...
> 这种困惑源于将"引用"理解为无成本的轻量指针，而忽略了引用背后的**时效契约**——它必须指向在特定时段内保证存活的数据。
...
> 边界测试回答：
> 'static 可以通过泄漏安全地构造吗？HRTB 的闭包能接受多短的引用？
> NLL 在循环引用交叉时是否仍然精确？
...
Elision 不是语法便捷性的简单堆砌，而是一组基于 Hindley-Milner 风格模式匹配的完备推导规则。
...
> **修正**:
> `for<'a> Fn(&'a str) -> &'a str` 要求闭包对**所有**生命周期 `'a` 都返回与输入相同生命周期的引用。
