...
> **EN**: Patterns
> **Summary**: Rust 模式匹配的权威规范：解构、可反驳性、各种模式形式（literal、identifier、wildcard、rest、range、reference、struct、tuple、slice、path、or-patterns）及其绑定模式。
>
> **受众**: [初学者]
> **内容分级**: [综述级]
> **Bloom 层级**: 理解 → 应用
...
**模式（pattern）** 用于将值与结构进行匹配，并可选地将变量绑定到这些结构内部的值。模式还用于变量声明、函数/闭包参数等场景。
...
当引用值被非引用模式匹配时，编译器会自动按 `ref` 或 `ref mut` 绑定，避免手动写 `&`。
...
匹配任意单个值，不绑定、不 move、不借用。总是不可反驳。

### Rest pattern (`..`)

匹配零个或多个剩余元素，用于元组、元组结构体、切片模式。不可反驳。
...
| [Destructuring](../02_intermediate/02_generics.md) | 模式解构与泛型结合使用 |
