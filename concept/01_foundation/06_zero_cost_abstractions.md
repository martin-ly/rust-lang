...
- [零成本抽象（Zero-Cost Abstraction）：Rust 的性能哲学](06_zero_cost_abstractions.md)
  ...
  - [相关概念文件](#相关概念文件)
  - [权威来源索引](#权威来源索引)
  - [十二、边界测试：零成本抽象（Zero-Cost Abstraction）的编译错误](#十二边界测试零成本抽象的编译错误)
    ...
    - [12.3 边界测试：泛型（Generics）递归类型的大小计算（编译错误）](#123-边界测试泛型递归类型的大小计算编译错误)
    - [12.4 边界测试：`dyn Trait` 的大小未知（编译错误）](#124-边界测试dyn-trait-的大小未知编译错误)
    - [10.3 边界测试：泛型单态化（Monomorphization）导致的代码膨胀（编译错误/链接错误）](#103-边界测试泛型单态化导致的代码膨胀编译错误链接错误)
...
> 与 JavaScript 的 Promise（无自引用问题，因为所有变量在闭包（Closures）中按引用捕获）或 Go 的 goroutine（栈可增长和移动，通过指针重定位处理）不同
