...
> [Goodenough — Structured Exception Handling](https://doi.org/10.1145/512760.512764) ·
...
>
> **前置概念**:
> [Error Handling Basics](../08_error_handling/01_error_handling_basics.md) ·
...
**知识点**：`?` 运算符消除了 Go/Java 中冗长的 `if err != nil` 或 `try-catch` 样板代码，同时保持显式错误传播。[→ 错误处理（Error Handling）基础详解](../08_error_handling/01_error_handling_basics.md)
...
**知识点**：`unwrap` 是 Rust 中最常见的 panic 来源之一。生产代码应尽量避免，或用 `unwrap_or` / `unwrap_or_else` 替代。[→ 错误处理（Error Handling）基础详解](../08_error_handling/01_error_handling_basics.md)
...
**知识点**：`Option` 组合子允许在不展开值的情况下进行链式操作，避免嵌套 `match`。[→ 集合类型详解](../05_collections/01_collections.md)
...
**知识点**：Rust 提供多种 Option 解构语法，选择取决于"需要处理多少种情况"和"是否需要提前返回"。[→ 控制流详解](../04_control_flow/01_control_flow.md)
...
**知识点**：Rust 区分"可恢复错误"（`Result`）和"不可恢复错误"（`panic`）。选择正确的错误处理（Error Handling）策略是健壮程序的基础。[→ Panic 详解](../08_error_handling/03_panic_and_abort.md)
...
**知识点**：`Option` 组合子的链式调用是 Rust 函数式编程风格的典型表现，每个操作都是"失败即短路"。[→ 错误处理（Error Handling）基础详解](../08_error_handling/01_error_handling_basics.md)
...
本节聚焦「自定义错误」，核心内容为 Q8. 以下代码能否编译？解释 `From` trait 在错误处理（Error Handling）中…。
...
**知识点**：`From` trait 是实现统一错误类型的关键，`?` 运算符的自动转换能力消除了手动包装错误的大量样板代码。[→ 错误处理（Error Handling）进阶](../../02_intermediate/03_error_handling/01_error_handling.md)
...
**知识点**：`map` / `map_err` 是 `Result` 的"端点转换"工具，允许在不展开的情况下修改成功值或错误值。[→ 错误处理（Error Handling）基础详解](../08_error_handling/01_error_handling_basics.md)
...
> [Error Handling Basics](../08_error_handling/01_error_handling_basics.md) 权威页一致；
> 干扰项针对常见误解设计。

### Q11. 🟢【单选】`Option<T>` 的两个变体是什么？
...
**解析**：`Option<T>` 表示"可能存在也可能不存在的值"，变体为 `Some(T)` 与 `None`。`Ok`/`Err` 属于 `Result<T, E>`（A 是典型混淆：两者都是双变体枚举（Enum），但语义不同——`Option` 无错误信息，`Result` 的 `Err` 携带错误详情）。

</details>

---

### Q12. 🟡【单选】按照 Rust 官方建议，何时应该使用 `panic!` 而不是返回 `Result`？

- A. 任何错误都应 `panic!`，`Result` 只用于测试
- B. 错误不可恢复、程序进入无效状态时（如原型代码、测试、以及"按逻辑不可能发生"的情况）
- C. 库代码中的所有错误都应 `panic!`，方便调用方处理
- D. 性能要求高的场景，`panic!` 比 `Result` 快
...
**解析**：TRPL 第 9 章的准则：可恢复错误用 `Result` 把决策权交给调用方；不可恢复错误（违反程序不变量、bug、不可能分支）才 `panic!`。库代码随意 `panic!`（C）会剥夺调用方的错误处理（Error Handling）权，是反模式；`panic!` 的栈展开成本远高于返回 `Result`（D 恰好相反）。

</details>

---

### Q13. 🟡【多选】关于 `?` 运算符，下列说法正确的有？（选出所有正确项）

- A. 只能用于返回 `Result` 或 `Option`（等实现了对应 `Try` 语义）的函数中
- B. 传播错误时会自动调用 `From` 进行错误类型转换
- C. 与 `unwrap()` 行为完全相同
- D. 遇到 `Err`（或 `None`）时从当前函数提前返回
...
**解析**：`?` 是"匹配-提前返回"的语法糖：对 `Ok(v)` 解包出 `v`，对 `Err(e)` 执行 `return Err(From::from(e))`（B、D）。C 错误：`unwrap()` 遇到错误直接 **panic**，`?` 则是**传播**错误，行为截然不同——这正是 `?` 存在的意义。

</details>

---

### Q14. 🟢【判断】`unwrap()` 在值为 `Err` 或 `None` 时会 panic，因此生产代码中应谨慎使用，优先用 `?`、`match` 或组合子处理。（对 / 错）
...
**解析**：`unwrap` 等价于"若失败则崩溃程序"。原型与测试中可接受；生产代码中一次意外的 `None`/`Err` 会导致整个进程中止。`expect("理由")` 至少留下上下文，但仍属 panic 路径。Clippy 提供 `unwrap_used` lint 可强制禁用。

</details>

---

### Q15. 🟡【判断】Rust 的 `Result` 错误处理机制与 C++ 异常一样，依赖运行时栈展开，有隐式的性能开销。（对 / 错）
...
**解析**：`Result` 就是普通枚举（Enum）值，错误传播是正常的值返回，编译为常规分支代码，零额外运行时（Runtime）开销；错误的类型与处理路径在编译期完全可见（"显式错误处理"）。栈展开只发生在 `panic!` 时（且可用 `panic = "abort"` 关闭）。这正是 Rust 错误处理"零成本 + 不可忽略"双重优势的来源。
...
| 得分 | 评价 | 建议 |
|:---:|:---|:---|
| 10/10 | 🏆 错误处理（Error Handling）已内化 | 进阶至 [L2 错误处理深度](../../02_intermediate/03_error_handling/01_error_handling.md) 或 `anyhow`/`thiserror` 生态 |
| 7–9/10 | ✅ 核心概念掌握 | 强化 [L1 错误处理（Error Handling）练习](../../exercises/src/error_handling)，关注错题 |
| 4–6/10 | 🔄 需巩固基础 | 重读 [Error Handling Basics](../08_error_handling/01_error_handling_basics.md)，完成 rustlings 对应章节 |
| 0–3/10 | 📚 建议重新开始 | 从 [Error Handling Basics](../08_error_handling/01_error_handling_basics.md) 逐节阅读，配合 `exercises/src/error_handling/` 示例 |
...
| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| `Result` 显式传播 ⟹ 调用点知晓错误 | 使用 `?` / `match` / `if let` | 不遗漏错误分支 | 高 |
| `Option` 与 `Result` 转换 ⟹ 错误语义清晰 | 明确「无值」与「失败」 | 避免把缺失当错误 | 高 |
| panic 仅用于不可恢复 ⟹ 控制流不滥用 | 区分 bug 与预期错误 | 代码可维护性提升 | 高 |

> **过渡**: 测验后返回错误处理权威页，巩固 `?` 操作符与 `From` 错误转换的联合使用。
> **过渡**: 将错题按主题归类（传播、转换、panic、测试），可进入对应专项练习。
> **过渡**: 通过测验验证后，可尝试阅读 `crates/` 中错误处理相关示例并修改运行。

> 错误分支不遗漏 ⟸ `Result` 显式返回 ⟸ `?` 自动传播
> 不可恢复路径受控 ⟸ panic 用于 bug 信号 ⟸ 异常安全设计

---

## 反命题与边界

> **反命题**: "所有函数都应返回 `Result` 以最大化安全。" —— 错误。不可恢复的内部不变量破坏应使用 panic；过度使用 `Result` 会使 API 噪音增加且暗示错误可恢复。
> **边界**: 本测验聚焦 L1 基础；自定义错误类型、`thiserror`/`anyhow` 生态与 FFI 错误边界需另见对应概念页。
