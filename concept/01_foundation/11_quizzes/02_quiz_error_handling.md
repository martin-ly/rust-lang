...
**解析**：TRPL 第 9 章的准则：可恢复错误用 `Result` 把决策权交给调用方；不可恢复错误（违反程序不变量、bug、不可能分支）才 `panic!`。库代码随意 `panic!`（C）会剥夺调用方的错误处理权，是反模式；`panic!` 的栈展开成本远高于返回 `Result`（D 恰好相反）。
...
**解析**：`Result` 就是普通枚举值，错误传播是正常的值返回，编译为常规分支代码，零额外运行时（Runtime）开销；错误的类型与处理路径在编译期完全可见（"显式错误处理"）。栈展开只发生在 `panic!` 时（且可用 `panic = "abort"` 关闭）。这正是 Rust 错误处理"零成本 + 不可忽略"双重优势的来源。
