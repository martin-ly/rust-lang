...
Q1. 以下代码能否编译？解释闭包（Closures）的类型推断（Type Inference）、
...
「迭代器（Iterator）适配器」部分包含 Q6. 以下代码的输出是什么？解释 `filter`、`map` 和… 与  Q7. 以下代码的输出是什么？`fold` 与 `reduce` 的区别 两条主线，本节依次说明。
...
> [Closure Basics](../00_start/03_closure_basics.md)、
> [Iterator Patterns](../../02_intermediate/07_iterators_and_closures/01_iterator_patterns.md) 权威页一致；
> 干扰项针对常见误解设计。

### Q11. 🟡【单选】闭包以哪种方式捕获环境变量，是由什么决定的？

- A. 一律按移动（move）捕获
- B. 闭包体内**如何使用**被捕获的值：只读则不可变借用（Immutable Borrow），修改则可变借用，消耗则获取所有权
...
### Q12. 🟢【单选】迭代器适配器（如 `map`、`filter`）的核心执行特性是？

- A. 调用时立即遍历集合并完成转换
- B. 惰性求值：仅构建计算描述，直到被消费（`collect`、`for`、`sum` 等）才真正执行
- C. 总是先分配一个新的 `Vec` 存放中间结果
- D. 不能链式组合，每次只能用一个适配器
...
### Q13. 🟡【多选】关于 `Fn` / `FnMut` / `FnOnce` 三个闭包 trait，下列说法正确的有？（选出所有正确项）

- A. 所有闭包至少实现 `FnOnce`（因为任何闭包至少能被调用一次）
- B. 实现 `Fn` 的闭包可以多次调用且不修改捕获的环境
- C. trait 之间存在子集关系：`Fn` ⇒ `FnMut` ⇒ `FnOnce`
- D. 加了 `move` 的闭包一定只能实现 `FnOnce`
...
### Q14. 🟢【判断】Rust 迭代器是零成本抽象：编译器优化后，`map`/`filter`/`collect` 链的性能通常与手写索引循环相当。（对 / 错）
...
### Q15. 🟡【判断】`collect()` 只能从迭代器构建 `Vec`，其他集合需要先收集为 `Vec` 再转换。（对 / 错）
...
> **认知路径**: 通过本测验回顾闭包与迭代器核心概念，将错题映射回 [Closures Basics](../00_start/03_closure_basics.md) 与 L2 迭代器权威页进行针对性补强。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 闭包捕获环境 ⟹ 回调与延迟计算自然 | 理解 `Fn`/`FnMut`/`FnOnce` | 选择正确闭包 trait | 高 |
| 迭代器适配器链式 ⟹ 声明式数据流 | 组合 `map`/`filter`/`fold` | 避免手动索引循环 | 高 |
| 所有权消耗适配器区分 ⟹ 资源安全 | 区分 `iter`/`iter_mut`/`into_iter` | 不越界借用或重复消耗 | 高 |

> **过渡**: 测验后返回闭包基础页，巩固捕获环境与 trait 选择规则。
> **过渡**: 将错题按主题归类（闭包捕获、迭代器适配器、所有权消耗），可进入对应专项练习。
> **过渡**: 通过测验验证后，可尝试阅读 `crates/` 中迭代器与闭包相关示例并修改运行。

> 回调与流水线简洁 ⟸ 闭包环境捕获 ⟸ 适配器组合
> 越界/双重消耗避免 ⟸ 迭代器所有权语义清晰 ⟸ `iter`/`iter_mut`/`into_iter` 区分

---

## 反命题与边界

> **反命题**: "闭包总是借用其捕获变量，不会移动。" —— 错误。闭包按捕获方式可 move、可变借用或不可变借用；`move` 关键字强制按值捕获，且 `FnOnce` 会消耗所有权。
> **边界**: 本测验聚焦 L1 基础；自定义迭代器、流式异步迭代器与 ` lending iterator` 等进阶话题需另见对应概念页。
