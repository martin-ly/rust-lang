...
Rust 的所有权（Ownership）约束主要作用于存储层，确保每个资源在任意时刻只有一个所有者。
...
**解析**：借用（Borrowing）检查不改变任何运行时（Runtime）行为，它在编译期拒绝违反别名/可变性约束的程序，属于**静态语义**层面的分析（类似类型检查，但作用于"存储层资源的使用方式"）。A 错：检查发生在编译期而非运行时；C 错：可达性分析是运行时机制，Rust 没有 GC；D 与资源约束无关。
...
- D. `unsafe` 块内的代码完全不受类型系统（Type System）约束
...
> **认知路径**: 通过本测验回顾 PL 基础概念（变量模型、求值策略、效果、控制流、数据抽象），将错题映射回 `00_start` 与 `01_foundation` 对应权威页进行针对性补强。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 变量模型扎实 ⟹ 所有权/借用推理有根 | 理解绑定、作用域、可变性 | 预测 move/borrow 行为 | 高 |
| 求值策略区分 ⟹ 性能与语义可控 | 理解 eager/lazy、call-by-value | 选择迭代器与闭包策略 | 高 |
| 控制流与数据抽象联动 ⟹ 类型安全设计 | 掌握 match/enum/trait | 写出穷尽且可维护的代码 | 高 |

> **过渡**: 测验后按错题主题回到 `00_start` 与 `01_foundation` 权威页，做第二轮精读。
> **过渡**: 将 PL 基础与 L2 进阶主题结合，可理解泛型、trait、生命周期、错误处理的上层一致性。
> **过渡**: 通过测验验证后，可尝试在 `exercises/` 中完成对应基础练习。

> 基础概念贯通 ⟸ 错题回归权威页 ⟸ 多主题交叉测验
> 代码可维护性提升 ⟸ 穷尽匹配 + 显式效果 ⟸ 控制流与数据抽象联动

---

## 反命题与边界

> **反命题**: "PL 基础概念只影响面试，不影响日常编码。" —— 错误。所有权、借用、类型推断、控制流等基础概念直接决定 Rust 代码能否编译、是否安全、是否高效。
> **边界**: 本测验为 L1 综述级；深入单主题（如 unsafe、async、并发）需进入 L2–L7 对应权威页。
