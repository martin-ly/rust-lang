...
**解析**：`Option<T>` 表示"可能存在也可能不存在的值"，变体为 `Some(T)` 与 `None`。`Ok`/`Err` 属于 `Result<T, E>`（A 是典型混淆：两者都是双变体枚举，但语义不同——`Option` 无错误信息，`Result` 的 `Err` 携带错误详情）。
...
**解析**：`Result` 就是普通枚举值，错误传播是正常的值返回，编译为常规分支代码，零额外运行时开销；错误的类型与处理路径在编译期完全可见（"显式错误处理"）。栈展开只发生在 `panic!` 时（且可用 `panic = "abort"` 关闭）。这正是 Rust 错误处理"零成本 + 不可忽略"双重优势的来源。
