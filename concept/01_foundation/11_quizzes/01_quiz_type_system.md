...
本节围绕「枚举（Enum）与模式匹配（Pattern Matching）」展开，依次讨论 Q4. 以下代码的输出是什么？、Q5. 以下代码能否编译？解释 `if let` 的作用与Q6. 以下代码的输出是什么？解释 `_` 和 `..` 的作用。
...
- D：Rust 类型推断（Type Inference）是静态类型语言特性，不要求处处标注。
...
- A. `match` 必须穷尽枚举（Enum）的所有变体（或使用 `_` 通配），否则编译错误
- B. `enum` 的不同变体可以携带不同类型、不同数量的数据
- C. `enum` 值的大小不固定，运行时（Runtime）按变体动态分配
...
**解析**：编译后类型信息被擦除（单态化（Monomorphization）场景下甚至生成专用代码），运行时（Runtime）无类型标签检查。需注意区分：`dyn Trait` 的 vtable 是**动态分发**机制而非"运行时类型检查"，类型正确性仍在编译期验证。
...
本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：本文件是 测验：类型系统（Type System）（试点扩展） 的专项测验集。这类测验…、测验 2：在 测验：类型系统（试点扩展） 的测验中，若遇到不确定答案的…与测验 3：专项测验与概念文件末尾的嵌入式测验有什么区别？（理解层）。
...
> **认知路径**: 通过本测验回顾类型系统核心概念，将错题映射回 [Type System 权威页](../02_type_system/01_type_system.md) 与相关子页进行针对性补强。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 类型推断正确 ⟹ 减少显式标注 | 理解 Hindley-Milner 风格约束 | 代码简洁且编译通过 | 高 |
| trait 约束清晰 ⟹ 泛型 API 可预测 | 明确 `T: Trait` bound | 调用者错误在编译期暴露 | 高 |
| 类型别名与 newtype 区分 ⟹ 抽象安全 | 选择 `type` vs `struct` | 避免意外类型兼容 | 高 |

> **过渡**: 测验后返回类型系统权威页，巩固泛型、trait、生命周期交叉处的推理。
> **过渡**: 将错题按主题归类（所有权、泛型、trait、强制转换），可进入对应专项练习。
> **过渡**: 通过测验验证后，可尝试阅读 `crates/` 中类型系统相关示例并修改运行。

> 错题清零 ⟸ 概念回溯 ⟸ 权威页与测验对照
> 类型推理自信提升 ⟸ 多题型覆盖 ⟸ 常见陷阱识别

---

## 反命题与边界

> **反命题**: "测验得分高等于已掌握类型系统全部细节。" —— 错误。测验覆盖常见模式，但边缘情况（如 higher-ranked trait bounds、auto traits、coherence 规则）仍需在权威页与实践中深化。
> **边界**: 本测验聚焦 L1 基础；L2 及以上进阶类型系统话题（GATs、TAIT、特殊化）需另见对应概念页。
