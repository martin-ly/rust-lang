...
> **认知路径**: 通过本测验回顾所有权与借用核心概念，将错题映射回 [Ownership](../01_ownership_borrow_lifetime/01_ownership.md) 与 [Borrowing](../01_ownership_borrow_lifetime/02_borrowing.md) 权威页进行针对性补强。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 所有权单一 ⟹ 资源释放可预测 | 理解 `let` 创建所有者 | 无悬挂引用与重复释放 | 高 |
| 借用规则严格 ⟹ 数据竞争避免 | 区分 `&` 与 `&mut` | 编译期捕获别名冲突 | 高 |
| 生命周期推断正确 ⟹ 引用有效 | 理解 NLL 与作用域 | 引用不越界使用 | 高 |

> **过渡**: 测验后返回所有权与借用权威页，针对错题类型（move、borrow、lifetime、NLL）做第二轮精读。
> **过渡**: 将所有权概念与 `exercises/src/ownership_borrowing/` 练习结合，动手修复编译错误。
> **过渡**: 通过测验验证后，可进入 L2 生命周期进阶、部分重借用与 Tree Borrows 的学习。

> 悬挂引用避免 ⟸ 借用规则 + 生命周期推断 ⟸ 所有权模型
> 数据竞争减少 ⟸ `&mut` 独占 / `&` 共享 ⟸ 编译期别名检查

---

## 反命题与边界

> **反命题**: "Rust 的所有权就是垃圾回收的手动版。" —— 错误。所有权是编译期资源管理语义，由借用检查器静态验证；它没有运行时 GC，也不依赖引用计数。
> **边界**: 本测验聚焦 L1 基础；unsafe 指针、自引用结构、并发所有权与 Pin 等进阶话题需另见对应权威页。
