...
> [Strings and Text](../06_strings_and_text/01_strings_and_text.md)
>
> **对应练习**:
...
**知识点**：Rust 数值运算提供多种溢出处理策略，默认行为在 debug/release 间不一致是常见陷阱。[→ 数值类型详解](../02_type_system/03_numerics.md)
...
**知识点**：`[]` 索引访问在越界时 panic，`.get()` 返回 `Option` 提供安全替代。[→ 集合类型详解](../05_collections/01_collections.md)
...
**知识点**：Rust 的 enum + match 组合是语言核心特性，替代了其他语言中的 nullable 类型、union 和异常。[→ 枚举详解](../02_type_system/01_type_system.md)
...
**知识点**：`if let` / `while let` 是 Rust "**只想处理一个模式**"场景下的惯用写法。[→ 模式匹配（Pattern Matching）详解](../02_type_system/01_type_system.md)
...
**知识点**：`_` 忽略单个值，`..` 忽略剩余部分，两者是 Rust 模式匹配（Pattern Matching）中减少 boilerplate 的关键工具。[→ 模式匹配详解](../02_type_system/01_type_system.md)
...
**知识点**：Rust 的"无隐式转换"原则消除了 C 家族语言中大量隐蔽的类型转换 bug。[→ 类型强制与转换详解](../02_type_system/04_coercion_and_casting.md)
...
**知识点**：Iterator trait 是 Rust 标准库的核心抽象，`Option` 是处理可能缺失值的安全方式。[→ 迭代器详解](../../02_intermediate/07_iterators_and_closures/01_iterator_patterns.md)
...
**知识点**：`derive` 宏（Macro）通过编译期代码生成减少 boilerplate，是 Rust 元编程的入门工具。[→ 属性与宏详解](../09_macros_basics/01_attributes_and_macros.md)
...
> 本节按四级题型规范补充单选、多选与判断题，知识点与 [Type System](../02_type_system/01_type_system.md) 权威页一致；
> 干扰项针对常见误解设计。

### Q11. 🟢【单选】Rust 中 `i32` 与 `u32` 的区别是什么？

- A. `i32` 是无符号整数，`u32` 是有符号整数
- B. `i32` 是有符号整数（可表示负数），`u32` 是无符号整数（仅非负）
- C. 两者仅在占用内存大小上不同
- D. `u32` 不支持算术运算
...
**解析**：前缀 `i`（integer）表示有符号，`u`（unsigned）表示无符号，数字为位宽。`i32` 范围为 −2³¹ ~ 2³¹−1，`u32` 为 0 ~ 2³²−1，两者均占 4 字节。

**常见误解（干扰项分析）**：

- A：恰好把两者说反，是最常见的记忆错位。
- C：位宽相同（均为 32 位），区别在符号解释而非大小。
- D：无符号类型支持全部算术运算（除负数表示外）。

</details>

---

### Q12. 🟡【单选】`let x = 5;` 未标注类型时，编译器将 `x` 推断为什么类型？
...
**解析**：整数字面量在无其他约束时回退（fallback）到默认类型 `i32`；浮点字面量默认 `f64`。若后续使用施加了约束（如作为 `u8` 参数传入），推断会优先满足约束。

**常见误解（干扰项分析）**：

- A：误以为"默认选最大/最快"，实际 `i32` 是历史与性能平衡的选择。
- C：`usize` 仅用于索引与大小相关的场景，需上下文约束才会推断为它。
- D：Rust 类型推断是静态类型语言特性，不要求处处标注。

</details>

---

### Q13. 🟡【多选】关于 `enum` 与 `match`，下列说法正确的有？（选出所有正确项）

- A. `match` 必须穷尽枚举的所有变体（或使用 `_` 通配），否则编译错误
- B. `enum` 的不同变体可以携带不同类型、不同数量的数据
- C. `enum` 值的大小不固定，运行时按变体动态分配
- D. `_` 通配模式可以匹配所有未被显式列出的情况
...
**解析**：`match` 的穷尽性检查是编译期保证，消除"忘记处理某个变体"的整类 bug（A、D）；`enum` 是可判别联合，每个变体可携带异构数据（B）。C 错误：`enum` 大小在编译期确定，等于最大变体的大小加判别标签，不动态分配。

</details>

---

### Q14. 🟢【判断】Rust 是静态类型语言，类型检查在编译期完成，运行时没有类型检查开销。（对 / 错）
...
**解析**：编译后类型信息被擦除（单态化场景下甚至生成专用代码），运行时无类型标签检查。需注意区分：`dyn Trait` 的 vtable 是**动态分发**机制而非"运行时类型检查"，类型正确性仍在编译期验证。

</details>

---

### Q15. 🟡【判断】`as` 类型转换总是安全的，永远不会丢失数据。（对 / 错）
...
**解析**：`as` 转换可能静默丢失数据：`256u16 as u8 == 0`（截断）、`3.9f64 as i32 == 3`（向零取整）、大浮点转整数会饱和。`as` 虽不产生 UB，但语义上是"程序员自负后果"的显式转换。需要可失败转换时应使用 `TryFrom`，需要保证不丢失时用 `From`/`Into`。
...
| 4–6/10 | 🔄 需巩固基础 | 重读 [Type System](../02_type_system/01_type_system.md)，完成 rustlings 对应章节 |
| 0–3/10 | 📚 建议重新开始 | 从 [Type System](../02_type_system/01_type_system.md) 逐节阅读，配合 [crates/c02_type_system](../../crates/c02_type_system) 示例 |
