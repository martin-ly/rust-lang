...
> **过渡**: 掌握 `Display`/`Debug` trait 后，可进一步学习自定义格式说明符、宽度/精度与国际化格式化。
> **过渡**: 将格式化与生命周期结合，可理解为什么格式化宏借用参数而非消耗，以及 `format!` 与 `write!` 的所有权差异。
> **过渡**: 从 `std::fmt` 出发，可进入日志库（`tracing`、`log`）、序列化与错误消息构造的格式化实践。
