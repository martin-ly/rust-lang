> **内容分级**: [综述级]
> **本节关键术语**: 编码 (Encoding) · UTF-8 · OsString · CString · 字符串操作 · 字符 (char) — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
字符串编码的选型可压缩为两个决策，分别对应「内存中的表示」与「边界的转换」：

**字符串类型选型**（按不变量强度排序）：

| 类型 | 不变量 | 典型来源 | 转 `&str` 代价 |
|:---|:---|:---|:---|
| `String`/`&str` | 保证 UTF-8 | 程序内部文本 | 零成本（借用） |
| `OsString`/`OsStr` | 平台原生编码 | 文件路径、环境变量、命令行参数 | 可能失败（`to_str`） |
| `CString`/`CStr` | 无内 NUL + 尾 NUL | C FFI 边界 | UTF-8 校验（`to_str`） |
| `Vec<u8>`/`&[u8]` | 无编码假设 | 网络协议、二进制格式 | 显式 `from_utf8`（`Result`） |

**编码转换策略**：进入程序边界的字节流，应在「最早的系统边界」完成 UTF-8 校验并转为 `String`（`String::from_utf8`），之后内部全部走 `&str`；无法保证 UTF-8 的边界（文件名、Windows API）用 `OsStr` 保持原样，延迟到必须文本化时再处理失败分支。判定一次转换放在哪里，标准是：失败处理逻辑是否在该层有上下文——有则就地转换，无则保持字节/平台类型向上传递。
...
- [Strings and Text](01_strings_and_text.md) — 字符串与文本处理概述
...
编码边界测试按「失败发生在编译期还是运行期」分类，分界线是类型是否携带编码不变量：

- **运行时 panic / 校验失败**：无效 UTF-8 字节切片经 `str::from_utf8` 返回 `Err`（显式校验）；`String::from_utf8_unchecked` 跳过校验则制造 UB（`unsafe` 的全部责任）；`s[i..j]` 切在非字符边界 panic。这类测试验证「边界校验是否存在」而非「类型是否匹配」。
- **编译错误**：`OsStr` 与 `str` 混用（类型不同，必须经 `to_str`/`to_string_lossy` 显式转换）；`CString::new` 参数含内 NUL 是运行时 `Err` 而非编译错误（值依赖数据）；把 `String` 传给期望 `&CStr` 的 FFI 函数签名直接类型不匹配。编译期拒绝的全部是「类型层级错误」，运行期失败的全部是「值不满足不变量」。

跨平台差异集中在 `OsStr`：同一段路径处理代码，Unix 上 `to_str` 几乎总成功（路径多为 UTF-8），Windows 上可能因 WTF-8 代理对失败——测试矩阵必须覆盖两平台，或直接用 `to_string_lossy` 的替换语义声明「容忍损失」。
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| `OsStr`/`OsString` | 平台原生字符串，不保证 UTF-8 | `std::ffi` |
...
- **上位（is-a）**：[Strings and Text](01_strings_and_text.md) 文本类型的编码扩展层。
- **下位（实例）**：各编码类型转换实例见本页正文。
