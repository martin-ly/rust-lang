...
> **📎 交叉引用（Reference）**
>
> 本主题在 knowledge 中有系统化的知识索引：[所有权（Ownership）](LINK_PLACEHOLDER)
> **受众**: [初学者]
> **层次定位**: L1 基础概念 / 所有权（Ownership）子域
> **A/S/P 标记**: **S** — Structure（心智模型）
> **双维定位**: C×Und — 建立所有权唯一性心智模型
> **前置依赖**: 无（L1 入口概念）
> **后置延伸**: [L2 泛型（Generics）](LINK_PLACEHOLDER) · [L4 所有权形式化](LINK_PLACEHOLDER) · [L3 Unsafe](LINK_PLACEHOLDER)
...
- [Ownership（所有权）](#ownership所有权)
  ...
  - [三、形式化理论根基（Formal Foundation）](#三形式化理论根基formal-foundation)
    ...
    - [3.3 区域类型（Region Types / 生命周期（Lifetimes））](LINK_PLACEHOLDER)
  ...
  - [六、定理推理链（Theorem Chain）](#六定理推理链theorem-chain)
    - [6.1 核心定理：Safe Rust 无内存泄漏（模循环引用（Reference））](LINK_PLACEHOLDER)
  ...
  - [七、示例与反例（Examples \& Counter-examples）](#七示例与反例examples--counter-examples)
    ...
    - [7.5 反命题与边界分析](#75-反命题与边界分析)
      ...
      - [命题 2（运行时（Runtime）层）: "Move 后原变量绝对不可访问"](LINK_PLACEHOLDER)
      - [命题 3（语义层）: "所有权 = RAII = 自动释放"](#命题-3语义层-所有权--raii--自动释放)
      - [命题 4（工程层）: "所有权系统完全零成本抽象（Zero-Cost Abstraction）"](LINK_PLACEHOLDER)
  ...
  - [十一、边界测试：所有权规则的编译错误](#十一边界测试所有权规则的编译错误)
    ...
    - [11.6 边界测试：`Copy` 类型在闭包（Closures）中的捕获（编译错误）](LINK_PLACEHOLDER)
  ...
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    ...
    - [测验 3：借用（Borrowing）与所有权的关系（分析层）](LINK_PLACEHOLDER)
...
> **[来源: [Rust Reference: Ownership](https://doc.rust-lang.org/reference/)]** Rust 的所有权系统通过编译期检查实现零成本内存安全。 ✅
> **来源: [C++ Reference: std::unique_ptr](https://en.cppreference.com/w/cpp/memory/unique_ptr)** C++11 `unique_ptr` 提供运行时所有权管理，但编译器不检查 use-after-move。 ✅
...
> **来源: [Wikipedia: Ownership type](LINK_PLACEHOLDER)** 所有权类型系统（Type System）控制别名和可变状态访问，Rust 是工业级实现最完整的代表。 ✅
...
> 所有权唯一性保证资源的单一入口点，区域类型保证入口点的有效期可静态确定，二者合起来构成 Safe Rust 内存安全的完整形式化基础。 ✅
...
> **认知功能**: 此决策树将「是否实现 Copy」的抽象判断转化为**可操作的二进制冷查**。读者面对一个新类型时，按图索骥依次检查三个条件（标量值、无 Drop、语义等价），即可得出确定结论。它消除了「Copy 是否安全」的直觉猜测，将判断责任从「程序员的内存安全直觉」转移到了「结构化的条件检查」。 [来源: 💡 原创分析]
...
> **unsafe 语义**: `unsafe` 不是关闭借用（Borrowing）检查器，而是将编译期证明责任转移给程序员——程序员需手动维护内存安全不变量 来源: [Rustonomicon / 2025](https://doc.rust-lang.org/nomicon/)
...
> 🎓 **Brown 书强化**: 本节概念与框架直接来自 [Brown University Interactive Book — Fixing Ownership Errors](https://rust-book.cs.brown.edu/ch04-03-fixing-ownership-errors.html) 与 Will Crichton 等人的 OOPSLA 2023 论文 *A Grounded Conceptual Model for Ownership Types in Rust*。Brown 团队的研究表明，将编译器错误归类为“所有权转移”“借用冲突”“生命周期不匹配”三类，可显著提升学习者修复效率。
...
| Rust 使用仿射类型系统 | [Utrecht: Ownership Types] · [Wikipedia: Affine logic](https://en.wikipedia.org/wiki/Affine_logic) | ✅ |
...
> **关键洞察**: `unique_ptr` 是**运行时机制**（RAII + 指针管理），Rust 所有权是**编译期机制**（类型系统 + 线性逻辑）。C++ 程序员需要"自律"来正确使用 `std::move`，Rust 程序员被编译器"强制"遵守所有权规则。这不是说 C++ 不安全——而是 Rust 将"正确使用智能指针（Smart Pointer）"的责任从程序员转移到了编译器。
