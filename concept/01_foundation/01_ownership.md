...
> **认知功能**: 此思维导图是所有权系统的**全局认知地图**。放射式结构帮助读者将碎片知识归类到五大维度：核心规则（编译器强制执行什么）、形式化根基（为什么这些规则是数学上正确的）、运行时行为（内存实际发生了什么）、特殊机制（Copy/Drop 等例外与扩展）。建议读者在学习任何子概念后回到此图定位其所属分支，建立「局部细节 ↔ 全局结构」的双向关联。 [来源: 💡 原创分析]
> [来源: [Rust Reference](https://doc.rust-lang.org/reference/)]
