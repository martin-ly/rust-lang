...
> **双维定位**: S×App — 规范应用
> **前置依赖**:
> [Control Flow](01_control_flow.md) ·
> [Variables and Bindings](../03_values_and_references/03_variable_model.md) ·
> [Patterns](02_patterns.md)
> **后置概念**:
...
- **P2 生态/社区**: [docs.rs/if_chain — 生态权威 API 文档（嵌套 `if let` 表达式的宏（Macro）化实践）](https://docs.rs/if_chain)（2026-07-12 验证 HTTP 200）
...
- A. 函数调用参数按从左到右顺序求值
- B. 结构体（Struct）、数组、元组等复合表达式的元素按从左到右顺序求值
...
> **认知路径**: 从「Rust 几乎所有构造都是表达式」这一核心设计出发，理解块求值、分号语义与语句/表达式二分如何塑造代码风格。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 块表达式求值 ⟹ 尾表达式决定值 | 去掉尾部分号 | 函数体、分支、循环体均可产出值 | 高 |
| 分号将表达式转为语句 ⟹ 值为 `()` | 在表达式后加 `;` | 避免隐式返回与类型错配 | 高 |
| 发散表达式类型为 `!` ⟹ 可嵌入任意上下文 | 使用 `panic!`/`return`/`break` | 类型系统统一处理不可能返回 | 高 |

> **过渡**: 掌握表达式语言后，可理解为什么 `if`/`match`/`loop` 都能作为右值，简化赋值与返回。
> **过渡**: 从分号语义出发，可进一步学习宏与过程宏中「token 树」如何区分表达式与语句。
> **过渡**: 将表达式求值顺序与运算符优先级结合，可准确预测复合表达式和副作用顺序。

> 隐式返回减少 ⟸ 块尾表达式决定值 ⟸ 表达式语言统一函数/分支
> 类型错配前置 ⟸ 分号显式转语句 ⟸ 编译期检查 `()` 与期望类型

---

## 反命题与边界

> **反命题**: "Rust 中所有块都用 `return` 返回最后一行。" —— 错误。Rust 函数默认返回块尾表达式值；显式 `return` 仅用于提前返回，且 `return` 本身不是块的正常尾值。
> **边界**: 表达式求值顺序在操作数层面有明确规定，但函数参数求值顺序不保证完全固定；依赖副作用顺序的代码仍需谨慎。
