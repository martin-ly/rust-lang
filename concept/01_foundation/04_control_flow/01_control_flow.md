...
2. **`match`：穷尽性模式匹配**：`match` 是类型系统的直接投影——编译器按被匹配类型的全部变体集合校验分支覆盖（exhaustiveness），漏掉变体即编译错误 E0004。这使「增加 enum 变体后所有 `match` 自动报警」成为重构安全网。
...
本组测验围绕测验 1：`if` 作为表达式（理解层）、测验 2：`match` 穷尽性（应用层）、测验 3：`loop` 返回值（应用层）、测验 4：`if let` 语法（应用层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
...
2. **Rust 关联**：为什么没有 `call/cc`——全栈续延要求可拷贝的栈或闭包转换，与零成本抽象和确定性析构冲突；
