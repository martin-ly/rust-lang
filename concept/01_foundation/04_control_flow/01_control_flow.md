...
- **`loop` 与值返回**：`loop` 是唯一可带值的循环——`break expr` 中的 `expr` 成为整个 `loop` 表达式的值（`while`/`for` 的 `break` 不带值，因为「循环零次执行」时值无来源）。这使「重试直到成功」模式可写为 `let r = loop { match try() { Ok(v) => break v, Err(_) => continue } };`。
- **标签与嵌套循环控制**：`'outer: loop { ... break 'outer; }` 允许从任意层嵌套中精确跳出，是结构化语言对 `goto` 的受控替代——跳转目标必须是词法上可见的外层循环，编译器静态可检。
- **块表达式与尾部值**：`{ stmt; stmt; expr }` 是表达式，其值为末尾不带分号的表达式。此规则解释了「函数体最后一条表达式即返回值」，也解释了多分号错误 E0308（尾部加分号使块类型变为 `()`）的由来。
...
本节收集 6 个「编译器必然拒绝」的控制流用例，每个用例对应一条类型系统不变量：
...
控制流图（Control Flow Graph, CFG）是编译器内部对控制流的**唯一权威表示**：Rust 源码经 AST → HIR → MIR 逐层降低后，MIR 就是以基本块（basic block）为节点、以 terminator 跳转（`goto`/`switchInt`/`return`/`unwind`）为边的 CFG。借用检查（NLL）、drop elaboration、常量传播全部在 MIR CFG 上运行。
