...
「核心概念」部分按表达式 vs 语句、match：穷尽性模式匹配与if let / while let：简化的模式匹配的顺序逐层展开。
...
「技术细节」涉及 loop 与值返回、标签与嵌套循环控制与块表达式与尾部值，本节逐一说明其要点。
...
本节将「结构化程序定理（Böhm–Jacopini 定理）」分解为若干主题：形式化定义、三种结构化构造的 ASCII 示意图、与 Rust 的对应与代码示例：仅用三种结构实现欧几里得算法。
