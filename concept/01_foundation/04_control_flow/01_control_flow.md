> **内容分级**: [综述级]
>
> **本节关键术语**: 控制流 (Control Flow) · 条件分支 (Conditional) · 循环 (Loop) · 模式匹配 (Pattern Matching) · 守卫条件 (Guard) — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# 控制流：表达式导向的流程控制
>
> **EN**: Control Flow
...
> **后置概念**: [Generics](../../02_intermediate/01_generics/01_generics.md) · [Async](../../03_advanced/01_async/01_async.md)
...
Rust 控制流的三个核心概念，共同构成「一切皆表达式」的设计：

1. **表达式 vs 语句**：Rust 中几乎所有控制结构都是表达式（expression），有值、可赋给变量。`let x = if c { 1 } else { 2 };` 与 `let y = match v { ... };` 均合法。语句（statement）仅有两种：`let` 绑定与「表达式加分号」（丢弃值、类型变为 `()`）。判定一个构造是表达式还是语句，只需看它能否出现在 `let` 右侧——能则是表达式。
2. **`match`：穷尽性模式匹配**：`match` 是类型系统（Type System）的直接投影——编译器按被匹配类型的全部变体集合校验分支覆盖（exhaustiveness），漏掉变体即编译错误 E0004。这使「增加 enum 变体后所有 `match` 自动报警」成为重构安全网。
3. **`if let` / `while let`：单模式匹配**：当只关心一个模式时，`if let P = e { }` 等价于 `match e { P => ..., _ => () }` 的糖。代价是失去穷尽性检查——不匹配时静默跳过，因此「必须处理所有情况」的场景仍应使用完整 `match`。
...
控制流的三个技术细节决定「值如何流出控制结构」：

- **`loop` 与值返回**：`loop` 是唯一可带值的循环——`break expr` 中的 `expr` 成为整个 `loop` 表达式的值（`while`/`for` 的 `break` 不带值，因为「循环零次执行」时值无来源）。
- 这使「重试直到成功」模式可写为 `let r = loop { match try() { Ok(v) => break v, Err(_) => continue } };`。
- **标签与嵌套循环控制**：`'outer: loop { ... break 'outer; }` 允许从任意层嵌套中精确跳出，是结构化语言对 `goto` 的受控替代——跳转目标必须是词法上可见的外层循环，编译器静态可检。
- **块表达式与尾部值**：`{ stmt; stmt; expr }` 是表达式，其值为末尾不带分号的表达式。
- 此规则解释了「函数体最后一条表达式即返回值」，也解释了多分号错误 E0308（尾部加分号使块类型变为 `()`）的由来。
...
控制流的核心命题——「结构化控制流足以表达一切计算」——只在特定边界内成立，本节系统检验这些边界。

- **反命题树**（4.1）检验「所有条件逻辑都应使用 `match`」这一常见过度概括：当只关心单一模式时 `if let` 更简洁，纯布尔条件用 `if` 即可，盲目 `match` 反而引入噪音。
- **边界极限**（4.2）列出五类真实约束：整数类型无法穷尽匹配、分支类型必须统一、`async` 上下文中 `break/continue` 与 `Pin` 的交互、`const fn` 求值步数限制、深层嵌套模式的可读性代价。

判定依据：这些边界不是语言缺陷，而是**类型一致性（coherence）与可判定性**的必然推论——编译器要静态保证穷尽性与分支类型一致，就必须拒绝无法静态判定的写法。
...
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 迭代器（Iterator）
- [Async](../../03_advanced/01_async/01_async.md) — 异步控制流
...
本节收集 6 个「编译器必然拒绝」的控制流用例，每个用例对应一条类型系统（Type System）不变量：

| 测试 | 违反的不变量 | 错误码族 |
|:---|:---|:---|
| `loop` 的 `break` 返回值类型不一致 | 同一 `loop` 表达式所有 `break` 值必须同型 | E0308 |
| `if let`/`while let` 变量遮蔽 | 绑定只覆盖模式匹配成功的分支作用域 | E0425/E0382 |
| `loop` 表达式类型推断失败 | 无 `break` 值的 `loop` 类型为 `!`，不可强制为有值类型 | E0308 |
| `?` 在 `main` 中的返回类型 | `main` 的返回类型必须实现 `Termination` | E0277 |
| `match` 臂绑定与守卫 | 守卫不纳入穷尽性分析，绑定需在所有臂一致 | E0004/E0408 |

读法建议：先预测每段代码的错误信息，再与编译器实际输出比对——这是检验「是否真正理解规则而非记住结论」的最快方式。
...
本组测验围绕测验 1：`if` 作为表达式（理解层）、测验 2：`match` 穷尽性（应用层）、测验 3：`loop` 返回值（应用层）、测验 4：`if let` 语法（应用层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用（Borrowing）检查」或「运行时（Runtime）行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
...
Böhm–Jacopini 定理（1966）是结构化程序设计的理论基础：**任何可计算函数的控制流图，都可以仅用顺序（sequence）、选择（selection）与迭代（iteration）三种构造实现**，无需无约束的 `goto`。本节给出：

- **形式化定义**：对任意流程图程序 P，存在语义等价的结构化程序 P′，其控制流图只含三种基本块组合。证明思路是把任意图的节点按支配关系重排，用布尔标志变量编码非结构化跳转。
- **三种构造与 Rust 的对应**：顺序 = 语句序列；选择 = `if`/`match`；迭代 = `loop`/`while`/`for`。Rust 在此基础上增加 `break 'label`（多层级受控跳出）与 `?`（错误传播的隐式分支），二者都保持「单入口、静态可知出口」的结构化性质。
- **代码示例**：用三种结构实现欧几里得算法，展示「理论上 goto 可消除」的最小实例。

工程意义：定理保证了「禁用 goto 不损失表达力」，Rust 的控制流设计正是该定理的现代工程化——所有跳转的结构化版本都在语言内建，非结构化跳转（如长距离错误传播）由 `Result` + `?` 在类型系统中表达。
...
> **关联章节**: [Error Handling Basics](../08_error_handling/01_error_handling_basics.md) · [Async Control Flow](../../03_advanced/01_async/01_async.md)

---

### 7.2 Continuation / `call/cc` / CPS 变换 / Delimited Continuation

Rust 没有 `call/cc`，但 continuation 的概念对理解 `async/await` 不可或缺：每个 `async fn` 在编译期被转换为一个显式状态机（`Future::poll` 的 `match state` 分发），这正是**有界续延（delimited continuation）的一次性（one-shot）版本**——挂起点 `.await` 即 `shift`，`Future` 的轮询契约即 `reset` 边界。

本节目的是在 Rust 语义与 PL 理论之间建立对应：

1. **形式化定义**：continuation 作为「计算的剩余部分」的语义函数 κ(v) = C[v]；
2. **Rust 关联**：为什么没有 `call/cc`——全栈续延要求可拷贝的栈或闭包（Closures）转换，与零成本抽象（Zero-Cost Abstraction）和确定性析构冲突；
3. **CPS 变换**：`async` 块的本质是编译器自动执行的 CPS 变换，尾调用化保证状态机可轮询；
4. **代码示例**：用手写状态机模拟一个 `Future`，演示 `.await` 与 `shift` 的对应关系。

判定标准：能解释「为什么 `Future` 是 `poll` 驱动而非回调驱动」，即理解了 delimited continuation 的工程取舍。
...
> **关联章节**: [Async](../../03_advanced/01_async/01_async.md) · [Closures](../00_start/03_closure_basics.md) · [Error Handling](../08_error_handling/01_error_handling_basics.md)

---

### 7.3 控制流图（CFG）、基本块、边与支配树

控制流图（Control Flow Graph, CFG）是编译器内部对控制流的**唯一权威表示**：Rust 源码经 AST → HIR → MIR 逐层降低后，MIR 就是以基本块（basic block）为节点、以 terminator 跳转（`goto`/`switchInt`/`return`/`unwind`）为边的 CFG。借用（Borrowing）检查（NLL）、drop elaboration、常量传播全部在 MIR CFG 上运行。

本节围绕四个要点展开：

- **形式化定义**：基本块（单入口单出口的最长顺序指令序列）、边、入口/出口节点；
- **支配树（dominator tree）**：节点 d 支配 n 当且仅当所有入口到 n 的路径都经过 d——变量初始化分析（E0381）与 SSA 构造的 φ 节点放置都依赖支配关系；
- **与编译器优化的关系**：循环识别（回边 = 目标支配源的边）、死代码消除、分支权重标注；
- **代码示例**：用迭代式数据流算法手算一个极小 CFG 的支配集，复现编译器 `dominators` 算法的核心。

> **Rust 关联**：可用 `rustc -Z dump-mir=main`（nightly）观察真实 MIR CFG；本节示例与该输出格式一致。
...
> **关联章节**: [Borrowing](../01_ownership_borrow_lifetime/02_borrowing.md) · [Lifetimes](../01_ownership_borrow_lifetime/03_lifetimes.md) · [Formal Type Theory](../../04_formal/00_type_theory/01_type_theory.md)

---

### 7.4 循环不变量：与 Hoare 逻辑的衔接

本节聚焦「循环不变量：与 Hoare 逻辑的衔接」，覆盖形式化定义、部分正确性与完全正确性、Rust 关联、代码示例 1：二分查找的循环不变量等方面。论述顺序由定义到边界：先明确「循环不变量：与 Hoare 逻辑的衔接」在「控制流：表达式导向的流程控制」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「循环不变量：与 Hoare 逻辑的衔接」的判定标准，并指出它在全页论证链中的位置。
...
> **关联章节**: [Assert & Matches](../../02_intermediate/06_macros_and_metaprogramming/01_assert_matches.md) · [Range Types](../../02_intermediate/04_types_and_conversions/01_range_types.md)

---

### 7.5 `goto` 的消除与恢复：C `goto` / C++ `co_await` / Rust `?` / `break 'label` 的控制流本质

本节聚焦「`goto` 的消除与恢复：C `goto` / C++ `co_await` / Rust `?` / `break 'label` 的控制流本质」，覆盖形式化定义、C `goto`：自由但危险、C++ `co_await`：协程的恢复式控制流、Rust `break 'label` 与 `?`：结构化跳转的两种受…等方面。论述顺序由定义到边界：先明确「`goto` 的消除与恢复：C `goto` / C++ `co_await` / Rust `?` / `break 'label` 的控制流本质」在「控制流：表达式导向的流程控制」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「`goto` 的消除与恢复：C `goto` / C++ `co_await` / Rust `?` / `break 'label` 的控制流本质」的判定标准，并指出它在全页论证链中的位置。
...
> **关联章节**: [Error Handling Basics](../08_error_handling/01_error_handling_basics.md) · [Async Control Flow](../../03_advanced/01_async/01_async.md) · [Panic and Abort](../08_error_handling/03_panic_and_abort.md)
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| 表达式导向 | `if` / `match` / `loop` / 块均为表达式，尾表达式即值 | 本文 §1.1、§2.3 |
| 穷尽性 | `match` 编译期穷尽检查，漏分支报 E0004 | 本文 §1.2 |
| `loop` 值 | `break value` 使 `loop` 产出值，类型须一致 | 本文 §2.1 |
| 循环标签 | `'label: loop` 支持嵌套循环的精确 break/continue | 本文 §2.2 |
| 结构化 | 无 `goto`；跳转限于 `break` / `continue` / `return` / `?` | 本文 §一 |
...
- **P2 生态/社区**: [docs.rs/itertools — 生态权威 API 文档（迭代器（Iterator）与 `for` 控制流的生态扩展）](https://docs.rs/itertools)（2026-07-12 验证 HTTP 200）
