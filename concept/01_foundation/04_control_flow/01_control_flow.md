...
「边界测试：控制流的编译错误」部分按边界测试：`loop` 返回值类型不匹配（编译错误）、边界测试：`if let` 与 `while let` 的变量遮蔽（编…、边界测试：`loop` 表达式的类型推断（编译错误）、边界测试：`?` 运算符在 `main` 中的返回类型（编译错误）等6个方面的顺序逐层展开。
...
理解「Continuation / `call/cc` / CP…」需要把握形式化定义、Rust 关联、`async/await` 作为显式 Continuation 的 M…、代码示例等5个方面，本节依次展开。
...
本节将「控制流图（CFG）、基本块、边与支配树」分解为若干主题：形式化定义、`if-else` 的 CFG 与支配树 Mermaid 图、与编译器优化的关系、Rust 关联等5个方面。
