...
「嵌入式测验（Embedded Quiz）」涉及测验 1：`if` 作为表达式（理解层）、测验 2：`match` 穷尽性（应用层）、测验 3：`loop` 返回值（应用层）、测验 4：`if let` 语法（应用层）等5个方面，本节逐一说明其要点。
...
「循环不变量：与 Hoare 逻辑的衔接」部分按形式化定义、部分正确性与完全正确性、Rust 关联、代码示例 1：二分查找的循环不变量等5个方面的顺序逐层展开。
...
「`goto` 的消除与恢复：C `goto` / C++…」部分按形式化定义、C `goto`：自由但危险、C++ `co_await`：协程的恢复式控制流、Rust `break 'label` 与 `?`：结构化跳转的两种受…等7个方面的顺序逐层展开。
