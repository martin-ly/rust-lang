...
本节从 `let-else` 的基本语义、语法规则到与 `match` / `if let` 的对比，建立对这一构造的整体认识，为后续技术细节与常见模式打下基础。
...
本节深入 `else` 块的发散要求、绑定范围、可反驳模式约束，以及与 `let chains` 的互补关系，澄清使用中的常见误解。
...
本节展示 `let-else` 在 `Option`、`Result`、嵌套结构提取和 `?` 配合中的惯用写法，体现其在简化主路径方面的价值。
...
本节用反命题树和常见错误表梳理何时该用、何时不该用 `let-else`，并总结失败处理的最佳实践。
...
> **认知路径**: 从 `if let` 早退出发，理解 `let-else` 如何用 else 分支统一处理模式匹配失败，避免深层嵌套。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 模式匹配成功 ⟹ 绑定在后续作用域可用 | `let-else` 左侧匹配 | 成功路径扁平表达 | 高 |
| 模式匹配失败 ⟹ 必须进入 else 发散 | else 分支返回 `!` 或 `()` | 失败路径不泄漏未绑定变量 | 高 |
| let-else 替代 if-let 嵌套 ⟹ 早退逻辑清晰 | 单一模式需要早退 | 代码层级减少，可读性提升 | 高 |

> **过渡**: 掌握 `let-else` 后，可将其与 `?` 操作符、函数早退结合，写出更简洁的错误处理与解构代码。
> **过渡**: 对比 `let-else` 与 `if let ... else` 块，可理解 let-else 在变量作用域和发散要求上的严格语义。
> **过渡**: 将 let-else 与 `let chains` 结合，可处理多条件解构，同时注意 `let chains` 不支持 `||` 的约束。

> 未绑定变量不泄漏 ⟸ else 分支强制处理失败 ⟸ 模式匹配失败路径完整
> 嵌套层级减少 ⟸ let-else 统一成功/失败路径 ⟸ 早退逻辑显式化
