...
- [类型强制与转换：显式与隐式的边界](#类型强制与转换显式与隐式的边界)
  ...
  - [十、边界测试：类型转换的编译错误](#十边界测试类型转换的编译错误)
    ...
    - [10.2 边界测试：裸指针与引用（Reference）转换的生命周期（Lifetimes）丢失（编译错误 / 运行时（Runtime） UB）](#102-边界测试裸指针与引用转换的生命周期丢失编译错误--运行时-ub)
    ...
    - [10.5 边界测试：强制类型转换与 `Deref` 的自动解引用（Reference）（编译错误）](#105-边界测试强制类型转换与-deref-的自动解引用编译错误)
