...
本节用三组对照说明「标准 I/O 与进程（std I/O and Process）」：正确示例：安全复制文件、反例：忽略 Result与反例：使用字符串拼接路径。每组先给正确示例并标注其成立的类型系统依据，再给反例并标注编译器诊断（E0xxx）或运行时后果，最后给出修正方案。判读标准：正确示例应能通过 rustc 1.97 编译且无 clippy 警告，反例的失败点必须可定位到具体规则。
