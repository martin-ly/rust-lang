...
- **闭包的零成本实现**：每个闭包有独立的匿名类型，捕获的环境按值/引用存为该类型的字段，调用 `Fn::call` 经单态化内联后与普通函数调用等价。`move` 闭包与 `FnMut`/`FnOnce` 的区别只影响捕获的所有权语义，不影响调用开销。
...
- 测验 5（分析层）：闭包的内存布局——捕获环境即结构体字段，零成本源于内联而非魔法。
