...
> **本节关键术语**:
>
> 零成本抽象 (Zero-Cost Abstraction) · 内联 (Inlining) · 单态化 (Monomorphization) · 编译期优化 (Compile-Time Optimization) · 抽象开销 (Abstraction Overhead) — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# 零成本抽象：Rust 的性能哲学
>
...
> **前置概念**: [Ownership](../01_ownership_borrow_lifetime/01_ownership.md) · [Generics](../../02_intermediate/01_generics/01_generics.md) · [Traits](../../02_intermediate/00_traits/01_traits.md)
> **后置概念**: [Rust vs C++](../../05_comparative/01_systems_languages/01_rust_vs_cpp.md) · [Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md)

---

> **来源**:
> [TRPL Ch13 — Closures](https://doc.rust-lang.org/book/ch13-04-performance.html) ·
...
- [零成本抽象：Rust 的性能哲学](#零成本抽象rust-的性能哲学)
  ...
  - [十二、边界测试：零成本抽象的编译错误](#十二边界测试零成本抽象的编译错误)
    - [12.1 边界测试：泛型单态化与代码膨胀（逻辑错误 / 编译器限制）](#121-边界测试泛型单态化与代码膨胀逻辑错误--编译器限制)
  ...
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    ...
    - [测验 3：迭代器 vs 循环（应用层）](#测验-3迭代器-vs-循环应用层)
    - [测验 4：Trait 对象的开销（分析层）](#测验-4trait-对象的开销分析层)
    - [测验 5：闭包的内存布局（分析层）](#测验-5闭包的内存布局分析层)
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  - [📋 关键属性](#-关键属性)
  - [🔗 概念关系](#-概念关系)
...
零成本抽象（Zero-Cost Abstraction）的原始定义出自 Bjarne Stroustrup：「你不用的东西不付出代价；你用的东西，手写代码不可能做得更好」。在 Rust 中它落实为三个可验证的机制：

1. **定义的操作化**：「零成本」不是「无成本」，而是「抽象层在编译期完全消解」——生成的机器码与手写底层实现逐指令可比。判定方法是对照汇编（`cargo asm` / godbolt）：抽象版与手写版应生成等价代码。
2. **单态化（Monomorphization）**：泛型的零成本实现方式。编译器为每个实际类型参数生成一份特化代码（`Vec<i32>` 与 `Vec<String>` 是两份独立机器码），类型参数在运行时不存在，因此泛型函数无分派开销。代价是编译时间与代码体积，这是「零运行时成本」的另一面。
3. **迭代器与循环消除**：迭代器链（`map`/`filter`/`fold`）经内联与循环融合（loop fusion）编译为与手写 `for` 循环相同的机器码——这是零成本抽象的经典实证案例：高层函数式组合子不引入闭包分配或间接调用。

判定一个 Rust 抽象是否零成本，问两个问题：类型参数/行为差异是否在编译期确定？热路径上是否存在编译器无法内联的间接调用？两问皆「是/否」则零成本成立。
...
> **认知功能**: 此图对比 Rust **单态化（Monomorphization）**与 Java **类型擦除**的实现差异——Rust 为每个类型生成专门代码，Java 使用通用代码加运行时（Runtime）类型处理。
...
零成本的技术边界由编译管道与「哪些抽象故意不零成本」共同划定：

- **编译期优化管道**：`MIR → LLVM IR → 机器码` 三级优化。零成本的关键阶段是 MIR 层的内联（`#[inline]` 提示 + 启发式）与单态化，以及 LLVM 层的循环优化与死代码消除。`opt-level` 直接决定迭代器链能否融合——debug 构建下迭代器比手写循环慢数倍是预期行为，不构成对零成本的反例。
- **Trait 对象的运行时开销**：`dyn Trait` 是「有意的非零成本」选择——虚表指针（vptr）+ 数据指针的胖指针布局，每次方法调用一次间接跳转，且阻止内联。当「类型擦除」的收益（编译时间、二进制体积、异构集合）超过分派成本时才应选用；判定标准是调用频率与类型集合是否闭合。
- **闭包的零成本实现**：每个闭包有独立的匿名类型，捕获的环境按值/引用（Reference）存为该类型的字段，调用 `Fn::call` 经单态化内联后与普通函数调用等价。`move` 闭包与 `FnMut`/`FnOnce` 的区别只影响捕获的所有权语义，不影响调用开销。
...
| **迭代器（Iterator）适配器** | `.map().filter()` | **零**（Release） | 优先于手写循环 |
...
- **反命题 1：「零成本 = 免费」** —— 误解。Bjarne Stroustrup 的原义是「不为不使用的特性付费」+「手写底层代码不会更好」——抽象本身有**编译期成本**（单态化膨胀、编译时间）与**认知成本**（理解展开后的行为）。`Iterator` 链零运行时开销的前提是优化开启（debug 模式慢 10-50 倍是常态）。
- **反命题 2：「抽象层数越多越优雅」** —— 危险。每层泛型抽象增加错误信息深度与编译时间；当抽象的调用方只有一两个时，「为复用而抽象」是负收益。判定准则：抽象的收益 = 消除的重复 − 增加的理解成本，小于零就内联回去。

边界极限小节量化：单态化的体积成本（`cargo bloat`）、`dyn` 与泛型的分界点、以及内联提示的失效场景。
...
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 泛型与单态化
...
- [Cranelift Backend](../../07_future/02_preview_features/16_cranelift_backend_preview.md) — 快速编译后端
...
本节边界测试回答「零成本假设在什么条件下失效」，按失效模式分类：

- **代码膨胀（逻辑错误 / 编译器限制）**：泛型在 N 个类型上实例化生成 N 份代码，深度泛型嵌套（如多层 `Result` 包装 + 组合子）可使二进制体积与编译时间非线性增长。这不是编译错误而是工程约束，缓解手段是「非泛型外壳 + 泛型内核」拆分或选择性 `dyn`。
- **`inline` 提示与编译器优化**：`#[inline(always)]` 不保证内联（递归、过大函数体会被拒），过度内联反而因 I-cache 压力变慢——inline 提示是逻辑层面的调优问题，不产生错误但产生性能回退。
- **泛型递归类型的大小计算**（E0072）：`struct Node { next: Option<Node> }` 类型大小无限，编译期拒绝，必须经 `Box` 引入间接——这是「零成本」让位于「类型可判定」的边界。
- **`dyn Trait` 的大小未知**（E0277）：trait object 是 unsized 类型，不能直接按值存放/返回，必须置于指针之后。该错误标志着「静态分派假设被打破」的精确位置。

判定一个性能问题是否属于零成本边界，先看它落在四类中的哪一格，再决定是重构类型、调整内联还是接受成本。
...
本节 5 道测验覆盖零成本抽象的核心判别点：

- 测验 1（理解层）：零成本的准确定义——「不用的不付费 + 用得不比手写贵」两条款缺一不可；
- 测验 2（应用层）：单态化的双面性——性能收益与代码膨胀的权衡量化；
- 测验 3（应用层）：迭代器 vs 手写循环的优化等价性——什么条件破坏等价（如提前退出的副作用）；
- 测验 4（分析层）：`dyn Trait` 的非零成本——间接调用 + 禁止内联的双重代价；
- 测验 5（分析层）：闭包的内存布局——捕获环境即结构体（Struct）字段，零成本源于内联而非魔法。

作答建议：测验 3、5 建议用 Godbolt 实际对比汇编，「零成本」只有看过汇编才真正理解。
...
> **过渡**: 从抽象无开销到所有权（Ownership）无 GC，Rust 把运行时成本转移到编译期静态分析；这是 Rust 与 GC 语言在性能模型上的根本分野。
> **过渡**: 掌握零成本抽象后，下一步应学习泛型（Generics）与 trait 的单态化机制，以及何时显式选择 `dyn`/`Box` 承担动态分派成本。

## 📋 关键属性

| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| 定义 | 高层抽象编译后不引入运行时额外开销 | 继承 Stroustrup 原则 |
| 实现机制 | 单态化 + 内联 + 无 GC | 编译模型 |
| 原则 | 不使用则不付费，使用则无法更优手写 | 设计哲学 |
| 安全协同 | 借用（Borrowing）检查等静态分析零运行时成本 | 所有权模型 |
| 显式成本 | 动态分派/装箱是显式选择的成本 | `dyn` / `Box` |

## 🔗 概念关系

- **上位（is-a）**：Rust 设计哲学的核心原则，统领 [Type System](../02_type_system/01_type_system.md) 与所有权设计。
- **下位（实例）**：迭代器内联实例见 [Iterator Patterns](../../02_intermediate/07_iterators_and_closures/01_iterator_patterns.md)。
- **对偶**：与 GC 语言的运行时抽象相对，见 [Rust vs Go](../../05_comparative/01_systems_languages/03_rust_vs_go.md)。
- **组合**：与 [Move Semantics](../01_ownership_borrow_lifetime/05_move_semantics.md) 组合实现零成本资源转移。
- **依赖**：落地依赖 [Generics](../../02_intermediate/01_generics/01_generics.md) 单态化。
