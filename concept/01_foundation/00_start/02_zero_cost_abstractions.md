...
「核心概念」涉及零成本抽象的定义、单态化：泛型的零成本实现与迭代器与循环消除，本节逐一说明其要点。
...
理解「技术细节」需要把握编译期优化管道、Trait 对象的运行时开销与闭包的零成本实现，本节依次展开。
...
本节将「边界测试：零成本抽象的编译错误」分解为若干主题：边界测试：泛型单态化与代码膨胀（逻辑错误 / 编译器限制）、边界测试：`inline` 提示与编译器优化（逻辑错误）、边界测试：泛型递归类型的大小计算（编译错误）、边界测试：`dyn Trait` 的大小未知（编译错误）等8个方面。
