...
> **认知路径**: 从 Rust 语法基础出发，经由关键字表与原始标识符（Raw Identifier）机制，建立对保留词与词法边界的清晰认知，为阅读复杂代码和避免解析错误奠基。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---:|
| 关键字保留 ⟹ 解析无歧义 | 理解关键字表 | 能正确识别语句结构 | 高 |
| `r#` 转义 ⟹ 兼容旧接口 | 需要与保留关键字同名的标识符 | 可调用 `r#match` 等名称 | 高 |
| 保留关键字 ⟹ 版本演进 | 关注未来保留字 | 避免在 API 命名中使用潜在关键字 | 中 |

> **过渡**: 从关键字词汇到表达式与语句，核心变化是把孤立单词放入语法结构理解其语义角色。
> **过渡**: 掌握关键字后，下一步应学习控制流、模式匹配与模块系统，并注意关键字在不同上下文中的优先级。
> **过渡**: 关键字是编译器解析的第一道边界；错误使用关键字会导致解析失败而非类型错误，诊断信息也不同。

> 语法解析成功 ⟸ 关键字不冲突 ⟸ 词法分析正确性
> 未来兼容 ⟸ 避免使用保留字 ⟸ 版本升级平滑
...
> **反命题**: "原始标识符 `r#` 可以让关键字任意作为标识符，没有限制。" —— 错误。`r#` 只能转义当前已存在的保留关键字，且不能用于所有上下文（如宏 hygiene 中仍可能受限），也不应滥用破坏可读性。
> **边界**: 当跨 crate 或跨版本使用 `r#` 标识符时，外部代码可能因关键字版本差异而无法调用；公共 API 应尽量避免使用原始标识符。
