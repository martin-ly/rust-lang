...
- **圆括号 `()`**：函数调用与定义参数、元组构造、表达式分组（优先级覆盖）、元组结构体/变体。`()` 单独出现是单元类型/单元值；
- **花括号 `{}`**：块表达式（产生值，尾部表达式规则）、结构体字面量、`match` 臂体、`unsafe`/trait/impl 体。`{}` 单独出现在格式串中是占位符；
- **方括号 `[]`**：数组/切片类型与字面量、索引操作（`Index` trait 脱糖）、属性参数（`#[cfg(...)]` 内部分语法）。
...
| [Macros](../../03_advanced/03_proc_macros/01_macros.md) | `!`、`$`、`$(...)` 用于宏系统 |
