> **内容分级**: [综述级]
> **本节关键术语**: 闭包 (Closure) · 捕获 (Capture) · Fn · FnMut · FnOnce · 环境 (Environment) — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# 闭包基础：捕获环境与匿名函数
>
> **EN**: Closures
...
> **后置概念**: [Iterator](../../02_intermediate/07_iterators_and_closures/01_iterator_patterns.md) · [Async](../../03_advanced/01_async/01_async.md) · [Functional Patterns](../../02_intermediate/04_types_and_conversions/02_closure_types.md)
...
闭包（closure）是**捕获环境的匿名函数**。Rust 闭包的独特性在于捕获方式由类型系统（Type System）精确刻画：

- **三种捕获模式**：按不可变借用（Immutable Borrow）（`Fn`）、按可变借用（`FnMut`）、按值 move（`FnOnce`）——编译器根据闭包体对捕获变量的**使用方式**自动推断最小权限，`move` 关键字强制按值捕获；
- **闭包即结构体（Struct）**：每个闭包是编译器生成的匿名结构体，字段即捕获的变量，调用即调用相应的 `Fn*` trait 方法——这解释了为什么闭包类型无法写出、为什么两个语法相同的闭包类型不同；
- **与函数指针的区别**：`fn` 指针不捕获环境（固定大小、可跨 FFI），闭包携带环境（大小等于捕获变量之和）。

判定准则：需要环境用闭包，需要类型名或 FFI 用 `fn` 指针，需要存储不同类型闭包用 `Box<dyn Fn(...)>`。
...
本节展开「闭包基础：捕获环境与匿名函数」的技术细节：闭包作为函数参数、闭包与类型推断与move 闭包。重点是类型签名、所有权语义与编译期约束如何相互作用，而不是 API 罗列；每个小节给出可编译的最小示例，并标注对应反例的失败规则。读完后应能解释：为什么这种写法能通过编译，而那种写法会被借用（Borrowing）检查器或类型系统（Type System）拒绝。
...
> **move 洞察**: `move` 是**控制闭包所有权（Ownership）的显式工具**——它在需要延长数据生命周期（Lifetimes）或跨线程/异步（Async）边界时使用。
...
- **反命题 1：「闭包总是零成本」** —— 基本成立但有边界：捕获小环境且被内联时确实零开销；但 `Box<dyn Fn>` 引入堆分配与间接调用，捕获大环境的闭包按值传递时复制成本不可忽略。
- **反命题 2：「`move` 闭包一定获取所有权」** —— 不完全：`move` 让闭包**按值捕获**——对 `Copy` 类型只是复制；对引用（Reference）类型复制的是引用本身（`&T` 是 `Copy`）。`move || x.clone()` 中若 `x` 是引用，闭包只持有引用不持有数据。

边界极限小节量化：递归闭包的不可直接表达（需 `fn` 项或 Y 组合子技巧）、闭包作为返回值的 `impl Fn` 与生命周期捕获规则（1.82 精确捕获）、以及 `Fn` trait 继承链 `Fn: FnMut: FnOnce` 的语义。
...
- [对应测验](../11_quizzes/04_quiz_closures_iterators.md) — 闭包与迭代器（Iterator）（捕获、Fn traits、惰性求值、适配器）
...
> **权威来源**: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html), [The Rust Programming Language](https://doc.rust-lang.org/book/ch13-01-closures.html)
>
...
本节把「闭包基础：捕获环境与匿名函数」的规则推到编译器与运行时（Runtime）的边界上逐一实测：边界测试：闭包捕获环境后环境失效（编译错误）、边界测试：`Fn` vs `FnMut` vs `FnOnce` 不匹…、边界测试：`move` 闭包与 `Copy` 类型的交互（编译错误/逻…、边界测试：闭包类型与 `impl Trait` 返回（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> 2) 修改使用 → `&mut T`（`FnMut`）；
> 3) 移动/消耗使用 → `T`（`FnOnce`）。
> `move ||` 强制**按值捕获**所有变量（move 语义），用于延长闭包生命周期（Lifetimes）（如返回闭包或跨线程传递）。
> 常见陷阱：
> 4) 闭包捕获 `&s` 后，原变量 `s` 被借用（Borrowing），不能移动；
> 5) `move ||` 闭包尝试多次调用（若捕获变量未实现 `Copy`）；
> 6) 闭包返回后，捕获变量在闭包内 drop。
> 修复：使用 `move ||` 强制转移所有权，或在闭包使用后立即 drop 闭包释放借用（Borrowing）。
...
> **认知路径**: 从 L0 基础概念出发，经由本节的 **闭包基础：捕获环境与匿名函数** 核心原理，通向 L2 进阶模式与 L3 工程实践。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 闭包基础：捕获环境与匿名函数 基础定义 ⟹ 正确用法 | 理解语法与语义 | 能写出符合惯用法的代码 | 高 |
| 闭包基础：捕获环境与匿名函数 正确用法 ⟹ 常见陷阱 | 忽略边界条件 | 编译错误或运行时（Runtime） bug | 高 |
| 闭包基础：捕获环境与匿名函数 常见陷阱 ⟹ 深度掌握 | 系统学习反模式 | 能进行代码审查与优化 | 高 |

> 高阶函数安全 ⟸ 环境捕获类型检查 ⟸ Fn/FnMut/FnOnce
> 回调正确性 ⟸ 闭包生命周期推断 ⟸ 借用（Borrowing）规则
...
- **理解层**：`Fn`/`FnMut`/`FnOnce` 的推断规则——给定闭包体判断其实现哪个 trait（修改捕获变量 ⟹ `FnMut`；消耗捕获变量 ⟹ `FnOnce`）；
- **应用层**：`move` 闭包在 `thread::spawn` 中的必要性——`'static` 生命周期要求与所有权转移的关系；
- **分析层**：闭包类型推断失败诊断——为什么 `let f = |x| x;` 单独存在时可能触发 E0282（类型参数需要至少一处使用约束）。

作答建议：每题先写出闭包对应的「匿名结构体（Struct） + impl」展开形式，再核对答案。
...
闭包捕获了局部变量 `x` 的引用（Reference），但尝试返回闭包时，`x` 已经离开作用域（悬垂引用）。
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| 捕获 | 闭包捕获环境变量（借用 / 可变借用（Mutable Borrow） / 按值） | 捕获规则 |
