...
闭包（closure）是**捕获环境的匿名函数**。Rust 闭包的独特性在于捕获方式由类型系统精确刻画：

- **三种捕获模式**：按不可变借用（`Fn`）、按可变借用（`FnMut`）、按值 move（`FnOnce`）——编译器根据闭包体对捕获变量的**使用方式**自动推断最小权限，`move` 关键字强制按值捕获；
- **闭包即结构体**：每个闭包是编译器生成的匿名结构体，字段即捕获的变量，调用即调用相应的 `Fn*` trait 方法——这解释了为什么闭包类型无法写出、为什么两个语法相同的闭包类型不同；
...
本节展开「闭包基础：捕获环境与匿名函数」的技术细节：闭包作为函数参数、闭包与类型推断与move 闭包。重点是类型签名、所有权语义与编译期约束如何相互作用，而不是 API 罗列；每个小节给出可编译的最小示例，并标注对应反例的失败规则。读完后应能解释：为什么这种写法能通过编译，而那种写法会被借用检查器或类型系统拒绝。
...
- **反命题 2：「`move` 闭包一定获取所有权」** —— 不完全：`move` 让闭包**按值捕获**——对 `Copy` 类型只是复制；对引用类型复制的是引用本身（`&T` 是 `Copy`）。`move || x.clone()` 中若 `x` 是引用，闭包只持有引用不持有数据。
...
本节把「闭包基础：捕获环境与匿名函数」的规则推到编译器与运行时的边界上逐一实测：边界测试：闭包捕获环境后环境失效（编译错误）、边界测试：`Fn` vs `FnMut` vs `FnOnce` 不匹…、边界测试：`move` 闭包与 `Copy` 类型的交互（编译错误/逻…、边界测试：闭包类型与 `impl Trait` 返回（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
