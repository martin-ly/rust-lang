...
「技术细节」涉及闭包作为函数参数、闭包与类型推断与move 闭包，本节逐一说明其要点。
...
「边界测试：闭包的编译错误」部分按边界测试：闭包捕获环境后环境失效（编译错误）、边界测试：`Fn` vs `FnMut` vs `FnOnce` 不匹…、边界测试：`move` 闭包与 `Copy` 类型的交互（编译错误/逻…、边界测试：闭包类型与 `impl Trait` 返回（编译错误）等6个方面的顺序逐层展开。
