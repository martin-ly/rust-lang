...
本节展开「闭包基础：捕获环境与匿名函数」的技术细节：闭包作为函数参数、闭包与类型推断与move 闭包。重点是类型签名、所有权语义与编译期约束如何相互作用，而不是 API 罗列；每个小节给出可编译的最小示例，并标注对应反例的失败规则。读完后应能解释：为什么这种写法能通过编译，而那种写法会被借用（Borrowing）检查器或类型系统拒绝。
...
作答建议：每题先写出闭包对应的「匿名结构体 + impl」展开形式，再核对答案。
