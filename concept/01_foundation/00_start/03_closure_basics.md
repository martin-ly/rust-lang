...
「核心概念」涉及闭包的语法与捕获、Fn / FnMut / FnOnce与闭包与所有权，本节逐一说明其要点。
...
理解「嵌入式测验（Embedded Quiz）」需要把握测验 1：闭包捕获模式（理解层）、测验 2：`move` 关键字（应用层）、测验 3：Fn/FnMut/FnOnce（分析层）与测验 4：闭包与生命周期（分析层），本节依次展开。
