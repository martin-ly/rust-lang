...
本节围绕「引用透明（Referential Transparency）」展开，覆盖定义 与 引用透明的工程意义 两个方面。
...
「副作用的分类与模型」部分包含副作用的通用分类 与 副作用的组合爆炸问题 两条主线，本节依次说明。
...
本节将「Rust 的副作用控制机制」分解为若干主题： `&mut T` 作为写效果（Write Effect）、`unsafe` 作为未定义效果（Undefined Effect）、`Result<T, E>` 作为异常效果（Exception Eff…与`async` 作为并发效果（Concurrency Effect）。
...
本节从纯函数的定义 与  Rust 中的"准纯函数" 两个层面剖析「纯函数与不纯函数」。
...
本节从两种范型的核心差异 与  Rust 的混合范型定位 两个层面剖析「命令式 vs 函数式范型」。
...
本节将「反例与边界测试」分解为若干主题：反例：隐式副作用的 C/C++ 陷阱、边界测试：`unsafe` 中的副作用逃逸、边界测试：`const fn` 中的副作用逃逸（编译错误）与边界测试：闭包捕获的副作用。
...
理解「边界测试：效果与纯度的编译错误」需要把握边界测试：`const fn` 中调用非 `const` 方法（编译错…、边界测试：`unsafe` 块的传染性与 FFI 边界（编译错误）、边界测试：`const fn` 中的堆分配尝试（编译错误）与边界测试：类型不匹配的基础错误，本节依次展开。
