...
副作用（side effect）是「表达式求值之外对世界的可观察改变」。Rust 没有效果类型系统（Type System），但分类模型仍是推理代码行为的基础工具：

| 类别 | 例子 | Rust 中的可追踪性 |
|:---|:---|:---|
| 内存效果 | 堆分配、`&mut` 修改 | 部分可追踪：`&mut` 独占由借用（Borrowing）检查保证，内部可变性是显式例外 |
...
- **`&mut T` 作为写效果（Write Effect）**：可变借用（Mutable Borrow）是「本函数会修改该数据」的类型级声明。签名里没有 `&mut` 参数的函数，其可观察副作用范围被类型系统（Type System）压缩到「内部可变性 + 全局状态」两个已知例外上——读者据此可快速圈定副作用搜索范围。
...
- **`async` 作为并发效果（Concurrency Effect）**：`async fn` 的返回类型 `impl Future` 使「该函数可挂起」显式化——挂起点（`.await`）是语法可见的，「函数着色」（function coloring）问题因此转化为类型检查问题：在同步上下文调用异步（Async）函数是编译错误。
...
- **工程价值**：纯函数比例高的代码库测试成本低（无 mock）、并发安全（Concurrency Safety）（无共享可变状态）、可推理性强（等式推理成立）。
...
| 迭代 | 循环语句 | map/filter/fold | 迭代器（Iterator）链（惰性好组合）+ 循环（灵活）双轨 |
...
- **别名 + 可变的效果灾难**（C 风格）：两个指针同时修改同一内存，行为依赖访问顺序——Rust 防线：借用（Borrowing）检查在编译期拒绝，此反例在 Rust 中**不可表达**；
...
效果与纯度的编译错误验证一条总原则：**声明的效果边界不能被实现突破**——类型系统把「承诺」与「实现」的一致性（Coherence）做成可判定检查：
...
判定一个纯度相关错误，先识别被破坏的承诺类型（const 可求值性 / unsafe 边界 / 类型一致性（Coherence）），错误码只是承诺-实现不一致的投影。
...
| 来源 | 与本节对应的核心论点 |
|:---|:---|
| [Wikipedia — Referential Transparency](https://en.wikipedia.org/wiki/Referential_transparency) | 引用透明性的定义与函数式编程实践 |
...
- **组合**：与 [所有权](../01_ownership_borrow_lifetime/01_ownership.md)（别名×可变性控制写效果）、[错误处理（Error Handling）](../../02_intermediate/03_error_handling/01_error_handling.md)（`Result` 异常效果）组合。
...
> **过渡**: 从「副作用无处不在」的命令式语言到 Rust 的显式效果模型，核心变化是把内存写效果、异常效果、并发效果和未定义效果分别编码到类型系统的不同机制中。
> **过渡**: 理解引用透明与纯函数后，下一步应学习 `const fn` 的编译期纯度、`unsafe` 的未定义效果边界，以及 `async`/`Result` 如何显式化并发与异常效果。
> **过渡**: Rust 的效果系统尚未完成类型化；当前通过 `&mut`/`unsafe`/`async`/`Result` 等关键字实现「穷人版效果系统」，未来 keyword generics 可能统一这些标记。

> 编译期可求值 ⟸ `const fn` 无副作用 ⟸ 纯计算子集
> 安全抽象 ⟸ `unsafe` 边界显式化 ⟸ 形式化验证聚焦点

---

## 认知路径

> **认知路径**: 从「副作用无处不在」的通用 PL 视角出发，识别 Rust 如何通过类型系统把写效果、异常效果、未定义效果显式化，再映射到 `&mut`、`Result`、`unsafe` 等具体机制。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 显式化副作用类型 ⟹ 降低组合错误 | 区分 `&mut` / `Result` / `unsafe` | 在编译期捕获效果错配 | 高 |
| 引用透明 + 纯函数 ⟹ 可局部推理 | 函数无隐藏 IO / 可变状态 | 测试与重构成本下降 | 高 |
| `unsafe` 边界显式化 ⟹ 形式化聚焦 | 隔离未定义行为风险 | 审计范围缩小到 unsafe 块 | 高 |

---

## 反命题与边界

> **反命题**: "Rust 的 `fn` 函数默认是纯函数，因为编译器会阻止副作用。" —— 错误。Rust 不强制纯度；普通函数可以包含 IO、修改可变参数、panic 等副作用，只有 `const fn` 和 `unsafe` 边界对效果有显式约束。
> **边界**: 当函数使用内部可变性（`Cell`/`RefCell`/`Mutex`）或调用 `unsafe`/`extern` 时，效果面会超出签名直观展示的范围；此时需要审查实现而不仅是签名。
