...
>
> **EN**: Type System
> **Summary**: Type System. Rust's static type system foundations covering algebraic data types (enum/struct), pattern matching, generics, and trait bounds. Bridges practical programming to formal type theory.
>
> **📎 交叉引用**
>
...
> **[TRPL Ch6](https://doc.rust-lang.org/book/ch06-00-enums-and-pattern-matching.html)** Rust's enums are most similar to algebraic data types in functional languages, such as Haskell, F#, OCaml, and others. They allow you to define a type by enumerating its possible variants.
...
> **来源: [TRPL Ch9](https://doc.rust-lang.org/book/ch09-00-error-handling.html)** Result<T, E> 强制显式错误处理，避免异常带来的隐藏控制流。✅
...
| **[TRPL Ch6](https://doc.rust-lang.org/book/ch06-00-enums-and-pattern-matching.html)** | enum 与模式匹配 | T1 match 穷尽性 |
...
| enum 类似函数式语言的 ADT | [TRPL Ch6](https://doc.rust-lang.org/book/ch06-00-enums-and-pattern-matching.html) | ✅ |
...
> **来源**: [RFC 2000: Const Generics](https://rust-lang.github.io/rfcs/2000-2000-const-generics.html) · [Rust Reference: Const Generics](https://doc.rust-lang.org/reference/items/generics.html#const-generics) · [Wikipedia: Dependent type](https://en.wikipedia.org/wiki/Dependent_type)
...
> **来源: [Rust Reference: Type Inference](https://doc.rust-lang.org/reference/type-inference.html)** Rust 的类型推断基于 HM 算法，但 trait bound 求解和生命周期推断是独立的扩展。✅
...
> **来源: [Rust Reference: Type Inference](https://doc.rust-lang.org/reference/type-inference.html)** Rust 函数签名中的泛型参数必须显式声明（`fn id<T>(x: T) -> T`），但函数体内部和 `let` 绑定的局部推断遵循 HM 原则。✅
...
> **来源: [Rust Reference: Type Inference](https://doc.rust-lang.org/reference/type-inference.html)** 类型推断的边界是工程设计与理论完备性的权衡：显式标注提供接口契约，推断减少局部冗余。✅
...
> **来源: [Rust Reference: Type Inference](https://doc.rust-lang.org/reference/type-inference.html)** Rust 选择显式性优先：函数签名必须标注类型参数，数值字面量无上下文时 fallback 到 `i32`/`f64`，避免隐式多态带来的意外。✅
...
> **来源: [The Rustonomicon: Exotic Sizes](https://doc.rust-lang.org/nomicon/)** NPO 是 Rust "零成本抽象" 的核心体现之一。✅
...
> **来源**: [Rust Reference: Unions](https://doc.rust-lang.org/reference/items/unions.html) · [The Rustonomicon: Unions](https://doc.rust-lang.org/nomicon/unions.html)
...
> **来源: [Rust Reference: Types](https://doc.rust-lang.org/reference/types.html)** Rust 的 `struct`、`enum`、`trait` 均为名义类型构造；元组、数组、函数指针、切片为结构类型构造。✅
...
> **认知功能**:
> 此命题是对「结构类型更灵活」直觉的形式化检验。关键认知：孤儿规则的约束**不是实现上的偶然**，而是**名义类型与全局 coherence 之间的逻辑必然**。
...
> [RFC 1023: Rebalancing Coherence](https://rust-lang.github.io/rfcs/1023-1023-rebalancing-coherence.html) ·
> [arXiv:2412.15042] ·
> [Pierce, TAPL, Ch.15] ·
