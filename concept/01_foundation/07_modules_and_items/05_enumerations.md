...
- **未处理 `None`**：`Option` 上直接 `unwrap` 的 panic 风险——正确模式是 `match`/`if let`/`?` 传播或 `unwrap_or` 兜底，选择依据是「失败是否可恢复」；
- **`if let` 处理 `Result` 的误用**：`if let Ok(x) = res { ... }` 静默吞掉错误——错误被丢弃时至少应 `let _ = res;` 显式表明意图，更好的做法是 `?` 或日志；
- **变体携带值的所有权移动**：`match` 中绑定变体字段默认 move——绑定后原枚举部分失效（E0382），需要保留时用 `ref`/`match &e`；
- **判定表**：枚举建模选型（`Option` vs 自定义枚举 vs `Result`）与匹配方式（`match`/`if let`/`let else`）的处置对照。

核心教训：枚举的价值在穷尽性——任何绕过穷尽检查的写法（通配 `_ => {}` 吞错）都在放弃枚举的核心收益。
...
|:---|:---|:---|:---|
| 一组互斥状态 | 枚举（带标签联合） | 定理 1 | 状态可组合共存 ⟹ 用结构体（Struct）或位标志 |
...
| 匹配后仍需使用原值 | 引用（Reference）匹配（`ref`/`&`） | §6.3 | 按值匹配 ⟹ 变体数据被 move |
...
- C. 能编译，运行时（Runtime）遇到 `Write` 时 panic
