...
> **本节关键术语**: 模块 (Module) · 路径 (Path) · use · pub · crate — [完整对照表](../../00_meta/01_terminology/terminology_glossary.md)
>
...
理解「核心概念」需要把握模块层次结构、可见性系统与路径解析，本节依次展开。
...
「技术细节」部分按文件系统映射、use 语句与重导出与工作空间（Workspace）的顺序逐层展开。
...
理解「边界测试：模块系统的编译错误」需要把握边界测试：私有字段跨模块访问（编译错误）、边界测试：`use` 路径循环依赖（编译错误）、边界测试：`pub(crate)` 与 `pub(super)` 的层…、边界测试：`use` 语句的循环依赖（编译错误）等6个方面，本节依次展开。
...
> 5) `mod foo;` 在 `foo.rs` 中引用（Reference）自己 → 循环模块；
...
「嵌入式测验」涉及测验 1：可见性修饰符（记忆层）、测验 2：路径解析（理解层）、测验 3：文件系统映射（应用层）与测验 4：`pub use` 重导出（分析层），本节逐一说明其要点。
