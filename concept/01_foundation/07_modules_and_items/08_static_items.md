...
本节用三组对照说明「静态项（Static Items）」：正确示例：全局配置常量、反例：在 safe 代码中访问 `static mut`与反例：用 `static` 替代 `const` 导致内联问题。每组先给正确示例并标注其成立的类型系统依据，再给反例并标注编译器诊断（E0xxx）或运行时后果，最后给出修正方案。判读标准：正确示例应能通过 rustc 1.97 编译且无 clippy 警告，反例的失败点必须可定位到具体规则。
