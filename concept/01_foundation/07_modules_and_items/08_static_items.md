...
> **后置概念**: [Interior Mutability](../../02_intermediate/02_memory_management/02_interior_mutability.md) · [Concurrency](../../03_advanced/00_concurrency/01_concurrency.md) · [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md)
>
...
本节用三组对照说明「静态项（Static Items）」：正确示例：全局配置常量、反例：在 safe 代码中访问 `static mut`与反例：用 `static` 替代 `const` 导致内联问题。每组先给正确示例并标注其成立的类型系统（Type System）依据，再给反例并标注编译器诊断（E0xxx）或运行时（Runtime）后果，最后给出修正方案。判读标准：正确示例应能通过 rustc 1.97 编译且无 clippy 警告，反例的失败点必须可定位到具体规则。
...
- **反命题 1：「`const` 和 `static` 差不多，随便用」** —— 错误。语义差异有实际后果：① `const` 大数组每处使用都内联展开，代码体积膨胀；② 依赖地址唯一性的代码（如 `ptr::eq(&A, &B)` 判同）对 `const` 可能得到两个地址；③ `const` 泛型（Generics）参数单态化（Monomorphization）时每实例一份。判定准则：需要地址语义 → `static`，纯值语义 → `const`。
...
本节把「静态项（Static Items）」的规则推到编译器与运行时的边界上逐一实测：边界测试：静态项的生命周期 与 边界测试：内部可变性静态项。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
本节围绕「判断推理与决策树」展开，覆盖何时使用 `static`？ 与 与其他概念的辨析 两个方面。
...
> **过渡**: 理解 `static` 与 `const` 在内存位置与内联行为上的差异后，可正确选择全局状态存储方式。
> **过渡**: 掌握可变静态项的 `unsafe` 要求后，可进一步学习 `std::sync::LazyLock` / `once_cell` 等安全初始化模式。
> **过渡**: 将静态项与生命周期结合，可理解 `'static`  lifetime 在引用类型与全局数据中的含义。
