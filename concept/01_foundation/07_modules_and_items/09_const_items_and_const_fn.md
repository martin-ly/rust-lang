...
本节围绕「技术细节与示例」展开，依次讨论常量项、常量函数、常量上下文与`const fn` 的限制。
...
「示例与反例」部分按正确示例：编译期配置计算、反例：在 `const fn` 中分配堆内存与反例：试图修改 `const`的顺序逐层展开。
...
本节从边界测试：常量泛型 与 边界测试：`const fn` 中使用 `if`/`match` 两个层面剖析「边界测试」。
...
本节从测验 1：`const` vs `static` 与 测验 2：`const fn` 的限制 两个层面剖析「嵌入式测验（Embedded Quiz）」。
