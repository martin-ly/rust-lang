...
1. **模块层次结构**：`mod` 声明形成树状嵌套，crate 根（`lib.rs`/`main.rs`）为树根。模块是命名空间也是编译单元的边界——同一 crate 内模块间共享单态化与内联机会，跨 crate 则受 `extern crate` 边界约束。
...
- **工作空间（Workspace）**：多个 crate 共享一个 `Cargo.lock` 与 `target/` 目录，成员间用路径依赖引用。workspace 是「模块系统之上的 crate 间组织层」：crate 边界决定编译单元与版本单元，模块边界决定命名与可见性，两层正交。
...
- **私有字段跨模块访问**（E0616/E0451）：结构体字段的可见性与结构体本身独立——`pub struct` 的字段默认私有，跨模块构造需所有涉及字段可见（或用构造函数）。这是「表示隐藏」的编译期保证。
