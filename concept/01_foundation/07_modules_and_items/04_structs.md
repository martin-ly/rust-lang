...
> **前置概念**: [Type System](../02_type_system/01_type_system.md) · [Ownership](../01_ownership_borrow_lifetime/01_ownership.md) · [Functions](02_functions.md) · [Terminology Glossary](../../00_meta/01_terminology/01_terminology_glossary.md)
> **后置概念**: [Enumerations](05_enumerations.md) · [Implementations](06_implementations.md) · [Traits](../../02_intermediate/00_traits/01_traits.md)
>
...
> **认知路径**: 本节从“如何组合多个相关值”出发，依次建立命名字段型、元组型、单元型结构体（Struct）的完整图景。

1. **问题识别**: 当多个值在逻辑上属于同一实体时，如何用类型表达？
2. **概念建立**: 掌握三种结构体（Struct）形式、字段可见性、结构体更新语法。
...
>
> **定理 3** [Tier 1]: 结构体更新语法 `..old` 会移动未显式指定字段的所有权（Ownership） ⟹ 若字段未实现 `Copy`，原实例将部分失效。
...
- **更新语法与所有权**：`..base` 结构更新会 move `base` 中未显式覆盖的字段——`base` 其后部分失效（非 `Copy` 字段被移出），这是「更新语法后 base 不能用」报错（E0382）的根源；
- **字段初始化简写**：`Point { x, y }` 要求变量名与字段名完全一致——名字不同必须写全 `x: my_x`；
- **不可变字段赋值**：结构体可变性是**整体**的（无字段级 `mut`）——`let p` 不可变则任何字段不可改，需要部分可变时拆分为多个结构体或用 `Cell` 包装单字段；
- **判定表**：三种结构体形式（命名字段/元组/单元）与更新语法的合法组合对照。

每组反例附「错误版 + 编译信息 + 修复版」三元组，更新语法的所有权陷阱是本节最高频考点。
...
| 跨模块（Module）访问字段 | 字段显式标 `pub` | 定理 2 | 默认私有 ⟹ `field is private`（反向推理 1） |
...
模块（Module）外构造 `pub struct User { pub username: String, email: String }` 时指定了 `email` 字段，编译器拒绝。根因是？
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| 形式 | 命名字段、元组、单元三种结构体 | Reference |
| 字段可见性 | 字段默认私有，需逐字段 `pub` | 隐私规则 |
| 所有权 | 可整体 move 或部分 move（此后整体不可用） | 所有权规则 |
| 内存布局 | 默认布局未指定；`#[repr(C)]` 等可固定 | Reference 布局规则 |
| 更新语法 | `..base` 结构体更新语法消耗剩余字段 | 文法 |

## 🔗 概念关系

- **上位（is-a）**：[Items](12_items.md) 项体系的积类型（product type）。
- **下位（实例）**：单字段元组结构体见 [Newtype 模式](../../02_intermediate/04_types_and_conversions/03_newtype_and_wrapper.md)。
- **对偶**：与 [Enumerations](05_enumerations.md) 和类型（sum type）相对。
- **组合**：与 [Implementations](06_implementations.md) 组合附加行为。
- **依赖**：字段初始化顺序与有效性依赖 [构造与初始化](../../02_intermediate/00_traits/05_construction_and_initialization.md)。
