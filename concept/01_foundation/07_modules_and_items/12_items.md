...
本节按类别盘点 Rust 的项（item）——模块树中占据命名空间的声明单元。项的关键共性：**顺序无关**（模块内项可前向引用，与语句不同）与**惰性编译**（未被使用的泛型项不实例化）。六类示例：
...
判定准则：声明位置（模块/块/trait/impl）决定项的可见性与命名空间——函数、类型、宏各占独立命名空间（同名不冲突）。
...
判定准则：关联项解决「行为挂在类型上」的组织问题；外部项解决「与外部 ABI 对接」的链接问题——两者的正确性分别由类型系统与程序员保证。
