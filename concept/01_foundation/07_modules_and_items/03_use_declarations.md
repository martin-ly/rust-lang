...
> **过渡**: 掌握 `use` 路径引入后，可进一步学习 `pub use` 重导出如何塑造 crate 的公共 API 面。
> **过渡**: 将 use 声明与模块路径结合，可理解绝对路径 `crate::` 与相对路径 `self::`/`super::` 的选择。
> **过渡**: 从 `use` 出发，可进入 glob 导入、预导入（prelude）与宏命名空间等进阶话题。
