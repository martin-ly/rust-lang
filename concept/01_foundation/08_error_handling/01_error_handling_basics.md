...
本节检验错误处理领域两条最常见的过度概括，并给出各自的适用边界：

- **反命题 1：「所有错误都应该用 `Result` 传播」** —— 不成立。程序不变量被违反（数组越界、逻辑矛盾）属于 bug，应当 panic 快速失败；只有「调用者可合理恢复」的失败（IO、解析、网络）才属于 `Result` 领域。判定准则：**错误是否属于 API 契约的一部分**——是则 `Result`，否则 panic。
- **反命题 2：「`unwrap` 永远不该出现在生产代码」** —— 过度。原型、`main` 入口、以及「失败即程序无意义继续」的场景（如读取内置配置）中 `unwrap`/`expect` 是诚实的快速失败；真正该禁止的是库代码中对**外部输入**的 `unwrap`。

边界极限小节进一步量化：嵌套 `Result<Result<T, E>, E>` 的展开深度、`?` 与 `From` 转换的隐式边界、`catch_unwind` 的 `UnwindSafe` 约束——这些决定了错误处理代码的实际形态。
...
本节用六个边界用例固化错误处理的类型规则，覆盖编译期与运行期两个层面：
...
本组测验围绕测验
1：Result 与 ? 运算符（理解层）、测验
2：Option 与 Result 的转换（应用层）、测验
3：unwrap vs expect（分析层）、测验
4：错误传播链（应用层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。
每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用（Borrowing）检查」或「运行时行为是否符合预期」。
建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
