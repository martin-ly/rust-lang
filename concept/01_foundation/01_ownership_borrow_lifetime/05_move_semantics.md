...
C++ 的 move 语义是**运行期机制**：`std::move` 只是 static_cast 到右值引用，真正的移动由移动构造/移动赋值运算符执行——它们是用户可定义、可观察、可有副作用的代码。三个关键后果：

1. **moved-from 状态合法**：移动后源对象仍存在且可析构，处于「有效但未指定」状态——标准只保证析构与赋新值安全，读取是逻辑错误（但编译器不检查）；
2. **值类别体系**：lvalue/prvalue/xvalue 三分决定重载决议与引用绑定——`std::move` 产生 xvalue，绑定到 `T&&` 触发移动重载；
3. **拷贝省略（RVO/NRVO）**：C++17 起 prvalue 的拷贝/移动强制省略——「移动」有时根本不发生。

与 Rust 对照的意义：C++ move 是「可定制的运行期操作」，Rust move 是「编译期所有权转移 + 按位拷贝」——后者的 moved-from 状态不可访问（编译错误），这是两套体系最深的分野。
...
Rust 的 move 是**编译期所有权转移**：`let y = x;`（非 `Copy` 类型）把 `x` 的所有权转移给 `y`，`x` 被静态标记为已移动，此后使用触发 E0382。三个核心要点：
...
3. **函数边界的 move**：传参是 move（或借用），返回值是 move 出——所有权沿调用链流动，每个值的负责者（drop 执行者）在编译期唯一确定。
...
实践推论：Rust 中「返回大结构体是否低效」的担心通常多余（sret 已避免拷贝）；真正需要关心的是 `Box` 大对象避免栈上构造（`Box::new_uninit` 模式，仍 nightly 部分）。
