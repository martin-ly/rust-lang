...
> **相关链接**：完整分析见 §16；Rust 类型系统中的 `union` 见 [04_type_system.md](../02_type_system/01_type_system.md) §11.6；`ManuallyDrop` 与所有权（Ownership）例外见 [01_ownership.md](01_ownership.md) §8.3。
...
本组测验围绕测验 1：生命周期省略的边界（理解层）、测验 2：HRTB 的适用场景（应用层）、测验 3：自引用结构（分析层）、测验 4：生命周期子类型（分析层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
