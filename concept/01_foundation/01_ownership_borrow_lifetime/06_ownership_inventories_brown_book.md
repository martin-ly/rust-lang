...
> [TRPL — Understanding Ownership](https://doc.rust-lang.org/book/ch04-00-understanding-ownership.html) ·
...
4. **定期复习**：所有权（Ownership）是 Rust 的核心心智模型，建议在每个 Inventory 节点做一遍自测。
...
> **认知路径**: 从 Brown Book 的所有权清单自测出发，建立「读权限 → 写权限 → 移动权限」的三态视角，再映射到 Rust 借用检查器的实际错误码。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 所有权清单节点覆盖 move/borrow/lifetime ⟹ 能识别错误 | 逐题分析 Brown Book 样例 | 预测编译器拒绝原因 | 高 |
| 权限图可视化 ⟹ 理解借用检查器决策 | 阅读 Aquascope 输出 | 手工推导 `&mut` 冲突 | 高 |
| 清单自测通过 ⟹ 所有权心智模型成形 | 能解释 E0502/E0499 等错误 | 修复真实代码中的所有权 bug | 高 |

> **过渡**: 通过 Brown Book 的交互式许可图，学习者可以把「所有权规则」从抽象语法转化为对每行代码的权限读/写/移动判断。
> **过渡**: 完成清单后，应能独立预测任意 `let`、`&mut`、`=`、`return` 组合下的借用检查器结论，而不依赖编译器试错。
> **过渡**: 将清单中的模式与 [`02_borrowing.md`](02_borrowing.md) 的 5 种修复策略对应，可在真实代码审查中快速定位所有权错误。

> 借用冲突可预测 ⟸ 权限状态跟踪 ⟸ 清单节点练习
> 所有权修复策略正确 ⟸ 错误模式归类 ⟸ 清单样题对比

---

## 反命题与边界

> **反命题**: "Brown Book 清单只适用于初学者，对经验丰富的 Rust 开发者没有价值。" —— 错误。清单中的部分重借用、返回值生命周期、嵌套借用等样题同样能暴露对 Tree Borrows 等最新模型的理解偏差。
> **边界**: 清单侧重编译期所有权规则，不覆盖运行期内存分配、并发数据竞争或 unsafe 指针规则；后者需要 [Rustonomicon](https://doc.rust-lang.org/nomicon/) 等进阶资料。
