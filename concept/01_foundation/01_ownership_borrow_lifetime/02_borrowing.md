...
- **形式化视角**：`&T` 与 `&mut T` 在分离逻辑中分别建模为共享只读权限与临时独占权限，借用有效性可被归约为类型系统对权限转移的静态追踪。
...
2. **借用规则 vs 其他语言对比**：C++ 的引用/指针无任何存活性检查（悬垂引用是 UB），Go 的指针由 GC 兜底（安全但有运行时成本），Rust 的借用由借用检查器在编译期拒绝非法情况（安全且零成本）。三者是同一问题在「谁负责保证」轴上的三个端点。
