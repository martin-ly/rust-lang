...
> **认知功能**: 此图是借用冲突的**错误诊断速查表**。当编译器报错 E0502/E0499/E0503 时，读者可对照此图定位冲突根源：是不可变借用（Mutable Borrow）阻碍了可变借用？还是可变借用阻碍了新的借用？还是可变借用期间试图直接使用原变量？底部的 NLL 对比节点特别重要——它提醒读者在 NLL 后，引用的实际生命周期（Lifetimes）可能比你想象的更短，某些「看似冲突」的代码实际上是合法的。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **定义**：`&str` 是 UTF-8 字节切片的借用形式，它在底层与 `&[u8]` 共享连续内存布局，但附加了“有效 UTF-8”的不变式。借用语义上，`&str` 是不可变借用，其生命周期受原始字符串所有者的约束。

> **原理与合理性**：将字符串建模为字节的特化切片，使 Rust 能在零拷贝的情况下同时提供文本语义（`&str`）和原始字节访问（`as_bytes()`）。类型系统通过 `str: !Sized` 禁止直接拥有 `str`，只能通过 `&str` 或 `String` 等 owned 容器访问。
...
> **原理与合理性**：`Deref` 让自定义类型继承目标类型的接口而不引入运行时开销。自动借用规则保证：只要实现 `Deref`，`&Wrapper<T>` 在需要 `&T` 的上下文自动转换。这构成了 `String → &str`、`Box<T> → &T`、`Rc<T> → &T` 等智能指针行为的基础。
...
> **相关链接**：完整交互分析见 §8.2；类型系统中的 trait 机制见 [04_type_system.md](../../01_foundation/02_type_system/04_type_system.md)。
...
> **定义**：`Cow<T>`（Clone on Write）是一个枚举，要么持有对现有数据的**借用**（`Borrowed(&T)`），要么在需要修改时**克隆为拥有副本**（`Owned(T::Owned)`）。它是借用与所有权之间的“延迟拥有”抽象。
...
> **相关链接**：与 match guards 的穷尽性差异见 [04_type_system.md](../../01_foundation/02_type_system/04_type_system.md)；HM 类型推断背景见 [04_type_system.md](../../01_foundation/02_type_system/04_type_system.md) §12.4。
