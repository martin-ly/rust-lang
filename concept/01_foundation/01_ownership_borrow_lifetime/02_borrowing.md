...
理解「形式化理论根基（Formal Foundation）」需要把握分离逻辑（Separation Logic）视角、别名-可变分离（Aliasing XOR Mutation）与内存模型演进：Stacked Borrows → Tree Borro…，本节依次展开。
...
「定理推理链（Theorem Chain）」部分按借用 ⇒ 无数据竞争、借用有效性定理与定理一致性矩阵的顺序逐层展开。
...
「边界测试：借用规则的编译错误」部分按边界测试：可变借用与共享借用冲突（编译错误）、边界测试：生命周期不匹配（编译错误）、边界测试：悬垂引用（编译错误）、边界测试：迭代器借用期间修改集合（编译错误）等7个方面的顺序逐层展开。
