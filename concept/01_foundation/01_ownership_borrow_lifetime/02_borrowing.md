...
本节将「权威定义（Definition）」分解为若干主题： TRPL 官方定义、Wikipedia 对齐定义与形式化视角。
...
本节围绕「概念属性矩阵（Attribute Matrix）」展开，依次讨论借用类型核心矩阵、借用规则 vs 其他语言对比与借用状态转换矩阵。
...
本节将「示例与反例（Examples & Counter-exam…」分解为若干主题：正确示例：不可变借用共存、正确示例：可变借用的独占性、反例：可变 + 不可变借用共存（E0502）、反例：多个可变借用（E0499）等7个方面。
