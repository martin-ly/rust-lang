...
- [Borrowing（借用）](#borrowing借用)
  ...
  - [二、概念属性矩阵（Attribute Matrix）](#二概念属性矩阵attribute-matrix)
    - [2.1 借用（Borrowing）类型核心矩阵](#21-借用类型核心矩阵)
  ...
  - [七、示例与反例（Examples \& Counter-examples）](#七示例与反例examples--counter-examples)
    - [7.1 正确示例：不可变借用（Immutable Borrow）共存](#71-正确示例不可变借用共存)
    - [7.2 正确示例：可变借用的独占性](#72-正确示例可变借用的独占性)
    - [7.3 反例：可变 + 不可变借用（Immutable Borrow）共存（E0502）](#73-反例可变--不可变借用共存e0502)
    ...
    - [7.6 反命题与边界分析](#76-反命题与边界分析)
      ...
      - [命题 3: "共享引用（Reference） \&T 总是安全的"](#命题-3-共享引用-t-总是安全的)
  ...
  - [十三、`let chains`：模式匹配（Pattern Matching）的逻辑合取扩展（1.88 stable，RFC 2497）](#十三let-chains模式匹配的逻辑合取扩展188-stablerfc-2497)
  ...
  - [十四、边界测试：借用规则的编译错误](#十四边界测试借用规则的编译错误)
    ...
    - [14.2 边界测试：生命周期（Lifetimes）不匹配（编译错误）](#142-边界测试生命周期不匹配编译错误)
    - [14.3 边界测试：悬垂引用（编译错误）](#143-边界测试悬垂引用编译错误)
    - [14.4 边界测试：迭代器（Iterator）借用期间修改集合（编译错误）](#144-边界测试迭代器借用期间修改集合编译错误)
  ...
  - [实践](#实践)
  - [🎯 嵌入式测验](#-嵌入式测验)
    - [Q1: 可变引用（Mutable Reference）和不可变引用（Immutable Reference）的共存规则是什么？](#q1-可变引用和不可变引用的共存规则是什么)
