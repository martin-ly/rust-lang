...
理解「演进方向」需要把握 `&str` 作为 `&[u8]` 的字符串特化、`Deref` / `DerefMut` 与自动借用、`AsRef` / `AsMut` 的借用语义差异、`Cow<T>`：借用-所有权混合模式等6个方面，本节依次展开。
...
「嵌入式测验（Embedded Quiz）」涉及测验 1：借用规则基础（理解层）、测验 2：Reborrow 安全（应用层）、测验 3：Two-Phase Borrow（分析层）、测验 4：借用与所有权转移（应用层）等5个方面，本节逐一说明其要点。
