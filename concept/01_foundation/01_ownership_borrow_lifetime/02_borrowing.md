...
> [Slices](../02_type_system/04_type_system.md) ·
> [Interior Mutability](../../02_intermediate/02_memory_management/03_memory_management.md)
>
...
| **类型系统（Type System）** | [`./04_type_system.md`](../02_type_system/04_type_system.md) | 引用是类型的一部分，`&T`/`&mut T` 是类型构造器 |
...
理解「待补充与演进方向（TODOs）」需要把握 `&str` 作为 `&[u8]` 的字符串特化、`Deref` / `DerefMut` 与自动借用、`AsRef` / `AsMut` 的借用语义差异、`Cow<T>`：借用-所有权混合模式等6个方面，本节依次展开。
...
> **相关链接**：完整借用分析见 §8.4；字符串与文本权威页见 [09_strings_and_text.md](../../01_foundation/06_strings_and_text/09_strings_and_text.md)。
...
> **相关链接**：完整交互分析见 §8.2；类型系统（Type System）中的 trait 机制见 [04_type_system.md](../../01_foundation/02_type_system/04_type_system.md)。
...
> **相关链接**：完整语义差异见 §8.3； coercion 与类型转换见 [14_coercion_and_casting.md](../../01_foundation/02_type_system/14_coercion_and_casting.md)。
...
> **相关链接**：完整模式分析见 §8.1；字符串处理见 [09_strings_and_text.md](../../01_foundation/06_strings_and_text/09_strings_and_text.md)。
...
> **相关链接**：运行时 panic 边界见 [13_panic_and_abort.md](../../01_foundation/08_error_handling/13_panic_and_abort.md)；`Send`/`Sync` 与内部可变性见 [01_ownership.md](01_ownership.md) §11.5。
...
> **相关链接**：与 match guards 的穷尽性差异见 [04_type_system.md](../../01_foundation/02_type_system/04_type_system.md)；HM 类型推断（Type Inference）背景见 [04_type_system.md](../../01_foundation/02_type_system/04_type_system.md) §12.4。
