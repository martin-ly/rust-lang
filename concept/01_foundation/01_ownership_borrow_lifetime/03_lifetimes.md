...
「边界测试：生命周期规则的编译错误」涉及边界测试：生命周期标注缺失（编译错误）、边界测试：生命周期过短（编译错误）、边界测试：生命周期在闭包中的捕获（编译错误）、边界测试：方法签名中 self 引用的生命周期省略冲突（编译错误）等8个方面，本节逐一说明其要点。
...
本节从测验 1：生命周期省略规则（理解层）、测验 2：生命周期标注（应用层）、测验 3：悬垂引用（分析层）、测验 4：HRTB 场景（专家级）等5个方面切入，剖析「嵌入式测验（Embedded Quiz）」的核心内容。
