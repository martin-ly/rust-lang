...
- [Lifetimes（生命周期）](#lifetimes生命周期)
  ...
  - [判定表：生命周期标注与失效场景判定](#判定表生命周期标注与失效场景判定)
  - [相关概念](#相关概念)
  - [参考来源](#参考来源)
...
> (Source: [Rust Reference — Lifetime Elision](https://doc.rust-lang.org/reference/lifetime-elision.html))
...
本节的边界用例按「错误类型 × 修复模式」组织，覆盖生命周期标注的四个高频失败点：

- **标注缺失**：多个输入引用、返回引用时编译器无法确定返回的生命周期来源（E0106）——修复是显式标注 `'a` 声明关系，而非「随便加个生命周期」；
- **生命周期过短**：返回引用的数据在函数内创建（E0515/E0597）——根本修复是返回所有权而非引用；
- **闭包捕获**：闭包捕获引用后逃逸到更长作用域——`'static` 约束（如 `thread::spawn`）与局部引用的冲突需用 `move` + 所有权转移解决；
- **`'static` 误用**：把「活得足够久」误解为「必须是 `'static`」——`T: 'static` 只要求「不含非静态引用」，拥有所有权的类型天然满足。

每组用例附「错误版 + 修复版 + 判定准则」，错误信息中的生命周期关系图是最佳学习材料。
...
本节把「Lifetimes（生命周期）」组织成一条从直观到形式化的认知路径：边界测试：生命周期省略规则的三条规则（编译错误） 与 边界测试：静态生命周期 `'static` 的滥用与字符串字面量（编译…。起点是该主题最常见的使用场景，终点是它与类型系统（Type System）/所有权模型的形式化衔接；沿途每一步都标注「为什么必须如此」的推理依据。按路径完整走一遍，应能独立复现本页的全部结论。
...
> **权威来源**: [Rust Reference — Lifetime Elision](https://doc.rust-lang.org/reference/lifetime-elision.html) · [TRPL — Validating References with Lifetimes](https://doc.rust-lang.org/book/ch10-03-lifetime-syntax.html) · [Rustonomicon — Lifetimes](https://doc.rust-lang.org/nomicon/lifetimes.html)
>
...
|:---|:---|:---|:---|
| 函数参数不含引用 | 无需生命周期标注 | §6.2 决策树 T1 | — |
| 单输入引用且返回引用 | Elision 自动推导，无需标注 | Elision Rule 2（§6.2 T3） | 多个输入生命周期 ⟹ 规则不适用 |
| 方法含 `&self`/`&mut self` | 返回值生命周期默认绑定 self | Elision Rule 3 | 返回引用来自其他参数 ⟹ 需显式标注 |
| 多输入引用且返回引用 | 必须显式标注 `'a` | Elision 无法判定来源（§6.2 唯一例外） | 可重构为单一来源以恢复省略 |
| safe 代码返回局部变量的引用 | 编译拒绝：无悬垂指针 | 区域类型保证（§6.1 命题 T） | unsafe 裸指针/`transmute` 可绕过 ⟹ 悬垂 UB |
| 自引用结构未 `Pin` | 移动后内部引用失效 | §6.1 命题反例 F2 | 用 `Pin` 固定，或改用索引/arena 消除自引用 |
| `Rc`/`Arc` 循环引用 | 非悬垂，但内存泄漏 | 生命周期不表达所有权循环（§6.1 F3） | `Weak` 打破循环 |
| `Box::leak` 制造 `'static` | 安全但不可回收 | §6.1 四层分类（工程层） | 误以为 `'static` = 自动释放 ⟹ 泄漏 |
...
本节 5 道测验按认知层级递进，覆盖生命周期从理解到前沿的完整谱系：

- 测验 1（理解层）：省略规则三条——输入引用唯一时输出继承之、`&self` 优先、其余必须标注；
- 测验 2（应用层）：给定签名判断是否需要显式标注及应如何标注；
- 测验 3（分析层）：悬垂引用的构造与防护——为什么编译器拒绝是定理而非启发式；
- 测验 4（专家级）：HRTB（`for<'a>`）场景——「对所有生命周期成立」的量化约束何时必需；
- 测验 5（实验级）：NLL 与 Polonius 的差异——流敏感分析将放宽哪些当前被拒的代码。

作答建议：测验 4-5 需要结合 [Polonius 设计文档](https://github.com/rust-lang/polonius)，其余可凭本节内容作答。
