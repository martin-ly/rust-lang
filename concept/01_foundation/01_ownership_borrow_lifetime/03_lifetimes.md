...
本节把「Lifetimes（生命周期）」组织成一条从直观到形式化的认知路径：边界测试：生命周期省略规则的三条规则（编译错误） 与 边界测试：静态生命周期 `'static` 的滥用与字符串字面量（编译…。起点是该主题最常见的使用场景，终点是它与类型系统/所有权模型的形式化衔接；沿途每一步都标注「为什么必须如此」的推理依据。按路径完整走一遍，应能独立复现本页的全部结论。
