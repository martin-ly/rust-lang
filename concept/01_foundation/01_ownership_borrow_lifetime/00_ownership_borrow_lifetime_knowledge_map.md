...
本页从**结构（Structure）**视角梳理 Rust 所有权系统的概念层次、依赖关系与学习路径。
...
| 借用模式 | 检查时机 | 运行时开销 | 灵活性 | 安全性 |
| :--- | :--- | :--- | :--- | :--- |
| 不可变借用 | 编译时 | 零成本 | 高 | 完全安全 |
| 可变借用 | 编译时 | 零成本 | 中 | 完全安全 |
| `Cell<T>` | 编译时 | 零成本 | 中 | 限制在 `Copy` 类型 |
| `RefCell<T>` | 运行时 | 引用计数 | 高 | 运行时 panic |
...
| 规则2 | 只有一个输入生命周期 | 输出继承该生命周期 | `fn f(x: &i32) -> &i32` |
...
| 智能指针 | 所有权 | 线程安全 | 运行时开销 | 使用场景 |
| :--- | :--- | :--- | :--- | :--- |
| `Box<T>` | 独占 | ❌ | 零成本 | 堆分配 |
| `Rc<T>` | 共享 | ❌ | 引用计数 | 单线程共享 |
| `Arc<T>` | 共享 | ✅ | 原子引用计数 | 多线程共享 |
| `Cell<T>` | 独占 | ❌ | 零成本 | `Copy` 类型内部可变 |
| `RefCell<T>` | 独占 | ❌ | 运行时检查 | 运行时借用 |
...
高级所有权模式 → 复杂借用 → 生命周期省略 → trait 对象生命周期 → 智能指针高级用法 → 内部可变性 → 并发模式 → 性能优化 → 设计模式
...
> **过渡**: 从所有权过渡到借用，可以理解 Rust 如何在保证内存安全的同时支持灵活的引用访问。
>
