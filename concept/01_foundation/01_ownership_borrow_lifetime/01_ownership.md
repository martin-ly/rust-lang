...
>
> **本节关键术语**: 所有权 (Ownership) · 移动 (Move) · 借用 (Borrow) · 引用 (Reference) · 生命周期 (Lifetime) — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
...
- [Ownership（所有权）](#ownership所有权)
  ...
  - [五、决策/边界判定树（Decision / Boundary Tree）](#五决策边界判定树decision--boundary-tree)
    ...
    - [5.3 判定表：值语义场景的所有权处置](#53-判定表值语义场景的所有权处置)
  ...
  - [参考来源](#参考来源)
  - [学术背景：Brown Book 所有权研究引用](#学术背景brown-book-所有权研究引用)
...
> **[Ralf Jung, "The Meaning of Memory Safety", PLArch 2021](https://plv.mpi-sws.org/)** 内存安全的精确定义：程序不会出现「指向已释放内存的指针被解引用（Reference）」或「对同一内存的非原子并发写」。 ✅
...
> **[COR: ETH Zurich, Technical Report](https://inf.ethz.ch/)** 所有权跟踪的操作语义形式化，堆演进展现代数模型。 ⚠️（非 peer-reviewed 论文，为技术报告）

---

## 二、概念属性矩阵（Attribute Matrix）

属性矩阵把所有权的语义拆成三个可独立核查的维度，每个维度给出「判定式」表述——即给定一段代码，可以机械地判断它落在矩阵的哪一格：

1. **核心规则矩阵**：四条编译器强制规则（唯一所有权、作用域绑定、move 语义、Copy 例外）。判定依据是 rustc 的错误码体系：E0382（use of moved value）、E0505/E0597（作用域与借用冲突）等，每条规则都有对应的编译期诊断。
2. **跨语言对比矩阵**：把同一组属性（内存管理方式、安全保证阶段、运行时开销、形式化基础）横向映射到 C++、Haskell、Go。关键差异不在「能不能做到」，而在「由谁保证」：Rust 由编译器保证，C++ 由程序员纪律保证，Go 由运行时 GC 保证。
3. **所有权状态机**：把变量在任意程序点所处的状态抽象为 `Own(T)` / `Moved` / `Borrow(&T)` / `Borrow(&mut T)` / `Dropped` 五态，借用检查（Borrow Checking）本质上就是验证该状态机的每次迁移是否合法。

读法建议：先用规则矩阵建立「编译器会检查什么」的清单，再用状态机理解「检查在每个程序点如何发生」，最后用对比矩阵定位 Rust 方案在语言设计空间中的坐标。
...
所有权不是工程约定，而是有严格形式化谱系的内存管理方案。本节建立三层理论根基：

1. **线性类型（Linear Types）**：每个值恰好被消费一次——Rust 的 move 语义即线性类型的工程化松弛（relaxation）：`Copy` 类型放宽为仿射（affine，至多一次）之外的「可复制」特例，借用则引入受控的别名。
2. **区域推断（Region Inference）与所有权**：Cyclone 等研究型语言证明「静态区域 + 所有权」可在无 GC 下保证内存安全（Memory Safety）；Rust 的生命周期即区域（region），借用检查器即区域推断器。
3. **分离逻辑（Separation Logic）视角**：`&mut T` 授予对 `T` 的**独占访问权**——形式上等价于分离逻辑中的 `points-to` 谓词 `x ↦ v`；`&T` 的共享性通过「不可变 ⟹ 并发可共享」约束保证无数据竞争。

核心判定：Rust 的所有权规则 = 线性逻辑「每资源恰一拥有者」+ 借用规则「共享不可变 XOR 独占可变」的合取。理解这一合取，所有借用检查错误都可还原为对其中某一条的违反。
...
> 建议读者在学习任何子概念后回到此图定位其所属分支，建立「局部细节 ↔ 全局结构」的双向关联。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
本节从我的类型应该是 Copy 吗？" 决策树、所有权违规边界判定与判定表：值语义场景的所有权处置切入，剖析「决策/边界判定树（Decision / Boundary…」的核心内容。
...
> **认知功能**: 此决策树将「是否实现 Copy」的抽象判断转化为**可操作的二进制冷查**。读者面对一个新类型时，按图索骥依次检查三个条件（标量值、无 Drop、语义等价），即可得出确定结论。它消除了「Copy 是否安全」的直觉猜测，将判断责任从「程序员的内存安全（Memory Safety）直觉」转移到了「结构化的条件检查」。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **unsafe 语义**: `unsafe` 不是关闭借用（Borrowing）检查器，而是将编译期证明责任转移给程序员——程序员需手动维护内存安全（Memory Safety）不变量 来源: [Rustonomicon / 2025](https://doc.rust-lang.org/nomicon/index.html)
> **认知功能**: 此图建立了**「违规模式 → 编译错误/运行时后果」的快速查错映射**。当读者遇到 E0382、E0499、E0502 等错误时，可直接回查此图定位自己违反了哪条所有权规则。关键洞察：前四项是编译期拦截（安全），最后一项是运行时 UB（危险），这强化了「unsafe 是责任转移而非权限开放」的核心认知。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
| 只需读取、无需取得所有权 | 不可变借用（Immutable Borrow） `&T`，可多个共存 | 借用规则（详见 [02_borrowing.md](02_borrowing.md)） | 借用存活期间 move 所有者 ⟹ 编译错误（§5.2 B1） |
| 需要修改但不转移所有权 | 可变借用（Mutable Borrow） `&mut T`，独占访问 | 别名-可变分离（Aliasing XOR Mutation） | 与任何其他引用共存 ⟹ E0499/E0502（§5.2） |
...
| 新类型只含标量、无自定义 `Drop`、复制语义等价 | 实现 `Copy` | §5.1 决策树三条件 | 含堆资源或自定义 `Drop` ⟹ 不能 `Copy` |
| 所有者离开作用域 | 自动调用 `Drop` 释放资源 | RAII + 所有权 ⟹ 确定性释放（§6.1 前提 2） | `std::mem::forget` / `ManuallyDrop` 抑制 drop（§8.3 补充） |

## 六、定理推理链（Theorem Chain）

本节把「所有权 ⟹ 内存安全」展开为一条可逐步核验的推理链：

1. **每个值有且仅有一个所有者** ⟹ 析构点唯一确定 ⟹ 无 double-free；
2. **move 使源绑定失效（编译期标记）** ⟹ 移动后使用被静态拒绝 ⟹ 无 use-after-move；
3. **借用受所有者生命周期约束** ⟹ 引用存活区间 ⊆ 被引用值的存活区间 ⟹ 无悬垂引用（E0106/E0597 即此条违规）；
4. **`&mut` 独占 + `&` 共享只读** ⟹ 同一时刻不存在「可读别名 + 可写别名」组合 ⟹ 数据竞争在类型层面不可表达（配合 `Send`/`Sync` 标记 trait 扩展到并发）；
5. **Drop 顺序确定（声明逆序）** ⟹ 析构副作用可被静态推理 ⟹ RAII 资源管理可组合。

链条中任何一环被 `unsafe` 绕开，该环对应的安全保证即转移给程序员承担——这正是 `unsafe` 的形式化含义。
...
> **[💡 原创分析](../../00_meta/00_framework/methodology.md)** "模循环引用"的例外是引用计数本身的运行时限制，非编译期可证。 💡
...
> **[Girard 1987 (线性逻辑)](https://doi.org/10.1016/0304-3975(87)90045-4)** 资源消耗与产生的代数表示对应线性逻辑中的资源公理。 ✅
...
| **C3: Safe Rust 内存安全（Memory Safety）完备性** | L1+L2+T1+T2+T3+T4+T5 | 无 UAF + 无 DF + 无数据竞争（模循环引用/`forget`） | 全部 L4 公理集合 | — | `unsafe` 块突破公理、FFI 边界、循环引用泄漏 | — |

> **来源: [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/)** L1/L2/T4/T5 — 基于 Iris 框架中的资源代数 (Resource Algebra) 与分离逻辑分数权限。 ✅
> **[Girard 1987 (线性逻辑)](https://doi.org/10.1016/0304-3975(87)90045-4)** L1/L2 — 线性逻辑 ⊗ 与仿射逻辑 contraction 禁止。 ✅
...
> **来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)** C1/C2 — `mem::forget` 与裸指针突破 Safe Rust 保证。 ⚠️
> **[💡 原创分析](../../00_meta/00_framework/methodology.md)** C3 — "内存安全完备性" 是各定理的合取，模 `unsafe` 与循环引用。 💡
...
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/04_inter_layer_map.md) §4.1 "内存安全完备性"
...
本节示例按「正例建立直觉、反例划定边界」的原则组织，所有代码均按 rustc 1.97.0（Edition 2024）语义描述：

- **正确示例**展示所有权机制的常规用法：move 转移、`Copy` 类型的按位复制、函数传参与返回值中的所有权流动。阅读重点是观察「编译器在哪个精确的程序点把变量标记为 moved」。
- **反例**全部是可复现的编译错误，每条标注错误码（如 E0382 use of moved value、E0505 cannot move out because it is borrowed）。这些错误不是「风格建议」，而是类型系统（Type System）的硬性拒绝——绕过它们的唯一途径是 `unsafe` 或重构数据流。
- **判定方法**：给定任何一段涉及变量传递的代码，先问三个问题——值的所有者是谁？所有权在哪一行转移？转移后原变量是否还被使用？三个问题都能回答，则该代码的所有权行为已完全确定。

正例与反例成对阅读效果最佳：每个反例都是把对应正例中的某条规则打破后的结果，对照差异即可定位规则的精确边界。逐一说明其要点。
...
> **认知功能**: 此决策树展示了「编译期捕获所有内存错误」这一命题的**成立边界**。绿色节点表示定理成立的条件路径，红色节点表示反例触发条件。读者通过此图可建立关键认知：Safe Rust（无 unsafe、无 FFI）是编译期安全的，但一旦越过这两条边界，安全保证即失效。这构成了「信任 Rust 编译器」与「unsafe 时保持警惕」之间的认知分水岭。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> 读者常混淆这三类，此决策树通过条件分支强制分类，帮助建立「Move 不可突破，但存在安全例外和设计例外」的精确认知。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **认知功能**: 此图揭示了「RAII 自动释放」的三个**安全但非默认的逃逸门**。所有反例都是 Safe Rust 中的合法操作（mem::forget 甚至不是 unsafe），它们不改变类型的安全性，但改变了资源管理的默认契约。关键认知：所有权（Ownership） ≠ 强制释放，而是「默认自动释放 + 显式可控逃逸」。这纠正了「Rust 不允许内存泄漏」的常见误解。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> 此图帮助读者区分「核心机制零成本」与「扩展机制有成本」的边界，避免将所有权系统整体误判为「有运行时开销」。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
| 同时存在可变与不可变借用（Immutable Borrow） | `E0502` | 缩小借用作用域，或重构为单一可变借用 | 先完成只读遍历，再修改 |
...
> **[Girard 1987 (线性逻辑)](https://doi.org/10.1016/0304-3975(87)90045-4)** "Affine Logic: 资源不可复用" — 仿射逻辑允许 weakening（丢弃）但禁止 contraction（复制）。 ✅
> **[Girard 1987 (线性逻辑)](https://doi.org/10.1016/0304-3975(87)90045-4)** "线性逻辑 ⊗: 资源组合" — 张量积表示多个资源同时存在。 ✅
...
> **[💡 原创分析](../../00_meta/00_framework/methodology.md)** "5 条主线的 6 步递进" — 认知脚手架设计。 💡
...
- **上层概念**: 无（入口概念，无前置依赖）
- **下层概念**: [L2 泛型（Generics）](../../02_intermediate/01_generics/01_generics.md) · [Borrowing](02_borrowing.md) · [Lifetimes](03_lifetimes.md) · [Smart Pointers](../../02_intermediate/02_memory_management/01_memory_management.md)

| 概念 | 文件 | 关系 |
|:---|:---|:---|
| **借用（Borrowing）** | [`./02_borrowing.md`](02_borrowing.md) | 所有权临时授权，不转移归属 |
| **生命周期** | [`./03_lifetimes.md`](03_lifetimes.md) | 引用有效期约束，与所有权互补 |
| **类型系统（Type System）** | [`./04_type_system.md`](../02_type_system/01_type_system.md) | Copy/Move 语义由类型系统决定 |
| **Traits** | [`../02_intermediate/00_traits/01_traits.md`](../../02_intermediate/00_traits/01_traits.md) | `Drop`、`Copy`、`Clone`、`Send` 等 trait |
| **智能指针（Smart Pointer）** | [`../02_intermediate/02_memory_management/03_memory_management.md`](../../02_intermediate/02_memory_management/01_memory_management.md) | `Rc`、`Arc`、`Box` 扩展所有权语义 |
| **并发** | [`../03_advanced/00_concurrency/01_concurrency.md`](../../03_advanced/00_concurrency/01_concurrency.md) | `Send`/`Sync` + 所有权 = 无数据竞争 |
| **FFI / Unsafe** | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) | 所有权公理缺口与程序员责任 |
| **内部可变性** | [`../02_intermediate/02_memory_management/03_memory_management.md`](../../02_intermediate/02_memory_management/01_memory_management.md) | `RefCell` 运行时替代编译期检查 |
| **Pin 与自引用** | [`../03_advanced/01_async/02_async.md`](../../03_advanced/01_async/01_async.md) §8.5 | 位置不变性约束下的所有权 |
| **C++ 对比** | [`../05_comparative/01_rust_vs_cpp.md`](../../05_comparative/01_systems_languages/01_rust_vs_cpp.md) | `unique_ptr` vs Rust 所有权 |
...
C++ 的对象生命周期由构造/析构函数族（默认构造、拷贝构造、移动构造、析构）定义，Rust 则由**初始化即所有权确立**定义。关键差异：

- **无默认构造概念**：Rust 不存在「先默认构造再赋值」——变量必须先初始化后使用（E0381），`T::default()` 只是普通关联函数，无语言特权；
- **move 是 memcpy + 源失效，不调用任何代码**：C++ move 构造是用户可定义的运行时代码；Rust move 是编译期所有权转移，源对象不产生析构调用；
- **析构不可重入**：C++ 析构函数可抛出、可访问 `this`；Rust `Drop::drop` 取 `&mut self`，对象在 drop 后内存即被视为失效，禁止部分 move 后整体 drop（E0382 保护）；
- **复制语义**：C++ 拷贝构造隐式且可能昂贵；Rust 隐式复制仅限 `Copy` 类型（按位复制即完整语义），其余必须显式 `.clone()`。

迁移判定：C++ 中依赖拷贝构造副作用（如引用计数自增）的设计，在 Rust 中必须改用显式 `Clone` 实现或 `Rc`/`Arc`。
...
> [💡 原创分析](../../00_meta/00_framework/methodology.md) · [Rust Reference — §4.1.8](https://doc.rust-lang.org/reference/introduction.html) ✅
...
> **原理与合理性**：`forget` 在需要将资源控制权完全移交给外部系统、或故意 leak 内存以避免非法 double-free 时使用。因为 `forget` 后值不再被 drop，所以调用者必须承担资源清理责任；与 `ManuallyDrop` 不同，`forget` 是“一次性”放弃释放。

```rust,ignore
use std::mem;

let s = String::from("leak");
mem::forget(s); // s 的内存被泄漏，Drop 不会执行
```
...
> **原理与合理性**：两者都是所有权的“例外”场景，用于 unsafe 代码和低层布局。`ManuallyDrop` 通过 `ManuallyDrop::drop(&mut self)` 显式析构，避免编译器自动 drop。`MaybeUninit` 常用于 FFI、自定义集合或 union，因为它允许值在完全初始化前被操作，防止对未初始化数据调用 drop。

```rust,ignore
use std::mem::{ManuallyDrop, MaybeUninit};

let mut m = ManuallyDrop::new(String::from("manual"));
unsafe { ManuallyDrop::drop(&mut m); } // 显式 drop

let mut u: MaybeUninit<String> = MaybeUninit::uninit();
u.write(String::from("init"));
unsafe { let _ = u.assume_init(); } // 取得所有权后才负责 drop
```
...
> **原理与合理性**：Rust 的所有权在编译期静态检查，没有运行时开销；`unique_ptr` 依赖指针间接访问且可在运行时构造出空指针。Rust 的 `Clone`/`Copy` 区分值语义与引用语义，而 C++ 依赖拷贝/移动构造函数和 RAII 约定。

```cpp
// C++：需要显式 std::move，且 unique_ptr 可以为空
std::unique_ptr<int> a = std::make_unique<int>(42);
std::unique_ptr<int> b = std::move(a); // a 变为空
```

```rust,ignore
// Rust：赋值即 move，变量不可再用，无空状态
let a = Box::new(42);
let b = a;
// a 已 move，编译期禁止访问
```
...
> **[💡 原创分析](../../00_meta/00_framework/methodology.md)** "T: Send ⇔ 将 T 的值从线程 A move 到线程 B 是内存安全的" 是 Send 语义的形式化重述。 💡
> **补充: Sync trait**
...
> **相关链接**：union 与 FFI 的类型安全边界见 [04_lifetimes_advanced.md](04_lifetimes_advanced.md) §16；unsafe 所有权模式见 [Rustonomicon: FFI](https://doc.rust-lang.org/nomicon/ffi.html)。
...
本节用一组「编译器必然拒绝」的最小用例固化所有权规则，每个用例标注其违反的不变量与错误码：

| 测试意图 | 违反的不变量 | 典型错误码 |
|:---|:---|:---|
| move 后使用源变量 | 所有者唯一且 move 后源失效 | E0382 |
...
> **相关判定树**: [所有权判定树](../../00_meta/00_framework/concept_definition_decision_forest.md#二所有权判定树) · [内存安全 FTA](../../00_meta/00_framework/fault_tree_analysis_collection.md#二内存安全失效树)
> **相关谓词映射**: [own(τ) 谓词](../../00_meta/02_sources/02_rustbelt_predicate_map.md#二所有权谓词-ownτ-映射)
...
> **修正**: Rust 的索引操作 `v[i]` 在越界时 panic。如需安全访问，使用 `v.get(i)` 返回 `Option<&T>`。这体现了 Rust"显式错误处理（Error Handling）"的哲学——不静默失败，也不允许未定义行为。[来源: [Rust Standard Library](https://doc.rust-lang.org/std/index.html)]
...
> **与本页权威解释的关系**：
>
> - 本页从 **线性/仿射类型论** 与 **RustBelt/COR 形式化** 角度建立所有权的严格定义；Brown Book 则从 **可视化认知** 与 **错误修复脚手架** 角度降低学习门槛，两者互补。
> - Brown Book 的 *Ownership Inventory* 与 *Fixing Ownership Errors* 可作为本页定理链（T-001 → T-002 → T-003）的**应用层自测与诊断工具**，帮助学习者将形式化规则映射到具体编译器报错。
> - 建议阅读顺序：先读本页建立所有权唯一性、Move/Copy、RAII 的心智模型；再读 Brown Book 的交互式章节，通过可视化验证直觉；最后回到本页的 §7.7 修复所有权错误与边界测试，完成“理论 → 可视化 → 实践”的闭环。
