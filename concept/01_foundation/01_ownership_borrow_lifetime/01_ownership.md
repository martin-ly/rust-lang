...
>
> **本节关键术语**: 所有权 (Ownership) · 移动 (Move) · 借用 (Borrow) · 引用 (Reference) · 生命周期 (Lifetime) — [完整对照表](../../00_meta/01_terminology/terminology_glossary.md)
...
> **前置依赖**: 无（L1 入口概念）
> **后置延伸**: [L2 泛型（Generics）](../../02_intermediate/01_generics/02_generics.md) · L4 所有权（Ownership）形式化 · L3 Unsafe
...
本节将「权威定义（Definition）」分解为若干主题： Wikipedia 定义、TRPL 官方定义与形式化定义（RustBelt / COR）。
...
理解「概念属性矩阵（Attribute Matrix）」需要把握核心规则矩阵、跨语言对比矩阵与所有权状态机，本节依次展开。
...
| 值需要堆分配或构成递归类型 | `Box<T>` 持有唯一所有权 | RAII：所有者离开作用域自动 drop（§6.1） | 需要多所有者共享 ⟹ `Box` 不适用，见 [12_smart_pointers.md](../../02_intermediate/02_memory_management/12_smart_pointers.md) |
...
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/inter_layer_map.md) §4.1 "内存安全完备性"
...
「示例与反例（Examples & Counter-exam…」涉及正确示例：所有权转移、正确示例：Copy 类型、反例：使用已 move 的值（编译错误 E0382）、反例：在函数间错误地假设所有权保留等7个方面，本节逐一说明其要点。
...
- **上层概念**: 无（入口概念，无前置依赖）
- **下层概念**: [L2 泛型（Generics）](../../02_intermediate/01_generics/02_generics.md) · [Borrowing](02_borrowing.md) · [Lifetimes](03_lifetimes.md) · [Smart Pointers](../../02_intermediate/02_memory_management/03_memory_management.md)

| 概念 | 文件 | 关系 |
|:---|:---|:---|
| **借用（Borrowing）** | [`./02_borrowing.md`](02_borrowing.md) | 所有权临时授权，不转移归属 |
| **生命周期** | [`./03_lifetimes.md`](03_lifetimes.md) | 引用有效期约束，与所有权互补 |
| **类型系统（Type System）** | [`./04_type_system.md`](../02_type_system/04_type_system.md) | Copy/Move 语义由类型系统决定 |
| **Traits** | [`../02_intermediate/00_traits/01_traits.md`](../../02_intermediate/00_traits/01_traits.md) | `Drop`、`Copy`、`Clone`、`Send` 等 trait |
| **智能指针（Smart Pointer）** | [`../02_intermediate/02_memory_management/03_memory_management.md`](../../02_intermediate/02_memory_management/03_memory_management.md) | `Rc`、`Arc`、`Box` 扩展所有权语义 |
| **并发** | [`../03_advanced/00_concurrency/01_concurrency.md`](../../03_advanced/00_concurrency/01_concurrency.md) | `Send`/`Sync` + 所有权 = 无数据竞争 |
| **FFI / Unsafe** | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/03_unsafe.md) | 所有权公理缺口与程序员责任 |
| **内部可变性** | [`../02_intermediate/02_memory_management/03_memory_management.md`](../../02_intermediate/02_memory_management/03_memory_management.md) | `RefCell` 运行时替代编译期检查 |
| **Pin 与自引用** | [`../03_advanced/01_async/02_async.md`](../../03_advanced/01_async/02_async.md) §8.5 | 位置不变性约束下的所有权 |
...
> **相关链接**：union 与 FFI 的类型安全边界见 [30_lifetimes_advanced.md](30_lifetimes_advanced.md) §16；unsafe 所有权模式见 [Rustonomicon: FFI](https://doc.rust-lang.org/nomicon/ffi.html)。
...
> **相关谓词映射**: [own(τ) 谓词](../../00_meta/02_sources/rustbelt_predicate_map.md#二所有权谓词-ownτ-映射)
