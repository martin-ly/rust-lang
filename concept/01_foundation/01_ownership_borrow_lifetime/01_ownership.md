...
- [Ownership（所有权）](#ownership所有权)
  ...
  - [二、概念属性矩阵（Attribute Matrix）](#二概念属性矩阵attribute-matrix)
    ...
    - [2.3 所有权（Ownership）状态机](#23-所有权状态机)
  - [三、形式化理论根基（Formal Foundation）](#三形式化理论根基formal-foundation)
    ...
    - [3.3 区域类型（Region Types / 生命周期（Lifetimes））](#33-区域类型region-types--生命周期)
  ...
  - [六、定理推理链（Theorem Chain）](#六定理推理链theorem-chain)
    - [6.1 核心定理：Safe Rust 无内存泄漏（模循环引用（Reference））](#61-核心定理safe-rust-无内存泄漏模循环引用)
  ...
  - [七、示例与反例（Examples \& Counter-examples）](#七示例与反例examples--counter-examples)
    ...
    - [7.5 反命题与边界分析](#75-反命题与边界分析)
      ...
      - [命题 2（运行时（Runtime）层）: "Move 后原变量绝对不可访问"](#命题-2运行时层-move-后原变量绝对不可访问)
      - [命题 3（语义层）: "所有权 = RAII = 自动释放"](#命题-3语义层-所有权--raii--自动释放)
      - [命题 4（工程层）: "所有权系统完全零成本抽象（Zero-Cost Abstraction）"](#命题-4工程层-所有权系统完全零成本抽象)
  ...
  - [十一、边界测试：所有权规则的编译错误](#十一边界测试所有权规则的编译错误)
    ...
    - [11.6 边界测试：`Copy` 类型在闭包（Closures）中的捕获（编译错误）](#116-边界测试copy-类型在闭包中的捕获编译错误)
  ...
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    ...
    - [测验 3：借用（Borrowing）与所有权的关系（分析层）](#测验-3借用与所有权的关系分析层)
...
> **[Ralf Jung, "The Meaning of Memory Safety", PLArch 2021](https://web.archive.org/web/*/https://plv.mpi-sws.org/semantics-of-memory-safety/)** 内存安全的精确定义：程序不会出现「指向已释放内存的指针被解引用（Reference）」或「对同一内存的非原子并发写」。 ✅
