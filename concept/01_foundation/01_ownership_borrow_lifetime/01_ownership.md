...
2. **区域推断（Region Inference）与所有权**：Cyclone 等研究型语言证明「静态区域 + 所有权」可在无 GC 下保证内存安全；Rust 的生命周期即区域（region），借用检查器即区域推断器。
...
- **反例**全部是可复现的编译错误，每条标注错误码（如 E0382 use of moved value、E0505 cannot move out because it is borrowed）。这些错误不是「风格建议」，而是类型系统的硬性拒绝——绕过它们的唯一途径是 `unsafe` 或重构数据流。
