...
理解「形式化理论根基（Formal Foundation）」需要把握线性逻辑（Linear Logic）、仿射逻辑（Affine Logic）与区域类型（Region Types / 生命周期），本节依次展开。
...
本节将「定理推理链（Theorem Chain）」分解为若干主题：核心定理：Safe Rust 无内存泄漏（模循环引用）、所有权转移的代数结构与定理一致性矩阵。
...
本节围绕「补充：C++ 构造函数/析构函数语义 vs Rust 的所…」展开，依次讨论构造函数类别对比、初始化语义对比与析构函数对比。
...
「边界测试：所有权规则的编译错误」部分按边界测试：use-after-move（编译错误）、边界测试：Copy trait 不满足时的赋值（编译错误）、边界测试：循环引用导致所有权无法释放（逻辑错误）、边界测试：部分 move 后访问被 move 的字段（编译错误）等9个方面的顺序逐层展开。
