...
本节围绕「技术细节」展开，依次讨论 as 转换的完整矩阵、From/Into 与 TryFrom/TryInto与指针转换。
...
「边界测试：类型转换的编译错误」涉及边界测试：不安全的 `as` 转换导致截断（运行时错误）、边界测试：裸指针与引用转换的生命周期丢失（编译错误 / 运行时 UB）、边界测试： trait 对象强制转换的 `Sized` 约束（编译错误）、边界测试：`as` 关键字的转换限制（编译错误）等8个方面，本节逐一说明其要点。
