> **内容分级**: [综述级]
>
> **本节关键术语**: 强制转换 (Coercion) · 显式转换 (Casting) · as · Deref 强制转换 · 类型推断（Type Inference） — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# 类型强制与转换：显式与隐式的边界
>
...
> **前置概念**: [Type System](01_type_system.md) · [Ownership](../01_ownership_borrow_lifetime/01_ownership.md) · [Traits](../../02_intermediate/00_traits/01_traits.md)
> **后置概念**: [FFI](../../03_advanced/04_ffi/01_rust_ffi.md) · [Generics](../../02_intermediate/01_generics/01_generics.md)
...
- [Type System](01_type_system.md) — 类型系统（Type System）
...
本节把「类型强制与转换：显式与隐式的边界」的规则推到编译器与运行时的边界上逐一实测：边界测试：不安全的 `as` 转换导致截断（运行时错误）、边界测试：裸指针与引用转换的生命周期丢失（编译错误 / 运行时 UB）、边界测试： trait 对象强制转换的 `Sized` 约束（编译错误）、边界测试：`as` 关键字的转换限制（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> **本小节性质**: 交叉语义补充，**只增不删**；原有 coercion/cast 矩阵（§一–§五）保持不变。迁移操作见 [`migration_197_decision_tree.md`](../../07_future/00_version_tracking/migration_197_decision_tree.md) §5–§6；Pin 侧语义见 [`06_pin_unpin.md`](../../03_advanced/01_async/08_pin_unpin.md) §Rust 1.97.0 交叉语义。
...
> - **没变**：`Box::pin` / `futures::pin_mut!` 的用法（它们不涉及该 coercion，见 [`06_pin_unpin.md`](../../03_advanced/01_async/08_pin_unpin.md) §三 模式 4/5）。
...
- **P2 生态/社区**: [docs.rs/cast — 生态权威 API 文档（人机工程学数值转换 crate）](https://docs.rs/cast)（2026-07-12 验证 HTTP 200）

> **过渡**: 理解 Deref 强制与 `as` 显式转换的区别，可避免在类型边界处混淆隐式与显式转换。
> **过渡**: 掌握类型强制规则后，可进一步学习 trait object 的隐式转换、方法解析中的自动解引用与引用模式。
> **过渡**: 将强制/转换与类型安全结合，可理解为什么 `transmute` 等危险转换需要 `unsafe` 且极少使用。
