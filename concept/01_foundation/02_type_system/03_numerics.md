...
> **前置概念**: [Type System](01_type_system.md) · [Ownership](../01_ownership_borrow_lifetime/01_ownership.md)
...
- **反命题 1：「release 模式溢出是 bug」** —— 不准确：release 下整数溢出是**定义良好的回绕**（two's complement wrapping），不是 UB——这与 C 的 signed overflow = UB 有本质区别。但它仍可能是逻辑错误，所以 debug 模式默认开启溢出检查 panic。生产代码若依赖回绕语义，必须显式写 `wrapping_add` 表明意图。
- **反命题 2：「`as` 转换是安全的类型转换」** —— 危险：`as` 是**静默截断**——`300u16 as u8 == 44`，`f64 as u8` 饱和（1.45 起定义语义）。判定准则：值域可能超界时用 `u8::try_from(x)` 返回 `Result`；确知不溢出才用 `as`。

边界极限小节量化：浮点转整数的饱和规则、`usize` 在 32/64 位平台的差异、`checked_*` 的性能成本（通常被分支预测吸收）。
...
- [Zero Cost Abstractions](../00_start/02_zero_cost_abstractions.md) — 零成本抽象（Zero-Cost Abstraction）
...
本节把「数值类型与运算：从整数到浮点的完整图景」的规则推到编译器与运行时的边界上逐一实测：边界测试：整数溢出在 Debug 与 Release 中的差异（运行时…、边界测试：浮点数相等比较（逻辑错误）、边界测试：`as` 转换的截断风险（编译错误）、边界测试：浮点数作为 `match` 条件（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
- **理解层**：溢出语义四族方法（`wrapping_`/`checked_`/`saturating_`/`overflowing_`）的选择——给定场景判断应返回 `Option`、饱和值还是回绕值；
- **应用层**：`as` vs `From` vs `TryFrom` 的选择——`From` 只存在无损方向（`u8 → u32`），有损必须 `TryFrom` 或显式 `as`；
- **分析层**：浮点陷阱——`0.1 + 0.2 != 0.3` 的 IEEE 754 根源、`f64` 排序为何需要 `total_cmp`、金融计算为何禁用二进制浮点。

作答建议：每题先用 `rustc` 实际运行验证直觉，数值行为是「看起来显然、实际出错」的高发区。
