...
理解「核心概念」需要把握整数类型全景、浮点类型与 IEEE 754与溢出行为与饱和运算，本节依次展开。
...
本节将「嵌入式测验」分解为若干主题：测验 1：整数溢出行为（记忆层）、测验 2：`as` 与 `TryFrom` 的区别（理解层）、测验 3：NonZero 类型优化（应用层）、测验 4：浮点数陷阱（分析层）等5个方面。
