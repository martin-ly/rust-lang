...
本节从类型转换与 as、Wrapping、Saturating、Checked、Overflo…、NonZero 类型与优化与SIMD 与向量化切入，剖析「技术细节」的核心内容。
...
「边界测试：数值类型的编译错误」涉及边界测试：整数溢出在 Debug 与 Release 中的差异（运行时…、边界测试：浮点数相等比较（逻辑错误）、边界测试：`as` 转换的截断风险（编译错误）、边界测试：浮点数作为 `match` 条件（编译错误）等10个方面，本节逐一说明其要点。
