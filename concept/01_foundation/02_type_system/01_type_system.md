...
> **📎 交叉引用（Reference）**
>
> 本主题在 knowledge 中有系统化的知识索引：[类型系统（Type System）](01_type_system.md)
>
> **受众**: [初学者]
...
> **前置概念**: [Ownership](../01_ownership_borrow_lifetime/01_ownership.md)
> **后置概念**:
>
> [Traits](../../02_intermediate/00_traits/01_traits.md) ·
> [Generics](../../02_intermediate/01_generics/01_generics.md) ·
> [Algebraic Data Types](../../02_intermediate/00_traits/01_traits.md)
>
> **主要来源**:
>
...
- [Type System Basics（类型系统基础）](#type-system-basics类型系统基础)
  ...
  - [十二、补充：运算符重载（Operator Overloading）](#十二补充运算符重载operator-overloading)
    ...
    - [12.7 权威来源](#127-权威来源)
  - [从 `crates\c02_type_system\docs\tier_02_guides\02_compound_types_guide.md` 迁移的补充视角](#从-cratesc02_type_systemdocstier_02_guides02_compound_types_guidemd-迁移的补充视角)
...
> **[Pierce "Types and Programming Languages"](https://www.cis.upenn.edu/~bcpierce/tapl/)** Hindley-Milner 类型推断（Type Inference）算法及其扩展是 Rust 类型系统的基础。✅
> **[COR: ETH Zurich](https://inf.ethz.ch/)** Γ; Σ ⊢ e : τ {Σ'} 的所有权（Ownership）约束形式化表示 Rust 的堆状态演化。✅
...
> 来源: [Rust Reference §3, §6; TRPL §3](https://doc.rust-lang.org/reference/introduction.html)
...
> 这种代数视角是理解 match 穷尽性、NPO 优化、以及 Rust 类型系统表达力的关键。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
以下链条从代数结构的完备性出发，一直延伸到运行时（Runtime）安全保证。
...
> **[Category Theory for Programmers](https://bartoszmilewski.com/2014/10/28/category-theory-for-programmers-the-preface/)** enum 对应余积（Coproduct / Sum Type），struct 对应积（Product Type）。✅
> **[Pierce "Types and Programming Languages"](https://www.cis.upenn.edu/~bcpierce/tapl/)** 积与余积的组合构成代数数据类型的完备基。✅
...
> **[Wright & Felleisen 1994](https://doi.org/10.1007/BF00108803)** 类型安全定理（Progress + Preservation）是类型系统的标准元定理。✅
...
| C3: Never 类型完备性 | L1 | 发散函数参与任意控制流 | 空类型 (⊥) | — | 不稳定特性需每日构建版 | — |

> **一致性（Coherence）检查**: L1 ⟹ L2 ⟹ T1/T2/T3 ⟹ C1/C2/C3，形成**从代数结构到运行时安全**的递进链。T1 是连接 ADT 结构与程序正确性的枢纽定理。
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/04_inter_layer_map.md) §4.2 "类型系统一致性"
...
> 这帮助读者建立 Rust 安全模型的精确理解：类型系统保证的是 UB 不可达，而非逻辑正确性。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
| 工程 | `std::mem::forget`、Rc 循环引用（Reference） | 内存泄漏，非 UB，但属安全边界 |
...
> 读者应建立「match = 强制穷尽，if let = 选择性处理」的精确区分。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> 这帮助读者理解「为什么有时编译器要求我写类型标注」——不是因为推断算法弱，而是因为**信息论上的不可能**（多解）或**工程上的需求**（文档契约）。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 泛型参数化使 ADT 具备多态表达能力
- [00_meta/inter_layer_map.md](../../00_meta/04_navigation/04_inter_layer_map.md) — 跨层定理映射 §4.2 "类型系统一致性"
...
> 关键认知：没有「最佳」选择，只有「最适合当前约束」的选择——impl Trait 是默认值（零成本 + 隐藏实现），dyn Trait 是灵活性需求时的备选（vtable 开销），泛型约束是调用方需要知道具体类型时的选择。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **来源**:
> [Rust Reference: Diverging functions](https://doc.rust-lang.org/reference/types/never.html) ·
> [Wikipedia: Bottom type](https://en.wikipedia.org/wiki/Bottom_type) ·
...
> 本节从 HM 核心规则出发，逐步扩展到 Rust 的 trait bounds、生命周期（Lifetimes）与关联类型，建立类型推断的完整形式化图景。
>
> **交叉链接**:
>
> [L1 生命周期: Elision 规则](../01_ownership_borrow_lifetime/03_lifetimes.md) ·
> [L2 泛型: 约束推导](../../02_intermediate/01_generics/01_generics.md) ·
> [L4 类型论: 系统 F](../../04_formal/00_type_theory/01_type_theory.md)
...
> **[Damas & Milner 1982, *Principal Type-Schemes for Functional Programs*](https://doi.org/10.1145/582153.582176)** HM 算法的四条规则构成完整的类型推导系统，支持 let-多态性（let-polymorphism）。✅
> **[Pierce, *Types and Programming Languages*, Ch.22](https://www.cis.upenn.edu/~bcpierce/tapl/)** HM 算法是 ML 家族语言类型推断的理论基础，Var/App/Abs/Let 规则对应 λ-演算的四类语法节点。✅
...
> **[Robinson 1965, *A Machine-Oriented Logic Based on the Resolution Principle*](https://doi.org/10.1145/321250.321253)** 统一算法是自动定理证明与类型推断的共同基础，Robinson 证明了 MGU 的存在唯一性（若存在）。✅
...
> **[rustc dev guide: Type inference](https://rustc-dev-guide.rust-lang.org/type-inference.html)** rustc 的类型推断器将 HM 统一与 trait solver 分离：先统一类型变量，再求解 trait 约束。✅
...
> **[Damas & Milner 1982](https://doi.org/10.1145/582153.582176)** let-多态性是 HM 算法的核心创新：它将 `let` 绑定的右侧类型泛化，使多态性在不增加显式标注的情况下可用。✅
...
> ——这不是推断算法的缺陷，而是「理论完备性」与「工程实用性」的权衡。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **[Peyton Jones et al., *Haskell 98 Report*](https://www.haskell.org/definition/haskell98-report.pdf)** Haskell 的类型推断通过类型类扩展 HM，允许数值字面量保持多态直到使用上下文确定具体类型。✅
...
> **[Kfoury et al., *Typability and Type Checking in System F*](https://doi.org/10.1145/75277.75285)** System F（含显式 ∀ 类型）的类型推断是不可判定的；Rust 的显式泛型参数将类型推断限制在 HM 可判定片段内，确保编译终止。✅
...
> **交叉链接**: [L4 类型论: ADT 代数语义](../../04_formal/00_type_theory/01_type_theory.md) · [L3 unsafe: 无效枚举值](../../03_advanced/02_unsafe/01_unsafe.md) · [00_meta/inter_layer_map.md](../../00_meta/04_navigation/04_inter_layer_map.md) §4.2 "类型系统一致性"
...
> **来源: [Rust Reference: std::mem::discriminant](https://doc.rust-lang.org/reference/introduction.html)** `Discriminant<T>` 的比较等价于判断两个值是否属于同一变体，不触及变体携带的数据。✅
...
> **[Unsafe Code Guidelines: Enum Layout](https://rust-lang.github.io/unsafe-code-guidelines/layout/enums.html)** Rust 无 `#[repr]` 的 enum 布局未指定（unspecified），编译器可自由重排字段、压缩 tag、合并 niche。✅
...
> **来源: [The Rustonomicon: Exotic Sizes](https://doc.rust-lang.org/nomicon/index.html)** NPO 是 Rust "零成本抽象（Zero-Cost Abstraction）" 的核心体现之一。✅
...
> **[Unsafe Code Guidelines: Values](https://rust-lang.github.io/unsafe-code-guidelines/glossary.html#validity-invariant)** 类型的 "invalid value" 集合是 niche optimization 的形式化基础。编译器在布局计算时枚举类型的有效值范围，寻找可用于编码 enum 变体的非法位模式。✅
...
> **来源: [Rust Reference: repr Attribute](https://doc.rust-lang.org/reference/introduction.html)** `repr(u8)` 等整型参数强制 enum 的 discriminant 类型为对应整数类型，布局变为 "tag + union" 的固定结构。✅
...
> **来源: [Rust Reference: DiscriminantKind](https://doc.rust-lang.org/reference/introduction.html)** `DiscriminantKind` 是编译器内建 trait，用于支持泛型代码中对 enum 变体类型的元编程。当前仍未稳定。✅
...
> [L2 泛型: Variance](../../02_intermediate/01_generics/01_generics.md) ·
> [L3 Unsafe: 裸指针与类型双关](../../03_advanced/02_unsafe/01_unsafe.md) ·
> [L4 形式化: 子类型关系](../../04_formal/00_type_theory/01_type_theory.md)
...
> **[Cardelli & Wegner 1985, *On Understanding Types, Data Abstraction, and Polymorphism*](https://doi.org/10.1145/6041.6042)** 名义类型与结构类型的区分是类型系统设计的首要决策之一，它深刻影响语言的模块（Module）化、演化能力与类型检查算法。✅
...
**关键洞察**：单元类型 `()` 和元组类型 `(T, U)` 既有名义包装（`struct Unit;`、`struct T(A, B)`）又有结构原型（`()`、`(A, B)`），但**记录结构体（Struct） `struct T { a: A }` 只有名义形式**。这种不对称性反映了 Rust 的设计哲学：
...
> 元组结构体虽然可以模拟结构类型（`(f64, f64)` 与 `Point(f64, f64)` 布局相同），但 `Point` 和 `Vector` 因名称不同而互不兼容，这正是名义类型的保护作用。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **来源: [Rust Reference: PhantomData](https://doc.rust-lang.org/reference/introduction.html)** `PhantomData<T>` 是零大小类型（ZST），用于影响编译期的类型检查行为（如 drop check、auto trait 推导、variance）。✅
...
> **来源: [The Rustonomicon: PhantomData](https://doc.rust-lang.org/nomicon/index.html)** PhantomData 是"名义类型系统的幽灵杠杆"——它不产生代码，但改变编译器的类型推导路径。✅
...
> **来源: [Rust Reference: Orphan Rules](https://doc.rust-lang.org/reference/introduction.html)** "An impl is valid only if either the trait or the type is local to the current crate." ✅
...
> **[Rust Internals — Blog: "Coherence in Rust"](https://internals.rust-lang.org/t/coherence-rules-for-generic-binary-operator-traits/4778)** 名义类型的全局唯一身份使得编译器可以在 crate 边界上做出「是否存在重叠 impl」的确定性判断。✅ 二级来源
...
> **[arXiv:2412.15042](https://arxiv.org/abs/2412.15042)** C 的 `const Point*` 是「调用时决定的只读视图」，Rust 的 `&Point` vs `&mut Point` 是「类型声明时固定的所有权（Ownership）状态」。这种差异导致 C→Rust 自动化翻译必须做「污染性选择」——要么保守地全部标记为 mutable，要么激进地拆分类型，两者都有代价。✅
...
> **认知功能**: 此命题的 falsification 揭示了 Rust 类型二元性的本质。关键认知：**名义类型并非无处不在**。在 tuple、array、function pointer 和 lifetime 领域，结构等价仍然发挥作用。程序员不能简单地认为"只要名字不同就一定安全"——必须区分当前操作的是名义构造还是结构构造。 [💡 原创分析](../../00_meta/00_framework/methodology.md)
...
> **相关链接**：完整分析见 §11.1；生命周期与 `impl Trait` 的交互见 [30_lifetimes_advanced.md](../01_ownership_borrow_lifetime/04_lifetimes_advanced.md) §14。
...
> **相关链接**：Never type 权威页见 [02_never_type.md](02_never_type.md)；与 coercion 的交互见 [04_coercion_and_casting.md](04_coercion_and_casting.md)。
...
> **相关链接**：Const Generics 的完整规则见 §11.3；泛型基础见 [02_generics.md](../../02_intermediate/01_generics/01_generics.md)。
...
> **相关链接**：完整分析见 §11.5；coercion 与类型转换见 [04_coercion_and_casting.md](04_coercion_and_casting.md)。
...
> **相关链接**：完整分析见 §11.6；高级 `union` 安全边界见 [30_lifetimes_advanced.md](../01_ownership_borrow_lifetime/04_lifetimes_advanced.md) §16。
...
> **相关链接**：完整分析见 §11.7；数据抽象谱系见 [05_data_abstraction_spectrum.md](05_data_abstraction_spectrum.md)。
...
本节边界测试按「类型系统拒绝非法程序的六类典型场景」组织，每条测试都是 rustc 1.97.0 可复现的编译错误，并标注对应诊断码：

- **类型不匹配**（E0308）：期望类型与实际类型不符，包括隐式转换不存在的情况——Rust 不做 C/C++ 式的隐式数值提升，所有转换必须经 `as`、`From`/`Into` 或 deref coercion 显式发生。
- **泛型约束不满足**（E0277）：类型未实现约束中要求的 trait。诊断会列出缺失的 trait impl，修复方向通常是补 impl、收紧调用方约束或改用 `dyn Trait`。
- **match 非穷尽**（E0004）：穷尽性检查（exhaustiveness checking）是类型系统的直接应用——编译器用类型的全部变体集合校验分支覆盖。
- **impl Trait 位置差异**：参数位置等价于匿名泛型（调用方选类型），返回位置是存在类型（实现方选类型）；两者混用会触发类型不透明性相关的错误。

读法：每条边界测试回答「类型系统在这一格上为什么说不」，合起来构成类型系统拒绝面的完整轮廓。
...
> **修正**: `impl Trait` 在参数位置等价于匿名泛型参数，每个 `impl Trait` 参数独立。这意味着两个 `impl Iterator` 参数是**不同类型**，不能假设它们相同。返回位置的 `impl Trait` 则表示"某个实现该 trait 的具体类型"。[来源: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html)]
...
本组测验围绕测验 1：结构体与元组结构体（理解层）、测验 2：枚举与模式匹配穷尽性（应用层）、测验 3：Option 与 unwrap（分析层）、测验 4：类型推断边界（应用层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用（Borrowing）检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
...
> **适用对象**: 初学者 → 中级开发者
> **前置知识**: [2.1 基础类型指南](../../../crates/c02_type_system/docs/tier_02_guides/01_basic_types_guide.md)
> **预计学习时间**: 5-7 小时
> **最后更新**: 2025-12-11
...
- [2.2 Rust 类型系统 - 复合类型指南](#22-rust-类型系统---复合类型指南)
  ...
  - [2. 结构体 (Struct)](#2-结构体-struct)
    - [2.1 命名字段结构体](#21-命名字段结构体)
      ...
      - 字段初始化简写
      - 结构体更新语法
    - [2.2 元组结构体](#21-命名字段结构体)
      - 基本用法
      - 实际应用
    - [2.3 单元结构体](#21-命名字段结构体)
      - [基本概念](#1151-discriminant-的基本概念与-stdmemdiscriminant)
      - 实际应用2
    - [2.4 结构体方法](#21-命名字段结构体)
      - 定义方法
    - 2.5 关联函数
      - 定义关联函数
    - 2.6 结构体的可见性
...
- ✅ **理解** 枚举和模式匹配
- ✅ **掌握** 结构体和枚举的方法定义
- ✅ **应用** Option 和 Result 进行错误处理（Error Handling）
- ✅ **理解** 复合类型的内存布局
- ✅ **设计** 合理的数据结构
- ✅ **编写** 健壮的 Rust 程序
...
本节聚焦「复合类型概述」，覆盖什么是复合类型 与  Rust 复合类型分类。论述顺序由定义到边界：先明确「复合类型概述」在「Type System Basics（类型系统基础）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「复合类型概述」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「命名字段结构体」，覆盖定义和创建。论述顺序由定义到边界：先明确「命名字段结构体」在「Type System Basics（类型系统基础）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「命名字段结构体」的判定标准，并指出它在全页论证链中的位置。
...
- **P2 生态/社区**: [docs.rs/typenum — 生态权威 API 文档（类型级编程与类型系统实践）](https://docs.rs/typenum)（2026-07-12 验证 HTTP 200）
