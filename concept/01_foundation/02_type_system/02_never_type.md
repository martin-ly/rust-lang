...
- **强制规则（coercion）**：`!` 可以隐式强制到任意类型 `T`。直观理由是「一个永远不会产生的值，可以承诺是任何类型而不被拆穿」。这正是 `let x: i32 = loop {};` 能编译、以及 `match` 中发散臂不破坏类型一致性（Coherence）的形式依据。
...
2. **`Result<T, !>`**：表示「不可能失败」的计算。`Result<T, !>` 与 `T` 在类型论上同构（可通过 `unwrap` 的特例 `match r { Ok(t) => t }` 无风险解包），用于占位未来可能引入错误的 API，或与要求 `Result` 的泛型（Generics）接口适配。
...
2. **流处理中的不可能错误**：`Stream<Item = Result<T, !>>` 声明「此流永不失败」——`?` 传播时 `Err` 分支被类型系统证明不存在，错误处理（Error Handling）代码可安全省略；
...
- **`Some(!)` 不可构造**：`Option<!>` 只有 `None` 一个可构造值——模式匹配（Pattern Matching）自动退化为单分支；
...
> **过渡**: 掌握 `!` 作为发散表达式类型后，可理解 `panic!`/`loop {}`/`return` 为何能出现在任何类型位置。
> **过渡**: 从 `!` 的不可实例化性出发，可进一步学习空枚举 `enum Void {}`、never type 稳定化跟踪与穷尽性证明。
> **过渡**: 将 `!` 与 `()` 对偶比较，可深化对「单位类型 vs 底类型」在类型格中位置的理解。

---

## 反命题与边界

> **反命题**: "`!` 类型可以像 `()` 一样被构造和赋值。" —— 错误。`!` 没有值，不能实例化；任何尝试构造 `!` 值的代码都是不可达的或发散的。
> **边界**: 当前 `!` 在返回类型位置稳定，但作为泛型参数、局部变量类型等完整 never type 支持仍需 nightly `never_type` feature。
