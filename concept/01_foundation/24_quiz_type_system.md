...
> **Bloom 层级**: 理解 → 应用
> **定位**: 嵌入式互动测验扩展——验证 L1 类型系统核心概念（标量/复合类型、enum、模式匹配、类型转换）的掌握程度。
...
| 类型 | 存储位置 | 大小 | 是否可变 | 说明 |
|:---|:---|:---:|:---:|:---|
| `&str` | 指向字符串字面量或 `String` 的切片 | 固定（指针+长度） | ❌ | 字符串切片，不可变引用 |
...
**知识点**：`&str` 是"借用"，`String` 是"拥有"。Rust 字符串设计的核心是明确所有权边界。[→ 字符串详解](./09_strings_and_text.md)
...
- `#[derive(Debug)]` 为结构体**自动实现 `Debug` trait**
- `{:?}` 使用 `Debug` 格式化（紧凑单行）
...
**题目**: 在 测验：类型系统（试点扩展） 的测验中，若遇到不确定答案的题目，最佳的学习策略是什么？
