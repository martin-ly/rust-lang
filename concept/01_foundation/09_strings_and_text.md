...
- [字符串与文本：Rust 的 Unicode 处理与格式化系统](#字符串与文本rust-的-unicode-处理与格式化系统)
  ...
  - [二、技术细节](#二技术细节)
    - [2.1 字符串切片（String Slice）与索引](#21-字符串切片与索引)
  ...
  - [十二、边界测试：字符串与文本的编译错误](#十二边界测试字符串与文本的编译错误)
    - [12.1 边界测试：`String` 与 `&str` 的生命周期（Lifetimes）不匹配（编译错误）](#121-边界测试string-与-str-的生命周期不匹配编译错误)
    ...
    - [10.6 边界测试：`String::from_utf8` 的无效序列与损失转换（运行时（Runtime） panic）](#106-边界测试stringfrom_utf8-的无效序列与损失转换运行时-panic)
  - [实践](#实践)
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
    - [反命题与边界](#反命题与边界)
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    ...
    - [测验 4：format! 宏（Macro）（应用层）](#测验-4format-宏应用层)
    - [测验 5：字符串切片（Slice）边界（分析层）](#测验-5字符串切片边界分析层)
