...
>
> **本节关键术语**:
> 引用 (Reference) · 借用 (Borrowing) · 解引用 (Dereferencing) · 自动解引用 (Auto Deref) · 强制转换 (Coercion) — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
> **后置概念**: [Smart Pointers](../../02_intermediate/02_memory_management/01_memory_management.md) · [Generics](../../02_intermediate/01_generics/01_generics.md)
...
- 引用（Reference）语义：自动解引用、Deref 强制与类型转换
  ...
  - [七、多级引用（Reference）语义与部分重借用（Multi-level References \& Partial Reborrows）](#七多级引用语义与部分重借用multi-level-references--partial-reborrows)
    ...
    - [7.6 代码示例集](#76-代码示例集)
      - 7.6.1 嵌套引用（Reference）的构造与模式匹配（Pattern Matching）
    ...
    - [7.8 常见困惑解析](#78-常见困惑解析)
      - [困惑 1: `let r: &&i32 = &&5;` —— 中间引用（Reference）的生命周期（Lifetimes）](#困惑-1-let-r-i32--5--中间引用的生命周期)
...
引用（Reference）在 Rust 中同时承担三种角色，理解引用语义首先要区分这三重含义：

1. **类型构造子**：`&T` 与 `&mut T` 是类型，读作「对 `T` 的共享引用 / 可变引用（Mutable Reference）」。它们是指针的受检版本——永远非空、永远对齐、永远指向有效值（编译器保证）。
2. **运算符**：`&expr` 取地址创建引用，`*expr` 解引用。与 C 不同，Rust 中显式 `*` 极少需要——自动解引用（auto-deref）在方法调用与字段访问时按需插入。
3. **模式**：`let &x = r;` 中的 `&` 是模式匹配（Pattern Matching）的解构符，方向与运算符相反：它「剥掉」一层引用。

Deref 强制（deref coercion）是贯穿三者的粘合规则：当期望类型为 `&U` 而实参为 `&T` 且 `T: Deref<Target = U>` 时，编译器自动插入 `*`，这就是 `&String` 能传给 `&str` 参数的原因。判定一次 deref coercion 是否发生，只需比较「实参类型」与「形参期望类型」是否相差恰好若干层 `Deref` 链。
...
引用语义的技术细节集中在三个机制上，它们共同决定「一个引用表达式最终指向什么、能做什么」：

- **方法调用的自动引用**：`x.method()` 中，编译器按「值 → `&x` → `&mut x` → 逐层 deref 后重复」的顺序查找方法（method resolution）。这解释了为什么 `String` 能调用 `str` 的方法，以及为什么有时需要显式 `(*x).method()` 来绕过 deref 到 `Deref::Target` 上同名方法的遮蔽。
- **类型强制规则**：deref coercion（`&T → &U` when `T: Deref<Target=U>`）、`&mut T → &T` 的权限弱化、以及 unsized coercion（`&[T; N] → &[T]`、`&T → &dyn Trait`）。三类强制都只发生在「期望类型已知」的强制点（coercion site）：函数实参、`let` 标注、返回值等。
- **与借用（Borrowing）检查的交互**：每次创建引用即开始一次借用，借用存活区间由 NLL 按「最后一次使用点」推导；可变引用（Mutable Reference）经过的每次 reborrow 会临时冻结原引用。引用表达式的合法性最终都归约为借用检查器对该区间的判定。
...
本节的反命题树用于破除三个高频误解，每条给出「命题 → 判定依据 → 反例」的完整结构：

- **反命题 1：「`&T` 一定指向只读数据」**。判定依据：`&T` 保证的是「通过该引用不能修改」，而非「数据不会变化」——`Cell<T>` / `RefCell<T>` 等内部可变性（Interior Mutability）类型允许经 `&T` 修改内部状态。反例：`Cell::set(&self, ...)` 签名只要求 `&self`。
- **反命题 2：「多级引用与一级引用完全等价」**。判定依据：`&&T` 自动解引用到 `T` 是因为 `&T: Deref<Target=T>`，但 `&mut &mut T` 不能自动解引用两层（`&mut T` 的 `Deref` 只剥一层且涉及可变性约束）；此外 `&&T` 本身是独立的值，有其自身的地址与生命周期（Lifetimes）。
- **反命题 3：「引用的生命周期（Lifetimes）可以无限延长」**。判定依据：生命周期上界是被引用值的作用域，下界是引用的最后一次使用；`'static` 只来自常量/静态项与泄漏（`Box::leak`），普通栈值的引用永远无法提升到 `'static`。

边界极限部分则给出这些命题失效的临界代码形态，每条均可用 rustc 1.97.0 直接复现。
...
- [Type System](../02_type_system/01_type_system.md) — Rust 类型系统（Type System）
- [Memory Management](../../02_intermediate/02_memory_management/01_memory_management.md) — 内存管理与智能指针（Smart Pointer）
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 泛型（Generics）与参数多态
...
> **定位**: 深入 Rust 的**多级引用**（`&&T`、`&mut &T`）与**部分重借用（Borrowing）**（partial reborrow）机制，澄清嵌套引用在借用检查器中的行为、编译器对结构体（Struct）字段级粒度的跟踪，以及 Tree Borrows 模型下的权限语义。
...
> **后置概念**: [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) · [Formal Methods](../../04_formal/01_ownership_logic/01_linear_logic.md)
...
多级引用（`&&T`、`&mut &mut T` 等）在 Rust 中不是边缘语法，而是泛型（Generics）代码（迭代器（Iterator）、闭包（Closures）捕获、智能指针（Smart Pointer）解引用链）中每天都会遇到的类型形态。本节回答三个判定问题：

1. **嵌套是否无限可解**：`&T` 链上 `Deref` 自动解引用可递归到任意深度（`&&&&T` → `T` 的字段访问与方法调用自动穿透）；但 `&mut` 链只在**唯一一层**自动解引用——`&mut &mut T` 可自动转为 `&mut T`，更深则需显式 `**`。
2. **可变引用的弱化方向不可逆**：`&mut T` 可重借为 `&T`（可变 ⟹ 共享），但 `&&mut T` 永远无法变成 `&mut &mut T`——共享引用的不可变性是类型级承诺，穿透它即 UB 领域。
3. **何时该扁平化**：超过两级的引用通常是设计信号——应引入中间绑定、用 `Box`/`Rc` 固定层级，或重构泛型（Generics）约束（`T: Deref<Target = U>`）。

后续小节逐层展开共享嵌套、可变嵌套与弱化不可逆性的类型规则。
...
本节聚焦「代码示例集」，覆盖嵌套引用的构造与模式匹配（Pattern Matching）、结构体（Struct）字段的部分重借用、`split_mut` 创建不相交可变引用与迭代器（Iterator）可变链。论述顺序由定义到边界：先明确「代码示例集」在「引用语义：自动解引用、Deref 强制与类型转换」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「代码示例集」的判定标准，并指出它在全页论证链中的位置。
...
本节收集三个在学习引用语义时反复出现、且官方文档未集中解答的困惑，每个给出「错误直觉 → 正确模型 → 验证代码」的完整链路：

- **困惑 1（中间引用的生命周期）**：`let r: &&i32 = &&5;` 能编译是因为临时值提升（temporary promotion）把内层 `&5` 提升为 `'static` 常量；换成非 const 表达式（如 `&&x.clone()`）则立即触发 E0716。判定准则：只有 const-evaluable 的临时值才享提升。
- **困惑 2（`&mut &T` vs `&&mut T`）**：前者是「可变地持有一个只读引用」——可换绑引用目标，不能改指向的值；后者是「只读地持有一个可变引用」——可通过 `**r` 改值，但不能换绑。记忆口诀：**最外层决定你对引用本身的权限，最内层决定你对数据的权限**。
- **困惑 3（自动解引用的不对称）**：`&&&&T` 自动解引用到 `T` 依赖 `Deref` 的递归应用，而 `&mut &mut T` → `&mut T` 的自动化只发生一次，因为编译器对 `DerefMut` 的自动应用受「不能产生多个可变别名」约束。

三个困惑的共同根源：自动解引用是**编译器按接收上下文驱动的类型适配**，不是无条件的隐式转换。
...
> [来源: [Rust Reference: Types] · [Rust Reference: Subtyping](https://doc.rust-lang.org/reference/subtyping.html) · [Rust Reference: Type Coercions](https://doc.rust-lang.org/reference/type-coercions.html)]（一级来源）
> **与类型系统（Type System）的关联**: 详见 [`04_type_system.md`](../02_type_system/01_type_system.md) 对名义类型与结构类型的完整分析——其中第 11.7 节专门论证了引用构造的结构本质与目标类型名义约束的交互关系。
...
> **权威来源对齐变更日志**: 2026-05-22 新增多级引用语义、部分重借用、Tree Borrows 模型、as_ref/as_mut 嵌套引用转换 [Authority Source Sprint Batch 9](../../00_meta/02_sources/05_international_authority_index.md)
...
本节的边界用例全部围绕「引用的权限与生命周期不变量」设计，每段代码标注它是编译期拒绝还是运行期 UB：

| 测试 | 违反的规则 | 后果 |
|:---|:---|:---|
| 多级解引用层级错误 | `Deref` 自动应用次数 ≠ 手写 `*` 次数 | 编译错误（E0614 解引用不匹配） |
| `&str` 与 `String` 混用 | 无隐式 `String` → `&str` 之外方向的转换 | 编译错误（E0308） |
| `&mut` 重借后使用原始引用 | 重借期间原引用冻结（NLL 活跃区重叠） | 编译错误（E0506/E0502） |
| 用内部可变性外的手段突破 `&T` | `&T` 是「无数据竞争下的只读」承诺 | UB（`unsafe` 转 `&mut`） |
| 返回局部变量引用 | 引用存活 ⊆ 值存活 | 编译错误（E0106/E0515） |

判定方法：先判断用例属于「编译期拒绝」还是「编译通过但 UB」——后者正是 Miri 的用武之地，也是 `unsafe` 审计的重点。
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| 自动解引用 | 方法调用按接收者类型自动加 `&`/`&mut`/解引用直至匹配 | 本文 §1.2、§2.1 |
| Deref 强制 | `&T → &U`（`T: Deref<Target = U>`），单向且可链式 | 本文 §1.3 |
| 多级引用 | `&&T`、`&mut &mut T` 等多级引用按规则展开 | 本文 §7.1 |
| 部分重借用 | 可变引用可按字段拆分重借用，互不冲突 | 本文 §7.2 |
| 借用模型 | Stacked Borrows → Tree Borrows 演进 | 本文 §7.4 |

## 🔗 概念关系

- **上位（is-a）**：[借用](../01_ownership_borrow_lifetime/02_borrowing.md) 的语义细化层。
- **下位（实例）**：自动解引用、Deref 强制、多级引用、部分重借用、Tree Borrows。
- **对偶**：值语义 ⇄ 引用语义，见 [值 vs 引用语义](02_value_vs_reference_semantics.md)。
- **组合**：与 [类型强制与转换](../02_type_system/04_coercion_and_casting.md) 共享 Deref 强制机制。
- **依赖**：依赖 [生命周期](../01_ownership_borrow_lifetime/03_lifetimes.md) 的有效性规则。
...
> **过渡**: 理解自动解引用后，可进一步学习 Deref 强制在方法调用、函数参数匹配中的具体规则，避免过度显式加 `&` 或 `*`。
> **过渡**: 掌握引用语义后，可进入多级引用、部分重借用与 Tree Borrows 等进阶模型，理解编译器对引用的精细跟踪。
> **过渡**: 将引用语义与值语义对比，可解释为什么 Rust 默认 move 而非拷贝，以及 `Copy` trait 如何改变这一行为。
