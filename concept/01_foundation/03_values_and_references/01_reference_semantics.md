...
- **与借用（Borrowing）检查的交互**：每次创建引用即开始一次借用，借用存活区间由 NLL 按「最后一次使用点」推导；可变引用经过的每次 reborrow 会临时冻结原引用。引用表达式的合法性最终都归约为借用检查器对该区间的判定。
...
- **反命题 3：「引用的生命周期可以无限延长」**。判定依据：生命周期上界是被引用值的作用域，下界是引用的最后一次使用；`'static` 只来自常量/静态项与泄漏（`Box::leak`），普通栈值的引用永远无法提升到 `'static`。
...
3. **何时该扁平化**：超过两级的引用通常是设计信号——应引入中间绑定、用 `Box`/`Rc` 固定层级，或重构泛型约束（`T: Deref<Target = U>`）。
...
本节聚焦「代码示例集」，覆盖嵌套引用的构造与模式匹配（Pattern Matching）、结构体（Struct）字段的部分重借用、`split_mut` 创建不相交可变引用与迭代器可变链。论述顺序由定义到边界：先明确「代码示例集」在「引用语义：自动解引用、Deref 强制与类型转换」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「代码示例集」的判定标准，并指出它在全页论证链中的位置。
