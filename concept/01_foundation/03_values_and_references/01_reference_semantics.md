...
1. **类型构造子**：`&T` 与 `&mut T` 是类型，读作「对 `T` 的共享引用 / 可变引用」。它们是指针的受检版本——永远非空、永远对齐、永远指向有效值（编译器保证）。
...
- **与借用检查的交互**：每次创建引用即开始一次借用，借用存活区间由 NLL 按「最后一次使用点」推导；可变引用经过的每次 reborrow 会临时冻结原引用。引用表达式的合法性最终都归约为借用检查器对该区间的判定。
...
- **反命题 2：「多级引用与一级引用完全等价」**。判定依据：`&&T` 自动解引用到 `T` 是因为 `&T: Deref<Target=T>`，但 `&mut &mut T` 不能自动解引用两层（`&mut T` 的 `Deref` 只剥一层且涉及可变性约束）；此外 `&&T` 本身是独立的值，有其自身的地址与生命周期。
...
多级引用（`&&T`、`&mut &mut T` 等）在 Rust 中不是边缘语法，而是泛型代码（迭代器、闭包捕获、智能指针解引用链）中每天都会遇到的类型形态。本节回答三个判定问题：
...
本节聚焦「代码示例集」，覆盖嵌套引用的构造与模式匹配、结构体字段的部分重借用、`split_mut` 创建不相交可变引用与迭代器可变链。论述顺序由定义到边界：先明确「代码示例集」在「引用语义：自动解引用、Deref 强制与类型转换」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「代码示例集」的判定标准，并指出它在全页论证链中的位置。
