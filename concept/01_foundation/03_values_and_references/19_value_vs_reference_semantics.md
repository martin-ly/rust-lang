...
2. **概念建立**: 掌握 值语义 vs 引用（Reference）语义 的核心定义、关键术语与类型系统（Type System）/运行时（Runtime）边界。
3. **机制推理**: 通过 ⟹ 定理链将语法规则、编译期检查与运行时（Runtime）语义串联起来。
4. **边界辨析**: 借助反命题/反例理解常见错误与值语义 vs 引用（Reference）语义的适用边界。
5. **迁移应用**: 将 值语义 vs 引用（Reference）语义 与前置/后置概念链接，形成跨层知识网络。
...
> **关键洞察**：Rust 的独特之处在于，它把"值语义"从"复制数据"推进到"转移所有权"，从而在不使用垃圾回收的情况下，同时获得值语义的可预测性和引用语义的共享能力（通过显式借用（Borrowing））。
