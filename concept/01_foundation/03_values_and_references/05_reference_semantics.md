...
> **后置概念**: [Smart Pointers](../../02_intermediate/02_memory_management/03_memory_management.md) · [Generics](../../02_intermediate/01_generics/02_generics.md)
...
- 引用（Reference）语义：自动解引用、Deref 强制与类型转换
  ...
  - [七、多级引用（Reference）语义与部分重借用（Multi-level References \& Partial Reborrows）](#七多级引用语义与部分重借用multi-level-references--partial-reborrows)
    ...
    - [7.2 部分重借用（Partial Reborrows）](#72-部分重借用partial-reborrows)
      ...
      - [7.2.2 部分重借用（Borrowing）的类型学限制](#722-部分重借用的类型学限制)
    ...
    - [7.5 `as_ref()` / `as_mut()` 与嵌套引用（Reference）](#75-as_ref--as_mut-与嵌套引用)
      - [7.5.1 嵌套引用（Reference）的类型转换](#751-嵌套引用的类型转换)
...
「核心概念」涉及引用的多重含义、自动解引用机制与Deref 强制，本节逐一说明其要点。
...
本节将「技术细节」分解为若干主题：方法调用的自动引用、类型强制规则与与借用检查的交互。
...
- [Type System](../02_type_system/04_type_system.md) — Rust 类型系统（Type System）
- [Memory Management](../../02_intermediate/02_memory_management/03_memory_management.md) — 内存管理与智能指针（Smart Pointer）
- [Generics](../../02_intermediate/01_generics/02_generics.md) — 泛型（Generics）与参数多态
...
「多级引用类型」部分按共享引用的嵌套：`&T` → `&&T` → `&&&T`、可变引用的嵌套：`&mut T`、`&mut &T`、`&mut &m…与弱化的不可逆性的顺序逐层展开。

#### 7.1.1 共享引用的嵌套：`&T` → `&&T` → `&&&T`

在 Rust 中，引用本身也是一等类型，可以被再次引用。多级共享引用 `&&T` 并非 C 中的"指针的指针"的简单类比——每一级引用都携带独立的生命周期（Lifetimes），并受借用检查器的完整约束。
...
可变引用（Mutable Reference）的嵌套引入了更复杂的语义层次：
...
Rust 借用检查器对复合类型（结构体（Struct）、元组、数组）的跟踪粒度可以达到**字段级别**。这意味着从 `&mut Parent` 中部分重借用一个字段，不会导致整个父结构被锁定。
...
Rust Internals 社区（quinedot, kpreid, 2023）深入讨论了部分重借用的一个核心限制：**当前 Rust 类型系统（Type System）无法表达"对父结构体（Struct）的可变引用（Mutable Reference），排除某个子字段"的签名**。
...
一种常见的工作模式是使用**临时作用域**来缩小借用的生命周期（Lifetimes）：
...
本节围绕「代码示例集」展开，依次讨论嵌套引用的构造与模式匹配、结构体字段的部分重借用、`split_mut` 创建不相交可变引用与迭代器可变链。
...
理解「常见困惑解析」需要把握困惑 1: `let r: &&i32 = &&5;` —— 中间引用…、困惑 2: `&mut &T` vs `&&mut T`与困惑 3: 为什么 `&&&&T` 自动解引用但 `&mut &mut…，本节依次展开。
...
| 目标类型 | 重借用粒度 | 类型系统（Type System）规则 |
|:---|:---|:---|
...
> **定理 R1（引用层级的规则分离）**：在 Rust 中，引用操作的**可解引用性、弱化、生命周期（Lifetimes）子类型化**由结构规则支配；而引用操作的**类型等价性、可互换性**由目标类型的名义/结构性质支配。两者在多级引用中并行生效，互不抵消。
...
本节将「边界测试：引用语义的编译错误」分解为若干主题：边界测试：多级引用自动解引用层级（编译错误）、边界测试：`&str` 与 `String` 的混用（编译错误）、边界测试：`&mut` 的重新借用与原始引用失效（编译错误）、边界测试：内部可变性与 `&T` 的不可变性矛盾（编译错误/运行时 U…等6个方面。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：`Deref` 强制转换（Deref Coercion）允许…、测验 2：`&mut T` 是否自动实现 `Deref` 到 `&T`…、测验 3：为什么 `Box<T>` 可以像 `&T` 一样被解引用使用…、测验 4：`DerefMut` 与 `Deref` 的关系是什么？什么…等5个方面。
...
| 引用语义：自动解引用、Deref 强制与类型转换 正确用法 ⟹ 常见陷阱 | 忽略边界条件 | 编译错误或运行时（Runtime） bug | 高 |
