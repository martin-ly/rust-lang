...
- **Rust 特有的三分**：值表达式产生的临时值有**临时生命周期**（语句结束即 drop，`let x = &temp();` 中延长到 `x` 的作用域——temporary lifetime extension）；
...
- **遮蔽 vs 可变**：`let x = x + 1;`（新绑定）与 `x = x + 1;`（存储更新）语义不同——前者旧 `x` 仍存在（只是不可见），可被闭包捕获；混淆二者是「闭包捕获了意外的值」类 bug 的根源；
- **临时值生命周期**：`let r = &format!("{}", x);` 能编译（生命周期延长），但 `match &format!(...) { ... }` 中临时值活到 match 结束——延长规则的边界位置（`let` 直接绑定 vs 嵌套表达式）需要精确记忆；
- **move 后的部分可用**：结构体字段级 move（`let a = s.f1;` 后 `s.f2` 仍可用）——所有权跟踪是字段粒度的，但 `Drop` 类型的部分 move 被禁止（E0382）；
- **`Copy` 的位复制语义**：`Copy` 类型的「复制」是按位拷贝且源仍有效——实现 `Drop` 的类型不能 `Copy`（E0184），这是所有权模型的一致性约束。
...
本节把「变量模型：从通用 PL 视角看 Rust 的所有权」的规则推到编译器与运行时的边界上逐一实测：边界测试：`let` 与 `const` 的初始化差异（编译错误）、边界测试：`const` 与 `static` 的 `Drop` 限制…、边界测试：变量遮蔽（shadowing）与不可变引用的冲突（编译错误）、边界测试：未初始化变量的强制使用（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
