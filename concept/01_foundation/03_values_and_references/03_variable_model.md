...
本节从环境模型（Environment Model）、存储模型（Store Model）与Rust 的所有权：存储模型的线性扩展切入，剖析「通用 PL 视角：变量的两种经典模型」的核心内容。
...
「值类别（Value Categories）：从 C++ 到…」部分包含 C++ 的值类别体系 与  Rust 的表达式分类 两条主线，本节依次说明。
...
本节从通用 PL 谱系 与  Rust 的极端值语义 两个层面剖析「值语义 vs 引用语义」。
