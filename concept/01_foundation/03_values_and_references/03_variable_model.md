...
- [变量模型：从通用 PL 视角看 Rust 的所有权](#变量模型从通用-pl-视角看-rust-的所有权)
  - [📑 目录](#-目录)
  - [🧠 知识结构图](#-知识结构图)
  - [一、核心命题](#一核心命题)
  - [二、通用 PL 视角：变量的两种经典模型](#二通用-pl-视角变量的两种经典模型)
    - [2.1 环境模型（Environment Model）](#21-环境模型environment-model)
    - [2.2 存储模型（Store Model）](#22-存储模型store-model)
    - [2.3 Rust 的所有权：存储模型的线性扩展](#23-rust-的所有权存储模型的线性扩展)
  - [三、值类别（Value Categories）：从 C++ 到 Rust](#三值类别value-categories从-c-到-rust)
    - [3.1 C++ 的值类别体系](#31-c-的值类别体系)
    - [3.2 Rust 的表达式分类](#32-rust-的表达式分类)
  - [四、值语义 vs 引用语义](#四值语义-vs-引用语义)
    - [4.1 通用 PL 谱系](#41-通用-pl-谱系)
    - [4.2 Rust 的极端值语义](#42-rust-的极端值语义)
  - [五、环境（Environment）与存储（Store）的形式化](#五环境environment与存储store的形式化)
    - [5.1 操作语义中的环境-存储模型](#51-操作语义中的环境-存储模型)
    - [5.2 Rust 的所有权作为环境-存储约束](#52-rust-的所有权作为环境-存储约束)
  - [六、L-value 与 R-value 的 Rust 诠释](#六l-value-与-r-value-的-rust-诠释)
    - [6.1 C 语言的 L/R-value 起源](#61-c-语言的-lr-value-起源)
    - [6.2 Rust 的 place expression 与 value expression](#62-rust-的-place-expression-与-value-expression)
  - [七、反例与边界测试](#七反例与边界测试)
    - [7.1 反例：混淆绑定与赋值](#71-反例混淆绑定与赋值)
    - [7.2 边界测试：Move 后的环境-存储状态](#72-边界测试move-后的环境-存储状态)
    - [7.3 边界测试：引用与指针的语义差异](#73-边界测试引用与指针的语义差异)
  - [八、跨语言变量模型对比矩阵](#八跨语言变量模型对比矩阵)
  - [九、知识来源关系](#九知识来源关系)
  - [十、边界测试：变量模型的编译错误](#十边界测试变量模型的编译错误)
    - [10.1 边界测试：`let` 与 `const` 的初始化差异（编译错误）](#101-边界测试let-与-const-的初始化差异编译错误)
    - [10.2 边界测试：`const` 与 `static` 的 `Drop` 限制（编译错误）](#102-边界测试const-与-static-的-drop-限制编译错误)
    - [10.3 边界测试：变量遮蔽（shadowing）与不可变引用的冲突（编译错误）](#103-边界测试变量遮蔽shadowing与不可变引用的冲突编译错误)
    - [10.4 边界测试：未初始化变量的强制使用（编译错误）](#104-边界测试未初始化变量的强制使用编译错误)
    - [10.5 边界测试：变量遮蔽与不可变重新绑定（编译错误）](#105-边界测试变量遮蔽与不可变重新绑定编译错误)
    - [10.6 边界测试：`Drop` 的顺序与变量声明顺序的依赖（逻辑错误）](#106-边界测试drop-的顺序与变量声明顺序的依赖逻辑错误)
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    - [测验 1：在 Rust 中，`let x = 5;` 之后 `x` 绑定到值 `5`。从变量模型角度看，`x` 是值的名称还是内存位置的名称？（理解层）](#测验-1在-rust-中let-x--5-之后-x-绑定到值-5从变量模型角度看x-是值的名称还是内存位置的名称理解层)
    - [测验 2：`let mut x = 5; x = 6;` 中，`x = 6` 是修改了原有内存，还是重新绑定了一个新值？（理解层）](#测验-2let-mut-x--5-x--6-中x--6-是修改了原有内存还是重新绑定了一个新值理解层)
    - [测验 3：Rust 的变量绑定在离开作用域时会发生什么？谁负责释放资源？（理解层）](#测验-3rust-的变量绑定在离开作用域时会发生什么谁负责释放资源理解层)
    - [测验 4：`Copy` trait 对变量模型有什么影响？实现 `Copy` 的类型在赋值时行为有何不同？（理解层）](#测验-4copy-trait-对变量模型有什么影响实现-copy-的类型在赋值时行为有何不同理解层)
    - [测验 5：为什么 Rust 没有 C/C++ 意义上的"未初始化变量"读取？（理解层）](#测验-5为什么-rust-没有-cc-意义上的未初始化变量读取理解层)
  - [实践](#实践)
  - [权威来源对照](#权威来源对照)
  - [总结](#总结)
  - [📋 关键属性](#-关键属性)
...
3. **Rust 的所有权 = 存储模型的线性扩展**：Rust 保留两级映射，但在类型系统（Type System）中附加线性约束——每个地址在同一时刻至多被一个名字「拥有」，move 即把「名字 → 地址」的映射从环境中原子地转移到新名字。别名只允许以受检的借用（Borrowing）形式临时存在。
...
| 极端值语义 | **Rust** | 默认 move（唯一所有权），共享必须显式借用（Borrowing） |

**Rust 的「极端」之处**在于：默认值语义 + 无隐式拷贝构造函数。C++ 的 `T b = a;` 可能调用用户定义的拷贝构造（任意副作用），Rust 的 `let b = a;` 只有两种可能——`Copy` 类型的按位复制（无副作用，可证明），或 move（所有权转移，`a` 失效）。共享状态必须经过类型系统（Type System）可见的借用或显式的 `Rc`/`Arc`，不存在「悄悄共享」的第三条路。
...
变量模型的形式化骨架是**环境与存储的分离**——这是理解 Rust 所有权在语义层面位置的关键：

- **环境（Environment, ρ）**：变量名 → 位置的映射（`x ↦ ℓ₁`）。Rust 的 `let` 绑定即环境扩展；遮蔽（shadowing）创建新位置而非更新旧映射；
- **存储（Store, σ）**：位置 → 值的映射（`ℓ₁ ↦ 42`）。赋值是存储更新；move 是「存储内容转移 + 源位置标记为未初始化」；
- **Rust 的扩展：所有权标记**：每个位置附加所有者信息——`let y = x;`（move 语义）把 `ℓ₁` 的内容转移到 `ℓ₂` 并将 `ℓ₁` 标记为 moved-out，此后环境查询 `x` 得到「已移动」错误（E0382 的语义解释）。

形式化的价值：借用规则可表述为「存储位置的访问权限代数」——`&x` 从环境查位置后授予只读权限，`&mut x` 授予独占权限并冻结其他权限。编译器的借用检查即此代数的静态求解。
...
L-value（可定位、可取地址的表达式）与 R-value（纯值）的经典二分在 Rust 中有精确对应，但规则更细：

- **位置表达式（place expression）**：变量、字段访问（`s.x`）、解引用（`*p`）、索引（`v[i]`）——可出现在赋值左侧、可被借用（`&place`）；
- **值表达式（value expression）**：字面量、函数调用、算术运算——只产生临时值，默认不可借用（临时值提升规则例外：`&42` 中的 `42` 被提升为静态）；
- **Rust 特有的三分**：值表达式产生的临时值有**临时生命周期（Lifetimes）**（语句结束即 drop，`let x = &temp();` 中延长到 `x` 的作用域——temporary lifetime extension）；
- **与移动语义的关系**：位置表达式作为值使用时触发 move（非 `Copy`）或 copy（`Copy` 类型）——「读位置」是有效果的操作，这是 Rust 与 C++ 左值模型的本质差异。

判定准则：能取 `&` 的是位置表达式；作为右操作数使用时，问「这里发生 move、copy 还是借用」。
...
- **遮蔽 vs 可变**：`let x = x + 1;`（新绑定）与 `x = x + 1;`（存储更新）语义不同——前者旧 `x` 仍存在（只是不可见），可被闭包（Closures）捕获；混淆二者是「闭包捕获了意外的值」类 bug 的根源；
- **临时值生命周期（Lifetimes）**：`let r = &format!("{}", x);` 能编译（生命周期延长），但 `match &format!(...) { ... }` 中临时值活到 match 结束——延长规则的边界位置（`let` 直接绑定 vs 嵌套表达式）需要精确记忆；
- **move 后的部分可用**：结构体（Struct）字段级 move（`let a = s.f1;` 后 `s.f2` 仍可用）——所有权跟踪是字段粒度的，但 `Drop` 类型的部分 move 被禁止（E0382）；
- **`Copy` 的位复制语义**：`Copy` 类型的「复制」是按位拷贝且源仍有效——实现 `Drop` 的类型不能 `Copy`（E0184），这是所有权模型的一致性（Coherence）约束。

每个反例附「环境-存储图」分析，训练用形式化模型预测编译器行为。
...
本节把「变量模型：从通用 PL 视角看 Rust 的所有权」的规则推到编译器与运行时（Runtime）的边界上逐一实测：边界测试：`let` 与 `const` 的初始化差异（编译错误）、边界测试：`const` 与 `static` 的 `Drop` 限制…、边界测试：变量遮蔽（shadowing）与不可变引用（Immutable Reference）的冲突（编译错误）、边界测试：未初始化变量的强制使用（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> **认知路径**: 从「变量 = 标识符 + 值 + 生命周期 + 所有权」四元模型出发，理解 Rust 变量绑定如何同时决定栈/堆分配、可变性、借用与移动语义。

### 核心推理链

| 定理 | 前提 | 结论 | 置信度 |
|:---|:---|:---|:---|
| 绑定引入所有权 ⟹ 资源生命周期显式化 | 理解 `let` 创建所有者 | 预测 drop 时机与 move 行为 | 高 |
| 可变性默认不可变 ⟹ 别名控制简化 | 需要 `mut` 显式声明 | 减少共享可变状态导致的数据竞争 | 高 |
| 遮蔽 (shadowing) 与重新绑定 ⟹ 名称复用安全 | 区分遮蔽与修改 | 在同一作用域内安全转换类型/值 | 高 |

> **过渡**: 掌握变量模型后，可进一步学习 NLL、Two-Phase Borrows 等借用检查器细化规则。
> **过渡**: 将变量模型与求值策略（值语义 vs 引用语义）结合，可解释函数参数传递、闭包捕获与迭代器消耗行为。
> **过渡**: 从变量绑定出发，可进入模式匹配、解构与 `ref`/`ref mut` 绑定模式的学习。

> 内存安全可预测 ⟸ 所有者唯一且生命周期显式 ⟸ 变量绑定模型
> 并发数据竞争减少 ⟸ 默认可变禁止 + 显式借用 ⟸ 变量可变性规则

---

## 反命题与边界

> **反命题**: "Rust 变量与 C/C++ 变量一样，只是带类型的内存槽。" —— 错误。Rust 变量绑定同时承载所有权/借用语义，`let` 会触发 move/copy，生命周期由作用域与借用规则共同决定，而非纯内存地址。
> **边界**: 变量模型解释栈与堆分配的高层语义，但不承诺具体内存布局；`#[repr]`、Box、RVO 等可改变实际分配与优化。
