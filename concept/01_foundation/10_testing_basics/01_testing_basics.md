...
> **内容分级**: [综述级]
> **本节关键术语**: 测试 (Testing) · 单元测试 (Unit Test) · 集成测试 (Integration Test) · assert · #[cfg(test)] — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# 测试基础：从单元测试到集成测试
>
> **EN**: Testing
...
Rust 测试体系的三个层级对应三种编译产物与运行方式，理解其差异是组织测试代码的前提：

1. **单元测试**（2.1）：与被测代码编译进**同一 crate**（`#[cfg(test)]` 条件编译），可访问私有项；靠 `#[test]` 属性标记，`cargo test` 用测试 harness 驱动执行，默认多线程并行；
2. **集成测试**（2.2）：`tests/` 目录下每个文件是**独立 crate**，只能通过公共 API 测试——这既是限制（私有项不可见）也是价值（强制检验 API 可用性）；共享代码放 `tests/common/mod.rs`（注意不是 `common.rs`，否则会被当作独立测试目标）；
3. **文档测试**（2.3）：文档注释中的 ```rust 代码块被提取为独立测试——保证示例不腐烂，这也是本库概念页代码示例的首选验证通道。

判定准则：测试「实现细节」用单元测试，测试「公开契约」用集成测试，示例代码一律走文档测试或可编译示例。
...
- **反命题 1：「测试越多覆盖率越高越好」** —— 过度。Rust 的类型系统（Type System）已经静态消除整类错误（空指针、数据竞争、use-after-free），测试资源应投向类型系统**管不到**的领域：业务不变量、边界输入、并发时序、与外部系统的契约。判定准则：每个测试应能回答「它捕获哪一类编译器漏掉的错误」。
- **反命题 2：「`should_panic` 足以验证错误路径」** —— 不充分。裸 `#[should_panic]` 无法区分「预期的 panic」与「意外的 panic」——必须用 `expected = "..."` 锁定 panic 消息子串；更严格的做法是把错误路径改为返回 `Result` 并断言具体错误变体。

边界极限小节量化：并行测试的共享状态冲突（需 `serial_test` 或进程级隔离）、集成测试的可见性边界（E0603 触发条件）、以及 `should_panic` 消息匹配的子串语义。
...
- [Security Practices](../../06_ecosystem/07_security_and_cryptography/01_security_practices.md) — 安全实践
