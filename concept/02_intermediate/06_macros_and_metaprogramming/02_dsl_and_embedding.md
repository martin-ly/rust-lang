> **内容分级**: [综述级]
> **本节关键术语**: 领域特定语言 (DSL) · 嵌入 DSL · 宏 (Macro) · 流利接口 (Fluent Interface) · Builder 模式 — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
三层的选择判定：解析任务 → parser combinator；构建任务（输出结构化值）→ 类型安全 DSL；语法与宿主差异大且需编译期校验 → 过程宏（Procedural Macro） DSL。
...
- **反命题 1：「类型状态机（typestate）DSL 总是值得」** —— 有边界。typestate 把非法状态转为编译错误的代价是类型数量爆炸与错误信息晦涩——状态数 × 转换数超过 ~20 时，泛型（Generics）签名的可读性崩溃。判定准则：状态机是 API 的核心契约（如协议握手、构建器必填字段）⟹ typestate 值得；内部实现细节 ⟹ 运行期 `enum` + 断言更简单。
- **反命题 2：「宏 DSL 能提供任意语法」** —— 不准确。`macro_rules!` 的输入必须是合法 token 树——可以设计类 SQL/类 HTML 方言（`sqlx`/`maud`），但无法突破 Rust 词法（自定义运算符、裸字符串外的新字面量）。需要真正自由语法 ⟹ 过程宏 + 自定义解析器（`syn` 之外手写 parser）。

边界极限小节量化：构建器链式调用的所有权流转、typestate 的非法转换拒绝、以及 DSL 错误从编译期滑向运行期的临界点。
...
判定 DSL 错误归属：E0599/E0382 → 类型层规则生效（好事：错误被编译期拦截）；panic/Err → 值层规则生效（检查 `build()` 的校验逻辑）；recursion limit → 宏实现层。
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| 形式 | 声明宏片段（`expr`/`ty`/`pat` 等）匹配生成 | `macro_rules!` |
| 求值 | 编译期展开，零运行时解析开销 | 宏系统 |
| 类型安全 | 生成代码仍经完整类型检查 | 编译管线 |
| 卫生 | 标识符部分卫生，需 `$crate` 等技巧 | 卫生规则 |
| 分工 | 简单 DSL 用声明宏，复杂语法用过程宏 | 生态分工 |
