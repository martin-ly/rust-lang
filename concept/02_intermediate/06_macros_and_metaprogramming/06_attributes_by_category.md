...
- **派生**：`#[derive(...)]`——内建 derive 是编译器特判，第三方 derive 走过程宏（Procedural Macro）管线；
...
本节用三组对照说明「属性分类详解（Attributes by Category）」：正确示例：组合使用属性、反例：属性位置错误与反例：条件编译条件拼写错误。每组先给正确示例并标注其成立的类型系统依据，再给反例并标注编译器诊断（E0xxx）或运行时（Runtime）后果，最后给出修正方案。判读标准：正确示例应能通过 rustc 1.97 编译且无 clippy 警告，反例的失败点必须可定位到具体规则。
...
- **反命题 1：「`#[inline(always)]` 总能提升性能」** —— 错误。强制内联增大代码体积、恶化 I-cache 与分支预测，且剥夺编译器的成本模型决策权。判定准则：先用 `#[inline]`（提示）或什么都不标（跨 crate 时泛型（Generics）自动可内联），只有 profile 证明调用开销是瓶颈且内联后确实更快才用 `always`。
...
| 属性作用域 | 项级/模块（Module）级 | crate 级 | 内部属性 `#![...]` 影响整个范围 |
...
本节把「属性分类详解（Attributes by Category）」的规则推到编译器与运行时（Runtime）的边界上逐一实测：边界测试：内部属性 与 边界测试：cfg_attr。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
