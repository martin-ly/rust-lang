...
- **什么时候不需要宏**（5.2）：CPP 中 `#define` 的多数用途在 Rust 中有更好的非宏替代——常量用 `const`（有类型、有作用域）、条件编译用 `#[cfg]`（语法级而非文本级）、内联函数用 `#[inline]` 泛型（类型检查保留）；
- **什么时候用过程宏**（5.3）：需要解析语法结构（derive）、需要生成基于输入分析的大量代码、需要外部数据（读文件/环境）参与生成——这些超出 `macro_rules!` 的模式匹配能力。
