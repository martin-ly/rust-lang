...
- **反命题 1：「编译期计算越多越好」** —— 过度。编译期计算（const fn、宏展开、类型级编程）的成本是编译时间与错误信息复杂度——`const fn` 的求值步数限制、宏递归深度、类型级编程的指数级 trait 求解都是真实上限。判定准则：编译期计算只用于「运行期无法表达」的事（类型安全、零成本抽象），能在运行期廉价完成的不要上移。
- **反命题 2：「`TypeId` 可用于跨 crate 类型识别」** —— 有边界。`TypeId` 在同一编译内稳定，但**不承诺跨编译/跨版本稳定**——把 `TypeId` 持久化（存数据库/网络传输）是未定义行为级别的假设。跨边界类型识别应用显式的类型注册表（字符串名 + 版本）。

边界极限小节量化：过程宏的 token 解析失败模式、const 泛型表达式边界、以及编译期递归深度。
...
每组用例给出「恰好失败版 + 修复/替代方案」，覆盖元编程报错的主要谱系。
...
> migrated from `crates/c11_macro_system_proc/docs/tier_04_advanced/01_macro_metaprogramming.md`

### 类型级编程

| 技术 | 作用 | 典型用法 |
|:---|:---|:---|
| `PhantomData<T>` | 标记未使用的类型参数，影响型变与生命周期（Lifetimes） | 单位系统、类型状态 |
| 类型状态模式 | 用泛型状态将状态机编码进类型 | 编译期保证合法状态转换 |
| 类型级计算 | Peano 数字、布尔运算在类型系统（Type System）完成 | 编译期数值约束 |
| HList（异构列表） | 类型安全地保存不同元素 | 类型级反射、泛型 Builder |

### 编译期计算

- **`const fn`**：编译期可执行的纯函数，用于常量泛型与数组长度计算。
- **编译期断言**：`const_assert!` 宏或 `let _ = [(); (COND as usize)];` 技巧在编译期捕获不变量违反。
- **宏驱动代码生成**：用 `macro_rules!` 或过程宏生成重复样板。

### 零成本抽象

- **强制内联**：`#[inline(always)]` 消除函数调用开销。
- **泛型单态化（Monomorphization）**：每个具体类型生成独立代码，实现静态分发零开销。
- **编译期多态**：通过 trait bound 在编译期选择实现。

### 高级宏模式

| 模式 | 用途 | 复杂度 |
|:---|:---|:---|
| TT Munching | 逐 token 处理复杂宏输入 | 高 |
| Push-Down Accumulation | 在递归中累积结果 | 高 |
| Internal Rules | 宏内部辅助规则 | 中 |
| Callback 模式 | 将代码片段作为宏参数回传 | 中 |

### 选型决策树

```text
需要元编程？
├── 类型级约束 → PhantomData / 类型状态 / const 泛型
├── 编译期计算 → const fn / 编译期断言
├── 复杂语法解析 → TT Munching / 过程宏
└── 代码生成 → macro_rules! / derive 宏
```
...
| 元编程：Rust 的编译期代码生成与变换 正确用法 ⟹ 常见陷阱 | 忽略边界条件 | 编译错误或运行时（Runtime） bug | 高 |
