...
声明宏是 Rust 元编程的声明式入口，它通过模式匹配在编译期生成代码，而非像过程宏那样命令式地操作 AST。理解声明宏的定位、基本结构、卫生性和片段分类符，是写出安全、可维护宏代码的基础。以下四个小节从概念定位到具体语法机制，逐步建立对 `macro_rules!` 的系统认识。
...
在掌握声明宏的基本语法后，需要进一步理解其底层机制：如何处理变长参数、如何通过递归消费 token tree、如何在本 crate 内外正确解析路径，以及如何控制宏的可见性。这些细节决定了宏能否在复杂场景中保持稳定、避免命名冲突，并在跨 crate 使用时仍然可靠。
...
声明宏虽然强大，但并非所有重复代码都适合用它解决。本节通过反命题树澄清声明宏与泛型、过程宏的选型边界，并列出其能力极限——包括类型分析缺失、标识符拼接限制、错误信息定位困难和递归深度约束。明确这些边界有助于在工程中选择最小够用的抽象手段。
...
声明宏的边界测试聚焦于两类问题：一是宏模式与 Rust 语法之间的解析冲突，例如 `expr` 片段后不能随意接运算符；二是宏展开后的行为是否符合预期，例如卫生性隔离、递归终止和参数多次求值。通过观察这些编译错误与逻辑错误，可以在实际开发中更早地识别宏定义中的隐患。
...
本节测验覆盖声明宏的三个核心判别点：片段分类符的匹配范围、卫生性的作用，以及递归宏的限制。作答时应结合前文中的代码示例，注意区分「哪些检查由宏模式完成」与「哪些检查由 Rust 编译器在宏展开后完成」。
