...
> **主要来源**: [TRPL Ch 17 — OOP Features of Rust] · [Rust Reference — TypeId] · [C++ Reference — typeid]
---
...
> **运行时类型识别（RTTI）不是动态类型的专利，而是静态类型系统在运行时对类型信息的有限暴露。
...
- 转换失败时，指针版本返回 `nullptr`，引用版本抛出 `std::bad_cast`。
...
Rust 通过 `dyn Any` 提供类似 `void*` + RTTI 的能力，但完全受类型系统约束：
...
`TypeId` 是一个不透明的、可比较的值，用于在运行时判断两个类型是否相同。
...
- 不需要虚函数表中的 RTTI 信息；类型标识来自单态化生成的 `TypeId`。
...
> **关键洞察**：Rust 不提供子类型向下转换（`dyn Base` → `dyn Derived`），因为这会破坏借用检查器的静态保证。`Any` 只支持"类型相等"的恢复，而不是"子类型包含"的恢复。
...
| **L3** | RTTI 是静态类型系统向运行时的有限"泄漏"；Rust 通过 trait 对象和生命周期约束，将这种泄漏控制在显式、可审计的边界内。 |
...
> **Checklist**: 已覆盖 C++ RTTI / Rust `Any` / `TypeId` / `downcast_ref` / 形式化对比 / 典型用例。