...
> **后置概念**: [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) ·
...
Rust 内存管理的权威定义从三个层面回答「谁、何时、按什么规则释放内存」：

- **Wikipedia 对齐视角**：内存管理是「为程序提供分配与回收内存手段」的机制谱系——手动管理（C）、垃圾回收（Java/Go）、所有权 + RAII（Rust/C++）是谱系上三个代表点。Rust 的独特之处是把「释放时机」完全静态化：无 GC 线程、无引用计数默认开销。
- **TRPL 官方视角**：内存经所有权系统管理——值有唯一所有者，所有者离开作用域时值被 drop。栈分配由调用帧管理（零开销），堆分配经 `Box`/`Vec`/`String` 等拥有型类型包装，释放由 RAII（`Drop` trait）确定性触发。
- **形式化视角**：释放的确定性来自仿射类型——每个资源恰好被消费一次（move 或 drop），不存在「无人负责的内存」（泄漏需要显式 `mem::forget`/`Box::leak`/循环引用）也不存在「多人释放」（double-free 被类型排除）。

判定一个内存问题的类别，先定位分配位置（栈/堆/静态），再定位所有权形态（独占 `Box`/共享 `Rc`/`Arc`/借用），释放规则由二者完全确定。
...
本节把「Memory Management（内存管理）」的规则推到编译器与运行时的边界上逐一实测：测试 1: Rc<RefCell<T>> 循环引用极限、测试 2: RefCell 嵌套借用边界、测试 3: Box::leak 与 ManuallyDrop 边界与测试 4: Arc 跨线程原子序边界。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> **[TRPL Ch15](https://doc.rust-lang.org/book/ch15-00-smart-pointers.html) · [Rustonomicon: Interior Mutability](https://doc.rust-lang.org/nomicon/interior-mutability.html)** 编译期检查是静态证明（零开销但保守），RefCell 是动态监控（运行时开销但更灵活）。 ✅
...
| 类型系统（Type System）基础 | [01_foundation/02_type_system/01_type_system.md](../../01_foundation/02_type_system/01_type_system.md) | 智能指针的类型约束 |
...
| Pin 与自引用 | [03_advanced/01_async/01_async.md](../../03_advanced/01_async/01_async.md) | 堆内存语义 |
| Unsafe | [03_advanced/02_unsafe/01_unsafe.md](../../03_advanced/02_unsafe/01_unsafe.md) | 裸指针与 ManuallyDrop 边界 |
| 形式化验证 | [04_formal/04_rustbelt.md](../../04_formal/02_separation_logic/01_rustbelt.md) | 内存安全证明 |
...
> **来源**: [Rust Reference: ManuallyDrop](https://doc.rust-lang.org/std/mem/struct.ManuallyDrop.html) · [Rust Reference: std::mem::forget](https://doc.rust-lang.org/std/mem/fn.forget.html) · [Rustonomicon: Special memory](https://doc.rust-lang.org/nomicon/exotic-sizes.html) · [Wikipedia: Memory management](https://en.wikipedia.org/wiki/Memory_management)

---

### 5.9 `Vec<T>` / `String` / `HashMap` 的内存布局与扩容策略

三种标准库容器的内存布局与扩容策略，是内存管理「理论规则」的具体工程实例：

- **`Vec<T>`**：三元组布局（指针、长度、容量）共 24 字节（64 位平台），元素连续存放。扩容策略：容量耗尽时至少翻倍（`Vec::grow` 调用分配器的 `grow`/`realloc`），均摊 push 为 O(1)；`reserve_exact` 按需精确扩容；`shrink_to_fit` 释放冗余。`T` 为零大小类型（ZST）时容量为 `usize::MAX` 且不分配——扩容逻辑对 ZST 特判。
- **`String`**：本质是 `Vec<u8>` + UTF-8 不变量（所有公开方法维护「字节序列恒为合法 UTF-8」）。布局与扩容策略完全继承 `Vec`；`+`/`push_str` 的拼接经 `reserve` + 字节拷贝完成，`from_utf8` 的转换在已校验路径零拷贝（`Vec<u8> → String` 只是类型重标记）。
- **`HashMap<K, V>`**：自 Rust 1.36 起基于 SwissTable 算法（hashbrown 并入标准库）：元数据控制字节数组（每组 16 个 1 字节控制位，SIMD 并行匹配）+ 紧凑元素存储，负载因子约 87.5%。查找先 SIMD 扫控制字节（一次比较 16 槽），命中才读完整键——这是其相对传统链式哈希表 2–3 倍缓存命中优势的来源。扩容在超过负载阈值时整体 rehash。

判定容器内存行为的方法：布局决定「单次访问的缓存成本」，扩容策略决定「批量插入的均摊成本与峰值占用」——两者合起来给出容器性能的一阶模型。
...
> 详见 [§5.5 补充：`Pin<&mut T>` 的堆内存语义与自引用安全](#55-补充pinmut-t-的堆内存语义与自引用安全) 与 [Pin 概念](../../03_advanced/01_async/08_pin_unpin.md)。
...
内存管理边界测试按「编译期拒绝 / 运行时 panic / 未定义行为」三档分类，分界线是检查成本能否静态承担：
...
本组测验围绕测验 1：Stack vs Heap（理解层）、测验 2：Drop 与 RAII（应用层）、测验 3：Box 与递归类型（应用层）、测验 4：内存布局对齐（分析层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
