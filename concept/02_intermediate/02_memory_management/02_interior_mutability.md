...
内部可变性（Interior Mutability）是「经共享引用（Reference） `&T` 修改数据」的受控机制，与默认的外部可变性（`&mut T` 独占写）形成互补。三个核心概念：

1. **外部 vs 内部可变性对比**：外部可变性在「引用（Reference）创建点」检查——`&mut T` 存在期间禁止任何其他访问路径（编译期）；内部可变性把检查推迟到「数据访问点」——`Cell`/`RefCell` 的每次读写执行规则校验（复制语义或运行时借用计数）。前者零成本但约束强，后者灵活但检查有运行时代价或能力限制。
2. **类型谱系**：`Cell<T>`（单线程，按位复制，`T: Copy` 语义最简）→ `RefCell<T>`（单线程，运行时借用检查，`borrow`/`borrow_mut` 动态守卫）→ `Mutex<T>`/`RwLock<T>`（多线程，操作系统级互斥）→ `Atomic*`（多线程，无锁原子操作（Atomic Operations））。谱系的横轴是「线程安全性」（`Send`/`Sync` 约束递增），纵轴是「检查时机」（编译期 → 运行时 → 硬件）。
3. **运行时借用检查**：`RefCell` 用两个计数器（共享借用数、可变借用（Mutable Borrow）标志）在运行时执行「别名 XOR 可变」规则——违规时 panic（`borrow_mut` 在已有借用存活时触发）。这把编译器 E0502 的静态规则原样搬到了运行时。
...
- **`RefCell<T>`：动态借用规则**：`borrow()` → `Ref<T>`（共享计数 +1，守卫 drop 时 −1），`borrow_mut()` → `RefMut<T>`（独占标志置位）。规则与编译期借用完全同构：可变借用（Mutable Borrow）存在时任何借用 panic，共享借用存在时 `borrow_mut` panic。`try_borrow` 系列把 panic 换为 `Result`。成本：每次借用一次计数读写（约 1–2ns），加潜在的 panic 分支。
...
三者的共同设计：可变性权限与「守卫值的生命周期（Lifetimes）」绑定（RAII），权限的获取与释放都经类型系统（Type System）追踪——内部可变性从未绕过借用规则，只是改变了检查时机。
...
|:---|:---|:---|:---|
| 独占访问即可满足可变性 | 外部可变性（`&mut`），编译期安全零开销 | §4.1 反命题树 TRUE | 共享所有权（Ownership）下 ⟹ 编译拒绝 |
...
| 对外暴露的 API | 内部可变性封装在模块（Module）内，暴露 safe API | 最佳实践 | `RefCell` 泄漏到公共接口 ⟹ 设计意图存疑 |
...
作答建议：测验 5 先写出完整的 `Send`/`Sync` 推理链再核对——这是并发类型系统（Type System）的枢纽题。
