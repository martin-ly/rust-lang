...
**解析**：`Box<T>` 是最简单的智能指针：把值放到堆上，指针本身独占该值的所有权，遵循与普通所有权完全相同的移动/drop 规则。A 是 `Rc`/`Arc`；D 需要 `Arc` + 锁；C 与堆分配矛盾。`Box` 的价值在于：定大小指针包装不定大小/递归类型、大对象避免栈拷贝、trait 对象（`Box<dyn Trait>`）。
...
- A. `RefCell<T>` 把借用规则的检查从编译期推迟到运行时
