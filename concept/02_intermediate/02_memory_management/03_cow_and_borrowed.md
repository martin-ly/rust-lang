...
本节展开「Cow：写时克隆与零拷贝抽象」的技术细节：Cow 的核心操作、常见使用模式与与 AsRef/ToOwned 的关系。重点是类型签名、所有权语义与编译期约束如何相互作用，而不是 API 罗列；每个小节给出可编译的最小示例，并标注对应反例的失败规则。读完后应能解释：为什么这种写法能通过编译，而那种写法会被借用检查器或类型系统拒绝。
...
- **反命题 2：「`Cow` 就是智能指针」** —— 不准确。`Cow` 是**枚举**（`Borrowed(&'a B)` / `Owned(B::Owned)`），不是指针包装——它有生命周期参数，不能做 `'static` 存储（除非 `Cow<'static>` 且只用 Owned 变体）。它也不实现引用计数：克隆 `Cow::Owned` 是深拷贝。
