> **内容分级**: [综述级]
> **本节关键术语**: 智能指针 (Smart Pointer) · Box · Rc · Arc · RefCell · 自定义智能指针 — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# 智能指针：堆内存管理与共享语义
>
> **EN**: Smart Pointers
> **Summary**: Smart Pointers — Box, Rc/Arc, RefCell/Cell: their ownership semantics and composition patterns.
...
智能指针是「拥有数据并附加语义的指针类型」——通过 `Deref`/`Drop` trait 获得「用起来像引用、行为有定制」的能力。Rust 智能指针谱系按所有权（Ownership）模型分四类：

- **`Box<T>`**：独占堆分配——最简单的智能指针，语义即「大小已知但值在堆上」，用于递归类型、大数据移动、`dyn` 封装；
- **`Rc<T>`/`Arc<T>`**：引用计数共享所有权（Ownership）——`Rc` 单线程（非原子计数）、`Arc` 跨线程（原子计数 + `Send`/`Sync`），`Weak` 变体打破循环；
- **`RefCell<T>`/`Cell<T>`**：内部可变性——借用（Borrowing）规则的运行期版本，单线程专用；
- **组合模式**：`Rc<RefCell<T>>`（单线程共享可变）、`Arc<Mutex<T>>`（跨线程共享可变）——组合是「所有权 × 可变性 × 线程性」三轴的精确选型。

判定准则：能用栈不用 `Box`；能独占不共享；共享需求先确认是否真共享（还是该重构所有权）；跨线程一律 `Arc` 系。
...
本节展开「智能指针：堆内存管理与共享语义」的技术细节：RefCell 与 Cell：内部可变性、智能指针的组合模式与性能特征对比。重点是类型签名、所有权语义与编译期约束如何相互作用，而不是 API 罗列；每个小节给出可编译的最小示例，并标注对应反例的失败规则。读完后应能解释：为什么这种写法能通过编译，而那种写法会被借用（Borrowing）检查器或类型系统（Type System）拒绝。
...
- **反命题 1：「`Rc` 循环引用只是泄漏，不是 bug」** —— 泄漏在 Rust 中是安全操作，但**逻辑上是 bug**：`Rc<RefCell<Node>>` 的父子互指使计数永不归零，长寿命服务中累积成内存增长。判定准则：所有权图必须是有向无环的——环的某条边必须是 `Weak`（弱引用不计数，访问时 `upgrade` 检查存活）；
- **反命题 2：「`Arc<Mutex<T>>` 是并发万能的默认解」** —— 过度。它把并发问题压缩为「锁内操作的原子性」，但锁粒度、持锁时长、锁顺序仍需设计——且 `Mutex` 在异步（Async）代码中应换 `tokio::sync::Mutex`（标准 guard 跨 await 是 `!Send`）。

边界极限小节量化：`Box::leak` 的 `'static` 转换代价（内存永不回收）、`Pin` 与智能指针的组合、以及引用计数的原子操作（Atomic Operations）成本（比非原子贵 2-3 倍于高竞争）。
...
> **边界要点**: 智能指针的边界主要与**循环引用（Reference）**、**运行时（Runtime） panic**、**死锁**、**原子开销**和**Send/Sync 限制**相关。
...
| 单线程共享可变 | `Rc<RefCell<T>>` | §三 选型决策矩阵 | 重复 `borrow_mut` ⟹ 运行时（Runtime） panic（§4.2 边界 2） |
...
- **上层概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Borrowing](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) · [Memory Management](01_memory_management.md)
- **下层概念**: [Pin](../../03_advanced/01_async/08_pin_unpin.md) · [Cow](03_cow_and_borrowed.md)
...
- [Memory Management](01_memory_management.md) — 内存管理
- [Pin](../../03_advanced/01_async/08_pin_unpin.md) — Pin 不动性
- [Cow](03_cow_and_borrowed.md) — 写时克隆
...
> `&mut self` 允许移动 `self`（若 `Self: !Unpin`），但 `self.ptr` 指向 `self.data` 的地址，移动后 `ptr` 成为悬垂指针。
> `Pin` 是解决自引用的关键：`Pin<&mut Self>` 保证 `self` 在内存中不可移动（`!Unpin`），允许安全创建自引用。
> 但 `Pin` 的 API 严格：`Pin::new_unchecked` 要求调用者保证对象永不被移动；`Box::pin` 是安全创建 `Pin<Box<T>>` 的标准方式。
> 自引用是 async/await、生成器、Futures 的核心机制。这与 C++ 的 `this` 指针（无 Pin 约束，移动后自引用 UB）或 Swift 的引用类型（堆分配不移动）不同——Rust 通过 `Pin` 类型系统（Type System）显式标记不可移动对象。
...
> **修正**:
> `Arc<T>` 实现 `Send` + `Sync` 当且仅当 `T: Send + Sync`。`RefCell<T>` 不实现 `Sync`（单线程运行时（Runtime）借用（Borrowing）检查），所以 `Arc<RefCell<T>>` 不能跨线程。
> 线程安全的内部可变性：
>
> 1) `Arc<Mutex<T>>` — 互斥锁；
> 2) `Arc<RwLock<T>>` — 读写锁；
> 3) `Arc<AtomicUsize>` — 无锁原子操作（Atomic Operations）。
> `RefCell` 的设计：单线程场景下零开销（无原子操作），但线程间共享导致编译错误。
> 这与 C++ 的 `std::shared_ptr<std::mutex>`（可跨线程，但需手动锁管理）或 Java 的 `AtomicInteger`（线程安全，但无 RefCell 的借用语义）不同——Rust 的类型系统（Type System）通过 `Send`/`Sync` 在编译期排除不安全的线程共享。
> 来源: [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html) · 来源: [Rust Standard Library]
...
本组测验围绕测验 1：Box<T> 的用途（理解层）、测验 2：Rc vs Arc（应用层）、测验 3：Weak 指针的作用（应用层）、测验 4：Deref 与智能指针（分析层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时（Runtime）行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
...
- C. 提供运行时（Runtime）借用（Borrowing）检查
...
`Box` 是**唯一所有权（Ownership）**，不同于 `Rc`（共享所有权）或 `RefCell`（运行时（Runtime）借用（Borrowing）检查）。
</details>
...
`Arc` 使用原子操作（Atomic Operations）维护引用计数，因此是 `Send + Sync`（当 `T` 满足条件时）。单线程场景优先使用 `Rc` 以获得更好性能。
</details>
...
> **认知路径**: 从 L0 基础概念出发，经由本节的 **智能指针（Smart Pointer）：堆内存管理与共享语义** 核心原理，通向 L2 进阶模式与 L3 工程实践。
...
| 智能指针：堆内存管理与共享语义 正确用法 ⟹ 常见陷阱 | 忽略边界条件 | 编译错误或运行时（Runtime） bug | 高 |
