...
智能指针是「拥有数据并附加语义的指针类型」——通过 `Deref`/`Drop` trait 获得「用起来像引用、行为有定制」的能力。Rust 智能指针谱系按所有权模型分四类：
...
- **`RefCell<T>`/`Cell<T>`**：内部可变性——借用规则的运行期版本，单线程专用；
...
本节展开「智能指针：堆内存管理与共享语义」的技术细节：RefCell 与 Cell：内部可变性、智能指针的组合模式与性能特征对比。重点是类型签名、所有权语义与编译期约束如何相互作用，而不是 API 罗列；每个小节给出可编译的最小示例，并标注对应反例的失败规则。读完后应能解释：为什么这种写法能通过编译，而那种写法会被借用检查器或类型系统拒绝。
...
- **反命题 2：「`Arc<Mutex<T>>` 是并发万能的默认解」** —— 过度。它把并发问题压缩为「锁内操作的原子性」，但锁粒度、持锁时长、锁顺序仍需设计——且 `Mutex` 在异步代码中应换 `tokio::sync::Mutex`（标准 guard 跨 await 是 `!Send`）。

边界极限小节量化：`Box::leak` 的 `'static` 转换代价（内存永不回收）、`Pin` 与智能指针的组合、以及引用计数的原子操作成本（比非原子贵 2-3 倍于高竞争）。
