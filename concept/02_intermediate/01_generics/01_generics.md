...
- **E0310（生命周期约束不足）**：泛型类型参数默认不含「活得足够久」的保证，跨引用边界使用时需 `T: 'static` 或显式生命周期 bound。
...
- **`min_specialization`**：允许在「更具体 impl 是更通用 impl 的子集」时提供专用实现（如标准库内部 `Vec<T>` 从 `&[T]: Copy` 切片克隆时的特化）。当前仍是 nightly 特性，卡点在于特化与生命周期交互的可靠性证明（soundness），稳定化没有时间表。
- **编译时间优化**：单态化的代价是编译期与二进制体积，演进方向包括共享单态化（相同布局的实例共用代码）、更积极的 `-Zshare-generics` 行为，以及通过 `dyn` 边界的显式设计减少实例数量。
- **Type-level Programming**：`generic_const_exprs` 推进后，`[T; N + 1]` 这类依赖常量参数的运算将逐步可用，把定长运算从宏技巧转为类型系统原生能力。
