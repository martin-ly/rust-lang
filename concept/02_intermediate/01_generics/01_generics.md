...
1. **泛型参数类型矩阵**：三类参数——类型参数 `T`（最常用）、生命周期参数 `'a`（标注引用关系）、常量参数 `const N: usize`（值进类型，数组长度、维度的类型化）。判定参数种类：出现在类型位置的是类型/常量参数，出现在引用前缀的是生命周期参数。
...
3. **推论（Const Generics ⟹ 类型级编程）**：把值（如数组长度 `N`）提升为类型参数，使「定长数组运算」「维度检查」等性质进入类型系统，编译期即可排除维度不匹配。
4. **约束多态的类型安全**：`T: Trait` 形式的约束把 ad-hoc 多态（重载式行为）纳入全称量化框架，保证任何满足约束的替换都不会产生「方法不存在」类错误。

判定要点：凡声称「泛型有运行时开销」或「泛型错误只能在实例化时发现」的论述，都与上述第 2、1 环矛盾——它们分别对应 C 宏/C++ 模板的行为，而非 Rust 泛型。
...
- **E0277（类型大小未知 / bound 不满足）**：泛型参数默认隐含 `Sized` 约束，使用 `dyn Trait` 或切片等不定长类型时需显式 `T: ?Sized`；另一类 E0277 是 bound 声明不足，函数体用了未声明的能力。
...
边界极限测试探测的是泛型机制在**规模与嵌套深度**上的实际极限，而非语法正确性。三组测试各对应一种真实工程约束：单态化代码膨胀测试对比泛型与 `dyn Trait` 在实例数量增长时的二进制体积与缓存行为，回答「何时该用动态分发主动放弃单态化」；生命周期递归传递测试把 HRTB（`for<'a> F: Fn(&'a T)`）嵌套到闭包与 trait 对象边界，标定类型检查器可推断的深度；Const Generics 类型级运算测试考察编译期常量表达式（如 `N * 2`、`N - 1`）在当前稳定版支持到何种复杂度。运行这些测试时注意：它们中的一部分**预期失败或预期触发编译器限制**，结果应记录为能力边界而非缺陷。
...
| 关联类型需借用 `Self` 数据 | GAT（见 [40_generic_associated_types.md](../00_traits/07_generic_associated_types.md)） | GAT 规则 | 用 HRTB 绕行可行但表达力受限 |
...
本页的知识结构由概念定义（泛型 = 参数化的类型与函数）、属性特征（编译期单态化、零成本、定义处约束检查）、关系连接（上位：参数多态；对偶：`dyn Trait` 动态分发；组合：trait bound + 生命周期 bound）、思维导图（泛型语法的四个作用位置：函数、结构体、枚举、impl）四部分构成，另附术语对齐与权威来源映射，作为本层泛型主题的导航枢纽。
...
类型状态模式（Typestate Pattern）把状态机的每个状态编码为独立的零大小类型，状态转移实现为**消费 `self`、返回下一状态类型**的方法。其本质是利用所有权系统的移动语义：旧状态值被消费后不可再用，非法转移（如在 `Closed` 状态调用 `write`）在类型层面根本不存在可调用的方法，从而在编译期排除「状态-操作不匹配」类错误。代价是每个状态-操作组合都要写一个 `impl` 块，状态多时模板代码膨胀——可用泛型 + 标记 trait 或宏缓解。典型应用：文件句柄（`Closed → Open → Reading`）、网络连接握手、构建器（builder）的必填字段检查。下面的基础类型状态示例给出最小骨架。
