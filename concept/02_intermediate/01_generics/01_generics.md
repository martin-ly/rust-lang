...
> **📎 交叉引用（Reference）**
>
> 本主题在 knowledge 中有系统化的知识索引：[泛型（Generics）](01_generics.md)
> **受众**: [进阶]
...
> **后置延伸**:
> [L3 Async](../../03_advanced/01_async/01_async.md) ·
> [L4 类型论](../../04_formal/00_type_theory/01_type_theory.md) ·
> [L7 效果系统](../../07_future/02_preview_features/01_effects_system.md)
> **跨层映射**: L2→L4 参数多态 ↔ System F | L2→L7 泛型（Generics）效果 → Effect System
> **定理链编号**:
> T-230 参数多态保持 → T-231 单态化（Monomorphization） [来源: [Rust Reference — Monomorphization](https://doc.rust-lang.org/reference/items/generics.html) ·
> [Itanium C++ ABI](https://itanium-cxx-abi.github.io/cxx-abi/abi.html)]正确性 → T-232 约束满足可判定
> **层级**: L2 进阶概念
> **前置概念**: [Type System Basics](../../01_foundation/02_type_system/01_type_system.md) · [Traits](../00_traits/01_traits.md)
> **后置概念**: [Advanced Lifetimes](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md) · [GATs](../../03_advanced/01_async/01_async.md) · [Const Generics [来源: [RFC 2000](https://rust-lang.github.io/rfcs//2000-const-generics.html)]]
> **主要来源**:
...
泛型（Generics）是 Rust 参数多态（parametric polymorphism）的载体，三个权威定义视角：

- **Wikipedia 对齐视角**：泛型允许「用类型参数编写算法与数据结构，在使用时实例化」——Rust 属于「编译期实例化」流派（与 C++ 模板、ML 多态同族），区别于 Java 的运行期类型擦除。
- **TRPL 官方视角**：泛型是「消除重复的具体定义」的工具——`Option<T>` 一份定义服务所有 `T`。编译器通过单态化把泛型代码还原为每个使用处的具体代码，因此「泛型无运行时成本」。
- **形式化视角**：泛型对应 System F 的二阶 λ 演算中的 `Λα. τ`（类型抽象）与 `τ[σ]`（类型应用）；trait bound 是有界量化 `∀α ≤ B. τ`；const generics 引入依赖类型的受限片段（值出现在类型位置）。

三视角的衔接：TRPL 给出工程承诺（零成本重复消除），Wikipedia 给出家族定位（编译期实例化），形式化给出「为什么类型检查在定义处即可完成」——有界量化保证泛型体只需对 bound 检查一次，实例化无需重新检查（与 C++ 模板的「实例化时才报错」形成根本差异）。
...
泛型的属性矩阵沿「参数种类 × 实现机制 × 约束演进」三个维度组织：

1. **泛型参数类型矩阵**：三类参数——类型参数 `T`（最常用）、生命周期参数 `'a`（标注引用（Reference）关系）、常量参数 `const N: usize`（值进类型，数组长度、维度的类型化）。判定参数种类：出现在类型位置的是类型/常量参数，出现在引用前缀的是生命周期参数。
2. **泛型实现机制对比**：Rust/C++ 走单态化（每实例一份代码，零分派成本，体积膨胀）；Java/C# 走类型擦除/装箱（一份代码，运行时转型或装箱成本）；Go 1.18+ 走「GC shape 单态化」折中（按内存布局分组实例化）。三者的差异本质是「类型信息在运行时留存多少」。
3. **泛型约束演进矩阵**：从裸 `T`（只能 move/drop）到 `T: Trait`（能力声明）、`T: A + B`（合取）、`where` 子句（复杂约束）、关联类型约束（`T: Iterator<Item = u8>`）、HRTB（`for<'a> Fn(&'a T)`）。约束的每一步增强都把「实例化时可用的操作集」扩大，同时把「可实例化的类型集」缩小。

矩阵读法：写泛型时先定参数种类（矩阵 1），再选最小够用约束（矩阵 3），最后确认实例化机制带来的体积/时间代价可接受（矩阵 2）。
...
> **形式化对应**: 生命周期（Lifetimes）参数在类型论中对应 **区域类型 (Region Types, Tofte & Talpin 1994)**，即引用（Reference）有效性的形式化约束。详见 L1 生命周期 §4 和 [L4 所有权（Ownership）形式化](../../04_formal/01_ownership_logic/02_ownership_formal.md) §2.2。[Tofte & Talpin 1994 — Region Types](https://doi.org/10.1145/176454.176456)
...
泛型的形式化基础是一条从类型论到机器码的推理链，理解它能回答「为什么 Rust 泛型既不损失类型安全也不损失性能」。链条的四个环节依次是：

1. **引理（参数多态 ⟹ System F 类型规则）**：Rust 的 `<T>` 对应 System F 的全称量词 `∀T`，泛型函数在实例化之前就被类型检查——这是「检查一次、处处安全」的理论依据，区别于 C++ 模板的实例化后检查。
2. **定理（单态化 ⟹ 零成本抽象 ⟹ 语义保持）**：编译器为每个实际用到的类型参数生成一份专用代码（monomorphization），结果与手写重复代码逐指令等价，故抽象无运行时成本；又因为约束（trait bound）在泛型定义处已校验，单态化只替换类型不改变可证明的性质，语义得以保持。
3. **推论（Const Generics ⟹ 类型级编程）**：把值（如数组长度 `N`）提升为类型参数，使「定长数组运算」「维度检查」等性质进入类型系统（Type System），编译期即可排除维度不匹配。
4. **约束多态的类型安全**：`T: Trait` 形式的约束把 ad-hoc 多态（重载式行为）纳入全称量化框架，保证任何满足约束的替换都不会产生「方法不存在」类错误。

判定要点：凡声称「泛型有运行时开销」或「泛型错误只能在实例化时发现」的论述，都与上述第 2、1 环矛盾——它们分别对应 C 宏（Macro）/C++ 模板的行为，而非 Rust 泛型。
...
> **跨层映射**:
> 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/04_inter_layer_map.md) §4.2 "类型系统（Type System）一致性（Coherence）"
> **过渡到示例与反例**:
...
本节的示例按「约束纪律」与「边界错误」两类组织。正确示例示范两种核心写法：泛型函数通过 `T: PartialOrd + Copy` 这类 bound 精确声明所需能力（能力声明越少，可复用面越大）；常量泛型把长度、容量等编译期常量纳入类型，使 `ArrayVec<T, N>` 这类结构无需堆分配。反例则覆盖泛型代码的两类典型编译错误：

- **E0277（类型大小未知 / bound 不满足）**：泛型参数默认隐含 `Sized` 约束，使用 `dyn Trait` 或切片（Slice）等不定长类型时需显式 `T: ?Sized`；另一类 E0277 是 bound 声明不足，函数体用了未声明的能力。
- **E0310（生命周期约束不足）**：泛型类型参数默认不含「活得足够久」的保证，跨引用（Reference）边界使用时需 `T: 'static` 或显式生命周期 bound。

两组错误共同说明：Rust 泛型的错误几乎全部在**编译期、定义处或调用处**暴露，不存在运行时才爆炸的泛型代码。
...
> 详见 [L4 形式化验证](../../04_formal/02_separation_logic/01_rustbelt.md) §2 "索引类型与依赖类型的边界"。
...
> **跨层映射**: 类型状态机的编译期保证 ↔ [L3 异步状态机](../../03_advanced/01_async/01_async.md) §3 "async/await 状态转换" 的运行时状态机形成对比：前者将状态合法性证明推至编译期，后者在运行时管理状态。Const Generics 的类型状态机是**零运行时开销**的，所有状态转换在类型层面完成验证。
...
边界极限测试探测的是泛型机制在**规模与嵌套深度**上的实际极限，而非语法正确性。三组测试各对应一种真实工程约束：单态化代码膨胀测试对比泛型与 `dyn Trait` 在实例数量增长时的二进制体积与缓存行为，回答「何时该用动态分发主动放弃单态化」；生命周期递归传递测试把 HRTB（`for<'a> F: Fn(&'a T)`）嵌套到闭包（Closures）与 trait 对象边界，标定类型检查器可推断的深度；Const Generics 类型级运算测试考察编译期常量表达式（如 `N * 2`、`N - 1`）在当前稳定版支持到何种复杂度。运行这些测试时注意：它们中的一部分**预期失败或预期触发编译器限制**，结果应记录为能力边界而非缺陷。
...
> **[Rust Performance Book: Compile Times](https://nnethercote.github.io/perf-book/compile-times.html)** · **[rustc dev guide](https://rustc-dev-guide.rust-lang.org/overview.html)** 泛型的单态化在编译期生成大量代码，是 Rust 编译时间的主要瓶颈之一。以下策略可在工程实践中缓解这一问题。
...
> **来源**: [Rust Performance Book](https://nnethercote.github.io/perf-book/compile-times.html) · [rustc dev guide: Monomorphization](https://rustc-dev-guide.rust-lang.org/overview.html)
...
> **来源**: [Rust Performance Book: Compile Times](https://nnethercote.github.io/perf-book/compile-times.html) · [rustc dev guide: LTO](https://rustc-dev-guide.rust-lang.org/overview.html) · [LLVM Blog: ThinLTO](https://blog.llvm.org/2016/06/thinlto-scalable-and-incremental-lto.html)
...
> [../00_traits/07_generic_associated_types.md](../00_traits/07_generic_associated_types.md)；本节保留泛型系统中的形式化定位。
...
1. HKT 会显著增加类型系统（Type System）的认知复杂性和编译器实现成本；
...
| 需要编译期已知的长度/容量 | const generics（见 [02_const_generics.md](02_const_generics.md)） | 泛型参数空间（类型/生命周期/const） | 需要常量算术（`N+1`）⟹ `generic_const_exprs` 未稳定 |
| 关联类型每个实现唯一 | 关联类型 | 泛型参数空间对比 | 每实现需多个 ⟹ 改用泛型参数 |
| 关联类型需借用（Borrowing） `Self` 数据 | GAT（见 [40_generic_associated_types.md](../00_traits/07_generic_associated_types.md)） | GAT 规则 | 用 HRTB 绕行可行但表达力受限 |
...
| 概念 | 文件 | 关系 |
|:---|:---|:---|
| Const Generics（常量泛型） | [02_const_generics.md](02_const_generics.md) | const 参数语法与 stable 边界的权威页 |
...
| 类型系统基础 | [01_foundation/02_type_system/01_type_system.md](../../01_foundation/02_type_system/01_type_system.md) | 泛型的理论前提 |
| 异步（Async）与 Future | [03_advanced/01_async/01_async.md](../../03_advanced/01_async/01_async.md) | 关联类型泛型（GATs）的典型应用 |
| 并发与 Send/Sync | [03_advanced/00_concurrency/01_concurrency.md](../../03_advanced/00_concurrency/01_concurrency.md) | 泛型约束的线程安全应用 |
| 形式化验证 | [04_formal/04_rustbelt.md](../../04_formal/02_separation_logic/01_rustbelt.md) | 泛型系统的逻辑基础 |
| Rust 版本特性演进 | [07_future/05_rust_version_tracking.md](../../07_future/00_version_tracking/01_rust_version_tracking.md) | `use<..>` precise capturing 等类型系统扩展 |
| 异步与泛型 | [03_advanced/01_async/01_async.md](../../03_advanced/01_async/01_async.md) | `AsyncFn` trait 家族、GATs 在 async 中的应用 |
...
- **`min_specialization`**：允许在「更具体 impl 是更通用 impl 的子集」时提供专用实现（如标准库内部 `Vec<T>` 从 `&[T]: Copy` 切片（Slice）克隆时的特化）。当前仍是 nightly 特性，卡点在于特化与生命周期交互的可靠性证明（soundness），稳定化没有时间表。
- **编译时间优化**：单态化的代价是编译期与二进制体积，演进方向包括共享单态化（相同布局的实例共用代码）、更积极的 `-Zshare-generics` 行为，以及通过 `dyn` 边界的显式设计减少实例数量。
- **Type-level Programming**：`generic_const_exprs` 推进后，`[T; N + 1]` 这类依赖常量参数的运算将逐步可用，把定长运算从宏（Macro）技巧转为类型系统原生能力。
- **`impl Trait` 的位置扩展**：返回位置 `impl Trait`（RPIT）与参数位置的本质差异（存在类型 vs 全称量词糖）已稳定，`impl Trait` in trait（RPITIT）自 1.75 稳定后，async trait 的静态分发写法成为新基线。

选择建议：库作者应以稳定特性为公共 API 边界，nightly 特性限于内部实现并隔离在 feature flag 之后。
...
> 详见 [§9.2 补充：`min_specialization` 的当前状态与使用](#92-补充min_specialization-的当前状态与使用) 与 [Traits 中的 Specialization](../00_traits/04_advanced_traits.md)。
...
> 详见 [§9.4 补充：Type-level Programming](#94-补充type-level-programmingpeano-算术与-typenum) 与 [类型级编程](03_type_level_programming.md)。
...
> 详见 [§9.5 补充：GATs 的完整形式化视角](#95-补充generic-associated-types-gats-的完整形式化视角) 与 [高级 Traits](../00_traits/04_advanced_traits.md)。
...
泛型规则的编译错误围绕「约束-实例一致性」展开，按约束失效的位置分类：

- **泛型参数未约束**（E0277）：函数体内使用了 bound 未声明的操作（如对裸 `T` 调用 `+`）。Rust 在定义处检查泛型体——「先检查后实例化」使错误永远指向泛型定义而非遥远的调用点（与 C++ 模板错误信息的位置差异）。
- **递归类型的大小无限**（E0072）：`struct S { next: S }` 无间接层，大小不可计算。修复是引入 `Box`/`Rc`——类型大小的可判定性优先于表达便利。
- **关联类型实现不一致**：impl 中关联类型的取值与 trait 声明/其他 impl 冲突，或 `type Item` 漏定义（E0046）。关联类型是「每个 impl 确定一次」的承诺，不一致直接破坏该承诺。
- **泛型默认类型参数不满足约束**：`struct S<T = f64> where T: Ord` 中 `f64: !Ord`，默认参数自身须满足声明的全部约束——默认值不豁免检查。

判定泛型错误的统一方法：把「约束集合」与「使用操作集」对照——错误 ⟺ 存在操作不在约束声明的能力集中，或实例类型不满足约束。九个边界测试是这条对照规则在不同语法位置的展开。
...
五道嵌入式测验沿「定义 → 实例化 → 约束 → 编译模型」递进：测验 1 核对泛型函数中类型参数的推断来源（调用处实参而非返回值）；测验 2 要求为泛型结构体写出正确的 `impl<T>` 块（注意 `impl` 后必须重复声明类型参数）；测验 3 考察多个 trait bound 的 `+` 组合与 `where` 子句写法；测验 4 给出两段等价逻辑（泛型版与 `dyn` 版），要求分析单态化带来的二进制体积差异与各自适用场景。错题回查：测验 3 出错看 5.1 节的约束写法，测验 4 出错看第七节的膨胀测试结论。
...
本页的知识结构由概念定义（泛型 = 参数化的类型与函数）、属性特征（编译期单态化、零成本、定义处约束检查）、关系连接（上位：参数多态；对偶：`dyn Trait` 动态分发；组合：trait bound + 生命周期 bound）、思维导图（泛型语法的四个作用位置：函数、结构体、枚举（Enum）、impl）四部分构成，另附术语对齐与权威来源映射，作为本层泛型主题的导航枢纽。
...
类型状态模式（Typestate Pattern）把状态机的每个状态编码为独立的零大小类型，状态转移实现为**消费 `self`、返回下一状态类型**的方法。其本质是利用所有权（Ownership）系统的移动语义：旧状态值被消费后不可再用，非法转移（如在 `Closed` 状态调用 `write`）在类型层面根本不存在可调用的方法，从而在编译期排除「状态-操作不匹配」类错误。代价是每个状态-操作组合都要写一个 `impl` 块，状态多时模板代码膨胀——可用泛型 + 标记 trait 或宏缓解。典型应用：文件句柄（`Closed → Open → Reading`）、网络连接握手、构建器（builder）的必填字段检查。下面的基础类型状态示例给出最小骨架。
