...
> **后置概念**: [Async/Await](../../03_advanced/01_async/01_async.md)
---
...
**知识点**：trait bound 是 Rust 泛型（Generics）的"类型类约束"，编译器通过它进行单态化（Monomorphization）生成具体代码。(Source: [Rust Reference: Generic Parameters](https://doc.rust-lang.org/reference/items/generics.html))[→ 泛型详解](01_generics.md)
...
本节围绕「泛型（Generics）与单态化（Monomorphization）」展开，覆盖 Q4. 以下代码能否编译？`Option<T>` 和 `Result<… 与  Q5. 以下代码能否编译？解释关联类型的作用 两个方面。
...
**知识点**：关联类型减少泛型（Generics）参数的冗余，使 trait 方法签名更简洁。[→ 高级 Trait 详解](../00_traits/04_advanced_traits.md)
...
**知识点**：`?Sized` 是 Rust 中少有的 "opt-out" 约束，用于处理编译期大小未知的类型（DST）。[→ 泛型详解](01_generics.md)
...
本节围绕「综合应用」展开，依次讨论 Q8. 以下代码能否编译？解释 `impl Trait` 作为返回类型…、Q9. 以下代码的输出是什么？解释泛型结构体（Struct）的实现与Q10. 以下代码能否编译？解释 `Copy` 和 `Clone` 的…。
...
**知识点**：`impl Trait` 是语法糖，编译器在编译期确定具体类型。它不提供运行时（Runtime）多态能力。[→ 泛型详解](01_generics.md)
...
**知识点**：Rust 允许为泛型结构体（Struct）的特定实例类型提供额外方法，这是零成本抽象（Zero-Cost Abstraction）的典型案例。[→ 泛型详解](01_generics.md)
...
> [Traits](../00_traits/01_traits.md)、[Generics](01_generics.md) 权威页一致；
> 干扰项针对常见误解设计。

### Q11. 🟢【单选】Rust 泛型在编译时通过什么机制实现？

- A. 运行时（Runtime）类型擦除，所有泛型共享一份代码
- B. 单态化（monomorphization）：为每个实际使用的类型组合生成专用代码
- C. 动态分发，通过 vtable 调用
- D. 宏（Macro）展开，按字面替换
...
### Q12. 🟡【单选】`impl Trait`（作为参数/返回类型）与 `dyn Trait` 的核心区别是？

- A. 完全相同，只是新旧两种写法
- B. `impl Trait` 是静态分发（编译期确定唯一具体类型），`dyn Trait` 是动态分发（运行期经 vtable 查找）
- C. `dyn Trait` 性能更好，因为有内联缓存
- D. `impl Trait` 可以用来构建 trait 对象并存入集合
...
**解析**：`fn f(x: impl Draw)` 等价于泛型 `fn f<T: Draw>(x: T)`，单态化、零分发成本，但一次调用只接受一个具体类型；`&dyn Draw` 是胖指针（数据 + vtable），可异构混合（`Vec<Box<dyn Draw>>`），代价是运行期间接调用且失去部分优化机会。D 恰好把两者弄反——异构集合正是 `dyn` 的主场。

</details>

---

### Q13. 🟡【多选】关于关联类型（associated type）与泛型 trait 参数，下列说法正确的有？（选出所有正确项）

- A. 同一类型实现带关联类型的 trait 时，每个关联类型只能指定一次
- B. 泛型 trait 参数允许同一类型以不同参数多次实现同一 trait（如 `From<i32>` 与 `From<String>`）
- C. 关联类型适合"实现者与该类型一一对应"的场景（如 `Iterator::Item`）
- D. 使用关联类型必然引入动态分发
...
**解析**：关联类型由 impl 唯一确定（A），泛型参数则允许同一 trait 的多个实例化并存（B）——这是两者最实质的取舍；当"一个实现天然只对应一个产出类型"时选关联类型，API 更简洁（C）。D 错：关联类型与泛型一样是**静态**解析的，`Iterator::Item` 的单态化迭代没有任何 vtable。

</details>

---

### Q14. 🟢【判断】孤儿规则（orphan rule）要求：为某类型实现某 trait 时，trait 与类型至少有一方必须定义在当前 crate 中。（对 / 错）
...
**解析**：孤儿规则（Orphan Rule）保证任意 crate 组合下 trait 实现不会冲突（coherence）：你可以"为自己的类型实现外部 trait"（如为本地类型实现 `Serialize`），也可以"为外部类型实现自己的 trait"，但不能"为外部类型实现外部 trait"（如给 `Vec<T>` 实现 `Display`）。需要突破时可用 newtype 模式包装外部类型。

</details>

---

### Q15. 🔴【判断】单态化意味着泛型代码零运行时开销，代价是编译时间增加与二进制体积膨胀（每个类型组合一份机器码）。（对 / 错）
...
**解析**：这是"零成本抽象（Zero-Cost Abstraction）"的完整图景：运行时无分发、无装箱、可内联优化（收益）；每个 `T` 组合都实例化一份代码（成本，即 code bloat）。工程上可用"泛型外壳 + 非泛型内核"或 `dyn Trait` 在需要时换取更小体积。注意"零成本"限定的是**运行时**，编译期成本是真实存在的——这也是大型泛型项目编译慢的根源之一。
...
| 得分 | 评价 | 建议 |
|:---:|:---|:---|
| 10/10 | 🏆 Trait/泛型已内化 | 进阶至 [L3 高级泛型](../../03_advanced/02_unsafe/01_unsafe.md) 或 [L4 类型理论](../../04_formal/00_type_theory/01_type_theory.md) |
| 7–9/10 | ✅ 核心概念掌握 | 强化 [L2 练习](../../exercises/src/generics_traits)，关注错题对应的概念文件 |
| 4–6/10 | 🔄 需巩固基础 | 重读 [Traits](../00_traits/01_traits.md) · [Generics](01_generics.md)，完成 rustlings 对应章节 |
