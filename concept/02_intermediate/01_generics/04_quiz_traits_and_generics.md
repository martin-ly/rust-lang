...
**解析**：孤儿规则保证任意 crate 组合下 trait 实现不会冲突（coherence）：你可以"为自己的类型实现外部 trait"（如为本地类型实现 `Serialize`），也可以"为外部类型实现自己的 trait"，但不能"为外部类型实现外部 trait"（如给 `Vec<T>` 实现 `Display`）。需要突破时可用 newtype 模式包装外部类型。
...
**解析**：这是"零成本抽象"的完整图景：运行时无分发、无装箱、可内联优化（收益）；每个 `T` 组合都实例化一份代码（成本，即 code bloat）。工程上可用"泛型外壳 + 非泛型内核"或 `dyn Trait` 在需要时换取更小体积。注意"零成本"限定的是**运行时**，编译期成本是真实存在的——这也是大型泛型项目编译慢的根源之一。
