...
> **前置概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md)
> **后置概念**: [Macros](../../03_advanced/03_proc_macros/01_macros.md) · [Cargo Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md)
...
- [模块系统高级主题：可见性规则与工程实践](#模块系统高级主题可见性规则与工程实践)
  - [📑 目录](#-目录)
  - [一、核心概念](#一核心概念)
...
> **关键洞察**: 一个 Package 可包含**多个 Crate**（1 个 lib + 多个 bin + tests + examples + benches），但每个 Crate 是独立编译的单元 (Source: [TRPL Ch7 — Packages and Crates](https://doc.rust-lang.org/book/ch07-01-packages-and-crates.html), [Cargo Book](https://doc.rust-lang.org/cargo/index.html))。
...
> **关键洞察**: `pub(crate)` 是 Rust 模块系统的**最佳实践**——它允许 crate 内部任意模块访问，但对外部 crate 隐藏。这比 C++ 的 `friend` 或 Java 的包可见性更精确 (Source: [Rust Reference — Visibility and Privacy](https://doc.rust-lang.org/reference/visibility-and-privacy.html))。
...
- **反命题 1：「`pub use` 只是语法糖」** —— 不准确。`pub use` 是**重导出**：它改变项的公开路径，是 API 设计工具（lib.rs 用 `pub use` 把深层模块的内部结构压平成扁平公共 API），同时影响文档生成与 semver 兼容性判断。
- **反命题 2：「`pub(crate)` 与私有等价于安全」** —— 过度。可见性控制的是**名称可达性**，不是安全边界：同 crate 内 `unsafe` 代码仍可越界访问私有数据，且 `pub(crate)` 项在内部重构自由度上优于 `pub`，但不提供任何运行期保护。
- **反命题 3：「模块必须对应文件」** —— 错误。内联 `mod foo { ... }` 不创建文件；`#[path = "..."]` 可改变文件映射。模块是**命名空间与可见性边界**，文件布局只是其实现方式。

边界极限小节量化：循环 `use` 的合法形态、glob 导入的歧义优先级（显式项 > glob 项）、以及 `pub(in path)` 的精确授权场景。
...
- **上层概念**: [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md)
- **下层概念**: [Macros](../../03_advanced/03_proc_macros/01_macros.md) · [Cargo Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md)

- [Cargo Toolchain](../../06_ecosystem/00_toolchain/01_toolchain.md) — Cargo 与 Workspace
- Macros — 宏（Macro）与模块交互
- [Traits](../00_traits/01_traits.md) — Trait 的可见性设计

---

> **权威来源**: [Rust Reference — Modules](https://doc.rust-lang.org/reference/items/modules.html), [Rust Reference — Visibility and Privacy](https://doc.rust-lang.org/reference/visibility-and-privacy.html), [The Rust Programming Language](https://doc.rust-lang.org/book/ch07-00-managing-growing-projects-with-packages-crates-and-modules.html), [Cargo Book](https://doc.rust-lang.org/cargo/index.html), [Rust Edition Guide — Path Changes](https://doc.rust-lang.org/edition-guide/rust-2018/path-changes.html)
>
...
> **对应 Crate**: [`c09_design_pattern`](../crates/c09_design_pattern) · [`c11_macro_system_proc`](../crates/c11_macro_system_proc)
>
...
- **理解层**：`pub`/`pub(crate)`/`pub(super)`/私有的四级可见性在模块树中的实际可达范围——作答时建议先画出模块树再标注每级的边界；
- **应用层**：`pub use` 重导出的 API 塑形——给定内部模块结构，设计对外扁平 API 的重导出方案；
- **分析层**：可见性错误诊断——E0603（私有项导入）与 E0432（未解析导入）的根因判别：前者是权限问题，后者是路径问题，修复策略完全不同。

常见失分点：忘记 `use` 引入的名字默认是私有的（外部 `use` 不到），需要 `pub use` 才能再导出。
