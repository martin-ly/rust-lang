...
> **认知功能**: 编译期 vs 运行时参数化决策工具。需要值参数化类型时，先沿决策树判断 Const Generics 是否适用，避免在边界外强行使用。关键洞察：Const Generics 是依赖类型的有限形式，有明确的类型论边界——理解边界才能正确选择 typenum、枚举或运行时检查等替代方案。[来源: 💡 原创分析]
...
`typenum` 的运算基于**类型级运算符 trait**（`operator_aliases` 模块）：
