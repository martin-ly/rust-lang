...
> **主要来源**: [TRPL: Ch4.1-4.3](https://doc.rust-lang.org/book/ch04-01-what-is-ownership.html) ·
> [TRPL: Ch15](https://doc.rust-lang.org/book/ch15-00-smart-pointers.html) ·
> [Rust Reference: Memory Model] ·
...
> **[Wikipedia: Smart pointer]** `Box<T>` provides the simplest form of heap-allocated smart pointer with unique ownership, analogous to `std::unique_ptr` in C++. ✅ 已验证
...
> **[TRPL: Ch15]** The following smart pointer comparison matrix summarizes ownership, mutability, and thread-safety trade-offs covered in TRPL Chapter 15. ✅ 已验证
...
| `RefCell<T>` | 运行时借用检查 | 内部可变性 | 若 `T: Send`，❌ !Sync | 单线程内部可变 |
...
> **[TRPL: Ch15] · [std docs: Cell/RefCell/Mutex/RwLock]** 内部可变性模式将编译期借用检查移至运行时或硬件层，是所有权系统的重要扩展。 ✅
...
> **[Wikipedia: Reference counting]** Reference counting enables multiple owners of heap-allocated data; `Arc` extends this with atomic operations for thread-safe concurrent access. ✅ 已验证
...
> **[Rust Reference: Interior mutability]** `RefCell<T>` implements the interior mutability pattern by deferring borrow checking to runtime, allowing mutation through shared references in single-threaded contexts. ✅ 已验证
...
> **[Rust Reference: Pin] · [RFC 2349]** 栈 `Pin<&mut T>` 的生命周期受借用约束，堆 `Pin<Box<T>>` 通过 `Box::pin` 获得长期地址稳定性。 ✅
...
> **[Wikipedia: Dynamic array] · [Rust Reference: Vec]** 动态数组的指数扩容策略是均摊 O(1) 的标准实现，扩容因子通常在 1.5–2× 之间。 ✅

#### `String`：UTF-8 字节数组的特化

> **[Rust Reference: String]** `String` 是 `Vec<u8>` 的包装，保证内部字节始终为合法 UTF-8。 ✅
...
> **[Wikipedia: Hash table] · [std docs: HashMap]** Rust `HashMap` 采用 Robin Hood 哈希 + 线性探测，将探测距离的方差控制在较低水平。 ✅
...
> **[TRPL: Ch15] · [Rustonomicon: Ownership]** 反命题分析覆盖编译期、运行时、语义和工程四个层面。 ✅
...
> **[TRPL: Ch15] · [Rust Reference: Safety] · [Rustonomicon]** Rust 不将内存泄漏视为 unsafe，这是明确的设计决策。 ✅
...
> **[TRPL: Ch15] · [std docs: RefCell]** RefCell 与编译期借用检查在安全性上等价，但在错误处理时机和运行时开销上有本质差异。 ✅
...
> **[TRPL: Ch16] · [std docs: Mutex/Arc]** `Arc<Mutex<T>>` 阻止数据竞争，但不阻止死锁或 poisoning——后者属于并发正确性而非内存安全范畴。 ✅
...
> **[TRPL: Ch15] · [std docs: Box/Rc/RefCell]** 智能指针的核心语法覆盖堆分配、共享所有权和内部可变性三种基本模式。 ✅
...
> **[TRPL: Ch15] · [Rustonomicon: Interior Mutability]** 编译期检查是静态证明（零开销但保守），RefCell 是动态监控（运行时开销但更灵活）。 ✅
...
> **[TRPL: Ch16] · [std docs: Arc/Mutex]** `Arc<T>` 是 `Rc<T>` 的线程安全版本（原子计数），`Mutex<T>` 是 `RefCell<T>` 的线程安全版本（互斥锁）。 ✅
...
> **[Rust Reference: Safety] · [Rustonomicon: Leaking]** Rust 的安全保证明确排除内存泄漏——防止泄漏需要 GC 或更强的类型约束，Rust 选择将其排除在 unsafe 定义之外。 ✅
...
> **[Rust Reference: ManuallyDrop] · [Rust Reference: std::mem::forget] · [Rustonomicon: Drop flags]** `mem::forget` 运行时阻止析构，`ManuallyDrop` 编译期禁用 drop 标志，两者语义等价但使用场景不同。 ✅
...
> **[Rust Reference: Vec]** `Vec<T>` 的内存布局由指针、长度和容量三元组构成，与 C++ `std::vector` 概念类似。 ✅
...
> **[Wikipedia: Dynamic array] · [Rust Reference: Vec]** 动态数组的均摊 O(1) push 是算法分析的经典结论。 ✅
...
> **[Rust Reference: String]** `String` 是 `Vec<u8>` 的包装，附加 UTF-8 有效性不变式。 ✅
...
> **[std docs: HashMap] · [Wikipedia: Hash table]** SwissTable 通过 SIMD 并行查找和分离的 control bytes 提升缓存友好性，自 Rust 1.36 起成为标准库默认实现。 ✅
...
> **[RFC 2349] · [Rust Reference: Pin]** Pin 投影保持 `!Unpin` 类型的地址稳定性，是自引用结构在 Safe Rust 中安全表达的基础。 ✅
...
> **[Rust Project Goals 2026 — Beyond the &] · [RFC: Arbitrary Self Types v2]** Field Projection 允许所有智能指针直接访问字段而不丢失容器语义。 ✅
