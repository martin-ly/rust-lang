...
- [Memory Management（内存管理）](#memory-management内存管理)
  ...
  - [四、定理推理链（Theorem Chain）](#四定理推理链theorem-chain)
    - [4.1 引理：Box ⟹ 堆分配 + 唯一所有权](#41-引理boxt--堆分配--唯一所有权)
  ...
  - [七、边界极限测试代码（Boundary Limit Tests）](#七边界极限测试代码boundary-limit-tests)
    - [7.1 测试 1: Rc\<RefCell\> 循环引用极限](#71-测试-1-rcrefcellt-循环引用极限)
...
| 类型系统（Type System）基础 | [01_foundation/04_type_system.md](../01_foundation/04_type_system.md) | 智能指针的类型约束 |
