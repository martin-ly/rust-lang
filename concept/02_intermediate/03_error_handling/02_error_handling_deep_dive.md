> **内容分级**: [综述级]
> **本节关键术语**: 错误处理深入 (Error Handling Deep Dive) · 错误链 (Error Chain) · 回溯 (Backtrace) · anyhow · 错误转换 — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
每条反命题都附可复现的临界代码，验证方式：把命题中的「永远/所有」替换为具体场景谓词，构造谓词为假的实例。
...
本节的边界用例覆盖错误处理库生态的典型失败点：

- **thiserror 与 anyhow 混用**：`#[from]` 自动转换与 anyhow 的 `Context` 叠加导致错误链重复包装——判定准则：库边界用 thiserror（具体类型），应用层汇总用 anyhow，混用点明确转换；
- **`Result` 嵌套与 `?` 传播限制**：`Result<Result<T, E>, E>` 需要 `?` 两次或 `flatten`——嵌套通常是设计信号（应合并错误变体）；
- **`#[from]` 类型歧义**：多个字段类型相同（如两个 `io::Error` 来源）使 derive 无法推断——需 `#[from]` 显式标注或手写 `From`；
- **eyre 与 anyhow 混用**：两个「应用层错误」库上下文不互通，`Report` 与 `Error` 互转丢失 context 链——一个应用只选一个；
- **`source()` 链循环**：手写 `Error::source` 形成环导致 `Display` 递归打印栈溢出（运行期）——错误链必须是有向无环图。

每组用例给出最小复现与修复，错误链设计是本节核心考点。
