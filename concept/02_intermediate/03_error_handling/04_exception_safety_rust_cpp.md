...
> **后置概念**:
> [Error Handling Deep Dive](02_error_handling_deep_dive.md) ·
...
> [Rust Reference — Runtime and Unwinding](https://doc.rust-lang.org/reference/runtime.html) ·
...
对照要点：Rust 的「异常安全」不需要分级承诺——`Result` 把错误处理编码进类型（编译器执行），panic + Drop 把清理语义内建（RAII 在 unwind 中同样运行）。C++ 靠纪律的三级保证，在 Rust 中前两级由类型系统（Type System）覆盖，第三级对应「不 panic 的代码」这一可审计属性。
...
- [对应测验](../08_quizzes/30_quiz_cpp_rust_foundations.md) — C/C++ → Rust 工程层基础对比（RTTI、宏（Macro）、异常安全、构造、move 语义）
