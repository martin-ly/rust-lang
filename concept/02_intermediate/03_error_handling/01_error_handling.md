...
错误处理（Error Handling）的权威定义在三个层面上递进：
...
| `Box<dyn Error>` vs 具体错误枚举（Enum） | 库边界用具体枚举（可穷尽匹配）；应用层汇总用 `Box<dyn Error + Send + Sync>` 或 `anyhow` |
...
本节把「Error Handling（错误处理）」的规则推到编译器与运行时（Runtime）的边界上逐一实测：测试 1: ? 运算符在闭包中的限制、测试 2: From 转换链的边界、测试 3: panic 边界与 catch_unwind与测试 4: Result 与 Option 的组合边界。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
**跨层映射**: Backtrace 的运行时成本 ↔ [§9.3.6](#936-与-paniclocation-的对比) `Location` 的编译期零成本 ↔ [../04_formal/04_rustbelt.md](../../04_formal/02_separation_logic/01_rustbelt.md) §3 "运行时与编译期保证的边界"
...
**跨层映射**: `#[track_caller]` 的编译期定位 ↔ [§9.3](#93-stdbacktracebacktrace-与错误追踪) `Backtrace` 的运行时定位 ↔ [../04_formal/04_rustbelt.md](../../04_formal/02_separation_logic/01_rustbelt.md) §3 "编译期保证与运行时观察的边界"
...
| 泛型（Generics）系统 | [02_generics.md](../01_generics/01_generics.md) | Result<T, E> 的泛型参数约束 |
| 所有权（Ownership）与生命周期（Lifetimes） | 01_foundation/01_ownership_borrow_lifetime/01_ownership.md | panic 时的资源清理 |
| 类型系统（Type System）基础 | [01_foundation/02_type_system/01_type_system.md](../../01_foundation/02_type_system/01_type_system.md) | 和类型的理论基础 |
| 并发与 Send/Sync | [03_advanced/00_concurrency/01_concurrency.md](../../03_advanced/00_concurrency/01_concurrency.md) | 跨线程错误传播 |
| 异步与 Future | [03_advanced/01_async/01_async.md](../../03_advanced/01_async/01_async.md) | async 中的 ? 运算符 |
| 形式化验证 | [04_formal/04_rustbelt.md](../../04_formal/02_separation_logic/01_rustbelt.md) | 错误处理的逻辑基础 |
...
| 安全级别分类 | basic/strong/nothrow 三级保证，靠程序员文档承诺 | 由类型系统（Type System）承担：`Result` 强制处理，`panic!` 对应「bug 级」失败 |
...
错误处理的编译错误集中在「`?` 传播链的类型一致性」上，按失效点分类：

- **`?` 在错误类型不匹配时使用**（E0277）：`From<E1> for E2` 缺失——`?` 的隐式转换无路径。修复三选一：手写 `From` impl、`thiserror` 的 `#[from]` 派生、或 `map_err` 显式转换。错误信息会给出期望的 trait 路径。
- **`panic` 在 `const fn` 中**（E0015/E0493）：编译期求值语境无 unwinding 机制，stable 上 `const fn` 的 panic 仅限字面量消息的受限形式（`const_panic` 已部分稳定：`panic!("literal")` 可用，格式化参数不行）。
- **`Result` 未处理**（`#[must_use]` 警告）：不是硬错误但 lint 默认 warn——`Result` 标注 `#[must_use]`，丢弃返回值触发警告；生产代码应 `-D warnings` 升级为错误。
- **`?` 在闭包中的类型推断失败**：闭包返回类型由推断确定，`?` 要求返回 `Result`/`Option`，但闭包其他路径返回裸值时推断失败——显式标注闭包返回类型或改用函数解决。

统一判定法：错误传播链上的每个 `?` 都隐含一次 `From` 转换，链合法 ⟺ 每次转换的 impl 存在且唯一——九个边界测试是这条规则在语法变体上的展开。
...
> **相关问题树**: [错误处理问题树](../../00_meta/04_navigation/10_problem_graph.md#七错误处理问题树)
