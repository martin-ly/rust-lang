...
当 `Option` 与 `Result` 在类型中嵌套时，直接匹配会写出多层 `match`，既冗长又容易颠倒语义。`transpose` 与 `flatten` 提供了一种在**不改变容器语义的前提下重新组织嵌套结构**的方式，使错误处理与可选值能够平滑地组合。掌握这两个组合子，对于处理批量解析、配置合并等场景尤其关键。
...
Rust 的错误处理不仅需要正确的类型转换，还需要在**库代码**与**应用代码**之间建立清晰的分层。库层需要稳定、可恢复的错误类型，因此使用 `thiserror` 生成精确枚举；应用层则更关注快速传播与可读诊断，因此使用 `anyhow` 承载上下文链。两者结合，既保留了类型系统的严谨性，又兼顾了工程效率。
...
即使掌握了正确的组合子，实际编码中仍容易因追求简洁而破坏错误链、重复求值或混淆分层。以下反例展示了错误处理中最常见的陷阱：信息丢失、惰性求值被绕过，以及 anyhow 与 thiserror 的混用。理解这些陷阱有助于在代码审查中识别潜在问题，并写出更稳健的错误传播策略。
...
边界测试用于验证错误处理组合子在极端或容易误解的场景下是否仍然符合预期。通过观察 `ok_or_else` 的惰性、`transpose` 对 `Some(Err)` 的处理，以及错误链成环的后果，可以加深对所有权、求值顺序和类型转换的理解。
