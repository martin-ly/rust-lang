...
- **对偶**：静态分派（泛型单态化（Monomorphization））⇄ 动态类型识别（类型擦除 + 运行时下转）。
...
`downcast_ref::<T>` 要求 `T: 'static`；含非 `'static` 生命周期（Lifetimes）的引用类型无法通过约束。Rust 的 RTTI 刻意排除含生命周期的类型，避免运行期类型信息与借用（Borrowing）检查脱节。
