...
1. **`Any` trait**：所有 `'static` 类型自动实现——`Box<dyn Any>` 是类型擦除的容器，提供 `is::<T>()`/`downcast()` 系列方法。`'static` 约束是关键限制：含非静态引用（Reference）的类型无法擦除（防止悬垂的类型识别）；
...
反模式警示：用 `Any` 实现「函数重载」（按运行时类型分派逻辑）——这在 Rust 中应优先用 `enum`（封闭集合）或泛型（Generics）（编译期分派）；`Any` 分派只在类型集合**运行时开放**（插件、FFI 边界）时合理。
...
- **对偶**：静态分派（泛型（Generics）单态化（Monomorphization））⇄ 动态类型识别（类型擦除 + 运行时下转）。
- **组合**：与 [类型擦除](../../03_advanced/06_low_level_patterns/03_type_erasure.md)、[错误处理（Error Handling）深入](../03_error_handling/02_error_handling_deep_dive.md)（错误类型的动态擦除）组合。
...
`downcast_ref::<T>` 要求 `T: 'static`；含非 `'static` 生命周期（Lifetimes）的引用（Reference）类型无法通过约束。Rust 的 RTTI 刻意排除含生命周期的类型，避免运行期类型信息与借用（Borrowing）检查脱节。
