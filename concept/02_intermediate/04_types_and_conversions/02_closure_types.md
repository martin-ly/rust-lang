...
- **`move` 关键字的作用**：强制闭包按值捕获所有用到的变量（所有权转移进闭包结构体），而非默认的「按借用推断」。`move` 不改变闭包实现的 trait 集合（仍由闭包体决定），只改变环境的存放方式——典型用途是把捕获移入新线程（`'static` 要求）或延长环境生命期至闭包生命期。
