...
判定一个闭包的完整类型画像，按顺序回答：捕获了什么（字段）→ 闭包体如何使用捕获（trait 集）→ 是否 `move`（环境所有权）→ 传给什么签名（泛型/指针/dyn 决定分派方式）。
...
- **反命题 2：「`move` 闭包总是必要的」** —— 过度。`move` 只在两种情况下必要：① 闭包需 `'static`（`thread::spawn`/`tokio::spawn`）——必须拥有捕获值而非借用；② 需要按值捕获而闭包体只需借用（如捕获 `String` 但只调用 `.len()`）。滥用 `move` 导致外层变量过早失效——判定准则是「闭包是否比环境活得久」。
