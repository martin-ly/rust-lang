...
> **后置概念**: [Async](../../03_advanced/01_async/01_async.md) ·
> [Generics](../01_generics/01_generics.md)
...
闭包的技术细节集中在「编译器如何为闭包生成类型与调用代码」：

- **编译器自动推导规则**：每个闭包表达式生成一个唯一的匿名结构体类型，捕获的环境成为其字段；`Fn`/`FnMut`/`FnOnce` 三个 trait 按「闭包体对捕获的使用方式」自动实现——只读捕获 → `Fn`，修改捕获 → `FnMut`，消耗（move 出）捕获 → `FnOnce`。推导取「能满足体的最弱 trait」：一个 `FnOnce` 闭包不会因调用方只需 `Fn` 而变强。
- **闭包与函数指针**：不捕获环境的闭包可强制转换为 `fn` 指针（零成本，类型擦除）；捕获环境的闭包与 `fn` 指针不兼容——后者无处存放环境。泛型参数 `F: Fn()` 保持闭包的独特类型（可内联），`&dyn Fn()`/`Box<dyn Fn()>` 做类型擦除（一次间接调用）。
- **`move` 关键字的作用**：强制闭包按值捕获所有用到的变量（所有权（Ownership）转移进闭包结构体），而非默认的「按借用（Borrowing）推断」。`move` 不改变闭包实现的 trait 集合（仍由闭包体决定），只改变环境的存放方式——典型用途是把捕获移入新线程（`'static` 要求）或延长环境生命期至闭包生命期。

判定一个闭包的完整类型画像，按顺序回答：捕获了什么（字段）→ 闭包体如何使用捕获（trait 集）→ 是否 `move`（环境所有权（Ownership））→ 传给什么签名（泛型/指针/dyn 决定分派方式）。
...
- **反命题 1：「闭包和函数指针可以互换」** —— 有严格边界。`fn(i32) -> i32` 指针是固定大小的类型（可存储、可跨 FFI），闭包是匿名结构体（大小 = 捕获环境）——只有**无捕获**闭包能强制为 `fn` 指针，捕获环境的闭包永远不行。`Box<dyn Fn>` 是捕获闭包的统一存储方案，代价是堆分配 + 间接调用。
- **反命题 2：「`move` 闭包总是必要的」** —— 过度。`move` 只在两种情况下必要：① 闭包需 `'static`（`thread::spawn`/`tokio::spawn`）——必须拥有捕获值而非借用（Borrowing）；② 需要按值捕获而闭包体只需借用（如捕获 `String` 但只调用 `.len()`）。滥用 `move` 导致外层变量过早失效——判定准则是「闭包是否比环境活得久」。

边界极限小节量化：`Fn`/`FnMut`/`FnOnce` 的自动实现规则、闭包递归的不可直接表达、以及 `impl Fn` 返回位置的生命周期捕获。
...
- [Async](../../03_advanced/01_async/01_async.md) — 异步编程（async 块是特殊闭包）
- [Generics](../01_generics/01_generics.md) — 泛型与参数多态
...
闭包的编译错误本质是「环境、trait、生命周期」三方契约的违约，按违约方分类：

- **闭包类型推断与 `fn` 指针不兼容**：捕获环境的闭包传给 `fn()` 参数报「expected fn pointer, found closure」——修复是改签名为泛型 `F: Fn()` 或 `&dyn Fn()`。这是「捕获即改变类型类别」的直接体现。
- **`Fn` trait 的自动实现与 `move` 闭包**：误以为 `move` 使闭包只能 `FnOnce`——实际上 `move || x + 1`（只读使用捕获的整数）仍实现 `Fn`，可多次调用；`move` 只影响「捕获如何进入闭包」，trait 实现由闭包体决定。反向错误（假设非 `move` 闭包 `'static`）在 `thread::spawn` 处报 E0373「closure may outlive borrowed value」。
- **闭包递归的类型推断失败**：闭包不能直接递归——`let f = |n| f(n-1)` 中 `f` 在定义体内引用自身，类型推断形成循环（E0391）且环境捕获未初始化变量。标准解法：`fn` 函数、Y 组合子风格的显式不动点、或 `Rc<RefCell<Option<Box<dyn Fn>>>>` 技巧。
- **`dyn Fn` 与泛型闭包的性能差异**（逻辑错误）：热路径把泛型回调改为 `Box<dyn Fn>` 引入间接调用 + 堆分配 + 禁止内联——不报错但性能回退，基准测试可测。

判定闭包错误的第一步：区分「类型层面违约」（前两类，编译期）与「语义层面误解」（后两类，逻辑/性能）——前者的错误信息直接给出缺失的 trait 或生命周期，后者需要基准与语义推理。
...
本节 5 道测验覆盖闭包类型的核心判别点：

- 测验 1（理解层）：`Fn`/`FnMut`/`FnOnce` 的选择——由闭包体对捕获变量的使用方式决定（读 ⟹ Fn，改 ⟹ FnMut，消耗 ⟹ FnOnce），trait 是自动推断不可手动选择；
- 测验 2（应用层）：`move` 的作用——强制按值捕获，解决 `'static` 与所有权转移需求；
- 测验 3（应用层）：闭包与函数指针的互换边界——无捕获可强制，有捕获需 `dyn`；
- 测验 4（分析层）：捕获模式推导——给定闭包体写出等价的「匿名结构体 + 字段」；
- 测验 5（评价层）：闭包作为返回值——`impl Fn` 的捕获规则与 `Box<dyn Fn>` 的取舍。

作答建议：测验 4 的「结构体展开法」是理解一切闭包现象的通用工具。
...
| 属性 | 取值 / 判定 | 依据 |
|---|---|---|
| trait 层级 | `FnOnce` ⊃ `FnMut` ⊃ `Fn` 三档调用契约 | `std::ops` |
| 捕获推断 | 编译器按闭包体推断最小捕获方式 | 捕获规则 |
| 类型唯一 | 每个闭包有唯一匿名类型 | 类型系统 |
| `move` 闭包 | 强制按值捕获环境 | 文法 |
| 成本 | 单态化（Monomorphization）内联，调用零间接 | 零成本抽象（Zero-Cost Abstraction） |

## 🔗 概念关系

- **上位（is-a）**：[Traits](../00_traits/01_traits.md) 的 `Fn` 族契约实例化。
- **下位（实例）**：异步闭包见 [Async Closures](../../03_advanced/01_async/07_async_closures.md)。
