...
本节用三组对照说明「联合体（Unions）」：正确示例：与 C 结构体互操作、反例：读取非活跃字段与反例：为非 Copy 字段实现 Copy。每组先给正确示例并标注其成立的类型系统依据，再给反例并标注编译器诊断（E0xxx）或运行时后果，最后给出修正方案。判读标准：正确示例应能通过 rustc 1.97 编译且无 clippy 警告，反例的失败点必须可定位到具体规则。
