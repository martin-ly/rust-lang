...
- **内存模型**：所有字段共享同一存储（大小 = 最大字段，对齐 = 最大对齐）——同一字节的多种解释，这是 C 式类型双关（type punning）在 Rust 的合法入口；
- **unsafe 边界**：写字段是安全的，**读字段是 `unsafe`**——编译器不知道哪个字段是活跃的，读取未初始化或非当前变体的字段是 UB（若读出无效位模式，如对 `bool` 读出 `2`）；
- **字段约束**：字段必须是 `Copy` 或包在 `ManuallyDrop` 中——union 无法知道该运行哪个字段的 `Drop`，所以带析构的类型必须手动管理。

典型合法用途：FFI 对接 C union、协议解析的零拷贝重解释、嵌入式寄存器映射。判定准则：能用 `enum` 就不要用 `union`——`enum` 携带判别标签（tag），`union` 把「哪个变体活跃」的追踪责任完全交给程序员。
...
本节用三组对照说明「联合体（Unions）」：正确示例：与 C 结构体互操作、反例：读取非活跃字段与反例：为非 Copy 字段实现 Copy。每组先给正确示例并标注其成立的类型系统（Type System）依据，再给反例并标注编译器诊断（E0xxx）或运行时（Runtime）后果，最后给出修正方案。判读标准：正确示例应能通过 rustc 1.97 编译且无 clippy 警告，反例的失败点必须可定位到具体规则。
...
- **反命题 1：「`union` 是 `transmute` 的安全替代」** —— 方向反了：`union` 字段读取同样是 `unsafe`，健全性责任相同。`union` 的优势仅在「多次重解释同一块内存」时比连续 `transmute` 更清晰，且字段访问受布局检查；单次转换用 `transmute` 或更好的 `bytemuck`（带类型约束的位重解释）。
- **反命题 2：「Rust `union` 与 C `union` 完全兼容」** —— 有边界：`#[repr(C)]` union 布局兼容 C，但① Rust union 默认 `repr(Rust)` 布局不承诺；② C 的「公共初始序列」读取规则在 Rust 中无对应宽松——读非活跃字段（即使公共前缀相同）在 Rust 模型下更严格，Miri 会标记。

边界极限小节量化：活跃字段追踪的程序员义务、`ManuallyDrop` 字段的手动析构模式、以及 `union` 在 `const` 上下文中的使用限制。
...
| 模式匹配（Pattern Matching） | 不支持 | 需 unsafe | 不能对 union 使用 `match` |
...
本节把「联合体（Unions）」的规则推到编译器与运行时（Runtime）的边界上逐一实测：边界测试：`ManuallyDrop` 与联合体 与 边界测试：联合体大小。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
| 安全状态机 | `enum` | `union` | enum 模式匹配（Pattern Matching） exhaustiveness |
...
本节测验覆盖 `union` 的三个核心判别点：

- **理解层**：`union` vs `enum` 的语义差异——判别标签的有无、大小计算方式、安全边界位置；
- **应用层**：FFI 中 C union 的映射——`#[repr(C)] union` 声明、活跃变体的外部追踪（通常由 C API 的 tag 字段或调用上下文决定）；
- **分析层**：读取未活跃字段的 UB 分析——给定 union 读写序列判断哪次读取合法（最近写入的字段），以及 `ManuallyDrop` 字段的析构责任归属。

作答建议：所有 union 相关判断都先问「此刻哪个字段是活跃的、谁保证的」——答不出这个问题就不要用 union。
