...
> **过渡**: 从 union 的直观描述转向其形式化定义，需要先把日常经验中的模糊直觉转化为可验证的术语与规则。

> **过渡**: 在建立 union 的核心命题之后，下一步是审视这些命题在边界条件下的稳定性——这正是反命题与反例的价值所在。

> **过渡**: 最后，将 union 与相邻概念连接，形成从 L1 到 L7 的纵向认知路径，避免孤立记忆。
...
> **反命题 1**: "联合体与枚举相同" ⟹ 不成立。枚举有标签，联合体没有；枚举模式匹配安全，联合体读取需 unsafe。
...
**解析**: 联合体默认不会自动 drop 其字段。若字段需要 drop，应使用 `ManuallyDrop<T>` 并手动管理生命周期，否则可能导致内存泄漏或重复 drop。
