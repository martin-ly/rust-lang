...
> **双维定位**: C×App
> **前置概念**: [Type System Basics](../../01_foundation/02_type_system/01_type_system.md) · [Traits](../00_traits/01_traits.md) · [Error Handling](../03_error_handling/01_error_handling.md)
> **后置概念**: [Newtype and Wrapper Types](03_newtype_and_wrapper.md) · [Coercion and Casting](../../01_foundation/02_type_system/04_coercion_and_casting.md)
>
...
本节把「类型转换（Type Conversions）」的规则推到编译器与运行时的边界上逐一实测：边界测试：`TryInto` 溢出 与 边界测试：强制与转换的区别。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
本节围绕「判断推理与决策树」展开，覆盖选择哪种转换方式？ 与 与其他概念的辨析 两个方面。
