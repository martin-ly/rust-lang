> **内容分级**: [综述级]
>
> **本节关键术语**: 范围类型 (Range Type) · core::range · std::ops::Range · 区间运算 · 迭代器范围 (Range Iterator) — [完整对照表](../../00_meta/01_terminology/terminology_glossary.md)
>
...
本节从范围类型的数学语义、`std::ops::Range`：运行时迭代器语义、`core::range`：编译期值语义与`IntoIterator` vs `Iterator`：设计权衡切入，剖析「核心概念」的核心内容。
...
本节从 `Copy` 的语义影响 与 与 `for` 循环的交互 两个层面剖析「形式化语义」。
...
理解「跨语言对比」需要把握 Python：`range()` 函数、C++20：`std::ranges`与Rust：`core::range::Range`，本节依次展开。
...
「边界测试：范围类型的编译错误」部分按边界测试：范围模式在非 match 中使用（编译错误）、边界测试：`Range` 与 `RangeInclusive` 的类型…、边界测试：`RangeInclusive` 的 `Copy` 缺失（编…、边界测试：`Iterator::size_hint` 与无限范围（逻辑…等6个方面的顺序逐层展开。
