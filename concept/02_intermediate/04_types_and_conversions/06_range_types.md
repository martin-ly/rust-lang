...
- [Rust 范围类型语义：`std::ops::Range` → `core::range`](#rust-范围类型语义stdopsrange--corerange)
  ...
  - [一、核心概念](#一核心概念)
    ...
    - [1.2 `std::ops::Range`：运行时（Runtime）迭代器（Iterator）语义](#12-stdopsrange运行时迭代器语义)
  ...
  - [十、边界测试：范围类型的编译错误](#十边界测试范围类型的编译错误)
    ...
    - [10.5 边界测试：不可变借用（Immutable Borrow）与可变借用的冲突](#105-边界测试不可变借用与可变借用的冲突)
  - [嵌入式测验（Embedded Quiz）](#嵌入式测验embedded-quiz)
    ...
    - [测验 4：当使用 `Range` 作为切片（Slice）索引超出范围时会发生什么？（理解层）](#测验-4当使用-range-作为切片索引超出范围时会发生什么理解层)
