...
本组题目考察默认语义的分岔：C++ 默认**值语义**（赋值即拷贝，引用（Reference）需显式 `&`/`&&`），Rust 按类型区分——`Copy` 类型按位复制，其余默认 **move 语义**（赋值即转移所有权），共享靠显式借用（Borrowing） `&T`/`&mut T`。引用语义上两者最大差异：C++ 引用是别名（无空、不可重绑），Rust 引用是带生命周期（Lifetimes）与可变性规则的指针，可重借、可存容器。
...
本组题目要求综合前七组知识做迁移判定：给定一段使用 RTTI、宏、friend、移动构造的 C++ 代码，判断其在 Rust 中的正确对应物（或证明其不可直接迁移）。作答策略：先识别每处 C++ 机制所属类别（类型识别/代码生成/访问控制/所有权），再套用对应组的迁移规则——逐项映射后仍残留的「无对应物」部分，正是需要重新设计的架构信号。
