...
本组题目对比两种宏系统的本质：C 预处理器是**编译前的文本替换**（无语法感知、无卫生性、错误信息指向展开结果），Rust `macro_rules!` 是**解析后的语法树片段替换**（片段分类符保证语法良构、局部变量卫生）。`SQUARE(x)` 的双重求值陷阱（`SQUARE(i++)`）在 Rust 中由 `expr` 片段的「先求值后绑定」语义消除。
...
本组题目考察访问控制的两种哲学：C++ `friend` 是**类型粒度的例外授权**（特定类/函数可访问私有成员），Rust 没有 friend，最接近的是**模块粒度的可见性**——`pub(crate)`/`pub(in path)` 把「亲密协作」表达为「同一模块树内可见」。迁移判定：C++ 中 friend 的用途在 Rust 中通常应重构为同一模块内的自由函数或 `pub(crate)` 方法。
...
本组题目对比两种 move 的本质差异：C++ move 是**用户可定义的运行时操作**（移动构造/移动赋值，源对象进入「有效但未指定」状态，析构仍运行）；Rust move 是**编译期所有权转移 + 按位拷贝**（无代码运行，源绑定被静态失效，无析构）。这解释了为什么 Rust 没有「移动后状态」问题——移动后使用直接被编译器拒绝（E0382）。
...
本组题目考察默认语义的分岔：C++ 默认**值语义**（赋值即拷贝，引用需显式 `&`/`&&`），Rust 按类型区分——`Copy` 类型按位复制，其余默认 **move 语义**（赋值即转移所有权），共享靠显式借用 `&T`/`&mut T`。引用语义上两者最大差异：C++ 引用是别名（无空、不可重绑），Rust 引用是带生命周期与可变性规则的指针，可重借、可存容器。
