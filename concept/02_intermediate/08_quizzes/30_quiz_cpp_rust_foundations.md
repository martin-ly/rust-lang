...
本组题目考察 C++ 与 Rust 在「运行期类型信息」上的根本分歧：C++ 的 RTTI 建立在继承层次上（`dynamic_cast` 沿子类型关系查询），Rust 的 `Any` 建立在**类型相等**上（`TypeId` 是类型的编译期唯一标识，`downcast_ref` 只做相等比较）。理解这一差异是判断「哪些 C++ 多态技巧无法移植」的起点。
...
本组题目对比两种宏（Macro）系统的本质：C 预处理器是**编译前的文本替换**（无语法感知、无卫生性、错误信息指向展开结果），Rust `macro_rules!` 是**解析后的语法树片段替换**（片段分类符保证语法良构、局部变量卫生）。`SQUARE(x)` 的双重求值陷阱（`SQUARE(i++)`）在 Rust 中由 `expr` 片段的「先求值后绑定」语义消除。
...
本组题目考察访问控制的两种哲学：C++ `friend` 是**类型粒度的例外授权**（特定类/函数可访问私有成员），Rust 没有 friend，最接近的是**模块（Module）粒度的可见性**——`pub(crate)`/`pub(in path)` 把「亲密协作」表达为「同一模块树内可见」。迁移判定：C++ 中 friend 的用途在 Rust 中通常应重构为同一模块内的自由函数或 `pub(crate)` 方法。
...
本组题目对比两种 move 的本质差异：C++ move 是**用户可定义的运行时（Runtime）操作**（移动构造/移动赋值，源对象进入「有效但未指定」状态，析构仍运行）；Rust move 是**编译期所有权（Ownership）转移 + 按位拷贝**（无代码运行，源绑定被静态失效，无析构）。这解释了为什么 Rust 没有「移动后状态」问题——移动后使用直接被编译器拒绝（E0382）。
...
本组题目考察默认语义的分岔：C++ 默认**值语义**（赋值即拷贝，引用（Reference）需显式 `&`/`&&`），Rust 按类型区分——`Copy` 类型按位复制，其余默认 **move 语义**（赋值即转移所有权（Ownership）），共享靠显式借用（Borrowing） `&T`/`&mut T`。引用语义上两者最大差异：C++ 引用是别名（无空、不可重绑），Rust 引用是带生命周期（Lifetimes）与可变性规则的指针，可重借、可存容器。
...
本组题目要求综合前七组知识做迁移判定：给定一段使用 RTTI、宏（Macro）、friend、移动构造的 C++ 代码，判断其在 Rust 中的正确对应物（或证明其不可直接迁移）。作答策略：先识别每处 C++ 机制所属类别（类型识别/代码生成/访问控制/所有权），再套用对应组的迁移规则——逐项映射后仍残留的「无对应物」部分，正是需要重新设计的架构信号。
...
- A. 两者机制与定位完全相同
- B. C++ 异常用于常规错误处理（Error Handling）、可被任意 `catch`；Rust panic 定位不可恢复错误，常规错误用 `Result` 显式传递
...
**解析**：两语言把"可恢复错误"放在不同位置：C++ 主流用异常（控制流隐式跳转），Rust 用 `Result` 值（控制流显式、类型系统（Type System）强制处理）；panic 对应的是 C++ 中"违反前置条件/逻辑错误"的场景。C 错：默认 `unwind` 策略下未被捕获的 panic 终止当前线程，进程级 `panic = "abort"` 则直接中止——没有"静默忽略"。
...
**解析**：借用（Borrowing）检查器 + 所有权 + `Send`/`Sync` 分别封死这三类 UB：引用不能超出数据生命周期（A）、别名-可变分离保证无数据竞争（B）、移动语义 + 唯一所有者保证恰好释放一次（C）。D 有教学价值：整数溢出在 Rust 中**不是 UB**——debug 构建 panic、release 构建按定义良好的二进制补码环绕；它是"被定义的错误行为"，不属于"静态阻止的 UB"。
