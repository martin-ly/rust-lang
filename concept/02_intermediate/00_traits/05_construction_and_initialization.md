...
> **双维定位**: C×Ana
> **前置概念**: [Variable Model](../../01_foundation/03_values_and_references/03_variable_model.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md)
> **后置概念**: [Rust vs C++](../../05_comparative/01_systems_languages/01_rust_vs_cpp.md) · [C++ Surface Features](../../05_comparative/00_paradigms/03_cpp_rust_surface_features.md)
...
C++ 的构造体系是语言中最复杂的子系统之一，三个支柱：

1. **构造函数族**：默认构造、拷贝构造、移动构造、委托构造、explicit 构造——加上初始化列表的求值顺序规则（按声明序而非列表序，经典陷阱）；
2. **聚合初始化**：`{...}` 列表初始化与构造函数的重载决议交互——`vector<int> v{3}` 与 `v(3)` 的不同语义是 C++ 初始化语法的著名坑；
3. **三/五/零法则**：自定义析构 ⟹ 通常需要自定义拷贝/移动（Rule of Five）；Rule of Zero 主张用 RAII 成员让编译器生成全部特殊成员函数。

与 Rust 对照的价值：C++ 构造体系的复杂性源于「构造是对象状态的逐步建立」——Rust 用「先完整初始化再使用」（无部分构造状态）+「move 即 memcpy」+「无拷贝构造（Clone 显式）」三刀切掉了绝大部分机制。理解 C++ 的复杂，才能理解 Rust 的简洁不是能力缺失。
...
Rust 的初始化模型建立在一个不变量上：**值在绑定前必须完整初始化**（E0381 强制执行）。四种构造途径：

1. **结构体（Struct）字面量**：`Point { x: 1, y: 2 }`——所有字段必须给出（或 `..Default::default()` 更新语法），无「默认构造后赋值」的两阶段；
2. **`new` 约定**：`fn new(...) -> Self` 只是普通关联函数的命名惯例（无语言特权）——复杂构造逻辑（验证、分配、部分失败）都走此路径，失败返回 `Result<Self, E>`（`try_new` 惯例）；
3. **`Default` trait**：`T::default()` 提供「合理的零值」——`#[derive(Default)]` 逐字段默认，枚举（Enum）需 `#[default]` 标注变体；
4. **`const fn` 编译期构造**：`const X: T = T::new(...)`——构造逻辑在编译期执行，`static`/数组长度等常量上下文可用。

判定准则：简单结构用字面量；有不变量要维护用 `new` + 私有字段；有自然零值实现 `Default`；需要编译期构造写 `const fn`。
