...
本节介绍负实现的基本语义、语法限制，以及它与 auto trait 的关系，为后续技术细节与应用场景奠定概念基础。
...
本节从语义规则、正实现冲突到泛型交互，展开负实现在类型系统中的精确行为，帮助读者理解 `impl !Trait for T` 如何参与约束求解与自动推导。
...
本节通过 `!Send` / `!Sync`、`!Unpin` 与毒化字段替代方案，说明负实现在标准库与自定义类型中的主要用途。
...
本节用反命题树和常见错误表梳理使用负实现前必须检查的约束条件，包括 auto trait、孤儿规则、正实现冲突与 nightly 限制。
