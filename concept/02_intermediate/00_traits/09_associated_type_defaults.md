...
本节介绍关联类型默认值的基本语义与语法形式，并说明它与 specialization 的密切联系，为后续技术细节与应用场景奠定概念基础。
...
本节解释默认实现不能假设默认关联类型的原因、覆盖默认类型时必须满足的 bound 约束，以及关联类型默认值与泛型参数默认值的差异。
...
本节展示关联类型默认值在 trait 渐进式演进、默认迭代器类型和配置 trait 中的典型用法，说明它如何提高 API 的向后兼容性。
...
本节用反命题树和常见错误表梳理采用关联类型默认值前的关键判定条件，帮助避免默认实现假设默认类型等典型错误。
...
理解关联类型默认值不仅需要掌握其语法，更需要清楚它的失败模式与稳定化边界。本节通过两个典型反例——在 stable Rust 中直接使用该 nightly 特性，以及在默认实现中错误地假设默认关联类型的具体形态——展示编译器如何落实 RFC 2532 的设计约束，并帮助读者在迁移到稳定替代方案前识别不可行的使用方式。
