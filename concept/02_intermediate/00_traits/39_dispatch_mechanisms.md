> **内容分级**: [专家级]
> **本节关键术语**: 静态分发 (Static Dispatch) · 动态分发 (Dynamic Dispatch) · 单态化 (Monomorphization) · Trait 对象 (Trait Object) · 对象安全 (Object Safety) — [完整对照表](../../00_meta/01_terminology/terminology_glossary.md)
>
...
理解「知识结构」需要把握概念定义、属性特征、关系连接与思维导图，本节依次展开。
...
本节从什么是分派 与 静态 vs 动态 两个层面剖析「分派机制基础」。
...
本节将「静态分发」分解为若干主题：单态化、性能优势与代码膨胀。
...
理解「动态分发」需要把握 Trait 对象、虚函数表与性能开销，本节依次展开。
...
理解「选择合适的分发」需要把握决策树、性能对比、使用场景、性能测试与分析等6个方面，本节依次展开。
...
理解「实战案例」需要把握案例 1: 插件系统、案例 2: GUI 组件、案例 3: 日志系统、案例 4: 序列化系统等5个方面，本节依次展开。
...
本节将「性能优化技术」分解为若干主题：内联优化（Inlining）、Devirtualization（去虚化）与缓存优化（Cache Locality）。
...
本节从静态分派的汇编 与 动态分派的汇编 两个层面剖析「汇编级分析」。
...
本节围绕「高级优化技巧」展开，依次讨论分支预测友好的设计、小对象优化（Small Object Optimization）与专门化（Specialization）。
