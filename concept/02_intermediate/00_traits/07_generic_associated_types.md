...
理解「形式化：类型 × Trait × 生命周期三维交叉」需要把握类型族视角、良构性（Well-formedness）规则与为什么不是完整 HKT，本节依次展开。
...
本节围绕「实质代码示例」展开，依次讨论示例一：Lending Iterator —— 可变滑动窗口、示例二：流式解析器 —— 零拷贝 token 流与示例三：自引用借用 —— 数据库游标式访问。
