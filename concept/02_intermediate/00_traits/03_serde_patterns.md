...
理解「边界测试：Serde 模式的编译错误」需要把握边界测试：反序列化时字段缺失（运行时错误）、边界测试：`#[serde(flatten)]` 与重复字段（编译错误…、边界测试：反序列化的 `deny_unknown_fields`（运行…、边界测试：枚举的 `untagged` 反序列化歧义（运行时错误）等7个方面，本节依次展开。
