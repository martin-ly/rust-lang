> **内容分级**: [综述级]
> **本节关键术语**: 序列化 (Serialization) · 反序列化 (Deserialization) · serde · Serialize · Deserialize · 自定义反序列化 — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
...
> **前置概念**: [Traits](01_traits.md) · [Macros](../../03_advanced/03_proc_macros/01_macros.md) · [Generics](../01_generics/01_generics.md)
> **后置概念**: [Core Crates](../../06_ecosystem/02_core_crates/01_core_crates.md) · [Application Domains](../../06_ecosystem/06_data_and_distributed/01_application_domains.md)
...
性能要点：serde 的零拷贝反序列化（`#[serde(borrow)]` + `&'a str` 字段）使 JSON 解析可直接引用（Reference）输入缓冲区——这是 `serde_json::from_slice` 高性能的来源。
...
- [Macros](../../03_advanced/03_proc_macros/01_macros.md) — 过程宏（Procedural Macro）机制
- [Generics](../01_generics/01_generics.md) — 泛型（Generics）与参数多态
- [Type System](../../01_foundation/02_type_system/01_type_system.md) — Serde 依赖的类型系统基础
- [Structs](../../01_foundation/07_modules_and_items/04_structs.md) · [Enumerations](../../01_foundation/07_modules_and_items/05_enumerations.md) — 被序列化的数据建模单元
- [Core Crates](../../06_ecosystem/02_core_crates/01_core_crates.md) — 核心 crate 生态
...
本节把「Serde 序列化模式：Rust 的类型驱动数据转换」的规则推到编译器与运行时的边界上逐一实测：边界测试：反序列化时字段缺失（运行时错误）、边界测试：`#[serde(flatten)]` 与重复字段（编译错误…、边界测试：反序列化的 `deny_unknown_fields`（运行…、边界测试：枚举的 `untagged` 反序列化歧义（运行时错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
