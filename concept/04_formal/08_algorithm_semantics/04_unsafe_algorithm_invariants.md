...
- [不安全算法的语义不变量（Semantic Invariants of Unsafe Algorithms）](#不安全算法的语义不变量semantic-invariants-of-unsafe-algorithms)
  ...
  - [三、反例与边界分析](#三反例与边界分析)
    ...
    - [3.5 编译期捕捉：未对齐指针读取](#35-编译期捕捉未对齐指针读取)
    - [3.6 编译期捕捉：重叠缓冲区使用 `copy_nonoverlapping`](#36-编译期捕捉重叠缓冲区使用-copy_nonoverlapping)
    - [3.7 编译期捕捉：先 `set_len` 再初始化元素](#37-编译期捕捉先-set_len-再初始化元素)
  - [四、验证工具](#四验证工具)
