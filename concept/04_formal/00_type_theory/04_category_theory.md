...
理解「核心概念」需要把握范畴的基本直觉、函子（Functor）与单子（Monad），本节依次展开。
...
本节从 Option 作为单子 与  Result 与错误处理 两个层面剖析「技术细节」。
...
本节围绕「边界测试：范畴论视角的编译错误」展开，依次讨论边界测试：`Option` 与 `Result` 的 monad 定律…、边界测试：`Iterator` 的 functor 映射与所有权（编译…与Iterator 作为函子。
