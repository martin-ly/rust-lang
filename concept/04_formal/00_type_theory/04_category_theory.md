...
> [Awodey — Category Theory](https://doi.org/10.1093/acprof:oso/9780198568612.001.0001) ·
...
范畴论（category theory）为 Rust 的类型构造提供统一语言，本节建立核心概念的最小工作集：

- **范畴（Category）**：对象 + 态射 + 复合 + 单位——Rust 中对象 = 类型，态射 = 函数 `fn(A) -> B`，复合 = 函数组合，单位 = 恒等函数；
- **函子（Functor）**：范畴间的结构保持映射——`Option`/`Vec`/`Result` 的类型构造子是函子（`map` 保持复合与单位，`fmap (g ∘ f) = fmap g ∘ fmap f`）；
- **自然变换**：函子间的映射——泛型（Generics）函数 `fn<T>(Option<T>) -> Vec<T>` 是自然变换的实例（「对任意类型均匀定义」即自然性）；
- **单子（Monad）**：自函子 + 单位 + 乘法——`Option` 的 `Some`/`flatten` 对应单位/乘法，`?` 算子是单子绑定的语法形式。

范畴论在 Rust 中的角色：不是编程必需品，而是「为什么这些抽象能组合」的解释——理解自然性即理解「泛型代码为何无需类型检查每种实例」。
...
> [来源: [Functor — Haskell base 文档](https://hackage.haskell.org/package/base/docs/Data-Functor.html)]
...
技术细节把范畴论的构造实例化为 Rust 的具体类型，两个核心实例：

- **Option 作为单子**：`Option<T>` 配备 `Some`（return/η：单位）与 `and_then`（bind/μ：展平 `Option<Option<T>>`）满足三条单子定律——左单位元（`Some(x).and_then(f) == f(x)`）、右单位元（`opt.and_then(Some) == opt`）、结合律。验证方法：对任意具体值展开两边表达式即得等式。`?` 运算符是 `and_then` 的语法糖变体。
- **Result 与错误处理**：`Result<T, E>` 是带错误语境的单子，`map`/`and_then` 只作用于 `Ok` 分支，`Err` 自动短路——这对应 Kleisli 范畴中的组合：每个函数 `f: T -> Result<U, E>` 是 Kleisli 箭头，`and_then` 是 Kleisli 组合。工程推论：错误处理代码的"扁平化"（避免嵌套 match）不是风格偏好而是单子结构的应用；凡是需要短路+传递语境的场景（权限检查链、IO 链）都可复用同一模式。
...
范畴论视角的边界测试验证"结构保持映射"是否在 Rust 类型系统（Type System）中真的保持：

- **`Option` 与 `Result` 的 monad 定律**：左单位元 `Some(x).and_then(f) == f(x)` 对纯函数 f 恒成立；边界——f 有副作用（如打印）时求值顺序差异使"定律"只在语义等价意义下成立。测试写法：构造两边表达式断言相等，覆盖 `None`/`Some`、链式组合三种情形。
- **`Iterator` 的 fusion 与函子定律**：`map(f).map(g)` 与 `map(|x| g(f(x)))` 语义等价（函子复合定律），且 rustc 的优化使二者性能相同；边界——`map` 的闭包（Closures）捕获环境时，捕获的可变状态在两次遍历间持续，函子定律只在"每次遍历独立"的假设下成立。

判定依据：定律验证用纯函数无争议；涉及副作用时只能验证"语义等价类"而非严格等式。 的 functor 映射与所有权（编译…与Iterator 作为函子。
...
本节检验范畴论应用于 Rust 的三条常见误判：

1. **「`Iterator::map` 就是函子」**：部分真——类型构造子 `Iterator` trait 不是自函子（迭代器项类型变化但 `Iterator` 本身不是类型构造子）；真正的函子是 `Option`/`Result` 这类 `F<T>` 形式——误判源于「有 map 方法就是函子」的表层相似；
2. **「Rust 有 HKT 就能写完整范畴论」**：GAT 提供了 HKT 的受限形式，但「高阶函子的函子」等构造仍受限——范畴论在 Rust 中的表达边界即 GAT 的表达边界；
3. **「学范畴论才能用好泛型」**：工程上反——范畴论是泛型设计的**事后解释**，熟练使用不需要范畴知识；但设计「可组合抽象库」（如 parser combinator）时，范畴直觉（结合律/单位律）是质量判据。

每条误判附代码反例与「正确的范畴对应物」对照表。
...
本组测验围绕测验 1：范畴论的基本直觉（理解层）、测验 2：函子（Functor）（应用层）、测验 3：单子（Monad）的核心操作（应用层）、测验 4：Rust 中的 Monad 替代（分析层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用（Borrowing）检查」或「运行时（Runtime）行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
...
- 使用 `?` 运算符作为特殊的单子绑定语法
- `generic_associated_types`（1.65 已稳定）可部分缓解，但 HKT 仍未完全支持
...
mindmap
  root((范畴论与 Rust 从函子到单子))
    ...
    技术细节
      ...
      2 Result 与错误处理
    边界测试 范畴论视角的编译错误
      1 边界测试 Option 与 Result 的 monad 定律违反 编译错误
      2 边界测试 Iterator 的 functor 映射与所有权 编译错误
      3 Iterator 作为函子
    范畴模式矩阵
    反命题与边界分析
      1 反命题树
