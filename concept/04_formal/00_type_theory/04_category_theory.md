...
- **自然变换**：函子间的映射——泛型函数 `fn<T>(Option<T>) -> Vec<T>` 是自然变换的实例（「对任意类型均匀定义」即自然性）；
...
范畴论视角的边界测试验证"结构保持映射"是否在 Rust 类型系统中真的保持：
...
- **`Iterator` 的 fusion 与函子定律**：`map(f).map(g)` 与 `map(|x| g(f(x)))` 语义等价（函子复合定律），且 rustc 的优化使二者性能相同；边界——`map` 的闭包捕获环境时，捕获的可变状态在两次遍历间持续，函子定律只在"每次遍历独立"的假设下成立。
...
本组测验围绕测验 1：范畴论的基本直觉（理解层）、测验 2：函子（Functor）（应用层）、测验 3：单子（Monad）的核心操作（应用层）、测验 4：Rust 中的 Monad 替代（分析层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
