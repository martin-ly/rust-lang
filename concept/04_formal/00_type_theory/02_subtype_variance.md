...
> **前置概念**: [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Lifetimes](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md) · [Generics](../../02_intermediate/01_generics/01_generics.md)
> **后置概念**: [Type Theory](01_type_theory.md) · [RustBelt](../02_separation_logic/01_rustbelt.md)
...
本节展开子类型与变型（variance）的技术内核：

1. **子类型规则**：`T <: U` 意味着「T 可用在 U 的任何位置」（Liskov 替换原则的类型化）；Rust 的子类型几乎只存在于生命周期维度——`&'a T <: &'b T` 当 `'a: 'b`（'a 比 'b 活得久）；
2. **变型定义**：类型构造子 F 的变型 = 子类型关系经 F 后的方向——协变（`T<:U ⟹ F<T><:F<U>`）、逆变（方向反转）、不变（仅当 T=U）；判定口诀：「出现在输出位置 → 协变，输入位置 → 逆变」；
3. **Rust 类型构造子的变型表**：`&'a T` 对 T 协变、`&'a mut T` 对 T **不变**（关键！）、`Box<T>` 协变、`fn(T)->U` 对 T 逆变幻变、`Cell<T>`/`RefCell<T>` 不变（内部可变性要求）；
4. **变型检查**：rustc 按结构自动推导用户类型的变型（`PhantomData<T>` 是手动标注变型的工具）。

技术细节附「为什么 `&mut` 必须不变」的完整反例推导（可变引用（Mutable Reference）协变 ⟹ 类型混淆攻击）。
...
本节检验变型的三条高频误判，每条附编译期复现：

1. **「`&'static mut T` 可以传给 `&'a mut T`」**：错——`&mut` 对生命周期**协变**但对 T **不变**；`'static: 'a` 成立所以生命周期方向可对，但若借此推导「`&mut Vec<&'static str>` 可传给 `&mut Vec<&'a str>`」则失败（Vec 内 T 位置不变）——E0308 的经典来源；
2. **「协变总是安全的」**：协变在「只读位置」安全，`Cell<T>` 的内部可变性打破「读安全」假设——故 `Cell` 不变；误判协变普遍性会错误设计泛型（Generics）容器；
3. **「逆变很少见，可忽略」**：`fn(T) -> U` 的参数逆变与 trait 对象 `dyn Fn(T)` 直接相关——忽略逆变会在「函数作为参数传递」时遇到神秘的生命周期错误（HRTB 与逆变交互）。

反例与 `PhantomData` 修复方案成对给出。
...
- [Generics](../../02_intermediate/01_generics/01_generics.md) — 泛型与参数多态
- [Type Theory](01_type_theory.md) — 类型理论基础
- [RustBelt](../02_separation_logic/01_rustbelt.md) — Rust 安全性的形式化模型
...
本节把「子类型与变型：Rust 类型系统中的协变、逆变与不变」的规则推到编译器与运行时的边界上逐一实测：边界测试：协变与逆变的生命周期误用（编译错误）、边界测试：`UnsafeCell` 的不变性（编译错误）、边界测试：逆变与 `fn` 参数的不变性（编译错误）、边界测试：`UnsafeCell` 的不变性（编译错误/运行时 UB）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
```mermaid
mindmap
  root((子类型与变型 Rust 类型系统中的协变 逆变与不变))
    核心概念
      1 子类型关系 static 是 a 的子类型
      2 变型三态 协变 逆变 不变
      3 Rust 中的变型规则
    技术细节
      1 生命周期位置的变型推导
