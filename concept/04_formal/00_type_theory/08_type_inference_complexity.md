...
> **前置依赖**: [Type Theory](01_type_theory.md) · [Type Inference](03_type_inference.md) · [Trait Solver in rustc](../05_rustc_internals/03_trait_solver_in_rustc.md)
> **后置延伸**: [Type Checking and Inference in rustc](07_type_checking_and_inference.md) · [Subtype Variance](02_subtype_variance.md)
> **来源**: · [类型推断（Type Inference）](03_type_inference.md) · [Pierce — Types and Programming Languages](https://www.cis.upenn.edu/~bcpierce/tapl/) · [Hindley — The Principal Type-Scheme of an Object in Combinatory Logic](https://doi.org/10.2307/2270762) · [Jung et al. — RustBelt: Securing the Foundations of Rust](https://plv.mpi-sws.org/rustbelt/popl18/) · [Itanium C++ ABI](https://itanium-cxx-abi.github.io/cxx-abi/abi.html)
>
...
本节聚焦「约束生成与 Robinson 合一」，覆盖约束生成（Constraint Generation） 与  Robinson 合一（Robinson Unification）。论述顺序由定义到边界：先明确「约束生成与 Robinson 合一」在「Type Inference Complexity（类型推断复杂度）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「约束生成与 Robinson 合一」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「复杂度：为什么是 PSPACE」，覆盖PSPACE 上界、PSPACE 下界与PSPACE-完全性。论述顺序由定义到边界：先明确「复杂度：为什么是 PSPACE」在「Type Inference Complexity（类型推断复杂度）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「复杂度：为什么是 PSPACE」的判定标准，并指出它在全页论证链中的位置。
...
本节以 `None` 缺元素类型为反例，展示推断变量无传播目标时的失败与标注修正。
