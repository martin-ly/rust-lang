...
本节从依赖类型与细化类型两条主线展开，先建立「值进入类型」的核心直觉（Π、Σ、索引族），再过渡到在现有类型上附加谓词的细化类型；五个子节共同构成从全依赖类型到轻量细化的连续光谱，为后文分析 Rust 的受限片段提供形式词汇。
...
本节把 §1 的直觉形式化：先给出 Π/Σ 的类型规则草图，再说明细化类型如何依托 SMT 求解器自动化验证，并通过 Liquid Haskell 与 Curry-Howard 视角把「类型即命题」的对应关系具体化。
...
本节把依赖/细化类型光谱上的代表性系统并置对比：从完整依赖类型语言，到通过注释嫁接的细化类型，再到工业验证语言；能力矩阵汇总各系统在表达力、自动化验证与工业采用上的位置。
...
本节通过一系列反例划定 Rust 的能力边界：从运行时值、const 表达式限制到谓词验证与 Phantom 模拟缺口，逐条说明为什么完整依赖类型与 Rust 的设计公理存在结构性张力。
...
本节把依赖类型与细化类型的理论收益落到工业场景：先纵览安全关键领域的验证需求，再聚焦数组边界与向量长度、协议状态机两个经典用例，展示形式化保证如何从类型层面向系统层面迁移。
...
本节汇总本页引用的权威文献、Rust 生态形式化工具、相关概念页与关键论文，按「官方规范—语言生态—相关权威页—学术论文」四层组织，便于读者按层次延伸阅读与追溯来源。
