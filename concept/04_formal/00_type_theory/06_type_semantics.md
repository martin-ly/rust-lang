...
> **前置依赖**: [Type Theory](01_type_theory.md) · [Ownership Formalization](../01_ownership_logic/02_ownership_formal.md)
> **后置延伸**: [Axiomatic Semantics](../03_operational_semantics/05_axiomatic_semantics.md) · [RustBelt](../02_separation_logic/01_rustbelt.md) · [Effects System](../../07_future/02_preview_features/01_effects_system.md)
...
类型语义（type semantics）回答「类型意味着什么」——类型不仅是语法分类，而是有严格数学解释的语义对象。本节确立三个权威解释框架：

1. **集合论语义（朴素）**：类型 = 值的集合，`T` 的解释是所有类型为 T 的值——直观但遇到递归类型与泛型（Generics）时破产（集合论悖论）；
2. **域论语义（Scott）**：类型 = 完全偏序域（domain），递归类型由不动点构造解释——支持 `enum List { Nil, Cons(i32, Box<List>) }` 这类自指定义；
3. **范畴论语义**：类型 = 对象，函数 = 态射——`fn(A) -> B` 是 Hom 集元素，泛型是自然变换——本节给出从 Rust 语法到范畴构造的映射表。

与 Rust 的衔接：`Sized`/DST 区分在三个框架中分别对应「有限集/无限集」「域的紧致性」「对象的有限呈现」——同一区分的三种读法。
...
类型语义问的是"类型意味着什么"，而非"类型检查如何实现"。下表沿**指称对象、等价判定、多态处理**三轴对比四种主流方法：

| 方法 | 指称对象 | 类型等价判定 | 多态处理 | 对 Rust 的适用度 |
|:---|:---|:---|:---|:---|
| 域理论（Domain Theory） | 完全偏序/Scott 域 | 域同构 | 参数化域 | 中：需处理 `!` 与发散 |
| 集合论语义 | 值的集合 | 集合相等 | 交集/子集 | 低：无法表达所有权（Ownership） |
| 逻辑语义（Curry-Howard） | 命题 | 证明等价 | 全称量化 | 高：与 trait bound 天然契合 |
| 子结构语义（线性逻辑） | 资源 | 资源等价 | 受重量化 | 最高：直接建模所有权 |

Rust 类型语义的特殊性在于它**同时需要逻辑语义与子结构语义**：trait bound `T: Clone` 是逻辑断言，而 `T` 只能移动一次是资源约束。单一形式化框架无法覆盖两者，这正是 RustBelt 选择高阶分离逻辑（Iris）作为基础的原因。
...
Rust 有三类在主流语言中没有直接对应物的类型构造，它们的语义需要专门处理：

- **借用语义 `&T` / `&mut T`**：指称上不是"指向 T 的指针"，而是"对 T 的带期限访问权"。`&T` 是共享只读权（可复制），`&mut T` 是独占读写权（不可复制、不可重入）。生命周期参数 `'a` 指称该访问权的有效区间，借用检查即验证访问权不重叠冲突。
- **资源管理语义 `Box<T>` / `Rc<T>` / `Arc<T>`**：三者都是堆分配，但所有权语义不同——`Box` 是唯一所有权（移动语义），`Rc` 是线程内共享所有权（运行时（Runtime）引用计数），`Arc` 是跨线程共享所有权（原子计数）。它们的 `Drop` 语义分别对应"确定性释放"、"最后一个引用释放"与"线程安全的最后引用释放"。
- **never 类型 `!`**：指称为空类型（无值），是发散计算的类型。它可强制转换为任何类型，对应逻辑上的爆炸律 `⊥ → P`，使 `panic!()` 和 `loop {}` 能出现在任意类型期望位置。T>` / `Arc<T>`、代数效应语义：`Option<T>` / `Result<T, E>`与存在与全称类型：`dyn Trait` 与 `impl Trait`的顺序逐层展开。
...
本节给出子类型与变型在三种语义框架下的解释，回答「`T <: U` 在语义层意味着什么」：

- **集合论语义**：`T <: U` ⟹ ⟦T⟧ ⊆ ⟦U⟧（类型解释的子集）——生命周期子类型 `'a: 'b` 读作「'a 的区间包含 'b 的区间」，方向与直觉相反（长命 ⊆ 短命的「可用位置集」）；
- **强制（coercion）语义**：`T <: U` ⟹ 存在隐式转换函数 `T → U`（erase 后恒等）——Rust 的子类型擦除后无运行期操作（纯静态关系），这与 Scala 的视图边界形成对照；
- **变型的语义**：类型构造子 F 的协变 = F 是「保序映射」（范畴论的函子性）；逆变 = 「反序映射」（`fn` 参数位置的 contravariant functor）；不变 = 两侧皆不保持（`Cell` 的双向读写位置）。

三种解释的等价性定理（在合适范畴中）附后——本节是 02_subtype_variance 的语义深化页，两页 canonical 分工。
...
1. **「类型就是值的集合」**：朴素集合论语义在递归类型处破产——`Box<List>` 的自指构造需要域论不动点解释，否则推出「类型包含自身为真子集」的悖论；反例与修复（域论）并列给出；
2. **「零大小类型（ZST）没有语义」**：错——`()`/`PhantomData<T>` 的语义在「类型层面」而非「值层面」：ZST 承载不变量（所有权标记、协变性标记）而不占用运行时表示——「语义 ≠ 内存布局」是本节要点；
3. **「子类型 = 子集」**：`&'a T` 对 `&'b T`（`'a: 'b`）的子类型关系在集合语义下方向易搞反——协变/逆变必须用「使用位置」判据，集合包含只是协变特例。

每条误判附最小反例程序与正确的语义框架参照。
...
| **高阶类型** | GAT（Generic Associated Types）有限支持 | System F-ω 的完整高阶多态 | Rust 类型系统（Type System）故意限制高阶类型以控制复杂度 |
...
类型语义的边界测试验证语义解释的 soundness——即"类型系统的接受/拒绝与实际执行安全性一致"：

- **协变数组的 soundness 漏洞（编译错误）**：经典反例是 Java 数组协变导致的 ArrayStoreException。Rust 的 `&mut [&'a str]` 对 `'a` 协变但对元素写入通过 `&mut` 受不变性保护——试图把 `&mut Vec<&'static str>` 传给期望 `&mut Vec<&'short str>` 的函数会被拒绝，语义上正确：否则函数内可写入短生命周期引用导致悬垂。
- **`dyn Trait` 与 `impl Trait` 的语义分野**：`impl Trait` 是存在类型的**全称实例化**（编译期选定单一类型，单态化（Monomorphization）），`dyn Trait` 是存在类型的**运行时打包**（擦除 + vtable）。边界测试：`impl Trait` 不能在两个分支返回不同类型（E0308），而 `Box<dyn Trait>` 可以——这不是限制而是语义差异的必然推论。生命周期子类型的悬垂引用（编译错误），本节逐一说明其要点。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：什么是"类型安全性"（Type Safety）？它通常包含哪…、测验 2：`&mut T` 在类型语义上如何区别于 `&T`？这种区别…、测验 3：Rust 的生命周期 `'a` 在类型语义中扮演什么角色？（…、测验 4：什么是"类型擦除"（Type Erasure）？`dyn T…等5个方面。
...
本节给出类型语义的定理推理链，根定理是**语义与语法的一致性**：类型系统的语法规则在语义解释下保持有效性（soundness）。派生链：

1. **类型保持链（Preservation）**：`⊢ e : T` 且 `e → e'` ⟹ `⊢ e' : T`——归约不破坏类型；Rust 语境的对应：move 后的「部分移动状态」在语义层仍是良类型（类型系统跟踪部分初始化）；
2. **进展链（Progress）**：`⊢ e : T` ⟹ e 是值或可归约——「良类型程序不卡住」；unsafe 是进展定理的边界（裸指针解引用可能 stuck）；
3. **变型健全链**：协变位置只读 ⟹ 子类型上溯安全；`&mut` 读写双位置 ⟹ 必须不变——链的形式化即「变型 ⟹ 无类型混淆」的证明；
4. **递归类型链**：equi-recursive vs iso-recursive 两种解释的等价性条件——Rust 选 iso-recursive（`Box` 的显式展开即「fold/unfold」）。

每链附「失效条件」标注（unsafe、类型双关 transmute）与 TAPL 对应章节。
...
本节把「Type Semantics（类型语义）」的规则推到编译器与运行时的边界上逐一实测：边界测试：`PhantomData` 的语义必要性（编译错误） 与 边界测试：`Pin<T>` 的类型语义（编译错误）。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
- [类型论基础](01_type_theory.md) — HM 类型系统（Type System）、System F、多态
- [所有权（Ownership）形式化](../01_ownership_logic/02_ownership_formal.md) — COR、RustBelt、权限系统
- [公理语义：霍尔逻辑与 wp 计算](../03_operational_semantics/05_axiomatic_semantics.md) — Hoare 三元组、谓词转换器
- [操作语义：程序行为的形式化定义](../03_operational_semantics/03_operational_semantics.md) — 小步/大步语义
- [指称语义：CPO 与不动点](../03_operational_semantics/01_denotational_semantics.md) — Scott-Strachey 语义
...
> **认知功能**: 本 mindmap 从本页「Type Semantics 类型语义」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
