...
> **后置延伸**: [Axiomatic Semantics](../03_operational_semantics/05_axiomatic_semantics.md) · [RustBelt](../02_separation_logic/01_rustbelt.md) · [Effects System](../../07_future/03_preview_features/01_effects_system.md)
...
1. **集合论语义（朴素）**：类型 = 值的集合，`T` 的解释是所有类型为 T 的值——直观但遇到递归类型与泛型时破产（集合论悖论）；
...
| 集合论语义 | 值的集合 | 集合相等 | 交集/子集 | 低：无法表达所有权 |
...
- **资源管理语义 `Box<T>` / `Rc<T>` / `Arc<T>`**：三者都是堆分配，但所有权语义不同——`Box` 是唯一所有权（移动语义），`Rc` 是线程内共享所有权（运行时引用计数），`Arc` 是跨线程共享所有权（原子计数）。它们的 `Drop` 语义分别对应"确定性释放"、"最后一个引用释放"与"线程安全的最后引用释放"。
...
- **`dyn Trait` 与 `impl Trait` 的语义分野**：`impl Trait` 是存在类型的**全称实例化**（编译期选定单一类型，单态化），`dyn Trait` 是存在类型的**运行时打包**（擦除 + vtable）。边界测试：`impl Trait` 不能在两个分支返回不同类型（E0308），而 `Box<dyn Trait>` 可以——这不是限制而是语义差异的必然推论。生命周期子类型的悬垂引用（编译错误），本节逐一说明其要点。
