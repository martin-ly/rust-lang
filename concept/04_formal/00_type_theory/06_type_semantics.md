...
「概念属性矩阵」部分的核心主题是类型语义方法对比矩阵，本节展开说明。
...
「Rust 特有类型的语义」部分按借用语义：`&T` 与 `&mut T`、资源管理语义：`Box<T>` / `Rc<T>` / `Arc<T>`、代数效应语义：`Option<T>` / `Result<T, E>`与存在与全称类型：`dyn Trait` 与 `impl Trait`的顺序逐层展开。
...
「子类型与变型的语义解释」部分包含生命周期子类型 与 协变、逆变与不变 两条主线，本节依次说明。
...
「边界测试」涉及边界测试：协变数组的 soundness 漏洞（编译错误）、边界测试：`dyn Trait` 与 `impl Trait` 的语义…与边界测试：生命周期子类型的悬垂引用（编译错误），本节逐一说明其要点。
...
本节从定理一致性矩阵 与 反命题决策树 两个层面剖析「定理推理链」。
