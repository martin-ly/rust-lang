...
本节围绕「约束生成与 Robinson 合一」展开，覆盖约束生成（Constraint Generation） 与  Robinson 合一（Robinson Unification） 两个方面。
...
理解「复杂度：为什么是 PSPACE」需要把握 PSPACE 上界、PSPACE 下界与PSPACE-完全性，本节依次展开。
...
本节从测验 1：HM 类型推断为什么能保持多项式时间？、测验 2：Rust 类型推断最坏情况下的复杂度类是什么？依据哪些来源？与测验 3：`rustc` 中哪个结构负责保存推断变量与待求解约束？切入，剖析「嵌入式测验」的核心内容。
