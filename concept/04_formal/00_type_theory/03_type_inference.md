...
> **前置概念**: [Type Theory](01_type_theory.md) · [Generics](../../02_intermediate/01_generics/01_generics.md) · [Trait](../../02_intermediate/00_traits/01_traits.md)
> **后置概念**: [RustBelt](../02_separation_logic/01_rustbelt.md) · [Subtype Variance](02_subtype_variance.md)

---

> **来源**:
> [Hindley 1969 — Principal Type-Schemes](https://doi.org/10.1093/comjnl/12.2.166) ·
> · [Pierce — Types and Programming Languages](https://www.cis.upenn.edu/~bcpierce/tapl/) ·
> [Hindley — The Principal Type-Scheme of an Object in Combinatory Logic](https://doi.org/10.2307/2270762) ·
...
1. **约束生成**：遍历 AST 为每个表达式生成类型变量与约束（`x: i32`、`f: α -> β`）——Rust 的约束包括子类型（生命周期）、trait 约束（`α: Trait`）与投影约束（`<α as Trait>::Assoc = β`）；
2. **约束求解**：Hindley-Milner 的 unification（合一）算法是核心——Rust 的求解器（trait solver）扩展 HM 以处理 trait 求解（回溯 + 候选缓存），最坏情况非多项式（与命题逻辑可满足性相关），实践中靠「约束局部性」保持快速；
3. **推断的局部化**：Rust 刻意不做全程序推断——函数签名必须显式标注（`fn` 边界是推断的「防火墙」），`let` 局部变量可省略——此设计使推断可判定、错误信息可定位、签名即文档。

与 HM 系语言（ML/Haskell）的对照：它们的全程序推断表达力更强但错误信息指向远离错误源——Rust 的选择是工程优先。
...
本节把「类型推断：Hindley-Milner 算法与 Rust 的工业实现」的规则推到编译器与运行时（Runtime）的边界上逐一实测：边界测试：泛型方法链中的类型推断失败（编译错误）、边界测试：闭包参数类型推断的歧义（编译错误）、泛型函数的类型推断与生命周期推断。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
1. **「`let x = ...` 的推断是全局的」**：错——推断只利用「初始化表达式 + 后续使用」的局部约束；反例：`let v = vec![];` 单独存在时 E0282（无法推断元素类型），补充 `v.push(1)` 后推断成功——使用点参与约束；
2. **「标注类型只是给编译器提示」**：标注是**约束**不是提示——`let x: u32 = 1u8;` 报错而非「提示被采纳」；标注参与合一，错误标注即冲突；
3. **「turbofish `::<T>` 与标注等价」**：`collect::<Vec<_>>()` 提供的是「部分约束」（只约束外层类型），`let v: Vec<i32>` 提供完整约束——在 trait 投影复杂时可推断性不同（部分约束可能仍歧义）。

每条误判附最小复现与编译器错误的正确读法。
...
本组测验围绕测验 1：Hindley-Milner（HM）类型推断的核心算法是什么…、测验 2：Rust 的类型推断与 Haskell 的 HM 推断有什么…、测验 3：`let x = vec![1, 2, 3];` 中 `x`…、测验 4：为什么 Rust 有时需要显式类型标注（如 `collect…等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用（Borrowing）检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
