...
> **后置概念**: [RustBelt](../02_separation_logic/04_rustbelt.md) · [Subtype Variance](06_subtype_variance.md)
...
「技术细节」部分的核心主题是统一（Unification），本节展开说明。
...
本节从边界测试：泛型方法链中的类型推断失败（编译错误）、边界测试：闭包参数类型推断的歧义（编译错误）、泛型函数的类型推断与生命周期推断切入，剖析「边界测试：类型推断的编译错误」的核心内容。
...
> **泛型（Generics）推断**: Rust 的泛型推断是**双向的**——可以从参数推断，也可以从使用点推断（如 `v.push(42)` 推断 `Vec<i32>`）。
...
> **生命周期（Lifetimes）推断洞察**: Rust 的生命周期推断是**两层结构**——签名层的 Elision 简化常见模式，函数体内的推断处理复杂借用（Borrowing）关系。
...
> **陷阱总结**: 类型推断的陷阱主要与**可读性**、**数字类型**、**闭包捕获**、**泛型方法**和**生命周期（Lifetimes）**相关。
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：Hindley-Milner（HM）类型推断的核心算法是什么…、测验 2：Rust 的类型推断与 Haskell 的 HM 推断有什么…、测验 3：`let x = vec![1, 2, 3];` 中 `x`…、测验 4：为什么 Rust 有时需要显式类型标注（如 `collect…等5个方面的顺序逐层展开。
...
Rust 不支持全局 HM 推断：1) 函数签名通常需显式标注（除简单单表达式函数）；2) 不支持高阶类型的完整推断；3) 生命周期（Lifetimes）是显式标注或基于规则的推断。
</details>
...
> 本节选编自 `crates/c04_generic/docs/tier_02_guides/04_type_inference.md`，
> 作为 canonical 类型推断概念页的工程实践补充。

### Turbofish 使用场景

当编译器无法从上下文确定泛型类型时，使用 `::<>` 显式指定：

```rust
let v = Vec::<i32>::new();
let s = "1,2,3".split(',').collect::<Vec<&str>>();
...
| 场景 | 是否需要标注 | 示例 |
| :--- | :--- | :--- |
| 函数签名 | ✅ 必须 | `fn add(a: i32, b: i32) -> i32` |
| 局部变量初始化 | 通常不需要 | `let v = vec![1, 2, 3];` |
| 空集合构造 | ✅ 需要 | `let v: Vec<i32> = Vec::new();` |
| 链式方法调用结果 | 有时需要 | `.collect::<Vec<_>>()` |
| 闭包参数 | 通常可由使用处推断 | `let f = \|x\| x + 1;` |

### 常见推断错误速查

- `type annotations needed`：上下文不足，添加 `::<T>` 或变量类型标注。
