...
> **前置概念**: [Patterns](../../06_ecosystem/03_design_patterns/01_patterns.md) · [Concurrency Patterns](../../03_advanced/00_concurrency/03_concurrency_patterns.md) · [Async Patterns](../../03_advanced/01_async/03_async_patterns.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Paradigm Matrix](../../05_comparative/00_paradigms/01_paradigm_matrix.md)
> **后置概念**: [Distributed Systems](../../06_ecosystem/04_web_and_networking/01_distributed_systems.md) · [System Design Principles](../../06_ecosystem/03_design_patterns/03_system_design_principles.md)
...
模式组合的四种代数原语，借用（Borrowing）函数式编程的组合子思想：

| 原语 | 记号 | 语义 | Rust 对应 |
|---|---|---|---|
| 并行组合 | A ⊗ B | 两模式独立共存，无交互 | 模块（Module）内并存 |
| 串行复合 | A ∘ B | B 的输入依赖 A 的输出 | `Iterator` 组合子链 |
| 选择和 | A ⊕ B | 按条件二选一 | `enum` + `match` |
| 迭代闭包（Closures） | A* | 重复应用直至收敛 | 循环/递归 |

组合不是无成本的：两个模式共享同一可变状态时，借用检查会把组合约束暴露为编译错误（如 Observer ⊗ Singleton 中观察者回调需 `&mut` 而单例被 `Arc` 共享）。这正是 Rust 相对 GoF 时代的增益——**组合冲突从运行期缺陷前移到编译期诊断**。

判定依据：组合前先画出状态所有权（Ownership）图，确认每个 `&mut` 路径独占。
...
模式组合的价值不在于叠加，而在于**职责边界的互补**。协同组合的共同特征是：各模式解决的横切关注点（creational/structural/behavioral）互不重叠，例如 Strategy 定义算法族、Decorator 在不改变接口的前提下叠加行为、Observer 解耦状态传播。

冲突则源于关注点的重叠或控制权的争夺：

| 组合 | 关系 | 原因 |
|---|---|---|
| Strategy + Template Method | 冲突 | 都定义算法骨架，一个用组合一个用继承 |
| Observer + Mediator | 谨慎 | 事件路由职责重叠，需明确谁拥有订阅表 |
| Singleton + DI | 冲突 | 全局可变状态与可替换注入天然对立 |

判定依据：组合前画一张“谁创建、谁持有、谁通知”的职责图，任一格子有两个模式即重新划分。
...
决策树把“选哪个模式”从经验问题转化为**特征匹配问题**：每个分支节点对应一个可判定的问题特征，叶节点是模式候选。

- **问题特征 → 模式选择**: 创建类问题按“实例数量/构造复杂度/家族关系”三分（Singleton / Builder / Abstract Factory）；行为类问题按“算法可换/状态驱动/通知传播”三分（Strategy / State / Observer）。
- **并发场景的模式选择**: 共享可变状态 → Actor 或 Channel（消息传递优先于锁）；流水线处理 → Producer-Consumer；只读快照共享 → `Arc` + 不可变数据（COW）。

判定依据：若某个特征无法明确回答（如“状态是否可变”取决于需求未定），说明需求分析未完成，此时选模式为时过早——先用最简单的直接实现，等特征稳定再重构。
...
Rust 特有的模式组合源于所有权系统，三种最具生产力：

1. **Typestate + Builder**：构建器每个步骤消费 `self` 返回新类型（`Builder<Unset>` → `Builder<Set>`），必填字段缺失是编译错误而非运行期 panic——把「对象构造协议」编码进类型。
2. **Ownership + RAII + Scope Guard**：`Drop` 实现保证资源释放与作用域绑定，`scopeguard` crate 把任意闭包变成守卫；组合 Mutex 守卫时守卫本身持锁，借用检查保证锁内临界区不可逃逸。
3. **Trait Object + Strategy**：`Box<dyn Strategy>` 提供运行期可替换，与泛型（Generics）策略（零成本静态分派）形成「同一模式两种成本模型」的自由选择。

判定依据：这些组合的共同点是把**时序/协议约束从文档移到类型**；凡发现代码靠注释约定调用顺序，即存在可 typestate 化的组合机会。
...
分布式环境的失败模式（部分失败、网络分区、时钟漂移）要求模式按**容错语义**组合，而非按代码结构组合。两个经典组合覆盖同步调用链与长事务两类场景：

- **CircuitBreaker ∘ Retry ∘ Timeout**: 顺序不可交换——最外层断路器防止对已知故障端点浪费重试；中间重试只应对瞬时故障；最内层超时为每次尝试兜底。顺序反过来（Timeout ∘ Retry ∘ CircuitBreaker）会让重试风暴在断路器打开前就已压垮下游。
- **Saga**: 把分布式事务分解为本地事务 + 补偿动作（compensation），编排式（orchestration）适合流程复杂、需集中可观测的场景；编舞式（choreography）适合参与方少、事件流简单的场景。

判定依据：组合顺序即语义顺序，画出调用栈从外到内验证每一层的假设是否成立。
...
模式组合的失败案例比成功案例更有信息量。Singleton 与依赖注入的组合是经典冲突：全局单例绕过构造注入，使测试无法替换依赖（反例展示用 `OnceLock` 全局态破坏 DI 的整个过程）。Observer + Singleton 则撞上所有权：观察者注册需要共享可变状态，全局单例的 `'static` 生命周期（Lifetimes）与借用检查直接冲突，编译期即暴露组合不合法。这类反例的价值在于把“模式手感”转化为可编译验证的约束。式组合的状态空间爆炸，本节依次展开。
...
组合安全性定理试图回答“两个各自安全的模式组合后是否仍安全”。核心定理形式为：若模式 A 的安全不变量由类型 T_A 编码、模式 B 由 T_B 编码，且 T_A 与 T_B 的组合不违反孤儿规则（Orphan Rule）与借用规则，则组合模式继承两者的不变量。失效路径主要有两条：共享可变状态突破 `&mut` 排他性（需 `RefCell`/锁降级为运行时（Runtime）检查）、生命周期交集无法命名（需重构为 arena/索引模式）。定理系统给出组合前的检查清单。
...
组合代数的边界测试验证三类编译期约束：状态机组合中跨状态的非法转换（typestate 使 `Closed` 状态调用 `read()` 编译失败）；组合子链上的所有权断裂（中间环节消费了后续环节需要的值，移动语义报错）；嵌套过深的组合子类型导致编译器递归溢出（`recursion_limit` 与编译时间的实际边界）。三类用例分别标定组合代数在正确性、资源管理与可编译性上的边界。类型爆炸（编译错误/编译超时），本节逐一说明其要点。
...
> **修正**: 组合子模式（Combinator Pattern）通过小函数组合构建复杂行为。Rust 的所有权（Ownership）系统要求组合链中的每个中间结果必须正确传递或释放。部分 move（如 `w.0` 被 move 但 `w` 仍被视为整体）会导致编译错误。解构（`let Wrapper(inner) = w`）是安全的替代方案，它明确声明"消耗整个结构体（Struct），提取其字段"。这与 Haskell 的无限惰性组合链不同——Rust 的组合是严格的、所有权感知的。来源: [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html)
...
> **认知功能**: 本 mindmap 从本页「模式组合代数 设计模式的结构化关联与冲突分析」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
