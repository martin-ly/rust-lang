...
> **后置概念**: [算法工程实践](../../06_ecosystem/11_domain_applications/08_algorithm_engineering_practice.md)
> **相关概念**: [数据结构与 Rust](../../06_ecosystem/11_domain_applications/09_data_structures_in_rust.md) · [算法与竞赛编程](../../06_ecosystem/11_domain_applications/07_algorithms_competitive_programming.md) · [形式化设计模式理论](11_formal_design_pattern_theory.md) · [前沿算法技术](../../06_ecosystem/11_domain_applications/11_cutting_edge_algorithms.md)
> **L5 对比视角**: [范式矩阵](../../05_comparative/00_paradigms/01_paradigm_matrix.md) · [Rust vs C++ 形式系统与性能](../../05_comparative/01_systems_languages/01_rust_vs_cpp.md) · [安全边界对比](../../05_comparative/03_domain_comparisons/01_safety_boundaries.md)
>
...
证明迭代算法正确性的标准工具链由三部分构成，本节依次建立：

1. **循环不变式**：在循环每次迭代前后都成立的谓词，通过“初始化—保持—终止”三步证明算法正确——本质上是数学归纳法在程序上的投影。难点从来不在证明框架，而在**找到**正确的不变式：它必须弱到初始化时显然成立，又强到终止时蕴含目标。
2. **二分搜索实例**：不变式“答案（若存在）始终在 `[left, right)` 内”直接决定了边界更新的写法（`left = mid + 1` / `right = mid`），也解释了为什么 `left + (right - left) / 2` 的写法避免溢出——正确性证明与工程细节是一体的。
3. **Hoare 三元组**：`{P} S {Q}` 把前置条件、语句、后置条件形式化，是循环不变式方法的一般化，也是 Kani/Prusti 等工具规范语言的理论来源。
...
形式化验证在 Rust 工程中的落地有两条互补路径，本节分别给出工具与代码模式：

- **Kani 有界模型检查**：基于 CBMC 的符号执行，在 `#[kani::proof]` 函数中用 `kani::any()` 生成任意输入、`kani::assume()` 约束前题，验证“任意长度 ≤ 8 的有序数组上二分搜索不越界”这类全称属性。有界（bound）意味着结论是“深度内无反例”而非数学证明，工程上通常足以覆盖边界条件。
- **不变量断言**：在关键路径插入 `assert!`/`debug_assert!` 把循环不变式变为运行时可检查规约，配合 fuzzing（cargo-fuzz）形成“轻量形式化”——这是多数项目投入产出比最高的验证手段。

选择依据：安全关键模块（Module）上 Kani，全项目基线用断言 + fuzz。
...
| **类型系统（Type System）证明** | 自动 | 类型级不变量 | Typestate / const generics | 表达能力受限，无法表达所有性质 |
