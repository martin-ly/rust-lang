...
> **前置概念**: [Type System](../../01_foundation/02_type_system/04_type_system.md) · [Lifetimes](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md) · [Generics](../../02_intermediate/01_generics/02_generics.md)
> **后置概念**: [Type Theory](02_type_theory.md) · [RustBelt](../02_separation_logic/04_rustbelt.md)
...
本节将「技术细节」分解为若干主题：生命周期位置的变型推导、结构体与枚举的变型与函数指针的变型。
...
- [RustBelt](../02_separation_logic/04_rustbelt.md) — Rust 安全性的形式化模型
...
「边界测试：子类型变异性的编译错误」涉及边界测试：协变与逆变的生命周期误用（编译错误）、边界测试：`UnsafeCell` 的不变性（编译错误）、边界测试：逆变与 `fn` 参数的不变性（编译错误）、边界测试：`UnsafeCell` 的不变性（编译错误/运行时 UB）等7个方面，本节逐一说明其要点。
...
> **修正**: `UnsafeCell<T>` 对 `T` 是**不变**（invariant）的——不允许任何生命周期缩短或延长。这是因为 `UnsafeCell` 提供内部可变性，若允许协变，可能将短生命周期引用（Reference）存储在期望长生命周期的上下文中，导致悬垂引用。`&mut T` 对 `T` 也是不变的，`Box<T>` 对 `T` 是协变的，`*const T` 对 `T` 是协变的，`*mut T` 对 `T` 是不变的。变异性的选择是 Rust 类型系统（Type System）安全性的关键设计。来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：Rust 中 `&'a T` 对生命周期 `'a` 是什么变…、测验 2：`&mut T` 对 `T` 是什么变型？为什么不是协变？（…、测验 3：`Box<T>`、`Vec<T>`、`Option<T>`…、测验 4：`UnsafeCell<T>` 对 `T` 是什么变型？为什…等5个方面。
...
生命周期子类型化：可以将长生命周期的引用（Reference）传给要求短生命周期参数的函数。编译器自动处理协变转换。
</details>
