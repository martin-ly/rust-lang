...
>
> **EN**: Formal Methods
> **Summary**: ``` P * Q ``` <details> <summary>💡 点击展开答案与解析</summary> **答案**：`P * Q` 表示memory堆可以**分割**为两个不相交的部分，一部分满足 `P`，另一部分满足 `Q`。 **formal methods定义（教学类比）**： ``` h ⊨ P * Q   ⟺   ∃h₁, h₂.  h = h₁ ⊎ h₂  ∧  h₁ ⊨ P  ∧  h₂ ⊨ Q ``` - `h` 是整个memory堆（heap） - `h₁ ⊎ h₂` 表示 `h₁` 和 `h₂` 的**不相交并集**（disjoint union） - `h₁` 和 `h₂` 没有重叠的memory地址 **与
...
> **⚠️ 声明**: 本测验使用形式化符号辅助直觉理解，所呈现的"定理/引理/推论"为**教学类比**，非经机器验证的严格数学证明。如需严格形式化验证，请参考 Verus、Kani、Coq。
>
> **Bloom 层级**: 分析 → 评价
...
| `x ↦ v`（单点堆） | `let x = v;`（所有权） |
| `P * Q` | 两个不相交的所有权资源 |
| 框架规则（Frame Rule） | 借用检查器：验证局部操作时不影响无关资源 |
...
**知识点**：分离逻辑的 `*` 是 Rust 所有权系统"资源独占"特性的数学根基。框架规则是局部推理（local reasoning）的基础。[→ 分离逻辑详解](./11_separation_logic.md)
...
| **Iris 分离逻辑** | 使用高阶分离逻辑（higher-order separation logic）表达所有权和生命周期 |
...
**知识点**：RustBelt 证明了 Rust 的安全保证有坚实的数学基础——不是工程经验，而是可验证的定理。[→ RustBelt 详解](./04_rustbelt.md)
...
**Rust 的增强**：Rust 的类型系统还保证：

- **内存安全**：没有 use-after-free、double-free、悬垂指针
...
| C/C++ | 无（类型系统不保证内存安全） |
| Java | 类型安全 + 内存安全（GC） |
| Rust | 类型安全 + 内存安全 + 数据竞争自由（无 GC） |

**知识点**：Progress & Preservation 是类型理论的"圣杯"。Rust 通过所有权系统将这些经典定理扩展到了内存安全和并发安全领域。[→ 类型语义详解](./21_type_semantics.md)
...
**知识点**：线性逻辑为 Rust 所有权系统提供了精确的数学模型。理解 `⊗` 和 `⊸` 有助于深入理解为什么 Rust 的所有权规则是这样的。[→ 线性逻辑详解](./01_linear_logic.md)
...
| **BorrowSanitizer** | 运行时检测 | 别名违规、use-after-free | 测试期（类似 ASan） |
...
**知识点**：没有单一工具能检测所有问题。Kani 覆盖所有路径但受状态空间限制，Miri 精确但只覆盖单路径，Sanitizers 有运行时开销。[→ 验证工具链详解](./05_verification_toolchain.md)
...
**知识点**：Hoare 逻辑是程序验证的基石。分离逻辑通过 `*` 运算符扩展了 Hoare 逻辑，使其能够表达堆内存的局部操作——这正是 Rust 所有权系统需要的数学语言。[→ Hoare 逻辑详解](./15_hoare_logic.md)
...
**Soundness Bug**：编译器错误地接受了本应拒绝的不安全代码，导致 well-typed 程序在运行时出现未定义行为。
...
| Soundness bug | 借用检查器遗漏了悬垂引用 | 编译通过，运行期 UB、安全保证崩溃 |

**历史 Soundness Bug 示例**：

| Bug | 描述 | 状态 |
|:---|:---|:---:|
| `self` 参数生命周期 | `fn foo(self: &Self)` 在某些情况下绕过生命周期检查 | 已修复 |
| `match` 绑定模式 | 某些模式绑定创建了无效引用 | 已修复 |
...
**知识点**：Soundness 是 Rust 的核心承诺。每个 soundness bug 都是对这个承诺的破坏，因此 Rust 团队将其列为最高优先级。[→ RustBelt 详解](./04_rustbelt.md)
...
**知识点**：框架规则是"组合性验证"的数学基础——复杂系统的验证可以分解为组件的局部验证。[→ 分离逻辑详解](./11_separation_logic.md)
...
|:---|:---|:---|
| 模型 | 栈结构：新引用压栈，旧引用失效 | 树结构：引用形成父子关系 |
...
**知识点**：Tree Borrows 是 Rust 内存模型的最新研究方向，旨在平衡优化能力和代码兼容性。Miri 的 `-Zmiri-tree-borrows` 标志启用该模型。[→ RustBelt 详解](./04_rustbelt.md)
...
| 4–6/10 | 🔄 需巩固基础 | 重读 [RustBelt](./04_rustbelt.md) · [Separation Logic](./11_separation_logic.md) |
| 0–3/10 | 📚 建议重新开始 | 从 [Linear Logic](./01_linear_logic.md) 基础概念开始 |
