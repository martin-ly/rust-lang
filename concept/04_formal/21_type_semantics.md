...
> **后置延伸**: [Axiomatic Semantics](./20_axiomatic_semantics.md) · [RustBelt](./04_rustbelt.md)
...
- v1.0 (2026-05-25): 初始创建——类型语义专题，覆盖进步/保持定理、Rust 特有类型的语义、子类型与变型
...
> **[Pierce 2002 — TAPL, Ch.8](https://www.cis.upenn.edu/~bcpierce/tapl/)** 类型系统的核心元定理是**进步**（Progress）与**保持**（Preservation）：
>
...
其中 `Σ` 是**所有权/借用状态**，这是标准 λ 演算的类型系统中所没有的。RustBelt 的形式化证明表明：Rust 的扩展进步/保持定理**蕴含内存安全**——良类型程序不会产生悬垂指针、数据竞争或重复释放。[来源: [Pierce 2002, Ch.8](https://www.cis.upenn.edu/~bcpierce/tapl/)] · [来源: [RustBelt — Jung et al. 2018](https://plv.mpi-sws.org/rustbelt/popl18/)]
...
>
> 在大多数语言中，类型安全 ≠ 内存安全（C/C++ 是典型反例）。Rust 的独特贡献在于：**类型系统被设计为内存安全的充分条件**。

Rust 的类型安全 → 内存安全 的推理链：
...
> **关键洞察**: 这不是偶然的——Rust 的类型系统**从设计之初就以内存安全为目标**。`&mut T` 不是"可写引用"，而是**独占访问令牌**；`Box<T>` 不是"堆指针"，而是**线性资源**；`Option<T>` 不是"可为空的值"，而是**显式存在性证明**。这些设计使 Cardelli 的"类型安全 → 内存安全"蕴含关系在 Rust 中成为定理，而非巧合。[来源: [Cardelli 1996](https://dl.acm.org/doi/10.1145/6041.6042)] · [来源: [Rust Reference — Type Safety](https://doc.rust-lang.org/reference/behavior-considered-undefined.html)]
...
| **类型特征** | **语义类别** | **形式化表达** | **Rust 语法** | **内存安全影响** |
|:---|:---|:---|:---|:---|
| `i32`, `bool` | 标量类型 | 简单值（SML/NJ 风格）| `let x: i32 = 5;` | 无（栈分配，Copy）|
| `&T` | 共享借用 | 只读能力（Capability）| `let r = &x;` | ✅ 禁止修改 |
| `&mut T` | 独占借用 | 可写能力（线性资源）| `let r = &mut x;` | ✅ 禁止别名 |
| `Box<T>` | 堆所有权 | 线性类型（Linear Type）| `let b = Box::new(x);` | ✅ 唯一释放责任 |
| `Rc<T>` | 共享所有权 | 仿射类型 + 引用计数 | `let r = Rc::new(x);` | ✅ 运行时检查（非零成本）|
| `Arc<T>` | 线程安全共享 | 仿射类型 + 原子引用计数 | `let a = Arc::new(x);` | ✅ 原子操作保证 |
| `Option<T>` | 存在类型 | `T + ⊥`（可为空和）| `let o: Option<i32> = None;` | ✅ 强制空值检查 |
| `Result<T, E>` | 计算效应 | `T ⊕ E`（错误和）| `let r: Result<i32, Err> = Ok(5);` | ✅ 强制错误处理 |
...
在标准类型论中，引用（reference）通常被视为指针的同义词。Rust 的借用类型 `&T` 和 `&mut T` 的语义远比指针丰富——它们是**能力**（Capability），而非**地址**。
...
> **与 C++ 引用的对比**: C++ 的 `const T&` 和 `T&` 在语法上类似 Rust 的 `&T` 和 `&mut T`，但语义有本质区别：
>
> - C++ 引用**不保证**别名自由——`const T&` 可能通过其他路径被修改（`mutable` 成员、`const_cast`）
...
`Box<T>` 是**线性类型**（Linear Type）的直接体现：每个 `Box<T>` 值有且只有一个所有者，赋值时所有权转移，原变量失效，所有者离开作用域时自动释放。
...
`Arc<T>` 的语义核心不是引用计数本身，而是**线程安全保证**：`Arc<T>: Send + Sync` 当且仅当 `T: Send + Sync`。这是类型语义与并发语义的交汇点——类型系统通过 trait bounds 自动推导并发安全性。
...
`dyn Trait` 是**动态分发**（Dynamic Dispatch）的类型语义：编译时不知道具体类型，运行时通过 vtable 查找方法实现。这带来了**类型擦除**（Type Erasure）和**运行时开销**（间接调用 + vtable 查找）。
...
`impl Trait` 是 Rust 的**零成本抽象**核心：它在语义上提供多态（存在/全称量化），在实现上通过单态化消除运行时开销。
...
> - C++ 的模板对应 Rust 的 `impl Trait`（编译时单态化，零成本），但 C++ 模板无 trait bounds 的显式语义约束
...
在标准类型论中，子类型（Subtyping）通常基于**结构包含**（structural inclusion）：`Cat <: Animal` 因为 Cat 具有 Animal 的所有特征。Rust 的生命周期子类型 `<'a: 'b>`（`'a` 比 `'b` 长）基于**时间包含**（temporal inclusion）。
...
> **来源**: [Rust RFC 1238 — Non-Lexical Lifetimes](https://rust-lang.github.io/rfcs/1238-nonlexical-lifetimes.html) · [Rust Reference — Subtyping](https://doc.rust-lang.org/reference/subtyping.html)
...
| **泛型特化** | 不稳定特性（`min_specialization`）| 无限制特化与类型推断的完备性冲突 | 目前限制特化场景以避免 soundness 漏洞 |

> **来源**: [Rust RFC 1210 — impl specialization](https://rust-lang.github.io/rfcs/1210-impl-specialization.html) · [Rust RFC 1598 — Generic Associated Types](https://rust-lang.github.io/rfcs/1598-generic_associated_types.html) · [Rust RFC 2000 — Const Generics](https://rust-lang.github.io/rfcs/2000-const-generics.html)
...
```rust,compile_fail
fn main() {
    let mut cats: Vec<&str> = vec!["meow"];
    let animals: Vec<&str> = cats;  // Vec<T> 是协变的
    // 若允许，以下代码将导致类型混淆:
    // animals.push("bark");        // 字符串切片不可变，但逻辑上 animals 应可放入任何 &str
}

// 实际上 Rust 的 Vec<T> 对 T 是协变的，但上述代码不会编译失败。
// 真正的问题是 Java 风格的数组协变：
// Object[] objects = new String[1];
// objects[0] = 42;                // ArrayStoreException 在运行时抛出

// Rust 通过以下机制避免:
// 1. Vec<T> 是协变的，但 &mut Vec<T> 是不变的
// 2. 写入操作需要 &mut self，不允许子类型替换
```
...
fn main() {
    let d1 = get_drawable();
    let d2 = get_drawable();
    // let v: Vec<dyn Drawable> = vec![Box::new(d1), Box::new(d2)];
    // ❌ 编译错误: impl Trait 返回的是具体类型（单态化），不是 dyn Trait
    // 每个 impl Trait 的返回位置对应一个匿名存在类型，不同调用可能对应不同类型
}
```
...
> **修正**: 生命周期子类型系统通过**区域约束求解**在编译期检测悬垂引用。编译器为每个引用标注生命周期参数，然后求解约束系统：若存在约束 `'a : 'local`（'a 必须比局部生命周期长），而函数签名要求返回 `'a`，则编译器推断出矛盾，拒绝编译。这是进步/保持定理的直接应用：`dangling_reference` 在返回时试图构造一个不满足保持条件的引用，类型系统通过生命周期约束拒绝这一操作。[来源: [Rust Reference — Lifetimes](https://doc.rust-lang.org/reference/items/lifetimes.html)] · [RFC 2094 — NLL](https://rust-lang.github.io/rfcs/2094-nll.html) · [RustBelt — Lifetime Semantics](https://plv.mpi-sws.org/rustbelt/popl18/)
...
`dyn Trait` 是**动态分发**的存在类型：编译时不知道具体类型，运行时查 vtable。`impl Trait` 是**静态分发**的存在/全称类型：编译器知道具体类型，通过单态化消除运行时开销。选择 `dyn` 还是 `impl` 是工程权衡，不是风格偏好。
...
| **T-133: 借用规则 Soundness** | `&T` / `&mut T` 生命周期有效 | 无悬垂指针、无数据竞争 | 借用检查器 | RFC 2094 · RustBelt |
| **T-134: 所有权线性性** | `Box<T>` 单所有权 | 无重复释放、无泄漏（safe Rust）| Move 语义 | RustBelt · Linear Logic |
...
| **T-137: 全称类型引入** | `∀α. α : Trait → F(α)` | 单态化后类型保持 | `impl Trait` | Pierce TAPL Ch.23 · RFC 1522 |
...
> **修正**: `PhantomData<T>` 是 Rust 类型系统的**语义桥接器**。它在运行时不占内存（ZST，Zero-Sized Type），但在编译期向类型系统传递关键语义信息："这个结构在逻辑上与 `T` 相关联"。这是 Rust 实现**零成本抽象**的典型模式——语义信息在编译期使用，运行时不带来任何开销。没有 `PhantomData`，像 `Vec<T>`、`Box<T>` 这样的容器无法在内部使用裸指针的同时保持正确的变型推断。[来源: [Rust Reference — PhantomData](https://doc.rust-lang.org/std/marker/struct.PhantomData.html)] · [Rust Nomicon — PhantomData](https://doc.rust-lang.org/nomicon/phantom-data.html)
...
struct SelfReferential {
    data: String,
    // ptr: *const String,   // 指向 data 的指针（自引用）
}
...
fn move_self_ref() {
    ...
    let moved = s;            // Move: s 被移动到 moved
    // println!("{}", unsafe { *s.ptr });  // ❌ 悬垂指针！
}
...
> **修正**: `Pin<&mut T>` 的类型语义是**位置稳定性**（Location Stability）——保证 `T` 在内存中的地址不变。这对于自引用结构（如 `async fn` 编译后的状态机）至关重要，因为状态机内部可能包含指向自身的指针。`Pin` 不是普通引用，而是**带有不变式的引用**：`Pin<&mut T>` 不仅提供可变访问，还提供"不移动"的契约。这个契约通过类型系统强制：`Pin` 的构造函数是 `unsafe`，但构造后的使用是安全的——这是 Rust **安全抽象**（Safe Abstraction）范式的典范。[来源: [Rust Reference — Pin](https://doc.rust-lang.org/std/pin/struct.Pin.html)] · [RFC 2349 — Pin](https://rust-lang.github.io/rfcs/2349-pin.html) · [Rust Async Book — Pinning](https://rust-lang.github.io/async-book/04_pinning/01_chapter.html)

---

## 相关概念文件

- [类型论基础](./02_type_theory.md) — HM 类型系统、System F、多态
