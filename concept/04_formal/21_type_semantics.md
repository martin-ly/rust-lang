...
| `Arc<T>` | 线程安全共享 | 仿射类型 + 原子引用计数 | `let a = Arc::new(x);` | ✅ 原子操作（Atomic Operations）保证 |
...
`Arc<T>` 的语义核心不是引用计数本身，而是**线程安全保证**：`Arc<T>: Send + Sync` 当且仅当 `T: Send + Sync`。这是类型语义与并发语义的交汇点——类型系统通过 trait bounds 自动推导并发安全（Concurrency Safety）性。
...
`impl Trait` 是 Rust 的**零成本抽象（Zero-Cost Abstraction）**核心：它在语义上提供多态（存在/全称量化），在实现上通过单态化（Monomorphization）消除运行时开销。
