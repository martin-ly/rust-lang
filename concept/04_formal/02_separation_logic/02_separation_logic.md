...
本节从边界测试：独占资源的分割与重组（编译错误）、边界测试：`Box::leak` 与资源永久转移（运行时行为）、边界测试：分离逻辑中的帧规则违反（编译错误）、边界测试：GhostCell 的分离逻辑建模（编译错误）等6个方面切入，剖析「边界测试：分离逻辑的编译错误」的核心内容。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：分离合取 *（理解层）、测验 2：帧规则（Frame Rule）（应用层）、测验 3：所有权 ↔ 分离逻辑映射（分析层）、测验 4：GhostCell 与分离逻辑（专家级）等5个方面。
