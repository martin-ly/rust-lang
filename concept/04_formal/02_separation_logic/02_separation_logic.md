...
> **前置概念**: [Linear Logic](../01_ownership_logic/01_linear_logic.md) · [Ownership Formalization](../01_ownership_logic/02_ownership_formal.md) · [RustBelt](01_rustbelt.md) · [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md)
> **后置概念**: [Verification Toolchain](../04_model_checking/01_verification_toolchain.md) · [Type Theory](../00_type_theory/01_type_theory.md)
>
...
分离逻辑（Separation Logic, Reynolds 2002）是霍尔逻辑对堆与别名的扩展，本节展开三个技术核心：

1. **分离合取 `P ∗ Q`**：「P 与 Q 在不相交的内存区域上成立」——`∗` 而非 `∧` 是全部创新的来源：`x ↦ 1 ∗ y ↦ 2` 蕴含 `x ≠ y`（别名信息内建）；
2. **Frame 规则**：`{P} C {Q}` ⟹ `{P ∗ R} C {Q ∗ R}`（当 C 不修改 R 涉及的变量）——局部推理的形式化：只规约被触及的内存，其余自动保持；Rust 借用（Borrowing）检查的「`&mut` 独占期间其余引用休眠」正是 frame 规则的类型化版本；
3. **魔杖 `P ∗- Q`**：「若补上 P 区域则得 Q」——资源的分期与归还的推理工具，对应借用结束时的「权限归还」。

技术细节的组织：先语法语义（堆模型、谓词语义），再推理规则（含 frame），最后与 Rust 借用规则的逐项对照表。
...
1. **「`∗` 只是加强版的 `∧`」**：错——`P ∗ P` 不蕴含 `P`（资源不可复制），分离逻辑是子结构逻辑（无 contraction/weakening）；把 `∗` 读作 `∧` 会在 frame 规则处推出矛盾；
2. **「分离逻辑只能推 `&mut`」**：共享借用同样可建模——分数权限（fractional permissions，`1/2` 权限只读、合起来恢复写）是共享借用的标准形式化；Rust 的 `&T`  ⟷ 任意正分数权限；
3. **「frame 规则总是合法」**：frame 有副作用条件（C 不访问 R 的资源）——`unsafe` 代码中裸指针可越界访问，使 frame 规则失效；这正是 `unsafe` 需要额外论证（Miri 的 Stacked Borrows 检查别名纪律）的逻辑根源。
...
- [Type Theory](../00_type_theory/01_type_theory.md) — 类型论
...
本节把「分离逻辑：Rust 所有权的形式化根基」的规则推到编译器与运行时的边界上逐一实测：边界测试：独占资源的分割与重组（编译错误）、边界测试：`Box::leak` 与资源永久转移（运行时行为）、边界测试：分离逻辑中的帧规则违反（编译错误）、边界测试：GhostCell 的分离逻辑建模（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
本组测验围绕测验 1：分离合取 *（理解层）、测验 2：帧规则（Frame Rule）（应用层）、测验 3：所有权 ↔ 分离逻辑映射（分析层）、测验 4：GhostCell 与分离逻辑（专家级）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
...
- A. 借用（Borrowing）检查器允许函数使用未被参数引用的局部变量
