...
本节聚焦「核心机制」，覆盖别名模型基础、LLVM 插桩与 Retag Intrinsics与错误报告示例。论述顺序由定义到边界：先明确「核心机制」在「BorrowSanitizer 运行时（Runtime）别名模型检测」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「核心机制」的判定标准，并指出它在全页论证链中的位置。
...
1. safe 层：借用（Borrowing）检查器（编译期）；
2. unsafe 层：Miri（解释执行，覆盖 Stacked/Tree Borrows 全规则）；
3. 性能敏感场景：BorrowSanitizer 作为 Miri 太慢时的补充采样。

**陷阱要点**：「sanitizer 通过」只证明本次执行路径未触发可观测违规，不是形式化意义上的无 UB；形式化保证仍要靠分离逻辑/类型系统（Type System）层面的证明义务。
