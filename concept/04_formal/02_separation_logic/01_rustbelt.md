...
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/04_inter_layer_map.md) §4.1 "内存安全（Memory Safety）完备性" · §5.2 "定理一致性检查"
...
> (Source: [RustBelt — POPL 2018](https://plv.mpi-sws.org/rustbelt/popl18/))
...
`Mutex<T>` 是 CSL 资源不变量的经典实例。设 `m` 为 `Mutex<T>` 地址，`l` 为被保护数据地址： (Source: [RustBelt Project](https://plv.mpi-sws.org/rustbelt/))
...
以下伪代码展示如何用 CSL 注释描述 Rust 并发原语的契约： (Source: [PLDI 2025 — Tree Borrows](https://plv.mpi-sws.org/rustbelt/))
...
1. `&mut T` 必须独占（无其他活跃引用（Reference））
2. `&T` 必须有效（指向已初始化且未变性的内存）
3. `*const T`/`*mut T` 的使用必须恢复上述不变量后再创建 safe 引用（Reference）
...
步骤3: "RustBelt 怎么证明安全的?"
   ...
   Coq 机械检验，不受直觉误差影响。
   [Jung et al., POPL 2018](https://www.sigplan.org/Conferences/POPL/)
...
| **HashMap** | 内存安全（Memory Safety） + 哈希不变量 | 🔍 待验证 | 极高 | SwissTable / 开放寻址 / 树化协议 |
...
> **过渡**: 标准库原语的验证是 RustBelt 理论成果向工业代码延伸的必经之路。从学术证明到工程师日常可用，需要验证工具链的桥接——§8 给出完整工具链光谱与选型指南。 (Source: [POPL 2018 — RustBelt](https://dl.acm.org/doi/10.1145/3158154))
...
| **层次** | **对象** | **工具** |
|:---|:---|:---|
| **L0 内存安全（Memory Safety）** | UAF, DF, 数据竞争 | Rust 编译器 |
...
3. 每次循环迭代时不变式成立（body_invariant） (Source: [Wikipedia — Separation Logic](https://en.wikipedia.org/wiki/Separation_logic))
...
> **过渡**: 验证工具链构成了从"编译器保证"到"数学证明"的连续光谱。理解这些权衡后，以下思维导图总览 RustBelt 定理依赖关系与工具生态位置。 (Source: [Wikipedia — Affine Logic](https://en.wikipedia.org/wiki/Affine_logic))
...
| Unsafe | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) | 验证边界 · C1 边界层 · L3 Unsafe 映射 |
...
> **关键洞察**：读锁允许多个 reader **共享**资源不变量，写锁要求**独占**资源。Iris 的 **"fractional permissions"**（分数权限）精确建模了 "n 个读者各持 1/n 份额" 的语义。 (Source: [Wikipedia — Linear Logic](https://en.wikipedia.org/wiki/Linear_logic))
...
- **演进方向**: 补充各工具的具体代码示例（Prusti `#[requires]`/`#[ensures]`、Kani `#[kani::proof]`、Verus `proof fn`、Creusot 前置/后置条件）—— 已完成 §7.1–7.4
- **演进方向**: 补充验证工具与 CI/CD 的集成方案（Kani 在 AWS 的流水线实践、Miri 在 crater 中的回归检测）—— 已完成 §7.5
- **演进方向**: 补充 RefinedRust 的自动化分离逻辑推导示例 —— 已完成 §7.6
- **演进方向**: 补充 RustHornBelt 对 unsafe 功能正确性验证的扩展说明 —— 已完成 §7.7
- **演进方向**: 补充 §3 CSL 中 `RwLock` 与 `Condvar` 的形式化规约（读锁共享协议的 Iris 建模）—— 已完成 §7.8
- **演进方向**: 补充 `Vec` 重新分配时指针失效的形式化处理（借用与重分配的交互）—— 已完成 §7.9
...
本节把「RustBelt & Verification Toolchain（RustBelt 与验证工具链）」的规则推到编译器与运行时的边界上逐一实测：边界测试：违反唯一所有权（编译错误）、边界测试：Send/Sync 自动推导失败（编译错误）、边界测试：drop 后使用（编译错误）、边界测试：共享借用期间可变借用（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> **相关谓词映射**: [own(τ) / shr(κ, ℓ) 完整映射](../../00_meta/02_sources/02_rustbelt_predicate_map.md) · [标准库谓词规范](../../00_meta/02_sources/02_rustbelt_predicate_map.md#五谓词组合与-rust-代码对应)
> **相关 FTA**: [Unsafe 契约失效树](../../00_meta/00_framework/fault_tree_analysis_collection.md#六unsafe-契约失效树)
...
> **修正**: `Cell<T>` 通过 `UnsafeCell` 提供内部可变性，其 `set` 方法接受 `&self`。RustBelt 为 `UnsafeCell` 赋予特殊谓词 `na(τ, ℓ)`（non-atomic），允许从共享引用进行可变访问，但这排除了别名读取保证。(Source: [RustBelt Paper](https://plv.mpi-sws.org/rustbelt/))
...
> **修正**: `mem::forget` 消耗所有权但不执行 `Drop`，导致资源泄漏。在 RustBelt 中，`own(τ)` 谓词包含释放义务；`forget` 通过将释放义务"遗忘"来形式化资源泄漏。Rust 2021 起 `mem::forget` 为安全函数，因为资源泄漏不被视为 unsafe。(Source: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html))
