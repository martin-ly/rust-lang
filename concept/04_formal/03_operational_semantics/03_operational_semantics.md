...
- [操作语义：程序行为的形式化定义](#操作语义程序行为的形式化定义)
  ...
  - [十、边界测试：操作语义的编译错误](#十边界测试操作语义的编译错误)
    ...
    - [10.3 边界测试：求值顺序与副作用的确定性（编译错误 E0502）](#103-边界测试求值顺序与副作用的确定性编译错误-e0502)
    - [10.8 边界测试：所有权移动后使用的形式化禁止（编译错误 E0382）](#108-边界测试所有权移动后使用的形式化禁止编译错误-e0382)
    - [10.9 边界测试：共享借用与可变借用互斥（编译错误 E0502）](#109-边界测试共享借用与可变借用互斥编译错误-e0502)
...
本节展开操作语义的技术构造，三个核心组件：

1. **语法与值**：核心演算 λRust 的语法子集——项（term）、值（value，归约的终点）、求值上下文（evaluation context，标记「下一步归约发生在哪里」）；
2. **归约规则**：小步语义（small-step）的转移关系 `t → t'`——每条规则形如「前提状态下，项按一条规则重写」；β 归约、let 替换、分支选择是基本规则；Rust 特有的 move 在语义层对应「资源转移」规则；
3. **类型保持与进展（Preservation & Progress）**：类型系统的健全性即两条定理——归约保持类型（well-typed 项归约后仍 well-typed）、well-typed 项不会卡住（stuck）——Rust 借用（Borrowing）检查的形式化意义正是「排除了别名导致的 stuck 状态」。

技术细节的组织顺序对应 TAPL Ch.3/Ch.8 的标准展开，每规则附 λRust 语法的实例。
...
1. **「操作语义 = 实现」**：错——语义是规范（specification），编译器是实现；λRust 的归约规则定义「正确行为」，rustc 可以任何方式达到该行为（优化改变中间状态不得改变可观察结果）；
2. **「小步语义能描述并发」**：经典小步语义是顺序的交错（interleaving）模型——真实并发（弱内存序）需要额外的内存模型公理（C++11/Rust 采用 SC-DRF 框架），单交错语义无法表达 store buffering 等弱序现象；
3. **「求值顺序是语义规定」**：Rust 的求值顺序部分未指定（函数参数求值顺序）——语义给出的是「所有合法顺序的结果一致性（Coherence）」保证，依赖特定顺序的代码是未指定行为风险点。
...
形式化 Rust 的项目各有覆盖边界，比较矩阵回答"为什么没有单一形式化覆盖全部 Rust"：

- **语言子集差异**：RustBelt 形式化的是 λRust（核心演算），KRust 基于 K 框架覆盖 MIR 子集，RustSEM 直接面向源码语义——三者的"Rust"不是同一个语言子集，无法直接比较覆盖度。
- **验证目标差异**：RustBelt 证明 unsafe 抽象的声音性（类型系统层面），Kani/CBMC 验证具体程序属性（模型检测层面），Ferrocene 规约面向认证（工程层面）。目标不同决定了形式化的抽象层不同。
- **Trait 系统的形式化缺口**：所有当前模型对 trait 求解（含 coherence、特化、关联类型）的覆盖都不完整——这是各项目一致承认的公开缺口，也是 Rust 形式化的下一个攻坚点。

完整比较矩阵沿"语言子集 / 验证目标 / 工具成熟度 / 机器验证程度"四轴展开，关键发现列于 5.3。化缺口等7个方面，本节逐一说明其要点。
...
- **发现 1：Trait 系统在所有当前形式化模型中均为「未形式化」状态**——推论：任何依赖 trait 求解正确性的证明（如泛型（Generics）代码的验证）目前都建立在未验证的假设上；实践中通过对具体单态化（Monomorphization）实例验证来回避。
- **发现 2：仅 RustSEM 与 KRust 直接形式化 Rust 源码/MIR**——其余项目（RustBelt、Aeneas）形式化的是核心演算或中间表示 LLBC；推论：源码级语义与演算级语义之间存在未验证的翻译间隙。
- **发现 3：RustBelt 通过 Iris ghost state 提供了 unsafe 抽象的验证框架**——推论：标准库之外的 unsafe 代码（如 `crossbeam`）要获得同等保证，需重写为 Iris 证明，成本极高。
- **发现 4：没有任何形式化覆盖过程宏（Procedural Macro）或编译期执行**——宏展开前的源码才是程序员看到的真相，形式化的是展开后的代码；推论：宏生成代码的验证必须依赖展开后分析，无法静态保证宏本身的 hygiene 之外的语义性质。
...
本节把「操作语义：程序行为的形式化定义」的规则推到编译器与运行时的边界上逐一实测：边界测试：求值顺序的未定义行为（运行时 UB）、边界测试：panic 的栈展开语义（运行时行为）、边界测试：形式化规则违反导致的编译错误（编译错误）、边界测试：悬垂引用的形式化禁止（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
- B. 所有权（Ownership）转移和生命周期（Lifetimes）
- C. 宏（Macro）扩展
...
2. **借用创建**：`&x` 创建引用的同时引入生命周期（Lifetimes）约束
...
> **认知功能**: 本 mindmap 从本页「操作语义 程序行为的形式化定义」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
