...
3. **类型保持与进展（Preservation & Progress）**：类型系统的健全性即两条定理——归约保持类型（well-typed 项归约后仍 well-typed）、well-typed 项不会卡住（stuck）——Rust 借用检查的形式化意义正是「排除了别名导致的 stuck 状态」。
...
3. **「求值顺序是语义规定」**：Rust 的求值顺序部分未指定（函数参数求值顺序）——语义给出的是「所有合法顺序的结果一致性」保证，依赖特定顺序的代码是未指定行为风险点。
...
- **发现 1：Trait 系统在所有当前形式化模型中均为「未形式化」状态**——推论：任何依赖 trait 求解正确性的证明（如泛型代码的验证）目前都建立在未验证的假设上；实践中通过对具体单态化实例验证来回避。
...
- **发现 4：没有任何形式化覆盖过程宏或编译期执行**——宏展开前的源码才是程序员看到的真相，形式化的是展开后的代码；推论：宏生成代码的验证必须依赖展开后分析，无法静态保证宏本身的 hygiene 之外的语义性质。
