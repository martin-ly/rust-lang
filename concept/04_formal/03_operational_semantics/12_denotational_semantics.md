...
「边界测试：指称语义的编译错误」涉及边界测试：非终止计算与 `loop {}` 的类型（编译错误）、边界测试：`panic!` 的指称与 `Result` 的指称分离（编…、所有权即线性性与生命周期即区域，本节逐一说明其要点。
...
本节从边界测试：发散函数（`!`）的指称语义（编译错误）、边界测试：`unsafe` 代码的语义鸿沟（运行时 UB）与边界测试：不动点语义与递归类型的无限展开（编译错误）切入，剖析「权威来源索引」的核心内容。
