...
本节把「Hoare 逻辑：程序验证的形式化基础与 Rust 契约」的规则推到编译器与运行时（Runtime）的边界上逐一实测：边界测试：前置条件与后置条件的类型编码（编译错误）、边界测试：循环不变量的违反（逻辑错误）、边界测试：霍尔逻辑的不变量与 Rust 的 `while let`（编…、边界测试： weakest precondition 与 `unsaf…等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
