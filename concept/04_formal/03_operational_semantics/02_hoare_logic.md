...
本节从边界测试：前置条件与后置条件的类型编码（编译错误）、边界测试：循环不变量的违反（逻辑错误）、边界测试：霍尔逻辑的不变量与 Rust 的 `while let`（编…、边界测试： weakest precondition 与 `unsaf…等6个方面切入，剖析「边界测试：Hoare 逻辑的编译错误」的核心内容。
...
本节围绕「嵌入式测验（Embedded Quiz）」展开，依次讨论测验 1：Hoare 三元组（理解层）、测验 2：最弱前置条件（Weakest Precondition）（应…、测验 3：循环不变量（应用层）、测验 4：Rust 类型系统对 Hoare 逻辑的编码（分析层）等5个方面。
