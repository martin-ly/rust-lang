...
- **与操作语义的一致性**：指称解释应与操作语义**充分抽象**（adequacy）——操作语义终止当且仅当指称非 ⊥；Rust 无完整指称语义（工作集中在操作语义/λRust），本节给出片段解释与文献锚点。
...
1. **「指称语义能处理所有 Rust 构造」**：边界——`unsafe`、内联汇编、平台相关行为的指称需要「非标准域」（如带硬件模型的域），现有工作只覆盖安全子集；把片段解释当完整语义会过度外推；
