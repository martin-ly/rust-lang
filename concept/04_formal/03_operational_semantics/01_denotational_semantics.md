...
- [指称语义与领域理论](#指称语义与领域理论)
  ...
  - [认知路径](#认知路径)
    ...
    - [反命题与边界](#反命题与边界)
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：闭包引用自身（rustc 1.97.0 实测）](#反例闭包引用自身rustc-1970-实测)
    - [✅ 修正：用 `fn`（自引用合法）或显式 fix 组合子](#-修正用-fn自引用合法或显式-fix-组合子)
...
本节给出 Rust 核心构造的指称解释（denotation）——把语法映射到数学对象：

- **类型的指称**：`i32` ⟷ 32 位整数集（带环绕算术的商结构）；`Option<T>` ⟷ 提升域（T 的解释加 ⊥ 元素，对应 `None`）；`&'a T` ⟷ 「生命周期 'a 内有效的只读引用」——指称中含时间/区域参数；
- **表达式的指称**：`if`/`match` ⟷ 条件函数；`loop` ⟷ 最小不动点（域论的 Kleene 构造）；move 表达式 ⟷ 「资源转移」在指称层体现为线性函数；
- **与操作语义的一致性（Coherence）**：指称解释应与操作语义**充分抽象**（adequacy）——操作语义终止当且仅当指称非 ⊥；Rust 无完整指称语义（工作集中在操作语义/λRust），本节给出片段解释与文献锚点。

价值：指称视角回答「这个构造**是什么**」（数学对象），与操作视角的「怎么算」互补。
...
本节把「指称语义与领域理论」的规则推到编译器与运行时的边界上逐一实测：边界测试：非终止计算与 `loop {}` 的类型（编译错误）、边界测试：`panic!` 的指称与 `Result` 的指称分离（编…、所有权即线性性与生命周期即区域。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
1. **「指称语义能处理所有 Rust 构造」**：边界——`unsafe`、内联汇编（Inline Assembly）、平台相关行为的指称需要「非标准域」（如带硬件模型的域），现有工作只覆盖安全子集；把片段解释当完整语义会过度外推；
2. **「指称等价 = 行为等价」**：指称等价是「最粗的一致等价」——两个程序指称相同必有相同可观察行为，反之不然（资源消耗、时间不在指称中）；优化论证用指称等价时须另证资源性质；
3. **「不动点总存在且唯一」**：域论保证**最小**不动点存在（连续函数，Kleene 定理），但非唯一——`loop {}` 与 `loop { break }` 的指称区分正依赖「最小」选择；误判「不动点 = 唯一解」会在递归类型语义处出错。
...
- [Operational Semantics](03_operational_semantics.md) — 操作语义
- [Linear Logic](../01_ownership_logic/01_linear_logic.md) — 线性逻辑
- [Category Theory](../00_type_theory/04_category_theory.md) — 范畴论
- [RustBelt](../02_separation_logic/01_rustbelt.md) — RustBelt
...
本节聚焦「权威来源索引」，覆盖边界测试：发散函数（`!`）的指称语义（编译错误）、边界测试：`unsafe` 代码的语义鸿沟（运行时 UB）与边界测试：不动点语义与递归类型的无限展开（编译错误）。论述顺序由定义到边界：先明确「权威来源索引」在「指称语义与领域理论」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「权威来源索引」的判定标准，并指出它在全页论证链中的位置。
...
本节以闭包自引用为反例，展示指称语义中的不动点必须经显式 fix 构造、Rust 无隐式递归闭包。

### 反例：闭包引用自身（rustc 1.97.0 实测）

```rust,compile_fail,E0425
fn main() {
    let fact = |n: u64| if n == 0 { 1 } else { n * fact(n - 1) }; // ❌ fact 未在作用域
    println!("{}", fact(5));
}
```

**错误**：`E0425 cannot find function fact in this scope`——闭包绑定时自身尚未进入作用域，无隐式不动点。

### ✅ 修正：用 `fn`（自引用合法）或显式 fix 组合子
...
mindmap
  root((指称语义与领域理论))
    ...
    Rust 的指称解释
      1 类型即域
    边界测试 指称语义的编译错误
      1 边界测试 非终止计算与 loop {} 的类型 编译错误
      2 边界测试 panic 的指称与 Result 的指称分离 编译错误
      2 所有权即线性性
    反命题与边界分析
      1 反命题树
      2 边界极限
