...
- **中间地带**：call-by-sharing（传对象的引用副本——Java/Python 的「按值传引用」），call-by-push-value（Levy 的统一框架，严格/惰性为两端特例）。
...
- **借用 = 显式 call-by-reference**：`&T`/`&mut T` 参数把「按引用」变为类型可见的选择——函数签名即揭示传递策略；
- **惰性是库而非语言特性**：`Iterator`（惰性的拉式序列）、`once_cell::Lazy`（惰性初始化）、`Fn` 闭包（延迟计算）——惰性在 Rust 中**显式化**，类型标注揭示求值时机；
...
- **CBV + Move 的线性扩展**：严格求值下参数先求值再传递，Move 语义规定求值后所有权转移而非复制——类型系统必须为每个值追踪"是否已移动"，这在惰性语言中无对应物（惰性求值的 thunk 天然单次强制）。
- **交互的边界**：`const fn` 是编译期严格求值子语言，其类型约束（不允许可变借用跨越 const 边界的某些形式）体现了求值时机与类型规则的耦合——编译期求值必须是纯的，纯度要求反过来限制了可用类型构造。
