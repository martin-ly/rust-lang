...
本节从严格 vs 非严格求值 与 四种核心求值策略 两个层面剖析「求值策略谱系」。
...
本节围绕「Rust 的求值策略定位」展开，依次讨论默认策略：Call-by-Value + 严格求值、显式引用传递：受限的 Call-by-Reference、短路求值：非严格性的局部表达与惰性构造：Future 与 Iterator。
...
本节围绕「Lambda 演算中的归约策略」展开，覆盖三种归约策略 与  Rust 的求值顺序 两个方面。
...
本节专门讨论「求值策略与类型系统的交互」下的严格性 vs 类型系统表达能力。
