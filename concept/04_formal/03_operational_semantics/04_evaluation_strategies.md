...
求值策略（evaluation strategy）决定「参数何时求值、以何形式传递」，本节建立完整谱系：

- **严格端（eager）**：call-by-value（先求值再传值——Rust/C/C++ 默认）、call-by-reference（传引用——C++ `&` 参数）；
- **惰性端（lazy）**：call-by-name（传未求值表达式，每次使用重算——Algol 60）、call-by-need（传未求值表达式，首次使用记忆化——Haskell）；
- **中间地带**：call-by-sharing（传对象的引用（Reference）副本——Java/Python 的「按值传引用」），call-by-push-value（Levy 的统一框架，严格/惰性为两端特例）。

谱系的组织轴是「求值时机」与「共享性」两维——理解谱系即理解「Rust 为什么默认严格但提供惰性工具」（迭代器 = 显式惰性序列）。
...
Rust 在求值策略谱系中的精确定位及其工程后果：

- **默认 call-by-value**：参数按值传递，move 语义使「传值」成本可控（大对象移动不复制）——与 C++ 的「传值 = 复制」默认形成对照（C++ 需显式 `&&`/move）；
- **借用（Borrowing） = 显式 call-by-reference**：`&T`/`&mut T` 参数把「按引用」变为类型可见的选择——函数签名即揭示传递策略；
- **惰性是库而非语言特性**：`Iterator`（惰性的拉式序列）、`once_cell::Lazy`（惰性初始化）、`Fn` 闭包（Closures）（延迟计算）——惰性在 Rust 中**显式化**，类型标注揭示求值时机；
- **代价与收益**：严格求值 + 显式惰性 = 内存与性能可预测（无 Haskell 的 thunk 泄漏问题），代价是惰性组合需显式 `.collect()`/`Box::leak` 等干预点。

判别准则：需要惰性时问「这个惰性该由类型标注揭示吗」——应该，则用迭代器/闭包显式表达。
...
Lambda 演算中的归约策略为 Rust 的求值顺序提供理论坐标：

- **三种归约策略**：normal-order（最左最外，对应惰性）、applicative-order（最左最内，对应严格/CBV）、call-by-name（按需替换，不求值参数）。Church-Rosser 定理保证：若归约终止，不同策略得到同一范式——但终止性本身依赖策略（normal-order 终止性最强）。
- **Rust 的求值顺序**：函数参数与操作数的求值顺序**未指定**（编译器可选），但每个完整表达式内的副作用点由序列点（语句边界）分隔。与 C++ 的对比：C++17 前大量求值顺序是 UB，Rust 中顺序未指定但**绝不是 UB**——借用检查保证任何顺序下都不存在别名冲突，这是"未指定但安全"与"未定义"的本质区别。

实践推论：不要编写依赖求值顺序的代码（如 `f(x, x = mutate())` 风格），应拆分为带显式序列的多条语句。
...
求值策略与类型系统的交互核心是**严格性如何约束类型系统的表达能力**：

- **严格性 vs 类型系统表达能力**：惰性语言（Haskell）中类型必须包含"⊥（发散）"作为每个类型的隐式成员，类型安全定理需处理部分正确性；严格语言（Rust）中类型只描述已求值的形式，`!` 类型显式承担发散的角色——发散从"隐式无处不在"变为"显式可标注"。
- **CBV + Move 的线性扩展**：严格求值下参数先求值再传递，Move 语义规定求值后所有权（Ownership）转移而非复制——类型系统必须为每个值追踪"是否已移动"，这在惰性语言中无对应物（惰性求值的 thunk 天然单次强制）。
- **交互的边界**：`const fn` 是编译期严格求值子语言，其类型约束（不允许可变借用（Mutable Borrow）跨越 const 边界的某些形式）体现了求值时机与类型规则的耦合——编译期求值必须是纯的，纯度要求反过来限制了可用类型构造。
...
本节把「求值策略：Call-by-Value, Call-by-Name, Call-by-Need」的规则推到编译器与运行时的边界上逐一实测：反例：严格求值的性能陷阱、边界测试：求值顺序的确定性验证与边界测试：Move 作为 CBV 的线性扩展。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：Rust 默认使用什么求值策略？函数参数在调用前还是调用后求…、测验 2：Call-by-Name 和 Call-by-Need 有什…、测验 3：`&&` 和 `||` 的短路求值（short-circui…、测验 4：Rust 的 `Iterator` 惰性求值与 Haskel…等5个方面的顺序逐层展开。
...
> **认知功能**: 本 mindmap 从本页「求值策略 Call-by-Value」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
