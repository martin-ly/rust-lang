...
2. **Rust 的定位**：Rust 类型系统位于矩阵右下角——表达力仅限内存安全/数据竞争类性质，但验证完全自动且零运行时成本。这解释了为什么 Rust 生态的形式化工具（Kani、Prusti、Creusot）都选择**在类型系统之外**补充功能性规约，而非替换它。
...
本节将经典公理语义逐条"翻译"到 Rust 语义上，核心困难只有一个：**别名（aliasing）**。传统 Hoare 逻辑默认变量互不干扰，而 Rust 的可变借用 `&mut T` 恰恰是"受控的独占别名"，这使得 Rust 的公理化呈现出独特形态：
...
3. **框架规则链**：修改集合（modifies clause）+ frame ⟹ 「未提及状态自动保持」 ⟹ Prusti 中可变引用参数即「隐式 modifies」——借用检查保证了 frame 条件的成立（无别名绕过）；
