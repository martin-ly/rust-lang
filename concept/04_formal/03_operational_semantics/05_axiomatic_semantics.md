...
公理语义（Axiomatic Semantics）用**逻辑断言**而非状态转移来刻画程序行为：程序的意义由「满足什么前后条件」定义，核心工具是霍尔三元组（Hoare triple）`{P} C {Q}`——前置条件 P 成立时执行 C，若终止则后置条件 Q 成立。本节确立三个权威参照：
...
| **Rust 对应** | `03_operational_semantics.md` | `01_denotational_semantics.md` | **本文件** |
...
公理语义（Axiomatic Semantics）用逻辑断言描述程序行为：与其回答"程序如何执行"，它回答"执行前后什么性质成立"。下表沿**规约语言 → 自动化程度 → Rust 工具落地**三个轴对比六种主流方法，阅读时注意三条判定线索：

1. **表达力 vs 自动化**：Hoare 逻辑表达力最弱但推理最简单；分离逻辑能推理别名与并发，代价是证明负担显著上升；抽象解释与符号执行全自动，但以精度（假阳性）或可扩展性（路径爆炸）为代价。
2. **Rust 的定位**：Rust 类型系统（Type System）位于矩阵右下角——表达力仅限内存安全（Memory Safety）/数据竞争类性质，但验证完全自动且零运行时成本。这解释了为什么 Rust 生态的形式化工具（Kani、Prusti、Creusot）都选择**在类型系统之外**补充功能性规约，而非替换它。
3. **工具链选择依据**：验证功能性正确性选 Creusot/Prusti（wp 演算）；验证 unsafe 代码的内存安全选 Kani（符号执行）或 Miri（解释执行）；验证并发与别名不变式则进入 RustBelt/Iris 的分离逻辑领地。

判定口诀：性质越接近"资源与别名"，越需要分离逻辑；性质越接近"输入输出关系"，wp 演算越经济；只想低成本排除崩溃类错误，抽象解释与类型系统优先。
...
本节将经典公理语义逐条"翻译"到 Rust 语义上，核心困难只有一个：**别名（aliasing）**。传统 Hoare 逻辑默认变量互不干扰，而 Rust 的可变借用（Mutable Borrow） `&mut T` 恰恰是"受控的独占别名"，这使得 Rust 的公理化呈现出独特形态：

- **赋值公理需要所有权条件**：`x = e;` 不仅更新值，还可能使 `e` 的原绑定失效（move），并使 `x` 的旧值被 drop。后置条件必须同时陈述值、所有权与 drop 效应三者。
- **wp 计算沿所有权链传播**：`wp(x = y, Q)` 中若 `y` 是非 `Copy` 类型，前置条件必须要求 `y` 之后不再被使用，否则推导出的规约在 Rust 中不可满足。
- **借用规则即不变式**：`&mut` 的独占性可表述为分离逻辑中的 `points-to` 断言 `x ↦ v`，借用检查器静态保证了这些断言在程序各点成立。
- **unsafe 是公理边界**：`unsafe` 块内的操作不受借用规则保护，公理化时只能把 unsafe 块视为"信任基（trusted base）"，其正确性需用 RustBelt 式的方法单独证明。
...
公理语义在 Rust 生态已落地为三条工具链，各自对应一种验证范式：

- **Prusti（Viper 后端）**：用 `#[requires]` / `#[ensures]` 契约标注 Rust 函数，翻译为 Viper 中间验证语言后由分离逻辑验证器求解。优势：直接复用 Rust 语法；局限：泛型与复杂 trait 的翻译覆盖不完整。
- **Creusot（Why3 后端）**：把 Rust 翻译为 WhyML，用 wp 演算生成验证条件，可交互式证明。优势：理论根基最完整（Pearlite 规约语言）；局限：学习曲线陡。
- **Kani（符号执行）**：把 MIR 编译为 CBMC 的 GOTO 程序，有界模型检测断言。优势：无需规约知识，写 `kani::assert` 即可；局限：路径有界，只能证伪不能证全。

选型判定：要证明功能性正确性且有形式化背景 → Creusot；要轻量契约检查 → Prusti；要给 unsafe 代码找反例 → Kani。
...
本节检验公理语义应用中的三条常见误判及其反例：

1. **「霍尔三元组证明终止性」**：错——部分正确性三元组 `{P} C {Q}` 只保证「若终止则 Q」，完全正确性需 `[P] C [Q]` 加变体（variant）证明；反例：`{true} while true do skip {false}` 在部分正确性下成立；
2. **「霍尔逻辑能直接处理 Rust 借用」**：经典霍尔逻辑假设变量独立，别名（`&mut` 与裸指针）使赋值公理失效——分离逻辑的「frame rule」才是别名推理的正解，本节给出同一堆更新在两种逻辑下的对照；
3. **「公理化可机械穷举」**：循环不变式与最弱前置的计算一般不可判定（需要人工或启发式给出不变式）——SMT 求解器只验证给定不变式，不发明它。
...
- **操作语义**（`03_operational_semantics.md`）：公理语义的**可靠性基础**——若公理证明程序满足规约，操作语义保证程序执行确实满足该规约
- **指称语义**（`01_denotational_semantics.md`）：公理语义的**完备性基础**——指称语义证明类型系统（Type System）的 soundness，为公理规约的良定义性提供保证
...
公理验证的失效模式分三类，各有明确的判定特征：

1. **wp 计算的无限 descending chain（逻辑错误）**：递归规约中若后置条件不比前置条件"更小"，wp 推导不终止。Prusti/Creusot 通过要求显式 `decreases` 子句或变体（variant）截断——与 Rust `recursion_limit` 同理。
2. **借用不变式违反的验证失败（验证错误）**：验证器报 "assertion might not hold" 时，区分两类根因：规约过弱（缺少必要的前置条件）或程序真有 bug。判定方法：先用 Kani 在有限深度内找反例，找到则为真 bug，找不到再加强规约。
3. **unsafe 边界的公理缺口**：`unsafe` 块内操作不受类型系统保护，验证器只能把块内视为公理。判定方法：把 unsafe 块的不变式提取为独立的 `#[trusted]` 契约，单独人工审查——这是 RustBelt "信任基最小化" 原则的工程化。测试：unsafe 块的公理逃逸（运行时 UB）。
...
本节给出公理语义在 Rust 语境的定理推理链，根定理是**验证条件的充分性**：若所有生成的 VC 可证，则程序满足规约。派生链：

1. **赋值公理链**：`{P[x:=E]} x := E {P}` ⟹ 「前置条件向后替换」 ⟹ Rust 中 move 赋值需额外考虑「源变量失效」——公理扩展为「消耗性赋值公理」；
2. **循环规则链**：不变式 I + 变体 V ⟹ 「部分正确性 + 终止性」分离证明 ⟹ Rust `loop`/`while` 的验证 = 找到 I 与 V（工具不发明，人工给出）；
3. **框架规则链**：修改集合（modifies clause）+ frame ⟹ 「未提及状态自动保持」 ⟹ Prusti 中可变引用（Mutable Reference）参数即「隐式 modifies」——借用检查保证了 frame 条件的成立（无别名绕过）；
4. **健全性链**：VC 生成正确 + SMT 求解正确 ⟹ 验证结论可信 ⟹ 信任基础（TCB）= VC 生成器 + SMT + 公理化（整数建模等）。

每条链附失效条件（如 SMT 超时 ⟹ unknown ⟹ 需重构规约）。
...
本节聚焦「工具链深度对比矩阵」。论述顺序由定义到边界：先明确「工具链深度对比矩阵」在「Axiomatic Semantics（公理语义）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「工具链深度对比矩阵」的判定标准，并指出它在全页论证链中的位置。
...
本节把「Axiomatic Semantics（公理语义）」的规则推到编译器与运行时的边界上逐一实测：边界测试：Prusti 对泛型 Trait 的验证失败、边界测试：Kani 的路径爆炸与有界验证与边界测试：Creusot 的 Ghost 代码与零成本抽象。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
- [操作语义：程序行为的形式化定义](03_operational_semantics.md) — 小步/大步语义、求值上下文
- [指称语义：CPO 与不动点](01_denotational_semantics.md) — Scott-Strachey 语义
- [类型论基础](../00_type_theory/01_type_theory.md) — HM 类型系统（Type System）、System F、Rust 类型扩展
- [所有权形式化](../01_ownership_logic/02_ownership_formal.md) — COR、RustBelt、权限系统
- [分离逻辑：Rust 所有权的形式化根基](../02_separation_logic/02_separation_logic.md) — 框架规则、Iris、CSL
