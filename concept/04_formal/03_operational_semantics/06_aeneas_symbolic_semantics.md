...
> **前置依赖**: [L4 所有权（Ownership）形式化](../01_ownership_logic/02_ownership_formal.md) · L4 RustBelt · L4 分离逻辑 · L3 Unsafe
> **后置延伸**: [L6 形式化验证工具](../../06_ecosystem/08_formal_verification/02_formal_verification_tools.md) · [L7 形式化方法](../../07_future/04_research_and_experimental/02_formal_methods.md)
...
Rust 借用（Borrowing）检查器在编译期拒绝大量内存不安全程序，但它本身的正确性如何证明？传统上有两条路：
...
LLBC（Low-Level Borrow Calculus）是 Aeneas 的核心中间表示，本节给出其设计：

- **动机**：Rust 源码的借用是隐式的（编译器推导），验证需要**显式**的借用结构——LLBC 把借用、重借、借用结束（`end_borrow`）变成 IR 中的一等指令；
- **核心构造**：loan（贷款，借用创建的权限记录）、symbolic value（符号值，抽象掉具体数据的值）、region abstraction（区域抽象，函数边界的借用聚合）；
- **与 MIR 的关系**：LLBC 由 MIR 翻译而来，但**消除了控制流图中的借用隐式性**——翻译定理保证「MIR 语义 ⟷ LLBC 语义」的模拟关系；
- **验证接口**：LLBC 程序可继续翻译到 F*/Lean/HOL4 等证明助手的函数式模型——「命令式 Rust ⟹ 纯函数模型」的翻译是 Aeneas 的核心贡献。

本节给出 LLBC 指令表与一段 MIR → LLBC 的翻译示例。
...
本节聚焦「符号化执行语义」，覆盖符号值与符号状态、路径约束、Borrow / Loan 规则（教学类比）与状态重组（Reorganization）。论述顺序由定义到边界：先明确「符号化执行语义」在「Aeneas Symbolic Semantics（Aeneas 符号化语义）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「符号化执行语义」的判定标准，并指出它在全页论证链中的位置。
...
本节给出 Aeneas 翻译的声音性（soundness）论证框架：

- **模拟关系（simulation）**：LLBC 的每一步符号执行对应源 MIR 程序的一步具体执行——形式上「具体状态 ⟼ 符号状态」的抽象关系在每一步保持；
- **声音性定理**：若 LLBC 符号执行未发现错误（panic/UB），则源程序在对应输入下不触发该错误——符号执行是具体执行的**过度近似**（over-approximation）还是精确对应，取决于抽象的精度；Aeneas 对借用结构是精确的（借用语义无近似），对数值是符号的（依赖后端 SMT）；
- **边界**：不支持的构造（inline asm、部分 `unsafe` 模式、并发）在声音性定理之外——工具会拒绝翻译而非静默近似；
- **与 Kani 的分工**：Kani 有界模型检查「具体语义 + 深度界」，Aeneas 符号翻译「借用结构精确 + 函数式后端」——本节给出选择决策表。
...
本节聚焦「Aeneas 工具链与验证流程」，覆盖Aeneas 流程 与  Rust / Aeneas 风格示例。论述顺序由定义到边界：先明确「Aeneas 工具链与验证流程」在「Aeneas Symbolic Semantics（Aeneas 符号化语义）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「Aeneas 工具链与验证流程」的判定标准，并指出它在全页论证链中的位置。
...
**反例：假设任意 Rust crate 都能经 Aeneas 翻译。**

Aeneas 的目标子集 LLBC（Low-Level Borrow Calculus）刻意排除了若干 Rust 特性：裸指针的任意算术、动态 trait 对象的完整语义、部分 unsafe 惯用法。直接对含这些特性的 crate 运行翻译，会在前端被拒或被迫大幅改写。

**修正对照**：

1. 先圈定「可翻译子集」：把验证目标模块（Module）与 unsafe/FFI 边界隔离；
2. 对子集外代码提供公理化 shim（声明其行为契约而非翻译其实现）；
3. 翻译失败信息按 LLBC 限制清单逐条归因，而非当作工具 bug。

**陷阱要点**：操作语义工具的「子集边界」是设计决定而非实现债务——绕开子集限制的改写必须先论证改写前后的语义等价，否则验证结论不覆盖原程序。
...
- [L4 RustBelt](../02_separation_logic/01_rustbelt.md)
- [L4 分离逻辑](../02_separation_logic/02_separation_logic.md)
- [L4 所有权（Ownership）形式化](../01_ownership_logic/02_ownership_formal.md)
- [L3 Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md)
- [L6 形式化验证工具](../../06_ecosystem/08_formal_verification/02_formal_verification_tools.md)
