...
| **T-127: 类型安全 → 内存安全（Memory Safety）** | Progress + Preservation | well-typed 程序不触发段错误 | Rust 的类型系统（Type System） | Pierce TAPL Ch.8 · RustBelt |
...
| **标准库验证** | 有限 | 有限 | 较多（核心原语）|
| **学习曲线** | 中等 | 中等 | 低 |
| **典型场景** | 内存安全（Memory Safety）教学验证 | 功能性规约深度验证 | 快速反例生成/回归测试 |
