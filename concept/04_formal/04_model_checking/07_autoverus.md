...
本节聚焦「Verus 核心机制」，覆盖三段式程序结构 与 与 Rust 借用（Borrowing）检查器的关系。论述顺序由定义到边界：先明确「Verus 核心机制」在「AutoVerus / Verus 自动证明生态」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「Verus 核心机制」的判定标准，并指出它在全页论证链中的位置。
...
- [AutoVerus / Verus 预览跟踪（L7 版本/生态动向）](../../07_future/02_preview_features/33_autoverus_preview.md)
