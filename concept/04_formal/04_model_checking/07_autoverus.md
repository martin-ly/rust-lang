...
> **后置延伸**: [Safety Tags](../../07_future/03_preview_features/03_safety_tags_preview.md) · [BorrowSanitizer](../02_separation_logic/04_borrow_sanitizer_in_formal.md)
>
> **来源**:
> [Verus GitHub](https://github.com/verus-lang/verus) ·
> [Verus 文档](https://verus-lang.github.io/verus/guide/) ·
> [AutoVerus 论文 (OOPSLA 2025)](https://doi.org/10.1145/3763174) ·
> [arXiv 版本](https://arxiv.org/abs/2409.13082) ·
...
本节聚焦「Verus 核心机制」，覆盖三段式程序结构 与 与 Rust 借用检查器的关系。论述顺序由定义到边界：先明确「Verus 核心机制」在「AutoVerus / Verus 自动证明生态」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「Verus 核心机制」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「AutoVerus：LLM 驱动的自动化证明」，覆盖设计原则、三阶段工作流与评估结果。论述顺序由定义到边界：先明确「AutoVerus：LLM 驱动的自动化证明」在「AutoVerus / Verus 自动证明生态」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「AutoVerus：LLM 驱动的自动化证明」的判定标准，并指出它在全页论证链中的位置。
...
verus! {
fn abs(x: i32) -> (r: i32)
    ensures r >= 0          // 忘了写 r == x || r == -x
{
    if x < 0 { 0 } else { x }  // 错误实现也满足弱 ensures
}
}
```
...
1. 规格评审与代码评审同级：验证只能保证「实现满足规格」，不能保证「规格表达了意图」；
2. 用「规格测试」——为 spec 写可满足性/紧致性检查（如对 `abs` 断言 `abs(-3) == 3` 的具体值验证）；
3. `requires` 过弱会把错误推到调用点：验证报错位置 ≠ 缺陷位置。

**陷阱要点**：演绎验证的信任基础（TCB）包含规格、公理化 shim 与求解器；把「verified」徽章等同于「正确」是最常见的工程误读。
...
```mermaid
mindmap
  root((AutoVerus / Verus 自动证明生态))
    权威定义
    Verus 核心机制
      1 三段式程序结构
      2 与 Rust 借用检查器的关系
    AutoVerus LLM 驱动的自动化证明
      1 设计原则
      2 三阶段工作流
      3 评估结果
    Verus / AutoVerus 在 Rust 生态中的位置
    当前状态与时间表
```
