...
>
> **前置概念**: [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md)
...
「分离逻辑与所有权（Ownership）」部分包含 Q1. 分离逻辑（Separation Logic）中，`*`（分离合… 与  Q2. RustBelt 的核心贡献是什么？它如何证明 Rust 的类… 两条主线，本节依次说明。
...
**知识点**：分离逻辑的 `*` 是 Rust 所有权系统"资源独占"特性的数学根基。框架规则是局部推理（local reasoning）的基础。[→ 分离逻辑详解](../02_separation_logic/02_separation_logic.md)
...
**知识点**：RustBelt 证明了 Rust 的安全保证有坚实的数学基础——不是工程经验，而是可验证的定理。[→ RustBelt 详解](../02_separation_logic/01_rustbelt.md)
...
本节围绕「类型安全与定理」展开，覆盖 Q3. 以下"定理"（教学类比）描述的是什么性质？ 与  Q4. 线性逻辑（Linear Logic）中，`⊗`（张量积）和 `… 两个方面。
...
**知识点**：Progress & Preservation 是类型理论的"圣杯"。Rust 通过所有权系统将这些经典定理扩展到了内存安全（Memory Safety）和并发安全（Concurrency Safety）领域。[→ 类型语义详解](../00_type_theory/06_type_semantics.md)
...
本节从 Q5. Kani、Miri 和 BorrowSanitizer 的检测… 与  Q6. Hoare 三元组 `{P} C {Q}` 的含义是什么？如何… 两个层面剖析「验证工具链」。
...
**知识点**：Hoare 逻辑是程序验证的基石。分离逻辑通过 `*` 运算符扩展了 Hoare 逻辑，使其能够表达堆内存的局部操作——这正是 Rust 所有权系统需要的数学语言。[→ Hoare 逻辑详解](../03_operational_semantics/02_hoare_logic.md)
...
本节围绕「形式化与工程实践」展开，覆盖 Q7. "Soundness Bug"（健全性漏洞）在 Rust 编译… 与  Q8. 以下"定理"（教学类比）描述的是什么？为什么它对 Rust 的… 两个方面。
...
**知识点**：Soundness 是 Rust 的核心承诺。每个 soundness bug 都是对这个承诺的破坏，因此 Rust 团队将其列为最高优先级。[→ RustBelt 详解](../02_separation_logic/01_rustbelt.md)
...
**知识点**：框架规则是"组合性验证"的数学基础——复杂系统的验证可以分解为组件的局部验证。[→ 分离逻辑详解](../02_separation_logic/02_separation_logic.md)
...
本节从 Q9. Verus、Creusot 和 Prusti 在 Rust 验… 与  Q10. 以下代码在 Miri 下会报告什么？解释 Tree Borr… 两个层面剖析「综合应用」。
...
**知识点**：Rust 的形式化验证生态正在快速发展。选择工具时需考虑验证目标、标注负担和自动化程度之间的权衡。[→ 验证工具链详解](01_verification_toolchain.md)
...
**知识点**：Tree Borrows 是 Rust 内存模型的最新研究方向，旨在平衡优化能力和代码兼容性。Miri 的 `-Zmiri-tree-borrows` 标志启用该模型。[→ RustBelt 详解](../02_separation_logic/01_rustbelt.md)
...
> [Separation Logic](../02_separation_logic/02_separation_logic.md)、
> [RustBelt](../02_separation_logic/01_rustbelt.md)、
> [Verification Toolchain](01_verification_toolchain.md) 权威页一致；
> 干扰项针对常见误解设计。

### Q11. 🟡【单选】分离逻辑中，`emp` 谓词表示什么？

- A. 空堆：不拥有任何内存资源
- B. 空类型（ uninhabited type ）
- C. 任意堆，是分离合取的单位元
- D. 程序出错的错误状态
...
**解析**：`emp` 断言"资源堆为空"，它是分离合取 `*` 的**单位元**：`P * emp ⟺ P`。C 错在把"单位元"说成"任意堆"——`emp` 不是"什么都满足"，而是"恰好什么都不拥有"。在 Rust 视角下，`emp` 对应"不持有任何所有权"的状态，框架规则正是靠它把无关资源隔离在证明之外。

</details>

---

### Q12. 🔴【单选】RustBelt 中的 λRust（lambda-Rust）的作用是？

- A. 一个替代 rustc 的生产级编译器
- B. 一个核心演算：在 Coq/Iris 中形式化 Rust 的核心语言（含所有权、借用（Borrowing）、unsafe），在其上机械证明类型安全
- C. 一个运行时（Runtime）监控器，拦截所有 unsafe 调用
- D. Rust 宏（Macro）系统的形式化模型
...
**解析**：λRust 是 RustBelt 的形式化载体：先定义足够表达 Rust 关键机制的核心语言，再用 Iris（高阶分离逻辑框架）在 Coq 中证明其类型系统（Type System）健全——包括若干标准库 unsafe 抽象（`Cell`、`Rc`、`Mutex` 等）的健全性。A、C 错：它既不生成机器码也不在运行时介入；D 错：宏系统不在其范围内。

</details>

---

### Q13. 🔴【多选】关于 Rust 验证工具的定位，下列说法正确的有？（选出所有正确项）

- A. Miri：动态分析，解释执行 MIR 检测 UB
- B. Kani：有界模型检查，按路径/深度展开验证性质
- C. Prusti / Creusot / Verus：演绎验证，基于规约（specification）证明程序满足契约
- D. 这些工具都能证明任意 Rust 程序完全正确
...
**解析**：三类工具构成互补光谱：动态检测（Miri，易用但只覆盖执行路径）、有界/无界模型检查（Kani，自动化但受状态爆炸限制）、演绎验证（Prusti/Creusot/Verus，能力强但需手写规约）。D 错：由图灵完备语言的不可判定性，"证明任意程序完全正确"不可能；每种工具都在自动化程度、表达能力、健全性假设之间取舍，详见 [Verification Toolchain](01_verification_toolchain.md)。

</details>

---

### Q14. 🟢【判断】分离逻辑的框架规则（Frame Rule）允许在证明中保留未被操作触及的堆部分，这与 Rust 借用"只关注被借用部分"的模块化思想相呼应。（对 / 错）
...
**解析**：框架规则：`{P} C {Q}` 蕴含 `{P * R} C {Q * R}`（C 不涉及 R 时）——证明只陈述被改变的部分，其余资源自动"带过"。Rust 的 `&mut` 借用同理：函数只需声明它借用的参数，调用方的其他数据天然不受影响。这正是分离逻辑能自然建模 Rust 的原因：两者共享"**局部推理（local reasoning）**"的同一直觉。

</details>

---

### Q15. 🔴【判断】类型安全定理（progress + preservation）保证良类型程序不会"卡住（stuck）"，因此 Rust 程序中不可能发生 panic。（对 / 错）
...
**解析**：类型安全保证的是"不出现**未定义**的卡住状态"（如对数字调用方法、野指针解引用（Reference））。panic 是**定义良好**的受控中止——它是语言语义内的正常行为（索引越界、unwrap 失败），不是类型安全的反例。同理，类型安全也不排除死锁、内存泄漏、逻辑错误；"类型安全 ⊇ 内存安全（Memory Safety）"但远不等于"程序正确"。
...
| 4–6/10 | 🔄 需巩固基础 | 重读 [RustBelt](../02_separation_logic/01_rustbelt.md) · [Separation Logic](../02_separation_logic/02_separation_logic.md) |
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：本文件是 测验：形式化方法概念（L4 试点扩展） 的专项测验…、测验 2：在 测验：形式化方法概念（L4 试点扩展） 的测验中，若遇到…与测验 3：专项测验与概念文件末尾的嵌入式测验有什么区别？（理解层）。
