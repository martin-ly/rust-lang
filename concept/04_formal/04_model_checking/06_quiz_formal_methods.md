...
「分离逻辑与所有权」部分包含 Q1. 分离逻辑（Separation Logic）中，`*`（分离合… 与  Q2. RustBelt 的核心贡献是什么？它如何证明 Rust 的类… 两条主线，本节依次说明。
...
- B. 一个核心演算：在 Coq/Iris 中形式化 Rust 的核心语言（含所有权、借用、unsafe），在其上机械证明类型安全
- C. 一个运行时监控器，拦截所有 unsafe 调用
- D. Rust 宏系统的形式化模型
...
**解析**：λRust 是 RustBelt 的形式化载体：先定义足够表达 Rust 关键机制的核心语言，再用 Iris（高阶分离逻辑框架）在 Coq 中证明其类型系统健全——包括若干标准库 unsafe 抽象（`Cell`、`Rc`、`Mutex` 等）的健全性。A、C 错：它既不生成机器码也不在运行时介入；D 错：宏系统不在其范围内。
...
**解析**：类型安全保证的是"不出现**未定义**的卡住状态"（如对数字调用方法、野指针解引用）。panic 是**定义良好**的受控中止——它是语言语义内的正常行为（索引越界、unwrap 失败），不是类型安全的反例。同理，类型安全也不排除死锁、内存泄漏、逻辑错误；"类型安全 ⊇ 内存安全"但远不等于"程序正确"。
