...
本节聚焦「安装与基本用法」，覆盖安装、运行测试、运行单个示例或二进制与常用 Miri 环境变量。论述顺序由定义到边界：先明确「安装与基本用法」在「Miri：Rust 未定义行为动态检测器」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「安装与基本用法」的判定标准，并指出它在全页论证链中的位置。
...
本节聚焦「反命题与边界」，覆盖反命题树 与 边界极限。论述顺序由定义到边界：先明确「反命题与边界」在「Miri：Rust 未定义行为动态检测器」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「反命题与边界」的判定标准，并指出它在全页论证链中的位置。
