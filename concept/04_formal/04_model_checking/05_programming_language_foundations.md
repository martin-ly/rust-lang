...
> **内容定位**: **project-specific** — 本节为通用程序语言理论（λ 演算、停机问题、System F）在 Rust 知识体系中的教学桥梁，非 Rust 官方规范。Rust 权威语义请参见 [`类型系统基础`](../../01_foundation/02_type_system/01_type_system.md)。
...
> [Ownership Formal](../01_ownership_logic/02_ownership_formal.md) ·
> [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md)
> **后置延伸**:
> [Effects System](../../07_future/02_preview_features/01_effects_system.md) ·
> [Evolution](../../07_future/04_research_and_experimental/03_evolution.md)
>
> **来源**:
...
> [SF — Software Foundations](https://softwarefoundations.cis.upenn.edu/) ·
> [CS 242 Stanford](https://stanford-cs242.github.io/f19/) ·
> [RustBelt](https://plv.mpi-sws.org/rustbelt/)
...
- [通用程序语言理论基础：Rust 的 PL 基座](#通用程序语言理论基础rust-的-pl-基座)
  ...
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：`if` 表达式两分支类型不一致（rustc 1.97.0 实测）](#反例if-表达式两分支类型不一致rustc-1970-实测)
    - [✅ 修正：统一分支类型](#-修正统一分支类型)
...
本节从编程语言基础视角展开求值策略理论，聚焦验证相关的两个方面：

1. **策略对语义的影响**：call-by-value 与 call-by-need 在「副作用存在时」产生不同可观察行为——验证工具必须固定策略语义（Kani/Prusti 均假设 Rust 的严格语义）；Church-Rosser 定理保证纯 λ 演算下策略不影响结果，副作用打破该保证；
2. **策略对验证成本的影响**：惰性语言的验证需推理「何时求值」（thunk 的生命周期（Lifetimes）），严格语言的验证只需推理值——这是 Rust 验证工具比 Haskell 验证工具成熟的理论原因之一。

本节与 04_evaluation_strategies 的关系：该页是求值策略的谱系全景，本节是其在验证语境下的深入——两页通过 canonical 链接分工。
...
副作用模型刻画「表达式除返回值外还做什么」，本节建立从理论到 Rust 的映射：

- **效应系统的理论**：效应（effect）= 计算的可观察副行为（IO、状态、异常、非终止）——ML/Haskell 用单子编码效应，Koka/Frank 用代数效应（algebraic effects）使其可组合；
- **Rust 的隐式效应分类**：Rust 无效应系统，但类型系统（Type System）隐含三种效应标记——`unsafe`（可能 UB 的效应）、`async`（可挂起的效应）、`panic`（非局部退出的效应）——三者皆不可「擦除」，与效应系统的「效应行（effect row）」思想同构；
- **验证视角**：效应推理决定「哪些等价变换合法」——纯函数可任意重排/记忆化，含效应代码的变换受序约束；Rust 编译器的优化合法性正建立在对 `unsafe` 效应边界的保守假设上。

本节为后续 CPS 与 continuation 节的效应推理铺垫。
...
Continuation 是"程序剩余计算"的具化，CPS（Continuation-Passing Style）把它显式化为函数参数。与 Rust 的三重关联：

- **什么是 Continuation**：表达式 `f(g(x))` 在求值 `g(x)` 时，continuation 是 "λv. f(v)"——把结果代入后要做的事。CPS 变换把所有函数改为接受额外 continuation 参数、不再返回（尾调用风格）。
- **async/await 是 CPS 的语法糖**：`async fn` 的每个 `.await` 点对应一个 continuation 边界——await 后的代码被编译器捕获为状态机的后续状态，等价于 CPS 中的 continuation 参数。区别：Rust 的 CPS 变换是编译器内部的（generator → state machine），程序员不可见，且无尾调用保证。
- **Pin 与 Continuation 的安全性**：continuation 捕获的环境若含自引用（Reference）（如 await 点跨越的局部引用），状态机结构体（Struct）会自引用——`Pin` 保证其地址稳定，使自引用 continuation 安全。这是 CPS 理论中没有的问题（lambda 演算无地址概念），是 Rust 把 CPS 落地到无 GC 系统的原创性贡献。tion 与 CPS」的核心内容。
...
结构化程序定理与 Rust 控制流的关系是"定理的必然性如何变成语言的强制性"：

- **结构化程序定理**（Böhm-Jacopini 1966）：任何可计算函数都可用顺序、选择、循环三种控制结构组合表达，无需 goto。推论：goto 不是表达力的来源，只是便利。
- **Rust 的类型驱动控制流**：Rust 不仅没有 goto，还把控制流编码进类型——`Result` 强制处理错误分支（`?` 是类型指导的控制流），`match` 的穷尽性检查保证选择结构覆盖所有情形，`Option` 消除空指针分支。判定依据：Rust 控制流的完备性 = 结构化定理的三种结构 + 类型强制的分支完备性；这使"未处理的错误路径"从运行时（Runtime）异常变为编译错误。

边界：`break 'label` 与 `loop` 表达式提供了结构化 goto 的受控形式，表达能力等价但作用域受类型约束。
...
本节聚焦「变量模型：环境 vs 存储」，覆盖两个概念 与  Rust 的所有权作为存储约束。论述顺序由定义到边界：先明确「变量模型：环境 vs 存储」在「通用程序语言理论基础：Rust 的 PL 基座」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「变量模型：环境 vs 存储」的判定标准，并指出它在全页论证链中的位置。
...
> **权威来源**:
> [Verus](https://github.com/verus-lang/verus) · [Kani](https://model-checking.github.io/kani/) ·
> [Rust Reference](https://doc.rust-lang.org/reference/introduction.html) ·
> [RustBelt](https://plv.mpi-sws.org/rustbelt/) · [TRPL](https://doc.rust-lang.org/book/title-page.html) ·
> [TAPL — Types and Programming Languages](https://www.cis.upenn.edu/~bcpierce/tapl/) · [Oxide](https://arxiv.org/abs/1903.00982)
...
本节以 `if` 分支类型不一致为反例，展示表达式类型规则的最基础形式。
