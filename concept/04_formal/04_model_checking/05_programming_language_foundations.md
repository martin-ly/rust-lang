...
2. **策略对验证成本的影响**：惰性语言的验证需推理「何时求值」（thunk 的生命周期），严格语言的验证只需推理值——这是 Rust 验证工具比 Haskell 验证工具成熟的理论原因之一。
...
- **Rust 的隐式效应分类**：Rust 无效应系统，但类型系统隐含三种效应标记——`unsafe`（可能 UB 的效应）、`async`（可挂起的效应）、`panic`（非局部退出的效应）——三者皆不可「擦除」，与效应系统的「效应行（effect row）」思想同构；
...
- **Pin 与 Continuation 的安全性**：continuation 捕获的环境若含自引用（如 await 点跨越的局部引用），状态机结构体会自引用——`Pin` 保证其地址稳定，使自引用 continuation 安全。这是 CPS 理论中没有的问题（lambda 演算无地址概念），是 Rust 把 CPS 落地到无 GC 系统的原创性贡献。tion 与 CPS」的核心内容。
...
- **Rust 的类型驱动控制流**：Rust 不仅没有 goto，还把控制流编码进类型——`Result` 强制处理错误分支（`?` 是类型指导的控制流），`match` 的穷尽性检查保证选择结构覆盖所有情形，`Option` 消除空指针分支。判定依据：Rust 控制流的完备性 = 结构化定理的三种结构 + 类型强制的分支完备性；这使"未处理的错误路径"从运行时异常变为编译错误。
