...
本节围绕「求值策略（Evaluation Strategy）」展开，依次讨论三种基本策略、Rust 的 CBV 与 Move 语义与为什么 Rust 不是 CBN/CBNeed。
...
本节将「副作用模型（Effect System）」分解为若干主题：什么是副作用、Rust 的副作用控制与与 Haskell IO Monad 的对比。
...
本节从什么是 Continuation、async/await 是 CPS 的语法糖与Pin 与 Continuation 的安全性切入，剖析「Continuation 与 CPS」的核心内容。
...
「结构化程序定理与 Rust 控制流」部分包含结构化程序定理 与  Rust 的类型驱动控制流 两条主线，本节依次说明。
...
「变量模型：环境 vs 存储」部分包含两个概念 与  Rust 的所有权作为存储约束 两条主线，本节依次说明。
...
本节将「嵌入式测验（Embedded Quiz）」分解为若干主题：测验 1：Lambda 演算（Lambda Calculus）为什么被…、测验 2：什么是"柯里化"（Currying）？Rust 的闭包支持柯…、测验 3：系统 F（System F）是什么？Rust 的泛型与它有什…、测验 4：什么是"停机问题"（Halting Problem）？它对程…等5个方面。
