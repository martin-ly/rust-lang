...
- v1.2 (2026-05-26): 补充 VerusBelt (PLDI 2026 Distinguished Paper)、Miri POPL 2026 论文、Miri 深度原理章节（7.3）、验证工具对比矩阵扩展 Miri 列 [Web Authority AlignmentSprint](../../00_meta/02_sources/05_international_authority_index.md)
- v1.3 (2026-05-26): 权威内容对齐 R22：补充 Rusted Types (ICSE 2026) — Rust 类型混淆漏洞静态检测 [ICSE 2026](https://conf.researchr.org/home/icse-2026)
- v1.3 (2026-05-26): 权威内容对齐：补充 Ravencheck (SOAP 2026) — Rust 的 Extended Effectively Propositional (EEPR) 自动验证，可判定逻辑片段的全自动验证路径 [Web Authority AlignmentBatch 14](../../00_meta/02_sources/05_international_authority_index.md)
...
> 内部原理见 [`04_rustbelt.md`](../02_separation_logic/01_rustbelt.md) §7–§8。
...
场景化 ROI 评估用"验证成本 vs 缺陷代价"矩阵指导工具选型，三个典型场景：

- **场景 A：安全关键网络协议（TLS/QUIC 实现）**——缺陷代价极高（远程代码执行），验证预算充足。推荐组合：Kani 对状态机做有界模型检测 + Prusti 对解析器契约验证 + Miri 对 unsafe 加密原语解释执行。参考：AWS s2n-quic 在 CI 中运行 Kani proof harness，缺陷拦截率公开报告显著高于纯测试。
- **场景 B：操作系统内核页表管理**——缺陷代价极高但验证对象重度依赖 unsafe 与硬件抽象。推荐：RustBelt 式手工证明关键不变式（一次性高成本）+ Miri 日常回归。形式化投资回报集中在"证明一次、长期复用"的不变式上。
- **场景 C：日常 Web 服务业务逻辑**——缺陷代价中等，验证预算有限。推荐：类型系统 + 属性测试（proptest）即可，Kani 仅用于钱/权限相关的核心模块（Module）。ROI 判定：验证投入不超过该模块缺陷预期损失的 20%。辑与场景 D: 新并发算法研究的顺序逐层展开。
...
运行成本:       人天级交互式证明
─────────────────────────────────────────
ROI: ★★☆☆☆ 低–中 — 仅限学术/核心基础设施 (Source: [Aeneas — ICFP 2022](https://arxiv.org/abs/2206.07185)) ✅
```
...
本节给出 Rust 项目的分层验证策略——验证投入应与风险层级匹配，五层金字塔：

1. **L1 类型系统（免费层）**：借用检查 + 类型安全覆盖内存安全（Memory Safety）类缺陷——设计层面的「让非法状态不可表示」最大化此层收益；
2. **L2 测试层**：单元/属性测试（proptest）+ Miri（UB 检测）——Miri 是所有 `unsafe` 代码的必跑项；
3. **L3 模糊与静态分析层**：cargo-fuzz（输入空间探索）、clippy 的 correctness lint、semgrep 规则——适合解析器与协议代码；
4. **L4 模型检查层**：Kani（有界模型检查，适合无循环/浅循环的算法正确性）、Crucible——适合关键算法与安全模块；
5. **L5 演绎验证层**：Prusti/Creusot/Verus（霍尔逻辑式完整规约）——仅安全关键组件值得投入（航空/医疗/加密原语）。

策略原则：下层覆盖率达阈值再投资上层——跳过 L2 直接上 L5 是验证工程的头号反模式。
...
1. **「模型检查 = 证明」**：有界模型检查（Kani）只证「深度 k 内无错」——unwind 边界外的行为未覆盖；正确姿势是「边界 + 不变式」组合论证；
2. **「Miri 通过 = unsafe 正确」**：Miri 是解释执行的单路径检查——并发交错（需 Loom）、未覆盖输入路径、硬件弱序不在其范围；
3. **「规约越完整越好」**：演绎验证的成本主要在规约维护——过度规约（规约比代码复杂）使验证成为负担；规约应聚焦「最坏失效模式」；
4. **「验证消除测试」**：验证与测试覆盖不同失效类——验证证「规约内的正确性」，测试发现「规约外的意外」（规格本身错误）；
5. **「工具输出即结论」**：SMT 超时/未知结果被误读为「通过」——工具输出的三值语义（sat/unsat/unknown）必须显式处理。
...
| 所有权（Ownership）形式化 | [`./03_ownership_formal.md`](../01_ownership_logic/02_ownership_formal.md) | 别名模型与 Miri 检测 |
...
验证工具的深度原理决定了各自的适用边界，三类工具三种范式：

- **Prusti：基于 Viper 的分离逻辑验证**——Rust 函数 + 契约 → Viper 中间语言 → 符号执行 + SMT 求解。分离逻辑原生支持堆推理，使 `Box`/引用的验证表达自然；边界：trait 与闭包的翻译不完整。
- **Kani：基于 CBMC 的有界模型检测**——MIR → GOTO 程序 → SAT/SMT 编码，展开循环至有界深度。优势在反例生成（找到 bug 给出具体输入）；边界：有界深度意味着"未找到 bug ≠ 无 bug"，证明是 under-approximation。
- **Miri：基于 Tree Borrows 的解释执行**——直接解释 MIR，运行时（Runtime）检查未定义行为（悬垂、越界、别名违规）。优势：精确捕获 UB，零假阳性；边界：指数级慢，只能跑小测试，且只检查实际执行到的路径。

选型判定：要证明 → Prusti；要找反例 → Kani；要查 UB → Miri。三者覆盖互补，安全关键项目应全部启用。s 的动态 UB 检测、Verus：基于 Z3 的演绎验证等12个方面的顺序逐层展开。
...
> (Source: [POPL 2026 — Ralf Jung et al., "Miri: Practical Undefined Behavior Detection for Rust"](https://github.com/rust-lang/miri))
...
> (Source: [Miri Book](https://rustc-dev-guide.rust-lang.org/miri.html)) ·
...
> **Rust 独有发现**: rustc 的 ICE 通常以 **panic + 安全清理** 终止，而非传统编译器的 segfault——这是 Rust 的"fail-safe"设计在编译器自身的体现。更严重的是 **"Safe Rust → UB"** 症状：若 rustc bug 导致安全 Rust 代码在运行时（Runtime）产生未定义行为，则直接违反了 Rust 的核心安全保证。这类 bug 极为罕见但后果严重。
...
> **形式模型意义**: Rusted Types 揭示了 Rust"类型系统保证"与"实际运行时（Runtime）代码"之间的**语义鸿沟**。
> 编译器的类型检查只覆盖 safe 子集；当开发者进入 `unsafe` 领域时，类型系统的保证需要通过额外的静态分析来恢复。
> 这与 RustBelt 的"逻辑关系"方法和 Miri 的动态检测形成了"静态 + 动态 + 逻辑"的三层验证互补。
> (Source: [ICSE 2026 — Zhuang et al., "Rusted Types: Static Detection of Rust Type Confusion Bugs"](https://conf.researchr.org/details/icse-2026/icse-2026-research-track/54/Rusted-Types-Static-Detection-of-Rust-Type-Confusion-Bugs)) · 可信度: ✅
...
> (Source: [SOAP 2026 — Kim et al., "Ravencheck: Effectively-Propositional Reasoning for Rust"](https://plv.colorado.edu/papers.html)) · 可信度: 🟡（会议论文，详细技术待发表后更新）

---

## 八、验证边界：编译错误示例

> **(Source: [Rust Reference](https://doc.rust-lang.org/reference/introduction.html))** 验证工具的目标是捕获运行时错误；但 Rust 编译器本身已能在编译期拒绝大量不安全模式。
...
> (Source: [Kani Documentation](https://model-checking.github.io/kani/)) · (Source: [CBMC](https://github.com/diffblue/cbmc))
...
本组测验围绕测验 1：验证工具分类（理解层）、测验 2：Miri 的检测范围（应用层）、测验 3：验证 ROI 决策（评价层）、测验 4：a-mir-formality 的作用（理解层）等方面设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
