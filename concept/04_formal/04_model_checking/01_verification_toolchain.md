...
- **场景 C：日常 Web 服务业务逻辑**——缺陷代价中等，验证预算有限。推荐：类型系统 + 属性测试（proptest）即可，Kani 仅用于钱/权限相关的核心模块。ROI 判定：验证投入不超过该模块缺陷预期损失的 20%。辑与场景 D: 新并发算法研究的顺序逐层展开。
...
1. **L1 类型系统（免费层）**：借用检查 + 类型安全覆盖内存安全类缺陷——设计层面的「让非法状态不可表示」最大化此层收益；
...
- **Miri：基于 Tree Borrows 的解释执行**——直接解释 MIR，运行时检查未定义行为（悬垂、越界、别名违规）。优势：精确捕获 UB，零假阳性；边界：指数级慢，只能跑小测试，且只检查实际执行到的路径。
