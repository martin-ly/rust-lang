...
- [航空航天认证与形式化方法 (Aerospace Certification \& Formal Methods)](#航空航天认证与形式化方法-aerospace-certification--formal-methods)
  ...
  - [认知路径](#认知路径)
    - [核心推理链](#核心推理链)
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：未检查索引在运行时 panic（rustc 1.97.0 实测）](#反例未检查索引在运行时-panicrustc-1970-实测)
    - [✅ 修正：`get` 返回 `Option`，错误显式处理](#-修正get-返回-option错误显式处理)
...
航空航天软件认证是安全关键系统开发中最严格的软件质量保证流程。
DO-178C（Software Considerations in Airborne Systems and Equipment Certification）是国际航空界广泛采用的软件适航标准，由 RTCA（Radio Technical Commission for Aeronautics）发布。
它为机载软件的开发、验证和认证提供了系统化的框架。

Rust 编程语言的所有权（Ownership）系统和形式化验证生态，为 DO-178C 的合规性提供了独特的技术路径。
本文件系统性地映射 DO-178C/DO-333 的目标到 Rust 工具链，分析 Rust 在航空航天安全关键软件开发中的形式化验证能力、认证路径与现存缺口。
...
> **认知功能**: DO-178C 的核心洞察是**验证强度必须与失效影响成正比**——A 级软件的验证深度远高于 D 级软件。
> Rust 的所有权（Ownership）系统可以在编译期消除一整类内存安全（Memory Safety）错误，相当于为所有等级提供了"零成本"的额外验证层。
> (Source: [DO-178C Introductory Course - RTCA](https://www.rtca.org/))

### 1.2 DO-333 Formal Methods Supplement
>
>

DO-333 是 DO-178C 的技术补充（Technology Supplement），专门定义如何在 DO-178C 框架内使用形式化方法满足验证目标。
DO-333 承认三种形式化方法类别：
...
> - 形式化验证的覆盖范围必须明确声明
> (Source: [DO-333 Section 2.3](https://www.rtca.org/))
...
DO-178C 定义五个软件等级（Software Level），从 A（灾难性失效）到 E（无安全影响）。
每个等级对应不同数量的验证目标：
...
> (Source: [DO-178C Section 6.0](https://www.rtca.org/))
...
> (Source: [DO-333 FM.6.1 - Theorem Proving Objectives](https://www.rtca.org/))
...
> (Source: [DO-178C Section 6.4](https://www.rtca.org/))
...
> (Source: [SPARK Pro - AdaCore](https://www.adacore.com/about-spark))
...
> **Property Preservation 洞察**: Ferrocene 目前采用的策略是**过程合规 + 降级使用**——在 LLVM 后端尚未完全形式化的情况下，通过限制优化级别、增加测试覆盖和独立验证来弥补。这与传统 C 编译器（如 GCC/Clang）的鉴定策略类似，但 Rust 的前端（借用（Borrowing）检查器 + MIR）提供了更强的语义保证基础。
> (Source: [Ferrocene - Qualification Strategy](https://ferrocene.dev/))
...
> (Source: [DO-330 Tool Qualification Guidance](https://www.rtca.org/))
...
> (Source: [Ferrocene - Certification Scope](https://ferrocene.dev/))
...
> **Rust 优势洞察**: Rust 的所有权系统本质上是一个**全自动的、编译期运行的轻量级形式化验证器**。它验证的属性（内存安全（Memory Safety）、无数据竞争）恰好是航空航天软件中最难通过传统测试穷尽的属性。这意味着 Rust 程序进入形式化验证阶段时，已经排除了大量底层错误，使验证资源可以集中在功能正确性上。
...
> (Source: [Rust Verification Roadmap](https://alastairreid.github.io/rust-verification-tools/))
...
- [形式化方法](02_formal_methods.md) — Rust 形式化验证工具概览
- [Hoare 逻辑](../03_operational_semantics/02_hoare_logic.md) — 程序验证的形式化基础
- [RustBelt](../02_separation_logic/01_rustbelt.md) — Rust 类型系统（Type System）的 Iris 形式化
- [Separation Logic](../02_separation_logic/02_separation_logic.md) — 分离逻辑与内存模型
- [Unsafe](../../03_advanced/02_unsafe/01_unsafe.md) — Unsafe Rust 的边界
- [并发安全（Concurrency Safety）](../../03_advanced/00_concurrency/01_concurrency.md) — 并发模型与数据竞争
...
本节把「航空航天认证与形式化方法 (Aerospace Certification & Formal Methods)」的规则推到编译器与运行时的边界上逐一实测：边界测试：MISRA C 规则的 Rust 类比（编译错误）、边界测试：确定性执行与 `const fn`（编译错误）、边界测试：SPARK 模式的 Rust 近似与 `no_panic`（…、边界测试：MC/DC 覆盖率与短路逻辑（逻辑错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
**题目**: DO-178C 标准中的 A/B/C/D/E 五个软件等级（DAL）分别对应什么失效条件？

<details>
<summary>✅ 答案与解析</summary>

A（灾难性）— 导致飞机坠毁或死亡；B（危险）— 严重降低安全裕度；C（重大）— 明显降低安全裕度；D（较小）— 轻微降低安全裕度；E（无安全影响）— 不影响安全。
</details>

---

### 测验 2：Ferrocene 是什么？它在 Rust 航空航天认证中解决了什么问题？（理解层）

**题目**: Ferrocene 是什么？它在 Rust 航空航天认证中解决了什么问题？

<details>
<summary>✅ 答案与解析</summary>

Ferrocene 是经过 DO-178C / EUROCAE ED-12C 预认证的 Rust 工具链，提供确定性的编译器行为和形式化验证支持，解决了 Rust 编译器更新频繁带来的认证可重复性问题。
</details>

---

### 测验 3：为什么航空航天领域特别需要形式化验证，而普通 Web 应用通常不需要？（理解层）

**题目**: 为什么航空航天领域特别需要形式化验证，而普通 Web 应用通常不需要？

<details>
<summary>✅ 答案与解析</summary>

航空航天系统的故障代价极高（生命安全），且无法通过测试穷尽所有边界条件。形式化验证提供数学级别的正确性保证，补充了测试的局限性。
</details>

---

### 测验 4：MISRA C 和 Rust 在安全性保证上有什么根本区别？（理解层）

**题目**: MISRA C 和 Rust 在安全性保证上有什么根本区别？

<details>
<summary>✅ 答案与解析</summary>

MISRA C 是一组编码规范和静态检查规则，依赖开发者遵守和工具扫描。Rust 通过类型系统（Type System）和所有权在编译期强制保证内存安全（Memory Safety），无需额外的编码规范约束。
</details>

---

### 测验 5：Rust 的所有权系统如何帮助满足 DO-178C 的"数据耦合"和"控制耦合"要求？（理解层）

**题目**: Rust 的所有权系统如何帮助满足 DO-178C 的"数据耦合"和"控制耦合"要求？

<details>
<summary>✅ 答案与解析</summary>

所有权和借用（Borrowing）规则在编译期消除了别名突变（alias mutation）和悬垂指针，天然限制了模块（Module）间的隐式数据共享和未预期的控制流影响，降低了耦合度。
</details>
...
本节以未检查索引为反例，展示「假定输入合法」在运行时的失效方式，及 `get` 的显式错误处理（Error Handling）修正。
