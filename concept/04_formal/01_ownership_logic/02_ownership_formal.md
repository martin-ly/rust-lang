...
> **后置延伸**: [L4 RustBelt](../02_separation_logic/01_rustbelt.md) · [L7 形式化方法](../../07_future/04_research_and_experimental/02_formal_methods.md) · [L3 Unsafe](../../03_advanced/02_unsafe/01_unsafe.md)
...
> **ROD 迁移备注**: 本文档是所有权（Ownership）形式化的主入口。关于借用（Borrowing）检查的**可判定性**与**复杂度**，详见 [Borrow Checking Decidability](04_borrow_checking_decidability.md)；关于 Aeneas 符号化语义对借用规则的自动化证明，详见 [Aeneas Symbolic Semantics](../03_operational_semantics/06_aeneas_symbolic_semantics.md)。
...
> **前置概念**: [Type System](../00_type_theory/02_subtype_variance.md)
...
> **[RustBelt: POPL 2018](https://doi.org/10.1145/3158154)** RustBelt is the first formal (and machine-checked) foundations for safe encapsulation of unsafe code in a realistic systems language. We present a novel semantic model of Rust based on *Iris*, a higher-order concurrent separation logic framework.
...
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/04_inter_layer_map.md) §3.1 "L1-L4 形式化映射" · §4.1 "内存安全（Memory Safety）完备性"
...
> **深入阅读**: Pin 的工程语义详见 [`02_async.md`](../../03_advanced/01_async/01_async.md) §8；自引用结构的安全构造见 [`03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) §5。
...
| 概念 | 文件 | 关系 |
|:---|:---|:---|
| 所有权 | [`../01_foundation/01_ownership_borrow_lifetime/01_ownership.md`](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md) | 形式化对象 |
| 生命周期（Lifetimes） | [`../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md`](../../01_foundation/01_ownership_borrow_lifetime/03_lifetimes.md) | 区域类型对应 |
| 借用（Borrowing） | [`../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md`](../../01_foundation/01_ownership_borrow_lifetime/02_borrowing.md) | 分数权限对应 |
| 线性逻辑 | [`./01_linear_logic.md`](01_linear_logic.md) | 理论基础 |
| 类型论 | [`./02_type_theory.md`](../00_type_theory/01_type_theory.md) | 约束求解 |
| RustBelt | [`./04_rustbelt.md`](../02_separation_logic/01_rustbelt.md) | 验证实现 |
| Unsafe | [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) | 别名模型应用 |
| Pin / 自引用 | [`../03_advanced/01_async/02_async.md`](../../03_advanced/01_async/01_async.md) §3.2 | 📎 详见: Pin 的形式化语义与 location stability
...
| 标签一致性（Coherence） | Resource algebra（资源代数） | 确保资源不重复、不丢失 |
...
> **来源**: [Reed 2009 · Patina] · [Jung et al. 2018 · Iris] · [RustBelt: POPL 2018](https://doi.org/10.1145/3158154)
...
> **跨层映射**: 本文件 LTL 规约 ↔ [`../03_advanced/01_async/02_async.md`](../../03_advanced/01_async/01_async.md) §3.2 "Pin 的形式化语义" · [`../03_advanced/01_async/02_async.md`](../../03_advanced/01_async/01_async.md) §5.1 定理矩阵 L2 "Pin ⟹ 自引用安全"
...
> **来源**: [Kani Book: Writing proof harnesses] · [Kani GitHub: Pin verification examples] · [RefinedRust PLDI 2024 — Automated deductive verification for unsafe Rust](https://doi.org/10.1145/3656425)
...
> **跨层映射更新**:
> `L3::Miri` 默认模型变更 ↔ [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md) §5.6 Tree Borrows 演进 · [`../07_future/05_rust_version_tracking.md`](../../07_future/00_version_tracking/01_rust_version_tracking.md) §3 前沿特性跟踪
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../../00_meta/04_navigation/04_inter_layer_map.md) §4.2 "别名模型与 Miri 检测"
> **过渡: L4 → L3**
...
> 工程实践中的对应见 [`../03_advanced/02_unsafe/03_unsafe.md`](../../03_advanced/02_unsafe/01_unsafe.md)（unsafe 逃逸门）与 [`../03_advanced/00_concurrency/01_concurrency.md`](../../03_advanced/00_concurrency/01_concurrency.md)（并发安全（Concurrency Safety）的编译期保证）。
> **过渡: L4 → L5**
...
> **补充来源**
> **相关谓词映射**:
> [RustBelt 核心谓词](../../00_meta/02_sources/02_rustbelt_predicate_map.md#一rustbelt-核心谓词定义) ·
> [定理推导链](../../00_meta/02_sources/02_rustbelt_predicate_map.md#六从谓词到定理的推导链)

## 十、边界测试：所有权形式化的编译错误

所有权形式化的边界测试直接构造形式化预言的编译期反例：
...
> Rust 的所有权形式化将"悬垂指针"这一运行时（Runtime）错误转化为编译期类型错误，通过**区域类型**（region types）和**生命周期包含**（lifetime inclusion）的约束系统实现。
...
本组测验围绕测验 1：所有权的形式化直觉（记忆层）、测验 2：借用作为能力（理解层）、测验 3：Hoare 三元组与 Unsafe（应用层）与测验 4：分离逻辑与并发（分析层）设计，按 Bloom 认知层级从记忆/理解递进到应用/分析。每题给出一段最小化代码或一条论断，判定目标是「能否通过 rustc 1.97（edition 2024）的类型检查与借用检查」或「运行时行为是否符合预期」。建议先遮住答案自行作答，再核对编译器诊断（E0xxx）与修复方案——每道错题都对应一条语言规则的边界，这正是本节要建立的判定依据。
