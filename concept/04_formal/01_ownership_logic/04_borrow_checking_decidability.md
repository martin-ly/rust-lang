...
以 Datalog 风格将借用（Borrowing）检查表达为约束传播，可处理 NLL 拒绝的部分安全模式（条件借用、循环内精确分析）。`-Zpolonius` 在每日构建版可用，被视为未来默认路线。 (Source: [Polonius Repository](https://github.com/rust-lang/polonius))
...
2. **约束生成阶段**：对每个借用生成「outlives 约束」——`'a: 'b @ P` 读作「区域 'a 在程序点 P 包含 'b」；约束是**条件化**的（与程序点绑定，这是 NLL 相对词法生命周期（Lifetimes）的核心创新）；
...
- **模块（Module）化边界**：跨函数借用检查**不做**——函数签名的生命周期标注即「模块接口的契约」，体内检查以契约为前提；此设计使检查**可增量**（函数级并行）且错误信息**可定位**；
- **不可判定的相邻问题**：「程序是否内存安全（Memory Safety）」（含 `unsafe`、FFI、任意指针算术）不可判定（Rice 定理推论）——借用检查是「安全子集上的可判定近似」，`unsafe` 是边界出口。
...
本节聚焦「可运行示例」，覆盖NLL：最后一次使用点后重新借用、可变借用（Mutable Borrow）的独占性、共享与可变不能共存与两阶段借用。论述顺序由定义到边界：先明确「可运行示例」在「Borrow Checking Decidability（借用检查可判定性）」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「可运行示例」的判定标准，并指出它在全页论证链中的位置。
...
本节以引用（Reference）比被引用者长寿为反例，展示借用检查可判定性的核心——区域包含关系的静态判定。
