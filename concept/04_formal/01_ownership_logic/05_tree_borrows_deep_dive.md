...
Tree Borrows 将每次对某块内存的借用（Borrowing）建模为一棵**权限树（permission tree）**：根节点对应最初获得该内存访问权的引用或裸指针，子节点代表通过重新借用（reborrow）派生出的新引用。
每个节点携带一个 **tag** 与四种权限状态之一。
对内存的每次访问都会沿着树传播，只禁用真正冲突的兄弟分支，而不是像 Stacked Borrows 那样弹出整个栈。
因此，父引用在子树结束、且未发生写冲突的前提下可以恢复有效，两阶段可变借用（two-phase borrow）也能被原生表达为 `Reserved → Active` 的状态迁移。
下面分树结构、权限状态与转换规则三节展开。
