...
> **前置概念**: [Linear Logic](01_linear_logic.md) · [Type System](../../01_foundation/02_type_system/01_type_system.md) · [Ownership](../../01_foundation/01_ownership_borrow_lifetime/01_ownership.md)
> **后置概念**: [RustBelt](../02_separation_logic/01_rustbelt.md) · [Session Types](https://en.wikipedia.org/wiki/Session_type)
>
...
> **前置依赖**: [Traits](../../02_intermediate/00_traits/01_traits.md) · [Generics](../../02_intermediate/01_generics/01_generics.md)
...
本节展开线性逻辑在编程语言中的应用技术，三个方向：

1. **线性类型系统**：把线性逻辑的资源规则类型化——`dup`（无 contraction）禁止复制、`drop`（无 weakening）要求消费；Rust 的所有权 + `Copy` 分界 = 「线性世界 + 受控经典世界」的实用折中；
...
1. **「Rust 就是线性类型的」**：不准确——Rust 是**仿射**（affine）类型：资源**至多**用一次（可丢弃），线性要求**恰好**一次；`let x = ...; // 不用` 合法（仅警告）——差别在 `Drop`：Rust 自动 drop 使「忘记消费」无害，纯线性类型中丢弃是类型错误；
2. **「`Copy` 破坏线性性」**：`Copy` 是**受控放松**——只有「按位复制安全」（无堆资源、无 drop 副作用）的类型可实现；线性逻辑的 `!A` 模态是同一思想：经典区域与线性区域的受控接口；
3. **「会话类型库能完全表达协议」**：typestate 近似只能表达**正则**协议——带数据依赖分支的协议（「若 ack 则…否则…」的动态结构）超出类型表达能力，需运行时检查补充。

每条误判附代码反例与理论参照（Wadler 1990、session types 文献）。
...
- [RustBelt](../02_separation_logic/01_rustbelt.md) — 形式化验证
...
本节把「线性逻辑在 Rust 中的工程应用」的规则推到编译器与运行时的边界上逐一实测：边界测试：资源线性消耗与 `Drop` 的冲突（编译错误）、边界测试：`Vec` 的线性所有权与索引（运行时 panic）、边界测试：线性资源的隐式复制（编译错误）、边界测试：`Copy` 与 `Drop` 的互斥性（编译错误）等方面。每个用例标注预期结果（编译错误 / 运行时 panic / 逻辑错误），并用 rustc 1.97 验证：能复现的给出诊断信息与触发条件，不能复现的说明原因。这些用例共同回答一个问题——规则在极限处是否仍然成立，以及违反时编译器能否兜底。
...
> **修正**: 线性逻辑要求资源**恰好使用一次**。Rust 的所有权系统是一种**仿射类型系统（Type System）**（affine）——资源最多使用一次，但允许通过 `Drop` 隐式丢弃。这与纯线性逻辑（不允许丢弃）不同，更符合系统编程需求。`#[derive(Clone)]` 显式 opt-in 复制语义，避免隐式复制导致的性能问题。这与 C++ 的拷贝构造函数（默认复制）形成鲜明对比——Rust 的默认是 move，复制需显式。[Rust Reference](https://doc.rust-lang.org/reference/introduction.html)]
...
> **修正**: `Vec` 的索引操作 `v[i]` 在越界时 panic，体现了 Rust"快速失败"的哲学。从线性逻辑视角，`Vec` 是一个资源容器，其元素是子资源。越界访问试图获取不存在的子资源，是资源使用错误。`get()` 返回 `Option<&T>`，将可能的失败显式编码到类型中，调用者必须处理 `None` 情况。这是 Rust 将运行时错误转为编译期类型安全边界的典型模式。[Rust Standard Library](https://doc.rust-lang.org/std/index.html)]
...
「嵌入式测验（Embedded Quiz）」部分按测验 1：线性逻辑中的"线性"（linearity）指什么？与 Rus…、测验 2：在线性逻辑中，`A ⊗ B`（张量积）和 `A & B`（w…、测验 3：Rust 的 `clone()` 在线性逻辑视角下是什么操作…、测验 4：为什么 Rust 的借用（`&T` 和 `&mut T`）可…等5个方面的顺序逐层展开。
...
```mermaid
mindmap
  root((线性逻辑在 Rust 中的工程应用))
    核心概念
      1 从线性逻辑到所有权
      2 资源作为类型
      3 Session Types 与通信协议
    技术细节
      1 所有权即线性类型
      2 仿射类型与 Drop
      3 类型状态模式
