...
> **后置概念**: [Rustc Trait Solver](03_trait_solver_in_rustc.md) · [Type Layout](08_type_layout.md)
> **定理链**: Generic Function → Monomorphization → Static Dispatch / Dynamic Dispatch
> **本节关键术语**: 单态化 (Monomorphization) · 静态分发 (Static Dispatch) · 动态分发 (Dynamic Dispatch) · 类型擦除 (Type Erasure) · Fat Pointer · VTable — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
...
1. **问题识别**: 为什么泛型（Generics）编译器行为在 Rust 中值得关注？它直接决定二进制大小、运行时（Runtime）性能和编译时间，是零成本抽象（Zero-Cost Abstraction）的核心。
2. **概念建立**: 掌握单态化（Monomorphization）、静态分发、动态分发、类型擦除、胖指针和 VTable 的核心机制。
3. **机制推理**: 通过 ⟹ 定理链将泛型函数、类型参数、实例化和分发方式串联起来。
4. **边界辨析**: 借助反命题/反例理解单态化（Monomorphization）膨胀和动态分发开销的适用边界。
...
| 目标 | 推荐方案 | 原因 |
|:---|:---|:---|
| 最大化运行时（Runtime）性能 | 泛型 / `impl Trait` | 可内联、零开销 |
...
`dyn Trait` 的 VTable 布局、`Any::downcast_ref` 的类型擦除与恢复，以及 `core::raw_vtable` 等底层操作都涉及 unsafe 语义。详见 [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md)。

## 七、相关概念

| 概念 | 关系 |
|:---|:---|
| [Rustc Trait Solver](03_trait_solver_in_rustc.md) | 泛型约束求解 |
| [Type Layout](08_type_layout.md) | 单态化后类型布局 |
| [Application Binary Interface](05_application_binary_interface.md) | trait object 的 ABI |
| [Unsafe Rust](../../03_advanced/02_unsafe/01_unsafe.md) | VTable 和类型擦除涉及 unsafe |
