> **内容分级**: [综述级]
> **本节关键术语**: Query System · `TyCtxt` · Dep Graph · Red-Green Algorithm · Incremental Compilation · Salsa · `rustc_middle` — [完整对照表](../../00_meta/01_terminology/01_terminology_glossary.md)
>
# Rustc 查询系统与增量编译
...
- [Rustc 查询系统与增量编译](#rustc-查询系统与增量编译)
  ...
  - [⚠️ 反例与陷阱](#️-反例与陷阱)
    - [反例：无间接层的递归类型（rustc 1.97.0 实测）](#反例无间接层的递归类型rustc-1970-实测)
    - [✅ 修正：加 `Box` 间接层](#-修正加-box-间接层)
...
rustc 查询系统（query system）是增量编译与懒求值的基石，三个核心抽象：

1. **查询（query）**：带键值缓存的纯函数——`fn query(Key) -> Value`，如 `type_of(DefId)`、`mir_built(DefId)`；编译器被重构为「查询的调用图」而非阶段流水线；
2. **查询键与缓存**：结果按 key 缓存于 `QueryCache`——`DefId` 类键使「函数级粒度」的复用成为可能；缓存跨编译会话持久化（incremental 目录）；
3. **提供者（provider）**：查询的实现函数注册表——`provide` 宏（Macro）把实现挂到查询名上，支持「同一查询多种实现」（如本地 crate 与外部 crate 元数据）。

设计收益：懒求值（只算需要的）、自动并行（查询间无数据依赖可并行）、增量（依赖追踪自动失效）——理解查询系统是理解 rustc 架构的第一入口。
...
本节展开增量编译的核心算法——red-green 依赖追踪（源自 Salsa 框架，rustc 自研实现）：

- **依赖图**：查询调用形成 DAG——`typeck(f)` 依赖 `type_of(g)`（f 调用 g），边在查询执行时自动记录（`DepGraph`）；
- **red-green 染色**：新编译会话重放依赖图——输入未变的子图标绿（结果复用），受变更影响的标红（重算）；判断「未变」用 DepNode 的指纹（fingerprint，内容的哈希）；
- **指纹与序列化**：查询结果必须可哈希 + 可序列化（`Ty` 的稳定哈希特殊处理）——这解释了 rustc 内部类型表示的诸多约束（arena 分配、interning）；
- **失败模式**：指纹碰撞（理论上）与「粗粒度依赖」（一个查询依赖过多 ⟹ 级联标红）——查询粒度设计是增量性能的关键。

本节附 red-green 过程的逐步图示与调试入口（`-Zincremental-info`）。
...
本节聚焦「典型输出解读」，覆盖冷编译（无缓存）、无修改再次编译与修改 `greet::hello` 后。论述顺序由定义到边界：先明确「典型输出解读」在「Rustc 查询系统与增量编译」中的确切含义与适用范围，再给出可核验的例证或数据，最后标注它与相邻主题的分界线。读完后应能用一句话复述「典型输出解读」的判定标准，并指出它在全页论证链中的位置。
...
本节以无间接层递归类型为反例，展示大小可判定性对递归数据定义的约束。
...
> **认知功能**: 本 mindmap 从本页「Rustc 查询系统与增量编译」的章节结构提炼，一级分支对应核心主题，叶子节点为关键子概念，可作为本页的快速导航与复习索引。
