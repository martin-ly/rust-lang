...
本节建立代数效应的完整概念骨架：从「把计算中的请求显式化」的定义出发，依次展开操作签名、处理器、续延与泛代数视角；五个子节共同说明 effect handler 如何把效果的意义从效果的调用中解耦。
...
本节从操作语义、类型效应、单子等价、限定续延互编码与自由单子五个角度，把 §1 的直觉推进为可推理的形式工具，为后续对比 Rust 的关键字效果奠定技术基础。
...
本节把代数效应的通用框架映射到 Rust 现实：先识别 Rust 以关键字形式实现的隐性 effect system，再以 async/await 为具体案例，最后论证为什么 Rust 选择关键字泛型而非通用 handler。
...
本节对比四类代表性实现：工业运行时处理器、静态行多态语言、学术原型与 Rust 的关键字效果；能力矩阵把表达力、运行时成本与生态成熟度放到同一坐标系中。
...
本节通过常见误读与反例澄清代数效应的边界：Rust `async` 不是通用 handler，回调无法等价替代续延，异常只是 handler 的特例；最后给出与 monad 变换器的选型边界。
