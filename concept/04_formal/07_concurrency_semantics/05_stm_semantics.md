...
本节从事务内存的「原子块」直觉出发，梳理从硬件事务内存到可组合事务的历史谱系，再沿着正确性条件与实现策略两条轴建立 STM 的理论坐标。
...
本节深入到实现机制：先说明事务变量与读写集如何封装共享状态，再展开提交协议、可组合阻塞/选择、锁不可组合性与小步语义，把「原子块」直觉还原为可实现的算法组件。
...
本节从 Rust 设计公理出发解释「无 STM」并非功能缺失，而是借用检查器与零成本抽象的理性结果；随后给出所有权、通道、异步与无锁等替代谱系，并评估库级 STM 的能力上限。
...
本节把 STM 置于多语言生态中比较：Haskell 的类型级纯度、Clojure 的 MVCC、ScalaSTM 的库级实现、C++ TM TS 与 HTM 的兴衰，最终汇总为能力矩阵。
...
本节通过反例与边界讨论纠正 STM 的常见误判：事务不能回滚世界、高竞争下乐观并发可能反转性能、Rust `stm` crate 不等于 Haskell STM；最后给出 STM、锁、无锁与消息传递的选型框架。
