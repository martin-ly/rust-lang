...
> **Summary**: 从 Hindley-Milner 的立方时间可判定性出发，说明 Rust 类型推断因高阶多态、trait/生命周期约束与关联类型而进入 PSPACE，并映射到 rustc 的 `typeck`、`InferCtxt` 与 trait solver 实现。
> **受众**: [研究者]
...
> **Bloom 层级**: 分析 → 评价
> **A/S/P 标记**: **F** — Formal
> **双维定位**: F×Type — 类型系统复杂度与可判定性
> **定位**: 解释 Rust 类型推断为什么从 HM 的 $O(n^3)$ 跃迁到 PSPACE-完全，以及这一理论结论如何在 rustc 的约束求解器中得到工程化实现。
...
用教学类比的语言，可以把 HM 看作一个**约束生成 + Robinson 合一**的两阶段算法：先为每个子表达式生成类型等式，再用合一求解最一般的替换。因为 HM 只允许受限的多态性（let-多态性）且不含子类型、trait 或生命周期，约束空间足够简单，从而保证多项式时间可解。
...
| **生命周期 / 区域约束** | `'a: 'b`、`&'a T` | 偏序约束、子类型、借用检查交互 |
...
由于 $\text{APTIME} = \text{PSPACE}$，这一交替多项式时间算法给出了 PSPACE 上界。实际上 rustc 并不会真的枚举所有替换，而是使用更聪明的约束传播与缓存，但理论模型说明空间需求可被多项式限制。
...
把每个布尔变量编码为类型变量，把量词编码为泛型约束，把布尔连接词编码为 trait 约束，使得：
...
若调用点无法确定具体迭代器类型，`I::Item` 就无法归约，需要显式 turbofish 或类型标注。
...
| 4. 复杂度结论 | PSPACE 上界/下界如何建立？ | 能评估类型系统设计的可判定性 |
...
| 来源 | 可信度 | 说明 |
|:---|:---:|:---|
| Rehman et al. 2023 | ✅ 一级 | Rust 类型推断 PSPACE 完全性分析 |
| [Vytiniotis et al. 2011](https://doi.org/10.1017/S0956796811000098) | ✅ 一级 | OutsideIn(X)：带局部假设的模块化类型推断 |
...
> **权威来源**: [Rustc Dev Guide](https://rustc-dev-guide.rust-lang.org/), [The Rust Reference](https://doc.rust-lang.org/reference/), [The Rust Programming Language](https://doc.rust-lang.org/book/title-page.html)
>
