...
- [操作语义：程序行为的形式化定义](#操作语义程序行为的形式化定义)
  ...
  - [十、边界测试：操作语义的编译错误](#十边界测试操作语义的编译错误)
    - [10.1 边界测试：求值顺序与副作用（运行时 UB）](#101-边界测试求值顺序与副作用运行时-ub)
    - [10.2 边界测试： panic 的栈展开与资源释放（运行时行为）](#102-边界测试-panic-的栈展开与资源释放运行时行为)
  - [三、应用映射](#三应用映射)
  ...
  - [权威来源索引](#权威来源索引)
  - [十、边界测试：操作语义的编译错误](#十边界测试操作语义的编译错误-1)
    - [10.1 边界测试：求值顺序与副作用（运行时 UB）](#101-边界测试求值顺序与副作用运行时-ub-1)
    ...
    - [10.5 边界测试：MIR 常量求值的循环限制（编译错误）](#105-边界测试mir-常量求值的循环限制编译错误)
    - [10.5 边界测试：MIR 的 `SwitchInt` 与穷尽性检查（编译错误）](#105-边界测试mir-的-switchint-与穷尽性检查编译错误)
    - [10.3 边界测试：求值顺序的明确性与未定义行为边界（编译错误）](#103-边界测试求值顺序的明确性与未定义行为边界编译错误)
    - [10.6 边界测试：返回局部变量的悬垂引用](#106-边界测试返回局部变量的悬垂引用)
...
### 10.1 边界测试：求值顺序与副作用（运行时 UB）

```rust
fn main() {
    let mut x = 0;
    // ⚠️ 运行时 UB: 同一位置的多个可变引用
...
> **修正**: Rust 的操作语义基于**小步语义**（small-step semantics）和**内存模型**（Stacked Borrows / Tree Borrows）。关键规则是：同一时间只能有一个活跃的可变引用（`&mut T`）或任意数量的共享引用（`&T`）。违反此规则是未定义行为（UB），即使通过 unsafe 代码实现。Miri（Rust 的解释器）通过追踪引用的"借用标签"（borrow tags）在运行时检测此类违规。这与 C 的未定义行为（依赖编译器实现）不同——Rust 的 UB 有明确的形式化定义，可通过工具验证。[来源: [Rust Operational Semantics](https://doc.rust-lang.org/nomicon/)]

### 10.2 边界测试： panic 的栈展开与资源释放（运行时行为）
...
fn main() {
    let mut v = vec![1, 2, 3];
    {
        ...
        // 若此处 panic，栈展开会调用 Guard::drop
        // panic!("intentional");
    } // 正常释放时 Guard::drop 被调用
    println!("{:?}", v); // [1, 2, 3, -1]
}
```
...
> **修正**: Rust 的操作语义基于**小步归约**（small-step reduction）和**内存模型**（Tree Borrows / Stacked Borrows）。关键规则：同一时间只能有一个活跃的可变引用，或任意数量的共享引用。违反此规则是未定义行为（UB）。Miri 通过追踪每个引用的"借用标签"（borrow tag）在运行时检测违规。这与 C 的未定义行为（依赖编译器实现）不同——Rust 的 UB 有明确的形式化定义，可通过工具验证。[来源: [Rust Operational Semantics](https://doc.rust-lang.org/nomicon/)]
...
> **修正**: 这是 Rust 借用检查器的核心规则在操作语义层面的体现：同一时间，对同一内存位置，要么存在任意数量的不可变引用 `&T`（读访问），要么存在一个可变引用 `&mut T`（写访问），二者互斥。形式化上，这一规则对应于线性逻辑中的**资源分裂**（resource splitting）：所有权 `own(x)` 可分裂为两个只读能力 `read(x)` ∧ `read(x)`，或一个读写能力 `write(x)`，但不能同时分裂为 `read(x)` ∧ `write(x)`。编译器在生成 MIR（Mid-level IR）时，通过**借用检查**（borrow check）验证引用生命周期的不相交性。此规则是 Rust"零成本抽象"的基石：无需运行时锁即可保证内存安全。[来源: [Rust Reference — Lifetimes](https://doc.rust-lang.org/reference/items/associated-items.html)] · [来源: [Stacked Borrows Paper](https://doi.org/10.1145/3371106)]
...
const X: usize = slow(1_000_000);
// ❌ 编译错误: MIR 常量求值有步数限制，过大可能超时
...
> **修正**: Rust 的编译期常量求值（MIR 解释器）有**步数限制**（默认约 100 万步），防止无限循环或过长编译。`slow(1_000_000)` 可能超过步数限制，编译错误。这与 C++ 的 `constexpr`（编译期求值深度有限制，但通常更大）或 Zig 的 `comptime`（图灵完全，但同样可能编译缓慢）类似——编译期计算的能力与编译时间权衡。Rust 的设计目标：常量求值应快速完成，不拖慢编译。复杂计算应在运行期执行，或使用 `lazy_static`/`once_cell` 延迟初始化。这与 Haskell 的 CTFE（Compile Time Function Execution，GHC 支持但有限制）或 D 的 `enum` 计算（同样有限制）相同——编译期不是通用计算环境。[来源: [Rust Reference — Const Evaluation](https://doc.rust-lang.org/reference/const_eval.html)] · [来源: [Rust Const Eval Limits](https://rust-lang.github.io/const-eval/)]

### 10.5 边界测试：MIR 的 `SwitchInt` 与穷尽性检查（编译错误）
