...
> **Summary**: Axiomatic Semantics. Core Rust concept with theoretical foundations and practical applications.
> **受众**: [研究者]
...
含义：较长生命周期 `'a` 的借用可以安全地降级为较短生命周期 `'b` 的借用。
...
- **指称语义**（`12_denotational_semantics.md`）：公理语义的**完备性基础**——指称语义证明类型系统的 soundness，为公理规约的良定义性提供保证
...
> **修正**: Rust 编译器的借用检查器可以被视为一个**全自动的、零成本的公理验证器**。它自动推断并验证了借用规则的不变式（3.3 节），无需程序员手动书写 `{P} C {Q}`。这是 Hoare 逻辑从学术走向工业的最成功实践——将公理规约"编译"进类型系统。[来源: [Rust Reference — Borrowing](https://doc.rust-lang.org/reference/expressions.html?highlight=borrow#evaluation-order)] · [来源: [RustBelt Paper](https://doi.org/10.1145/3158154)]
...
操作语义告诉我们程序**如何**执行（一步一步的状态转换），指称语义告诉我们程序**表示什么**数学对象（不动点、CPO）。但它们都无法直接回答：**我怎么知道这个程序是正确的？**

公理语义通过**霍尔三元组** `{P} C {Q}` 将正确性转化为逻辑命题：给定前置条件 `P`，执行命令 `C` 后，后置条件 `Q` 必然成立。这是从"描述程序"到"规约程序"的范式跃迁。
