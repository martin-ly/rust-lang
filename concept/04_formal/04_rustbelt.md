...
> **主要来源**: [RustBelt: POPL 2018](https://doi.org/10.1145/3158154) · [Iris Project](https://iris-project.org/) · [Creusot](https://creusot.rs/) · [Verus](https://github.com/verus-lang/verusverus/guide/) · [Kani: AWS] · [Aeneas](https://github.com/AeneasVerif/aeneas) · [RefinedRust] · [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html)
...
- v2.0 (2026-05-13): 重构定理一致性（Coherence）矩阵至 11 行，新增反命题决策树 3 组，扩展认知路径 5 步，补充层次一致性标注（L1–L3），强化 Wikipedia / POPL 2018 / Iris 引用（Reference）
...
|:---|:---|:---|:---|:---|:---|:---|
| **L1-A** | Iris 高阶分离逻辑 | Hoare 逻辑 + 高阶幽灵状态 + 原子性 | 并发资源推理的模块（Module）化组合 | L1-B, L2-A, T1, T3 | 物理内存模型与 Iris 抽象模型不一致；Coq 内核存在 bug | **L1** |
| **L1-B** | Iris 不变量（Invariants） | L1-A + 持久性模态 `□` | 跨线程共享状态的持久断言 | T1, C1 | 原子操作（Atomic Operations）语义被违反；编译器重排超出 C11 模型 | **L1** |
| **L1-C** | 并发分离逻辑（CSL） | L1-A + 资源分区的并行组合 | 无锁数据结构的并发正确性 | T1, T4 | 错误 `Ordering`；ABA 问题超出逻辑模型范围 | **L1** |
| **L2-A** | 协议类型（Protocol Types） | L1-A + 状态机语义 | 所有权（Ownership）转移协议的形式化规约 | T3, C1 | 协议规范不完整；状态迁移条件遗漏 | **L2** |
| **L2-B** | 生命周期（Lifetimes）逻辑（Lifetime Logic） | L2-A + 区域（Region）约束 | 借用（Borrowing）引用（Reference）的有效性保证 | T2, T3 | `unsafe` 绕过生命周期检查；`dangling` 指针手动构造 | **L2** |
...
> **跨层映射**: 本文件定理 ↔ [`00_meta/inter_layer_map.md`](../00_meta/inter_layer_map.md) §4.1 "内存安全（Memory Safety）完备性" · §5.2 "定理一致性检查"
...
| **L2 语义层** | L2-A, L2-B, L2-C | 协议类型、生命周期逻辑、λRust 操作语义 | 将 Rust 特有机制（借用（Borrowing）、生命周期（Lifetimes）、MIR）形式化为数学对象 |
...
> - **L3 并发**: [`../03_advanced/01_concurrency.md`](../03_advanced/01_concurrency.md) §2 "Send/Sync 语义" ↔ T1（无数据竞争）。CSL 是并发安全（Concurrency Safety）的逻辑根基，Mutex/Arc 的形式化规约见 §3
...
> ```
>
> 该规则直接对应 Rust `Send` 语义：捕获不相交资源的闭包（Closures）可安全并行执行。
...
**命题三分析**: Iris 是**通用**的高阶并发分离逻辑框架（L1 层），可实例化到多种语言。但 RustBelt 的 L2/L3 层——协议类型、生命周期（Lifetimes）逻辑、λRust 语义——深度绑定于 Rust 的所有权-借用-生命周期体系。将 Iris 应用于其他语言需重建 L2 层，工作量接近于重新发表一篇 RustBelt 级别的论文。
...
RustBelt 证明了 Rust 的哪个子集是内存安全（Memory Safety）的？
...
这与 C 的"信任程序员"（无任何验证）或 Java 的 JVM（运行时（Runtime）检查，无形式化内存模型）不同——Rust 提供从类型系统（Type System）到形式化验证的多层安全网。
</details>
...
- **Kani**：基于 CBMC 的有界模型检测，适合验证并发安全（Concurrency Safety）属性
...
- A. "RustBelt 证明了 Rust 类型系统（Type System）的 soundness"
...
| **Vec** | 内存安全（Memory Safety） + 越界检查 + 重新分配 | ⚠️ 部分（逻辑关系已建模） | 高 | 完整 Vec 状态机复杂 |
...
| **高** | 并发、原子操作（Atomic Operations）、资源不变量 | `Mutex` 对接平台线程原语；`Arc` 证明引用计数协议原子性；`Vec` 处理重分配指针失效 |
...
**验证原理**: Creusot 将 Rust 程序翻译为 **Why3**，Why3 生成证明义务（proof obligations）交给多种 SMT 求解器（Alt-Ergo、CVC5、Z3）。预言（prophecy）变量允许验证异步（Async）代码中"未来才会确定的值"。
...
fn main() {
    ...
    let r2 = &mut v[1]; // ❌ 编译错误: cannot borrow `v` as mutable more than once
    // Rust 编译器将整个 Vec 视为一个借用单元
    // 即使 r1 和 r2 指向不同元素，也不能同时可变借用
}
```

> **修正**: 使用 `split_first_mut()` 或 `split_at_mut()` 安全地获取多个不重叠的可变引用（Mutable Reference）。

### 11.2 边界测试：Send/Sync 自动推导失败（编译错误）

```rust,compile_fail
use std::rc::Rc;

struct Wrapper(Rc<i32>); // Rc<T> 不是 Send

fn main() {
    let w = Wrapper(Rc::new(42));
...
> **修正**: 使用 `Arc<T>`（原子引用计数）替代 `Rc<T>`，使类型实现 `Send`/`Sync`。
...
fn main() {
    ...
    let _ = r; // E0382: r 已被 drop（move）
}
```

> **修正**: `Drop::drop` 获取所有权（move），之后变量不可用。如需复用，使用 `ManuallyDrop` 或重新构造。
...
fn main() {
    ...
    let r2 = &v[0];   // 嵌套共享借用
    // ❌ 编译错误: 在共享借用存在时获取可变借用
    let r3 = &mut v;  // E0502: cannot borrow `v` as mutable
    println!("{}", r1.len()); // r1 仍活跃
}
```
