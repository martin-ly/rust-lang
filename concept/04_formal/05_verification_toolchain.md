...
- [Verification Toolchain Selection Guide（验证工具链选择指南）](#verification-toolchain-selection-guide验证工具链选择指南)
  ...
  - [三、a-mir-formality：Rust 类型系统（Type System）规范](LINK_PLACEHOLDER)
  ...
  - [七、验证工具深度原理分析](#七验证工具深度原理分析)
    ...
    - [7.5 Aeneas：基于借用（Borrowing）的函数式翻译](LINK_PLACEHOLDER)
  ...
  - [八、验证边界：编译错误示例](#八验证边界编译错误示例)
    - [编译错误 1：Safe Rust 中直接解引用（Reference）裸指针](LINK_PLACEHOLDER)
    - [编译错误 2：`unsafe impl Send` 错误应用](#编译错误-2unsafe-impl-send-错误应用)
    - [编译错误 3：`const fn` 中调用非 const 操作](#编译错误-3const-fn-中调用非-const-操作)
    - [编译错误 4：生命周期（Lifetimes）不匹配导致悬垂引用](LINK_PLACEHOLDER)
    - [编译错误 5：`move` 闭包（Closures）捕获引用后外部继续使用](LINK_PLACEHOLDER)
  - [权威来源索引](#权威来源索引)
