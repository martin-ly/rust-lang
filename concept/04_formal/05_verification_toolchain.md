...
> 这与 Rust 的"零成本抽象"哲学形成有趣对比：Kani 是"低成本验证"，用有界性换取易用性。
...
> **核心发现**: HIR 和 MIR 占据了 **80.07%** 的 rustc bug——这正是 Rust 独特语言机制（类型系统、借用检查、所有权分析）最集中的阶段。
...
> **过渡**: Verification Toolchain Selection Guide（验证工具链选择指南） 的设计理念体现了 Rust 零成本抽象与安全保证的核心权衡，理解这一权衡有助于迁移到更高级的并发与形式化验证领域。
