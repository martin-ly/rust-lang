...
- [Verification Toolchain Selection Guide（验证工具链选择指南）](#verification-toolchain-selection-guide验证工具链选择指南)
  ...
  - [三、a-mir-formality：Rust 类型系统（Type System）规范](#三a-mir-formalityrust-类型系统规范)
  ...
  - [七、验证工具深度原理分析](#七验证工具深度原理分析)
    ...
    - [7.5 Aeneas：基于借用（Borrowing）的函数式翻译](#75-aeneas基于借用的函数式翻译)
  ...
  - [八、验证边界：编译错误示例](#八验证边界编译错误示例)
    - [编译错误 1：Safe Rust 中直接解引用（Reference）裸指针](#编译错误-1safe-rust-中直接解引用裸指针)
    - [编译错误 2：`unsafe impl Send` 错误应用](#编译错误-2unsafe-impl-send-错误应用)
    - [编译错误 3：`const fn` 中调用非 const 操作](#编译错误-3const-fn-中调用非-const-操作)
    - [编译错误 4：生命周期（Lifetimes）不匹配导致悬垂引用](#编译错误-4生命周期不匹配导致悬垂引用)
    - [编译错误 5：`move` 闭包（Closures）捕获引用后外部继续使用](#编译错误-5move-闭包捕获引用后外部继续使用)
  - [权威来源索引](#权威来源索引)
...
> [来源: [PLDI 2026 — Hance et al., "VerusBelt: A Semantic Foundation for Verus's Proof-Oriented Extensions to the Rust Type System"](https://verus-lang.github.io/verus/guide/)]
...
> **核心发现**: HIR 和 MIR 占据了 **80.07%** 的 rustc bug——这正是 Rust 独特语言机制（类型系统（Type System）、借用检查、所有权分析）最集中的阶段。
...
>
> Kani 的适用场景：有限状态协议（mutex、channel）、小数据结构的属性、unsafe 代码的内存安全边界。
