# 概念知识体系自动化审计报告 v2.0

> 生成时间: 2026-05-24T11:14
> 扫描文件数: 204
> 版本对齐: Rust 1.95.0 stable

## 摘要

| 指标 | 数值 | 状态 |
|:---|:---|:---|
| 扫描文件数 | 204 | — |
| 跨文件链接 ≥3 | 204/204 | ✅ |
| 死链接文件 | 0 | ✅ |
| 命名规范符合 | 204/204 | ✅ |
| 代码块问题文件 | 0 | ✅ |
| 显式 Bloom 标注 | 204/204 | ✅ |
| 平均来源标注率 | 31.9% | ✅ |
| TODO 待完成 | 0 | ✅ |
| TODO 已完成 | 258 | — |

## 来源标注率偏低的文件 (< 10%)

| 文件 | 标注数 | 估算段落数 | 标注率 |
|:---|:---|:---|:---|
| concept/03_advanced/19_parallel_distributed_pattern_spectrum.md | 13 | 110 | 7.1% |
| concept/06_ecosystem/35_pattern_composition_algebra.md | 2 | 90 | 1.1% |

## 方法论说明

- **跨文件链接**: 检测指向其他 `.md` 文件的相对链接，目标文件必须存在
- **Bloom 层级**: 基于认知层级关键词的启发式检测 + 显式标注检查
- **来源标注率**: 标注数 / (段落数 + 论断数 × 2)，期望 ≥ 15%
- **命名规范**: `NN_english_name.md`，入口文件 (`00.md`–`07.md`) 除外

> 本报告与 `scripts/concept_consistency_auditor.py` 互补：本脚本做快速健康检查，后者做深度概念冲突检测。
