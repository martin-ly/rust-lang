...
- 扫描 **2243** 个权威 URL。按诚实口径把 **403 反爬**（53 个，全在 P1 学术站 arxiv/acm/ieee，链接本身很可能有效、仅脚本 UA 被拦）从『失效』中剥离单列后：**真失效 194（8.6%）** = 185×404 + 9 其它（SSL EOF / Timeout / 400 / 418 / 编码），按 tier P0=72 / P1=9 / P2=113；**反爬 403 = 53（2.4%）** 待浏览器人工复核（不计入被对齐内容失效）。
...
- 含义：在诚实口径下『对齐有效性 ≈ **91.4%**』（2243 中 194 真失效）。这 194 是历史积累的真失效（TRPL/Reference/Nomicon 旧章节重命名、crate/仓库被 yanked 或改名、Ferrocene 旧路径），**不能臆测新址**（否则以新错链替换旧错链）；53 个 P1 学术 403 链接本身多半有效，需浏览器人工复核。
