...
**日期**: 2026-07-12  **语义健康总分**: **93.6 / 100**  **等级**: **OK** (OK≥85 / WARN≥60 / FAIL<60)

> 与 `kb_quality_dashboard.md`（数量型：⟹/mermaid/代码块计数）互补；本仪表盘衡量语义质量（一致性/闭环/交叉/唯一/KG 完整）。

| 维度 | 权重 | 得分 | 关键证据 |
|---|:---:|:---:|:---|
| 元数据一致性 | 30% | 96.3 | flagged 18/483（D1互斥/D2脱节/D3重声明/D4版本/D5 nightly/D6套话） |
| 拓扑实质度 | 25% | 90.7 | 定义套话率 0.011 / 关系塌缩 0.833 / 跳出 0.165 / 死端 0 / 判定定量 0.933 |
| 去重健康 | 20% | 85.0 | 重叠命中 592，可处理（MERGE+DOCS_INTERNAL） 53 |
...
**总分**: 93.6 = 0.30×96.3 + 0.25×90.7 + 0.20×85.0 + 0.25×100.0
