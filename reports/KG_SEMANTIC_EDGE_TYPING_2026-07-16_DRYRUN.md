...
| 谓词 | 前 | 后 | Δ |
|:---|---:|---:|---:|
| ex:relatedTo | 6806 | 6791 | -15 |
| ex:dependsOn | 834 | 834 | +0 |
| ex:entails | 756 | 756 | +0 |
...
| I3-self-evident-kind | retyped relatedTo | ex:PanicAndAbort | ex:instanceOf | ex:ErrorHandling | 01_foundation/08_error_handling/03_panic_and_abort.md 标题“Panic 与 Abort：不可恢复错误的处理机制”——panic/abort 是错误处理机制的一种（不可恢复分支） |
...
| I3-self-evident-kind | retyped relatedTo | ex:LockingPrimitives | ex:instanceOf | ex:Concurrency | 03_advanced/00_concurrency/07_lock_free.md:11“深入探讨 Rust 中的无锁编程”（位于 00_concurrency 章节）；01_concurrency.md:181-182 将锁/原子操作列为并发同步手段——无锁原语是并发技术的一种 |
...
| A1-page-application | retyped relatedTo | ex:SerdePatterns | ex:appliesTo | ex:Traits | 02_intermediate/00_traits/03_serde_patterns.md:4“Serde 序列化模式：Rust 的类型驱动数据转换”（位于 00_traits 章节；关键术语 Serialize/Deserialize trait）——serde 是 trait 机制的应用 |
...
- JSON: `reports/KG_SEMANTIC_EDGE_TYPING_2026-07-16_DRYRUN.json`
