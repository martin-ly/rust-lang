...
**日期**: 2026-07-16  **语义健康总分**: **99.5 / 100**  **等级**: **OK** (OK≥85 / WARN≥60 / FAIL<60)

> 与 `kb_quality_dashboard.md`（数量型：⟹/mermaid/代码块计数）互补；本仪表盘衡量语义质量（一致性/闭环/交叉/唯一/KG 完整）。

| 维度 | 权重 | 得分 | 关键证据 |
|---|:---:|:---:|:---|
| 元数据一致性 | 30% | 99.4 | flagged 3/521（D1互斥/D2脱节/D3重声明/D4版本/D5 nightly/D6套话） |
| 拓扑实质度 | 25% | 98.6 | 定义套话率 0.0 / 关系塌缩 0.749 / 跳出 0.028 / 死端 0 / 判定定量 0.98 |
| 去重健康 | 20% | 100.0 | 重叠命中 549，可处理（MERGE+DOCS_INTERNAL） 0 |
...
**总分**: 99.5 = 0.30×99.4 + 0.25×98.6 + 0.20×100.0 + 0.25×100.0
