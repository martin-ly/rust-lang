...
- [x] 禁止在 C 类目录新建文件（除非明确标记为临时研究）
- [x] 每季度运行一次重复检测脚本
- [x] 研究笔记完成 90 天后必须决定：迁移到主轨 / 归档 / 删除
...
- [x] 禁止在 C 类目录新建文件（除非明确标记为临时研究）
- [x] 每季度运行一次 `scripts/detect_content_overlap.py` 重复检测
- [x] 研究笔记完成 90 天后必须决定：迁移到主轨 / 归档 / 删除
- [x] 每次归档后运行 `fix_archived_research_notes_links.py` 清理引用残留
