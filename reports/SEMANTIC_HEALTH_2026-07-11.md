...
**日期**: 2026-07-11  **语义健康总分**: **54.2 / 100**  **等级**: **FAIL** (OK≥85 / WARN≥60 / FAIL<60)

> 与 `kb_quality_dashboard.md`（数量型：⟹/mermaid/代码块计数）互补；本仪表盘衡量语义质量（一致性/闭环/交叉/唯一/KG 完整）。

| 维度 | 权重 | 得分 | 关键证据 |
|---|:---:|:---:|:---|
| 元数据一致性 | 30% | 36.3 | flagged 302/474（D1互斥/D2脱节/D3重声明/D4版本/D5 nightly/D6套话） |
| 拓扑实质度 | 25% | 15.5 | 定义套话率 0.255 / 关系塌缩 0.992 / 跳出 0.307 / 死端 3 / 判定定量 0.095 |
| 去重健康 | 20% | 84.7 | 重叠命中 572，可处理（MERGE+DOCS_INTERNAL） 54 |
...
**总分**: 54.2 = 0.30×36.3 + 0.25×15.5 + 0.20×84.7 + 0.25×90
