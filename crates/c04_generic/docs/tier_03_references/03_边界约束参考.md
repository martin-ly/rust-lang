...
**边界约束完整参考 (Bound Constraints Complete Reference)**:

- **定义**: 边界约束完整参考手册，包括 Trait Bounds、Where 子句、HRTB、生命周期约束等
- **类型**: 语法参考文档
- **范畴**: 泛型编程、类型系统
- **版本**: Rust 1.92.0+
- **相关概念**: Trait Bounds、Where 子句、HRTB、生命周期约束、约束传播

### 属性特征

**核心属性**:

- **Trait Bounds 语法**: 完整 BNF 语法、基础约束语法、约束位置
- **Where 子句完整语法**: BNF 语法定义、基础 Where 子句、关联类型约束、复杂 Where 子句
- **高阶 Trait Bounds (HRTB)**: HRTB 语法、常见 HRTB 模式、HRTB 与生命周期的关系
- **生命周期约束**: 生命周期约束语法、静态生命周期约束
...
- 边界约束完整参考 --[covers]--> 边界约束完整内容
- 泛型程序 --[uses]--> 边界约束

**依赖关系**:

- 边界约束完整参考 --[depends-on]--> 语言规范
- 泛型编程 --[depends-on]--> 边界约束
...
- [04\_关联类型参考.md](./04_关联类型参考.md) - 关联类型
- [../tier*02_guides/01*泛型基础指南.md](../tier_02_guides/01_泛型基础指南.md) - 泛型基础
