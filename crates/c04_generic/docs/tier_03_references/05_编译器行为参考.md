...
**编译器行为完整参考 (Compiler Behavior Complete Reference)**:

- **定义**: 编译器行为完整参考手册，包括单态化、静态分发、动态分发、类型擦除等
- **类型**: 技术参考文档
- **范畴**: 编译器机制、性能优化
- **版本**: Rust 1.92.0+
- **相关概念**: 单态化、静态分发、动态分发、类型擦除、Fat Pointer、编译器优化

### 属性特征

**核心属性**:

- **Monomorphization (单态化)**: 定义、工作原理、优势与代价、代码膨胀示例
- **静态分发 vs 动态分发**: 静态分发、动态分发、性能对比
- **Type Erasure (类型擦除)**: 定义、类型擦除过程、类型擦除的限制
- **Fat Pointer 结构**: 定义、内存布局、VTable 结构
...
- **静态分发**: 快（可内联）
- **动态分发**: 慢（间接调用）
- **适用场景**: 性能优化、代码大小优化、编译时间优化

### 关系连接

**组合关系**:

- 编译器行为完整参考 --[covers]--> 编译器行为完整内容
- 性能优化程序 --[uses]--> 编译器行为

**依赖关系**:

- 编译器行为完整参考 --[depends-on]--> 编译器机制
- 性能优化 --[depends-on]--> 编译器行为
...
- [../tier*04_advanced/03*零成本抽象.md](../tier_04_advanced/03_零成本抽象.md) - 零成本抽象详解
- [../tier*02_guides/05*实战模式指南.md](../tier_02_guides/05_实战模式指南.md) - 实战模式
