...
where
    T: Clone,
{
    Box::new(items.iter().filter(|_| true).take(100).cloned())
}
...
pub mod shared_state_demo {
    ...
    pub fn mutex_counter(thread_count: usize, iterations: usize) -> i32 {
        ...
        let handles: Vec<_> = (0..thread_count)
            .map(|_| spawn_counter_thread(Arc::clone(&counter), iterations))
            .collect();
    ...
    pub fn rwlock_accumulate(readers: usize, writers: usize, iterations: usize) -> i32 {
        ...
        // 启动读取线程
        for _ in 0..readers {
            handles.push(spawn_reader_thread(Arc::clone(&data), iterations));
        }

        // 启动写入线程
        for _ in 0..writers {
            handles.push(spawn_writer_thread(Arc::clone(&data), iterations));
        }
