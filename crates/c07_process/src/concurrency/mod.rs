...
impl SyncManager {
    ...
    /// 创建互斥锁
    pub fn create_mutex(&mut self, name: &str) -> SyncResult<Arc<crate::concurrency::mutex::ProcessMutex>> {
    ...
    /// 创建读写锁
    pub fn create_rwlock(&mut self, name: &str) -> SyncResult<Arc<crate::concurrency::rwlock::ProcessRwLock>> {
    ...
    /// 创建条件变量
    pub fn create_condvar(&mut self, name: &str) -> SyncResult<Arc<crate::concurrency::condvar::ProcessCondVar>> {
    ...
    /// 创建信号量
    pub fn create_semaphore(&mut self, name: &str, permits: usize) -> SyncResult<Arc<crate::concurrency::semaphore::ProcessSemaphore>> {
        let semaphore = crate::concurrency::semaphore::ProcessSemaphore::new(name, permits, self.config.clone());
    ...
    /// 创建屏障
    pub fn create_barrier(&mut self, name: &str, parties: usize) -> SyncResult<Arc<crate::concurrency::barrier::ProcessBarrier>> {
        let barrier = crate::concurrency::barrier::ProcessBarrier::new(name, parties, self.config.clone());
