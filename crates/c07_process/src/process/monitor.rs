...
impl ProcessMonitor {
    ...
    pub fn get_stats(&self) -> ProcessStats {
        ...
        let running = processes.values()
            ...
            .count();
        let sleeping = processes.values()
            .filter(|p| p.status == crate::types::ProcessStatus::Sleeping)
            .count();
        let stopped = processes.values()
    ...
    pub fn cleanup_old_data(&self, max_age: Duration) {
        ...
        metrics.cpu_usage_history.retain(|(timestamp, _)| *timestamp >= cutoff);
        metrics.memory_usage_history.retain(|(timestamp, _)| *timestamp >= cutoff);
        metrics.disk_io_history.retain(|(timestamp, _)| *timestamp >= cutoff);
        metrics.network_io_history.retain(|(timestamp, _)| *timestamp >= cutoff);
    }
}
