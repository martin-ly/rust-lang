...
async fn demonstrate_basic_error_handling(manager: &EnhancedErrorManager) -> Result<()> {
    ...
    let error_id1 = manager
        .record_error(&process_error, "process_manager", context.clone())
        .await;
    ...
    let error_id2 = manager
        .record_error(&ipc_error, "ipc_manager", context.clone())
        .await;
    ...
    let error_id3 = manager
        .record_error(&sync_error, "sync_manager", context.clone())
        .await;
...
async fn demonstrate_error_recovery(manager: &EnhancedErrorManager) -> Result<()> {
    ...
    let recoverable_error =
        ProcessError::StartFailed("Temporary process start failure".to_string());
    let mut context = HashMap::new();
    context.insert("retry_count".to_string(), "0".to_string());
    context.insert("max_retries".to_string(), "3".to_string());

    let error_id = manager
        .record_error(&recoverable_error, "process_manager", context.clone())
        .await;
...
async fn demonstrate_error_chain_tracking(manager: &EnhancedErrorManager) -> Result<()> {
    ...
    let error_id1 = manager
        .record_error(&error1, "process_manager", context.clone())
        .await;
    ...
    let error2 =
        IpcError::ConnectionFailed("IPC connection failed due to process failure".to_string());
    let error_id2 = manager
        .record_error(&error2, "ipc_manager", context.clone())
        .await;
    ...
    let error_id3 = manager
        .record_error(&error3, "sync_manager", context.clone())
        .await;
...
async fn demonstrate_advanced_error_handling(manager: &EnhancedErrorManager) -> Result<()> {
    ...
    let error_id = manager
        .record_error(&system_error, "system_monitor", context.clone())
        .await;
    ...
    let error_id2 = manager
        .record_error(&network_error, "network_manager", context.clone())
        .await;
    ...
    let error_id3 = manager
        .record_error(&concurrency_error, "concurrency_manager", context.clone())
        .await;
    ...
    if history.len() > 1 {
        let mut error_times: Vec<std::time::SystemTime> =
