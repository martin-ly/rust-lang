//! Rust 1.97.0 stable 特性 —— 进程与 FFI
//! Rust 1.97.0 stabilized features —— process and FFI
//!
//! 本文件展示与进程管理、FFI 和系统编程相关的 Rust 1.97.0 stable API 与行为变更。
...
///
/// 涉及特性：
/// - `std::ptr::fn_addr_eq`（Rust 1.97.0 stable）：可移植的函数指针地址比较
...
impl Rust197ProcessFeatures {
    /// 比较两个函数指针的地址是否相等。
    ///
    /// Rust 1.97.0 使用 `std::ptr::fn_addr_eq(f1, f2)` 处理 provenance 问题。
