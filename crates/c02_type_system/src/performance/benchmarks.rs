...
pub mod rust_189_benchmarks {
    ...
    pub fn benchmark_const_generics() -> Vec<BenchmarkResult> {
        ...
        let const_generic_result = runner.run("常量泛型数组", || {
            let arr = ConstGenericArray::new([0; 100]);
            let _sum: i32 = arr.data.iter().sum();
        });
    ...
    pub fn benchmark_tait() -> Vec<BenchmarkResult> {
        ...
        // 传统返回类型 vs TAIT
        let traditional_result = runner.run("传统返回类型", || {
            let _result = traditional_function();
        });
        results.push(traditional_result);
        
        let tait_result = runner.run("TAIT返回类型", || {
            let _result = tait_function();
        });
...
pub fn run_all_benchmarks() -> PerformanceAnalysis {
    ...
    let gats_results = rust_189_benchmarks::benchmark_gats();
    let tait_results = rust_189_benchmarks::benchmark_tait();
    ...
    for result in gats_results {
        analyzer.add_result(result);
    }
    for result in tait_results {
        analyzer.add_result(result);
    }
