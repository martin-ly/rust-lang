...
impl ExpressionEvaluator {
    ...
    pub fn evaluate(&self, expr: &ComplexExpression) -> Result<LiteralValue, String> {
        match expr {
            ...
            // 变量查找
            ComplexExpression::Variable(name) => self
                .variables
                .get(name)
                .cloned()
            ...
            ComplexExpression::BinaryOp {
                ...
                match (left_val, operator, right_val) {
                    ...
                    (LiteralValue::Integer(a), BinaryOperator::Div, LiteralValue::Integer(b))
                        if b != 0 =>
                    {
                        Ok(LiteralValue::Integer(a / b))
                    }
                    ...
                    (LiteralValue::Float(a), BinaryOperator::Div, LiteralValue::Float(b))
                        if b != 0.0 =>
                    {
                        Ok(LiteralValue::Float(a / b))
                    }
            ...
            ComplexExpression::FunctionCall { name, arguments } => {
                ...
                match name.as_str() {
                    ...
                    "abs" if arg_values.len() == 1 => match &arg_values[0] {
...
impl PatternMatchingOptimizer {
    ...
    fn optimize_expression(&self, expr: &ComplexExpression) -> ComplexExpression {
        match expr {
            ...
            ComplexExpression::BinaryOp {
                ...
                if let (ComplexExpression::Literal(a), ComplexExpression::Literal(b)) =
            ...
            ComplexExpression::UnaryOp { operator, operand } => {
                if let ComplexExpression::Literal(lit) = operand.as_ref()
                    && let Ok(result) = self.fold_unary_constant(operator, lit)
                {
                    return ComplexExpression::Literal(result);
                }
            ...
            // 其他表达式递归优化
            ComplexExpression::FunctionCall { name, arguments } => {
                let optimized_args: Vec<ComplexExpression> = arguments
                    .iter()
    ...
    fn fold_constants(
        ...
        match (a, op, b) {
            ...
            (LiteralValue::Integer(x), BinaryOperator::Div, LiteralValue::Integer(y))
                if *y != 0 =>
            {
                Ok(LiteralValue::Integer(x / y))
            }
            ...
            NestedData::Object(fields) => {
                let filtered_fields: HashMap<String, NestedData> = fields
                    ...
                    .filter_map(|(key, value)| {
                        self.filter(value, predicate.clone())
                            .map(|v| (key.clone(), v))
                    })
                    .collect();
...
pub fn demonstrate_advanced_pattern_matching() {
    ...
    let conditional_expr = ComplexExpression::Conditional {
        ...
        true_branch: Box::new(ComplexExpression::Literal(LiteralValue::String(
            "x > 5".to_string(),
        ))),
        false_branch: Box::new(ComplexExpression::Literal(LiteralValue::String(
            "x <= 5".to_string(),
        ))),
    };
    ...
    // 转换数据
    let transformed = nested_matcher.transform(&nested_data, |data| match data {
    ...
    // 过滤数据
    let filtered = nested_matcher.filter(&nested_data, |data| match data {
...
mod tests {
    ...
    fn test_nested_pattern_matching() {
        ...
        let data = NestedData::Branch {
            name: "root".to_string(),
            children: vec![NestedData::Leaf("test".to_string())],
        };
