    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AppError::Validation(e) => {
                write!(f, "Validation error in field '{}': {}", e.field, e.message)
            }
            ...
            AppError::System(e) => write!(
                f,
                "System error in component '{}': {}",
                e.component, e.message
            ),
            AppError::Config(e) => write!(f, "Config error for key '{}': {}", e.key, e.message),
            AppError::Permission(e) => write!(
                f,
                "Permission error for resource '{}' action '{}'",
                e.resource, e.action
            ),
            AppError::Resource(e) => write!(
                f,
                "Resource error for {} '{}': {}",
                e.resource_type, e.resource_id, e.message
            ),
            AppError::Timeout(e) => write!(
                f,
                "Timeout error for operation '{}': {}",
                e.operation, e.message
            ),
            ...
            match strategy {
                ...
                RecoveryStrategy::Fallback { default_value } => {
                    ...
                    Err(AppError::Unknown(
                        "Fallback not implemented for type T".to_string(),
                    ))
                }
                RecoveryStrategy::Skip => Err(AppError::Unknown("Operation skipped".to_string())),
                RecoveryStrategy::Degrade { fallback_service } => Err(AppError::Unknown(format!(
                    "Degraded to service: {}",
                    fallback_service
                ))),
...
impl ErrorHandler {
    ...
    pub fn handle_error(&self, error: AppError, context: ErrorContext) -> Result<(), AppError> {
        ...
        self.monitor
            .log_error(transformed_error.clone(), context.clone(), level);

        // 尝试恢复
        self.recovery
            .recover(&transformed_error, || Err(transformed_error.clone()))
    }
