...
**类型型变参考 (Type Variance Reference)**:

- **定义**: 类型型变（Variance）完整参考，包括协变、逆变、不变等
- **类型**: 技术参考文档
- **范畴**: 类型系统、泛型系统
- **版本**: Rust 1.92.0+
- **相关概念**: 型变、协变、逆变、不变、子类型关系、PhantomData

### 属性特征

**核心属性**:

- **协变 (Covariance)**: 保持子类型关系、常见协变类型
- **逆变 (Contravariance)**: 反转子类型关系、函数参数
- **不变 (Invariance)**: 不允许子类型替换、可变引用
- **生命周期的型变**: 生命周期子类型、引用的型变

**性能特征**:

- **编译时处理**: 型变在编译时处理
- **零运行时开销**: 型变零运行时开销
- **适用场景**: 类型安全、内存安全、子类型关系

### 关系连接

**组合关系**:

- 类型型变参考 --[covers]--> 型变完整内容
- 类型安全程序 --[uses]--> 型变

**依赖关系**:

- 类型型变参考 --[depends-on]--> 类型系统
- 类型安全 --[depends-on]--> 型变
