...
**生命周期指南 (Lifetime Guide)**:

- **定义**: 系统学习 Rust 生命周期系统的实践指南
- **类型**: 实践指南文档
- **范畴**: 类型系统、内存安全
- **版本**: Rust 1.0+
- **相关概念**: 生命周期、生命周期标注、借用检查器、生命周期省略、HRTB

**生命周期 (Lifetime)**:

- **定义**: 编译期构造的证明变量，表示引用的有效范围
- **类型**: 内存安全机制
- **属性**: 生命周期标注、生命周期省略、生命周期参数、生命周期子类型
- **关系**: 与所有权、借用相关
...
- **生命周期标注**: 语法 `'a`、生命周期参数
- **生命周期省略**: 编译器自动推断、三条规则
- **静态生命周期**: `'static` 生命周期、使用场景
- **生命周期子类型**: 协变和逆变、生命周期关系
- **HRTB**: 高阶 trait bounds、生命周期约束
...
static MUTABLE: std::sync::atomic::AtomicI32 = std::sync::atomic::AtomicI32::new(0);
...
1. **[3.1 类型转换参考](../tier_03_references/01_type_conversions_reference.md)** - 类型转换技术
2. **[3.2 类型型变参考](../tier_03_references/02_variance_reference.md)** - 型变和子类型
3. **[3.3 分派机制参考](../tier_03_references/03_dispatch_mechanisms_reference.md)** - 静态和动态分发
...
- [2.3 泛型编程指南](03_generics_programming_guide.md)
- [2.4 Trait系统指南](04_trait_system_guide.md)
- [1.0 项目概览](../tier_01_foundations/01_project_overview.md)
- [1.1 主索引导航](../tier_01_foundations/02_navigation.md)

**深度分析**:

- [生命周期原理](../tier_03_references/01_type_system_specification.md)
- [借用检查器详解](../tier_03_references/02_variance_reference.md)
