...
impl TextPreprocessor {
    ...
    pub fn build_vocabulary(&mut self, texts: &[String]) {
        ...
        // 过滤低频词并构建词汇表
        let mut vocab_items: Vec<_> = word_counts.into_iter()
...
impl SentimentAnalyzer {
    ...
    pub fn new() -> Self {
        let positive_words = vec![
            "good".to_string(), "great".to_string(), "excellent".to_string(), "amazing".to_string(), "wonderful".to_string(),
            "fantastic".to_string(), "awesome".to_string(), "brilliant".to_string(), "perfect".to_string(), "love".to_string()
        ];
        
        let negative_words = vec![
            "bad".to_string(), "terrible".to_string(), "awful".to_string(), "horrible".to_string(), "disgusting".to_string(),
            "hate".to_string(), "worst".to_string(), "disappointing".to_string(), "ugly".to_string(), "stupid".to_string()
        ];
    ...
    pub fn analyze(&self, text: &str) -> SentimentResult {
        ...
        SentimentResult {
            sentiment,
            score: if word_count > 0 { score / word_count as f64 } else { 0.0 },
...
impl NamedEntityRecognizer {
    ...
    pub fn new() -> Self {
        Self {
            person_patterns: vec![
                "Mr.".to_string(), "Ms.".to_string(), "Dr.".to_string(),
                "Professor".to_string(), "President".to_string(),
            ],
            location_patterns: vec![
                "City".to_string(), "State".to_string(), "Country".to_string(),
                "Street".to_string(), "Avenue".to_string(), "Road".to_string(),
            ],
            organization_patterns: vec![
                "Inc.".to_string(), "Corp.".to_string(), "Ltd.".to_string(),
                "Company".to_string(), "University".to_string(), "Institute".to_string(),
            ],
        }
    }
    ...
    pub fn recognize(&self, text: &str) -> Vec<NamedEntity> {
        ...
        for (i, word) in words.iter().enumerate() {
            // 检查人名
            if self.person_patterns.iter().any(|pattern| word.contains(pattern)) {
            ...
            // 检查地名
            if self.location_patterns.iter().any(|pattern| word.contains(pattern)) {
                entities.push(NamedEntity {
                    text: word.to_string(),
            ...
            // 检查机构名
            if self.organization_patterns.iter().any(|pattern| word.contains(pattern)) {
                entities.push(NamedEntity {
                    text: word.to_string(),
