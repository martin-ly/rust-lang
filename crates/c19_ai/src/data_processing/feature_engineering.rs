...
pub enum FeatureTransformation {
    ...
    /// 特征组合
    FeatureCombination { columns: Vec<String>, operation: CombinationOperation },
    /// 特征分箱
    FeatureBinning { column: String, bins: usize, strategy: BinningStrategy },
    /// 特征编码
    FeatureEncoding { column: String, method: EncodingMethod },
    /// 特征选择
    FeatureSelection { method: SelectionMethod, k: usize },
    /// 降维
    DimensionalityReduction { method: ReductionMethod, n_components: usize },
}
...
impl FeatureEngineer {
    ...
    /// 应用单个特征变换
    fn apply_transformation(&self, df: DataFrame, transformation: &FeatureTransformation) -> Result<DataFrame, String> {
        match transformation {
            ...
            FeatureTransformation::FeatureBinning { column, bins, strategy } => {
            ...
            FeatureTransformation::DimensionalityReduction { method, n_components } => {
    ...
    /// 创建多项式特征
    fn create_polynomial_features(&self, mut df: DataFrame, degree: u32, columns: &[String]) -> Result<DataFrame, String> {
        let mut new_columns = df.columns.clone();
        let mut new_data = df.data.clone();
        
        for col_name in columns {
            let col_idx = df.columns.iter()
    ...
    /// 组合特征
    fn combine_features(&self, mut df: DataFrame, columns: &[String], operation: &CombinationOperation) -> Result<DataFrame, String> {
        if columns.len() < 2 {
            return Err("至少需要两个列进行组合".to_string());
        }
        
        let indices: Result<Vec<usize>, String> = columns.iter()
            .map(|col| df.columns.iter()
                .position(|c| c == col)
            ...
            for &idx in &indices[1..] {
                result = match operation {
                    ...
                    CombinationOperation::Divide => {
                        if row[idx] != 0.0 { result / row[idx] } else { 0.0 }
                    }
    ...
    /// 特征分箱
    fn bin_feature(&self, mut df: DataFrame, column: &str, bins: usize, strategy: &BinningStrategy) -> Result<DataFrame, String> {
        let col_idx = df.columns.iter()
    ...
    /// 特征编码
    fn encode_feature(&self, mut df: DataFrame, column: &str, method: &EncodingMethod) -> Result<DataFrame, String> {
        let col_idx = df.columns.iter()
            .position(|col| col == column)
            .ok_or_else(|| format!("列 '{}' 不存在", column))?;
        ...
        match method {
            EncodingMethod::OneHot => {
                ...
                for (i, unique_val) in unique_values.iter().enumerate() {
                    ...
                    for row in &mut df.data {
                        let is_match = if (row[col_idx] - unique_val).abs() < 1e-10 { 1.0 } else { 0.0 };
                        row.push(is_match);
                    }
                }
            }
            EncodingMethod::Label => {
                ...
                for row in &mut df.data {
                    let encoded = unique_values.iter()
    ...
    /// 特征选择
    fn select_features(&self, df: DataFrame, method: &SelectionMethod, k: usize) -> Result<DataFrame, String> {
        match method {
    ...
    /// 降维
    fn reduce_dimensions(&self, df: DataFrame, method: &ReductionMethod, n_components: usize) -> Result<DataFrame, String> {
        match method {
            ReductionMethod::PCA => {
...
mod tests {
    ...
    fn test_polynomial_features() {
        ...
        let result = engineer.create_polynomial_features(df, 2, &["col1".to_string()]).unwrap();
    ...
    fn test_feature_combination() {
        let mut df = DataFrame::new("test".to_string(), vec!["col1".to_string(), "col2".to_string()]);
        df.add_row(vec![2.0, 3.0]).unwrap();
        df.add_row(vec![4.0, 5.0]).unwrap();
        
        let engineer = FeatureEngineer::new("test".to_string());
        let result = engineer.combine_features(df, &["col1".to_string(), "col2".to_string()], &CombinationOperation::Add).unwrap();
