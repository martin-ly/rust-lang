...
**深入学习**: [借用系统](../tier_02_guides/02_borrowing_practice_guide.md)
...
- ✅ NLL 通过逻辑证明更精确地计算引用的有效范围（逻辑关系，非物理时间）
- ✅ 错误信息非常详细（逻辑证明的失败，非运行时内存冲突）
...
**定义**: 指向已失效资源的引用。从引用一致性视角看，悬垂引用是**逻辑关系的错误**，而非内存地址的失效。

**Rust 的防护**（引用一致性视角）:
Rust 编译器通过**编译期逻辑证明**保证不会产生悬垂引用：
...
- ✅ 借用检查器通过约束求解确保引用有效性（逻辑关系，非内存状态）
- ✅ 零运行时开销（编译期证明，非运行时检查）

**深入学习**: [内存安全](../tier_03_references/08_memory_safety_reference.md)
...
1. **输入位置**: 每个引用参数获得独立的生命周期参数（证明变量）
2. **输出位置**: 如果只有一个输入生命周期，输出使用它（逻辑依赖关系）
3. **方法**: 如果有 `&self` 或 `&mut self`，输出使用它的生命周期（逻辑关系）
...
**定义**: 资源控制权从一个变量转移到另一个变量。从引用一致性视角看，移动是**资源控制权的转移**，而非内存拷贝。
...
**深入学习**: [所有权系统](../tier_02_guides/01_ownership_quick_start.md)
...
**深入学习**: [并发安全](../tier_04_advanced/05_cross_thread_ownership.md)
...
**定义**: Rust 1.31+ 引入的更智能的借用检查，基于实际使用而非词法作用域。从引用一致性视角看，NLL 通过**逻辑证明**更精确地计算引用的有效范围，而非简单地追踪内存状态。
...
- ✅ 更灵活的借用规则（逻辑证明，非内存检查）
- ✅ 减少不必要的作用域限制（逻辑关系，非物理时间）
- ✅ Rust 1.90 中已非常成熟（编译期逻辑证明的完善）
...
**定义**: Rust 资源管理的核心机制，每个资源都有唯一的所有者。从引用一致性视角看，所有权是**资源构造性存在的证明**，而非内存管理机制。
...
**深入学习**: [高级所有权模式](../tier_03_references/06_advanced_ownership_patterns_reference.md)
...
- **不可变引用** `&T`: 只读访问许可证（存在性证明）
- **可变引用** `&mut T`: 独占写入能力的证明（唯一性证明）
...
**定义**: 变量有效的代码区域。从引用一致性视角看，作用域是**资源的逻辑作用域**，而非内存的物理区域。

**规则**（引用一致性视角）:

- 变量从声明点开始有效（逻辑证明）
- 变量在离开作用域时失效（逻辑关系，非物理时间）
- 离开作用域自动调用 `drop`（资源释放，非内存释放）
...
- ✅ 自动资源管理（编译期证明，非运行时检查）
- ✅ RAII 模式（资源获取即初始化，逻辑证明）
- ✅ 确定性析构（编译期证明的资源生命周期，非内存管理）

**深入学习**: [作用域规则](../tier_02_guides/04_scope_management_practice.md)
...
- 📚 [Tier 2: 核心概念层](../tier_02_guides/README.md)
