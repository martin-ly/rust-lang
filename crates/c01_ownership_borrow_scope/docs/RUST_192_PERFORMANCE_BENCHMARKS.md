...
- [Rust 1.92.0 所有权系统性能基准测试](#rust-1920-所有权系统性能基准测试)
  ...
  - [📊 测试结果](#-测试结果)
    - [1. MaybeUninit 性能测试](#1-maybeuninit-性能测试)
      - [测试场景: SafeMaybeUninit 操作性能](#测试场景-safemaybeuninit-操作性能)
    - [2. 联合体原始引用性能测试](#2-联合体原始引用性能测试)
      - [测试场景: Rust192Union 原始引用操作](#测试场景-rust192union-原始引用操作)
    - [3. 零大小数组性能测试](#3-零大小数组性能测试)
      - [测试场景: Rust192ZeroSizedArray 操作](#测试场景-rust192zerosizedarray-操作)
    - [4. 所有权转移性能测试](#4-所有权转移性能测试)
      - [测试场景: 不同大小的 Vec 所有权转移](#测试场景-不同大小的-vec-所有权转移)
    - [5. 借用检查性能测试](#5-借用检查性能测试)
      - [测试场景: BorrowTracker 记录借用](#测试场景-borrowtracker-记录借用)
    - [6. 生命周期推断性能测试](#6-生命周期推断性能测试)
      - [测试场景: LifetimeTracker 记录生命周期](#测试场景-lifetimetracker-记录生命周期)
    - [7. 所有权跟踪性能测试](#7-所有权跟踪性能测试)
      - [测试场景: OwnershipTracker 所有权转移](#测试场景-ownershiptracker-所有权转移)
    - [8. track\_caller 性能测试](#8-track_caller-性能测试)
      - [测试场景: #\[track\_caller\] 函数调用](#测试场景-track_caller-函数调用)
    - [9. 高阶生命周期性能测试](#9-高阶生命周期性能测试)
      - [测试场景: 高阶生命周期函数调用](#测试场景-高阶生命周期函数调用)
    - [10. 标准库 API 性能测试](#10-标准库-api-性能测试)
      - [10.1 NonZero::div\_ceil 性能](#101-nonzerodiv_ceil-性能)
      - [10.2 rotate\_right 性能](#102-rotate_right-性能)
      - [10.3 迭代器比较性能](#103-迭代器比较性能)
  - [📈 性能对比图表](#-性能对比图表)
    - [性能提升汇总表](#性能提升汇总表)
  - [🔍 详细分析](#-详细分析)
    - [内存管理性能分析](#内存管理性能分析)
