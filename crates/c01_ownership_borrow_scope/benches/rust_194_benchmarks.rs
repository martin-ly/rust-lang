...
fn bench_array_windows_vs_windows(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, _data| {
                b.iter(|| {
                    let sum: f64 = data.array_windows::<3>().map(|&[a, b, c]| a + b + c).sum();
                    black_box(sum);
                });
            },
        );
        ...
        // windows (传统)
        group.bench_with_input(BenchmarkId::new("windows", size), &data, |b, _data| {
            b.iter(|| {
                let sum: f64 = data.windows(3).map(|w| w[0] + w[1] + w[2]).sum();
                black_box(sum);
            });
        });

        // 手动索引
        group.bench_with_input(BenchmarkId::new("manual_index", size), &data, |b, _data| {
            b.iter(|| {
                let mut sum = 0.0;
    ...
    for window_size in [2, 3, 4, 5].iter() {
        if *window_size == 2 {
            group.bench_function("window_2", |b| {
                b.iter(|| {
                    let sum: f64 = data.array_windows::<2>().map(|&[a, b]| a + b).sum();
                    black_box(sum);
                });
            });
...
static BENCH_LAZYLOCK: LazyLock<Vec<i32>> = LazyLock::new(|| (0..1000).collect());
