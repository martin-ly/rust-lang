...
use c01_ownership_borrow_scope::scope::{
    Scope, ScopeType, ScopeManager, ScopeAnalyzer, VariableInfo, ScopeError
};
...
fn test_scope_manager_basic_operations() {
    ...
    // 进入作用域
    manager.enter_scope("main".to_string(), ScopeType::Function).unwrap();
...
fn test_scope_manager_nested_scopes() {
    ...
    // 主作用域
    manager.enter_scope("main".to_string(), ScopeType::Function).unwrap();
    manager.declare_variable("main_var".to_string(), "i32".to_string(), "100".to_string(), false, None).unwrap();
    
    // 嵌套作用域
    manager.enter_scope("inner".to_string(), ScopeType::Block).unwrap();
    manager.declare_variable("inner_var".to_string(), "String".to_string(), "hello".to_string(), false, None).unwrap();
...
fn test_scope_analyzer() {
    ...
    // 创建嵌套作用域结构
    manager.enter_scope("main".to_string(), ScopeType::Function).unwrap();
    manager.enter_scope("loop".to_string(), ScopeType::Loop).unwrap();
    manager.enter_scope("conditional".to_string(), ScopeType::Conditional).unwrap();
    
    // 添加一些变量
    manager.declare_variable("x".to_string(), "i32".to_string(), "42".to_string(), false, None).unwrap();
    manager.declare_variable("y".to_string(), "String".to_string(), "hello".to_string(), true, None).unwrap();
...
fn test_scope_manager_complex_scenario() {
    ...
    // 模拟复杂的程序结构
    manager.enter_scope("main".to_string(), ScopeType::Function).unwrap();
    manager.declare_variable("main_var".to_string(), "i32".to_string(), "0".to_string(), true, None).unwrap();
    
    // 循环作用域
    manager.enter_scope("for_loop".to_string(), ScopeType::Loop).unwrap();
    manager.declare_variable("i".to_string(), "usize".to_string(), "0".to_string(), true, None).unwrap();
    
    // 条件分支
    manager.enter_scope("if_branch".to_string(), ScopeType::Conditional).unwrap();
    manager.declare_variable("temp".to_string(), "i32".to_string(), "100".to_string(), false, None).unwrap();
...
fn test_scope_manager_edge_cases() {
    ...
    // 单个作用域的情况
    manager.enter_scope("single".to_string(), ScopeType::Block).unwrap();
    assert_eq!(manager.get_scope_depth(), 1);
    assert!(manager.get_current_scope().is_some());
...
fn test_scope_visibility_rules() {
    ...
    // 创建作用域层次结构
    manager.enter_scope("global".to_string(), ScopeType::Module).unwrap();
    manager.declare_variable("global_var".to_string(), "i32".to_string(), "0".to_string(), false, None).unwrap();
    
    manager.enter_scope("function".to_string(), ScopeType::Function).unwrap();
    manager.declare_variable("func_var".to_string(), "String".to_string(), "hello".to_string(), false, None).unwrap();
    
    manager.enter_scope("block".to_string(), ScopeType::Block).unwrap();
    manager.declare_variable("block_var".to_string(), "f64".to_string(), "3.14".to_string(), false, None).unwrap();
