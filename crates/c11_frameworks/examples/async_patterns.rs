...
use tokio::{
    sync::{
        mpsc, oneshot, broadcast, Mutex, RwLock, Semaphore,
        Barrier, Notify, watch,
    },
    time::{sleep, Duration, timeout, interval, Instant},
...
pub struct BatchProcessor<T> {
    ...
    processor: Arc<dyn Fn(Vec<T>) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send>> + Send + Sync>,
}
    ...
    T: Send + 'static,
{
    ...
    where
        F: Fn(Vec<T>) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send>> + Send + Sync + 'static,
    {
        Self {
            batch_size,
            flush_interval,
...
async fn event_bus_example() -> Result<(), String> {
    ...
    // 发布事件
    event_bus.publish("test", "Hello, World!".to_string()).await?;
...
mod tests {
    ...
    async fn test_task_manager() {
        ...
        manager.spawn_task("test".to_string(), async {
            sleep(Duration::from_millis(100)).await;
        }).await.unwrap();
