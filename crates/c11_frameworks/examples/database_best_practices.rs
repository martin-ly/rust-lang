...
impl TransactionManager {
    ...
    pub async fn begin_transaction(&mut self, id: &str) -> DatabaseResult<()> {
        println!("🔄 开始事务: {}", id);
        self.transactions
            .insert(id.to_string(), TransactionState::Started);
        DatabaseResult::Success(())
    }
...
impl MigrationManager {
    ...
    pub async fn rollback(&mut self, version: &str) -> DatabaseResult<()> {
        ...
        for migration in self.migrations.iter().rev() {
            if migration.version.as_str() > version
                && self.applied_migrations.contains(&migration.version)
            {
...
async fn main() {
    ...
    migration_manager.add_migration(Migration {
        ...
        up_sql: "CREATE TABLE users (id UUID PRIMARY KEY, name VARCHAR(100), email VARCHAR(100))"
            .to_string(),
    ...
    migration_manager.add_migration(Migration {
        ...
        up_sql: "CREATE TABLE orders (id UUID PRIMARY KEY, user_id UUID, amount DECIMAL(10,2))"
            .to_string(),
