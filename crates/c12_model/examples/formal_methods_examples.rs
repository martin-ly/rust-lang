...
impl FiniteStateMachine {
    ...
    pub fn can_transition(&self, event: &str) -> bool {
        self.transitions
            .iter()
            .any(|t| t.from == self.current_state && t.event == event)
    }
    ...
    pub fn check_invariant(&self, invariant: &str) -> bool {
        match invariant {
            "always_safe" => {
                ...
                self.states
                    .values()
                    .all(|state| state.get_property("safe").unwrap_or(true))
            }
            "eventually_goal" => {
                // 检查是否存在从当前状态到目标状态的路径
...
impl ModelChecker {
    ...
    fn check_formula(
        ...
        match formula {
            TemporalFormula::Atomic(prop) => self
                .fsm
                .states
                .get(state)
            ...
            TemporalFormula::And(left, right) => {
                self.check_formula(left, state, visited)
                    && self.check_formula(right, state, visited)
            }
...
fn main() {
    ...
    println!(
        "   满足不变式 'always_safe': {}",
        fsm.check_invariant("always_safe")
    );
    println!();
