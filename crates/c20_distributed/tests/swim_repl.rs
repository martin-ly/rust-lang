...
impl SwimTransport for DummyPing { 
    fn ping(&self, _to: &str) -> bool { true }
    fn gossip(&self, _to: &str, _events: &[swim::SwimEvent]) -> bool { true }
}

#[test]
fn swim_probe_alive() {
    let node = swim::SwimNode::new("n0".into(), DummyPing)
        .with_params(Duration::from_millis(500), Duration::from_millis(5000), 2);
