...
fn build(nodes: &[&str]) -> (LocalReplicator<u64>, Vec<String>) {
    ...
    let mut v = Vec::new();
    for n in nodes { ring.add_node(n); v.push((*n).to_string()); }
...
fn required_acks_majority() {
    assert_eq!(MajorityQuorum::required_acks(1, ConsistencyLevel::Quorum), 1);
    assert_eq!(MajorityQuorum::required_acks(2, ConsistencyLevel::Quorum), 2);
    assert_eq!(MajorityQuorum::required_acks(3, ConsistencyLevel::Quorum), 2);
    assert_eq!(MajorityQuorum::required_acks(5, ConsistencyLevel::Quorum), 3);
    assert_eq!(MajorityQuorum::required_acks(4, ConsistencyLevel::Quorum), 3);
    assert_eq!(MajorityQuorum::required_acks(5, ConsistencyLevel::Eventual), 1);
}
...
fn replicate_to_nodes_ok_and_err() {
    ...
    for i in 2..nodes.len() { r.successes.insert(nodes[i].clone(), false); }
