...
use c20_distributed::{
    ...
    Governance, Principal, Resource, Action, AclRule,
    TokenBucket, CircuitConfig, CircuitBreaker,
};
...
fn main() {
    ...
    let sd_cfg = ServiceDiscoveryConfig {
        strategy: DiscoveryStrategy::Config { config_path: "services.json".to_string(), reload_interval: Duration::from_millis(200) },
    ...
    let mut instances = sd.discover_services(service_name).expect("discover services");
    ...
    gov0.acl.replace_rules(vec![
        AclRule { principal: Principal::Service("client".to_string()), resource: Resource("user-service".to_string()), action: Action::Read, allow: true }
    ]);
    gov0.limiters.insert("client".to_string(), TokenBucket::new(50, 50));
    gov0.breakers.insert("user-service".to_string(), CircuitBreaker::new(CircuitConfig { error_threshold: 5, open_ms: 1000 }));
    ...
    cfg_mgr.subscribe(move |snap| {
        let strategy_key = snap.values.get("lb.strategy").and_then(|v| match v { c20_distributed::ConfigValue::String(s) => Some(s.as_str()), _ => None });
        ...
        if let Ok(mut u) = lb_updater_clone.lock() {
            ...
            if let Ok(new_instances) = u.sd.discover_services("user-service") {
                ...
                println!("[config] lb.strategy = {:?}, instances = {}", new_strategy, new_instances.len());
            }
        }
        ...
        if let Ok(mut ch) = chaos_clone.lock() {
            ...
            let part_enabled = snap.values.get("chaos.partition_enabled").and_then(as_bool).unwrap_or(false);
            let peers = snap.values.get("chaos.partition_peers").and_then(as_string_vec).unwrap_or_default();
            let cfg = ChaosConfig { latency_ms: latency, jitter_ms: jitter, drop_rate, partition_enabled: part_enabled, partition_peers: peers.into_iter().collect() };
            ch.update(cfg);
        }

        if let Ok(mut g) = gov_clone.lock() {
            // 限流设置
            let cap = snap.values.get("rl.client.capacity").and_then(as_u64).unwrap_or(50);
            let refill = snap.values.get("rl.client.refill_per_sec").and_then(as_u64).unwrap_or(50);
            g.limiters.insert("client".to_string(), TokenBucket::new(cap, refill));

            // 熔断设置
            let th = snap.values.get("cb.user_service.error_threshold").and_then(as_u64).unwrap_or(5) as u32;
            let open_ms = snap.values.get("cb.user_service.open_ms").and_then(as_u64).unwrap_or(1000);
            g.breakers.insert("user-service".to_string(), CircuitBreaker::new(CircuitConfig { error_threshold: th, open_ms }));
        }
    });
    ...
    for tick in 0..5 {
        ...
        instances = lb_updater.lock().unwrap().sd.discover_services(service_name).unwrap();
        lb.update_servers(instances.clone());

        // 从 LB 选择实例并“发起请求”
        for req in 0..5 {
            // 限流
            if let Ok(mut g) = gov.lock() { if let Some(limiter) = g.limiters.get_mut("client") { if !limiter.allow() { println!("rate-limited: client"); continue; } } }

            // ACL
            let allowed = gov.lock().ok().map(|g| g.acl.is_allowed(&Principal::Service("client".to_string()), &Resource(service_name.to_string()), &Action::Read)).unwrap_or(true);
            if !allowed { println!("acl-deny: client -> {}", service_name); continue; }

            // 熔断器开关
            if let Ok(mut g) = gov.lock() { if let Some(cb) = g.breakers.get_mut(service_name) { if !cb.allow_request() { println!("circuit-open: skip"); continue; } } }

            let chosen = lb.select_server(None).expect("select server");
            // 混沌：延迟/丢包/分区
            if let Ok(ch) = chaos.lock() { ch.inject_latency(); }
            let dropped = chaos.lock().ok().map(|c| c.should_drop()).unwrap_or(false);
            let partitioned = chaos.lock().ok().map(|c| c.is_partitioned_with(&chosen.name)).unwrap_or(false);
            let ok = !(dropped || partitioned);
            println!("tick={tick} req={req} -> {} {} ok={ok}", chosen.name, chosen.address);
            if let Ok(mut g) = gov.lock() { if let Some(cb) = g.breakers.get_mut(service_name) { cb.on_result(ok); } }
        }
        ...
        if tick == 3 {
            ...
            if let Ok(mut ch) = chaos.lock() {
                ch.update(ChaosConfig { latency_ms: 30, drop_rate: 0.2, jitter_ms: 10, partition_enabled: true, partition_peers: ["user-service".to_string()].into_iter().collect() });
            }
...
struct Updater { sd: ServiceDiscoveryManager }
...
fn as_u64(v: &c20_distributed::ConfigValue) -> Option<u64> {
    match v { c20_distributed::ConfigValue::Integer(i) => Some(*i as u64), c20_distributed::ConfigValue::Float(f) => Some(*f as u64), _ => None }
}

fn as_f64(v: &c20_distributed::ConfigValue) -> Option<f64> {
    match v { c20_distributed::ConfigValue::Float(f) => Some(*f), c20_distributed::ConfigValue::Integer(i) => Some(*i as f64), _ => None }
}

fn as_bool(v: &c20_distributed::ConfigValue) -> Option<bool> {
    match v { c20_distributed::ConfigValue::Boolean(b) => Some(*b), _ => None }
}
...
fn as_string_vec(v: &c20_distributed::ConfigValue) -> Option<Vec<String>> {
    match v {
        c20_distributed::ConfigValue::Array(arr) => {
            let mut out = Vec::new();
            for e in arr { if let c20_distributed::ConfigValue::String(s) = e { out.push(s.clone()); } }
            Some(out)
        }
        _ => None,
    }
