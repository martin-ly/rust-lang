...
impl SagaStep for Debit {
    fn execute(&mut self) -> Result<(), c20_distributed::DistributedError> { self.0.fetch_add(1, Ordering::SeqCst); Ok(()) }
    fn compensate(&mut self) -> Result<(), c20_distributed::DistributedError> { self.0.fetch_sub(1, Ordering::SeqCst); Ok(()) }
}
