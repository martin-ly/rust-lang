...
pub trait SwimTransport {
    ...
    fn ack(&self, from: &str) -> bool { self.ping(from) }
...
impl<T: SwimTransport> SwimNode<T> {
    ...
    pub fn with_params(mut self, probe_interval: Duration, suspect_timeout: Duration, fanout: usize) -> Self {
...
impl<T: SwimTransport> SwimNode<T> {
    ...
    pub fn probe(&self, peer: &str) -> SwimEvent {
        ...
        SwimEvent::new(
            peer.to_string(), 
            if ok { SwimMemberState::Alive } else { SwimMemberState::Suspect },
            incarnation
        )
    }
    ...
    pub fn probe_indirect<'a>(&self, target: &str, relays: impl IntoIterator<Item=&'a str>) -> SwimEvent {
        if self.transport.ping(target) {
            return SwimEvent::new(target.to_string(), SwimMemberState::Alive, self.get_incarnation());
        }
        ...
        SwimEvent::new(target.to_string(), SwimMemberState::Suspect, self.get_incarnation())
    }

    /// 执行完整的SWIM探测协议
    pub fn swim_probe(&mut self, target: &str, peers: &[String]) -> SwimEvent {
        // 记录探测时间
        self.last_probe_time.insert(target.to_string(), Instant::now());
        ...
        // 间接探测
        let mut available_peers: Vec<&String> = peers.iter()
    ...
    pub fn check_suspect_timeouts(&mut self) -> Vec<SwimEvent> {
        ...
        let expired_suspects: Vec<String> = self.suspect_timers.iter()
        ...
        for node_id in expired_suspects {
            ...
            events.push(SwimEvent::new(node_id.clone(), SwimMemberState::Faulty, self.get_incarnation()));
        }
    ...
    pub fn handle_swim_event(&mut self, event: &SwimEvent) -> bool {
        match event.state {
            ...
            SwimMemberState::Suspect => {
                // 记录可疑时间
                self.suspect_timers.insert(event.node_id.clone(), Instant::now());
                true
            },
...
impl MembershipView {
    ...
    pub fn update_from_event(&mut self, event: &SwimEvent) -> bool {
        let ent = self.members.entry(event.node_id.clone()).or_insert(MemberInfo {
            state: event.state,
            version: Version(0),
            incarnation: 0,
            last_seen: SystemTime::now(),
    ...
    pub fn cleanup_faulty_members(&mut self, max_age: Duration) {
        ...
        self.members.retain(|_, info| {
            if info.state == SwimMemberState::Faulty {
                now.duration_since(info.last_seen).unwrap_or(Duration::from_secs(0)) < max_age
            } else {
                true
            }
        });
    ...
    pub fn alive_count(&self) -> usize {
        self.members.values().filter(|info| info.state == SwimMemberState::Alive).count()
    }
}
