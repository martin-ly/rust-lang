...
impl<E> MinimalRaft<E> {
    ...
    pub fn set_apply_scoped<'a>(&'a mut self, f: &'a mut (dyn FnMut(&E) + Send)) -> ScopedApply<'a, E> {
        ScopedApply { raft: self, apply: f }
    }
        ...
        E: Clone,
    {
        ...
        while self.last_applied < self.commit_index {
            ...
            if let Some((_, entry)) = self.log.get(idx) {
                if let Some(ref mut cb) = apply { (cb)(entry); }
            }
            self.last_applied += 1;
        }
...
impl<E: Clone> RaftNode<E> for MinimalRaft<E> {
    ...
    fn handle_append_entries(&mut self, req: AppendEntriesReq<E>) -> Result<AppendEntriesResp, DistributedError> {
        ...
        let res = match taken.as_mut() {
            Some(cb) => self.handle_append_entries_core(req, Some(cb.as_mut() as &mut (dyn FnMut(&E) + Send))),
...
impl<'a, E> ScopedApply<'a, E> {
    pub fn inner(&mut self) -> &mut MinimalRaft<E> { self.raft }
}

impl<'a, E: Clone> RaftNode<E> for ScopedApply<'a, E> {
    fn state(&self) -> RaftState { self.raft.state() }
    fn current_term(&self) -> Term { self.raft.current_term() }
    fn handle_append_entries(&mut self, req: AppendEntriesReq<E>) -> Result<AppendEntriesResp, DistributedError> {
