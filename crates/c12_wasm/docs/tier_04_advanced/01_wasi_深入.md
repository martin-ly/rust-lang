...
- **WASI 基础**: WASI 系统接口、跨平台抽象
- **本地操作系统运行 WASM**: WasmEdge、wasmtime、wasmer 运行时对比
- **文件系统**: 读取文件、写入文件、文件系统操作
- **网络接口**: HTTP 请求、网络通信
- **时间接口**: 时间获取、时间操作

**Rust 1.92.0 新特性**:

- **改进的 WASI 支持**: 更好的 WASI 0.2 支持
- **增强的 WASM 工具链**: 更完善的 WASM 工具链
- **优化的运行时**: 更高效的 WASM 运行时
...
- WASI 深入 --[covers]--> WASI 完整内容
- WASM 应用 --[uses]--> WASI 深入

**依赖关系**:

- WASI 深入 --[depends-on]--> WASM 运行时
- WASM 开发 --[depends-on]--> WASI 深入
...
| :--- | :--- | :--- | :--- | :--- | :--- || **WasmEdge** | 高   | 中   | 中     | ✅ WASI 0.2 | ✅          |
| **wasmtime** | 高   | 高   | 中     | ✅ WASI 0.2 | ✅ 改进     |
| **wasmer**   | 中   | 中   | 高     | ✅ WASI 0.2 | ✅          |
