> **EN**: Rust 1.92.0 WASM Best Practices (c12_wasm example index)
> **Summary**: A stub page pointing to the canonical concept authority for Rust WebAssembly engineering practices. The c12_wasm crate provides runnable WASM examples.

# Rust 1.92.0 WASM 最佳实践（c12_wasm 示例索引）

> **权威来源**: WASM 内存管理、性能优化、FFI 安全、代码组织等工程实践完整解释见
> [`concept/06_ecosystem/11_domain_applications/17_webassembly_advanced.md`](../../../concept/06_ecosystem/11_domain_applications/17_webassembly_advanced.md)。

本文件原为 `c12_wasm` crate 的通用 WASM 最佳实践指南。根据 AGENTS.md §6.4 治理规则，
通用 Rust 概念解释已迁移至 `concept/06_ecosystem/11_domain_applications/17_webassembly_advanced.md`，
此处仅保留索引与 canonical 链接。

## 本 crate 相关示例

- `crates/c12_wasm/examples/`：WASM 可运行示例。
- `crates/c12_wasm/src/bin/`：WASM 演示程序。

## 快速导航

| 主题 | 权威来源 |
| :--- | :--- |
| WASM 性能优化与二进制体积 | [`concept/06_ecosystem/11_domain_applications/17_webassembly_advanced.md`](../../../concept/06_ecosystem/11_domain_applications/17_webassembly_advanced.md) |
| WASM 安全沙箱与 FFI | 同上 |
| `wasm-bindgen` 与组件模型 | 同上 |
| WebAssembly 核心概念 | [`concept/06_ecosystem/11_domain_applications/03_webassembly.md`](../../../concept/06_ecosystem/11_domain_applications/03_webassembly.md) |
| 速查练习 | [`concept/SUMMARY.md`](../../../concept/SUMMARY.md) |
