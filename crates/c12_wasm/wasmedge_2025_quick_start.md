...
kubectl apply -f deployment/k8s/wasm_deployment.yaml
...
kubectl create configmap grafana-dashboards \
  --from-file=deployment/monitoring/grafana_dashboard.json \
  -n monitoring
...
```bash
# 1. 复制 CI 配置到你的仓库
cp deployment/ci/github_actions.yml .github/workflows/wasm-ci-cd.yml
...
```bash
# 完整 K8s 资源定义
deployment/k8s/wasm_deployment.yaml
...
```bash
# GitHub Actions
deployment/ci/github_actions.yml
  ├─ 代码检查
  ├─ 构建 Wasm
  ├─ 构建 Docker 镜像
  ├─ 安全扫描
...
# 告警规则
deployment/monitoring/alerts/wasm_alerts.yml
  └─ 20+ 条生产级告警
