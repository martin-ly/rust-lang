...
use c13_microservice::{
    ...
    grpc::advanced_services::{
        AdvancedGrpcService, AuthService, BatchOperation, BatchService, CacheService,
    },
};
use tracing::info;
...
async fn demo_auth_service(auth_service: &AuthService) -> Result<()> {
    ...
    let profile = auth_service
        .get_user_profile(&validated_token.user_id)
        .await?;
    info!("用户档案: {:?}", profile);
...
async fn demo_cache_service(cache_service: &CacheService) -> Result<()> {
    ...
    cache_service
        .set(key.to_string(), value.to_vec(), 300)
        .await?;
    ...
    if let Some(cached_value) = cache_service.get(key).await? {
        info!(
            "缓存命中: {} = {}",
            key,
            String::from_utf8_lossy(&cached_value)
        );
    }
    ...
    let cache_items = vec![
        (
            "session:abc",
            "{\"user_id\":\"123\",\"expires\":\"2024-12-31\"}",
        ),
    ...
    for (key, value) in cache_items {
        cache_service
            .set(key.to_string(), value.as_bytes().to_vec(), 600)
            .await?;
        info!("缓存设置: {} = {}", key, value);
    }
