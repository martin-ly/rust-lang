...
async fn simulate_business_operations(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("开始模拟业务操作");
    ...
    for i in 0..10 {
        ...
        otel_manager.logger().info(
            &format!("处理请求: {} {}", method, path),
            Some(fields),
        );
...
async fn generate_system_reports(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("生成系统报告");
    ...
    info!("性能监控操作数: {}", system_report.performance_summary.monitored_operations);
...
async fn demonstrate_error_handling(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("演示错误处理功能");
...
async fn demonstrate_performance_monitoring(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("演示性能监控功能");
    ...
    for (operation, duration) in operations {
        ...
        otel_manager.metrics().record_labeled_timer(
            "operation_duration",
            labels,
            duration,
        );
    ...
    // 获取特定操作的性能数据
    if let Some(perf_data) = otel_manager.performance_monitor().get_operation_performance("slow_operation") {
...
async fn demonstrate_distributed_tracing(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("演示分布式追踪功能");
    
    // 创建根span
    if let Some(mut root_span) = otel_manager.tracer().start_root_span("user_registration".to_string()) {
        ...
        // 创建子span
        if let Some(mut validation_span) = otel_manager.tracer().start_span("validate_user_data".to_string()) {
            validation_span.add_attribute("validation.rules".to_string(), "email,password,username".to_string());
        ...
        // 创建另一个子span
        if let Some(mut db_span) = otel_manager.tracer().start_span("save_user_to_database".to_string()) {
...
async fn final_system_report(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("生成最终系统报告");
    ...
    println!("\n--- 性能摘要 ---");
    println!("  总操作数: {}", final_report.performance_summary.total_operations);
    println!("  总错误数: {}", final_report.performance_summary.total_errors);
    println!("  错误率: {:.2}%", final_report.performance_summary.error_rate * 100.0);
    println!("  平均响应时间: {:.2}ms", final_report.performance_summary.avg_response_time_ms);
    println!("  最慢操作: {}", final_report.performance_summary.slowest_operation);
    println!("  监控操作数: {}", final_report.performance_summary.monitored_operations);
    
    println!("\n--- 错误统计 ---");
    println!("  总错误数: {}", final_report.error_statistics.total_errors);
    println!("  已解决: {}", final_report.error_statistics.resolved_errors);
    println!("  未解决: {}", final_report.error_statistics.unresolved_errors);
    println!("  严重错误: {}", final_report.error_statistics.critical_severity);
    println!("  高级错误: {}", final_report.error_statistics.high_severity);
    println!("  中级错误: {}", final_report.error_statistics.medium_severity);
    ...
    println!("  直方图: {}", final_report.metrics_summary.total_histograms);
