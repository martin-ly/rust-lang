...
use c13_microservice::opentelemetry::{
    ...
    LocalLogConfig, LocalLogLevel, LogFormat,
...
async fn demonstrate_integrated_logging(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("=== 演示集成日志功能 ===");
...
async fn demonstrate_performance_with_local_logs(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("=== 演示性能监控与本地日志 ===");
    ...
    for i in 0..operation_count {
        ...
        if i % 100 == 0 {
            ...
            rate_fields.insert("elapsed_time_ms".to_string(), elapsed.as_millis().to_string());
    ...
    result_fields.insert("total_time_ms".to_string(), total_time.as_millis().to_string());
...
async fn demonstrate_error_tracking_with_local_logs(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("=== 演示错误追踪与本地日志 ===");
    
    // 记录不同类型的错误
    let error_scenarios = vec![
        ("validation_error", "用户邮箱格式无效", ErrorSeverity::Medium),
    ...
    stats_fields.insert("total_errors".to_string(), error_stats.total_errors.to_string());
    stats_fields.insert("critical_errors".to_string(), error_stats.critical_severity.to_string());
    stats_fields.insert("high_errors".to_string(), error_stats.high_severity.to_string());
    stats_fields.insert("medium_errors".to_string(), error_stats.medium_severity.to_string());
    stats_fields.insert("low_errors".to_string(), error_stats.low_severity.to_string());
...
async fn demonstrate_health_monitoring_with_local_logs(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("=== 演示健康监控与本地日志 ===");
    ...
    health_fields.insert("overall_health".to_string(), format!("{:?}", system_report.overall_health));
    health_fields.insert("health_checks_count".to_string(), system_report.health_checks.len().to_string());
    ...
    let mut perf_fields = HashMap::new();
    perf_fields.insert("total_operations".to_string(), perf_summary.total_operations.to_string());
    perf_fields.insert("total_errors".to_string(), perf_summary.total_errors.to_string());
    perf_fields.insert("error_rate".to_string(), format!("{:.2}%", perf_summary.error_rate * 100.0));
    perf_fields.insert("avg_response_time_ms".to_string(), format!("{:.2}", perf_summary.avg_response_time_ms));
    perf_fields.insert("slowest_operation".to_string(), perf_summary.slowest_operation.clone());
...
async fn demonstrate_distributed_tracing_with_local_logs(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("=== 演示分布式追踪与本地日志 ===");
    
    // 创建根span
    if let Some(mut root_span) = otel_manager.tracer().start_root_span("user_registration_flow".to_string()) {
...
async fn generate_comprehensive_reports(otel_manager: &OpenTelemetryManager) -> Result<(), Box<dyn std::error::Error>> {
    info!("=== 生成综合报告 ===");
    ...
    report_fields.insert("report_type".to_string(), "comprehensive_system_report".to_string());
    report_fields.insert("timestamp".to_string(), final_report.timestamp.to_string());
    report_fields.insert("overall_health".to_string(), format!("{:?}", final_report.overall_health));
    report_fields.insert("health_checks_count".to_string(), final_report.health_checks.len().to_string());
    report_fields.insert("total_operations".to_string(), final_report.performance_summary.total_operations.to_string());
    report_fields.insert("total_errors".to_string(), final_report.error_statistics.total_errors.to_string());
    ...
    if let Some(local_log_manager) = otel_manager.local_log_manager() {
        ...
        log_stats_fields.insert("cache_entries".to_string(), log_stats.cache_entries.to_string());
        log_stats_fields.insert("cache_size_bytes".to_string(), log_stats.cache_size_bytes.to_string());
        log_stats_fields.insert("current_file_size_bytes".to_string(), log_stats.current_file_size_bytes.to_string());
        log_stats_fields.insert("current_file_path".to_string(), log_stats.current_file_path.to_string_lossy().to_string());
        
        otel_manager.log_local(
            LocalLogLevel::Info,
            "本地日志统计",
            Some(log_stats_fields)
        );
