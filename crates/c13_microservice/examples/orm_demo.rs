...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // SQLx数据库连接
    let sqlx_db =
        SqlxDatabase::new("postgresql://user:password@localhost:5432/microservice".to_string());
    sqlx_db.connect().await?;

    // Diesel数据库连接
    let diesel_db =
        DieselDatabase::new("postgresql://user:password@localhost:5432/microservice".to_string());
    diesel_db.connect().await?;

    // SeaORM数据库连接
    let sea_orm_db =
        SeaOrmDatabase::new("postgresql://user:password@localhost:5432/microservice".to_string());
