...
async fn demonstrate_basic_logging() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = LocalLogConfig {
        ...
        rotation_strategy: RotationStrategy::Size { max_size_bytes: 1024 * 1024 }, // 1MB
    ...
    info!("日志统计: 缓存条目={}, 缓存大小={}字节, 当前文件大小={}字节", 
...
async fn demonstrate_size_rotation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = LocalLogConfig {
        ...
        rotation_strategy: RotationStrategy::Size { max_size_bytes: 1024 }, // 1KB，便于演示
        compression_strategy: CompressionStrategy::None,
        max_files: 3,
    ...
    for i in 0..100 {
        ...
        if i % 10 == 0 {
            let stats = manager.get_stats();
            info!("已写入{}条日志，当前文件大小: {}字节", i + 1, stats.current_file_size_bytes);
        }
...
async fn demonstrate_time_rotation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..20 {
        ...
        manager.log(LocalLogLevel::Info, &format!("时间轮转测试消息 {}", i), Some(fields));
...
async fn demonstrate_compression() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = LocalLogConfig {
        ...
        level: LocalLogLevel::Info,
        rotation_strategy: RotationStrategy::Size { max_size_bytes: 2 * 1024 }, // 2KB
    ...
    // 写入大量日志以触发轮转和压缩
    for i in 0..200 {
        let message = format!("压缩测试消息 {} - 这是一条比较长的日志消息，用于测试压缩功能的效果", i);
        manager.log(LocalLogLevel::Info, &message, None);
...
async fn demonstrate_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = LocalLogConfig {
        ...
        level: LocalLogLevel::Info,
        rotation_strategy: RotationStrategy::Size { max_size_bytes: 10 * 1024 * 1024 }, // 10MB
    ...
    for i in 0..log_count {
        ...
        manager.log(LocalLogLevel::Info, &format!("性能测试消息 {}", i), Some(fields));
    ...
    info!("性能测试完成: 写入{}条日志，耗时{:?}，平均速率: {:.2} 条/秒", 
          log_count, total_time, rate);
    
    // 获取最终统计
    manager.flush()?;
    let stats = manager.get_stats();
    info!("最终统计: 缓存条目={}, 缓存大小={}字节, 文件大小={}字节", 
          stats.cache_entries, stats.cache_size_bytes, stats.current_file_size_bytes);
...
async fn demonstrate_different_formats() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (format, format_name) in formats {
        ...
        let config = LocalLogConfig {
            ...
            level: LocalLogLevel::Info,
            rotation_strategy: RotationStrategy::Size { max_size_bytes: 1024 * 1024 },
            compression_strategy: CompressionStrategy::None,
            max_files: 3,
            cache_size_bytes: 1024 * 1024,
            async_write: true,
        ...
        manager.log(LocalLogLevel::Info, "这是一条信息日志", Some(fields.clone()));
