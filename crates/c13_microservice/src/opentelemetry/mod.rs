...
pub use observability::{
    ...
    ErrorSeverity, SystemReport, PerformanceSummary, ErrorStatistics,
    LogStatistics, MetricsSummary, TracingSummary
};
// 重新导出日志相关类型，避免命名冲突
pub use otel_logging::{
    StructuredLogger, SlogLogger, LogAggregator, LogConfig, LogEntry, LogLevel, LogLevelFilter,
    AsyncLogWriter, init_logging
};
// 重新导出本地日志相关类型
pub use local_logging::{
    LocalLogManager, LocalLogConfig, LocalLogEntry, LocalLogLevel, LogFormat,
    RotationStrategy, CompressionStrategy, LogStats
};
...
impl OpenTelemetryManager {
    ...
    pub fn new(config: OpenTelemetryConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let mut system_reporter = SystemStatusReporter::new(
    ...
    /// 启用本地日志管理器
    pub fn enable_local_logging(&mut self, config: LocalLogConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// 记录本地日志
    pub fn log_local(&self, level: LocalLogLevel, message: &str, fields: Option<HashMap<String, String>>) {
    ...
    pub fn record_http_request(&self, method: &str, path: &str, status_code: u16, duration: Duration) {
        ...
        // 记录日志
        self.logger.log_http_request(method, path, status_code, duration.as_millis() as u64);
        ...
        let level = if success { LocalLogLevel::Info } else { LocalLogLevel::Error };
        self.log_local(level, &format!("HTTP {} {} - Status: {}", method, path, status_code), Some(fields));
        
        // 记录指标
        self.metrics.increment_counter(metric_names::HTTP_REQUESTS_TOTAL, 1);
        self.metrics.record_histogram(metric_names::HTTP_REQUEST_DURATION, duration.as_secs_f64());
    ...
    /// 记录数据库查询
    pub fn record_database_query(&self, query: &str, duration: Duration, rows_affected: Option<u64>) {
        // 记录日志
        self.logger.log_database_query(query, duration.as_millis() as u64, rows_affected);
        ...
        // 记录指标
        self.metrics.increment_counter(metric_names::DATABASE_QUERIES_TOTAL, 1);
        self.metrics.record_histogram(metric_names::DATABASE_QUERY_DURATION, duration.as_secs_f64());
    ...
    /// 记录性能指标
    pub fn record_performance(&self, operation: &str, duration: Duration, additional_fields: Option<HashMap<String, String>>) {
        // 记录日志
        self.logger.log_performance(operation, duration.as_millis() as u64, additional_fields.clone());
        ...
        if self.config.tracing_enabled {
            if let Some(mut span) = self.tracer.start_span(operation.to_string()) {
                span.add_attribute("operation.duration_ms".to_string(), duration.as_millis().to_string());
    ...
    pub fn set_performance_alert_threshold(&self, operation: &str, threshold_ms: f64) {
        self.performance_monitor.set_alert_threshold(operation, threshold_ms);
    }
    
    /// 设置错误告警阈值
    pub fn set_error_alert_threshold(&self, error_type: &str, threshold: u64) {
        self.error_tracker.set_error_alert_threshold(error_type, threshold);
    }
