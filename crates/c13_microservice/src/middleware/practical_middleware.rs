...
impl RateLimitState {
    ...
    pub async fn is_allowed(&self, client_id: &str, config: &RateLimitMiddleware) -> bool {
        ...
        // 检查每分钟限制
        if !self
            .check_minute_limit(client_id, config, now, minute_ago)
            .await
        {
            return false;
        }
        ...
        // 检查每小时限制
        if !self
            .check_hour_limit(client_id, config, now, hour_ago)
            .await
        {
            return false;
        }
...
impl HealthCheckMiddleware {
    ...
    pub async fn perform_health_check(&self) -> HealthCheckResult {
        ...
        if self.check_dependencies {
            ...
            result
                .checks
                .insert("database".to_string(), "ok".to_string());
...
impl MiddlewareManager {
    ...
    pub async fn process_request(
        ...
        self.request_tracing
            .log_request_start(method, path, &request_id);
        ...
        if !self
            .rate_limit_state
            .is_allowed(client_id, &self.rate_limit)
            .await
        {
            warn!(
                request_id = %request_id,
        ...
        // 记录请求完成
        self.request_tracing
            .log_request_end(&request_id, status_code, duration);
...
mod tests {
    ...
    async fn test_middleware_manager() {
        ...
        // 测试正常请求
        let result = manager
            .process_request("GET", "/api/users", "client1")
            .await;
        match result {
            RequestResult::Success {
                request_id,
