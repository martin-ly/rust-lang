...
impl Blockchain {
    ...
    pub fn new(difficulty: usize) -> Self {
        let genesis_transactions = vec![
            Transaction::new("genesis".to_string(), "genesis".to_string(), 0)
        ];
        let genesis_block = Block::new(0, BlockHash { data: [0; 32] }, genesis_transactions, difficulty);
    ...
    pub fn add_transaction(&mut self, transaction: Transaction) -> Result<(), String> {
        ...
        if !validation.is_valid {
            return Err(format!("Transaction validation failed: {:?}", validation.errors));
        }
        ...
        if let Some(last_block) = self.chain.last() {
            for tx in &last_block.transactions {
                ...
                if tx.sender != "system" && tx.sender != "genesis" {
                    ...
                    self.balances.insert(tx.sender.clone(), current_balance.saturating_sub(tx.amount));
                }
                
                // 增加接收者余额
                let current_balance = self.balances.get(&tx.receiver).unwrap_or(&0);
                self.balances.insert(tx.receiver.clone(), current_balance.saturating_add(tx.amount));
            }
        }
    }
