...
//use c12_middlewares::config::{RedisConfig, PostgresConfig, KafkaConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "kv-redis")]
    {
        ...
        // 批量设置
        let pairs: &[(&str, &[u8])] = &[("key1", b"value1"), ("key2", b"value2"), ("key3", b"value3")];
    ...
    #[cfg(feature = "mq-kafka")]
    {
        ...
        let producer = c12_middlewares::mq::kafka_client::KafkaProducer::new_with_config(kafka_config.clone())?;
        let consumer = c12_middlewares::mq::kafka_client::KafkaConsumer::new_with_config(kafka_config, &["demo_topic"])?;
        
        producer.send("demo_topic", b"Hello Kafka with config!").await?;
...
fn main() {
    ...
    println!("请使用: cargo run --example comprehensive_demo --features kv-redis,sql-postgres,tokio");
}
