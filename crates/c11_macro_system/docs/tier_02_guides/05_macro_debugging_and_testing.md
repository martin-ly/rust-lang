# C11 Macro System - Tier 2: 宏调试与测试
...
- [C11 Macro System - Tier 2: 宏调试与测试](#c11-macro-system---tier-2-宏调试与测试)
  - [📋 目录](#-目录)
  - [📐 知识结构](#-知识结构)
    - [概念定义](#概念定义)
    - [属性特征](#属性特征)
...
**宏调试与测试指南 (Macro Debugging and Testing Guide)**:

- **定义**: 宏调试和测试的实践指南，包括调试工具、测试策略和错误处理
- **类型**: 调试和测试指南文档
- **范畴**: 宏系统、调试、测试
- **版本**: Rust 1.0+
- **相关概念**: 宏调试、宏测试、错误处理、性能测试

### 属性特征

**核心属性**:

- **调试工具**: cargo expand、println! 调试、IDE 支持
- **宏展开**: 查看展开结果、部分展开
- **错误处理**: 编译错误、自定义错误消息
- **测试策略**: 集成测试、编译失败测试、快照测试
...
- 宏调试与测试指南 --[uses]--> 多种调试和测试工具
- 宏开发 --[uses]--> 宏调试与测试

**依赖关系**:

- 宏调试与测试 --[depends-on]--> 调试工具
- 宏质量保证 --[depends-on]--> 宏调试与测试
