    ...
    pub fn explain_cxx_bridge() {
        println!(
            "cxx crate 桥接模式核心概念:\n1. 共享类型 (Shared Types): 在 #[cxx::bridge] 中定义的 \
             struct，C++ 和 Rust 都能看到完整定义\n2. 不透明类型 (Opaque Types): Rust 只能持有 \
             UniquePtr<T> 或 Pin<&mut T>，C++ 拥有完整实现\n3. 函数绑定: unsafe extern \"C++\" \
             块声明 C++ 函数，extern \"Rust\" 块暴露 Rust 函数给 C++\n4. 构建流程: build.rs 调用 \
             cxx_build::bridge 自动生成 C++ 头文件和实现\n5. 内存安全: cxx \
             编译器检查所有跨边界调用，禁止不安全的指针传递"
        );
    }
    ...
    pub fn explain_bindgen_workflow() {
        println!(
            "bindgen 工作流程:\n1. 编写 wrapper.h 包含需要绑定的 C/C++ 头文件\n2. build.rs 中调用 \
             bindgen::Builder::default().header(\"wrapper.h\").generate()\n3. 生成的 Rust \
             代码包含 extern \"C\" 函数声明和 #[repr(C)] 结构体\n4. 手动编写安全封装层（raw FFI \
...
pub fn explain_cxx_bridge() {
    #[cfg(feature = "cxx-interop")]
    println!(
        "cxx bridge active: SharedPoint size = {} bytes",
        std::mem::size_of::<cxx_bridge::SharedPoint>()
    );
    #[cfg(not(feature = "cxx-interop"))]
