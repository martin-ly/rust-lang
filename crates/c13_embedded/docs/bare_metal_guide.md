> **EN**: Bare-Metal Embedded Rust Learning Guide (c13_embedded example index)
> **Summary**: A stub page pointing to the canonical concept authority for Rust embedded systems. The c13_embedded crate provides no_std, MMIO, and interrupt handling examples.

# Bare-Metal 嵌入式 Rust 学习指南（c13_embedded 示例索引）
...
本文件原为 `c13_embedded` crate 的通用嵌入式系统概念教程。根据 AGENTS.md §6.4 治理规则，
通用 Rust 概念解释已迁移至 `concept/06_ecosystem/05_systems_and_embedded/`，此处仅保留索引与
canonical 链接。

## 本 crate 相关示例

- `crates/c13_embedded/examples/`：裸机与嵌入式抽象可运行示例。
- `crates/c13_embedded/src/bin/`：嵌入式驱动与中断处理演示。
