...
impl StateManager {
    ...
    pub fn create_instance_state(&self, instance_id: String, initial_state: String) -> Result<(), StateError> {
    ...
    fn validate_state_definition(&self, definition: &StateDefinition) -> Result<(), StateError> {
        ...
        if definition.name.is_empty() {
            return Err(StateError::InvalidStateName("State name cannot be empty".to_string()));
        }
        
        // 检查状态名称格式 / Check state name format
        if !crate::types::utils::validate_state_name(&definition.name) {
            return Err(StateError::InvalidStateName(format!("Invalid state name: {}", definition.name)));
        }
        
        // 检查超时时间 / Check timeout duration
        if let Some(timeout) = definition.timeout {
            if timeout.is_zero() {
                return Err(StateError::InvalidTimeout("Timeout cannot be zero".to_string()));
            }
        }
        ...
        if let Some(retry_strategy) = &definition.retry_strategy {
            if retry_strategy.max_retries == 0 {
                return Err(StateError::InvalidRetryStrategy("Max retries cannot be zero".to_string()));
            }
            if retry_strategy.retry_delay.is_zero() {
                return Err(StateError::InvalidRetryStrategy("Retry delay cannot be zero".to_string()));
            }
        }
    ...
    fn execute_entry_actions(&self, state_name: &str, instance_id: &str) -> Result<(), StateError> {
        ...
        for action in &state_def.entry_actions {
            ...
            println!("执行进入动作 / Executing entry action: {} for instance: {}", action, instance_id);
        }
    ...
    fn execute_exit_actions(&self, state_name: &str, instance_id: &str) -> Result<(), StateError> {
        ...
        for action in &state_def.exit_actions {
            // 这里可以实现具体的动作执行逻辑
            // Here you can implement specific action execution logic
            println!("执行退出动作 / Executing exit action: {} for instance: {}", action, instance_id);
        }
...
impl StateMachineImpl {
    ...
    pub async fn handle_event(&self, instance_id: &str, event: StateEvent) -> Result<(), StateError> {
        match event {
            StateEvent::Transition { to_state, data, reason } => {
                self.transition_state(instance_id, to_state, data, reason).await
            }
            StateEvent::Lock => {
                self.state_manager.lock_instance_state(instance_id)
            }
            StateEvent::Unlock => {
                self.state_manager.unlock_instance_state(instance_id)
            }
            StateEvent::Timeout => {
                self.handle_timeout(instance_id).await
            }
        }
    }
    ...
    async fn transition_state(
        ...
        // 执行状态转换 / Execute state transition
        self.state_manager.transition_state(instance_id, to_state, data, reason)
    }
