...
impl PluginManager {
    ...
    pub fn register_plugin(&mut self, plugin: Box<dyn WorkflowMiddleware>, info: PluginInfo) -> Result<(), String> {
        let plugin_id = info.id.clone();
        
        if self.plugins.contains_key(&plugin_id) {
            return Err(format!("插件 {} 已存在 / Plugin {} already exists", plugin_id, plugin_id));
        }
        ...
        tracing::info!("插件已注册 / Plugin registered: {} v{}", info.name, info.version);
        Ok(())
    }
    ...
    pub fn activate_plugin(&mut self, plugin_id: &str) -> Result<(), String> {
        if let Some(state) = self.plugin_states.get_mut(plugin_id) {
            match *state {
                ...
                _ => Err(format!("插件 {} 无法激活，当前状态: {:?} / Plugin {} cannot be activated, current state: {:?}", 
                    plugin_id, state, plugin_id, state)),
            }
        } else {
            Err(format!("插件 {} 不存在 / Plugin {} does not exist", plugin_id, plugin_id))
        }
    }
        ...
        if let Some(state) = self.plugin_states.get_mut(plugin_id) {
            match *state {
                ...
                PluginState::Inactive => {
                    tracing::warn!("插件已处于停用状态 / Plugin already inactive: {}", plugin_id);
                    Ok(())
                }
                _ => Err(format!("插件 {} 无法停用，当前状态: {:?} / Plugin {} cannot be deactivated, current state: {:?}", 
                    plugin_id, state, plugin_id, state)),
            }
        } else {
            Err(format!("插件 {} 不存在 / Plugin {} does not exist", plugin_id, plugin_id))
    ...
    pub fn unload_plugin(&mut self, plugin_id: &str) -> Result<(), String> {
        if let Some(state) = self.plugin_states.get_mut(plugin_id) {
            match *state {
                ...
                PluginState::Active => {
                    Err(format!("插件 {} 正在使用中，无法卸载 / Plugin {} is in use, cannot unload", plugin_id, plugin_id))
                }
                _ => Err(format!("插件 {} 无法卸载，当前状态: {:?} / Plugin {} cannot be unloaded, current state: {:?}", 
                    plugin_id, state, plugin_id, state)),
            }
        } else {
            Err(format!("插件 {} 不存在 / Plugin {} does not exist", plugin_id, plugin_id))
...
impl WorkflowMiddleware for PluginMiddlewareWrapper {
    ...
    async fn before_request(&self, context: &mut MiddlewareContext) -> Result<(), String> {
        if !self.is_available() {
            return Err(format!("插件 {} 不可用 / Plugin {} is not available", self.plugin_id, self.plugin_id));
        }
        
        tracing::debug!("执行插件中间件 / Executing plugin middleware: {}", self.plugin_id);
...
impl PluginLoader {
    ...
    pub async fn load_plugin_from_config(&mut self, config: PluginConfig) -> Result<(), String> {
        tracing::info!("从配置加载插件 / Loading plugin from configuration: {}", config.name);
...
impl WorkflowMiddleware for MockPlugin {
    ...
    async fn after_request(&self, _context: &mut MiddlewareContext) -> Result<(), String> {
        tracing::debug!("模拟插件请求后处理 / Mock plugin after request processing: {}", self.id);
        Ok(())
    }
    
    async fn handle_error(&self, _context: &mut MiddlewareContext, error: &str) -> Result<(), String> {
        tracing::error!("模拟插件错误处理 / Mock plugin error handling: {} - {}", self.id, error);
        Ok(())
    }
}
