...
impl WorkflowMiddlewareChain {
    ...
    pub async fn execute(&mut self) -> Result<MiddlewareContext, String> {
        ...
        for middleware in &self.middlewares {
            middleware.before_request(&mut self.context).await
                .map_err(|e| format!("Middleware {} before_request failed: {}", middleware.name(), e))?;
        }
        
        // 执行请求后中间件 / Execute after request middlewares
        for middleware in &self.middlewares {
            middleware.after_request(&mut self.context).await
                .map_err(|e| format!("Middleware {} after_request failed: {}", middleware.name(), e))?;
        }
    ...
    pub async fn handle_error(&mut self, error: &str) -> Result<(), String> {
        for middleware in &self.middlewares {
            middleware.handle_error(&mut self.context, error).await
                .map_err(|e| format!("Middleware {} handle_error failed: {}", middleware.name(), e))?;
        }
        Ok(())
    }
}
...
impl WorkflowMiddlewareManager {
    ...
    pub async fn register_middleware(&self, middleware: Box<dyn WorkflowMiddleware>) -> Result<(), String> {
        let name = middleware.name().to_string();
        let mut middlewares = self.middlewares.write().await;
        
        if middlewares.contains_key(&name) {
            return Err(format!("中间件 {} 已存在 / Middleware {} already exists", name, name));
        }
    ...
    /// 创建中间件链 / Create middleware chain
    pub async fn create_chain(&self, context: MiddlewareContext) -> Result<WorkflowMiddlewareChain, String> {
    ...
    pub async fn list_middlewares(&self) -> Vec<MiddlewareInfo> {
        let middlewares = self.middlewares.read().await;
        middlewares.values().map(|m| MiddlewareInfo {
...
pub enum MiddlewareEvent {
    ...
    MiddlewareExecutionCompleted { name: String, request_id: String, duration: u64 },
    /// 中间件执行错误 / Middleware execution error
    MiddlewareExecutionError { name: String, request_id: String, error: String },
}
        ...
        while let Some(event) = self.event_receiver.recv().await {
            match event {
                MiddlewareEvent::MiddlewareRegistered { name, version } => {
                    tracing::info!("中间件已注册 / Middleware registered: {} v{}", name, version);
                }
                MiddlewareEvent::MiddlewareExecutionStarted { name, request_id } => {
                    tracing::debug!("中间件执行开始 / Middleware execution started: {} for request {}", name, request_id);
                }
                MiddlewareEvent::MiddlewareExecutionCompleted { name, request_id, duration } => {
                    tracing::debug!("中间件执行完成 / Middleware execution completed: {} for request {} in {}ms", name, request_id, duration);
                }
                MiddlewareEvent::MiddlewareExecutionError { name, request_id, error } => {
                    tracing::error!("中间件执行错误 / Middleware execution error: {} for request {}: {}", name, request_id, error);
                }
            }
        }
        Ok(())
...
mod tests {
    ...
    async fn test_middleware_context() {
        ...
        assert_eq!(context.get_header("Content-Type"), Some(&"application/json".to_string()));
