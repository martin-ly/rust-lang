    ...
    pub fn validate(&self) -> Result<(), WorkflowValidationError> {
        ...
        for final_state in &self.final_states {
            if !self.states.contains(final_state) {
                return Err(WorkflowValidationError::InvalidFinalState(final_state.clone()));
            }
        }
        ...
        for transition in &self.transitions {
            if !self.states.contains(&transition.from_state) {
                return Err(WorkflowValidationError::InvalidTransitionFrom(transition.from_state.clone()));
            }
            if !self.states.contains(&transition.to_state) {
                return Err(WorkflowValidationError::InvalidTransitionTo(transition.to_state.clone()));
            }
        }
...
impl WorkflowInstance {
    ...
    pub fn mark_error(&mut self, error_message: String) {
        ...
        self.data.add_metadata("error".to_string(), serde_json::Value::String(error_message));
        self.updated_at = Instant::now();
    }
}
