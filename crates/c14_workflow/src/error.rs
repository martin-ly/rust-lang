...
impl RetryStrategy {
    ...
    pub fn calculate_next_delay(&self) -> Duration {
        if self.current_retry == 0 {
            ...
            let delay = self.initial_delay.mul_f64(self.backoff_factor.powi(self.current_retry as i32));
...
impl ErrorAnalyzer {
    ...
    pub fn analyze_patterns(&mut self) {
        ...
        for error in &self.error_history {
            ...
            pattern_groups.entry(error_type).or_insert_with(Vec::new).push(error);
        }
    ...
    fn create_error_pattern(&self, error_type: &str, errors: Vec<&EnhancedError>) -> ErrorPattern {
        ...
        let first_occurrence = errors.iter().map(|e| e.context.timestamp).min().unwrap_or(Instant::now());
        let last_occurrence = errors.iter().map(|e| e.context.timestamp).max().unwrap_or(Instant::now());
    ...
    fn calculate_average_severity(&self, errors: &[&EnhancedError]) -> ErrorSeverity {
        let severity_scores: Vec<u8> = errors.iter()
    ...
    fn cleanup_old_history(&mut self) {
        ...
        self.error_history.retain(|error| error.context.timestamp > cutoff_time);
    }
...
pub mod utils {
    ...
    pub fn create_standard_context(location: &str, additional_data: Option<HashMap<String, serde_json::Value>>) -> ErrorContext {
