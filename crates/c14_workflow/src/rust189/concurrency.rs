...
impl ConcurrentWorkflowExecutor {
    ...
    pub async fn register_workflow(&self, definition: WorkflowDefinition) -> Result<(), ConcurrentExecutorError> {
    ...
    pub async fn execute_workflow(&self, workflow_id: String) -> Result<WorkflowResult, ConcurrentExecutorError> {
    ...
    async fn execute_workflow_internal(&self, workflow_id: String) -> Result<WorkflowResult, ConcurrentExecutorError> {
        let definition = {
        ...
        for step in sorted_steps {
            ...
            if let Some(dependency) = definition.dependencies.iter().find(|d| d.step_id == step.id) {
    ...
    /// 执行步骤 / Execute step
    async fn execute_step(&self, step: &WorkflowStep) -> Result<StepResult, ConcurrentExecutorError> {
        let start_time = std::time::Instant::now();
        
        // 模拟步骤执行 / Simulate step execution
        let result = if let Some(timeout) = step.timeout {
            tokio::time::timeout(timeout, self.simulate_step_execution(&step.function)).await
    ...
    fn topological_sort(
        ...
        fn visit(
            ...
            if temp_visited.contains(step_id) {
                return Err(ConcurrentExecutorError::CircularDependency(step_id.to_string()));
            }
        ...
        for step in steps {
            if !visited.contains(&step.id) {
                visit(&step.id, steps, dependencies, &mut sorted, &mut visited, &mut temp_visited)?;
            }
        }
...
pub mod utils {
        ...
        for task in tasks {
            ...
            let handle = tokio::spawn(async move {
                let permit = semaphore_clone.acquire().await
                    .map_err(|_| ConcurrentExecutorError::SemaphoreAcquisitionFailed).unwrap();
                let result = task.await;
                drop(permit);
                result
            });
    ...
    where
        T: Send + 'static,
        F: Fn(&T) -> Pin<Box<dyn Future<Output = bool> + Send + 'static>> + Send + Sync + Clone + 'static,
    {
...
mod tests {
    ...
    async fn test_concurrent_workflow_executor() {
        ...
        let result = executor.execute_workflow("test_workflow".to_string()).await.unwrap();
    ...
    async fn test_concurrent_utils() {
        ...
        let results = utils::map_concurrently(
            items,
            |item| Box::pin(async move { item * 2 }),
            3,
        ).await.unwrap();
