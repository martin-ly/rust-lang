...
impl ImprovedError {
    ...
    pub fn from_source(kind: ErrorKind, message: String, source: Arc<dyn Error + Send + Sync>) -> Self {
        Self {
            kind,
            message,
            source: Some(source),
...
impl ErrorRecoverer {
    ...
    pub async fn recover(&self, error: &ImprovedError) -> Result<RecoveryResult, RecoveryError> {
        if let Some(strategy) = self.recovery_strategies.get(&error.kind) {
            match strategy {
                RecoveryStrategy::Retry { max_attempts, delay, backoff_multiplier } => {
                    self.retry_recovery(error, *max_attempts, *delay, *backoff_multiplier).await
                }
                RecoveryStrategy::Fallback { fallback_function, parameters } => {
                    self.fallback_recovery(error, fallback_function, parameters).await
                }
                RecoveryStrategy::Ignore => {
                    Ok(RecoveryResult::Ignored)
                }
                RecoveryStrategy::Custom { strategy_name, parameters } => {
    ...
    async fn retry_recovery(
        ...
        Err(RecoveryError::RecoveryFailed(format!("Retry failed after {} attempts", max_attempts)))
    }
    ...
    async fn fallback_recovery(
        ...
        if self.simulate_fallback_operation(fallback_function, parameters).await {
            Ok(RecoveryResult::Recovered {
                strategy: "fallback".to_string(),
                attempts: 1,
            })
        } else {
            Err(RecoveryError::RecoveryFailed("Fallback operation failed".to_string()))
        }
    }
    ...
    async fn custom_recovery(
        ...
        if self.simulate_custom_recovery(strategy_name, parameters).await {
            Ok(RecoveryResult::Recovered {
                strategy: strategy_name.to_string(),
                attempts: 1,
            })
        } else {
            Err(RecoveryError::RecoveryFailed("Custom recovery failed".to_string()))
        }
    }
    ...
    /// 模拟回退操作 / Simulate fallback operation
    async fn simulate_fallback_operation(&self, _function: &str, _parameters: &HashMap<String, String>) -> bool {
        // 模拟回退成功 / Simulate fallback success
        true
    }
    
    /// 模拟自定义恢复操作 / Simulate custom recovery operation
    async fn simulate_custom_recovery(&self, _strategy: &str, _parameters: &HashMap<String, String>) -> bool {
        ...
        if error.is_recoverable() {
            match self.error_recoverer.recover(&error).await {
                ...
                Err(recovery_error) => {
                    ...
                    return Err(ErrorHandlingError::RecoveryFailed(recovery_error.to_string()));
                }
            }
        }
...
impl ErrorLogger {
    ...
    fn format_error(&self, error: &ImprovedError) -> String {
        match self.log_format {
            LogFormat::Text => {
                format!("[{}] {}: {}", error.kind, error.message, error.context.len())
            }
            LogFormat::Json => {
                serde_json::json!({
                    "kind": error.kind,
            ...
            LogFormat::Structured => {
                format!("kind={:?} message={} context_count={}", error.kind, error.message, error.context.len())
            }
        }
    }
...
impl ErrorMetrics {
    ...
    pub fn record_recovery_success(&mut self, kind: &ErrorKind) {
        ...
        let rate = self.recovery_success_rates.entry(kind.clone()).or_insert(0.0);
