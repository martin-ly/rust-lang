...
use crate::rust189::async_features::{AsyncWorkflowExecutor, TaskPriority, AsyncWorkflowBuilder, TaskResult};
use serde_json::json;
...
async fn lifetime_syntax_example() -> Result<(), Box<dyn std::error::Error>> {
    tracing::info!("运行生命周期语法检查改进示例 / Running lifetime syntax check improvements example");
    ...
    tracing::info!("生命周期处理结果 / Lifetime processing result: {}", processed_data);
    
    // 测试生命周期推断 / Test lifetime inference
    let inferred_result = processor.infer_lifetime("input_data");
    tracing::info!("生命周期推断结果 / Lifetime inference result: {}", inferred_result);
...
async fn const_generics_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("小数组长度 / Small array length: {}", ConstGenericArray::<i32, 5>::len());
    tracing::info!("中数组长度 / Medium array length: {}", ConstGenericArray::<i32, 10>::len());
    tracing::info!("大数组长度 / Large array length: {}", ConstGenericArray::<i32, 20>::len());
    ...
    tracing::info!("推断数组长度 / Inferred array length: {}", ConstGenericArray::<i32, 15>::len());
    ...
    tracing::info!("工作流状态数组长度 / Workflow states array length: {}", states_slice.len());
...
async fn cross_platform_test_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Windows 平台支持状态 / Windows platform support status: {}", is_supported);
    ...
    for platform in platforms {
        ...
        tracing::info!("平台 {} 支持状态 / Platform {} support status: {}", platform, platform, supported);
        
        if !supported {
            test.skip_for_platform(platform);
            tracing::info!("已跳过平台 {} 的测试 / Skipped tests for platform {}", platform, platform);
        }
    }
...
async fn ffi_improvements_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("FFI 结构 i128 值 / FFI struct i128 value: {}", ffi_struct.i128_value);
    tracing::info!("FFI 结构 u128 值 / FFI struct u128 value: {}", ffi_struct.u128_value);
    ...
    if let Some(converted_struct) = unsafe { FFI128Bit::from_c_struct(c_struct_ptr) } {
        ...
        tracing::info!("转换后的 i128 值 / Converted i128 value: {}", converted_struct.i128_value);
        tracing::info!("转换后的 u128 值 / Converted u128 value: {}", converted_struct.u128_value);
    }
...
async fn api_stabilization_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = api.use_result_flatten("test_key".to_string(), "test_value".to_string()).await;
    match result {
        Ok(value) => {
            tracing::info!("Result::flatten 测试成功 / Result::flatten test successful: {}", value);
        }
        Err(error) => {
            tracing::error!("Result::flatten 测试失败 / Result::flatten test failed: {}", error);
        }
    }
...
pub async fn run_workflow_rust189_integration_example() -> Result<(), Box<dyn std::error::Error>> {
    tracing::info!("运行工作流 Rust 1.89 特性集成示例 / Running workflow Rust 1.89 features integration example");
    ...
    tracing::info!("生命周期感知处理结果 / Lifetime-aware processing result: {}", processed);
    
    // 测试常量泛型缓存 / Test const generic cache
    let cache_slice = features.use_const_generic_cache();
    tracing::info!("常量泛型缓存长度 / Const generic cache length: {}", cache_slice.len());
    
    // 测试跨平台支持 / Test cross-platform support
    let is_supported = features.is_cross_platform_supported();
    tracing::info!("跨平台支持状态 / Cross-platform support status: {}", is_supported);
    ...
    let api_result = features.use_stabilized_api("integration_test".to_string(), "integration_value".to_string()).await;
    match api_result {
        Ok(value) => {
            tracing::info!("稳定化 API 测试成功 / Stabilized API test successful: {}", value);
        }
        Err(error) => {
            tracing::error!("稳定化 API 测试失败 / Stabilized API test failed: {}", error);
        }
    }
...
pub async fn run_async_workflow_features_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for task in tasks {
        ...
        executor.submit_task(task_id, async move { task.execute().await }, priority).await?;
    }
    ...
    match result {
        Ok(workflow_result) => {
            tracing::info!("工作流执行成功 / Workflow execution successful: {:?}", workflow_result);
        }
        Err(e) => {
