...
async fn core_middleware_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager.register_middleware(Box::new(
        core::AuthorizationMiddleware::new()
    )).await?;
    ...
    manager.register_middleware(Box::new(
        core::MonitoringMiddleware::new()
    )).await?;
    
    manager.register_middleware(Box::new(
        core::RateLimitingMiddleware::new()
    )).await?;
    
    // 获取所有中间件信息 / Get all middleware information
    let middlewares = manager.list_middlewares().await;
    tracing::info!("注册了 {} 个核心中间件 / Registered {} core middlewares", middlewares.len(), middlewares.len());
    
    for middleware in &middlewares {
        tracing::info!("中间件: {} v{} - {} / Middleware: {} v{} - {}", 
            middleware.name, middleware.version, middleware.description,
            middleware.name, middleware.version, middleware.description);
    }
    ...
    tracing::info!("中间件链执行结果 / Middleware chain execution result: {:?}", result);
...
async fn extension_middleware_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager.register_middleware(Box::new(
        extensions::CachingMiddleware::new()
    )).await?;
    
    manager.register_middleware(Box::new(
        extensions::CompressionMiddleware::new()
    )).await?;
    
    manager.register_middleware(Box::new(
        extensions::EncryptionMiddleware::new()
    )).await?;
    
    manager.register_middleware(Box::new(
        extensions::RetryMiddleware::new()
    )).await?;
    
    manager.register_middleware(Box::new(
        extensions::TimeoutMiddleware::new()
    )).await?;
    
    // 获取所有中间件信息 / Get all middleware information
    let middlewares = manager.list_middlewares().await;
    tracing::info!("注册了 {} 个扩展中间件 / Registered {} extension middlewares", middlewares.len(), middlewares.len());
    
    for middleware in &middlewares {
        tracing::info!("扩展中间件: {} v{} - {} / Extension middleware: {} v{} - {}", 
            middleware.name, middleware.version, middleware.description,
            middleware.name, middleware.version, middleware.description);
    }
    ...
    // 执行中间件链 / Execute middleware chain
    let result = chain.execute().await?;
    tracing::info!("扩展中间件链执行结果 / Extension middleware chain execution result: {:?}", result);
...
async fn plugin_middleware_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(info) = plugin_info {
        tracing::info!("插件信息 / Plugin info: {} v{} - {} / {} v{} - {}", 
    ...
    tracing::info!("活跃插件: {:?} / Active plugins: {:?}", active_plugins, active_plugins);
    ...
    let wrapper = plugins::PluginMiddlewareWrapper::new("custom_plugin".to_string(), Box::new(custom_middleware));
    ...
    match result {
        ...
        Err(error) => {
            tracing::error!("插件中间件执行失败: {} / Plugin middleware execution failed: {}", error, error);
        }
    }
...
async fn middleware_manager_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager.register_middleware(Box::new(core::AuthorizationMiddleware::new())).await?;
    manager.register_middleware(Box::new(core::LoggingMiddleware::new())).await?;
    manager.register_middleware(Box::new(core::MonitoringMiddleware::new())).await?;
    manager.register_middleware(Box::new(core::RateLimitingMiddleware::new())).await?;
    manager.register_middleware(Box::new(extensions::CachingMiddleware::new())).await?;
    manager.register_middleware(Box::new(extensions::CompressionMiddleware::new())).await?;
    manager.register_middleware(Box::new(extensions::EncryptionMiddleware::new())).await?;
    manager.register_middleware(Box::new(extensions::RetryMiddleware::new())).await?;
    manager.register_middleware(Box::new(extensions::TimeoutMiddleware::new())).await?;
    
    // 获取所有中间件信息 / Get all middleware information
    let middlewares = manager.list_middlewares().await;
    tracing::info!("总共注册了 {} 个中间件 / Total of {} middlewares registered", middlewares.len(), middlewares.len());
    
    // 按优先级分组中间件 / Group middlewares by priority
    let mut priority_groups: HashMap<MiddlewarePriority, Vec<&MiddlewareInfo>> = HashMap::new();
    for middleware in &middlewares {
        priority_groups.entry(middleware.priority).or_insert_with(Vec::new).push(middleware);
    }
    
    for (priority, middlewares) in priority_groups {
        tracing::info!("优先级 {:?} 有 {} 个中间件 / Priority {:?} has {} middlewares", 
            priority, middlewares.len(), priority, middlewares.len());
        for middleware in middlewares {
            tracing::info!("  - {} v{} / - {} v{}", middleware.name, middleware.version, middleware.name, middleware.version);
        }
    }
    ...
    event_handler.send_event(MiddlewareEvent::MiddlewareRegistered {
        name: "test_middleware".to_string(),
        version: "1.0.0".to_string(),
    }).await?;
    
    event_handler.send_event(MiddlewareEvent::MiddlewareExecutionStarted {
        name: "test_middleware".to_string(),
        request_id: "req_6".to_string(),
    }).await?;
    
    event_handler.send_event(MiddlewareEvent::MiddlewareExecutionCompleted {
        name: "test_middleware".to_string(),
        request_id: "req_6".to_string(),
        duration: 100,
    }).await?;
...
pub async fn run_middleware_performance_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("总迭代次数: {} / Total iterations: {}", iterations, iterations);
    tracing::info!("总耗时: {}ms / Total duration: {}ms", duration.as_millis(), duration.as_millis());
    tracing::info!("平均耗时: {}μs / Average duration: {}μs", avg_duration.as_micros(), avg_duration.as_micros());
...
pub async fn run_middleware_configuration_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager.register_middleware(Box::new(auth_middleware)).await?;
    
    // 配置日志中间件 / Configure logging middleware
    let logging_middleware = core::LoggingMiddleware::new();
    manager.register_middleware(Box::new(logging_middleware)).await?;
    
    // 配置缓存中间件 / Configure caching middleware
    let caching_middleware = extensions::CachingMiddleware::new();
    manager.register_middleware(Box::new(caching_middleware)).await?;
    ...
    manager.register_middleware(Box::new(retry_middleware)).await?;
    
    // 配置超时中间件 / Configure timeout middleware
    let timeout_middleware = extensions::TimeoutMiddleware::new()
        .with_timeout(std::time::Duration::from_secs(30));
    manager.register_middleware(Box::new(timeout_middleware)).await?;
    
    // 获取配置后的中间件信息 / Get configured middleware information
    let middlewares = manager.list_middlewares().await;
    tracing::info!("配置了 {} 个中间件 / Configured {} middlewares", middlewares.len(), middlewares.len());
    
    for middleware in &middlewares {
        tracing::info!("配置的中间件: {} v{} - {} / Configured middleware: {} v{} - {}", 
            middleware.name, middleware.version, middleware.description,
            middleware.name, middleware.version, middleware.description);
    }
