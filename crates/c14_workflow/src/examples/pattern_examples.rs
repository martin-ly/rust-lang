...
async fn creational_patterns_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let creational_patterns: Vec<_> = patterns.iter()
        .filter(|p| p.category == PatternCategory::Creational)
        .collect();
    
    tracing::info!("找到 {} 个创建型模式 / Found {} creational patterns", creational_patterns.len(), creational_patterns.len());
    ...
    if let Some(pattern) = builder_pattern {
        ...
        tracing::info!("建造者模式应用结果 / Builder pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = factory_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("工厂模式应用结果 / Factory pattern application result: {:?}", result);
    }
    
    // 测试原型模式 / Test prototype pattern
    let prototype_pattern = factory.create_pattern("WorkflowPrototype", PatternCategory::Creational);
    if let Some(pattern) = prototype_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("原型模式应用结果 / Prototype pattern application result: {:?}", result);
    }
    
    // 测试单例模式 / Test singleton pattern
    let singleton_pattern = factory.create_pattern("WorkflowSingleton", PatternCategory::Creational);
    if let Some(pattern) = singleton_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("单例模式应用结果 / Singleton pattern application result: {:?}", result);
    }
...
async fn structural_patterns_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let patterns = factory.get_all_patterns();
    let structural_patterns: Vec<_> = patterns.iter()
        .filter(|p| p.category == PatternCategory::Structural)
        .collect();
    
    tracing::info!("找到 {} 个结构型模式 / Found {} structural patterns", structural_patterns.len(), structural_patterns.len());
    ...
    if let Some(pattern) = adapter_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("适配器模式应用结果 / Adapter pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = bridge_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("桥接模式应用结果 / Bridge pattern application result: {:?}", result);
    }
    
    // 测试组合模式 / Test composite pattern
    let composite_pattern = factory.create_pattern("WorkflowComposite", PatternCategory::Structural);
    if let Some(pattern) = composite_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("组合模式应用结果 / Composite pattern application result: {:?}", result);
    }
    
    // 测试装饰器模式 / Test decorator pattern
    let decorator_pattern = factory.create_pattern("WorkflowDecorator", PatternCategory::Structural);
    if let Some(pattern) = decorator_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("装饰器模式应用结果 / Decorator pattern application result: {:?}", result);
    }
...
async fn behavioral_patterns_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let patterns = factory.get_all_patterns();
    let behavioral_patterns: Vec<_> = patterns.iter()
        .filter(|p| p.category == PatternCategory::Behavioral)
        .collect();
    
    tracing::info!("找到 {} 个行为型模式 / Found {} behavioral patterns", behavioral_patterns.len(), behavioral_patterns.len());
    
    // 测试责任链模式 / Test chain of responsibility pattern
    let chain_pattern = factory.create_pattern("WorkflowChainOfResponsibility", PatternCategory::Behavioral);
    if let Some(pattern) = chain_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("责任链模式应用结果 / Chain of responsibility pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = command_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("命令模式应用结果 / Command pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = observer_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("观察者模式应用结果 / Observer pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = state_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("状态模式应用结果 / State pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = strategy_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("策略模式应用结果 / Strategy pattern application result: {:?}", result);
    }
...
async fn concurrent_patterns_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let patterns = factory.get_all_patterns();
    let concurrent_patterns: Vec<_> = patterns.iter()
        .filter(|p| p.category == PatternCategory::Concurrent)
        .collect();
    
    tracing::info!("找到 {} 个并发模式 / Found {} concurrent patterns", concurrent_patterns.len(), concurrent_patterns.len());
    ...
    if let Some(pattern) = actor_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("Actor 模式应用结果 / Actor pattern application result: {:?}", result);
    }
    
    // 测试生产者-消费者模式 / Test producer-consumer pattern
    let producer_consumer_pattern = factory.create_pattern("WorkflowProducerConsumer", PatternCategory::Concurrent);
    if let Some(pattern) = producer_consumer_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("生产者-消费者模式应用结果 / Producer-consumer pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = pipeline_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("管道模式应用结果 / Pipeline pattern application result: {:?}", result);
    }
    ...
    if let Some(pattern) = reactor_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("反应器模式应用结果 / Reactor pattern application result: {:?}", result);
    }
    
    // 测试线程池模式 / Test thread pool pattern
    let thread_pool_pattern = factory.create_pattern("WorkflowThreadPool", PatternCategory::Concurrent);
    if let Some(pattern) = thread_pool_pattern {
        ...
        let result = pattern.apply(&context)?;
        tracing::info!("线程池模式应用结果 / Thread pool pattern application result: {:?}", result);
    }
...
async fn pattern_combination_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (pattern_name, category) in patterns_to_apply {
        if let Some(pattern) = factory.create_pattern(pattern_name, category) {
            let result = pattern.apply(&context)?;
            tracing::info!("模式 {} 应用成功 / Pattern {} applied successfully: {:?}", pattern_name, pattern_name, result);
        }
    }
    ...
    tracing::info!("总共有 {} 个可用模式 / Total of {} available patterns", all_patterns.len(), all_patterns.len());
    ...
    for (category, count) in category_counts {
        tracing::info!("类别 {:?} 有 {} 个模式 / Category {:?} has {} patterns", category, count, category, count);
    }
...
pub async fn run_pattern_performance_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for pattern_info in &patterns {
        ...
        if let Some(pattern) = factory.create_pattern(&pattern_info.name, pattern_info.category) {
            ...
            match result {
                Ok(_) => {
                    tracing::info!("模式 {} 性能测试完成，耗时: {}ms / Pattern {} performance test completed, duration: {}ms", 
                        pattern_info.name, duration.as_millis(), pattern_info.name, duration.as_millis());
                }
                Err(error) => {
                    tracing::error!("模式 {} 性能测试失败: {} / Pattern {} performance test failed: {}", 
                        pattern_info.name, error, pattern_info.name, error);
                }
            }
        }
    }
