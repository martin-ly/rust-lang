...
async fn simple_workflow_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    engine.register_workflow("simple_workflow".to_string(), workflow_def).await?;
    ...
    let instance_id = engine.start_workflow("simple_workflow", initial_data).await?;
    tracing::info!("工作流实例已启动 / Workflow instance started: {}", instance_id);
...
async fn workflow_state_management_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let engine = WorkflowEngine::new();
    engine.register_workflow("state_management_workflow".to_string(), workflow_def).await?;
    ...
    let instance_id = engine.start_workflow("state_management_workflow", test_data).await?;
    
    // 模拟状态转换 / Simulate state transitions
    let states = vec!["initial", "validating", "processing", "validating_result", "completed"];
...
async fn workflow_error_handling_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let engine = WorkflowEngine::new();
    engine.register_workflow("error_handling_workflow".to_string(), workflow_def).await?;
    ...
    // 启动工作流 / Start workflow
    let instance_id = engine.start_workflow("error_handling_workflow", error_data).await?;
    
    // 模拟错误处理流程 / Simulate error handling flow
    let error_states = vec!["start", "processing", "error_handling", "retry", "processing", "completed"];
...
async fn workflow_data_passing_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let engine = WorkflowEngine::new();
    engine.register_workflow("data_passing_workflow".to_string(), workflow_def).await?;
    ...
    // 启动工作流 / Start workflow
    let instance_id = engine.start_workflow("data_passing_workflow", complex_data).await?;
