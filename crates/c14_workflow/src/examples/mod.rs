...
impl ExampleResult {
    ...
    pub fn success(example_name: String, execution_time: std::time::Duration, data: serde_json::Value) -> Self {
        Self {
            example_name,
            success: true,
            execution_time,
    ...
    pub fn failure(example_name: String, execution_time: std::time::Duration, message: String) -> Self {
        Self {
            example_name,
            success: false,
            execution_time,
...
impl ExampleRunner {
        ...
        Fut: std::future::Future<Output = Result<serde_json::Value, String>>,
    {
        ...
        match example_fn(&self.config).await {
            Ok(data) => {
                ...
                tracing::info!("示例 {} 执行成功 / Example {} executed successfully", name, name);
                Ok(())
            }
            Err(error) => {
                let execution_time = start_time.elapsed();
                ...
                self.results.push(result);
                tracing::error!("示例 {} 执行失败 / Example {} execution failed: {}", name, name, error);
                Err(error)
            }
        }
    }
    ...
    pub fn generate_report(&self) -> String {
        ...
        let total_time: std::time::Duration = self.results.iter()
        ...
        format!(
            ...
            if total > 0 { total_time.as_millis() / total as u128 } else { 0 }
        )
    }
}
...
mod tests {
    ...
    async fn test_example_runner() {
        ...
        let result = runner.run_example("test_example".to_string(), |_config| async {
    ...
    fn test_example_result() {
        ...
        assert_eq!(result.message, "示例执行成功 / Example executed successfully");
    }
