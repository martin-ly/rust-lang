...
impl WorkflowBuilder {
    ...
    pub fn build(self) -> Result<WorkflowDefinition, String> {
        if self.states.is_empty() {
            return Err("工作流必须至少有一个状态 / Workflow must have at least one state".to_string());
        }
        ...
        if !self.states.contains(&initial_state) {
            return Err("初始状态必须在状态列表中 / Initial state must be in states list".to_string());
        }
        
        for final_state in &self.final_states {
            if !self.states.contains(final_state) {
                return Err(format!("最终状态 {} 必须在状态列表中 / Final state {} must be in states list", final_state, final_state));
            }
        }
...
impl WorkflowPattern for WorkflowBuilderPattern {
    ...
    fn apply(&self, context: &WorkflowContext) -> Result<WorkflowResult, String> {
        ...
        let builder_name = context.data.get("builder_name")
            .and_then(|v| v.as_str())
            .unwrap_or("default_workflow");
        
        let states = context.data.get("states")
        ...
        let workflow_def = WorkflowBuilder::new(builder_name.to_string())
            .description("使用建造者模式构建的工作流 / Workflow built using builder pattern".to_string())
            .add_states(states)
        ...
        let result = WorkflowResult {
            ...
            message: "工作流建造者模式应用成功 / Workflow builder pattern applied successfully".to_string(),
        };
