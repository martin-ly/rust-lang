...
impl WorkflowSingletonManager {
    ...
    pub fn get_or_create_workflow(&self, name: String, factory: impl FnOnce() -> WorkflowDefinition) -> Result<WorkflowDefinition, String> {
        let mut instances = self.instances.lock().map_err(|_| "锁获取失败 / Lock acquisition failed".to_string())?;
        
        if let Some(instance) = instances.get(&name) {
            tracing::info!("返回现有工作流实例 / Returning existing workflow instance: {}", name);
            Ok(instance.clone())
        } else {
            tracing::info!("创建新的工作流实例 / Creating new workflow instance: {}", name);
...
impl WorkflowPattern for WorkflowSingletonPattern {
    ...
    fn apply(&self, context: &WorkflowContext) -> Result<WorkflowResult, String> {
        ...
        let workflow_name = context.data.get("workflow_name")
            .and_then(|v| v.as_str())
            .unwrap_or("default_singleton_workflow");
        
        let workflow_type = context.data.get("workflow_type")
            .and_then(|v| v.as_str())
            .unwrap_or("simple");
        ...
        let workflow_def = singleton_manager.get_or_create_workflow(
            ...
            || {
                ...
                match workflow_type {
                    ...
                    "complex" => WorkflowDefinition {
                        name: workflow_name.to_string(),
                        description: Some("复杂单例工作流 / Complex singleton workflow".to_string()),
                        version: "1.0.0".to_string(),
                        states: vec![
                            "init".to_string(),
                            "process".to_string(),
                    ...
                    _ => WorkflowDefinition {
                        name: workflow_name.to_string(),
                        description: Some("默认单例工作流 / Default singleton workflow".to_string()),
                        version: "1.0.0".to_string(),
                        states: vec![
                            "start".to_string(),
                            "end".to_string(),
...
mod tests {
    ...
    fn test_workflow_singleton_manager() {
        ...
        let result = manager.get_or_create_workflow("test_workflow".to_string(), || workflow_def.clone());
        assert!(result.is_ok());
