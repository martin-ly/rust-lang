...
impl WorkflowPrototypeRegistry {
    ...
    pub fn clone_prototype(&self, name: &str, new_name: Option<String>) -> Result<WorkflowDefinition, String> {
        ...
        if let Some(name) = new_name {
            ...
            cloned.name = format!("{}_clone_{}", prototype.name, uuid::Uuid::new_v4().to_string()[..8].to_string());
        }
        
        // 更新元数据以标识这是克隆的 / Update metadata to identify this as a clone
        cloned.metadata.insert("is_clone".to_string(), json!(true));
        cloned.metadata.insert("original_prototype".to_string(), json!(name));
        cloned.metadata.insert("cloned_at".to_string(), json!(chrono::Utc::now().to_rfc3339()));
...
impl WorkflowPattern for WorkflowPrototypePattern {
    ...
    fn apply(&self, context: &WorkflowContext) -> Result<WorkflowResult, String> {
        ...
        let prototype_name = context.data.get("prototype_name")
            .and_then(|v| v.as_str())
            .unwrap_or("simple");
        
        let new_name = context.data.get("new_name")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
    ...
    fn validate(&self, context: &WorkflowContext) -> Result<(), String> {
        ...
        if !self.registry.has_prototype(prototype_name) {
            return Err(format!("原型不存在 / Prototype not found: {}", prototype_name));
        }
