...
impl BenchmarkSuite {
    ...
    async fn run_single_benchmark(&self, benchmark: &mut PerformanceBenchmark) -> Result<BenchmarkResult, BenchmarkError> {
    ...
    async fn run_test_scenario(&self, scenario: &mut TestScenario) -> Result<ScenarioResult, BenchmarkError> {
        let start_time = Instant::now();
        
        // 根据工作负载类型执行测试 / Execute test based on workload type
        let metrics = match scenario.workload.workload_type {
            WorkloadType::Light => {
                self.run_light_workload(&scenario.workload).await?
            }
            WorkloadType::Medium => {
                self.run_medium_workload(&scenario.workload).await?
            }
            WorkloadType::Heavy => {
                self.run_heavy_workload(&scenario.workload).await?
            }
            WorkloadType::Stress => {
                self.run_stress_workload(&scenario.workload).await?
            }
            WorkloadType::Endurance => {
                self.run_endurance_workload(&scenario.workload).await?
            }
        };
    ...
    async fn run_light_workload(&self, _workload: &Workload) -> Result<PerformanceMetrics, BenchmarkError> {
    ...
    async fn run_medium_workload(&self, _workload: &Workload) -> Result<PerformanceMetrics, BenchmarkError> {
    ...
    async fn run_heavy_workload(&self, _workload: &Workload) -> Result<PerformanceMetrics, BenchmarkError> {
    ...
    async fn run_stress_workload(&self, _workload: &Workload) -> Result<PerformanceMetrics, BenchmarkError> {
    ...
    async fn run_endurance_workload(&self, _workload: &Workload) -> Result<PerformanceMetrics, BenchmarkError> {
    ...
    fn calculate_overall_metrics(&self, scenario_results: &[ScenarioResult]) -> PerformanceMetrics {
        ...
        PerformanceMetrics {
            throughput_ops_per_sec: scenario_results.iter().map(|r| r.metrics.throughput_ops_per_sec).sum::<f64>() / count,
            average_latency_ms: scenario_results.iter().map(|r| r.metrics.average_latency_ms).sum::<f64>() / count,
            p50_latency_ms: scenario_results.iter().map(|r| r.metrics.p50_latency_ms).sum::<f64>() / count,
            p95_latency_ms: scenario_results.iter().map(|r| r.metrics.p95_latency_ms).sum::<f64>() / count,
            p99_latency_ms: scenario_results.iter().map(|r| r.metrics.p99_latency_ms).sum::<f64>() / count,
            max_latency_ms: scenario_results.iter().map(|r| r.metrics.max_latency_ms).fold(0.0, f64::max),
            memory_usage_mb: scenario_results.iter().map(|r| r.metrics.memory_usage_mb).sum::<f64>() / count,
            cpu_usage_percent: scenario_results.iter().map(|r| r.metrics.cpu_usage_percent).sum::<f64>() / count,
            error_rate_percent: scenario_results.iter().map(|r| r.metrics.error_rate_percent).sum::<f64>() / count,
            availability_percent: scenario_results.iter().map(|r| r.metrics.availability_percent).sum::<f64>() / count,
        }
    }
    ...
    fn calculate_performance_score(&self, metrics: &PerformanceMetrics) -> f64 {
        ...
        throughput_score + latency_score + memory_score + cpu_score + error_score + availability_score
    }
    ...
    fn generate_recommendations(&self, report: &BenchmarkReport) -> Vec<String> {
        ...
        for result in &report.benchmark_results {
            ...
            if result.overall_metrics.error_rate_percent > 1.0 {
                recommendations.push(format!("基准测试 {} 的错误率过高，需要改进错误处理", result.name));
            }
            
            if result.overall_metrics.memory_usage_mb > 1000.0 {
                recommendations.push(format!("基准测试 {} 的内存使用过高，需要优化内存管理", result.name));
            }
        }
