...
impl InternationalPatternLibrary {
    ...
    pub fn check_pattern_compliance(&self, pattern_id: &str, implementation: &PatternImplementation) -> PatternCompliance {
        ...
        for standard in self.standards.values() {
            if let Some(pattern) = standard.patterns.iter().find(|p| p.id == pattern_id) {
                ...
                let met_requirements = total_requirements - compliance.missing_elements.len() - compliance.missing_rules.len();
                
                if total_requirements > 0 {
                    compliance.compliance_score = (met_requirements as f64 / total_requirements as f64) * 100.0;
                }
                
                // 生成建议 / Generate recommendations
                compliance.recommendations = self.generate_pattern_recommendations(pattern, &compliance);
    ...
    fn generate_pattern_recommendations(&self, pattern: &WorkflowPattern, compliance: &PatternCompliance) -> Vec<String> {
        let mut recommendations = Vec::new();
        
        if compliance.compliance_score < 80.0 {
            recommendations.push(format!("模式 {} 的合规性需要改进，当前分数: {:.1}%", pattern.name, compliance.compliance_score));
        }
        
        if !compliance.missing_elements.is_empty() {
            recommendations.push(format!("缺少 {} 个必需元素，建议添加这些元素", compliance.missing_elements.len()));
        }
        
        if !compliance.missing_rules.is_empty() {
            recommendations.push(format!("缺少 {} 个必需规则，建议实现这些规则", compliance.missing_rules.len()));
        }
...
mod tests {
    ...
    fn test_bpmn_standard() {
        ...
        assert_eq!(bpmn_standard.standard_name, "Business Process Model and Notation 2.0");
    ...
    fn test_workflow_patterns() {
        ...
        let pattern_ids: std::collections::HashSet<String> = bpmn_standard.patterns.iter()
            .map(|p| p.id.clone())
            .collect();
