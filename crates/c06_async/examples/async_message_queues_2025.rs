...
impl AsyncMessageConsumer {
                    ...
                    match self.process_message(&received_message).await {
                        ...
                        Err(e) => {
                            ...
                            if received_message.message.receive_count >= self.queue.global_config.max_receive_count {
                                info!("消息达到最大接收次数，将移动到死信队列: {}", received_message.message.id);
                            }
                        }
                    }
                }
    ...
    async fn process_message(&self, received_message: &ReceivedMessage) -> Result<()> {
        ...
        info!("处理消息: {} (内容: {})", received_message.message.id, received_message.message.body);
        Ok(())
    }
...
impl AsyncMessageRouter {
    ...
    pub async fn route_message(&self, source_queue: &str, message: Message) -> Result<Vec<String>> {
        ...
        info!("消息路由: {} -> {:?} (耗时: {:?})", message.id, routed_queues, routing_time);
        Ok(routed_queues)
    }
...
async fn main() -> Result<()> {
    ...
    for _ in 0..5 {
        if let Some(received_message) = queue.receive_message("user-events", None).await? {
            info!("接收消息: {} -> {}", received_message.message.id, received_message.message.body);
    ...
    // 发送优先级消息
    producer.send_message_with_priority("user-events", "高优先级消息".to_string(), MessagePriority::High).await?;
    
    // 发送延迟消息
    producer.send_delayed_message("user-events", "延迟消息".to_string(), Duration::from_secs(1)).await?;
