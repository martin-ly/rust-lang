...
impl ServiceDiscovery {
    ...
    pub fn register_service(&self, service_name: &str, instances: Vec<ServiceInstance>) {
        ...
        info!(
            service_name,
            "Registered service with {} instances",
        ...
        T: for<'de> Deserialize<'de>,
    {
        ...
        if let Some(circuit_breaker) = self.circuit_breakers.get(&instance.id) {
            ...
            Err(anyhow::anyhow!(
                "Circuit breaker not found for instance: {}",
                instance.id
            ))
        }
    }
                        ...
                        match health_check.await {
                            Ok((is_healthy, response_time)) => {
                                ...
                                if let Some(instance) =
                                    instances.iter_mut().find(|i| i.id == instance_id)
                                {
        ...
        for _i in 1..=10 {
            let result = retry_policy
                .execute(|| {
                    ...
                    Box::pin(async move {
                        sd.call_service::<UserData>("user-service", "/users/1")
                            .await
                    })
                })
                .await;
