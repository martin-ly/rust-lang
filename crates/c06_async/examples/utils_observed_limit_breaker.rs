...
async fn main() -> anyhow::Result<()> {
    ...
    // 2) 构造熔断器与令牌桶
    let breaker = c06_async::utils::circuit_breaker::CircuitBreaker::new_with_half_open_max(
        5,
    ...
    async fn flaky_backend(input: u32) -> Result<String, anyhow::Error> {
        // 让前 2 次调用失败，之后成功
        if input % 5 < 2 {
            Err(anyhow::anyhow!("backend temporary error"))
        } else {
            Ok(format!("ok: {}", input))
        }
    }
    ...
    for i in 0..20u32 {
        ...
        tasks.push(tokio::spawn(async move {
            ...
            let res = runner
                .run(
                    move |attempt| flaky_backend(base + attempt),
    ...
    let mut ok = 0usize;
    for t in tasks {
        if t.await??.starts_with("ok:") {
            ok += 1;
        }
    }
    println!(
        "completed_ok = {} (metrics at http://127.0.0.1:9899/metrics)",
        ok
    );
