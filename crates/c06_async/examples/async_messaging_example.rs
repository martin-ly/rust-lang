...
impl AsyncMessageSystem {
    ...
    async fn process_message(&self, message: Message) {
        ...
        // 更新统计
        {
            ...
            stats
                .messages_by_type
                .entry(message_type_name.to_string())
...
impl MessageProducer {
    ...
    pub async fn send_batch_messages(
        ...
        for (recipient, message_type, priority) in messages {
            let message_id = self
                .system
                .send_message(self.name.clone(), recipient, message_type, priority)
                .await?;
...
async fn main() -> Result<()> {
    ...
    // 启动订阅者
    let subscriber1_task = tokio::spawn(async move { subscriber1.start_listening().await });

    let subscriber2_task = tokio::spawn(async move { subscriber2.start_listening().await });
    ...
    // 发送文本消息
    let text_msg_id = producer1
        .send_text("User1", "Hello, World!", MessagePriority::Normal)
        .await?;
    println!("📝 发送文本消息，ID: {}", text_msg_id);

    // 发送图片消息
    let image_data = vec![0u8; 1024]; // 模拟图片数据
    let image_msg_id = producer1
        .send_image("User2", image_data, MessagePriority::High)
        .await?;
    println!("🖼️ 发送图片消息，ID: {}", image_msg_id);

    // 发送命令消息
    let cmd_msg_id = producer2
        .send_command("User3", "restart_service", MessagePriority::Critical)
        .await?;
    ...
    let batch_messages = vec![
        (
            "User1".to_string(),
            MessageType::Text("批量消息1".to_string()),
            MessagePriority::Normal,
        ),
        (
            "User2".to_string(),
            MessageType::Text("批量消息2".to_string()),
            MessagePriority::High,
        ),
        (
            "User3".to_string(),
            MessageType::Notification("系统通知".to_string()),
            MessagePriority::Low,
        ),
    ];
...
mod tests {
    ...
    async fn test_message_producer() {
        ...
        let message_id = producer
            .send_text("recipient", "test", MessagePriority::Normal)
            .await
            .unwrap();
        assert!(message_id > 0);
    }
