...
impl SmolAsyncDemo {
    ...
    pub async fn demonstrate_fast_startup() -> Result<()> {
        ...
        // 快速启动多个任务
        let startup_tasks = (0..20)
            .map(|i| tokio::spawn(async move { format!("startup_task_{}", i) }))
            .collect::<Vec<_>>();
...
async fn main() -> Result<()> {
    ...
    integration_framework
        .demonstrate_runtime_adapter_pattern()
        .await?;
    integration_framework
        .demonstrate_task_composition_pattern()
        .await?;
    integration_framework
        .demonstrate_async_sync_conversion()
        .await?;
    integration_framework
        .demonstrate_aggregation_composition()
        .await?;
...
mod tests {
    ...
    async fn test_tokio_async_demo() {
        assert!(
            TokioAsyncDemo::demonstrate_multithreaded_runtime()
                .await
                .is_ok()
        );
    ...
    async fn test_smol_async_demo() {
        assert!(
            SmolAsyncDemo::demonstrate_lightweight_design()
                .await
                .is_ok()
        );
    ...
    async fn test_performance_comparison() {
        assert!(
            AsyncPerformanceComparison::demonstrate_performance_characteristics()
                .await
                .is_ok()
        );
    }
