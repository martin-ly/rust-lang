...
impl SimpleAsyncSessionManager {
    ...
    pub async fn get_active_sessions(&self) -> Vec<Session> {
        ...
        sessions
            .values()
            .filter(|session| {
                session.is_active && session.last_accessed.elapsed() <= self.session_timeout
            })
            .cloned()
            .collect()
    }
...
async fn main() -> Result<()> {
    ...
    let can_read = access_control
        .check_permission("user1", &Permission::Read)
        .await;
    info!("用户1读取权限: {}", can_read);

    let can_write = access_control
        .check_permission("user2", &Permission::Write)
        .await;
    ...
    let session_id1 = session_manager
        .create_session("user1".to_string(), Some("192.168.1.1".to_string()))
        .await;
    info!("创建会话1: {}", session_id1);

    let session_id2 = session_manager
        .create_session("user1".to_string(), Some("192.168.1.2".to_string()))
        .await;
    info!("创建会话2: {}", session_id2);

    let validation_result = session_manager
        .validate_session(&session_id1, Some("192.168.1.1".to_string()))
        .await;
