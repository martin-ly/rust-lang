...
impl Clone for StreamProcessor {
    fn clone(&self) -> Self {
        Self {
            ...
            metrics: ProcessingMetrics {
                items_processed: std::sync::atomic::AtomicU64::new(
                    self.metrics
                        .items_processed
                        .load(std::sync::atomic::Ordering::Relaxed),
                ),
                items_dropped: std::sync::atomic::AtomicU64::new(
                    self.metrics
                        .items_dropped
                        .load(std::sync::atomic::Ordering::Relaxed),
                ),
                processing_errors: std::sync::atomic::AtomicU64::new(
                    self.metrics
                        .processing_errors
                        .load(std::sync::atomic::Ordering::Relaxed),
                ),
    ...
    pub async fn run_pipeline(&self) -> Result<()> {
        ...
        let processing_task = tokio::spawn(async move {
            ...
            while let Some(item) = rx.recv().await {
                ...
                if queue_size > (config.buffer_size as f64 * config.backpressure_threshold) as usize
                {
                    warn!(
                        queue_size,
                        "Backpressure detected, dropping item {}", item.id
                    );
                    continue;
                }
        ...
        let metrics_task = tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                info!(
                    processed = metrics_clone
                        .items_processed
                        .load(std::sync::atomic::Ordering::Relaxed),
                    dropped = metrics_clone
                        .items_dropped
                        .load(std::sync::atomic::Ordering::Relaxed),
                    errors = metrics_clone
                        .processing_errors
                        .load(std::sync::atomic::Ordering::Relaxed),
                    avg_time_ms = Duration::from_nanos(
                        metrics_clone
                            .avg_processing_time
                            .load(std::sync::atomic::Ordering::Relaxed)
                    )
