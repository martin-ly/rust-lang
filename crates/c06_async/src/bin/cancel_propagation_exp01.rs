...
async fn main() {
    ...
    let t1 = tokio::spawn(Abortable::new(async move {
        subtask("t1", 300).await
    }, reg1));

    let t2 = tokio::spawn(Abortable::new(async move {
    ...
    match first {
        futures::future::Either::Left((a, b)) => {
            if matches!(a, Ok(Ok(Err(()))) | Ok(Err(Aborted)) | Err(_)) { h2.abort(); }
            let _ = b.await; // 等待另一边
        }
        futures::future::Either::Right((a, b)) => {
            if matches!(a, Ok(Ok(Err(()))) | Ok(Err(Aborted)) | Err(_)) { h1.abort(); }
            let _ = b.await;
        }
    }
