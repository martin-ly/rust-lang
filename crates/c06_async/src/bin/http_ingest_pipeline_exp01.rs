...
async fn main() -> anyhow::Result<()> {
    ...
    let proc_latency = Histogram::with_opts(
        HistogramOpts::new("ingest_latency_seconds", "Per-item latency")
            .buckets(vec![0.005, 0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0]),
    )?;
    ...
    tokio::spawn({
        ...
        async move {
            ...
            let listener = TcpListener::bind(("127.0.0.1", 9899))
                .await
                .expect("bind metrics");
            loop {
    ...
    let worker = tokio::spawn(async move {
        ...
        while let Some(job) = rx.recv().await {
            ...
            let res = runner
                ...
                .await;
            handled += 1;
            if handled.is_multiple_of(50) {
                tracing::info!(handled, "progress");
            }
            let el = start.elapsed().as_secs_f64();
            proc_latency.observe(el);
            if res.is_err() {
                errors.with_label_values(&["worker"]).inc();
            } else {
                ingested.with_label_values(&["done"]).inc();
            }
