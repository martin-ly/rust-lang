//! Rust 1.97.0 stable 特性 —— 异步编程
//! Rust 1.97.0 stabilized features —— async programming
//!
//! 本文件展示与异步编程相关的 Rust 1.97.0 stable 行为变更。
...
impl Rust197AsyncFeatures {
    ...
    ///
    /// Rust 1.97.0 起 `pin!` 会阻止隐式 deref coercion，避免 `Pin<&mut T>` 在宏内
    /// 意外转换为其他类型。
    ...
    ///
    /// Rust 1.97.0 的 `must_use` lint 会将 `Result<T, E>` 视为与 `T` 等效，因此
    /// 忽略该返回值会触发警告。
...
mod tests {
    ...
    fn test_must_use_result() {
        // 在 Rust 1.97.0+ 中，以下未使用 Result 会触发 must_use 警告；
        // 显式忽略以保持代码干净。
