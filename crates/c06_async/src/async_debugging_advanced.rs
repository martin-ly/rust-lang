    ...
    pub async fn complete_flow(&self, flow_id: &str) -> Result<()> {
...

        {
            ...
            if let Some(flow) = flows.get_mut(flow_id) {
                ...
                flow.metrics.total_duration = flow
                    .start_time
...
impl AsyncDebugDecorator {
        ...
        F: std::future::Future<Output = Result<T>>,
    {
        let flow_id = if self.debug_config.enable_auto_tracking {
            self.flow_manager
                .start_flow(operation_name.clone(), context.clone())
                .await
        } else {
        ...
        F: std::future::Future<Output = Result<T>>,
    {
        ...
        self.debug_execute(format!("{} {}", method, url), context, future)
            .await
    }
        ...
        F: std::future::Future<Output = Result<T>>,
    {
        ...
        self.debug_execute(format!("DB: {} {}", operation, table), context, future)
            .await
    }
}
...
pub async fn demonstrate_advanced_async_debugging() -> Result<()> {
    ...
    let flow_id = flow_manager
        .start_flow("用户登录流程".to_string(), context)
        .await;

    // 模拟异步操作
    let step1_id = flow_manager
        .start_step(
            &flow_id,
            "验证用户凭据".to_string(),
    ...
    // 4. 演示调试装饰器
    println!("\n🔧 2. 调试装饰器演示:");
    let result = debug_decorator
        .debug_execute("模拟业务操作".to_string(), HashMap::new(), async {
    ...
    // 5. 演示网络请求调试
    println!("\n🌐 3. 网络请求调试演示:");
    let network_result = debug_decorator
    ...
    // 6. 演示数据库操作调试
    println!("\n🗄️ 4. 数据库操作调试演示:");
    let db_result = debug_decorator
        .debug_database_operation("SELECT".to_string(), "users".to_string(), async {
...
mod tests {
    ...
    async fn test_execution_flow_manager() {
        ...
        let flow_id = manager
            .start_flow("测试流程".to_string(), HashMap::new())
            .await;
    ...
    async fn test_debug_decorator() {
        ...
        let result = decorator
            .debug_execute("测试操作".to_string(), HashMap::new(), async {
                Ok("成功".to_string())
            })
            .await
            .expect("初始化不应失败");
