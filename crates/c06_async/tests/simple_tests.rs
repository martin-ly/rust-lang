...
async fn test_simple_framework() {
    ...
    let task: Box<dyn AsyncTask> =
        Box::new(ExampleTask::new("test_task", TaskPriority::Normal, 10));
...
async fn test_error_handling() {
    ...
    // 创建一个会失败的任务
    let failing_task: Box<dyn AsyncTask> =
        Box::new(FailingTask::new("failing_task", TaskPriority::Normal, 10));
