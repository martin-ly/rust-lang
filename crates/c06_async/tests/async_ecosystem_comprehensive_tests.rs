...
use c06_async::async_runtime_integration_framework_simple::{
    AggregationCompositionService, AsyncRuntimeType, AsyncSyncConversionService, AsyncTask,
    DataProcessingComponent as SimpleDataProcessingComponent, ExampleTask, RuntimeConfig,
    RuntimeMetrics, SimpleAsyncRuntimeFramework, TaskPriority,
...
async fn test_aggregation_composition_framework() {
    ...
    // 测试顺序聚合
    let sequential_results = framework
    ...
    // 测试并行聚合
    let parallel_results = framework
    ...
    // 测试管道聚合
    let pipeline_results = framework
...
async fn test_async_sync_conversion_service() {
    ...
    // 测试异步到同步转换
    let async_to_sync_result = service
        .async_to_sync(async {
    ...
    // 测试同步到异步转换
    let sync_to_async_result = service
        .sync_to_async(|| {
