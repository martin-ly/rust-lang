...
**Pin 与 Unsafe 参考 (Pin and Unsafe Reference)**:

- **定义**: Pin、Unpin 机制及异步编程中 Unsafe 用法的技术参考
- **类型**: 技术参考文档
- **范畴**: 异步编程、内存安全
- **版本**: Rust 1.33+ (Pin), Rust 1.0+ (Unsafe)
- **相关概念**: Pin、Unpin、Unsafe、自引用结构、内存安全
...
- **固定内存**: 保证值不会被移动
- **自引用安全**: 支持自引用结构
- **Future 支持**: Future 需要 Pin
- **Unsafe 使用**: 在异步编程中使用 Unsafe

**性能特征**:

- **零开销**: Pin 本身零开销
- **适用场景**: 自引用结构、Future 实现、异步编程
...
- Pin --[depends-on]--> 编译器支持
- 自引用 Future --[depends-on]--> Pin
...
- **[异步语言特性参考](01_async_language_features_reference.md)** - Future trait
- **[Future与Executor机制](../tier_02_guides/02_future_and_executor_mechanisms.md)** - 实践指南
