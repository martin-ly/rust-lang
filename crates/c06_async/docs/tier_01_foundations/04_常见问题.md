...
> **当前状态提示（2026-06-25）**：`async fn in dyn trait`（AFIDT）仍为 nightly 实验特性，暂无稳定时间表。生产环境中 `dyn Trait` 的异步方法应继续使用 `async-trait`；仅泛型/`impl Trait` 场景可使用 Rust 1.75+ 稳定的原生 AFIT。
