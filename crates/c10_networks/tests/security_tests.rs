...
fn test_sequence_number_security() {
    // 测试正常序列号
    let normal_packet =
        Packet::with_sequence(PacketType::Tcp, Bytes::copy_from_slice(b"normal"), 12345);
    assert_eq!(normal_packet.header.sequence_number, Some(12345));

    // 测试边界序列号
    let boundary_packet =
        Packet::with_sequence(PacketType::Tcp, Bytes::copy_from_slice(b"boundary"), 0);
    assert_eq!(boundary_packet.header.sequence_number, Some(0));

    // 测试最大序列号
    let max_packet =
        Packet::with_sequence(PacketType::Tcp, Bytes::copy_from_slice(b"max"), u64::MAX);
...
fn test_acme_manager_security() {
    ...
    let _acme_manager = AcmeManager::new(
...
fn test_packet_filter_security() {
    ...
    // 测试正常数据包
    let normal_packet = Packet::new(PacketType::Http, Bytes::copy_from_slice(b"GET / HTTP/1.1"));
    assert!(filter.matches(&normal_packet));

    // 测试过小的数据包
    let small_packet = Packet::new(PacketType::Http, Bytes::copy_from_slice(b"GET /"));
    ...
    // 测试错误类型的数据包
    let wrong_type_packet = Packet::new(PacketType::Raw, Bytes::copy_from_slice(b"GET / HTTP/1.1"));
...
fn test_concurrency_security() {
    ...
    let stats = Arc::new(std::sync::Mutex::new(
        c10_networks::packet::PacketStats::new(),
    ));
