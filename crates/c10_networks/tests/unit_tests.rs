...
fn test_performance_errors() {
    ...
    assert_eq!(
        format!("{}", memory_error),
        "Insufficient memory: required 1024, available 512"
    );
...
fn test_security_errors() {
    ...
    assert_eq!(
        format!("{}", malicious_error),
        "Malicious request detected: Suspicious activity"
    );
}
...
fn test_packet_creation() {
    ...
    assert_eq!(
        packet.total_size(),
        std::mem::size_of::<c10_networks::packet::PacketHeader>() + payload.len()
    );
}
...
fn test_packet_header() {
    ...
    // 测试链式操作
    let header_with_seq = header.with_sequence_number(12345).with_flags(0xFF);
...
fn test_empty_packet() {
    ...
    assert_eq!(
        empty_packet.total_size(),
        std::mem::size_of::<c10_networks::packet::PacketHeader>()
    );
}
...
fn test_packet_type_equality() {
    ...
    assert_ne!(
        PacketType::Custom("test".to_string()),
        PacketType::Custom("other".to_string())
    );
}
...
fn test_packet_type_hash() {
    ...
    assert_eq!(
        map.get(&PacketType::Custom("test".to_string())),
        Some(&"custom")
    );
}
