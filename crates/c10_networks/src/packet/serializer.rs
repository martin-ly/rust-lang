...
impl PacketSerializer {
    ...
    fn serialize_header(&mut self, header: &super::PacketHeader) -> NetworkResult<()> {
        ...
        self.buffer
            .extend_from_slice(&packet_type_raw.to_be_bytes());
        self.buffer.extend_from_slice(&header.length.to_be_bytes());
        self.buffer
            .extend_from_slice(&header.timestamp.to_be_bytes());

        // 序列化序列号
        let sequence_number = header.sequence_number.unwrap_or(0);
        self.buffer
            .extend_from_slice(&sequence_number.to_be_bytes());
    ...
    ) -> NetworkResult<Vec<SerializeResult>> {
