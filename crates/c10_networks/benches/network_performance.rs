...
fn bench_websocket_frame_creation(c: &mut Criterion) {
    ...
    group.bench_function("close_frame", |b| {
        b.iter(|| {
            let frame = WebSocketFrame::close(black_box(Some(1000)), black_box(Some("Normal closure")));
            black_box(frame)
        })
    });
...
fn bench_error_handling(c: &mut Criterion) {
    ...
    group.bench_function("error_creation", |b| {
        b.iter(|| {
            let error = c10_networks::error::NetworkError::Protocol(
...
fn bench_network_io_simulation(c: &mut Criterion) {
    ...
    c.bench_function("network_io_simulation", |b| {
        b.iter(|| {
            ...
            // 模拟数据处理
            let processed = buffer.iter().map(|&x| x.wrapping_add(1)).collect::<Vec<u8>>();
