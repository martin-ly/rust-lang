...
#[derive(Clone, Debug)]
enum CacheEntry {
    Success { ips: Vec<std::net::IpAddr>, expire_at: Instant },
    Failure { expire_at: Instant },
}
...
impl NegativeCache {
    fn new(ttl_ok: Duration, ttl_fail: Duration) -> Self {
        Self { inner: HashMap::new(), ttl_ok, ttl_fail }
    }
        ...
        if let Some(entry) = self.inner.get(key) {
            match entry {
                CacheEntry::Success { ips, expire_at } if *expire_at > Instant::now() => {
                    return Some(CacheEntry::Success { ips: ips.clone(), expire_at: *expire_at })
                }
                CacheEntry::Failure { expire_at } if *expire_at > Instant::now() => {
                    return Some(CacheEntry::Failure { expire_at: *expire_at })
                }
                _ => {}
            }
        }
    ...
    fn set_success(&mut self, key: String, ips: Vec<std::net::IpAddr>) {
        self.inner.insert(key, CacheEntry::Success { ips, expire_at: Instant::now() + self.ttl_ok });
    }

    fn set_failure(&mut self, key: String) {
        self.inner.insert(key, CacheEntry::Failure { expire_at: Instant::now() + self.ttl_fail });
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let domain = std::env::args().nth(1).unwrap_or_else(|| "nonexistent.example.invalid".to_string());
