...
async fn main() {
    ...
    let msg = UdpCustomMessage { version: 1, msg_type: 1, payload: b"hello".to_vec() };
    match udp_custom_roundtrip(&addr, &msg).await {
        Ok(resp) => println!("resp: v={} t={} payload={:?}", resp.version, resp.msg_type, String::from_utf8_lossy(&resp.payload)),
