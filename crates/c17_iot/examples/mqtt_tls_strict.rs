...
async fn main() -> anyhow::Result<()> {
    ...
    let port: u16 = env::var("MQTT_PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8883);
    ...
    client.publish(&pub_topic, QoS::AtLeastOnce, false, b"hello-tls-strict").await?;

    for _ in 0..10 { let _ = eventloop.poll().await; }
    Ok(())
}
