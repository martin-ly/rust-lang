...
use c08_algorithms::algorithms::sorting::parallel::{ParallelMergeSort, ParallelQuickSort};
...
fn test_combinations_generator() {
    ...
    let combinations: Vec<Vec<char>> =
        GeneratorAlgorithms::combinations_generator(items, 2).collect();

    let expected = vec![vec!['a', 'b'], vec!['a', 'c'], vec!['b', 'c']];
