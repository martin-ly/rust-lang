...
- [异步与同步算法的等价关系理论](#异步与同步算法的等价关系理论)
  - [目录](#目录)
  - [1. 理论基础](#1-理论基础)
    - [1.1 计算等价性](#11-计算等价性)
      - [定义1.1（图灵等价性）](#定义11图灵等价性)
    - [1.2 执行语义差异](#12-执行语义差异)
    - [1.3 形式化语义](#13-形式化语义)
      - [同步语义（Big-Step）](#同步语义big-step)
      - [异步语义（Small-Step with Continuations）](#异步语义small-step-with-continuations)
  - [2. 执行模型对比](#2-执行模型对比)
    - [2.1 调用栈 vs 状态机](#21-调用栈-vs-状态机)
      - [同步：调用栈模型](#同步调用栈模型)
      - [异步：状态机模型](#异步状态机模型)
    - [2.2 阻塞 vs 挂起](#22-阻塞-vs-挂起)
      - [同步阻塞](#同步阻塞)
      - [异步挂起](#异步挂起)
  - [3. 控制流与执行流分析](#3-控制流与执行流分析)
    - [3.1 控制流图（Control Flow Graph）](#31-控制流图control-flow-graph)
      - [定义3.1（CFG）](#定义31cfg)
      - [示例：二分查找](#示例二分查找)
    - [3.2 执行流（Execution Flow）](#32-执行流execution-flow)
      - [同步执行流](#同步执行流)
      - [异步执行流](#异步执行流)
    - [3.3 等价关系](#33-等价关系)
      - [定理3.1（执行结果等价）](#定理31执行结果等价)
      - [定理3.2（副作用顺序差异）](#定理32副作用顺序差异)
  - [4. 形式化证明](#4-形式化证明)
    - [4.1 CPS等价性](#41-cps等价性)
      - [定理4.1（CPS变换保持语义）](#定理41cps变换保持语义)
    - [4.2 异步变换正确性](#42-异步变换正确性)
      - [定理4.2（Async/Await变换）](#定理42asyncawait变换)
  - [5. CPS变换与异步转换](#5-cps变换与异步转换)
    - [5.1 CPS变换规则](#51-cps变换规则)
      - [基本规则](#基本规则)
      - [Rust示例](#rust示例)
    - [5.2 循环的CPS变换](#52-循环的cps变换)
  - [6. Rust实现对比](#6-rust实现对比)
    - [6.1 算法实现对比](#61-算法实现对比)
      - [快速排序](#快速排序)
      - [深度优先搜索](#深度优先搜索)
    - [6.2 性能对比](#62-性能对比)
  - [7. 性能与语义分析](#7-性能与语义分析)
