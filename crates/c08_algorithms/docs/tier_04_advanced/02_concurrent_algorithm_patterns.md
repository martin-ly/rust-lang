...
- [并发算法模式](#并发算法模式)
  ...
  - [7. 参考资料](#7-参考资料)
    ...
    - [Rust 并发库](#rust-并发库)
    - [形式化验证](#形式化验证)
  - [**下一步**: 参见 `03_distributed_algorithms.md`](#下一步-参见-03_分布式算法md)
...
**并发算法模式 (Concurrent Algorithm Patterns)**:
...
- **并发计算模型**: Actor 模型、CSP (Communicating Sequential Processes)、Reactor 模式
- **无锁数据结构**: 无锁队列、无锁栈、CAS 操作与 ABA 问题
- **并发算法设计**: 数据并行、任务并行、流水线并行
- **同步原语**: 屏障 (Barrier)、信号量 (Semaphore)、读写锁 (RwLock)
- **并发算法分析**: 加速比与效率、工作深度模型、负载均衡
...
- **高并发**: 支持大规模并发
- **高性能**: 高效的并发算法
- **适用场景**: 并行计算、高性能计算、分布式系统

### 关系连接

**组合关系**:

- 并发算法模式 --[covers]--> 并发算法完整内容
- 并行程序 --[uses]--> 并发算法模式

**依赖关系**:

- 并发算法模式 --[depends-on]--> 并发原语
- 并行计算 --[depends-on]--> 并发算法模式
...
| 并发模式         | 性能 | 复杂度 | 适用场景   | 类型安全 | Rust 1.92.0 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Actor 模型**   | 中   | 高     | 分布式系统 | ✅       | ✅          |
| **CSP**          | 中   | 低     | 通道通信   | ✅       | ✅          |
...
- **[Herlihy-Shavit]** Herlihy, Shavit. _The Art of Multiprocessor Programming_
- **[McCool et al.]** McCool, Reinders, Robison. _Structured Parallel Programming_
- **[Mattson et al.]** Mattson, Sanders, Massingill. _Patterns for Parallel Programming_
...
**下一步**: 参见 [`03_distributed_algorithms.md`](03_distributed_algorithms.md)
---
