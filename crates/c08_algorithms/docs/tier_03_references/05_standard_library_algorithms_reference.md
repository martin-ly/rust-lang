...
| 功能           | 标准库                         | C08 自定义           | 备注                |
| :--- | :--- | :--- | :--- |
| **稳定排序**   | `slice::sort()`                | `sort_sync(&mut data, SortingAlgo::Merge)` | Timsort vs 归并排序 |
| **不稳定排序** | `slice::sort_unstable()`       | `sort_sync(&mut data, SortingAlgo::Quick)` | 更快                |
| **部分排序**   | `slice::select_nth_unstable()` | `quickselect_kth()`  | 第k小元素           |
| **并行排序**   | ❌                             | `sort_parallel(&mut data, SortingAlgo::...)` ⭐ | Rayon并行           |
| **异步排序**   | ❌                             | `sort_async(data, SortingAlgo::...).await` ⭐    | Tokio异步           |
...
let mut arr = vec![3, 1, 4, 1, 5, 9, 2, 6];
sort_sync(&mut arr, SortingAlgo::Quick); // O(n log n)，不稳定

// C08：并行排序（大数据集）
use c08_algorithms::sorting::{sort_parallel, SortingAlgo};
let mut arr: Vec<i32> = (0..1_000_000).rev().collect();
sort_parallel(&mut arr, SortingAlgo::Quick); // 4-6x加速
```
#### 性能对比 (n = 100,000)
...
| 功能                   | 标准库                          | C08 自定义                 | 备注        |
| :--- | :--- | :--- | :--- |
| **二分搜索**           | `slice::binary_search()`        | `binary_search_sync()` 返回 `Result<Option<usize>>` | 性能相当    |
...
match binary_search_sync(&arr, &7).unwrap() {
...
let mut arr = vec![3, 1, 4, 1, 5, 9, 2, 6, 5, 3];
let kth = quickselect_kth(&mut arr, 3).copied().unwrap();
println!("第3小元素: {}", kth); // 3
```
...
let positions = kmp_search(text, "Hello");
...
// ⭐ 并行排序（大数据集）
use c08_algorithms::sorting::{sort_parallel, SortingAlgo};
sort_parallel(&mut large_array, SortingAlgo::Quick); // 4-6x加速

// ⭐ 异步算法（I/O混合）
use c08_algorithms::graph::dijkstra_async;
use std::collections::HashMap;
let graph: HashMap<String, Vec<(String, f64)>> = graph;
let (dist, _prev) = dijkstra_async(graph, start).await.unwrap();
...
let seg_tree = SegmentTree::from_slice(&arr);
```
---
...
// 混合使用示例：社交网络分析
fn analyze_social_network(graph: &HashMap<usize, Vec<(usize, f64)>>, start: usize) {
    // 1. 使用 C08 计算最短路径
    let (distances, _prev) = dijkstra_sync(graph, &start);
...
- **[01\_算法分类参考](01_algorithm_categories_reference.md)** - C08 算法完整索引
