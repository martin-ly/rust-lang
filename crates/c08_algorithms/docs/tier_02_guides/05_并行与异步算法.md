...
**并行与异步算法 (Parallel and Async Algorithms)**:

- **定义**: 使用并行和异步技术实现的算法，用于提升性能和并发能力
- **类型**: 算法实现技术
- **范畴**: 算法学、并发编程
- **版本**: Rust 1.0+ (Rayon), Rust 1.39+ (Tokio)
- **相关概念**: 数据并行、任务并行、异步算法、并发数据结构
...
- **数据并行**: Rayon 并行迭代器
- **任务并行**: fork-join 模式、分治算法
- **异步算法**: Tokio 异步运行时、异步流处理
- **并发数据结构**: 原子操作、并发 HashMap

**性能特征**:

- **加速比**: 理想情况下接近核心数
- **并发能力**: 支持高并发执行
- **适用场景**: 大数据处理、高并发算法、I/O 密集型算法
...
- 并行与异步算法 --[uses]--> 并行和异步技术
- 高性能算法 --[uses]--> 并行与异步算法

**依赖关系**:

- 并行与异步算法 --[depends-on]--> Rayon/Tokio
- 并行算法 --[depends-on]--> 多核 CPU
...
- [算法性能参考](../tier_03_references/04_算法性能参考.md) - 基准测试
