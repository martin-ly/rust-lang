...
impl ParallelSort {
    ...
    fn partition<T: Ord>(arr: &mut [T]) -> usize {
        ...
        let median = |i: usize, j: usize, k: usize, s: &mut [T]| -> usize {
            ...
            if (s[i] <= s[j] && s[j] <= s[k]) || (s[k] <= s[j] && s[j] <= s[i]) {
                j
            } else if (s[j] <= s[i] && s[i] <= s[k]) || (s[k] <= s[i] && s[i] <= s[j]) {
                i
            } else {
                k
            }
    ...
    fn find_best_split(
        ...
        for feature_index in 0..features[0].len() {
            ...
            for i in 0..values.len() - 1 {
                ...
                let gain =
                    self.calculate_information_gain(features, targets, feature_index, threshold);
...
impl SHA256 {
    ...
    fn process_block(&mut self, block: &[u8]) {
        ...
        for i in 16..64 {
            let s0 = Self::right_rotate(w[i - 15], 7)
                ^ Self::right_rotate(w[i - 15], 18)
                ^ (w[i - 15] >> 3);
            let s1 = Self::right_rotate(w[i - 2], 17)
