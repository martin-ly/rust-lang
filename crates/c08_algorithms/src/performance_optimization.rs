...
impl ConcurrentOptimizedProcessor {
    ...
    pub fn process_data_parallel(&self, data: Vec<i32>) -> Vec<i32> {
        let chunk_size = (data.len() + self.thread_pool.workers.len() - 1) / self.thread_pool.workers.len();
...
impl PerformanceProfiler {
    ...
    pub fn get_min_time(&self, name: &str) -> Option<Duration> {
        self.measurements.get(name).and_then(|durations| {
            durations.iter().min().copied()
        })
    }
    
    pub fn get_max_time(&self, name: &str) -> Option<Duration> {
        self.measurements.get(name).and_then(|durations| {
            durations.iter().max().copied()
        })
    }
...
impl BenchmarkRunner {
    ...
    pub fn compare_benchmarks<F1, F2>(&mut self, name1: &str, name2: &str, iterations: usize, test1: F1, test2: F2)
    where F1: Fn(), F2: Fn() {
...
impl MemoryProfiler {
    ...
    pub fn record_allocation(&mut self, name: &str, size: usize) {
        *self.allocations.entry(name.to_string()).or_insert(0) += size;
        ...
        let current_usage = self.allocations.get(name).unwrap_or(&0) - self.deallocations.get(name).unwrap_or(&0);
...
impl PerformanceExamples {
    ...
    pub fn memory_optimization_example() {
        ...
        println!("对象池利用率: {:.2}%", object_pool.get_utilization() * 100.0);
