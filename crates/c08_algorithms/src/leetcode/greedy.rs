...
pub fn get_all_problems() -> Vec<LeetCodeProblem> {
    vec![
        ...
        LeetCodeProblem {
            ...
            description: "给你一个整数数组 prices ，其中 prices[i] 表示某支股票第 i \
                          天的价格。在每一天，你可以决定是否购买和/或出售股票。你在任何时候 最多 \
                          只能持有 一股 股票。你也可以先购买，然后在 同一天 出售。返回 你能获得的 \
                          最大 利润 。"
                .to_string(),
        ...
        LeetCodeProblem {
            ...
            description: "在一条环路上有 n 个加油站，其中第 i 个加油站有汽油 gas[i] \
                          升。你有一辆油箱容量无限的的汽车，从第 i 个加油站开往第 i+1 \
                          个加油站需要消耗汽油 cost[i] \
                          升。你从其中的一个加油站出发，开始时油箱为空。给定两个整数数组 gas 和 \
                          cost ，如果你可以绕环路行驶一周，则返回出发时加油站的编号，否则返回 -1 \
                          。"
            .to_string(),
