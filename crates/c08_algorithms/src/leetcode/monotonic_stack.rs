...
pub fn get_all_problems() -> Vec<LeetCodeProblem> {
    vec![
        ...
        LeetCodeProblem {
            ...
            description: "给定 n 个非负整数，用来表示柱状图中各个柱子的高度。每个柱子彼此相邻，\
                          且宽度为 1 。求在该柱状图中，能够勾勒出来的矩形的最大面积。"
                .to_string(),
        ...
        LeetCodeProblem {
            ...
            description: "给定一个仅包含 0 和 1 、大小为 rows x cols 的二维二进制矩阵，找出只包含 \
                          1 的最大矩形，并返回其面积。"
                .to_string(),
        ...
        LeetCodeProblem {
            ...
            description: "nums1 中数字 x 的 下一个更大元素 是指 x 在 nums2 中对应位置 右侧 的 \
                          第一个 比 x 大的元素。给你两个 没有重复元素 的数组 nums1 和 nums2 \
                          ，下标从 0 开始计数，其中nums1 是 nums2 的子集。对于每个 0 <= i < \
                          nums1.length ，找出满足 nums1[i] == nums2[j] 的下标 j ，并且在 nums2 \
                          确定 nums2[j] 的 下一个更大元素 \
                          。如果不存在下一个更大元素，那么本次查询的答案是 -1 。返回一个长度为 \
                          nums1.length 的数组 ans 作为答案，满足 ans[i] 是如上所述的 \
                          下一个更大元素 。"
                .to_string(),
        ...
        LeetCodeProblem {
            ...
            description: "给定一个循环数组 nums （ nums[nums.length - 1] 的下一个元素是 nums[0] \
                          ），返回 nums 中每个元素的 下一个更大元素 。数字 x 的 下一个更大的元素 \
