...
pub fn quickselect_kth<T: Ord>(data: &mut [T], k: usize) -> Option<&T> {
    if k >= data.len() { return None; }
    let mut l = 0usize; let mut r = data.len() - 1;
    loop {
        let p = partition(data, l, r);
        if k == p { break; }
        if k < p { if p == 0 { break; } r = p - 1; } else { l = p + 1; }
    }
    Some(&data[k])
}
...
fn partition<T: Ord>(a: &mut [T], l: usize, r: usize) -> usize {
    let mut i = l;
    for j in l..r { if a[j] <= a[r] { a.swap(i, j); i += 1; } }
    a.swap(i, r); i
}
...
pub fn karatsuba_mul(a: &str, b: &str) -> String {
    fn strip(s: &str) -> &str { let s2 = s.trim_start_matches('0'); if s2.is_empty() { "0" } else { s2 } }
    fn add_str(x: &str, y: &str) -> String {
        ...
        let mut out = Vec::new();
        while i >= 0 || j >= 0 || carry > 0 { let xi = if i>=0 { (xb[i as usize]-b'0') as i32 } else {0}; let yj = if j>=0 { (yb[j as usize]-b'0') as i32 } else {0}; let s=xi+yj+carry; out.push((s%10) as u8 + b'0'); carry=s/10; i-=1; j-=1; }
        out.reverse(); String::from_utf8(out).unwrap()
    }
    fn sub_str(x: &str, y: &str) -> String { // assume x>=y
        let (xb, yb) = (x.as_bytes(), y.as_bytes());
        let (mut i, mut j, mut borrow) = (xb.len() as isize - 1, yb.len() as isize - 1, 0i32);
        let mut out = Vec::new();
        while i >= 0 { let xi=(xb[i as usize]-b'0') as i32 - borrow; let yj=if j>=0 {(yb[j as usize]-b'0') as i32}else{0}; let mut d=xi - yj; if d<0 { d+=10; borrow=1; } else { borrow=0; } out.push((d as u8)+b'0'); i-=1; j-=1; }
        while out.len()>1 && *out.last().unwrap()==b'0' { out.pop(); }
        out.reverse(); String::from_utf8(out).unwrap()
    }
    fn mul_small(a: &str, b: &str) -> String {
        if a.len() <= 32 && b.len() <= 32 {
            let ai: u128 = a.parse().unwrap_or(0); let bi: u128 = b.parse().unwrap_or(0); return (ai*bi).to_string();
        }
        karatsuba(a, b)
    }
    fn karatsuba(a: &str, b: &str) -> String {
        ...
        if n <= 32 { return mul_grade_school(a, b); }
        let m = n/2;
    ...
    fn mul_grade_school(a: &str, b: &str) -> String {
        ...
        for i in (0..a.len()).rev() { for j in (0..b.len()).rev() { let ai=(a[i]-b'0') as u32; let bj=(b[j]-b'0') as u32; let p=ai*bj+prod[i+j+1]; prod[i+j+1]=p%10; prod[i+j]+=p/10; } }
        let mut i=0; while i+1<prod.len() && prod[i]==0 { i+=1; }
        let s: String = prod[i..].iter().map(|&d| (d as u8 + b'0') as char).collect(); s
    }
    fn split_pad(s: &str, m: usize) -> (&str, &str) { let l = s.len(); if l<=m { ("0", s) } else { (&s[..l-m], &s[l-m..]) } }
    fn add_shift(s: &str, k: usize) -> String { let mut out = s.to_string(); out.extend(std::iter::repeat('0').take(k)); out }
...
mod tests {
    ...
    fn test_quickselect_and_karatsuba() {
        ...
        assert_eq!(kth, 5);
        assert_eq!(karatsuba_mul("12345678901234567890", "9876543210"), (12345678901234567890u128*9876543210u128).to_string());
    }
