...
impl NaiveBayesClassifier {
    ...
    fn calculate_gaussian_probability(&self, feature_value: f64, mean: f64, variance: f64) -> f64 {
        if variance == 0.0 {
            return if (feature_value - mean).abs() < 1e-10 { 1.0 } else { 0.0 };
        }
    ...
    /// 计算特征的均值和方差
    fn calculate_feature_stats(&self, data: &Dataset, labels: &Labels, class: Label) -> Vec<(f64, f64)> {
        let class_data: Vec<&DataPoint> = data.iter()
            .zip(labels.iter())
        ...
        for feature_idx in 0..n_features {
            let values: Vec<f64> = class_data.iter()
                .map(|point| point[feature_idx])
                .collect();
...
impl SupervisedLearning for NaiveBayesClassifier {
    fn train(&mut self, data: &Dataset, labels: &Labels) -> MLResult<()> {
        ...
        // 获取所有类别
        let mut classes: Vec<Label> = labels.iter().cloned().collect();
        classes.sort();
        classes.dedup();
        ...
        for (class, count) in class_counts.iter() {
            class_priors.insert(*class, (*count as f64 + self.alpha) / (total_samples + self.alpha * classes.len() as f64));
        }
