...
fn cross(o: Pt, a: Pt, b: Pt) -> f64 { (a.x - o.x) * (b.y - o.y) - (a.y - o.y) * (b.x - o.x) }
fn dist2(a: Pt, b: Pt) -> f64 { (a.x - b.x).powi(2) + (a.y - b.y).powi(2) }

/// 凸包：返回按逆时针顺序的顶点（包含共线最外点）
pub fn convex_hull(mut pts: Vec<Pt>) -> Vec<Pt> {
    pts.sort_by(|p, q| p.x.partial_cmp(&q.x).unwrap().then(p.y.partial_cmp(&q.y).unwrap()));
    let n = pts.len();
    if n <= 1 { return pts; }
...
mod tests {
    ...
    fn test_convex_hull_and_diameter() {
        let pts = vec![Pt{ x:0.0,y:0.0}, Pt{ x:1.0,y:0.0}, Pt{ x:1.0,y:1.0}, Pt{ x:0.0,y:1.0}, Pt{ x:0.5,y:0.5}];
