        ...
        match arg.as_str() {
            "--format" => {
                if let Some(v) = iter.next() {
                    format = if v.eq_ignore_ascii_case("json") { OutputFormat::Json } else { OutputFormat::Csv };
                }
            }
            "--output" => { output = iter.next(); }
            "--select" => { select = iter.next(); }
            "--preset" => { preset = iter.next(); }
            "--repeat" => { if let Some(v) = iter.next() { repeat = v.parse().unwrap_or(1); } }
            _ => {}
        }
    }
    Args { format, output, select, preset, repeat }
}
...
fn main() {
    ...
    let mut write_row = |category: &str, algo: &str, variant: &str, param: &str, size: usize, unit: &str, micros: u128| {
        match args.format {
            OutputFormat::Csv => {
                let _ = writeln!(sink, "{category},{algo},{variant},{param},{size},{unit},{micros}");
            }
    ...
    for &n in &sizes_sort {
        ...
        for _ in 0..args.repeat { let (_r, us) = time_it(|| { sort_sync(&mut v, SortingAlgo::Quick); () }); best = best.min(us); }
        write_row("sorting", "quick", "sync", "n", n, "items", best);

        let mut v: Vec<i32> = (0..n as i32).rev().collect();
        let mut best = u128::MAX;
        for _ in 0..args.repeat { let (_r, us) = time_it(|| { sort_parallel(&mut v, SortingAlgo::Quick); () }); best = best.min(us); }
    ...
    for &n in &[100_000usize, 500_000] {
        ...
        let mut best = u128::MAX;
        for _ in 0..args.repeat { let (_r, us) = time_it(|| binary_search_sync(&data, &(n as i32 - 1)).unwrap()); best = best.min(us); }
        write_row("searching", "binary", "sync", "n", n, "items", best);
        let mut best = u128::MAX;
        for _ in 0..args.repeat { let (_r, us) = time_it(|| parallel_search(&data, &42)); best = best.min(us); }
    ...
    for i in 0..10_000 { g.insert(i, vec![i + 1]); }
    g.insert(10_000, vec![]);
    if args.select.as_deref().unwrap_or("").contains("graph") || args.select.is_none() {
    let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| bfs_shortest_path_sync(&g, &0, &10_000)); best = best.min(us); }
    ...
    for i in 0..10_000 { wg.insert(i, vec![(i + 1, 1.0)]); }
    wg.insert(10_000, vec![]);
    let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| dijkstra_sync(&wg, &0)); best = best.min(us); }
    ...
    let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| max_subarray_sum_parallel(&nums)); best = best.min(us); }
    write_row("dac", "max_subarray", "parallel", "n", 200_000, "items", best);
    }

    let a = vec![b'A'; 2000];
    let b = vec![b'A'; 2000];
    if args.select.as_deref().unwrap_or("").contains("dp") || args.select.is_none() {
    let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| lcs_parallel(&a, &b)); best = best.min(us); }
    ...
    for &n in &[10_000usize, 100_000] {
        ...
        let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| kmp_search(&text, &pattern)); best = best.min(us); }
        write_row("strings", "kmp", "sync", "n", n, "chars", best);
        let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| rabin_karp_search(&text, &pattern)); best = best.min(us); }
        write_row("strings", "rk", "sync", "n", n, "chars", best);
    }
    let text = "ahishers".to_string();
    let patterns = vec!["he".to_string(), "she".to_string(), "hers".to_string(), "his".to_string()];
    let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| {
    ...
    // 回溯：N 皇后计数
    if args.select.as_deref().unwrap_or("").contains("backtracking") || args.select.is_none() {
    for &n in &[10usize, 11] {
        let mut best = u128::MAX; for _ in 0..args.repeat { let (_r, us) = time_it(|| nqueens_count_parallel(n)); best = best.min(us); }
    ...
    // flush stdout
    match args.format {
        OutputFormat::Csv => { let _ = sink.flush(); }
        OutputFormat::Json => {
            let _ = writeln!(sink, "{}", serde_json::to_string_pretty(&json_items).unwrap());
        }
    }
}
