...
**声明宏实践指南 (Declarative Macro Practice Guide)**:

- **定义**: 使用 `macro_rules!` 进行代码生成和 DSL 构建的实践指南
- **类型**: 实践指南文档
- **范畴**: 宏系统、元编程
- **版本**: Rust 1.0+
- **相关概念**: 声明宏、模式匹配、代码生成、DSL

**声明宏 (Declarative Macro)**:

- **定义**: 使用 `macro_rules!` 定义的宏，通过模式匹配进行代码生成
- **类型**: 宏系统
- **属性**: 模式匹配、代码生成、卫生性
- **关系**: 与过程宏、函数宏相关
...
- 声明宏实践指南 --[uses]--> 模式匹配和重复语法
- DSL 构建 --[uses]--> 声明宏

**依赖关系**:

- 声明宏 --[depends-on]--> 编译器支持
- 代码生成 --[depends-on]--> 声明宏
