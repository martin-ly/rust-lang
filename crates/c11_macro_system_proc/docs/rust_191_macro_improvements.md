> **EN**: Rust 1.91 Macro System Improvements (c11_macro_system_proc example index)
> **Summary**: A stub page pointing to the canonical concept authority for Rust 1.91 macro system improvements.

# Rust 1.91 宏系统改进文档（c11_macro_system_proc 示例索引）

> **权威来源**: Rust 1.91 宏系统改进（宏展开性能、const 上下文、错误消息、过程宏编译等）已整理至
> [`concept/07_future/00_version_tracking/rust_1_91_stabilized.md`](../../../concept/07_future/00_version_tracking/rust_1_91_stabilized.md)。

本文件原为 `c11_macro_system_proc` crate 的 Rust 1.91 宏系统改进记录。根据 AGENTS.md §6.4 治理规则，
通用 Rust 概念解释已迁移至 `concept/07_future/00_version_tracking/`，此处仅保留索引与 canonical 链接。

## 本 crate 相关示例

- `crates/c11_macro_system_proc/examples/`：过程宏可运行示例。
- `crates/c11_macro_system_proc/src/bin/`：宏元编程演示程序。

## 快速导航

| 主题 | 权威来源 |
| :--- | :--- |
| Rust 1.91 稳定特性 | [`concept/07_future/00_version_tracking/rust_1_91_stabilized.md`](../../../concept/07_future/00_version_tracking/rust_1_91_stabilized.md) |
| 过程宏 | [`concept/03_advanced/03_proc_macros/07_proc_macro.md`](../../../concept/03_advanced/03_proc_macros/07_proc_macro.md) |
| 宏模式 | [`concept/02_intermediate/06_macros_and_metaprogramming/17_macro_patterns.md`](../../../concept/02_intermediate/06_macros_and_metaprogramming/17_macro_patterns.md) |
