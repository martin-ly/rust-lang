...
- [4.5 性能优化](#45-性能优化)
  ...
  - [1. 迭代器优化](#1-迭代器优化)
    ...
    - [使用iter而非into_iter](#使用iter而非into_iter)
    - [迭代器融合](#迭代器融合)
  - [2. 分支预测](#2-分支预测)
...
- **范畴**: 控制流、性能优化
- **版本**: Rust 1.92.0+
- **相关概念**: 性能优化、迭代器优化、分支预测、内联优化、SIMD

### 属性特征

**核心属性**:

- **迭代器优化**: 避免 collect 后再迭代、使用 iter 而非 into_iter、迭代器融合
- **分支预测**: likely/unlikely 提示、减少分支
- **内联优化**: 使用 #[inline]、泛型单态化
- **SIMD 与向量化**: 使用 slice 方法、使用迭代器（易向量化）

**性能特征**:

- **性能提升**: 通过优化提升性能
- **零成本抽象**: 保持零成本抽象
- **适用场景**: 性能关键路径、性能优化、代码优化

### 关系连接

**组合关系**:

- 性能优化 --[covers]--> 性能优化完整内容
- 高性能程序 --[uses]--> 性能优化

**依赖关系**:

- 性能优化 --[depends-on]--> 控制流和函数
