...
- [2.1 条件语句指南](#21-条件语句指南)
  ...
  - [3. if let 表达式](#3-if-let-表达式)
    - [3.1 基本用法](#31-基本用法)
    - [3.2 链式 if let (Rust 1.96.1+)](#32-链式-if-let-rust-1961)
...
**条件语句指南 (Conditional Statements Guide)**:
...
- **if 表达式**: 基本条件判断、表达式返回值
- **match 表达式**: 模式匹配、完整性检查、守卫条件
- **if let 表达式**: 简化 Option/Result 处理、链式 if let
- **let else 语句**: 早期返回、错误处理
...
- 条件语句指南 --[covers]--> 多种条件语句
- 控制流程序 --[uses]--> 条件语句

**依赖关系**:

- 条件语句 --[depends-on]--> 布尔表达式
- 程序逻辑控制 --[depends-on]--> 条件语句
...
- [循环结构指南](02_loops_guide.md)
- [模式匹配指南](04_pattern_matching_guide.md)
