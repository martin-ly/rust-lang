...
> **学习时间**: 1-2天
> **相关文档**: [主索引](../tier_01_foundations/02_navigation.md) | [项目概览](../tier_01_foundations/01_project_overview.md)
...
**循环结构指南 (Loop Structures Guide)**:

- **定义**: 系统化学习 Rust 循环结构的实践指南，包括 loop、while、for 等
- **类型**: 实践指南文档
- **范畴**: 控制流、迭代器
- **版本**: Rust 1.0+
- **相关概念**: 循环结构、迭代器、控制流、循环控制
...
- **loop 循环**: 无限循环、循环标签、返回值
- **while 循环**: 条件循环、while let 模式
- **for 循环**: 迭代器循环、范围表达式
...
- 循环结构指南 --[covers]--> 多种循环结构
- 控制流程序 --[uses]--> 循环结构

**依赖关系**:

- 循环结构 --[depends-on]--> 迭代器
- 重复执行逻辑 --[depends-on]--> 循环结构
...
迭代器与闭包在控制流中协同：闭包作为适配器参数，实现过滤、转换、折叠等：

```rust
fn main() {
    let nums = vec![1, 2, 3, 4, 5];

    // 闭包 + filter + map：过滤偶数并平方
    let result: Vec<_> = nums.iter()
        .filter(|&x| x % 2 == 0)      // 闭包：谓词
        .map(|x| x * x)                // 闭包：转换
        .collect();
    ...
    // 闭包 + filter_map：过滤并转换（Option）
    let parsed: Vec<i32> = ["1", "a", "2", "b", "3"]
        .iter()
        .filter_map(|s| s.parse().ok())  // 闭包返回 Option
        .collect();
...
详见 [迭代器参考](../tier_03_references/02_iterators_reference.md)、[闭包深入](../tier_04_advanced/02_closures_in_depth.md)。
