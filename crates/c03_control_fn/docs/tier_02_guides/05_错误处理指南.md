...
- [2.5 错误处理指南](#25-错误处理指南)
  ...
  - [7. 实战案例](#7-实战案例)
    ...
    - [案例3: 配置管理](#案例3-配置管理)
    - [案例4: 数据验证](#案例4-数据验证)
    - [7.5 边界案例与进阶](#75-边界案例与进阶)
      - [From/Into 错误映射](#frominto-错误映射)
      - [anyhow vs thiserror 选型](#anyhow-vs-thiserror-选型)
      - [早返回与 RAII](#早返回与-raii)
  - [8. 最佳实践](#8-最佳实践)
  - [9. 常见陷阱](#9-常见陷阱)
  - [10. 小结](#10-小结)
...
- **定义**: 系统化学习 Rust 错误处理的实践指南，包括 Result、Option、panic 机制
- **类型**: 实践指南文档
- **范畴**: 错误处理、类型系统
- **版本**: Rust 1.0+
- **相关概念**: Result、Option、错误处理、panic、错误传播

**错误处理 (Error Handling)**:

- **定义**: 处理程序中可能出现的错误情况的机制
- **类型**: 编程范式
- **属性**: 可恢复错误、不可恢复错误、错误传播
- **关系**: 与 Result、Option、panic 相关
...
- 错误处理指南 --[covers]--> 多种错误处理机制
- 健壮程序 --[uses]--> 错误处理

**依赖关系**:

- 错误处理 --[depends-on]--> 类型系统
- 程序可靠性 --[depends-on]--> 错误处理
...
`From` 使 `?` 能自动转换错误类型；`Into` 由 `From` 自动实现：
...
// ? 自动调用 Into::into，将 io::Error/ParseIntError 转为 AppError
...
| 场景 | 推荐 | 说明 |
| :--- | :--- | :--- || 应用层（main、二进制） | **anyhow** | `Result<T, anyhow::Error>`，便于快速传播、添加上下文 |
| 库/API 层 | **thiserror** | 定义结构化错误类型，便于调用方匹配与处理 |
| 组合使用 | 常见 | 库用 thiserror 定义错误，应用用 anyhow 包装 |
...
详见 [错误处理进阶](../tier_04_advanced/04_错误处理进阶.md)。

#### 早返回与 RAII

**早返回**：遇错即 `return Err(...)`，避免深层嵌套：

```rust
fn process(path: &str) -> Result<String, std::io::Error> {
    let f = std::fs::File::open(path)?;  // 失败则早返回
...
struct Guard;
impl Drop for Guard {
    fn drop(&mut self) { println!("资源已释放"); }
}

fn with_guard() -> Result<(), std::io::Error> {
    let _g = Guard;
    std::fs::read_to_string("x.txt")?;  // 出错时 _g 仍会 drop
    Ok(())
}
```

详见 [错误处理进阶 - RAII 模式](../tier_04_advanced/04_错误处理进阶.md#5-异常安全性)。
