...
use c03_control_fn::async_control_flow_190::AsyncResourceManager;
...
async fn test_performance_benchmark() {
    ...
    let duration = benchmark.benchmark("test", 10, || async {
        42
    }).await;

    assert!(duration.average_time >= Duration::ZERO);
...
async fn test_memory_safety() {
    ...
    for i in 0..1000 {
        let conn = DatabaseConnection::new(i.to_string(), format!("test://localhost:{}", i));
        resources.push(conn);
    }
...
async fn test_performance_characteristics() {
    ...
    // 测试简单操作性能
    let simple_time = benchmark.benchmark("simple_operation", 1000, || async {
        42 * 2
    }).await;

    // 测试复杂操作性能
    let complex_time = benchmark.benchmark("complex_operation", 100, || async {
        (0..1000).map(|x| x * x).sum::<i32>()
    }).await;

    // 性能应该在合理范围内
    assert!(simple_time.average_time < Duration::from_millis(1));
    assert!(complex_time.average_time < Duration::from_millis(10));
}
