...
///
/// Rust 1.97 稳定化的核心控制流 API：
/// - `io::ErrorKind::QuotaExceeded` — 磁盘配额耗尽
/// - `io::ErrorKind::CrossesDevices` — 跨设备操作
/// - `BuildHasherDefault::new` — 默认哈希构建器构造
pub struct Rust197ControlFlowFeatures;

impl Rust197ControlFlowFeatures {
    /// 演示新的 `io::ErrorKind` 变体在错误处理中的应用
    ///
    /// 根据错误类型选择不同的恢复策略。
    ...
    /// 使用 `BuildHasherDefault::new` 创建自定义哈希构建器
    ///
