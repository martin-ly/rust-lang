//! Rust 1.94.0 引入的设计模式特性（1.94.0 稳定）
//! Rust 1.94 designpatternfeatures (stable in 1.94.0) implementation module
//!
//! 本模块展示了 Rust 1.94.0 引入并稳定的真实特性在设计模式场景中的应用，包括：
//! This module demonstrates Rust 1.94 truefeatures (stable in 1.94.0) design pattern application including
...
/// array_windows state machine conversion
///
/// 展示如何在状态机模式中使用 array_windows 检测无效的状态转换
/// in state machine in array_windows ineffective state conversion
...
impl CharPositionMapper {
    ...
    /// 将字符串转换为位置序列
    /// will conversion as position sequence
...
/// Get Rust 1.94.0 design pattern features (stable in 1.94.0)sinformation
