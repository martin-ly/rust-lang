> **EN**: CSP vs Async Analysis: Formal Semantics and Rust Practice (c09_design_pattern example index)
> **Summary**: A stub page pointing to the canonical concept authority for comparing CSP (Go) and async/await (Rust) concurrency models. The c09_design_pattern crate provides runnable comparison examples.
...
- `crates/c09_design_pattern/examples/`：CSP 与 async 对比可运行示例。
- `crates/c09_design_pattern/src/bin/`：并发模型演示程序。
