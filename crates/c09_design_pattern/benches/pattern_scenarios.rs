...
fn deco1(f: fn(u64) -> u64) -> impl Fn(u64) -> u64 {
    move |x| f(x).wrapping_mul(2)
}
fn deco2(f: fn(u64) -> u64) -> impl Fn(u64) -> u64 {
    move |x| f(x).wrapping_add(11)
}
...
pub fn scenario_benchmarks(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("chain", 1024), &1024, |b, &n| {
        b.iter(|| {
            ...
            for i in 0..n {
                out = chain_process(black_box(&format!("{}", i)));
            }
            black_box(out)
        })
    });
    ...
    group.bench_with_input(BenchmarkId::new("decorator", 1_000), &1_000u64, |b, &n| {
        ...
        b.iter(|| {
            let mut acc = 0u64;
            for i in 0..n {
                acc = acc.wrapping_add(black_box(f1(i)) ^ black_box(f2(i)));
            }
            black_box(acc)
        })
    });
    ...
    group.bench_with_input(BenchmarkId::new("proxy", 1_000), &1_000u64, |b, &n| {
        b.iter(|| {
            let mut acc = 0u64;
            for i in 0..n {
                acc = acc.wrapping_add(black_box(proxy_call(i % 2 == 0, i)));
            }
            black_box(acc)
        })
    });
