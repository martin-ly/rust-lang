...
fn benchmark_singleton(c: &mut Criterion) {
    ...
    group.bench_function("get_instance", |b| {
        ...
        b.iter(|| {
            let _instance =
                singleton.get_instance(|| black_box(String::from("Benchmark Instance")));
        });
    });
    ...
    group.bench_function("repeated_get_instance", |b| {
        let singleton = Singleton::new();
        let _initial = singleton.get_instance(|| black_box(String::from("Initial Instance")));

        b.iter(|| {
            let _instance =
                singleton.get_instance(|| black_box(String::from("Should Not Be Called")));
        });
    });
...
fn benchmark_memory_usage(c: &mut Criterion) {
    ...
    group.bench_function("singleton_memory_efficiency", |b| {
        b.iter(|| {
            ...
            for _ in 0..1000 {
                let _instance =
                    singleton.get_instance(|| black_box(String::from("Memory Test Instance")));
            }
        ...
        b.iter(|| {
            ...
            for _ in 0..10 {
                ...
                let handle = thread::spawn(move || {
                    let _instance = singleton_clone
                        .get_instance(|| black_box(String::from("Concurrent Instance")));
                });
                handles.push(handle);
            }
...
fn benchmark_scalability(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        group.bench_with_input(
            BenchmarkId::new("flyweight_scale", size),
            size,
            |b, &size| {
                b.iter(|| {
                    let mut factory = OptimizedFlyweightFactory::new();
