...
mod integration_tests {
    ...
    fn test_singleton_with_factory_integration() {
        ...
        let factory = singleton_factory.get_instance(|| Box::new(CircleFactory));
        ...
        // 两个工厂应该是同一个实例（通过内容比较）
        assert_eq!(
            factory.create_shape().area(),
            factory2.create_shape().area()
        );
    }
    ...
    fn test_proxy_with_flyweight_integration() {
        ...
        // 创建享元对象
        let flyweight =
            flyweight_factory.get_flyweight("PROXY_TEST", "Proxy Flyweight State".to_string());
    ...
    fn test_multithreaded_pattern_integration() {
        ...
        for i in 0..5 {
            ...
            let handle = thread::spawn(move || {
                let factory = singleton_clone.get_instance(|| Box::new(CircleFactory));
    ...
    fn test_memory_optimization_integration() {
        ...
        // 测试重用
        let reused_flyweight =
            flyweight_factory.get_flyweight("MEMORY_TEST_0", "Different State".to_string());
...
mod performance_tests {
    ...
    fn test_singleton_performance() {
        ...
        // 多次获取单例实例
        for _ in 0..10000 {
            let _instance = singleton.get_instance(|| String::from("Performance Test Instance"));
        }
