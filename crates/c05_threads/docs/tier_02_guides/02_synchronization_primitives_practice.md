...
- **定义**: 用于协调多个线程访问共享资源的基本构建块
- **类型**: 系统原语
- **范畴**: 并发编程
- **版本**: Rust 1.0+
- **相关概念**: 线程、数据竞争、死锁、并发控制

**Mutex (互斥锁)**:

- **定义**: 互斥锁，保证同一时间只有一个线程可以访问共享资源
- **类型**: 同步原语
- **属性**: 互斥性、线程安全、锁中毒处理
- **关系**: 与 RwLock、Condvar 相关

**RwLock (读写锁)**:

- **定义**: 读写锁，允许多个读操作或单个写操作
- **类型**: 同步原语
- **属性**: 读共享、写独占、性能优化
- **关系**: 与 Mutex、并发访问相关
...
- **互斥性**: Mutex 保证互斥访问
- **读写分离**: RwLock 区分读写操作
- **条件等待**: Condvar 提供条件等待机制
- **屏障同步**: Barrier 实现多线程同步点

**性能特征**:

- **Mutex**: 简单但可能成为瓶颈
- **RwLock**: 读多写少场景性能更好
- **适用场景**: 共享资源访问、线程协调、同步点
...
- 同步原语 --[depends-on]--> 操作系统同步支持
- 线程安全 --[depends-on]--> 同步原语
...
static RESOURCE: std::sync::OnceLock<String> = std::sync::OnceLock::new();
...
- [← 上一篇：线程基础与生命周期](01_thread_basics_and_lifetimes.md)
- [→ 下一篇：消息传递模式](03_message_passing_patterns.md)
