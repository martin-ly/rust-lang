...
> **文档版本**: v2.0.0 | **Rust 版本**: 1.96.1+ | **最后更新**: 2025-12-11

## 目录

- [C05 Tier 2 实践指南 04：并行编程指南](#c05-tier-2-实践指南-04并行编程指南)
  - [目录](#目录)
  - [📐 知识结构](#-知识结构)
    - [概念定义](#概念定义)
    - [属性特征](#属性特征)
    ...
    - [思维导图](#思维导图)
  - [1. 概述](#1-概述)
    - [1.1 什么是数据并行](#11-什么是数据并行)
    - [1.2 Rayon 核心特性](#12-rayon-核心特性)
    - [1.3 安装 Rayon](#13-安装-rayon)
  - [2. Rayon 并行迭代器](#2-rayon-并行迭代器)
    - [2.1 基础用法](#21-基础用法)
    - [2.2 par\_iter() 家族](#22-par_iter-家族)
    - [2.3 并行 map/filter/reduce](#23-并行-mapfilterreduce)
    - [2.4 链式操作](#24-链式操作)
  - [3. 并行集合操作](#3-并行集合操作)
    - [3.1 并行排序](#31-并行排序)
    - [3.2 并行搜索](#32-并行搜索)
    - [3.3 并行分区](#33-并行分区)
  - [4. 自定义并行任务](#4-自定义并行任务)
    - [4.1 使用 join() 分叉-合并](#41-使用-join-分叉-合并)
    - [4.2 使用 scope() 生成任务](#42-使用-scope-生成任务)
    - [4.3 使用 ThreadPool 自定义线程池](#43-使用-threadpool-自定义线程池)
  - [5. 分治算法](#5-分治算法)
    - [5.1 快速排序](#51-快速排序)
    - [5.2 归并排序](#52-归并排序)
  - [6. 数据并行模式](#6-数据并行模式)
    ...
    - [6.3 并行分组](#63-并行分组)
  - [7. 性能调优](#7-性能调优)
    - [7.1 选择合适的粒度](#71-选择合适的粒度)
    - [7.2 避免过小的任务](#72-避免过小的任务)
    - [7.3 基准测试](#73-基准测试)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 测量再优化](#81-测量再优化)
    - [8.2 避免不必要的并行](#82-避免不必要的并行)
    - [8.3 配置线程池](#83-配置线程池)
  - [9. 常见陷阱](#9-常见陷阱)
    - [9.1 错误的共享状态](#91-错误的共享状态)
    - [9.2 非线程安全类型](#92-非线程安全类型)
  - [10. 参考资源](#10-参考资源)
    - [10.1 官方文档](#101-官方文档)
    - [10.2 推荐阅读](#102-推荐阅读)
...
- **定义**: 使用多个执行单元同时执行任务，充分利用多核 CPU 的计算能力
- **类型**: 编程范式
- **范畴**: 并发编程、高性能计算
- **版本**: Rust 1.0+ (Rayon)
- **相关概念**: 数据并行、任务并行、工作窃取、负载均衡

**数据并行 (Data Parallelism)**:

- **定义**: 将相同的操作应用于不同的数据片段，充分利用多核 CPU
- **类型**: 并行模式
- **属性**: 数据分片、并行执行、结果合并
- **关系**: 与任务并行、并行算法相关
...
- 并行程序 --[uses]--> 并行编程技术
- Rayon --[provides]--> 并行迭代器

**依赖关系**:

- 并行编程 --[depends-on]--> 多核 CPU
- Rayon --[depends-on]--> 工作窃取调度器
...
- [← 上一篇：消息传递模式](03_message_passing_patterns.md)
- [→ 下一篇：异步集成实践](05_async_integration_practice.md)
