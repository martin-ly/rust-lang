...
- **定义**: 将线程和异步编程结合使用的技术，充分利用各自的优势
- **类型**: 编程技术
- **范畴**: 并发编程、异步编程
- **版本**: Rust 1.39+ (Tokio)
- **相关概念**: 线程、异步运行时、混合架构、spawn_blocking
...
- **混合架构**: 线程 + 异步运行时
- **桥接机制**: `spawn_blocking` 连接线程和异步
- **通道互操作**: oneshot、mpsc、watch 通道
- **资源管理**: 自动管理线程和任务资源
...
- 混合架构 --[uses]--> 线程和异步
- 异步集成 --[uses]--> spawn_blocking

**依赖关系**:

- 异步集成 --[depends-on]--> 异步运行时
- 异步集成 --[depends-on]--> 线程支持
...
- [`async-std [已归档]`](<https://docs.rs/async-std> [已归档]) - 另一个异步运行时
- [`rayon`](https://docs.rs/rayon) - 数据并行

### 10.4 内部文档链接

- [← 上一篇：并行编程指南](04_parallel_programming_guide.md)
