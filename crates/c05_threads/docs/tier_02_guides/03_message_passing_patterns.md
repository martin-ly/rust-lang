...
- **定义**: 一种并发编程范式，通过发送消息在线程间通信，而不是共享内存
- **类型**: 并发编程范式
- **范畴**: 并发编程
- **版本**: Rust 1.0+
- **相关概念**: 通道、发送者、接收者、所有权转移

**Channel (通道)**:

- **定义**: 用于在线程间传递消息的通信机制
- **类型**: 并发原语
- **属性**: 发送者、接收者、消息队列、背压处理
- **关系**: 与消息传递、线程通信相关
...
| :--- | :--- | :--- | :--- | :--- || 标准库 mpsc         | 无界      | 中   | ❌   | 通用场景   |
| Crossbeam bounded   | 有界      | 高   | ✅   | 高性能场景 |
| Crossbeam unbounded | 无界      | 高   | ❌   | 高吞吐场景 |
| Tokio mpsc          | 有界/无界 | 高   | ✅   | 异步场景   |
...
- [← 上一篇：同步原语实践](02_synchronization_primitives_practice.md)
- [→ 下一篇：并行编程指南](04_parallel_programming_guide.md)
