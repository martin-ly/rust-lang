...
fn main() {
    // Blocking
    {
        ...
        let c = {
            ...
            thread::spawn(move || {
                let mut out = Vec::new();
                for _ in 0..8 { if let Some(v) = ch.recv() { out.push(v); thread::sleep(Duration::from_millis(5)); } }
    ...
    // Dropping
    {
        ...
        for i in 0..16 { let _ = ch.send(i); }
        let mut out = Vec::new();
        while let Some(v) = ch.recv() { out.push(v); }
        println!("dropping -> len={}, dropped~{}", out.len(), 16 - out.len());
    }

    // Adaptive
    {
        let cfg = bp::BackpressureConfig { strategy: bp::BackpressureStrategy::Adaptive, buffer_size: 8, high_watermark: 0.7, low_watermark: 0.3, drop_threshold: 0.9, adaptation_interval: Duration::from_millis(50) };
        let ch = bp::AdaptiveBackpressureChannel::new(cfg);
        for i in 0..16 { let _ = ch.send(i); }
        let mut out = Vec::new();
        for _ in 0..16 { if let Some(v) = ch.recv() { out.push(v); } }
