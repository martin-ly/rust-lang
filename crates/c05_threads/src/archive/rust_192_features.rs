...
//! - `get_stats()`:
//! - `get_stats()`: Get统计信息
//! - `get_stats()`: Getstatisticsinformation
...
impl<T> ThreadSafeUninitBuffer<T> {
    ...
    /// - `index` invalidrangeinside（0 <= index < capacity）
    /// - 该位置之前未被初始化，或已正确清理
...
impl ThreadPoolManager {
    ...
    /// Getqueueintaskquantity（thread-safe）
