...
pub fn demonstrate_thread_management() {
    ...
    // 创建通道
    let receiver = comm_manager.create_channel("test_channel".to_string()).unwrap();
    
    // 注册消息处理器
    comm_manager.register_message_handler("test".to_string(), |message| {
        println!("收到消息: {} 从 {} 到 {}", message.content, message.sender, message.receiver);
    });
    ...
    comm_manager.send_message("test_channel".to_string(), message).unwrap();
...
mod tests {
    ...
    fn test_thread_manager() {
        ...
        let thread_id = manager.create_thread("test".to_string(), 2, || {
            thread::sleep(Duration::from_millis(10));
        }).unwrap();
    ...
    fn test_thread_communication() {
        ...
        comm_manager.send_message("test".to_string(), message).unwrap();
