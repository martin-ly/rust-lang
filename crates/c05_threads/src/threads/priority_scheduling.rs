...
impl ThreadPriorityScheduler {
    ...
    pub fn register_thread(&self, thread_id: usize, priority: ThreadPriority, policy: SchedulingPolicy) {
    ...
    pub fn set_thread_priority(&self, thread_id: usize, priority: ThreadPriority) -> Result<(), String> {
        let mut threads = self.threads.lock().unwrap();
    ...
    pub fn boost_thread_priority(&self, thread_id: usize, new_priority: ThreadPriority) -> Result<(), String> {
        let mut threads = self.threads.lock().unwrap();
        if let Some(info) = threads.get_mut(&thread_id) {
    ...
    fn set_os_thread_priority(&self, thread_id: usize, priority: ThreadPriority) -> Result<(), String> {
    ...
    pub fn enable_priority_boost(&self, enabled: bool) {
        self.priority_boost_enabled.store(enabled, Ordering::Relaxed);
    }
...
impl Clone for ThreadPriorityScheduler {
    fn clone(&self) -> Self {
        Self {
            ...
            priority_boost_enabled: AtomicBool::new(self.priority_boost_enabled.load(Ordering::Relaxed)),
        }
    }
}
...
impl RealTimeScheduler {
    ...
    pub fn register_real_time_thread(&self, thread_id: usize, deadline: Duration) -> Result<(), String> {
        // 注册为实时线程
        self.scheduler.register_thread(thread_id, ThreadPriority::RealTime, SchedulingPolicy::RealTime);
        ...
        for (thread_id, deadline) in deadline_monitor.iter() {
            if now > *deadline {
                ...
                *missed_deadlines.entry(*thread_id).or_insert(0) += 1;
                ...
                // 提升优先级
                if let Err(e) = self.scheduler.boost_thread_priority(*thread_id, ThreadPriority::Critical) {
...
pub fn demonstrate_priority_scheduling() {
    ...
    // 注册实时线程
    rt_scheduler.register_real_time_thread(4, Duration::from_millis(100)).unwrap();
    rt_scheduler.register_real_time_thread(5, Duration::from_millis(50)).unwrap();
...
mod tests {
    ...
    fn test_thread_priority_scheduler() {
        ...
        scheduler.set_thread_priority(1, ThreadPriority::Critical).unwrap();
        assert_eq!(scheduler.get_thread_priority(1), Some(ThreadPriority::Critical));
    }
    ...
    fn test_priority_boost() {
        ...
        scheduler.boost_thread_priority(1, ThreadPriority::High).unwrap();
        assert_eq!(scheduler.get_thread_priority(1), Some(ThreadPriority::High));
        
        scheduler.restore_thread_priority(1).unwrap();
        assert_eq!(scheduler.get_thread_priority(1), Some(ThreadPriority::Normal));
    }
