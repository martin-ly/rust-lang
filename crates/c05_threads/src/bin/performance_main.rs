...
use c05_threads::{
    ...
    performance_benchmarks::{
        ...
        benchmark_concurrent_algorithms,
...
fn demo_concurrent_algorithms(data: &[i32]) {
    ...
    for &thread_count in &thread_counts {
        ...
        let duration = start.elapsed();
        println!("  并发映射 ({} 线程): {:?} (样本: {:?})", 
            thread_count, duration, &mapped[..5]);
    }
}
...
fn demo_performance_comparison(data: &[i32]) {
    ...
    println!("    串行: {:?} (样本: {:?})", serial_duration, &serial_result[..5]);
    
    // 并行处理
    let start = Instant::now();
    let parallel_result = parallel_map(data, 8, |&x| x * 2 + 1);
    let parallel_duration = start.elapsed();
    println!("    并行: {:?} (样本: {:?})", parallel_duration, &parallel_result[..5]);
    ...
    println!("    结果一致性: {}", if is_consistent { "✅" } else { "❌" });
}
