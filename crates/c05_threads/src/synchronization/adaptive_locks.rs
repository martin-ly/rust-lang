...
impl<T> AdaptiveMutex<T> {
    ...
    fn adapt_spin_threshold(&self) {
        ...
        if contention_ratio > 0.5 {
            ...
            self.spin_threshold.store(new_threshold.max(10), Ordering::Relaxed);
        } else if contention_ratio < 0.1 {
            // 低竞争，增加自旋
            let new_threshold = (current_threshold as f32 * 1.2) as usize;
            self.spin_threshold.store(new_threshold.min(1000), Ordering::Relaxed);
        }
    }
...
impl<T> AdaptiveSpinLock<T> {
    ...
    fn adapt_spin_duration(&self, wait_time: Duration) {
        ...
        if wait_micros > current_duration * 2 {
            ...
            self.spin_duration.store(new_duration.min(10000), Ordering::Relaxed); // 最大10毫秒
        } else if wait_micros < current_duration / 2 {
            // 等待时间较短，减少自旋时间
            let new_duration = current_duration / backoff_factor;
            self.spin_duration.store(new_duration.max(100), Ordering::Relaxed); // 最小100微秒
        }
    }
...
pub fn demonstrate_adaptive_locks() {
    ...
    println!("自适应互斥锁统计:");
    println!("  总获取次数: {}", stats.total_acquires.load(Ordering::Relaxed));
    println!("  总等待次数: {}", stats.total_waits.load(Ordering::Relaxed));
    println!("  总自旋次数: {}", stats.total_spins.load(Ordering::Relaxed));
