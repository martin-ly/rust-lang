...
impl ScopedThreadsDemo {
    ...
    pub fn basic_scoped_example(&mut self) {
        ...
        thread::scope(|s| {
            ...
            s.spawn(|| {
                for v in s0.iter_mut() { *v *= 2; }
            });
            s.spawn(|| {
                for v in s1.iter_mut() { *v *= 2; }
            });
            s.spawn(|| {
                for v in s2.iter_mut() { *v *= 2; }
            });
            s.spawn(|| {
                for v in s3.iter_mut() { *v *= 2; }
            });
        });
    ...
    pub fn complex_data_structure_example(&mut self) {
        ...
        thread::scope(|s| {
            ...
            for i in 0..4 {
                ...
                let _end = if i == 3 { entries_len } else { (i + 1) * chunk_size };
    ...
    pub fn performance_comparison_example(&mut self) {
        ...
        let handles: Vec<_> = (0..4)
            .map(|i| {
                ...
                thread::spawn(move || {
                    ...
                    let end = if i == 3 { guard.len() } else { (i + 1) * chunk_size };
        ...
        thread::scope(|s| {
            ...
            s.spawn(|| { for v in s0.iter_mut() { *v *= 2; } });
            s.spawn(|| { for v in s1.iter_mut() { *v *= 2; } });
            s.spawn(|| { for v in s2.iter_mut() { *v *= 2; } });
            s.spawn(|| { for v in s3.iter_mut() { *v *= 2; } });
        });
            ...
            for i in 0..2 {
                outer_scope.spawn(move || {
                    ...
                    thread::scope(|inner_scope| {
                        ...
                        let start = i * chunk_size;
                        let end = if i == 1 { data_len } else { (i + 1) * chunk_size };
                        ...
                        for j in 0..2 {
                            inner_scope.spawn(move || {
                                ...
                                let _sub_end = if j == 1 { end } else { start + (j + 1) * sub_chunk_size };
...
impl ScopedThreadsBestPractices {
    ...
    pub fn error_propagation() {
        ...
        let final_results = results.lock().unwrap();
        let errors: Vec<_> = final_results.iter()
