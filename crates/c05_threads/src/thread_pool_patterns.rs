...
/// 线程池设计模式深度解析
///
/// 涵盖固定/动态线程池、工作队列设计、关闭策略、以及主流线程池 crate 的对比。
...
/// 工作窃取调度概念深度解析
///
/// 讲解工作窃取的核心思想、Chase-Lev 双端队列、Owner/Thief 操作模式，
...
/// 作用域线程深度解析
///
/// 深入讲解 `std::thread::scope` 的生命期安全保证、并行模式及与 rayon/crossbeam 的对比。
...
/// 无锁数据结构概念深度解析
///
/// 由于本仓库启用 `unsafe_code = "forbid"` 的 workspace lint，
