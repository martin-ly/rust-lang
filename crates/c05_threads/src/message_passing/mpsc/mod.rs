...
pub use crossbeam_channel::{bounded, unbounded, Receiver, Sender, TryRecvError, TrySendError, RecvTimeoutError, SendError};
...
pub fn send_all<T>(tx: &Sender<T>, items: impl IntoIterator<Item = T>) -> Result<(), T> {
    for item in items { tx.send(item).map_err(|e| e.into_inner())?; }
    Ok(())
}
