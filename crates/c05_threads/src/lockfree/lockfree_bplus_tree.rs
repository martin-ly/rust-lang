    ...
    V: Clone + Send + Sync,
{
    ...
    pub fn new(is_leaf: bool, order: usize) -> Self {
        Self {
            ...
            values: if is_leaf { Vec::with_capacity(order) } else { Vec::new() },
            children: if is_leaf { Vec::new() } else { Vec::with_capacity(order + 1) },
            is_leaf,
    ...
    /// 在内部节点中插入键
    pub fn insert_internal(&self, key: K, child_ptr: *mut BPlusTreeNode<K, V>, order: usize) -> Result<(), K> {
