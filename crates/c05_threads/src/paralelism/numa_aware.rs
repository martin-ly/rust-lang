...
impl NumaAwareParallelCompute {
        ...
        R: Send + Sync + Clone + 'static,
    {
        ...
        for (task_id, task) in tasks.into_iter().enumerate() {
            ...
            node_tasks.entry(node_id).or_insert_with(Vec::new).push((task_id, task));
        }
        ...
        final_results.iter().map(|opt| opt.clone().unwrap()).collect()
    }
        ...
        R: Send + Sync + Clone + 'static,
    {
        ...
        for (i, item) in data.into_iter().enumerate() {
            let node_id = i % self.topology.node_count();
            node_data.entry(node_id).or_insert_with(Vec::new).push((i, item));
        }
...
pub fn demonstrate_numa_aware() {
    ...
    println!("  总内存: {} GB", topology.total_memory / (1024 * 1024 * 1024));
    
    for node in topology.get_nodes() {
        println!("  节点 {}: {} 个CPU核心, {} GB内存", 
                 node.id, node.cpu_count, node.memory_size / (1024 * 1024 * 1024));
    }
