...
| concept code blocks | 4811 块分 10 桶；全量实测 2990（candidate 1732 pass/202 fail + compile_fail 733 ok/1 标注腐烂 + dep 116 pass/167 fail/39 无 rmeta）；2026-07-13 修复腐烂 30 块（误标 compile_fail 17 + 缺上下文/真腐烂 13），剩余 rot=370 中缺上下文类 ~250 登记清单待后续轮次（`reports/CONCEPT_CODE_BLOCKS_BASELINE_2026_07_13.md`；运行期间并行 P1 回填使块数持续增长，以报告运行时刻为准） | exit 0（观察；--strict 当前 rot>0 会 exit 1，未启用） | ⏳ 新增观察门（2026-07-13，P2；缺上下文清单治理后可评估转正） |
