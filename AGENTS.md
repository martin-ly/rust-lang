...
20. `python scripts/check_metadata_consistency.py --strict`（元数据 D1–D6；D2 豁免 2 项 / D5 豁免 30 项（15 项 2026-07-12 + 14 项 2026-07-13 W0–W5 新建页逐文件复核 + 1 项 2026-07-16 GPU/HPC 页工具链事实复核）均已显式登记于检查器白名单并附理由；2026-07-13 另修复 NIGHTLY_RE 词边界误报；2026-07-14 复测 D1–D6 全 0、--strict exit 0，2026-07-14 转阻断）
...
| # | 观察门 | 检查目标 | 当前基线 | 转阻断条件 |
|---:|---|---|---|---|
| O1 | `python scripts/check_stub_purity.py --strict` | `docs/`/`knowledge/`/`content/`/`crates/*/docs/` 中声明为 stub/redirect 的文件是否保留通用概念完整正文 | 伪 stub 51 / 空壳页 0 / 高重复 0（`reports/STUB_PURITY_BASELINE_2026_07_15.md`） | 伪 stub=0 且空壳页=0 |
...
| 季度国际来源语义抽样审计 | 每季度 | `.kimi/templates/quarterly_international_source_audit.md`；维护者抽样 5–8 个核心 `concept/` 页与 Reference/Nomicon/TRPL 对比 |
| 月度语义深度评审 | 每月 | `.kimi/templates/monthly_semantic_review.md` |
| pre-commit 检查 | 每次本地提交 | `scripts/git_hooks/pre-commit` |

---

**最后更新**：2026-07-16（已对齐 Rust 1.97.0 stable；23 门全阻断 + 5 个语义观察门；完成语义模型 sprint 的 KG v3 刷新与谓词兜底；新增 `scripts/fallback_kg_generic_to_related.py`）
