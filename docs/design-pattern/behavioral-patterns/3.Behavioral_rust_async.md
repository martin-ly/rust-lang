以下是使用 Rust 2024 版本的异步特性实现的行为型设计模式示例，包括责任链模式、命令模式、观察者模式等。
...
以上示例展示了如何在 Rust 中使用异步特性实现多种行为型设计模式。每种模式都有其特定的用途和实现方式，适用于不同的场景。通过使用 Rust 的异步特性，开发者可以构建出高性能且响应迅速的应用程序。
