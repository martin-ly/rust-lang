...
- [Rust 所有权、借用与生命周期惯用法 {#rust特有惯用法}](#rust-所有权借用与生命周期惯用法-rust特有惯用法)
  ...
  - [工程案例与代码示例](#工程案例与代码示例)
    - [1. Option::take()安全转移所有权](#1-optiontake安全转移所有权)
    - [2. 借用与可变性](#2-借用与可变性)
    - [3. 生命周期标注](#3-生命周期标注)
...
- **move语义**：资源所有权转移，防止二次释放。
- **Clone/Copy**：显式复制资源，避免隐式副本。
- **`Option<T>`与所有权转移**：take()/replace()安全转移所有权。
...
- **闭包捕获**：move闭包转移所有权，非move闭包借用外部变量。
- **最佳实践**：优先借用，必要时转移所有权，避免RefCell滥用。

---

## 工程案例与代码示例

### 1. Option::take()安全转移所有权
