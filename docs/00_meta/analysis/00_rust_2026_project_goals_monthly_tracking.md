...
> **EN**: Rust 2026 Project Goals Monthly Tracking
...
> **创建日期**: 2026-05-08
> **最后更新**: 2026-05-08
> **Rust 版本**: Stable 1.97.0 | Beta 1.98.0 | Nightly 1.99.0
...
- [Rust 2026 Project Goals — 月度跟踪报告 {#rust-2026-project-goals-月度跟踪报告}](#rust-2026-project-goals--月度跟踪报告-rust-2026-project-goals-月度跟踪报告)
  - [📋 目录 {#目录}](#-目录-目录)
  - [一、Rust 2026 Project Goals 概述 {#一rust-2026-project-goals-概述}](#一rust-2026-project-goals-概述-一rust-2026-project-goals-概述)
  - [二、目标分类体系 {#二目标分类体系}](#二目标分类体系-二目标分类体系)
  - [三、与本项目对齐状态矩阵 {#三与本项目对齐状态矩阵}](#三与本项目对齐状态矩阵-三与本项目对齐状态矩阵)
    - [图例 {#图例}](#图例-图例)
    - [3.1 语言特性 (Language) {#31-语言特性-language}](#31-语言特性-language-31-语言特性-language)
    - [3.2 工具链 (Tooling) {#32-工具链-tooling}](#32-工具链-tooling-32-工具链-tooling)
    - [3.3 生态系统 (Ecosystem) {#33-生态系统-ecosystem}](#33-生态系统-ecosystem-33-生态系统-ecosystem)
    - [3.4 嵌入式/内核 (Embedded/Kernel) {#34-嵌入式内核-embeddedkernel}](#34-嵌入式内核-embeddedkernel-34-嵌入式内核-embeddedkernel)
    - [3.5 形式化验证 (Formal Methods) {#35-形式化验证-formal-methods}](#35-形式化验证-formal-methods-35-形式化验证-formal-methods)
  - [四、月度跟踪机制 {#四月度跟踪机制}](#四月度跟踪机制-四月度跟踪机制)
    - [4.1 更新触发条件 {#41-更新触发条件}](#41-更新触发条件-41-更新触发条件)
    - [4.2 更新操作流程 {#42-更新操作流程}](#42-更新操作流程-42-更新操作流程)
    - [4.3 责任分配建议 {#43-责任分配建议}](#43-责任分配建议-43-责任分配建议)
  - [五、2026-05 快照：当前覆盖度统计 {#五2026-05-快照当前覆盖度统计}](#五2026-05-快照当前覆盖度统计-五2026-05-快照当前覆盖度统计)
    - [5.1 总体覆盖度量化 {#51-总体覆盖度量化}](#51-总体覆盖度量化-51-总体覆盖度量化)
    - [5.2 按覆盖深度分布（可视化） {#52-按覆盖深度分布可视化}](#52-按覆盖深度分布可视化-52-按覆盖深度分布可视化)
    - [5.3 关键洞察 {#53-关键洞察}](#53-关键洞察-53-关键洞察)
    - [5.4 近期新增覆盖（2026-05-08 批次） {#54-近期新增覆盖2026-05-08-批次}](#54-近期新增覆盖2026-05-08-批次-54-近期新增覆盖2026-05-08-批次)
  - [六、下一步优先级建议 {#六下一步优先级建议}](#六下一步优先级建议-六下一步优先级建议)
    - [🔴 P0 — 紧急（高影响力 + 官方里程碑临近或社区需求迫切） {#p0-紧急高影响力-官方里程碑临近或社区需求迫切}](#-p0--紧急高影响力--官方里程碑临近或社区需求迫切-p0-紧急高影响力-官方里程碑临近或社区需求迫切)
    - [🟡 P1 — 重要（中等影响力 + 持续社区关注） {#p1-重要中等影响力-持续社区关注}](#-p1--重要中等影响力--持续社区关注-p1-重要中等影响力-持续社区关注)
    - [🟢 P2 — 规划（长期价值 + 资源允许时推进） {#p2-规划长期价值-资源允许时推进}](#-p2--规划长期价值--资源允许时推进-p2-规划长期价值-资源允许时推进)
  - [七、参考与链接 {#七参考与链接}](#七参考与链接-七参考与链接)
    - [7.1 官方来源 {#71-官方来源}](#71-官方来源-71-官方来源)
    - [7.2 本项目相关报告 {#72-本项目相关报告}](#72-本项目相关报告-72-本项目相关报告)
    - [7.3 已覆盖目标的文档索引 {#73-已覆盖目标的文档索引}](#73-已覆盖目标的文档索引-73-已覆盖目标的文档索引)
...
|------|----------|----------|-----------|---------------|
| 语言特性 | `LANG` | ~18 | 新语法、类型系统演进、borrow checker、unsafe 边界、Edition 2024+ | `docs/01_core/`、`docs/06_research/` |
| 工具链 | `TOOL` | ~16 | 编译器后端、Cargo 功能、IDE/LSP、调试、profile、构建加速 | `docs/09_toolchain/`、`crates/` |
| 生态系统 | `ECO` | ~14 | crates.io、semver、脚本化、互操作、包管理、文档生成 | `docs/04_guides/`、`docs/08_usage_guides/` |
| 嵌入式/内核 | `EMBED` | ~10 | `no_std`、嵌入式 HAL、Rust for Linux、内核模块、实时系统 | `docs/06_research/`、`content/` |
| 形式化验证 | `FORMAL` | ~8 | Miri UB 检测、Polonius、模型检查、定理证明、安全规范 | `docs/06_research/`、`docs/08_usage_guides/` |
...
| # | 2026 目标 | 状态 | 本项目覆盖情况 |
|---|----------|------|---------------|
| L01 | Polonius 下一代 Borrow Checker | ✅ | [`docs/06_research/06_polonius_next_gen_borrow_checker.md`](../../06_research/06_polonius_next_gen_borrow_checker.md) — 完整覆盖原理、Datalog 核心引擎、与 NLL 对比、实际使用方式 |
| L02 | Unsafe Fields (unsafe 粒度细化) | ✅ | [`docs/08_usage_guides/25_unsafe_fields_preview.md`](../../08_usage_guides/25_unsafe_fields_preview.md) — 实验性语法预览，覆盖动机、提议语法、访问规则、与现有 unsafe 块的区别 |
| L03 | `gen` 关键字 / 异步生成器 | 🟡 | `crates/c06_async/` 中有部分 async 示例，但缺少 `gen` 块专题文档和 `Coroutine` trait 解析 |
| L04 | `match` ergonomics (if let guards) | 🟡 | 语言基础部分有涉及，但缺少 1.95.0+ 新 guard 语法的专门说明和迁移示例 |
...
| # | 2026 目标 | 状态 | 本项目覆盖情况 |
|---|----------|------|---------------|
| T01 | Cranelift 后端提速编译 | ✅ | [`docs/09_toolchain/13_cranelift_backend_guide.md`](../../09_toolchain/13_cranelift_backend_guide.md) — 安装、配置、项目级与单次编译、实测数据完整 |
| T02 | Miri UB 检测普及化 | ✅ | [`docs/08_usage_guides/16_miri_practical_guide.md`](../../08_usage_guides/16_miri_practical_guide.md) — 466 行实战指南，覆盖 UAF、OOB、数据竞争、未初始化内存等常见 UB 模式 |
| T03 | Cargo TOML v1.1 支持 | ✅ | [`docs/09_toolchain/16_toml_v11_cargo_guide.md`](../../09_toolchain/16_toml_v11_cargo_guide.md) — 多行内联表、尾部逗号、扩展裸键规则等新特性详解 |
...
| # | 2026 目标 | 状态 | 本项目覆盖情况 |
|---|----------|------|---------------|
| E01 | C++ ↔ Rust 互操作 (cxx / bindgen) | ✅ | [`docs/04_guides/02_cxx_interop_guide.md`](../../04_guides/02_cxx_interop_guide.md) — 覆盖 bindgen 和 cxx 两条技术路线、共享类型与不透明类型、ABI 兼容性 |
...
| E14 | 行业采用案例收集与分析 | 🟡 | [`content/production/`](../../../archive/content/production) 有部分案例，但缺少结构化模板和量化指标 |
...
| # | 2026 目标 | 状态 | 本项目覆盖情况 |
|---|----------|------|---------------|
| K01 | Rust for Linux (RFL) 生产化 | ✅ | [`docs/06_research/08_rust_for_linux.md`](../../06_research/08_rust_for_linux.md) — 完整时间线、里程碑解读、驱动开发指南、从实验到生产的演进分析 |
...
| # | 2026 目标 | 状态 | 本项目覆盖情况 |
|---|----------|------|---------------|
| F01 | Miri 作为 UB 检测基准工具 | ✅ | 见 T02 `05_miri_practical_guide.md`，覆盖栈/堆 UAF、OOB、数据竞争、对齐违规等 10+ UB 模式 |
| F02 | Polonius 正确性证明与落地 | ✅ | 见 L01 `04_polonius_next_gen_borrow_checker.md`，涵盖 Datalog 事实生成、约束求解和正确性论证 |
| F03 | Verus / 定理证明集成 | ✅ | [`docs/06_research/12_verusbelt_pldi_2026.md`](../../06_research/12_verusbelt_pldi_2026.md) — PLDI 2026 研究跟踪，覆盖层叠语义、擦除定理、与 Verus 验证框架的关系 |
...
| 目标 | 新增/升级文档 | 类型 | 说明 |
|------|-------------|------|------|
| Miri UB 检测 | `docs/08_usage_guides/16_miri_practical_guide.md` | 新增 | 466 行实战指南，覆盖 10+ 常见 UB 模式 |
| Cranelift 后端 | `docs/09_toolchain/13_cranelift_backend_guide.md` | 新增 | 349 行配置与评测，含实测编译时间对比 |
| Polonius | `docs/06_research/06_polonius_next_gen_borrow_checker.md` | 新增 | 327 行深度解析，关联官方 2026 goal |
| Unsafe Fields | `docs/08_usage_guides/25_unsafe_fields_preview.md` | 新增 | 281 行实验性预览，覆盖语法和语义 |
| TOML v1.1 | `docs/09_toolchain/16_toml_v11_cargo_guide.md` | 新增 | 392 行 Cargo 集成指南 |
| VerusBelt | `docs/06_research/12_verusbelt_pldi_2026.md` | 新增 | 229 行 PLDI 2026 研究跟踪 |
| C++ 互操作 | `docs/04_guides/02_cxx_interop_guide.md` | 新增 | 151 行 cxx + bindgen 技术路线对比 |
| Rust for Linux | `docs/06_research/08_rust_for_linux.md` | 新增 | 272 行 RFL 生产化时间线与驱动开发 |
...
| 目标 | 文档路径 | 类型 | 行数 |
|------|----------|------|------|
| Miri | [`docs/08_usage_guides/16_miri_practical_guide.md`](../../08_usage_guides/16_miri_practical_guide.md) | Markdown | 466 |
| Cranelift | [`docs/09_toolchain/13_cranelift_backend_guide.md`](../../09_toolchain/13_cranelift_backend_guide.md) | Markdown | 349 |
| Polonius | [`docs/06_research/06_polonius_next_gen_borrow_checker.md`](../../06_research/06_polonius_next_gen_borrow_checker.md) | Markdown | 327 |
| TOML v1.1 | [`docs/09_toolchain/16_toml_v11_cargo_guide.md`](../../09_toolchain/16_toml_v11_cargo_guide.md) | Markdown | 392 |
| Unsafe Fields | [`docs/08_usage_guides/25_unsafe_fields_preview.md`](../../08_usage_guides/25_unsafe_fields_preview.md) | Markdown | 281 |
| VerusBelt | [`docs/06_research/12_verusbelt_pldi_2026.md`](../../06_research/12_verusbelt_pldi_2026.md) | Markdown | 229 |
| C++ 互操作 | [`docs/04_guides/02_cxx_interop_guide.md`](../../04_guides/02_cxx_interop_guide.md) | Markdown | 151 |
| Rust for Linux | [`docs/06_research/08_rust_for_linux.md`](../../06_research/08_rust_for_linux.md) | Markdown | 272 |
