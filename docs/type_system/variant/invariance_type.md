...
在 Rust 中，不变性（Invariance）通过确保类型参数的严格匹配来防止类型错误。以下是一个具体的例子，展示不变性如何防止类型错误：
...
可变性是指一个值是否可以被修改。在 Rust 中，默认情况下所有变量都是不可变的，需要使用 `mut` 关键字来声明可变变量。
...
协变是指如果类型 A 是类型 B 的子类型，那么 `F<A>` 也是 `F<B>` 的子类型。在 Rust 中，这主要体现在生命周期上：
...
逆变是指如果类型 A 是类型 B 的子类型，那么 `F<B>` 是 `F<A>` 的子类型（方向相反）。在 Rust 中，函数参数是逆变的：
...
不变性是指类型参数既不是协变也不是逆变。在 Rust 中，可变引用 `&mut T` 对于 T 是不变的：
...
可变性是指一个值是否可以被修改。在 Rust 中，默认情况下所有变量都是不可变的。
