...
- [4.2 关联类型](#42-关联类型)
  ...
  - [4.2.3 Rust中的关联类型](#423-rust中的关联类型)
  ...
  - [4.2.8 关联类型与其他语言特性的比较](#428-关联类型与其他语言特性的比较)
...
关联类型（Associated Types）是Rust类型系统中的一个重要特性，它允许在特征定义中声明依赖于实现类型的类型占位符。
与泛型参数不同，关联类型在特征实现时必须指定具体类型，从而提供了一种强大的类型抽象机制。
本节将从形式化的角度详细阐述关联类型的概念、理论基础、语义模型以及在Rust中的应用。
