...
- [Rust 类型系统：类型安全与推断](#rust-类型系统类型安全与推断)
  - [目录](#目录)
    - [1. 类型安全的静态保证](#1-类型安全的静态保证)
      ...
      - [1.2. 资源安全（编译期逻辑证明）：所有权、借用与生命周期](#12-资源安全编译期逻辑证明所有权借用与生命周期)
...
从引用一致性视角看，Rust 的类型安全并非由单一特性提供，而是其多个核心机制协同工作所 **涌现 (emergent)** 的一种属性。编译器（编译期逻辑证明系统）在编译期逻辑证明（约束求解，非静态分析）阶段，通过强制执行一系列严格规则，从根本上消除了许多传统语言中的安全漏洞。

#### 1.1. 核心原则：涌现的安全

类型安全意味着程序不会产生因类型不匹配而导致的未定义行为。在 Rust 中，这通过一个零成本的静态保证体系实现，其基石如下。

#### 1.2. 资源安全（编译期逻辑证明）：所有权、借用与生命周期

从引用一致性视角看，这是 Rust 安全性的基石，其工作机制已在 **第 1 章** 详细讨论。类型系统（构造性证明系统）在此扮演的角色是：

- **强制所有权（资源控制权的逻辑证明）**：确保任何时刻数据都有唯一所有者，防止二次释放（编译期证明的资源生命周期）。
- **验证借用（能力转移与受限授权）**：通过 `&T` 和 `&mut T` 的类型差异，在编译期逻辑证明中执行"一写或多读"的别名规则（编译期排他性契约的验证）。
- **检查生命周期（编译期构造的证明变量）**：将生命周期 `'a` 作为类型的一部分 (`&'a T`)，确保任何引用都不会比其指向的数据活得更久（编译期构造的证明变量），从而杜绝悬垂指针（逻辑证明的失败，非内存地址失效）。
