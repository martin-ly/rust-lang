...
> **[来源: [Rust Reference](https://doc.rust-lang.org/reference/)]**
>
- [Linux Foundation Certified Rust Programmer (LFRS) 认证映射 {#linux-foundation-certified-rust-programmer-lfrs-认证映射}](#linux-foundation-certified-rust-programmer-lfrs-认证映射-linux-foundation-certified-rust-programmer-lfrs-认证映射)
  - [📑 目录 {#目录}](#-目录-目录)
  - [考点总览 {#考点总览}](#考点总览-考点总览)
  - [考点 1: 语法、类型与控制流 {#考点-1-语法类型与控制流}](#考点-1-语法类型与控制流-考点-1-语法类型与控制流)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9)
  - [考点 2: Ownership、Borrowing 与 Lifetimes {#考点-2-ownershipborrowing-与-lifetimes}](#考点-2-ownershipborrowing-与-lifetimes-考点-2-ownershipborrowing-与-lifetimes)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-1)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-1)
  - [考点 3: Structs、Enums 与模式匹配 {#考点-3-structsenums-与模式匹配}](#考点-3-structsenums-与模式匹配-考点-3-structsenums-与模式匹配)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-2)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-2)
  - [考点 4: 模块与 Crates {#考点-4-模块与-crates}](#考点-4-模块与-crates-考点-4-模块与-crates)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-3)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-3)
  - [考点 5: 集合与字符串 {#考点-5-集合与字符串}](#考点-5-集合与字符串-考点-5-集合与字符串)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-4)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-4)
  - [考点 6: 错误处理 {#考点-6-错误处理}](#考点-6-错误处理-考点-6-错误处理)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-5)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-5)
  - [考点 7: Traits 与泛型 {#考点-7-traits-与泛型}](#考点-7-traits-与泛型-考点-7-traits-与泛型)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-6)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-6)
  - [考点 8: 测试 {#考点-8-测试}](#考点-8-测试-考点-8-测试)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-7)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-7)
  - [考点 9: 闭包、迭代器与 Smart Pointers {#考点-9-闭包迭代器与-smart-pointers}](#考点-9-闭包迭代器与-smart-pointers-考点-9-闭包迭代器与-smart-pointers)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-8)
    - [补充计划 {#补充计划}](#补充计划-补充计划)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-8)
  - [考点 10: 并发（线程 / 通道 / async） {#考点-10-并发线程-通道-async}](#考点-10-并发线程--通道--async-考点-10-并发线程-通道-async)
    - [映射文件 {#映射文件-9}](#映射文件-映射文件-9-9)
    - [缺失内容 {#缺失内容}](#缺失内容-缺失内容)
    - [练习验证 {#练习验证-9}](#练习验证-练习验证-9-9)
  - [学习路径建议 {#学习路径建议}](#学习路径建议-学习路径建议)
    - [路径 A：按 LFRS 考点顺序学习 {#路径-a按-lfrs-考点顺序学习}](#路径-a按-lfrs-考点顺序学习-路径-a按-lfrs-考点顺序学习)
    - [路径 B：以练习驱动 {#路径-b以练习驱动}](#路径-b以练习驱动-路径-b以练习驱动)
  - [量化评估标准 {#量化评估标准}](#量化评估标准-量化评估标准)
  - [相关文档 {#相关文档}](#相关文档-相关文档)
  - [相关概念 {#相关概念}](#相关概念-相关概念)
...
| 注释与文档 | `docs/03_reference/quick_reference/09_control_flow_functions_cheatsheet.md` | 速查卡 |
...
| 路径系统 | `docs/03_reference/quick_reference/12_modules_cheatsheet.md` | 绝对路径、相对路径、super |
...
| 迭代器 | `docs/03_reference/quick_reference/08_collections_iterators_cheatsheet.md` | 迭代器适配器 |
...
| 属性测试 | `docs/04_guides/05_fuzzing_guide.md` | proptest 使用指南 |
| 基准测试 | `crates/c08_algorithms/src/performance_benchmarks.rs` | Criterion 实例 |
| Miri | `docs/04_guides/09_miri_guide.md` | 未定义行为检测 |
| 覆盖率 | `docs/04_guides/14_test_coverage.md` | tarpaulin 使用 |
...
| 迭代器 | `docs/03_reference/quick_reference/08_collections_iterators_cheatsheet.md` | 适配器、消费者 | ✅ |
...
4. 考点 4 → 阅读本项目 workspace 结构 + `docs/03_reference/quick_reference/12_modules_cheatsheet.md`
...
- [03_google_rust_mapping.md](03_google_rust_mapping.md) - Google Comprehensive Rust 映射
- [05_pragmatic_guidelines_checklist.md](../08_usage_guides/19_pragmatic_guidelines_checklist.md) - 代码审查清单
