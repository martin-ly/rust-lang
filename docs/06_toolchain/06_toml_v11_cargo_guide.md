...
- [Cargo 与 TOML v1.1 新特性指南 {#cargo-与-toml-v11-新特性指南}](#cargo-与-toml-v11-新特性指南-cargo-与-toml-v11-新特性指南)
  - [目录 {#目录}](#目录-目录)
  - [1. TOML 版本演进 {#1-toml-版本演进}](#1-toml-版本演进-1-toml-版本演进)
    - [为什么 Cargo 需要跟进 TOML v1.1 {#为什么-cargo-需要跟进-toml-v11}](#为什么-cargo-需要跟进-toml-v11-为什么-cargo-需要跟进-toml-v11)
  - [2. TOML v1.1 核心新特性 {#2-toml-v11-核心新特性}](#2-toml-v11-核心新特性-2-toml-v11-核心新特性)
    - [2.1 多行内联表 (Multi-line Inline Tables) {#21-多行内联表-multi-line-inline-tables}](#21-多行内联表-multi-line-inline-tables-21-多行内联表-multi-line-inline-tables)
    - [2.2 尾部逗号 (Trailing Commas) {#22-尾部逗号-trailing-commas}](#22-尾部逗号-trailing-commas-22-尾部逗号-trailing-commas)
    - [2.3 扩展裸键规则 (Extended Bare Keys) {#23-扩展裸键规则-extended-bare-keys}](#23-扩展裸键规则-extended-bare-keys-23-扩展裸键规则-extended-bare-keys)
  - [3. 在 Cargo.toml 中的实际应用 {#3-在-cargotoml-中的实际应用}](#3-在-cargotoml-中的实际应用-3-在-cargotoml-中的实际应用)
    - [3.1 复杂依赖配置 {#31-复杂依赖配置}](#31-复杂依赖配置-31-复杂依赖配置)
    - [3.2 Workspace 成员配置 {#32-workspace-成员配置}](#32-workspace-成员配置-32-workspace-成员配置)
    - [3.3 Profile 配置优化 {#33-profile-配置优化}](#33-profile-配置优化-33-profile-配置优化)
  - [4. 与 TOML v1.0 的兼容性 {#4-与-toml-v10-的兼容性}](#4-与-toml-v10-的兼容性-4-与-toml-v10-的兼容性)
    - [4.1 向后兼容性保证 {#41-向后兼容性保证}](#41-向后兼容性保证-41-向后兼容性保证)
    - [4.2 团队协作注意事项 {#42-团队协作注意事项}](#42-团队协作注意事项-42-团队协作注意事项)
  - [5. Cargo 1.96 的 TOML 解析改进 {#5-cargo-196-的-toml-解析改进}](#5-cargo-196-的-toml-解析改进-5-cargo-196-的-toml-解析改进)
    - [5.1 底层解析器升级 {#51-底层解析器升级}](#51-底层解析器升级-51-底层解析器升级)
    - [5.2 `cargo add` 的行为变化 {#52-cargo-add-的行为变化}](#52-cargo-add-的行为变化-52-cargo-add-的行为变化)
    - [5.3 诊断改进 {#53-诊断改进}](#53-诊断改进-53-诊断改进)
  - [6. 最佳实践与反模式 {#6-最佳实践与反模式}](#6-最佳实践与反模式-6-最佳实践与反模式)
    - [6.1 ✅ 推荐做法 {#61-推荐做法}](#61--推荐做法-61-推荐做法)
    - [6.2 ❌ 避免的做法 {#62-避免的做法}](#62--避免的做法-62-避免的做法)
    - [6.3 决策流 {#63-决策流}](#63-决策流-63-决策流)
  - [7. 参考文献 {#7-参考文献}](#7-参考文献-7-参考文献)
    - [TOML 规范 {#toml-规范}](#toml-规范-toml-规范)
    - [Cargo 文档 {#cargo-文档}](#cargo-文档-cargo-文档)
    - [相关工具 {#相关工具}](#相关工具-相关工具)
  - [复查记录 {#复查记录}](#复查记录-复查记录)
...
```toml
# ✅ TOML v1.1 允许尾部逗号 — 减少 git diff 噪音 {#toml-v11-允许尾部逗号-减少-git-diff-噪音}
[dependencies]
...
```diff
# 无尾部逗号的 diff（2 行变更） {#无尾部逗号的-diff2-行变更}
-    "macros"
+    "macros",
+    "sync"
...
```toml
# ✅ TOML v1.1 允许更多字符作为裸键 {#toml-v11-允许更多字符作为裸键}
...
# 多行内联表使 workspace 依赖更易维护 {#多行内联表使-workspace-依赖更易维护}
...
```bash
# Cargo 1.96+ 默认使用多行格式添加复杂依赖 {#cargo-196-默认使用多行格式添加复杂依赖}
cargo add tokio --features rt-multi-thread,macros,sync

# Cargo.toml 结果（TOML v1.1 格式） {#cargotoml-结果toml-v11-格式}
[dependencies]
tokio = { version = "1.43", features = [
    "rt-multi-thread",
    "macros",
...
```bash
# TOML 解析错误现在提供更精确的定位 {#toml-解析错误现在提供更精确的定位}
$ cargo check
...
# 2. 数组始终使用尾部逗号 {#2-数组始终使用尾部逗号}
exclude = [
    "target/",
    "*.log",
]

# 3. Workspace 依赖集中管理 {#3-workspace-依赖集中管理}
[workspace.dependencies]
...
```toml
# 1. 不要为了多行而多行 — 简单配置保持单行 {#1-不要为了多行而多行-简单配置保持单行}
[dependencies]
# ❌ 过度展开 {#过度展开}
serde = {
    version = "1.0",
...
# 2. 不要在需要 TOML v1.0 兼容的项目中使用新语法 {#2-不要在需要-toml-v10-兼容的项目中使用新语法}
# （MSRV < 1.96 的开源库） {#msrv-196-的开源库}

# 3. 避免 Unicode 键名（即使 TOML v1.1 允许） {#3-避免-unicode-键名即使-toml-v11-允许}
# ❌ 不推荐 {#不推荐}
[profile.发布]
opt-level = 3
```
