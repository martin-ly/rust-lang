# Rustdoc 1.96 改进深度解析 {#rustdoc-196-改进深度解析}
...
## 二、Deprecation Notes 渲染方式变更 {#二deprecation-notes-渲染方式变更}

### 2.1 变更前行为（< 1.96） {#21-变更前行为-196}
...
### 2.3 对 Crate 作者的影响 {#23-对-crate-作者的影响}
...
## 三、`missing_doc_code_examples` Lint 范围缩小 {#三missing_doc_code_examples-lint-范围缩小}
...
### 3.3 为什么做这个变更 {#33-为什么做这个变更}

1. **减少误报**: impl items 的方法通常已有 trait 文档中的示例，重复添加示例增加维护负担
2. **DRY 原则**: 文档应集中于定义处（trait），而非每个实现处重复
3. **实际反馈**: 社区报告 impl items 上的 lint 过于嘈杂

### 3.4 对项目的影响 {#34-对项目的影响}

```rust
# 1.96 之前: 可能需要在每个 impl 块中添加重复示例 {#196-之前-可能需要在每个-impl-块中添加重复示例}
# 1.96 之后: 只需在 trait 定义处添加示例 {#196-之后-只需在-trait-定义处添加示例}

# 示例: 移除不必要的 impl item 文档示例后，lint 不再报错 {#示例-移除不必要的-impl-item-文档示例后lint-不再报错}
pub trait Drawable {
    /// 绘制此对象
    ///
    /// # Examples
