> **版本状态声明**:
>
> 本文档覆盖 **Rust 1.96.0 (2026-05-28)** 稳定版内容，含 1.95 关键特性回顾。
> **分级**: [A]
>
...
# Rust 1.96 稳定特性全景 {#rust-196-稳定特性全景}
...
## Rust 1.95 新特性（回顾） {#rust-195-新特性回顾}

> **[来源: Rust 1.95 Release Notes](https://github.com/rust-lang/rust/releases/tag/1.95.0)**

### 1. `if let` 守卫 (if let guards) {#1-if-let-守卫-if-let-guards}
...
### 2. `RangeInclusive` 迭代性能优化 {#2-rangeinclusive-迭代性能优化}

Rust 1.95 对 `RangeInclusive` 的迭代性能进行了内部优化。

---

## Rust 1.96 稳定特性 {#rust-196-稳定特性}
...
#### 3. `expr` Metavariable 传递给 `cfg` {#3-expr-metavariable-传递给-cfg}
...
#### 4. Never 类型在 Tuple 中强制转换 {#4-never-类型在-tuple-中强制转换}
...
#### 5. `ManuallyDrop` 常量作为模式 {#5-manuallydrop-常量作为模式}
...
#### 6. s390x Vector Registers 内联汇编 {#6-s390x-vector-registers-内联汇编}

Rust 1.96 在 `s390x` 架构上稳定了向量寄存器（`v0`–`v31`）的内联汇编输入/输出支持。
此前这些向量寄存器只能作为 clobber 使用，现在可以作为 `vreg` 约束参与数据传递，
便于直接调用 SIMD/向量指令。
...
/// 在 s390x 上通过向量寄存器将 128 位值清零。
/// 需要启用 `vector` target feature。
#[cfg(target_arch = "s390x")]
#[target_feature(enable = "vector")]
unsafe fn s390x_vector_xor_zero(mut v: u128) -> u128 {
    unsafe {
        asm!(
            "vx {0}, {0}, {0}",   // v = v ^ v ^ v，结果为零
            inout(vreg) v,
...
> **说明**: `#[cfg(target_arch = "s390x")]` 保证该代码在非 s390x 平台上被完全忽略，
> 因此不会影响其他架构的编译；实际在 s390x 上运行时需目标支持 `vector` 特性。
...
#### 7. `assert_matches!` / `debug_assert_matches!` {#7-assert_matches-debug_assert_matches}
...
#### 8. `core::range` 类型族 {#8-corerange-类型族}
...
#### 9. `From<T>` for `LazyLock<T, F>` / `LazyCell<T, F>` / `AssertUnwindSafe<T>` {#9-fromt-for-lazylockt-f-lazycellt-f-assertunwindsafet}
...
#### 9a. `LazyLock` / `LazyCell` 访问器方法 (`get`, `get_mut`, `force_mut`) {#9a-lazylock-lazycell-访问器方法-get-get_mut-force_mut}
...
#### 10. `NonZero` 整数范围迭代 {#10-nonzero-整数范围迭代}
...
#### 11. Git + Alternate Registry 共存 {#11-git-alternate-registry-共存}
...
#### 12. `target.'cfg(..)'.rustdocflags` {#12-targetcfgrustdocflags}
...
```bash
# 环境变量优先级高于 cargo 配置 {#环境变量优先级高于-cargo-配置}
RUSTDOCFLAGS="--cfg custom" cargo doc
```

Cargo 配置中的 `rustdocflags` 与 `RUSTDOCFLAGS` **合并**而非覆盖，但环境变量的标志在命令行中更靠后，可能覆盖同名配置。

#### 13. 安全修复：CVE-2026-5222 / CVE-2026-5223 {#13-安全修复cve-2026-5222-cve-2026-5223}
...
#### 指针 “valid for read/write” 契约重构 {#指针-valid-for-readwrite-契约重构}

Rust 1.96 对标准库中“指针有效可读/可写”（valid for read/write）的定义进行了重构：

- **基础定义现在不包含 null**：按照新的统一约定，一个指针要“valid for read/write”，
  首先必须是非空指针；null 指针不再被默认视为有效。
- **null 例外下沉到具体方法**：对于历史上明确允许 null 的特殊场景
  （例如零大小类型 `T` 的 `<*const T>::read` / `<*mut T>::write`），
  标准库在相应方法的 Safety 说明中单独注明 null 例外，而不是让基础定义包容 null。

这只是一次文档/契约层面的澄清，绝大多数安全代码的行为没有变化；
但写 `unsafe` 代码时应以具体方法的 Safety 说明为准，而不要假设“null 天然可读可写”。
...
fn read_or_default<T: Copy>(ptr: *const T, fallback: T) -> T {
    if ptr.is_null() {
        // 不要对 null 调用 read；依赖具体方法是否允许 null 例外
        fallback
    } else {
        // SAFETY: 调用前已确认指针非空，且调用者保证其有效、对齐
        unsafe { ptr.read() }
    }
}

fn write_if_non_null<T>(ptr: *mut T, value: T) {
    if ptr.is_null() {
        // 处理方式取决于 API 约定；通常直接返回或报错
        return;
    }
    // SAFETY: 调用前已确认指针非空，且调用者保证其有效、对齐、可写
    unsafe { ptr.write(value); }
}
```

> **注意**: 如果某个方法文档明确说明“允许 null（通常针对 ZST）”，则仍然可以按文档执行；
> 否则应始终将 null 视为无效指针。

#### JSON Target Spec 内部变更 {#json-target-spec-内部变更}

Rust 的目标规范在内部以 JSON 描述（自定义目标通过 `--target foo.json` 加载）。
Rust 1.96 对内部 JSON target spec 做了以下收紧/调整，主要影响使用自定义目标或维护目标定义的开发者：

- `aarch64` 软浮点目标现在必须显式设置 `"rustc_abi": "softfloat"`。
- 对 `llvm-target` 等 ABI 相关值的校验更严格，并与 `cfg(target_abi)` 的取值保持一致。

这些变更**不改变**普通 Rust 代码的公开接口，但构建自定义目标时需要检查目标 JSON 是否仍能通过校验。
建议始终使用与当前 rustc 版本配套的目标 JSON schema：

```bash
rustc +nightly -Z unstable-options --print target-spec-json-schema
```

> **[来源: The rustc book - Custom Targets](https://doc.rust-lang.org/rustc/targets/custom.html)**
...
| s390x 向量寄存器内联汇编 | `docs/06_toolchain/06_19_rust_1_96_features.md` | — | 文档 |
| 指针 valid for read/write 契约重构 | `docs/06_toolchain/06_19_rust_1_96_features.md` | — | 文档 |
| JSON Target Spec 内部变更 | `docs/06_toolchain/06_19_rust_1_96_features.md` | — | 文档 |

## 与 1.95 特性对比 {#与-195-特性对比}
...
>
> **权威来源对齐变更日志**: 2026-05-29 全面重写 1.96 部分，删除未进入 stable 的虚假特性（`truncate_front`、`int_format_into`、`RefCell::try_map`、`proc_macro_value` 等），补充实际稳定内容 [来源: Official Release Notes]；2026-06-25 补充 s390x 向量寄存器内联汇编、指针 valid for read/write 契约重构、JSON Target Spec 内部变更 [来源: Official Release Notes / Rust Reference]
