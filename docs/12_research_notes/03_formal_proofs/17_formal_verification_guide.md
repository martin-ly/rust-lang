...
> **迁移说明**: 本文件于 2026-07-12 自归档 `archive/research_notes_2026_06_25/` 迁移为活跃文件（原归档路径被活跃文档引用，归档治理 ARCHIVE_GOVERNANCE_2026_07_12）。
...
- [形式化工具验证指南 {#形式化工具验证指南}](#形式化工具验证指南-形式化工具验证指南)
  - [📑 目录 {#目录}](#-目录-目录)
  - [🎯 概述 {#概述}](#-概述-概述)
    - [验证目标 {#验证目标}](#验证目标-验证目标)
  - [形式化论证（验证框架） {#形式化论证验证框架}](#形式化论证验证框架-形式化论证验证框架)
  - [🛠️ 工具选择 {#工具选择}](#️-工具选择-工具选择)
    - [Coq {#coq}](#coq-coq)
    - [Isabelle/HOL {#isabellehol}](#isabellehol-isabellehol)
  - [📚 验证准备工作 {#验证准备工作}](#-验证准备工作-验证准备工作)
    - [1. 环境准备 {#1-环境准备}](#1-环境准备-1-环境准备)
    - [2. 理论准备 {#2-理论准备}](#2-理论准备-2-理论准备)
    - [3. 验证框架设计 {#3-验证框架设计}](#3-验证框架设计-3-验证框架设计)
  - [🔬 验证实施步骤 {#验证实施步骤}](#-验证实施步骤-验证实施步骤)
    - [步骤 1: 所有权模型验证 {#步骤-1-所有权模型验证}](#步骤-1-所有权模型验证-步骤-1-所有权模型验证)
    - [步骤 2: 借用检查器验证 {#步骤-2-借用检查器验证}](#步骤-2-借用检查器验证-步骤-2-借用检查器验证)
    - [步骤 3: 生命周期验证 {#步骤-3-生命周期验证}](#步骤-3-生命周期验证-步骤-3-生命周期验证)
    - [步骤 4: 类型系统验证 {#步骤-4-类型系统验证}](#步骤-4-类型系统验证-步骤-4-类型系统验证)
    - [步骤 5: 异步状态机验证 {#步骤-5-异步状态机验证}](#步骤-5-异步状态机验证-步骤-5-异步状态机验证)
    - [步骤 6: Pin 与自引用验证 {#步骤-6-pin-与自引用验证}](#步骤-6-pin-与自引用验证-步骤-6-pin-与自引用验证)
  - [📋 验证任务清单 {#验证任务清单}](#-验证任务清单-验证任务清单)
    - [所有权模型验证 {#所有权模型验证}](#所有权模型验证-所有权模型验证)
    - [借用检查器验证 {#借用检查器验证}](#借用检查器验证-借用检查器验证)
    - [生命周期验证 {#生命周期验证}](#生命周期验证-生命周期验证)
    - [类型系统验证 {#类型系统验证}](#类型系统验证-类型系统验证)
    - [异步状态机验证 {#异步状态机验证}](#异步状态机验证-异步状态机验证)
    - [Pin 与自引用验证 {#pin-与自引用验证}](#pin-与自引用验证-pin-与自引用验证)
  - [🔗 相关资源 {#相关资源}](#-相关资源-相关资源)
    - [学习资源 {#学习资源}](#学习资源-学习资源)
    - [相关项目 {#相关项目}](#相关项目-相关项目)
    - [工具链扩展任务（Rust → 证明助手） {#工具链扩展任务rust-证明助手}](#工具链扩展任务rust--证明助手-工具链扩展任务rust-证明助手)
    - [工具资源 {#工具资源}](#工具资源-工具资源)
...
**Coq 证明骨架**：所有权唯一性（T-OW2）的 Coq 定理骨架已创建，见 [coq_skeleton/OWNERSHIP_UNIQUENESS.v](../deprecated/coq_skeleton/OWNERSHIP_UNIQUENESS.v)、[COQ_ISABELLE_PROOF_SCAFFOLDING](10_coq_isabelle_proof_scaffolding.md)。
...
**定理 FV-T1（验证与定理衔接）**：若 $V$ 验证 $T$，则 $T$ 的证明在验证工具的逻辑框架内成立；与 [PROOF_INDEX](21_proof_index.md) 的手工证明互为补充。

*证明*：由 Def FV1；机器验证保证证明结构正确；手工证明提供直觉与文档；二者一致则论证完备。∎

**引理 FV-L1（六类验证覆盖）**：所有权、借用、生命周期、类型系统、异步、Pin 六类验证覆盖 Rust 核心安全机制；与 [formal_methods/README](../02_formal_methods/README.md) FM-T1 对应。
...
**推论 FV-C1**：验证任务清单完成 ⇔ 六类均有机器可检查证明；与 [ARGUMENTATION_GAP_INDEX](../06_concept_models/03_argumentation_gap_index.md) 论证缺口追踪衔接。
...
- [ ] 生命周期的形式化定义
- [ ] 类型系统的形式化定义
- [ ] 异步状态机的形式化定义（参见 [10_async_state_machine.md](../02_formal_methods/02_async_state_machine.md)）
- [ ] Pin 与自引用类型的形式化定义（参见 [10_pin_self_referential.md](../02_formal_methods/10_pin_self_referential.md)）
...
**目标**：验证 Future 状态一致性、并发安全与进度保证（参见 [10_async_state_machine.md](../02_formal_methods/02_async_state_machine.md)）
...
**目标**：验证 Pin 保证、自引用类型安全与 Pin 投影安全（参见 [10_pin_self_referential.md](../02_formal_methods/10_pin_self_referential.md)）
...
| 工具 | 输入 | 输出 | 对接状态 | 任务入口 |
| :--- | :--- | :--- | :--- | :--- |
| **Aeneas** | Safe Rust (MIR/THIR) | Coq/F*/HOL4/Lean | 📋 计划中 | [AENEAS_INTEGRATION_PLAN](01_aeneas_integration_plan.md) |
| **coq-of-rust** | THIR | Rocq (Coq) | 📋 计划中 | [COQ_OF_RUST_INTEGRATION_PLAN](04_coq_of_rust_integration_plan.md) |

完成 Aeneas/coq-of-rust 对接后，在 [PROOF_INDEX](21_proof_index.md) 中标注对应定理为 L3（机器可检查）。
