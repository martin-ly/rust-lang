...
**Axiom BP1**：形式化完备的研究笔记与 [PROOF_INDEX](../03_formal_proofs/21_proof_index.md) 的论证链一致；公理→引理→定理→推论可追溯。

**定理 BP-T1（完备性蕴涵可追溯）**：若 $N$ 满足 Def BP1，则 $N$ 的核心定理可追溯至 [FORMAL_PROOF_SYSTEM_GUIDE](../03_formal_proofs/16_formal_proof_system_guide.md) 的概念-公理-定理映射。
...
[FORMAL_PROOF_CRITICAL_ANALYSIS_AND_PLAN_2026_02](../03_formal_proofs/15_formal_proof_critical_analysis_and_plan_2026_02.md) 批判性分析。
...
与 [formal_methods](../02_formal_methods/README.md)、
[type_theory](../05_type_theory/README.md)、
[PROOF_INDEX](../03_formal_proofs/21_proof_index.md) 的交叉引用。

**实践对照表**：

| 检查项 | 示例 | 形式化链接 |
| :--- | :--- | :--- |
| **形式化** | Def OW1、定理 T2 | [ownership_model](../02_formal_methods/09_ownership_model.md) |
| **代码** | `let s = String::from("x"); take_ownership(s);` | 对应 T2 移动语义 |
| **场景** | 所有权（Ownership）转移、借用（Borrowing）、生命周期（Lifetimes） | [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) |
| **反例** | 双重可变借用（Mutable Borrow）、悬垂引用 | [SAFE_UNSAFE_COMPREHENSIVE_ANALYSIS](../03_formal_proofs/28_safe_unsafe_comprehensive_analysis.md) |
| **衔接** | 定理引用 PROOF_INDEX | [PROOF_INDEX](../03_formal_proofs/21_proof_index.md) |
...
无与 [formal_methods](../02_formal_methods/README.md)/[type_theory](../05_type_theory/README.md)/[PROOF_INDEX](../03_formal_proofs/21_proof_index.md) 的交叉引用。
...
1. **补形式化**：为核心概念添加 Def（含符号、前置条件、结论）；为结论添加定理及证明思路；引用 [PROOF_INDEX](../03_formal_proofs/21_proof_index.md) 中已有证明。
...
**逐文档自检**：按 [CONTENT_ENHANCEMENT](../13_meta_reports/08_content_enhancement.md) 实质内容检查清单逐项核对；若任一项为空或仅占位，则需修复。
见 [FAQ Q2.5](05_faq.md#q25-如何判断研究笔记是否有实质内容) 快速判断指引。
...
```markdown
有关所有权模型的详细讨论，请参见 [所有权模型形式化](../02_formal_methods/09_ownership_model.md)。

有关类型系统的更多信息，请参考 [类型系统基础](../05_type_theory/05_type_system_foundations.md)。
```
...
```markdown
✅ 正确: [研究路线图](../13_meta_reports/15_research_roadmap.md)
❌ 错误: 研究路线图
```
...
```markdown
✅ 正确: 查看 [研究路线图](../13_meta_reports/15_research_roadmap.md) 了解研究计划
❌ 错误: 点击 [这里](../13_meta_reports/15_research_roadmap.md)
```
...
```bash
# 检查链接有效性 {#检查链接有效性}
find docs/12_research_notes -name "*.md" -exec grep -l "\[.*\](.*)" {} \;
...
- [所有权模型形式化](../02_formal_methods/09_ownership_model.md) - 所有权模型的形式化定义
- [借用检查器证明](../02_formal_methods/03_borrow_checker_proof.md) - 借用检查器的正确性证明
```
...
- ✅ 使用 [质量检查清单](../13_meta_reports/11_quality_checklist.md)
- ✅ 逐项检查质量要求
- ✅ 确保所有要求满足
