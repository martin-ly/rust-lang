...
**定理 PA-T1（案例与定理衔接）**：若案例 $C$ 与 [ownership_model](../02_formal_methods/09_ownership_model.md) T2/T3、
[borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) T1、
[async_state_machine](../02_formal_methods/02_async_state_machine.md) T6.2 一致，则 $C$ 为 Safe 且满足形式化保证。

*证明*：由各定理陈述；案例实现满足定理结论即一致；
组合见 [COMPREHENSIVE_SYSTEMATIC_OVERVIEW](../13_meta_reports/06_comprehensive_systematic_overview.md) CSO-T1。∎

**引理 PA-L1（unsafe 案例边界）**：若案例 $C$ 含 `unsafe` 块，则 $C$ 与定理一致 ⟺ $C$ 的 unsafe 使用满足 [SAFE_UNSAFE_COMPREHENSIVE_ANALYSIS](../03_formal_proofs/28_safe_unsafe_comprehensive_analysis.md) 契约；安全抽象对外 API 为 Safe。

*证明*：由 Def PA1；unsafe 块引入契约；安全抽象将 unsafe 封装，对外满足 Safe 规则；契约满足则与定理一致。∎

**推论 PA-C1**：案例分析可引用（Reference） [PROOF_INDEX](../03_formal_proofs/21_proof_index.md) 与 [FORMAL_PROOF_SYSTEM_GUIDE](../03_formal_proofs/16_formal_proof_system_guide.md) 的论证链，建立与实际项目的追溯关系。
...
- **落地**：按「最佳实践总结」的四个领域逐条对照；异步、错误处理、并发原语选型可结合 [async_state_machine](../02_formal_methods/02_async_state_machine.md)、[concurrency_performance](../09_experiments/02_concurrency_performance.md)。
...
- **所有权模型** [10_ownership_model.md](../02_formal_methods/09_ownership_model.md)：Redox、Tock、Firecracker、SafeVec 等案例中的资源管理与 `unsafe` 边界，可对照所有权规则做形式化抽查。
- **借用检查器** [10_borrow_checker_proof.md](../02_formal_methods/03_borrow_checker_proof.md)：TiKV、Actix、Linkerd 等并发与迭代场景，可对照借用规则验证无数据竞争。
- **异步状态机** [10_async_state_machine.md](../02_formal_methods/02_async_state_machine.md)：Tokio、Actix、ScyllaDB 驱动、案例 1–2 的 async 设计，可对应 Future/Poll/Waker 形式化。

### 与类型理论、实验研究的关联 {#与类型理论实验研究的关联}

> **来源: [Wikipedia - Rust (programming language)](https://en.wikipedia.org/wiki/Rust_(programming_language))**

- **类型系统（Type System） / Trait** [10_type_system_foundations.md](../05_type_theory/05_type_system_foundations.md)、[10_trait_system_formalization.md](../05_type_theory/04_trait_system_formalization.md)：各案例中的泛型（Generics）、`impl Trait`、派生与 Trait 对象，可作类型论与 Trait 形式化的实例。
- **性能基准** [10_performance_benchmarks.md](../09_experiments/05_performance_benchmarks.md)、**并发性能** [10_concurrency_performance.md](../09_experiments/02_concurrency_performance.md)：案例 1–2 的吞吐、延迟、并发模式可与实验的「结果分析模板」对照，用于选型与调优。
- **内存分析** [10_memory_analysis.md](../09_experiments/04_memory_analysis.md)、**编译器优化** [10_compiler_optimizations.md](../09_experiments/01_compiler_optimizations.md)：案例中的分配策略、`-O2`/LTO 等可与实验指南结合，做上线前检查。
...
| 案例 | 形式化定理 | 衔接要点 |
| :--- | :--- | :--- |
| Redox, Tock, Firecracker | [ownership_model](../02_formal_methods/09_ownership_model.md) T2/T3、BOX-T1 | 资源 RAII、唯一所有权、无双重释放 |
| Tokio, Actix, ScyllaDB | [async_state_machine](../02_formal_methods/02_async_state_machine.md) T6.1–T6.3、SPAWN-T1 | Future、Send/Sync、无数据竞争 |
| TiKV, Linkerd | [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) T1、CHAN-T1、MUTEX-T1 | 通道、锁、借用规则 |
...
| 所有案例 | [type_system_foundations](../05_type_theory/05_type_system_foundations.md) T1–T3 | 良型、进展性、保持性 |
| 组合案例 | [04_compositional_engineering](../08_software_design_theory/05_compositional_engineering/README.md) CE-T1–T3 | 模块（Module）组合、CE-T1/T2/T3 |
| unsafe 案例 | [SAFE_UNSAFE_COMPREHENSIVE_ANALYSIS](../03_formal_proofs/28_safe_unsafe_comprehensive_analysis.md)、PA-L1 | 安全抽象、契约 |

**引用**：案例分析可引用 [PROOF_INDEX](../03_formal_proofs/21_proof_index.md) 建立与形式化定理的追溯关系；见 PA-T1、PA-L1、PA-C1。
