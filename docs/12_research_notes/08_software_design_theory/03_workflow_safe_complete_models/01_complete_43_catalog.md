...
- [43 种完全模型索引 {#43-种完全模型索引}](#43-种完全模型索引-43-种完全模型索引)
  - [📑 目录 {#目录}](#-目录-目录)
  - [定义 {#定义}](#定义-定义)
  - [构成方案 {#构成方案}](#构成方案-构成方案)
    - [扩展模式（20） {#扩展模式20}](#扩展模式20-扩展模式20)
    - [扩展模式简要说明 {#扩展模式简要说明}](#扩展模式简要说明-扩展模式简要说明)
  - [扩展模式 Rust 代码示例 {#扩展模式-rust-代码示例}](#扩展模式-rust-代码示例-扩展模式-rust-代码示例)
    - [Domain Model {#domain-model}](#domain-model-domain-model)
    - [Unit of Work {#unit-of-work}](#unit-of-work-unit-of-work)
    - [Data Mapper {#data-mapper}](#data-mapper-data-mapper)
    - [Value Object {#value-object}](#value-object-value-object)
    - [Registry (Service Locator) {#registry-service-locator}](#registry-service-locator-registry-service-locator)
    - [Identity Map {#identity-map}](#identity-map-identity-map)
    - [Service Layer {#service-layer}](#service-layer-service-layer)
    - [Repository {#repository}](#repository-repository)
    - [DTO {#dto}](#dto-dto)
    - [Event Sourcing {#event-sourcing}](#event-sourcing-event-sourcing)
    - [Specification {#specification}](#specification-specification)
    - [Table Data Gateway (DAO) {#table-data-gateway-dao}](#table-data-gateway-dao-table-data-gateway-dao)
    - [Active Record {#active-record}](#active-record-active-record)
    - [Gateway（外部系统集成） {#gateway外部系统集成}](#gateway外部系统集成-gateway外部系统集成)
    - [Model View Controller (MVC) {#model-view-controller-mvc}](#model-view-controller-mvc-model-view-controller-mvc)
    - [Front Controller {#front-controller}](#front-controller-front-controller)
    - [Remote Facade {#remote-facade}](#remote-facade-remote-facade)
    - [Lazy Load {#lazy-load}](#lazy-load-lazy-load)
    - [Plugin (Dependency Injection) {#plugin-dependency-injection}](#plugin-dependency-injection-plugin-dependency-injection)
    - [Optimistic Offline Lock {#optimistic-offline-lock}](#optimistic-offline-lock-optimistic-offline-lock)
  - [安全边界 {#安全边界}](#安全边界-安全边界)
  - [与 23 安全的关系 {#与-23-安全的关系}](#与-23-安全的关系-与-23-安全的关系)
  - [与 23 安全的分层关系 {#与-23-安全的分层关系}](#与-23-安全的分层关系-与-23-安全的分层关系)
  - [扩展模式选型 {#扩展模式选型}](#扩展模式选型-扩展模式选型)
  - [扩展模式选型决策树 {#扩展模式选型决策树}](#扩展模式选型决策树-扩展模式选型决策树)
  - [扩展模式形式化对应（深入） {#扩展模式形式化对应深入}](#扩展模式形式化对应深入-扩展模式形式化对应深入)
  - [扩展模式典型场景（实质内容） {#扩展模式典型场景实质内容}](#扩展模式典型场景实质内容-扩展模式典型场景实质内容)
  - [权威来源 {#权威来源}](#权威来源-权威来源)
...
// 或 AtomicU64 compare_exchange；见 [ownership_model](../../02_formal_methods/09_ownership_model.md) Def ATOMIC1
```
...
| 模式 | 形式化对应 | 与 23 安全组合 |
| :--- | :--- | :--- |
| Domain Model | 结构体（Struct） + 方法；无贫血；见 [ownership_model](../../02_formal_methods/09_ownership_model.md) 规则 1–3 | 与 State、Strategy 组合 |
| Service Layer | 模块（Module）依赖、trait 组合；见 [03_integration_theory](../05_compositional_engineering/04_integration_theory.md) IT-T1 | 编排 Repository、Factory |
| Repository | 见 [02_effectiveness_proofs](../05_compositional_engineering/02_effectiveness_proofs.md) CE-T1；trait 泛型（Generics）约束 | 可与 Factory Method、Builder 组合 |
| Unit of Work | 批量提交；所有权收集；见 ownership 规则 3 drop 顺序 | 与 Repository、Data Mapper 组合 |
| Data Mapper | `From`/`Into` 转换；所有权（Ownership）转移；见 [ownership_model](../../02_formal_methods/09_ownership_model.md) | 与 Repository 组合 |
| Table Data Gateway | 表级 API；`async fn`；见 [async_state_machine](../../02_formal_methods/02_async_state_machine.md) | 与 Repository 二选一 |
| Active Record | 对象持 Connection；`save`/`load`；见 ownership 规则 2 | 简单 CRUD；与 DTO 区别：有行为 |
| Gateway | trait + FFI/HTTP；见 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) Def EXTERN1 | 外部集成；FFI 时可能 unsafe |
| MVC | 模块分层；见 [05_boundary_system](../06_boundary_system/README.md) | 与 Front Controller 组合 |
...
| Remote Facade | 粗粒度接口；batch 减少 RPC；见 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) CHAN1 | 与 DTO 组合 |
| Value Object | `Clone`、`PartialEq`；不可变；见 [06_rust_idioms](../01_rust_idioms.md) Def NW1 | 与 Newtype、DTO 衔接 |
| Registry | `OnceLock<HashMap>`；见 [singleton](../01_design_patterns_formal/01_creational/05_singleton.md) | 服务定位；与 Plugin 二选一 |
| Identity Map | `HashMap<Id, Arc<T>>`；见 [ownership_model](../../02_formal_methods/09_ownership_model.md) Def ARC1 | 会话内去重 |
| Lazy Load | `OnceLock`、`Option`；见 [proxy](../01_design_patterns_formal/02_structural/07_proxy.md) | 延迟加载 |
| Plugin | `trait` + `Box<dyn Trait>`；依赖注入；见 [strategy](../01_design_patterns_formal/03_behavioral/09_strategy.md) | 可替换实现 |
| Optimistic Offline Lock | `version: u64`、CAS；见 [ownership_model](../../02_formal_methods/09_ownership_model.md) Def ATOMIC1 | 乐观并发 |
| Specification | `trait Spec` + `and`/`or`；组合模式；见 [composite](../01_design_patterns_formal/02_structural/03_composite.md) | 业务规则组合 |
| Event Sourcing | `Vec<Event>` + `fold`；无共享可变；见 [ownership_model](../../02_formal_methods/09_ownership_model.md) | 与 Command、Memento 概念衔接 |
