...
- [Rust 惯用模式与设计理论衔接 {#rust-惯用模式与设计理论衔接}](#rust-惯用模式与设计理论衔接-rust-惯用模式与设计理论衔接)
  - [📑 目录 {#目录}](#-目录-目录)
  - [宗旨 {#宗旨}](#宗旨-宗旨)
  - [层次推进（阅读顺序） {#层次推进阅读顺序}](#层次推进阅读顺序-层次推进阅读顺序)
  - [一、RAII（资源获取即初始化） {#一raii资源获取即初始化}](#一raii资源获取即初始化-一raii资源获取即初始化)
    - [1.1 定义与形式化 {#11-定义与形式化}](#11-定义与形式化-11-定义与形式化)
    - [1.2 典型场景 {#12-典型场景}](#12-典型场景-12-典型场景)
    - [1.3 完整代码示例 {#13-完整代码示例}](#13-完整代码示例-13-完整代码示例)
    - [1.4 常见陷阱 {#14-常见陷阱}](#14-常见陷阱-14-常见陷阱)
    - [1.5 与设计模式衔接 {#15-与设计模式衔接}](#15-与设计模式衔接-15-与设计模式衔接)
  - [二、Newtype 模式 {#二newtype-模式}](#二newtype-模式-二newtype-模式)
    - [2.1 定义与形式化 {#21-定义与形式化}](#21-定义与形式化-21-定义与形式化)
    - [2.2 典型场景 {#22-典型场景}](#22-典型场景-22-典型场景)
    - [2.3 完整代码示例 {#23-完整代码示例}](#23-完整代码示例-23-完整代码示例)
    - [2.4 常见陷阱 {#24-常见陷阱}](#24-常见陷阱-24-常见陷阱)
    - [2.5 与设计模式衔接 {#25-与设计模式衔接}](#25-与设计模式衔接-25-与设计模式衔接)
  - [三、类型状态模式（Typed State） {#三类型状态模式typed-state}](#三类型状态模式typed-state-三类型状态模式typed-state)
    - [3.1 定义与形式化 {#31-定义与形式化}](#31-定义与形式化-31-定义与形式化)
    - [3.2 典型场景 {#32-典型场景}](#32-典型场景-32-典型场景)
    - [3.3 常见陷阱 {#33-常见陷阱}](#33-常见陷阱-33-常见陷阱)
    - [3.4 与设计模式衔接 {#34-与设计模式衔接}](#34-与设计模式衔接-34-与设计模式衔接)
  - [四、Builder 变体（与 GoF 对照） {#四builder-变体与-gof-对照}](#四builder-变体与-gof-对照-四builder-变体与-gof-对照)
  - [五、与 23/43 模型衔接 {#五与-2343-模型衔接}](#五与-2343-模型衔接-五与-2343-模型衔接)
  - [六、Error handling 与 Result/? 惯用 {#六error-handling-与-result-惯用}](#六error-handling-与-result-惯用-六error-handling-与-result-惯用)
    - [6.1 定义与形式化 {#61-定义与形式化}](#61-定义与形式化-61-定义与形式化)
    - [6.2 典型场景 {#62-典型场景}](#62-典型场景-62-典型场景)
    - [6.3 完整代码示例：错误传播链 {#63-完整代码示例错误传播链}](#63-完整代码示例错误传播链-63-完整代码示例错误传播链)
    - [6.4 常见陷阱 {#64-常见陷阱}](#64-常见陷阱-64-常见陷阱)
  - [七、Option/Result 组合模式 {#七optionresult-组合模式}](#七optionresult-组合模式-七optionresult-组合模式)
    - [7.1 定义与形式化 {#71-定义与形式化}](#71-定义与形式化-71-定义与形式化)
    - [7.2 典型场景 {#72-典型场景}](#72-典型场景-72-典型场景)
    - [7.3 完整代码示例：配置解析链 {#73-完整代码示例配置解析链}](#73-完整代码示例配置解析链-73-完整代码示例配置解析链)
    - [7.4 与 Builder 必填校验衔接 {#74-与-builder-必填校验衔接}](#74-与-builder-必填校验衔接-74-与-builder-必填校验衔接)
  - [八、Cow（Clone-on-Write）模式 {#八cowclone-on-write模式}](#八cowclone-on-write模式-八cowclone-on-write模式)
    - [8.1 定义与形式化 {#81-定义与形式化}](#81-定义与形式化-81-定义与形式化)
...
**Axiom RAII1**：RAII 与 [ownership_model](../02_formal_methods/09_ownership_model.md) 规则 3 一致；drop 顺序为创建逆序。

**定理 RAII-T1**：RAII 实现等价于 ownership 规则 3；`Drop::drop` 在 `scope_end` 时调用；由 [ownership_model](../02_formal_methods/09_ownership_model.md) 定理 T3、BOX-T1。
...
**定理 NW-T1**：Newtype 满足 [ownership_model](../02_formal_methods/09_ownership_model.md) 规则 1–3；`T` 的 ownership 语义直接传递；由 Def 1.3 无环、接口一致。
...
**定理 TS-T1**：类型状态与 [Builder](01_design_patterns_formal/01_creational/02_builder.md) B-T2 一致；类型状态 Builder 即 Def TS1 实例。
...
- **State**：与 [state](01_design_patterns_formal/03_behavioral/08_state.md) 互补；编译期 vs 运行时状态机
...
| 变体 | 说明 | 形式化文档 |
| :--- | :--- | :--- |
| Option + ok_or | 运行时校验 | [builder](01_design_patterns_formal/01_creational/02_builder.md) |
| 类型状态 Builder | 编译期强制 | [builder](01_design_patterns_formal/01_creational/02_builder.md) B-T2 |
...
| Newtype | 与 Adapter、Value Object 衔接 | [02_complete_43_catalog](03_workflow_safe_complete_models/01_complete_43_catalog.md) Value Object |
...
**定理 EH-T1**：`?` 与 [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) Def QUERY1 一致；错误传播不违反借用（Borrowing）规则。
...
**定理 OR-T1**：`Option`/`Result` 与 [LANGUAGE_SEMANTICS_EXPRESSIVENESS](../03_formal_proofs/20_language_semantics_expressiveness.md) 构造性语义一致；无 null，无异常隐式传播。
...
| 需求 | 选型 | 形式化对应 |
| :--- | :--- | :--- |
| 独占堆 | `Box<T>` | [ownership_model](../02_formal_methods/09_ownership_model.md) Def BOX1 |
| 单线程共享 | `Rc<T>` | Def RC1 |
...
- [ownership_model](../02_formal_methods/09_ownership_model.md)
- [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) Def QUERY1
