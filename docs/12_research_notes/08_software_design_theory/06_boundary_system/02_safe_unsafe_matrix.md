...
- [安全 vs 非安全边界矩阵 {#安全-vs-非安全边界矩阵}](#安全-vs-非安全边界矩阵-安全-vs-非安全边界矩阵)
  - [📑 目录 {#目录}](#-目录-目录)
  - [形式化定义与公理 {#形式化定义与公理}](#形式化定义与公理-形式化定义与公理)
  - [定义（非形式化对照） {#定义非形式化对照}](#定义非形式化对照-定义非形式化对照)
  - [UB 分类与安全契约 {#ub-分类与安全契约}](#ub-分类与安全契约-ub-分类与安全契约)
  - [可见性边界与 unsafe 封装 {#可见性边界与-unsafe-封装}](#可见性边界与-unsafe-封装-可见性边界与-unsafe-封装)
  - [安全边界决策树 {#安全边界决策树}](#安全边界决策树-安全边界决策树)
  - [设计模式 × 安全边界 {#设计模式-安全边界}](#设计模式--安全边界-设计模式-安全边界)
    - [创建型（5） {#创建型5}](#创建型5-创建型5)
    - [结构型（7） {#结构型7}](#结构型7-结构型7)
    - [行为型（11） {#行为型11}](#行为型11-行为型11)
  - [执行模型 × 安全边界 {#执行模型-安全边界}](#执行模型--安全边界-执行模型-安全边界)
  - [扩展模式（43 完全之 20）安全边界 {#扩展模式43-完全之-20安全边界}](#扩展模式43-完全之-20安全边界-扩展模式43-完全之-20安全边界)
  - [反例：违反安全边界 {#反例违反安全边界}](#反例违反安全边界-反例违反安全边界)
  - [实现检查清单 {#实现检查清单}](#实现检查清单-实现检查清单)
  - [常见错误与排查 {#常见错误与排查}](#常见错误与排查-常见错误与排查)
  - [场景化决策完整示例（实质内容） {#场景化决策完整示例实质内容}](#场景化决策完整示例实质内容-场景化决策完整示例实质内容)
    - [示例 1：需全局配置的单例 {#示例-1需全局配置的单例}](#示例-1需全局配置的单例-示例-1需全局配置的单例)
    - [示例 2：需共享可变状态的 Observer {#示例-2需共享可变状态的-observer}](#示例-2需共享可变状态的-observer-示例-2需共享可变状态的-observer)
    - [示例 3：需 FFI 的 Gateway {#示例-3需-ffi-的-gateway}](#示例-3需-ffi-的-gateway-示例-3需-ffi-的-gateway)
...
若 $A$ 对外 API 满足 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) 与 [ownership_model](../../02_formal_methods/09_ownership_model.md) 的 Safe 规则，则称 $A$ 为**安全抽象**。

**Axiom SBM1**：Safe 代码不触发 UB；违反借用（Borrowing）/所有权（Ownership）规则导致编译错误。
...
*证明*：由 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) T1、[ownership_model](../../02_formal_methods/09_ownership_model.md) T2/T3，Safe API 保证内存安全（Memory Safety）与无数据竞争。
...
**推论 SBM-C2**：设计模式与执行模型组合时，若每子组件 Safe，则组合 Safe；由 SBM-T1 与模块（Module）组合 CE-T1、CE-T2。
...
| 类型系统（Type System） UB | 非法 transmute、破坏 niche | `mem::transmute` 到不合法枚举（Enum） |
| 调用约定 UB | FFI 调用约定/ABI 不匹配 | `extern "C"` 函数签名错误 |
| 生命周期（Lifetimes） UB | 构造不满足 outlives 的引用 | 返回悬垂 `&T` |
...
1. **前置条件**：调用 Safe API 前必须满足的条件在 Safe Rust 中不可违反（由类型系统（Type System）保证）。
...
| 模式 | 安全边界 | 实现要点 |
| :--- | :--- | :--- |
| Adapter | 纯 Safe | 结构体（Struct）包装 `inner: S`，实现目标 trait |
...
| State | 纯 Safe | 枚举或类型状态泛型（Generics） |
...
- [SAFE_UNSAFE_COMPREHENSIVE_ANALYSIS](../../03_formal_proofs/28_safe_unsafe_comprehensive_analysis.md)
- [THEORETICAL_AND_ARGUMENTATION_SYSTEM_ARCHITECTURE](../../06_concept_models/16_theoretical_and_argumentation_system_architecture.md) § 3
