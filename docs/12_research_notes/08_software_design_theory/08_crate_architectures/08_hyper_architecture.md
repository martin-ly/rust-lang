...
# Hyper Crate 架构解构 {#hyper-crate-架构解构}

> **EN**: Hyper Architecture
> **Summary**: Hyper Crate 架构解构 Hyper Architecture.
>
> **最后更新**: 2026-06-09
> **概念族**: 软件设计 / Crate 架构
> **内容分级**: [归档级]
...
Hyper 是 Rust 生态中历史最悠久、应用最广泛的 HTTP 实现库，自 2014 年创建以来，它始终是 Rust 异步（Async） HTTP 栈的基石。

Reqwest（HTTP 客户端）、Axum（HTTP 服务端）、Tonic（gRPC）等知名 crate 均构建于 Hyper 之上。

Hyper 的设计核心围绕两个目标：**协议正确性**（strict adherence to HTTP/1.1 和 HTTP/2 规范）与**性能**（零拷贝解析、最小化内存分配、高效的异步（Async） I/O）。
...
### 2.1 Body Trait：流式数据的统一抽象 {#21-body-trait流式数据的统一抽象}
...
> [来源: [http_body::Body trait 文档](https://docs.rs/http-body/1.0/http_body/trait.Body.html), [hyper::body 模块（Module）](https://docs.rs/hyper/1.0/hyper/body/index.html)]

`Body` trait 是 Hyper 处理流式数据的关键。

与早期版本将 body 表示为 `Stream` 不同，1.0 版本使用 `Frame<Data>` 统一表示 data chunk 和 trailer。

这使得：

- **背压传播**: 下游消费慢时，`poll_frame` 返回 `Pending`，上游暂停读取。
- **零拷贝传输**: `Data: Buf` 允许直接操作内存切片（Slice），避免不必要的 `Vec<u8>` 拷贝。
- **类型擦除**: `BoxBody` 将具体 body 类型擦除为 trait object，用于需要统一类型的场景。

### 2.2 请求与响应的泛型结构 {#22-请求与响应的泛型结构}
...
Hyper 的连接管理模块（Module）分为客户端（`client::conn`）和服务端（`server::conn`）两部分，均建立在 Tokio 的异步 I/O 原语之上。

### 3.1 客户端连接池 {#31-客户端连接池}
...
### 3.2 HTTP/2 实现：h2 Crate {#32-http2-实现h2-crate}
...
### 4.1 make_service_fn：连接级 Service 创建 {#41-make_service_fn连接级-service-创建}
...
每个 TCP 连接创建一个独立的 Service 实例，这使得连接级别的状态隔离成为可能。

### 4.2 Tower Service 桥接 {#42-tower-service-桥接}
...
### 5.1 httparse：零分配 HTTP 解析器 {#51-httparse零分配-http-解析器}
...
- **零堆分配**: 解析结果全部以切片（`&[u8]`）形式引用（Reference）原始缓冲区。
- **SIMD 加速**: 在 x86_64 平台上使用 SSE4.2 指令加速头部搜索。
- **安全性**: 无 `unsafe` 代码（或极少量的边界检查优化）。

### 5.2 HeaderMap：高效头部存储 {#52-headermap高效头部存储}
...
### 6.1 HTTP/1.1 的背压 {#61-http11-的背压}
...
### 7.1 Unsafe 代码的消除 {#71-unsafe-代码的消除}
...
Hyper 的架构体现了 Rust 系统编程的精髓——在类型系统（Type System）的约束下实现零成本抽象（Zero-Cost Abstraction），同时保持协议实现的严格正确性。

| 设计维度 | Hyper 的实现策略 |
| :--- | :--- |
| **协议解析** | `httparse` 零分配解析 + `http` crate 类型系统（Type System） |
...
Hyper 的成功证明了一个观点：在 Rust 中，网络协议库不需要为了性能而牺牲安全性。

通过精心的 trait 设计和底层优化，完全可以同时达到 C 级性能和内存安全（Memory Safety）保证。
...
- [Reqwest HTTP 客户端架构](11_reqwest_architecture.md)
