> **Canonical 说明**: 本目录为 `docs/12_research_notes/08_software_design_theory/08_crate_architectures/` 下的 crate 架构分析索引。若只需要使用指南与生态定位，请优先参考 `concept/06_ecosystem/`、`knowledge/06_ecosystem/` 与 `content/ecosystem/` 中的对应深度文。本目录保留架构级深度内容，并与权威来源通过 canonical 标注形成互补。
>
...
| 10 | **Reqwest** | HTTP 客户端 | ClientBuilder → RequestBuilder → Response | 泛型（Generics） JSON/Form 体、中间件扩展 | ✅ 复用 Hyper 连接池 | [11_reqwest_architecture.md](11_reqwest_architecture.md) |
| 11 | **Wgpu** | GPU 图形 | Adapter → Device → Queue → Pipeline | BindGroup 类型安全、Naga 着色器验证 | ✅ 显式 GPU 内存管理 | [13_wgpu_architecture.md](13_wgpu_architecture.md) |
| 12 | **Actix-web** | Web 框架 | HttpServer → App → Route (Actor 模型) | Actor trait、Transform 中间件、FromRequest | ✅ Actor 无锁消息传递 | [14_actix_web_architecture.md](14_actix_web_architecture.md) |
| 13 | **Rayon** | 数据并行 | ParallelIterator + join() + scope() | Send 边界、fork-join 类型安全 | ✅ 顺序回退无开销 | [16_rayon_architecture.md](16_rayon_architecture.md) |
| 14 | **nalgebra / ndarray** | 科学计算 | Matrix<T,R,C,S> / ArrayBase<S,D> | 类型级维度、Const<N>、Dimension trait | ✅ BLAS 可选后端 | [17_nalgebra_architecture.md](17_nalgebra_architecture.md) |
| 15 | **SQLx (进阶)** | SQL 工具 | 编译期宏展开与连接池深度分析 | `query!` 宏的 Token 流处理、连接池状态机 | ✅ 编译期 SQL 检查 | [18_sqlx_advanced_architecture.md](18_sqlx_advanced_architecture.md)（已重定向 → [SQLx 深度解析](../../../../knowledge/06_ecosystem/databases/02_sqlx_deep_dive.md)） |
| 16 | **Tonic** | gRPC 框架 | `service` 宏 + `prost` 编解码 + Tower 中间件 | gRPC 状态机类型、流式响应 `Streaming<T>` | ✅ 编译期服务定义 | [10_tonic_architecture.md](10_tonic_architecture.md) |
| 17 | **wasm-bindgen** | WASM 绑定 | `#[wasm_bindgen]` 宏 + JS 胶水生成 | 跨语言类型映射、内存所有权（Ownership）桥接 | ✅ 零成本 FFI 抽象 | [12_wasm_bindgen_architecture.md](12_wasm_bindgen_architecture.md) |
| 18 | **Tracing** | 可观测性 | `Span` + `Event` + `Subscriber` + `Layer` | 结构化字段 `Value` trait、零成本条件编译 | ✅ 无 Subscriber 时零开销 | [19_tracing_architecture.md](19_tracing_architecture.md) |
| 19 | **Crossbeam** | 无锁并发 | `epoch::pin` + 无锁队列/通道 | EBR 内存回收、`AtomicCell` 类型安全 | ✅ Lock-free 保证 | [20_crossbeam_architecture.md](20_crossbeam_architecture.md) |
| 20 | **Ratatui** | TUI 框架 | `Widget::render` + `Buffer::diff` | 即时模式渲染、`Backend` trait 抽象 | ✅ 差分渲染 O(变更) | [21_ratatui_architecture.md](21_ratatui_architecture.md) |
| 21 | **mio** | IO 多路复用 | `Poll` + `Registry` + `Token` + `Waker` | 跨平台 epoll/kqueue/IOCP 统一抽象 | ✅ 与直接 epoll 零额外开销 | [22_mio_architecture.md](22_mio_architecture.md) |
| 22 | **redis-rs** | 缓存 / 消息 / 分布式协调 | `Client` → `MultiplexedConnection` / `ConnectionManager` / `PubSub` / `ClusterClient` | `FromRedisValue` / `ToRedisArgs` trait、命令返回类型静态化 | ✅ 单连接多路复用零额外连接开销 | [23_redis_architecture.md](23_redis_architecture.md) |
| 23 | **mongodb-rust-driver** | 文档数据库 / NoSQL | `Client` → `Database` → `Collection<T>` | `Serialize`/`Deserialize` 类型参数、`bson::doc!` 编译期构造 | ✅ 内置连接池、BSON 原生模型 | [24_mongodb_architecture.md](24_mongodb_architecture.md) |
| 24 | **regex** | 文本处理 / 模式匹配（Pattern Matching） | `Regex` / `RegexBuilder` / `RegexSet` | 不可变 `Regex` 共享、`Captures<'t>` 生命周期（Lifetimes）借用 | ✅ 线性时间保证、DFA/NFA/Hybrid 引擎 | [25_regex_architecture.md](25_regex_architecture.md) |
| 25 | **chrono** | 日期时间 / 时区 | `DateTime<Tz>` / `NaiveDateTime` / `Duration` | `TimeZone` trait 类型化时区、`TimeDelta` 正负 Duration | ✅ Naive 与带时区类型在编译期分离 | [26_chrono_architecture.md](26_chrono_architecture.md) |
| 26 | **rdkafka** | 消息队列 / 流处理 | `ClientConfig` → `FutureProducer` / `StreamConsumer` | 异步 Future / Stream、Consumer Group 语义静态化 | ✅ 复用 librdkafka 高性能协议栈 | [27_kafka_architecture.md](27_kafka_architecture.md) |
| 27 | **kube-rs** | 云原生 / Kubernetes | `Client` → `Api<K>` / `Controller` / `watcher` | `Resource` trait、`#[derive(CustomResource)]`、类型化 API 路径 | ✅ 编译期资源类型与 API Group/Version 匹配 | [28_kube_rs_architecture.md](28_kube_rs_architecture.md) |
| 28 | **lapin** | 消息队列 / AMQP / RabbitMQ | `Connection` → `Channel` → `Publisher` / `Consumer` | 异步 Channel、Publisher Confirm、Stream Consumer 类型化 | ✅ 纯 Rust AMQP 协议实现 | [29_lapin_architecture.md](29_lapin_architecture.md) |
| 29 | **GUI / 跨平台 UI** | 桌面 / Web / 原生 GUI | Tauri / Dioxus / Leptos / egui / iced | Command 桥接、响应式信号、即时模式、Elm 架构 | ✅ 单一代码库多平台渲染 | [30_gui_cross_platform_ui_architecture.md](30_gui_cross_platform_ui_architecture.md) |
| 30 | **meilisearch-sdk** | 全文搜索 | `Client` → `Index` → `SearchQuery` / `TaskInfo` | serde 类型化文档、HTTP Client trait、搜索 Builder | ✅ 无运行时反射的 REST 客户端 | [31_meilisearch_architecture.md](31_meilisearch_architecture.md) |
| 31 | **surrealdb** | 文档-图数据库 | `Surreal<C: Connection>` → CRUD / `query` | 引擎类型参数、serde 边界、参数化 SurrealQL | ✅ 同一 API 覆盖远程与嵌入式引擎 | [32_surrealdb_architecture.md](32_surrealdb_architecture.md) |
| 32 | **vector** | 向量搜索 / ANN | `Index<V>` → `search` | `Vector` trait 固定维度、索引类型参数 | ✅ 纯内存零依赖 | [33_vector_architecture.md](33_vector_architecture.md) |
| 33 | **sentry** | 错误追踪 / 可观测性 | `Client` / `Hub` / `Scope` / `Transaction` | RAII Guard、线程本地 Hub、Scope 闭包（Closures） | ✅ 无 Recorder 时事件丢弃成本低 | [34_sentry_architecture.md](34_sentry_architecture.md) |
| 34 | **metrics** | 指标门面 / Prometheus | `Recorder` trait + `counter!` / `gauge!` / `histogram!` | `Key` 与标签、`Counter`/`Gauge`/`Histogram` 句柄 | ✅ 未安装 Recorder 时近似零开销 | [35_metrics_architecture.md](35_metrics_architecture.md) |
| 35 | **ort** | AI 推理 / ONNX Runtime | `Session` / `Tensor` / `ExecutionProvider` | 元素类型参数 `Tensor<T>`、`DynValue` 提取校验 | ✅ 复用 ONNX Runtime C++ 后端，Rust 层零额外拷贝 | [36_ort_architecture.md](36_ort_architecture.md) |
| 36 | **tract** | AI 推理 / 纯 Rust 引擎 | `Onnx` / `TypedModel` / `SimplePlan` | 类型化计算图、执行计划不可变 | ✅ 零外部动态库、编译期图优化 | [37_tract_architecture.md](37_tract_architecture.md) |
| 37 | **aws-sdk-rust** | 云 SDK / AWS | `SdkConfig` / `Client` / Operation Builder | Builder 必填字段类型约束、`SdkError<E>` 服务错误分离 | ✅ Smithy 生成强类型 API，避免运行时字段缺失 | [38_aws_sdk_architecture.md](38_aws_sdk_architecture.md) |
| 38 | **azure-sdk-rust** | 云 SDK / Azure | `TokenCredential` / `Pager<T>` / `Poller<T>` | 凭证 trait 抽象、响应 `into_model()` 类型解包 | ✅ 统一核心抽象跨服务复用 | [39_azure_sdk_architecture.md](39_azure_sdk_architecture.md) |
| 39 | **Salvo** | Web 框架 | `Router` → `#[handler]` → `Request` / `Response` | 宏驱动 Handler、`FlowCtrl` 中间件链 | ✅ 基于 Hyper 的高性能路由 | [40_salvo_architecture.md](40_salvo_architecture.md) |
| 40 | **ntex** | Web 框架 / 可组合网络服务 | `HttpServer` → `App` → `#[web::get]` | 自研 HTTP 协议栈、运行时无关 Service trait | ✅ async/await 原生、Actix-web 兼容 API | [41_ntex_architecture.md](41_ntex_architecture.md) |
| 41 | **Askama** | 模板引擎 | `#[derive(Template)]` → `render()` | 编译期 Jinja-like 模板、字段类型检查 | ✅ 运行时零模板解析开销 | [42_askama_architecture.md](42_askama_architecture.md) |
| 42 | **Maud** | 模板引擎 | `html!` 过程宏（Procedural Macro） → `Markup` | 编译期 HTML DSL、`Render` trait 组件化 | ✅ 模板即 Rust 代码 | [43_maud_architecture.md](43_maud_architecture.md) |
...
- [23_redis_architecture.md](23_redis_architecture.md) — redis-rs 缓存、消息队列与分布式协调
- [24_mongodb_architecture.md](24_mongodb_architecture.md) — mongodb-rust-driver 文档数据库、NoSQL 与异步数据访问
- [25_regex_architecture.md](25_regex_architecture.md) — regex 正则表达式引擎、文本匹配与验证
- [26_chrono_architecture.md](26_chrono_architecture.md) — chrono 日期时间、时区、Duration 与格式化解析
- [27_kafka_architecture.md](27_kafka_architecture.md) — rdkafka 消息队列、流处理与 Consumer Group
- [28_kube_rs_architecture.md](28_kube_rs_architecture.md) — kube-rs Kubernetes 客户端、Controller 与 CRD 派生
- [29_lapin_architecture.md](29_lapin_architecture.md) — lapin AMQP/RabbitMQ 异步消息队列
- [30_gui_cross_platform_ui_architecture.md](30_gui_cross_platform_ui_architecture.md) — GUI / 跨平台 UI 生态（Tauri / Dioxus / Leptos / egui / iced）
- [31_meilisearch_architecture.md](31_meilisearch_architecture.md) — meilisearch-sdk 全文搜索、索引与查询
- [32_surrealdb_architecture.md](32_surrealdb_architecture.md) — surrealdb 文档-图数据库与 SurrealQL
- [33_vector_architecture.md](33_vector_architecture.md) — vector 向量最近邻搜索
- [34_sentry_architecture.md](34_sentry_architecture.md) — sentry 错误追踪与性能监控
- [35_metrics_architecture.md](35_metrics_architecture.md) — metrics 指标门面与 Prometheus 导出
- [36_ort_architecture.md](36_ort_architecture.md) — ort ONNX Runtime Rust 绑定
- [37_tract_architecture.md](37_tract_architecture.md) — tract 纯 Rust 推理引擎
- [38_aws_sdk_architecture.md](38_aws_sdk_architecture.md) — aws-sdk-rust AWS SDK
- [39_azure_sdk_architecture.md](39_azure_sdk_architecture.md) — azure-sdk-rust Azure SDK
