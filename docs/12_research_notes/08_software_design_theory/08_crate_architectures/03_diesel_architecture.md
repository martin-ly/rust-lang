...
# Diesel crate 架构解构 {#diesel-crate-架构解构}

> **EN**: Diesel Architecture
> **Summary**: Diesel crate 架构解构 Diesel Architecture.
> **概念族**: 软件设计 / Crate 架构
> **内容分级**: [归档级]
...
## 3. Typestate 模式详解 {#3-typestate-模式详解}
...
## 4. Connection 生命周期管理 {#4-connection-生命周期管理}
...
## 5. Schema 宏与代码生成 {#5-schema-宏与代码生成}
...
# 2. 编辑迁移 SQL（up.sql / down.sql） {#2-编辑迁移-sqlupsql-downsql}
...
# 4. 生成 Rust schema 模块（build.rs 或 diesel.toml 配置） {#4-生成-rust-schema-模块buildrs-或-dieseltoml-配置}
...
### 7.1 动态查询（运行时确定的列与表） {#71-动态查询运行时确定的列与表}
