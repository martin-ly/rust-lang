...
**陈述**：若各模块 $M_i$ 满足 [ownership_model](../../02_formal_methods/09_ownership_model.md) 定理 T2、T3（所有权（Ownership）唯一性、内存安全（Memory Safety）），则组合 $C = M_1 \oplus \cdots \oplus M_n$ 满足内存安全。
...
**陈述**：若各模块满足 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) 定理 T1（数据竞争自由），且跨线程传递仅 Send 类型、共享仅 Sync 类型，则组合保持数据竞争自由。
...
**陈述**：若各模块良型，且 [type_system_foundations](../../05_type_theory/05_type_system_foundations.md) 进展性 T1、保持性 T2、类型安全 T3 成立，则组合程序良型且类型安全。
...
**陈述**：设模式 $A$、$B$ 各自满足 CE-T1、CE-T2、CE-T3（作为独立模块）。若组合 $A \circ B$ 的接口满足 [03_integration_theory](04_integration_theory.md) IT-T1（跨模块所有权保持）、IT-T2（Send/Sync 传递）、IT-L1（生命周期（Lifetimes）约束），则 $A \circ B$ 保持 CE-T1、CE-T2、CE-T3。
...
本部分定理纳入 [PROOF_INDEX](../../03_formal_proofs/21_proof_index.md)，按「组合软件工程」领域分类。
