...
- [执行模型边界分析 {#执行模型边界分析}](#执行模型边界分析-执行模型边界分析)
  - [📑 目录 {#目录}](#-目录-目录)
  - [五模型 × 三维边界 {#五模型-三维边界}](#五模型--三维边界-五模型-三维边界)
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
  - [执行确定性形式化 {#执行确定性形式化}](#执行确定性形式化-执行确定性形式化)
  - [Rust 1.93 执行模型相关变更 {#rust-193-执行模型相关变更}](#rust-193-执行模型相关变更-rust-193-执行模型相关变更)
  - [静态判定 vs 运行时（Runtime）验证 {#静态判定-vs-运行时验证}](#静态判定-vs-运行时验证-静态判定-vs-运行时验证)
  - [确定性判定决策树 {#确定性判定决策树}](#确定性判定决策树-确定性判定决策树)
  - [并发 vs 并行判定 {#并发-vs-并行判定}](#并发-vs-并行判定-并发-vs-并行判定)
  - [并发选型决策树（Actor、channel、async、Mutex） {#并发选型决策树actorchannelasyncmutex}](#并发选型决策树actorchannelasyncmutex-并发选型决策树actorchannelasyncmutex)
  - [决策树：选择执行模型 {#决策树选择执行模型}](#决策树选择执行模型-决策树选择执行模型)
  - [执行模型与设计模式映射 {#执行模型与设计模式映射}](#执行模型与设计模式映射-执行模型与设计模式映射)
  - [边界证明思路 {#边界证明思路}](#边界证明思路-边界证明思路)
  - [常见组合 {#常见组合}](#常见组合-常见组合)
  - [与形式化基础衔接 {#与形式化基础衔接}](#与形式化基础衔接-与形式化基础衔接)
...
设 $M$ 为执行模型（同步/异步（Async）/并发/并行/分布式），$B_s$ 为安全边界，$B_p$ 为支持边界，$B_e$ 为表达边界。则：
...
**定理 EB-T2（边界一致性（Coherence））**：$B_s$、$B_p$、$B_e$ 与 [05_boundary_system](../06_boundary_system/README.md) 三维矩阵一致。
...
**推论 EB-EX-C2**：五模型与 [05_boundary_system](../06_boundary_system/README.md) 三维矩阵一致；执行模型选型可复用安全/支持/表达决策树。
...
且程序满足 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) 定理 T1（数据竞争自由）及 Send/Sync 约束，则执行无数据竞争；

执行顺序的非确定性不导致 UB。

*证明*：由 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) T1；

Send/Sync 保证跨线程传递安全；ownership/borrow 保证无别名违规；

Interleaved/Parallel 的调度非确定性仅影响执行顺序，不影响内存安全（Memory Safety）。∎
...
详见 [07_rust_1.93_full_changelog](../../../09_toolchain/04_rust_1_93_full_changelog.md)、

[05_rust_1.93_vs_1.92_comparison](../../../09_toolchain/03_rust_1_93_vs_1_92_comparison.md)。
...
| **Parallel、rayon** | 静态 | join/scope 的闭包（Closures）需 Send；编译器可判定 |
| **死锁** | 运行时（Runtime） | 锁顺序、循环等待无法静态判定；需 Miri、测试 |
...
与 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md) CHAN-T1、MUTEX-T1 形式化衔接。
...
│   ├── 一对一、多生产者单消费者 → std::sync::mpsc

│   │   └── 形式化：CHAN-T1；消息传递无共享可变

│   ├── 多生产者多消费者、高吞吐 → crossbeam::channel

│   │   └── 来源: [Crossbeam Docs](https://docs.rs/crossbeam/latest/crossbeam/)

│   ├── 一对多广播 → tokio::sync::broadcast / crossbeam::channel clone
...
| 机制 | 适用场景 | 形式化边界 | crate |
| :--- | :--- | :--- | :--- |
| **async** | I/O 并发、高连接数 | T6.1–T6.3、Send 约束 | tokio |
...
| 模型 | 安全证明要点 | 表达证明要点 |
| :--- | :--- | :--- |
| 同步 | 无共享可变；单线程所有权（Ownership） | 顺序语义与 GoF 一致 |
| 异步 | Future 不共享可变；Pin 保证自引用（Reference）安全 | async/await 等价于 Promise |
...
| 模型 | 引用定理 | 文档 |
| :--- | :--- | :--- |
| 同步 | ownership T2/T3、borrow T1、type_system T1–T3 | [ownership_model](../../02_formal_methods/09_ownership_model.md)、[type_system_foundations](../../05_type_theory/05_type_system_foundations.md) |
| 异步 | async T6.1–T6.3（状态一致性、并发安全（Concurrency Safety）、进度保证）、pin T1–T3、Send 边界 | [async_state_machine](../../02_formal_methods/02_async_state_machine.md)、[pin_self_referential](../../02_formal_methods/10_pin_self_referential.md) |
| 并发 | borrow T1（数据竞争自由）、Send/Sync 语义、EB-DET-T1、CHAN-T1、MUTEX-T1 | [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md)、[async_state_machine](../../02_formal_methods/02_async_state_machine.md) |
| 并行 | 同上 + rayon 不变式、EB-DET-T1、SPAWN-T1 | [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md)、[async_state_machine](../../02_formal_methods/02_async_state_machine.md) |
| 分布式 | 序列化类型安全、FFI 契约 | [SAFE_UNSAFE_COMPREHENSIVE_ANALYSIS](../../03_formal_proofs/28_safe_unsafe_comprehensive_analysis.md) |

**确定性形式化**：Def EB-DET1、定理 EB-DET-T1 与 [FORMAL_PROOF_SYSTEM_GUIDE](../../03_formal_proofs/16_formal_proof_system_guide.md) Send/Sync、borrow T1 衔接；静态 vs 运行时验证见 § [静态判定 vs 运行时验证](#静态判定-vs-运行时验证)；确定性判定决策树见上文。
...
| 决策树 | 本文 § 并发选型决策树、§ 决策树：选择执行模型；[DECISION_GRAPH_NETWORK](../../../07_thinking/02_decision_graph_network.md) |
