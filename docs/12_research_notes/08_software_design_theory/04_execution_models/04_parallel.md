...
> **EN**: Parallel
> **Summary**: 并行执行模型形式化 Parallel. (stub/archive redirect)
> **概念族**: 软件设计 / 执行模型
> **内容分级**: [归档级]
>
> **分级**: [B]
...
- [并行执行模型形式化 {#并行执行模型形式化}](#并行执行模型形式化-并行执行模型形式化)
  - [📑 目录 {#目录}](#-目录-目录)
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
  - [与并发的区别 {#与并发的区别}](#与并发的区别-与并发的区别)
  - [Rust 实现与代码示例 {#rust-实现与代码示例}](#rust-实现与代码示例-rust-实现与代码示例)
    - [Rayon 数据并行 {#rayon-数据并行}](#rayon-数据并行-rayon-数据并行)
    - [std::thread 任务并行 {#stdthread-任务并行}](#stdthread-任务并行-stdthread-任务并行)
  - [典型场景（实质内容） {#典型场景实质内容}](#典型场景实质内容-典型场景实质内容)
    - [与设计模式组合 {#与设计模式组合}](#与设计模式组合-与设计模式组合)
    - [常见陷阱 {#常见陷阱}](#常见陷阱-常见陷阱)
  - [Rayon 工作窃取与调度 {#rayon-工作窃取与调度}](#rayon-工作窃取与调度-rayon-工作窃取与调度)
  - [原子操作（Atomic Operations）与无锁 {#原子操作与无锁}](#原子操作与无锁-原子操作与无锁)
  - [分治与递归并行 {#分治与递归并行}](#分治与递归并行-分治与递归并行)
  - [典型场景扩展 {#典型场景扩展}](#典型场景扩展-典型场景扩展)
  - [与异步（Async）组合 {#与异步组合}](#与异步组合-与异步组合)
  - [反例 {#反例}](#反例-反例)
  - [边界 {#边界}](#边界-边界)
...
**定理 PL-T1**：Rayon 等库保证数据竞争自由；由 Send/Sync 与 [borrow_checker_proof](../../02_formal_methods/03_borrow_checker_proof.md)。

**引理 PL-L1（无共享可变）**：`par_iter` 闭包（Closures）捕获为 move 或只读引用（Reference）；各子任务无共享可变；归约满足结合律时结果确定。

*证明*：由 Axiom PL1；Rayon 工作窃取划分迭代器（Iterator）；闭包（Closures） `|x| x * 2` 无共享状态；`sum` 为结合归约。∎
...
| 组合 | 说明 |
| :--- | :--- |
| 并行 + Iterator | `par_iter` 扩展 `Iterator`；见 [iterator](../01_design_patterns_formal/03_behavioral/04_iterator.md) |
| 并行 + Strategy | 可替换算法；`par_iter().map(\|x\| strategy.apply(x))` |
| 并行 + Flyweight | 共享不可变；`Arc` 在闭包中 move；见 [flyweight](../01_design_patterns_formal/02_structural/06_flyweight.md) |
...
**Def 1.4（原子操作（Atomic Operations））**:
...
**形式化对应**：`rayon::join` 为 fork-join；`split_at_mut` 保证 disjoint 借用（Borrowing）；`Send` 允许跨线程传递。
...
| 权威对应 | ✅ | [formal_methods](../../02_formal_methods/README.md)、[Rayon](https://github.com/rayon-rs/rayon) |
