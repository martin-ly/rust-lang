...
**定理 AS-T1**：由 [async_state_machine](../../02_formal_methods/02_async_state_machine.md) 定理 6.1（状态一致性（Coherence））、6.2（并发安全（Concurrency Safety））、6.3（进度保证）。

**定理 AS-T2**：由 [pin_self_referential](../../02_formal_methods/10_pin_self_referential.md)，Pin 保证自引用 Future 移动安全。
...
**推论 AS-C1**：有限 Future 终将 Ready；由 [async_state_machine](../../02_formal_methods/02_async_state_machine.md) T6.3；无限延迟需超时/取消显式处理。
