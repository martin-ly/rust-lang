...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Structural Patterns – Composite](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
`Box` 或 `Vec` 递归结构保证有界深度；由 [ownership_model](../../../02_formal_methods/09_ownership_model.md) 无泄漏、无悬垂。
...
遍历时 `&self` 借用全部子节点；`&mut self` 可变遍历需小心别名。由 [borrow_checker_proof](../../../02_formal_methods/03_borrow_checker_proof.md) 保证无数据竞争。
...
由 CO-T1、CO-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
以下示例已在 **Rust 1.97.0+ (Edition 2024)** 语义下校验，使用 `递归 enum、统一 trait 接口` 等现代惯用法。
...
`operation(&self)` 遍历子组件并递归调用，均为不可变借用（Immutable Borrow）；可变操作需 `&mut self` 并保证遍历时不重复借用同一节点。
...
| 模式 | 关系 |
| :--- | :--- |
| [Visitor](../03_behavioral/11_visitor.md) | 遍历 Composite 常用 Visitor |
| [Decorator](04_decorator.md) | 同为组合；Decorator 为链式，Composite 为树 |
| [Chain of Responsibility](../03_behavioral/01_chain_of_responsibility.md) | 链 vs 树；委托传递 |
...
| 变体 | 说明 | 适用 |
| :--- | :--- | :--- |
| 枚举（Enum）递归 `enum { Leaf(T), Composite(Vec<Node>) }` | 同质节点；简单 | AST、配置树 |
