...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Structural Patterns – Decorator](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
由 [borrow_checker_proof](../../../02_formal_methods/03_borrow_checker_proof.md)，委托时 `&self.inner` 借用有效，无数据竞争。
...
由 DE-T1、DE-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
装饰器拥有被装饰组件；调用链上所有权不转移，装饰器在调用 `component.method()` 时产生临时不可变借用（Immutable Borrow）。
...
| 模式 | 关系 |
| :--- | :--- |
| [Strategy](../03_behavioral/09_strategy.md) | 装饰器可持有多态策略 |
| [Adapter](01_adapter.md) | 同为包装；Decorator 同接口，Adapter 转换接口 |
| [Composite](03_composite.md) | Decorator 为链式，Composite 为树 |
