...
- [Adapter 形式化分析 {#adapter-形式化分析}](#adapter-形式化分析-adapter-形式化分析)
  - [📑 目录 {#目录}](#-目录-目录)
  ...
  - [典型场景 {#典型场景}](#典型场景-典型场景)
  - [完整场景示例：第三方 HTTP 客户端适配 {#完整场景示例第三方-http-客户端适配}](#完整场景示例第三方-http-客户端适配-完整场景示例第三方-http-客户端适配)
  - [相关模式 {#相关模式}](#相关模式-相关模式)
  - [实现变体 {#实现变体}](#实现变体-实现变体)
...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Structural Patterns – Adapter](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
由 [ownership_model](../../../02_formal_methods/09_ownership_model.md)，$A$ 拥有 $S$，委托调用时 `&self.inner` 借用有效，无悬垂。
...
由 [borrow_checker_proof](../../../02_formal_methods/03_borrow_checker_proof.md)，委托链上无双重可变借用。

**证明**：

1. **委托模式**：`op_T(&self)` → `self.inner.op_S()`
   - `op_T` 接收 `&self`（不可变借用（Immutable Borrow））
...
Adapter 为纯 Safe；仅用结构体（Struct）包装、委托、`impl Trait`，无 `unsafe`。
...
由 AD-T1、AD-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
Adapter 实现 `Target` trait，将外部接口转换为目标接口；可结合泛型（Generics） `Adapter<T>` 支持多种被适配类型。
...
| 模式 | 关系 |
| :--- | :--- |
| [Decorator](04_decorator.md) | 同为包装；Adapter 转换接口，Decorator 同接口延伸 |
| [Facade](05_facade.md) | Facade 简化多接口；Adapter 转换单接口 |
| [Bridge](02_bridge.md) | Bridge 解耦抽象与实现；Adapter 适配已有接口 |
