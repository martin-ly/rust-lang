...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Structural Patterns – Facade](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
模块（Module）系统与 `pub` 可见性保证封装边界。由 Rust 模块语义。
...
Facade 为纯 Safe；仅用结构体（Struct）聚合、私有字段、`pub fn` 委托，无 `unsafe`。
...
由 FA-T1、FA-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
以下示例已在 **Rust 1.97.0+ (Edition 2024)** 语义下校验，使用 `模块封装、简化 API` 等现代惯用法。
...
简化方法通常只需要 `&self`，通过不可变借用（Immutable Borrow）调用多个子系统；需确保子系统方法签名与借用需求兼容。
...
| 借用检查 | 高层方法不可变借用（Immutable Borrow）子系统 | 无数据竞争 |
| 生命周期 | 引用型 Facade 需显式生命周期 | 引用有效性 |
| trait / 关联类型 | 模块（Module） `pub use` 控制接口 | 编译期多态安全 |
...
| 模式 | 关系 |
| :--- | :--- |
| [Adapter](01_adapter.md) | Facade 简化多接口；Adapter 转换单接口 |
| [Mediator](../03_behavioral/05_mediator.md) | Facade 协调子系统；Mediator 协调同事 |
| [Proxy](07_proxy.md) | Proxy 委托单对象；Facade 聚合多对象 |
