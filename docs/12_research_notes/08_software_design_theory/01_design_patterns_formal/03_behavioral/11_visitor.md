...
- [Visitor 形式化分析 {#visitor-形式化分析}](#visitor-形式化分析-visitor-形式化分析)
  - [📊 目录 {#目录}](#-目录-目录)
  - [权威来源对照 {#权威来源对照}](#权威来源对照-权威来源对照)
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
    - [Def 1.1（Visitor 结构） {#def-11visitor-结构}](#def-11visitor-结构-def-11visitor-结构)
    - [Axiom VI1（访问完备公理） {#axiom-vi1访问完备公理}](#axiom-vi1访问完备公理-axiom-vi1访问完备公理)
    - [定理 VI-T1（单分发完备定理） {#定理-vi-t1单分发完备定理}](#定理-vi-t1单分发完备定理-定理-vi-t1单分发完备定理)
    - [定理 VI-T2（穷尽匹配定理） {#定理-vi-t2穷尽匹配定理}](#定理-vi-t2穷尽匹配定理-定理-vi-t2穷尽匹配定理)
    - [推论 VI-C1（近似表达） {#推论-vi-c1近似表达}](#推论-vi-c1近似表达-推论-vi-c1近似表达)
  ...
  - [完整场景示例：AST 美化打印 {#完整场景示例ast-美化打印}](#完整场景示例ast-美化打印-完整场景示例ast-美化打印)
  ...
  - [反例：常见误用及编译器错误 {#反例常见误用及编译器错误}](#反例常见误用及编译器错误-反例常见误用及编译器错误)
    - [反例 1：新增元素类型未实现 visit {#反例-1新增元素类型未实现-visit}](#反例-1新增元素类型未实现-visit-反例-1新增元素类型未实现-visit)
    - [反例 2：访问者中可变借用元素 {#反例-2访问者中可变借用元素}](#反例-2访问者中可变借用元素-反例-2访问者中可变借用元素)
    - [反例 3：遍历中修改元素集合 {#反例-3遍历中修改元素集合}](#反例-3遍历中修改元素集合-反例-3遍历中修改元素集合)
...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Behavioral Patterns – Visitor](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
Visitor 与 [expressive_inexpressive_matrix](../../06_boundary_system/01_expressive_inexpressive_matrix.md) 表一致；$\mathit{ExprB}(\mathrm{Visitor}) = \mathrm{Approx}$。
...
以下示例已在 **Rust 1.97.0+ (Edition 2024)** 语义下校验，使用 `双分派、trait Visitor、enum accept` 等现代惯用法。
...
双分派过程中：`shape.accept(&mut visitor)` 不可变借用（Immutable Borrow） shape，可变借用（Mutable Borrow） visitor；visitor 方法内部只读借用具体元素。
...
| 生命周期 | 元素引用（Reference）在 visit 期间有效 | 引用有效性 |
...
| 模式 | 关系 |
| :--- | :--- |
| [Composite](../02_structural/03_composite.md) | 遍历 Composite 常用 Visitor |
| [Interpreter](03_interpreter.md) | 同为 AST 处理；Interpreter 求值，Visitor 遍历 |
| [Iterator](04_iterator.md) | 遍历方式不同；Visitor 深度优先，Iterator 可定制 |
