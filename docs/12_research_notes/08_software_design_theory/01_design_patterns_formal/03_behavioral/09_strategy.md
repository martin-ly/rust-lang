...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Behavioral Patterns – Strategy](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
trait 定义策略接口；`impl Trait` 或 `dyn Trait` 实现多态；由 [trait_system_formalization](../../../05_type_theory/04_trait_system_formalization.md) 解析正确性。
...
策略调用时借用规则：`&self` 不可变调用策略；`&mut self` 可变时仍满足互斥。由 [borrow_checker_proof](../../../02_formal_methods/03_borrow_checker_proof.md)。
...
由 SR-T1、SR-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
策略可由上下文拥有（`Box<dyn Strategy>`）或引用（`&dyn Strategy`）。引用版本生命周期受限；拥有版本支持运行时（Runtime）切换。
...
| 模式 | 关系 |
| :--- | :--- |
| [Decorator](../02_structural/04_decorator.md) | 装饰器可持有多态策略 |
| [State](08_state.md) | 策略可替换；State 可转换 |
...
| 变体 | 说明 | 适用 |
| :--- | :--- | :--- |
| 泛型（Generics） `Context<S: Strategy>` | 编译期单态化（Monomorphization），零成本 | 策略类型已知 |
