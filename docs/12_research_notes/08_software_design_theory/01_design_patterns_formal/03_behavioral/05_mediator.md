...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Behavioral Patterns – Mediator](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
`Rc`/`Weak` 或 `Arc` 管理循环引用时避免自引用；由 [ownership_model](../../../02_formal_methods/09_ownership_model.md) 与借用规则。
...
由 ME-T1、ME-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
组件调用 `notify(&self, ...)` 无需可变借用（Mutable Borrow），消息传递避免共享可变状态；接收端通过 `recv()` 获得消息所有权。
...
| 模式 | 关系 |
| :--- | :--- |
| [Observer](07_observer.md) | 同为解耦；Mediator 集中路由，Observer 一对多 |
| [Facade](../02_structural/05_facade.md) | Facade 简化接口；Mediator 协调多对象 |
| [Chain of Responsibility](01_chain_of_responsibility.md) | 链式传递 vs 集中路由 |
