...
`Iterator` trait 由标准库定义；`for` 糖语法保证类型安全。由 [type_system_foundations](../../../05_type_theory/05_type_system_foundations.md)。
...
`&mut self` 可变借用保证迭代器内部状态一致；由 [borrow_checker_proof](../../../02_formal_methods/03_borrow_checker_proof.md)。
...
由 IT-T1、IT-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
| 模式 | 关系 |
| :--- | :--- |
| [Visitor](11_visitor.md) | 遍历方式不同；Iterator 顺序，Visitor 深度优先 |
| [Composite](../02_structural/03_composite.md) | 可对 Composite 实现 Iterator |
| [Chain of Responsibility](01_chain_of_responsibility.md) | 链式传递 vs 迭代消费 |
