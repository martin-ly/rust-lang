...
- [Observer 形式化分析 {#observer-形式化分析}](#observer-形式化分析-observer-形式化分析)
  ...
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
    ...
    - [Axiom OB2（借用（Borrowing）约束公理） {#axiom-ob2借用约束公理}](#axiom-ob2借用约束公理-axiom-ob2借用约束公理)
...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Behavioral Patterns – Observer](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
`mpsc` 或 `broadcast` channel 为纯 Safe；消息传递无共享可变。由 [borrow_checker_proof](../../../02_formal_methods/03_borrow_checker_proof.md) 与 Send/Sync。
...
共享 `Rc<RefCell<Vec<Callback>>>` 需 `RefCell` 运行时（Runtime）借用检查；`Mutex` 为 Safe 抽象。
...
由 [ownership_model](../../../02_formal_methods/09_ownership_model.md) 及 unsafe 契约，得证。$\square$
...
由 OB-T1、OB-T2 及 [safe_unsafe_matrix](../../06_boundary_system/02_safe_unsafe_matrix.md) SBM-T1，得证。$\square$
...
以下示例已在 **Rust 1.97.0+ (Edition 2024)** 语义下校验，使用 `channel、回调 trait、广播` 等现代惯用法。
...
| 生命周期 | 事件字符串克隆后无生命周期依赖 | 引用（Reference）有效性 |
...
| 模式 | 关系 |
| :--- | :--- |
| [Command](02_command.md) | 观察者可接收命令；命令可作为事件 |
| [Mediator](05_mediator.md) | 同为解耦；Observer 一对多，Mediator 集中路由 |
| [State](08_state.md) | 状态转换可通知观察者 |
...
**编译器错误**：无法同时可变/不可变借用（Immutable Borrow） Subject。
