...
- [Memento 形式化分析 {#memento-形式化分析}](#memento-形式化分析-memento-形式化分析)
  - [📊 目录 {#目录}](#-目录-目录)
  - [权威来源对照 {#权威来源对照}](#权威来源对照-权威来源对照)
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
    - [Def 1.1（Memento 结构） {#def-11memento-结构}](#def-11memento-结构-def-11memento-结构)
    - [Axiom MO1（状态完整公理） {#axiom-mo1状态完整公理}](#axiom-mo1状态完整公理-axiom-mo1状态完整公理)
    - [Axiom MO2（兼容性公理） {#axiom-mo2兼容性公理}](#axiom-mo2兼容性公理-axiom-mo2兼容性公理)
    - [定理 MO-T1（Clone 实现定理） {#定理-mo-t1clone-实现定理}](#定理-mo-t1clone-实现定理-定理-mo-t1clone-实现定理)
    - [定理 MO-T2（状态一致性定理） {#定理-mo-t2状态一致性定理}](#定理-mo-t2状态一致性定理-定理-mo-t2状态一致性定理)
    - [推论 MO-C1（近似表达） {#推论-mo-c1近似表达}](#推论-mo-c1近似表达-推论-mo-c1近似表达)
  ...
  - [反例：常见误用及编译器错误 {#反例常见误用及编译器错误}](#反例常见误用及编译器错误-反例常见误用及编译器错误)
    - [反例 1：备忘录持有发起者引用 {#反例-1备忘录持有发起者引用}](#反例-1备忘录持有发起者引用-反例-1备忘录持有发起者引用)
    - [反例 2：恢复后修改备忘录影响发起者 {#反例-2恢复后修改备忘录影响发起者}](#反例-2恢复后修改备忘录影响发起者-反例-2恢复后修改备忘录影响发起者)
    - [反例 3：备忘录字段未完整捕获状态 {#反例-3备忘录字段未完整捕获状态}](#反例-3备忘录字段未完整捕获状态-反例-3备忘录字段未完整捕获状态)
...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Behavioral Patterns – Memento](https://rust-unofficial.github.io/patterns/)) | Rust 惯用实现与模式定位 |
...
Memento 与 [expressive_inexpressive_matrix](../../06_boundary_system/01_expressive_inexpressive_matrix.md) 表一致；$\mathit{ExprB}(\mathrm{Memento}) = \mathrm{Approx}$（无私有封装）。
...
以下示例已在 **Rust 1.97.0+ (Edition 2024)** 语义下校验，使用 `不可变快照、serde 序列化、撤销栈` 等现代惯用法。
...
`save(&self)` 只读借用发起者；`restore(&mut self, &EditorState)` 可变借用（Mutable Borrow）发起者并只读借用备忘录。
...
| 模式 | 关系 |
| :--- | :--- |
| [Command](02_command.md) | 撤销需 Memento 保存状态 |
| [State](08_state.md) | 保存/恢复状态 |
| [Prototype](../01_creational/04_prototype.md) | Clone 可作 Memento 实现 |
...
| 快照类型 | 显式 `Snapshot` 结构体（Struct） | 版本兼容、校验 |
