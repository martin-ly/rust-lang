...
- [设计模式边界矩阵汇总 {#设计模式边界矩阵汇总}](#设计模式边界矩阵汇总-设计模式边界矩阵汇总)
  - [📑 目录 {#目录}](#-目录-目录)
  - [形式化定义与公理 {#形式化定义与公理}](#形式化定义与公理-形式化定义与公理)
  - [模式 × 三维边界 {#模式-三维边界}](#模式--三维边界-模式-三维边界)
  - [边界定义 {#边界定义}](#边界定义-边界定义)
  - [边界异常说明 {#边界异常说明}](#边界异常说明-边界异常说明)
  - [与证明体系衔接 {#与证明体系衔接}](#与证明体系衔接-与证明体系衔接)
  - [近似表达模式速查 {#近似表达模式速查}](#近似表达模式速查-近似表达模式速查)
  - [按维度速查 {#按维度速查}](#按维度速查-按维度速查)
  - [选型决策树（模式 → 边界） {#选型决策树模式-边界}](#选型决策树模式--边界-选型决策树模式-边界)
  - [设计模式表征能力形式化树图 {#设计模式表征能力形式化树图}](#设计模式表征能力形式化树图-设计模式表征能力形式化树图)
    - [Mermaid 形式化树图 {#mermaid-形式化树图}](#mermaid-形式化树图-mermaid-形式化树图)
    - [ASCII 形式化树图（模式→实现路径→定理） {#ascii-形式化树图模式实现路径定理}](#ascii-形式化树图模式实现路径定理-ascii-形式化树图模式实现路径定理)
  - [模式组合约束 DAG（D1.5） {#模式组合约束-dagd15}](#模式组合约束-dagd15-模式组合约束-dagd15)
    - [推荐组合（保持 CE-T1–T3） {#推荐组合保持-ce-t1t3}](#推荐组合保持-ce-t1t3-推荐组合保持-ce-t1t3)
    - [禁止/慎用组合 {#禁止慎用组合}](#禁止慎用组合-禁止慎用组合)
  - [与 43 完全模型衔接 {#与-43-完全模型衔接}](#与-43-完全模型衔接-与-43-完全模型衔接)
...
设 $P$ 为 GoF 设计模式，$B_s(P)$、$B_p(P)$、$B_e(P)$ 分别为安全、支持、表达边界。定义见 [05_boundary_system](../06_boundary_system/README.md) Def B1–B3。

**Def 1.2（三维边界一致性（Coherence））**:

若 $B_s(P)$、$B_p(P)$、$B_e(P)$ 与 [safe_unsafe_matrix](../06_boundary_system/02_safe_unsafe_matrix.md)、

[supported_unsupported_matrix](../06_boundary_system/03_supported_unsupported_matrix.md)、

[expressive_inexpressive_matrix](../06_boundary_system/01_expressive_inexpressive_matrix.md) 三矩阵对应一致，
...
*证明*：由 [05_boundary_system](../06_boundary_system/README.md) 定理 B-T1，三维边界由各矩阵定义唯一确定；设计模式为 05 边界体系的子集。∎

**定理 BMP-T2（23 模式与 05 矩阵一致）**：23 种 GoF 模式的本表与 [05_boundary_system](../06_boundary_system/README.md) 三矩阵对应一致；无冲突。
...
$\mathit{ExprB}(P) = \mathrm{Approx}$ 由 [expressive_inexpressive_matrix](../06_boundary_system/01_expressive_inexpressive_matrix.md) 定理 EIM-T2、EIM-L1 确定。

*证明*：由 [expressive_inexpressive_matrix](../06_boundary_system/01_expressive_inexpressive_matrix.md)；

上述模式依赖全局可变、继承或双重分发，Rust 用 OnceLock、channel、match 等替代；

依 Def 1.1 为 Approx。∎

**推论 BMP-C1**：等价表达模式（21 种）满足零成本抽象（Zero-Cost Abstraction）；

近似表达模式可能有额外间接（如 channel）。

*证明*：由 [expressive_inexpressive_matrix](../06_boundary_system/01_expressive_inexpressive_matrix.md) 推论 EIM-C1。∎
...
详见 [05_boundary_system](../06_boundary_system/README.md)。
...
- **近似表达**：有明确差异点；见 [expressive_inexpressive_matrix](../06_boundary_system/01_expressive_inexpressive_matrix.md) 详解。
...
| 组合 | 约束 | 引用（Reference） |
| :--- | :--- | :--- |
| Builder + Factory | 工厂返回 Builder；IT-T1 满足 | [CE-PAT1](../05_compositional_engineering/02_effectiveness_proofs.md#定理-ce-pat1模式组合-ce-保持) |
...
| Repository + Service + DTO | 模块（Module）依赖 DAG；所有权（Ownership）沿调用链 | [03_integration_theory](../05_compositional_engineering/04_integration_theory.md) |
...
**引用**：[04_compositional_engineering](../05_compositional_engineering/README.md)、

[CE-PAT1](../05_compositional_engineering/02_effectiveness_proofs.md#定理-ce-pat1模式组合-ce-保持)。
...
扩展 20 种模式见 [02_complete_43_catalog](../03_workflow_safe_complete_models/01_complete_43_catalog.md)；
