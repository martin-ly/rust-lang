...
├── 13_research_notes_organization.md   # 本文件：实际结构、归档约定、维护清单
...
完整列表以 `formal_methods/` 下实际文件为准；详见 [formal_methods/README](../02_formal_methods/README.md)。
...
- [ ] 空目录检查：运行 `find docs/12_research_notes -type d -empty`，确保无未说明的空目录
...
- [x] `docs/12_research_notes` 下无未说明的空目录
- [x] README/INDEX 中关于子目录的链接已指向 `docs/12_research_notes` 下实际路径
