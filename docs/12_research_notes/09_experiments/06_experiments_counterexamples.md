...
- [实验与性能研究反例边界 {#实验与性能研究反例边界}](#实验与性能研究反例边界-实验与性能研究反例边界)
  - [目录 {#目录}](#目录-目录)
  - [1. 基准测试未禁用优化 {#1-基准测试未禁用优化}](#1-基准测试未禁用优化-1-基准测试未禁用优化)
    - [现象 {#现象-6}](#现象-现象-6)
    - [后果 {#后果-1}](#后果-后果-1)
    - [修复方案 {#修复方案-6}](#修复方案-修复方案-6)
  - [2. 编译器常量折叠掩盖真实性能 {#2-编译器常量折叠掩盖真实性能}](#2-编译器常量折叠掩盖真实性能-2-编译器常量折叠掩盖真实性能)
    - [现象 {#现象-6}](#现象-现象-6-1)
    - [后果 {#后果-1}](#后果-后果-1-1)
    - [修复方案 {#修复方案-6}](#修复方案-修复方案-6-1)
  - [3. 微基准未预热缓存 {#3-微基准未预热缓存}](#3-微基准未预热缓存-3-微基准未预热缓存)
    - [现象 {#现象-6}](#现象-现象-6-2)
    - [问题 {#问题-3}](#问题-问题-3)
    - [修复方案 {#修复方案-6}](#修复方案-修复方案-6-2)
  - [4. 错误使用 `black_box` {#4-错误使用-black\_box}](#4-错误使用-black_box-4-错误使用-black_box)
    - [现象 {#现象-6}](#现象-现象-6-3)
    - [问题 {#问题-3}](#问题-问题-3-1)
    - [修复方案 {#修复方案-6}](#修复方案-修复方案-6-3)
  - [5. 单线程基准推断多线程性能 {#5-单线程基准推断多线程性能}](#5-单线程基准推断多线程性能-5-单线程基准推断多线程性能)
    - [现象 {#现象-6}](#现象-现象-6-4)
    - [问题 {#问题-3}](#问题-问题-3-2)
    - [修复方案 {#修复方案-6}](#修复方案-修复方案-6-4)
  - [6. 内存分析忽略分配器差异 {#6-内存分析忽略分配器差异}](#6-内存分析忽略分配器差异-6-内存分析忽略分配器差异)
    - [现象 {#现象-6}](#现象-现象-6-5)
    - [问题 {#问题-3}](#问题-问题-3-3)
    - [修复方案 {#修复方案-6}](#修复方案-修复方案-6-5)
  - [7. 宏（Macro）展开性能测试未控制输入规模 {#7-宏展开性能测试未控制输入规模}](#7-宏展开性能测试未控制输入规模-7-宏展开性能测试未控制输入规模)
    - [现象 {#现象-6}](#现象-现象-6-6)
...
忽略了锁竞争、缓存一致性（Coherence）、伪共享（false sharing）等多线程因素。
...
- [性能基准测试](05_performance_benchmarks.md)
- [并发性能研究](02_concurrency_performance.md)
- [编译器优化](01_compiler_optimizations.md)
- [宏展开性能分析](03_macro_expansion_performance.md)
- [内存分析](04_memory_analysis.md)
