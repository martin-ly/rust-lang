...
**Def MA1（内存实验验证）**：内存分析实验 $E$ 验证 [ownership_model](../02_formal_methods/09_ownership_model.md) 定理 T2/T3，当且仅当 $E$ 观测到无悬垂、无双重释放、无泄漏。
...
**引用**：[experiments/README](README.md) 定理 EX-T1；[formal_methods/README](../02_formal_methods/README.md) FM-T1。
...
**分析要点**：`size_of`/`align_of` 与 [ALIGNMENT_GUIDE](../../03_reference/06_alignment_guide.md) 对齐知识衔接；`#[repr(C)]` 用于 FFI 与布局控制。
...
- **Rust**: 1.97.0+；**Valgrind**: 3.18+（Linux）；**Miri**: `rustup component add miri`
...
- **所有权模型**：见 [10_ownership_model.md](../02_formal_methods/09_ownership_model.md)。内存分析中的「移动/复制/Drop」可对照所有权规则验证无泄漏。
- **借用（Borrowing）检查器**：见 [10_borrow_checker_proof.md](../02_formal_methods/03_borrow_checker_proof.md)。引用与生命周期不影响堆分配量，但可通过 Miri 与借用规则共同保证无 UB。

### 与实验研究的集成 {#与实验研究的集成}

> **来源: [The Rust Programming Language](https://doc.rust-lang.org/book/)**

- **性能基准测试**：见 [05_performance_benchmarks.md](05_performance_benchmarks.md)。内存分配基准（栈/堆/预分配）与本研究的数据收集可共用 `cargo bench` 与 Criterion 输出。
- **编译器优化**：见 [01_compiler_optimizations.md](01_compiler_optimizations.md)。`-C link-dead-code`、`opt-level` 会影响可执行体大小与分配内联，分析时需固定编译选项。
