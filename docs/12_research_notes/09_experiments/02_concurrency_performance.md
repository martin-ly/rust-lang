...
- [并发性能研究 {#并发性能研究}](#并发性能研究-并发性能研究)
  ...
  - [📚 理论基础 {#理论基础}](#-理论基础-理论基础)
    ...
    - [相关概念 {#相关概念}](#相关概念-相关概念)
    - [理论背景 {#理论背景}](#理论背景-理论背景)
      - [The Rust Performance Book 并行视角 {#the-rust-performance-book-并行视角}](#the-rust-performance-book-并行视角-the-rust-performance-book-并行视角)
  - [🔬 实验设计 {#实验设计}](#-实验设计-实验设计)
  ...
  - [💻 代码示例 {#代码示例}](#-代码示例-代码示例)
    - [示例 1：Mutex vs RwLock 性能测试 {#示例-1mutex-vs-rwlock-性能测试}](#示例-1mutex-vs-rwlock-性能测试-示例-1mutex-vs-rwlock-性能测试)
    - [示例 2：通道性能测试 {#示例-2通道性能测试}](#示例-2通道性能测试-示例-2通道性能测试)
    - [示例 3：异步任务性能测试 {#示例-3异步任务性能测试}](#示例-3异步任务性能测试-示例-3异步任务性能测试)
  - [📊 实验结果 {#实验结果}](#-实验结果-实验结果)
    - [1. 同步原语性能对比 {#1-同步原语性能对比}](#1-同步原语性能对比-1-同步原语性能对比)
    - [2. 通道性能对比 {#2-通道性能对比}](#2-通道性能对比-2-通道性能对比)
  ...
  - [📐 性能优化建议与工具改进 {#性能优化建议与工具改进}](#-性能优化建议与工具改进-性能优化建议与工具改进)
    ...
    - [原子内存顺序选型决策树 {#原子内存顺序选型决策树}](#原子内存顺序选型决策树-原子内存顺序选型决策树)
    - [死锁检测与运行时验证工具 {#死锁检测与运行时验证工具}](#死锁检测与运行时验证工具-死锁检测与运行时验证工具)
    - [工具改进 {#工具改进}](#工具改进-工具改进)
    - [性能报告 {#性能报告}](#性能报告-性能报告)
...
4. **并发安全（Concurrency Safety）开销**：评估并发安全的性能开销
...
**Def CP1（并发实验验证）**：并发性能实验 $E$ 验证 [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) T1、[async_state_machine](../02_formal_methods/02_async_state_machine.md) T6.2，当且仅当 $E$ 在观测下无数据竞争。
...
*证明*：由 [experiments/README](README.md) 定理 EX-T1；TSan 与定理结论一致即验证。∎

**引理 CP-L1（Send/Sync 与 borrow T1 衔接）**：若类型 $T$ 满足 `Send + Sync` 且跨线程共享，则 borrow T1 保证无数据竞争；TSan 观测与 borrow T1、async T6.2 结论一致。

*证明*：由 [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) T1、[async_state_machine](../02_formal_methods/02_async_state_machine.md) T6.2；Send/Sync 为跨线程传递的必要条件；满足则无数据竞争。∎
...
**引用**：[experiments/README](README.md) 定理 EX-T1、EX-T2；[FORMAL_PROOF_SYSTEM_GUIDE](../03_formal_proofs/16_formal_proof_system_guide.md)。
...
- **减少锁竞争**：缩小临界区、使用读写锁（`RwLock`）或原子操作（Atomic Operations）（`Atomic*`）。
...
async fn tokio_unbounded_channel_benchmark() {
...
- **Rust**: 1.97.0+；**Tokio**：`tokio = { version = "1", features = ["full"] }`；**Criterion**：工作区已配置
...
[ownership_model](../02_formal_methods/09_ownership_model.md) Def ATOMIC1；

[06_boundary_analysis](../08_software_design_theory/04_execution_models/06_boundary_analysis.md) § 静态判定 vs 运行时验证。
...
**说明**：死锁无法静态判定；见 [06_boundary_analysis](../08_software_design_theory/04_execution_models/06_boundary_analysis.md) § 静态判定 vs 运行时验证。
...
按「结果分析模板」+ 各原语/通道/异步的 bench 曲线，可形成并发性能报告；建议包含「同步 vs 通道 vs 异步的选型」「线程数与扩展性」「与 1.96+ 的兼容性」三部分。
...
- **异步状态机**：见 [10_async_state_machine.md](../02_formal_methods/02_async_state_machine.md)。异步任务的 Poll、Waker 与状态转换，可与本研究的 async 基准对应；并发安全定理与「无数据竞争」可共同验证。
- **借用（Borrowing）检查器**：见 [10_borrow_checker_proof.md](../02_formal_methods/03_borrow_checker_proof.md)。Rust 的并发原语（Mutex、Arc、channel）在类型与借用层面保证数据竞争自由，本研究的性能数据不改变该结论，但可指导「在安全前提下选更快实现」。
...
- **性能基准测试**：见 [05_performance_benchmarks.md](05_performance_benchmarks.md)。并发一节与本文的 Mutex/RwLock、通道、async 可共用 `cargo bench` 与 Criterion 流程。
- **内存分析**：见 [04_memory_analysis.md](04_memory_analysis.md)。`Arc`、有界通道的缓冲、Tokio 任务队列与 `thread_local` 分配器会影响内存；分析时需区分配置（线程数、任务数、通道容量）。
