...
- [宏（Macro）展开性能研究 {#宏展开性能研究}](#宏展开性能研究-宏展开性能研究)
  - [📑 目录 {#目录}](#-目录-目录)
  - [🎯 研究目标 {#研究目标}](#-研究目标-研究目标)
    - [核心问题 {#核心问题}](#核心问题-核心问题)
    - [预期成果 {#预期成果}](#预期成果-预期成果)
  ...
  - [💻 代码示例 {#代码示例}](#-代码示例-代码示例)
    ...
    - [示例 2：过程宏（Procedural Macro）性能测试 {#示例-2过程宏性能测试}](#示例-2过程宏性能测试-示例-2过程宏性能测试)
...
本研究旨在分析 Rust 宏展开对编译时间和运行时（Runtime）性能的影响，评估不同宏实现的性能特征，包括：
...
**Def MP1（宏展开实验验证）**：宏展开实验 $E$ 验证 [type_system_foundations](../05_type_theory/05_type_system_foundations.md) 保持性 T2，当且仅当 $E$ 观测到宏展开后代码良型且类型检查通过。
...
**引用（Reference）**：[experiments/README](README.md) 定理 EX-T1；[RUST_193_LANGUAGE_FEATURES_COMPREHENSIVE_ANALYSIS](../12_version_research/01_rust_193_language_features_comprehensive_analysis.md) 宏相关特性。
...
- **声明式宏（`macro_rules!`）**：基于模式匹配（Pattern Matching）展开，Rust 1.96+ 继续支持并持续优化展开性能。
...
- **类型系统基础**：见 [10_type_system_foundations.md](../05_type_theory/05_type_system_foundations.md)。宏在类型检查之前展开，其生成的类型与 trait 需满足类型规则；复杂宏可先 `cargo expand` 再交给类型系统分析。
- **Trait 系统**：见 [10_trait_system_formalization.md](../05_type_theory/04_trait_system_formalization.md)。派生宏（`#[derive]`）生成 trait 实现，其正确性与 hygiene 可对照 Trait 形式化；`Serialize`/`Deserialize` 等是过程宏的典型应用。
...
- **编译器优化**：见 [01_compiler_optimizations.md](01_compiler_optimizations.md)。宏展开后的代码参与后续优化（内联、死代码消除）；分析优化效果时需区分「宏生成」与「手写」路径。
- **性能基准测试**：见 [05_performance_benchmarks.md](05_performance_benchmarks.md)。宏生成代码的运行时 bench 可纳入同一 `cargo bench` 流程；编译时间可用 `time cargo build` 与 CI 集成。
