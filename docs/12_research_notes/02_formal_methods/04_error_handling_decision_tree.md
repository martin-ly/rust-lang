...
> **分级**: [B]
> **Bloom 层级**: L5-L6
> 一个系统化的错误处理（Error Handling）策略选择指南
...
> **[ACM - Error Handling Patterns](https://dl.acm.org/)** · **[IEEE - Fault-Tolerant Software Design](https://ieeexplore.ieee.org/) <!-- link: known-broken -->**

- [Rust 错误处理决策树 {#rust-错误处理决策树}](#rust-错误处理决策树-rust-错误处理决策树)
  - [目录 {#目录}](#目录-目录)
  - [决策树总览 {#决策树总览}](#决策树总览-决策树总览)
  - [决策维度详解 {#决策维度详解}](#决策维度详解-决策维度详解)
    - [维度 1: 错误类型 - 可恢复 vs 不可恢复 {#维度-1-错误类型---可恢复-vs-不可恢复}](#维度-1-错误类型---可恢复-vs-不可恢复-维度-1-错误类型---可恢复-vs-不可恢复)
    - [维度 2: 错误传播范围 {#维度-2-错误传播范围}](#维度-2-错误传播范围-维度-2-错误传播范围)
      - [2.1 本地处理场景 {#21-本地处理场景}](#21-本地处理场景-21-本地处理场景)
      - [2.2 错误传播场景 {#22-错误传播场景}](#22-错误传播场景-22-错误传播场景)
    - [维度 3: 错误处理策略 {#维度-3-错误处理策略}](#维度-3-错误处理策略-维度-3-错误处理策略)
      - [策略选择矩阵 {#策略选择矩阵}](#策略选择矩阵-策略选择矩阵)
      - [3.1 重试策略决策 {#31-重试策略决策}](#31-重试策略决策-31-重试策略决策)
      - [3.2 降级策略 {#32-降级策略}](#32-降级策略-32-降级策略)
    - [维度 4: 库 vs 应用 {#维度-4-库-vs-应用}](#维度-4-库-vs-应用-维度-4-库-vs-应用)
      - [对比表 {#对比表}](#对比表-对比表)
  - [对比分析 {#对比分析}](#对比分析-对比分析)
    - [Result vs Option vs panic {#result-vs-option-vs-panic}](#result-vs-option-vs-panic-result-vs-option-vs-panic)
      - [详细对比 {#详细对比-1}](#详细对比-详细对比-1)
      - [转换关系 {#转换关系}](#转换关系-转换关系)
    - [thiserror vs anyhow {#thiserror-vs-anyhow}](#thiserror-vs-anyhow-thiserror-vs-anyhow)
      - [详细对比 {#详细对比-1}](#详细对比-详细对比-1-1)
      - [混合使用模式 {#混合使用模式}](#混合使用模式-混合使用模式)
    - [自定义错误类型设计 {#自定义错误类型设计}](#自定义错误类型设计-自定义错误类型设计)
      - [设计原则 {#设计原则}](#设计原则-设计原则)
      - [推荐模式 {#推荐模式}](#推荐模式-推荐模式)
    - [错误链和上下文 {#错误链和上下文}](#错误链和上下文-错误链和上下文)
      - [anyhow 上下文链 {#anyhow-上下文链}](#anyhow-上下文链-anyhow-上下文链)
      - [thiserror 错误源 {#thiserror-错误源}](#thiserror-错误源-thiserror-错误源)
  - [最佳实践 {#最佳实践}](#最佳实践-最佳实践)
    - [1. 错误类型设计模式 {#1-错误类型设计模式}](#1-错误类型设计模式-1-错误类型设计模式)
      - [模式 A: 分层错误架构 {#模式-a-分层错误架构}](#模式-a-分层错误架构-模式-a-分层错误架构)
      - [模式 B: 错误状态码映射 {#模式-b-错误状态码映射}](#模式-b-错误状态码映射-模式-b-错误状态码映射)
      - [模式 C: 错误 Builder {#模式-c-错误-builder}](#模式-c-错误-builder-模式-c-错误-builder)
    - [2. 错误转换和映射 {#2-错误转换和映射}](#2-错误转换和映射-2-错误转换和映射)
      - [2.1 自动转换 (`From` trait) {#21-自动转换-from-trait}](#21-自动转换-from-trait-21-自动转换-from-trait)
      - [2.2 映射错误 (`map_err`) {#22-映射错误-map\_err}](#22-映射错误-map_err-22-映射错误-map_err)
      - [2.3 错误类型转换矩阵 {#23-错误类型转换矩阵}](#23-错误类型转换矩阵-23-错误类型转换矩阵)
    - [3. 错误报告和日志 {#3-错误报告和日志}](#3-错误报告和日志-3-错误报告和日志)
      - [3.1 结构化日志集成 {#31-结构化日志集成}](#31-结构化日志集成-31-结构化日志集成)
      - [3.2 用户友好的错误报告 {#32-用户友好的错误报告}](#32-用户友好的错误报告-32-用户友好的错误报告)
      - [3.3 错误聚合和监控 {#33-错误聚合和监控}](#33-错误聚合和监控-33-错误聚合和监控)
    - [4. 测试错误处理 {#4-测试错误处理}](#4-测试错误处理-4-测试错误处理)
      - [4.1 测试错误类型 {#41-测试错误类型}](#41-测试错误类型-41-测试错误类型)
      - [4.2 测试错误传播 {#42-测试错误传播}](#42-测试错误传播-42-测试错误传播)
      - [4.3 测试错误处理逻辑 {#43-测试错误处理逻辑}](#43-测试错误处理逻辑-43-测试错误处理逻辑)
  - [反模式警示 {#反模式警示}](#反模式警示-反模式警示)
    - [❌ 反模式 1: 滥用 `unwrap()` {#反模式-1-滥用-unwrap}](#-反模式-1-滥用-unwrap-反模式-1-滥用-unwrap)
    - [❌ 反模式 2: 过度使用 `String` 作为错误 {#反模式-2-过度使用-string-作为错误}](#-反模式-2-过度使用-string-作为错误-反模式-2-过度使用-string-作为错误)
    - [❌ 反模式 3: 错误的 `?` 使用导致信息丢失 {#反模式-3-错误的-使用导致信息丢失}](#-反模式-3-错误的--使用导致信息丢失-反模式-3-错误的-使用导致信息丢失)
    - [❌ 反模式 4: 混淆 `Option` 和 `Result` {#反模式-4-混淆-option-和-result}](#-反模式-4-混淆-option-和-result-反模式-4-混淆-option-和-result)
    - [❌ 反模式 5: 忽略错误 {#反模式-5-忽略错误}](#-反模式-5-忽略错误-反模式-5-忽略错误)
    - [❌ 反模式 6: 过度详细的错误类型 {#反模式-6-过度详细的错误类型}](#-反模式-6-过度详细的错误类型-反模式-6-过度详细的错误类型)
    - [❌ 反模式 7: 跨线程边界传递非 Send 错误 {#反模式-7-跨线程边界传递非-send-错误}](#-反模式-7-跨线程边界传递非-send-错误-反模式-7-跨线程边界传递非-send-错误)
    - [❌ 反模式 8: 在热路径中创建错误字符串 {#反模式-8-在热路径中创建错误字符串}](#-反模式-8-在热路径中创建错误字符串-反模式-8-在热路径中创建错误字符串)
  - [代码示例 {#代码示例}](#代码示例-代码示例)
    - [完整示例 1: 分层错误处理架构 {#完整示例-1-分层错误处理架构}](#完整示例-1-分层错误处理架构-完整示例-1-分层错误处理架构)
    - [完整示例 2: 带重试和断路器的错误处理 {#完整示例-2-带重试和断路器的错误处理}](#完整示例-2-带重试和断路器的错误处理-完整示例-2-带重试和断路器的错误处理)
    - [完整示例 3: anyhow + thiserror 混合使用 {#完整示例-3-anyhow-thiserror-混合使用}](#完整示例-3-anyhow--thiserror-混合使用-完整示例-3-anyhow-thiserror-混合使用)
  - [附录 {#附录}](#附录-附录)
    - [A. 快速决策参考表 {#a-快速决策参考表}](#a-快速决策参考表-a-快速决策参考表)
    - [B. 常用 crate 对比 {#b-常用-crate-对比}](#b-常用-crate-对比-b-常用-crate-对比)
    - [C. 进一步阅读 {#c-进一步阅读}](#c-进一步阅读-c-进一步阅读)
...
| **示例** | 文件不存在、网络超时、无效输入 | 数组越界、空指针解引用（Reference）、内部不一致 |
...
| 边界类型 | 传播方式 | 注意事项 |
| :--- | :--- | :--- |
| 函数边界 | `Result<T, E>` + `?` | 保持错误类型一致性（Coherence） |
| 模块（Module）边界 | 定义模块级错误类型 | 使用 `thiserror` 派生 |
| Crate 边界 | 公开错误类型 | 文档化错误条件 |
| 线程边界 | `channel` + 专用错误类型 | 错误必须实现 `Send` |
| 异步（Async）边界 | `Result` in `Future` | 正确处理 `JoinHandle` |
...
> **[IEEE - Fault-Tolerant Software Design](https://ieeexplore.ieee.org/) <!-- link: known-broken -->**
...
> **来源: [RFC 2504 - Try Trait](https://rust-lang.github.io/rfcs/3058-try-trait-v2.html)**
> **来源: [Rust RFCs](https://github.com/rust-lang/rfcs)**
> **来源: [Rust Standard Library](https://doc.rust-lang.org/std/)**
> **来源: [POPL](https://www.sigplan.org/Conferences/POPL/)**
> **来源: [PLDI](https://www.sigplan.org/Conferences/PLDI/)**
