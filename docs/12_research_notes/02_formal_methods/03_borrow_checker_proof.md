...
> **权威来源（Canonical）**: 本文件为借用检查器证明研究笔记；通用 Rust 概念解释请以 concept 权威页为准：[`concept L4 借用检查可判定性`](../../../concept/04_formal/01_ownership_logic/04_borrow_checking_decidability.md) · [`concept L1 borrowing`](../../../concept/01_foundation/01_ownership_borrow_lifetime/02_borrowing.md)
>
...
- [借用检查器证明 {#借用检查器证明}](#借用检查器证明-借用检查器证明)
  ...
  - [📚 理论基础 {#理论基础}](#-理论基础-理论基础)
    - [相关概念 {#相关概念}](#相关概念-相关概念)
    - [理论背景 {#理论背景}](#理论背景-理论背景)
    - [数据竞争的形式化定义 {#数据竞争的形式化定义}](#数据竞争的形式化定义-数据竞争的形式化定义)
    - [借用检查算法的理论基础 {#借用检查算法的理论基础}](#借用检查算法的理论基础-借用检查算法的理论基础)
    - [借用规则正确性的理论基础 {#借用规则正确性的理论基础}](#借用规则正确性的理论基础-借用规则正确性的理论基础)
    ...
    - [CAV工具与Borrow Checker {#cav工具与borrow-checker}](#cav工具与borrow-checker-cav工具与borrow-checker)
    - [CMU 15-799 并发验证 {#cmu-15-799-并发验证}](#cmu-15-799-并发验证-cmu-15-799-并发验证)
      - [并发分离逻辑与 Rust {#并发分离逻辑与-rust}](#并发分离逻辑与-rust-并发分离逻辑与-rust)
      - [并发分离逻辑形式化 {#并发分离逻辑形式化}](#并发分离逻辑形式化-并发分离逻辑形式化)
  ...
  - [🆕 最新别名模型与操作语义（Tree Borrows / RustSEM / RustBelt / Oxide） {#最新别名模型与操作语义tree-borrows-rustsem-rustbelt-oxide}](#-最新别名模型与操作语义tree-borrows--rustsem--rustbelt--oxide-最新别名模型与操作语义tree-borrows-rustsem-rustbelt-oxide)
    ...
    - [3. RustBelt：Iris 分离逻辑与数据竞争自由证明 {#3-rustbeltiris-分离逻辑与数据竞争自由证明}](#3-rustbeltiris-分离逻辑与数据竞争自由证明-3-rustbeltiris-分离逻辑与数据竞争自由证明)
    - [4. Oxide：借用检查的类型规则视角 {#4-oxide借用检查的类型规则视角}](#4-oxide借用检查的类型规则视角-4-oxide借用检查的类型规则视角)
    - [5. 与 Miri 的协同验证图景 {#5-与-miri-的协同验证图景}](#5-与-miri-的协同验证图景-5-与-miri-的协同验证图景)
  - [🔬 形式化定义 {#形式化定义}](#-形式化定义-形式化定义)
    - [§1 概念定义层 {#1-概念定义层}](#1-概念定义层-1-概念定义层)
    - [§2 属性关系层（公理） {#2-属性关系层公理}](#2-属性关系层公理-2-属性关系层公理)
  - [🧮 定理与证明 {#定理与证明}](#-定理与证明-定理与证明)
    - [§3 解释论证层 {#3-解释论证层}](#3-解释论证层-3-解释论证层)
  ...
  - [✅ 证明目标 {#证明目标}](#-证明目标-证明目标)
    ...
    - [证明方法 {#证明方法}](#证明方法-证明方法)
  - [Rust 1.93 与并发/裸指针扩展（形式化占位） {#rust-193-与并发裸指针扩展形式化占位}](#rust-193-与并发裸指针扩展形式化占位-rust-193-与并发裸指针扩展形式化占位)
  - [unsafe 契约与 borrow/ownership 衔接（Phase 3） {#unsafe-契约与-borrowownership-衔接phase-3}](#unsafe-契约与-borrowownership-衔接phase-3-unsafe-契约与-borrowownership-衔接phase-3)
  - [控制流与借用衔接（Phase 5） {#控制流与借用衔接phase-5}](#控制流与借用衔接phase-5-控制流与借用衔接phase-5)
  - [FFI、extern、C variadic、? 操作符（Phase 6） {#ffiexternc-variadic-操作符phase-6}](#ffiexternc-variadic-操作符phase-6-ffiexternc-variadic-操作符phase-6)
  - [与 Aeneas 对比 {#与-aeneas-对比}](#与-aeneas-对比-与-aeneas-对比)
    - [方法对比 {#方法对比}](#方法对比-方法对比)
    - [可变借用的两种视角 {#可变借用的两种视角}](#可变借用的两种视角-可变借用的两种视角)
      - [本文视角：分离逻辑 {#本文视角分离逻辑}](#本文视角分离逻辑-本文视角分离逻辑)
      - [Aeneas 视角：预言变量 {#aeneas-视角预言变量}](#aeneas-视角预言变量-aeneas-视角预言变量)
    - [borrow\_generated\_from 与借用有效性 {#borrow\_generated\_from-与借用有效性}](#borrow_generated_from-与借用有效性-borrow_generated_from-与借用有效性)
      - [Aeneas: borrow\_generated\_from {#aeneas-borrow\_generated\_from}](#aeneas-borrow_generated_from-aeneas-borrow_generated_from)
      - [本文: Def 1.3 (借用有效性) {#本文-def-13-借用有效性}](#本文-def-13-借用有效性-本文-def-13-借用有效性)
    - [数据竞争自由的两种证明 {#数据竞争自由的两种证明}](#数据竞争自由的两种证明-数据竞争自由的两种证明)
      - [本文证明 (Theorem 1) {#本文证明-theorem-1}](#本文证明-theorem-1-本文证明-theorem-1)
      - [Aeneas 保证 {#aeneas-保证}](#aeneas-保证-aeneas-保证)
    - [证明负担对比 {#证明负担对比}](#证明负担对比-证明负担对比)
    - [互补使用场景 {#互补使用场景}](#互补使用场景-互补使用场景)
    - [与本文形式化的整合 {#与本文形式化的整合}](#与本文形式化的整合-与本文形式化的整合)
    - [参考文献 {#参考文献}](#参考文献-参考文献)
...
### MIT 课程对齐：内存安全与数据竞争自由 {#mit-课程对齐内存安全与数据竞争自由}

> **来源: [POPL](https://www.sigplan.org/Conferences/POPL/)**

#### MIT 6.826: Computer Systems Security 形式化对比 {#mit-6826-computer-systems-security-形式化对比}
...
| **Temporal Safety** | $\forall t: \text{Access}(addr, t) \rightarrow \text{Valid}(addr, t)$ | 借用状态追踪 + 所有权环境 | [ownership_model](09_ownership_model.md) 规则 2-3 |
...
| **Double-free** | $\exists t_1 < t_2: \text{Free}(addr, t_1) \land \text{Free}(addr, t_2)$ | 所有权唯一性保证单次释放 | [ownership_model](09_ownership_model.md) 定理 2 |
...
#### MIT 6.858: Computer Systems 与 Symbolic Execution {#mit-6858-computer-systems-与-symbolic-execution}
...
#### 数据竞争自由与 MIT 课程的联系 {#数据竞争自由与-mit-课程的联系}
...
| 6.858 并发安全（Concurrency Safety） | 锁、信号量（运行时（Runtime）） | `Mutex`、`RwLock` + 借用检查（编译期） |
...
| 6.858 | Lecture | Type safety → 借用规则 + 类型系统（Type System） | ✅ |

### CAV工具与Borrow Checker {#cav工具与borrow-checker}
...
### CMU 15-799 并发验证 {#cmu-15-799-并发验证}
...
#### Resource Invariants 与 Mutex {#resource-invariants-与-mutex}
...
| Linearizability | Def ATOMIC1 (原子操作（Atomic Operations）) | ✅ |
...
- **Rust 解决方案**:
  - 可变借用（Mutable Borrow） `&mut T` 保证在整个借用期间独占访问
...
## 🆕 最新别名模型与操作语义（Tree Borrows / RustSEM / RustBelt / Oxide） {#最新别名模型与操作语义tree-borrows-rustsem-rustbelt-oxide}
...
### 1. Tree Borrows：借用检查器的别名语义精化 {#1-tree-borrows借用检查器的别名语义精化}
...
### 2. RustSEM：内存级借用规则的可执行语义 {#2-rustsem内存级借用规则的可执行语义}
...
### 3. RustBelt：Iris 分离逻辑与数据竞争自由证明 {#3-rustbeltiris-分离逻辑与数据竞争自由证明}
...
### 4. Oxide：借用检查的类型规则视角 {#4-oxide借用检查的类型规则视角}
...
### 5. 与 Miri 的协同验证图景 {#5-与-miri-的协同验证图景}
...
- $Immutable$：不可变借用（Immutable Borrow） `&T`
- $Mutable$：可变借用 `&mut T`
...
- 根据 [send_sync_formalization](12_send_sync_formalization.md) 定理 SEND-T1：$\text{Send}(T) \to \text{SafeToTransferAcrossThreads}(T)$
...
- 引用生命周期超出作用域
- 借用检查器使用 lifetime_formalization 的生命周期推断
...
- 被引用对象在 $p$ 点已释放
- 根据 [ownership_model](09_ownership_model.md) 定义 2.2，所有权转移后原变量失效
...
- NLL 求解约束系统找到最小满足解
- 根据 lifetime_formalization 定理 LF-T1，约束求解正确
...
- 根据 [send_sync_formalization](12_send_sync_formalization.md) 定理 SEND-T1 和 SYNC-T1
- `Send` 保证跨线程转移安全
- `Sync` 保证跨线程共享安全
...
## ⚠️ 反例：违反借用规则导致数据竞争 {#反例违反借用规则导致数据竞争}
...
数据竞争自由与所有权唯一性共同保证内存安全，形式化见 [ownership_model](09_ownership_model.md)。
...
Polonius 与 lifetime_formalization 中的推断、约束求解一致。
...
满足 [borrow_checker_proof](03_borrow_checker_proof.md) 定理 1 数据竞争自由。
...
由 [borrow_checker_proof](03_borrow_checker_proof.md) Axiom 1。
...
与 [type_system_foundations](../05_type_theory/05_type_system_foundations.md) Def DEREF-NULL1 衔接。
...
则其与 [borrow_checker_proof](03_borrow_checker_proof.md) 定理 1 数据竞争自由相容。

形式化：$\text{pre}(C) \land \text{borrow\_compliant}(r) \rightarrow \text{DataRaceFree}(P)$。

由 unsafe 契约与借用规则无冲突。

**定理 UNSAFE-T2（unsafe 与所有权衔接）**：`unsafe` 中对 `Box::from_raw`、`ManuallyDrop` 等使用若保持 [ownership_model](09_ownership_model.md) 规则 2、3（唯一所有者、作用域释放），则与定理 3 内存安全相容。
...
**定理 MATCH-T1（穷尽性与不可达）**：穷尽 match 保证所有值被处理；若存在 `_ => unreachable!()` 或 `!` 分支，则类型系统保证该分支不可达。与 [borrow_checker_proof](03_borrow_checker_proof.md) Axiom 3 结合：match 各分支内借用作用域独立，无跨分支冲突。

**Def FOR1（for 迭代与借用）**：`for x in iter` 等价于 `IntoIterator::into_iter` 取得所有权或借用；迭代期间 `iter` 被可变借用（`IterMut`）或不可变借用（`Iter`）；迭代中修改集合违反借用规则。

**定理 FOR-T1（迭代中修改集合）**：`for x in &mut v { v.push(...); }` 被借用检查器拒绝——`&mut v` 与 `v.push` 的 `&mut self` 冲突。由 [borrow_checker_proof](03_borrow_checker_proof.md) Axiom 1 直接保证。
...
**定理 EXTERN-T1**：extern 函数与借用检查器边界：Rust 仅检查 Rust 侧；调用 extern 时传入的引用若在 extern 调用期间被外部代码修改，则需 `unsafe` 契约保证。与 [borrow_checker_proof](03_borrow_checker_proof.md) 定理 1 相容——extern 不引入 Rust 借用冲突。
...
<a id="定理-query-t1"></a>**定理 QUERY-T1**：`?` 与借用：`e` 在 `?` 前求值完成，借用可结束；`?` 所在函数返回类型与 `e` 的 `E` 相容；不影响 [borrow_checker_proof](03_borrow_checker_proof.md) Axiom 1、2。
...
### borrow_generated_from 与借用有效性 {#borrow_generated_from-与借用有效性}

> **来源: [Rust Reference - doc.rust-lang.org/reference](https://doc.rust-lang.org/reference/)**

#### Aeneas: borrow_generated_from {#aeneas-borrow_generated_from}
...
#### 本文: Def 1.3 (借用有效性) {#本文-def-13-借用有效性}
...
### 数据竞争自由的两种证明 {#数据竞争自由的两种证明}

> **来源: [ACM](https://dl.acm.org/)**

#### 本文证明 (Theorem 1) {#本文证明-theorem-1}
...
| 本文 | Aeneas | 整合方式 |
| :--- | :--- | :--- |
| Def 1.1 (借用类型) | `BorrowKind` 枚举（Enum） | 直接对应 |
...
| 类型 | 位置 |
| :--- | :--- |
| 思维导图 | [MIND_MAP_COLLECTION](../../07_thinking/03_mind_map_collection.md) §2、C01；本文 §思维导图 |
| 多维矩阵 | [MULTI_DIMENSIONAL_CONCEPT_MATRIX](../../07_thinking/04_multi_dimensional_concept_matrix.md) §1；README §六篇并表 第 2 行 |
| 证明树 | 本文 §证明树；[PROOF_GRAPH_NETWORK](../../07_thinking/05_proof_graph_network.md) |
...
> **来源: [Wikipedia - Borrow Checker](https://en.wikipedia.org/wiki/Borrow_Checker)**
> **来源: [Wikipedia - Memory Safety](https://en.wikipedia.org/wiki/Memory_Safety)**
