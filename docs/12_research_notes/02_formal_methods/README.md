...
> **状态**: ✅ 已完成
> **docs 全结构**: DOCS_STRUCTURE_OVERVIEW § 2.7 formal_methods
...
- **内存与所有权（Ownership）**：Box、Rc/Arc、Cell/RefCell、MaybeUninit、Deref/Drop、repr、const &mut static（Phase 1–6）
- **并发与异步（Async）**：通道、Mutex/RwLock、原子操作（Atomic Operations）、thread::spawn（Phase 2、4、6）
- **FFI 与 unsafe**：裸指针、union、transmute、extern、C variadic（Phase 3、4、6）
- **控制流**：match、for、? 操作符（Phase 5、6）

**已完成**：所有权规则 1–8、借用（Borrowing）规则、生命周期（Lifetimes）、Pin、async 状态机核心定理；Phase 1–6 全部形式化。✅ **100% 完成**。
...
- [borrow_checker_proof](03_borrow_checker_proof.md) Def MATCH1/FOR1/QUERY1、定理 MATCH-T1/FOR-T1/QUERY-T1
- [type_system_foundations](../05_type_theory/05_type_system_foundations.md) 定理 T2（保持性）、T3（类型安全）
...
- [🔬 形式化方法研究 {#形式化方法研究}](#-形式化方法研究-形式化方法研究)
  ...
  - [控制流形式化 {#控制流形式化}](#控制流形式化-控制流形式化)
  - [📊 目录 {#目录}](#-目录-目录)
  - [🎯 研究目标 {#研究目标}](#-研究目标-研究目标)
  - [📚 研究主题 {#研究主题}](#-研究主题-研究主题)
    - [1. 所有权模型形式化 {#1-所有权模型形式化}](#1-所有权模型形式化-1-所有权模型形式化)
    - [2. 借用检查器证明 {#2-借用检查器证明}](#2-借用检查器证明-2-借用检查器证明)
    - [3. 异步状态机形式化 {#3-异步状态机形式化}](#3-异步状态机形式化-3-异步状态机形式化)
    - [4. 生命周期形式化 {#4-生命周期形式化}](#4-生命周期形式化-4-生命周期形式化)
    - [5. Pin 和自引用（Reference）类型形式化 {#5-pin-和自引用类型形式化}](#5-pin-和自引用类型形式化-5-pin-和自引用类型形式化)
    - [6. Send/Sync 形式化 {#6-sendsync-形式化}](#6-sendsync-形式化-6-sendsync-形式化)
    - [7. Actor 模型形式化 ⭐ 新增 {#7-actor-模型形式化-新增}](#7-actor-模型形式化--新增-7-actor-模型形式化-新增)
  - [formal\_methods 六篇并表 {#formal\_methods-六篇并表}](#formal_methods-六篇并表-formal_methods-六篇并表)
  - [形式化论证汇总 {#形式化论证汇总}](#形式化论证汇总-形式化论证汇总)
  - [公理-定理形式化索引 {#公理-定理形式化索引}](#公理-定理形式化索引-公理-定理形式化索引)
  - [📝 研究笔记 {#研究笔记}](#-研究笔记-研究笔记)
    - [已完成 ✅ {#已完成}](#已完成--已完成)
  - [🔗 相关资源 {#相关资源}](#-相关资源-相关资源)
    - [核心文档 {#核心文档}](#核心文档-核心文档)
    - [代码实现 {#代码实现}](#代码实现-代码实现)
    - [学术资源 {#学术资源}](#学术资源-学术资源)
  - [国际权威对标（Authoritative References） {#国际权威对标authoritative-references}](#国际权威对标authoritative-references-国际权威对标authoritative-references)
    - [权威来源对照表 {#权威来源对照表}](#权威来源对照表-权威来源对照表)
    - [权威论文与规范（含链接） {#权威论文与规范含链接}](#权威论文与规范含链接-权威论文与规范含链接)
    - [与本目录的对应说明 {#与本目录的对应说明}](#与本目录的对应说明-与本目录的对应说明)
    - [权威来源快速链接 {#权威来源快速链接}](#权威来源快速链接-权威来源快速链接)
    - [Ferrocene FLS 章节与本目录对应 {#ferrocene-fls-章节与本目录对应}](#ferrocene-fls-章节与本目录对应-ferrocene-fls-章节与本目录对应)
    - [国际权威奖项与认可 {#国际权威奖项与认可}](#国际权威奖项与认可-国际权威奖项与认可)
  - [📖 研究方法 {#研究方法}](#-研究方法-研究方法)
    - [形式化工具 {#形式化工具}](#形式化工具-形式化工具)
    - [形式化方法 {#形式化方法}](#形式化方法-形式化方法)
    - [证明策略 {#证明策略}](#证明策略-证明策略)
  - [🚀 快速开始 {#快速开始}](#-快速开始-快速开始)
    - [创建新的研究笔记 {#创建新的研究笔记}](#创建新的研究笔记-创建新的研究笔记)
    - [研究流程 {#研究流程}](#研究流程-研究流程)
...
1. **所有权系统**：形式化定义所有权规则，证明内存安全（Memory Safety）
2. **借用检查器**：形式化借用规则，证明数据竞争自由
3. **异步系统**：形式化 Future/Poll 状态机，证明并发安全（Concurrency Safety）
...
**相关笔记**: [09_ownership_model.md](09_ownership_model.md)
...
**相关笔记**: [03_borrow_checker_proof.md](03_borrow_checker_proof.md)
...
**相关笔记**: [02_async_state_machine.md](02_async_state_machine.md)
...
**相关笔记**: 06_lifetime_formalization.md
...
**相关笔记**: [12_send_sync_formalization.md](12_send_sync_formalization.md)
...
## formal_methods 六篇并表 {#formal_methods-六篇并表}

下表为本目录六篇核心文档的**概念×公理×定理×证明方法×反例**并表；用于跨篇对比与 [HIERARCHICAL_MAPPING_AND_SUMMARY](../06_concept_models/12_hierarchical_mapping_and_summary.md) 衔接。各子文档内可注明「本概念在 README §formal_methods 六篇并表 第 x 行」。

| 概念 | 公理/定义 | 定理 | 证明方法 | 反例 |
| :--- | :--- | :--- | :--- | :--- |
| **所有权（Ownership）** | 规则 1–3, Def 1.1–1.5 | T2 唯一性, T3 内存安全（Memory Safety） | 结构归纳、反证 | [ownership_model](09_ownership_model.md) 使用已移动值、双重释放 |
| **借用（Borrowing）** | 规则 5–8 | T1 数据竞争自由, T2 | 规则归纳 | [borrow_checker_proof](03_borrow_checker_proof.md) 双重可变借用（Mutable Borrow） |
...
**推论 FM-C1**：若 $P$ 违反 $\mathcal{M}$ 中任一定理，则 $P$ 非 Safe 或非良型；反例见 [FORMAL_PROOF_SYSTEM_GUIDE](../03_formal_proofs/16_formal_proof_system_guide.md) 反例索引。
...
| [ownership_model](09_ownership_model.md) | 所有权规则 1–8、T2/T3、RC/ARC/CELL/REFCELL/BOX、MAYBEUNINIT/ATOMIC/UNION/TRANSMUTE、DROP/DEREF/REPR/CONST_MUT_STATIC | 唯一性、RAII、Rc/Arc、Cell/RefCell、MaybeUninit、atomic、union、transmute、Drop/Deref、repr、const &mut static |
| [borrow_checker_proof](03_borrow_checker_proof.md) | 借用规则、T1、CHAN/MUTEX/RAW、UNSAFE、MATCH/FOR、EXTERN/CVARIADIC/QUERY | 互斥借用、通道、Mutex、裸指针、unsafe 契约、match/for/?、extern、C variadic |
...
- [x] [所有权模型形式化](09_ownership_model.md) - 100%；含 RC/ARC/CELL/REFCELL/BOX 扩展
- [x] [借用检查器证明](03_borrow_checker_proof.md) - 100%；含 CHAN/MUTEX/RAW 扩展
- [x] [异步状态机形式化](02_async_state_machine.md) - 100%
- x] [生命周期形式化 - 100%
- [x] [Pin 和自引用类型形式化](10_pin_self_referential.md) - 100%
- [x] [Send/Sync 形式化](12_send_sync_formalization.md) - 100%；Def SEND1/SYNC1、SEND-T1/SYNC-T1、与 spawn/Future/Arc 衔接
...
- [形式化论证系统梳理指南](../03_formal_proofs/16_formal_proof_system_guide.md) - 论证缺口分析、反例索引、公理-定理证明树
- [形式化工程系统 - 理论基础](../../15_rust_formal_engineering_system/01_theoretical_foundations/README.md)
...
| FLS 章节 | 直接链接 | 本目录对应 |
| :--- | :--- | :--- |
| [Ch. 5 Patterns](https://spec.ferrocene.dev/patterns.html) | 5.1 [Refutability](https://spec.ferrocene.dev/patterns.html#refutability)、5.13 [Pattern Matching](https://spec.ferrocene.dev/patterns.html#pattern-matching) | [02_reference/quick_reference/02_control_flow_functions_cheatsheet.md](../../03_reference/quick_reference/09_control_flow_functions_cheatsheet.md) |
| [Ch. 15 Ownership and Destruction](https://spec.ferrocene.dev/ownership-and-deconstruction.html) | 15.1 [Ownership](https://spec.ferrocene.dev/ownership-and-deconstruction.html#ownership)、15.4 [Borrowing](https://spec.ferrocene.dev/ownership-and-deconstruction.html#borrowing)、15.6–15.9 [Destruction](https://spec.ferrocene.dev/ownership-and-deconstruction.html#destruction) | [ownership_model](09_ownership_model.md)、[borrow_checker_proof](03_borrow_checker_proof.md) Def OW1、规则 1–8、DROP1 |
| [Ch. 16 Exceptions and Errors](https://spec.ferrocene.dev/exceptions-and-errors.html) | 16.1 [Panic](https://spec.ferrocene.dev/exceptions-and-errors.html#panic)、16.2 [Abort](https://spec.ferrocene.dev/exceptions-and-errors.html#abort) | [02_reference/quick_reference/02_error_handling_cheatsheet.md](../../03_reference/quick_reference/10_error_handling_cheatsheet.md)、[EDGE_CASES_AND_SPECIAL_CASES](../../03_reference/02_edge_cases_and_special_cases.md) |
...
1. **操作语义**: 定义程序执行的行为
2. **类型系统（Type System）**: 定义类型规则和类型检查
3. **霍尔逻辑**: 证明程序正确性
4. **模型检查**: 验证系统属性
...
1. 复制模板文件（如 `09_ownership_model.md`）
2. 填写研究问题和目标
3. 添加形式化定义和证明
4. 提供代码示例和验证
5. 更新本 README 的链接
...
**状态**: ✅ **100% 完成**；Phase 1–6 全部补全，见 [00_completeness_gaps](00_completeness_gaps.md)。**意见与可持续推进**（Send/Sync 独立形式化、安全可判定机制全面梳理、完备特性对比、思维表征四类绑定）：[SAFE_DECIDABLE_MECHANISMS_AND_FORMAL_METHODS_PLAN](11_safe_decidable_mechanisms_and_formal_methods_plan.md)。**完备性检查表**（六篇×六维）：[FORMAL_METHODS_COMPLETENESS_CHECKLIST](05_formal_methods_completeness_checklist.md)。
