...
> **权威来源（Canonical）**: 本文件为所有权反例集（反例，独特内容）；通用 Rust 概念解释请以 concept 权威页为准：[`concept L1 ownership`](../../../concept/01_foundation/01_ownership_borrow_lifetime/01_ownership.md)
>
> 根据 AGENTS.md §2 Canonical 规则：本文仅保留本文独特内容（所有权反例与边界（反例集，非概念正文）），不重复 concept/ 中的概念定义、规则与定理推导。

> **EN**: Ownership Counterexamples
> **Summary**: 所有权与借用（Borrowing）反例边界 Ownership Counterexamples.
> **内容分级**: [核心级]
> **层级**: L6 (反例边界)
> **Bloom 层级**: L5-L6
...
运行时（Runtime）数据竞争、未定义行为（UB）。编译器不会报错，因为 `unsafe impl` 是用户承诺。
...
- [所有权模型](09_ownership_model.md)
- [借用检查器证明](03_borrow_checker_proof.md)
- [生命周期形式化](../05_type_theory/03_lifetime_formalization.md)
- [Pin 与自引用](10_pin_self_referential.md)
- [Send/Sync 形式化](12_send_sync_formalization.md)
