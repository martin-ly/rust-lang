...
| 文档 | 说明 |
| :--- | :--- |
| [variance_theory](../05_type_theory/06_variance_theory.md) | 型变理论形式化定义与定理 |
| lifetime_formalization | 生命周期形式化（型变在生命周期中的体现）|
| [MIND_MAP_COLLECTION](../../07_thinking/03_mind_map_collection.md) | 思维导图集合索引 |
...
**原因**: 这些类型允许内部修改，如果允许型变，可能通过子类型关系绕过借用（Borrowing）规则。
...
| **Tree Borrows** | 借用树中的只读子树可共享，等价于「共享引用（Reference）对生命周期/内容是协变的」；独占写子树要求不变。 |
...
**推论（型变必要性再确认）**: `&mut T` 的不变性不是实现细节，而是上述所有形式化模型保持一致性（Coherence）的共同前提。
...
| 泛型（Generics）结构体设计 | ✅ 中 | 含 `&T`/`&mut T`/`fn(T)` 时影响子类型 |
| 闭包（Closures）/回调类型 | ✅ 中 | 函数指针参数逆变 |
