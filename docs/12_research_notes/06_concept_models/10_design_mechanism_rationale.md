...
- [Rust 设计机制论证：理由与完整论证 {#rust-设计机制论证理由与完整论证}](#rust-设计机制论证理由与完整论证-rust-设计机制论证理由与完整论证)
  - [📑 目录 {#目录}](#-目录-目录)
  ...
  - [📍 Pin：堆/栈区分使用场景的完整论证 {#pin堆栈区分使用场景的完整论证}](#-pin堆栈区分使用场景的完整论证-pin堆栈区分使用场景的完整论证)
    - [1. 问题动机：为何需要 Pin？ {#1-问题动机为何需要-pin}](#1-问题动机为何需要-pin-1-问题动机为何需要-pin)
    - [2. 设计决策：为何区分堆与栈？ {#2-设计决策为何区分堆与栈}](#2-设计决策为何区分堆与栈-2-设计决策为何区分堆与栈)
    - [3. 形式化论证：堆/栈固定语义 {#3-形式化论证堆栈固定语义}](#3-形式化论证堆栈固定语义-3-形式化论证堆栈固定语义)
    - [4. 决策树：Pin 使用场景选型 {#4-决策树pin-使用场景选型}](#4-决策树pin-使用场景选型-4-决策树pin-使用场景选型)
    - [5. 反例：违反堆/栈约束的后果 {#5-反例违反堆栈约束的后果}](#5-反例违反堆栈约束的后果-5-反例违反堆栈约束的后果)
  - [🔒 所有权：为何采用移动语义而非复制语义？ {#所有权为何采用移动语义而非复制语义}](#-所有权为何采用移动语义而非复制语义-所有权为何采用移动语义而非复制语义)
  - [📐 借用：为何可变借用独占？ {#借用为何可变借用独占}](#-借用为何可变借用独占-借用为何可变借用独占)
  - [⏱️ 生命周期：为何需要显式标注？ {#生命周期为何需要显式标注}](#️-生命周期为何需要显式标注-生命周期为何需要显式标注)
...
**核心问题**：自引用（Reference）类型在移动时会导致悬垂指针。移动改变内存地址，但自引用仍指向旧地址。
...
**Pin 的设计目标**：通过类型系统（Type System）保证被 Pin 的值**位置稳定**（不被移动），从而自引用安全。
...
*证明*：`Box` 的所有权保证其指向的堆块不被移动；`Pin` 包装后禁止通过 `Pin` 接口进行移动操作。见 [pin_self_referential](../02_formal_methods/10_pin_self_referential.md) 定理 1。
...
**动机**：无 GC 下需确保内存安全（Memory Safety）（无悬垂、无双重释放、无泄漏）。
...
**定理 OM-T1**：默认移动 + 显式 Copy 保证无双重释放、无泄漏。*证明*：由 [ownership_model](../02_formal_methods/09_ownership_model.md) T2、T3；移动转移所有权，Copy 创建副本，二者均由唯一所有者管理。∎

**论证**：

- 若默认复制：大对象、RAII 资源复制语义不明确，易泄漏或双重释放。
- 若默认移动：每个值恰有一个所有者，作用域结束自动释放；复制需显式，语义清晰。
- 形式化：见 [ownership_model](../02_formal_methods/09_ownership_model.md) 定理 2、3。
...
**动机**：避免数据竞争。多线程下，若允许多个可变借用（Borrowing），则可能同时写同一内存。

**设计决策**：可变借用独占，不可变借用（Immutable Borrow）可多个但不可与可变并存。

**Def BC1（借用互斥）**：对同一位置 $x$，$\text{borrow}_{\text{mut}}(x)$ 有效时，$\text{borrow}(x)$ 与 $\text{borrow}_{\text{mut}}(x)$ 均不可并存。

**Axiom BC1**：借用规则 5–8（见 [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md)）静态可检查；违反则编译错误。

**定理 BC-T1**：满足借用规则则数据竞争自由。*证明*：由 [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) T1；互斥保证无并发写。∎

**论证**：

- 互斥规则：$\text{borrow}_{\text{mut}}(x) \Rightarrow \neg \text{borrow}(x) \land \neg \text{borrow}_{\text{mut}}(x)$（其他引用）。
- 由 [borrow_checker_proof](../02_formal_methods/03_borrow_checker_proof.md) 定理 1：满足规则则数据竞争自由。

**反例**：双重可变借用 → 编译错误。

---

## ⏱️ 生命周期：为何需要显式标注？ {#生命周期为何需要显式标注}
...
- 推断可覆盖多数情况；复杂泛型（Generics）、多参数、返回引用时需显式。
...
**论证**：见 [variance_theory](../05_type_theory/06_variance_theory.md)。若 `&mut T` 协变→可写入短生命周期值→悬垂。若 `fn(T)` 参数协变→可传入长寿引用期望短寿→悬垂。
...
**设计决策**：`Future::poll(self: Pin<&mut Self>)`，运行时（Runtime）用 `Pin<Box<dyn Future>>` 等堆固定。

**论证**：

- 自引用 Future 若被移动→悬垂。
- Pin 保证 poll 间不移动；`Box::pin` 在堆上固定，满足 Pin 保证。
- 见 [async_state_machine](../02_formal_methods/02_async_state_machine.md)、[pin_self_referential](../02_formal_methods/10_pin_self_referential.md)。
...
- **形式化专篇**：[send_sync_formalization](../02_formal_methods/12_send_sync_formalization.md) Def SEND1/SYNC1、SEND-T1/SYNC-T1、SYNC-L1、与 spawn/Future/Arc 衔接；[async_state_machine](../02_formal_methods/02_async_state_machine.md) 定理 6.2 依赖 Send/Sync 约束。
...
- 形式化：见 [trait_system_formalization](../05_type_theory/04_trait_system_formalization.md) 定理 1–3。
...
**反例**：宏（Macro）意外捕获外部变量；非卫生过程宏导致命名冲突。
...
**动机**：函数式编程、回调、迭代器（Iterator）适配。需捕获环境变量。
...
**反例**：闭包捕获可变引用（Mutable Reference）后再次借用；跨线程传递非 Send 闭包（Closures）。
...
**动机**：避免 null 引用、显式错误处理（Error Handling）。强制调用者处理「无值」或「错误」。
...
*证明*：由 match 穷尽规则；`?` 操作符将 `Err` 传播至调用者，调用者需处理或标注 `-> Result<_, E>`。见 [LANGUAGE_SEMANTICS_EXPRESSIVENESS](../03_formal_proofs/20_language_semantics_expressiveness.md) 定理 3.1。∎

**推论 OR-C1**：`Option`/`Result` 与构造性逻辑 $T \lor E$ 对应；`!` (never) 对应 $\bot$；无隐式 null。

**论证**：类型系统强制处理 None/Err；`?` 操作符传播错误；构造性逻辑（Curry-Howard）对应 $T \lor E$。见 [LANGUAGE_SEMANTICS_EXPRESSIVENESS](../03_formal_proofs/20_language_semantics_expressiveness.md)。
...
| 类型 | 位置 |
| :--- | :--- |
| 思维导图 | [MIND_MAP_COLLECTION](../../07_thinking/03_mind_map_collection.md) §8 设计机制论证 |
| 决策树 | 本文各机制「使用场景/决策树」；[DECISION_GRAPH_NETWORK](../../07_thinking/02_decision_graph_network.md) |
| 多维矩阵 | [DESIGN_MECHANISM_RATIONALE 矩阵总览](#设计机制论证矩阵总览)；[UNIFIED_SYSTEMATIC_FRAMEWORK](17_unified_systematic_framework.md) |

*依据*：[HIERARCHICAL_MAPPING_AND_SUMMARY](12_hierarchical_mapping_and_summary.md) § 文档↔思维表征。
...
| 文档 | 用途 |
| :--- | :--- |
| [pin_self_referential](../02_formal_methods/10_pin_self_referential.md) | Pin 形式化定义、定理、反例 |
| [COMPREHENSIVE_SYSTEMATIC_OVERVIEW](../13_meta_reports/06_comprehensive_systematic_overview.md) | 全面系统化梳理、语义归纳 |
| [LANGUAGE_SEMANTICS_EXPRESSIVENESS](../03_formal_proofs/20_language_semantics_expressiveness.md) | 构造性语义、表达能力边界 |
| [FORMAL_PROOF_SYSTEM_GUIDE](../03_formal_proofs/16_formal_proof_system_guide.md) | 论证缺口、概念-公理-定理映射 |
| [MIND_MAP_COLLECTION](../../07_thinking/03_mind_map_collection.md) | 设计机制论证思维导图（§8） |
