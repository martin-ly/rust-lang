...
> **权威来源（Canonical）**: 本文件为类型系统构造能力研究入口（TCON 矩阵）；通用 Rust 概念解释请以 concept 权威页为准：[`concept L2 类型系统`](../../../concept/01_foundation/02_type_system/01_type_system.md)
>
...
- [类型系统构造能力 {#类型系统构造能力}](#类型系统构造能力-类型系统构造能力)
  - [📑 目录 {#目录}](#-目录-目录)
  - [文档宗旨 {#文档宗旨}](#文档宗旨-文档宗旨)
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
  - [TCON 矩阵 {#tcon-矩阵}](#tcon-矩阵-tcon-矩阵)
  - [Rust 1.93 新增类型/表达式 {#rust-193-新增类型表达式}](#rust-193-新增类型表达式-rust-193-新增类型表达式)
  - [类型理论缺口对构造能力的影响 {#类型理论缺口对构造能力的影响}](#类型理论缺口对构造能力的影响-类型理论缺口对构造能力的影响)
  - [类型构造决策树 {#类型构造决策树}](#类型构造决策树-类型构造决策树)
  - [确定性判定 {#确定性判定}](#确定性判定-确定性判定)
  - [与已有证明衔接 {#与已有证明衔接}](#与已有证明衔接-与已有证明衔接)
...
**定理 TCON-T1（构造与类型安全）**：若 $\tau$ 可构造（$\mathit{Determinism}(\tau) \neq \mathrm{Impossible}$），则构造出的值 $v$ 满足 $\Gamma \vdash v : \tau$（由 [type_system_foundations](05_type_system_foundations.md) 保持性 T2）。

*证明*：由 [type_system_foundations](05_type_system_foundations.md) 定理 2（保持性）；构造即求值，求值后类型保持。∎
...
| **GAT** | `T::Item<'a>` | Unique（约束唯一） | 见 [advanced_types](01_advanced_types.md) |
| **const 泛型** | `[T; N]` 其中 `const N` | Unique（N 为 const 表达式） | 见 [advanced_types](01_advanced_types.md) |
...
以下 Rust 1.93 新增或变更的特性影响类型构造能力；详见 [07_rust_1.93_full_changelog](../../09_toolchain/04_rust_1_93_full_changelog.md)、[00_completeness_gaps](../02_formal_methods/00_completeness_gaps.md)。
...
[00_completeness_gaps](../02_formal_methods/00_completeness_gaps.md) 中 ⚠️ 部分缺口可能影响类型构造路径；本表标注其对 TCON 的影响。
...
*证明*：由 [type_system_foundations](05_type_system_foundations.md) 类型推导算法；约束不足则多解或无解；显式注解等价于添加约束。∎
...
| 文档 | 衔接点 |
| :--- | :--- |
| [type_system_foundations](05_type_system_foundations.md) | 类型环境、类型判断、进展性 T1、保持性 T2；构造即求值 |
| [trait_system_formalization](04_trait_system_formalization.md) | impl Trait、dyn Trait、关联类型、GAT；coherence、对象安全 |
| [advanced_types](01_advanced_types.md) | GAT、const 泛型、impl Trait 捕获规则 |
| [variance_theory](06_variance_theory.md) | 型变影响子类型；某些构造路径受型变约束 |
| [00_completeness_gaps](../02_formal_methods/00_completeness_gaps.md) | LUB coercion、Copy specialization 等 1.93 构造相关缺口 |
...
| 文档 | 用途 |
| :--- | :--- |
| [type_system_foundations](05_type_system_foundations.md) | 类型系统基础、类型安全定理 |
| [trait_system_formalization](04_trait_system_formalization.md) | Trait、泛型、impl/dyn |
| [advanced_types](01_advanced_types.md) | GAT、const 泛型 |
| [RUST_193_LANGUAGE_FEATURES_COMPREHENSIVE_ANALYSIS](../12_version_research/01_rust_193_language_features_comprehensive_analysis.md) | 92 项特性与类型族对应 |
| [UNIFIED_SYSTEMATIC_FRAMEWORK](../06_concept_models/17_unified_systematic_framework.md) | 全局矩阵与决策树索引 |
