...
以下定义依赖 [type_system_foundations](05_type_system_foundations.md) 与 [trait_system_formalization](04_trait_system_formalization.md) 中的基础概念：
...
**Axiom AT1**：GAT 类型推导规则与 [type_system_foundations](05_type_system_foundations.md) 定理 4、5 一致；约束违反则编译错误。

**Axiom AT2**：const 泛型参数必须为编译时常量；运行时常量不能作为类型参数；违反则编译错误。

**定理 AT-T1 (GAT 类型安全)**：若 GAT $A[P]$ 的类型推导正确，则 GAT 的使用是类型安全的。

*证明*：由 Def 1.1–1.3；GAT 约束 $A[P] : B[P]$ 在类型推导时检查；[type_system_foundations](05_type_system_foundations.md) 定理 4、5 保证推导正确性；违反则编译错误。依 Axiom AT1。∎
...
**引理 AT-L1 (GAT 与 trait 衔接)**：GAT 约束 $A[P] : B[P]$ 在 `impl` 解析时检查；满足 [trait_system_formalization](04_trait_system_formalization.md) 解析正确性。
...
- GATs RFC
- const 泛型 RFC
- [高级类型与 GATs（本篇）](01_advanced_types.md)
...
GATs 扩展 Trait 的关联类型为类型级函数；const 泛型参与类型与值层面的推导；与 [type_system_foundations](05_type_system_foundations.md)、[trait_system_formalization](04_trait_system_formalization.md) 在扩展后的类型规则下一致。
