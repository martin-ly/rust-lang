...
- [二、子系统详细设计](#二子系统详细设计)
  - [1. 命令处理子系统](#1-命令处理子系统)
    - [1.1 命令处理流程](#11-命令处理流程)
    - [1.2 代表性命令处理器实现](#12-代表性命令处理器实现)
  - [2. 事件处理子系统](#2-事件处理子系统)
    - [2.1 事件总线实现](#21-事件总线实现)
    - [2.2 订单创建事件处理](#22-订单创建事件处理)
  - [3. 工作流子系统](#3-工作流子系统)
    - [3.1 Temporal工作流集成](#31-temporal工作流集成)
    - [3.2 工作流活动实现](#32-工作流活动实现)
  - [4. 外部系统集成子系统](#4-外部系统集成子系统)
    - [4.1 抽象服务客户端](#41-抽象服务客户端)
  - [4. 外部系统集成子系统（续）](#4-外部系统集成子系统续)
    - [4.1 抽象服务客户端（续）](#41-抽象服务客户端续)
    - [4.2 外部系统适配器工厂](#42-外部系统适配器工厂)
  - [5. 查询服务子系统](#5-查询服务子系统)
    - [5.1 CQRS查询层](#51-cqrs查询层)
    - [5.2 API端点](#52-api端点)
  - [6. 监控与可观测性子系统](#6-监控与可观测性子系统)
    - [6.1 分布式追踪集成](#61-分布式追踪集成)
    - [6.2 指标监控实现](#62-指标监控实现)
  - [6. 监控与可观测性子系统（续）](#6-监控与可观测性子系统续)
    - [6.2 指标监控实现（续）](#62-指标监控实现续)
  - [7. 配置与服务发现子系统](#7-配置与服务发现子系统)
    - [7.1 动态配置管理](#71-动态配置管理)
    - [7.2 服务注册与发现](#72-服务注册与发现)
- [三、数据模型设计](#三数据模型设计)
  - [1. 事件存储表结构](#1-事件存储表结构)
  - [2. 读模型表结构](#2-读模型表结构)
- [四、部署架构](#四部署架构)
  - [1. 容器化与服务编排](#1-容器化与服务编排)
- [四、部署架构（续）](#四部署架构续)
  - [1. 容器化与服务编排（续）](#1-容器化与服务编排续)
...
- [六、总结与最佳实践](#六总结与最佳实践)
  - [1. 架构设计关键点](#1-架构设计关键点)
  - [2. Rust实现优势](#2-rust实现优势)
  - [3. 集成开源库最佳实践](#3-集成开源库最佳实践)
  - [4. 最终架构特点](#4-最终架构特点)
