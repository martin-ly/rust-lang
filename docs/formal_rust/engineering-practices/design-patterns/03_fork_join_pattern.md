...
- [6. 应用示例](#6-应用示例)
  ...
  - [6.2 并发矩阵乘法](#62-并发矩阵乘法)
  - [6.3 并发快速排序](#63-并发快速排序)
- [7. 形式化验证](#7-形式化验证)
  - [7.1 计算正确性](#71-计算正确性)
  - [7.2 并发保证](#72-并发保证)
- [8. 高级特质](#8-高级特质)
...
Fork-Join 模式是一种并发编程模型，通过分治策略将任务分解为子任务并发执行，然后合并结果。
...
2. 并发执行：$\forall t_1, t_2 \in T: t_1 \parallel t_2 \implies \text{execute}(t_1) \parallel \text{execute}(t_2)$
...
**定理 7.2 (并发保证)** Fork-Join 系统满足并发保证：
$$\forall t_1, t_2 \in T: t_1 \parallel t_2 \implies \text{execute}(t_1) \parallel \text{execute}(t_2)$$
