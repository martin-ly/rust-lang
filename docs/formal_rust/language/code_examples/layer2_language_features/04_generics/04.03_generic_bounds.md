...
泛型约束（trait bound）用于限制泛型参数必须实现某些trait，保证泛型代码的类型安全和功能完备。
Rust通过`T: Trait`语法实现trait bound，支持多重约束和where子句。
相关理论详见[trait bound理论](../../04_generics/02_trait_bounds.md)与[trait系统](../../12_traits/01_trait_system.md)。
