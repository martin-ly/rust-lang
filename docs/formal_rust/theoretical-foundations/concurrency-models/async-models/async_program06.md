...
2. **并发性(Parallelism)**：
   操作A与操作B并发，当且仅当A与B在相同的时间间隔内物理上同时执行。
...
1. **异步并发(Async Parallel)**：
   - 每个并发单元使用异步模型
   - 并发工作器内部使用事件循环

2. **并发流处理(Parallel Streaming)**：
   - 流水线并发
   - 异步消息传递
