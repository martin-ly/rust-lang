...
`Pin`类型解决了异步Rust 中最棘手的问题之一：自借用结构。在异步状态机中，某些状态可能包含指向自身其他部分的借用：

```rust
async fn self_referential() {
    let mut s = String::from("Hello");
    let ptr = &mut s;  // 创建指向s的借用
...
问题在于，如果状态机结构被移动，所有内部借用可能变得无效。`Pin`通过防止移动来解决此问题：
...
- 如果`T: !Unpin`，则`Pin<&mut T>`确保`T`不会通过此借用被移动
- 这允许`T`包含自借用，因为相对位置保持固定
- 这是异步状态机安全运行的关键
...
Rust不在标准库中包含异步运行时，而是将选择权交给用户。主要异步运行时的特质对比：
...
```math
对于返回类型R = impl Trait:
1. 找出函数体中所有影响R的借用参数 {p_1: &'a T_1, ..., p_n: &'b T_n}
...
Rust 2024引入了`AsyncFn`特质，允许在trait 中直接定义异步方法：
...
Rust的类型系统通过特质和标记类型提供了多层安全保证：

```rust
// 安全标记特质
trait Send {}  // 可以安全地在线程间移动
trait Sync {}  // 可以安全地在线程间共享借用
