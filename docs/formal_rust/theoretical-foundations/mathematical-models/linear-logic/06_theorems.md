...
- [内存安全保证](../23_security_verification/01_formal_security_model.md#内存安全保证)
- [并发安全性](../05_concurrency/01_formal_concurrency_model.md#并发安全性)
- [借用类型系统](../02_type_system/01_formal_type_system.md#借用类型)
...
**定理 1.9** (生命周期有界性)
对于所有通过生命周期检查的程序，借用的生命周期不会超过被借用值的生命周期，因此不存在悬垂借用。

**证明**：

1. 生命周期检查确保所有借用都有有效的生命周期
2. 生命周期约束确保借用的生命周期 $\alpha$ 不会超出被借用对象的生命周期 $\beta$
3. 形式化表示为: $\alpha \subseteq \beta$
4. 这确保了借用在使用时，被借用的值一定存在
5. 因此，不存在悬垂借用
...
**定理 6.2** (悬垂借用避免)
对于所有通过Rust借用检查的程序，不存在悬垂借用。

**证明**：

1. 借用检查确保所有借用都有有效的生命周期
2. 生命周期约束确保借用不会超出被借用对象的生命周期
3. 因此，不存在悬垂借用
