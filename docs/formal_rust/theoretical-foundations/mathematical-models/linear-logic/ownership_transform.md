...
设计返回值时，可以根据使用场景选择返回借用或拥有所有权的数据：
...
1. **借用模式**：组件保持不变，数据通过借用传递，适合固定处理流程
...
`RefCell` 允许在拥有不可变借用时进行内部可变性操作，常用于需要动态借用规则检查的场景：
...
fn main() {
    ...
    // 检查借用计数
    println!("借用计数: {}", Rc::strong_count(&resource)); // 输出: 3
...
| 需要多个借用同时修改 | `Rc<RefCell<T>>` | 提供内部可变性和共享所有权 |
