...
- [path](#path)
- [权限的定义](#权限的定义)
  - [可读（Readable, R）](#可读readable-r)
  - [可写（Writable, W）](#可写writable-w)
  - [可拥有（Ownable, O）](#可拥有ownable-o)
- [权限的变化](#权限的变化)
  - [变量初始化](#变量初始化)
  - [不可变借用](#不可变借用)
  - [可变借用](#可变借用)
  ...
  - [权限的可视化](#权限的可视化)
- [借用的作用域定义](#借用的作用域定义)
  - [借用的生命周期（Lifetime）](#借用的生命周期lifetime)
  - [借用的作用域范围](#借用的作用域范围)
  - [借用的作用域与变量的作用域](#借用的作用域与变量的作用域)
  - [借用的作用域与函数调用](#借用的作用域与函数调用)
  - [借用的作用域与返回值](#借用的作用域与返回值)
  - [绕过借用检查器的合法方式](#绕过借用检查器的合法方式)
    - [使用 unsafe 块](#使用-unsafe-块)
    - [使用 std::cell 和 std::rc 模块](#使用-stdcell-和-stdrc-模块)
    - [使用生命周期注解](#使用生命周期注解)
  - [绕过借用检查器的不推荐方式](#绕过借用检查器的不推荐方式)
    - [使用 transmute](#使用-transmute)
    - [使用 std::ptr 模块](#使用-stdptr-模块)
...
### 可读（Readable, R）
    ...
    表示程序可以读取路径的值。对应于不可变借用（&T）。
    如果一个路径具有 R 权限，程序可以读取该路径的值，但不能修改它。

### 可写（Writable, W）

    表示程序可以修改路径的值。对应于可变借用（&mut T）。
    如果一个路径具有 W 权限，程序可以修改该路径的值，但不能同时有其他借用（包括不可变借用）指向该路径。
...
### 不可变借用
    ...
    创建不可变借用时，路径的 W 和 O 权限被移除，只剩下 R 权限。
    例如：let y = &x;，此时 x 的权限变为 R，y 的权限为 R。
    不可变借用可以有多个，但不能与可变借用共存。

### 可变借用

    创建可变借用时，路径的 R 和 O 权限被移除，只剩下 W 权限。
    例如：let z = &mut x;，此时 x 的权限变为 W，z 的权限为 W。
    可变借用必须是唯一的，不能与其他借用（包括不可变借用）共存。

### 借用结束

    当借用超出作用域时，其权限被恢复到原来的变量。
...
在 Rust 中，借用的作用域（scope）是指借用（reference）在代码中有效且可以被使用的范围。
作用域的定义和管理是 Rust 所有权系统的重要组成部分，它确保借用在使用时指向的值仍然是有效的，从而避免悬挂指针等未定义行为。
...
### 借用的生命周期（Lifetime）
    ...
    借用的作用域与其生命周期密切相关。
    生命周期是指借用在程序中有效的时间范围。
    Rust 的借用检查器通过静态分析确保借用在其生命周期内始终有效。
    生命周期参数（Lifetime Parameters）：在某些情况下，需要显式指定借用的生命周期。
    例如，函数的参数和返回值中包含借用时，需要使用生命周期参数来描述借用之间的关系。
...
### 借用的作用域范围
    ...
    借用的作用域通常从借用的创建点开始，到借用的最后一个使用点结束。
    具体来说：
    借用的创建点：借用的作用域从借用被创建的地方开始。
    例如，通过 & 操作符创建借用：
    let r = &x;
    在这里，r 的作用域从这行代码开始。
    借用的最后一个使用点：借用的作用域持续到借用最后一次被使用的地方。
    例如：
...
### 借用的作用域与返回值
    ...
    当函数返回一个借用时，返回值的借用作用域必须在调用者的上下文中有效。
    例如：
...
总结
借用的作用域是 Rust 所有权系统的重要组成部分，它确保借用在使用时指向的值仍然是有效的。
借用的作用域从借用的创建点开始，到借用的最后一个使用点结束，并且通常嵌套在变量的作用域内。
通过生命周期参数，Rust 的借用检查器能够静态地分析借用的作用域，确保程序的内存安全。
...
#### 使用生命周期注解
    ...
    注意：
    生命周期注解可以帮助 Rust 编译器更好地理解借用的生命周期，从而避免错误。
...
#### 使用 transmute
    ...
    fn main() {
        let x = 5;
        let r = unsafe { mem::transmute::<&i32, &mut i32>(&x) }; // 将不可变借用转换为可变借用
        *r += 1; // 修改值
