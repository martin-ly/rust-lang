...
- [类型理论对于多态类型的定义和解释](#类型理论对于多态类型的定义和解释)
  ...
  - [3. 类型理论对于多态类型的定义和解释](#3-类型理论对于多态类型的定义和解释)
    - [3.1 多态类型的实际案例](#31-多态类型的实际案例)
      - [3.1.1 C++中的模板](#311-c 中的模板)
      - [3.1.2 Java 中的接口](#312-java 中的接口)
      - [3.1.3 Python 中的鸭子类型](#313-python 中的鸭子类型)
    - [3.2 总结](#32-总结)
...
在这个示例中，`print_value`函数可以接受任何实现了`Display`特质的类型。
...
在这个示例中，`myDog`是`Animal`类型的借用，但它指向一个`Dog`对象，表现出子类型多态性。
