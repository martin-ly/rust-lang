...
- [幽灵类型与零大小类型 (Phantom Types and Zero-Sized Types)](#幽灵类型与零大小类型-phantom-types-and-zero-sized-types)
  - [📊 目录](#-目录)
  - [摘要](#摘要)
  - [理论基础](#理论基础)
    - [1. 幽灵类型的形式定义](#1-幽灵类型的形式定义)
    - [2. 零大小类型的形式定义](#2-零大小类型的形式定义)
  - [Rust 中的实现](#rust-中的实现)
    - [1. 标准库中的 PhantomData](#1-标准库中的-phantomdata)
    - [2. 常见的零大小类型](#2-常见的零大小类型)
  - [形式化语义](#形式化语义)
    - [1. 所有权语义与 PhantomData](#1-所有权语义与-phantomdata)
    - [2. 生命周期语义](#2-生命周期语义)
  - [应用场景](#应用场景)
    - [1. 类型状态模式](#1-类型状态模式)
    - [2. 单位与维度系统](#2-单位与维度系统)
    - [3. 类型标记与类型安全](#3-类型标记与类型安全)
    - [4. 变型和协变性控制](#4-变型和协变性控制)
  - [性能考量](#性能考量)
  - [形式化验证](#形式化验证)
  - [结论](#结论)
  - [参考文献](#参考文献)
...
幽灵类型(Phantom Types)和零大小类型(Zero-Sized Types, ZSTs)是 Rust 类型系统中的高级特质，它们虽然在运行时不占用内存空间，但在编译时提供重要的类型安全保证。本文探讨这些类型的理论基础、形式化定义以及在 Rust 中的实际应用。
...
`PhantomData<&'a T>` 表示结构体在逻辑上包含一个 $T$ 的借用，生命周期为 $\text{'a}$：
...
幽灵类型和零大小类型展示了 Rust 类型系统的表达能力，允许程序员在不增加运行时开销的情况下增强静态类型安全性。这些特质连接了类型理论与实用程序设计，使得高级类型安全模式能够实现在零成本抽象的约束下。
