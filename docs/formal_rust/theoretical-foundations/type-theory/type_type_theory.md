...
Rust 的类型约束通过特质边界来实现，这与类型论中的有界量化（bounded quantification）相对应。
...
特质边界限定了泛型参数必须满足的性质，从类型论角度看，这是一种受限的通用性。
...
生命周期可以看作是值（借用的有效范围）对类型的影响，这是依存类型的核心思想。
...
特质对象允许处理实现相同特质的不同类型，实现了受限的子类型多态。
