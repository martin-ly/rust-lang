...
- [02.06 类型转换与强制（Type Casting \& Coercion）](#0206-类型转换与强制type-casting--coercion)
  - [📊 目录](#-目录)
  - [理论简述](#理论简述)
  - [核心代码示例](#核心代码示例)
  - [详细注释](#详细注释)
...
类型转换（Type Casting）和类型强制（Type Coercion）是Rust类型系统的重要机制，用于在不同类型之间进行安全或显式的转换。Rust区分“显式转换”（如`as`关键字）和“隐式强制”（如借用类型自动转换）。类型转换的正确性和安全性由类型系统严格保证，相关理论详见[类型转换与型变](../../02_type_system/05_type_casting_and_coercion.md)与[型变规则](../../02_type_system/06_variance.md)。
...
- **常见错误**：
  ...
  - 对借用类型的错误强制（如`let s: &String = &str_var;`）
