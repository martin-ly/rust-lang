...
- [从仿射类型论视角看待 Rust 的类型系统设计与型变](#从仿射类型论视角看待-rust-的类型系统设计与型变)
  ...
  - [5. 仿射类型与 Copy 特质](#5-仿射类型与-copy-特质)
  - [6. Clone 特质作为显式资源复制](#6-clone-特质作为显式资源复制)
  - [7. Drop 特质与资源释放](#7-drop-特质与资源释放)
  - [8. 仿射类型与泛型](#8-仿射类型与泛型)
...
不变性（尤其是可变借用的不变性）是保护仿射资源不被误用的关键机制。

## 5. 仿射类型与 Copy 特质

标准的仿射类型不允许复制，
但 Rust 通过 Copy 特质为简单类型放宽了这一限制。
...
Rust 的生命周期系统确保借用不会超过其借用资源的生命周期，
这是仿射类型安全的补充。
...
5. **生命周期系统**：确保借用不会超过资源生命周期，补充了仿射类型安全。

Rust 的创新之处在于，它不仅采用了仿射类型的核心原则，
还通过借用系统、生命周期和类型特质等机制对其进行了扩展，创造了一个既安全又实用的类型系统。
