...
3. **借用类型（Reference Types）**:
   - **借用** (`&T`): 对某个值的借用，可以是可变或不可变的。
   - **裸指针** (`*const T`, `*mut T`): 对内存地址的直接借用，`*const` 是不可变裸指针，`*mut` 是可变裸指针。

4. **复合借用类型（Smart Pointers）**:
   - **箱式指针** (`Box`): 堆分配的所有权指针。
   - **借用计数指针** (`Rc`): 多所有权、自动清理的借用计数指针。
   - **原子借用计数指针** (`Arc`): `Rc` 的线程安全版本。
...
Rust 的类型系统是其语言的核心特质之一，它为编译时安全性、性能和代码清晰度提供了基础。
...
3. **所有权（Ownership）**: Rust 的所有权系统确保了内存安全，通过保证在任何给定时间，要么只有一个可变借用，要么有多个不可变借用，但不会同时存在。

4. **借用（Borrowing）**: 与所有权系统相关，借用允许在不转移所有权的情况下使用值。

5. **生命周期（Lifetimes）**: 生命周期是 Rust 中的一个概念，用于确保借用不会比它们指向的数据活得更长。
...
Rust 的类型系统是其安全性和性能的关键，它通过在编译时检查类型来预防许多常见的编程错误，如空指针解借用、数据竞争和内存泄漏等。
...
3. **借用类型** (`&T`)：对某个值的借用，允许在不转移所有权的情况下使用值。
...
8. **切片类型** (`&[T]`)：对数组或另一个切片的借用，允许访问连续的一系列元素。
...
13. **生命周期** (`'a`)：确保借用有效且不会悬挂，是 Rust 内存安全保证的关键部分。
...
- **借用和指针类型** 允许间接访问内存中的数据，借用提供了借用的概念，而指针则更接近于 C/C++ 中的原始指针。
...
15. **借用类型**：
    - 允许访问其他值的类型，分为不可变借用 `&T` 和可变借用 `&mut T`。
