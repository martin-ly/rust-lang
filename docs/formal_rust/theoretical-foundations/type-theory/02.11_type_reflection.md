...
- [02.11 类型反射（Type Reflection）](#0211-类型反射type-reflection)
  - [📊 目录](#-目录)
  - [理论简述](#理论简述)
  - [核心代码示例](#核心代码示例)
  - [详细注释](#详细注释)
...
类型反射（Type Reflection）指程序在运行时获取类型信息、动态检查类型或实现类型相关的元操作。
Rust通过`std::any::TypeId`、`Any` trait等有限支持类型反射，主要用于类型判断、动态分派和插件系统。
相关理论详见[类型反射](../../02_type_system/10_type_reflection.md)与[Any trait](../../19_advanced_language_features/06_type_reflection.md)。
