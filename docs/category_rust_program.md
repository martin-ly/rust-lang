...
这使得代码更加通用和灵活，可以在编译时根据传入的类型进行具体化，而无需运行时的类型检查。例如：
...
通过 Trait，可以在不同的类型之间实现共享行为，从而提高代码的可扩展性和模块化。例如：
...
不同的类型实现相同的 Trait，就能在同一个泛型函数中通用。例如：
...
在这个例子中，`Display` Trait 定义了一个 `display` 方法，`Person` 和 `Book` 结构体都实现了这个 Trait。
