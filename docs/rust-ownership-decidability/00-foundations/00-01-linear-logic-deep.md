...
- [6. 反例分析（12+ 个详细案例）](#6-反例分析12-个详细案例)
  ...
  - [6.9 Option 中的线性资源](#69-option-中的线性资源)
...
多焦点（Multifocusing）是聚焦技术的扩展，允许同时处理多个同步连接词。
...
6. **Danos, V. & Regnier, L. (1989)**. The Structure of Multiplicatives. *Archive for Mathematical Logic*, 28:181-203.
   - 证明网正确性标准

7. **Honda, K. (1993)**. Types for Dyadic Interaction. *CONCUR'93*.
   - 会话类型的开创工作

8. **Caires, L. & Pfenning, F. (2010)**. Session Types as Intuitionistic Linear Propositions. *CONCUR'10*.
   - 会话类型与线性逻辑的对应

9. **Reed, J. & Pierce, B.C. (2010)**. Distance Makes the Types Grow Stronger. *ICFP'10*.
   - 线性类型与资源管理

10. **McBride, C. (2016)**. I Got Plenty o' Nuttin'. *SPLASH'16*.
    - 线性类型在依赖类型中的应用

### 12.3 Rust 相关资源

11. **Matsakis, N.D. & Klock, F.S. (2014)**. The Rust Language. *HILT'14*.
    - Rust 的早期形式化描述

12. **Jung, R., et al. (2017)**. RustBelt: Securing the Foundations of the Rust Programming Language. *POPL'18*.
    - Rust 的分离逻辑形式化

13. **Weiss, A., et al. (2018)**. Oxide: The Essence of Rust. *CoRR abs/1903.00982*.
    - Rust 类型的代数效应解释

14. **Tov, J.A. & Pucella, R. (2011)**. Practical Affine Types. *POPL'11*.
    - 仿射类型系统的设计

15. **Bierhoff, K. & Aldrich, J. (2007)**. Modular Typestate Checking of Aliased Objects. *OOPSLA'07*.
    - 类型状态与资源管理
