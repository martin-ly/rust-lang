...
1. **概念精确化**：将所有权、借用、生命周期等直觉概念形式化为可操作的数学对象。
2. **定理体系化**：从公理出发，通过引理和推论，证明内存安全、数据竞争自由、类型安全等核心性质。
...
| **Tree Borrows** | ETH | 树结构借用模型 + Rocq | `borrow_checker_proof` 演进 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **RustBelt + Iris** | 分离逻辑 | 所有权、借用、核心库 | Coq 证明 | 高 | T-OW2 / T-BR1 / T-TY3 |
...
| **Miri** | MIR 解释器 | UB 动态检测 | 运行时报告 | 低 | 借用规则反例 |
