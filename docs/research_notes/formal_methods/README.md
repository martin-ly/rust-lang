...
- **并发与异步**：通道、Mutex/RwLock、原子操作（Atomic Operations）、thread::spawn（Phase 2、4、6）
...
- [🔬 形式化方法研究 {#形式化方法研究}](#-形式化方法研究-形式化方法研究)
  ...
  - [📚 研究主题 {#研究主题}](#-研究主题-研究主题)
    ...
    - [5. Pin 和自引用类型形式化 {#5-pin-和自引用类型形式化}](#5-pin-和自引用类型形式化-5-pin-和自引用类型形式化)
...
| **借用（Borrowing）** | 规则 5–8 | T1 数据竞争自由, T2 | 规则归纳 | [borrow_checker_proof](10_borrow_checker_proof.md) 双重可变借用 |
