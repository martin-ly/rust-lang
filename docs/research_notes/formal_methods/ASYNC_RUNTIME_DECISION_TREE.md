# Rust 异步运行时选型决策树

> 版本: 1.0.0
> 更新日期: 2026-02-21
> 适用场景: 系统架构设计、技术选型评估

---

## 📋 目录

- [Rust 异步运行时选型决策树](#rust-异步运行时选型决策树)
  - [📋 目录](#-目录)
  - [快速决策流程图](#快速决策流程图)
  - [运行时概览](#运行时概览)
  - [决策维度详解](#决策维度详解)
    - [1. 应用场景维度](#1-应用场景维度)
    - [2. 性能需求维度](#2-性能需求维度)
      - [高吞吐量场景 (\>100k RPS)](#高吞吐量场景-100k-rps)
      - [低延迟场景 (P99 \< 1ms)](#低延迟场景-p99--1ms)
      - [实时性场景 (硬实时要求)](#实时性场景-硬实时要求)
    - [3. 功能需求维度](#3-功能需求维度)
    - [4. 生态兼容性维度](#4-生态兼容性维度)
  - [详细对比分析](#详细对比分析)
    - [Tokio](#tokio)
      - [架构特点](#架构特点)
      - [性能特征](#性能特征)
      - [生态成熟度](#生态成熟度)
      - [适用场景](#适用场景)
    - [async-std](#async-std)
      - [架构特点](#架构特点-1)
      - [性能特征](#性能特征-1)
      - [生态成熟度](#生态成熟度-1)
      - [适用场景](#适用场景-1)
    - [smol](#smol)
      - [架构特点](#架构特点-2)
      - [性能特征](#性能特征-2)
      - [生态成熟度](#生态成熟度-2)
      - [适用场景](#适用场景-2)
    - [embassy](#embassy)
      - [架构特点](#架构特点-3)
      - [性能特征](#性能特征-3)
      - [生态成熟度](#生态成熟度-3)
      - [适用场景](#适用场景-3)
  - [代码示例对比](#代码示例对比)
    - [1. 基础TCP Echo服务器](#1-基础tcp-echo服务器)
      - [Tokio 版本](#tokio-版本)
      - [async-std 版本](#async-std-版本)
      - [smol 版本](#smol-版本)
      - [embassy 版本 (嵌入式)](#embassy-版本-嵌入式)
    - [2. HTTP 服务器对比](#2-http-服务器对比)
      - [Tokio + Axum](#tokio--axum)
      - [async-std + tide](#async-std--tide)
    - [3. 定时器与并发对比](#3-定时器与并发对比)
      - [Tokio](#tokio-1)
      - [smol](#smol-1)
      - [embassy (嵌入式定时器)](#embassy-嵌入式定时器)
  - [迁移指南](#迁移指南)
    - [从 async-std 迁移到 Tokio](#从-async-std-迁移到-tokio)
      - [Cargo.toml 变更](#cargotoml-变更)
      - [入口点变更](#入口点变更)
      - [网络操作变更](#网络操作变更)
      - [文件IO变更](#文件io变更)
      - [任务生成变更](#任务生成变更)
    - [从标准库迁移到异步](#从标准库迁移到异步)
    - [运行时混合使用](#运行时混合使用)
  - [决策检查清单](#决策检查清单)
    - [初步评估](#初步评估)
    - [性能需求评估](#性能需求评估)
    - [功能需求评估](#功能需求评估)
    - [生态与维护评估](#生态与维护评估)
    - [最终决策矩阵](#最终决策矩阵)
  - [参考资源](#参考资源)
    - [官方文档](#官方文档)
    - [性能基准测试](#性能基准测试)
    - [社区讨论](#社区讨论)
  - [版本历史](#版本历史)

---

## 快速决策流程图

```
                          ┌─────────────────────┐
                          │   开始选型决策       │
                          └─────────┬───────────┘
                                    │
                                    ▼
                    ┌───────────────────────────────┐
                    │   目标平台是什么？             │
                    └───────────┬───────────────────┘
                                │
            ┌───────────────────┼───────────────────┐
            │                   │                   │
            ▼                   ▼                   ▼
    ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
    │   嵌入式/     │   │   服务器/     │   │   桌面/通用   │
    │   微控制器    │   │   云端服务    │   │   应用        │
    └───────┬───────┘   └───────┬───────┘   └───────┬───────┘
            │                   │                   │
            ▼                   ▼                   ▼
    ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
    │   embassy     │   │   需要高吞吐  │   │   需要高性能  │
    │   /nrf-rs     │   │   /高并发？   │   │   IO多路复用？│
    │               │   │               │   │               │
    │   ✅ 推荐     │   └───────┬───────┘   └───────┬───────┘
    └───────────────┘           │                   │
                        ┌───────┴───────┐   ┌───────┴───────┐
                        │               │   │               │
                        ▼               ▼   ▼               ▼
                ┌───────────┐   ┌───────────┐       ┌───────────┐
                │    是     │   │    否     │       │    是     │
                └─────┬─────┘   └─────┬─────┘       └─────┬─────┘
                      │               │                   │
                      ▼               ▼                   ▼
              ┌───────────────┐ ┌───────────────┐ ┌───────────────┐
              │    Tokio      │ │  需要标准库   │ │    Tokio      │
              │               │ │  兼容？       │ │   /smol       │
              │  ✅ 推荐      │ │               │ │               │
              └───────────────┘ └───────┬───────┘ │  ✅ 推荐      │
                                        │         └───────────────┘
                              ┌─────────┴─────────┐
                              │                   │
                              ▼                   ▼
                      ┌───────────┐       ┌───────────┐
                      │    是     │       │    否     │
                      └─────┬─────┘       └─────┬─────┘
                            │                   │
                            ▼                   ▼
                    ┌───────────────┐   ┌───────────────┐
                    │   async-std   │   │     smol      │
                    │               │   │               │
                    │  ✅ 推荐      │   │  ✅ 推荐      │
                    └───────────────┘   └───────────────┘
```

---

## 运行时概览

| 运行时 | 定位 | 维护状态 | 生态成熟度 | 学习曲线 |
|--------|------|----------|------------|----------|
| **Tokio** | 企业级生产环境首选 | ⭐⭐⭐⭐⭐ 非常活跃 | ⭐⭐⭐⭐⭐ 最成熟 | 中等 |
| **async-std** | 类标准库API | ⭐⭐⭐ 维护放缓 | ⭐⭐⭐⭐ 较成熟 | 低 |
| **smol** | 轻量可组合 | ⭐⭐⭐⭐ 活跃 | ⭐⭐⭐ 中等 | 中等 |
| **embassy** | 嵌入式专用 | ⭐⭐⭐⭐⭐ 非常活跃 | ⭐⭐⭐ 嵌入式领域成熟 | 中高 |
| **futures** | 基础抽象层 | ⭐⭐⭐⭐ 稳定维护 | ⭐⭐⭐⭐⭐ 最基础 | 高 |

---

## 决策维度详解

### 1. 应用场景维度

```
┌─────────────────────────────────────────────────────────────────────┐
│                        应用场景决策矩阵                              │
├─────────────────┬─────────────────┬─────────────────────────────────┤
│    场景类型      │    推荐运行时    │           理由                   │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ Web服务/API服务 │     Tokio       │ 完整的HTTP生态, Actix/Axum支持   │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ 微服务/云原生   │     Tokio       │ tracing/metrics集成, gRPC生态    │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ 数据库/存储引擎 │     Tokio       │ io_uring支持, 高性能网络IO       │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ 嵌入式MCU       │    embassy      │ no_std支持, 零开销抽象           │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ 实时控制系统    │ embassy/tokio   │ 可配置调度器, 优先级支持         │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ 桌面GUI应用     │ smol/async-std  │ 与GUI框架集成容易                │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ CLI工具         │ tokio/smol      │ 启动快, 资源占用低               │
├─────────────────┼─────────────────┼─────────────────────────────────┤
│ 区块链/共识系统 │ tokio/smol      │ 确定性执行, 自定义调度           │
└─────────────────┴─────────────────┴─────────────────────────────────┘
```

### 2. 性能需求维度

#### 高吞吐量场景 (>100k RPS)

- **首选**: Tokio (io_uring + work-stealing)
- **备选**: smol (简化模型, 低上下文切换)

#### 低延迟场景 (P99 < 1ms)

- **首选**: Tokio (优化过的调度器)
- **备选**: embassy (裸机级别控制)

#### 实时性场景 (硬实时要求)

- **首选**: embassy-executor + 自定义调度
- **备选**: tokio with rt-multi-thread + 线程亲和性

### 3. 功能需求维度

| 功能需求 | Tokio | async-std | smol | embassy |
|----------|-------|-----------|------|---------|
| TCP/UDP  | ✅ 完整 | ✅ 完整 | ✅ 完整 | ✅ 嵌入式优化 |
| HTTP/WebSocket | ✅ 丰富生态 | ✅ 可用 | ⚠️ 需第三方 | ❌ 不适用 |
| 定时器 | ✅ 高性能 | ✅ 标准 | ✅ 简单 | ✅ 硬件定时器 |
| 文件IO | ✅ io_uring | ✅ 标准 | ✅ 标准 | ❌ 不适用 |
| 同步原语 | ✅ 丰富 | ✅ 标准 | ⚠️ 基础 | ✅ 嵌入式专用 |
| 通道(channel) | ✅ 多类型 | ✅ mpsc | ✅ 基础 | ✅ 无锁实现 |
| TLS/加密 | ✅ rustls集成 | ✅ 可用 | ⚠️ 需配置 | ⚠️ 有限支持 |
| 信号处理 | ✅ 完整 | ✅ Unix | ❌ 无 | ❌ 中断处理 |

### 4. 生态兼容性维度

```
                    Tokio    async-std    smol    embassy
                    ─────────────────────────────────────────
Serde                ✅         ✅         ✅        ✅
Hyper/HTTP          ✅         ✅         ⚠️        ❌
Tonic/gRPC          ✅         ❌         ❌        ❌
SQLx               ✅         ✅         ⚠️        ❌
Redis              ✅         ✅         ⚠️        ❌
AWS SDK            ✅         ❌         ❌        ❌
Tracing            ✅         ⚠️         ⚠️        ⚠️
Metrics            ✅         ⚠️         ⚠️        ❌
                    ─────────────────────────────────────────
生态成熟度          最高        中等        较低      嵌入式专用
```

---

## 详细对比分析

### Tokio

#### 架构特点

```
┌────────────────────────────────────────────────────────────────┐
│                        Tokio 架构                               │
├────────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                    应用层                                 │  │
│  │  (Axum/Actix-web/Tonic/tokio-postgres/...)              │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   Runtime 层                              │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌──────────────────┐ │  │
│  │  │  Multi-Thread│  │ Current-    │  │ Work-Stealing    │ │  │
│  │  │  Scheduler   │  │ Thread      │  │ Queue            │ │  │
│  │  └─────────────┘  └─────────────┘  └──────────────────┘ │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                    I/O 驱动层                             │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌──────────────────┐ │  │
│  │  │  epoll/kqueue│  │  io_uring   │  │  Event Loop      │ │  │
│  │  └─────────────┘  └─────────────┘  └──────────────────┘ │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                    系统调用层                             │  │
│  │         (mio / io_uring / WASI / ...)                    │  │
│  └──────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────┘
```

#### 性能特征

| 指标 | 数值/等级 | 说明 |
|------|-----------|------|
| 吞吐量 | ⭐⭐⭐⭐⭐ | 100k+ RPS (HTTP, 单节点) |
| 延迟P99 | ⭐⭐⭐⭐ | < 1ms (优化配置下) |
| 内存占用 | ⭐⭐⭐ | 中等 (per-task overhead) |
| 启动时间 | ⭐⭐⭐ | 中等 (runtime初始化) |
| 扩展性 | ⭐⭐⭐⭐⭐ | 优秀的多核扩展 |

#### 生态成熟度

- **HTTP服务器**: Axum, Actix-web, warp - 都非常成熟
- **数据库**: SQLx, deadpool, bb8 - 生产就绪
- **消息队列**: lapin(RabbitMQ), rdkafka(Kafka)
- **gRPC**: Tonic - 官方推荐, 非常完善
- **可观测性**: tracing, metrics, opentelemetry - 完整支持

#### 适用场景

✅ 大型Web服务/API网关
✅ 高并发网络服务
✅ 微服务架构
✅ 实时数据处理
⚠️ 资源受限环境 (编译体积较大)
❌ 硬实时系统 (非确定性延迟)

---

### async-std

#### 架构特点

```
┌────────────────────────────────────────────────────────────────┐
│                     async-std 架构                              │
├────────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   标准库兼容API                           │  │
│  │  (async fs, async net, async process, ...)              │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   Runtime 层                              │  │
│  │  ┌─────────────────────────────────────────────────────┐ │  │
│  │  │         async-global-executor                       │ │  │
│  │  │   (可配置: tokio/async-std/smol backend)           │ │  │
│  │  └─────────────────────────────────────────────────────┘ │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   底层实现层                              │  │
│  │         (async-io + blocking + ...)                      │  │
│  └──────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────┘
```

#### 性能特征

| 指标 | 数值/等级 | 说明 |
|------|-----------|------|
| 吞吐量 | ⭐⭐⭐⭐ | 接近Tokio, 略低 |
| 延迟P99 | ⭐⭐⭐⭐ | < 2ms |
| 内存占用 | ⭐⭐⭐ | 与Tokio相当 |
| 启动时间 | ⭐⭐⭐ | 中等 |
| 扩展性 | ⭐⭐⭐⭐ | 良好 |

#### 生态成熟度

- 生态发展放缓, 部分核心开发者转向其他项目
- 仍有不少项目使用, 但新项目推荐Tokio
- API设计优秀, 对标准库API的映射清晰

#### 适用场景

✅ 需要标准库风格API的项目
✅ 中小型Web服务
✅ 从同步代码迁移的项目
⚠️ 长期维护的大型项目 (活跃度下降)
❌ 需要最新生态支持的新项目

---

### smol

#### 架构特点

```
┌────────────────────────────────────────────────────────────────┐
│                       smol 架构                                 │
├────────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   应用代码                                │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                smol 组合式运行时                          │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │  │
│  │  │ async-io │  │ async-   │  │ async-   │  │ polling  │  │  │
│  │  │          │  │ executor │  │ channel  │  │          │  │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘  │  │
│  │                                                          │  │
│  │  特点: 每个组件可独立使用, 按需组合                       │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   底层平台抽象                            │  │
│  │         (epoll/kqueue/IOCP/WASI/...)                     │  │
│  └──────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────┘
```

#### 性能特征

| 指标 | 数值/等级 | 说明 |
|------|-----------|------|
| 吞吐量 | ⭐⭐⭐⭐ | 高, 设计简洁 |
| 延迟P99 | ⭐⭐⭐⭐ | 低, 调度简单 |
| 内存占用 | ⭐⭐⭐⭐⭐ | 极低 (轻量级设计) |
| 启动时间 | ⭐⭐⭐⭐⭐ | 极快 |
| 扩展性 | ⭐⭐⭐⭐ | 良好 |

#### 生态成熟度

- 组件化设计, 可被其他运行时复用
- 生态相对较小但质量高
- 适合需要精细控制的场景

#### 适用场景

✅ 资源受限环境
✅ 嵌入式Linux系统
✅ 需要定制运行时的场景
✅ 启动速度要求高的CLI工具
✅ 混合多种异步模型的项目
⚠️ 需要丰富开箱即用功能的场景

---

### embassy

#### 架构特点

```
┌────────────────────────────────────────────────────────────────┐
│                     embassy 架构                                │
├────────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   应用层 (embassy-rs)                     │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │  │
│  │  │ embassy- │  │ embassy- │  │ embassy- │  │embassy-  │  │  │
│  │  │ net      │  │ usb      │  │ ble      │  │ lorawan  │  │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘  │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   执行器层                                │  │
│  │  ┌─────────────────────────────────────────────────────┐ │  │
│  │  │              embassy-executor                       │ │  │
│  │  │  ┌─────────────┐  ┌─────────────┐  ┌──────────────┐ │ │  │
│  │  │  │  集成型     │  │  线程模式   │  │  多核模式    │ │ │  │
│  │  │  │  (bare-metal)│  │  (std)      │  │  (SMP)       │ │ │  │
│  │  │  └─────────────┘  └─────────────┘  └──────────────┘ │ │  │
│  │  └─────────────────────────────────────────────────────┘ │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                   硬件抽象层 (HAL)                        │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │  │
│  │  │embassy-  │  │embassy-  │  │embassy-  │  │  ...     │  │  │
│  │  │stm32     │  │nrf       │  │rp        │  │          │  │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘  │  │
│  └──────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────┘
```

#### 性能特征

| 指标 | 数值/等级 | 说明 |
|------|-----------|------|
| 吞吐量 | ⭐⭐⭐ | MCU受限, 但优化极佳 |
| 延迟P99 | ⭐⭐⭐⭐⭐ | 可预测, 低抖动 |
| 内存占用 | ⭐⭐⭐⭐⭐ | 极低 (KB级别) |
| 实时性 | ⭐⭐⭐⭐⭐ | 支持硬实时调度 |
| 功耗 | ⭐⭐⭐⭐⭐ | 优秀的异步休眠支持 |

#### 生态成熟度

- 嵌入式领域最活跃的async运行时
- 支持主流MCU平台: STM32, nRF, RP2040, ESP32
- 完善的无线协议栈: BLE, WiFi, LoRaWAN, Thread

#### 适用场景

✅ 嵌入式/物联网设备
✅ 裸机(bare-metal)系统
✅ 电池供电设备 (低功耗)
✅ 实时控制系统
✅ 传感器/执行器网络
❌ 通用服务器应用
❌ 需要标准库的场景

---

## 代码示例对比

### 1. 基础TCP Echo服务器

#### Tokio 版本

```rust
use tokio::net::{TcpListener, TcpStream};
use tokio::io::{AsyncReadExt, AsyncWriteExt};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let listener = TcpListener::bind("127.0.0.1:8080").await?;
    println!("Tokio server listening on 127.0.0.1:8080");

    loop {
        let (mut socket, addr) = listener.accept().await?;
        println!("New connection from: {}", addr);

        tokio::spawn(async move {
            let mut buf = vec![0; 1024];

            loop {
                match socket.read(&mut buf).await {
                    Ok(0) => {
                        println!("Client {} disconnected", addr);
                        return;
                    }
                    Ok(n) => {
                        if socket.write_all(&buf[0..n]).await.is_err() {
                            return;
                        }
                    }
                    Err(e) => {
                        eprintln!("Failed to read from socket; err = {:?}", e);
                        return;
                    }
                }
            }
        });
    }
}
```

#### async-std 版本

```rust
use async_std::net::{TcpListener, TcpStream};
use async_std::prelude::*;
use async_std::task;

#[async_std::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let listener = TcpListener::bind("127.0.0.1:8080").await?;
    println!("async-std server listening on 127.0.0.1:8080");

    loop {
        let (mut socket, addr) = listener.accept().await?;
        println!("New connection from: {}", addr);

        task::spawn(async move {
            let mut buf = vec![0; 1024];

            loop {
                match socket.read(&mut buf).await {
                    Ok(0) => {
                        println!("Client {} disconnected", addr);
                        return;
                    }
                    Ok(n) => {
                        if socket.write_all(&buf[0..n]).await.is_err() {
                            return;
                        }
                    }
                    Err(e) => {
                        eprintln!("Failed to read from socket; err = {:?}", e);
                        return;
                    }
                }
            }
        });
    }
}
```

#### smol 版本

```rust
use smol::io::{AsyncReadExt, AsyncWriteExt};
use smol::net::TcpListener;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    smol::block_on(async {
        let listener = TcpListener::bind("127.0.0.1:8080").await?;
        println!("smol server listening on 127.0.0.1:8080");

        loop {
            let (mut socket, addr) = listener.accept().await?;
            println!("New connection from: {}", addr);

            smol::spawn(async move {
                let mut buf = vec![0; 1024];

                loop {
                    match socket.read(&mut buf).await {
                        Ok(0) => {
                            println!("Client {} disconnected", addr);
                            return;
                        }
                        Ok(n) => {
                            if socket.write_all(&buf[0..n]).await.is_err() {
                                return;
                            }
                        }
                        Err(e) => {
                            eprintln!("Failed to read from socket; err = {:?}", e);
                            return;
                        }
                    }
                }
            }).detach();
        }
    })
}
```

#### embassy 版本 (嵌入式)

```rust
#![no_std]
#![no_main]

use embassy_executor::Spawner;
use embassy_net::{tcp::TcpSocket, Config, Stack, StackResources};
use embassy_stm32::eth::{Ethernet, PacketQueue};
use embassy_stm32::peripherals::ETH;
use embassy_stm32::rng::Rng;
use embassy_stm32::time::Hertz;
use embassy_stm32::{bind_interrupts, eth, peripherals, rng};
use embassy_time::{Duration, Timer};
use static_cell::make_static;

bind_interrupts!(struct Irqs {
    ETH => eth::InterruptHandler;
    RNG => rng::InterruptHandler<peripherals::RNG>;
});

#[embassy_executor::main]
async fn main(spawner: Spawner) {
    let p = embassy_stm32::init(Default::default());

    // 初始化以太网
    let mut rng = Rng::new(p.RNG, Irqs);
    let mac_addr = [0x00, 0x00, 0xDE, 0xAD, 0xBE, 0xEF];

    static PACKETS: PacketQueue<4, 4> = PacketQueue::new();
    let eth = Ethernet::new(
        PACKETS.borrow().into(),
        p.ETH,
        Irqs,
        p.PA1,
        p.PA2,
        p.PC1,
        p.PA7,
        p.PC4,
        p.PC5,
        p.PG13,
        p.PB13,
        p.PG11,
        mac_addr,
    );

    let config = Config::dhcpv4(Default::default());
    let seed = rng.random_u64();

    // 启动网络栈
    static RESOURCES: StackResources<3> = StackResources::new();
    let (stack, runner) = embassy_net::init(eth, config, RESOURCES, seed);
    spawner.spawn(net_task(runner)).unwrap();

    // 等待网络连接
    loop {
        if stack.is_config_up() {
            break;
        }
        Timer::after(Duration::from_millis(100)).await;
    }

    // 启动TCP echo服务
    spawner.spawn(tcp_echo_task(stack)).unwrap();
}

#[embassy_executor::task]
async fn net_task(mut runner: embassy_net::Runner<'static, Ethernet<'static, ETH>>) -> ! {
    runner.run().await
}

#[embassy_executor::task]
async fn tcp_echo_task(stack: &'static Stack<Ethernet<'static, ETH>>) {
    let mut rx_buffer = [0; 1024];
    let mut tx_buffer = [0; 1024];

    loop {
        let mut socket = TcpSocket::new(stack, &mut rx_buffer, &mut tx_buffer);
        socket.set_timeout(Some(Duration::from_secs(10)));

        if let Err(e) = socket.accept(8080).await {
            log::error!("Accept error: {:?}", e);
            continue;
        }

        log::info!("New connection from {:?}", socket.remote_endpoint());

        loop {
            match socket.read(&mut rx_buffer).await {
                Ok(0) => break,
                Ok(n) => {
                    if let Err(e) = socket.write_all(&rx_buffer[..n]).await {
                        log::error!("Write error: {:?}", e);
                        break;
                    }
                }
                Err(e) => {
                    log::error!("Read error: {:?}", e);
                    break;
                }
            }
        }
    }
}
```

### 2. HTTP 服务器对比

#### Tokio + Axum

```rust
use axum::{
    routing::get,
    Router,
    extract::Path,
    http::StatusCode,
    response::Json,
};
use serde::{Deserialize, Serialize};
use std::net::SocketAddr;

#[derive(Serialize)]
struct User {
    id: u64,
    name: String,
}

async fn get_user(Path(id): Path<u64>) -> Result<Json<User>, StatusCode> {
    if id == 0 {
        return Err(StatusCode::NOT_FOUND);
    }
    Ok(Json(User {
        id,
        name: format!("User {}", id),
    }))
}

async fn health() -> &'static str {
    "OK"
}

#[tokio::main]
async fn main() {
    let app = Router::new()
        .route("/health", get(health))
        .route("/users/:id", get(get_user));

    let addr = SocketAddr::from(([127, 0, 0, 1], 3000));
    println!("Listening on {}", addr);

    axum::Server::bind(&addr)
        .serve(app.into_make_service())
        .await
        .unwrap();
}
```

#### async-std + tide

```rust
use tide::{Request, Response, StatusCode};
use serde::{Deserialize, Serialize};

#[derive(Serialize, Deserialize)]
struct User {
    id: u64,
    name: String,
}

#[async_std::main]
async fn main() -> tide::Result<()> {
    let mut app = tide::new();

    app.at("/health").get(|_| async { Ok("OK") });

    app.at("/users/:id").get(|req: Request<()>| async move {
        let id: u64 = req.param("id")?.parse()?;
        if id == 0 {
            return Ok(Response::new(StatusCode::NotFound));
        }
        let user = User {
            id,
            name: format!("User {}", id),
        };
        tide::Body::from_json(&user)
    });

    app.listen("127.0.0.1:3000").await?;
    Ok(())
}
```

### 3. 定时器与并发对比

#### Tokio

```rust
use tokio::time::{sleep, Duration, interval};
use tokio::sync::mpsc;

#[tokio::main]
async fn main() {
    // 单一定时器
    sleep(Duration::from_secs(1)).await;

    // 间隔定时器
    let mut ticker = interval(Duration::from_secs(1));
    for _ in 0..3 {
        ticker.tick().await;
        println!("Tick!");
    }

    // 并发执行任务
    let (tx, mut rx) = mpsc::channel(100);

    let producer = async move {
        for i in 0..5 {
            tx.send(i).await.unwrap();
            sleep(Duration::from_millis(100)).await;
        }
    };

    let consumer = async move {
        while let Some(v) = rx.recv().await {
            println!("Received: {}", v);
        }
    };

    tokio::join!(producer, consumer);
}
```

#### smol

```rust
use smol::Timer;
use smol::channel::unbounded;
use std::time::Duration;

fn main() {
    smol::block_on(async {
        // 单一定时器
        Timer::after(Duration::from_secs(1)).await;

        // 间隔定时器 (手动实现)
        for i in 0..3 {
            Timer::after(Duration::from_secs(1)).await;
            println!("Tick {}!", i);
        }

        // 并发执行任务
        let (tx, rx) = unbounded::<i32>();

        let producer = async move {
            for i in 0..5 {
                tx.send(i).await.unwrap();
                Timer::after(Duration::from_millis(100)).await;
            }
        };

        let consumer = async move {
            while let Ok(v) = rx.recv().await {
                println!("Received: {}", v);
            }
        };

        smol::future::race(producer, consumer).await.unwrap();
    });
}
```

#### embassy (嵌入式定时器)

```rust
use embassy_time::{Duration, Timer, Ticker};
use embassy_sync::channel::Channel;
use embassy_sync::blocking_mutex::raw::ThreadModeRawMutex;

static CHANNEL: Channel<ThreadModeRawMutex, i32, 100> = Channel::new();

#[embassy_executor::main]
async fn main(spawner: Spawner) {
    // 单一定时器
    Timer::after(Duration::from_secs(1)).await;

    // 间隔定时器
    let mut ticker = Ticker::every(Duration::from_secs(1));
    for _ in 0..3 {
        ticker.next().await;
        info!("Tick!");
    }

    // 启动生产者和消费者任务
    spawner.spawn(producer()).unwrap();
    spawner.spawn(consumer()).unwrap();
}

#[embassy_executor::task]
async fn producer() {
    for i in 0..5 {
        CHANNEL.send(i).await;
        Timer::after(Duration::from_millis(100)).await;
    }
}

#[embassy_executor::task]
async fn consumer() {
    loop {
        let v = CHANNEL.receive().await;
        info!("Received: {}", v);
    }
}
```

---

## 迁移指南

### 从 async-std 迁移到 Tokio

#### Cargo.toml 变更

```toml
# 旧依赖 (async-std)
[dependencies]
async-std = { version = "1.12", features = ["attributes"] }
tide = "0.16"

# 新依赖 (Tokio)
[dependencies]
tokio = { version = "1.35", features = ["full"] }
axum = "0.7"
tower = "0.4"
```

#### 入口点变更

```rust
// 旧代码
#[async_std::main]
async fn main() {
    // ...
}

// 新代码
#[tokio::main]
async fn main() {
    // ...
}
```

#### 网络操作变更

```rust
// 旧代码 (async-std)
use async_std::net::TcpListener;
use async_std::io::prelude::*;

let listener = TcpListener::bind("127.0.0.1:8080").await?;
let mut stream = async_std::net::TcpStream::connect("127.0.0.1:8080").await?;
stream.write_all(b"hello").await?;

// 新代码 (Tokio)
use tokio::net::TcpListener;
use tokio::io::AsyncWriteExt;

let listener = TcpListener::bind("127.0.0.1:8080").await?;
let mut stream = tokio::net::TcpStream::connect("127.0.0.1:8080").await?;
stream.write_all(b"hello").await?;
```

#### 文件IO变更

```rust
// 旧代码
use async_std::fs::File;
use async_std::io::prelude::*;

let mut file = File::open("file.txt").await?;
let mut contents = String::new();
file.read_to_string(&mut contents).await?;

// 新代码
use tokio::fs::File;
use tokio::io::AsyncReadExt;

let mut file = File::open("file.txt").await?;
let mut contents = String::new();
file.read_to_string(&mut contents).await?;
```

#### 任务生成变更

```rust
// 旧代码
let handle = async_std::task::spawn(async {
    // 任务内容
});
handle.await;

// 新代码
let handle = tokio::spawn(async {
    // 任务内容
});
handle.await?;
```

### 从标准库迁移到异步

```rust
// 同步代码
use std::net::TcpListener;
use std::io::{Read, Write};
use std::thread;

fn main() -> std::io::Result<()> {
    let listener = TcpListener::bind("127.0.0.1:8080")?;

    for stream in listener.incoming() {
        let stream = stream?;
        thread::spawn(move || {
            handle_connection(stream);
        });
    }
    Ok(())
}

fn handle_connection(mut stream: std::net::TcpStream) {
    let mut buffer = [0; 1024];
    loop {
        match stream.read(&mut buffer) {
            Ok(0) => break,
            Ok(n) => {
                stream.write_all(&buffer[0..n]).unwrap();
            }
            Err(_) => break,
        }
    }
}

// 异步代码 (Tokio)
use tokio::net::TcpListener;
use tokio::io::{AsyncReadExt, AsyncWriteExt};

#[tokio::main]
async fn main() -> tokio::io::Result<()> {
    let listener = TcpListener::bind("127.0.0.1:8080").await?;

    loop {
        let (mut socket, _) = listener.accept().await?;
        tokio::spawn(async move {
            handle_connection(&mut socket).await;
        });
    }
}

async fn handle_connection(socket: &mut tokio::net::TcpStream) {
    let mut buffer = vec![0; 1024];
    loop {
        match socket.read(&mut buffer).await {
            Ok(0) => break,
            Ok(n) => {
                socket.write_all(&buffer[0..n]).await.unwrap();
            }
            Err(_) => break,
        }
    }
}
```

### 运行时混合使用

在某些场景下, 可能需要混合使用多个运行时的组件:

```rust
// 使用 smol 的 async-io 驱动 Tokio
use tokio::runtime::Runtime;

fn main() {
    let rt = Runtime::new().unwrap();

    rt.block_on(async {
        // 使用 smol 的 IO (如果需要在 tokio 中使用 smol 兼容的库)
        smol::future::block_on(async {
            // smol 异步代码
        });
    });
}
```

---

## 决策检查清单

### 初步评估

- [ ] 目标平台是否支持标准库 (`std`)?
  - 否 → 考虑 **embassy** (no_std)
  - 是 → 继续评估

- [ ] 项目类型?
  - 嵌入式/IoT → **embassy**
  - Web服务/API → 继续评估
  - 桌面应用 → 继续评估
  - CLI工具 → 继续评估

### 性能需求评估

- [ ] 是否需要 >100k RPS 吞吐量?
  - 是 → **Tokio** (io_uring优化)
  - 否 → 继续评估

- [ ] 是否要求 P99 延迟 < 1ms?
  - 是 → **Tokio** 或 **smol**
  - 否 → 继续评估

- [ ] 是否有硬实时要求?
  - 是 → **embassy** (嵌入式) / **Tokio** + 线程亲和性
  - 否 → 继续评估

### 功能需求评估

- [ ] 是否需要 HTTP/WebSocket?
  - 是 → **Tokio** (Axum/Actix-web) 或 **async-std** (tide)
  - 否 → 继续评估

- [ ] 是否需要 gRPC?
  - 是 → **Tokio** (Tonic)
  - 否 → 继续评估

- [ ] 是否需要特定数据库驱动?
  - 检查驱动支持的运行时
  - Tokio 生态最完整

### 生态与维护评估

- [ ] 是否长期维护的企业项目?
  - 是 → **Tokio** (最活跃, 最成熟)
  - 否 → 可选 **async-std** 或 **smol**

- [ ] 团队是否有特定运行时经验?
  - 考虑学习成本

- [ ] 是否需要特定库的兼容?
  - 检查目标库的文档中的运行时要求

### 最终决策矩阵

| 条件组合 | 推荐运行时 | 理由 |
|----------|------------|------|
| 嵌入式/无std | embassy | 唯一选择 |
| 企业级Web服务 | Tokio | 生态最完善 |
| 高并发+低延迟 | Tokio | 性能最优 |
| 资源受限Linux | smol | 轻量高效 |
| 标准库API偏好 | async-std | API风格一致 |
| 混合/实验项目 | smol | 灵活可组合 |

---

## 参考资源

### 官方文档

- [Tokio Documentation](https://tokio.rs/)
- [async-std Book](https://book.async.rs/)
- [smol Documentation](https://docs.rs/smol/)
- [embassy Documentation](https://embassy.dev/)

### 性能基准测试

- [Web Framework Benchmarks](https://www.techempower.com/benchmarks/)
- [Rust Async Benchmarks](https://github.com/jasonwilliams/benchmark)

### 社区讨论

- [Rust Async Working Group](https://github.com/rust-lang/wg-async)
- [Reddit r/rust](https://www.reddit.com/r/rust/)

---

## 版本历史

| 版本 | 日期 | 更新内容 |
|------|------|----------|
| 1.0.0 | 2026-02-21 | 初始版本 |

---

> **注意**: 本决策树基于2026年初的技术状态。Rust异步生态发展迅速, 建议在做最终决策前查阅最新的官方文档和社区讨论。
