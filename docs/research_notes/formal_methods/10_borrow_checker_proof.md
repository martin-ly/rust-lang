...
- 引用生命周期超出作用域
- 借用检查器使用 [lifetime_formalization](./10_lifetime_formalization.md) 的生命周期推断
...
- NLL 求解约束系统找到最小满足解
- 根据 [lifetime_formalization](./10_lifetime_formalization.md) 定理 LF-T1，约束求解正确
...
Polonius 与 [lifetime_formalization](./10_lifetime_formalization.md) 中的推断、约束求解一致。
