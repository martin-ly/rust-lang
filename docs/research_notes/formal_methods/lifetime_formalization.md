...
**定理 2 (引用有效性)**: 在生命周期系统下，所有引用在其生命周期内有效。
...
**结论**: 由以上三个步骤，所有引用在其生命周期内有效。$\square$

**定理 3 (生命周期推断算法正确性)**: 生命周期推断算法生成的约束系统是一致的，当且仅当程序是良型的。
