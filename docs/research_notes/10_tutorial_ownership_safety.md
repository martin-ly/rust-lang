...
- [教程：所有权（Ownership）与内存安全（Memory Safety） {#教程所有权与内存安全}](#教程所有权与内存安全-教程所有权与内存安全)
  - [📑 目录 {#目录}](#-目录-目录)
  - [引言 {#引言-1}](#引言-引言-1)
  - [第一部分：为什么需要所有权 {#第一部分为什么需要所有权}](#第一部分为什么需要所有权-第一部分为什么需要所有权)
    - [内存管理的困境 {#内存管理的困境}](#内存管理的困境-内存管理的困境)
    - [Rust的解决方案 {#rust的解决方案}](#rust的解决方案-rust的解决方案)
  - [第二部分：所有权规则 {#第二部分所有权规则}](#第二部分所有权规则-第二部分所有权规则)
    - [三大规则 {#三大规则}](#三大规则-三大规则)
    - [作用域与生命周期（Lifetimes） {#作用域与生命周期}](#作用域与生命周期-作用域与生命周期)
    - [内存表示 {#内存表示}](#内存表示-内存表示)
  - [第三部分：移动语义 {#第三部分移动语义}](#第三部分移动语义-第三部分移动语义)
    - [什么是移动 {#什么是移动}](#什么是移动-什么是移动)
    - [浅复制的问题(Copy vs Move) {#浅复制的问题copy-vs-move}](#浅复制的问题copy-vs-move-浅复制的问题copy-vs-move)
  - [第四部分：Copy trait {#第四部分copy-trait}](#第四部分copy-trait-第四部分copy-trait)
    - [哪些类型实现Copy {#哪些类型实现copy}](#哪些类型实现copy-哪些类型实现copy)
    - [自定义类型的Copy {#自定义类型的copy}](#自定义类型的copy-自定义类型的copy)
  - [第五部分：借用（Borrowing） {#第五部分借用}](#第五部分借用-第五部分借用)
    - [为什么需要借用 {#为什么需要借用}](#为什么需要借用-为什么需要借用)
    - [借用规则 {#借用规则}](#借用规则-借用规则)
    - [借用与作用域 {#借用与作用域}](#借用与作用域-借用与作用域)
  - [第六部分：悬垂引用（Reference） {#第六部分悬垂引用}](#第六部分悬垂引用-第六部分悬垂引用)
    - [编译器如何防止 {#编译器如何防止}](#编译器如何防止-编译器如何防止)
    - [正确的做法 {#正确的做法}](#正确的做法-正确的做法)
  - [第七部分：字符串切片（String Slice） {#第七部分字符串切片}](#第七部分字符串切片-第七部分字符串切片)
    - [String vs \&str {#string-vs-str}](#string-vs-str-string-vs-str)
    - [字符串字面量 {#字符串字面量}](#字符串字面量-字符串字面量)
  - [第八部分：实践模式 {#第八部分实践模式}](#第八部分实践模式-第八部分实践模式)
    - [模式1: 所有权转移 {#模式1-所有权转移}](#模式1-所有权转移-模式1-所有权转移)
    - [模式2: 借用检查 {#模式2-借用检查}](#模式2-借用检查-模式2-借用检查)
    - [模式3: 可变借用（Mutable Borrow） {#模式3-可变借用}](#模式3-可变借用-模式3-可变借用)
  - [第九部分：常见错误 {#第九部分常见错误}](#第九部分常见错误-第九部分常见错误)
  - [第十部分：形式化视角 {#第十部分形式化视角}](#第十部分形式化视角-第十部分形式化视角)
    - [所有权定理 {#所有权定理}](#所有权定理-所有权定理)
    - [与形式化文档的关联 {#与形式化文档的关联}](#与形式化文档的关联-与形式化文档的关联)
  - [总结 {#总结-1}](#总结-总结-1)
  - [引言 {#引言-1}](#引言-引言-1-1)
  - [第一部分：问题是什么？ {#第一部分问题是什么}](#第一部分问题是什么-第一部分问题是什么)
    - [C/C++中的内存错误 {#cc中的内存错误}](#cc中的内存错误-cc中的内存错误)
    - [传统解决方案的局限 {#传统解决方案的局限}](#传统解决方案的局限-传统解决方案的局限)
  - [第二部分：所有权的核心规则 {#第二部分所有权的核心规则}](#第二部分所有权的核心规则-第二部分所有权的核心规则)
    - [规则很简单 {#规则很简单}](#规则很简单-规则很简单)
    - [直观理解：图书馆比喻 {#直观理解图书馆比喻}](#直观理解图书馆比喻-直观理解图书馆比喻)
  - [第三部分：为什么这能保证安全？ {#第三部分为什么这能保证安全}](#第三部分为什么这能保证安全-第三部分为什么这能保证安全)
    - [安全保证1：没有悬垂指针 {#安全保证1没有悬垂指针}](#安全保证1没有悬垂指针-安全保证1没有悬垂指针)
    - [安全保证2：没有双重释放 {#安全保证2没有双重释放}](#安全保证2没有双重释放-安全保证2没有双重释放)
    - [安全保证3：没有数据竞争 {#安全保证3没有数据竞争}](#安全保证3没有数据竞争-安全保证3没有数据竞争)
  - [第四部分：形式化视角 {#第四部分形式化视角}](#第四部分形式化视角-第四部分形式化视角)
    - [定理 T-OW2: 所有权唯一性 {#定理-t-ow2-所有权唯一性}](#定理-t-ow2-所有权唯一性-定理-t-ow2-所有权唯一性)
    - [定理 T-BR1: 数据竞争自由 {#定理-t-br1-数据竞争自由}](#定理-t-br1-数据竞争自由-定理-t-br1-数据竞争自由)
  - [第五部分：实际示例 {#第五部分实际示例}](#第五部分实际示例-第五部分实际示例)
    - [示例1: 函数参数传递 {#示例1-函数参数传递}](#示例1-函数参数传递-示例1-函数参数传递)
    - [示例2: 返回值 {#示例2-返回值}](#示例2-返回值-示例2-返回值)
    - [示例3: 借用临时使用 {#示例3-借用临时使用}](#示例3-借用临时使用-示例3-借用临时使用)
  - [第六部分：常见疑问 {#第六部分常见疑问}](#第六部分常见疑问-第六部分常见疑问)
    - [Q: 所有权检查有运行时（Runtime）开销吗？ {#q-所有权检查有运行时开销吗}](#q-所有权检查有运行时开销吗-q-所有权检查有运行时开销吗)
    - [Q: 所有权系统限制太多？ {#q-所有权系统限制太多}](#q-所有权系统限制太多-q-所有权系统限制太多)
    - [Q: 学习所有权很难？ {#q-学习所有权很难}](#q-学习所有权很难-q-学习所有权很难)
  - [总结 {#总结-1}](#总结-总结-1-1)
    - [所有权为什么安全？ {#所有权为什么安全}](#所有权为什么安全-所有权为什么安全)
    - [下一步学习 {#下一步学习}](#下一步学习-下一步学习)
...
| Rust | 所有权系统 | 零成本抽象（Zero-Cost Abstraction） | 学习曲线陡峭 |
...
**定理 (内存安全（Memory Safety）)**: 良类型的Rust程序不会出现以下错误:
...
| 智能指针（Smart Pointer） | 部分自动化 | 循环引用、仍然可能出错 |
