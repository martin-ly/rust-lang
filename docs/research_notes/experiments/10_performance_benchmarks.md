...
- [性能基准测试研究 {#性能基准测试研究}](#性能基准测试研究-性能基准测试研究)
  - [📑 目录 {#目录}](#-目录-目录)
  - [📊 目录 {#目录-1}](#-目录-目录-1)
  - [🎯 研究目标 {#研究目标}](#-研究目标-研究目标)
    - [核心问题 {#核心问题}](#核心问题-核心问题)
  ...
  - [📚 理论基础 {#理论基础}](#-理论基础-理论基础)
    - [相关概念 {#相关概念}](#相关概念-相关概念)
    - [理论背景 {#理论背景}](#理论背景-理论背景)
      - [Criterion.rs 统计方法 {#criterionrs-统计方法}](#criterionrs-统计方法-criterionrs-统计方法)
      - [The Rust Performance Book 方法论 {#the-rust-performance-book-方法论}](#the-rust-performance-book-方法论-the-rust-performance-book-方法论)
    - [形式化论证与实验衔接 {#形式化论证与实验衔接}](#形式化论证与实验衔接-形式化论证与实验衔接)
  - [🔬 实验设计 {#实验设计}](#-实验设计-实验设计)
    - [实验设计原则 {#实验设计原则}](#实验设计原则-实验设计原则)
    - [1. 内存分配性能测试 {#1-内存分配性能测试}](#1-内存分配性能测试-1-内存分配性能测试)
    - [2. 并发性能测试 {#2-并发性能测试}](#2-并发性能测试-2-并发性能测试)
    - [3. 序列化性能测试 {#3-序列化性能测试}](#3-序列化性能测试-3-序列化性能测试)
    - [4. 字符串处理性能测试 {#4-字符串处理性能测试}](#4-字符串处理性能测试-4-字符串处理性能测试)
    - [5. 集合操作性能测试 {#5-集合操作性能测试}](#5-集合操作性能测试-5-集合操作性能测试)
    - [测试框架和工具选择 {#测试框架和工具选择}](#测试框架和工具选择-测试框架和工具选择)
    - [Rust 1.96+ / Edition 2024 工具链 {#rust-196-edition-2024-工具链}](#rust-196--edition-2024-工具链-rust-196-edition-2024-工具链)
  - [💻 实验实现 {#实验实现}](#-实验实现-实验实现)
    - [1. 基准测试代码实现 {#1-基准测试代码实现}](#1-基准测试代码实现-1-基准测试代码实现)
      - [内存分配性能测试实现 {#内存分配性能测试实现}](#内存分配性能测试实现-内存分配性能测试实现)
      - [并发性能测试实现 {#并发性能测试实现}](#并发性能测试实现-并发性能测试实现)
      - [序列化性能测试实现 {#序列化性能测试实现}](#序列化性能测试实现-序列化性能测试实现)
    - [2. 测试环境配置 {#2-测试环境配置}](#2-测试环境配置-2-测试环境配置)
    - [3. 数据收集机制 {#3-数据收集机制}](#3-数据收集机制-3-数据收集机制)
    - [4. 结果分析工具实现 {#4-结果分析工具实现}](#4-结果分析工具实现-4-结果分析工具实现)
  - [💻 代码示例 {#代码示例}](#-代码示例-代码示例)
    - [示例 1：使用 Criterion 进行基准测试 {#示例-1使用-criterion-进行基准测试}](#示例-1使用-criterion-进行基准测试-示例-1使用-criterion-进行基准测试)
    - [示例 2：内存分配性能测试 {#示例-2内存分配性能测试}](#示例-2内存分配性能测试-示例-2内存分配性能测试)
    - [示例 3：并发性能测试 {#示例-3并发性能测试}](#示例-3并发性能测试-示例-3并发性能测试)
  - [💻 代码示例1 {#代码示例1}](#-代码示例1-代码示例1)
    - [示例 1：内存分配基准测试 {#示例-1内存分配基准测试}](#示例-1内存分配基准测试-示例-1内存分配基准测试)
    - [示例 2：并发性能基准测试 {#示例-2并发性能基准测试}](#示例-2并发性能基准测试-示例-2并发性能基准测试)
    - [示例 3：序列化性能基准测试 {#示例-3序列化性能基准测试}](#示例-3序列化性能基准测试-示例-3序列化性能基准测试)
  - [📋 数据收集执行指南 {#数据收集执行指南}](#-数据收集执行指南-数据收集执行指南)
    - [环境要求 {#环境要求}](#环境要求-环境要求)
    - [执行步骤 {#执行步骤}](#执行步骤-执行步骤)
    - [数据留存 {#数据留存}](#数据留存-数据留存)
  - [📊 实验结果 {#实验结果}](#-实验结果-实验结果)
    - [内存分配性能 {#内存分配性能}](#内存分配性能-内存分配性能)
    - [并发性能 {#并发性能}](#并发性能-并发性能)
  ...
  - [🆕 Rust 1.93.0 性能相关更新 {#rust-1930-性能相关更新}](#-rust-1930-性能相关更新-rust-1930-性能相关更新)
    - [全局分配器 thread\_local 支持 {#全局分配器-thread\_local-支持}](#全局分配器-thread_local-支持-全局分配器-thread_local-支持)
    - [MaybeUninit 新方法性能影响 {#maybeuninit-新方法性能影响}](#maybeuninit-新方法性能影响-maybeuninit-新方法性能影响)
    - [asm! 块中的 cfg 属性 {#asm-块中的-cfg-属性}](#asm-块中的-cfg-属性-asm-块中的-cfg-属性)
    - [状态机代码生成改进（2025年目标） {#状态机代码生成改进2025年目标}](#状态机代码生成改进2025年目标-状态机代码生成改进2025年目标)
    - [musl 1.2.5 更新 {#musl-125-更新}](#musl-125-更新-musl-125-更新)
  - [🔗 形式化链接 {#形式化链接}](#-形式化链接-形式化链接)
    - [实验与定理对应关系 {#实验与定理对应关系}](#实验与定理对应关系-实验与定理对应关系)
    - [相关研究笔记 {#相关研究笔记}](#相关研究笔记-相关研究笔记)
    - [形式化证明 {#形式化证明}](#形式化证明-形式化证明)
    - [理论体系 {#理论体系}](#理论体系-理论体系)
...
- **微基准测试**：测试单个函数或操作的性能
- **宏（Macro）基准测试**：测试整个系统或应用的性能
- **压力测试**：测试系统在极限负载下的表现

#### Criterion.rs 统计方法 {#criterionrs-统计方法}
...
- **多次采样与迭代**：自动 warm-up 后采集多组样本，降低测量噪声。
- **置信区间（Confidence Interval）**：对每次基准输出均值、中位数及 95% 置信区间，量化测量不确定性。
- **回归检测（Regression Detection）**：通过 `--save-baseline` / `--baseline` 比较两次运行，自动标记统计显著的性能回退。
- **异常值剔除**：基于四分位距（IQR）检测并报告离群样本。

这些统计方法与形式化证明互补：统计显著性提供经验证据，但不能替代类型系统（Type System）或所有权（Ownership）的形式化保证。

#### The Rust Performance Book 方法论 {#the-rust-performance-book-方法论}

> **来源: [The Rust Performance Book – Profiling](https://nnethercote.github.io/perf-book/profiling.html)**

性能优化应遵循"先测量、后优化"原则：

1. 使用 `cargo bench` / Criterion 建立可复现基线。
2. 使用 `perf`、`flamegraph`、`cachegrind` 定位热点。
3. 针对热点做最小改动，再用 Criterion 验证改进是否统计显著。
...
*证明*：由 [experiments/README](README.md) 定理 EX-T1、EX-T2；验证 + 可重复性 ⇒ 经验支持。∎
...
**引用**：见 [experiments/README](README.md) 定理 EX-T1、EX-T2；[FORMAL_PROOF_SYSTEM_GUIDE](../10_formal_proof_system_guide.md)。
...
- **不同数据类型性能**：基本类型、结构体（Struct）、枚举（Enum）、嵌套结构
...
- **编译器优化探索**：`cargo rustc --release -- -C opt-level=3 -C lto=thin`、`cargo bloat --release`。
- **可重复性**：固定 `rust-toolchain.toml`、提交 `Cargo.lock`、记录 CPU governor / 频率与操作系统版本。
...
**示例填写**（典型 x86_64、Rust 1.96+、release）：
...
**结论填写**：与初步结果对比，说明是否符合预期；若与 Rust 1.96+ 的 thread_local 分配器、MaybeUninit 等相关，可单独注明。
...
- [Criterion.rs Book](https://bheisler.github.io/criterion.rs/book/) - 统计驱动的基准测试框架官方书籍
- [The Rust Performance Book](https://nnethercote.github.io/perf-book/) - Rust 性能优化权威指南
- [Rust Standard Library](https://doc.rust-lang.org/std/) - `std::hint::black_box` 等 API 参考
...
Rust 1.93.0 稳定化了 `MaybeUninit<T>` 切片（Slice）的新方法，这些方法对性能有积极影响：
...
- 预期性能提升：10-30%（特定场景）
- 对异步（Async）状态机、压缩算法、视频解码器等场景有显著影响
...
| 性能实验形式化 | [../experiments/README.md](README.md) | 实验形式化论证 (EX-T1, EX-T2) |
