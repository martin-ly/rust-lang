...
| **概念** | outlives、区域类型（[lifetime_formalization](formal_methods/10_lifetime_formalization.md)） |
| **属性** | 引用有效性、无悬垂 |
| **关系** | 与借用协同；泛型生命周期 `'a` |
| **解释** | 引用不能比其引用对象活得更久 |
| **示例** | `fn f<'a>(x: &'a i32) -> &'a i32 { x }` |
| **论证** | [DESIGN_MECHANISM_RATIONALE](./10_design_mechanism_rationale.md) § 生命周期 |
| **形式化** | [lifetime_formalization](formal_methods/10_lifetime_formalization.md) 定理 T2 |
| **反例** | 返回局部引用 |
...
| **概念** | Pin 不变式（[pin_self_referential](formal_methods/10_pin_self_referential.md)） |
| **属性** | 自引用安全、Future 位置稳定 |
| **关系** | Future 依赖 Pin；Unpin 为默认 |
| **解释** | 自引用结构移动→悬垂；Pin 禁止移动 |
| **示例** | `Box::pin(x)` 堆固定；`Pin::new(&mut x)` 仅 Unpin |
| **论证** | [DESIGN_MECHANISM_RATIONALE](./10_design_mechanism_rationale.md) § Pin 堆/栈区分 |
| **形式化** | [pin_self_referential](formal_methods/10_pin_self_referential.md) 定理 T1–T3 |
...
| **概念** | 自动 trait；[async_state_machine](formal_methods/10_async_state_machine.md) |
| **属性** | 结构性质；编译器自动推导 |
| **关系** | `T: Sync` ⇔ `&T: Send`；Rc 非 Send |
| **解释** | 跨线程传递需 Send；共享需 Sync |
| **示例** | `thread::spawn(\|\| { ... })` 闭包需 Send |
| **论证** | [DESIGN_MECHANISM_RATIONALE](./10_design_mechanism_rationale.md) § Send/Sync |
| **形式化** | [async_state_machine](formal_methods/10_async_state_machine.md) SPAWN1、Send/Sync 语义 |
...
| **论证** | [trait_system_formalization](type_theory/10_trait_system_formalization.md) |
...
| **论证** | [borrow_checker_proof](formal_methods/10_borrow_checker_proof.md) Def MATCH1 |
| **形式化** | 模式匹配规则、穷尽检查 |
| **反例** | 非穷尽 match 编译错误 |
...
| **论证** | [borrow_checker_proof](formal_methods/10_borrow_checker_proof.md) Def FOR1 |
| **形式化** | 迭代与借用规则 |
| **反例** | 迭代中修改集合 |
...
| **论证** | [DESIGN_MECHANISM_RATIONALE](./10_design_mechanism_rationale.md) 定理 OR-T1 |
...
| **示例** | `let x = f()?;` |
| **论证** | [borrow_checker_proof](formal_methods/10_borrow_checker_proof.md) Def QUERY1 |
| **形式化** | 控制流与错误传播规则 |
| **反例** | 非 Result 类型使用 ? |
