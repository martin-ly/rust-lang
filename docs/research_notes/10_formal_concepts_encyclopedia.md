...
- [Rust形式化概念百科 {#rust形式化概念百科}](#rust形式化概念百科-rust形式化概念百科)
  - [目录 {#目录}](#目录-目录)
  - [一、所有权（Ownership）与借用（Borrowing） {#一所有权与借用}](#一所有权与借用-一所有权与借用)
    ...
    - [1.2 借用 (Borrowing) {#12-借用-borrowing}](#12-借用-borrowing-12-借用-borrowing)
      - [不可变借用（Mutable Borrow） (\&T) {#不可变借用-t}](#不可变借用-t-不可变借用-t)
  ...
  - [二、类型系统（Type System） {#二类型系统}](#二类型系统-二类型系统)
    - [2.1 类型安全 {#21-类型安全}](#21-类型安全-21-类型安全)
    - [2.2 型变 (Variance) {#22-型变-variance}](#22-型变-variance-22-型变-variance)
    - [2.3 Trait系统 {#23-trait（Trait）系统}](#23-trait系统-23-trait系统)
  - [三、生命周期（Lifetimes） {#三生命周期}](#三生命周期-三生命周期)
    - [3.1 生命周期省略（Lifetime Elision）规则 {#31-生命周期省略规则}](#31-生命周期省略规则-31-生命周期省略规则)
    - [3.2 生命周期边界 {#32-生命周期边界}](#32-生命周期边界-32-生命周期边界)
  - [四、并发与异步（Async） {#四并发与异步}](#四并发与异步-四并发与异步)
    - [4.1 Send（Send）与Sync（Sync） {#41-send与sync}](#41-send与sync-41-send与sync)
    - [4.2 异步与Future（Future） {#42-异步与future}](#42-异步与future-42-异步与future)
