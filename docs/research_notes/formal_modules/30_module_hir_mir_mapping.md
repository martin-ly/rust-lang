...
描述 Rust 模块树在编译器前端如何被降维为 HIR（High-level Intermediate Representation）的 ItemTree，以及 MIR 如何保留模块边界与可见性信息，为理解编译期名称解析、宏展开和 borrow check 提供中间层视角。
...
MIR 的设计目标是程序分析与代码生成，其基本单元是函数体（`Body`）和常量求值。模块属于“命名空间组织”概念，在运行时不存在；因此模块边界被消解为 `DefId` 引用，borrow checker 和优化器无需关心 item 定义在哪个模块。
...
| 模块边界影响 borrow check | 跨模块借用 | borrow check 只关注 lifetime 与所有权，不关心模块；但跨模块可见性影响能否写出该借用 |
| `#[cfg]` 与模块 | `#[cfg(feature = "x")] mod m;` | 在 AST/HIR 转换前由 cfg 处理，未启用模块不会进入 HIR |
| 泛型函数在 MIR 中 | `fn f<T>(x: T) {}` | MIR 保存泛型体，单态化在 monomorphization 阶段生成具体实例 |
