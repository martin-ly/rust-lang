...
- [Rust 设计机制论证：理由与完整论证 {#rust-设计机制论证理由与完整论证}](#rust-设计机制论证理由与完整论证-rust-设计机制论证理由与完整论证)
  ...
  - [🔒 所有权（Ownership）：为何采用移动语义而非复制语义？ {#所有权为何采用移动语义而非复制语义}](#-所有权为何采用移动语义而非复制语义-所有权为何采用移动语义而非复制语义)
  - [📐 借用：为何可变借用（Mutable Borrow）独占？ {#借用为何可变借用独占}](#-借用为何可变借用独占-借用为何可变借用独占)
  - [⏱️ 生命周期（Lifetimes）：为何需要显式标注？ {#生命周期为何需要显式标注}](#️-生命周期为何需要显式标注-生命周期为何需要显式标注)
  - [📊 型变：为何协变/逆变/不变三种？ {#型变为何协变逆变不变三种}](#-型变为何协变逆变不变三种-型变为何协变逆变不变三种)
  - [🔄 异步（Async）：为何 Future 需要 Pin？ {#异步为何-future-需要-pin}](#-异步为何-future-需要-pin-异步为何-future-需要-pin)
  ...
  - [📦 宏：为何声明宏（Declarative Macro）与过程宏（Procedural Macro）分离？ {#宏为何声明宏与过程宏分离}](#-宏为何声明宏与过程宏分离-宏为何声明宏与过程宏分离)
  - [🔄 闭包（Closures）：为何三种捕获方式？ {#闭包为何三种捕获方式}](#-闭包为何三种捕获方式-闭包为何三种捕获方式)
  - [🎯 模式匹配（Pattern Matching）：为何穷尽？ {#模式匹配为何穷尽}](#-模式匹配为何穷尽-模式匹配为何穷尽)
