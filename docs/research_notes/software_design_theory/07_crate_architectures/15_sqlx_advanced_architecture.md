...
SQLx 是 Rust 生态中的异步 SQL 工具包，年下载量超过 3000 万次 来源: [crates.io 统计, 2025](https://crates.io/)。

与其他 Rust ORM（如 Diesel）不同，SQLx 采用**查询优先（query-first）**
...
### 3.1 `Executor` — 统一查询执行接口 {#31-executor-统一查询执行接口}
...
### 3.2 `FromRow` — 行到结构体的类型安全映射 {#32-fromrow-行到结构体的类型安全映射}
...
### 3.3 `Type`, `Encode`, `Decode` — 类型系统的三支柱 {#33-type-encode-decode-类型系统的三支柱}
...
## 4. 编译期查询验证：宏系统的工程实现 {#4-编译期查询验证宏系统的工程实现}

>

> **[来源: [Rust Cookbook](https://rust-lang-nursery.github.io/rust-cookbook/)]**

### 4.1 `query!` 宏的工作流程 {#41-query-宏的工作流程}
...
### 4.2 `query_as!` 与显式类型的映射 {#42-query_as-与显式类型的映射}
...
## 5. 连接池与异步执行模型 {#5-连接池与异步执行模型}

>

> **[来源: [Rust Reference](https://doc.rust-lang.org/reference/)]**

### 5.1 `Pool` 的内部结构 {#51-pool-的内部结构}
...
## 6. 迁移系统：`sqlx migrate` {#6-迁移系统sqlx-migrate}

>

> **[来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/)]**

### 6.1 迁移的设计原则 {#61-迁移的设计原则}
...
## 7. SQLx vs Diesel：两种范型的对比 {#7-sqlx-vs-diesel两种范型的对比}
...
## 9. 反模式边界：何时不应使用 SQLx {#9-反模式边界何时不应使用-sqlx}

>

> **[来源: [Rust By Example](https://doc.rust-lang.org/rust-by-example/)]**

### 9.1 无编译期数据库访问的场景 {#91-无编译期数据库访问的场景}
...
# 在可访问 DB 的环境中生成离线数据 {#在可访问-db-的环境中生成离线数据}

$ cargo sqlx prepare -- --lib



# 生成 sqlx-data.json，后续离线构建时宏读取此文件 {#生成-sqlx-datajson后续离线构建时宏读取此文件}
...
### 9.3 需要 ORM 级联操作的场景 {#93-需要-orm-级联操作的场景}
...
### 9.4 多数据库异构部署的复杂性 {#94-多数据库异构部署的复杂性}
