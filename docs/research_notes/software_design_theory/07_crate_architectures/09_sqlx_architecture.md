...
与传统的 Diesel（ORM 风格）或 rust-postgres（纯运行时驱动）不同，SQLx 通过过程宏（procedural macros）在编译期连接数据库，验证查询语句的语法正确性、列名匹配以及类型兼容性。这意味着如果数据库表结构发生变更，相关查询代码会在 `cargo build` 阶段即报错，而非运行时才暴露问题。
