...
## 3. 关键 Trait 定义 {#3-关键-trait-定义}

>
> **[来源: [The Rust Programming Language](https://doc.rust-lang.org/book/)]**

### 3.1 生成的 `Service` Trait — 服务端契约 {#31-生成的-service-trait-服务端契约}
...
### 3.2 `Service<http::Request<Body>>` — Tower 统一的请求抽象 {#32-servicehttprequestbody-tower-统一的请求抽象}
...
### 3.3 `Streaming<T>` — 流类型的方向安全 {#33-streamingt-流类型的方向安全}
...
`Streaming<T>` 在服务端表示**客户端发送的输入流**，在客户端表示**服务端发送的输出流**。
...
## 4. Protobuf 代码生成：`tonic-build` {#4-protobuf-代码生成tonic-build}

>
> **[来源: [Rust Cookbook](https://rust-lang-nursery.github.io/rust-cookbook/)]**

### 4.1 构建脚本集成 {#41-构建脚本集成}
...
### 4.2 生成的 Client 使用 {#42-生成的-client-使用}
...
## 5. 拦截器：Tower Layer 的横切切面 {#5-拦截器tower-layer-的横切切面}

>
> **[来源: [Rust Reference](https://doc.rust-lang.org/reference/)]**

### 5.1 Interceptor 作为同步函数 {#51-interceptor-作为同步函数}
...
Tonic 的 interceptor 是**同步函数**——这是有意的设计选择。

Interceptor 只应执行轻量级逻辑（认证、日志、元数据注入），不应阻塞。

需要异步操作（如数据库查询）时，应使用 Tower `Layer` 实现自定义 `Service`。

### 5.2 Interceptor 在 Tower 中的位置 {#52-interceptor-在-tower-中的位置}
...
## 6. 流处理：gRPC 四种通信模式 {#6-流处理grpc-四种通信模式}

>
> **[来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/)]**

### 6.1 通信模式类型表 {#61-通信模式类型表}
...
### 7.1 HTTP/2 多路复用 {#71-http2-多路复用}

>
> **[来源: [docs.rs](https://docs.rs/)]**

Tonic 基于 Hyper 的 HTTP/2 实现，单一 TCP 连接上可同时承载多个 gRPC 调用（stream）。
...
### 7.2 Protobuf 编解码性能 {#72-protobuf-编解码性能}
...
### 7.3 与 tonic 基准对比（示意） {#73-与-tonic-基准对比示意}
...
## 8. 反模式边界：何时不应使用 Tonic {#8-反模式边界何时不应使用-tonic}

>
> **[来源: [Rust Standard Library](https://doc.rust-lang.org/std/)]**

### 8.1 需要浏览器直接访问的 API {#81-需要浏览器直接访问的-api}

>
> **[来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/)]**

gRPC 依赖 HTTP/2 trailer 和二进制帧，浏览器无法直接发起 gRPC 调用（除非使用 gRPC-Web 代理）。

如果 API 主要面向浏览器客户端，应优先考虑 REST/JSON（Axum + serde_json）或 GraphQL。
...
### 8.2 需要人类可读协议的场景 {#82-需要人类可读协议的场景}

>
> **[来源: [Rust By Example](https://doc.rust-lang.org/rust-by-example/)]**

Protobuf 是二进制格式，调试时需要 `grpcurl` 或专用工具。

如果协议需要频繁的人工检查（如支付回调、第三方 webhook），JSON over HTTP 更友好。

### 8.3 `.proto` 的版本演化失误 {#83-proto-的版本演化失误}
