...
## 1. 引言：Rust 异步生态的基石 {#1-引言rust-异步生态的基石}
...
### 2.2 Runtime 构建模式 {#22-runtime-构建模式}
...
## 3. Task 调度：Work-Stealing 队列 {#3-task-调度work-stealing-队列}
...
### 3.3 Waker 与任务唤醒 {#33-waker-与任务唤醒}
...
## 4. IO Driver：Reactor 模式 {#4-io-driverreactor-模式}

>

> **[来源: [Rust Reference](https://doc.rust-lang.org/reference/)]**

### 4.1 mio 集成与跨平台抽象 {#41-mio-集成与跨平台抽象}
...
### 4.2 AsyncFd：桥接同步文件描述符 {#42-asyncfd桥接同步文件描述符}
...
### 4.3 IO 驱动与调度器的协作 {#43-io-驱动与调度器的协作}
...
## 5. Timer Wheel：分层时间轮 {#5-timer-wheel分层时间轮}

>

> **[来源: [Rust By Example](https://doc.rust-lang.org/rust-by-example/)]**

### 5.1 分层时间轮原理 {#51-分层时间轮原理}
...
### 5.2 Tokio Timer API {#52-tokio-timer-api}
...
### 5.3 定时器与 IO 的整合 {#53-定时器与-io-的整合}
...
### 6.1 Future Trait 与 Pin {#61-future-trait-与-pin}
...
### 6.2 Send / Sync 跨越 await 点 {#62-send-sync-跨越-await-点}
...
### 6.3 tokio::select! 与竞态条件 {#63-tokioselect-与竞态条件}
...
### 7.1 Semaphore：并发控制 {#71-semaphore并发控制}
...
### 7.2 有界通道 (Bounded Channels) {#72-有界通道-bounded-channels}
...
### 7.3 RwLock 与协作式调度 {#73-rwlock-与协作式调度}
