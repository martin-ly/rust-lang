...
## 2. 核心架构: HttpServer → App → Route → Handler
>
...
| `Route` | 路径模式匹配、方法守卫、提取器链 | `actix_web::Route` |
| `Handler` | 用户定义的请求处理函数 | `Handler<Args>` trait |

### 2.2 请求流经 Actor 系统的完整路径
>
> **[来源: [Rustonomicon](https://doc.rust-lang.org/nomicon/)]**

下面的 Mermaid 图展示了 HTTP 请求从网络层到达 Handler 的完整生命周期，以及 Actor 模型在其中的作用：
...
Actix-web 的并发能力根植于 Actix actor 框架。理解以下三个核心抽象，是掌握 Actix-web 性能特征的前提。
...
每个实现了 `Actor` trait 的类型都拥有自己的状态和一个 mailbox。外部通过向 mailbox 发送消息来与 Actor 通信，Actor 按顺序处理消息，**同一 Actor 的消息处理是串行的，从而消除了数据竞争**。
...
在 Actix-web 中，`HttpServer` 的内部 Worker 就是 Actor。HTTP 请求被封装为消息投递到 Worker 的 mailbox，Worker 的 `Handler` 实现负责调用路由系统。

### 3.4 Actor 模型在 Actix-web 中的实际体现
>
> **[来源: [The Rust Programming Language](https://doc.rust-lang.org/book/)]**
...
**重要洞察**: Worker Actor 的 mailbox 机制天然提供了**反压 (backpressure)**。当 Handler 处理速度低于请求到达速度时，未处理请求会在 mailbox 中排队，而非无限制地创建任务，从而避免内存爆炸。
...
Extractor（提取器）是 Actix-web 最具特色的设计之一。它允许 Handler 以声明式的方式从 HTTP 请求中提取数据。
...
### 5.2 `ServiceRequest` / `ServiceResponse`
>
> **[来源: [Rust Reference](https://doc.rust-lang.org/reference/)]**
...
| **Handler 签名** | 支持 `async fn` + 多参数提取器；也支持非异步闭包 | 严格 `async fn` + 元组提取器 (`(State<S>, Json<T>)`) |
...
> [来源: Actix-web GitHub — Performance Benchmarks]
> [来源: Axum 文档 — Comparison with other frameworks](https://docs.rs/axum/latest/axum/#comparison-with-other-frameworks)
