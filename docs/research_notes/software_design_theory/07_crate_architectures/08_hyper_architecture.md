...
Hyper 是 Rust 生态中历史最悠久、应用最广泛的 HTTP 实现库，自 2014 年创建以来，它始终是 Rust 异步 HTTP 栈的基石。

Reqwest（HTTP 客户端）、Axum（HTTP 服务端）、Tonic（gRPC）等知名 crate 均构建于 Hyper 之上。

Hyper 的设计核心围绕两个目标：**协议正确性**（strict adherence to HTTP/1.1 和 HTTP/2 规范）与**性能**（零拷贝解析、最小化内存分配、高效的异步 I/O）。
...
通过精心的 trait 设计和底层优化，完全可以同时达到 C 级性能和内存安全保证。
