...
Tracing 是由 Tokio 团队开发的**结构化诊断与遥测框架**，年下载量超过 1.5 亿次 [来源: crates.io 统计, 2025]。与传统日志库（如 `log` crate）的"扁平字符串输出"不同，Tracing 的核心创新在于**将诊断信息建模为层次化的事件流**：`Span` 表示具有持续时间的上下文区间，`Event` 表示瞬时点状事件，二者共同构成一棵树形结构，天然支持分布式追踪（distributed tracing）的语义。
