...
Reqwest 核心库有意保持精简，中间件能力通过 `reqwest_middleware` crate 提供。该 crate 引入了 `Middleware` trait，允许在请求发送前后插入自定义逻辑：
...
这种设计的优势在于**代码复用**：`blocking` 模块与异步模块共享相同的 `hyper` 连接池、TLS 实现和协议处理逻辑，仅在最外层通过 `block_on` 桥接。但代价是 `blocking::Client` 会占用一个 OS 线程运行内部事件循环，不适合在已有异步运行时中混用。
