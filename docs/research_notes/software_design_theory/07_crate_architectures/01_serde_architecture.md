...
> 若只需要使用指南与生态定位，请优先参考：
>
> - [Serde 使用模式](../../../../concept/02_intermediate/00_traits/09_serde_patterns.md)
...
### 3.1 `Serialize` — 数据结构的序列化承诺 {#31-serialize-数据结构的序列化承诺}
...
### 3.2 `Deserialize` — 反序列化的生命周期感知 {#32-deserialize-反序列化的生命周期感知}
...
### 3.3 `Serializer` — 格式写入的 25 种原子操作 {#33-serializer-格式写入的-25-种原子操作}
...
`Serializer` 为每种 Rust 原生类型和复合类型（序列、映射、结构体（Struct）、枚举（Enum））提供了对应的序列化方法。关联类型（如 `SerializeSeq`、`SerializeMap`）进一步将复合类型的**状态化写入**委托给专门的子 trait，避免在序列化中途传递额外的上下文参数。
...
### 3.4 `Deserializer` — 格式解析的入口 {#34-deserializer-格式解析的入口}
...
### 3.5 `Visitor` — 类型构造的回调机制 {#35-visitor-类型构造的回调机制}
...
## 4. 类型系统利用：零成本抽象的工程实践 {#4-类型系统利用零成本抽象的工程实践}
...
### 4.1 静态分发：单态化消除虚拟调用 {#41-静态分发单态化消除虚拟调用}
...
### 4.2 编译期格式选择：类型即配置 {#42-编译期格式选择类型即配置}
...
## 5. 零拷贝反序列化：生命周期管理 {#5-零拷贝反序列化生命周期管理}
...
## 6. derive macro 实现：从属性到生成代码 {#6-derive-macro-实现从属性到生成代码}
...
## 8. 反模式边界：何时不应使用 Serde {#8-反模式边界何时不应使用-serde}
...
### 8.1 需要精确内存布局的二进制协议 {#81-需要精确内存布局的二进制协议}
...
### 8.5 小结：Serde 的适用光谱 {#85-小结serde-的适用光谱}
...
| The Rust Programming Language | <https://doc.rust-lang.org/book/> | Trait、泛型（Generics）、生命周期基础 |
| Rust Reference | <https://doc.rust-lang.org/reference/> | 过程宏（Procedural Macro）、生命周期、类型系统细节 |
...
- [Trait 系统与泛型编程](../../../../concept/02_intermediate/00_traits/01_traits.md)
