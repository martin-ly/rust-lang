...
与传统运行时使用字符串反射解析参数的 CLI 框架不同，Clap 通过 derive 宏在编译期生成完整的参数元数据结构和解析状态机，运行时仅需执行确定性的状态转换，无需任何动态类型检查或反射开销。

> [来源: Clap 官方文档, https://docs.rs/clap/latest/clap/]
...
Derive API 本质上是 Builder API 的声明式语法糖，由过程宏在编译期展开为等价的 Builder 调用。
...
> [来源: Clap Derive 文档, https://docs.rs/clap/latest/clap/_derive/index.html]
...
> [来源: Clap 过程宏源码, https://github.com/clap-rs/clap/tree/master/clap_derive]
> [来源: syn crate 文档, https://docs.rs/syn/latest/syn/]
> [来源: quote crate 文档, https://docs.rs/quote/latest/quote/]
...
Clap 深度利用 Rust 的类型系统实现参数解析的类型安全，将字符串到类型的转换编码为 trait 约束，不兼容的类型转换在编译期即被拒绝。
...
> [来源: Clap Builder 文档, https://docs.rs/clap/latest/clap/builder/index.html]
> [来源: Rust Reference, FromStr trait, https://doc.rust-lang.org/std/str/trait.FromStr.html]
...
> [来源: Clap Subcommands 文档, https://docs.rs/clap/latest/clap/_derive/_tutorial/chapter_2/index.html]
> [来源: Rust Reference, 枚举类型, https://doc.rust-lang.org/reference/items/enumerations.html]
...
> [来源: Clap 验证文档, https://docs.rs/clap/latest/clap/builder/struct.Arg.html#method.value_parser]
> [来源: Rust Reference, 错误处理, https://doc.rust-lang.org/book/ch09-00-error-handling.html]
...
Clap v4 中 `Command` 对象在首次使用时 lazily 构建，构建过程纯函数式。Arg 定义存储于栈上静态数组，可能的取值列表使用字符串切片，仅 help 文本和错误消息需要堆分配。
...
> [来源: Clap 性能文档, https://docs.rs/clap/latest/clap/_faq/index.html#performance]
> [来源: Rust Reference, 零成本抽象, https://doc.rust-lang.org/reference/items/generics.html]
...
| Clap Derive 指南 | <https://docs.rs/clap/latest/clap/_derive/index.html> | Derive 宏完整用法 |
| Clap GitHub | <https://github.com/clap-rs/clap> | 源码、CHANGELOG、RFC |
| The Rust Programming Language | <https://doc.rust-lang.org/book/> | Trait、宏、错误处理基础 |
| Rust Reference | <https://doc.rust-lang.org/reference/> | 过程宏、枚举、泛型 |
