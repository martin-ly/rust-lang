...
**定理 B-T1（边界一致性（Coherence））**：对任意模式 $D$，$B_s(D)$、$B_p(D)$、$B_e(D)$ 由各矩阵文档的 Def 与定理唯一确定；与 [02_workflow_safe_complete_models](../02_workflow_safe_complete_models/README.md) 的 23/43 分类一致。
...
**场景**：需跨平台 UI 组件族（按钮、文本框）；运行时（Runtime）根据平台选择。
...
**步骤**：查模式 → Singleton；判安全 → [safe_unsafe_matrix](10_safe_unsafe_matrix.md) 创建型 → **OnceLock 为纯 Safe**；判支持 → 原生 `std::sync::OnceLock`。
...
> **来源: [Rust API Guidelines – Unsafe Code](https://rust-lang.github.io/api-guidelines/type-safety.html)**
