...
- [Iterator 形式化分析 {#iterator-形式化分析}](#iterator-形式化分析-iterator-形式化分析)
  ...
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
    ...
    - [Axiom IT2（可变借用（Mutable Borrow）公理） {#axiom-it2可变借用公理}](#axiom-it2可变借用公理-axiom-it2可变借用公理)
    - [定理 IT-T1（Iterator trait 类型安全定理） {#定理-it-t1iterator-trait-类型安全定理}](#定理-it-t1iterator-trait-类型安全定理-定理-it-t1iterator-trait-类型安全定理)
    - [定理 IT-T2（可变借用（Borrowing）安全定理） {#定理-it-t2可变借用安全定理}](#定理-it-t2可变借用安全定理-定理-it-t2可变借用安全定理)
  ...
  - [反例：常见误用及编译器错误 {#反例常见误用及编译器错误}](#反例常见误用及编译器错误-反例常见误用及编译器错误)
    ...
    - [反例 2：返回局部引用（Reference） {#反例-2返回局部引用}](#反例-2返回局部引用-反例-2返回局部引用)
...
| 权威来源 | 对应章节 / 条款 | 与本模式关系 |
| :--- | :--- | :--- |
| Rust Design Patterns | [Behavioral Patterns – Iterator](https://rust-unofficial.github.io/patterns/patterns/behavioural/iterator.html) | Rust 惯用实现与模式定位 |
...
以下示例已在 **Rust 1.96.1+ (Edition 2024)** 语义下校验，使用 `Iterator trait、IntoIterator、自定义迭代器` 等现代惯用法。
