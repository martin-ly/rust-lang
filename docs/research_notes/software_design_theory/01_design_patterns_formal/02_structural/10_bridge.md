...
- [Bridge 形式化分析 {#bridge-形式化分析}](#bridge-形式化分析-bridge-形式化分析)
  ...
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
    ...
    - [Axiom BR2（委托借用（Borrowing）公理） {#axiom-br2委托借用公理}](#axiom-br2委托借用公理-axiom-br2委托借用公理)
...
- trait 定义 $\mathcal{I}$，$\mathcal{A}$ 通过泛型（Generics） `T: Impl` 或 `Box<dyn Impl>` 使用
- **双向扩展**：抽象和实现可独立添加新变体
...
1. **泛型版本**：
   ...
   - 编译期单态化（Monomorphization）：为每个 `R` 生成特定代码
...
3. **独立性**：
   ...
   - 添加新形状：定义新结构体（Struct），使用现有 `Renderer`
...
以下示例已在 **Rust 1.96.1+ (Edition 2024)** 语义下校验，使用 `组合替代继承、trait 作为实现层` 等现代惯用法。
...
| 生命周期 | trait object 可标注 `dyn Renderer + 'a` | 引用（Reference）有效性 |
...
| :--- | :--- | :--- |
| `trait` | 实现接口 | 方法签名约束 |
| 泛型（Generics） `<R: Renderer>` | 抽象持有实现 | 编译期类型检查 |
...
| 枚举（Enum）实现 | `enum Impl { A, B }` | 有限实现集 |
