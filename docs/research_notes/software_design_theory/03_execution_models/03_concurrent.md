...
> **EN**: Concurrent
> **Summary**: 并发执行模型形式化 Concurrent. (stub/archive redirect)
> **概念族**: 软件设计 / 执行模型
> **内容分级**: [归档级]
>
> **分级**: [B]
...
- [并发执行模型形式化 {#并发执行模型形式化}](#并发执行模型形式化-并发执行模型形式化)
  - [📑 目录 {#目录}](#-目录-目录)
  - [形式化定义 {#形式化定义}](#形式化定义-形式化定义)
  - [Rust 并发原语 {#rust-并发原语}](#rust-并发原语-rust-并发原语)
  - [Crossbeam 扩展原语 {#crossbeam-扩展原语}](#crossbeam-扩展原语-crossbeam-扩展原语)
  - [代码示例 {#代码示例}](#代码示例-代码示例)
    - [消息传递 {#消息传递}](#消息传递-消息传递)
    - [共享状态 {#共享状态}](#共享状态-共享状态)
  - [典型场景（实质内容） {#典型场景实质内容}](#典型场景实质内容-典型场景实质内容)
    - [与设计模式组合 {#与设计模式组合}](#与设计模式组合-与设计模式组合)
    - [常见陷阱（扩展） {#常见陷阱扩展}](#常见陷阱扩展-常见陷阱扩展)
  - [原子操作（Atomic Operations）与内存顺序 {#原子操作与内存顺序}](#原子操作与内存顺序-原子操作与内存顺序)
  - [死锁与避免 {#死锁与避免}](#死锁与避免-死锁与避免)
  - [通道与共享状态选型 {#通道与共享状态选型}](#通道与共享状态选型-通道与共享状态选型)
  - [典型反例 {#典型反例}](#典型反例-典型反例)
  - [边界 {#边界}](#边界-边界)
...
- 无数据竞争：由类型系统（Type System）保证（Send/Sync）

**Def 1.2（Send）**:

$T : \mathrm{Send}$ 当且仅当 $T$ 可安全跨线程边界转移所有权（Ownership）。形式化：若 $v : T$ 在线程 $t_1$ 中，转移至 $t_2$ 后 $t_1$ 不再访问，且 $t_2$ 可安全使用。
...
**定理 CC-T1**：由 [borrow_checker_proof](../../formal_methods/10_borrow_checker_proof.md) 数据竞争自由；Send/Sync 为结构性质，组合保持。

**定理 CC-T2**：由 [async_state_machine](../../formal_methods/10_async_state_machine.md) 定理 6.2，Send + Sync 保证并发安全（Concurrency Safety）。

**引理 CC-L1（通道无共享）**：`mpsc::channel` 的 Sender/Receiver 分离所有权；无共享可变；消息传递语义不违反借用（Borrowing）规则。

*证明*：由 Def 1.2、1.3；Send 转移所有权；Receiver 独占消费；无 `&T` 跨线程共享，故无数据竞争。∎

**推论 CC-C1**：Mutex/Arc 组合时，$T$ 需 Send；Arc 内部需 Sync；由 [borrow_checker_proof](../../formal_methods/10_borrow_checker_proof.md) T1、[ownership_model](../../formal_methods/10_ownership_model.md) T2。∎
...
| 原语 | 用途 | 所有权（Ownership）/借用（Borrowing） |
| :--- | :--- | :--- |
| `std::thread::spawn` | 创建线程 | 闭包（Closures）需 `Send`，捕获值转移 |
| `mpsc::channel` | 消息传递 | `Sender`/`Receiver` 分离，无共享 |
| `Mutex<T>` | 互斥共享 | 持有所有权，`lock()` 返回 `MutexGuard` |
| `RwLock<T>` | 读写锁 | 多读或单写 |
| `Arc<T>` | 跨线程共享 | 引用（Reference）计数，$T$ 需 `Send` |
| `crossbeam::channel` | MPMC 通道 | 有界/无界，性能优于 std |
| `crossbeam::scope` | 作用域线程 | 借用栈数据，自动 join |
| `crossbeam::epoch` | 无锁回收 | 基于 epoch 的内存安全（Memory Safety）回收 |

> **来源: [Crossbeam Docs](https://docs.rs/crossbeam/latest/crossbeam/)**

---

## Crossbeam 扩展原语 {#crossbeam-扩展原语}

> **来源: [Crossbeam Docs](https://docs.rs/crossbeam/latest/crossbeam/)**

**Def 1.5（Scoped Threads）**：

`crossbeam::scope` 允许子线程借用父线程的栈数据，因为 scope 保证所有子线程在 scope 结束前 `join`。
...
*证明*：由 `scope` 契约，所有 `spawn` 的线程在 scope 闭包返回前完成；借用生命周期（Lifetimes）不超出 scope。∎
...
| 组合 | 说明 |
| :--- | :--- |
| 并发 + Observer | `mpsc` 或 `broadcast` 替代回调；见 [observer](../01_design_patterns_formal/03_behavioral/10_observer.md) |
| 并发 + Flyweight | `Arc` 跨线程共享不可变；见 [flyweight](../01_design_patterns_formal/02_structural/10_flyweight.md) |
