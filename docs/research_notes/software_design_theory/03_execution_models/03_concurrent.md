...
- [并发执行模型形式化 {#并发执行模型形式化}](#并发执行模型形式化-并发执行模型形式化)
  ...
  - [原子操作与内存顺序 {#原子操作与内存顺序}](#原子操作与内存顺序-原子操作与内存顺序)
...
**定理 CC-T1**：由 [borrow_checker_proof](../../../research_notes/formal_methods/10_borrow_checker_proof.md) 数据竞争自由；Send/Sync 为结构性质，组合保持。

**定理 CC-T2**：由 [async_state_machine](../../../research_notes/formal_methods/10_async_state_machine.md) 定理 6.2，Send + Sync 保证并发安全。
...
**推论 CC-C1**：Mutex/Arc 组合时，$T$ 需 Send；Arc 内部需 Sync；由 [borrow_checker_proof](../../../research_notes/formal_methods/10_borrow_checker_proof.md) T1、[ownership_model](../../../research_notes/formal_methods/10_ownership_model.md) T2。∎
...
| `crossbeam::epoch` | 无锁回收 | 基于 epoch 的内存安全回收 |
