...
>
> **1.96.0 语言特性基本对齐**：`assert_matches!`、`core::range`、`NonZero` 迭代等核心特性已覆盖。
>
...
| `From<bool> for {f32,f64}` | **1.68.0** | 1.68.0 ✅ | ✅ 已修正 |
| `VecDeque::new` const | **1.68.0** | 1.68.0 ✅ | ✅ 已修正 |
| `assert_matches!` | **1.95** | 1.96.0 | ⚠️ 部分标注有误 |
| `core::range` | **1.95** | 1.96.0 | ⚠️ 部分标注有误 |
...
| `From<bool> for {f32,f64}` 在 `rust_196_features.rs` 中 | 实际 1.68.0，文件已正确标注 | 无需修正 |
| `VecDeque::new` const 在 exercises 中 | 实际 1.68.0，已修正 | ✅ 已修正 |
| `assert_matches!` 部分标注为 1.96.0 | 实际 1.95 | 修正标签 |
| `core::range` 部分标注为 1.96.0 | 实际 1.95 | 修正标签 |
