...
- [FFI模式与C互操作](#ffi模式与c互操作)
  ...
  - [6. Rust 1.94 FFI 新特性](#6-rust-194-ffi-新特性)
    - [6.1 C-unwind ABI](#61-c-unwind-abi)
    - [6.2 改进的 bindgen 支持](#62-改进的-bindgen-支持)
    - [6.3 跨语言异常传播](#63-跨语言异常传播)
  - [7. 工具链使用](#7-工具链使用)
    - [7.1 bindgen](#71-bindgen)
    - [7.2 cbindgen](#72-cbindgen)
    - [7.3 调试与测试](#73-调试与测试)
  - [8. 实际案例](#8-实际案例)
    - [8.1 图像处理库包装](#81-图像处理库包装)
    - [8.2 数据库驱动实现](#82-数据库驱动实现)
    - [8.3 硬件抽象层](#83-硬件抽象层)
  - [总结](#总结)
  - [参考资源](#参考资源)
...
extern "C-unwind" fn c_unwind_convention() {} // 允许 panic 跨边界传播
...
// C++ 互操作：C-unwind 与 C++ 异常兼容
#[no_mangle]
pub extern "C-unwind" fn rust_function_for_cpp() -> i32 {
    // 这个 panic 会被转换成 C++ 异常
    if something_wrong() {
...
// 对应 C++ 代码：
// extern "C" int rust_function_for_cpp();  // 使用 C-unwind
//
// try {
//     int result = rust_function_for_cpp();
...
// 场景2：回调函数
#[no_mangle]
pub extern "C-unwind" fn register_callback(
...
// C 代码必须使用支持异常的平台 ABI
#[no_mangle]
pub extern "C-unwind" fn rust_api_entry() {
    // 复杂的 Rust 代码
    // 如果 panic，会安全地展开栈
}
```
...
// 🎉 Rust 1.94: 改进的 ABI 兼容性检查
// bindgen 可以生成编译期 ABI 兼容性验证
#[cfg(feature = "abi-check")]
const _: () = assert!(
    std::mem::size_of::<BindgenStruct>() == 4,
    "ABI mismatch: BindgenStruct size"
);
```
...
impl RustError {
    fn new(code: i32, msg: &str) -> Self {
        ...
        Self { code, message: c_msg }
    }
}

// 使用 C-unwind 传播错误
#[no_mangle]
pub extern "C-unwind" fn fallible_operation() -> i32 {
    match do_something() {
        Ok(result) => result,
        Err(e) => {
            // 将错误转换为 panic，通过 C-unwind 传播
            panic!("RustError: {:?}", RustError::new(-1, &e.to_string()));
        }
    }
}
...
fn main() {
    ...
    let bindings = bindgen::Builder::default()
        ...
        // 布局测试
        .layout_tests(true)
        // 🎉 Rust 1.94: 使用 C-unwind 生成回调
        .default_calling_convention(bindgen::CallingConvention::C)
        // 生成
        .generate()
...
/// 🎉 Rust 1.94: 使用 C-unwind ABI 导出
///
/// 这个函数可以安全地 panic，异常会传播给调用方
#[no_mangle]
pub extern "C-unwind" my_fallible_function() -> i32 {
    might_panic()
}
```
...
6. **🎉 Rust 1.94 新特性**:
   - `extern "C-unwind"` ABI 稳定，允许 panic 跨 FFI 边界安全传播
   - 改进的 bindgen 和 cbindgen 支持
   - 更好的跨语言异常传播机制
...
- [Rust 1.94 Release Notes - FFI Improvements](https://blog.rust-lang.org/)
