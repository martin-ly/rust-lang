...
>
- [2026年迁移指南 {#2026年迁移指南}](#2026年迁移指南-2026年迁移指南)
  - [📑 目录 {#目录}](#-目录-目录)
  - [迁移清单 {#迁移清单}](#迁移清单-迁移清单)
    - [1. 工具链更新 {#1-工具链更新}](#1-工具链更新-1-工具链更新)
    - [2. 代码现代化 {#2-代码现代化}](#2-代码现代化-2-代码现代化)
      - [lazy\_static → LazyLock {#lazy\_static-lazylock}](#lazy_static--lazylock-lazy_static-lazylock)
      - [async-trait → 原生async trait {#async-trait-原生async-trait}](#async-trait--原生async-trait-async-trait-原生async-trait)
      - [生成器 → gen关键字 {#生成器-gen关键字}](#生成器--gen关键字-生成器-gen关键字)
    - [3. 配置更新 {#3-配置更新}](#3-配置更新-3-配置更新)
      - [Cargo.toml {#cargotoml}](#cargotoml-cargotoml)
    - [4. CI/CD更新 {#4-cicd更新}](#4-cicd更新-4-cicd更新)
  - [Rust 1.96 迁移章节 {#rust-196-迁移章节}](#rust-196-迁移章节-rust-196-迁移章节)
    - [概述 {#概述}](#概述-概述)
    - [从 1.94 到 1.96 的完整步骤 {#从-194-到-196-的完整步骤}](#从-194-到-196-的完整步骤-从-194-到-196-的完整步骤)
      - [步骤 1: 环境准备 {#步骤-1-环境准备}](#步骤-1-环境准备-步骤-1-环境准备)
      - [步骤 2: 依赖更新 {#步骤-2-依赖更新}](#步骤-2-依赖更新-步骤-2-依赖更新)
      - [步骤 3: 代码迁移 {#步骤-3-代码迁移}](#步骤-3-代码迁移-步骤-3-代码迁移)
      - [步骤 4: 测试验证 {#步骤-4-测试验证}](#步骤-4-测试验证-步骤-4-测试验证)
      - [步骤 5: 持续集成更新 {#步骤-5-持续集成更新}](#步骤-5-持续集成更新-步骤-5-持续集成更新)
    - [常见问题 {#常见问题}](#常见问题-常见问题)
      - [Q1: if let guards 导致编译错误？ {#q1-if-let-guards-导致编译错误}](#q1-if-let-guards-导致编译错误-q1-if-let-guards-导致编译错误)
      - [Q2: RangeInclusive 类型不匹配？ {#q2-rangeinclusive-类型不匹配}](#q2-rangeinclusive-类型不匹配-q2-rangeinclusive-类型不匹配)
      - [Q3: 新的 lint 警告导致 CI 失败？ {#q3-新的-lint-警告导致-ci-失败}](#q3-新的-lint-警告导致-ci-失败-q3-新的-lint-警告导致-ci-失败)
      - [Q4: Pin 转换失败？ {#q4-pin-转换失败}](#q4-pin-转换失败-q4-pin-转换失败)
      - [Q5: 元组 coercion 不生效？ {#q5-元组-coercion-不生效}](#q5-元组-coercion-不生效-q5-元组-coercion-不生效)
    - [版本兼容性表 {#版本兼容性表}](#版本兼容性表-版本兼容性表)
    - [回滚指南 {#回滚指南}](#回滚指南-回滚指南)
  - [参考资源 {#参考资源}](#参考资源-参考资源)
  - [相关概念 {#相关概念}](#相关概念-相关概念)
...
Rust 1.95.0/1.96/1.97 引入了多项重要特性，包括 `if let guards` (1.95.0)、Range 类型改进（1.96.0）和 Rust 1.97.0 新 API。本章节指导您从 1.94.0 平滑迁移到 1.97.0。
...
```toml
[lints.rust]
# 新的 lint 规则 (Rust 1.96) {#新的-lint-规则-rust-196}
unused_tuple_struct_fields = "warn"
redundant_guards = "warn"
...
- [详细特性文档](../09_toolchain/08_rust_1_96_features.md)
- [特性速查表](../03_reference/quick_reference/20_rust_196_features_cheatsheet.md)
