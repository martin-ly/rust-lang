...
>
- [故障排查指南 {#故障排查指南}](#故障排查指南-故障排查指南)
  - [📑 目录 {#目录}](#-目录-目录)
  - [📋 概述 {#概述}](#-概述-概述)
  - [🔧 编译错误 {#编译错误}](#-编译错误-编译错误)
    - [1. 所有权错误 {#1-所有权错误}](#1-所有权错误-1-所有权错误)
    - [2. 生命周期错误 {#2-生命周期错误}](#2-生命周期错误-2-生命周期错误)
    - [3. 类型不匹配 {#3-类型不匹配}](#3-类型不匹配-3-类型不匹配)
  - [🐛 运行时错误 {#运行时错误}](#-运行时错误-运行时错误)
    - [1. Panic 错误 {#1-panic-错误}](#1-panic-错误-1-panic-错误)
    - [2. 死锁 {#2-死锁}](#2-死锁-2-死锁)
    - [3. 内存泄漏 {#3-内存泄漏}](#3-内存泄漏-3-内存泄漏)
  - [⚡ 性能问题 {#性能问题}](#-性能问题-性能问题)
    - [1. 编译时间过长 {#1-编译时间过长}](#1-编译时间过长-1-编译时间过长)
    - [2. 运行时性能问题 {#2-运行时性能问题}](#2-运行时性能问题-2-运行时性能问题)
  - [🔌 网络问题 {#网络问题}](#-网络问题-网络问题)
    - [1. 连接超时 {#1-连接超时}](#1-连接超时-1-连接超时)
    - [2. DNS 解析失败 {#2-dns-解析失败}](#2-dns-解析失败-2-dns-解析失败)
  - [🧪 测试问题 {#测试问题}](#-测试问题-测试问题)
    - [1. 测试失败 {#1-测试失败}](#1-测试失败-1-测试失败)
    - [2. 异步测试问题 {#2-异步测试问题}](#2-异步测试问题-2-异步测试问题)
  - [📚 调试技巧 {#调试技巧}](#-调试技巧-调试技巧)
    - [1. 使用 println {#1-使用-println}](#1-使用-println-1-使用-println)
    - [2. 使用 dbg! 宏 {#2-使用-dbg-宏}](#2-使用-dbg-宏-2-使用-dbg-宏)
    - [3. 使用调试器 {#3-使用调试器}](#3-使用调试器-3-使用调试器)
    - [4. 使用日志 {#4-使用日志}](#4-使用日志-4-使用日志)
  - [🔍 常见问题 FAQ {#常见问题-faq}](#-常见问题-faq-常见问题-faq)
    - [Q: 如何查看详细的编译错误？ {#q-如何查看详细的编译错误}](#q-如何查看详细的编译错误-q-如何查看详细的编译错误)
    - [Q: 如何清理编译缓存？ {#q-如何清理编译缓存}](#q-如何清理编译缓存-q-如何清理编译缓存)
    - [Q: 如何更新依赖？ {#q-如何更新依赖}](#q-如何更新依赖-q-如何更新依赖)
    - [Q: 如何查看依赖树？ {#q-如何查看依赖树}](#q-如何查看依赖树-q-如何查看依赖树)
  - [错误码快速查找 {#错误码快速查找}](#错误码快速查找-错误码快速查找)
  - [使用场景 {#使用场景}](#使用场景-使用场景)
    - [场景1: 编译错误排查 {#场景1-编译错误排查}](#场景1-编译错误排查-场景1-编译错误排查)
    - [场景2: 运行时问题诊断 {#场景2-运行时问题诊断}](#场景2-运行时问题诊断-场景2-运行时问题诊断)
    - [场景3: 性能问题优化 {#场景3-性能问题优化}](#场景3-性能问题优化-场景3-性能问题优化)
    - [场景4: 生产环境问题 {#场景4-生产环境问题}](#场景4-生产环境问题-场景4-生产环境问题)
  - [形式化链接 {#形式化链接}](#形式化链接-形式化链接)
  - [📚 相关资源 {#相关资源}](#-相关资源-相关资源)
  - [🆕 Rust 1.95+ 特性 {#rust-195-特性}](#-rust-195-特性-rust-195-特性)
    - [新特性概览 {#新特性概览}](#新特性概览-新特性概览)
    - [代码示例 {#代码示例}](#代码示例-代码示例)
  - [Rust 1.95+ 故障排查指南 {#rust-195-故障排查指南}](#rust-195-故障排查指南-rust-195-故障排查指南)
    - [LazyLock 初始化问题排查 {#lazylock-初始化问题排查}](#lazylock-初始化问题排查-lazylock-初始化问题排查)
    - [array\_windows 边界问题 {#array\_windows-边界问题}](#array_windows-边界问题-array_windows-边界问题)
    - [ControlFlow 类型推断问题 {#controlflow-类型推断问题}](#controlflow-类型推断问题-controlflow-类型推断问题)
  - [相关概念 {#相关概念}](#相关概念-相关概念)
...
**形式化引用（Reference）**：T-OW2/T-OW3 (所有权（Ownership）)、T-BR1 (借用（Borrowing）)、T-LF2 (生命周期)。
编译错误对应 [ownership_model](../12_research_notes/02_formal_methods/09_ownership_model.md)、[borrow_checker_proof](../12_research_notes/02_formal_methods/03_borrow_checker_proof.md)、lifetime_formalization。
...
常见错误码 → 本项目文档映射见 [02_error_code_mapping.md](../03_reference/03_error_code_mapping.md)。
...
2. 参考 [18_performance_tuning_guide.md](18_performance_tuning_guide.md) 优化
...
| :--- | :--- |
| :--- | :--- |
| **错误参考** | [02_error_code_mapping.md](../03_reference/03_error_code_mapping.md) |
