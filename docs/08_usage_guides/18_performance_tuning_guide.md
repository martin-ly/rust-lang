...
- [性能调优完整指南 {#性能调优完整指南}](#性能调优完整指南-性能调优完整指南)
  ...
  - [📋 概述 {#概述}](#-概述-概述)
  - [🚀 编译优化 {#编译优化}](#-编译优化-编译优化)
    - [1. Release 模式 {#1-release-模式}](#1-release-模式-1-release-模式)
    - [2. 特性标志优化 {#2-特性标志优化}](#2-特性标志优化-2-特性标志优化)
    - [3. 增量编译 {#3-增量编译}](#3-增量编译-3-增量编译)
  - [💾 内存优化 {#内存优化}](#-内存优化-内存优化)
    - [1. 预分配容量 {#1-预分配容量}](#1-预分配容量-1-预分配容量)
    - [2. 使用切片而非 Vec {#2-使用切片而非-vec}](#2-使用切片而非-vec-2-使用切片而非-vec)
    - [3. 使用 Cow 避免克隆 {#3-使用-cow-避免克隆}](#3-使用-cow-避免克隆-3-使用-cow-避免克隆)
    - [4. 使用 Box 减少栈分配 {#4-使用-box-减少栈分配}](#4-使用-box-减少栈分配-4-使用-box-减少栈分配)
    - [5. 内存池模式 {#5-内存池模式}](#5-内存池模式-5-内存池模式)
  - [⚡ 运行时优化 {#运行时优化}](#-运行时优化-运行时优化)
    - [1. 迭代器优化 {#1-迭代器优化}](#1-迭代器优化-1-迭代器优化)
    - [2. 避免不必要的克隆 {#2-避免不必要的克隆}](#2-避免不必要的克隆-2-避免不必要的克隆)
    - [3. 使用 `#[inline]` 提示 {#3-使用-inline-提示}](#3-使用-inline-提示-3-使用-inline-提示)
    - [4. 使用 `#[cold]` 标记冷路径 {#4-使用-cold-标记冷路径}](#4-使用-cold-标记冷路径-4-使用-cold-标记冷路径)
    - [5. SIMD 优化 {#5-simd-优化}](#5-simd-优化-5-simd-优化)
  - [🔄 并发优化 {#并发优化}](#-并发优化-并发优化)
    - [1. 使用 Arc 而非 Rc {#1-使用-arc-而非-rc}](#1-使用-arc-而非-rc-1-使用-arc-而非-rc)
    - [2. 减少锁竞争 {#2-减少锁竞争}](#2-减少锁竞争-2-减少锁竞争)
    - [3. 使用无锁数据结构 {#3-使用无锁数据结构}](#3-使用无锁数据结构-3-使用无锁数据结构)
    - [4. 工作窃取调度 {#4-工作窃取调度}](#4-工作窃取调度-4-工作窃取调度)
    - [5. 并行迭代器 {#5-并行迭代器}](#5-并行迭代器-5-并行迭代器)
  - [🌐 异步优化 {#异步优化}](#-异步优化-异步优化)
    - [1. 使用 select! 而非 join {#1-使用-select-而非-join}](#1-使用-select-而非-join-1-使用-select-而非-join)
    - [2. 使用有界通道 {#2-使用有界通道}](#2-使用有界通道-2-使用有界通道)
    - [3. 批量处理 {#3-批量处理}](#3-批量处理-3-批量处理)
    - [4. 避免阻塞 {#4-避免阻塞}](#4-避免阻塞-4-避免阻塞)
  - [📊 性能分析 {#性能分析}](#-性能分析-性能分析)
    - [1. 使用 criterion 基准测试 {#1-使用-criterion-基准测试}](#1-使用-criterion-基准测试-1-使用-criterion-基准测试)
    - [2. 使用 perf 分析 {#2-使用-perf-分析}](#2-使用-perf-分析-2-使用-perf-分析)
    - [3. 使用 cargo-flamegraph {#3-使用-cargo-flamegraph}](#3-使用-cargo-flamegraph-3-使用-cargo-flamegraph)
    - [4. 使用 valgrind (Linux) {#4-使用-valgrind-linux}](#4-使用-valgrind-linux-4-使用-valgrind-linux)
  - [🎯 优化策略 {#优化策略}](#-优化策略-优化策略)
    - [1. 测量优先 {#1-测量优先}](#1-测量优先-1-测量优先)
    - [2. 热点分析 {#2-热点分析}](#2-热点分析-2-热点分析)
    - [3. 渐进优化 {#3-渐进优化}](#3-渐进优化-3-渐进优化)
  - [使用场景 {#使用场景}](#使用场景-使用场景)
    - [场景1: 编译时优化 {#场景1-编译时优化}](#场景1-编译时优化-场景1-编译时优化)
    - [场景2: 内存密集型应用优化 {#场景2-内存密集型应用优化}](#场景2-内存密集型应用优化-场景2-内存密集型应用优化)
    - [场景3: 高并发系统优化 {#场景3-高并发系统优化}](#场景3-高并发系统优化-场景3-高并发系统优化)
    - [场景4: 性能瓶颈定位 {#场景4-性能瓶颈定位}](#场景4-性能瓶颈定位-场景4-性能瓶颈定位)
  - [形式化链接 {#形式化链接}](#形式化链接-形式化链接)
  - [📚 相关资源 {#相关资源}](#-相关资源-相关资源)
  - [🆕 Rust 1.95+ 特性 {#rust-195-特性}](#-rust-195-特性-rust-195-特性)
    - [新特性深度解析 {#新特性深度解析}](#新特性深度解析-新特性深度解析)
      - [1. `array_windows()` - 零开销滑动窗口迭代 {#1-array\_windows---零开销滑动窗口迭代}](#1-array_windows---零开销滑动窗口迭代-1-array_windows---零开销滑动窗口迭代)
      - [2. `ControlFlow<B, C>` - 流控制的零成本抽象 {#2-controlflowb-c---流控制的零成本抽象}](#2-controlflowb-c---流控制的零成本抽象-2-controlflowb-c---流控制的零成本抽象)
      - [3. `LazyLock/LazyCell` 增强 {#3-lazylocklazycell-增强}](#3-lazylocklazycell-增强-3-lazylocklazycell-增强)
      - [4. `f32/f64::consts` 新增数学常量 {#4-f32f64consts-新增数学常量}](#4-f32f64consts-新增数学常量-4-f32f64consts-新增数学常量)
  - [相关概念 {#相关概念}](#相关概念-相关概念)
...
**形式化引用（Reference）**：T-OW3 (内存安全（Memory Safety）)、[ownership_model](../12_research_notes/02_formal_methods/09_ownership_model.md)。
内存优化与所有权（Ownership）/借用（Borrowing）规则一致；并发优化见 SEND-T1、SYNC-T1。
...
```bash
# Linux {#linux}
# 编译带调试信息的 release 版本 {#编译带调试信息的-release-版本}
[profile.release]
debug = true
...
```bash
# 安装 cargo-valgrind {#安装-cargo-valgrind}
cargo install cargo-valgrind

# 运行分析 {#运行分析}
cargo valgrind --bin my_app

# 使用 callgrind 进行详细分析 {#使用-callgrind-进行详细分析}
...
```toml
# 应用 Release 模式优化 {#应用-release-模式优化}
[profile.release]
opt-level = 3
lto = true
codegen-units = 1
...
| 链接类型 | 目标文档 |
| :--- | :--- |
| **核心模块（Module）** | C01 所有权（Ownership） |
| :--- | :--- |
| :--- | :--- |
| :--- | :--- |
| **相关指南** | [17_performance_testing_report.md](17_performance_testing_report.md) |
| :--- | :--- |
| :--- | :--- |
...
- C06 异步（Async）
- C08 算法
- [17_performance_testing_report.md](17_performance_testing_report.md)
...
- 缓存友好：连续内存访问模式
- 编译期优化：`N` 为 const 泛型（Generics），编译器可展开循环
...
- `get_mut()`: 可变访问（单线程场景）
- `force_mut()`: 强制初始化并获取可变引用（Mutable Reference）
