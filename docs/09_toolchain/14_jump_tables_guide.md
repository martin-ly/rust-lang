...
> **分级**: [A]
> **Bloom 层级**: L2
> **稳定版本**: Rust 1.93.0（原 `-Zno-jump-tables`），1.97.1+ 持续可用
> **适用版本**: 1.97.1+ (MSRV 1.97.1)
>
> **受众**: [初学者] / [进阶]
> **内容分级**: [综述级]
...
- [`-C jump-tables=bool` 编译器优化指南 {#c-jump-tablesbool-编译器优化指南}](#-c-jump-tablesbool-编译器优化指南-c-jump-tablesbool-编译器优化指南)
  - [目录 {#目录}](#目录-目录)
  - [一、什么是跳转表 (Jump Tables) {#一什么是跳转表-jump-tables}](#一什么是跳转表-jump-tables-一什么是跳转表-jump-tables)
    - [工作原理 {#工作原理}](#工作原理-工作原理)
    - [跳转表的优缺点 {#跳转表的优缺点}](#跳转表的优缺点-跳转表的优缺点)
  - [二、`-C jump-tables=bool` 语法 {#二-c-jump-tablesbool-语法}](#二-c-jump-tablesbool-语法-二-c-jump-tablesbool-语法)
    - [配置位置 {#配置位置}](#配置位置-配置位置)
  - [三、使用场景 {#三使用场景}](#三使用场景-三使用场景)
    - [场景 1：Spectre 缓解 {#场景-1spectre-缓解}](#场景-1spectre-缓解-场景-1spectre-缓解)
    - [场景 2：分支预测优化 {#场景-2分支预测优化}](#场景-2分支预测优化-场景-2分支预测优化)
    - [场景 3：嵌入式系统 {#场景-3嵌入式系统}](#场景-3嵌入式系统-场景-3嵌入式系统)
  - [四、对性能的影响 {#四对性能的影响}](#四对性能的影响-四对性能的影响)
    - [基准测试建议 {#基准测试建议}](#基准测试建议-基准测试建议)
    - [典型结果 {#典型结果}](#典型结果-典型结果)
  - [五、常见配置模式 {#五常见配置模式}](#五常见配置模式-五常见配置模式)
    - [模式 1：安全关键构建 {#模式-1安全关键构建}](#模式-1安全关键构建-模式-1安全关键构建)
    - [模式 2：与 PGO 配合 {#模式-2与-pgo-配合}](#模式-2与-pgo-配合-模式-2与-pgo-配合)
    - [模式 3：条件编译中的选择 {#模式-3条件编译中的选择}](#模式-3条件编译中的选择-模式-3条件编译中的选择)
  - [六、参考 {#六参考}](#六参考-六参考)
    - [相关文档 {#相关文档}](#相关文档-相关文档)
...
```toml
# .cargo/config.toml (嵌入式) {#cargoconfigtoml-嵌入式}
...
```bash
# 1. 训练阶段（默认配置收集 profile） {#1-训练阶段默认配置收集-profile}
cargo build --release
...
- [Rust 1.93 特性速查表](../03_reference/quick_reference/17_rust_190_to_193_features_cheatsheet.md)
