...
> **权威来源（Canonical）**: 本文件为控制流与函数速查卡（速查，独特内容）；通用 Rust 概念解释请以 concept 权威页为准：[`concept L1 控制流`](../../../concept/01_foundation/04_control_flow/01_control_flow.md) · [`concept L1 函数`](../../../concept/01_foundation/07_modules_and_items/02_functions.md)
>
...
- [🔄 Rust 控制流与函数速查卡 {#rust-控制流与函数速查卡}](#-rust-控制流与函数速查卡-rust-控制流与函数速查卡)
  ...
  - [🎯 条件语句 {#条件语句}](#-条件语句-条件语句)
    ...
    - [match 表达式 {#match-表达式}](#match-表达式-match-表达式)
    - [if let 表达式 {#if-let-表达式}](#if-let-表达式-if-let-表达式)
    - [let-else 语句 (Rust 1.65+) {#let-else-语句-rust-165}](#let-else-语句-rust-165-let-else-语句-rust-165)
  - [🔁 循环结构 {#循环结构}](#-循环结构-循环结构)
    - [loop 循环 {#loop-循环}](#loop-循环-loop-循环)
    - [while 循环 {#while-循环}](#while-循环-while-循环)
    - [for 循环 {#for-循环}](#for-循环-for-循环)
    - [循环控制 {#循环控制}](#循环控制-循环控制)
  - [🎭 模式匹配 {#模式匹配}](#-模式匹配-模式匹配)
    - [基本模式 {#基本模式}](#基本模式-基本模式)
    - [解构模式 {#解构模式}](#解构模式-解构模式)
  ...
  - [📝 函数定义 {#函数定义}](#-函数定义-函数定义)
    - [基本函数 {#基本函数}](#基本函数-基本函数)
    - [函数参数 {#函数参数}](#函数参数-函数参数)
    - [函数返回值 {#函数返回值}](#函数返回值-函数返回值)
    - [函数指针 {#函数指针}](#函数指针-函数指针)
  - [🔒 闭包 {#闭包}](#-闭包-闭包)
    - [基本闭包 {#基本闭包}](#基本闭包-基本闭包)
    - [闭包捕获 {#闭包捕获}](#闭包捕获-闭包捕获)
    - [闭包类型 {#闭包类型}](#闭包类型-闭包类型)
    - [闭包作为参数 {#闭包作为参数}](#闭包作为参数-闭包作为参数)
    - [闭包作为返回值 {#闭包作为返回值}](#闭包作为返回值-闭包作为返回值)
  - [🎯 常用模式 {#常用模式}](#-常用模式-常用模式)
    - [早期返回 {#早期返回}](#早期返回-早期返回)
    - [链式调用 {#链式调用}](#链式调用-链式调用)
    - [模式匹配与解构 {#模式匹配与解构}](#模式匹配与解构-模式匹配与解构)
    - [函数式编程 {#函数式编程}](#函数式编程-函数式编程)
    - [递归函数 {#递归函数}](#递归函数-递归函数)
  - [🚫 反例速查 {#反例速查}](#-反例速查-反例速查)
    - [反例 1: match 未覆盖所有分支 {#反例-1-match-未覆盖所有分支}](#反例-1-match-未覆盖所有分支-反例-1-match-未覆盖所有分支)
    - [反例 2: 闭包捕获可变引用导致冲突 {#反例-2-闭包捕获可变引用导致冲突}](#反例-2-闭包捕获可变引用导致冲突-反例-2-闭包捕获可变引用导致冲突)
  - [📚 相关文档 {#相关文档}](#-相关文档-相关文档)
  ...
  - [💡 使用场景 {#使用场景}](#-使用场景-使用场景)
    - [场景 1: 命令行参数解析 {#场景-1-命令行参数解析}](#场景-1-命令行参数解析-场景-1-命令行参数解析)
    - [场景 2: 递归下降解析器 {#场景-2-递归下降解析器}](#场景-2-递归下降解析器-场景-2-递归下降解析器)
    - [场景 3: 自定义迭代器 {#场景-3-自定义迭代器}](#场景-3-自定义迭代器-场景-3-自定义迭代器)
  - [⚠️ 边界情况 {#边界情况}](#️-边界情况-边界情况)
    - [边界 1: 闭包捕获陷阱 {#边界-1-闭包捕获陷阱}](#边界-1-闭包捕获陷阱-边界-1-闭包捕获陷阱)
    - [边界 2: 递归深度限制 {#边界-2-递归深度限制}](#边界-2-递归深度限制-边界-2-递归深度限制)
    - [边界 3: 模式匹配穷尽性 {#边界-3-模式匹配穷尽性}](#边界-3-模式匹配穷尽性-边界-3-模式匹配穷尽性)
...
- [控制流研究笔记](../../12_research_notes/README.md)

### 相关速查卡 {#相关速查卡}
>
> **[来源: [Rust Standard Library](https://doc.rust-lang.org/std/)]**

- [错误处理（Error Handling）速查卡](10_error_handling_cheatsheet.md) - 错误处理模式
- [类型系统（Type System）速查卡](27_type_system.md) - 类型与函数
- [集合与迭代器速查卡](08_collections_iterators_cheatsheet.md) - 迭代器与循环
- [所有权（Ownership）系统速查卡](14_ownership_cheatsheet.md) - 所有权与闭包
...
- [借用检查器证明](../../12_research_notes/02_formal_methods/03_borrow_checker_proof.md) — 控制流相关的借用规则证明
- [Send/Sync 形式化](../../12_research_notes/02_formal_methods/12_send_sync_formalization.md) — 闭包在多线程环境下的安全性
