...
> **权威来源（Canonical）**: 本文件为 **TRPL 第三版与 `concept/` 知识体系的对齐差异分析**（独特比较/评估内容）；各 Rust 概念的深度解释以 `concept/` 权威页为准。逐章映射见 [`docs/01_learning/learning_mvp_path.md`](01_learning/learning_mvp_path.md)（映射权威页）与 [`concept/00_meta/trpl_3rd_ed_mapping.md`](../concept/00_meta/trpl_3rd_ed_mapping.md)（重定向 stub）。
>
