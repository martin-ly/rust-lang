...
>
- [依赖关系图 {#依赖关系图}](#依赖关系图-依赖关系图)
  - [📑 目录 {#目录}](#-目录-目录)
  - [Crate 依赖图 {#crate-依赖图}](#crate-依赖图-crate-依赖图)
    - [视觉表示 {#视觉表示}](#视觉表示-视觉表示)
  - [依赖矩阵 {#依赖矩阵}](#依赖矩阵-依赖矩阵)
  - [外部依赖统计 {#外部依赖统计}](#外部依赖统计-外部依赖统计)
    - [核心运行时 {#核心运行时}](#核心运行时-核心运行时)
    - [序列化 {#序列化}](#序列化-序列化)
    - [网络 {#网络}](#网络-网络)
    - [WebAssembly {#webassembly}](#webassembly-webassembly)
    - [宏开发 {#宏开发}](#宏开发-宏开发)
  - [依赖类型说明 {#依赖类型说明}](#依赖类型说明-依赖类型说明)
    - [编译时依赖 {#编译时依赖}](#编译时依赖-编译时依赖)
    - [运行时依赖 {#运行时依赖}](#运行时依赖-运行时依赖)
    - [可选依赖 {#可选依赖}](#可选依赖-可选依赖)
  - [依赖更新策略 {#依赖更新策略}](#依赖更新策略-依赖更新策略)
    - [自动更新 {#自动更新}](#自动更新-自动更新)
    - [手动审查 {#手动审查}](#手动审查-手动审查)
    - [锁定策略 {#锁定策略}](#锁定策略-锁定策略)
  - [相关概念 {#相关概念}](#相关概念-相关概念)
...
| 包 | 版本 | 用途 | 使用 Crate |
|----|------|------|-----------|
| tokio | 1.51.0 | 异步（Async）运行时 | c01-c07, c10, c12 |
