...
>
- [Polonius 借用检查器跟踪报告 {#polonius-借用检查器跟踪报告}](#polonius-借用检查器跟踪报告-polonius-借用检查器跟踪报告)
  - [📑 目录 {#目录}](#-目录-目录)
  - [1. 项目背景与动机 {#1-项目背景与动机}](#1-项目背景与动机-1-项目背景与动机)
    - [1.1 为什么需要 Polonius？ {#11-为什么需要-polonius}](#11-为什么需要-polonius-11-为什么需要-polonius)
  - [2. NLL 的局限 {#2-nll-的局限}](#2-nll-的局限-2-nll-的局限)
    - [2.1 基于位置的借用模型 {#21-基于位置的借用模型}](#21-基于位置的借用模型-21-基于位置的借用模型)
    - [2.2 典型拒绝案例 {#22-典型拒绝案例}](#22-典型拒绝案例-22-典型拒绝案例)
    - [2.3 NLL 的架构瓶颈 {#23-nll-的架构瓶颈}](#23-nll-的架构瓶颈-23-nll-的架构瓶颈)
  - [3. Polonius 新算法原理 {#3-polonius-新算法原理}](#3-polonius-新算法原理-3-polonius-新算法原理)
    - [3.1 核心思想：基于 Datalog 的数据流分析 {#31-核心思想基于-datalog-的数据流分析}](#31-核心思想基于-datalog-的数据流分析-31-核心思想基于-datalog-的数据流分析)
    - [3.2 起源 (Origin) 与 贷款 (Loan) 的精确追踪 {#32-起源-origin-与-贷款-loan-的精确追踪}](#32-起源-origin-与-贷款-loan-的精确追踪-32-起源-origin-与-贷款-loan-的精确追踪)
    - [3.3 三维度分析 {#33-三维度分析}](#33-三维度分析-33-三维度分析)
    - [3.4 路径敏感性的突破 {#34-路径敏感性的突破}](#34-路径敏感性的突破-34-路径敏感性的突破)
  - [4. 与当前 Borrow Checker 的对比示例 {#4-与当前-borrow-checker-的对比示例}](#4-与当前-borrow-checker-的对比示例-4-与当前-borrow-checker-的对比示例)
    - [4.1 示例矩阵 {#41-示例矩阵}](#41-示例矩阵-41-示例矩阵)
    - [4.2 详细代码对比 {#42-详细代码对比}](#42-详细代码对比-42-详细代码对比)
      - [示例 A：枚举（Enum）变体借用 (当前编译器拒绝) {#示例-a枚举变体借用-当前编译器拒绝}](#示例-a枚举变体借用-当前编译器拒绝-示例-a枚举变体借用-当前编译器拒绝)
      - [示例 B：循环内精确索引借用 {#示例-b循环内精确索引借用}](#示例-b循环内精确索引借用-示例-b循环内精确索引借用)
  - [5. 预计稳定化时间线跟踪 {#5-预计稳定化时间线跟踪}](#5-预计稳定化时间线跟踪-5-预计稳定化时间线跟踪)
    - [5.1 里程碑时间线 {#51-里程碑时间线}](#51-里程碑时间线-51-里程碑时间线)
    - [5.2 当前状态 (2026-04-24) {#52-当前状态-2026-04-24}](#52-当前状态-2026-04-24-52-当前状态-2026-04-24)
    - [5.3 启用 Polonius (nightly) {#53-启用-polonius-nightly}](#53-启用-polonius-nightly-53-启用-polonius-nightly)
  - [6. 对本项目的影响 {#6-对本项目的影响}](#6-对本项目的影响-6-对本项目的影响)
    - [6.1 教学价值 {#61-教学价值}](#61-教学价值-61-教学价值)
    - [6.2 代码实践建议 {#62-代码实践建议}](#62-代码实践建议-62-代码实践建议)
  - [7. 参考文献 {#7-参考文献}](#7-参考文献-7-参考文献)
  - [8. 附录：相关 Issue 跟踪 {#8-附录相关-issue-跟踪}](#8-附录相关-issue-跟踪-8-附录相关-issue-跟踪)
  - [相关概念 {#相关概念}](#相关概念-相关概念)
...
Rust 的借用检查器是其内存安全（Memory Safety）保证的核心。
...
```bash
# 切换到 nightly 工具链 {#切换到-nightly-工具链}
rustup default nightly

# 使用 Polonius 编译 {#使用-polonius-编译}
...
Polonius 代表了 Rust 类型系统（Type System）理论的前沿进展。在本知识工程中：
...
- [Rust 版本跟踪 (concept)](../../concept/07_future/00_version_tracking/05_rust_version_tracking.md) — Project Goals 2026 全局状态跟踪
