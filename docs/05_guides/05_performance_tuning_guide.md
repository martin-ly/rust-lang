...
- [性能调优完整指南 {#性能调优完整指南}](#性能调优完整指南-性能调优完整指南)
  ...
  - [💾 内存优化 {#内存优化}](#-内存优化-内存优化)
    ...
    - [2. 使用切片（Slice）而非 Vec {#2-使用切片而非-vec}](#2-使用切片而非-vec-2-使用切片而非-vec)
  ...
  - [⚡ 运行时（Runtime）优化 {#运行时优化}](#-运行时优化-运行时优化)
    - [1. 迭代器（Iterator）优化 {#1-迭代器优化}](#1-迭代器优化-1-迭代器优化)
  ...
  - [🆕 Rust 1.95+ 特性 {#rust-195-特性}](#-rust-195-特性-rust-195-特性)
    - [新特性深度解析 {#新特性深度解析}](#新特性深度解析-新特性深度解析)
      ...
      - [2. `ControlFlow<B, C>` - 流控制的零成本抽象（Zero-Cost Abstraction） {#2-controlflowb-c---流控制的零成本抽象}](#2-controlflowb-c---流控制的零成本抽象-2-controlflowb-c---流控制的零成本抽象)
...
**形式化引用（Reference）**：T-OW3 (内存安全（Memory Safety）)、[ownership_model](../research_notes/formal_methods/10_ownership_model.md)。
