...
>
- [形式化验证整合指南 {#形式化验证整合指南}](#形式化验证整合指南-形式化验证整合指南)
  - [📑 目录 {#目录}](#-目录-目录)
  - [🎯 概述 {#概述}](#-概述-概述)
  - [🧪 Miri - 内存安全验证 {#miri---内存安全验证}](#-miri---内存安全验证-miri---内存安全验证)
    - [Tree Borrows 模式 {#tree-borrows-模式}](#tree-borrows-模式-tree-borrows-模式)
    - [代码示例 {#代码示例}](#代码示例-代码示例)
    - [CI/CD 集成 {#cicd-集成}](#cicd-集成-cicd-集成)
  - [🔍 Kani - 模型检查 {#kani---模型检查}](#-kani---模型检查-kani---模型检查)
    - [基础验证 {#基础验证}](#基础验证-基础验证)
    - [高级特性 {#高级特性}](#高级特性-高级特性)
  - [📐 Prusti - 契约编程 {#prusti---契约编程}](#-prusti---契约编程-prusti---契约编程)
    - [前置/后置条件 {#前置后置条件}](#前置后置条件-前置后置条件)
    - [循环不变量 {#循环不变量}](#循环不变量-循环不变量)
  - [🎓 工具选择决策树 {#工具选择决策树}](#-工具选择决策树-工具选择决策树)
  - [🔗 综合示例 {#综合示例}](#-综合示例-综合示例)
    - [完整的验证套件 {#完整的验证套件}](#完整的验证套件-完整的验证套件)
  - [📊 验证覆盖率目标 {#验证覆盖率目标}](#-验证覆盖率目标-验证覆盖率目标)
  - [相关概念 {#相关概念}](#相关概念-相关概念)
...
本指南介绍如何在 Rust 项目中整合形式化验证工具，确保代码的内存安全、功能正确性和并发安全（Concurrency Safety）。
...
| 模块（Module） | Miri | Kani | Prusti | 目标覆盖率 |
|------|------|------|--------|-----------|
| C01 所有权（Ownership） | ✅ | ⚠️ | ⚠️ | 80% |
| C02 类型系统（Type System） | ✅ | ✅ | ⚠️ | 70% |
| C04 泛型（Generics） | ✅ | ✅ | ⚠️ | 60% |
| C05 并发 | ✅ | ⚠️ | ❌ | 50% |
| C06 异步（Async） | ✅ | ❌ | ❌ | 40% |
| C08 算法 | ✅ | ✅ | ✅ | 90% |
