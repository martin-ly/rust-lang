...
- [5. **索引操作符**](#5-索引操作符)
  - [示例代码](#示例代码)
    - [1. **实现 `Add` 和 `Sub`**](#1-实现-add-和-sub)
    - [2. **实现 `BitAnd` 和 `BitOr`**](#2-实现-bitand-和-bitor)
    - [3. **实现 `AddAssign` 和 `SubAssign`**](#3-实现-addassign-和-subassign)
    - [4. **实现 `Index` 和 `IndexMut`**](#4-实现-index-和-indexmut)
  - [总结](#总结)
  - [Rust 中 `Neg` 和 `Not` 的用法示例](#rust-中-neg-和-not-的用法示例)
    - [1. `Neg`（一元负号）](#1-neg一元负号)
    - [2. `Not`（逻辑非）](#2-not逻辑非)
  - [*总结*](#总结)
  - [`Neg` 可用的数据类型](#neg-可用的数据类型)
  - [`Not` 可用的数据类型](#not-可用的数据类型)
  - [**总结**](#总结)
