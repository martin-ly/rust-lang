...
- [Rust 边界条件与特例示例 {#rust-边界条件与特例示例}](#rust-边界条件与特例示例-rust-边界条件与特例示例)
  - [目录 {#目录}](#目录-目录)
  - [集合边界 {#集合边界}](#集合边界-集合边界)
    - [空 Vec 与零长度 {#空-vec-与零长度}](#空-vec-与零长度-空-vec-与零长度)
    - [空 HashMap / BTreeMap {#空-hashmap-btreemap}](#空-hashmap--btreemap-空-hashmap-btreemap)
    - [空切片 {#空切片}](#空切片-空切片)
  - [算法特例 {#算法特例}](#算法特例-算法特例)
    - [空输入的排序与搜索 {#空输入的排序与搜索}](#空输入的排序与搜索-空输入的排序与搜索)
    - [单元素与双元素 {#单元素与双元素}](#单元素与双元素-单元素与双元素)
  - [并发特例 {#并发特例}](#并发特例-并发特例)
    - [零个线程 / 空任务列表 {#零个线程-空任务列表}](#零个线程--空任务列表-零个线程-空任务列表)
    - [通道已关闭 {#通道已关闭}](#通道已关闭-通道已关闭)
    - [Mutex poison {#mutex-poison}](#mutex-poison-mutex-poison)
  - [数值溢出 {#数值溢出}](#数值溢出-数值溢出)
    - [整数溢出（debug 下 panic） {#整数溢出debug-下-panic}](#整数溢出debug-下-panic-整数溢出debug-下-panic)
    - [除零 {#除零}](#除零-除零)
  - [字符串与切片 {#字符串与切片}](#字符串与切片-字符串与切片)
    - [空字符串 {#空字符串}](#空字符串-空字符串)
    - [字节边界上的字符切分 {#字节边界上的字符切分}](#字节边界上的字符切分-字节边界上的字符切分)
    - [零长度范围 {#零长度范围}](#零长度范围-零长度范围)
  - [unsafe 与 FFI {#unsafe-与-ffi}](#unsafe-与-ffi-unsafe-与-ffi)
    - [空指针解引用 {#空指针解引用}](#空指针解引用-空指针解引用)
    - [FFI 边界：C 传入空指针 {#ffi-边界c-传入空指针}](#ffi-边界c-传入空指针-ffi-边界c-传入空指针)
    - [悬垂引用典型反例 {#悬垂引用典型反例}](#悬垂引用典型反例-悬垂引用典型反例)
  - [WASM 特例 {#wasm-特例}](#wasm-特例-wasm-特例)
    - [无 std 环境 {#无-std-环境}](#无-std-环境-无-std-环境)
    - [阻塞 API 在 WASM {#阻塞-api-在-wasm}](#阻塞-api-在-wasm-阻塞-api-在-wasm)
  - [panic 边界 {#panic-边界}](#panic-边界-panic-边界)
    - [panic 与 unwrap {#panic-与-unwrap}](#panic-与-unwrap-panic-与-unwrap)
    - [断言边界 {#断言边界}](#断言边界-断言边界)
  - [空 Future 与异步 {#空-future-与异步}](#空-future-与异步-空-future-与异步)
    - [空 select {#空-select}](#空-select-空-select)
    - [已完成 Future {#已完成-future}](#已完成-future-已完成-future)
  - [Rust 1.93 行为变更特例 {#rust-193-行为变更特例}](#rust-193-行为变更特例-rust-193-行为变更特例)
    - [BTreeMap::append 行为变更 {#btreemapappend-行为变更}](#btreemapappend-行为变更-btreemapappend-行为变更)
    - [Copy specialization 移除 {#copy-specialization-移除}](#copy-specialization-移除-copy-specialization-移除)
    - [vec::IntoIter 与 RefUnwindSafe {#vecintoiter-与-refunwindsafe}](#vecintoiter-与-refunwindsafe-vecintoiter-与-refunwindsafe)
  - [更多边界情况代码示例 {#更多边界情况代码示例}](#更多边界情况代码示例-更多边界情况代码示例)
    - [所有权边界 {#所有权边界}](#所有权边界-所有权边界)
      - [部分移动后使用 {#部分移动后使用}](#部分移动后使用-部分移动后使用)
      - [Copy 类型的隐式复制 {#copy-类型的隐式复制}](#copy-类型的隐式复制-copy-类型的隐式复制)
    - [生命周期边界 {#生命周期边界}](#生命周期边界-生命周期边界)
      - [生命周期省略规则边界 {#生命周期省略规则边界}](#生命周期省略规则边界-生命周期省略规则边界)
      - [NLL (Non-Lexical Lifetimes) 边界 {#nll-non-lexical-lifetimes-边界}](#nll-non-lexical-lifetimes-边界-nll-non-lexical-lifetimes-边界)
    - [泛型边界 {#泛型边界}](#泛型边界-泛型边界)
      - [递归类型大小边界 {#递归类型大小边界}](#递归类型大小边界-递归类型大小边界)
      - [零大小类型 (ZST) {#零大小类型-zst}](#零大小类型-zst-零大小类型-zst)
    - [模式匹配边界 {#模式匹配边界}](#模式匹配边界-模式匹配边界)
      - [穷尽性检查边界 {#穷尽性检查边界}](#穷尽性检查边界-穷尽性检查边界)
    - [并发边界 {#并发边界-1}](#并发边界-并发边界-1)
      - [死锁边界 {#死锁边界}](#死锁边界-死锁边界)
      - [Send/Sync 自动派生边界 {#sendsync-自动派生边界}](#sendsync-自动派生边界-sendsync-自动派生边界)
    - [unsafe 边界 {#unsafe-边界-1}](#unsafe-边界-unsafe-边界-1)
      - [裸指针解引用边界 {#裸指针解引用边界}](#裸指针解引用边界-裸指针解引用边界)
      - [未对齐指针边界 {#未对齐指针边界}](#未对齐指针边界-未对齐指针边界)
    - [迭代器边界 {#迭代器边界}](#迭代器边界-迭代器边界)
      - [迭代器失效边界 {#迭代器失效边界}](#迭代器失效边界-迭代器失效边界)
  - [🔗 形式化边界分析 {#形式化边界分析}](#-形式化边界分析-形式化边界分析)
    - [所有权与借用边界 {#所有权与借用边界}](#所有权与借用边界-所有权与借用边界)
    - [类型系统边界 {#类型系统边界}](#类型系统边界-类型系统边界)
    - [并发边界 {#并发边界-1}](#并发边界-并发边界-1-1)
    - [unsafe 边界 {#unsafe-边界-1}](#unsafe-边界-unsafe-边界-1-1)
  - [相关文档 {#相关文档}](#相关文档-相关文档)
    - [速查卡 {#速查卡}](#速查卡-速查卡)
...
| 边界情况 | 形式化规则 | 相关文档 |
| :--- | :--- | :--- |
| 裸指针 | $\text{deref}(p)$ 合法仅当 $\text{nonnull}(p)$ | [borrow_checker_proof](../research_notes/formal_methods/10_borrow_checker_proof.md) |
| 未对齐访问 | 需使用 `read_unaligned` | [Rust Reference](https://doc.rust-lang.org/reference/behavior-considered-undefined.html) |
| FFI 边界 | `extern` 函数类型布局一致 | [borrow_checker_proof](../research_notes/formal_methods/10_borrow_checker_proof.md) |
