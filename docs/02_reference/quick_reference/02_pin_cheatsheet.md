...
> **权威来源（Canonical）**: 本文件为Pin 与自引用结构速查卡（速查，独特内容）；通用 Rust 概念解释请以 concept 权威页为准：[`concept L3 Pin/Unpin`](../../../concept/03_advanced/01_async/06_pin_unpin.md)
>
> 根据 AGENTS.md §2 Canonical 规则：本文仅保留本文独特内容（Pin 代码模式与使用决策树速查），不重复 concept/ 中的概念定义、规则与定理推导。
