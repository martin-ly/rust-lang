...
> **权威来源（Canonical）**: 本文件为字符串与格式化速查卡（速查，独特内容）；通用 Rust 概念解释请以 concept 权威页为准：[`concept L1 字符串`](../../../concept/01_foundation/06_strings_and_text/01_strings_and_text.md) · [`concept L1 格式化`](../../../concept/01_foundation/06_strings_and_text/03_formatting_and_display.md)
>
...
- [📝 Rust 字符串与格式化速查卡 {#rust-字符串与格式化速查卡}](#-rust-字符串与格式化速查卡-rust-字符串与格式化速查卡)
  ...
  - [🆕 字符串创建 {#字符串创建}](#-字符串创建-字符串创建)
    - [基本创建 {#基本创建}](#基本创建-基本创建)
    - [从其他类型创建 {#从其他类型创建}](#从其他类型创建-从其他类型创建)
  - [✂️ 字符串操作 {#字符串操作}](#️-字符串操作-字符串操作)
    - [追加内容 {#追加内容}](#追加内容-追加内容)
    - [删除内容 {#删除内容}](#删除内容-删除内容)
    - [替换 {#替换}](#替换-替换)
    - [查找和检查 {#查找和检查}](#查找和检查-查找和检查)
    - [分割 {#分割}](#分割-分割)
    - [修剪 {#修剪}](#修剪-修剪)
  - [🔄 字符串转换 {#字符串转换}](#-字符串转换-字符串转换)
    - [String ↔ \&str {#string-str}](#string--str-string-str)
    - [大小写转换 {#大小写转换}](#大小写转换-大小写转换)
    - [数字转换 {#数字转换}](#数字转换-数字转换)
    - [字符和字节 {#字符和字节}](#字符和字节-字符和字节)
  - [🖨️ 格式化输出 {#格式化输出}](#️-格式化输出-格式化输出)
    - [基本宏 {#基本宏}](#基本宏-基本宏)
    - [format! 宏 {#format-宏}](#format-宏-format-宏)
    - [write! 和 writeln {#write-和-writeln}](#write-和-writeln-write-和-writeln)
  - [🎨 格式化选项 {#格式化选项}](#-格式化选项-格式化选项)
    - [对齐和填充 {#对齐和填充}](#对齐和填充-对齐和填充)
    - [数字格式化 {#数字格式化}](#数字格式化-数字格式化)
    - [浮点数格式化 {#浮点数格式化}](#浮点数格式化-浮点数格式化)
    - [字符串格式化 {#字符串格式化}](#字符串格式化-字符串格式化)
    - [指针和引用 {#指针和引用}](#指针和引用-指针和引用)
  - [🎯 常用模式 {#常用模式}](#-常用模式-常用模式)
    - [字符串拼接 {#字符串拼接}](#字符串拼接-字符串拼接)
    - [字符串模板 {#字符串模板}](#字符串模板-字符串模板)
    - [错误消息格式化 {#错误消息格式化}](#错误消息格式化-错误消息格式化)
    - [表格格式化 {#表格格式化}](#表格格式化-表格格式化)
    - [进度条格式化 {#进度条格式化}](#进度条格式化-进度条格式化)
  - [💡 代码示例 {#代码示例}](#-代码示例-代码示例)
    - [示例 1: 实现 Display trait {#示例-1-实现-display-trait}](#示例-1-实现-display-trait-示例-1-实现-display-trait)
    - [示例 2: 自定义格式化参数 {#示例-2-自定义格式化参数}](#示例-2-自定义格式化参数-示例-2-自定义格式化参数)
    - [示例 3: 安全的字符串切片 {#示例-3-安全的字符串切片}](#示例-3-安全的字符串切片-示例-3-安全的字符串切片)
    - [示例 4: 字符串模板引擎 {#示例-4-字符串模板引擎}](#示例-4-字符串模板引擎-示例-4-字符串模板引擎)
    - [示例 5: CSV 解析器 {#示例-5-csv-解析器}](#示例-5-csv-解析器-示例-5-csv-解析器)
  - [🎯 使用场景 {#使用场景}](#-使用场景-使用场景)
    - [场景: 日志格式化系统 {#场景-日志格式化系统}](#场景-日志格式化系统-场景-日志格式化系统)
  - [🚫 反例速查 {#反例速查}](#-反例速查-反例速查)
    - [反例 1: 在循环中拼接字符串 {#反例-1-在循环中拼接字符串}](#反例-1-在循环中拼接字符串-反例-1-在循环中拼接字符串)
    - [反例 2: 按字节索引切片 UTF-8 {#反例-2-按字节索引切片-utf-8}](#反例-2-按字节索引切片-utf-8-反例-2-按字节索引切片-utf-8)
    - [反例 3: 错误处理从字节到字符串的转换 {#反例-3-错误处理从字节到字符串的转换}](#反例-3-错误处理从字节到字符串的转换-反例-3-错误处理从字节到字符串的转换)
    - [反例 4: format!  panic 导致的拒绝服务 {#反例-4-format-panic-导致的拒绝服务}](#反例-4-format--panic-导致的拒绝服务-反例-4-format-panic-导致的拒绝服务)
    - [反例 5: 在热路径上频繁分配字符串 {#反例-5-在热路径上频繁分配字符串}](#反例-5-在热路径上频繁分配字符串-反例-5-在热路径上频繁分配字符串)
  - [📚 相关文档 {#相关文档}](#-相关文档-相关文档)
