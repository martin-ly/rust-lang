...
- **借用（Borrowing）**：通过借用访问值而不取得所有权。
- **可变借用（Mutable Borrow）**：对值的唯一可变借用，允许修改。
- **不可变借用（Immutable Borrow）**：对值的只读借用，可有多个。
- **生命周期（Lifetime）**：借用或值在内存中的有效时间区间。
- **作用域（Scope）**：变量或借用的可见范围。
- **Copy类型**：可按位复制的类型，赋值后原值依然有效。
- **Move语义**：所有权转移，原变量失效。
- **Drop特质**：作用域结束时自动释放资源的机制。
- **RAII**：资源获取即初始化，自动管理资源生命周期。
- **RefCell**：单线程内部可变性容器，运行时借用检查。
- **Mutex**：多线程可变性容器，保证并发安全。
- **Arc**：原子借用计数智能指针，实现多线程共享所有权。
- **Weak**：非拥有型弱借用，打破循环依赖防止内存泄漏。
- **悬垂借用（Dangling Reference）**：指向已释放内存的借用，Rust编译器禁止。
