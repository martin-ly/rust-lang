...
1. 在任意给定时间，要么只能有一个可变借用，要么只能有多个不可变借用
2. 借用必须总是有效的
...
fn main() {
    ...
    let r1 = &s;     // 没问题
    let r2 = &s;     // 没问题
    // let r3 = &mut s; // 错误：不能在存在不可变借用的同时创建可变借用
...
&i32        // 借用
&'a i32     // 带有显式生命周期的借用
&'a mut i32 // 带有显式生命周期的可变借用
```

### 4.2 函数中的生命周期

当函数返回借用时，通常需要指定生命周期参数：
...
- 不可变借用允许多个借用同时存在，但不能修改数据
- 可变借用允许修改数据，但一次只能有一个借用
- 生命周期确保借用不会比它们借用的数据存在更长时间
...
fn main() {
    let mut s = SelfRef {
        ...
        reference: &String::new(), // 临时借用
    };
    
    // 重新设置借用，使其指向自身的 value
    // 这种方式很危险，很容易造成悬垂借用
    // s.reference = &s.value; // 编译错误：借用检查器会阻止这种自借用
...
- 自借用结构在 Rust 中难以实现，通常需要使用 `Pin` 或内部可变性
- 对于复杂借用关系，考虑使用索引而不是借用
