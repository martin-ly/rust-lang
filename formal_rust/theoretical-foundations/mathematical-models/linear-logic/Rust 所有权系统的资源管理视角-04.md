...
fn immutable_list_example() {
    ...
    print_list(&list2);
    print_list(&list1); // list1 仍然可用，因为只共享了不可变借用
}
...
3. **闭包特质与所有权**
   - `Fn` vs `FnMut` vs `FnOnce` 特质的所有权语义
   - 闭包类型推导与使用场景
...
fn optimize_copies() {
    ...
    fn optimized_search(haystack: &[String], needle: &str) -> bool {
        for item in haystack {
            if item.contains(needle) { // 直接使用借用
                return true;
            }
        }
        false
...
fn detect_patterns() {
    ...
    // 潜在问题：闭包捕获可变借用但保存在长生命周期结构中
    let processor = move |val| {
        data.push(val); // 捕获 data 的可变借用
        data.len()
    };
    
    // 分析器可能发出警告：闭包捕获的可变借用可能超出原始数据生命周期
    processor(4);
...
fn performance_patterns() {
    ...
    let len1 = process(data.clone()); // 静态分析工具可能建议改为借用
...
fn attribute_examples() {
    ...
    // let key_ref = &key; // 假设：检查工具警告不应该创建借用
...
impl QuantumRegister {
    ...
    fn entangle(&mut self, control: usize, target: usize) {
        ...
        // 使用 split_at_mut 或其他方法来安全地获取多个可变借用
...
fn ownership_thinking() {
    ...
    let handle = shared_data.clone(); // 增加借用计数
...
所有权系统既是一种变量操作规则，也是一种类型系统设计；既是资源管理机制，也是并发安全保障；既是编程语言特质，也是系统设计思维方式。这种多层次的影响力使得所有权系统不仅仅是 Rust 语言的核心特质，更是对编程语言理论和实践的重要贡献。
