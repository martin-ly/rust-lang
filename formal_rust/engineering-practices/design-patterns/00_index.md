...
设计模式语义模块是Rust语言形式化理论的重要应用层，涵盖了软件设计中常用模式的形式化语义定义，包括创建型、结构型、行为型、并发型和并发型设计模式。本模块建立了严格的理论基础，为Rust语言的软件架构设计提供了形式化的语义定义。
...
- **[05_parallel_patterns/00_index.md](05_parallel_patterns/00_index.md)** - 并发型设计模式
  - 分治模式语义
  - 映射-归约模式语义
  - 流水线模式语义
  - 扇入-扇出模式语义
