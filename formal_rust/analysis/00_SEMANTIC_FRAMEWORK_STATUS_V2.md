...
- **错误传播单子理论**：Result类型的单子结构，Coq/Lean证明，?操作符传播，反例：panic未捕获。

### 并发语义层

- **Future单子组合理论**：Future组合的单子结构，异步状态机建模，自动化测试，标准库Future，反例：状态丢失。
- **线程安全类型系统定理**：Send/Sync trait安全性，Polonius自动化推理，标准库Arc/Mutex，反例：Rc/RefCell跨线程。
