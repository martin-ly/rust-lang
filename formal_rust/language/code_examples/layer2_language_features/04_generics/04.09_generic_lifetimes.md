...
泛型生命周期用于在泛型类型、函数、trait中抽象和约束引用的生命周期，保证内存安全和类型安全。
Rust通过生命周期参数`'a`等机制实现静态借用检查。
相关理论详见[生命周期系统理论](../../01_ownership_borrowing/03_lifetime_system.md)与[泛型系统](../../04_generics/01_formal_generics_system.md)。
