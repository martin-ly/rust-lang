...
裸指针（*const T, *mut T）允许绕过Rust安全检查直接操作内存，适用于底层编程和FFI。使用裸指针需手动管理生命周期和内存安全，易导致悬垂指针和未定义行为。相关理论详见[裸指针机制](../../11_memory_management/04_raw_pointer.md)与[内存安全理论](../../11_memory_management/02_memory_safety.md)。
