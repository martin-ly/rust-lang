...
- **理论层次**: 第二层：语言特征形式化层
- **相关概念**: 12.02 (堆), 12.03 (栈内存), 12.04 (内存布局)
- **难度级别**: 高级
...
- $\text{Memory}(H, S, L)$ 表示堆集合 $H$、栈内存集合 $S$ 和生命周期集合 $L$
- $\forall h \in H$ 表示对所有堆 $h$ 在堆集合 $H$ 中
- $\exists s \in S$ 表示存在栈内存 $s$ 在栈内存集合 $S$ 中
- $\text{Heap}(h)$ 表示堆 $h$ 的分配
...
// 堆管理
fn heap_memory() {
    let boxed_value = Box::new(42); // 堆分配
    println!("堆值: {}", *boxed_value);
    // Box离开作用域时自动释放堆
}
...
fn main() {
    ...
    println!("结构体体体体是否64字节对齐: {}", aligned.is_aligned());
    println!("结构体体体体大小: {}", std::mem::size_of::<AlignedStruct>());
