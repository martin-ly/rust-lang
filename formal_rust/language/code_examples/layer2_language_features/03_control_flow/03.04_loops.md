...
循环结构体体体是控制流的重要组成部分，Rust支持`loop`、`while`、`for`三种主要循环语法。循环结构体体体用于重复执行代码块，支持break、continue等控制语句，结合模式匹配和迭代器实现灵活的数据处理。相关理论详见[控制流与循环](../../03_control_flow/04_loops.md)与[迭代器模式](../../08_algorithms/01_iterator_pattern.md)。
