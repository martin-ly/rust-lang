...
- [Rust函数与所有权、借用、生命周期机制分析](#rust函数与所有权借用生命周期机制分析)
  ...
  - [2. 函数类型的原理与使用规范](#2-函数类型的原理与使用规范)
    ...
    - [2.4 生成器函数](#24-生成器函数)
      - [2.4.1 **生成器的内部结构体体体**](#241-生成器的内部结构体体体)
...
闭包内存布局在编译时会转化为匿名结构体体体体，捕获的变量作为结构体体体体字段。
...
生成器内部结构体体体保存了恢复执行所需的状态和捕获的环境变量。
...
3. **函数类型和内存管理**：
   ...
   - 闭包类型是由编译器生成的匿名结构体体体体，大小依赖于捕获变量的数量和大小
...
2. **挑战**：
   ...
   - 生命周期标注增加了学习曲线和认知负担
   - 某些函数式模式（如递归数据结构体体体中的高阶函数）实现较为复杂
