...
- Rust 2018引入非词法生命周期（NLL），提升了生命周期推断的灵活性和安全性。
...
- 生命周期参数与async/await、生成器的深度集成，提升异步安全性。
