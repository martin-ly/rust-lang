...
1. [不变性（Immutability）](#1-不变性immutability)
    1.1 [定义与本质](#11-定义与本质)
    1.2 [多维视角分析](#12-多维视角分析)
    1.3 [代码示例](#13-代码示例)
    1.4 [适用场景与优势](#14-适用场景与优势)
    1.5 [局限与批判](#15-局限与批判)
    1.6 [交叉引用](#16-交叉引用)
2. [可变性（Mutability）](#2-可变性mutability)
    2.1 [定义与本质](#21-定义与本质)
    2.2 [多维视角分析](#22-多维视角分析)
    2.3 [代码示例](#23-代码示例)
    2.4 [适用场景与优势](#24-适用场景与优势)
    2.5 [局限与批判](#25-局限与批判)
    2.6 [交叉引用](#26-交叉引用)
3. [内部可变性（Interior Mutability）](#3-内部可变性interior-mutability)
    3.1 [定义与本质](#31-定义与本质)
    3.2 [多维视角分析](#32-多维视角分析)
    3.3 [代码示例](#33-代码示例)
    3.4 [适用场景与优势](#34-适用场景与优势)
    3.5 [局限与批判](#35-局限与批判)
    3.6 [交叉引用](#36-交叉引用)
4. [批判性分析与未来展望](#4-批判性分析与未来展望)
5. [变量状态变化流程图](#5-变量状态变化流程图)
6. [本地导航与相关主题](#6-本地导航与相关主题)
...
- [2. 可变性](#2-可变性mutability)
- [3. 内部可变性](#3-内部可变性interior-mutability)
- [所有权系统理论](../04_ownership_system/01_ownership_theory.md)
...
**定义 2.1（可变性）** 设 $v$ 为变量，若存在 $t_1, t_2 \in L(v)$，使得 $v(t_1) \neq v(t_2)$，则称 $v$ 具有可变性。
...
**定义 3.1（内部可变性）** 变量本身不可变，但其内部状态可通过特殊机制（如 `Cell`、`RefCell`）在运行时变更。
