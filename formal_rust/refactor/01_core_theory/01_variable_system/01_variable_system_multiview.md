...
控制流结构（如if、match、loop、while、for等）通常以块`{}`为主体，创建内部作用域。分支、循环体内定义的变量仅在当前分支/迭代的作用域内有效。
...
生命周期（lifetime）是引用保持有效的时间区间，通常与作用域相关但不完全等同。Rust通过生命周期参数和借用检查器，静态保证引用安全，防止悬垂引用。
