...
1. [16.1 状态机建模](#161-状态机建模)
    1.1 [异步函数的状态机转换](#1611-异步函数的状态机转换)
    1.2 [所有权转移与借用的状态图](#1612-所有权转移与借用的状态图)
    1.3 [生命周期推导的自动机模型](#1613-生命周期推导的自动机模型)
2. [16.2 图表与可视化](#162-图表与可视化)
    2.1 [借用关系图与生命周期图](#1621-借用关系图与生命周期图)
    2.2 [内存布局可视化与工程案例](#1622-内存布局可视化与工程案例)
3. [16.3 批判性分析与未来展望](#163-批判性分析与未来展望)
4. [16.4 交叉引用](#164-交叉引用)
...
- **定义 16.2（所有权状态）** 设 $S = \{Owned, Borrowed, Released\}$，变量状态转移遵循所有权与借用规则。
...
- **定义 16.5（内存布局图）** 结构体字段与内存对齐关系可用有向图建模。
...
| 未来展望       | 自动化状态机建模与可视化工具将成为工程与教学的重要方向。               |
