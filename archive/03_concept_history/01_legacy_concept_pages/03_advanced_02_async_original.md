...
- §8.7取消安全系统分析（含3种安全模式与形式化定义）、§8.8 Waker契约与活性（含决策树），建立异步（Async）语义模型完整推理链
...
> **章节过渡**：定义之后需辨析 async 在并发光谱中的精确位置。以下矩阵将 async 与线程、并行对比，澄清"异步≠并行≠并发"的常见误解；随后给出 Future 组合子与运行时（Runtime）选型矩阵，为工程决策提供依据。
...
> **章节过渡**：属性矩阵回答了"是什么"，本节回答"为什么安全"。Rust 的 async/await 安全性建立在两个形式化支柱上：(1) 编译器将 async fn 转换为状态机，(2) Pin 保证该状态机在挂起期间内存地址恒定。二者共同构成"零成本 + 内存安全（Memory Safety）"的基石。
...
> **[来源: Rust Compiler: rustc_mir_transform::async_lowering; [RFC 2394](https://rust-lang.github.io/rfcs//2394-async_await.html) §3.2; without.boats blog: Pinning in Rust Futures]**
...
> **[来源: `04_formal/03_ownership_formal.md` §9.5; [RFC 2349](https://rust-lang.github.io/rfcs//2349-pin.html) §3; RustBelt Pin 证明; Vardi & Wolper 1986 — LTL]**
...
> **[来源: RFC 2394]** Rust `async/await` 基于 `Future` trait 和编译器状态机转换，承诺零成本抽象（Zero-Cost Abstraction）。 ✅
...
| **C1** | `!Send` 类型跨 await ⟹ 编译错误 | 状态机含 `Rc`/裸指针/`MutexGuard` 等 | `tokio::spawn` 及跨线程调度被类型系统（Type System）拒绝 | 子类型拒绝公理 | — | `unsafe impl Send for T` 恶意/错误绕过 | 数据竞争（运行时 UB），破坏线程安全 |
...
> **[来源: futures-rs: StreamExt::next 源码]** `next()` 通过 `Next` 结构体（Struct）实现 `Future` trait，其 `poll` 方法直接委托给底层 `Stream::poll_next`。[来源: Rust Async Book: Streams]
...
> **[来源: futures-rs: Sink trait 文档]** `Sink` 的设计灵感来自 `Iterator` 的逆过程，但增加了异步缓冲和刷新阶段。`send` 是 `poll_ready` + `start_send` + `poll_flush` 的组合子，确保每次发送后数据不滞留缓冲。[来源: [RFC 2394](https://rust-lang.github.io/rfcs//2394-async_await.html) 附录: Async I/O 抽象]
...
> **[Rust Reference: pin_macro]** Rust 1.68+ 引入 `std::pin::pin!` 宏（Macro），允许在栈上创建 `Pin<&mut T>`，避免 `Box::pin` 的堆分配开销。✅ 已验证
