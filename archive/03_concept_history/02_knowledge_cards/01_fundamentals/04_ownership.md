...
> **修正**: `Copy` 和 `Drop` 是**互斥**的：若类型实现了 `Drop`，它不能实现 `Copy`。
> 原因：`Copy` 意味着按位复制是安全的（无资源管理），`Drop` 意味着有自定义清理逻辑（通常管理非内存资源，如文件描述符、网络连接）。
> `String` 不实现 `Copy`（它管理堆内存），所以包含 `String` 的 struct 也不能 `Copy`。
> 实现 `Copy` 的条件：
>
> 1) 所有字段都 `Copy`；
> 2) 无 `Drop` 实现。
