...
- [安全审计指南](.#安全审计指南)
  ...
  - [1. 审计框架](.#1-审计框架)
    - [1.1 审计类型](.#11-审计类型)
    - [1.2 审计生命周期](.#12-审计生命周期)
  - [2. 静态代码审计](.#2-静态代码审计)
    - [2.1 审计工具链](.#21-审计工具链)
    - [2.2 unsafe代码审计](.#22-unsafe代码审计)
  - [3. 依赖安全审计](.#3-依赖安全审计)
    - [3.1 依赖评估矩阵](.#31-依赖评估矩阵)
    - [3.2 依赖审计配置](.#32-依赖审计配置)
    - [3.3 SBOM生成与验证](.#33-sbom生成与验证)
  - [4. 运行时安全监控](.#4-运行时安全监控)
    - [4.1 安全事件检测](.#41-安全事件检测)
    - [4.2 入侵检测](.#42-入侵检测)
  - [5. 审计报告模板](.#5-审计报告模板)
    - [5.1 执行摘要](.#51-执行摘要)
    - [5.2 详细审计清单](.#52-详细审计清单)
  - [6. 持续安全监控](.#6-持续安全监控)
    - [6.1 CI集成](.#61-ci集成)
