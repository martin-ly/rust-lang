...
- [嵌入式系统概览](#嵌入式系统概览)
  - [Rust嵌入式系统的特点](#rust嵌入式系统的特点)
- [嵌入式系统应用](#嵌入式系统应用)
  - [1. 硬件抽象层 (Hardware Abstraction Layer)](#1-硬件抽象层-hardware-abstraction-layer)
    - [1.1 GPIO抽象](#11-gpio抽象)
    - [1.2 UART抽象](#12-uart抽象)
  - [2. 实时操作系统 (Real-Time Operating System)](#2-实时操作系统-real-time-operating-system)
    - [2.1 任务调度器](#21-任务调度器)
    - [2.2 信号量](#22-信号量)
  - [3. 中断处理 (Interrupt Handling)](#3-中断处理-interrupt-handling)
    - [3.1 中断向量表](#31-中断向量表)
  - [4. 内存管理 (Memory Management)](#4-内存管理-memory-management)
    - [4.1 静态内存分配器](#41-静态内存分配器)
- [总结](#总结)
