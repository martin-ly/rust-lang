...
- **可变性与同一性**：JS对象的可变性进一步破坏了值的同一性。一个对象在不同时间点可以有不同的状态，但其借用（identity）不变，这与HoTT 中基于值本身的同一性概念不同。
...
- **组合类型（对象）**：JS对象类似于记录（Record）或依赖对（Σ-type），但其动态性（属性可增删）和可变性使其难以对应HoTT 中的静态构造。
