...
- **不可变借用** (`&T`)：允许多个同时存在的只读借用
- **可变借用** (`&mut T`)：独占的读写借用，与其他借用互斥
- **借用检查器**：编译时验证借用规则的静态分析工具

#### 3. 生命周期系统

- **生命周期注解**：表示借用有效期的编译时标记
- **生命周期推导**：编译器自动推断生命周期的算法
- **生命周期约束**：确保借用安全性的类型系统规则
...
- **[模块 28: 高级类型特质](../28_advanced_type_features/00_index.md)** - 高级类型构造
...
- **定义 1.6**: [生命周期](01_formal_ownership_system.md#生命周期定义) - 借用有效期范围
