...
- [Rust所有权系统数学基础](#rust所有权系统数学基础)
  - [📊 目录](#-目录)
  - [1. 集合论基础](#1-集合论基础)
    - [1.1 基本集合定义](#11-基本集合定义)
      - [变量集合 $\\mathbb{X}$](#变量集合-mathbbx)
      - [值集合 $\\mathbb{V}$](#值集合-mathbbv)
      - [类型集合 $\\mathbb{T}$](#类型集合-mathbbt)
    - [1.2 所有权关系的形式化定义](#12-所有权关系的形式化定义)
      - [所有权关系 $\\text{Own}$](#所有权关系-textown)
      - [所有权关系的公理](#所有权关系的公理)
    - [1.3 借用关系的形式化定义](#13-借用关系的形式化定义)
      - [借用关系 $\\text{Borrow}$](#借用关系-textborrow)
      - [借用关系的公理](#借用关系的公理)
  - [2. 线性逻辑基础](#2-线性逻辑基础)
    - [2.1 线性逻辑连接词](#21-线性逻辑连接词)
      - [线性合取 $\\otimes$](#线性合取-otimes)
      - [线性蕴含 $\\multimap$](#线性蕴含-multimap)
    - [2.2 线性类型规则](#22-线性类型规则)
      - [线性函数类型](#线性函数类型)
      - [线性应用规则](#线性应用规则)
  - [3. 分离逻辑基础](#3-分离逻辑基础)
    - [3.1 分离合取 $\*$](#31-分离合取-)
      - [堆分离](#堆分离)
      - [分离蕴含 $\\mathrel{-!!\*}$](#分离蕴含-mathrel-)
    - [3.2 借用规则的形式化](#32-借用规则的形式化)
      - [不可变借用](#不可变借用)
      - [可变借用](#可变借用)
  - [4. 区域类型系统](#4-区域类型系统)
    - [4.1 区域定义](#41-区域定义)
      - [区域 $\\rho$](#区域-rho)
      - [区域借用类型](#区域借用类型)
    - [4.2 区域包含关系](#42-区域包含关系)
      - [区域包含 $\\subseteq$](#区域包含-subseteq)
      - [区域子类型](#区域子类型)
  - [5. 生命周期理论](#5-生命周期理论)
  ...
  - [6. 移动语义](#6-移动语义)
    - [6.1 移动关系](#61-移动关系)
      - [移动 $\\text{Move}$](#移动-textmove)
      - [移动后的状态](#移动后的状态)
    - [6.2 Copy 与 Clone](#62-copy-与-clone)
      - [Copy 特质](#copy-特质)
      - [Clone 特质](#clone-特质)
  - [7. 内存安全定理](#7-内存安全定理)
    - [7.1 内存安全定义](#71-内存安全定义)
      - [内存安全 $\\text{MemorySafe}$](#内存安全-textmemorysafe)
    - [7.2 关键定理](#72-关键定理)
      - [定理 1: 所有权保证内存安全](#定理-1-所有权保证内存安全)
      - [定理 2: 借用检查器正确性](#定理-2-借用检查器正确性)
  - [8. 形式化验证](#8-形式化验证)
    - [8.1 验证方法](#81-验证方法)
      - [模型检查](#模型检查)
      - [定理证明](#定理证明)
