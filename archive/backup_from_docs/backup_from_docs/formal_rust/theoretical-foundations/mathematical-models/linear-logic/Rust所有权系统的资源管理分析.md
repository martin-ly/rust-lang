...
3. **非对称处理**：
   ...
   - 值类型 vs 借用类型：前者默认在栈上，后者涉及堆内存

4. **形式化表达**：

如果定义 `T` 为类型，`O(v)` 表示值 `v` 的所有者，`&T` 和 `&mut T` 分别为不可变和可变借用类型，则：
...
match some_option {
    Some(value) => {
        // value拥有所有权（如果Some 中的值实现了Copy则是复制）
    }
    None => {
        // 不涉及所有权转移
    }
...
1. **内部可变性**：通过 `RefCell`、`Mutex` 等类型在运行时检查借用规则，允许在不可变借用的情况下修改内容。

2. **生命周期标注**：显式指定借用的生命周期关系，解决编译器无法推断的情况。
