...
- **异步/生成器生命周期擦除**：
  - `async fn` 返回的 Future 类型生命周期擦除，Pin/Unpin trait 的安全性保证。
  - 工程难题：生命周期推导与 trait object、闭包、async/await 的交互。
- **FFI与生命周期**：
  - Rust与C交互时生命周期信息丢失，需手动保证内存安全。
- **并发场景下的生命周期**：
  - 多线程共享借用的 Send/Sync trait 自动推导与生命周期约束。
