...
- [02 借用系统形式化理论 {#借用系统}](#02-借用系统形式化理论-借用系统)
  ...
  - [附：索引锚点与导航](#附索引锚点与导航)
    ...
    - [借用环境 {#借用环境}](#借用环境-借用环境)
    - [借用关系 {#借用关系}](#借用关系-借用关系)
    - [不可变借用 {#不可变借用}](#不可变借用-不可变借用)
    - [可变借用 {#可变借用}](#可变借用-可变借用)
    - [借用组合 {#借用组合}](#借用组合-借用组合)
    - [基本借用规则 {#基本借用规则}](#基本借用规则-基本借用规则)
    - [借用检查算法 {#借用检查算法}](#借用检查算法-借用检查算法)
    - [借用传播 {#借用传播}](#借用传播-借用传播)
    - [检查器架构 {#检查器架构}](#检查器架构-检查器架构)
    - [检查算法 {#检查算法}](#检查算法-检查算法)
    - [错误检测 {#错误检测}](#错误检测-错误检测)
    - [生命周期标注 {#生命周期标注}](#生命周期标注-生命周期标注)
    - [生命周期推断 {#生命周期推断}](#生命周期推断-生命周期推断)
    - [生命周期省略 {#生命周期省略}](#生命周期省略-生命周期省略)
    - [数据结构体体体借用 {#数据结构体体体借用}](#数据结构体体体借用-数据结构体体体借用)
    - [函数借用 {#函数借用}](#函数借用-函数借用)
    - [并发借用 {#并发借用}](#并发借用-并发借用)
    - [借用安全定理 {#借用安全定理}](#借用安全定理-借用安全定理)
    - [数据竞争避免定理 {#数据竞争避免定理}](#数据竞争避免定理-数据竞争避免定理)
    - [借用检查完备性定理 {#借用检查完备性定理}](#借用检查完备性定理-借用检查完备性定理)

## 1. 概述

借用系统是Rust所有权系统的核心组成部分，通过编译时静态分析确保内存安全和线程安全。
借用系统基于分离逻辑和线性类型理论，提供了严格的借用规则和检查机制。
...
- [不可变借用](03_lifetime_system.md#不可变借用) (本文档)
- [可变借用](03_lifetime_system.md#可变借用) (本文档)
...
- [函数借用](03_lifetime_system.md#72-函数借用) (本文档)
...
- [借用环境](03_lifetime_system.md#借用环境) (本文档)
- [基本借用规则](03_lifetime_system.md#基本借用规则) (本文档)
...
- [借用检查算法](03_lifetime_system.md#借用检查算法) (本文档)
...
- [生命周期推断](03_lifetime_system.md#生命周期推断) (本文档)
...
用于跨文档引用，指向本篇借用系统总体说明与规则总览。

### 借用环境 {#借用环境}

用于跨文档引用，统一指向“2.1 借用环境”。

### 借用关系 {#借用关系}

用于跨文档引用，统一指向“2.2 借用关系”。

### 不可变借用 {#不可变借用}

用于跨文档引用，统一指向“3.1 不可变借用”。

### 可变借用 {#可变借用}

用于跨文档引用，统一指向“3.2 可变借用”。

### 借用组合 {#借用组合}

用于跨文档引用，统一指向“3.3 借用组合”。

### 基本借用规则 {#基本借用规则}

用于跨文档引用，统一指向“4.1 基本借用规则”。

### 借用检查算法 {#借用检查算法}

用于跨文档引用，统一指向“4.2 借用检查算法”。

### 借用传播 {#借用传播}

用于跨文档引用，统一指向“4.3 借用传播”。

### 检查器架构 {#检查器架构}

用于跨文档引用，统一指向“5.1 检查器架构”。

### 检查算法 {#检查算法}

用于跨文档引用，统一指向“5.2 检查算法”。

### 错误检测 {#错误检测}

用于跨文档引用，统一指向“5.3 错误检测”。

### 生命周期标注 {#生命周期标注}

用于跨文档引用，统一指向“6.1 生命周期标注”。

### 生命周期推断 {#生命周期推断}

用于跨文档引用，统一指向“6.2 生命周期推断”。

### 生命周期省略 {#生命周期省略}

用于跨文档引用，统一指向“6.3 生命周期省略”。

### 数据结构体体体借用 {#数据结构体体体借用}

用于跨文档引用，统一指向“7.1 数据结构体体体借用”。

### 函数借用 {#函数借用}

用于跨文档引用，统一指向“7.2 函数借用”。

### 并发借用 {#并发借用}

用于跨文档引用，统一指向“7.3 并发借用”。

### 借用安全定理 {#借用安全定理}

用于跨文档引用，统一指向“8.1 借用安全定理”。

### 数据竞争避免定理 {#数据竞争避免定理}

用于跨文档引用，统一指向“8.2 数据竞争避免定理”。

### 借用检查完备性定理 {#借用检查完备性定理}

用于跨文档引用，统一指向“8.3 借用检查完备性定理”。
