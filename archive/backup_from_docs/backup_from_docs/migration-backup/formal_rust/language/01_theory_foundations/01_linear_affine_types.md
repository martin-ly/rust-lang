...
线性类型理论（Linear Type Theory）和仿射类型理论（Affine Type Theory）是Rust所有权系统的主要理论基础。这些类型系统对值的使用施加了严格的约束，确保资源的安全管理。本章节将详细探讨这些理论及其在Rust中的应用。
