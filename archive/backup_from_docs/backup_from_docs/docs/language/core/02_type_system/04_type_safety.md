...
- [Rust 的类型系统安全推理](#rust-的类型系统安全推理)
  ...
  - [2. Rust 如何实现类似仿射类型系统的特性](#2-rust-如何实现类似仿射类型系统的特性)
    - [2.1 单一所有权](#21-单一所有权)
    - [2.2 借用机制](#22-借用机制)
    - [2.3 生命周期](#23-生命周期)
...
生命周期是描述引用有效范围的抽象概念，确保引用不会超出其指向的数据的作用域。
...
从引用一致性视角看，一个已经被释放或不再有效的资源（逻辑证明的失败，非内存地址失效）。
...
从引用一致性视角看，当函数返回时，s 会被释放（编译期证明的资源生命周期），而返回的引用会指向一个无效的资源（逻辑证明的失败，非内存地址失效）。
