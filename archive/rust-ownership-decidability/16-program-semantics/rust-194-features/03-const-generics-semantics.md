...
- [Const Generics 的依赖类型语义](#const-generics-的依赖类型语义)
  ...
  - [2. 语法定义](#2-语法定义)
    - [2.1 扩展的类型语法](#21-扩展的类型语法)
    - [2.2 常量表达式语法](#22-常量表达式语法)
    - [2.3 常量种类 (Const Kind)](#23-常量种类-const-kind)
  - [3. 操作语义](#3-操作语义)
    - [3.1 常量表达式求值](#31-常量表达式求值)
    - [3.2 数组构造语义](#32-数组构造语义)
    - [3.3 数组访问语义](#33-数组访问语义)
  - [4. 类型规则](#4-类型规则)
    - [4.1 常量泛型声明](#41-常量泛型声明)
    - [4.2 数组类型规则](#42-数组类型规则)
    - [4.3 类型应用规则](#43-类型应用规则)
    - [4.4 类型等价规则](#44-类型等价规则)
  - [5. 形式化定义](#5-形式化定义)
    - [5.1 常量求值函数](#51-常量求值函数)
    - [5.2 类型等价](#52-类型等价)
    - [5.3 常量表达式等价](#53-常量表达式等价)
    - [5.4 常量上下文](#54-常量上下文)
  - [6. 安全定理](#6-安全定理)
    - [6.1 类型安全定理](#61-类型安全定理)
    - [6.2 数组边界安全](#62-数组边界安全)
    - [6.3 常量求值终止性](#63-常量求值终止性)
    - [6.4 类型等价一致性](#64-类型等价一致性)
  - [7. Rust 代码示例](#7-rust-代码示例)
    - [7.1 基本 Const Generic 类型](#71-基本-const-generic-类型)
    - [7.2 数组类型操作](#72-数组类型操作)
    - [7.3 类型级别的常量计算](#73-类型级别的常量计算)
    - [7.4 Const Generic 约束](#74-const-generic-约束)
    - [7.5 Const 表达式求值](#75-const-表达式求值)
    - [7.6 数组长度泛型](#76-数组长度泛型)
    - [7.7 泛型特化](#77-泛型特化)
  - [8. 依赖类型限制](#8-依赖类型限制)
    - [8.1 Rust 的限制](#81-rust-的限制)
    - [8.2 与完全依赖类型的比较](#82-与完全依赖类型的比较)
  - [9. 与其他特性的交互](#9-与其他特性的交互)
    - [9.1 与生命周期](#91-与生命周期)
    - [9.2 与 Trait Bound](#92-与-trait-bound)
    - [9.3 与 impl Trait](#93-与-impl-trait)
  - [10. 实现细节](#10-实现细节)
    - [10.1 编译时求值](#101-编译时求值)
    - [10.2 单态化爆炸问题](#102-单态化爆炸问题)
  - [11. 总结](#11-总结)
<a id="const-generics-使得-rust-能够表达更精确的类型约束同时保持零成本抽象"></a>
