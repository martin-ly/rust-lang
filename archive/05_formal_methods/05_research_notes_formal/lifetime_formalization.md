...
> **创建日期**: 2025-01-27
> **最后更新**: 2026-02-27
> **更新内容**: 添加生命周期边界定义 (Def 2.4)；添加生命周期参数定义 (Def 2.5)
> **Rust 版本**: 1.96.0+ (Edition 2024)
> **状态**: ✅ 已完成 (Week 1 任务 P1-W1-T3)
...
**定义 2.4 (生命周期边界)**：生命周期边界 $\partial\ell$ 是生命周期 $\ell$ 的开始和结束点：

$$\partial\ell = (\text{start}(\ell), \text{end}(\ell))$$

其中：

- $\text{start}(\ell)$：生命周期开始点（引用创建）
- $\text{end}(\ell)$：生命周期结束点（最后一次使用或作用域结束）

**NLL (Non-Lexical Lifetime)**：Rust 2018+ 使用非词法生命周期，$\text{end}(\ell)$ 是引用的最后一次使用，而非作用域结束。

---

**定义 2.5 (生命周期参数)**：泛型生命周期参数 $\mathcal{L}$ 是函数或类型的泛型参数：

$$\mathcal{L} = \{'a, 'b, \ldots\}$$

**生命周期参数约束**：
$$'a: 'b \iff \ell_a \supseteq \ell_b$$
...
*证明*：（1）⇒：若程序良型，则 [borrow_checker_proof](10_borrow_checker_proof.md) 与 [ownership_model](10_ownership_model.md) 保证引用使用合法；约束生成规则（Def 3.1）生成与程序语义一致的约束；由 Axiom LF2，一致则有解。（2）⇐：若约束系统一致，则存在满足所有约束的 $\Lambda$；约束反映程序语义，故程序良型。∎
...
4. **Ferrocene FLS** — Rust 1.93 形式化规范
   ...
   - 与本目录: outlives、引用有效性 T2 对应；Rust 官方采纳 2025
...
$\text{lft}(r) \subseteq \text{lft}(\text{target}(r))$：引用寿命不超过被引用者；与所有权 $\Omega$、借用规则 $\text{Scope}(r)$ 结合，NLL/Polonius 与 [ownership_model](10_ownership_model.md)、[borrow_checker_proof](10_borrow_checker_proof.md) 一致。
...
2. **异步与 Pin**：async 块中 `&'a x` 的 `'a` 编译进状态机，与 [async_state_machine](10_async_state_machine.md) 的 Pin 不变式一致。
...
**国际权威对标**：RustBelt、Polonius；区域类型、NLL、datalog 形式化；[FLS Ch. 15.3–15.4](https://spec.ferrocene.dev/ownership-and-deconstruction.html) References、Borrowing。
