...
**分析**：泛型代码的完全分析需要**全称量词**（对所有类型T），这超出了静态分析的能力。
Rust采用**单态化(Monomorphization)**，在实例化时检查，但这意味着某些错误只能在特定使用时发现。
...
**分析**：高阶多态涉及**二阶逻辑**，类型推断是**不可判定**的（Wells, 1999）。
Rust通过限制语法（如显式生命周期标注）保持可判定性，但表达能力受限。
...
**分析**：检测递归类型是否"良基(Well-Founded)"需要**归纳证明**。
虽然Rust的简单规则（禁止直接递归）是可判定的，但更复杂的场景（如相互递归）需要近似。
