...
- **标准库与trait生态的安全证明**：标准库中如Cell、RefCell、Rc等组件的安全性，已通过RustBelt等项目递归形式化验证，未来可扩展到异步、并发等新特性。
- **异步/并发类型系统的递归论证**：随着async/await、Send/Sync等特性的普及，类型系统的并发安全性、死锁不可达性等成为递归论证的新热点。
