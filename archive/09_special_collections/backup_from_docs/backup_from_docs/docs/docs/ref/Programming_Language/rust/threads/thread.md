...
- [1. 使用 `thread_local!` 宏](#1-使用-thread_local-宏)
  - [示例代码](#示例代码)
  - [2. 访问线程局部存储](#2-访问线程局部存储)
    - [-示例代码](#示例代码)
  - [3. 线程局部存储的生命周期](#3-线程局部存储的生命周期)
  - [4. 线程局部存储的优势](#4-线程局部存储的优势)
  - [5. 线程局部存储的使用场景](#5-线程局部存储的使用场景)
  - [6. 线程局部存储的限制](#6-线程局部存储的限制)
  - [7. 第三方库支持](#7-第三方库支持)
  - [总结](#总结)
- [*PoisonError*](#poisonerror)
  - [处理 `PoisonError`](#处理-poisonerror)
    - [1. **`into_inner` 方法**](#1-into_inner-方法)
    - [2. **`get_ref` 方法**](#2-get_ref-方法)
    - [3. **`get_mut` 方法**](#3-get_mut-方法)
  - [`PoisonError` 的机制](#poisonerror-的机制)
  - [避免 `PoisonError`](#避免-poisonerror)
- [`mutex` or `rwlock`](#mutex-or-rwlock)
  - [1. 使用 `Mutex` 保证数据一致性](#1-使用-mutex-保证数据一致性)
    - [示例代码：使用 Mutex 保护全局状态](#示例代码使用-mutex-保护全局状态)
  - [2. 使用 `RwLock` 提高并发性能](#2-使用-rwlock-提高并发性能)
    - [示例代码：使用 RwLock 保护共享数据](#示例代码使用-rwlock-保护共享数据)
  - [3. 使用 `Arc` 实现多线程共享](#3-使用-arc-实现多线程共享)
    - [示例代码：使用 Arc 和 Mutex](#示例代码使用-arc-和-mutex)
  - [4. 处理锁竞争和死锁](#4-处理锁竞争和死锁)
  - [5. 使用 `PoisonError` 处理毒化锁](#5-使用-poisonerror-处理毒化锁)
    - [示例代码：处理毒化锁](#示例代码处理毒化锁)
  - [*总结*](#总结)
  - [设计策略](#设计策略)
  - [Rust 的保护性机制](#rust-的保护性机制)
  - [示例](#示例)
    - [使用 `catch_unwind` 捕获 `panic`](#使用-catch_unwind-捕获-panic)
    - [使用 `Mutex` 和 `PoisonError`](#使用-mutex-和-poisonerror)
  - [**总结**](#总结)
  - [1. **线程取消（Thread Cancellation）**](#1-线程取消thread-cancellation)
  - [2. **堆栈展开（Stack Unwinding）**](#2-堆栈展开stack-unwinding)
  - [3. **程序终止**](#3-程序终止)
  - [4. **跨线程影响**](#4-跨线程影响)
  - [5. **如何捕获 `panic`**](#5-如何捕获-panic)
  - [-总结-](#总结)
  - [1. 使用 `Result` 和 `Option`](#1-使用-result-和-option)
    - [使用 `Result` 处理错误](#使用-result-处理错误)
    - [使用 `Option` 处理可能缺失的值](#使用-option-处理可能缺失的值)
  - [2. 避免直接调用 `unwrap` 和 `expect`](#2-避免直接调用-unwrap-和-expect)
    - [使用 `if let` 处理 `Option`](#使用-if-let-处理-option)
    - [使用 `match` 处理 `Result`](#使用-match-处理-result)
  - [3. 单元测试和模糊测试](#3-单元测试和模糊测试)
    - [单元测试](#单元测试)
    - [模糊测试（使用 `libfuzzer-sys`）](#模糊测试使用-libfuzzer-sys)
  - [4. 使用封装和抽象](#4-使用封装和抽象)
    - [封装文件读取逻辑](#封装文件读取逻辑)
  - [5. 使用防御性编程](#5-使用防御性编程)
    - [防御性检查示例](#防御性检查示例)
  ...
  - [7. 审查第三方依赖](#7-审查第三方依赖)
    - [审查依赖的 `Cargo.toml`](#审查依赖的-cargotoml)
  - [-总结](#总结)
  - [Rust 错误处理惯用法](#rust-错误处理惯用法)
  - [`unwrap_or` 的解释和示例](#unwrap_or-的解释和示例)
    - [解释](#解释)
    - [-示例](#示例)
  - [`unwrap_or_else` 的解释和示例](#unwrap_or_else-的解释和示例)
    - [-解释](#解释)
    - [--示例](#示例)
  - [`match` 的解释和示例](#match-的解释和示例)
    - [--解释](#解释)
    - [---示例](#示例)
  - [`unwrap` 的解释和示例](#unwrap-的解释和示例)
    - [---解释](#解释)
    - [---示例-](#示例)
  - [`expect` 的解释和示例](#expect-的解释和示例)
    - [----解释](#解释)
    - [----示例](#示例)
  - [`Result` 的错误处理惯用法](#result-的错误处理惯用法)
    - [----示例--](#示例)
  - [**总结***](#总结)
