...
- [3.2 类型安全与静态检查](#32-类型安全与静态检查)
  ...
  - [3.2.3 Rust类型系统的特性](#323-rust类型系统的特性)
...
Rust的类型系统是一个静态类型、强类型系统，它在编译时进行严格的类型检查，确保类型一致性和安全性。
本章节将从形式化的角度详细阐述Rust的类型安全性和静态检查机制，包括其数学基础、形式化定义、类型规则以及与其他类型系统的比较。
