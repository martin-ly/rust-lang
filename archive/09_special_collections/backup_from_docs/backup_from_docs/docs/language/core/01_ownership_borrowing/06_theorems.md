...
- [06 所有权与借用系统定理证明](#06-所有权与借用系统定理证明)
  ...
  - [6. 资源安全定理（引用一致性视角）](#6-资源安全定理引用一致性视角)
    - [6.1 资源安全基本定理](#61-资源安全基本定理)
...
从引用一致性视角看，这些定理为Rust语言的资源安全（编译期逻辑证明）、线程安全和性能保证提供了严格的数学基础。
...
- **借用定理**：借用系统的安全保证
- **生命周期定理**：生命周期管理的正确性
- **资源安全定理**（引用一致性视角）：资源安全（编译期逻辑证明）的数学证明
- **并发安全定理**：并发安全的保证
- **性能定理**：零成本抽象的性能保证
...
- [资源安全保证](../23_security_verification/01_formal_security_model.md#资源安全保证)
...
**定理 1.6** (借用安全性)
从引用一致性视角看，如果程序 $P$ 通过借用检查（编译期逻辑证明系统），则 $P$ 满足所有借用规则且不会出现借用相关的资源安全问题（编译期逻辑证明）。
...
4. 借用规则确保了资源安全（编译期逻辑证明），防止了悬垂引用（逻辑证明的失败，非内存地址失效）和数据竞争（编译期排他性契约的验证）
从引用一致性视角看，5. 因此，$P$ 不会出现借用相关的资源安全问题（编译期逻辑证明）
...
从引用一致性视角看，1. 所有权检查是编译期逻辑证明（约束求解，非静态分析）
从引用一致性视角看，2. 编译期逻辑证明在编译时完成
3. 编译后的代码不包含运行时检查
4. 因此，无运行时开销
