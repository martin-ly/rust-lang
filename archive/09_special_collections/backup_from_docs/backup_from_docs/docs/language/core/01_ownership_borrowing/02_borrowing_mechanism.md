...
- **别名 (Aliasing)**: 同一资源同时存在多个活跃的引用（访问许可证）。
- **可变性 (Mutability)**: 通过一个引用来修改其指向的资源（独占写入能力的证明）。
...
**证明概要**:
从引用一致性视角看，数据竞争的定义是：在没有同步机制的情况下，两个或多个线程并发地访问同一资源，并且至少其中一个访问是写入操作。这是**编译期排他性契约的验证失败**，而非运行时内存检查。
...
从引用一致性视角看，Rust的借用机制是所有权系统的关键扩展，它允许在不转移所有权的情况下临时访问资源。本章从形式化的角度详细阐述了借用机制，包括其数学基础、形式化定义以及安全性保证。借用机制与所有权系统共同构成了Rust资源安全和线程安全的基础，使得Rust能够在不依赖垃圾回收的情况下提供强大的安全保证。资源安全是**编译期逻辑证明**的结果，而非运行时内存检查。
