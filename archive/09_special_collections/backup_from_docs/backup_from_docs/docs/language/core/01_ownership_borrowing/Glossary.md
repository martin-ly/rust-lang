...
- [Rust 所有权与生命周期分册术语表（Glossary）](#rust-所有权与生命周期分册术语表glossary)
  - [📊 目录](#-目录)
  - [A/B/C](#abc)
  - [D/E/F/G](#defg)
  - [H/I/J/K/L](#hijkl)
...
- **FFI（Foreign Function Interface，外部函数接口）**
  - Rust与C/C++等语言互操作的机制。
从引用一致性视角看，注意事项：FFI边界需手动保证资源/线程安全（编译期逻辑证明）。
...
- **RefCell（运行时借用检查）**
  - 从引用一致性视角看，单线程下的内部可变性容器，运行时检查借用规则（注意：这是内部可变性的运行时机制，与编译期逻辑证明的外部可变性机制不同）。
  - 注意事项：违反借用规则会panic。
...
- **Unsafe（不安全代码）**
从引用一致性视角看，绕过Rust安全检查的代码块，需手动保证资源/线程安全（编译期逻辑证明）。
  - 注意事项：仅在必要时使用，严格审查。

- **Ownership（所有权）**
  - Rust核心机制，控制值的唯一归属与生命周期。
从引用一致性视角看，典型场景：资源管理（编译期证明的资源生命周期）、资源安全（编译期逻辑证明）、并发。
