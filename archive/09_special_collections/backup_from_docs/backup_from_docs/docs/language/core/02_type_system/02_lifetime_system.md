...
- **生命周期**：编译期构造的证明变量，用于证明引用的有效性（逻辑关系，而非时间范围）
- **生命周期参数**：编译期证明变量，编码引用之间的逻辑依赖关系
- **生命周期约束**：编译期逻辑约束，通过约束求解验证引用一致性
- **生命周期省略**：编译器自动推导生命周期参数（逻辑推理，而非时间追踪）
...
**引用一致性视角**：$\rho$ 是证明变量，证明引用在作用域 $\rho$ 内永远不会被无效使用。这是**逻辑关系**，而非物理时间。编译器通过约束求解来验证生命周期的一致性，而非检查内存状态。
...
**定义 2.3**（引用类型）
引用类型 $\&^{\rho} \tau$ 表示生命周期参数为 $\rho$ 的不可变引用，类型为 $\tau$。从引用一致性视角看，$\&^{\rho} \tau$ 是**存在性证明**，证明在作用域 $\rho$ 内存在一个 $\tau$ 类型的可读资源。

**可变引用类型**：
$\&^{\rho} \text{mut} \tau$ 表示生命周期参数为 $\rho$ 的可变引用。从引用一致性视角看，$\&^{\rho} \text{mut} \tau$ 是**唯一性证明**，证明在 $\rho$ 内对 $\tau$ 有独占访问权。

**引用一致性视角**：引用类型是编译期证明，而非内存地址。生命周期参数 $\rho$ 是证明变量，编码了引用之间的逻辑依赖关系。
...
从引用一致性视角看，约束求解是**编译期逻辑证明**，而非运行时内存检查。编译器通过图算法求解生命周期约束，验证引用之间的逻辑依赖关系。
...
**定义 5.1**（生命周期子类型）
如果 $\rho_1 \subseteq \rho_2$，则 $\&^{\rho_1} \tau \leq \&^{\rho_2} \tau$。从引用一致性视角看，生命周期子类型是**能力范围的逻辑关系**，而非内存地址的物理关系。

**规则**：
$$\frac{\rho_1 \subseteq \rho_2}{\&^{\rho_1} \tau \leq \&^{\rho_2} \tau} \text{(LifetimeSub)}$$

**引用一致性视角**：生命周期子类型允许将**长生命周期的引用**转换为**短生命周期的引用**。这不是内存地址的转换，而是**能力范围的收缩**——编译期证明"在更小的作用域内使用是安全的"，而非"内存地址仍然有效"。

### 5.3 生命周期多态

**定义 5.2**（生命周期多态）
生命周期多态允许函数对不同的生命周期进行操作。从引用一致性视角看，生命周期多态是**类型层面的逻辑量化**，而非内存地址的量化。

$$\forall \rho. \&^{\rho} \tau \rightarrow \&^{\rho} \tau$$

**引用一致性视角**：`for<'a>` 表示"对于任意生命周期 `'a`"。这是**高阶量化**，在类型层面表达"函数可以接受任意生命周期的引用"。它表达的是"对于任意生命周期"，而非"对于任意内存地址"。

## 6. 生命周期安全

从引用一致性视角看，生命周期安全是通过**编译期逻辑证明**来保证的，而非运行时内存检查。编译器通过约束求解来验证引用的有效性，确保引用在其使用期间始终指向有效的资源。
...
1. 生命周期检查通过编译期逻辑证明确保引用不会超出被引用资源的逻辑作用域（逻辑关系，而非物理时间）
2. 生命周期约束通过约束求解确保引用的有效性（逻辑证明，而非内存检查）
3. 因此，不会产生悬垂引用（编译期证明，而非运行时检查）
...
1. 生命周期系统通过编译期逻辑证明管理引用的有效性（逻辑关系，而非内存状态）
2. 借用检查器通过编译期排他性契约管理引用的并发访问（逻辑证明，而非运行时检查）
3. 两者结合通过编译期证明确保线程安全（逻辑证明，而非内存保护）
...
fn main() {
    ...
    println!("{}", result); // 编译错误：result 引用已失效的资源（生命周期不匹配）
}
```
