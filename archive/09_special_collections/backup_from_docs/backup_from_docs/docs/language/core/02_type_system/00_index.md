...
从引用一致性视角看，Rust类型系统（构造性证明系统）是语言的核心特性，提供了强大的编译期逻辑证明（约束求解，非静态分析）、资源安全（编译期逻辑证明）和零成本抽象。本主题涵盖：
...
- 类型推导：自动推断变量和表达式类型
- 类型多态：通过泛型和trait实现
从引用一致性视角看，类型安全：防止类型错误和资源不安全（编译期逻辑证明）
...
| 本模块概念 | 相关模块概念 | 关系描述 |
|------------|--------------|----------|
| 类型安全 | [资源安全（编译期逻辑证明）](../01_ownership_borrowing/01_formal_ownership_system.md#资源安全定义编译期逻辑证明) | 从引用一致性视角看，类型安全是资源安全（编译期逻辑证明）的基础 |
