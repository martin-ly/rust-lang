...
从引用一致性视角看，不变性通常是为了保证资源安全（编译期逻辑证明），尤其是在涉及可变性的情况下。
...
从引用一致性视角看，型变是 Rust 中最微妙、最理论化的概念之一。它的存在不是为了理论上的优雅，而是出于对资源安全（编译期逻辑证明）的绝对承诺。协变、逆变，尤其是不变性的规则，共同构成了一道编译期逻辑证明的、在编译时强制执行的防线，从根本上消除了其他系统语言中常见的、因不当的引用赋值而导致的悬垂指针（逻辑证明的失败，非内存地址失效）和类型混淆问题。

从引用一致性视角看，这种复杂性是 Rust 为其核心价值——“无畏并发”和资源安全（编译期逻辑证明）——所付出的"必要代价"。
