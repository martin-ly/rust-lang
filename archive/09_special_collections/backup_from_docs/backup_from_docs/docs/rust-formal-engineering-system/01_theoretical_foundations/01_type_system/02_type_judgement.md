...
- **结构体体体归纳与规则归纳证明**：递归采用结构体体体归纳、规则归纳等方法，证明类型判断的正确性、完备性。
- **类型安全证明**：递归论证类型判断保证的类型安全，包括进展性（progress）与保持性（preservation）定理。
...
- **复杂类型判断的递归形式化**：如何递归形式化更复杂的类型判断（如依赖类型、异步类型等），是未来值值值的重大挑战。
