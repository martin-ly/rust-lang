...
- [1. 分离逻辑基础](#1-分离逻辑基础)
  - [1.1 分离逻辑语法](#11-分离逻辑语法)
    - [定义 1.1.1 (分离逻辑公式)](#定义-111-分离逻辑公式)
    - [定义 1.1.2 (分离合取)](#定义-112-分离合取)
  - [1.2 分离逻辑语义](#12-分离逻辑语义)
    - [定义 1.2.1 (堆模型)](#定义-121-堆模型)
    - [定义 1.2.2 (分离逻辑语义)](#定义-122-分离逻辑语义)
  - [1.3 分离逻辑推理规则](#13-分离逻辑推理规则)
    - [公理 1.3.1 (分离合取交换律)](#公理-131-分离合取交换律)
    - [公理 1.3.2 (分离合取结合律)](#公理-132-分离合取结合律)
    - [公理 1.3.3 (单位元)](#公理-133-单位元)
    - [公理 1.3.4 (分配律)](#公理-134-分配律)
- [2. 资源逻辑](#2-资源逻辑)
  - [2.1 资源概念](#21-资源概念)
    - [定义 2.1.1 (资源)](#定义-211-资源)
    - [定义 2.1.2 (资源分离)](#定义-212-资源分离)
  - [2.2 资源逻辑语法](#22-资源逻辑语法)
    - [定义 2.2.1 (资源逻辑公式)](#定义-221-资源逻辑公式)
  - [2.3 资源逻辑语义](#23-资源逻辑语义)
    - [定义 2.3.1 (资源状态)](#定义-231-资源状态)
    - [定义 2.3.2 (资源逻辑语义)](#定义-232-资源逻辑语义)
- [3. 内存模型](#3-内存模型)
  - [3.1 内存抽象](#31-内存抽象)
    - [定义 3.1.1 (内存)](#定义-311-内存)
    - [定义 3.1.2 (内存分离)](#定义-312-内存分离)
  - [3.2 内存操作](#32-内存操作)
    - [定义 3.2.1 (内存读取)](#定义-321-内存读取)
    - [定义 3.2.2 (内存写入)](#定义-322-内存写入)
    - [定义 3.2.3 (内存分配)](#定义-323-内存分配)
    - [定义 3.2.4 (内存释放)](#定义-324-内存释放)
- [4. Rust所有权模型](#4-rust所有权模型)
  - [4.1 所有权语义](#41-所有权语义)
    - [定义 4.1.1 (所有权)](#定义-411-所有权)
    - [定义 4.1.2 (所有权转移)](#定义-412-所有权转移)
  - [4.2 借用语义](#42-借用语义)
    - [定义 4.2.1 (借用)](#定义-421-借用)
    - [定义 4.2.2 (不可变借用)](#定义-422-不可变借用)
    - [定义 4.2.3 (可变借用)](#定义-423-可变借用)
  - [4.3 生命周期](#43-生命周期)
    - [定义 4.3.1 (生命周期)](#定义-431-生命周期)
    - [定义 4.3.2 (生命周期约束)](#定义-432-生命周期约束)
- [5. 分离逻辑在Rust中的应用](#5-分离逻辑在rust中的应用)
  - [5.1 内存安全证明](#51-内存安全证明)
    - [定理 5.1.1 (内存安全)](#定理-511-内存安全)
    - [定理 5.1.2 (所有权安全)](#定理-512-所有权安全)
  - [5.2 并发安全证明](#52-并发安全证明)
    - [定理 5.2.1 (并发安全)](#定理-521-并发安全)
- [6. 形式化验证](#6-形式化验证)
  - [6.1 霍尔逻辑扩展](#61-霍尔逻辑扩展)
    - [定义 6.1.1 (分离霍尔逻辑)](#定义-611-分离霍尔逻辑)
  - [6.2 验证示例](#62-验证示例)
    - [示例 6.2.1 (简单分配)](#示例-621-简单分配)
    - [示例 6.2.2 (借用验证)](#示例-622-借用验证)
- [7. 实践应用](#7-实践应用)
  - [7.1 Rust代码示例](#71-rust代码示例)
  - [7.2 形式化验证工具](#72-形式化验证工具)
- [8. 总结](#8-总结)
