...
- [02 所有权系统正确性证明](#02-所有权系统正确性证明)
  - [目录](#目录)
  - [章节简介](#章节简介)
  - [1. 理论基础与所有权模型](#1-理论基础与所有权模型)
  - [2. 内存安全与借用检查](#2-内存安全与借用检查)
  - [3. 生命周期与资源管理](#3-生命周期与资源管理)
  - [4. Rust所有权正确性形式化思路](#4-rust所有权正确性形式化思路)
  - [5. 代码示例与工程意义](#5-代码示例与工程意义)
  - [6. 参考文献](#6-参考文献)

## 章节简介

本章系统梳理Rust所有权系统的理论基础、形式化定义、核心定理与证明思路，涵盖所有权、借用、生命周期、内存安全、借用检查、资源自动管理等。
通过分离逻辑、状态机等建模，给出安全定理、推理过程与工程意义。
...
- **生命周期参数**（如 `'a`）：标注引用作用域，保证引用不悬垂。
- **生命周期推断**：编译器自动分析大部分生命周期。
- **资源自动释放**：RAII模式下，所有权离开作用域即自动释放。

> **推理**：生命周期系统保证引用的所有访问均在有效期内，资源自动释放避免内存泄漏。
