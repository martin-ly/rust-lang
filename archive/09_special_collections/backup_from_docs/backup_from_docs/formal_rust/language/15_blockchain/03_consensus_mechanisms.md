...
- 协议与参数
  - 形式化不变量：Safety（不会产生两个冲突决定）、Liveness（在有界分区与同步窗口下最终达成）、Validity
  - 参数校验：`n ≥ 3f + 1`、阈值/法定人数推导一致；视图切换/超时曲线有证明或实验报告
- 随机性与抗偏置
  - VRF/随机选举必须域分离与KAT覆盖；熵源统一且可审计；禁止自研随机方案
- 消息与身份
  - 签名/聚合签名采用审计库；重放保护（nonce/epoch/高度）；Anti-Equivocation 检测与惩罚
- 实现与运行
  - 状态机显式阶段与超时；视图切换/leader 轮换有可回放日志
  - 崩溃/恢复/重放有确证流程；交叉分片包含性与最终性证明接口
- 测试与验证
  - 故障注入：延迟/乱序/丢包/分区/双花/拜占庭行为库
  - 模型检查：小规模协议栈用 Kani/状态空间裁剪；性质断言落地到测试矩阵
  - 并发：Loom 覆盖通道/锁序；死锁和竞态最小反例入库

### 测试矩阵（摘要）

| 维度 | 场景 | 期望 | 工具 |
|---|---|---|---|
| Safety | 双领导/双提案 | 不产生双重提交 | 断言+Kani |
| Liveness | 短暂分区后恢复 | 在Δ后提交成功 | 仿真+属性测试 |
| Validity | 无效提案 | 必须拒绝 | 单元/属性 |
| DoS | 洪泛/重放 | 速率限制/丢弃 | 模糊/压测 |
