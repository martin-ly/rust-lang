...
**交叉借用**: [1.3.1 内存布局语义](01_memory_layout_semantics.md), [1.4.1 所有权规则语义](../04_ownership_system_semantics/01_ownership_rules_semantics.md)
...
- [1.3.3 Rust栈堆内存语义模型深度分析](#133-rust栈堆内存语义模型深度分析)
  ...
  - [1.3.3.7 相关借用与扩展阅读](#1337-相关借用与扩展阅读)
    - [1.3.3.7.1 内部交叉借用](#13371-内部交叉借用)
...
- $\text{Reference} : \text{Address} \rightarrow \text{Value}$ - 地址解借用
...
fn memory_leak_detection() {
    ...
    struct SafeNode {
        ...
        prev: Option<Weak<RefCell<SafeNode>>>,  // 弱借用避免循环
    }
