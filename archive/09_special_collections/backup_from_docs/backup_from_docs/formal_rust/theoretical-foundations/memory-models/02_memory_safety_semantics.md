...
**交叉借用**: [1.4.1 所有权规则语义](../04_ownership_system_semantics/01_ownership_rules_semantics.md), [1.3.3 栈堆语义](03_stack_heap_semantics.md)
...
- [1.3.2 Rust内存安全语义模型深度分析](#132-rust内存安全语义模型深度分析)
  ...
  - [1.3.2.6 相关借用与扩展阅读](#1326-相关借用与扩展阅读)
    - [1.3.2.6.1 内部交叉借用](#13261-内部交叉借用)
...
fn smart_pointer_temporal_safety() {
    ...
    // 当root被释放时，child的parent Weak借用自动失效
    drop(root);
...
- [Miri](https://github.com/rust-lang/miri) - Rust 中级解释器，检测未定义行为
