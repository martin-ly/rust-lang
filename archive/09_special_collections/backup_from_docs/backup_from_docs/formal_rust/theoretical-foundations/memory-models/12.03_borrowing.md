...
- [12.03 借用（Borrowing）](#1203-借用borrowing)
  - [📊 目录](#-目录)
  - [理论简述](#理论简述)
  - [核心代码示例](#核心代码示例)
  - [详细注释](#详细注释)
...
借用是Rust所有权系统的核心机制，允许在不转移所有权的情况下安全访问数据。分为不可变借用（&T）和可变借用（&mut T），编译器通过借用检查器保证无数据竞争和悬垂借用。相关理论详见[借用系统理论](../../01_ownership_borrowing/02_borrowing_system.md)与[内存管理机制](../../11_memory_management/01_memory_management_theory.md)。
...
- **Q: Rust借用和C++借用有何不同？**
  - A: Rust借用有严格的生命周期和可变性检查，C++借用无此机制。
- **Q: 如何解决借用相关编译错误？**
  - A: 检查借用的作用域、可变性和生命周期。
- **Q: 借用常见应用场景？**
