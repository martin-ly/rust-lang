...
Rust的类型系统提供了强大的类型安全性和灵活性，其中类型转换和型变规则是理解Rust类型系统的关键概念。
类型转换允许在不同类型之间进行转换，而型变规则则定义了如何在类型层次结构中进行安全的类型替换。
...
上转型是指将子类型转换为父类型的过程。
在Rust 中，通常通过trait实现来实现上转型。上转型是安全的，因为子类型包含父类型的所有特质。
...
类型转换的形式论证可以通过类型系统的安全性来进行。
上转型是安全的，因为子类型的所有特质都包含在父类型中。
下转型的安全性则依赖于运行时检查。
...
- 设 \( S \) 为子类型，\( P \) 为父类型。
- 规则：如果 \( x: S \) 则 \( x: P \)。
- 证明：子类型 \( S \) 的所有特质都包含在父类型 \( P \) 中，因此上转型是安全的。
...
型变的形式论证可以通过类型系统的安全性来进行。
协变和逆变的安全性依赖于类型的结构和上下文。

**协变的形式论证**：

- 设 \( S \) 为子类型，\( P \) 为父类型。
- 规则：如果 \( x: S \) 则 \( x: P \)。
- 证明：子类型 \( S \) 的所有特质都包含在父类型 \( P \) 中，因此协变是安全的。
...
协变和逆变为类型转换提供了理论基础，确保在类型层次结构中进行安全的类型替换。
...
Rust的类型系统通过类型转换和型变规则提供了强大的类型安全性和灵活性。
