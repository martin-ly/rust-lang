# Rust 中不变（Invariance）的特质

在 Rust 中，不变（Invariance）是一种类型变体（Variance）特质，表示如果类型 `A` 是类型 `B` 的子类型（`A <: B`），则泛型类型 `F<A>` 和 `F<B>` 之间没有子类型关系。
这意味着不能将 `F<A>` 隐式转换为 `F<B>`，也不能将 `F<B>` 隐式转换为 `F<A>`。

## 目录

- [Rust 中不变（Invariance）的特质](#rust-中不变invariance的特质)
  - [目录](#目录)
  - [常见情况与示例](#常见情况与示例)
...
通过理解不变特质，可以更好地设计和编写安全、高效的 Rust 代码。
...
2. **协变**：允许将长生命周期的借用用于需要短生命周期借用的地方
3. **逆变**：允许将接受更宽松类型的函数用于需要接受更严格类型的函数的地方
4. **不变性**：特别是对于 `&mut T`，确保不会有多个可变借用指向同一数据，防止数据竞争
...
// 不可变借用
let r = &x;
// *r = 10; // 错误：不能通过不可变借用修改值

// 可变借用
let mr = &mut y;
*mr = 10; // 正确：可以通过可变借用修改值
```
...
- 子类型关系与泛型类型的关系方向相同
- 在 Rust 中，不可变借用 `&T` 和 `Box<T>` 对于 T 是协变的
...
fn main() {
    ...
    // 在函数类型中，参数是逆变的
    // 可以将接受任何生命周期借用的函数赋值给需要接受'static借用的函数变量
...
fn example() {
    // 函数 |s: &str| {} 可以接受任何生命周期的字符串借用
    // 由于逆变性，它可以被用在需要接受静态生命周期借用的地方
...
fn main() {
    ...
    let long_lived = String::from("长生命周期");
    {
        ...
        let _: &str = long_ref; // 可以将长生命周期借用赋值给短生命周期借用
    }
    ...
    // 即使类型完全相同，也不能有两个活跃的可变借用
    // let another_ref = &mut value; // 错误：不能同时有两个可变借用
}
