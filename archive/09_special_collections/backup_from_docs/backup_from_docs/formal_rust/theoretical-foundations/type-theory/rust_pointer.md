...
- [rust pointer](#rust-pointer)
  - [目录](#目录)
  - [1. 借用（References）](#1-借用references)
...
其中安全指针主要包括借用和各种智能指针，而不安全指针主要是原始指针。

## 1. 借用（References）

- **不可变借用：`&T`**  
  - **特点**：只读借用数据，保证在借用期间数据不会被修改。  
  - **安全性**：由借用检查器（borrow checker）在编译期保证，不允许数据竞争。  
  - **使用场景**：读取数据、作为参数传递时使用，确保数据不会意外修改。

- **可变借用：`&mut T`**  
  - **特点**：允许对数据进行修改，但同一作用域内同一时间只能有一个可变借用。  
  - **安全性**：同样由编译器借用检查器保证，防止同一数据的多重修改。  
  - **使用场景**：需要更改数据时使用；
  在使用可变借用时要注意不能同时存在其他借用（无论是可变还是不可变）。
...
- **可变原始指针：`*mut T`**  
  - **特点**：
    ...
    - 没有编译器自动提供的内存安全保证。
  - **安全性**：需要在 `unsafe` 块中进行解借用和其它操作，使用时程序员必须手动保证内存安全。  
...
- **内部可变性相关指针**  
  - **Cell`<T>` 和 RefCell`<T>`**：提供在不可变借用中获得“可变”行为的能力，
  用于实现内部可变性。  
...
- 与借用（如 `&mut T`）不同，原始指针不受编译器借用规则的约束，
  也不具备生命周期信息，因此它们的使用都需要在 `unsafe` 块中进行，以确保内存安全。
- 原始指针可以为空（null pointer），并且没有自动解借用行为，所有解借用操作均为不安全行为。
...
- 由于原始指针不保证内存安全，所以在解借用（例如 `*ptr = 100`）时
...
1. **安全借用（&T 和 &mut T）**：由编译器静态检查，保证内存和线程安全。
