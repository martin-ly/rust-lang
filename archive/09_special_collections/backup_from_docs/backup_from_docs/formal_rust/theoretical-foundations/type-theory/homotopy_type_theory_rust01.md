...
虽然我们已经探讨了同伦类型理论的主要概念及其在Rust 中的应用，但值得进一步讨论一些专门设计用于支持依赖类型和定理证明的语言，以及它们与Rust等主流语言的关系。
...
enum Circle {
    Base,
    // 在真正的HoTT 中，还有一个路径构造器：loop : Base ≡ Base
}
```
...
fn verified_learning() {
    ...
    println!("最重要的特质: {:?}", important_features);
}
```
