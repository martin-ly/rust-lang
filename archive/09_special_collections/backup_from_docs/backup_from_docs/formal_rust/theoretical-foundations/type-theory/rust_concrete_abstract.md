...
在 Rust 中，`impl Trait` 是一种用于定义抽象类型的语法，它允许你在函数签名中指定返回某种特质的类型，而不需要明确指定具体的类型。
这种方式可以用于隐藏具体实现，提供更大的灵活性。

### 1. `impl Trait` 是抽象类型

`impl Trait` 确实可以被视为一种抽象类型，因为它表示某种特质的实现，但并不暴露具体的类型。这使得函数可以返回实现了特定特质的任何类型，而不需要在函数签名中指定具体类型。
...
在上面的示例中，`create_shape` 函数返回一个实现了 `Shape` 特质的类型，但具体类型是隐藏的。

### 2. `impl Trait` 是否可以用于类型别名

`impl Trait` 不能直接用于类型别名。类型别名只能用于具体类型，而 `impl Trait` 是一种语法，用于在函数返回类型中表示某种特质的实现。你不能像使用类型别名那样直接将 `impl Trait` 赋值给一个类型别名。
...
- `impl Trait` 是一种抽象类型，允许在函数中返回实现特定特质的类型，但不暴露具体类型。
