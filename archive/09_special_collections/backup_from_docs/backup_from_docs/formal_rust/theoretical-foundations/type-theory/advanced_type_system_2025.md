...
高阶类型系统 (HKT) 允许类型构造函数作为参数，实现更高级的抽象。在Rust 中，这通常通过关联类型和trait来实现。
...
2. **单子 (Monad)**：顺序计算抽象
3. **应用函子 (Applicative Functor)**：并发计算抽象
...
依赖类型系统允许类型依赖于值，实现更精确的类型安全。这在Rust 中通过const generics和类型级编程实现。
