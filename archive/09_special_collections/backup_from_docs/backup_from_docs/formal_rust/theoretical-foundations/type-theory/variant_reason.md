...
| 生命周期多态 | 协变 | 长生命周期借用可用于短生命周期上下文 |
| 函数多态 | 逆变 | 接受更一般参数的函数可用于需要更特定参数的场景 |
| 可变借用 | 不变性 | 防止通过类型转换破坏借用规则 |
...
fn example<'long, 'short>(long_ref: &'long str) where 'long: 'short {
    ...
    // 2. 如果借用在 'short 内有效，那么它在 'long 内也有效
    // 3. 因此，将长生命周期借用用作短生命周期借用是安全的
}
```
...
fn main() {
    ...
    
    {
        ...
        // 那么 storage 将存储一个指向已释放内存的借用
    }
    
    // 协变规则防止短生命周期借用存储到需要长生命周期的地方
    // 如果上面的代码被允许，这里将导致悬垂借用
    // println!("{:?}", storage);
}
```
...
fn main() {
    ...
    // 如果允许上述转换，将导致多个可变借用同时存在
    // r2.push(4);
    // r3.push(5); // 数据竞争！
    
    // 不变性确保可变借用的唯一性
    r1.push(4); // 安全：r1 是唯一的可变借用
}
```
