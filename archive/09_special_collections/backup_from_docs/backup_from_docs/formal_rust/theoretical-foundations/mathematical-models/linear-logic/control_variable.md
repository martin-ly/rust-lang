...
**解释**：
Rust 默认变量不可变，这有助于减少 bug 并使代码更安全、更容易并发化。
...
**定义**：
内部可变性是 Rust 的一种设计模式，允许你在仅有不可变借用的情况下修改数据。
...
**定义**：
生命周期是指借用有效的区间。
Rust 编译器通过生命周期分析确保借用不会比其借用的数据存在得更久。

**解释**：
生命周期是 Rust 内存安全模型的关键部分，防止悬垂借用（借用已释放的内存）。
...
fn main() {
    ...
    // 以下代码会导致编译错误，因为 result 借用了已释放的 string2
    // println!("最长的字符串是：{}", result);
}
```
...
**定义**：
Rust 允许通过借用"借用"值而不获取所有权，遵循以下规则：

1. 任意时刻，只能有一个可变借用或任意数量的不可变借用
2. 借用必须始终有效
...
**定义**：
在函数签名中，生命周期参数明确了输入借用和输出借用之间的关系。
...
fn main() {
    ...
    let s1 = String::from("长生命周期");
    {
        ...
        let selected = select(&s1, &s2, true); // 选择 s1，所以返回借用有效
        println!("选择的字符串: {}", selected);
    } // s2 生命周期结束
    // 此处仍可使用 selected，因为它借用 s1
}
```
...
1. **不变性与可变性**是最基础的特质，决定了值能否被修改。
   - 外部可变性通过 `mut` 关键字直接控制
   - 内部可变性通过 `Cell`/`RefCell` 等类型在不可变借用中实现可变性
