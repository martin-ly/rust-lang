...
- [Rust 类型设计中的所有权模型综合分析](#rust-类型设计中的所有权模型综合分析)
  ...
  - [3. 不同类型下的所有权特质](#3-不同类型下的所有权特质)
    - [3.1 原生类型的所有权特质](#31-原生类型的所有权特质)
    - [3.2 组合类型的所有权特质](#32-组合类型的所有权特质)
    - [3.3 聚合类型的所有权特质](#33-聚合类型的所有权特质)
    - [3.4 容器类型的所有权特质](#34-容器类型的所有权特质)
    - [3.5 集合类型的所有权特质](#35-集合类型的所有权特质)
    - [3.6 智能指针的所有权特质](#36-智能指针的所有权特质)
  - [4. 类型解构与所有权转移](#4-类型解构与所有权转移)
    - [4.1 结构体解构](#41-结构体解构)
    - [4.2 元组解构](#42-元组解构)
    - [4.3 数组与切片解构](#43-数组与切片解构)
    - [4.4 枚举解构](#44-枚举解构)
    - [4.5 解构中的借用与借用](#45-解构中的借用与借用)
...
借用（borrowing）允许在不转移所有权的情况下访问数据，通过借用实现：

1. **不可变借用** `&T`：允许读取但不能修改数据，可以同时存在多个
2. **可变借用** `&mut T`：允许读取和修改数据，但在同一作用域内只能有一个
...
生命周期（lifetime）标注用于确保借用的有效性，它们是编译器用来验证借用的有效期：

1. **生命周期参数**：使用 `'a`、`'b` 等表示
2. **生命周期约束**：确保借用不会超过被借用数据的生命周期
...
1. **`Box<T>`**：堆分配的单一所有者指针
2. **`Rc<T>`**：借用计数的共享所有权指针（单线程）
3. **`Arc<T>`**：原子借用计数的共享所有权指针（线程安全）
...
fn smart_pointer_types() {
    ...
    println!("借用计数: {}", Rc::strong_count(&rc1)); // 输出: 3
...
- 切片借用（`&[T]`、`&str`）虽然是借用类型，但它们实现了 `Copy`
...
fn smart_pointer_ownership() {
    ...
    // Rc<T> 的共享所有权
    let rc1 = Rc::new(String::from("共享数据"));
    let rc2 = Rc::clone(&rc1); // 增加借用计数，不转移所有权
    
    println!("rc1: {}, rc2: {}", rc1, rc2); // 两者都可以使用
    println!("借用计数: {}", Rc::strong_count(&rc1)); // 输出: 2
...
fn struct_destructuring() {
    ...
    // 使用 ref 关键字创建借用而非获取所有权
    let Person { ref name, ref age } = third_person;
    
    println!("借用 - 姓名: {}, 年龄: {}", name, age);
...
fn tuple_destructuring() {
    ...
    println!("借用 - 字符串: {}, 整数: {}, 布尔值: {}", s3, i3, b3);
    // third_tuple 仍然有效
...
fn ref_patterns_in_destructuring() {
    ...
    *i += 1;
    ...
    // 匹配借用
    let r = &String::from("借用");
    
    match r {
        // & 模式匹配借用，value 绑定到借用的值
...
fn variable_creation() {
    ...
    let copied_from_ref = original_ref.clone(); // 克隆借用指向的值
...
fn variable_assignment() {
    ...
    let r1 = &data; // 创建对 data 的借用
    let r2 = r1; // 复制借用
    
    println!("r1 = {}, r2 = {}", r1, r2); // 两个借用都有效
    
    // 可变借用赋值
    let mut value = String::from("可变值");
    let mr1 = &mut value; // 创建可变借用
    
    // 不能同时有另一个借用
    // let mr2 = &mut value; // 错误：不能同时有多个可变借用
    // let r3 = &value; // 错误：不能同时有可变借用和不可变借用
...
2. **资源管理**：原变量被遮蔽后，如果无其他借用，其资源会被释放
3. **类型改变**：遮蔽允许改变变量类型
...
fn variable_shadowing() {
    ...
    // data_ref 仍然借用原始数据，不受遮蔽影响
    println!("借用: {}, 新变量: {}", data_ref, data);
...
2. **生命周期延长**：借用可以延长临时变量的生命周期
3. **悬垂借用防止**：编译器防止创建悬垂借用
...
1. **可变借用参数**：`&mut T` 允许读取和修改数据
2. **唯一性**：同一时间只能有一个可变借用
3. **修改型 API**：通过可变借用修改数据的 API
...
fn mutable_reference_parameters() {
    ...
    let r1 = &data; // 不可变借用
    // let r2 = &mut data; // 错误：不能同时有可变借用和不可变借用
    
    println!("不可变借用: {}", r1); // r1 的最后一次使用
...
1. **输入借用生命周期**：指定输入借用参数的生命周期
2. **输出借用生命周期**：指定返回借用的生命周期
3. **生命周期约束**：确保返回借用不会比输入借用存活更长
...
1. **自借用问题**：结构体包含指向自身其他部分的借用
2. **pinning**：使用 `Pin` 固定自借用数据在内存中的位置
3. **生命周期参数**：自借用类型需要复杂的生命周期管理
...
fn self_referential_types() {
    ...
    println!("值: {}, 借用: {}", pinned.value, pinned.get_ref());
    
    // 更实用的方法：避免直接自借用，而是使用索引
    struct IndexBased {
        values: Vec<String>,
        // 使用索引而非借用
        current_index: usize,
