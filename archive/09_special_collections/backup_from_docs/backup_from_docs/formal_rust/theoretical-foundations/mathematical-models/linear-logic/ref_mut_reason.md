# Rust 中的借用、借用与所有权：概念关系与多层级借用分析


## 📊 目录

- [Rust 中的借用、借用与所有权：概念关系与多层级借用分析](#rust-中的借用借用与所有权概念关系与多层级借用分析)
  - [📊 目录](#-目录)
  - [一、基本概念定义](#一基本概念定义)
    - [1.1 关键字与操作符](#11-关键字与操作符)
    - [1.2 特质与方法](#12-特质与方法)
  - [二、概念之间的关系](#二概念之间的关系)
    - [2.1 借用与借用的关系](#21-借用与借用的关系)
    - [2.2 可变性与借用的关系](#22-可变性与借用的关系)
    - [2.3 `ref` 与 `&` 的关系](#23-ref-与--的关系)
    - [2.4 `Borrow` 与 `AsRef` 的关系](#24-borrow-与-asref-的关系)
    - [2.5 `to_owned` 与 `clone` 的关系](#25-to_owned-与-clone-的关系)
  - [三、多层级借用与解借用](#三多层级借用与解借用)
    - [3.1 借用的层级关系](#31-借用的层级关系)
    - [3.2 自动解借用与方法调用](#32-自动解借用与方法调用)
    - [3.3 多层级可变借用](#33-多层级可变借用)
  - [四、等价关系与推理](#四等价关系与推理)
    - [4.1 模式匹配中的 `ref` 与 `&` 等价关系](#41-模式匹配中的-ref-与--等价关系)
    - [4.2 多层解借用的等价关系](#42-多层解借用的等价关系)
    - [4.3 借用方法的等价关系](#43-借用方法的等价关系)
  - [五、借用检查规则与证明](#五借用检查规则与证明)
    - [5.1 基本借用规则](#51-基本借用规则)
    - [5.2 借用检查与生命周期](#52-借用检查与生命周期)
    - [5.3 非词法生命周期（NLL）](#53-非词法生命周期nll)
  - [六、复杂示例与分析](#六复杂示例与分析)
    - [6.1 多层级借用与可变性](#61-多层级借用与可变性)
    - [6.2 自借用结构体与生命周期](#62-自借用结构体与生命周期)
    - [6.3 借用与所有权转移](#63-借用与所有权转移)
    - [6.4 `to_owned` 与 `Borrow` 的相互作用](#64-to_owned-与-borrow-的相互作用)
  - [七、等价关系的形式化证明](#七等价关系的形式化证明)
    - [7.1 `ref` 与 `&` 的等价性证明](#71-ref-与--的等价性证明)
    - [7.2 多层解借用的等价性证明](#72-多层解借用的等价性证明)
    - [7.3 借用方法的等价性证明](#73-借用方法的等价性证明)
  - [八、总结与最佳实践](#八总结与最佳实践)
    - [8.1 借用与借用的关键概念](#81-借用与借用的关键概念)
    - [8.2 多层级借用的最佳实践](#82-多层级借用的最佳实践)
    - [8.3 借用与所有权转换的最佳实践](#83-借用与所有权转换的最佳实践)
    - [8.4 借用检查器的工作原理](#84-借用检查器的工作原理)
