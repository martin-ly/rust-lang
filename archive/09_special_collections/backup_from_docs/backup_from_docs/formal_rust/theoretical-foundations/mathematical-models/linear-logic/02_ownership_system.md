...
- [Rust所有权系统形式化理论 V32](#rust所有权系统形式化理论-v32)
  - [📊 目录](#-目录)
  - [所有权系统概览](#所有权系统概览)
    - [Rust所有权系统的特点](#rust所有权系统的特点)
  - [基础所有权概念](#基础所有权概念)
    - [1. 所有权状态 (Ownership States)](#1-所有权状态-ownership-states)
      - [1.1 所有权状态定义](#11-所有权状态定义)
      - [1.2 所有权跟踪](#12-所有权跟踪)
    - [2. 借用规则 (Borrowing Rules)](#2-借用规则-borrowing-rules)
      - [2.1 借用规则定义](#21-借用规则定义)
    - [3. 生命周期系统 (Lifetime System)](#3-生命周期系统-lifetime-system)
      - [3.1 生命周期定义](#31-生命周期定义)
      - [3.2 生命周期推导规则](#32-生命周期推导规则)
    - [4. 移动语义 (Move Semantics)](#4-移动语义-move-semantics)
      - [4.1 移动语义定义](#41-移动语义定义)
    - [5. 所有权安全定理 (Ownership Safety Theorems)](#5-所有权安全定理-ownership-safety-theorems)
      - [5.1 内存安全定理 (Memory Safety Theorem)](#51-内存安全定理-memory-safety-theorem)
      - [5.2 无数据竞争定理 (No Data Race Theorem)](#52-无数据竞争定理-no-data-race-theorem)
    - [6. 所有权系统算法](#6-所有权系统算法)
      - [6.1 借用检查算法](#61-借用检查算法)
      - [6.2 生命周期推导算法](#62-生命周期推导算法)
    - [7. 高级所有权特质](#7-高级所有权特质)
      - [7.1 智能指针所有权](#71-智能指针所有权)
      - [7.2 所有权转移优化](#72-所有权转移优化)
  - [总结](#总结)
...
**规则 (B-1)**: 在任意时刻，只能有一个可变借用或多个不可变借用
**规则 (B-2)**: 借用必须始终有效（生命周期规则）
