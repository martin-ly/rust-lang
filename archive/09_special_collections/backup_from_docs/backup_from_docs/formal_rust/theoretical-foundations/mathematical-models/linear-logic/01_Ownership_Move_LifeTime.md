...
所有权是 Rust 中的一个基本特质，它意味着每个值在 Rust 中都有一个被称为其所有者（owner）的变量。
所有权规则如下：
...
生命周期是 Rust 中的一个泛型概念，用于确保借用有效性。
每个借用都有一个生命周期，它定义了借用保持有效的时间范围。生命周期的主要规则是：

1. **非生命周期借用（Non-lifetime references）**：不能借用一个生命周期比它自身短的借用。
2. **生命周期注释（Lifetime Annotations）**：可以显式地指定借用的生命周期。

生命周期用于避免悬挂借用（dangling references），确保借用总是在其指向的数据有效时有效。
...
在这个例子中，`longest` 函数有两个字符串借用作为参数，并且它们具有相同的生命周期 `'a`。
这确保了返回的借用不会超出其有效范围。
...
- **生命周期**：确保借用总是指向有效的内存，防止悬挂借用的产生。

## 应用

- **所有权**：用于自动内存管理，避免内存泄漏。
- **移动**：在函数参数传递和返回值中，确保数据的完整性和封装性。
- **生命周期**：在复杂的数据结构和借用中，确保借用的有效性，避免潜在的借用错误。
