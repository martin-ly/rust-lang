...
异步状态机面临一个特殊挑战：
它们经常包含**自借用结构**，即结构体的一个字段借用另一个字段。
这在跨越 `.await` 点的借用中很常见：

```rust
async fn self_reference_example() {
    let mut data = [0u8; 1024];
    let slice = &data[0..100]; // 指向data的借用
...
`Pin<P>` 是 Rust 为解决自借用问题而设计的类型，提供了"固定"值不被移动的保证：
...
- 这允许 `T` 安全地包含指向自身的借用，因为借用的目标位置保持稳定

这是 `Future::poll` 签名中使用 `Pin<&mut Self>` 而非 `&mut self` 的核心原因，
确保异步状态机在轮询期间不会被移动，从而保证内部自借用的安全性。
...
1. **SIMT 执行模型 vs. 顺序执行**：
   - GPU 使用单指令多线程 (SIMT) 模型，大量线程并发执行相同指令
...
> **展望总结**：
Rust 的并发与异步模型很可能继续保持其核心特质 - 强类型安全保证和高性能，同时逐步改善用户体验和解决当前的痛点。
