...
Rust 凭借其内存安全和并发安全的特性，在这个领域展现出独特的优势。
...
1. **基于线程的并发**(`std::thread`):
   - 直接映射到操作系统线程
   - 适合 CPU 密集型和真正并行的工作负载
   - 通过 `Send` 和 `Sync` 特性确保线程安全
...
- **借用检查器**在编译时防止数据竞争
- **生命周期**保证引用的有效性
- **`Send`/`Sync` 特性**扩展安全保证到并发场景

这些特性为 CPU 领域的并发安全提供了坚实基础，然而，将这些概念扩展到 GPU 计算领域带来了新的挑战。

## GPU 计算特性与架构

### GPU 并行模型

GPU 采用与 CPU 根本不同的并行模型：

- **SIMD (单指令多数据)** 架构
- **大规模并行线程**：GPU 可同时执行数千个轻量级线程
- **线程组织**：通常组织为线程组、波面或工作组
- **执行模型**：并行线程执行相同的内核函数，但处理不同的数据元素
...
Rust 的所有权系统和类型安全特性为构建可靠的 GPU 计算程序提供了坚实基础，
