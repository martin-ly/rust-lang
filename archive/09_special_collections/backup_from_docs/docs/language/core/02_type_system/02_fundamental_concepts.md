...
从引用一致性视角看，**指针类型 (Pointer Types)**：提供对资源的间接访问（访问许可证，而非内存地址）。
...
从引用一致性视角看，Rust 的基础类型系统通过将来自类型理论的坚实概念（如积类型、和类型）与独特的资源管理模型（所有权（资源控制权的逻辑证明）、借用（能力转移与受限授权））相结合，提供了一套既富有表现力又极其安全的工具集。从简单的原始类型到复杂的复合结构，每一种类型都经过精心设计，以在编译期逻辑证明中强制执行不变量，从而将错误扼杀在摇篮中。
