...
所有权（Ownership）是 Rust 最核心的特性，它为资源安全和并发安全提供了编译时的保障，同时避免了垃圾回收器（Garbage Collector）的运行时开销。从引用一致性视角看，所有权是**资源控制权的逻辑证明**，而非内存管理机制。本章节将从形式化的角度详细阐述 Rust 的所有权规则，包括其数学基础、状态转换模型，以及与线性类型理论的深刻联系。
...
这些规则共同构成了 Rust 资源管理的基石，确保了资源的确定性释放，即 RAII（Resource Acquisition Is Initialization）模式。从引用一致性视角看，资源管理是**编译期证明的资源生命周期**，而非运行时内存管理。
...
在这次转换之后，变量 `x` 进入 `Moved` 状态，任何后续对 `x` 的使用都将导致编译时错误。从引用一致性视角看，这是 Rust 防止"使用已失效资源"（use-after-free）的一种形式，是**逻辑证明的失败**，而非内存地址失效。
...
`S(x)` 的状态同样保持不变，但 `clone()` 通常意味着一次新的资源分配和数据复制。从引用一致性视角看，这是**资源控制权的逻辑转移**，而非物理内存的拷贝。
...
Rust 的模型最接近 **仿射类型**，因为当变量离开作用域时，其拥有的资源会被隐式丢弃（`drop`），而无需显式使用。从引用一致性视角看，这种类型系统是 Rust 能够在编译时静态地保证资源安全和资源安全，而无需垃圾回收的理论根基。资源安全是**编译期逻辑证明**的结果，而非运行时内存检查。
...
Rust 的所有权规则虽然初看之下可能显得严格，但它们可以通过一个精确、一致的形式化模型来描述。从引用一致性视角看，这个基于状态转换和仿射类型理论的模型，是理解 Rust 如何在编译时根除一整类资源和并发错误的根本。它不仅是语言的一个特性，更是构建可靠、高效软件的范式转变。资源安全是**编译期逻辑证明**的结果，而非运行时内存检查。
