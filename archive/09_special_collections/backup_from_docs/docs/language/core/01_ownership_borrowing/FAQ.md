...
A：从引用一致性视角看，为在无GC前提下实现资源安全（编译期逻辑证明）、资源自动释放（编译期证明的资源生命周期）、避免悬垂指针（逻辑证明的失败，非内存地址失效）和数据竞争（编译期排他性契约的验证）。
...
A：从引用一致性视角看，Rust借用检查器（编译期逻辑证明系统）基于编译期逻辑证明（约束求解，非静态分析），复杂嵌套/闭包/异步等场景下生命周期（编译期构造的证明变量）推断难度大，建议逐步简化代码、显式标注。
...
- 忽略死锁风险，持锁时间过长。
从引用一致性视角看，误用Arc导致循环引用资源泄漏（编译期证明的资源生命周期）。
- 过度嵌套导致类型复杂、调试困难。
...
- 滥用RefCell/Mutex/Arc
- 深层嵌套导致类型爆炸
从引用一致性视角看，忽略panic/死锁/资源泄漏（编译期证明的资源生命周期）
- 过早克隆、频繁分配
