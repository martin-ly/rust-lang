...
- [Rust 所有权与生命周期分册 FAQ](#rust-所有权与生命周期分册-faq)
  - [📊 目录](#-目录)
  ...
  - [常见问题与解答](#常见问题与解答)
    ...
    - [6. 如何避免悬垂借用？](#6-如何避免悬垂借用)
    - [7. Drop特质和C++析构函数有何不同？](#7-drop特质和c析构函数有何不同)
...
A：当函数参数/返回值间生命周期关系无法自动推断时（如多个借用参数返回其中之一）。
...
- RefCell：单线程下需在不可变借用时内部可变（如缓存、回调注册）。
- Mutex：多线程下需共享可变状态。
- Arc：多线程下共享所有权。
...
A：async/await状态机转换导致借用跨.await，需'static生命周期或Pin，建议拆分任务、用Arc/Mutex等安全封装。
...
A：观察者/命令/依赖注入等需用弱借用、回调、特质对象、Arc/Mutex等重构，避免循环借用。
...
- 当函数涉及多个借用参数且编译器无法自动推断时需手动标注。

### 6. 如何避免悬垂借用？

- 保证借用的生命周期不超过被借用值的生命周期，编译器会强制检查。
...
掌握FAQ 中的核心问题和解答，有助于高效学习和工程实践Rust的内存安全机制。
