...
- [15.5 智能合约引擎](#155-智能合约引擎)
  ...
  - [示例与反例](#示例与反例)
    - [示例：简单虚拟机实现](#示例简单虚拟机实现)
    - [反例：无Gas限制的虚拟机](#反例无gas限制的虚拟机)
  - [练习](#练习)
  - [交叉引用与落地资源](#交叉引用与落地资源)
    - [快速导航](#快速导航)
  - [安全门禁清单（智能合约引擎）](#安全门禁清单智能合约引擎)
    - [测试矩阵（摘要）](#测试矩阵摘要)
...
- 字节码与执行
  - 指令集不变式：栈深/类型/内存边界/Gas 单调性；越界访问拒绝
  - 重入防护：可选全局/细粒度 `ReentrancyGuard`；外部调用点清单与属性测试
- 资源与隔离
  - Gas 计量可回溯；SSTORE/SLOAD 成本校验；拒绝无限循环与病态指令序列
  - 沙箱：FFI/主机调用白名单；文件/网络默认禁用
- 升级与兼容
  - 合约升级策略与状态迁移不变量；回滚脚本与影子读写
- 验证与KAT
  - KAT：ABI 编解码、算数/哈希/签名接口；模糊针对解析/ABI
  - 形式化：关键不变量用 Creusot/Prusti；Gas 单调性用模型检查

### 测试矩阵（摘要）

| 维度 | 场景 | 期望 | 工具 |
|---|---|---|---|
| 重入 | 外部调用重入 | 状态不破坏/拒绝 | 属性+模糊 |
| OOG | Gas 不足 | 原子回滚 | 单元+属性 |
| ABI | 畸形输入 | 解析失败且不越界 | fuzz |
| 升级 | 迁移 | 不变量保持 | Kani/回放 |
