...
- [不安全编程概览](#不安全编程概览)
  - [Rust不安全编程的特点](#rust不安全编程的特点)
- [不安全编程理论](#不安全编程理论)
  - [1. Unsafe块和函数 (Unsafe Blocks and Functions)](#1-unsafe块和函数-unsafe-blocks-and-functions)
    - [1.1 Unsafe语义](#11-unsafe语义)
    - [1.2 Unsafe函数实现](#12-unsafe函数实现)
  - [2. 原始指针 (Raw Pointers)](#2-原始指针-raw-pointers)
    - [2.1 指针类型系统](#21-指针类型系统)
    - [2.2 指针算术](#22-指针算术)
  - [3. 外部函数接口 (Foreign Function Interface)](#3-外部函数接口-foreign-function-interface)
    - [3.1 FFI类型系统](#31-ffi类型系统)
    - [3.2 FFI函数调用](#32-ffi函数调用)
  - [4. 内存布局 (Memory Layout)](#4-内存布局-memory-layout)
    - [4.1 类型布局](#41-类型布局)
- [总结](#总结)
