...
│   │
│   ├── 关联类型
│   │   └── 特征中的关联类型 (trait Iterator { type Item; })
│   │
│   └── 常量泛型
...
└── 实用类型模式
    ...
    └── 类型转换
        ├── 强制类型转换 (coercion)
        ├── 显式转换 (as, From/Into)
        ├── 转换特征 (From<T>, Into<T>, TryFrom<T>, TryInto<T>)
        └── 透明包装类型 (newtype pattern)
```

此思维导图综合了 Rust 类型系统的主要组成部分及其关系，涵盖了从基本类型结构到类型系统理论基础的各个方面。
它展示了 Rust 如何将多种类型理论概念（如仿射类型、代数数据类型、多态性等）整合到一个统一的类型系统中，以及这些概念如何相互关联以提供内存安全和类型安全保证。

思维导图分为六大部分：基本类型结构、类型参数化、类型约束系统、资源管理系统、型变系统和类型系统理论基础，以及实用类型模式。
这些部分共同构成了 Rust 强大而富有表现力的类型系统。
