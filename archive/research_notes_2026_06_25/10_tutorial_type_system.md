...
- [教程：类型系统](.#教程类型系统)
  ...
  - [第四部分：泛型](.#第四部分泛型)
    ...
    - [泛型结构体](.#泛型结构体)
    - [泛型枚举](.#泛型枚举)
  - [第五部分：Trait](.#第五部分trait)
    - [定义与实现](.#定义与实现)
    - [Trait作为参数](.#trait作为参数)
    - [where子句](.#where子句)
    - [关联类型](.#关联类型)
  - [第六部分：类型推导与标注](.#第六部分类型推导与标注)
    - [自动推导](.#自动推导)
    - [类型别名](.#类型别名)
    - [Never类型](.#never类型)
  - [第七部分：高级类型特性](.#第七部分高级类型特性)
    - [动态大小类型 (DST)](.#动态大小类型-dst)
    - [函数指针](.#函数指针)
    - [类型转换](.#类型转换)
  - [第八部分：形式化视角](.#第八部分形式化视角)
    - [类型安全定理](.#类型安全定理)
    - [与形式化文档关联](.#与形式化文档关联)
  - [总结](.#总结)
  - [引言](.#引言-1)
  - [第一部分：类型安全基础](.#第一部分类型安全基础)
    - [什么是类型安全？](.#什么是类型安全)
    - [Rust的类型安全保证](.#rust的类型安全保证)
  - [第二部分：类型安全定理](.#第二部分类型安全定理)
...
Rust的类型系统是其保证内存安全、线程安全和零成本抽象的基础。
本教程将深入讲解Rust类型系统的核心概念和高级特性。
...
> **来源: [Wikipedia - Hindley-Milner](https://en.wikipedia.org/wiki/Hindley_Milner)**
