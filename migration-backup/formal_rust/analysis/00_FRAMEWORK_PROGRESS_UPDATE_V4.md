...
- **所有权类型理论定理**：所有权转移、借用、生命周期安全，Polonius自动化推理，标准库Box/Vec，反例：悬垂引用。

### 控制语义层

- **错误传播单子定理**：Result类型的单子结构，范畴论建模，Coq/Lean证明，?操作符传播，反例：panic未捕获。
- **异常安全性分级定理**：强/基本异常安全的数学定义，操作语义推理，自动化测试，工程案例：panic安全API，反例：资源泄漏。

### 并发语义层

- **执行器代数结构定理**：异步调度的代数建模，TLA+模型，Tokio执行器，反例：死锁。
- **I/O事件循环语义定理**：Reactor模式建模，状态机分析，自动化测试，零拷贝I/O，反例：事件丢失。
