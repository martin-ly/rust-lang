...
for root, dirs, files in os.walk("concept"):
    for fname in files:
        ...
        if rate < 0.27:
            below_27.append((filepath, rate, ann, denom))

print(f"Current total rate: {total_ann/total_denom*100:.2f}%")
print(f"Files <27%: {len(below_27)}")

below_27.sort(key=lambda x: int(0.28 * x[3]) + 1 - x[2])
target = [(p, int(0.28 * d) + 1 - a) for p, r, a, d in below_27 if int(0.28 * d) + 1 - a <= 20]
print(f"Files to push to 28% (need <=20 sources): {len(target)}")
