...
MSRV_MINOR = 96  # 1.96.0


def get_git_last_commit(path: Path) -> datetime:
    """获取文件最近一次 git commit 时间"""
    try:
...
def classify_file(md_path: Path) -> tuple:
    """返回 (分类, 原因)"""
    ...
    if version_matches:
        ...
        if max(versions) < MSRV_MINOR - 2:  # 低于 MSRV-2 个版本
...
def main():
    ...
    for md_path in files:
        category, reason = classify_file(md_path)
        rel = str(md_path.relative_to(DOCS_DIR)).replace("\\", "/")
