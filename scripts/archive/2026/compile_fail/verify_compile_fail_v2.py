#!/usr/bin/env python3
"""验证 compile_fail 代码块 v2: 使用 bin + lib 双模式编译"""
...
RUSTC = "rustc"
TMP_DIR = Path("target/tmp/verify_compile_fail_v2")
REPORT_PATH = "reports/verify_compile_fail_v2_report.md"
MAX_WORKERS = 8
TIMEOUT = 15
...
def compile_code(source, test_path, crate_type):
    ...
    try:
        ...
        if exe.exists():
            exe.unlink()
        rlib = test_path.with_suffix('.rlib')
        if rlib.exists():
            rlib.unlink()
...
def try_compile(code):
    ...
    test_path = TMP_DIR / "test_wrapped.rs"
...
def compile_one(args):
    ...
    if not success:
        # 检查是否是真正的语义错误（非语法错误）
        has_real_error = 'error[' in stderr or ('error:' in stderr and 'expected' not in stderr)
        if has_real_error:
