...
for root, dirs, files in os.walk("concept"):
    for fname in files:
        ...
        if rate < 0.30:
            below_30.append((filepath, rate, ann, denom))

print(f"Current total rate: {total_ann/total_denom*100:.2f}%")
print(f"Files <30%: {len(below_30)}")

# 策略：按denom排序，优先处理短文件（效率最高）
below_30.sort(key=lambda x: x[3])

# 计算需要多少来源才能到30%
needed_30 = int(0.30 * total_denom) - total_ann
print(f"Sources needed for 30%: {needed_30}")

# 优先处理denom最小的文件，直到累计需要来源数达到needed_30的80%
target = []
cumulative = 0
for p, r, a, d in below_30:
    need = int(0.30 * d) + 1 - a
    if need > 0:
        target.append((p, need))
        cumulative += need
        if cumulative >= needed_30 * 0.8:
            break

print(f"Targeting {len(target)} files, cumulative need: {cumulative}")
...
sources = [
    ...
    ("MS Rust", "https://microsoft.github.io/rust-for-windows/"),
    ...
    ("USENIX", "https://www.usenix.org/conference/usenixsecurity"),
    ...
    ("TrustZone", "https://developer.arm.com/architectures/security-architectures/trustzone"),
