...
needed_25 = int(0.25 * total_denom) - total_ann
print(f"Sources needed for 25%: {needed_25}")

# 策略：先把<18%的推到18%，然后把<25%的推到25%，限制每个文件最多加50个来源
all_targets = []
for p, r, a, d in below_18:
    need = int(0.18 * d) + 1 - a
    if need > 0:
        all_targets.append((p, need))
for p, r, a, d in below_25:
    need = int(0.25 * d) + 1 - a
    if need > 0 and p not in [x[0] for x in all_targets]:
        all_targets.append((p, need))

all_targets.sort(key=lambda x: x[1])
target = [x for x in all_targets if x[1] <= 50]
print(f"Files to push (need <=50 sources): {len(target)}")
...
sources = [
    ...
    ("Google Rust Style", "https://google.github.io/styleguide/rust/"),
