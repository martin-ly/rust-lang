...
# 策略：优先推 <18% 的文件到 18%，然后推 <20% 到 20%
# 限制每个文件最多加30个来源
all_targets = []
for p, r, a, d in below_18:
    need = int(0.18 * d) + 1 - a
    if need > 0:
        all_targets.append((p, need, 0.18))
for p, r, a, d in below_20:
    need = int(0.20 * d) + 1 - a
    if need > 0 and p not in [x[0] for x in all_targets]:
        all_targets.append((p, need, 0.20))

all_targets.sort(key=lambda x: x[1])
target = [x for x in all_targets if x[1] <= 30]
print(f"Files to push (need <=30 sources): {len(target)}")
...
added = 0
files_updated = 0
for filepath, needed, _ in target:
    ...
    if new_sources:
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content)
        file_data[filepath] = content  # update for dedup
