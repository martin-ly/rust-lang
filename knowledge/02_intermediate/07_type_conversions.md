...
> **修正**: `TryFrom`/`TryInto` 是 Rust 的**安全转换** trait：转换可能失败时返回 `Result`。
> `u32::try_into::<u8>()` 在值 > 255 时返回 `Err(TryFromIntError)`。
> 处理模式：
>
> 1) `unwrap()` / `expect()` → 确信不会失败时使用（失败时 panic）；
> 2) `unwrap_or(default)` → 提供默认值；
> 3) `match` / `if let` → 显式处理错误；
> 4) `?` 运算符 → 在返回 `Result` 的函数中传播。
> `TryFrom` 与 `From` 的关系：`From<T>` 自动实现 `Into<T>` 和 `TryFrom<T>`（总是成功）。
> 这提供了类型安全的转换层级：`From`（无损）→ `TryFrom`（可能失败）→ `as`（强制转换，可能截断）。
