...
> **层次定位**: L1 基础概念 / 所有权子域
> **前置依赖**: 无（入门概念）
> **后置延伸**: [knowledge 借用](./borrowing.md) · [concept L1 所有权](../../concept/01_foundation/01_ownership.md)
...
- [Rust 所有权系统 (Ownership System)](#rust-所有权系统-ownership-system)
  ...
  - [🧠 核心概念](#-核心概念)
    - [什么是所有权](#什么是所有权)
    - [所有权规则](#所有权规则)
      - [作用域（Scope）的概念](#作用域scope的概念)
    - [移动语义（Move Semantics）](#移动语义move-semantics)
      - [场景一：整数类型（简单类型）](#场景一整数类型简单类型)
      - [场景二：String 类型（复杂类型）](#场景二string-类型复杂类型)
      - [为什么 String 和整数行为不同？](#为什么-string-和整数行为不同)
    - [Copy Trait](#copy-trait)
      - [哪些类型实现了 Copy？](#哪些类型实现了-copy)
      - [哪些类型**没有**实现 Copy？](#哪些类型没有实现-copy)
      - [显式克隆（Clone）](#显式克隆clone)
    - [所有权与函数](#所有权与函数)
      - [借用（Borrowing）简介](#借用borrowing简介)
    - [返回值与所有权](#返回值与所有权)
  - [💡 最佳实践](#-最佳实践)
  - [⚠️ 常见陷阱](#️-常见陷阱)
    - [陷阱 1：在循环中意外移动值](#陷阱-1在循环中意外移动值)
    - [陷阱 2：忘记 String 和 \&str 的区别](#陷阱-2忘记-string-和-str-的区别)
    - [陷阱 3：在函数中转移了还需要用的值](#陷阱-3在函数中转移了还需要用的值)
  - [🎮 动手练习](#-动手练习)
    - [练习 1：修复编译错误](#练习-1修复编译错误)
    - [练习 2：所有权转移链](#练习-2所有权转移链)
    - [练习 3：实现一个函数](#练习-3实现一个函数)
  - [📖 权威来源与延伸阅读](#-权威来源与延伸阅读)
    - [官方文档（一级来源）](#官方文档一级来源)
    - [学术来源（一级来源）](#学术来源一级来源)
    - [社区权威（二级来源）](#社区权威二级来源)
...
```mermaid
graph TD
    Q1[变量是否实现了 Copy?] -->|是| A1[按位复制，原变量仍可用]
    Q1 -->|否| Q2[是否是引用类型?]
    Q2 -->|是| A2[复制指针，原引用仍可用]
    Q2 -->|否| A3[所有权转移，原变量失效]
    A3 --> Q3[是否需要继续使用原变量?]
    Q3 -->|是| A4[使用 .clone() 显式复制]
    Q3 -->|否| A5[直接移动，无需额外操作]
```
...
// ❌ 编译错误: String 不实现 Copy，所以 Resource 不能实现 Copy
// 且若手动实现 Drop，也不能实现 Copy

fn main() {
    let r = Resource { data: String::from("hello") };
    let r2 = r;
    println!("{:?}", r.data); // 若 Copy 成功，此处可用
}
```

> **修正**: `Copy` 和 `Drop` 是**互斥**的：若类型实现了 `Drop`，它不能实现 `Copy`。原因：`Copy` 意味着按位复制是安全的（无资源管理），`Drop` 意味着有自定义清理逻辑（通常管理非内存资源，如文件描述符、网络连接）。`String` 不实现 `Copy`（它管理堆内存），所以包含 `String` 的 struct 也不能 `Copy`。实现 `Copy` 的条件：1) 所有字段都 `Copy`；2) 无 `Drop` 实现。这与 C 的 `struct`（总是可 bitwise copy，但可能有 double-free 风险）或 C++ 的拷贝构造函数（可自定义，但 `= default` 和 `= delete` 控制）不同——Rust 的 `Copy` 是编译期标记，严格限制为无资源管理的类型。[来源: [The Rust Programming Language](https://doc.rust-lang.org/book/ch04-01-what-is-ownership.html)] · [来源: [Rust Reference — Copy](https://doc.rust-lang.org/reference/special-types-and-traits.html#copy)]
