...
> **修正**: 方法中的**生命周期省略**有三条规则：
>
> 1) `&self` 或 `&mut self` 获得生命周期参数；
> 2) 其他每个引用参数获得独立生命周期；
> 3) 若只有一个输入生命周期（包括 `self`），它赋给所有输出。
> `first_word(&self, s: &str)` 有两个输入生命周期（`&self` 和 `s`），规则 3 不适用，必须显式标注：`fn first_word<'b>(&self, s: &'b str) -> &'b str` 或 `fn first_word(&self, s: &str) -> &str`（若返回 `self.text` 的生命周期）。
> 这与 C++ 的引用（无生命周期概念）或 Java 的引用（同样无概念）不同——Rust 的方法签名在引用复杂时需显式标注生命周期，编译器提供省略规则减少常见情况的噪音。
