...
>
> **权威来源对齐变更日志**: 2026-05-19 补全权威来源标注（TRPL、Rust Reference、Fähndrich & Leino OOPSLA 2003、Pierce TAPL） [Authority Source Sprint Batch 8](../../concept/00_meta/02_sources/international_authority_index.md)
...
>
> 1) 状态数增长时，转换矩阵的组合爆炸；
> 2) 需要 `PhantomData` 占位（无运行时开销）；
> 3) 异步状态机（`async fn`）与类型状态的交互复杂。
> 类型状态适用于关键路径（数据库连接、文件句柄、协议状态机），普通状态机用枚举 + 运行时检查。
