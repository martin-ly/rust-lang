...
> Rustonomicon — Uninitialized Memory / 2025; [RFC 1892 — MaybeUninit / 2017](https://rust-lang.github.io/rfcs/1892.html); 核心设计决策: `mem::uninitialized()` 因无法正确建模未初始化内存而被弃用，`MaybeUninit<T>` 提供类型安全的未初始化内存抽象; Unsafe Code Guidelines — Validity Invariant / 2025。
> 它绕过 Rust 的初始化要求，允许在不确定是否已初始化的情况下操作内存，是构建 `Vec`、栈数组、`ArrayVec` 等高性能抽象的基础。
>
> **⏱️ 预计学习时间**: 2-3 小时
> **📚 难度级别**: ⭐⭐⭐⭐ 高级
> **权威来源**:
...
- [Unsafe Code Guidelines — Validity](https://rust-lang.github.io/unsafe-code-guidelines/glossary.html#validity) [Rust Unsafe Code Guidelines WG / 2025](https://rust-lang.github.io/unsafe-code-guidelines/)
...
- Jung, R., et al. — *RustBelt: Securing the Foundations of the Rust Programming Language*. POPL 2018. [`MaybeUninit` 与 validity invariant 的形式化基础; 未初始化内存的语义模型](../../../concept/00_meta/02_sources/international_authority_index.md)
- Hu, W. & Dreyer, D. — *A Logical Relation for Monadic Encapsulation of State*. POPL 2011. [状态封装的逻辑关系; 与 Rust 所有权系统的形式化对比](../../../concept/00_meta/02_sources/international_authority_index.md)

### 跨语言来源

- C — `malloc` + 未初始化内存 [C 的未初始化内存是 UB 的主要来源; `MaybeUninit` 提供类型安全的替代](../../../concept/00_meta/02_sources/international_authority_index.md)
- C++ — `std::optional`, placement new [C++ 的可选类型与原地构造; 与 `MaybeUninit::assume_init()` 的设计对比](../../../concept/00_meta/02_sources/international_authority_index.md)
...
| [Stacked Borrows](https://plv.mpi-sws.org/rustbelt/stacked-borrows/) | 权威来源 | 别名模型 |
...
| [Rustlings](https://github.com/rust-lang/rustlings) | 权威来源 | 交互式练习 |
