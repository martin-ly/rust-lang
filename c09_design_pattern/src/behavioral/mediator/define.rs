...
impl Colleague for User {
    fn receive(&mut self, msg: &dyn Message, sender_id: usize) {
        if let Some(text_msg) = msg.as_any().downcast_ref::<TextMessage>() {
            println!("用户 {} 收到来自用户 {} 的文本消息: {}", self.name, sender_id, text_msg.content);
        } else if let Some(img_msg) = msg.as_any().downcast_ref::<ImageMessage>() {
            println!("用户 {} 收到来自用户 {} 的图片消息: {} 尺寸: {:?}", 
        ...
        if let Some(text_msg) = msg.as_any().downcast_ref::<TextMessage>() {
            for keyword in &self.keywords {
                if text_msg.content.contains(keyword) {
                    println!("机器人 {} 检测到关键词 '{}' 在用户 {} 的消息中", 
                             self.name, keyword, sender_id);
                    return;
                }
            }
            println!("机器人 {} 收到来自用户 {} 的消息，但没有匹配任何关键词", 
                     self.name, sender_id);
        }
    }
...
fn mediator_test() {
    ...
    let bot = Bot::new(String::from("InfoBot"), vec![String::from("info"), String::from("help")]);
    ...
    // 准备消息
    let text_msg1 = TextMessage { content: String::from("你好，Bob！") };
    let text_msg2 = TextMessage { content: String::from("我需要一些信息，能帮助我吗？") };
